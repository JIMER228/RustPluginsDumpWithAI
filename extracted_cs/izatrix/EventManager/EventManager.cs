...
{
    [Info("Event Manager", "Reneb / k1lly0u", "3.0.73", ResourceId = 740)]
    class EventManager : RustPlugin
    {
        ...
        static List<BasePlayer> unnetworkedPlayers;
        ...
        void SetVars()
        {
            ...
            spectateFilter = typeof(BasePlayer).GetField("spectateFilter", (BindingFlags.Instance | BindingFlags.NonPublic));
        ...
        public class RestoreInfo
        {
            ...
            public Dictionary<InventoryType, byte[]> inventory = new Dictionary<InventoryType, byte[]>();
        ...
        class RestorationManager
        {
            ...
            public void StorePlayer(BasePlayer player)
            {
                RestoreInfo info = new RestoreInfo
                {
                    ...
                    inventory = new Dictionary<InventoryType, byte[]>
                    {
                        {InventoryType.Belt, player.inventory.containerBelt.Save().ToProtoBytes() },
                        {InventoryType.Main, player.inventory.containerMain.Save().ToProtoBytes() },
                        {InventoryType.Wear, player.inventory.containerWear.Save().ToProtoBytes() }
                    },
            ...
            private bool RestoreItems(BasePlayer player, RestoreInfo info)
            {
                foreach (var array in info.inventory)
                {
                    ItemContainer container = array.Key == InventoryType.Belt ? player.inventory.containerBelt : array.Key == InventoryType.Main ? player.inventory.containerMain : player.inventory.containerWear;
                    container.Load(ProtoBuf.ItemContainer.Deserialize(array.Value));
                }               
                return true;                
            }
