...
namespace Oxide.Plugins
{
    ...
    class LootLogs : RustPlugin
    {
        ...
        static string dataDirectory = $"{Interface.Oxide.DataDirectory}{Path.DirectorySeparatorChar}LootLogs{Path.DirectorySeparatorChar}";
        #endregion
        ...
        {
            //if (!Interface.Oxide.DataFileSystem.ExistsDatafile($"LootLogs{Path.DirectorySeparatorChar}{type}{Path.DirectorySeparatorChar}foldercreator"))
                //Interface.Oxide.DataFileSystem.SaveDatafile($"LootLogs{Path.DirectorySeparatorChar}{type}{Path.DirectorySeparatorChar}foldercreator");
            var dateTime = DateTime.Now.ToString("yyyy-MM-dd");            
            var taketype = take ? "looted" : "deposited";
            LogToFile($"{dataDirectory}{type}{Path.DirectorySeparatorChar}{entityname}_{id}_{dateTime}.txt", $"{playername} {taketype} {item}");
        }
        void DeathLog(string type, string entityname, string id, string position, string killer)
        {
            ...
            var dateTime = DateTime.Now.ToString("yyyy-MM-dd");
            LogToFile($"{dataDirectory}DestroyedContainers{Path.DirectorySeparatorChar}{type}{Path.DirectorySeparatorChar}DeathLog_{dateTime}.txt", $"Name:{entityname} | BoxID:{id} | Position:{position} | Killer: {killer} | LogFile: oxide/data/LootLogs/{type}/{entityname}_{id}_xxxxxxxx.txt");
            LogToFile($"{dataDirectory}DestroyedContainers{Path.DirectorySeparatorChar}{type}{Path.DirectorySeparatorChar}DeathLog_{dateTime}.txt", "-------------------------------------------------------------------------");
        }
        ...
        {
            //Puts(dataDirectory);  
            Log("string 1", "string 2", "string 3", "string 4", "string 5", false);
            DeathLog("string", "name", "id", "position", "killer");
        }
        #endregion
