...
namespace Oxide.Plugins
{
    ...
    public class DiscordRewards : CovalencePlugin, IDiscordPlugin
    {
        #region Variables
        ...
        private void OnDiscordGuildMembersLoaded()
        {
            ...
            foreach (Snowflake id in data.verified2)
            {
                ...
                _guild.AddMemberRole(Client, member.User, role);
            }
        }
        ...
        private void OnDiscordDirectMessageCreated(DiscordMessage message)
        {
            ...
            DiscordChannel.Get(Client, message.ChannelId).Then(c =>
            {
                ...
                SaveData();
                if(role != null) _guild.AddMemberRole(Client, message.Author, role);
            });
