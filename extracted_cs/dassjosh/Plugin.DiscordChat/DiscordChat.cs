...
//DiscordChat created with PluginMerge v(1.0.10.0) by MJSU @ https://github.com/dassjosh/Plugin.Merge
namespace Oxide.Plugins
{
    [Info("Discord Chat", "MJSU", "3.0.6")]
    ...
    public partial class DiscordChat : CovalencePlugin, IDiscordPlugin, IDiscordPool
    {
        ...
        private void OnDiscordGuildCreated(DiscordGuild guild)
        {
            ...
            SetupChannel(guild, MessageSource.PluginPrivateMessages, _pluginConfig.PluginSupport.PrivateMessages.Channel, _pluginConfig.ChatSettings.UseBotToDisplayChat);
            ...
            if (_pluginConfig.PluginSupport.PrivateMessages.Channel.IsValid())
            {
        ...
        public void SetupChannel(DiscordGuild guild, MessageSource source, Snowflake id, bool wipeNonBotMessages = false, Action<DiscordMessage> callback = null)
        {
            ...
            if (channel == null && id.IsValid())
            {
                PrintWarning($"Failed to setup source {source} channel with ID: '{id}'. Not found in guild");
                return;
            }
        ...
        protected override void LoadDefaultMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                [LangKeys.Discord.Team.Message] = $":busts_in_silhouette: {DefaultKeys.TimestampNow.ShortTime} [{PlaceholderKeys.TeamId}] **{PlaceholderKeys.PlayerName}**: {PlaceholderKeys.PlayerMessage}",
                ...
                [LangKeys.Discord.PrivateMessages.Message] = $"[{DefaultKeys.TimestampNow.ShortTime}] {DefaultKeys.Player.Name} -> {DefaultKeys.PlayerTarget.Name}: {PlaceholderKeys.PlayerMessage}",
        ...
        public void RegisterPlaceholders()
        {
            ...
            _placeholders.RegisterPlaceholder<IPlayer, string>(this, PlaceholderKeys.TargetName, GetPlayerName);
            _placeholders.RegisterPlaceholder(this, PlaceholderKeys.DiscordTag, _pluginConfig.ChatSettings.DiscordTag);
            _placeholders.RegisterPlaceholder<IPlayer, ulong>(this, PlaceholderKeys.TeamId, GetTeamId);
        }
        ...
        public ulong GetTeamId(IPlayer player)
        {
            #if RUST
            return RelationshipManager.ServerInstance.playerToTeam.TryGetValue(ulong.Parse(player.Id), out RelationshipManager.PlayerTeam team) ? team.teamID : 0;
            #else
            return 0;
            #endif
        }
        ...
        #region Plugins\DiscordChat.PrivateMessages.cs
        private void OnPMProcessed(IPlayer sender, IPlayer target, string message)
        {
            Sends[MessageSource.PluginPrivateMessages]?.QueueMessage(Lang(LangKeys.Discord.PrivateMessages.Message, GetPrivateMessagesPlaceholders(sender, target, message)));
        }
        
        public PlaceholderData GetPrivateMessagesPlaceholders(IPlayer sender, IPlayer target, string message)
        {
            return GetDefault().AddPlayer(sender).AddTarget(target).Add(PlaceholderDataKeys.PlayerMessage, message);
        }
        #endregion
        ...
        public void RegisterTemplates()
        {
            ...
            DiscordMessageTemplate pluginPrivateMessages = CreateTemplateEmbed($"{PlaceholderKeys.TemplateMessage}",  new DiscordColor("00FFFF"));
            _templates.RegisterGlobalTemplateAsync(this, TemplateKeys.Chat.PrivateMessages.Message, pluginPrivateMessages, new TemplateVersion(1, 0, 0), new TemplateVersion(1, 0, 0));
        ...
        public TemplateKey GetTemplateName(MessageSource source)
        {
            switch (source)
            {
                ...
                case MessageSource.PluginPrivateMessages:
                return TemplateKeys.Chat.PrivateMessages.Message;
            }
        ...
        public static class LangKeys
        {
            ...
            public static class Discord
            {
                ...
                {
                    private const string Base = Discord.Base + nameof(PrivateMessages) + ".";
        ...
        public class PlaceholderKeys
        {
            ...
            public static readonly PlaceholderKey TargetName = new(nameof(DiscordChat), "target.name");
            public static readonly PlaceholderKey DiscordTag = new(nameof(DiscordChat), "discord.tag");
            public static readonly PlaceholderKey TeamId = new(nameof(DiscordChat), "team.id");
        }
        #endregion
        ...
        public static class TemplateKeys
        {
            ...
            public static class Chat
            {
                ...
                public static class PrivateMessages
                {
                    private const string Base = Chat.Base + nameof(PrivateMessages) + ".";
        ...
        public class PluginSupport
        {
            ...
            [JsonProperty("Private Messages Settings")]
            public PrivateMessagesSettings PrivateMessages { get; set; }
            ...
            public PluginSupport(PluginSupport settings)
            {
                ...
                PrivateMessages = new PrivateMessagesSettings(settings?.PrivateMessages);
                UFilter = new UFilterSettings(settings?.UFilter);
            }
        }
        #endregion

        #region Configuration\Plugins\PrivateMessagesSettings.cs
        public class PrivateMessagesSettings
        {
            [JsonProperty("Private Messages Channel ID")]
            public Snowflake Channel { get; set; }
            
            public PrivateMessagesSettings(PrivateMessagesSettings settings)
            {
