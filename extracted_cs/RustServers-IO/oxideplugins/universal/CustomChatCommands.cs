...
{
    [Info("CustomChatCommands", "PsychoTea", "2.1.0")]
    ...
    class CustomChatCommands : CovalencePlugin
    {
        ...
        void Init()
        {
            ...
            {
                { "Command On Cooldown", "This command is on cooldown for another {0} seconds." },
        ...
        void CustomCommand(IPlayer player, string command, string[] args)
        {
            ...
            if (IsOnCooldown(userID, cmd.Command))
            {
                ...
                player.Message(GetMessage("Command On Cooldown", player.Id, timeRemaining.ToString("N0")));
                return;
            }
        ...
        bool IsOnCooldown(ulong userID, string commandName)
        {
            ...
            Puts("2");
            Puts((search.First().ExpiryPoint > TimeSinceEpoch()).ToString());
            return search.First().ExpiryPoint > TimeSinceEpoch();
        }
        ...
        bool CanUseCommand(ulong userID, string commandName)
        {
            ...
            var playerMax = storedData.MaxUses[userID].Where(x => x.CommandName.ToLower() == commandName.ToLower());
            if (!playerMax.Any()) return true;

            return playerMax.First().Uses < configMax;
        }
