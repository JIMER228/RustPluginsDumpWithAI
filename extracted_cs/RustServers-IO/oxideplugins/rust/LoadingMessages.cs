...
{
    [Info("LoadingMessages", "VVoid", "1.0.3", ResourceId = 2762)]
    ...
    public class LoadingMessages : RustPlugin
    {
        ...
        private readonly FieldInfo _nextMessageTimeFld = typeof(ConnectionQueue).GetField("nextMessageTime", BindingFlags.NonPublic | BindingFlags.Instance);
        ...
        private void Unload()
        {
            ...
            if(_config.EnableQueueMessages)
                _nextMessageTimeFld.SetValue(ServerMgr.Instance.connectionQueue, 0f);
        }
        ...
        {
            _queueConnections = (List<Connection>)typeof(ConnectionQueue)
                .GetField("queue", BindingFlags.Instance | BindingFlags.NonPublic)
                .GetValue(ServerMgr.Instance.connectionQueue);
        } 
        ...
        private void SuppressDefaultQueueMessage() => _nextMessageTimeFld.SetValue(ServerMgr.Instance.connectionQueue, float.MaxValue);
