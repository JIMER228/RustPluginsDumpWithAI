...
{
	[Info("Hunt RPG", "PedraozauM / SW / Nogrod / Default", "1.5.4", ResourceId = 841)]
	public class HuntRPG : RustPlugin
	{
		...
		object OnEntityTakeDamage(BasePlayer entity, HitInfo hitInfo)
		{
			...
			if (player.userID < 2147483647) return null;
			...
			{
				{
					HMK.Help, string.Join("\n", new[]
					{
						...
						"To use an easy to use GUI to spend point type \"/hgui\""
					})
				},
				{
		...
		private void UpdateHud(BasePlayer player, bool repaint = false, bool points = true)
		{
			...
			if (rpgInfo.Preferences.ShowHud == 1)
			{
				if (points)
				{
					guiInfo.LastHudFirst = elements.Add(CreatePanel("0.0 0.51", "1 1", "0.16 0.16 0.16 0.8"), guiInfo.LastHud);
...
namespace Hunt.RPG.Keys
{
	...
	static class HKD
	{
		...
		public const float LevelMultiplier = 1.055f;
