...
namespace Oxide.Plugins
{
    ...
    class KarmaKills : RustPlugin
    {
        ...
        public class StoredData
        {
            ...
            public List<ulong> Heros = new List<ulong>();
        ...
        public class Data
        {
            ...
            public class Heros
            {
                public static void Add(BasePlayer player)
                {
                    if (!Exists(player))
                    {
                        storedData.Heros.Add(player.userID);
                    }
                }
                ...
                public static void Remove(BasePlayer player)
                {
                    if (Exists(player))
                    {
                        storedData.Heros.Remove(player.userID);
                        SaveData();
                    }
                }
                ...
                public static bool Exists(BasePlayer player)
                {
                    if (storedData.Heros.Exists(x => x == player.userID))
                    {
                        return true;
                    }
                    return false;
        ...
        private void LoadDefaultMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                ["Cmd_CurrentHeros"] = "The current Heros online are \n{0}",
                ["Cmd_NoHeros"] = "There's currently no Heros on the server",
            }, this);
        }
        ...
        void Loaded()
        {
            ...
            if (configFile.AnimalKills.AddKarma.Enabled && configFile.AnimalKills.RemoveKarma.Enabled)
                PrintWarning(
                    "It's not reccomended to have 'AddKarma' AND 'RemoveKarma' enabled at the same time (AnimalKills)");
            if (configFile.PlayerKills.AddKarma.Enabled && configFile.PlayerKills.RemoveKarma.Enabled)
                PrintWarning(
                    "It's not reccomended to have 'AddKarma' AND 'RemoveKarma' enabled at the same time (PlayerKills)");
        }
        ...
        void OnEntityDeath(BaseCombatEntity entity, HitInfo info)
        {
            ...
            if (entity.ToPlayer() != null)
            {
                ...
                else if (WasHero(entity.ToPlayer()) && configFile.Hero.RemoveKarma.Enabled)
                {
                    ...
                    Data.Heros.Remove(entity.ToPlayer());
                    Data.Bandits.Add(player);
                }
            }
        ...
        private void heroCmd(BasePlayer player, string command, string[] args)
        {
            ...
            if (heroNames.Count > 0)
                PrintToChat(player,
                    Lang("Cmd_CurrentHeros", player.UserIDString, string.Join(",", heroNames.ToArray())));
            else
                PrintToChat(player, Lang("Cmd_NoHeros", player.UserIDString));
        }
