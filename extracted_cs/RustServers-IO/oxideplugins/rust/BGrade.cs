...
namespace Oxide.Plugins
{
    ...
    public class BGrade : RustPlugin
    {
        ...
        private readonly MethodInfo _startBeingRotatable = typeof(BuildingBlock).GetMethod("StartBeingRotatable", Flags);
        ...
        {
            for(var i = 1; i < 5; i++) permission.RegisterPermission(Name + "." + i, this);
            permission.RegisterPermission(Name + "." + "nores", this);
        ...
        {
            int time;
            if (!LastAttacked.TryGetValue(position, out time)) return false;
        ...
        private class BGradePlayer : MonoBehaviour
        {
            ...
            {
                if (!Instance.AllowTimer) return 0;
                if (updateTime) UpdateTime();
                if (_time != 0) return _time;
                return Instance.DefaultTimer;
            }
            ...
            public bool IsTimerValid() => _timer != null && !_timer.Destroyed;
            ...
            public void OnDestroy() => Destroy(this);
        }
        ...
        private void OnServerSave() => QueueWorkerThread(worker => CheckLastAttacked());

        private void Unload() => DestroyAll<BGradePlayer>();
        ...
        private void OnEntityBuilt(Planner plan, GameObject gameObject)
        {
            ...
            if (player == null || buildingBlock == null) return;

            if (!player.CanBuild() || !player.HasAnyPermission(RegisteredPerms)) return;

            var bgradePlayer = player.GetComponent<BGradePlayer>();
            if (bgradePlayer == null) return;
            ...
            var oldHookCall = Interface.CallDeprecated("CanAutoGrade", "CanBGrade", HookExpire, player, playerGrade, buildingBlock, plan);
            var hookCall = Interface.Call("CanBGrade", player, playerGrade, buildingBlock, plan);

            if (oldHookCall is int || hookCall is int)
            {
                DealWithHookResult(player, buildingBlock, oldHookCall is int ? (int) oldHookCall : (int) hookCall, gameObject);
                return;
            }
            ...
            _startBeingRotatable.Invoke(buildingBlock, null);
        ...
        {
            if (!(combatEntity is BuildingBlock)) return;
        ...
        private void OnPlayerDisconnected(BasePlayer player)
        {
            var bgradePlayer = player.GetComponent<BGradePlayer>();
            if (bgradePlayer == null) return;
            UnityEngine.Object.Destroy(player.GetComponent<BGradePlayer>());
        }
        ...
        private void BGradeCommand(BasePlayer player, string command, string[] args)
        {
            ...
            switch (args[0].ToLower())
            {
                ...
                case "4":
                {
                    ...
                    if (AllowTimer && time > 0) chatMsgs.Add("Notice.Time".Lang(player.UserIDString, time));
                ...
                case "help":
                {
                    ...
                    if(AllowTimer) chatMsgs.Add("Command.Help.0".Lang(player.UserIDString, command));
                    for (var i = 1; i < 5; i++)
                    {
                    ...
                    if (chatMsgs.Count <= 2 && !player.HasPluginPerm("all"))
                    {
                        player.ChatMessage("Permission".Lang(player.UserIDString));
                        return;
                    }
                    ...
                    if (bgradePlayer != null)
                    {
                        ...
                        if(AllowTimer) chatMsgs.Add("Command.Settings.Timer".Lang(player.UserIDString, bgradePlayer.GetTime(false)));
...
namespace Oxide.Plugins.BGradeExt
{
    public static class BGradeExtensions
    {
        ...
        public static bool HasPermission(this BasePlayer player, string perm) => permission.UserHasPermission(player.UserIDString, perm);

        public static bool HasPluginPerm(this BasePlayer player, string perm) => permission.UserHasPermission(player.UserIDString, BGrade.Instance.Name.ToLower() + "." + perm);
