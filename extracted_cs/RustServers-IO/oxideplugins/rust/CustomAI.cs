...
namespace Oxide.Plugins {
    ...
    class CustomAI : RustPlugin {
        ...
        class AIAnimal : MonoBehaviour {
            ...
            bool IsNearBuilding(Vector3 point) {
                RaycastHit hitInfo;
                if (Physics.Raycast(new Ray(new Vector3(point.x, point.y + 50, point.z), Vector3.down), out hitInfo, float.PositiveInfinity, layerBuilding))
                    return true;
                if (Physics.Raycast(new Ray(new Vector3(point.x + 0.8f, point.y + 50, point.z + 0.8f), Vector3.down), out hitInfo, float.PositiveInfinity, layerBuilding))
                    return true;
                if (Physics.Raycast(new Ray(new Vector3(point.x - 0.8f, point.y + 50, point.z - 0.8f), Vector3.down), out hitInfo, float.PositiveInfinity, layerBuilding))
                    return true;
                return false;
        ...
        void OnServerInitialized() {
            ...
            if (config.RemoveAllScientists) {
                var npcPlayerApexArray = GameObject.FindObjectsOfType<NPCPlayerApex>();
                if (npcPlayerApexArray.Length == 0) return;
                int i = 0;
                for (; i < npcPlayerApexArray.Length; i++)
                    npcPlayerApexArray[i].Kill();
                Puts($"Removed {i} scientists (Config => Remove all scientists => true)");
            }
        }
