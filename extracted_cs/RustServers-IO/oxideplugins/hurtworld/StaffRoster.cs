...
{
    [Info("StaffRoster", "Noviets", "1.0.7", ResourceId = 2048)]
    ...
    class StaffRoster : HurtworldPlugin
    {
        ...
        void staffCommand(PlayerSession session, string command, string[] args)
        {
            ...
            {
                UpdateList();
                hurt.SendChatMessage(session, null, Msg("stafflist",session.SteamId.ToString()));
                foreach(var staffmember in StaffList)
                {
                    hurt.SendChatMessage(session, null, staffmember.Key.ToString()+" Status: "+staffmember.Value);
                }
            }
            if(args.Length == 1)
            {
                if(session.IsAdmin || permission.UserHasGroup(session.SteamId.ToString(),"Admin") || permission.UserHasGroup(session.SteamId.ToString(),"Moderator"))
                {
                    if(args[0].ToLower() == Msg("afkcmd") || args[0].ToLower() == Msg("busycmd") ||args[0].ToLower() == Msg("availablecmd") ||args[0].ToLower() == Msg("offdutycmd"))
                    {
                        ...
                        hurt.SendChatMessage(session, null, Msg("invalidstatusmsg",session.SteamId.ToString()).Replace("{available}", Msg("availablecmd")).Replace("{afk}", Msg("afkcmd")).Replace("{busy}", Msg("busycmd")).Replace("{offduty}", Msg("offdutycmd")));
                }
                else
