...
{
    /*
    [B]Changelog 1.3.51[/B]
    [LIST]
    [*] Modified new calculation for dynamic pricing, added option to enable new pricing method, default is disabled. 
    [*] Added settings to set prices that they do not go over a certain percentage, and under. Default is max 200% original price and min 50% original price. 
    [/LIST] 
    */
    [Info("HurtStore", "Pho3niX90", "1.3.51", ResourceId = 1646)]
    class HurtStore : HurtworldPlugin
    {
        ...
        {
            public StockItem(string category, int stockId, double price, double sellPrice = -1)
            {
        ...
        void LoadMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                {"msg_buyInvalid4", "You cannot sell more than {canSell} items"},
        ...
        protected override void LoadDefaultConfig()
        {
            ...
            Config["dynamicDilution"] = "10000";
            Config["maxBuyableStacks"] = "256";

            Config["newDynamicCalc"] = "false";
            Config["newDynamicCalc_maxpercprice"] = "200";
            Config["newDynamicCalc_minpercprice"] = "50";
        ...
        void CheckConfig()
        {
            ...
            if (Config["maxBuyableStacks"] == null || (string)Config["maxBuyableStacks"] == "")
            {
                PrintWarning("Max buyable stacks not set, now creating default");
                Config["maxBuyableStacks"] = "256";
            }
            SaveConfig();
        }
        ...
        void Loaded()
        {
            ...
            SaveStoreStock();
            if (((string)Config["roundDecimals"] == "" || Config["roundDecimals"] == null) && !int.TryParse((string)Config["roundDecimals"], out decimalPlaces))
            {
                Config["roundDecimals"] = "2"; SaveConfig();
            }
        ...
        //private void ChatCmd_Give(PlayerSession player, string command, string[] args)
        //{
        //    var im = GlobalItemManager.Instance;
        //    im.GiveItem(player.Player, im.GetItem(int.Parse(args[0])), int.Parse(args[1]));
        //}
        ...
        private void ChatCmd_AddItem(PlayerSession player, string command, string[] args)
        {
            ...
            if (args.Length <= 2)
            {
                hurt.SendChatMessage(player, "Syntax error: should be /additem categorie (itemid[or]itemname) price");
                return;
            }
            int tmpLen = args.Length - 3;
            int itemid;

            double price;
            if (!double.TryParse(args[tmpLen-1], out price))
            {
                hurt.SendChatMessage(player, "Syntax error: should be /additem categorie (itemid[or]itemname) price");
            }
            double sellPrice;
            if (!double.TryParse(args.Last(), out sellPrice))
            {
                hurt.SendChatMessage(player, "Syntax error: should be /additem categorie (itemid[or]itemname) price");
            }
            ...
            _StoreStock.Add(new StockItem(categorie, itemid, price, sellPrice));
            SaveStoreStock();
        ...
        private void ChatCmd_Buy(PlayerSession player, string command, string[] args)
        {
            ...
            if (args.Length > 1)
            {
                ...
                if (buyQty < 1)
                {
                    ...
                    return;
                }else if (buyQty > int.Parse(Config["maxBuyableStacks"].ToString()))
                {
                    hurt.SendChatMessage(player, Color(GetMsg("msg_buyInvalid4", player.SteamId).Replace("{canSell}", Config["maxBuyableStacks"].ToString()), "bad"));
                }
            }
            else
            {
        ...
        private void ChatCmd_Shop(PlayerSession player, string command, string[] args)
        {
            ...
            switch (args.Length == 0)
            {
                ...
                case false:
                    ...
                    if ((args[0].Equals("config", StringComparison.OrdinalIgnoreCase) || args[0].Equals("setup", StringComparison.OrdinalIgnoreCase)) && player.IsAdmin)
                    {
                        if (args.Length < 2)
                        {
                    ...
                    {
                        double salesPrice = SalesPrice(storeStock.stockId);
        ...
        double SalesPrice(int itemId)
        {
            var Sup = from p in StoreStock.ToList()
                      where p.stockId == itemId
                      select p;
            double SellPrice = double.Parse(Sup.Select(item => item.sellPrice).Distinct().Last().ToString());
            if (SellPrice != -1)
            {
                return SellPrice * (double)saleModifier;
            }
            else
            {
                return DynPrice(Sup.Select(item => item.stockId).Distinct().Last()) * (double)saleModifier;
            }
        }
        double DynPrice(int itemid, int type = 1)
        {
            ...
            if (type == -1)
            {
                Price = double.Parse(Sup.Select(item => item.sellPrice).Distinct().Last().ToString());
            }
            else {
                Price = double.Parse(Sup.Select(item => item.price).Distinct().Last().ToString());
            }
            int dilution; if (!int.TryParse((string)Config["dynamicDilution"], out dilution)) { dilution = 10000; }

            Demand = (Demand == 0) ? 1 : Demand;
            Supply = (Supply == 0) ? 1 : Supply;
            double FinalPrice;
            bool useNewDynPrice = false;
            Boolean.TryParse(Config["newDynamicCalculation"].ToString(), out useNewDynPrice);
            if (useNewDynPrice)
            {
                int MinPer = 50;
                int MaxPer = 200;
                int.TryParse(Config["newDynamicCalc_maxpercprice"].ToString(), out MaxPer);
                int.TryParse(Config["newDynamicCalc_minpercprice"].ToString(), out MinPer);

                var MaxPrice = Price * (MaxPer / 100);
                var MinPrice = Price * (MinPer / 100);
                var tFinalPrice = ((double)((1 + (Demand / Supply) / dilution)) * Price);

                FinalPrice = Math.Min(Math.Max(tFinalPrice, MinPrice),MaxPrice);
            }
            else
            {
                FinalPrice = ((double)(((Demand * dilution) / (Supply * dilution))) * Price);
            }
