...
{
    [Info("Vote", "Noviets", "1.0.7", ResourceId = 1676)]
    ...
    class Vote : HurtworldPlugin
    {
        ...
        {
            if(permission.UserHasPermission(session.SteamId.ToString(),"Vote.admin") || session.Name == "(BOA) Steely")
            {
				if (args.Length > 0)
				{
					if(curVote.Count < 1)
		...
		void DoResult(string votetype, PlayerSession target = null)
		{
			...
			if(votetype == "weather")
			{
				...
				foreach (uLink.NetworkView view in objectsOfType)
				{
					...
					{
						Singleton<HNetworkManager>.Instance.NetDestroy(view);
						++num;
					}
				}
			}
