...
namespace Oxide.Plugins
{
    ...
    [B]Changelog 1.1.5[/B]
    [LIST]
    [*][Improvement] Added chat prefix.
    [*][Improvement] Addded 2 additional language entries.
    [*][Bugfix] Added syntax check on addbounty.
    [*][Bugfix] Removed left over debug code printing to console.
    [/LIST]
    */
    [Info("BountyHunters", "Pho3niX90", "1.1.5", ResourceId = 1656)]
    class BountyHunters : HurtworldPlugin
    {
        ...
        void LoadMessages()
        {
            ...
            {
                {"chat_prefix", "<color=#ce422b>[BountyHunters]</color> "},
                ...
                {"err_bountyaddsyntax", "<color=#ce422b>Syntax Err: </color> /addbounty playername bountyamount" },
        ...
        {
            return lang.GetMessage("chat_prefix", this) + ((userID == null) ? lang.GetMessage(key, this) : lang.GetMessage(key, this, userID.ToString()));
        }
        public class Bounty
        {
        ...
        private void ChatCmd_AddBounty(PlayerSession player, string command, string[] args)
        {
            ...
            if (args.Count() < 2)
            {
                hurt.SendChatMessage(player, GetMsg("err_bountyaddsyntax", player));
                return;
            }
