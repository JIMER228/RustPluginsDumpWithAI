...
namespace Oxide.Plugins
{
    ...
    public class Meteors : HurtworldPlugin
    {
        ...
        public class MeteorInstance
        {
            ...
            public IEnumerator UpdateMeteor(float interval)
            {
                try
                {
                    ...
                    Meteors.Instance.hurt.BroadcastChat(null, string.Format(Meteors.Instance.lang.GetMessage(msg_meteor_hot, Meteors.Instance), this.heatDuration.ToString()));
                    ...
                    Meteors.Instance.hurt.BroadcastChat(null, string.Format(Meteors.Instance.lang.GetMessage(msg_meteor_cold, Meteors.Instance), this.heatDuration));
            ...
            private bool IsValidSession(PlayerSession session)
            {
                return (session != null) &&
                    (session.IsLoaded) &&
        ...
        public void ScheduleMeteor()
        {
            ...
            hurt.BroadcastChat(null, string.Format(lang.GetMessage(msg_meteor_earlyfallwarning, this), futureFallSite.Name, earlyAnnouncementTime));
        ...
        {
            float speed = 100;
            Vector3 targetPos = fallSite.position + new Vector3(UnityEngine.Random.Range(-fallSite.radius, fallSite.radius), 0, UnityEngine.Random.Range(-fallSite.radius, fallSite.radius));
            Vector3 startingPos = fallSite.position + new Vector3(UnityEngine.Random.Range(-1500, 1500), 1000, UnityEngine.Random.Range(-1500, 1500));

            hurt.BroadcastChat(null, string.Format(lang.GetMessage(msg_meteor_located_fallwarning, this), fallSite.Name));
        ...
        {
            hurt.BroadcastChat(null, string.Format(lang.GetMessage(msg_throwmeteor_fall_warning, this), targetPlayer.Name));

            float speed = 100f;
            Vector3 final = targetPlayer.WorldPlayerEntity.transform.position;
            Vector3 starting = final + new Vector3(UnityEngine.Random.Range(-2500, 2500), 1000, UnityEngine.Random.Range(-2500, 2500));
        ...
        void cmdRemoveFallSite(PlayerSession session, string command, string[] args)
        {
            if (!session.IsAdmin)
            {
                hurt.SendChatMessage(session, null, lang.GetMessage(msg_notadmin, this, session.SteamId.ToString()));
                return;
            }
            ...
            {
                hurt.SendChatMessage(session, null, lang.GetMessage(msg_removefallsite_badusage, this, session.SteamId.ToString()));
                return;
            }

            if (index >= this.fallsSitesDB.fallSites.Count)
            {
                hurt.SendChatMessage(session, null, lang.GetMessage(msg_removefallsite_notfound, this, session.SteamId.ToString()));
                return;
            }

            this.fallsSitesDB.RemoveAt(index);
            hurt.SendChatMessage(session, null, lang.GetMessage(msg_removefallsite_done, this, session.SteamId.ToString()));
        }
        ...
        void cmdAddFallSite(PlayerSession session, string command, string[] args)
        {
            ...
            if (args.Length <= 1)
            {
                hurt.SendChatMessage(session, null, lang.GetMessage(msg_fallsite_badusage1, this, session.SteamId.ToString()));
                hurt.SendChatMessage(session, null, lang.GetMessage(msg_fallsite_badusage2, this, session.SteamId.ToString()));
                hurt.SendChatMessage(session, null, lang.GetMessage(msg_fallsite_badusage3, this, session.SteamId.ToString()));
                return;
            }
            ...
            hurt.SendChatMessage(session, null, lang.GetMessage(msg_fallsite_added, this, session.SteamId.ToString()));
        }
        ...
        void cmdSetMeteorsEnabled(PlayerSession session, string command, string[] args)
        {
            ...
            if (args.Length != 1)
            {
                hurt.SendChatMessage(session, null, lang.GetMessage(msg_setenabled_badusage, this, session.SteamId.ToString()));
                return;
            }
        ...
        void cmdSetMeteorsInterval(PlayerSession session, string command, string[] args)
        {
            ...
            if (args.Length == 1)
            {
                ...
                {
                    hurt.SendChatMessage(session, null, lang.GetMessage(msg_setinterval_badusage, this, session.SteamId.ToString())
                    );
                    return;
                }
            ...
            hurt.SendChatMessage(session, null, string.Format(lang.GetMessage(msg_setinterval_value, this, session.SteamId.ToString()),
        ...
        void cmdSetMeteorsLootFactor(PlayerSession session, string command, string[] args)
        {
            ...
            if (args.Length > 1)
            {
                hurt.SendChatMessage(session, null, lang.GetMessage(msg_setlootfactor_badusage, this, session.SteamId.ToString()));
                return;
            }
            ...
            hurt.SendChatMessage(session, null, string.Format(lang.GetMessage(msg_setlootfactor_value, this, session.SteamId.ToString()), this.cfg_MeteorsLootFactor.value.ToString()));
        }
        ...
        void cmdSetMeteorsHeatDuration(PlayerSession session, string command, string[] args)
        {
            ...
            if (args.Length > 1)
            {
                hurt.SendChatMessage(session, null, lang.GetMessage(msg_setheatduration_badusage, this, session.SteamId.ToString()));
                return;
            }
            ...
            hurt.SendChatMessage(session, null, string.Format(lang.GetMessage(msg_setheatduration_value, this, session.SteamId.ToString()), this.cfg_MeteorsHeatDuration.value.ToString()));
        }
        ...
        void cmdThrowMeteor(PlayerSession session, string command, string[] args)
        {
            ...
            if (int.TryParse(args[0], out fallSiteIndex))
            {
                if (fallSiteIndex >= this.fallsSitesDB.fallSites.Count)
                {
                    hurt.SendChatMessage(session, null, lang.GetMessage(msg_throwmeteor_playernotfound, this, session.SteamId.ToString()));
                    return;
                }
        ...
        void cmdMeteorInfo(PlayerSession session, string command, string[] args)
        {
            if ((args.Length == 1) && (args[0].ToLower() == "help"))
            {
                ...
                hurt.SendChatMessage(session, null, lang.GetMessage(msg_help_cmdsheader, this, steamID));
                hurt.SendChatMessage(session, null, lang.GetMessage(msg_help_cmd0, this, steamID));
                hurt.SendChatMessage(session, null, lang.GetMessage(msg_help_cmd1, this, steamID));
                hurt.SendChatMessage(session, null, lang.GetMessage(msg_help_cmd2, this, steamID));
                hurt.SendChatMessage(session, null, lang.GetMessage(msg_help_cmd3, this, steamID));
                hurt.SendChatMessage(session, null, lang.GetMessage(msg_help_cmd4, this, steamID));
                hurt.SendChatMessage(session, null, lang.GetMessage(msg_help_cmd5, this, steamID));
                hurt.SendChatMessage(session, null, lang.GetMessage(msg_help_cmd0, this, steamID));
                hurt.SendChatMessage(session, null, lang.GetMessage(msg_help_cmd7, this, steamID));
                return;
            }
            ...
            if (lootMeteors.Count() == 0)
            {
                ...
                {
                    hurt.SendChatMessage(session, null, lang.GetMessage(msg_meteorinfo_none, this, session.SteamId.ToString()));
                    return;
                }

                int minutesLeft = (int)(1 + this.cfg_MeteorsInterval.value - ((Time.time - nextRandomMeteorTimerStart) / 60.0f) );
                hurt.SendChatMessage(session, null, string.Format(lang.GetMessage(msg_meteorinfo_next, this, session.SteamId.ToString()), minutesLeft));
                return;
            }
            ...
            foreach (MeteorInstance mi in lootMeteors)
            {
                ...
                hurt.SendChatMessage(session, null, string.Format(lang.GetMessage(msg_meteorinfo_current, this, session.SteamId.ToString()), fallsiteName, heatMinutesLeft));
            }
        }
    }
}
