...
namespace Oxide.Plugins
{
    ...
    class Teleportation : HurtworldPlugin
    {
        ...
        void cmdHome(PlayerSession player, string command, string[] args)
        {
            ...
            if (HasStakeAuthority(player, homes[id(player)][home].vector))
            {
                if (GetConfig(false, "Settings", "Home : Surrender on Teleport"))
                    StartSurrender(player);

                SendChatMessage(player,
                    GetMsg("Teleporting Soon", player.SteamId).Replace("{time}", homeTeleportTimer.ToString()));
        ...
        bool HasStakeAuthority(PlayerSession player, Vector3 vector)
        {
            bool hasAuthority = false;
            float radius = GetConfig(10f, "Settings", "Home : Stake Radius");
            List<OwnershipStakeServer> entities = StakesInArea(vector, radius);
