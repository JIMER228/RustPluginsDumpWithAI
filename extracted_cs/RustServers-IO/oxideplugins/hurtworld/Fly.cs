...
{
    [Info("Fly", "Mughisi", "1.1.3", ResourceId = 1634)]
    public class Fly : HurtworldPlugin
    {
        ...
        {
            if (!players.ContainsKey(session) || !players[session].IsFlying) return;
        ...
        private Vector3 Ground(PlayerSession session)
        {
            ...
            RaycastHit hit;
            if (Physics.Raycast(loc, dir, out hit)) return new Vector3(hit.point.x, hit.point.y + 1f, hit.point.z);
            return loc;
        }
