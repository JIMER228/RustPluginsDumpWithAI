...
{
    /*
    [B]Changelog 1.5.51[/B]
    [LIST]
    [*] Fixed [EcoBanks] appearing in Shops and other dependant plugins
    [/LIST]
    */
    [Info("EconomyBanks", "Pho3niX90", "1.5.51", ResourceId = 1653)]
    class EconomyBanks : HurtworldPlugin
    {
        ...
        private void Chat_Wipe(PlayerSession player, string command, string[] args)
        {
            if (args.Length < 1)
            {
                hurt.SendChatMessage(player, "Use /wipe all [backup] - if backup is added, it will backup the users bankaccounts before clearing.");
                return;
            }
