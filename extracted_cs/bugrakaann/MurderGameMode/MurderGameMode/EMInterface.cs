...
{
    [Info("EMInterface", "k1lly0u", "2.0.1")]
    ...
    public class EMInterface : RustPlugin
    {
        ...
        {
            CuiElementContainer container = UI.Container(UI_MENU, Configuration.Menu.Background.Get, new UI4(0.1f, 0.1f, 0.9f, 0.9f), true,"Overall");
        ...
        private void AddMenuButtons(BasePlayer player, CuiElementContainer container, string panel, MenuTab menuTab)
        {
            ...
            UI.Button(container, panel, menuTab == MenuTab.Event ? Configuration.Menu.Highlight.Get : Configuration.Menu.Button.Get, Message("UI.Menu.Rooms", player.userID), 13, new UI4(xMin, 0.9f, xMin + 0.14f, 0.94f), menuTab == MenuTab.Event ? "" : $"emui.event 0 {(int)MenuTab.Event}");
        ...
        #region Event Tab
        //Opens the previous selected page of room details
        Hash<ulong, EventManager.BaseEventGame> selectedRoom = new Hash<ulong, EventManager.BaseEventGame>();
        private void CreateEventDetails(BasePlayer player, CuiElementContainer container, string panel, int page = 0)
        {
            ...
            UI.Label(container, UI_MENU, Message("UI.Room.Selected", player.userID), 13, new UI4(0.01f, 0.845f, 0.499f, 0.885f), TextAnchor.MiddleLeft);

            //Show opened rooms to right 
            if (EventManager.BaseManager.Count > 0) // Show current event details on the left and current scores on the right
            {
                #region Current Event Info
                //Overriding event infos
                EventManager.BaseEventGame eventGame;
                selectedRoom.TryGetValue(player.userID, out eventGame);
                int i = 0;
                int a = 0;
                if(eventGame != null) 
                { 
                    ...
                    CreateListEntryLeft(container, Message("UI.Room.Arena", player.userID), eventGame.gameroom.place, GetVerticalPos(i += 1, 0.841f));
                    CreateListEntryLeft(container, Message("UI.Room.OwnerName", player.userID), eventGame.gameroom.ownerName, GetVerticalPos(i += 1, 0.841f));
                    CreateListEntryLeft(container, Message("UI.Room.RoomID", player.userID), eventGame.gameroom.roomID.ToString(), GetVerticalPos(i += 1, 0.841f));

                    CreateListEntryLeft(container, Message("UI.Room.HasPassword"), eventGame.gameroom.hasPassword.ToString(), GetVerticalPos(i += 1, 0.841f));
                    if(eventGame.gameroom.hasPassword)
                        CreateListEntryLeft(container, Message("UI.Room.Password"), eventGame.gameroom.password, GetVerticalPos(i += 1, 0.841f));
                    ...
                    string.Format(Message("UI.Players.Format", player.userID), eventGame.eventPlayers.Count, eventGame.Config.MaximumPlayers, eventGame.spectators.Count),
                    ...
                    UI.Button(container, UI_MENU, UI.Color("#7c0000", 1f), "Close Room", 10, UI.TransformToUI4(657f, 767f, 9f, 30f, 1536f, 864f), $"emui.closeroom {eventGame.gameroom.roomID}", TextAnchor.MiddleCenter);
                ...
                #region Room buttons in Rooms Tab
                foreach (EventManager.BaseEventGame game in EventManager.BaseManager.Values)
                {
                    float xMin = GetHorizontalPos(a / 20, 0.501f, 0.122f);
                    UI.Button(container, UI_MENU, Configuration.Menu.Highlight.Get, $"{game.gameroom.place}: {game.gameroom.ownerName}", 9, new UI4(xMin, GetVerticalPos((a%20)+1 , 0.841f), xMin + 0.120f, GetVerticalPos((a%20)+1, 0.841f) + ELEMENT_HEIGHT), $"emui.getroom {game.gameroom.roomID}");
                    a++;
                }
                if (a > 79)
                {
                    CreateMenuPopup(player, "Reached maximum container room capacity.",10f);
                }
                UI.Label(container, UI_MENU, Message("UI.Room.AvailableRooms", player.userID), 13, new UI4(0.506f, 0.845f, 0.995f, 0.885f), TextAnchor.MiddleLeft);
        ...
        private void CreateAdminOptions(BasePlayer player, CuiElementContainer container, string panel, MenuArgs args)
        {
            ...
            yMin = GetVerticalPos(i += 1, 0.836f);
            // Oyuncu kickleme fonksiyonunu yazmayı unutma
        ...
        private void EventCreatorMenu(BasePlayer player, CuiElementContainer container, string panel)
        {
            ...
            else
            {
                ...
                if(!EventManager.AdminEditStaticsRoom.IsAdminAlreadyEditingRoom(player))
                UI.Button(container, UI_MENU, Configuration.Menu.Highlight.Get, "Edit Static Objects", 12, new UI4(0.775f, 0.845f, 0.845f, 0.885f), $"emui.editstaticobjects {eventConfig.EventName}");
                else
                    UI.Button(container, UI_MENU, Configuration.Menu.Highlight.Get, "Finish editing", 12, new UI4(0.775f, 0.845f, 0.845f, 0.885f), $"emui.finisheditingstaticobjects");
        ...
        [ConsoleCommand("emui.getroom")]
        private void ccmdGetRoomInfo(ConsoleSystem.Arg arg)
        {
            BasePlayer player = arg.Player();
            if (player == null) return;
            if (!arg.HasArgs(0)) return;
            if(selectedRoom.Keys.Contains(player.userID))
                selectedRoom.Remove(player.userID);
            if(EventManager.BaseManager.Keys.Contains(arg.GetInt(0)))
                selectedRoom.Add(player.userID, EventManager.BaseManager[arg.GetInt(0)]);
        ...
        [ConsoleCommand("emui.closeroom")]
        private void cmdCloseRoom(ConsoleSystem.Arg arg)
        {   
            BasePlayer player = arg.Player();
            if (player == null)
                return;
            ...
            EventManager.BaseEventGame room;
            if(EventManager.BaseManager.TryGetValue(roomID, out room))
            {
                room.EndEvent();
                OpenMenu(player, new MenuArgs(MenuTab.Event));
            }
        ...
        [ConsoleCommand("emui.closeevent")]//argüman olarak roomID sok.
        private void ccmdCloseEvent(ConsoleSystem.Arg arg)
        {
            ...
            if (player.IsAdmin || permission.UserHasPermission(player.UserIDString, EventManager.ADMIN_PERMISSION))
            {
                EventManager.BaseEventGame game = EventManager.BaseManager[arg.GetInt(0)];
                CreateMenuPopup(player, $"Closed event {game.Config.EventName} to new players");
                game.CloseEvent();
            }
        }
        #endregion
        ...
        public static class UI
        {
            ...
            public static void AddBlur(CuiElementContainer container,string color, string parent = "HUD" ,string material= "assets/content/ui/uibackgroundblur.mat", string sprite = "assets/content/materials/highlight.png")
            {
                container.Add(new CuiElement
                {
                    ...
                    Components =
                    {
                        new CuiRawImageComponent { Color = color, Sprite = sprite, Material = material},
                        new CuiRectTransformComponent {AnchorMin = "0 0", AnchorMax = "1 1"}
                    }
                });
            }
            public static void Panel(CuiElementContainer container, string panel,string name, string color, UI4 dimensions)
            {
                container.Add(new CuiPanel
                {
                    Image = { Color = color },
            ...
            public static void Button(CuiElementContainer container, string panel, string color, string text, int size, UI4 dimensions, string command, TextAnchor align = TextAnchor.MiddleCenter,string font = "RobotoCondensed-Bold.ttf")
            {
            ...
            public static void Button(CuiElementContainer container, string panel, string color, string text,string textcolor, int size, UI4 dimensions, string command, TextAnchor align = TextAnchor.MiddleCenter, string font = "RobotoCondensed-Bold.ttf")
            {
                container.Add(new CuiButton
                {
                    Button = { Color = color, Command = command, FadeIn = 0f },
                    RectTransform = { AnchorMin = dimensions.GetMin(), AnchorMax = dimensions.GetMax() },
                    Text = { Text = text, FontSize = size, Align = align, Font = font, Color= textcolor }
                },
                panel);
            }

            public static void Input(CuiElementContainer container, string panel, string text, int size, string command, UI4 dimensions,int charslimit = 300,bool isPassword = false)
            {
                container.Add(new CuiElement
                {
        ...
        public class UI4
        {
            ...
            /// <summary>
            /// Used for shifting ui elements
            /// </summary>
            /// <returns>transform values of previous ui element</returns>
            public UI4 SetOffset(float yOffset,int yIndex,float xOffset = 0,int xIndex = 0)
            {
                return new UI4(xMin + (xOffset * xIndex), yMin - (yOffset * yIndex), xMax + (xOffset * xIndex), yMax - (yOffset * yIndex));
            }
        ...
        private ConfigData GetBaseConfig()
        {
            return new ConfigData
            {
                ...
                {                    
                    ...
                    Highlight = new ConfigData.UIColor { Hex = "#953737", Alpha = 1f },
                },     
                Scoreboard = new ConfigData.ScoreboardColors
                {
                    ...
                    Highlight = new ConfigData.UIColor { Hex = "#953737", Alpha = 0.8f },
        ...
        private readonly Dictionary<string, string> Messages = new Dictionary<string, string>
        {
            ...
            ["UI.Room.Arena"] = "Arena",
            ["UI.Room.RoomID"] = "Room ID",
            ["UI.Room.HasPassword"] = "Has Password",
            ["UI.Room.Password"] = "Password",
            ["UI.Room.OwnerName"] = "Owner Name",
            ...
            ["UI.Room.AvailableRooms"] ="Available Rooms",
