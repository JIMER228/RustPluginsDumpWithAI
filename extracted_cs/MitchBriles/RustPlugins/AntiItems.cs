...
{
    [Info("AntiItems", "Author redBDGR, Maintainer nivex", "1.0.12")]
    ...
    class AntiItems : RustPlugin
    {
        ...
        private void OnEntityDeath(BasePlayer player, HitInfo info)
        {
            ...
            RemoveItems(player, player.inventory.containerMain);
            ItemManager.DoRemoves(); // shouldn't be needed
        }
        ...
        private void DoItems(BasePlayer player)
        {
            ...
            for (var i = 0; i < componentList.Count; i++)
            {
                ...
                if (item == null)
                {
                    Puts($"{y[i]} was not able to be created properly. Perhaps the name of it is wrong");
                    continue;
                }
                if (!item.MoveToContainer(player.inventory.containerMain, 24 + i, true))
                {
                    item.Remove();
                }
            }
