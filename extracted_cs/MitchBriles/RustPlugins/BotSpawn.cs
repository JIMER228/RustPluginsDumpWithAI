...
namespace Oxide.Plugins 
{
    ...
    class BotSpawn : RustPlugin
    {
        ...
        public Dictionary<string, List<string>> KitList = new Dictionary<string, List<string>>();
        public Dictionary<string, List<string>> NamesList = new Dictionary<string, List<string>>();
                    ...
                    if (target > 0)
                    {
                        ...
                        if (amount > 0)
                        {
                            timer.Repeat(2, amount, () =>
                            {
                                ...
                                {
                                    SpawnBots(name, AllProfiles[name], "biome", null, spawnlist[i], -1, true);
                                    i++;
                                }
                            });
                        }
        ...
        int GetNextSpawn(string name, DataProfile profile)
        {
            ...
            {
                if (availableStatSpawns[name].Count == 0) 
                    return -1;
        ...
        void AdjustQueue(string profile, int num)
        {
            popinfo[profile].queued += num;

            //catch, not a fix
            if (popinfo[profile].queued < 0)
                popinfo[profile].queued = 0;
        }
                    ...
                    timer.Repeat(1f, targetAmount - current, () =>
                    {
                        if (profile.Value.UseCustomSpawns && spawnsData.CustomSpawnLocations.ContainsKey(profile.Key))
                        {
                            ...
                            if (num > 0)
                            {
                                ...
                                SpawnBots(profile.Key, AllProfiles[profile.Key], null, null, new Vector3(), spawnnum, false);
                        ...
                        else
                        {
                            if (timers.ContainsKey(profile.Key))
                            {
                                if (spawnLists[profile.Key].Count > 0)
                                    SpawnBots(profile.Key, AllProfiles[profile.Key], "biome", null, spawnLists[profile.Key][random.Next(spawnLists[profile.Key].Count)], -1, false);
                            }
                            else
                            {
                                SpawnBots(profile.Key, AllProfiles[profile.Key], null, null, new Vector3(), -1, false);
                                if (profile.Value.Announce_Spawn && profile.Value.Announcement_Text != String.Empty)
                                    PrintToChat(profile.Value.Announcement_Text); 
                            }
                        }
        ...
        #region BotSetup  
        void DeployNpcs(Vector3 location, string name, DataProfile profile, string group, int num) => SpawnBots(name, profile, "Attack", group, location, num, true);
        void SpawnBots(string name, DataProfile zone, string type, string group, Vector3 location, int spawnNum, bool initial)
        {
            ...
            if (location == Vector3.zero && type != "AirDrop" && type != "HackableLockedCrate" && zone.UseCustomSpawns && spawnsData.CustomSpawnLocations[name].Count > 0)
            {
                ...
                {
                    PrintWarning(lang.GetMessage("noNav", this), name, spawnsData.CustomSpawnLocations[name].IndexOf(customLoc)+1);
                    return;
                }
            }
            else
            ...
            NextTick(() =>
            {
                ...
                if (configData.Global.Kits_In_Order)
                {
                    if (KitList[name].Count == 0)
                        KitList[name] = zone.Kit.ToList(); 

                    if (KitList[name].Count > 0)
                    {
                        kit = KitList[name][0];
                        KitList[name].RemoveAt(0);
                    }

                    if (NamesList[name].Count == 0)
                        NamesList[name] = zone.BotNames.ToList();

                    if (NamesList[name].Count > 0)
                    {
                        botname = NamesList[name][0];
                        NamesList[name].RemoveAt(0);
                    }
                }
                else
                {
                    if (zone.Kit.Count > 0)
                        kit = zone.Kit[random.Next(zone.Kit.Count)];

                    if (zone.BotNames.Count > 0)
                        botname = zone.BotNames[random.Next(zone.BotNames.Count)];
                }
                ...
                SetName(zone, npc, botname); 
                GiveKit(npc, zone, kit);
                ...
                ToggleAggro(npc, 0, zone.Aggro_Range);
        ...
        void SetName(DataProfile zone, NPCPlayerApex npc, string name)
        {
        ...
        void GiveKit(NPCPlayerApex npc, DataProfile zone, string kit)
        {
            ...
            if (kit != string.Empty)
            {
                ...
                if (checkKit == null)
                {
                    PrintWarning($"Kit {kit} does not exist - Spawning default {type}.");
                }
                else
                {
                    ...
                    if (!weaponInBelt)
                    {
                        PrintWarning($"Kit {kit} has no items in belt - Spawning default {type}.");
                    } 
                    else
                    {
        ...
        {
            return NPCPlayers.ContainsKey(userID) ? true : (object)null;
        }

        private object CanBeTargeted(BaseCombatEntity player, BaseEntity entity)
        {
                ...
                if (attackPlayer != null)
                {
                    ...
                    if (Vector3.Distance(attackPlayer.transform.position, bot.transform.position) > bot.Stats.AggressionRange)
                    {
                        ...
                        timer.Once(20, () =>
                        {
                            if (bot == null)
                                return;
                            ToggleAggro(bot, 0, bData.profile.Aggro_Range);
                        });
                    }
        ...
        void OnEntityKill(NPCPlayerApex npc, HitInfo info) 
        {
            if (npc?.userID != null && NPCPlayers.ContainsKey(npc.userID) && !botInventories.ContainsKey(npc.userID))
            {
                ...
                //void OnBotSpawnNpcKilled(string ProfileName, Vector3 position, bool RespawningProfile, bool KilledByBotSpawn)
                Interface.Oxide.CallHook("OnBotSpawnNpcKilled", bData.monumentName, npc.transform.position, bData.respawn, DontRespawn.Contains(npc.userID));
                ...
                {
                    if (bData.profile.RustRewardsValue > 0)
                    {
                        var weapon = info?.Weapon?.GetItem()?.info?.shortname ?? info?.WeaponPrefab?.ShortPrefabName;
                        RustRewards?.Call("GiveRustReward", info.InitiatorPlayer, 0, bData.profile.RustRewardsValue, npc, weapon, Vector3.Distance(info.InitiatorPlayer.transform.position, npc.transform.position), null);
                    }
                    ...
                    if (CanRespawn(bData.monumentName, 1, false) == 1)
                    {
                        timer.Once(bData.profile.Respawn_Timer, () =>
                        {
                            if (AllProfiles.ContainsKey(bData.monumentName)) 
                            {
                                ...
                                {
                                    SpawnBots(bData.monumentName, AllProfiles[bData.monumentName], "biome", null, spawnList[spawnPos], -1, false);    
                    ...
                    timer.Once(bData.profile.Respawn_Timer, () => 
                    {
                        ...
                        if (AllProfiles.ContainsKey(bData.monumentName))
                        {
                            if (CanRespawn(bData.monumentName, 1, true) == 1)
                            {
                                ...
                                SpawnBots(bData.monumentName, AllProfiles[bData.monumentName], null, null, new Vector3(), spawnnum, false);
                                if (bData.profile.Announce_Spawn && bData.profile.Announcement_Text != String.Empty)
                                    PrintToChat(bData.profile.Announcement_Text);
                            }
                            else if (bData.profile.UseCustomSpawns)
                                availableStatSpawns[bData.monumentName].Add(bData.CustomSpawnNum);
                        }
                    });
                }
                else
                {
                    ...
                    if (bData.profile.UseCustomSpawns)
                        availableStatSpawns[bData.monumentName].Add(bData.CustomSpawnNum);
                }
            }
        }
            ...
            if (AllProfiles.ContainsKey("AirDrop"))
            {
                ...
                if (prof.AutoSpawn == true && prof.Day_Time_Spawn_Amount > 0)
                {
                    ...
                    timer.Repeat(1f, profile.Day_Time_Spawn_Amount, () =>
                    {
                        ...
                        SpawnBots("AirDrop", profile, "AirDrop", null, new Vector3(), -1, true); 
                    });
                }
            }
        }
                ...
                if (AllProfiles.ContainsKey("HackableLockedCrate"))
                {
                    ...
                    if (prof.AutoSpawn == true && prof.Day_Time_Spawn_Amount > 0)
                    {
                        ...
                        timer.Repeat(1f, profile.Day_Time_Spawn_Amount, () =>
                        {
                            ...
                            SpawnBots("HackableLockedCrate", profile, "HackableLockedCrate", null, new Vector3(), -1, true);
                        });
                    }
                }
            });
        ...
        void OnEntitySpawned(NPCPlayerCorpse corpse)
        {
            ...
            timer.Once(0.1f, () =>
            {
                ...
                if (configData.Global.Create_Skulls)
                {
        ...
        void SelectWeapon(NPCPlayerApex npcPlayer)
        {
            ...
            else if (npcPlayer.AttackTarget == null)
            {
                ...
                ToggleAggro(npcPlayer, 0, bData.profile.Aggro_Range);
        ...
        {
            var bData = npcPlayer.GetComponent<BotData>();
            return (bData != null && (bData.inAir || bData.stationary)) ? true : (object)null;   
        }
        ...
        object OnNpcTarget(NPCPlayerApex npcPlayer, BaseEntity entity)
        {
            ...
            if (botVictim != null)
            {
                ...
                if (vicIsMine && !attackerIsMine && !configData.Global.NPCs_Attack_BotSpawn)  
                    return true;

                if (!attackerIsMine)
                    return null;

                if (npcPlayer.IsWounded())
                    return true;
                ...
                if (!vicIsMine && !configData.Global.BotSpawn_Attacks_NPCs)
                    return true;
            }
            ...
            if (victim != null)
            {
                ...
                {
                    var distance = Vector3.Distance(npcPlayer.transform.position, victim.transform.position);
                    if (npcPlayer.lastAttacker != victim && distance > npcPlayer.Stats.DeaggroRange)
                        return true;
            ...
            return ((entity.name.Contains("agents/") && !(entity is BasePlayer)) || (entity is HTNPlayer && configData.Global.Ignore_HTN)) ? true : (object)null;
        }

        object OnNpcTarget(BaseNpc npc, NPCPlayer npcPlayer)
        {
        ...
        void CheckMonuments(bool add) 
        {
            foreach (var monumentInfo in TerrainMeta.Path.Monuments.OrderBy(x => x.displayPhrase.english))
            {
                ...
                if (monumentInfo.name != null)
                {
                    if (monumentInfo.name.Contains("harbor_1"))  
                        displayPhrase = "Small " + displayPhrase;
                    if (monumentInfo.name.Contains("harbor_2"))
                        displayPhrase = "Large " + displayPhrase;
                }
        ...
        private void SetupProfiles()
        {
            ...
            {
                KitList.Add(profile.Key, profile.Value.Kit.ToList());
                NamesList.Add(profile.Key, profile.Value.BotNames.ToList());
                        ...
                        if (amount > 0)
                            timer.Repeat(0.5f, amount, () =>
                            {
                                if (AllProfiles.Contains(profile) && CanRespawn(profile.Key, 1, true) == 1)
                                {
                                    ...
                                    SpawnBots(profile.Key, AllProfiles[profile.Key], null, null, new Vector3(), point, true);
                                }
                            });
                    }
                }
        ...
        void AddProfile(string name, ConfigProfile monument, Vector3 pos)
        {
        ...
        private void botspawncmd(ConsoleSystem.Arg arg)
        {
            if (arg.Player() != null && !arg.Player().IsAdmin)
                return;

            if (arg.Args[arg.Args.Length - 1] == "True")
                arg.Args = arg.Args.SkipLast(arg.Args.Length - 1).ToArray();

            Botspawn(arg.Player(), "botspawn", arg.Args);
        }
        ...
        void Botspawn(BasePlayer player, string command, string[] args)
        {
            ...
            if (args != null && args.Length == 1)
            {
                ...
                {
                    foreach (var profile in AllProfiles)
                        ShowProfiles(player, profile.Value.Location, profile.Key, 10);
                    return;
                }
                if (args[0] == "info")
                {
            ...
            else if (args != null && args.Length == 2)
            {
                ...
                if (args[0] == "spawn")
                {
                    ...
                    foreach (var entry in AllProfiles.Where(entry => entry.Key.ToLower() == profile.ToLower()))
                    {
                        ...
                        {
                            Reply(player, TitleText + lang.GetMessage("norespawn", this));
                            continue;
                        }
                        ...
                        {
                            Reply(player, String.Format(lang.GetMessage("Title", this) + lang.GetMessage("targetzero", this), entry.Key));
                            return;
                        }
                        ...
                        Reply(player, String.Format(TitleText + lang.GetMessage("deployed", this), entry.Key, entry.Value.Location));
                        return;
                    }
                    Reply(player, TitleText + lang.GetMessage("noprofile", this));
                    return;
                }
                if (args[0] == "kill")
                {
                    ...
                    NextTick(() =>
                    {
                        if (found)
                        {
                            Reply(player, String.Format(TitleText + lang.GetMessage("killed", this), profile));
                            foreach (var npc in killList.ToList())
                                npc.Kill();
                        }
                        else
                            Reply(player, TitleText + lang.GetMessage("nonpcs", this));
                    });
                    return;
                }
                ...
                if (args[0] == "removespawn")
                {
                    ...
                    SendReply(player, TitleText + lang.GetMessage("notthatmany", this), pn, num);
                    return;
                }
                if (args[0] == "movespawn")
                {
            ...
            else if (args != null && args.Length == 3)
            {
                if (args[0] == "toplayer")
                {
                    ...
                    Vector3 location = CalculateGroundPos(target.transform.position);
                    var found = false;
                    if (target == null)
                    {
                        Reply(player, String.Format(TitleText + lang.GetMessage("namenotfound", this), name));
                        return;
                    }
                    foreach (var entry in AllProfiles.Where(entry => entry.Key.ToLower() == profile.ToLower()))
                    {
                        if (TargetAmount(AllProfiles[entry.Key]) == 0)
                        {
                            Reply(player, String.Format(TitleText + lang.GetMessage("targetzero", this), entry.Key));
                            return;
                        }
                        timer.Repeat(1f, TargetAmount(AllProfiles[entry.Key]), () => DeployNpcs(location, entry.Key, entry.Value, null, -1));
                        Reply(player, String.Format(TitleText + lang.GetMessage("deployed", this), entry.Key, target.displayName));
                        found = true;
                        return;
                    }
                    if (!found)
                ...
                Reply(player, TitleText + lang.GetMessage("error", this));
            }
            else
                Reply(player, TitleText + lang.GetMessage("error", this));
        }
        ...
        public List<ulong> DeadNPCPlayerIds = new List<ulong>();
        ...
        public class BotData : MonoBehaviour
        {
            ...
            public string monumentName, group;
            ...
            void Roam() 
            {
                if (Time.time - npc.lastAttackedTime > 15 && npc.Stats.Hostility == 1) 
                {
                    botSpawn.ToggleAggro(npc, 0, profile.Aggro_Range); 
                    botSpawn.WipeMemory(npc);
                }

                if (npc.AttackTarget == null && npc.GetNavAgent.isOnNavMesh)
                {
        ...
        public void ToggleAggro(NPCPlayerApex npcPlayer, int hostility, float distance)
        {
            ...
            if (bData != null)
            {
                ...
                npcPlayer.Stats.DeaggroRange = distance + bData.profile.DeAggro_Range - bData.profile.Aggro_Range;
        ...
        public class Global
        {
            ...
            public bool Suicide_Boom = true, Create_Skulls = true, Kits_In_Order = false, PeaceKeepers_Ignore_Melee = false, NPCs_Attack_BotSpawn = true, HTNs_Attack_BotSpawn, BotSpawn_Attacks_NPCs = true, APC_Safe = true, Turret_Safe = true, Animal_Safe = true, Supply_Enabled, Staggered_Despawn = false;
        ...
        public class BaseProfile
        {
            ...
            [JsonProperty(Order = 41)] public double RustRewardsValue = 0.0;
        }
        ...
        private bool IsBotSpawnNPC(ulong id) => NPCPlayers.ContainsKey(id);
