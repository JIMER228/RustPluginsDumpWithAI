    ...
    class AdminMenu : RustPlugin
    {
        ...
        private void CreateMenuPermissions(BasePlayer player, int page = 0, string filter = "")
        {
            ...
            for (int i = page * 72; i < permList.Count; i++)
            {
                ...
                float[] position = CalculateElementPos(count);
        ...
        private void CreateGiveMenu(ref CuiElementContainer container, ItemType itemType, int page, string playerId)
        {
            ...
            for (int y = itemIndex; y < length; y++)
            {
                ...
                float[] position = CalculateElementPos(i);
        ...
        /*
        Hey it's ya boy Salsa. I just compressed the methods CalculateButtonPos, CalculateButtonPosVert,
        and CalculateItemPos, consisting of about 260 lines of code total, into a single method: CalculateElementPos,
        which contains 4 lines of code. Your welcome.
        */

        private float[] CalculateElementPos(int number)
        {
            Vector2 dimensions = new Vector2(0.158f, 0.06f);
            Vector2 posMin = (new Vector2(0.014f, 0.78f)) +
                             (new Vector2((0.005f + dimensions.x) * (number % 6),
                                         (-0.007f - dimensions.y) * (number / 6)));
