...
namespace Oxide.Plugins
{
    ...
    class AliasSystem : RustPlugin
    {
		...
		{	
            ...
            foreach(Alias current in data.aliases)
            {
		...
		void ChatAlias(BasePlayer player, string command, string[] args)
		{
			...
			{
				SendChatMessage(player, "Command Alias", "You do not have permission to use this command.");
				return;
			}
		...
		void cmdAlias(BasePlayer player, string command, string[] args)
		{
			...
			switch(args[0])
			{
				case "add":
					...
					data.aliases.Add(new Alias(args[1], args[2]));
					SaveData();
				...
				case "remove":
					...
					else SendChatMessage(player, "Command Alias", $"Could not find alias \"{args[1]}\"");
