...
namespace Oxide.Plugins
{
	...
	public class Kits : RustPlugin
	{
		...
		private class Configuration
		{
			...
			[JsonProperty(PropertyName = "Show Display Name?")]
			...
			[JsonProperty(PropertyName = "Self-loading images")]
			...
			bool First = false)
		{
			...
			if (allKits.Count == 0)
				...
				for (var i = 0; i < kitsList.Count; i++)
				{
					...
					if (_config.ImageSettings.Enabled && !string.IsNullOrEmpty(kit.Image))
						container.Add(new CuiElement
						{
							...
							Components =
							{
								...
								{
									AnchorMin = _config.ImageSettings.AnchorMin,
									AnchorMax = _config.ImageSettings.AnchorMax,
									OffsetMin = _config.ImageSettings.OffsetMin,
									OffsetMax = _config.ImageSettings.OffsetMax
								}
							}
						});
					...
					container.Add(new CuiButton
					{
						...
						{
							AnchorMin = "0 1", AnchorMax = "0 1",
							OffsetMin = $"0 -{_config.UI.KitHeight}", OffsetMax = $"105 -{_config.UI.KitHeight - 25}"
						},
						Text =
						{
		...
		private void RefreshKitUi(ref CuiElementContainer container, BasePlayer player, Kit kit)
		{
			...
			container.Add(new CuiPanel
			{
				RectTransform = { AnchorMin = "0 1", AnchorMax = "1 1", OffsetMin = "0 -135", OffsetMax = "0 0" },
				Image = { Color = "0 0 0 0" }
			...
			if (kit.Amount > 0)
			{
				...
				var width = kit.Amount == 1 ? totalWidth : totalWidth / kit.Amount * 0.9f;

				var margin = (totalWidth - width * kit.Amount) / (kit.Amount - 1);

				var xSwitch = -(totalWidth / 2f);
				...
				for (var i = 0; i < kit.Amount; i++)
				{
					container.Add(new CuiPanel
					{
						...
						{
							AnchorMin = "0.5 1", AnchorMax = "0.5 1",
							OffsetMin = $"{xSwitch} -125",
							OffsetMax = $"{xSwitch + width} -120"
						},
						Image =
						{
