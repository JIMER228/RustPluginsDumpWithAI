...
namespace Oxide.Plugins
{
    ...
    class ExtraSeating : RustPlugin
    {
        ...
        public class Data { public Dictionary<ulong, bool> preferences = new Dictionary<ulong, bool>(); }

        #endregion
        private void Init()
        {
            config = Config.ReadObject<PluginConfig>();
            data = Interface.Oxide.DataFileSystem.ReadObject<Data>("ExtraSeating_Data");
            if (data == null)
            {
                data = new Data();
                Interface.Oxide.DataFileSystem.WriteObject("ExtraSeating_Data", data);
            }
        }
        ...
        void OnEntitySpawned(BaseNetworkable entity)
        {
            ...
            if (!data.preferences.ContainsKey(vehicle.OwnerID))
            {
                data.preferences.Add(vehicle.OwnerID, false);
                return;
            }
            else if (!data.preferences[vehicle.OwnerID]) return;
        ...
        [ChatCommand("es")]
        private void cmdES(BasePlayer player, string command, string[] args)
        {
            if (!data.preferences.ContainsKey(player.userID))
                data.preferences.Add(player.userID, false);
            data.preferences[player.userID] = !data.preferences[player.userID];
            SendChatMessage(player, "ExtraSeating", "Extra seating toggled " + (data.preferences[player.userID] ? "on" : "off"));
            Interface.Oxide.DataFileSystem.WriteObject("ExtraSeating_Data", data);
            return;
        }
