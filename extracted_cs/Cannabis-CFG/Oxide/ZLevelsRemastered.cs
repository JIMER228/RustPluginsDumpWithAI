...
{
    [Info("ZLevelsRemastered", "Default", "2.9.25")]
    ...
    class ZLevelsRemastered : RustPlugin
    {
        ...
        void LoadVariables()
        {
            ...
            blacklisteditems = (List<string>)GetConfig("Functions", "blacklistedItems", new List<string>());
        ...
        void OnPlayerConnected(BasePlayer player)
        {
            ...
            CreateGUI(player);
            /*
            long multiplier = 100; //TODO Rework and make this work and push an update
        ...
        {
            //var items = blacklistedItems.Keys.ToList();
            //Puts($"{items.ToString()}");
            //items.Contains("skull.human");
            

            if (!initialized || !enableDispenserGather || entity == null || !(entity is BasePlayer) || item == null || dispenser == null /*|| blacklistedItems.Contains(item.name.ToString())*/)
                return;
        ...
        {
            if (!initialized || !enableCollectiblePickup || item == null || player == null || !hasRights(player.UserIDString) /*|| blacklistedItems.Contains(item.name.ToString())*/)
                return;
            CheckPlayer(player);
            if (!playerPrefs.PlayerInfo[player.userID].ONOFF)
                return;
