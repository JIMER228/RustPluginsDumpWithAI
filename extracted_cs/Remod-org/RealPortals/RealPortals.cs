...
{
    [Info("RealPortals", "RFC1920", "1.0.7")]
    ...
    internal class RealPortals : RustPlugin
    {
        ...
        private void OnPlayerDisconnected(BasePlayer player)
        {
            ...
            Item foundInBelt = player.inventory.containerBelt.FindItemByItemID(1803831286);
            foundInBelt?.GetHeldEntity()?.Kill();
            foundInBelt?.DoRemove();
            player.inventory.containerBelt.MarkDirty();

            Item foundInMain = player.inventory.containerMain.FindItemByItemID(1803831286);
        ...
        private void cmdPortal(IPlayer iplayer, string command, string[] args)
        {
            ...
            if (args.Length == 1 && args[0] == "tool")
            {
                ...
                if (player.inventory.containerBelt.FindItemByItemID(1803831286) != null)
                {
                    Message(iplayer, "alreadyin", "belt");
                    return;
                }
                if (player.inventory.containerMain.FindItemByItemID(1803831286) != null)
                {
        ...
        private void FindSnapPosition(Vector3 position, Quaternion rotation, out Vector3 newpos, out Quaternion newrot)
        {
            newpos = default(Vector3);
            newrot = default(Quaternion);

            List<BaseEntity> list = new List<BaseEntity>();
            Vis.Entities(position, 3f, list);
            foreach (BaseEntity ent in list)
            {
                Puts(ent.ShortPrefabName);
            }
            newpos = position; newrot = rotation;
        }
        ...
        private void SpawnPortal(BasePlayer player)
        {
            ...
            Quaternion rotation = player.transform.rotation;// GetNetworkRotation();
            ...
            if (true)
            {
                Vector3 newpos;
                Quaternion newrot;
                FindSnapPosition(pos, rotation, out newpos, out newrot);
                pos = newpos;
                rotation = newrot;
            }

            BaseEntity newPortal = GameManager.server.CreateEntity(PortalPrefab, pos, rotation, true);
        ...
        {
            portals = Interface.GetMod().DataFileSystem.ReadObject<Dictionary<int, PortalPair>>(Name + "/portals");
        }

        private void SaveData()
        {
            Interface.GetMod().DataFileSystem.WriteObject(Name + "/portals", portals);
        }
        #endregion data
