...
{
    [Info("Plagued", "Psi|ocybin/RFC1920", "0.3.9", ResourceId = 1991)]
    ...
    internal class Plagued : RustPlugin
    {
        ...
        {
            private static readonly Core.SQLite.Libraries.SQLite sqlite = Interface.GetMod().GetLibrary<Core.SQLite.Libraries.SQLite>();
            ...
            private Dictionary<ulong, Association> associations = new();
            private Dictionary<ulong, Kin> kins = new();
            private List<ulong> kinRequests = new();
            ...
            {
                player = newPlayer;
                Interface.GetMod().LogInfo("Loading player: " + player.displayName);

                Core.Database.Sql sql = new();
                sql.Append(InsertPlayer, player.userID, player.displayName);
                sqlite.Insert(sql, sqlConnection, create_results =>
                {
                    if (create_results == 1)
                    {
                        Interface.GetMod().LogInfo("New user created!");
                    }
                    ...
                    sqlite.Query(sql, sqlConnection, results =>
                    {
                        ...
                        {
                            Interface.GetMod().LogInfo("Something wrong has happened: Could not find the player with the given user_id!");
                        }
            ...
            {
                List<string> associatesList = new();
        ...
        public class ProximityDetector : MonoBehaviour
        {
            ...
            private void NotifyPlayerProximity(List<BasePlayer> players) => Interface.GetMod().CallHook("OnPlayerProximity", player, players);
            private void NotifyPlayerAlone() => Interface.GetMod().CallHook("OnPlayerAlone", player);
        }
        #endregion
