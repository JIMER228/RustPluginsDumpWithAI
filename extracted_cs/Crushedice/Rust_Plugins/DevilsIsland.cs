...
{
    [Info("Devil's Island", "Nick Holmes/Default", "0.7.6", ResourceId = 1372)]
    ...
    public class DevilsIsland : GameModePlugin<DevilsIslandConfig, DevilsIslandState>
    {
        ...
        void RulesCommand(BasePlayer player, string command, string[] args)
        {
			...
			Text.Synopsis+ "\n"+
			"\n"+
			Text.PlayerCommandSection+"\n"+
			Text.StatusCommandHint+"\n"+
			Text.ClaimCommandHint+"\n"+
			Text.RebelCommandHint+"\n"+
			Text.WhereCommandHint+"\n"+
			"\n"+
			Text.BossCommandSection+"\n"+
			Text.TaxCommandHint+"\n"+
			Text.LootCommandHint+"\n"+
			Text.RecruitCommandHint+"\n"+
			Text.heliStrikeCommandHint+ GameConfig.heliStrikePrice_Quantity.ToString()+ GameConfig.heliStrikePrice_ItemDef.displayName.english+"\n"+
			"\n"+
			"\n"+
			"Outlaw Commands: \n"+
			"<color=lime>/decoy <i>player</i></color> For a few minutes, the Boss sees <i>player</i>'s location, instead of yours. Costs {0} {1}"+ GameConfig.DecoyPrice_Quantity+ GameConfig.heliStrikePrice_ItemDef.displayName.english;
			
            PrintToChat(player, RulesText);
        }
        ...
        void HeliCommmand(BasePlayer player, string command, string[] args)
        {
		   ...
		   //--- Assign the Target as BasePlayer to Global Value---//
		   HeliTarget = FindPlayer(args[0]);
        ...
        void WhereCommand(BasePlayer player, string command, string[] args)
        {
			...
			if(GameConfig.BossWaypointEnabled)
				DrawBossLocation(player,BossLoc,15f);
        }
		...
		private void OnEntitySpawned(BaseNetworkable entity)
        {
            ...
            {
				//---Get the BaseEnt. of this heli, and check his ownerid---///
				var Bas = entity as BaseEntity;
				Puts(Convert.ToString(Bas.OwnerID));

				if(Bas.OwnerID == 1337)
				{
					Puts("ACTIVEHELLI FOUND");
				...
				//AI._targetList = new List<PatrolHelicopterAI.targetinfo>();
				//AI._targetList.First().ply = target;
		...
		private void HeliKillHandle(BaseCombatEntity victimEntity, HitInfo hitInfo)
		{
			...
			var KillerEntity = victimEntity.lastAttacker ?? hitInfo?.Initiator as BaseNetworkable;
			
			var player = victimEntity.ToPlayer() ?? null;
			var prefabname = KillerEntity?.ShortPrefabName ?? string.Empty;
            var longprefabname = KillerEntity?.PrefabName ?? string.Empty;
            if (string.IsNullOrEmpty(prefabname) || string.IsNullOrEmpty(longprefabname)) return;
			
			if (prefabname.Contains("patrolhelicopter") && !prefabname.Contains("gibs"))
            {
				var baseent = KillerEntity as BaseEntity;
				Puts(Convert.ToString(baseent.OwnerID));

				if(baseent.OwnerID == 1337)
				{
					if( player == HeliTarget)
					{
						
						//Again getting the baseheli for future use
					var baseheli = KillerEntity.GetComponent<BaseHelicopter>();
					
					PrintToChat("The Heli did his Job and vanished from the surface");
					//---Job done, Target killed, get rid of the heli.
					KillerEntity.Kill();
					HeliTarget = null;
					}
				}
        ...
        void OnEntityDeath(BaseCombatEntity entity, HitInfo info)
        {
            ...
            if (State.IsBoss(victim))
            {
                ...
                {
                    PrintToChat("Boss {0} died foolishly. You can /claimboss the title.", State.BossName);
                    State.SetBoss(null);
					State.TaxRate = 0.10f;
                }
                return;
        ...
        void OnPlayerDisconnected(BasePlayer player, string reason)
        {
            ...
            {
                PrintToChat("The Boss has been sacked for sleeping on the job, feel free to /claimboss it");
                State.SetBoss(null);
            }
        }
		...
		private static BasePlayer FindPlayer(string nameOrId)
        {
            foreach (var activePlayer in BasePlayer.activePlayerList)
            {
                if (activePlayer.UserIDString == nameOrId)
                    return activePlayer;
                if (activePlayer.displayName.Contains(nameOrId))
                    return activePlayer;
		...
		void DrawBossLocation(BasePlayer player, Vector3 BossPos, float time)
        {
            var color = new ExtColor(1, 0.1, 0.1, 0.995);
			float arrowHeight = 100f;
			float arrowLevitation = 35f;
			float arrowHeadSize = 8;

            
            player.SendConsoleCommand("ddraw.arrow", time, color, new Vector3(BossPos.x, BossPos.y + arrowHeight + arrowLevitation, BossPos.z), new Vector3(BossPos.x, BossPos.y + arrowLevitation, BossPos.z), arrowHeadSize);
		...
		class ExtColor
        {
            ...
            public ExtColor(double red, double green, double blue, double alpha)
            {
                this.red = red;
                this.green = green;
                this.blue = blue;
...
{    
    public class DevilsIslandState : IGameState
    {
		...
		private static PatrolHelicopterAI InstanceAI;
        ...
        public void OrderheliStrike(string targetPartialName)
        {
            ...
            Outlaw target;
			var targets = FindPlayer(targetPartialName);
			var targetent = targets as BaseCombatEntity;
            
			//--- Giving the entity an Ownerid to identify THIS heli later---///
			var ent = GameManager.server.CreateEntity("assets/prefabs/npc/patrol helicopter/patrolhelicopter.prefab", new Vector3(), new Quaternion(), true);
            ent.OwnerID = 1337;
			...
			if (ent != null && Outlaws.TryResolveByPartialName(targetPartialName, out target))
            {
				...
				var Base = ent.GetComponent<BaseHelicopter>();
                AI.SetInitialDestination(targets.transform.position + new Vector3(0.0f, 10f, 0.0f), 0.25f);
				ent.Spawn();
					
				Base.spotlightTarget = targets.transform.position ;
					//AI.moveSpeed = 30f;
					//Base.engineSpeed = 100f;
					//AI.throttleSpeed = 20f;
					//AI.maxSpeed = 25f;
					AI.rightGun._target = targetent;
					AI.leftGun._target = targetent;
					AI.rightGun.fireRate = 0.005f;
				
					Base.SendNetworkUpdateImmediate(true);
    ...
    public class DevilsIslandConfig : IGameConfig
    {
        ...
        public void UpdateConfigFile()
        {
			...
			changed |= DefaultValue("BossPositionWaypoint", false);
		...
		public bool BossWaypointEnabled { get { return (bool)configFile["BossPositionWaypoint"]; } }
    ...
    {
        public const string NoBoss = "There is no Boss - you can <color=lime>/claimboss</color> it\n";
        ...
        public const string ClaimCommandHint = "<color=lime>/claimboss</color> if there is no Boss, you can take the job";
        ...
        public const string Broadcast_ClaimAvailable = "No one is the Boss, use <color=lime>/claimboss</color> to become the Boss.";
        ...
        public const string Error_Loot_NoBoss = "If you want the loot, use <color=lime>/claimboss</color> to become the Boss.";
        ...
        public const string Error_TaxChange_NoBoss = "If you want to change the tax rate, use <color=lime>/claimboss</color> to become the Boss";
        ...
        public const string Broadcast_BossSuicide = "Boss {0} could not take it any more. Use <color=lime>/claimboss</color> to become the new Boss";
