...
{
    [Info("Event Manager", "Reneb / k1lly0u", "3.0.6", ResourceId = 740)]
    class EventManager : RustPlugin
    {
        ...
        //{
        //    if (player == null || player.children == null || player.children.Count == 0) return;
        ...
        List<BasePlayer> unnetworkedPlayers = new List<BasePlayer>();
        ...
        void RemoveFromNetwork(BasePlayer player)
        {
            if (!unnetworkedPlayers.Contains(player))
            {
        ...
        void AddToNetwork(BasePlayer player)
        {
            if (unnetworkedPlayers.Contains(player))
            {
        ...
        class RestorationManager
        {
            ...
            public bool ReadyToRestore(BasePlayer player)
            {
                ...
                if (ins.unnetworkedPlayers.Contains(player))
                    ins.AddToNetwork(player);
