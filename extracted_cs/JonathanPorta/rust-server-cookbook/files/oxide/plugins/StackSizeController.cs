...
*** Fixed the compile error. ItemManager is no longer a singleton.
*** Removed the item suggest when using /stack until I can fix it later. Now it will just give you an error if the shortname is invalid.
******************************************************************************/
...
{
    [Info("Stack Size Controller", "Waizujin", 1.6, ResourceId = 1185)]
    ...
    public class StackSizeController : RustPlugin
    {
        ...
        private void StackCommand(BasePlayer player, string command, string[] args)
        {
            ...
            List<ItemDefinition> items = ItemManager.itemList.FindAll(x => x.shortname.Contains(args[0]));
