...
{
    [Info("Kill Awards", "VisEntities", "1.3.0")]
    ...
    public class KillAwards : RustPlugin
    {
        ...
        [PluginReference]
        private readonly Plugin GearCore, Economics, ServerRewards, XPerience;
        ...
        private class RewardConfig
        {
            ...
            [JsonProperty("XP Rewarded (XPerience)")]
            public double XpRewarded { get; set; }
        ...
        private void Award(BasePlayer player)
        {
            ...
            if (_config.KillMilestones.TryGetValue(playerData.Kills, out RewardConfig reward))
            {
                ...
                if (!string.IsNullOrEmpty(reward.GearSetToEquip) && GearCoreUtil.EquipGearSet(player, reward.GearSetToEquip, clearInventory: false))
                {
                ...
                if (reward.XpRewarded > 0 && PluginLoaded(XPerience))
                {
                    XPerienceUtil.GiveXp(player, reward.XpRewarded);
                    MessagePlayer(player, Lang.XpRewarded, reward.XpRewarded);
                }
                ...
                {
                    string personalMsg = ReplacePlaceholders(reward.PersonalMessage, player, playerData.Kills);
                    MessagePlayer(player, personalMsg);
                }

                if (!string.IsNullOrEmpty(reward.GlobalMessage))
                {
                    string globalMsg = ReplacePlaceholders(reward.GlobalMessage, player, playerData.Kills);
        ...
        private string ReplacePlaceholders(string text, BasePlayer player, int killCount)
        {
            return text
        ...
        public static class XPerienceUtil
        {
            public static void GiveXp(BasePlayer player, double amount)
            {
                if (!PluginLoaded(_plugin.XPerience))
                    return;

                if (player == null || amount <= 0)
                    return;

                _plugin.XPerience.Call("GiveXP", player, amount);
            }
        }
        ...
        private class Lang
        {
            ...
            public const string XpRewarded = "XpRewarded";
        }
        ...
        protected override void LoadDefaultMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                [Lang.XpRewarded] = "You have been awarded {0} XP!",
            }, this, "en");
        }
