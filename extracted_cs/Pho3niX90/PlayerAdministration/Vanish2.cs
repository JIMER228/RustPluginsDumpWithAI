...
namespace Oxide.Plugins {
    ...
    public class Vanish2 : RustPlugin {
        ...
        public class Configuration {
            [JsonProperty("Autmatically update permissions from old versions of vanish (only needs to be run once)")]
        ...
        void OnEnterZone(string ZoneID, BasePlayer player) {
            if (!(player is HumanNPC) && !(player is HTNPlayer) && !(player is NPCPlayer)) {
                if (ZoneID.Equals("47039943") || ZoneID.Equals("47240925")) {
                    Puts($"Dissapearing {player.displayName}");
                    Disappear(player);
                } else {
                    Puts($"Reappearing {player.displayName}");
                    Reappear(player);
                }
            }
        }
        ...
        private void Disappear(BasePlayer player) {
            ...
            player.impactEffect = new GameObjectRef();
