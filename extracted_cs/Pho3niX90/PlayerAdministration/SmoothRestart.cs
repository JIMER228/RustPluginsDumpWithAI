...
namespace Oxide.Plugins {
    [Info("SmoothRestart", "Pho3niX90", "1.2.5", ResourceId = 1826)]
    public class SmoothRestart : RustPlugin {
        ...
        bool useWebhook;
        string webhookUrl = "http://197.189.212.202/ogp_api.php?gamemanager/restart&token=a1e7e3c78e77860774c8c679ec21bf56310d2a289e26bcbc8ac02aa4111384c2&ip=197.189.212.202&port=27015&mod_key=default";
        string discordAnnounce = "https://discordapp.com/api/webhooks/678875289157107715/TJl1SEUfAd12r51VmbH_Q2LQbPltFj5w-2p6EUbPxb9dWgkY-RukGcXS9rvi9CeqPobe";
        ...
        void LoadVariables() {
            ...
            useWebhook = Convert.ToBoolean(GetConfig("WebhookApi", "useWebhook", false));
            webhookUrl = GetConfig("WebhookApi", "webhookUrl", "").ToString();
            ...
            lang.RegisterMessages(new Dictionary<string, string>
                                  {
                                    ...
                                    {"RestartWipeInit", "<color=orange>Server will wipe in <color=red>{0}</color> minutes!</color>"},
                                    ...
                                    {"RestartWipeInitSec", "<color=orange>Server will wipe in <color=red>{0}</color> seconds!</color>"},
        ...
        void SaveServer() {
            rust.RunServerCommand("server.save", new object[0]);
            rust.RunServerCommand("server.writecfg", new object[0]);
        }
        ...
        void CheckDevBlog(int countNews) {
            ...
            try { webrequest.Enqueue(url, null, (code, response) => APIResponse(code, response, "devblog", countNews), this, RequestMethod.GET); } catch { timer.Once(60f, () => CheckDevBlog(4)); }
        }
        ...
        void CheckOxideCommits() {
            ...
            userAgent.Add("User-Agent", "OxideMod");
            try { webrequest.Enqueue(url, null, (code, response) => APIResponse(code, response, "oxide", 0), this, RequestMethod.GET, userAgent); } catch { timer.Once(60f, CheckOxideCommits); }
        }

        void CallRestartWebhook(BasePlayer player, bool onlyGetUncached = false, bool connectedTime = true) {

            if (!useWebhook || !webhookUrl.Equals("")) {
                Puts("There was an error calling the webhook, please check if the url is valid.");
            }

            Puts($"Calling webhook {webhookUrl}");
            webrequest.Enqueue(webhookUrl, null, (code, response) => {
                if (code != 200 || response == null) {
                    Puts($"Couldn't get an answer from webhook to restart!");
                    return;
                }
                Puts("Webhook restarted successfully.");
        ...
        [ConsoleCommand("sr.wipe")]
        void smoothRestartConsoleCommandWipe(ConsoleSystem.Arg arg) {
            if (arg.Connection != null && arg.Connection.authLevel < 2) return;
            if (arg.Args != null && arg.Args.Length == 1) {
                if (arg.Args[0].ToLower() == "stop") {
                    if (countDownActive) {
        ...
        [ChatCommand("srsave")]
        void saveCmd(BasePlayer player, string command, string[] args) {
            if (player.IsAdmin) SaveServer();
        }
        ...
        void DoSmoothRestart(int timer, bool unitMinutes = true) {
            ...
            if (unitMinutes) {
                ...
                if (timer == 2 || timer == 5) {
                    SaveServer();
                    Puts("Saving from SmoothRestart");
                }

                if ((timer % 10 == 0 && timer != 0) || timer == 1 && timer != 0) {
                    if(isWipe) {
                        DiscordMessages?.Call("API_SendTextMessage", discordAnnounce, "Server will wipe in " + timer + " minutes");
                    } else {
                        DiscordMessages?.Call("API_SendTextMessage", discordAnnounce, "Server will restart in " + timer + " minutes");
                    }
                }

                if (patchRestartRunning)
                    msg = string.Format(lang.GetMessage(isWipe ? "RestartWipeInitPatch" : "RestartInitPatch", this), timer);
                else
                    msg = string.Format(lang.GetMessage(isWipe ? "RestartWipeInit" : "RestartInit", this), timer);
            } else
                msg = string.Format(lang.GetMessage(isWipe ? "RestartWipeInitSec" : "RestartInitSec", this), timer);
            Puts(StripTags(msg));
            if (timer == 1 && !unitMinutes) {
                DiscordMessages?.Call("API_SendTextMessage", discordAnnounce, isWipe ? $"@everyone {covalence.Server.Name} will now wipe, eta 5mins"  : $"@everyone {covalence.Server.Name} now restarting, eta 3mins");
            }
            if (SimpleUI_Enable)
                foreach (BasePlayer player in BasePlayer.activePlayerList) {
                    UIMessage(player, msg);
