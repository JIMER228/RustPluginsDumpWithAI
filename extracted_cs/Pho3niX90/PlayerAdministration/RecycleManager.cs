        ...
        private void RefreshIngredientList()
        {
            foreach (ItemDefinition itemInfo in ItemManager.itemList)
            {
                ...
                if (itemInfo.Blueprint.ingredients?.Count == 0)
                    continue;
                List<ItemInfo> x = itemInfo.Blueprint.ingredients?.Select(entry => new ItemInfo {itemAmount = (int) entry.amount / itemInfo.Blueprint.amountToCreate, itemName = entry.itemDef.shortname}).ToList();
