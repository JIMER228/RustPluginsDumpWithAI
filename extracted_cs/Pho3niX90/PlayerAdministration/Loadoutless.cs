...
{
    [Info("Loadoutless", "Kechless", "1.3.3")]
    ...
    class Loadoutless : RustPlugin
    {
        ...
        const string permission_resetdefault = "loadoutless.resetdefault";
        //FILES
        ...
        private void Init() {
            ...
            permission.RegisterPermission(permission_resetdefault, this);
        }
        ...
        public void resetPlayersLoadout() {
            PlayerLoadout currentUser;
            string[] raw_files = Interface.Oxide.DataFileSystem.GetFiles(file_main);
            if (raw_files != null) {
                foreach (string item in raw_files) {
                    currentUser = JsonConvert.DeserializeObject<PlayerLoadout>(item);
                    currentUser.items = JsonConvert.DeserializeObject<List<LoadoutItem>>(myConfig.Default_Loadout);
                    // WORKING ON!
