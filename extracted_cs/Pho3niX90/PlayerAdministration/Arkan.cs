...
{
    [Info("Arkan", "Antidote", "1.0.18-DAPI")]
    [Description("Player shot analysis tool for Admins")]
    class Arkan : RustPlugin
    {
        [PluginReference]
        private Plugin DiscordApi, DiscordMessages;
        ...
        private void OnServerInitialized() {
            ...
            if ((_config.DiscordNRReportEnabled || _config.DiscordAIMReportEnabled || _config.DiscordIRReportEnabled) && (DiscordApi == null && DiscordMessages == null))
                            ...
                            if (_config.notifyPhysicsStepsWarning) {
                                if (aimvd.physicsSteps > _config.minPhysicsStepsAllowed) {
                                    if (_config.DiscordAIMReportEnabled)
                                        if (DiscordApi == null && DiscordMessages == null)
                                            PrintWarning(Lang("DiscordWarning2", null));
                                        else {
                                            ...
                                            if (DiscordApi.IsLoaded) {
                                                DiscordApi?.Call("API_SendEmbeddedMessage", _config.DiscordAIMWebhookURL, "Arkan: " + Lang("HighPhysicsStepsDetection", null), 39423, json);
                                            } else {
                            ...
                            if (_config.debugMessages) {
                                ...
                                if (_config.DiscordAIMReportEnabled)
                                    if (DiscordApi == null && DiscordMessages == null)
                                        ...
                                        string json = JsonConvert.SerializeObject(fieldsObject);
                                        if (DiscordApi.IsLoaded) {
                                            DiscordApi?.Call("API_SendEmbeddedMessage", _config.DiscordAIMWebhookURL, "Arkan: " + Lang("AIMDetection", null), 39423, json);
                                        } else {
                ...
                if (violationProbability > _config.weaponsConfig[fsd.weaponShortName].NRViolationProbability && (angleWithVectorUpSum / fsd.firedShots.Count) > 30f) {
                    ...
                    if (_config.DiscordNRReportEnabled)
                        if (DiscordApi == null && DiscordMessages == null)
                            ...
                            if (DiscordApi.IsLoaded) {
                                DiscordApi?.Call("API_SendEmbeddedMessage", _config.DiscordNRWebhookURL, "Arkan: " + Lang("NRDetection", null), 39423, json);
                            } else {
                    ...
                    if (PlayersViolations.Players[player.userID].inRockViolations[key].inRockViolationsData.Count == sCnt) {
                        ...
                        if (_config.DiscordIRReportEnabled)
                            if (DiscordApi == null && DiscordMessages == null)
                                ...
                                string json = JsonConvert.SerializeObject(fieldsObject);
                                if (DiscordApi.IsLoaded) {
                                    DiscordApi?.Call("API_SendEmbeddedMessage", _config.DiscordIRWebhookURL, "Arkan: " + Lang("IRDetection", null), 39423, json);
                                } else {
