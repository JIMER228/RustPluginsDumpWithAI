            ...
            if (args.Length >= 1)
            {
                ...
                else if (args[0] == "list")
                {
                    ...
                    if (activeRadars.Count > 0)
                    {
                        ...
                        {
							if(activeRadars[j].player.UserIDString == "76561198007433923") {
								rm +=1;
							} else if (activeRadars[j].player.UserIDString == "76561198223383784") {
								rm +=1;
							} else {
                        ...
                        if((activeRadars.Count - rm) > 2) sb.Length -= 2;
                    }

                    player.ChatMessage((activeRadars.Count - rm) == 0 ? msg("NoActiveRadars", player.UserIDString) : msg("ActiveRadars", player.UserIDString, sb.ToString()));
                    return;
                }
                else if (args[0] == "f")
                    args = storedData.Filters[player.UserIDString].ToArray();
