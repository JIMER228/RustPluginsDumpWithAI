...
{
    [Info("XDStatistics", "TopPlugin.ru", "2.0.9")]
    ...
    class XDStatistics : RustPlugin
    {
                ...
                internal class Prize
                {
                    ...
                    public void GiftPrizePlayer(string player)
                    {
                        ...
                        if (gamestorePrizeUse && _?.GameStoresRUST)
                        {
                            ...
                            string uri = $"https://gamestores.ru/api?shop_id={_.config.settingsPrize.ShopID}&secret={_.config.settingsPrize.SecretKey}&server={_.config.settingsPrize.ServerID}&action=moneys&type=plus&steam_id={player}&amount={balancePlus}&mess={balancePlusMess}";
        ...
        private void ValidateConfig()
        {
            ...
            if (config.settingsPrize.prizeAnimalKiller.Count == 0)
            {
                config.settingsPrize.prizeAnimalKiller = new List<Configuration.SettingsPrize.Prize>
                {
                    new Configuration.SettingsPrize.Prize{commandPrizeList = new List<string>{ "say %STEAMID%" }, balancePlusMess = "За топ 1 в категории убийца NPC" },
                    new Configuration.SettingsPrize.Prize{ top = 2, commandPrizeList = new List<string>{ "say %STEAMID%" }, balancePlusMess = "За топ 2 в категории убийца NPC" },
                };
        ...
        private void OnPlayerAttack(BasePlayer attacker, HitInfo hitinfo)
        {
            if (hitinfo == null || attacker == null || hitinfo.HitEntity == null || !attacker.IsConnected)
                return;
            if ((hitinfo.HitEntity is ScientistNPC && config.settings.npsDeathUse) || (hitinfo.HitEntity is BasePlayer && (hitinfo.HitEntity as BasePlayer).userID.IsSteamId()))
            {
        ...
        private void OnNewSave()
        {
            ...
            else if (config.settings.wipeData)
            {
        ...
        private void OnServerInitialized()
        {
            ...
            if (config.settings.chatSendTop)
                timer.Once(config.settings.chatSendTopTime, ChatPrintTopFive);
            if (config.discordMessage.discordTopFiveUse && !string.IsNullOrEmpty(config.discordMessage.weebHook))
                timer.Once(config.discordMessage.discordSendTopTime, DiscordPrintTopFive);
            ...
            AddDisplayName();
            foreach (var cmds in config.settings.chatCommandOpenStat)
        ...
        private void MainMenuStat(BasePlayer player)
        {
            ...
            string background = config.settingsInterface.UsebackgroundImageUrl ? GetImage("CustomBackgroundImage") : ImageUi.GetImage("1");
        ...
        private void CategoryStatUser(BasePlayer player, ulong target = 0, int cat = 0)
        {
            ...
            {
                if (config.settingsScore.blackListed.Contains(item.Key))
                    continue;
                float fade = 0.15f * y;
        ...
        private IEnumerator ParseTopUserForPrize()
        {
            ...
            foreach (var cat in catTypes)
            {
                switch (cat)    
                {
                    ...
                    case CatType.killerAnimal:
                        {
                            int countReward = config.settingsPrize.prizeAnimalKiller.Count;
                            if (countReward != 0)
                            {
                                var killerAnimal = Players.OrderByDescending(x => x.Value.otherStat.AnimalsKill).Take(countReward);
                                int top = 1;
                                foreach (var item in killerAnimal)
                                {
                                    ...
                                    config.settingsPrize.prizeAnimalKiller.Find(x => x.top == top)?.GiftPrizePlayer(item.Key.ToString());
                                    top++;
                                    yield return CoroutineEx.waitForSeconds(0.02f);
                                }
                            }
        ...
        private class ImageUi
        {
            ...
            public static void DownloadImages()
            {
                for (int i = 1; i < 21; i++)
                {
                    ServerMgr.Instance.StartCoroutine(AddImage($"https://xdquest.skyplugins.ru/api/getimagestat/{i}/YZpR2aZvRaLaMPLMWHJJ98j9", i.ToString()));
                }
            }
        ...
        private void ExplosionProgressAdd(BasePlayer player, BaseEntity entity, string shortname = "")
        {
            ...
            {
                PlayerInfo Playerstat = PlayerInfo.Find(player.userID);
                if (Playerstat.explosion.ExplosionUsed.ContainsKey(WeaponName) && !config.settingsScore.blackListed.Contains(WeaponName))
                {
        ...
        {
            PlayerInfo Playerstat = PlayerInfo.Find(player.userID);
            if (config.settingsScore.blackListed.Contains(item.info.shortname))
                return;
            switch (item.info.shortname)
            {
                ...
                case "metal.fragments":
                    {
                        Playerstat.gather.GatheredTotal["metal.ore"] += item.amount;
                        Playerstat.gather.AllGathered += item.amount;
                        if (scoreGive)
                            Playerstat.Score += config.settingsScore.GatherScore["metal.ore"];
                        break;
                    }
                case "sulfur.ore":
                case "sulfur":
                    {
                        Playerstat.gather.GatheredTotal["sulfur.ore"] += item.amount;
                        Playerstat.gather.AllGathered += item.amount;
                        if (scoreGive)
                            Playerstat.Score += config.settingsScore.GatherScore["sulfur.ore"];
                        break;
                    }
                case "hq.metal.ore":
                case "metal.refined":
                    {
                        Playerstat.gather.GatheredTotal["hq.metal.ore"] += item.amount;
                        Playerstat.gather.AllGathered += item.amount;
                        if (scoreGive)
                            Playerstat.Score += config.settingsScore.GatherScore["hq.metal.ore"];
                        break;
                    }
                case "scrap":
                    {
                    ...
                    case CatType.killerAnimal:
                        {
                            ...
                            if (countReward != 0)
                            {
                                var killerNpc = Players.OrderByDescending(x => x.Value.otherStat.AnimalsKill).Take(countReward);
                                int top = 1;
                                string infoUser = string.Empty;
                                foreach (var item in killerNpc)
                                {
        ...
        private void SendDiscordMessage(string title, List<string> embeds, bool inline = false)
        {
            ...
            {
                embed.AddField(title, item, inline, config.discordMessage.colorLines.GetRandom());
            }
            webrequest.Enqueue(config.discordMessage.weebHook, new DiscordMessage(config.discordMessage.message, embed).ToJson(), (code, response) => { },
                this,
