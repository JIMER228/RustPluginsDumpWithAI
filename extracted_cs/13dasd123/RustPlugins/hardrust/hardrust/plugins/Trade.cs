    ...
    [Info("Trade", "OxideBro", "1.1.41")] public class Trade: RustPlugin
    {
        ...
        private void OnShopCompleteTrade(ShopFront shop)
        {
            ...
            if (trade != null)
            {
                ...
                timer.Once(1.0f, () =>
                {
                    trade.player1.EndLooting();trade.player2.EndLooting();DrawUIPlayer(trade.player1);DrawUIPlayer(trade.player2);timer.Once(4f, () =>
                    {
                        DestroyUIPlayer(trade.player1);DestroyUIPlayer(trade.player2);
                    });
                });
            }
        }
		...
		void CmdChatTrade(BasePlayer player, string command, string[] args)
        {
            ...
            switch (args[0])
            {
                ...
                pendings[target] = player;Reply(player, "PENDING.SENDER.FORMAT", target.displayName);Reply(target, "PENDING.RECIEVER.FORMAT", player.displayName);mytimer = timer.Once(getTime, () =>
                {
                ...
                case "cancel":
                    ...
                    pendings.Remove(player);
                    if (player2 ? .IsConnected == true) Reply(player2, "PENDING.CANCEL.SENDER", player.displayName);Reply(player2, "TRADE.CANCELED");
                    return;
                case "no":
                        if (player == null) return;if (!pendings.TryGetValue(player, out player2))
                    {
        ...
        void DrawUIPlayer(BasePlayer player)
        {
            ...
            {
                Name = "ContainerUI", Parent = "Overlay", Components = {
            ...
            {
                Name = "PlayerName", Parent = "ContainerUI", Components = {
                    new CuiTextComponent
                    {
                        Text = $"<size=18>Trade успешно завершён!</size>", Align = TextAnchor.UpperCenter, FadeIn = 0.5f
                    },
                    new CuiRectTransformComponent
                    {
                        AnchorMin = "0.343 0.550", AnchorMax = "0.635 0.620"
                    },
        ...
        Dictionary < string, string > Messages = new Dictionary < string, string > ()
        {
            ...
            {
                "TRADE.HELP",
                "Trade by RustPlugin.ru\nИспользуйте комманду <color=ff01f0>/trade \"НИК\"</color> для обмена\nЧто бы принять обмен, введите: <color=ff01f0>/trade yes</color> (или /trade accept)\nЧто бы отказаться от обмена введите: <color=ff01f0>/trade no </color> (или /trade cancel)"
            },
            {
                "PLAYER.NOT.FOUND",
                "Игрок '{0}' не найден!"
