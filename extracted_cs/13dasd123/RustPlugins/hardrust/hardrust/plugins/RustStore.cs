    ...
    public class RustStore : RustPlugin
    {
        ...
        public class CQueueImageCache
        {
            ...
            private IEnumerator WaitForRequest(QueueItem queueItem)
            {
                ...
                var id = FileStorage.server.Store(bytes, FileStorage.Type.png, CommunityEntity.ServerInstance.net.ID);
                if (FileStorage.server.Get(id, FileStorage.Type.png, CommunityEntity.ServerInstance.net.ID) == null)
                {
                ...
                case EItemType.GameItem:
                    ...
                    RequestItemGive(userId, itemJson, () =>
                    {
                        Action<int> giveItemFunc = (giveAmount) =>
                        {
                            if (isBp)
                            {
                                var bp = ItemManager.Create(ItemManager.blueprintBaseDef, 1, 0UL);
