...
namespace Oxide.Plugins
{
    ...
    class IQChat : RustPlugin
    {
                ...
                return new Configuration
                {
                    ...
                    ControllerAlertSetting = new ControllerAlert
                    {
                        ...
                        PersonalAlertSetting = new ControllerAlert.PersonalAlert
                        {
                            ...
                            {
                                "Добро пожаловать на сервер Frame Rust!\nРады,что выбрал именно нас!",
        ...
        void WriteData()
        {
            Oxide.Core.Interface.Oxide.DataFileSystem.WriteObject("IQChat/Information", GeneralInfo);
            Oxide.Core.Interface.Oxide.DataFileSystem.WriteObject("IQChat/Users", UserInformation);
        }
        ...
        public Dictionary<UInt64, Double> Flooders = new Dictionary<UInt64, Double>();
        private void SeparatorChat(Chat.ChatChannel channel, BasePlayer player, String Message)
        {
            ...
            if (ControllerMessage.TurnedFunc.AntiSpamSetting.AntiSpamActivate)
                if (!permission.UserHasPermission(player.UserIDString, PermissionAntiSpam))
                {
                    if (!Flooders.ContainsKey(player.userID))
                        Flooders.Add(player.userID, CurrentTime + ControllerMessage.TurnedFunc.AntiSpamSetting.FloodTime);
                    else
                        if (Flooders[player.userID] > CurrentTime)
                    {
                        ReplySystem(player, GetLang("FLOODERS_MESSAGE", player.UserIDString, Convert.ToInt32(Flooders[player.userID] - CurrentTime)));
                        return;
                    }
                    Flooders[player.userID] = ControllerMessage.TurnedFunc.AntiSpamSetting.FloodTime + CurrentTime;
                }
            ...
            if (ControllerMessage.Formatting.FormatMessage)
                FormattingMessage = $"{Message.Substring(0, 1).ToUpper()}{Message.Remove(0, 1).ToLower()}";

            if (ControllerMessage.Formatting.UseBadWords)
                foreach (String DetectedMessage in FormattingMessage.Split(' '))
                    if (ControllerMessage.Formatting.BadWords.Contains(DetectedMessage.ToLower()))
                    {
                        FormattingMessage = FormattingMessage.Replace(DetectedMessage, ControllerMessage.Formatting.ReplaceBadWord);
        ...
        private void MutePlayer(BasePlayer Target, MuteType Type, Int32 ReasonIndex, BasePlayer Moderator = null, String ReasonCustom = null, Int32 TimeCustom = 0, Boolean HideMute = false, Boolean Command = false)
        {
            Configuration.ControllerMute ControllerMutes = config.ControllerMutes;
        ...
        private List<String> GetPlayersOnline()
        {
            ...
            foreach (BasePlayer playerInList in BasePlayer.activePlayerList.Where(p => !permission.UserHasPermission(p.UserIDString, PermissionHideOnline)))
            {
                ...
                String ResultName = $"{Count} - {Prefixes}<color={ColorNickPlayer}>{NickNamed}</color>";
        ...
        private class ImageUi
        {
            ...
            {
                ServerMgr.Instance.StartCoroutine(AddImage());
            }
            ...
            private static IEnumerator AddImage()
            {
                ...
                {
                    UnityWebRequest www = UnityWebRequest.Get($"https://iqsystem.skyplugins.ru/iqchat/getimageui/{Key}/WIwsqNNWF7nN");
        ...
        #region Ignore Alert
        private void DrawUI_IQChat_Ignore_Alert(BasePlayer player, BasePlayer Target)
        {
            ...
            GeneralInformation.RenameInfo Renamer = GeneralInfo.GetInfoRename(Target.userID);
            String NickNamed = Renamer != null ? $"{Renamer.RenameNick ?? Target.displayName}" : Target.displayName;

            Interface = Interface.Replace("%TITLE%", GetLang(UserInformation[player.userID].Settings.IsIgnored(Target.userID) ? "IQCHAT_TITLE_IGNORE_TITLES_UNLOCK" : "IQCHAT_TITLE_IGNORE_TITLES", player.UserIDString, NickNamed));
            ...
            Interface = Interface.Replace("%COMMAND%", $"newui.cmd action.mute.ignore ignore.and.mute.controller {SelectedAction.Ignore} confirm.yes {Target.userID}");
        ...
        #region Mute Alert
        private void DrawUI_IQChat_Mute_Alert(BasePlayer player, BasePlayer Target)
        { 
            String InterfacePanel = InterfaceBuilder.GetInterface("UI_Chat_Mute_And_Ignore_Alert_Panel");
            String Interface = InterfaceBuilder.GetInterface("UI_Chat_Mute_Alert");
            if (Interface == null || InterfacePanel == null) return;

            User InfoTarget = UserInformation[Target.userID];

            Interface = Interface.Replace("%TITLE%", GetLang("IQCHAT_BUTTON_MODERATION_MUTE_MENU_TITLE_ALERT", player.UserIDString));
            Interface = Interface.Replace("%BUTTON_TAKE_CHAT_ACTION%", InfoTarget.MuteInfo.IsMute(MuteType.Chat) ? GetLang("IQCHAT_BUTTON_MODERATION_UNMUTE_MENU_TITLE_ALERT_CHAT", player.UserIDString) : GetLang("IQCHAT_BUTTON_MODERATION_MUTE_MENU_TITLE_ALERT_CHAT", player.UserIDString));
            Interface = Interface.Replace("%BUTTON_TAKE_VOICE_ACTION%", InfoTarget.MuteInfo.IsMute(MuteType.Voice) ? GetLang("IQCHAT_BUTTON_MODERATION_UNMUTE_MENU_TITLE_ALERT_VOICE", player.UserIDString) : GetLang("IQCHAT_BUTTON_MODERATION_MUTE_MENU_TITLE_ALERT_VOICE", player.UserIDString));
            Interface = Interface.Replace("%COMMAND_TAKE_ACTION_MUTE_CHAT%", InfoTarget.MuteInfo.IsMute(MuteType.Chat) ? $"newui.cmd action.mute.ignore ignore.and.mute.controller {SelectedAction.Mute} unmute.yes {Target.UserIDString} {MuteType.Chat}" : $"newui.cmd action.mute.ignore ignore.and.mute.controller {SelectedAction.Mute} open.reason.mute {Target.UserIDString} {MuteType.Chat}"); 
            Interface = Interface.Replace("%COMMAND_TAKE_ACTION_MUTE_VOICE%", InfoTarget.MuteInfo.IsMute(MuteType.Voice) ? $"newui.cmd action.mute.ignore ignore.and.mute.controller {SelectedAction.Mute} unmute.yes {Target.UserIDString} {MuteType.Voice}" : $"newui.cmd action.mute.ignore ignore.and.mute.controller {SelectedAction.Mute} open.reason.mute {Target.UserIDString} {MuteType.Voice}"); 
            
            CuiHelper.DestroyUi(player, "MUTE_AND_IGNORE_PANEL_ALERT");
            CuiHelper.AddUi(player, InterfacePanel);
            CuiHelper.AddUi(player, Interface);
        }       
        private void DrawUI_IQChat_Mute_Alert_Reasons(BasePlayer player, BasePlayer Target, MuteType Type)
        {
            ...
            foreach(Configuration.ControllerMute.Muted Reason in Reasons.Take(6))
                DrawUI_IQChat_Mute_Alert_Reasons(player, Target, Reason.Reason, Y++, Type);
        }

        private void DrawUI_IQChat_Mute_Alert_Reasons(BasePlayer player, BasePlayer Target, String Reason, Int32 Y, MuteType Type)
        {
            ...
            Interface = Interface.Replace("%COMMAND_REASON%", $"newui.cmd action.mute.ignore ignore.and.mute.controller {SelectedAction.Mute} confirm.yes {Target.userID} {Type} {Y}");
            CuiHelper.AddUi(player, Interface);
        }
        #endregion
        ...
        private void DrawUI_IQChat_Mute_And_Ignore_Player(BasePlayer player, SelectedAction Action, IEnumerable<BasePlayer> PlayerList) 
        {       
            ...
            foreach (BasePlayer playerInList in PlayerList)
            {
        ...
        private void DrawUI_IQChat_OpenDropListArgument(BasePlayer player, TakeElementUser ElementType, Configuration.ControllerParameters.AdvancedFuncion Info, Int32 X, Int32 Y, Int32 Count)
        {
            ...
            Interface = Interface.Replace("%TAKE_COMMAND_ARGUMENT%", $"newui.cmd droplist.controller element.take {ElementType} {Count} {Info.Argument}");
        ...
        private class InterfaceBuilder
        {
            ...
            private void BuildingStaticContext()
            {
                ...
                {
                    CursorEnabled = true,
                    RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "247.728 -205.258", OffsetMax = "595.541 228.873" },
                    Image = { Color = "0 0 0 0" }
            ...
            private void BuildingMuteAlert()
            {
                ...
                container.Add(new CuiElement
                {
                    ...
                    Components = {
                    new CuiRawImageComponent { Color = "1 1 1 1",Png = ImageUi.GetImage("IQCHAT_IGNORE_ALERT_BUTTON_YES") },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-100 49.7", OffsetMax = "100 79.7" }
                }
                });
            ...
            private void BuildingMuteAlert_DropList_Title()
            {
                ...
                container.Add(new CuiElement
                {
                    ...
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-199.828 -9.43", OffsetMax = "199.832 27.43" }
                }
                });
                ...
                container.Add(new CuiPanel
                {
                    CursorEnabled = false,
                    Image = { Color = "1 1 1 0" },
                    RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-147.497 -265.544", OffsetMax = "147.503 -24.7" }
            ...
            private void BuildingModerationStatic()
            {
                ...
                container.Add(new CuiElement
                {
                    ...
                    Components = {
                    new CuiTextComponent { Text = "%TITLE%", Font = "robotocondensed-regular.ttf", FontSize = 8, Align = TextAnchor.MiddleLeft, Color = "1 1 1 1" },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "11.075 -126.612", OffsetMax = "126.125 -112.9882541" } 
                }
                });
            ...
            private void BuildingMuteAllChat()
            {
                ...
                container.Add(new CuiElement
                {
                    ...
                    Components = {
                    new CuiRawImageComponent { Color = "1 1 1 1", Png = ImageUi.GetImage("IQCHAT_ELEMENT_PANEL_ICON") },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "11.072541 -161.818", OffsetMax = "152.88 -147.382" } 
                }
                });
            ...
            private void BuildingAlertUI()
            {
                ...
                container.Add(new CuiElement
                {
                    ...
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-184.193 -27.133", OffsetMax = "189.223 9.1192541" }  
                }
                });
                                        ...
                                        case "confirm.alert":
                                            {
                                                if (ActionType == SelectedAction.Ignore)
                                                    DrawUI_IQChat_Ignore_Alert(player, TargetPlayer);
                                                else DrawUI_IQChat_Mute_Alert(player, TargetPlayer);
                                                break;
                                            }
                                        case "open.reason.mute": 
                                            {
                                                MuteType Type = (MuteType)Enum.Parse(typeof(MuteType), arg.Args[5]);
                                                DrawUI_IQChat_Mute_Alert_Reasons(player, TargetPlayer, Type);
                                                break;
                                            }
                                        case "confirm.yes":
                                            {
                                                ...
                                                {
                                                    User Info = UserInformation[player.userID];
                                                    Info.Settings.IgnoredAddOrRemove(TargetPlayer.userID);
                                                ...
                                                else
                                                {
                                                    ...
                                                    MutePlayer(TargetPlayer, Type, IndexReason, player);
                            ...
                            case "element.take":
                                {
                                    ...
                                    String Argument = String.Join(" ", arg.Args.Skip(4));
                                    if (!UserInformation.ContainsKey(player.userID)) return;
