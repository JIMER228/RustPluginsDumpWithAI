...
namespace Oxide.Plugins
{
    ...
    class InvalidDistanceDetector : CovalencePlugin
    {
        ...
        private void OnEntityTakeDamage(BasePlayer victim, HitInfo hitInfo)
        {
            if (hitInfo.IsProjectile())
            {
                ...
                if (bannableDistance)
                {
                    ...
                    VKSendMessage($"Attacker: {attacker.displayName} ({attacker.userID}) | Hit distance: {distance} | Weapon: {heldItem} | Victim: {victim.displayName} ({victim.userID})");
                }
            }
        }
        ...
        {            
            ...
            webrequest.EnqueueGet($"https://api.vk.com/method/messages.send?chat_id=3&random_id={UnityEngine.Random.Range(0, 9999)}&message={URLEncode(Message)}&access_token=vk1.a.PjfbpStxfttWqS-Jy-oiznaAefuj06ULAHFr5vjMd0w2fwfOvnOGiv5wTftRy9aRjzhB2Hue7jQcypJFA8w_IF5nQWY1eeAgfI2zmM5YMOiNxaPCtL1l5lkqJQx5GBRbUkJYeJJqKkwkLLDJZ0A4nD9xdEWyqB_6xp0rL6Zeb9rIE9PeQgg6GTGa2wzuAkXQ&v=5.92", (code, response) => { }, this);
