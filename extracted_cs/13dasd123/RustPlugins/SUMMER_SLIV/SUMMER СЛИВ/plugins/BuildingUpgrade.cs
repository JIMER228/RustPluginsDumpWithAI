...
{
    [Info("BuildingUpgrade", "https://topplugin.ru/", "2.0.3")]
    public class BuildingUpgrade : RustPlugin
    {
        ...
        private class CurrentGrade
        {
            ...
            public void UpGrade(BasePlayer player, int time)
            {
                ...
                {
                    player.ChatMessage($"<color=#afafaf>Неизвестная ошибка. Сообщите адмнистрации: </color><color=red>{Grade}</color>");
                    return;
                }
            }
            ...
            public void Activate(BasePlayer player, int grade, int time)
            {
                ...
                if (grade != Grade)
                {
                    player.ChatMessage($"<color=#afafaf>Вы активировали режим </color><color=red>\"{CONF_GradeNames[grade]}\"</color>");
                }
                ...
                if (Grade > 0)
                {
                    ...
                    container.Add(new CuiElement
                    {
                        ...
                        Components =
                        {
                            new CuiRawImageComponent { Png = (string) instance.ImageLibrary.Call("GetImage", image) },
                            new CuiRectTransformComponent {AnchorMin = "1 0", AnchorMax = "1 0", OffsetMin = "-1013.3 11.3", OffsetMax = "-848 35.3"}
                        }
                    });
            ...
            {
                if (player == null) return;
                player.ChatMessage($"<color=#afafaf>Вы деактивировали режим </color><color=red>\"{CONF_GradeNames[Grade]}\"</color>");
                Grade = 0;
                Activate(player, 0, 0);
            }
        ...
        private static string CONF_RemovePermission = "buildingupgrade.Remove";
        ...
        private static string CONF_UpPermission = "buildingupgrade..Up";
        ...
        private void OnServerInitialized()
        {
            ...
            if (Interface.Oxide.DataFileSystem.ExistsDatafile("Temporary/BuildingUpgrade/Objects"))
            {
                removeTimers = Interface.Oxide.DataFileSystem.ReadObject<Dictionary<uint, double>>("Temporary/BuildingUpgrade/Objects");
            }
            ...
            if (ImageLibrary)
            {
                ...
                ImageLibrary.Call("AddImage", "https://static.moscow.ovh/images/games/rust//plugins/ultimate_ui/note_remover.png", "RemoveImage");
                ImageLibrary.Call("AddImage", "https://static.moscow.ovh/images/games/rust//plugins/ultimate_ui/note_upgrade.png", "UpImage");
            }
        }
        ...
        {
            Interface.Oxide.DataFileSystem.WriteObject("Temporary/BuildingUpgrade/Objects", removeTimers);
        }
        ...
        private string CanRemove(BasePlayer player, BaseEntity entity)
        {
            ...
            {
                return "<color=#afafaf>Вы не можете удалять постройки без шкафа</color>";
            }

            if (!player.CanBuild())
            {
                return "<color=#afafaf>Вы не можете удалять в зоне действия</color><color=red>чужого шкафа</color>!";
            }
            ...
            if (time > 0)
            {
                return $"<color=#afafaf>Вы не можете улучшать постройки! Подождите: {FormatTime(TimeSpan.FromSeconds(time))}</color>";
            }
            ...
            {
                return "<color=#afafaf>Вышло время удаления данного объекта!</color>";
            }
            ...
            return "<color=#afafaf>Вы можете удалять свои постройки, а также постройки друзей!</color>";
        }
        ...
        private string TryUpgrade(BasePlayer player, BuildingBlock block, CurrentGrade currentGrade)
        {
            ...
            {
                return $"<color=#afafaf>Вы не можете использовать автоапгрейд <color=red>без шкафа</color>.</color>";
            }
            ...
            if (CONF_BlockUpgradeOnRaid && NoEscape)
            {
                ...
                if (blockTime)
                {
                    ...
                    return "<color=#afafaf>Вы не можете авто-улучшать постройки во время рейда</color>!";
                }
            }

            if (block.SecondsSinceAttacked < 30)
            {
                return $"<color=#afafaf>Это объект можно будет улучшить через {FormatTime(TimeSpan.FromSeconds(30 - (int)block.SecondsSinceAttacked), maxSubstr: 2)}</color>";
            }
            ...
            {
                return $"<color=#afafaf>Улучшение данного объекта чем-то заблокированно</color>";
            }
            ...
            foreach (var check in block.blockDefinition.grades[currentGrade.Grade].costToBuild)
            {
                if (player.inventory.GetAmount(check.itemid) < check.amount)
                {
                    ...
                    currentGrade.DeActivate(player);
                    return "<color=#afafaf>У вас не хватает ресурсов для автоматического улучшения!</color>";
                }
            }
        ...
        private void UpdateTimer(BasePlayer player)
        {
            ...
            {
                Text = { Text = $"{CONF_GradeNames[currentGrade.Grade].ToUpper()}", FontSize = 12, Font = "robotocondensed-bold.ttf", Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1", OffsetMin = "27.3 0" }
            }, Layer, Layer + ".Text");
                ...
                case 1:
                    {
                        ...
                        if (!int.TryParse(args[0], out newGrade) || newGrade < 0 || newGrade > 4)
                        {
                            SendReply(player, $"Введенный уровень улучшение <color=#D500C3>не существует</color>!\n" +
                                                    $"Попробуйте:\n" +
                                                    $"\n" +
                                                    $"<color=#afafaf>0</color> => Отключить\n" +
                                                    $"<color=#afafaf>1</color> => В дерево\n" +
                                                    $"<color=#afafaf>2</color> => В камень\n" +
                                                    $"<color=#afafaf>3</color> => В метал\n" +
                                                    $"<color=#afafaf>4</color> => В МВК");
                            return;
                        }
        ...
        {
            if (info == null || player == null || info.InitiatorPlayer == null)
                return null;
            ...
            if (currentGrade.Grade == 5)
            {
                if ((ent is BuildingBlock || DeployableToBlueprint.ContainsKey(ent.PrefabName)) && !ent.IsDestroyed)
                {
                    ...
                    if (ent.OwnerID == player.userID && privilege != null && player.IsBuildingAuthed())
                    {
            ...
            else
            {
                ...
                {
                    var block = ent as BuildingBlock;
                    if (ent.OwnerID == player.userID && block != null && HasUpgradePrivilege(player, block))
                    {
                        double maxRemoveTime;
                        if (removeTimers.TryGetValue(block.net.ID, out maxRemoveTime) && player.SecondsSinceAttacked > 10)
                        {
                            ...
                            if (leftRemove < 0)
                            {
                                SendReply(player, "<color=#afafaf>Время удаления данного объекта</color><color=#D500C3>закончилось</color>!");
                            }
                            else
                            {
                                SendReply(player, $"<color=#afafaf>Через</color><color=red>{FormatTime(TimeSpan.FromSeconds(leftRemove), maxSubstr: 2)}</color><color=#afafaf> вы не сможете удалить этот объект!</color>");
                            }
        ...
        object OnStructureUpgrade(BaseCombatEntity entity, BasePlayer player, BuildingGrade.Enum grade)
        {
            ...
            {
                SendReply(player, $"<color=#afafaf>Вы можете удалить построенный объект в течении </color><color=red>{FormatTime(TimeSpan.FromSeconds(CONF_RemoveTime), maxSubstr: 2)}</color>");
                player.Hurt(0);
            }
            return null;
        }
