...
{
    [Info("Team Marker", "DarkPlugins.ru", "0.0.2")]
    public class TeamMarker : RustPlugin
    {
        ...
        private class MarkerHandler : MonoBehaviour
        {
            ...
            public void RefreshTeam()
            {
                ...
                {
                    foreach (var check in RelationshipManager.Instance.FindTeam(Player.currentTeam).members)
                    {
        ...
        private void OnPlayerInit(BasePlayer player) => player.GetOrAddComponent<MarkerHandler>();
