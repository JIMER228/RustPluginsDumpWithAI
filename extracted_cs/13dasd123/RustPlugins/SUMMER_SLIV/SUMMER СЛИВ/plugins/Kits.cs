...
namespace Oxide.Plugins
{
    ...
    class Kits: RustPlugin
    {
        ...
        { 
            ...
            {
                var item = ItemManager.CreateByName(kitItem.shortname, kitItem.amount);
        ...
        void Unload()
        {
            ...
            if (Process != null) ServerMgr.Instance.StopCoroutine(Process);
        ...
        [ConsoleCommand("kits.ui.close")]
        private void cmdConsoleCloseUI(ConsoleSystem.Arg args)
        {
            var player = args.Player();
            
            Init = false;
            CuiHelper.DestroyUi(player, "CustomUI.Kits");
        }
        ...
        private void cmdConsoleHandler(ConsoleSystem.Arg arg)
        {
            ...
            switch (cmd)
            {
                ...
                case "showinfokit":
                {
                    ...
                    {
                        Text = { Text = kitInfo.ShortName.ToUpper(), Align = TextAnchor.UpperCenter, FontSize = 40, Font = "robotocondensed-bold.ttf" },
                    ...
                    container.Add(new CuiElement
                    {
                        ...
                        Components =
                        {
                            new CuiImageComponent {Color = "0.58 0.00 0.00 0.6"},
                ...
                case "showavailablekits":
                {
                    ...
                    container.Add(new CuiElement
                    {
                        ...
                        Components =
                        {
                            new CuiImageComponent {Color = "0.23 0.57 0.29 0.0"},
                            new CuiRectTransformComponent {AnchorMin = "1 0", AnchorMax = "1 0", OffsetMin = "-291.3 22.6", OffsetMax = "-108 25.2"}
                        }
                    });
                    container.Add(new CuiButton
                    {
                        ...
                        Text = { Text = "", Align = TextAnchor.UpperCenter, FontSize = 18 },
                        RectTransform = { AnchorMin = "1 0", AnchorMax = "1 0", OffsetMin = "-291.3 22.6", OffsetMax = "-108 49.2" },
                    }, Layer);
                    container.Add(new CuiButton
                    {
                    ...
                    for (var i = 0; i < kits.Count;)
                    {
                        ...
                        if (time < 0)
                        {
                            container.Add(new CuiPanel
                            {
                                ...
                                Image = { Color = "0.58 0.00 0.00 0.6" }
        ...
        private void KitsCustomUI(BasePlayer player)
        {
            Init = true;
            ...
            container.Add(new CuiElement
            {
                Parent = "CustomUI.Kits",
                Components =
                {
                    new CuiButtonComponent { Command = "kits.ui.close", Close = "CustomUI.Kits", Color = "0 0 0 0" },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-1000 -1000", OffsetMax = "1000 1000" }
                }
            });

            double TotalCooldownTime = 0;
            for (var i = 0; i < GetKitsForPlayer(player).Count(); i++)
            {
                var kit = GetKitsForPlayer(player)[i];
                var kitsInfo = GetKitsInfo(player);
                var dataPlayer = GetKitData(kitsInfo, kit);
                var time = dataPlayer.cooldown - TimeHelper.GetTimeStamp();

                var r = GetKitsForPlayer(player).Count() * 10 + 50;
                var c = (double) GetKitsForPlayer(player).Count() / 2;
                var pos = i / c * Math.PI;
                ...
                {
                    Parent = "CustomUI.Kits",
                    Name = $"CustomUI.Kits.{kit.ShortName}",
                    Components =
                    {
                        new CuiImageComponent { Sprite = "assets/icons/reddit.png", Color = time < 0 ? HexToRGB(kit.kitcolor) : HexToRGB("#e04f58") },
                        new CuiRectTransformComponent { AnchorMin = $"{x - 35} {y - 35}", AnchorMax = $"{x + 35} {y + 35}" },
                    },
                });

                TotalCooldownTime += time;
                
                //Puts($"{TotalCooldownTime}");
                
                container.Add(new CuiElement
                {
                    Parent = $"CustomUI.Kits.{kit.ShortName}",
                    Name = $"CustomUI.Kits.{kit.ShortName}.Time",
                    Components =
                    {
                ...
                string text = time < 0 ? kit.ShortName : $"{kit.ShortName}\n{TimeHelper.FormatTime(TimeSpan.FromSeconds(time), 2)}";

                container.Add(new CuiElement
                {
                    Parent = $"CustomUI.Kits.{kit.ShortName}.Time",
                    Name = $"CustomUI.Kits.{kit.ShortName}.Update",
                    Components =
                    {
                        new CuiTextComponent { Text = text, FontSize = 12, Align = TextAnchor.MiddleCenter, Color = "1 1 1 1", Font = "robotocondensed-bold.ttf" },
                        new CuiRectTransformComponent{ AnchorMin = "0 0", AnchorMax = "1 1" },
                    }
                });
                ...
                if (time < 0)
                {
                    container.Add(new CuiButton
                    {
                        Button = { Color = "0 0 0 0", Command = $"UI_KITS givekit {kit.kitName} {i}", Close = "CustomUI.Kits" },
                        RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1" },
                        Text = { Text = "" }
                    }, $"CustomUI.Kits.{kit.ShortName}");
                }
                ...
                container.Add(new CuiElement
                {
                    ...
                    Components =
                    {
                        new CuiImageComponent { Sprite = "assets/icons/reddit.png", Color = HexToRGB("#5ba0ff") },
                ...
                container.Add(new CuiButton
                {
                    Button = { Color = "0 0 0 0", Command = $"UI_KITS showinfokit {kit.kitName}" },
                    RectTransform = { AnchorMin = "0.85 0.85", AnchorMax = "0.85 0.85", OffsetMin = "-10 -10", OffsetMax = "10 10" },
                    Text = { Text = "?", FontSize = 11, Align = TextAnchor.MiddleCenter, Color = "1 1 1 1", Font = "robotocondensed-bold.ttf" }
                }, $"CustomUI.Kits.{kit.ShortName}");
            }
            ...
            Process = ServerMgr.Instance.StartCoroutine(StartUpdate(player, TotalCooldownTime));
        }
        
        private IEnumerator StartUpdate(BasePlayer player, double TotalCooldownTime)
        {
            while (Init && player.IsConnected)
            {
                TotalCooldownTime = 0;
                
                foreach (var check in GetKitsForPlayer(player))
                {
                    CuiElementContainer container = new CuiElementContainer();
                    
                    var kitsInfo = GetKitsInfo(player);
                    var dataPlayer = GetKitData(kitsInfo, check);
                    var time = dataPlayer.cooldown - TimeHelper.GetTimeStamp();
                    
                    TotalCooldownTime += time;

                    string text = time < 0 ? check.kitName : $"{check.kitName}\n{TimeHelper.FormatTime(TimeSpan.FromSeconds(time), 2)}";

                    container.Add(new CuiElement
                    {
                        Parent = $"CustomUI.Kits.{check.kitName}.Time",
                        Name = $"CustomUI.Kits.{check.kitName}.Update",
                        Components =
                        {
                            new CuiTextComponent { Text = text, FontSize = 12, Align = TextAnchor.MiddleCenter, Color = "1 1 1 1", Font = "robotocondensed-bold.ttf" },
                            new CuiRectTransformComponent{ AnchorMin = "0 0", AnchorMax = "1 1" },
                    ...
                    CuiHelper.DestroyUi(player, $"CustomUI.Kits.{check.kitName}.Update");
                    CuiHelper.AddUi(player, container);
                }
                
                //Puts("Kits update start!");
                yield return new WaitForSeconds(1);
            }
            
            //Puts("Kits update stop!");
            yield break;
        }
        ...
        private void GiveItems(BasePlayer player, KitInfo kit)
        {
            ...
            {             
                GiveItem(player,BuildItem(kitItem.shortname,kitItem.amount,kitItem.Condition,kitItem.Weapon,kitItem.Content), kitItem.place == "Панель" ? player.inventory.containerBelt : kitItem.place == "Одежда" ? player.inventory.containerWear : player.inventory.containerMain);
            }
        }
        ...
        private Item BuildItem(string ShortName, int Amount, float Condition, Weapon weapon, List<ItemContent> Content)
        {
        ...
        public class KitInfo
        {
            ...
            [JsonProperty("Короткое название")] public string ShortName = "";
            ...
            [JsonProperty("Цвет кита")] public string kitcolor = "";
