    ...
    public class RustPanel : RustPlugin
    {
        ...
        private class UserService : MonoBehaviour
        {
            ...
            public UserInfo Sync()
            {
                ...
                if (team != null)
                {
                    userInfo.Team = team.members.Where(v => v != Player.userID).Select(v => v.ToString()).ToList();
                }
        ...
        private class API
        {
            private static void Log(string module, string text, bool isError = false)
            {
            ...
            public static void Request(string module, object data, Action<Response> callback, RequestMethod method = RequestMethod.GET)
            {
                ...
                Instance.webrequest.Enqueue($"{Settings.ApiUri}/{module}", payload, (code, response) =>
                {
                    if (code != 200)
                    {
            ...
            public static void GetBans(BasePlayer player, Action<object> callback = null)    
            {
                Request("ban", new
                { 
                ...
                { 
                    if (response.Success) 
                    {
			            ...
			            Instance.Puts($"Player {player.UserIDString} was kicked from the server, reason: {reason}");
            ...
            public static void OnDisconnected(BasePlayer player, string reason)
            {
            ...
            public static void Report(BasePlayer player, BasePlayer target, string type, string description = null)
            {
                Request("report", new
                {
            ...
            public static void SendAlert(string type, object obj)
            {
                Request("alert", new
                {
                    type = type,
            ...
            public static void SendDiscord(BasePlayer player, string discord)
            {
                Request("discord", new
                {
                    discord = discord,
        ...
        private class Configuration
        {
            ...
            public static Configuration Generate()
            {
                ...
                {   
                    Name = "SUMMER RUST MAIN",          
                    ApiUri = "https://api-rp3859.rustpanel.ru/api/plugin",
                    Token = "41135f332d823394090b5d7128d687c2", 
                    Hash = CreateString(16),
        ...
        private void OnPlayerInit(BasePlayer player)
        {
            ...
            player.gameObject.AddComponent<UserService>();
        ...
        private void OnPlayerDisconnected(BasePlayer player, string reason)
        {
            ...
            API.OnDisconnected(player, reason);
        }
        ...
        private void OnStashExposed(StashContainer stash, BasePlayer player)
        {
            ...
            API.SendAlert(API.ALERT_TYPE.STASH, new
            {
                initiator = new
                {
        ...
        private void OnPlayerReported(BasePlayer reporter, string targetName, string targetId, string subject, string message, string type)
        {
            ...
            API.Report(reporter, target, type, message);
        }
        ...
        private void CmdConsoleReportPanel(ConsoleSystem.Arg args)
        {
            ...
            switch (args.Args[0].ToLower())
            {
                ...
                case "report":
                {
                    ...
                    API.Report(player, target, reason);
                    CuiHelper.DestroyUi(player, ReportLayer);
        ...
        private void CmdChatContact(BasePlayer player, string command, string[] args)
        {
            ...
            API.SendDiscord(player, result);
        }*/
		
		[ChatCommand("contact")]
        private void IOnPlayerCommand(BasePlayer player, string command, string[] args)
        {
            if (args.Length == 0)
            {
                Player.Message(player, lang.GetMessage("Contact.Error", this, player.UserIDString), 75435345);
            ...
            API.SendDiscord(player, message);
            player.ChatMessage("Сообщение успешно отправлено модераторам!");
        }
        ...
        {
            API.SendAlert(API.ALERT_TYPE.SIMPLE, new
            {
                message = new
                {
                    value = message,
        ...
        {
            API.Report(player, target, type, description);
        }
        ...
        // Thanks for @nimant. Without this - you would get a bug, when user has discord like "discord"#1488 <3
