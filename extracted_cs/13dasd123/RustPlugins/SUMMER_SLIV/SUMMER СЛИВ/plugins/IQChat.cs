...
namespace Oxide.Plugins
{
    ...
    class IQChat : RustPlugin
    {
        ...
        #region Reference
        [PluginReference] Plugin IQPersonal, IQFakeActive, XDNotifications, IQRankSystem, IQModalMenu, IQReportSystemDLC;
        ...
        public void SetMutes(BasePlayer player) => IQReportSystemDLC?.CallHook("API_SET_MUTE", player.userID);
        #endregion
...
;
        ...
        #region Hooks     
        /*private bool OnPlayerChat(BasePlayer player, string message)
        {
           // if (Interface.Oxide.CallHook("CanChatMessage", player, message) != null) return false;
            SeparatorChat(player, message);
            return false;
        ...
        bool? OnPlayerChat(ConsoleSystem.Arg arg)
        {
            ...
            if (Interface.Oxide.CallHook("CanChatMessage", player, message) != null) return false;
            SeparatorChat(player, message);
            //Server.Run.Server.Normal($"sendmsgadmin [CHAT] {player.UserIDString} {player.displayName} - {message}");
            return false;
        }
...
foreach (var player in BasePlayer.activePlayerList)
         ...
         void DestroyAllGUI(BasePlayer player) { CuiHelper.DestroyUi(player, "IQCHAT_PARENT_ALERT_UI");}
        ...
        void OnPlayerDisconnected(BasePlayer player, string reason)
        {
            ...
            {
                string Avatar = Alert.ConnectedAvatarUse ? player.UserIDString : "";
                string Message = config.AlertSettings.DisconnectedReason ? GetLang("LEAVE_PLAYER_REASON", player.UserIDString, player.displayName, reason) : GetLang("LEAVE_PLAYER", player.UserIDString, player.displayName);
                ReplyBroadcast(Message, "", Avatar);
        ...
        private void SeparatorChat(BasePlayer player, string Message)
        {
            var DataPlayer = ChatSettingUser[player.userID];

            if (IsMutedUser(player.userID))
            {
                ReplySystem(player, GetLang("FUNC_MESSAGE_ISMUTED_TRUE", player.UserIDString, FormatTime(TimeSpan.FromSeconds(DataPlayer.MuteChatTime - CurrentTime()))));
                return;
            }
            ...
            ReplyChat(player, MessageSeparator);
        ...
        public void AutoSetupData(string id, string perm)
        {
            ...
            if (AutoSetup.UseSetupAutoPrefix)
                if (Prefix != null)
                {
                    ...
                    if (player != null)
                        ReplySystem(player, GetLang("PREFIX_SETUP", player.UserIDString, Prefix.Argument));
                }
                ...
                if (Prefix != null)
                {
                    if (config.MessageSetting.MultiPrefix)
                    {
                        if (DataPlayer.MultiPrefix.Contains(Prefix.Argument))
                        {
                            ...
                            BasePlayer player = BasePlayer.FindByID(userID);
                            if (player != null)
                                ReplySystem(player, GetLang("PREFIX_RETURNRED", player.UserIDString, Prefix.Argument));
                        }
                    }
                ...
                case MuteType.Chat:
                    {
                        ...
                        {
                            if (Target != null)
                                ReplySystem(Target, GetLang("FUNC_MESSAGE_MUTE_CHAT", Target.UserIDString, DisplayNameModerator, Target.displayName, FormatTime(TimeSpan.FromSeconds(MuteTime)), Reason));
                            if (Moderator != null)
                                ReplySystem(Moderator, GetLang("FUNC_MESSAGE_MUTE_CHAT", Target.UserIDString, DisplayNameModerator, Target.displayName, FormatTime(TimeSpan.FromSeconds(MuteTime)), Reason));
                        }
                        if (Moderator != null)
                            if (Moderator != Target)
                                SetMute(Moderator);
                ...
                case MuteType.Voice:
                    {
                        ...
                        {
                            if (Target != null)
                                ReplySystem(Target, GetLang("FUNC_MESSAGE_MUTE_VOICE", Target.UserIDString, DisplayNameModerator, Target.displayName, FormatTime(TimeSpan.FromSeconds(MuteTime)), Reason));
                            if (Moderator != null)
                                ReplySystem(Moderator, GetLang("FUNC_MESSAGE_MUTE_VOICE", Target.UserIDString, DisplayNameModerator, Target.displayName, FormatTime(TimeSpan.FromSeconds(MuteTime)), Reason));
                        }
                        break;
                    }
            }
                ...
                case MuteType.Chat:
                    {
                        ...
                        {
                            if (Target != null)
                                ReplySystem(Target, GetLang("FUNC_MESSAGE_UNMUTE_CHAT", Target.UserIDString, DisplayNameModerator, Target.displayName));
                            if (Moderator != null)
                                ReplySystem(Moderator, GetLang("FUNC_MESSAGE_UNMUTE_CHAT", Target.UserIDString, DisplayNameModerator, Target.displayName));
                        }
                        break;
                    }
                case MuteType.Voice:
                    {
                        ...
                        {
                            if (Target != null)
                                ReplySystem(Target, GetLang("FUNC_MESSAGE_UNMUTE_VOICE", Target.UserIDString, DisplayNameModerator, Target.displayName));
                            if (Moderator != null)
                                ReplySystem(Moderator, GetLang("FUNC_MESSAGE_UNMUTE_VOICE", Target.UserIDString, DisplayNameModerator, Target.displayName));
        ...
        public void RenameFunc(BasePlayer player, string NewName)
        {
            if (permission.UserHasPermission(player.UserIDString, config.RenamePermission))
            {
                ...
                ReplySystem(player, GetLang("COMMAND_RENAME_SUCCES", player.UserIDString, NewName));
            }
            else ReplySystem(player, GetLang("COMMAND_NOT_PERMISSION", player.UserIDString));
        }
        void AlertUI(BasePlayer player, string[] arg)
        {
            ...
            {
                ReplySystem(player, GetLang("FUNC_MESSAGE_NO_ARG_BROADCAST", player.UserIDString));
                return;
            }
            string Message = "";
        ...
        void IQChat_UI_Mute_Menu(BasePlayer player)
        {
            ...
            container.Add(new CuiButton
            {
                RectTransform = { AnchorMin = "0.9492017 0.318991426", AnchorMax = "0.9750086 0.6709784" },
            ...
            container.Add(new CuiLabel
            {
                RectTransform = { AnchorMin = "0.374989 0", AnchorMax = "0.938701426 1" },
        ...
        void Loaded_Players_Mute_Menu(BasePlayer player, int Page = 0, string Search = "")
        {
            ...
            container.Add(new CuiLabel
            {
                RectTransform = { AnchorMin = "0.3975857 0.318991426", AnchorMax = "0.4233935 0.6709784" },
        ...
        void MuteCustomAdmin(ConsoleSystem.Arg arg)
        {
            ...
            try
            {
                ...
                int ReasonIndex = int.Parse(arg.Args[1]);
                MutePlayer(target, TypeAction, ReasonIndex, arg.Player());
            }
            catch (Exception ex)
            {
                DebugEx.Log(ex.Data, StackTraceLogType.None);
            }
        ...
        void RenameMetods(BasePlayer player, string cmd, string[] arg)
        {
            if (arg.Length == 0 || arg == null)
            {
                ReplySystem(player, lang.GetMessage("COMMAND_RENAME_NOTARG", this, player.UserIDString));
                return;
            }
            string NewName = "";
        ...
        void PmChat(BasePlayer player, string cmd, string[] arg)
        {
            ...
            if (arg.Length == 0 || arg == null)
            {
                ReplySystem(player, lang.GetMessage("COMMAND_PM_NOTARG", this, player.UserIDString));
                return;
            }
            ...
            if (config.ReferenceSetting.IQFakeActiveSettings.UseIQFakeActive)
                if (IQFakeActive)
                    if (IsFake(NameUser))
                    {
                        ReplySystem(player, GetLang("COMMAND_PM_SUCCESS", player.UserIDString, string.Join(" ", arg.ToArray().ToArray()).Replace(NameUser, "")));
                        return;
                    }
            ...
            {
                ReplySystem(player, GetLang("FUNC_MESSAGE_PM_TURN_FALSE", player.UserIDString));
                return;
            }
            if (config.MessageSetting.IgnoreUsePM)
            {
                ...
                {
                    ReplySystem(player, GetLang("IGNORE_NO_PM", player.UserIDString));
                    return;
                }
                if (IsIgnored(player.userID, TargetUser.userID))
                {
                    ReplySystem(player, GetLang("IGNORE_NO_PM_ME", player.UserIDString));
                    return;
                }
            }
            ...
            {
                ReplySystem(player, GetLang("COMMAND_PM_NOT_NULL_MSG", player.UserIDString));
                return;
            }
            ...
            if (IQModalMenu)
            {
                if (ChatSettingUser[TargetUser.userID].GlobalChatTurn)
                    ReplySystem(TargetUser, GetLang("COMMAND_PM_SEND_MSG", player.UserIDString, DisplayNick, Message));
            }
            else ReplySystem(TargetUser, GetLang("COMMAND_PM_SEND_MSG", player.UserIDString, DisplayNick, Message));

            ReplySystem(player, GetLang("COMMAND_PM_SUCCESS", player.UserIDString, Message));
        ...
        void RChat(BasePlayer player, string cmd, string[] arg)
        {
            ...
            if (arg.Length == 0 || arg == null)
            {
                ReplySystem(player, GetLang("COMMAND_R_NOTARG", player.UserIDString));
                return;
            }
            if (!PMHistory.ContainsKey(player))
            {
                ReplySystem(player, GetLang("COMMAND_R_NOTMSG", player.UserIDString));
                return;
            }
            ...
            if (IQModalMenu)
            {
                if (ChatSettingUser[RetargetUser.userID].GlobalChatTurn)
                    ReplySystem(RetargetUser, GetLang("COMMAND_PM_SEND_MSG", player.UserIDString, DisplayNick, Message));
            }
            else ReplySystem(RetargetUser, GetLang("COMMAND_PM_SEND_MSG", player.UserIDString, DisplayNick, Message));
        ...
        void IgnorePlayerPM(BasePlayer player, string cmd, string[] arg)
        {
            ...
            if (arg.Length == 0 || arg == null)
            {
                ReplySystem(player, lang.GetMessage("INGORE_NOTARG", this, player.UserIDString));
                return;
            }
            string NameUser = arg[0];
            BasePlayer TargetUser = BasePlayer.Find(NameUser);
            if (TargetUser == null || NameUser == null)
            {
                ReplySystem(player, lang.GetMessage("COMMAND_PM_NOT_USER", this, player.UserIDString));
                return;
            }
        ...
        void ChatAlertPlayerInPM(ConsoleSystem.Arg arg)
        {
            ...
            ReplySystem(player, Message.Replace(player.userID.ToString(), ""));
        }
        ...
        void ReplyChat(BasePlayer player, string OutMessage)
        {
            ...
            if (MessageSetting.AntiSpamActivate)
            {
                if (Flooders.ContainsKey(player.userID))
                {
                    ...
                    {
                        ReplySystem(player, string.Format(lang.GetMessage("FLOODERS_MESSAGE", this, player.UserIDString), Convert.ToInt32(Flooders[player.userID] - CurrentTime())));
            ...
            BasePlayer.activePlayerList.ForEach(p => p.SendConsoleCommand("chat.add", player.userID, OutMessage));
        ...
        void ReplySystem(BasePlayer player, string Message, string CustomPrefix = "", string CustomAvatar = "", string CustomHex = "")
        {
            ...
            player.SendConsoleCommand("chat.add", Avatar, FormatMessage);
        }

        void ReplyBroadcast(string Message, string CustomPrefix = "", string CustomAvatar = "", Boolean AdminAlert = false)
        {
            BasePlayer.activePlayerList.ForEach(p => { ReplySystem(p, Message, CustomPrefix, CustomAvatar); });
        }
        ...
        void API_SEND_PLAYER(BasePlayer player, string PlayerFormat, string Message, string Avatar)
        {
            var MessageSettings = config.MessageSetting;
            string OutMessage = Message;
            ...
            player.SendConsoleCommand("chat.add", ulong.Parse(Avatar), $"{PlayerFormat}: {OutMessage}");
        ...
        void API_ALERT(string Message, string CustomPrefix = "", string CustomAvatar = "", string CustomHex = "")
        {
            foreach (var p in BasePlayer.activePlayerList)
                ReplySystem(p, Message, CustomPrefix, CustomAvatar, CustomHex);
        }
        void API_ALERT_PLAYER(BasePlayer player, string Message, string CustomPrefix = "", string CustomAvatar = "", string CustomHex = "") => ReplySystem(player, Message, CustomPrefix, CustomAvatar, CustomHex);
