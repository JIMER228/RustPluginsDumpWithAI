...
namespace Oxide.Plugins
{
    ...
    internal class IQTurret : RustPlugin
    {
        ...
        protected override void LoadConfig()
        {
            ...
            {
                PrintWarning(LanguageEn ? $"Error #847195 configuration readings 'oxide/config/{Name}', creating a new configuration!" : $"Ошибка #847195 чтения конфигурации 'oxide/config/{Name}', создаём новую конфигурацию!"); //
                LoadDefaultConfig();
            }
            NextTick(SaveConfig);
        }
            ...
            if (Type == TypeLimiter.Player)
            {
                foreach (KeyValuePair<UInt64, List<ControllerInformation>> Turrets in TurretList)
                    foreach (ControllerInformation ControllerInformation in Turrets.Value)
                        ...
                        && (ControllerInformation.turrel != null && ((ControllerInformation.turrel is SamSite && ControllerInformation.turrel.HasFlag(BaseEntity.Flags.Reserved8)) 
                                                                     || (ControllerInformation.turrel is AutoTurret && ControllerInformation.turrel.HasFlag(BaseEntity.Flags.On)))))
                            CountTurret++;
            }
            else
            {
        ...
        {
            if (turrel == null || turrel is NPCAutoTurret || !IDsPrefabs.Contains(turrel.prefabID) && TurretList.ContainsKey(turrel.skinID)) return;
        ...
        /// <summary>
        /// Обновление 1.3.x
        /// - Добавлена поддержка рейдблоков
        /// - Добавлена возможность запрещать использовать рубильники во время рейдблока
        /// - Мелкие изменения в методах
        /// - Добавлена поддержка SentryTurrets
        /// </summary>
