...
namespace Oxide.Plugins 
{
    ...
    class XFarmRoom : RustPlugin
    {
				...
				if(permission.UserHasPermission(player.UserIDString, perm.Key))
				{
					...
					if(StoredData.ContainsKey(player.userID))
					{
						if(config.Setting.UpdateOption && StoredData[player.userID].LastUpdate == 0 || !config.Setting.UpdateOption && DateTimeOffset.Now.ToUnixTimeSeconds() - StoredData[player.userID].LastUpdate >= config.Setting.UpdateSecond)
						{
							...
							Player.Reply(player, config.Setting.PrefixChat + lang.GetMessage("CHAT_UPDATE_ORES", this, player.UserIDString));
						}
					}
					else
		...
		private void LeaveFarmRoom(BasePlayer player, bool isunload = false)
		{
			...
			if(!isunload)
			{
				...
				Player.Reply(player, config.Setting.PrefixChat + lang.GetMessage("CHAT_LEAVE_ROOM", this, player.UserIDString));
			}
		...
		void cmdTPFarmRoom(BasePlayer player)
		{
			...
			{
				Player.Reply(player, config.Setting.PrefixChat + lang.GetMessage("CHAT_NO_PERM", this, player.UserIDString));
				return;
			}
			...
			if(!_players_in_room.ContainsKey(player.userID))
			{
				if(Cooldowns.ContainsKey(player.userID))
				{
					...
					{
						Player.Reply(player, config.Setting.PrefixChat + string.Format(lang.GetMessage("CHAT_CD_TP", this, player.UserIDString), Cooldowns[player.userID] + cd - DateTimeOffset.Now.ToUnixTimeSeconds()));
				...
				{
					Player.Reply(player, config.Setting.PrefixChat + string.Format(lang.GetMessage("CHAT_LIMIT_ITEMS", this, player.UserIDString), config.Setting.MaxCountItem));
				...
				if(!StoredData.ContainsKey(player.userID))
				{
					Player.Reply(player, config.Setting.PrefixChat + lang.GetMessage("CHAT_NO_ALL_PERM", this, player.UserIDString));
				...
				{
					if(no_permission)
						Player.Reply(player, config.Setting.PrefixChat + lang.GetMessage("CHAT_NOT_ORES_NOT_PERM", this, player.UserIDString));
					else
						if(config.Setting.UpdateOption)
							Player.Reply(player, config.Setting.PrefixChat + lang.GetMessage("CHAT_NOT_ORES_WIPE", this, player.UserIDString));
						else
							Player.Reply(player, config.Setting.PrefixChat + string.Format(lang.GetMessage("CHAT_NOT_ORES_UPDATE", this, player.UserIDString), StoredData[player.userID].LastUpdate + config.Setting.UpdateSecond - DateTimeOffset.Now.ToUnixTimeSeconds()));
				...
				if(_spawn_room_position.Count != 0)
				{
					...
					Player.Reply(player, config.Setting.PrefixChat + lang.GetMessage("CHAT_JOIN_ROOM", this, player.UserIDString));
				}
				else
					Player.Reply(player, config.Setting.PrefixChat + lang.GetMessage("CHAT_NO_ROOMS", this, player.UserIDString));
			}
			else
				Player.Reply(player, config.Setting.PrefixChat + lang.GetMessage("CHAT_ALREADY_IN_ROOM", this, player.UserIDString));
		}
		...
		private void GUIFarmRoom(BasePlayer player)
		{
			...
			foreach(var ore in StoredData[player.userID].Ore)
			{
				...
				container.Add(new CuiElement 
				{
					...
					Components =
					{
						new CuiRawImageComponent { Png = (string) ImageLibrary.Call("GetImage", ore.Key + 150), Color = ore_no_zero ? "1 1 1 1" : "1 1 1 0.2004272" },
				...
				container.Add(new CuiLabel
                {
                    ...
                    Text = { Text = $"{ore.Value}", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", FontSize = 12, Color = ore_no_zero ? "1 1 1 1" : "1 1 1 0.2004272" }
                }, ".GUI_ORE");
				
				container.Add(new CuiButton
				{
					RectTransform = { AnchorMin = "1 0.5", AnchorMax = "1 0.5", OffsetMin = "-23.5 -10.5", OffsetMax = "-2.5 10.5" },
					Button = { Color = ore_no_zero && in_list ? "0.55004272 0.65004272 0.45 1" : ore_no_zero && !in_list ? "0.85004272 0.49004272 0.44 1" : "0.85 0.49 0.44 0.5", Command = $"farm_ore {ore.Key} {!in_list}" },
					Text = { Text = "" }
				}, ".GUI_ORE");
			...
			if(config.Setting.UseUIButton)
				container.Add(new CuiButton
				{
					...
					Text = { Text = lang.GetMessage("UI_LEAVE", this, player.UserIDString), Align = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", FontSize = 13, Color = "1 1 1 0.2004272" }
				}, ".GUI_FARMROOM");
		...
		void cmdLeaveFarmRoom(BasePlayer player)
		{
			...
			if(_players_in_room.ContainsKey(player.userID))
				...
				Player.Reply(player, config.Setting.PrefixChat + lang.GetMessage("CHAT_NOT_IN_ROOM", this, player.UserIDString));
		}
		...
		void cmdSpawnRoomPosition(BasePlayer player)
		{
			if(player.IsAdmin)
			{
				...
				Player.Reply(player, config.Setting.PrefixChat + string.Format(lang.GetMessage("CHAT_SPAWN_POINTS", this, player.UserIDString), _spawn_room_position.Count));
			}
		}
		...
		private object OnPlayerCommand(BasePlayer player, string command, string[] args)
		{
			...
			{
				Player.Reply(player, config.Setting.PrefixChat + lang.GetMessage("CMD_BLOCK", this, player.UserIDString));
