...
namespace Oxide.Plugins
{
	...
	public class InfoPanel : RustPlugin
	{
		...
		private class ConfigData
		{
			...
			[JsonProperty("Настройка игроков")]
			...
			[JsonProperty("Настройка слиперов")]
			...
			[JsonProperty("Настройка координат")]
			...
			[JsonProperty("Настройка экономики")]
			...
			switch (Type)
			{
				...
				case "online":
					UI.CreateLabel(ref RefreshContainer, player, ".Refresh.Online", ".Online.Label",
						text: $"<b>{GetOnline()}/{ConVar.Server.maxplayers}</b>");
					break;
				case "balance":
				...
				case "time":
					UI.CreateLabel(ref RefreshContainer, player, ".Refresh.Time", ".Time.Label",
						text: TOD_Sky.Instance.Cycle.DateTime.ToString("<b>HH:mm</b>"));
					break;
				case "sleepers":
