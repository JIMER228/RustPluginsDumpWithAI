...
namespace Oxide.Plugins
{
    ...
    class WipeBlock : RustPlugin
    {
        ...
        private object OnReloadWeapon(BaseProjectile projectile, BasePlayer player)
        {
        ...
        private object OnReloadMagazine(BaseProjectile projectile, int desiredAmount, BasePlayer player)
        {
            if (projectile == null || player == null) return null;
            if (player.GetComponent<NPCPlayer>() != null || player.GetComponent<BaseNpc>() != null || player.IsNpc)
                return null;
        ...
        private void MessBlockUi(BasePlayer player, string shortname, string name)
        {
            ...
            {
                Image = { Color = "0.9686275 0.9176471 0.8784314 0.25" },
                RectTransform =
                     {AnchorMin = "0.7854167 0.3111114", AnchorMax = "0.9916667 0.4074074", OffsetMin = "0 0", OffsetMax = "0 0"},
                CursorEnabled = false,
            }, "Overlay", Layer);
            ...
            container.Add(new CuiElement
            {
                ...
                Components =
                 {
                     new CuiImageComponent {Color = "0.791199 0.7354013 0.6115956 0.5935043"},
                ...
                Components =
                 {
                     ...
                     {
                         Color = "1 1 1 1",
                         Text = $"Предмет заблокирован! Информация о заблокированных предметах\n/block <color=orange></color>",
                         FontSize = 10, Align = TextAnchor.MiddleCenter, Font = "robotocondensed-bold.ttf"
                     },
        ...
        private void BlockUi(BasePlayer player)
        {
            ...
            foreach (var check in config.items.Select((i, t) => new { A = i, B = t }))
            {
                ...
                string text = BlockTimeGui(check.A.Key) ? $"<color=#A9A9A9>{TimeSpan.FromSeconds((int)IsBlocked(check.A.Key)).ToShortString()}</color>" : "<color=#D69100FF>Доступно</color>";
                container.Add(new CuiButton
                {
                    RectTransform =
                        {
