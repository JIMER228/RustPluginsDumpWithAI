...
{
    [Info("StatsSystem", "TopPlugin.ru", "1.0.6")]
    class StatsSystem : RustPlugin
    {
        ...
        void OnDispenserGather(ResourceDispenser dispenser, BasePlayer player, Item item)
        {
            ...
            if (DB[player.userID].Res.ContainsKey(item.info.shortname))
            {
                ...
                DB[player.userID].Settings["Farm"] += item.amount;
                DB[player.userID].Farm += item.amount;
                return;
            }
        }
            ...
            if (info.InitiatorPlayer != null)
            {
                ...
                if (killer != player)
                {
                    if (DB.ContainsKey(killer.userID))
                    {
                        ...
                        DB[player.userID].Kill++;
                        DB[killer.userID].Points += 25;
                    }
                }
                if (DB.ContainsKey(player.userID))
                {
        ...
        void StatsUI(BasePlayer player, int page = 0)
        {
            ...
            container.Add(new CuiButton
            {
                ...
                Text = { Text = $"МЕСТО", Color = "1 1 1 1", Align = TextAnchor.MiddleCenter, FontSize = 12, Font = "robotocondensed-regular.ttf" }
            }, "TTT");
            ...
            container.Add(new CuiLabel
            {
                ...
                Text = { Text = "Очки даются:\nУбийство +25, добыча руды +3, разрушение бочки +3, сбитие вертолета +1000, уничтожение танка +1000\nОчки отнимаются:\nСмерть и самоубийство -30", Color = "1 1 1 0.8", Align = TextAnchor.MiddleLeft, FontSize = 12, Font = "robotocondensed-regular.ttf" }
            }, "InfoTop");
            ...
            if (page == 0)
            {
                for (int x = 0; x < DB.Take(10).Count(); x++)
                {
                    ...
                    {
                        RectTransform = { AnchorMin = $"0.5 0", AnchorMax = $"0.6 1", OffsetMax = "0 0" },
                        Text = { Text = $"{config.Bonus.ElementAt(x)}", Color = "1 1 1 0.8", Align = TextAnchor.MiddleCenter, FontSize = 12, Font = "robotocondensed-regular.ttf" }
                    }, "PlayerTop");
