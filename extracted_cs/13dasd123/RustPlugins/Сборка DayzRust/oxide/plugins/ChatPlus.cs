...
{
    [Info("ChatPlus", "RustPlugin.ru - Developed by Vlad-00003", "1.5.5", ResourceId = 82)]
    public class ChatPlus : CovalencePlugin
    {
		...
		private void PrintToChatMessageFake(string name, string userID, string message)
        {
            var prefix = config.Get(config.prefixes, "chatplus.default").Format;
            var nameColor = config.Get(config.names, "chatplus.default").Format;
            var messageColor = config.Get(config.messages, "chatplus.default").Format;
            name = string.Format(nameColor, RemoveTags(name));
            if (prefix.Length > 0)
                name = $"{prefix} {name}";
            message = RemoveTags(message);
            message = string.Format(messageColor, message);
            SendChat(name, message, "", userID);
        }

        private object OnPlayerOfflineChat(ulong playerId, string playerName, string message, Chat.ChatChannel channel)
        {
            if (GlobalMute)
            {
                return false;
            ...
            var pData = GetPlayerData(userId, playerName);
            
            if (SpamCheck(message) != null) return false;
            if (MuteData.IsMuted(userId))
                return false;
            if (config.CapsBlock)
            {
            ...
            {
                Mute(userId,playerName, new TimeSpan(0, 0, config.MuteBadWordsDefault), config.MuteReasonAutoMute);
            }
            ...
            var name = string.Format(nameColor, RemoveTags(playerName));
            message = string.Format(messageColor, message);
            ...
            {
                SendChat(name, message, censorMessage, userId);
                Log(LogType.Chat, $"{name}[{userId}]: {message}");
            }
            else
            {
                ...
                if (playerTeam == null)
                    return false;
                var onlineMembers = playerTeam.GetOnlineMemberConnections();
                if (onlineMembers != null)
                    SendChat(name,message,censorMessage,userId,onlineMembers.Select(x => x.player as BasePlayer));
                playerTeam.BroadcastTeamChat(playerId,Formatter.ToPlaintext(name),Formatter.ToPlaintext(message),"#fff");
                Log(LogType.TeamChat, $"{name}[{userId}]: {message}");
            }
            ...
            Chat.Record(new Chat.ChatEntry
            {
                ...
                UserId = userId,
                Username = Formatter.ToPlaintext(name),
                Color = "#5af",
                Time = Epoch.Current
            });
            ServerConsole.PrintColoured(ConsoleColor.DarkYellow,$"[{channel}] {Formatter.ToPlaintext(name)}({userId}) : ", ConsoleColor.DarkGreen, Formatter.ToPlaintext(message));
            return true;
        }
        object OnPlayerChat(BasePlayer player, string message, Chat.ChatChannel channel)
        {
            var sender = player.IPlayer;
            if (GlobalMute)
            {
            ...
            if (pData.IsAdmin && channel != Chat.ChatChannel.Team)
            {
            ...
            if (SpamCheck(sender, message) != null) return false;
            if (MuteCheck(sender) != null)
            {
                //TODO find a way to make DiscordMesssages die
                return false;
            }
            ...
            if (config.BadWordsBlock & mute)
            {
                Mute(sender, new TimeSpan(0, 0, config.MuteBadWordsDefault), config.MuteReasonAutoMute, sender);
            }
            var prefix = config.Get(config.prefixes, pData.Prefix).Format;
            var nameColor = config.Get(config.names, pData.NameColor).Format;
            var messageColor = config.Get(config.messages, pData.MessageColor).Format;

            var name = string.Format(nameColor, RemoveTags(sender.Name));
            message = string.Format(messageColor, message);
            censorMessage = string.Format(messageColor, censorMessage);
            ...
            if (channel != Chat.ChatChannel.Team)
            {
                SendChat(name, message, censorMessage, sender.Id);
            ...
            else
            {
                ...
                var onlineMembers = playerTeam.GetOnlineMemberConnections();
                if (onlineMembers != null)
                    SendChat(name,message,censorMessage,sender.Id,onlineMembers.Select(x => x.player as BasePlayer));
                playerTeam.BroadcastTeamChat(player.userID,Formatter.ToPlaintext(name),Formatter.ToPlaintext(message),"#fff");
                Log(LogType.TeamChat, $"{name}[{sender.Id}]: {message}");
            }
            Chat.Record(new Chat.ChatEntry
            {
                Channel = channel,
            ...
            ServerConsole.PrintColoured(ConsoleColor.DarkYellow,$"[{channel}] {Formatter.ToPlaintext(name)}({sender.Id}) : ", ConsoleColor.DarkGreen, Formatter.ToPlaintext(message));
            return true;
        }
        ...
        private string RemoveTags(string message)
        {
            ...
            return Formatter.ToPlaintext(message).EscapeRichText();
        }
        ...
        public string CensorBadWords(string input, out bool found)
        {
            ...
            {
                var firstIndex = temp.IndexOf(swear.Key, StringComparison.Ordinal);
                if (firstIndex >= 0 && swear.Value.All(exception => temp.IndexOf(exception, StringComparison.Ordinal) < 0))
                {
        ...
        {
            var text = Formatter.ToPlaintext(msg);
            switch (type)
            {
                case LogType.Chat:
                    Interface.Oxide.RootLogger.Write(Core.Logging.LogType.Info, $"[CHAT] {text}");
                    return;
                case LogType.Mute:
                    Interface.Oxide.RootLogger.Write(Core.Logging.LogType.Info, $"[MUTE] {text}");
                    return;
                case LogType.Pm:
                    Interface.Oxide.RootLogger.Write(Core.Logging.LogType.Info, $"[PM] {text}");
                    return;
                case LogType.ServerConsole:
                    Interface.Oxide.RootLogger.Write(Core.Logging.LogType.Info, $"[ServerConsole] {text}");
                    return;
                case LogType.TeamChat:
                    Interface.Oxide.RootLogger.Write(Core.Logging.LogType.Info, $"[TEAM CHAT] {text}");
                    return;
            }
        }
        #endregion
        ...
        void SendChat(string name, string message, string censorMessage = "", string userId = "0", IEnumerable<BasePlayer> receivers = null)
        {
            ...
            var channel = receivers == null ? 0 : 1;
            receivers = receivers ?? BasePlayer.activePlayerList;
            foreach (var player in receivers)
            {
                var msg = GetPlayerData(player).Censor ? censorMessage : message;
        ...
        private void Reply(IPlayer player, string langkey, params object[] args)
        {
            if (player == null)
                return;
            var message = string.Format(GetMsg(langkey, player), args);
        ...
        void Mute(string targetId, string targetName, TimeSpan? time, string reason)
        {
            if (string.IsNullOrEmpty(reason)) reason = config.MuteReason;
            string sender = "ChatPlus";
            string log;
            if (!time.HasValue)
            {
                mutes[targetId] = new MuteData(sender, reason);
                BroadcastChat("USER.MUTED.REASON", sender, targetName, reason, "Unlimited");
                log = string.Format(GetMsg("USER.MUTED.LOG"), sender, targetName, "Unlimited", reason);
                Interface.CallHook("OnBetterChatMuted", targetName, null, reason);
            }
            else
            {
                mutes[targetId] = new MuteData(sender, reason, time.Value);
                BroadcastChat("USER.MUTED.REASON", sender, targetName, reason, MuteData.TimeToString(time.Value));
                log = string.Format(GetMsg("USER.MUTED.LOG"), sender, targetName, MuteData.TimeToString(time.Value), reason);
                Interface.CallHook("OnBetterChatTimeMuted", targetName, null, time.Value, reason);
            }
            Log(LogType.Mute, log);
            SaveMutes();
        }
        void Mute(IPlayer target, TimeSpan? time, string reason, IPlayer player)
        {
            if (string.IsNullOrEmpty(reason)) reason = config.MuteReason;
            string sender = target == player ? "ChatPlus" : player.IsServer ? config.ConsoleName : player.Name;
            if (config.adminPrivilages.HideAdmins.HasValue && config.adminPrivilages.HideAdmins.Value &&
                !player.IsServer)
            ...
            if (!time.HasValue)
            {
                mutes[target.Id] = new MuteData(sender, reason);
                BroadcastChat("USER.MUTED.REASON", sender, target.Name, reason, "Unlimited");
                log = string.Format(GetMsg("USER.MUTED.LOG"), sender, target.Name, "Unlimited", reason);
                Interface.CallHook("OnBetterChatMuted", target, player, reason);
            }
            else
            {
                mutes[target.Id] = new MuteData(sender, reason, time.Value);
                BroadcastChat("USER.MUTED.REASON", sender, target.Name, reason, MuteData.TimeToString(time.Value));
                log = string.Format(GetMsg("USER.MUTED.LOG"), sender, target.Name, MuteData.TimeToString(time.Value), reason);
                Interface.CallHook("OnBetterChatTimeMuted", target, player, time.Value, reason);
            }
            Log(LogType.Mute, log);
            SaveMutes();
        }
        private List<IPlayer> GetPlayers(string NameOrID)
        {
            return players.FindPlayers(NameOrID).ToList();
        }
        private void SendChatHelp(IPlayer player)
        {
            string msg = GetMsg("CMD.CHAT.HELP", player);
            if (CanMuteAll(player)) msg += GetMsg("CMD.MUTE.ALL.HELP", player);
            if (IsAdmin(player)) msg += GetMsg("CMD.CHAT.HELP.PERMISSION.ADMIN", player);
            if (IsModerator(player)) msg += GetMsg("CMD.CHAT.HELP.PERMISSION.MODERATOR", player);
        ...
        private void cmdConsolePrefix(IPlayer player, string cmd, string[] Args)
        {
            ...
            var pdata = GetPlayerData(target);
            if (!PermissionService.HasPermission(target.Id, Args[1]))
            {
        ...
        void cmdChatMuteList(IPlayer player, string cmd, string[] Args)
        {
            ...
            else
            {
                ...
                {
                    msg += "\n" + string.Format(GetMsg("MUTE.LIST.BODY", player), mute.Value.Initiator, $"{players.FindPlayerById(mute.Key.ToString()).Name} ({mute.Key})", mute.Value.Reason, mute.Value.Remain);
                }
            }
            player.Reply(msg);
        }
        ...
        void cmdChatMute(IPlayer player, string cmd, string[] args)
        {
            ...
            if (MuteData.IsMuted(mutePlayer.Id))
            {
                Reply(player, "USER.ALREADY.MUTED", mutePlayer.Name, mutes[mutePlayer.Id].Reason, mutes[mutePlayer.Id].Remain);
                return;
            }
            TimeSpan time;
            if (args.Length == 1)
            {
                Mute(mutePlayer, null, null, player);
                return;
            }
            if (MuteData.StringToTime(args[1], out time))
            {
                Mute(mutePlayer, time, string.Join(" ", args.Skip(2).ToArray()), player);
                return;
            }
            Mute(mutePlayer, null, string.Join(" ", args.Skip(1).ToArray()), player);
        }
        [Command("unmute")]
        void cmdChatUnMute(IPlayer player, string cmd, string[] args)
        {
            ...
            recivers = recivers.Where(p => mutes.ContainsKey(p.Id)).ToList();
            if (recivers.Count == 0)
            {
            ...
            var mutePlayer = recivers.First();
            mutes.Remove(mutePlayer.Id);
            BroadcastChat("USER.UNMUTED", player.Name, mutePlayer.Name);
            string log = GetMsg("USER.UNMUTED.LOG");
            log = string.Format(log, player.Name, mutePlayer.Name);
            Log(LogType.Mute, log);
            SaveMutes();
        }
        [Command("chat")]
        void cmdChat(IPlayer player, string cmd, string[] args)
        {
            ...
            switch (args[0])
            {
                ...
                case "ignore":
                    ...
                    bool mode = args[1] == "add";
                    var reply = 0;
                    var recivers = players.FindPlayers(args[2]).ToList();
                    if (!recivers.Any())
                    {
        ...
        {
            var msg = string.Format(GetMsg(langkey, player), args);
            player.Command("chat.add", 2, PmAvatar() ? avatar : "0", covalence.FormatText(msg));
        }

        readonly Dictionary<string, string> pmHistory = new Dictionary<string, string>();
        void cmdChatPM(IPlayer player, string cmd, string[] args)
        {
            ...
            argList.RemoveAt(0);
            string message = RemoveTags(string.Join(" ", argList.ToArray()));
            ...
            var receiverName = RemoveTags(receiver.Name);
            if (GetPlayerData(receiver).BlackList.Contains(player.Id))
            {
                Reply(player, "PM.YOU.ARE.BLACK.LIST", receiverName);
                return;
            }
            pmHistory[player.Id] = receiver.Id;
            pmHistory[receiver.Id] = player.Id;
            var targetName = RemoveTags(player.Name);
            if (player.IsServer)
            {
                ConsoleIPly = player;
                Reply(player, "PM.SENDER.FORMAT", receiverName, message);
                RustChat(receiver, "PM.RECEIVER.FORMAT", "0", config.ConsoleName, message);
            }
            else
            {
                RustChat(player, "PM.SENDER.FORMAT", player.Id, receiverName, message);
                RustChat(receiver, "PM.RECEIVER.FORMAT", player.Id, targetName, message);
            }

            var bply = receiver.Object as BasePlayer;
            if (GetPlayerData(receiver).PMSound)
            {
                Effect.server.Run(config.PrivateSoundMessagePath, bply, 0, Vector3.zero, Vector3.zero);
            }
            Log(LogType.Pm, string.Format(GetMsg("PM.LOG"), $"{targetName}({player.Id})", $"{receiverName}({receiver.Id})", message));
        }
        void cmdChatR(IPlayer player, string cmd, string[] args)
        {
            ...
            string message = RemoveTags(string.Join(" ", args));
            ...
            var playerName = RemoveTags(player.Name);
            if (recieverUserId == "server_console")
            {
                RustChat(player, "PM.SENDER.FORMAT", player.Id, config.ConsoleName, message);
                Reply(ConsoleIPly, "PM.RECEIVER.FORMAT", playerName, message);
                Log(LogType.Pm, string.Format(GetMsg("PM.LOG"), $"{playerName}({player.Id})", $"Server console", message));
                return;
            }
            ...
            {
                RustChat(player, "PM.SENDER.FORMAT", player.Id, receiverName, message);
                RustChat(receiver, "PM.RECEIVER.FORMAT", player.Id, playerName, message);
            }
            var bply = receiver.Object as BasePlayer;
            if (GetPlayerData(receiver).PMSound)
            {
            ...
            Log(LogType.Pm, string.Format(GetMsg("PM.LOG"), $"{playerName}({player.Id})", $"{receiverName}({receiver.Id})", message));
        }
        #endregion
        ...
        private class MuteData
        {
            ...
            public static bool IsMuted(string userID, ulong playerid = 0) => mutes.ContainsKey(userID);
        ...
        PlayerData GetPlayerData(string playerId, string playerName)
        {
        ...
        private string GetMsg(string langKey) => lang.GetMessage(langKey, this);

        private string GetMsg(string langKey, BasePlayer player) =>
            lang.GetMessage(langKey, this, player?.UserIDString);

        private string GetMsg(string langKey, IPlayer player) => lang.GetMessage(langKey, this, player?.Id);
        ...
        protected override void LoadConfig()
        {
            base.LoadConfig();
            {
                ...
                if (config.names.All(c => c.Perm != "chatplus.default"))
                {
                ...
                if (config.messages.All(c => c.Perm != "chatplus.default"))
                {
                ...
                if (config.prefixes.All(c => c.Perm != "chatplus.default"))
                {
