...
{
    [Info("XSkinMenu", "rustmods.ru", "1.0.12")]
    class XSkinMenu : RustPlugin
    {
        private void InitializeLang()
        {
            ...
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ["TITLE"] = "MENÚ COOL SERVER SKINS",
                ["SETINFO"] = "¡PUEDES PERSONALIZAR EL MENÚ DEPENDIENDO DE LA SITUACIÓN!",
                ["ERRORSKIN"] = "¡LA PIEL QUE ELIGIÓ SE PUEDE CAMBIAR SOLO EN EL INVENTARIO O AL CREAR!",
                ["CLEARALL"] = "RESTABLECER TODAS LAS PIELES SELECCIONADAS",
                ["NOPERM"] = "¡No permisos!",
                ["NEXT"] = "PRÓXIMO",
                ["BACK"] = "ESPALDA",
                ["weapon"] = "ARMA",
                ...
                ["inventory"] = "CAMBIO DE PIEL EN INVENTARIO",
                ["entity"] = "CAMBIAR LA PIEL DE LOS OBJETOS",
                ["craft"] = "CAMBIA DE PIEL AL CREAR",
                ["clear"] = "CAMBIAR LA PIEL AL ELIMINAR",
                ["give"] = "CAMBIO DE PIEL AL CAER EN EL INVENTARIO",
        ...
        private void CategoryGUI(BasePlayer player, int page = 0)
        {
            ...
            foreach (var category in config.Category)
            {
                ...
                container.Add(new CuiButton
                {
                    ...
                    Text = { Text = lang.GetMessage(category.Key, this, player.UserIDString), Align = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", FontSize = 18, Color = "0.750927 0.750927 0.750927 1" }
                }, ".SkinBUTTON", ".BUTTON");
        ...
        private void SettingGUI(BasePlayer player)
        {
            ...
            container.Add(new CuiLabel
            {
                ...
                Text = { Text = lang.GetMessage("SETINFO", this, player.UserIDString), Align = TextAnchor.LowerCenter, Font = "robotocondensed-regular.ttf", FontSize = 12, Color = "0.750927 0.750927 0.750927 0.4" }
            }, ".SettingGUI");
            ...
            foreach (var s in setting)
            {
                ...
                container.Add(new CuiLabel
                {
                    ...
                    Text = { Text = lang.GetMessage(s.Key, this, player.UserIDString), Align = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", FontSize = 15, Color = "0.750927 0.750927 0.750927 1" }
                }, ".SM");
        ...
        private void ItemGUI(BasePlayer player, string category, int Page = 0)
        {
            ...
            {
                if (!player_data.ContainsKey(item.Key))
                    continue;

                ulong skinID = player_data[item.Key];

                bool c = player_data.ContainsKey(item.Key), s = skinID != 0;
                ...
                container.Add(new CuiElement
                {
                    ...
                    Components =
                    {
                        ...
                        {
                            ItemId = GetItemId(item.Key),
                            SkinId = 0,
                        },
					   // new CuiRawImageComponent { Png = (string) ImageLibrary.Call("GetImage", config.GUI.MainSkin && c && s ? $"{skinID}152" : item.Key + 150) },
                        new CuiRectTransformComponent { AnchorMin = "0 0", AnchorMax = "1 1", OffsetMin = "7.5 7.5", OffsetMax = "-7.5 -7.5" }
                    }
                });

                if (StoredDataSkins.ContainsKey(item.Key) && StoredDataSkins[item.Key].Count != 0 && c)
                {
                    container.Add(new CuiButton
                    {
                        RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1", OffsetMax = "0 0" },
                        Button = { Color = "0 0 0 0", Command = $"skin_c skin {item.Key}" },
                        Text = { Text = "" }
                    }, ".Item");
                    ...
                    if (s)
                        container.Add(new CuiButton
                        {
                            ...
                            Button = { Color = "1 1 1 0.75", Sprite = "assets/icons/clear.png", Command = $"skin_c clear {item.Key} {z} {category} {Page}" },
                            Text = { Text = "" }
                        }, ".Item", $".I + {z}");
                }
        ...
        private void SkinGUI(BasePlayer player, string item, int Page = 0)
        {
            ...
            foreach (var skin in StoredDataSkins[item].Skip(Page * 40))
            {
                ...
                if (config.Setting.DeleteButton && player.IsAdmin)
                    ...
                    {
                        RectTransform = { AnchorMin = "1 0", AnchorMax = "1 0", OffsetMin = "-20 5", OffsetMax = "-5 20" },
                        Button = { Color = "1 1 1 0.750927", Sprite = "assets/icons/clear.png", Command = $"xskin remove_ui {item} {skin} {Page}" },
                        Text = { Text = "" }
                    }, ".Skin");
        ...
        private void GUI(BasePlayer player)
        {
            ...
            container.Add(new CuiLabel
            {
                ...
                Text = { Text = lang.GetMessage("TITLE", this, player.UserIDString), Align = TextAnchor.MiddleCenter, FontSize = 18, Color = "1 1 1 0.750927" }
            }, ".SGUI");
