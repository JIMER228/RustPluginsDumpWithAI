...
namespace Oxide.Plugins
{
    ...
    class AutoCleanup : RustPlugin
    {
        ...
        {
            if (broadcastToChat)
                PrintToChat(Lang("<size=20><color=orange>Запущена оптимизация карты</color></size>\nВсе стенки на карте будут уничтожены.</color>", null));
            if (logToConsole)
                PrintWarning(Lang("<size=20><color=orange>Запущена оптимизация карты</color></size>\nВсе стенки на карте будут уничтожены.</color>", null));
