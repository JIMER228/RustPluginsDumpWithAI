...
namespace Oxide.Plugins
{
    ...
    class AdminMenu : ChaosPlugin
    {
	    ...
	    {
		    foreach(ConfigData.CustomCommands customCommand in Configuration.PlayerInfoCommands)
		    {
			    foreach(ConfigData.CustomCommands.PlayerInfoCommandEntry command in customCommand.Commands)
			    {
				    if (!string.IsNullOrEmpty(command.RequiredPermission) && command.RequiredPermission.StartsWith("adminmenu.", StringComparison.OrdinalIgnoreCase))
        ...
        private void CreateSubMenu(UIUser uiUser, BaseContainer parent)
        {
	        int[] subTypes = uiUser.MenuIndex == MenuType.Commands ? m_CommandSubTypes : 
					         ...
					         uiUser.MenuIndex == MenuType.Give ? m_ItemCategoryTypes : Array.Empty<int>();
	        ...
	        ImageContainer.Create(parent, Anchor.Center, new Offset(-535f, 245f, 535f, 270f))
		        ...
		        {
			        if (!HasPermissionForSubMenu(uiUser.Player, uiUser.MenuIndex, uiUser.SubMenuIndex))
				        return;
        ...
        private void CreateSelectionHeader(UIUser uiUser, BaseContainer parent, string label, bool pageUp, bool pageDown, bool showPlayerToggles)
        {
	        // Header Bar
        ...
        private void CreateCommandMenu(UIUser uiUser, BaseContainer parent)
        {
	        ...
	        {
		        List<ConfigData.CommandEntry> commands = uiUser.SubMenuIndex == 0 ? Configuration.ChatCommands : Configuration.ConsoleCommands;
        ...
        private void CreateGiveOverlay(UIUser uiUser, BasePlayer target, ItemDefinition itemDefinition)
        {
	        BaseContainer baseContainer = ButtonContainer.Create(ADMINMENU_UI, Layer.Overall, Anchor.FullStretch, Offset.zero)
				...
				{
					ImageContainer.Create(givePopup, Anchor.Center, new Offset(-100f, 95f, 100f, 115f))
						.WithStyle(m_PanelStyle)
					...
					ImageContainer.Create(givePopup, Anchor.Center, new Offset(-100f, -90f, 100f, 90f))
						.WithStyle(m_PanelStyle)
								...
								.WithChildren(giveButton =>
								{
									...
									ButtonContainer.Create(giveButton, Anchor.FullStretch, Offset.zero)
										...
										.WithCallback(m_CallbackHandler, arg =>
										{
											...
											target.GiveItem(ItemManager.Create(itemDefinition, uiUser.GiveAmount), BaseEntity.GiveItemReason.PickedUp);
			        ...
			        .WithLayoutGroup(m_PluginActionsLayout, m_PluginActions, 0, (int i, List<PluginAction> list, BaseContainer actions, Anchor anchor, Offset offset) =>
			        {
				        ...
				        BaseContainer.Create(actions, anchor, offset)
					        .WithLayoutGroup(m_InternalPluginActionsLayout, list, 0, (int ii, PluginAction t, BaseContainer innerGrid, Anchor anchor2, Offset offset2) =>
					        {
						        ...
						        if (Configuration.UsePlayerAdminPermissions && !t.HasPermission(uiUser))
							        return;
	    ...
	    private struct PluginAction
	    {
		    ...
		    public PluginAction(string name, Func<UIUser, bool> hasPermission, Action<UIUser> onClick)
		    {
			    this.Name = name;
		    ...
		    public PluginAction(string name, Func<bool> isViewable, Func<UIUser, bool> hasPermission, Action<UIUser> onClick)
		    {
			    this.Name = name;
        ...
        private void OnValueChanged(ConsoleSystem.Command c)
        {
	        Debug.Log($"onvchanged {c.FullName} {c.String}");
        }
        #endregion
        ...
        private void CreatePermissionsMenu(UIUser uiUser, BaseContainer parent)
        {
	        ...
	        else
	        {
		        ...
		        CreateSelectionHeader(uiUser, parent, FormatString("Label.TogglePermission", uiUser.Player, uiUser.PermissionTargetName.StripTags()), m_ListLayout.HasNextPage(uiUser.Page, dst.Count), uiUser.Page > 0, false);
        ...
        private void LayoutPermissionGrid(UIUser uiUser, BaseContainer parent, List<KeyValuePair<string, bool>> list)
        {
	        ImageContainer.Create(parent, Anchor.FullStretch, new Offset(25f, 0f, 0f, -30f))
		        ...
		        .WithLayoutGroup(m_ListLayout, list, uiUser.Page, (int i, KeyValuePair<string, bool> t, BaseContainer permissionLayout, Anchor anchor, Offset offset) =>
		        {
			        ...
			        bool hasPermission = (isUserPermission && permission.UserHasPermission(uiUser.PermissionTarget, t.Key)) || (isGroupPermission && permission.GroupHasPermission(uiUser.PermissionTarget, t.Key));
