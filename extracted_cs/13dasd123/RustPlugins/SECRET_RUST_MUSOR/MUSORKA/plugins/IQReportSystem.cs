...
{
    [Info("IQReportSystem", "Mercury", "0.2.7")]
    class IQReportSystem : RustPlugin
    {
        ...
        {
            if (AFKCheckedTry.ContainsKey(ulong.Parse(player.Id)))
            {
                BasePlayer Moderator = BasePlayer.FindByID(PlayerSaveCheck[ulong.Parse(player.Id)].ModeratorID);
                if (Moderator != null)
                    SendChat(Moderator, GetLang("PLAYER_AFK_ACTIVE_CMD", Moderator.UserIDString, command));

                AFKCheckedTry[ulong.Parse(player.Id)]++;
            }
        }
