    ...
    public class Backpack : RustPlugin
    {
        ...
        void OnEntityDeath(BaseCombatEntity ent, HitInfo info)
        {
            ...
            if (droppedItemContainer != null)
            {
                ...
                NextFrame(() =>
                {
                    if (droppedItemContainer?.IsDestroyed != false)
                        return;
                    droppedItemContainer.CancelInvoke(droppedItemContainer.RemoveMe);
                    droppedItemContainer.Invoke(droppedItemContainer.RemoveMe, KillTimeout);
                });
        ...
        void ShowUIPlayer(BasePlayer player, BasePlayer target)
        {
            ...
            {
                RectTransform = {
                    AnchorMin="0.5 0", AnchorMax="0.5 0", OffsetMin="-265 18", OffsetMax="-205 78"
                }
                ,
                Button = {
