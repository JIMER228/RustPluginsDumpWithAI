...
namespace Oxide.Plugins
{
    ...
    class DeathMessages : RustPlugin
    {
        ...
        class DeathMessage
        {
            ...
            public DeathMessage(Attacker attacker, Victim victim, string weapon, string damageType, double distance)
            {
                Attacker = attacker;
                Victim = victim;
                Weapon = weapon;
            ...
            private DeathReason InitializeReason()
            {
                ...
                else if (Weapon == "F1 Grenade" || Weapon == "Timed Explosive Charge" || Weapon == "Satchel Charge" || Weapon == "Beancan Grenade")
        ...
        class PluginConfig
        {
            ...
            [JsonProperty("Сообщения ({attacker} - инициатор,  {victim} - жертва, {weapon} - оружие,  {distance} - дистанция")]
            ...
            public static PluginConfig DefaultConfig()
            {
                    ...
                    Weapons = new Dictionary<string, string>
                {
                    ...
                    { "Revolver", "Revolver" },
                    { "Semi-Automatic Pistol", "P250" },
                    { "Semi-Automatic Rifle", "Semi Automic" },
                    { "Thompson", "Thompson" },
                    { "Waterpipe Shotgun", "Waterpipe Shotgun" },
                    { "Satchel Charge", "Satchel" },
                    { "Timed Explosive Charge", "С4" },
                    { "RocketSpeed", "RocketSpeed" },
                    { "Incendiary Rocket", "Rocket" },
                    { "Rocket", "Rocket" },
                    { "Spas-12 Shotgun", "Spas-12" },
                    {"Multiple Grenade Launcher", "Grenade Launcher" },
                    {"40mm.grenade.he", "Grenade Launcher" },
                },

                    Turrets = new Dictionary<string, string>
                {
                    { "Flame Turret", "Огненная турель" },
                    { "Guntrap", "Гантрап" },
                    { "Static Turret", "Турель" },
                },
                    ...
                    Messages = new Dictionary<string, string>
                {
                    ...
                    { "Flamethrower", "{attacker} сжег игрока {victim} ({weapon})" },
                    ...
                    { "Turret", "{victim} был убит  турелью" },
                    { "Guntrap", "{victim} был убит гантрапом" },
                    ...
                    { "Turret Sleeping", "{attacker} был убит турелью." }
                }
                };
            }
        }
        ...
        private void OnEntityDeath(BaseCombatEntity victim, HitInfo info)
        {
            try
            {
                ...
                {
                    LogToFile("log", $"[{DateTime.Now.ToShortTimeString()}] {info.Initiator} убил {victim} ({_weapon} с дистанции {_distance})", this, true);
                }

                if (_config.ShowColsole && _attacker.Type == AttackerType.Player)
                {
                    Puts($"[{DateTime.Now.ToShortTimeString()}] {info.Initiator} убил {victim} ({_weapon} с дистанции {_distance})");
                }

                AddNote(new DeathMessage(_attacker, _victim, _weapon, _damageType, _distance));
            }
