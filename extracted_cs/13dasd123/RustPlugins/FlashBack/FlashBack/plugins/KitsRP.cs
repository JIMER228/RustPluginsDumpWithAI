...
namespace Oxide.Plugins {
	[Info("KitsRP", "Rever", "1.1.0")]
	class KitsRP : RustPlugin {
		...
		public string KitsDataFName = "KitsRP.taked";
		public string KitsFName = "KitsRP.kits";
		public static string PNGDir = "file://" + Interface.Oxide.DataDirectory + Path.DirectorySeparatorChar + "Kits" + Path.DirectorySeparatorChar;
		...
		class PluginConfig {
			...
			public static PluginConfig CreateDefault() {
				return new PluginConfig {
					...
					DefaultKitImage    = PNGDir + "default.png",
					KitWidth           = 0.12f,
					MarginTop          = 0.04f,
					MarginBottom       = 0.03f,
		...
		private void SaveKits() { Interface.Oxide.DataFileSystem.WriteObject(KitsFName, _kits); }

		private void SaveTakedKits() { Interface.Oxide.DataFileSystem.WriteObject(KitsDataFName, _kitsData); }

		private void LoadMessages() {
			lang.RegisterMessages(new Dictionary<string, string> {
									  ["Kit Was Removed"]   = "<color=#008B8B>[Наборы]:</color> {kitname} был удалён",
									  ["Kit Doesn't Exist"] = "<color=#008B8B>[Наборы]:</color> Этого набора не существует",
									  ["Not Found Player"]  = "<color=#008B8B>[Наборы]:</color> Игрок не найден",
									  ["To Many Player"]    = "<color=#008B8B>[Наборы]:</color> Найдено несколько игроков",
									  ["Permission Denied"] = "<color=#008B8B>[Наборы]:</color> Ты не можешь использовать этот набор",
									  ["Limite Denied"]     = "<color=#008B8B>[Наборы]:</color> Ты уже использовал этот набор максимальное количество раз",
									  ["Cooldown Denied"]   = "<color=#008B8B>[Наборы]:</color> Ты сможешь использовать этот комплект через <color=red>{time}</color>",
									  ["Reset"]             = "<color=#008B8B>[Наборы]:</color> Все данные о использовании наборами игроками стерты!",
									  ["Kit Already Exist"] = "<color=#008B8B>[Наборы]:</color> Этот набор уже существует",
									  ["Kit Created"]       = "<color=#008B8B>[Наборы]:</color> Создан новый набор - {name}",
									  ["Kit Cloned"]        = "<color=#008B8B>[Наборы]:</color> Предметы были скопированы из инвентаря в набор",
									  ["UI Amount"]         = "Осталось: <cpolor=yellow>{amount}</color>",
									  ["Help"]              = "/kit name|add|clone|remove|list|reset",
									  ["Help Add"]          = "/kit add <имя>",
									  ["Help Clone"]        = "/kit clone <имя>",
									  ["Help Remove"]       = "/kit remove <имя>",
									  ["Help Give"]         = "/kit give <имя> <ник|steamID>",
		...
		private void Loaded() {
			...
			_kits     = Interface.Oxide.DataFileSystem.ReadObject<List<Kit>>(KitsFName);
			_kitsData = Interface.Oxide.DataFileSystem.ReadObject<Dictionary<ulong, Dictionary<string, KitData>>>(KitsDataFName);
			LoadMessages();
		}
		...
		void OnServerSave() { SaveTakedKits(); }

		private void OnServerInitialized() {
			foreach (var kit in _kits) {
				_imagesCache.Add(kit.Name, PNGDir + kit.Png + ".png");
				Debug.Log($"{Title.ToLower()}.{kit.Permission}");
				if (!permission.PermissionExists($"{Title.ToLower()}.{kit.Permission}"))
					permission.RegisterPermission($"{Title.ToLower()}.{kit.Permission}", this);
			}

			timer.Repeat(1, 0, RefreshCooldownKitsUI);
		}

		private void OnPlayerDisconnected(BasePlayer player) { _kitsGUI.Remove(player); }
		#endregion
		...
		private void CommandConsoleKit(ConsoleSystem.Arg arg) {
			...
			if (kit.Amount > 0) {
				if (playerData.Amount >= kit.Amount) {
					foreach (var kitname in _kitsGUI[player]) {
						CuiHelper.DestroyUi(player, $"ui.{Title}.{kitname}.button");
						CuiHelper.DestroyUi(player, $"ui.{Title}.{kitname}.amount");
						CuiHelper.DestroyUi(player, $"ui.{Title}.{kitname}");
					}
				...
				CuiHelper.DestroyUi(player, $"ui.{Title}.{value}.amount");
			...
			if (kit.Cooldown > 0) {
				...
				if (playerData.Cooldown > currentTime) {
					CuiHelper.DestroyUi(player, $"ui.{Title}.{value}.button");
		...
		private bool GiveKit(BasePlayer player, string kitname) {
			...
			if (!string.IsNullOrEmpty(kit.Permission) && !permission.UserHasPermission(player.UserIDString, $"{Title.ToLower()}.{kit.Permission}")) {
		...
		private void KitCommandAdd(BasePlayer player, string kitname) {
			...
			permission.RegisterPermission($"{Title.ToLower()}.default", this);
		...
		private void KitCommandGive(BasePlayer player, BasePlayer foundPlayer, string kitname) {
			...
			GiveItems(player, _kits.First(x => x.Name == kitname));
		}

		private void GiveItems(BasePlayer player, Kit kit) {
			foreach (var kitem in kit.Items) {
				GiveItem(player.inventory,
						 BuildItem(kitem.ShortName, kitem.Amount, kitem.SkinID, kitem.Condition, kitem.Weapon, kitem.Content),
		...
		private Item BuildItem(string ShortName, int Amount, ulong SkinID, float Condition, Weapon weapon, List<ItemContent> Content) {
			try {
				...
				if (weapon != null) {
					var projItem = item.GetHeldEntity() as BaseProjectile;
					if (projItem == null) return null;

					projItem.primaryMagazine.contents                                 = weapon.ammoAmount;
		...
		private void TriggerUI2(BasePlayer player) { InitilizeUI2(player); }

		private void TriggerUI3(BasePlayer player) { InitilizeUI3(player); }

		private void TriggerUI1(BasePlayer player) { InitilizeUI(player); }
		...
		private void InitilizeUI(BasePlayer player) {
			...
			container.Add(new CuiPanel {
							  Image = {
								  Color = HexToRustFormat(_config.MainBackgroundColor)
							  },
							  RectTransform = {
								  AnchorMin = _config.Position.AnchorMin,
								  AnchorMax = _config.Position.AnchorMax
							  },
							  CursorEnabled = true
						  },
						  name: $"ui.{Title}");
			container.Add(new CuiButton {
							  Button = {
								  Command = "kit ui",
								  Color   = _config.CloseButtonColor
							  },
							  RectTransform = {
								  AnchorMin = _config.CloseButtonPosition.AnchorMin,
								  AnchorMax = _config.CloseButtonPosition.AnchorMax
							  },
							  Text = {
								  Text     = "X",
								  FontSize = 20,
			...
			foreach (var kit in kits) {
				if (kits.Count > 14) {
					...
					container.Add(new CuiButton {
									  ...
									  RectTransform = {
										  AnchorMin = _config.CloseButtonPositionNext1.AnchorMin,
										  AnchorMax = _config.CloseButtonPositionNext1.AnchorMax
									  },
									  Text = {
										  Text     = "1",
										  FontSize = 20,
					...
					container.Add(new CuiButton {
									  ...
									  RectTransform = {
										  AnchorMin = _config.CloseButtonPositionNext2.AnchorMin,
										  AnchorMax = _config.CloseButtonPositionNext2.AnchorMax
									  },
									  Text = {
										  Text     = "2",
										  FontSize = 20,
		...
		private void DestroyUI(BasePlayer player) {
			...
			foreach (var kitname in _kitsGUI[player]) {
				CuiHelper.DestroyUi(player, $"ui.{Title}.{kitname}.time");
				CuiHelper.DestroyUi(player, $"ui.{Title}.{kitname}.mask");
				CuiHelper.DestroyUi(player, $"ui.{Title}.{kitname}.button");
				CuiHelper.DestroyUi(player, $"ui.{Title}.{kitname}.amount");
				CuiHelper.DestroyUi(player, $"ui.{Title}.{kitname}");
			}

			CuiHelper.DestroyUi(player, $"ui.{Title}");
			...
			foreach (var playerGUIData in _kitsGUI) {
				...
				foreach (var kitname in playerGUIData.Value) {
					...
					if (playerKitData.Cooldown > 0) {
						CuiHelper.DestroyUi(playerGUIData.Key, $"ui.{Title}.{kitname}.time");

						if (playerKitData.Cooldown < currentTime) {
							CuiHelper.DestroyUi(playerGUIData.Key, $"ui.{Title}.{kitname}.mask");
		...
		private void InitilizeKitsUI(ref CuiElementContainer container, BasePlayer player) {
			...
			foreach (var kit in kits) {
				...
				container.Add(new CuiPanel {
								  Image = {
									  Color = HexToRustFormat(_config.KitBackgroundColor)
								  },
								  RectTransform = {
									  AnchorMin = $"{pos} {_config.MarginBottom}",
									  AnchorMax = $"{pos + _config.KitWidth} {1.0f - _config.MarginTop}"
								  }
							  },
							  $"ui.{Title}",
							  $"ui.{Title}.{kit.Name}");
		...
		private void InitilizeKitImageUI(ref CuiElementContainer container, string kitname) {
			...
			container.Add(new CuiElement {
				Parent = $"ui.{Title}.{kitname}",
				Components = {
					imageComp,
					new CuiRectTransformComponent {
						AnchorMin = _config.Image.Position.AnchorMin,
						AnchorMax = _config.Image.Position.AnchorMax
					}
				}
			});
		}
		...
		private void InitilizeNameLabelUI(ref CuiElementContainer container, string kitname, string text) {
			var name = container.Add(new CuiPanel {
										 Image = {
											 Color = HexToRustFormat(_config.Label.BackgroundColor)
										 },
										 RectTransform = {
											 AnchorMin = _config.Label.Position.AnchorMin,
											 AnchorMax = _config.Label.Position.AnchorMax
										 }
									 },
									 $"ui.{Title}.{kitname}");
			container.Add(new CuiLabel {
							  Text = {
								  Color    = HexToRustFormat(_config.Label.ForegroundColor),
								  FontSize = _config.Label.FontSize,
								  Align    = _config.Label.TextAnchor,
								  Text     = text
							  }
						  },
						  name);
		...
		private void InitilizeMaskUI(ref CuiElementContainer container, string kitname) {
			var name = container.Add(new CuiPanel {
										 Image = {
											 Color = HexToRustFormat(_config.DisableMaskColor)
										 }
									 },
									 $"ui.{Title}.{kitname}",
									 $"ui.{Title}.{kitname}.mask");
		}

		private void InitilizeAmountLabelUI(ref CuiElementContainer container, string kitname, string text) {
			container.Add(new CuiPanel {
							  Image = {
								  Color = HexToRustFormat(_config.Amount.BackgroundColor)
							  },
							  RectTransform = {
								  AnchorMin = _config.Amount.Position.AnchorMin,
								  AnchorMax = _config.Amount.Position.AnchorMax
							  }
						  },
						  $"ui.{Title}.{kitname}",
						  $"ui.{Title}.{kitname}.amount");
			container.Add(new CuiLabel {
							  Text = {
								  Color    = HexToRustFormat(_config.Amount.ForegroundColor),
								  FontSize = _config.Amount.FontSize,
								  Align    = _config.Amount.TextAnchor,
								  Text     = text
							  }
						  },
						  $"ui.{Title}.{kitname}.amount");
		...
		private void InitilizeButtonUI(ref CuiElementContainer container, string kitname) {
			container.Add(new CuiButton {
						  ...
						  $"ui.{Title}.{kitname}",
						  $"ui.{Title}.{kitname}.button");
		}

		private void InitilizeCooldownLabelUI(ref CuiElementContainer container, string kitname, TimeSpan time) {
			container.Add(new CuiPanel {
							  Image = {
								  Color = HexToRustFormat(_config.Time.BackgroundColor)
							  },
							  RectTransform = {
								  AnchorMin = _config.Time.Position.AnchorMin,
								  AnchorMax = _config.Time.Position.AnchorMax
							  }
						  },
						  $"ui.{Title}.{kitname}",
						  $"ui.{Title}.{kitname}.time");
			container.Add(new CuiLabel {
							  Text = {
								  Color    = HexToRustFormat(_config.Time.ForegroundColor),
								  FontSize = _config.Time.FontSize,
								  Align    = _config.Time.TextAnchor,
								  Text     = "Будет доступен через:\n" + TimeExtensions.FormatShortTime(time)
							  }
						  },
						  $"ui.{Title}.{kitname}.time");
		}
		...
		private KitData GetPlayerData(ulong userID, string name) {
			if (!_kitsData.ContainsKey(userID)) _kitsData[userID]             = new Dictionary<string, KitData>();
			if (!_kitsData[userID].ContainsKey(name)) _kitsData[userID][name] = new KitData();
		...
		private List<Kit> GetKitsForPlayer(BasePlayer player) {
			return _kits.Where(kit => kit.Hide == false
								  && (string.IsNullOrEmpty(kit.Permission) || permission.UserHasPermission(player.UserIDString, $"{Title.ToLower()}.{kit.Permission}"))
								  && (kit.Amount == 0 || (kit.Amount > 0 && GetPlayerData(player.userID, kit.Name).Amount < kit.Amount)))
						.ToList();
		}
