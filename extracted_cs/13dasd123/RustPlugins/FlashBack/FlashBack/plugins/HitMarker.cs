            ...
            for (var i = damages.Count-1; i >= 0; i--)
            {
                ...
                else
                {
                    if(item.Value.isFriend)
                        CuiHelper.AddUi(player, HandleArgs(DamageGUI, item.Value.num, $"<size=22><color={(item.Value.isFriend ? "#2EFE2E" : (item.Value.isHead ? "red" : "white"))}>ДРУГ</color></size>", item.Value.xs, item.Value.ys, item.Value.xe, item.Value.ye));
                    else
