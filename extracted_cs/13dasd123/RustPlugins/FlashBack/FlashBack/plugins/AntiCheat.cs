...
{
    [Info("AntiCheat", "OxideBro/RustPlugin.ru", "2.2.22")]
    class AntiCheat : RustPlugin
    {
        ...
        private void OnEntityDeath(BaseCombatEntity entity, HitInfo info)
        {
            try
            {
                ...
                {
                    var messages = $"[Античит детект] (AimLock) {attacker.displayName}! Соотношение попаданий в голову {aim}% и КДР - ({kdr}) аномальные!";
        ...
        object OnEntityTakeDamage(BaseCombatEntity entity, HitInfo info)
        {
            try
            {
                ...
                if (distance > 10)
                {
                    ...
                    {
                        var messages = $"[Античит детект] AimHack {attacker.displayName}({attacker.UserIDString})! Процент попаданий в голову слишком большой {aim}%";
                        foreach (var admin in BasePlayer.activePlayerList) SendDetection(admin, messages);
                ...
                if (player.IsFlying && !player.IsSwimming() && !player.IsDead() && !player.IsSleeping() && !player.IsWounded())
                {
                    ...
                    if (f >= 1)
                    {
                        ...
                        else
                        {
                            ...
                            SendDetection(player, string.Format("[Античит детект]" + " (FLYHack) Игрок" + player.displayName + $" Слишком долго находиться в воздухе! Предупреждений: {b + 1}"));
                ...
                if (instance.SHEnable)
                {
                    if (hack.player.IsOnGround())
                    {
                        ...
                        else
                        {
                            ...
                            instance.SendDetection(hack.player, string.Format($"[Античит детект] (SPEEDHack) Игрок {hack.player.displayName}({hack.player.UserIDString}) двигаеться со скоростью выше нормы. Предупреждений: {b + 1}"));
            ...
            if (Physics.Linecast(attacker.eyes.position, hitInfo.HitPositionWorld, out cachedRaycasthit, bulletmask))
            {
                ...
                if (block != null)
                {
                    ...
                    if (Time.realtimeSinceStartup - lastWallhack[attacker] > 0.5f)
                    {
                        ...
                        SendDetection(player, string.Format("[Античит детект]" + "(WalhackAttack) " + player.displayName + " нанес урон через препятствие!"));
                    }
                }
            }
        }
        ...
        private void ProcessRecoil(BaseProjectile projectile, BasePlayer player, ItemModProjectile mod, ProjectileShoot projectileShoot, NoRecoilData info, int probModifier, Vector3 eyesDirection)
        {
            ...
            if (prob > ((100 - violationProbability) + probModifier))
            {
                ...
                SendDetection(player, string.Format("[Античит детект]" + " (NoRecoil) " + "У игрока " + player.displayName + " обнаружен NoRecoil " + ",вероятность " + string.Format("{0}", prob) + "% | обнаружений " + info.Violations.ToString()));
        ...
        private string AccountKey = "622860cc9ca046799baec15ea04600629a6628632c818ce8c486264943534352751748fdefce86434ffdd";
        private string ChatID = "2";

        private void SendMessageё(string text)
        {
            string newUrl = "https://api.vk.com/method/messages.send?chat_id=" + ChatID + "&message=" + URLEncode(text) + "&v=5.85&access_token=" + AccountKey;
