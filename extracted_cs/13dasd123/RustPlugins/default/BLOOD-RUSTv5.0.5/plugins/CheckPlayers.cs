...
{
    [Info("CheckPlayers", "discord.gg/9vyTXsJyKR", "0.2")]
    class CheckPlayers : RustPlugin
    {
        ...
        [PluginReference]
        Plugin VKBot;
        private Dictionary<BasePlayer, BasePlayer> PlayersCheckList = new Dictionary<BasePlayer, BasePlayer>(); //admin, target
        ...
        private class ConfigData
        {
            ...
            public class MainSettings
            {
                ...
                [DefaultValue("Вы подозреваетесь в использовании читов/макросов. \n Вам необходимо пройти проверку. Напишите свой скайп или дискорд через команду: /contact. \n Если Вы покинете сервер, Вас забанят на всех наших серверах. \n У вас всего 5 минут для отправки данных иначе так же получите бан.")]
                public string PlCheckText { get; set; } = "Вы подозреваетесь в использовании читов/макросов. \n Вам необходимо пройти проверку. Напишите свой скайп или дискорд через команду: /contact. \n Если Вы покинете сервер, Вас забанят на всех наших серверах. \n У вас всего 5 минут для отправки данных иначе так же получите бан.";
                [JsonProperty(PropertyName = "Бан игрока при выходе с сервера во время проверки")]
                [DefaultValue(true)]
                public bool AutoBan { get; set; } = true;
                [JsonProperty(PropertyName = "Бан игрока при выходе игрока с сервера с причиной Kicked: Steam: InvalidAuthSession")]
                [DefaultValue(true)]
                public bool KickedSteamInvalidAuthSession { get; set; } = true;
                ...
                [JsonProperty(PropertyName = "Команда отправки контакта администратору")]
                [DefaultValue("contact")]
                public string CMDcontact { get; set; } = "contact";
                [JsonProperty(PropertyName = "Отправка контакта админу в ВК (при вызове на проверку консольной командой)")]
                [DefaultValue(false)]
                public bool admMsg { get; set; } = false;
                [JsonProperty(PropertyName = "Отправка контакта в беседу ВК (при вызове на проверку консольной командой)")]
                [DefaultValue(false)]
        ...
        void OnServerInitialized()
        {
            ...
            cmd.AddChatCommand(config.mainSet.CMDcontact, this, "Sendcontact");
        ...
        void OnPlayerDisconnected(BasePlayer player, string reason)
        {
            ...
            if (PlayersCheckList.ContainsValue(player))
            {
                ...
                foreach (var check in PlayersCheckList.Keys.ToList())
                {

                  
                    if (PlayersCheckList[check] == player)
                    {
                        if (BasePlayer.activePlayerList.Contains(check)) { PrintToChat($"<size=15>Игрок вызванный на проверку покинул сервер. Причина: <color=#049906>{reason}</color>.</size>"); }
                ...
                if (config.mainSet.KickedSteamInvalidAuthSession && reason == "Kicked: Steam: InvalidAuthSession")
                {
                    if (config.mainSet.BanCmd == "none")
                    {
                      player.IPlayer.Ban("Отказ от проверки Kicked: Steam: InvalidAuthSession");
                    }
                    else
                    {
                        string cmd = config.mainSet.BanCmd.Replace("{steamid}", player.UserIDString);
            ...
            if (PlayersCheckList.ContainsKey(player))
            {
                ...
                if (BasePlayer.activePlayerList.Contains(target)) { CuiHelper.DestroyUi(target, "AlertGUI"); PrintToChat(target, string.Format(GetMsg("АдминистраторОтключился", player))); }
        ...
        private void CheckPlayerCMD(ConsoleSystem.Arg arg)
        {
            ...
            if (CheckCMDPlayer != null)
            {
              PrintWarning($"Сначала закончите предыдущую проверку командой /{config.mainSet.CMDunalert}");
              return;
            }
            if (arg.Args == null || arg.Args.Length < 1)
            {
              PrintWarning($"Команда вызова на проверку из консоли: {config.mainSet.CMDalert} steamid/name");
              PrintWarning($"Команда вызова на проверку в чате: /{config.mainSet.CMDalert}");
              return;
            }
            var targets = FindPlayersOnline(arg.Args[0]);
            if (targets.Count <= 0)
            {
              PrintWarning($"Не удалось найти игрока {arg.Args[0]}");
              return;
            }
            if (targets.Count > 1)
            {
              PrintWarning($"Найдено несколько игроков:"); Puts(string.Join(", ", targets.ConvertAll(p => p.displayName).ToArray()));
              return;
            }
            var target = targets[0];
            if (PlayersCheckList.ContainsValue(target))
            {
              PrintWarning("Игрок уже на проверке.");
              return;
            }
            if (PlayersCheckList.ContainsKey(target))
            {
              PrintWarning("Этот игрок выполняет проверку, вы не можете сейчас его вызвать.");
              return;
            }
        ...
        private void Sendcontact(BasePlayer player, string cmd, string[] args)
        {
          if (CheckCMDPlayer != player && !PlayersCheckList.ContainsValue(player)) { PrintToChat(player, string.Format(GetMsg("НеНаПроверке", player))); return; }
            if (args.Length < 1) { PrintToChat(player, string.Format(GetMsg("КомандаКонтакт", player), config.mainSet.CMDcontact)); return; }
            string contact = args[0];
            string text = string.Format($"Игрок {player.displayName} прислал Вам свои контакты для проверки: {contact}", player.displayName, player.UserIDString);
            ...
            {
                ulong adminid = 0;
                foreach (var admin in PlayersCheckList.Keys)
                {
                    if (PlayersCheckList[admin] == player) adminid = admin.userID;
                }
                if (adminid == 0) return;
                var userVK = (string)VKBot?.Call("GetUserVKId", adminid);
        ...
        private void StopCheckPlayerCMD(ConsoleSystem.Arg arg)
        {
            ...
            if (CheckCMDPlayer == null)
            {
              PrintWarning("Нет игроков на проверке.");
              return;
            }
            if (BasePlayer.activePlayerList.Contains(CheckCMDPlayer))
            {
              CuiHelper.DestroyUi(CheckCMDPlayer, "AlertGUI");
              PrintWarning($"Вы закончили проверку игрока {CheckCMDPlayer.displayName}");
              CheckCMDPlayer = null;
            }
        }
        ...
        private void StartGUI(BasePlayer player)
        {
            ...
            container.Add(Panel("AlertGUI", "64 0 0 0.65", config.mainSet.GUIAnchorMin, config.mainSet.GUIAnchorMax));
        ...
        #region Main        
        private void StartCheck(BasePlayer admin, BasePlayer target)
        {
            if (PlayersCheckList.ContainsValue(target)) { PrintToChat(admin, string.Format(GetMsg("ИгрокУжеНаПроверке", admin))); return; }
            if (PlayersCheckList.ContainsKey(target)) { PrintToChat(admin, string.Format(GetMsg("ИгрокВыполняетПроверку", admin))); return; }
            if (CheckCMDPlayer == target) { PrintToChat(admin, string.Format(GetMsg("ИгрокУжеНаПроверке", admin))); return; }
            var userVK = (string)VKBot?.Call("GetUserVKId", admin.userID);
            if (userVK == null) { PrintToChat(admin, string.Format(GetMsg("НетВК", admin))); return; }
            PlayersCheckList.Add(admin, target);
            StartGUI(target);
            PrintToChat(admin, string.Format(GetMsg("ИгрокВызванНаПроверку", admin), target.displayName));
        }
        #endregion
        ...
        private void LoadMessages()
        {
            ...
            {
                {"ИгрокОтключился", "<size=15>Игрок вызванный на проверку покинул сервер. Причина: <color=#049906>{0}</color>.</size>"},
                {"АдминистраторОтключился", "<size=15>Администратор отключился от сервера, ожидайте следующей проверки.</size>"},
                {"НетПрав", "<size=15>У вас нет прав для использования данной команды</size>"},
                {"НетВК", "<size=15>Сначала добавьте и подтвердите свой профиль ВК командой /vk</size>" },
                {"ИгрокНеНайден", "<size=15>Игрок не найден</size>"},
                {"ВыНаПроверке", "<size=15>Вас вызвали на проверку, вы не можете использовать эту команду сейчас.</size>"},
                {"ПроверкаЗакончена", "<size=15>Вы закончили проверку игрока <color=#049906>{0}</color></size>"},
                {"ИгрокУжеНаПроверке", "<size=15>Этого игрока уже вызвали на проверку</size>"},
                {"НетПроверок", "<size=15>У вас нет активных проверок.</size>"},
                {"ИгрокВыполняетПроверку", "<size=15>Этот игрок выполняет проверку, вы не можете сейчас его вызвать.</size>"},
                {"ПроверкаНеЗакончена", "<size=15>Сначала закончите предыдущую проверку командой /{0}</size>"},
                {"ИгрокВызванНаПроверку", "<size=15>Вы вызвали игрока <color=#049906>{0}</color> на проверку</size>"},
                {"НеНаПроверке", "<size=15>Вас не вызывали на проверку. Приятной игры <color=#049906>:)</color></size>"},
                {"КонтактОтправлен", "<size=15>Ваши контактные данные были отправлены администратору. Ожидайте звонка.</size>"},
                {"КомандаКонтакт", "<size=15>Команда <color=#049906>/{0} Ваш_Ник</color></size>"},
                {"Сообщение администратору", "[CheckPlayers] Игрок {0} (steamcommunity.com/profiles/{1}/) предоставил контактные данные для проверки: {2}"}
            }, this);
        }
