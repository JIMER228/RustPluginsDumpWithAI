...
{
    [Info("HeliCommander", "discord.gg/9vyTXsJyKR", "0.3.4")]
    ...
    class HeliCommander : RustPlugin
    {
        ...
        private object CanConvertToPilotEject(BaseHelicopter baseHelicopter) => baseHelicopter != null && baseHelicopter.GetComponent<UH1YController>() != null ? (object)false : null;
        ...
        {
            ConfigData.BaseHelicopter config = type == HelicopterType.CH47 ? Configuration.CH47 :
                                               type == HelicopterType.Transport ? Configuration.Transport as ConfigData.BaseHelicopter :
                                               type == HelicopterType.Mini ? Configuration.Mini as ConfigData.BaseHelicopter : 
                                               Configuration.UH1Y;
                    ...
                    if (position != null)
                    {
                        ...
                        {                            
                            case HelicopterType.UH1Y:
                                {
                                    List<BaseHelicopter> entities = Facepunch.Pool.GetList<BaseHelicopter>();
                            ...
                            case HelicopterType.CH47:
                                {
                                    List<CH47Helicopter> entities = Facepunch.Pool.GetList<CH47Helicopter>();
                                    Vis.Entities((Vector3)position, 5f, entities);
                                    count = entities.Count;
                                    Facepunch.Pool.FreeList(ref entities);
                                    if (count > 0)
                            ...
                            case HelicopterType.Mini:
                                {
                                    List<MiniCopter> entities = Facepunch.Pool.GetList<MiniCopter>();
                                    Vis.Entities((Vector3)position, 5f, entities);
                                    count = entities.Count;
                                    Facepunch.Pool.FreeList(ref entities);
                                    if (count > 0)
        ...
        private List<ConfigData.BaseHelicopter.BuildOptions.BuildOption> GetBuildCosts(HelicopterType type)
        {
            switch (type)
            {
                case HelicopterType.UH1Y:
                ...
                case HelicopterType.Invalid:
                default:
                    return new List<ConfigData.BaseHelicopter.BuildOptions.BuildOption>();
            }
        }
        ...
        private abstract class BaseController : MonoBehaviour
        {
            ...
            internal virtual ConfigData.BaseHelicopter Config { get; }

            protected ConfigData.BaseHelicopter.MovementSettings Movement { get; set; }
            ...
            protected List<BaseVehicle.MountPointInfo> mountPoints = Pool.GetList<BaseVehicle.MountPointInfo>();

            protected List<BasePlayer> occupants = Pool.GetList<BasePlayer>();
            ...
            protected virtual void OnDestroy()
            {
                ...
                Pool.FreeList(ref mountPoints);
                Pool.FreeList(ref occupants);
            }
            ...
            internal virtual void OnPlayerMounted(BaseMountable baseMountable, BasePlayer player)
            {
                ...
                if (isDriver)
                {
                    ...
                    message = Type == HelicopterType.CH47 || Type == HelicopterType.Transport ? $"{string.Format(GetMessage("toggle_lights", player.userID), Configuration.Buttons.Lights)}\n" : string.Empty;
            ...
            internal void DropInventory()
            {
                ...
                Inventory.Drop("assets/prefabs/misc/item drop/item_drop.prefab", 
                    (Transform.position + new Vector3(0f, 1.5f, 0f)) + (UnityEngine.Random.onUnitSphere * UnityEngine.Random.Range(2f, 3f)), Quaternion.identity);
            }
            #endregion
            ...
            internal void ManageDamage(HitInfo info)
            {
                ...
                if (Type == HelicopterType.Mini || Type == HelicopterType.Transport)
                    return;
        ...
        private abstract class BaseController<T> : BaseController where T : BaseCombatEntity
        {
            ...
            {
                if (WaterLevel.Factor(Component.WorldSpaceBounds().ToBounds()) > 0.7f)
                {
                    StopToDie();
                    return;
                }
                ...
                if (IsEngineOn() && (Commander == null || (ConsumeFuel && !FuelSystem.HasFuel())))
                    EngineOff();
        ...
        private class MiniController : BaseController<MiniCopter>
        {
            ...
            internal override ConfigData.BaseHelicopter Config => Configuration.Mini;
            ...
            protected override void Awake()
            {
                ...
                Component.ApplyWheelForce(Component.frontWheel, 0f, 50, 0f);
                Component.ApplyWheelForce(Component.leftWheel, 0f, 50, 0f);
                Component.ApplyWheelForce(Component.rightWheel, 0f, 50, 0f);
            }
            ...
            protected override void InitializeSettings()
            {
                ...
                if (UseCustomControls)
                    BaseMountable.FixedUpdateMountables.Remove(Component);
            }
            #endregion
            ...
            {                
                FuelSystem = Component.engineController.FuelSystem;
            ...
            protected override void ApplyForceAtWheels()
            {
                ...
                Component.ApplyWheelForce(Component.frontWheel, acceleration, braking, turning);
                Component.ApplyWheelForce(Component.leftWheel, acceleration, braking, 0f);
                Component.ApplyWheelForce(Component.rightWheel, acceleration, braking, 0f);
            }
            ...
            protected override void OnFixedUpdate()
            {
                ...
                if (Component.continuousClippingCheck && Component.AnyMounted())
                {
                    Vector3 center = Transform.TransformPoint(Component.bounds.center);

                    int mask = (Component.IsFlipped() ? 1218511105 : 1210122497);
                    if (Physics.OverlapBox(center, Component.bounds.extents, Transform.rotation, mask).Length != 0)
                        Component.CheckSeatsForClipping(mask);
            ...
            internal override bool IsGrounded() => Component.Grounded();
        ...
        private class TransportController : MiniController
        {
            ...
            internal override ConfigData.BaseHelicopter Config => Configuration.Transport;
        ...
        private class CH47Controller : BaseController<CH47Helicopter>, SamSite.ISamSiteTarget
        {
            ...
            internal override ConfigData.BaseHelicopter Config => Configuration.CH47;
            ...
            public bool IsValidSAMTarget(bool staticRespawn) => !BaseVehicle.InSafeZone(Component.triggers, Component.transform.position);
        ...
        private class UH1YController : BaseController<BaseHelicopter>, SamSite.ISamSiteTarget
        {
            #region Variables       
            ...
            internal override ConfigData.BaseHelicopter Config => Configuration.UH1Y;
            ...
            protected override void OnDestroy()
            {
                ...
                if (Component != null && !Component.IsDestroyed)
                {
                    ...
                    if (Net.sv.write.Start())
                    {
                        ...
                        Net.sv.write.EntityID(Component.net.ID);
                        Net.sv.write.UInt8((byte)BaseNetworkable.DestroyMode.None);
                        Net.sv.write.Send(new SendInfo(BasePlayer.activePlayerList.Select(x => x.Connection).ToList()));
                    }
            ...
            private void FireMG(bool isLeft, Vector3 targetPos)
            {
                ...
                {                   
                    Vector3 muzzlePos = !isLeft ? (Component.right_gun_muzzle.transform.position + (Component.right_gun_muzzle.transform.forward * 0.25f)) : (Component.left_gun_muzzle.transform.position + (Component.left_gun_muzzle.transform.forward * 0.25f));
                    ...
                    else
                    {
                        ...
                        if (raycastHit.collider)
                        {
                            ...
                            if (baseEntity != null && baseEntity != Component)
                            {
        ...
        private static void CreateAimHelper(BasePlayer player)
        {
            ...
            var container = UI.Container(UI_AIM, "0 0 0 0", new UI4(0.4f, 0.4f, 0.6f, 0.6f));
        ...
        {           
            if (uh1yController.GetWeapons().Rocket.Enabled && uh1yController.GetWeapons().Rocket.RequireAmmo)
            {
                ...
                var container = UI.Container(UI_AMMO_ROCKET, UI.Color(opt.Color1, opt.Color1A), new UI4(opt.Xmin, opt.YMin, opt.XMax, opt.YMax));
        ...
        private void ForceMountPlayerTo(BasePlayer player, BaseMountable baseMountable, BaseController baseController)
        {
            ...
            player.MountObject(baseMountable, 0);
        ...
        private void cmdBuildHeli(BasePlayer player, string command, string[] args)
        {
            ...
            if (args[0].ToLower() == "costs")
            {
                ...
                foreach (ConfigData.BaseHelicopter.BuildOptions.BuildOption cost in Configuration.CH47.Build.Costs)
                {
                ...
                foreach (ConfigData.BaseHelicopter.BuildOptions.BuildOption cost in Configuration.UH1Y.Build.Costs)
                {
                    ItemDefinition itemDefinition = ItemManager.FindItemDefinition(cost.Shortname);
                    if (itemDefinition == null)
                        continue;
                ...
                foreach (ConfigData.BaseHelicopter.BuildOptions.BuildOption cost in Configuration.Mini.Build.Costs)
                {
                    ItemDefinition itemDefinition = ItemManager.FindItemDefinition(cost.Shortname);
                    if (itemDefinition == null)
                        continue;
                ...
                foreach (ConfigData.BaseHelicopter.BuildOptions.BuildOption cost in Configuration.Transport.Build.Costs)
                {
                    ItemDefinition itemDefinition = ItemManager.FindItemDefinition(cost.Shortname);
                    if (itemDefinition == null)
                        continue;
            ...
            List<ConfigData.BaseHelicopter.BuildOptions.BuildOption> costs = GetBuildCosts(type);

            foreach (ConfigData.BaseHelicopter.BuildOptions.BuildOption cost in costs)
            {
                ItemDefinition itemDefinition = ItemManager.FindItemDefinition(cost.Shortname);
                if (itemDefinition == null)
                    continue;                
        ...
        private void ccmdSpawnHeli(ConsoleSystem.Arg arg)
        {
            ...
            if (arg.Args.Length >= 4)
            {
                ...
                if (float.TryParse(arg.GetString(1), out x))
                {
                    if (float.TryParse(arg.GetString(2), out y))
                    {
                        if (float.TryParse(arg.GetString(3), out z))
                        {
        ...
        {
            ConfigData.BaseHelicopter.PassengerOptions passengers = type == HelicopterType.UH1Y ? Configuration.UH1Y.Passengers : type == HelicopterType.Mini ? Configuration.Mini.Passengers : type == HelicopterType.Transport ? Configuration.Transport.Passengers : Configuration.CH47.Passengers;
        ...
        private ulong[] GetOccupantIDs(BaseController helicopter)
        {
            ...
            return baseController.GetOccupants().Select(x => x.userID).ToArray();
        }
        ...
        public class ConfigData
        {
            ...
            public class DefaultHelicopter : BaseHelicopter
            {
        ...
        private ConfigData GetBaseConfig()
        {
            return new ConfigData
            {
                ...
                {
                    Build = new ConfigData.BaseHelicopter.BuildOptions
                    {
                        Enabled = false,
                        Cooldown = true,
                        Costs = new List<ConfigData.BaseHelicopter.BuildOptions.BuildOption>
                        {
                            new ConfigData.BaseHelicopter.BuildOptions.BuildOption
                            {
                                Amount = 500,
                    ...
                    Damage = new ConfigData.BaseHelicopter.DamageOptions
                    {
                        Health = 7500,
                    ...
                    Inventory = new ConfigData.BaseHelicopter.InventoryOptions
                    {
                        DropInv = true,
                        DropLoot = false,
                        Enabled = true,
                    ...
                    Movement = new ConfigData.BaseHelicopter.MovementSettings
                    {       
                        PitchTilt = 30f,
                        Turn = 24f,
                        TurnTilt = 6f,
                    ...
                    Passengers = new ConfigData.BaseHelicopter.PassengerOptions
                    {
                        Enabled = true,
                        UseClans = true,
                        UseFriends = true,
                    ...
                    Fuel = new ConfigData.BaseHelicopter.FuelOptions
                    {
                        Enabled = true,
                        Consumption = 0.25f,
                    ...
                    Repair = new ConfigData.BaseHelicopter.RepairSettings
                    {
                        Amount = 10,
                        Damage = 30,
                        Enabled = true,
                    ...
                    Spawnable = new ConfigData.BaseHelicopter.SpawnableOptions
                    {
                        Enabled = true,
                        Max = 2,
                        Time = 1800,
        ...
        private class RestoreData
        {
            ...
            private Hash<uint, HelicopterData> restoreData = new Hash<uint, HelicopterData>();

            internal void AddData(BaseController controller)
            {
                restoreData[controller.GetEntity().net.ID] = new HelicopterData(controller);
            }

            internal void RemoveData(uint netId)
            {
                if (HasRestoreData(netId))
                    restoreData.Remove(netId);                
            }

            internal bool HasRestoreData(uint netId) => restoreData.ContainsKey(netId);
            ...
            {                
                if (restoreData.Count > 0)
                {
                    ...
                    {
                        KeyValuePair<uint, HelicopterData> helicopterData = restoreData.ElementAt(i);
            ...
            private class HelicopterData
            {
                ...
                {
                    ConfigData.BaseHelicopter config = baseController is UH1YController ? Configuration.UH1Y : 
                                                       baseController is TransportController ? Configuration.Transport : 
                                                       baseController is MiniController ? Configuration.Mini : 
                                                       Configuration.CH47 as ConfigData.BaseHelicopter;
