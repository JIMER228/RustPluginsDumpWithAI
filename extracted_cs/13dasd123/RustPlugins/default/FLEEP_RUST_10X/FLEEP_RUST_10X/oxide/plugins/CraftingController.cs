...
{
    [Info("Crafting Controller", "https://discord.gg/9vyTXsJyKR", "3.2.5")]
    ...
    public class CraftingController : RustPlugin
    {
        ...
        protected override void LoadConfig()
        {
            ...
            try
            {
                ...
                //foreach (var key in config.ToDictionary().Keys) configkeys.Add(key);
                //int i = -1;
                //foreach(var j in Config)
                //{
                //    i++;
                //    if (j.Key == configkeys[i]) continue;
                //    PrintError("Configuration appears to be outdated; updating and saving");
                //    SaveConfig();
                //    break;
                //}
            }
            ...
            {
                PrintError($"Configuration file {Name}.json is invalid; using defaults");
                LoadDefaultConfig();
            }
        }
        ...
        {
            PrintError($"Configuration changes saved to {Name}.json");
        ...
        private void OnServerInitialized()
        {
            ...
            List<ItemBlueprint> bplist = new List<ItemBlueprint>();
        ...
        private object OnItemCraft(ItemCraftTask task, BasePlayer crafter)
        {
            ...
            foreach (var bonusperm in permissionsBonusMultiplier)
            {
                ...
                task.blueprint.time *= (float)(config.BonusMultiplier[bonusperm.Split('.')[1].ToString()] / 100);
                break;
            }
