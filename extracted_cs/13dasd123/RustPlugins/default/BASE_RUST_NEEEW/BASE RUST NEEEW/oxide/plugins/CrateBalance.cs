...
{
    [Info("CrateBalance", "Oxide", "1.0.0")]
    class CrateBalance : RustPlugin
    {
        #region Cui builder
        ...
        private const string Layer = "ui.CrateBalance.bg";
        ...
        {
            Item item = ItemManager.CreateByName("glue", 60, 1848686990);
            item.name = "УРАН";
        ...
        private void UI_Open(BasePlayer p)
        {
            ...
            var container = new CuiElementContainer();
            container.Add(Button(Layer + ".close", "Overlay", "CrateBalance_close", HexToRustFormat(cfg.uISettings.colorBG), "0.9831251 0.4677778", "0.99875 0.578889"));
        ...
        private void UI_Close(BasePlayer p)
        {
            ...
            var container = new CuiElementContainer();
            container.Add(Button(Layer + ".open", "Overlay", "CrateBalance_open", HexToRustFormat(cfg.uISettings.colorBG), "0.9831251 0.4677778", "0.99875 0.578889"));
        ...
        #region Commands
        [ConsoleCommand("CrateBalance_open")]
        ...
        private void cmdNextCrate(BasePlayer p)
        {
            if (cratePosition == Vector3.zero)
                return;
            p.ChatMessage($"Следующий крейт через: {TimeSpan.FromSeconds(nextCrate - CurrentTime())}");
            Chguitimebitch(p);
        }

        [ConsoleCommand("CrateBalance_close")]
