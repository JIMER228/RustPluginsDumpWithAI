...
{
    [Info("MLRS Hellfire", "Timm3D", "1.0.5")]
    ...
    internal class MLRSHellfire : RustPlugin
    {
        ...
        class PluginConfig
        {
            ...
            [JsonProperty("MLRSFireInterval in milliseconds")]
            public float MLRSFireInterval { get; set; }

            [JsonProperty("RemoteMLRSFireInterval in milliseconds")]
            public float RemoteMLRSFireInterval { get; set; }

            [JsonProperty("HellfireFireInterval in milliseconds")]
        ...
        private const string Error_MLRS_NoMapMark = "</color><color=#F70000>No map mark set. Please mark a point on the map to attack.</color>\n" +
            "Type <color=#E9AC3C>/mlrs</color> for help.";
                ...
                case "p":
                    {
                        ...
                        var targetPlayer = GetTargetPlayer(args[1]);

                        if (targetPlayer != null)
                            targetPosition = targetPlayer.ServerPosition;
                        else
                        {
                            ChatReply(player, lang.GetMessage("Error_MLRS_CantFindPlayer", this, player.UserIDString));
                            return;
                        }
                ...
                case "map":
                    {
                        ...
                        if (player.ServerCurrentMapNote != null)
                            targetPosition = player.ServerCurrentMapNote.worldPosition;
                        else
                        {
                ...
                case "map":
                    {
                        ...
                        {
                            ChatReply(player, lang.GetMessage("Error_RemoteMLRS_NoMapMark", this, player.UserIDString));
                            return;
                        }
                ...
                case "p":
                    {
                        ...
                        {
                            ChatReply(player, lang.GetMessage("Error_HellFire_CantFindPlayer", this, player.UserIDString));
                            return;
                        }
                ...
                case "map":
                    {
                        ...
                        {
                            ChatReply(player, lang.GetMessage("Error_HellFire_NoMapMark", this, player.UserIDString));
                            return;
                        }
        ...
        private Vector3 GetAimToTarget(Vector3 startPosition, Vector3 targetPos, out float baseGravity)
        {
            ...
            baseGravity = MLRS.ProjectileDistToGravity(Mathf.Max(num2, 50f), y, num5, num);
        ...
        private BasePlayer GetTargetPlayer(string playername)
        {
            foreach (var targetPlayer in BasePlayer.activePlayerList.ToArray())
                if (targetPlayer.displayName.ToLower().Equals(playername.ToLower()))
                    return targetPlayer;
