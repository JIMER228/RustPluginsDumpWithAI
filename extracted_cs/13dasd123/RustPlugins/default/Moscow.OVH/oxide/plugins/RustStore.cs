    ...
    public class RustStore : RustPlugin
    {
        ...
        public class CStoreConfig
        {
            private readonly Dictionary<string, string> _messages = new Dictionary<string, string>()
            {
                ...
                ["SLOTS.RELEASE"] = "Освободите <b>{SLOTS}<\b>",
        ...
        public static void AddLog(BasePlayer player, string text)
        {
            if (Logs[player.userID].Count >= 4)
            {
                Logs[player.userID].RemoveAt(0);
            } 
            Logs[player.userID].Add(text);
        } 
        ...
        public static class CConsoleComands
        {
            ...
            public static bool StoreGive(ConsoleSystem.Arg arg)
            {
                ...
                GiveStoreItem(player, arg.GetString(0),
                    () =>
                    {
                        CuiHelper.DestroyUi(player, CGuiComponents.MainPanelName + arg.GetInt(1) + "D");
                        ...
                        {
                            {
                                new CuiButton
                                {
                                    ...
                                    Button        = {Color     = "0.4627 0.49 0.27 0.8"},
                                    Text          = {Text      = "УСПЕШНО\n" +
                                            "ПОЛУЧЕНО", Align = TextAnchor.MiddleCenter, FontSize = 22, Color = "0.929 0.882 0.847 0.4"}
                                },
                                CGuiComponents.MainPanelName + arg.GetInt(1), CGuiComponents.MainPanelName + arg.GetInt(1) + "D"
                            }
                        });
                        DrawLogs(player);
                    },
                    ...
                    {
                        string er = "ОШИБКА\nПОЛУЧЕНИЯ";
                        if (error.Contains("освободите"))
                        {
                            er = "НЕ\nХВАТАЕТ\n" +
                                 "МЕСТА";
                            AddLog(player, $"<size=15>Вам не хватает места ({error.Replace("sadasd", "")})</size>");
                        }

                        if (error.Contains("шкаф"))
                        {
                            er = "ЧУЖОЙ\nШКАФ";
                            AddLog(player, "Вы находитесь в зоне чужого шкафа");
                        }
                        ...
                        {
                            {
                                ...
                                {
                                    RectTransform = {AnchorMin = "0 0", AnchorMax = "1 1", OffsetMax = "0 0"},
                                    Button        = {Color     = "0.549 0.270 0.215 0.8"},
                                    Text = {Text = er, Align = TextAnchor.MiddleCenter, FontSize = 22, Color = "0.929 0.882 0.847 0.4"}
                                },
                                CGuiComponents.MainPanelName + arg.GetInt(1), CGuiComponents.MainPanelName + arg.GetInt(1) + "D"
                            }
                        });
        ...
        private string GetAmount(BasePlayer player)
        { 
            ...
            return storeCache.CachedCart.Where(p => !p.Value.isGived()).Count().ToString(); 
        }
        ...
        [ConsoleCommand("storeSecret")]
        private void sadsad(ConsoleSystem.Arg arg)
        {
            OpenStore(arg.Player());
        } 
        ...
        private void Storecmd(BasePlayer player, string command, string[] args)
        {
            if (args.Length == 0)
            {
                ...
                player.SendConsoleCommand("UI_RM_Handler choose 0");   
                return;
            }
        ...
        private static void GiveStoreItem(BasePlayer player, CItemJson itemJson, Action onGived, Action<string> onFail)
        {
            ...
            switch (itemJson.type)
            {
                ...
                case EItemType.GameItem:
                    ...
                    {
                        onFail(GetLangMessage(userId, "STORE.ERROR"));
                        AddLog(player, "Ошибка соединения с магазином");
                        return;
                    }
                    ...
                    if (needSlots > 0)
                    {
                        onFail("освободите " + FormatString(needSlots,
                    ...
                    RequestItemGive(userId, itemJson, () =>
                    {
                        Action<int> giveItemFunc = (giveAmount) =>
                        {
                            ...
                            AddLog(player, $"<size=15>Товар <b>{giveItem.info.displayName.english} (x{giveItem.amount})</b> успешно забран!</size>");
                            DrawLogs(player); 
        ...
        [ConsoleCommand("closestore")]
        void closeuimain(ConsoleSystem.Arg args)
        {
            BasePlayer player = args.Player();
            player.SendConsoleCommand("closemenu");
            CGui.Destroy(player.net.connection, CGuiComponents.ItemPatternName);
            CGui.Destroy(player.net.connection, CGuiComponents.MainPanelName);
        ...
        public static void GiveAllStoreItems(BasePlayer player)
        {
            ...
            if (storeItem == null)
            {
                ...
                Instance.timer.Once(1, () => { OpenStore(player); }); 
                return;
            }

            GiveStoreItem(player, storeItem,
                () => { GiveAllStoreItems(player); DrawLogs(player); ShowCart(player, 0); }, 
                (error) =>
                { 
                    storeCache.MarkGivedAll();
                    CGui.Destroy(player.net.connection, CGuiComponents.ModalPanelName);
        ...
        [ConsoleCommand("storegiveall")]
        private void cmsad(ConsoleSystem.Arg arg)
        {
        ...
        public static void ShowCart(BasePlayer player, int page)
        {
            ...
            if (playerItemsCount == 0)
            {
                ...
                {
                    RectTransform = { AnchorMin = "0.5 0.95", AnchorMax = "0.5 0.95", OffsetMin = $"{-500} {-120}", OffsetMax = $"{500} {70}" },
                    Text = { Text = "Здесь находятся ваши покупки с сайта <b>bloodrust.ru</b>, для того\n" +
                            "чтобы забрать предмет - нажмите на него", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", Color = "1 1 1 0.5", FontSize = 18 }
                }, CGuiComponents.MainPanelName); */
                   
                container.Add(new CuiLabel 
                {
                    RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = $"{-500} {-120}", OffsetMax = $"{500} {120}" },
                    Text = { Text = "<b><size=32>ВАША КОРЗИНА ПУСТА</size></b>\n" +
                            "Приобрести что-либо можно здесь: <b>bloodrust.ru</b>" +
                            "", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", Color = "0.929 0.882 0.847 0.5", FontSize = 24 }
                }, CGuiComponents.MainPanelName);
                container.Add(new CuiButton
                {
                    RectTransform = {AnchorMin = "0 0", AnchorMax = "1 1", OffsetMax                      = "0 0"},
                    Button        = { Color    = "0 0 0 0", Close = CGuiComponents.MainPanelName, Command = "closemenu"},
                    Text          = {Text      = ""}
                }, CGuiComponents.MainPanelName);
            ...
            var firsItemIndex = page * 24;
            var nextPage = page;
            var prevPage = page > 1 ? page - 1 : 0;

            var max = firsItemIndex + 24;
            ...
            if (playerItemsCount - firsItemIndex == 0)
            {
                page--;
            }
            ...
            //UI_RecountPosition(ref xSwitch, ref ySwitch, 24, 32); 
            
            /*container.Add(new CuiPanel
            {
                RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = $"{xSwitch} {ySwitch - ((float) ItemSide)}", OffsetMax = $"{xSwitch + ItemSide} {ySwitch}" },
            ...
            if (page == 0)
            {
                ...
                /*container.Add(new CuiLabel 
                {
                    RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin         = $"{-500} {275}", OffsetMax       = $"{500} {325}" },
                    Text          = { Text      = "Здесь находятся ваши покупки с сайта <b>bloodrust.ru</b>, для того\n" +
                            "чтобы забрать предмет - нажмите на него", Align      = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", Color = "1 1 1 0.5", FontSize = 18 }
                }, CGuiComponents.MainPanelName); */
            }
            ...
            if (player.IsBuildingBlocked())
            {
                ...
                {
                    RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = $"{-500} {-120}", OffsetMax = $"{500} {120}" },
                    Text = { Text = "<b><size=32>ВЫ НА ТЕРРИТОРИИ ЧУЖОГО ШКАФА</size></b>\n" +
                                    "Выйдите чтобы получить купленные предметы!" +
                                    "", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", Color = "0.929 0.882 0.847 0.5", FontSize = 24 }
                }, CGuiComponents.MainPanelName);
                container.Add(new CuiButton
                {
            ...
            CuiHelper.DestroyUi(player, "ALL");
            CuiHelper.DestroyUi(player, "LEFT");
            CuiHelper.DestroyUi(player, "RIGHT");
            container.Add(new CuiButton
            {
                RectTransform = {AnchorMin = "0 0", AnchorMax = "1 1", OffsetMax                      = "0 0"},
                Button        = { Color    = "0 0 0 0", Close = CGuiComponents.MainPanelName, Command = "closemenu"},
            ...
            container.Add(new CuiButton
            {
                RectTransform = { AnchorMin = "0.5 0", AnchorMax = "0.5 0", OffsetMin = $"5 15", OffsetMax = $"{400} {60}" },
                Button        = { Color     = "0.294 0.274 0.254 1", Command     = "store.giveitemall" },
                Text          = { Text      = "ЗАБРАТЬ ВСЁ", Align             = TextAnchor.MiddleCenter, Font = "robotocondensed-bold.ttf", Color = "0.929 0.882 0.847 0.8", FontSize = 30 }
            }, CGuiComponents.MainPanelName, "ALL");  
             
            container.Add(new CuiButton
            {
                RectTransform = { AnchorMin = "0.5 0", AnchorMax = "0.5 0", OffsetMin = $"5 65", OffsetMax = $"{197.5} {100}" },
                Button        = { Color     = page == 0 ? "0.284 0.38 0.168 0.5" : "0.284 0.38 0.168", Command     = page - 1 < 0 ? "" : $"store.refreshcart {page -1}" },
                Text          = { Text      = "НАЗАД", Align             = TextAnchor.MiddleCenter, Font = "robotocondensed-bold.ttf", Color =  page == 0 ? "0.647 0.917 0.188 0.5" : "0.647 0.917 0.188 1", FontSize = 24 }
            }, CGuiComponents.MainPanelName, "LEFT"); 
                
            //UI_RecountPosition(ref xSwitch, ref ySwitch, 31, 32); 
            ...
            container.Add(new CuiButton 
            {
                RectTransform = { AnchorMin = "0.5 0", AnchorMax = "0.5 0", OffsetMin = $"205 65", OffsetMax = $"{400} {100}" },
                Button        = { Color     = nextPage == page ? "0.284 0.38 0.168 0.5" : "0.284 0.38 0.168", Command     = nextPage == page ? "" : $"store.refreshcart {page + 1}" },
                Text          = { Text      = "ВПЕРЁД", Align             = TextAnchor.MiddleCenter, Font = "robotocondensed-bold.ttf", Color =  nextPage == page ? "0.647 0.917 0.188 0.5" : "0.647 0.917 0.188 1", FontSize = 24 }
            }, CGuiComponents.MainPanelName, "RIGHT"); 

            for (int i = 0; i < 24; i++)
                CuiHelper.DestroyUi(player, CGuiComponents.MainPanelName + i);
            
            for (var i = firsItemIndex; i < max; i++)
            { 
                UI_RecountPosition(ref xSwitch, ref ySwitch, i - firsItemIndex, max - firsItemIndex);

                if (playerItems.Length < i + 1)
                {
                    container.Add(new CuiPanel
                    {
                        RectTransform = { AnchorMin = "0.5 0.53", AnchorMax = "0.5 0.53", OffsetMin = $"{-5 + xSwitch} {ySwitch - ((float) ItemSide)}", OffsetMax = $"{-5 +xSwitch + ItemSide} {ySwitch}" },
                        Image         = { Color     = "0.294 0.274 0.254 0.5" }
                    }, CGuiComponents.MainPanelName, CGuiComponents.MainPanelName + (i - firsItemIndex));
                    
                    container.Add(new CuiLabel
                    {
                        RectTransform = {AnchorMin = "0 0", AnchorMax      = "1 1", OffsetMin                  = "0 0", OffsetMax = "0 0"},
                        Text          = {Text      = "", Align = TextAnchor.MiddleCenter, FontSize = 22, Color        = "1 1 1 0.1"}
                    }, CGuiComponents.MainPanelName + (i - firsItemIndex), CGuiComponents.MainPanelName + (i - firsItemIndex) + ".D");
                ...
                {
                    RectTransform = { AnchorMin = "0.5 0.53", AnchorMax = "0.5 0.53", OffsetMin = $"{-5 + xSwitch} {ySwitch - ((float) ItemSide)}", OffsetMax = $"{-5 +xSwitch + ItemSide} {ySwitch}" },
                    Image = { Color = "0.294 0.274 0.254 1" }
                }, CGuiComponents.MainPanelName, CGuiComponents.MainPanelName + (i - firsItemIndex));
                ...
                if (info.type == EItemType.GameItem || info.type == EItemType.Blueprint)
                {
                    ...
                    //Interface.Oxide.LogWarning("123");
                    if (info.type == EItemType.Blueprint)
                    {
                        container.Add(new CuiElement
                        { 
                            Parent = CGuiComponents.MainPanelName + (i - firsItemIndex),
                            Components =
                            {
                                new CuiRawImageComponent {Png             = _guiComponents.ItemPatternBlueprintImage, Color = "1 1 1 1"},
                    ...
                    container.Add(new CuiElement
                    {
                        ...
                        Components =
                        {
                            new CuiRawImageComponent {Png = (string) Instance.plugins.Find("ImageLibrary").Call("GetImage", gameItem.itemName) },
                    ...
                    container.Add(new CuiLabel 
                    {
                        RectTransform = {AnchorMin = "0 0", AnchorMax                    = "1 1", OffsetMin = "0 0", OffsetMax = "0 -5"},
                        Text          = {Text      = ItemManager.FindItemDefinition(gameItem.itemName).displayName.english, Align = TextAnchor.UpperCenter, FontSize = 14, Font = "robotocondensed-regular.ttf", Color = "1 1 1 0"}
                    }, CGuiComponents.MainPanelName + (i - firsItemIndex));

                    container.Add(new CuiLabel
                    {
                        RectTransform = {AnchorMin = "0 0", AnchorMax                    = "1 1", OffsetMin = "0 5", OffsetMax = "-7.5 0"},
                        Text          = {Text      = "x" + gameItem.quantity.ToString(), Align = TextAnchor.LowerRight, FontSize = 18, Font = "robotocondensed-regular.ttf", Color = "0.929 0.882 0.847 0.4"}
                    }, CGuiComponents.MainPanelName + (i - firsItemIndex));
                }
                else
                {
                    ...
                    if (_imageCache.GetCachedImage(iconUrl, out cachedImageId))
                    {
                        ...
                        { 
                            ...
                            Components =
                            {
                                new CuiRawImageComponent {Png             = cachedImageId, Color   = "1 1 1 1"},
                                new CuiRectTransformComponent { AnchorMin = "0 0", AnchorMax = "1 1", OffsetMin = "5 5", OffsetMax = "-5 -5"}
                            }
                        });
                    }
                    else
                    {
                        ...
                        {
                            RectTransform = {AnchorMin = "0 0", AnchorMax                          = "1 1", OffsetMin                = "0 0", OffsetMax = "0 0"},
                            Text          = {Text      = "НЕТ КАРТИНКИ", Align = TextAnchor.MiddleCenter, FontSize = 22, Color        = "1 1 1 0.3"}
                        }, CGuiComponents.MainPanelName + (i - firsItemIndex), CGuiComponents.MainPanelName + (i - firsItemIndex) + ".D");
                        
                        string name = CGuiComponents.MainPanelName + (i - firsItemIndex);
                        int ids = i - firsItemIndex;
                        _imageCache.CacheImage(iconUrl, id =>
                        { 
                            CuiHelper.DestroyUi(player, name + ".D");
                            CuiElementContainer newContainer = new CuiElementContainer();
                            
                            string newCachedImageId; 
                            if (_imageCache.GetCachedImage(iconUrl, out newCachedImageId))
                            {
                                //Interface.Oxide.LogWarning("123");
                                if (info.type == EItemType.Blueprint)
                                {
                                    newContainer.Add(new CuiElement
                                    { 
                                        Parent = name,
                                        Components =
                                        {
                                ...
                                { 
                                    ...
                                    Components =
                                    {
                                        new CuiRawImageComponent {Png             = newCachedImageId, Color = "1 1 1 1"},
                                        new CuiRectTransformComponent { AnchorMin = "0 0", AnchorMax     = "1 1", OffsetMin = "5 5", OffsetMax = "-5 -5"}
                                    }
                                });
                                 
                                newContainer.Add(new CuiButton
                                {
                                    RectTransform = { AnchorMin = "0 0", AnchorMax   = "1 1", OffsetMin = $"{0} {0}", OffsetMax = $"{0} {0}" },
                                    Button        = { Color     = "0 0 0 0", Command = $"store.giveitem {info.queueID} {ids}" },
                                    Text          = { Text      = "", Align          = TextAnchor.MiddleCenter, Font = "robotocondensed-bold.ttf", Color = nextPage == page ? "1 1 1 0.1": "1 1 1 0.7", FontSize = 80 }
                                }, name); 
                            }
                            
                            CuiHelper.AddUi(player, newContainer); 
                        }); 
                    } 
                } 
                ...
                {
                    RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1", OffsetMin = $"{0} {0}", OffsetMax = $"{0} {0}" },
                    Button        = { Color     = "0 0 0 0", Command     = $"store.giveitem {info.queueID} {i - firsItemIndex}" },
                    Text          = { Text      = "", Align             = TextAnchor.MiddleCenter, Font = "robotocondensed-bold.ttf", Color = nextPage == page ? "1 1 1 0.1": "1 1 1 0.7", FontSize = 80 }
                }, CGuiComponents.MainPanelName + (i - firsItemIndex));

                string fhgjhfjhfg; 
                if (_imageCache.GetCachedImage(iconUrl, out fhgjhfjhfg))
                {
        ...
        public static int ItemOnString = 6; 
        [JsonProperty("Количество строк в интерфейсе")]
        public static int StringAmount = 4;
        [JsonProperty("Сторона одного предмета")]
        public static int ItemSide = 125; 
        [JsonProperty("Отступ между предметами")]
        public static int ItemMargin = 10;
        
        private static void UI_RecountPosition(ref double xSwitch, ref double ySwitch, int count, int max)
        {
        ...
        private static Dictionary<ulong, List<string>> Logs = new Dictionary<ulong, List<string>>();

        public static void DrawLogs(BasePlayer player)
        {
            CuiElementContainer container = new CuiElementContainer();
            CuiHelper.DestroyUi(player, "LogContent");

            string result = "";
            foreach (var check in Logs[player.userID])
                result += $"- {check}\n";
            ...
            {
                RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1", OffsetMax = "0 0" },
                Text = { Text = result, Align = TextAnchor.UpperLeft, Font = "robotocondensed-regular.ttf", FontSize = 18, Color = "0.93 0.89 0.85 0.5" }
            }, "LogHandler", "LogContent");

            CuiHelper.AddUi(player, container);
        }
        public static void OpenStore(BasePlayer player)
        { 
            if (!Logs.ContainsKey(player.userID))
                Logs.Add(player.userID, new List<string>());   
            ...
            { 
                CursorEnabled = true,
                RectTransform = {AnchorMin = "-0.02 0", AnchorMax = "0.95 1", OffsetMin = "0 0", OffsetMax = "0 0"},
                Image = { Color = "0.117 0.121 0.109 0.90" } 
            }, "UI_RustMenu_Internal", CGuiComponents.MainPanelName);

            container.Add(new CuiPanel
            {
                RectTransform = {AnchorMin = "0.5 0.9", AnchorMax = "0.5 1", OffsetMin = "-400 5", OffsetMax = "400 -30"},
                Image = {Color = "0.294 0.274 0.254 1"}
            }, CGuiComponents.MainPanelName, "StoreHeader");

            container.Add(new CuiPanel
            {
                RectTransform = {AnchorMin = "0.5 0", AnchorMax = "0.5 0", OffsetMin = "-400 10", OffsetMax = "-10 100"},
                Image = {Color = "0.4 0.4 0.4 0"}
            }, CGuiComponents.MainPanelName, "LogHandler");
            ...
            {
                RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1", OffsetMax = "0 0" },
                Text = { Text = "КОРЗИНА", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-bold.ttf", FontSize = 22, Color = "0.929 0.882 0.847 0.8"}
            }, "StoreHeader");
        ...
        [ChatCommand("JOPA")]
        private void sadas(BasePlayer player) => SendErrorMessage(player, "ASDAS");
