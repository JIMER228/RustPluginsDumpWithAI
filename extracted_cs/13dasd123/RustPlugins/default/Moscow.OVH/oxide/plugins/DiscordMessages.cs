...
{
    [Info("DiscordMessages", "Slut", "2.1.3")]
    class DiscordMessages : CovalencePlugin
    {
        ...
        private void OnUserChat(IPlayer player, string message)
        {
            ...
            if (!player.HasPermission(AdminPermission)) message = message.Replace("@everyone", "@ everyone").Replace("@here", "@ here");
            Configuration.Chat chatConfig = GetFeatureConfig<Configuration.Chat>(FeatureType.PlayerChat);
        ...
        private void MessageCommand(IPlayer player, string command, string[] args)
        {
            ...
            {
                DateTime? messageCooldown = _data.Players[player.Id].MessageCooldown;
            ...
            public void Send()
            {
                _instance.webrequest.Enqueue(_url,_payload , (code, rawResponse) =>
                {
                    ...
                    if (response.IsRatelimit)
                    {
                        ...
                        handler.AddMessage(ratelimit, this);
                    }
                    if (response.IsBad)
                    {
        ...
        private class RateLimitHandler
        {
            ...
            private void RateTimerHandler()
            {
                while (true)
                {
                    ...
                    Request request = Messages.Dequeue();
        ...
        private void Init()
        {
            ...
            foreach (KeyValuePair<FeatureType, Configuration.WebhookObject> feature in _config.FeatureTypes)
            {
        ...
        {
            EmbedBuilder embed = JsonConvert.DeserializeObject<EmbedBuilder>(embedJsonString);
            FancyMessage payload = new FancyMessage()
                .SetEmbed(embed)
                .WithContent(content);
        ...
        private void API_SendFancyMessage(string webhookUrl, string embedName, int embedColor, string json, string content = null, Plugin plugin = null)
        {
            EmbedBuilder builder = new EmbedBuilder()
                .WithTitle(embedName)
                .SetColor(embedColor);
            foreach (EmbedBuilder.Field field in JsonConvert.DeserializeObject<EmbedBuilder.Field[]>(json)) builder.AddField(field);
            FancyMessage payload = new FancyMessage()
                .SetEmbed(builder)
                .WithContent(content);
            Request.Send(webhookUrl, payload, plugin);
        ...
        private void ReportCommand(IPlayer player, string command, string[] args)
        {
            ...
            {
                DateTime? reportCooldown = _data.Players[player.Id].ReportCooldown;
