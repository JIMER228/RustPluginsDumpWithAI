...
namespace Oxide.Plugins
{
    ...
    internal class TimedPermissions : CovalencePlugin
    {
        ...
        private string GetLeftTime(BasePlayer player, string permission)
        {
            Player permPlayer = Player.Get(player.UserIDString);
            if (permPlayer != null)
            {
                TimedAccessValue tp = permPlayer.GetTimedGroup(permission);
                if (tp == null)
                    return "КУПИТЕ";
                if (tp.Expired)
                    return "КУПИТЕ";
                ...
                {
                    var leftTime = tp.ExpireDate.Subtract(DateTime.Now);
                    return leftTime.ToShortString();
                }
            }
