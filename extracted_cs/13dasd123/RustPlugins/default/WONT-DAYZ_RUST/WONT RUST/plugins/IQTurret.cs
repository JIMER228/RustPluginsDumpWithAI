...
namespace Oxide.Plugins
{
    ...
    internal class IQTurret : RustPlugin
    {
        ...
        private void UnloadPlugin()
        {
            ...
            foreach (List<ControllerInformation> TurretInformation in TurretList.Values)
                foreach (ControllerInformation controllerInformation in TurretInformation)
                {
                    ...
                    if (turrel != null)
                    {
                        if (turrel is AutoTurret && turrel.HasFlag(BaseEntity.Flags.On) && (turrel as AutoTurret).currentEnergy == 0)
                            turrel.SetFlag(BaseEntity.Flags.On, false);

                        if (turrel is SamSite && turrel.HasFlag(BaseEntity.Flags.Reserved8) && (turrel as SamSite).currentEnergy == 0)
        ...
        {
            if (turrel == null || !IDsPrefabs.Contains(turrel.prefabID) && TurretList.ContainsKey(turrel.skinID)) return;
        ...
        protected override void LoadConfig()
        {
            ...
            {
                PrintWarning(LanguageEn ? $"Error #847195 configuration readings 'oxide/config/{Name}', creating a new configuration!" : $"Ошибка #85 чтения конфигурации 'oxide/config/{Name}', создаём новую конфигурацию!");
                LoadDefaultConfig();
            }
            NextTick(SaveConfig);
        }
        ...
        /// Обновление 1.0.х
        /// Исрпавления :
        /// - Исправлен рендер рубильника при перезагрузке сервера (тумблер становился невидимым)
        /// - Исправил проверку на рубильники на РТ
        /// Изменения : 
        /// - Значительно упростил инициализацию плагина
        /// - Изменил метод установки тумблеров
        /// - Малозначимые корректировки
        /// Нововведения :
        /// - Добавлена блокировка слотов электричества на тумблере
        /// - Добавлено сохранение статуса запущенных турелей
        ...
        [ConsoleCommand("trrrrrrrrrfdfsrr")]
        ...
        object OnSwitchToggle(IOEntity entity, BasePlayer player)
        {
            ...
            if (config.LimitController.UseLimitControll)
            {
                ...
                if (IsLimitPlayer(player.userID, ID))
                {
        ...
        private new void LoadDefaultMessages()
        {
            ...
            PrintWarning("Logs : #3212 | Языковой файл загружен успешно");
        }
        ...
        private void TurretToggle(BasePlayer player, ElectricSwitch electricSwitch)
        {
            ...
            if (config.LimitController.UseLimitControll)
            {
                ...
                Int32 LimitCount = (GetLimitPlayer(player.userID) - GetAmountTurretPlayer(player.userID, ID));
