...
{
    [Info("TopKits", "Developer x ", "1.0.0")]
    class TopKits : RustPlugin
    {
        #region Вар
        ...
        private void CommandConsoleKit(ConsoleSystem.Arg arg)
        {
            ...
            if (check.Cooldown > 0)
            {
                ...
                if (data.Cooldown > currentTime)
                {
                    ...
                    CuiHelper.DestroyUi(player, $"{check.Name}.Take");
                    CuiHelper.DestroyUi(player, $"{check.Name}.Times");
        ...
        private bool GiveKit(BasePlayer player, string kitname)
        {
            ...
            if (kit.Cooldown > 0)
            {
                ...
                if (data.Cooldown > currentTime)
                {
                    SendReply(player, $"Вы сможете использовать этот комплект через <color=#ee3e61>{FormatTime(TimeSpan.FromSeconds(data.Cooldown - currentTime))}</color>");
                    return false;
                }
            }
            ...
            SendReply(player, $"Вы успешно получили набор <color=#ee3e61>{kit.DisplayName}</color>");
            return true;
        }
        ...
        private void KitCommandRemove(BasePlayer player, string kitname)
        {
            ...
            SendReply(player, $"Набор <color=#ee3e61>{kitname}</color> был удалён");
        ...
        private void DrawUI(BasePlayer player, int page = 1)
        {
            ...
            {
                RectTransform = { AnchorMin = "0.25 0.25", AnchorMax = "0.75 0.75", OffsetMax = "0 0" },
            ...
            container.Add(new CuiButton
            {
                RectTransform = { AnchorMin = "0.386 0.12", AnchorMax = "0.6 0.17", OffsetMax = "0 0" },
                Button = { Color = "1 1 1 0.3", Command = "kit ui" },
                Text = { Text = "ЗАКРЫТЬ", Color = "1 1 1 0.5", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-bold.ttf", FontSize = 20 }
            }, Layer);

            container.Add(new CuiButton
            {
                RectTransform = { AnchorMin = "0.33 0.12", AnchorMax = "0.383 0.17", OffsetMax = "0 0" },
                Button = { Color = "0.46 0.73 0.43 0.5", Command = $"kits skip {page - 1}" },
                Text = { Text = $"<<", Color = "1 1 1 0.5", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-bold.ttf", FontSize = 20 }
            }, Layer);

            container.Add(new CuiButton
            {
                RectTransform = { AnchorMin = "0.603 0.12", AnchorMax = "0.653 0.17", OffsetMax = "0 0" },
                Button = { Color = "0.46 0.73 0.43 0.5", Command = $"kits skip {page + 1}" },
                Text = { Text = $">>", Color = "1 1 1 0.5", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-bold.ttf", FontSize = 20 }
            }, Layer);


            float gap = 0.01f, width = 1f, height = 0.1f, startxBox = 0f, startyBox = 1f - height, xmin = startxBox, ymin = startyBox;
            foreach (var check in GetKitsForPlayer(player).ToList().Skip((page - 1) * 6).Take(6))
            {
                ...
                container.Add(new CuiButton
                {
                    ...
                    Text = { Text = $"", Color = "1 1 1 0.5", Font = "robotocondensed-bold.ttf", FontSize = 22, Align = TextAnchor.UpperCenter }
                }, "Kits", $"{check.Name}");
                xmin += width + gap;
                if (xmin + width >= 1)
                {
                ...
                container.Add(new CuiButton
                {
                    RectTransform = { AnchorMin = "0 0", AnchorMax = "0.07 1", OffsetMax = "0 0" },
                ...
                container.Add(new CuiButton
                {
                    RectTransform = { AnchorMin = "0.075 0", AnchorMax = "0.5 1", OffsetMax = "0 0" },
                    Button = { Color = "0 0 0 0" },
                    Text = { Text = $" {check.DisplayName}", Color = "1 1 1 0.5", Font = "robotocondensed-regular.ttf", FontSize = 16, Align = TextAnchor.MiddleLeft }
                }, $"{check.Name}");
                ...
                {
                    ButtonUI(ref container, check.Name);
                }
            }
        ...
        private void ButtonUI(ref CuiElementContainer container, string name)
        {
            var check = _kits.Find(p => p.Name == name);
            container.Add(new CuiButton
            {
                Button = { Color = "0.46 0.73 0.43 0.5", Command = $"kit {name}" },
                RectTransform = { AnchorMin = "0.85 0", AnchorMax = "1 1", OffsetMax = "0 0" },
                Text = { Text = "ЗАБРАТЬ", Color = "1 1 1 0.5", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-bold.ttf", FontSize = 14 }
            }, $"{name}", $"{name}.button");

            container.Add(new CuiButton
            {
                Button = { Color = "0 0 0 0" },
                RectTransform = { AnchorMin = "0.75 0", AnchorMax = "0.845 1", OffsetMax = "0 0" },
                Text = { Text = "<color=#80ab68>✓</color>", Color = "1 1 1 0.5", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", FontSize = 20 }
            }, $"{name}", $"{name}.Take");

            container.Add(new CuiButton
            {
                RectTransform = { AnchorMin = "0.505 0", AnchorMax = "0.745 1", OffsetMax = "0 0" },
                Button = { Color = "0 0 0 0" },
                Text = { Text = $" {FormatTime(TimeSpan.FromSeconds(check.Cooldown))}", Color = "1 1 1 0.5", Font = "robotocondensed-regular.ttf", FontSize = 12, Align = TextAnchor.MiddleCenter }
            }, $"{name}", $"{name}.Times");
        }
        ...
        private void CooldownUI(ref CuiElementContainer container, string name, TimeSpan time)
        {
            container.Add(new CuiButton
            {
                Button = { Color = "0 0 0 0" },
                RectTransform = { AnchorMin = "0.505 0", AnchorMax = "0.745 1", OffsetMax = "0 0" },
                Text = { Text = FormatTime(time), Color = "1 1 1 0.5", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", FontSize = 12 }
            }, $"{name}", $"{name}.time");

            container.Add(new CuiButton
            {
                Button = { Color = "0.81 0.30 0.30 0.5" },
                RectTransform = { AnchorMin = "0.85 0", AnchorMax = "1 1", OffsetMax = "0 0" },
                Text = { Text = "НЕДОСТУПНО", Color = "1 1 1 0.5", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-bold.ttf", FontSize = 14 }
            }, $"{name}", $"{name}.Block");
            ...
            container.Add(new CuiButton
            {
                Button = { Color = "0 0 0 0" },
                RectTransform = { AnchorMin = "0.75 0", AnchorMax = "0.845 1", OffsetMax = "0 0" },
                Text = { Text = "<color=#ab6868>✘</color>", Color = "1 1 1 0.5", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", FontSize = 20 }
            }, $"{name}", $"{name}.Blocks");
        }
        ...
        private void InfoUI(BasePlayer player)
        {
            ...
            {
                CursorEnabled = true,
                RectTransform = { AnchorMin = "0.33 0.2", AnchorMax = "0.653 0.3", OffsetMax = "0 0" },
                Image = { Color = "0 0 0 0" },
            }, "Overlay", LayerInfo);
            ...
            container.Add(new CuiButton
            {
                RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1", OffsetMax = "0 0" },
                Button = { Color = "0 0 0 0" },
                Text = { Text = "НАБОР ПОЛУЧЕН", Color = "1 1 1 0.5", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-bold.ttf", FontSize = 14 }
            }, LayerInfo);

            CuiHelper.AddUi(player, container);
            timer.Once(3f, () => CuiHelper.DestroyUi(player, LayerInfo));
        }
            ...
            foreach (var player in _kitsGUI)
            {
                ...
                foreach (var name in player.Value)
                {
                    ...
                    if (playerKitData.Cooldown > 0)
                    {
                        ...
                        CuiHelper.DestroyUi(player.Key, $"{name}.Block");
                        CuiHelper.DestroyUi(player.Key, $"{name}.Blocks");
                        if (playerKitData.Cooldown < currentTime)
                        {
                            ButtonUI(ref container, name);
