    ...
    class HitMarker : RustPlugin
    {
        ...
        void cmdHitMarker(BasePlayer player, string cmd, string[] args)
        {
            if (!hitmarkeron.Contains(player))
            {
                ...
                SendReply(player,
                    "<size=17><color=yellow>Система</color> › Отображение урона — <color=yellow>LifeFine</color>Rust:</size>\n" + " " + "Вы включили показ урона.");
            }
            else
            {
                hitmarkeron.Remove(player);
                SendReply(player,
                    "<size=17><color=yellow>Система</color> › Отображение урона — <color=yellow>LifeFine</color>Rust:</size>\n" + " " + "Вы отключили показ урона.");
            }
        }
		...
		string GetDamageText(string action)
		{
			switch (action)
			{
				case "wound": return "<color=yellow>УПАЛ!</color>";							  
				case "kill":  return "<color=yellow>УМЕР!</color>";							  
			}
        ...
        {						
            ...
            for (var i = damages.Count-1; i >= 0; i--)
            {
				...
				if (item.Key < time)
					...
					CuiHelper.AddUi( player, HandleArgs( DamageGUI, item.Value.num, $"<color={item.Value.isFriend ? "yellow" : (item.Value.isHead ? "yellow" : "white")}>-{item.Value.damage}</color>", item.Value.xs, item.Value.ys, item.Value.xe, item.Value.ye ) );
