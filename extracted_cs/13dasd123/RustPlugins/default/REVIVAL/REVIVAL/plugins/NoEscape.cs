...
namespace Oxide.Plugins
{
    ...
    {
        [PluginReference] Plugin ELSclans, Friends, RustMap, VKBot, Duel;

        bool IsClanMember(ulong playerID, ulong targetID)
        {
            return (bool)(ELSclans?.Call("HasFriend", playerID, targetID) ?? false);
        }
        ...
        List<ulong> GetMembersClan(ulong playerId)
        {
            if (ELSclans)
            {
                ...
                if (ELSclans.ResourceId == 2087)
                {
                    var clan1tag = (string)ELSclans.Call("GetClanOf", playerId);
                    if (clan1tag != null)
                    {
                        var members = ((JObject)ELSclans.Call("GetClan", clan1tag))["members"].ToObject<List<ulong>>();
                ...
                if (ELSclans.ResourceId == 14)
                {
                    var clanmates = ELSclans.Call("GetClanMembers", playerId) as List<string>;
        ...
        float offlineScaleFriendsELSclans = 0.5f;
        ...
        protected override void LoadDefaultConfig()
        {
            ...
            GetConfig("Основное", "Время блокировки при нанесение урона по игрокам (Блокировка инициатора и жертвы)",
                ref blockAttackTime);
            GetConfig("Основное", "Поддержка плагина ELSclans", ref ELSclansAPI);
            ...
            GetConfig("Множитель", "Множитель урона если хозяин друг или соклановец (наносимый урон = урон*SCALE)",
                ref offlineScaleFriendsELSclans);
        ...
        void OnEntityDeath(BaseCombatEntity entity, HitInfo info, Item item)
        {
            ...
            if (EnabledBuildingPrivilage && entity?.ToPlayer()?.GetBuildingPrivilege() == null ||
                entity.ToPlayer()?.GetBuildingPrivilege()?.OwnerID == 0) return;

            InitImages();
            bool justCreated;

            var raid = GetRaid(entity.GetNetworkPosition(), out justCreated,
                EnabledBuildingPrivilage ? entity?.ToPlayer()?.GetBuildingPrivilege() : null);
            ...
            GetAroundPlayers(entity.GetNetworkPosition()).ForEach(p =>
                BlockPlayer(p, EnabledBuildingPrivilage ? entity?.ToPlayer()?.GetBuildingPrivilege() : null, "raid"));
            ...
            if (ELSclansAPI && ELSclans.Call("GetClanMembers", entity.OwnerID) != null)
            {
                bool sendRemoveOwnerMessage = false;
                var team = (ELSclans.Call("GetClanMembers", entity.OwnerID) as List<string>).Select(ulong.Parse);
        ...
        void OnEntityTakeDamage(BaseCombatEntity entity, HitInfo info, BaseCombatEntity victim)
        {
            try
            {
                ...
                if (useDamageScale)
                {
                    bool justCreated;
                    var raid = GetRaid(entity.GetNetworkPosition(), out justCreated,
                        EnabledBuildingPrivilage ? victimBP.GetBuildingPrivilege() : null);
                    ...
                    if (ELSclansAPI)
                    {
                        ...
                        if (justCreated && initiator && !sendRemoveOwnerMessage)
                        {
                            ...
                            {
                                damageTimers.Add(new DamageTimer(raid.owners.ToList(), 3600));
                                info.damageTypes.ScaleAll(offlineScaleFriendsELSclans);
                                sendRemoveOwnerMessage = true;
                                justCreated = false;
                    ...
                    if (ELSclansAPI && GetMembersClan(entity.OwnerID) != null)
                    {
        ...
        void NoEscapeTimerHandle()
        {
            foreach (var uid in timers.Keys.ToList())
            {
                ...
                if (--timers[uid].Seconds <= 0)
                {
                    ...
                    BasePlayer.activePlayerList.FirstOrDefault(p => p.userID == uid)?.ChatMessage(Messages["blocksuccess"]);
                }
                else
                {
                    if (EnabledGUITimer) DrawUITimer(BasePlayer.activePlayerList.FirstOrDefault(p => p.userID == uid));
                }
            }
        ...
        string UI =
            "[{\"name\":\"noescape_main2\",\"parent\":\"Hud\",\"components\":[{\"type\":\"UnityEngine.UI.Image\",\"color\":\"1 1 1 0.1119509\"},{\"type\":\"RectTransform\",\"anchormin\":\"{min}\",\"anchormax\":\"{max}\",\"offsetmax\":\"0 0\"}]},{\"name\":\"noescape_main3\",\"parent\":\"noescape_main2\",\"components\":[{\"type\":\"UnityEngine.UI.Image\",\"color\":\"1 0.68 0.31 1\"},{\"type\":\"RectTransform\",\"anchormin\":\"0 0\",\"anchormax\":\"{amax} 1\",\"offsetmax\":\"0 0\"}]},{\"name\":\"noescape_main4\",\"parent\":\"noescape_main2\",\"components\":[{\"type\":\"UnityEngine.UI.Text\",\"text\":\"{0}\",\"align\":\"MiddleCenter\"},{\"type\":\"UnityEngine.UI.Outline\",\"color\":\"0 0 0 1\",\"distance\":\"0.5 0\"},{\"type\":\"RectTransform\",\"anchormin\":\"0.001751313 0\",\"anchormax\":\"1.001751 1\",\"offsetmax\":\"0 0\"}]}]";
