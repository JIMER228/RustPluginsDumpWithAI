            ...
            else 
            {
                ...
                if(args[0] == "remove")
                {
                    if(args.Length != 1)
                    {
                        ...
                        if(target == null) target = BasePlayer.FindSleeping(args[1]);
                ...
                if(args[0] == "accept")
                {
                    if(_pendingRequests.ContainsKey(player.userID))
                    {
                        ...
                        if(target == null) target = BasePlayer.FindSleeping(targetID);
                ...
                { 
                    ...
                    if(arg.Args[0] == "remove")
                    {
                        if(arg.HasArgs(2))
                        {
                            ...
                            if(target == null) target = BasePlayer.FindSleeping(arg.Args[1]);
                    ...
                    if(arg.Args[0] == "noaccept")
                    {
                        if(_pendingRequests.ContainsKey(player.userID))
                        {
                            ...
                            if(target == null) target = BasePlayer.FindSleeping(targetID.ToString());
            ...
            if(baseEnt is BuildingPrivlidge)
            {
                ...
                if(!data.entitiesCupboard.Contains(cupboard.net.ID)) 
                {
                    ...
                    foreach(var friend in data.friends)
                    {
                        ...
                        if(friendPlayer == null) friendPlayer = BasePlayer.FindSleeping(friend.ToString());
