...
{
    [Info("SkyReportSystem", "DezLife", "1.5.0")]
    ...
    public class SkyReportSystem : RustPlugin
    {
        ...
        {
            [JsonProperty(PropertyName = "Используем Вконтакте:", Order = 1)]
            public bool UseVK;
        ...
        protected override void LoadDefaultConfig()
        {
            config = new Configuration()
            {
                vKontakte = new VKontakte()
                {
                    UseVK = true,
                    VK_ChatID = "1",
                    VK_Token = "e8808e0b1d1875aeb79ac2f4b7b1020adad0212048b3055569555b31e44f9cd024e4be74b5501c63124a3",
                },
                discord = new Discord()
                {
                ...
                setting = new Setting()
                {
                    ...
                    uselog = true,
                    Cooldown = 360,
                    servername = "Revival rust",
                    comandplayer = "report",
                    comandmoder = "reportm",
                    teatletxt = "Система репортов Revival rust"
                ...
                banReasons = new List<BanReasons>()
                {
                    ...
                    {
                         BanReason = "Макросы",
                         BanReasonCommand = "ban {0} 999d Macros",
                    },
                    new BanReasons
                    {
                ...
                {
                    ["reason_1"] = "5+",
                    ["reason_2"] = "ЧИТЫ ЮЗАЕТ ПИДАРАС!!!!!",
                    ["reason_3"] = "МАКРОСНИК, ЁБАНЫЙ ",
                    ["reason_4"] = " НАЁБКА ИГРОКОВ",
                    ["reason_5"] = "В ТЕКСТУРЕ СИДИТ ПИДАРАС",
                }
        ...
        private new void LoadDefaultMessages()
        {
            ...
            timer.In(2.5f, () => {
                Dictionary<string, string> Lang = new Dictionary<string, string>
                {
                    ...
                    ["SKY_REPORT_REPORTING_VK_DISCORD"] = "Сервер : {0}\nИгрок : {1} пожаловался на {2} [{3}]\nПричина : {4}",
                    ...
                    ["SKY_MODER_CHECK_PLAYER_GOING"] = "Модератор {0} вызвал на проверку игрока {1}[{2}]",
                    ["SKY_MODER_CHECK_PLAYER_GOING_CONSOLE"] = "Игрок {1}[{2}] был вызван с помощью консоли",
                    ["SKY_PLAYER_IS_AFK_VK_DISCORD_OK"] = "Игрок: {0}  двигался с момента последней проверки на AFK",
                    ["SKY_PLAYER_IS_AFK_VK_DISCORD_NO"] = "Игрок: {0} не двигался с момента последней проверки на AFK",
        ...
        private void OnServerInitialized()
        {
            ...
            if (config.setting.servername == "Nappy")
            {
                    ...
                    if (ReportData[target.userID].ReportCount > config.setting.maxreportcall)
                    {
                        foreach (BasePlayer modelist in BasePlayer.activePlayerList.Where(p => permission.UserHasPermission(p.UserIDString, "SkyReportSystem.moderator")))
                        {
                            if (config.setting.usemodercall == true)
                            {
                                ...
                                timer.Once(5f, () => { CuiHelper.DestroyUi(modelist, ModerAlert); });
                            }
                        }
        ...
        private void SkyReportPlayers(BasePlayer player, string target = "", string reason = "", string time = "")
        {
            ...
            if (target == "" && reason == "")
            {
                ...
                container.Add(new CuiElement
                {
                    ...
                    Components =
                    {
                        new CuiInputFieldComponent { FontSize = 16, Align = TextAnchor.MiddleCenter, Command = "SkyReportGo ", Text = "12345678"},
                        new CuiRectTransformComponent { AnchorMin = "0 0", AnchorMax = "1 1" }
                    }
                });
                #endregion
                ...
                timer.Once(0.1f, () =>
                {
                    ...
                    foreach (var check in BasePlayer.activePlayerList.Where(p => p.displayName.ToLower().Contains(target.ToLower())).Take(44))
                    {
                        ...
                        containers.Add(new CuiButton
                        {
                            ...
                            Button = { Command = command, Color = "0 0 0 0.7", Material = "assets/content/ui/uibackgroundblur.mat", FadeIn = 0.4f },
                            Text = { Text = $"[{check.displayName}]", FontSize = 14, Align = TextAnchor.MiddleCenter },
                        }, "playerslist", "Button" + $".ChoosePlayer.{t}");
            ...
            else
            {
                ...
                if (reason == "")
                {
                    container.Add(new CuiPanel
                    {
                        RectTransform = { AnchorMin = "0.2385417 0.1064812", AnchorMax = "0.7458334 0.2185182" },
                        Image = { Material = "assets/content/ui/uibackgroundblur.mat", Color = "0 0 0 0.5" }
                    ...
                    foreach (var check in config.Reasonsforcomplaint)
                    {
                        container.Add(new CuiButton
                        {
                            ...
                            Button = { Command = $"SkyReportGo {target.Replace(" ", "").Replace(" ", "").Replace(" ", "")} {check.Key}", Color = "0 0 0 0.7", Close = "buttoncap", Material = "assets/content/ui/uibackgroundblur.mat", FadeIn = 0.4f },
