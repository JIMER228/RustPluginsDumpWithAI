...
namespace Oxide.Plugins
{
	...
	public class Kits : RustPlugin
	{
		...
		private class KitItem
		{
			...
			{
				var item = ItemManager.Create(ItemDefinition, Amount > 1 ? Amount : 1, SkinID);
				...
				case "editkit":
				{
					...
					if (arg.HasArgs(5))
					{
						...
						if (_kitEditing.ContainsKey(player) && _kitEditing[player].ContainsKey(key))
						{
							...
							if (_kitEditing[player][key].Equals(newValue))
								return;
		...
		private void CmdKitsGiveKit(ConsoleSystem.Arg arg)
		{
			...
			var playerData = PlayerData.GetOrCreateKitData(target.UserIDString, kit.Name);
			if (playerData == null) return;

			playerData.HasAmount += amount;

			SendReply(arg, $"Player '{arg.Args[0]}' successfully received a kit '{arg.Args[1]}' ({amount} pcs)");
		...
		private void EditingKitUi(BasePlayer player, bool creating, int kitId = -1)
		{
			...
			var showInfo = Convert.ToBoolean(_kitEditing[player]["ShowInfo"]);
			...
			CheckBoxUi(ref container, EditingLayer + ".Main", EditingLayer + ".Editing.ShowInfo", "0.5 1", "0.5 1",
				...
				showInfo,
				$"UI_Kits editkit {creating} {kitId} ShowInfo {!showInfo}",
		...
		private void RefreshKitUi(ref CuiElementContainer container, BasePlayer player, Kit kit)
		{
			...
			{
				if (kit.Cooldown == 0 || (kit.Cooldown > 0 && playerData.Cooldown - 1 < GetCurrentTime()))
				{
					if (kit.Sale)
					{
						container.Add(new CuiPanel
			...
			bool checkAmount = true)
		{
			return IsAdmin(player) && showAll
				...
				{
					var data = PlayerData.GetOrLoadKitData(player.UserIDString, x.Name);
					if (x.Hide)
						return false;
					...
					if (checkAmount && !(x.Amount == 0 || (x.Amount > 0 &&
					                                       data != null && data.Amount < x.Amount)))
						return false;

					if (!(_config.ShowAllKits || string.IsNullOrEmpty(x.Permission) ||
					      player.HasPermission(x.Permission) || (data != null && data.HasAmount > 0)))
						return false;
		...
		{
			var kitData = PlayerData.GetNotLoadKitData(userId.ToString(), name);
		...
		private class PlayerData
		{
			...
			public static KitData GetOrLoadKitData(string userId, string kitName,
				bool addKit = true)
			{
			...
			private static void TryToWipe(string userId, ref PlayerData data)
			{
				if (_instance._config.AutoWipe && data != null &&
