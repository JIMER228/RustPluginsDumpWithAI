...
{
	[Info("Kill Records", "MACHIN3", "1.3.5")]
	...
	public class KillRecords : RustPlugin
	{
		...
		*/
		...
		private Dictionary<uint, Data> _dataCache;
		...
		{
			public Dictionary<uint, Data> EntityRecords = new Dictionary<uint, Data>();
		}
		private class Data
		{
			public uint entity;
		...
		private void Init()
		{
			...
			_dataCache = new Dictionary<uint, Data>();
		}
		...
		{
			Data data;
			if (!_dataCache.TryGetValue(entity.net.ID, out data))
			{
				_dataCache.Add(entity.net.ID, data = new Data
				{
					id = new List<string>(),
				});
			}
		...
		private void OnLootEntity(BasePlayer player, BaseEntity entity)
		{
			...
			if (_dataCache.ContainsKey(entity.net.ID) && _dataCache[entity.net.ID].id.Contains(player.UserIDString))
			{
				return;
			}
