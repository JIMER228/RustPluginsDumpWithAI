					...
					if (arg.HasArgs(4))
					{
						if (arg.Args[3] == "all")
							amount = buy
								...
								: Mathf.FloorToInt(ItemCount(player.inventory.AllItems(), item.ShortName, item.Skin) /
								                   (float) item.Amount);
						else
						...
						if (buy == false)
							amount = Mathf.Max(1,
								Mathf.Min(amount,
									Mathf.CeilToInt(ItemCount(player.inventory.AllItems(), item.ShortName, item.Skin) /
									                (float) item.Amount)));
					}
