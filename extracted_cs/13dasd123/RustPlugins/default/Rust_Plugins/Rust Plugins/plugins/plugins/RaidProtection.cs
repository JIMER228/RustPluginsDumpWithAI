...
{
	[Info("Raid Protection", "mr01sam", "2.1.8")]
	[Description("Configurable raid protection at the cost of a resource")]
	public class RaidProtection : CovalencePlugin
	{
		/*
		 * CURRENT CHANGELOG
		 * - Removed raidprotection.ui permission which causes spam
		 * - Fixed miscalculations with damage costs
		 * - Cupboard balances are now saved upon plugin unload as well as server save
		 * - Tesla coils can no longer be protected from their own shock damage
		 * - Loot container entities, like barrels and supply drops, are no longer protected
		 */
		...
		private void LoadAllTcs()
		{
			timer.In(1f, () =>
			{
				ProtectedCupboard.toolcupboards = new Dictionary<uint, ProtectedCupboard>();
		...
		{
			public static Dictionary<uint, ProtectedCupboard> toolcupboards = new Dictionary<uint, ProtectedCupboard>();

			public static ProtectedCupboard InitCupboard( BuildingPrivlidge priv )
			{
				if (priv != null && toolcupboards != null && !toolcupboards.ContainsKey(priv.net.ID))
				{
					toolcupboards.Add(priv.net.ID, new ProtectedCupboard(priv));
					PLUGIN.Debug($"Added TC with ID={priv.net.ID}");
					PLUGIN.Debug($"There are {toolcupboards.Count} TCs added");
					return toolcupboards[priv.net.ID];
				}
			...
			{
				if (priv != null && toolcupboards.ContainsKey(priv.net.ID))
				{
					toolcupboards.Remove(priv.net.ID);
		...
		private void cmd_protection( IPlayer player, string command, string[] args )
		{
			...
			else
			{
				...
				priv = (BuildingPrivlidge)BaseNetworkable.serverEntities.Find(id);
			}
		...
		{
			uint id;
			if (args.Length > 0 && uint.TryParse(args[0], out id))
			{
				...
				BuildingPrivlidge priv = (BuildingPrivlidge)BaseNetworkable.serverEntities.Find(id);
				if (basePlayer != null && priv != null)
				{
					OpenUi(basePlayer, (BuildingPrivlidge)BaseNetworkable.serverEntities.Find(id));
				}
			}
		}
		...
		{
			uint id;
			if (player != null && args.Length > 0 && uint.TryParse(args[0], out id))
			{
