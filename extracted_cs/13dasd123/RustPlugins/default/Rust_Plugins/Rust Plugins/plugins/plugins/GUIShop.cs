...
namespace Oxide.Plugins
{
    ...
    public class GUIShop : RustPlugin
    {
        ...
        private void CacheRects()
        {
            ...
            {
                _categoryRects[i] = new string[] { $"{(0.09 + (i * 0.056))} 0.78", $"{(0.14 + (i * 0.056))} 0.82" };
            }
        }
        ...
        private void Loaded()
        {
            ...
            _limits = Interface.Oxide.DataFileSystem.GetFile(nameof(GUIShop) + "/Limits"); // (Limit)
        ...
        private void OnPluginLoaded(Plugin name)
        {
            ...
            {
                PrintWarning("ImageLibrary has been Dedicted and GUIShop Images are now being Processed");
        ...
        private void LoadImages()
        {
            ...
            {
                //if (shopItem.Shortname.Contains("hazmatsuit.spacesuit") || shopItem.Shortname.Contains("attire.ninja.suit") || shopItem.Shortname.Contains("workcart")) continue;
                ...
                /*if (shopItem.SkinId > 0)
                    _guishopItemIcons.Add(new KeyValuePair<string, ulong>(shopItem.Shortname, shopItem.SkinId));
                else
                    _guishopItemIcons.Add(new KeyValuePair<string, ulong>(shopItem.Shortname, 0UL));*/
            }

            /*if (_guishopItemIcons.Count > 0)
            {
                ImageLibrary?.Call("LoadImageList", Name, _guishopItemIcons, null);
            }*/
        ...
        private CuiElementContainer CreateGUIShopItemEntry(ShopItem shopItem, float ymax, float ymin, string shopKey, string color, bool pricebuttons, bool cooldown, BasePlayer player)
        {
           ...
           // int[] autoFix = pricebuttons ? shopItem.SwapLimitToQuantitySoldLimit && shopItem.SellQuantity.Length == 1 && shopItem.SellLimit > 0 && shopItem.SellLimit % shopItem.SellQuantity.FirstOrDefault() == 0 ? shopItem.SellQuantity :  : shopItem.SwapLimitToQuantityBuyLimit && shopItem.BuyQuantity.Length > 0 ? : ;
        ...
        private void CreateGUIShopItemIcon(ref CuiElementContainer container, string item, float ymax, float ymin, ShopItem data, BasePlayer player)
        {
            ...
            //Puts($"{data.Image}, {data.SkinId}, {data.Shortname}, {rawImage.Png}, {((bool)ImageLibrary?.Call("HasImage", data.Shortname, skinID))}");
        }
        ...
        {
            if (player == null || player.IsNpc || !player.IsAlive()) return;
            ...
            {
                DestroyGUIShopButton(player); return;
            }
        ...
        private void ShowGUIShops(BasePlayer player, string shopKey, int from = 0, bool fullPaint = true, bool refreshFunds = false)
        {
            ...
            foreach (ShopItem data in shop.ShopItems.Skip(from * 7).Take(7))
            {
                ...
                /*double limitcooldown = Convert.ToDouble(data.BuyLimitResetCoolDown);
                if (data.BuyLimitResetCoolDown > 0 && itemLimit.BLimit[data.Shortname] >= data.BuyLimit)
                {
                    ...
                    itemCooldowns[name] = CurrentTime() + limitcooldown;
                    itemLimit.BLimit[data.Shortname] = 0;
                }
                
                double limitcooldown2 = Convert.ToDouble(data.SellLimitResetCoolDown);
                if (data.SellLimitResetCoolDown > 0 && itemLimit.SLimit[data.Shortname] >= data.SellLimit)
                {
                    ...
                    itemCooldowns2[name] = CurrentTime() + limitcooldown2;
                    itemLimit.SLimit[data.Shortname] = 0;
                }*/
        ...
        {
            if (!_config.ShopCategories.ContainsKey(shopKey) || !string.IsNullOrEmpty(_config.ShopCategories[shopKey].Permission) && !permission.UserHasPermission(player.UserIDString, _config.ShopCategories[shopKey].PrefixPermission))
            {
        ...
        private static Item Create(ItemDefinition template, int iAmount, string name, float con, ulong skin = 0)
        {
            ...
            obj.uid = Network.Net.sv.TakeUID();
            if (obj.hasCondition)
            {
        ...
        private object CanSell(BasePlayer player, string item, int amount)
        {
            ...
            {
                //Puts($"Is inside CanSell dunno why.. {itemdata.Shortname}"); //Fixed
                return Lang("NotEnoughSell", player.UserIDString);
            }
        ...
        private void CmdGUIShop(BasePlayer player, string command, string[] args)
        {
            ...
            var shop = _config.ShopCategories[_config.DefaultShop];
        ...
        private void ConsoleGUIShopBuy(ConsoleSystem.Arg arg, ShopCategory shopCategory)
        {
            ...
            bool isAll = arg.GetString(2).Equals("all");
            int amount = arg.GetString(2).Equals("all") ? GetAmountBuy(player, shopCategory, item) : arg.GetInt(2);
            if (amount == 0)
            {
            ...
            */
            ...
            if (_shopPage.ContainsKey(player.userID))
        ...
        // Fixed ShopID issues when using inside NPC shops ( current bug with global shops ) requires re-implementation
        ...
        private void ConsoleGUIShopTransparency(ConsoleSystem.Arg arg)
        {
            ...
            //string cat = arg.GetString(1).Replace("_", " ");
            //Puts($"before {cat}");
            string cat = GetPlayerData(player).ShopKey;

            /*ShopCategory category;
            if (string.IsNullOrEmpty(_config.DefaultShop) && GetNearestVendor(player, out category))
            {
            ...
            {
                PrintError($"Hey, Transparency change error occured contact the developer on umod.org or on discord @ Khan#8615 and send the config file");
                SendReply(player, "Get Admin, Transparency change error has occured, contact GUIShop developer");
            }
           // Puts($"after {cat}");*/
        ...
        bool GetNearestVendor(BasePlayer player, out ShopCategory shopCategory) //NPC helper redo.
        {
            shopCategory = null;
            ...
            BasePlayer closestNpc = null;

            foreach (var collider in colliders)
            {
                BasePlayer found = collider.ToBaseEntity() as BasePlayer;
                if (found == null || !IsPlayer(found.userID)) continue;

                float currentDist = Vector3.Distance(found.ServerPosition, player.ServerPosition);
            ...
            shopCategory = _config.ShopCategories.Select(x => x.Value).FirstOrDefault(i => i.EnableNPC && i.NpcIds.Contains(closestNpc.UserIDString) && (!string.IsNullOrEmpty(i.Permission) && permission.UserHasPermission(player.UserIDString, i.PrefixPermission) || string.IsNullOrEmpty(i.Permission)));
