...
{
    [Info("IQGradeRemove", "https://topplugin.ru / https://discord.com/invite/5DPTsRmd3G", "0.1.4")]
    ...
    class IQGradeRemove : RustPlugin
    {
        ...
        public String GradeErrorParse(BasePlayer player, BuildingBlock buildingBlock, BuildingGrade.Enum grade = BuildingGrade.Enum.None)
        {
            ...
            if (DeployVolume.Check(buildingBlock.transform.position, buildingBlock.transform.rotation, PrefabAttribute.server.FindAll<DeployVolume>(buildingBlock.prefabID), ~(1 << buildingBlock.gameObject.layer)))
        ...
        void ReturnedRemoveItems(BasePlayer player, BaseEntity buildingBlock)
        {
            ...
            {
                Item ItemReturned = buildingBlock is BaseOven ? ItemManager.CreateByName(buildingBlock.ShortPrefabName, 1) : ItemManager.Create(buildingBlock.GetComponent<BaseCombatEntity>().pickup.itemTarget, 1);
        ...
        public string RemoveErrorParseBuilding(BasePlayer player, BaseEntity buildingBlock)
        {
            ...
            if (Block.ShortnameNoteReturned.Contains(buildingBlock.ShortPrefabName))
        ...
        [ConsoleCommand("turned_all_object")]
        void TunredAllObject(ConsoleSystem.Arg arg)
        {
            BasePlayer player = arg.Player();
            if (player == null) return;
            if (arg == null || arg.Args == null || arg.Args.Length == 0 || arg.Args.Length > 1)
            { return; }
            var Data = DataPlayer[player.userID];
        ...
        void UPCommandConsole(ConsoleSystem.Arg arg)
        {
            ...
            if (arg == null || arg.Args == null || arg.Args.Length == 0 || arg.Args.Length > 2)
                Data.GradeUP(player);
            else
            {
                switch (arg.Args.Length)
                {
                    ...
                    case 1:
                        {
                            int GradeLevel;
                            if (!int.TryParse(arg.Args[0], out GradeLevel) || GradeLevel < 0 || GradeLevel > 4)
                                return;
        ...
        void Interface_New_Main(BasePlayer player)
        {
            ...
            container.Add(new CuiPanel
            {
                ...
                RectTransform = { AnchorMin = "0 0", AnchorMax = "0 0", OffsetMin = "0.092 0.274", OffsetMax = "190.01005 30" }  
            ...
            container.Add(new CuiPanel
            {
                ...
                RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1", OffsetMin = "159.871 1.976", OffsetMax = "-29.361005 -1.976" }   
