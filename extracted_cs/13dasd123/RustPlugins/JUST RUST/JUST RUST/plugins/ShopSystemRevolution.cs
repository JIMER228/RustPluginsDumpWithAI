...
{
    [Info("ShopSystemRevolution", "TopPlugin.ru", "1.0.0")]
        ...
        private IEnumerator DownloadImages()
        {
            ...
            foreach (var img in config.itemstores)
            {
                for (int i = 0; i < img.Value.Count; i++)
                {
                    ...
                    {
                        if (!(bool)ImageLibrary?.Call("HasImage", img.Value[i].ShortName, img.Value[i].SkinID)) ImageLibrary.Call("AddImage", $"http://api.skyplugins.ru/api/getskin/{img.Value[i].SkinID}/", img.Value[i].ShortName, img.Value[i].SkinID);
                    }
                    ...
                    {
                        if (!(bool)ImageLibrary?.Call("HasImage", img.Value[i].ShortName + 129)) ImageLibrary.Call("AddImage", $"http://api.skyplugins.ru/api/getimage/{img.Value[i].ShortName}/128", img.Value[i].ShortName + 129);
                        if (!(bool)ImageLibrary?.Call("HasImage", img.Value[i].ShortName + 64)) ImageLibrary.Call("AddImage", $"http://api.skyplugins.ru/api/getimage/{img.Value[i].ShortName}/64", img.Value[i].ShortName + 64);
                    }
