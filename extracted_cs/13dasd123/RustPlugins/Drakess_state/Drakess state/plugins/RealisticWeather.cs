...
{
    [Info("Realistic Weather", "OuTSMoKE|Nimant", "1.1.4", ResourceId = 2519)]    
    class RealisticWeather : RustPlugin
    {
        private static Timer StormTimer;
        private static int time = 0;
		
		private Dictionary<string, float> SaveWeather = new Dictionary<string, float>();
		...
		{			
			SaveWeather.Add("fog", SingletonComponent<Climate>.Instance.Overrides.Fog);
			SaveWeather.Add("rain", SingletonComponent<Climate>.Instance.Overrides.Rain);
			SaveWeather.Add("clouds", SingletonComponent<Climate>.Instance.Overrides.Clouds);
			SaveWeather.Add("wind", SingletonComponent<Climate>.Instance.Overrides.Wind);
		}
        ...
        private void Unload()
        {
			...
			SingletonComponent<Climate>.Instance.Overrides.Fog = SaveWeather["fog"];
			SingletonComponent<Climate>.Instance.Overrides.Rain = SaveWeather["rain"];
			SingletonComponent<Climate>.Instance.Overrides.Clouds = SaveWeather["clouds"];
			SingletonComponent<Climate>.Instance.Overrides.Wind = SaveWeather["wind"];						
        }

        [ConsoleCommand("rw")]
        private void cmdSpecial(ConsoleSystem.Arg arg)
        {
			...
			Puts($"Запущен погодный эффект '{stormType}'.");
            NewStormTimer(stormType);
        }

        private void NewStormTimer(string TypeOfStorm)
        {
            if (StormTimer != null)
                StormTimer.Destroy();
            time = 0;
            StormTimer = timer.Every(1f, () => ProcessStorm(TypeOfStorm, Storms[TypeOfStorm]));
        }

        private void ProcessStorm(string TypeOfStorm, Dictionary<int, List<string>> StormData)
        {
            time++;
            if (StormData.ContainsKey(time))
                foreach (var entry in StormData[time])
                {
                    if (entry == "STOP") { Puts($"Погодный эффект '{TypeOfStorm}' окончен."); StormTimer.Destroy(); time = 0; return; }                    
					RunEffect(entry);
                }
        }
		...
		private void RunEffect(string effect)
		{
			...
			try
			{
				value = Convert.ToSingle(effect.Substring(effect.IndexOf(" ")+1, effect.Length-effect.IndexOf(" ")-1));
			}
			catch {}
			
			if (value == -1f) return;
			
			if (effect.Contains("fog"))
				SingletonComponent<Climate>.Instance.Overrides.Fog = value;
			else
				if (effect.Contains("rain"))
					SingletonComponent<Climate>.Instance.Overrides.Rain = value;
				else
					if (effect.Contains("clouds"))
						SingletonComponent<Climate>.Instance.Overrides.Clouds = value;
					else
						if (effect.Contains("wind"))
							SingletonComponent<Climate>.Instance.Overrides.Wind = value;									
		}
		
        private static Dictionary<string, Dictionary<int, List<string>>> Storms = new Dictionary<string, Dictionary<int, List<string>>>
                {
