...
namespace Oxide.Plugins
{
    ...
    {
        [PluginReference]
        private Plugin Clans, Economics, ServerRewards, Friends, CompoundTeleport, ZoneManager, CombatBB, Vanish, PopupNotifications;
        ...
        public class PluginSettings
        {
            ...
            [JsonProperty(PropertyName = "Block Teleport (CombatBB)")]
            public bool BlockCombatBB { get; set; } = false;
        ...
        protected override void LoadDefaultMessages()
        {
            var en = new Dictionary<string, string>
            {
                ...
                {"TPCombatBBBlocked", "You can't teleport while blocked!"},
            ...
            var ru = new Dictionary<string, string>
            {
                ...
                {"TPCombatBBBlocked", "Вы не можете телепортироваться пока активна блокировка!"},
        ...
        private void cmdChatHomeTP(BasePlayer player, string command, string[] args)
        {
            ...
            if (config.Settings.BlockCombatBB && Convert.ToBoolean(CombatBB?.Call("IsBlockedZone", location)))
            {
                PrintMsgL(player, "TPCombatBBBlocked");
                return;
            }
        ...
        private string CheckPlayer(BasePlayer player, bool build = false, bool craft = false, bool origin = true, string mode = "home", bool allowcave = true)
        {
            ...
            if (config.Settings.BlockCombatBB && CombatBB != null)
            {
                if (Convert.ToBoolean(CombatBB?.Call("IsBlocked", player)))
                {
                    return "TPCombatBBBlocked";
                }
            }
