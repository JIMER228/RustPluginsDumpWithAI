...
{
    [Info("XDStatistics", "https://discord.gg/9vyTXsJyKR", "1.0.0")]
    [Description("Multifunctional statistics for your server!")]
    class XDStatistics : RustPlugin
    {
        /// <summary>
        /// Вернул ткань в категорию фермерство
        /// Отсортировал предметы по количеству в категории фермерство
        /// Исправил добавления ботов в статистику
        /// Исправил формат времени в оповещениях в чат
        /// Некоторые исправления подсчета сбитие вертолета
        /// Некоторые изменения в конфигурации а именно вам нужно заного настроить геймстор магазин
        /// </summary>
        ...
        protected override void LoadDefaultMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                ["STAT_UI_PVP_KDR"] = "K/D",
                ["STAT_UI_FAVORITE_WEAPON_KILLS"] = "Kills: {0} | Hits: {1}",
            ...
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                ["STAT_UI_PVP_KDR"] = "K/D",
                ["STAT_UI_FAVORITE_WEAPON_KILLS"] = "Убийств: {0} | Попаданий: {1}",
        ...
        private class Configuration
        {
            public class SettingsInterface
            {
                ...
                public string ColorTop3 = "0.8039216 0.498031724 0.1960784 0.49";
            }
            ...
            public class SettingsPrize
            {
                ...
                [JsonProperty("Награда в категории Большой онлайн | Award in the Big Online categoryy")]
                ...
                internal class Prize
                {
                    ...
                    public void GiftPrizePlayer(BasePlayer player)
                    {
                        if (commandPrizeUse)
                        {
                            foreach (string cmd in commandPrizeList)
                                _.Server.Command(cmd.Replace("%STEAMID%", player.UserIDString));
                        }
                        if (gamestorePrizeUse && _?.GameStoresRUST)
                        {
                            _.webrequest.Enqueue(
                           $"https://gamestores.ru/api/?shop_id={_.config.settingsPrize.ShopID}&secret={_.config.settingsPrize.SecretKey}&server={_.config.settingsPrize.ServerID}&action=moneys&type=plus&steam_id={player.UserIDString}&amount={balancePlus}",
                           "", (code, response) =>
                           {
                               switch (code)
                               {
                        ...
                        if (ovhPrizeUse)
                        {
                            if (_?.RustStore)
                            {
                                _.RustStore.CallHook("APIChangeUserBalance", player.userID, balancePlus, new Action<string>(result =>
                                {
                                    if (result == "SUCCESS")
                                        return;
                        ...
                        if (economicPrizeUse)
                        {
                            if (_?.Economics)
                            {
                                _.Economics.Call("Deposit", player.userID, (double)balanceEconomicsPlus);
                            }
                            else if (_?.IQEconomic)
                            {
                                _.IQEconomic.Call("API_SET_BALANCE", player.userID, balanceEconomicsPlus);
                            }
                            else if (_?.ServerRewards)
                            {
                                _.ServerRewards.Call("AddPoints", player.userID, balanceEconomicsPlus);
                            }
                        }
                    }
        ...
        private void ValidateConfig()
        {
            ...
            if (config.settingsPrize.prizeFarm.Count == 0)
            {
                ...
                {
                    new Configuration.SettingsPrize.Prize{commandPrizeList = new List<string>{ "say %STEAMID%" } },
                    new Configuration.SettingsPrize.Prize{ top = 2, commandPrizeList = new List<string>{ "say %STEAMID%" } },
                };
            }
        ...
        private class StoredData
        {
            ...
            public Dictionary<ulong, PlayerInfo> IgnoreReservedPlayer = new Dictionary<ulong, PlayerInfo>();
            public Dictionary<ulong, List<PrizePlayer>> PrizePlayer = new Dictionary<ulong, List<PrizePlayer>>();
            public StoredData()
            {
            }
        }
        ...
        private class PlayerInfo
        {
            ...
            {
                var player = PlayerStat.Players.ContainsKey(id) ? PlayerStat.Players[id] : null;
                if (player == null)
                    return;
            ...
            public static void PlayerClearData(ulong id)
            {
                ...
                PlayerStat.Players[id] = new PlayerInfo();
                PlayerStat.Players[id].Name = player.displayName;
            }
            public static void ClearDataWipe()
            {
                if (PlayerStat.IgnoreReservedPlayer != null)
                {
                    foreach (var item in PlayerStat.IgnoreReservedPlayer)
                        _.NextTick(() => {
                            PlayerStat.IgnoreReservedPlayer[item.Key] = new PlayerInfo();
                        });
                }
                if (PlayerStat.Players != null)
                {
                    foreach (var item in PlayerStat.Players)
                    {
                        if (item.Value.playedTime.PlayedForWipe <= 300)
                            _.NextTick(() => { PlayerStat.Players.Remove(item.Key); });
                        else
                            _.NextTick(() => { PlayerStat.Players[item.Key] = new PlayerInfo(); });
                    }
                }         
            }
            public static PlayerInfo Find(ulong id)
            {
                if (PlayerStat.Players.ContainsKey(id))
                {
                    return PlayerStat.Players[id];
                }

                if (PlayerStat.IgnoreReservedPlayer.ContainsKey(id))
                {
                    return PlayerStat.IgnoreReservedPlayer[id];
                }

                PlayerStat.Players.Add(id, new PlayerInfo());
        ...
        private static StoredData PlayerStat;

        private void LoadData() => PlayerStat = Interface.GetMod().DataFileSystem.ReadObject<StoredData>("XDStatistics/StatsUser");
        private void SaveData() => Interface.GetMod().DataFileSystem.WriteObject("XDStatistics/StatsUser", PlayerStat);
        ...
        {
            if (!PlayerStat.IgnoreReservedPlayer.ContainsKey(player.userID))
            {
            ...
            if (PlayerStat.PrizePlayer.ContainsKey(player.userID))
                GiftRewardWipe(player);
        ...
        private void OnServerInitialized()
        {
            ...
            #endregion
            timer.Every(config.settings.dataSaveTime * 60, () => { SaveData(); });
        }
        #endregion
        ...
        private void MainMenuStat(BasePlayer player)
        {
            ...
            container.Add(new CuiButton
            {
                RectTransform = { AnchorMin = "1 1", AnchorMax = "1 1",OffsetMin = "-100 -100",OffsetMax = "-60 -60"},
                Button = { Close = UI_INTERFACE, Color = "0 0 0 0" },
        ...
        private void MenuButton(BasePlayer player, int page = 0)
        {
            ...
            {
                CursorEnabled = false,
                Image = { Color = page == 0 ? "0.68 0.12 0.12 0.64" : "1 1 1 0.3" },
            ...
            {
                CursorEnabled = false,
                Image = { Color = page == 1 || page == 3 ? "0.68 0.12 0.12 0.64" : "1 1 1 0.3" },
            ...
            {
                CursorEnabled = false,
                Image = { Color = page == 2 ? "0.68 0.12 0.12 0.64" : "1 1 1 0.3" },
        ...
        private void SearchPageUser(BasePlayer player, string target = "")
        {
            ...
            container.Add(new CuiPanel
            {
                CursorEnabled = false,
                Image = { Color = "1 1 1 0" },
                RectTransform = { AnchorMin = "0.5 0.45", AnchorMax = "0.5 0.45", OffsetMin = "-378.454 -264.835", OffsetMax = "381.998 266.939" }
            ...
            container.Add(new CuiElement
            {
                ...
                Components = {
                    new CuiRawImageComponent { Color = "1 1 1 0", Png = ImageUi.GetImage("18") },
            ...
            int y = 0, x = 0;
            foreach (var players in PlayerStat.Players.Where(z => z.Value.Name.ToLower().Contains(target)))
            {
                ...
                container.Add(new CuiElement
                {
                    ...
                    Components = {
                        new CuiRawImageComponent { Color = "1 1 1 0", Png = ImageUi.GetImage("19") },
        ...
        private void UserStat(BasePlayer player, ulong target = 0)
        {
            ...
            container.Add(new CuiPanel
            {
                CursorEnabled = false,
                Image = { Color = "0 0 0 0" },
                RectTransform = { AnchorMin = "0.5 0.45", AnchorMax = "0.5 0.45", OffsetMin = "-400.254 -331.554", OffsetMax = "393.974 274.446" }
            ...
            container.Add(new CuiPanel
            {
                CursorEnabled = false,
                Image = { Color = "0 0 0 0" },
                RectTransform = { AnchorMin = "0.5 0.55", AnchorMax = "0.5 0.55", OffsetMin = "-400.254 -331.554", OffsetMax = "393.974 274.446" }
            ...
            container.Add(new CuiElement
            {
                ...
                Components = {
                    new CuiRawImageComponent { //Color = color
                        Color = "0 0 0 0", Png = ImageUi.GetImage("2") },
            ...
            container.Add(new CuiElement
            {
                ...
                Parent = UI_USER_STAT_INFO,
                Components = {
                    new CuiRawImageComponent { Color = "1 1 0 0", Png = ImageUi.GetImage("7") },
            ...
            container.Add(new CuiLabel
            {
                ...
                Text = { Text = GetLang("STAT_UI_PVP_KILLS", player.UserIDString), Font = "robotocondensed-regular.ttf", FontSize = 10, Align = TextAnchor.MiddleLeft, Color = "1 1 1 1" }
            ...
            container.Add(new CuiLabel
            {
                ...
                Text = { Text = statInfo.pVP.Kills.ToString(), Font = "robotocondensed-regular.ttf", FontSize = 10, Align = TextAnchor.MiddleRight, Color = "0.8538514 0.8491456 0.8867924 1" }
            ...
            container.Add(new CuiElement
            {
                ...
                Parent = UI_USER_STAT_INFO,
                Components = {
                    new CuiRawImageComponent { Color = "1 1 1 0", Png = ImageUi.GetImage("7") },
            ...
            float kdr = statInfo.pVP.Deaths == 0 ? statInfo.pVP.Kills : (float)Math.Round(((float)statInfo.pVP.Kills) / statInfo.pVP.Deaths, 2);
            ...
            container.Add(new CuiElement
            {
                ...
                Parent = UI_USER_STAT_INFO,
                Components = {
                    new CuiRawImageComponent { Color = "1 1 1 0", Png = ImageUi.GetImage("8") },
            ...
            if (weaponTop.Key != null)
            {
                container.Add(new CuiLabel
                {
                    RectTransform = { AnchorMin = "0 0.5", AnchorMax = "0 0.5", OffsetMin = "46.321 -7.014", OffsetMax = "176.509 8.414" },
        ...
        private void ButtonHideStat(BasePlayer player, PlayerInfo info)
        {
            ...
            if (!info.HidedStatistics)
            {
                container.Add(new CuiElement
                {
                    ...
                    Components = {
                        new CuiRawImageComponent { Color = "1 1 1 0", Png = ImageUi.GetImage("4")},
            ...
            else
            {
                container.Add(new CuiElement
                {
                    ...
                    Parent = "BUTTON_HIDE_STAT",
                    Components = {
                    new CuiRawImageComponent { Color = "1 1 1 0", Png = ImageUi.GetImage("3")},
                    new CuiRectTransformComponent { AnchorMin = "0 0.5", AnchorMax = "0 0.5", OffsetMin = "1.381 -6.404", OffsetMax = "14.381 6.596" }
                }
                });
            }
        ...
        private void DialogConfirmationDropStat(BasePlayer player)
        {
            ...
            container.Add(new CuiElement
            {
                ...
                Parent = UI_USER_STAT_INFO,
                Components = {
                    new CuiRawImageComponent { Color = "1 1 1 0", Png = ImageUi.GetImage("6") },
            ...
            container.Add(new CuiElement
            {
                ...
                Components = {
                    new CuiRawImageComponent { Color = "1 1 1 0", Png = ImageUi.GetImage("5")},
        ...
        private void CategoryStatUser(BasePlayer player, ulong target = 0, int cat = 0)
        {
            ...
            {
                CursorEnabled = false,
                Image = { Color = cat == 0 ? "0.68 0.12 0.12 0.64" : "0 0 0 0" },
            ...
            {
                CursorEnabled = false,
                Image = { Color = cat == 1 || cat == 3 ? "0.68 0.12 0.12 0.64" : "0 0 0 0" },
            ...
            {
                CursorEnabled = false,
                Image = { Color = cat == 2 ? "0.68 0.12 0.12 0.64" : "0 0 0 0" },
            ...
            foreach (var item in list)
            {
                ...
                container.Add(new CuiElement
                {
                    ...
                    Components = {
                    new CuiRawImageComponent { Color = "1 1 1 0", Png = item.Key == "all" ? ImageUi.GetImage("10") : ImageUi.GetImage("9"), FadeIn = fade },
        ...
        private void TopTen(BasePlayer player)
        {
            ...
            container.Add(new CuiPanel
            {
                CursorEnabled = false,
                Image = { Color = "1 1 1 0" },
                RectTransform = { AnchorMin = "0.5 0.45", AnchorMax = "0.5 0.45", OffsetMin = "-240 -360.001", OffsetMax = "640 266.939" }
            ...
            var KillerList = PlayerStat.Players.OrderByDescending(x => x.Value.pVP.Kills).Take(10);
            ...
            var KillerNpcList = PlayerStat.Players.OrderByDescending(x => x.Value.pVP.KillsNpc).Take(10);
            ...
            var TimeList = PlayerStat.Players.OrderByDescending(x => x.Value.playedTime.PlayedForWipe).Take(10);
            ...
            var FarmList = PlayerStat.Players.OrderByDescending(x => x.Value.gather.AllGathered).Take(10);
            int f = 0;
            ...
            container.Add(new CuiButton
            {
                Button = {Color = "0.37 0.37 0.37 0.2",Command = "UI_HandlerStat Page_swap 3"},
                Text = {Text = ">",Align = TextAnchor.MiddleCenter,FontSize = 18},
                RectTransform = {AnchorMin = "1 1", AnchorMax = "1 1", OffsetMin = "-381.151 -441.751", OffsetMax = "-361.009 -26.861"}
        ...
        void TopTwoOpen(BasePlayer player)
        {
            var container = new CuiElementContainer();
            container.Add(new CuiPanel
            {
            ...
            var ExpList = PlayerStat.Players.OrderByDescending(x => x.Value.explosion.AllExplosionUsed).Take(10);
            int z = 0;
            ...
            var ScoreList = PlayerStat.Players.OrderByDescending(x => x.Value.Score).Take(10);
            int s = 0;
            ...
            container.Add(new CuiButton
            {
                Button = {Color = "0.37 0.37 0.37 0.2",Command = "UI_HandlerStat Page_swap 1"},
                Text = {Text = "<",Align = TextAnchor.MiddleCenter,FontSize = 18},
                RectTransform = {AnchorMin = "0 1", AnchorMax = "0 1", OffsetMin = "9.575 -441.751", OffsetMax = "29.214 -26.861"}
                
            },"TOP_10_TABLE");
            container.Add(new CuiPanel
            {
                CursorEnabled = false,
                Image = {Color = "0.68 0.12 0.12 0.64" },
                RectTransform = { AnchorMin = "0.5 0", AnchorMax = "0.5 0", OffsetMin = "-46.608 0", OffsetMax = "46.608 1.871" }
            }, "Panel_56551", "Panel_8052");
            CloseLayer(player);
            CuiHelper.AddUi(player, container);
        ...
        #region OtherMetods
        private void GiftRewardWipe(BasePlayer player)
        {
            var playerGrant = PlayerStat.PrizePlayer[player.userID];
            ...
            foreach (var item in playerGrant)
            {
                ...
                {
                    case CatType.score:
                        config.settingsPrize.prizeScore.Find(x => x.top == item.top)?.GiftPrizePlayer(player);
                        player.ChatMessage(GetLang("STAT_TOP_PLAYER_WIPE_SCORE", player.UserIDString, item.top));
                        break;
                    case CatType.killer:
                        config.settingsPrize.prizeKiller.Find(x => x.top == item.top)?.GiftPrizePlayer(player);
                        player.ChatMessage(GetLang("STAT_TOP_PLAYER_WIPE_KILL", player.UserIDString, item.top));
                        break;
                    case CatType.time:
                        config.settingsPrize.prizeTime.Find(x => x.top == item.top)?.GiftPrizePlayer(player);
                        player.ChatMessage(GetLang("STAT_TOP_PLAYER_WIPE_TIME", player.UserIDString, item.top));
                        break;
                    case CatType.farm:
                        config.settingsPrize.prizeFarm.Find(x => x.top == item.top)?.GiftPrizePlayer(player);
                        player.ChatMessage(GetLang("STAT_TOP_PLAYER_WIPE_FARM", player.UserIDString, item.top));
                        break;
                    case CatType.raid:
                        config.settingsPrize.prizeRaid.Find(x => x.top == item.top)?.GiftPrizePlayer(player);
                        player.ChatMessage(GetLang("STAT_TOP_PLAYER_WIPE_EXP", player.UserIDString, item.top));
                        break;
                }
            }
            NextTick(() => { PlayerStat.PrizePlayer.Remove(player.userID); SaveData(); });
        }
        private void ParseTopUserForPrize()
        {
            PlayerStat.PrizePlayer.Clear();
                    ...
                    case CatType.score:
                        {
                            ...
                            if (countReward != 0)
                            {
                                var score = PlayerStat.Players.OrderByDescending(x => x.Value.Score).Take(countReward);
                                int top = 1;
                                foreach (var item in score)
                                {
                                    if (!PlayerStat.PrizePlayer.ContainsKey(item.Key))
                                    {
                                        PlayerStat.PrizePlayer.Add(item.Key, new List<PrizePlayer>());
                                    }
                                    var player = PlayerStat.PrizePlayer[item.Key];
                    ...
                    case CatType.killer:
                        {
                            ...
                            if (countReward != 0)
                            {
                                var killer = PlayerStat.Players.OrderByDescending(x => x.Value.pVP.Kills).Take(countReward);
                                int top = 1;
                    ...
                    case CatType.time:
                        {
                            ...
                            if (countReward != 0)
                            {
                                var time = PlayerStat.Players.OrderByDescending(x => x.Value.playedTime.PlayedForWipe).Take(countReward);
                                int top = 1;
                    ...
                    case CatType.farm:
                        {
                            ...
                            if (countReward != 0)
                            {
                                var farm = PlayerStat.Players.OrderByDescending(x => x.Value.gather.AllGathered).Take(countReward);
                                int top = 1;
                    ...
                    case CatType.raid:
                        {
                            ...
                            if (countReward != 0)
                            {
                                var raid = PlayerStat.Players.OrderByDescending(x => x.Value.explosion.AllExplosionUsed).Take(countReward);
                                int top = 1;
        ...
        private void PlayersTopOnChat()
        {
            ...
            switch (i)
            {
                case 0:
                    {
                        var playerList = PlayerStat.Players.OrderByDescending(x => x.Value.pVP.Kills).Take(5);
                ...
                case 1:
                    {
                        var playerList = PlayerStat.Players.OrderByDescending(x => x.Value.gather.AllGathered).Take(5);
                        foreach (var playerInfo in playerList)
                        {
                ...
                case 2:
                    {
                        var playerList = PlayerStat.Players.OrderByDescending(x => x.Value.explosion.AllExplosionUsed).Take(5);
                        foreach (var playerInfo in playerList)
                        {
                ...
                case 3:
                    {
                        var playerList = PlayerStat.Players.OrderByDescending(x => x.Value.playedTime.PlayedForWipe).Take(5);
                        foreach (var playerInfo in playerList)
                        {
                ...
                case 4:
                    {
                        var playerList = PlayerStat.Players.OrderByDescending(x => x.Value.otherStat.BuildingCrate).Take(5);
                        foreach (var playerInfo in playerList)
                        {
                ...
                case 5:
                    {
                        var playerList = PlayerStat.Players.OrderByDescending(x => x.Value.Score).Take(5);
                        foreach (var playerInfo in playerList)
                        {
                ...
                case 6:
                    {
                        var playerList = PlayerStat.Players.OrderByDescending(x => x.Value.harvesting.AllHarvesting).Take(5);
                        foreach (var playerInfo in playerList)
                        {
        ...
        private class ImageUi
        {
            ...
            public static void DownloadImages()
            {
                for (int i = 1; i < 21; i++)
                {
                    //ServerMgr.Instance.StartCoroutine(AddImage($"https://i.ibb.co/YbXY1Sv/3.png", i.ToString()));
                    
                    switch (i)
                    {
                        case 12:
                            ServerMgr.Instance.StartCoroutine(AddImage($"https://rustlabs.com/img/screenshots/barrel.png", i.ToString()));
                            break;
                        case 11:
                            ServerMgr.Instance.StartCoroutine(AddImage($"https://rustlabs.com/img/items180/hazmatsuit_scientist_peacekeeper.png", i.ToString()));
                            break;
                        case 13:
                            ServerMgr.Instance.StartCoroutine(AddImage($"https://rustlabs.com/img/items180/cratecostume.png", i.ToString()));
                            break;
                        case 15:
                            ServerMgr.Instance.StartCoroutine(AddImage($"https://i.ibb.co/McBNWgv/HeliIcon.png", i.ToString()));
                            break;
                        case 16:
                            ServerMgr.Instance.StartCoroutine(AddImage($"https://i.ibb.co/MNWT2C5/Bradly-Icon.png", i.ToString()));
                            break;
                        case 17:
                            ServerMgr.Instance.StartCoroutine(AddImage($"https://rustlabs.com/img/items180/hat.wolf.png", i.ToString()));
                            break;
                        case 18:
                            ServerMgr.Instance.StartCoroutine(AddImage($"https://i.ibb.co/CHNXbT2/search.png", i.ToString()));
                            break;
                        case 20:
                            ServerMgr.Instance.StartCoroutine(AddImage($"https://i.ibb.co/jRByKxY/20.png", i.ToString()));
                            break;
                        default:
                            ServerMgr.Instance.StartCoroutine(AddImage($"https://i.ibb.co/Gvkx8Yd/statimage.png", i.ToString()));
                            break;
                    }
                }
            }
        ...
        private static List<KeyValuePair<ulong, PlayerInfo>> FindPlayers(string name)
        {
            ...
            var playersData = PlayerStat.Players.Concat(PlayerStat.IgnoreReservedPlayer);
        ...
        private void CheckInMinute()
        {
            foreach (BasePlayer player in BasePlayer.activePlayerList)
            {
                if (player == null || !player.userID.IsSteamId() || PlayerStat.IgnoreReservedPlayer.ContainsKey(player.userID))
                    continue;
        ...
        {
            Int32 Top = 1;
            var RaitingNumber = PlayerStat.Players.OrderByDescending(x => x.Value.Score);
        ...
        private void ProgressAdd(BasePlayer player, Item item)
        {
            PlayerInfo Playerstat = PlayerInfo.Find(player.userID);
        ...
        private void ConsoleCommandOpenMenu(ConsoleSystem.Arg arg)
        {
            if (arg.Player() == null)
                return;
            if (PlayerStat.IgnoreReservedPlayer.ContainsKey(arg.Player().userID))
            {
        ...
        private void CmdIgnorePlayer(ConsoleSystem.Arg arg)
        {
            ...
            if (!PlayerStat.Players.ContainsKey(steamid) && !PlayerStat.IgnoreReservedPlayer.ContainsKey(steamid))
            {
                arg.ReplyWith(GetLang("STAT_CMD_5"));
                return;
            }

            PlayerInfo playerInfo = PlayerStat.Players.ContainsKey(steamid) ? PlayerStat.Players[steamid] : PlayerStat.IgnoreReservedPlayer[steamid];
            ...
            switch (arg.Args[0])
            {
                ...
                case "a":
                    {
                        if (PlayerStat.IgnoreReservedPlayer.ContainsKey(steamid))
                        {
                            arg.ReplyWith(GetLang("STAT_CMD_6", null, playerInfo.Name));
                            break;
                        }
                        PlayerStat.IgnoreReservedPlayer.Add(steamid, playerInfo);
                        PlayerStat.Players.Remove(steamid);
                ...
                case "remove":
                    {
                        ...
                        PlayerInfo info = PlayerStat.IgnoreReservedPlayer[steamid];
                        PlayerStat.Players.Add(steamid, info);
                        PlayerStat.IgnoreReservedPlayer.Remove(steamid);
