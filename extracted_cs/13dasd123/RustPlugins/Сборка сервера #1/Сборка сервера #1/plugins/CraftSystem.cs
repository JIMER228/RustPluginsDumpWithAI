...
{
    [Info("CraftSystem", "https://discord.gg/9vyTXsJyKR", "1.0.0")]
    class CraftSystem : RustPlugin
    {
        #region Вар
        string Layer = "Craft_UI";

        [PluginReference] Plugin ImageLibrary,BMenu;
        #endregion
        ...
        void CraftUI(BasePlayer player, int page = 0)
        {
            ...
            container.Add(new CuiLabel
            {
                ...
                Text = { Text = $"<size=35><b>ФРАГМЕНТЫ</b></size>\nЗдесь вы можете получить технологии, в обмен на фрагменты!", Color = "1 1 1 0.5", Align = TextAnchor.MiddleLeft, FontSize = 12, Font = "robotocondensed-regular.ttf" }
            }, "Title");
            ...
            foreach (var check in config.settings.Skip(page * 9).Take(9))
            {
                ...
                container.Add(new CuiLabel
                {
                    ...
                    Text = { Text = $"<size=20><b>{check.DisplayName}</b></size>\nДля технологии требуется {check.items.Count()} предметов! Верстак {workbench}", Color = "1 1 1 0.5", Align = TextAnchor.MiddleLeft, FontSize = 12, Font = "robotocondensed-regular.ttf" }
                }, "Text");
        ...
        void ItemUI(BasePlayer player, int z)
        {
            ...
            {
                RectTransform = { AnchorMin = "0.04 0", AnchorMax = "0.96 1", OffsetMax = "0 0" },
                Text = { Text = $"<size=20><b>ТРЕБОВАНИЯ:</b></size>\nСнизу вы можете посмотреть необходимые фрагменты для крафта!", Color = "1 1 1 0.5", Align = TextAnchor.MiddleLeft, FontSize = 12, Font = "robotocondensed-regular.ttf" }
            }, "ItemDesc");
            ...
            {
                RectTransform = { AnchorMin = "0.04 0", AnchorMax = "0.96 1", OffsetMax = "0 0" },
                Text = { Text = $"<size=22><b>{check.items.Count()}</b></size>\nФрагментов для крафта", Color = "1 1 1 0.5", Align = TextAnchor.MiddleCenter, FontSize = 12, Font = "robotocondensed-regular.ttf" }
            }, "ItemCounts");
