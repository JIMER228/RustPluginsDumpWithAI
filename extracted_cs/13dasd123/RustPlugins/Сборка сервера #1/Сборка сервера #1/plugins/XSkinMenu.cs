...
{ 
    [Info("XSkinMenu", "https://discord.gg/9vyTXsJyKR", "1.0.0")]
    class XSkinMenu : RustPlugin  
    {
			...
			public static SkinConfig GetNewConfiguration()
            {
                return new SkinConfig
                {
					...
					MenuS = new MenuSSetting
					{
						TButtonIcon = "",
						FButtonIcon = "",
						CTButton = "0.53 0.77 0.35 0.0",
						CFButton = "1 0.4 0.35 0.0"
					},
					Category = new Dictionary<string, List<string>>
					{
						...
						["item"] = new List<string> { "locker", "vending.machine", "fridge", "furnace", "table", "chair", "box.wooden.large", "box.wooden", "rug.bear", "rug", "sleepingbag", "water.purifier", "target.reactive" },
		...
		private void GUI(BasePlayer player)
		{
            ...
            {
				CursorEnabled = false,
                RectTransform = { AnchorMin = "0 0", AnchorMax = "0 0", OffsetMax = "1280 540" ,OffsetMin = "0 0"},
                Image = { Color = "0 0 0 0" }
            }, "SubContent_UI", "MainSkin");

			/*container.Add(new CuiButton
            {
                RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1" },
                Text = { Text = "" },
                Button = { Color = "0 0 0 0", Close = "MainSkin", Command = "chat.say /profile" }
            }, "MainSkin");*/

            container.Add(new CuiButton
            {
                RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-350 -260", OffsetMax = "600 290" },
                Text = { Text = "" },
			...
			container.Add(new CuiButton
            {
                RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-450 237.5", OffsetMax = "-420 265" },
            ...
            {
                RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-410 237.5", OffsetMax = "410 265" },
                Text = { Text = lang.GetMessage("TITLE", this, player.UserIDString), Align = TextAnchor.MiddleCenter, FontSize = 18, Color = "1 1 1 0.75" }
            }, ".SGUI");
			
			container.Add(new CuiPanel
            {
                RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-100 227.5", OffsetMax = "100 230.5" },
		...
		private void CategoryGUI(BasePlayer player, int page = 0)
		{
			...
			foreach(var category in config.Category)
			{
				string color = page == x ? "0.47 0.26 0.22 1" : "0 0 0 0";
				...
				container.Add(new CuiButton
                {
                    ...
                    Button = { Color = "1 1 1 0", Command = $"skin_c category {category.Key} {x}" },
                    Text = { Text = lang.GetMessage(category.Key, this, player.UserIDString), Align = TextAnchor.MiddleCenter, Font = "robotocondensed-bold.ttf", FontSize = 18, Color = "0.75 0.75 0.75 1" }
                }, ".SkinBUTTON", ".BUTTON");
		...
		private void ItemGUI(BasePlayer player, string category, int Page = 0)
		{
			...
			container.Add(new CuiPanel
            {
                RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-400 -270", OffsetMax = "400 177.5" },
                Image = { Color = "0 0 0 0" }
            }, ".SGUI", ".ItemGUI");
			
			int x = 0, y = 0, z = 0;
			
			foreach(var item in config.Category[category].Skip(Page * 36))
			{		
			    container.Add(new CuiPanel
                {
                    RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = $"{-450.5 + (x * 100)} {123.25 - (y * 100)}", OffsetMax = $"{-352.5 + (x * 100)} {218.25 - (y * 100)}" },
                    Image = { Color = "1 1 1 0" }
                }, ".ItemGUI", ".Item");
            ...
            {    
                ...
                Text = { Text = "<", Align = TextAnchor.MiddleCenter, FontSize = 26, Font = "robotocondensed-bold.ttf", Color = back ? "1 1 1 0.75" : "1 1 1 0.1" }
            }, ".ItemGUI");	
 
			container.Add(new CuiLabel
            {
                RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-15 -215.25", OffsetMax = "15 -185.25" },
                Text = { Text = $"{Page + 1}", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-bold.ttf", FontSize = 18, Color = "1 1 1 0.75" }
            }, ".ItemGUI");					
			...
			container.Add(new CuiButton
            {
                ...
                Text = { Text = ">", Align = TextAnchor.MiddleCenter, FontSize = 26, Font = "robotocondensed-bold.ttf", Color = next ? "1 1 1 0.75" : "1 1 1 0.1" }
            }, ".ItemGUI");
		...
		private void SkinGUI(BasePlayer player, string item, int Page = 0)
		{
            ...
            {
                RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-400 -270", OffsetMax = "400 177.5" },
                Image = { Color = "0.217 0.221 0.209 0" }
            }, ".SGUI", ".SkinGUI");
			
			int x = 0, y = 0;
			ulong s = StoredData[player.userID].Skins[item];
			
			foreach(var skin in StoredDataSkins[item].Skip(Page * 36))
			{		
			    container.Add(new CuiPanel
                {
                    RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = $"{-450.5 + (x * 100)} {123.25 - (y * 100)}", OffsetMax = $"{-352.5 + (x * 100)} {218.25 - (y * 100)}" },
		...
		private void SettingGUI(BasePlayer player)
		{
			...
			container.Add(new CuiLabel
            {
                ...
                Text = { Text = lang.GetMessage("SETINFO", this, player.UserIDString), Align = TextAnchor.LowerCenter, Font = "robotocondensed-bold.ttf", FontSize = 12, Color = "0.75 0.75 0.75 0.4" }
            }, ".SettingGUI");
            ...
            { 
                ...
                Image = { Color = "0.517 0.521 0.509 0.85", Material = "assets/icons/greyout.mat" }
            }, ".SettingGUI", ".SGUIM");
			...
			{		
			    ...
			    container.Add(new CuiLabel
                {
                    ...
                    Text = { Text = lang.GetMessage(s.Key, this, player.UserIDString), Align = TextAnchor.MiddleCenter, Font = "robotocondensed-bold.ttf", FontSize = 16, Color = "0.75 0.75 0.75 1" }
                }, ".SM");
