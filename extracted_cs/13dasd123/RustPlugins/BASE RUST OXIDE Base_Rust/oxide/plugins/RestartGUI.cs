...
namespace Oxide.Plugins
{
    ...
    public class RestartGUI : RustPlugin
    {
        ...
        {
            LoadConfig();
			InvokeHandler.Instance.InvokeRepeating(CheckTime, 1f, 1f);
        }

		void Unload()
		{
			InvokeHandler.Instance.CancelInvoke(CheckTime);
		}
        ...
        private void ExecuteRestart(int time)
        {
            ...
            CuiElementContainer container = new CuiElementContainer
            {
                ...
                new CuiElement
                {
                    ...
                    Components =
                    {
                        new CuiTextComponent{ Align = TextAnchor.MiddleCenter, Color = "1 1 1 1", FontSize = 16, Text = string.Format(config.RestartTimerText, $"{RestartConfig.Time}")},
                        new CuiOutlineComponent { Color = "0 0 0 0.5", Distance = "0.7 0.7" },
            ...
            foreach (var player in BasePlayer.activePlayerList) EffectNetwork.Send(new Effect(config.Effect, player, 0, Vector3.zero, Vector3.forward), player.net.connection);
