...
{
    [Info("Oysters", "https://discord.gg/9vyTXsJyKR", "1.0.0")]  
    class Oysters : RustPlugin
    {

		[PluginReference]
  		private Plugin Farmer, Friends;	
        ...
        object OnItemAction(Item item, string action, BasePlayer player)
        {
            ...
            if (action == "Gut" && item.skin == config.SkinID)
            {
				...
				if (item.text == player.UserIDString || config.EnableFriendsFarm && AreFriends(player.userID, GetUL(item.text)))
					Farmer?.Call("API_AddPlayerScores", player.userID, "oyster", 2);
		...
		private bool AreFriends(ulong userID1, ulong userID2)
		{
			var areFriends = (bool?)Friends.CallHook("AreFriends", userID1, userID2);
			if (areFriends == true)	
				return true;
			
			return AreTeamFriends(userID1, userID2);
		}
		
		private static bool AreTeamFriends(ulong playerID, ulong targetID) 
		{
			var player = BasePlayer.activePlayerList.FirstOrDefault(x=> x.userID == playerID) ?? BasePlayer.sleepingPlayerList.FirstOrDefault(x=> x.userID == playerID);
			if (player == null) return false;
			var target = BasePlayer.activePlayerList.FirstOrDefault(x=> x.userID == targetID) ?? BasePlayer.sleepingPlayerList.FirstOrDefault(x=> x.userID == targetID);
			if (target == null) return false;
			
			return player.currentTeam != (long)0 && player.currentTeam == target.currentTeam;
		}
        ...
        private class CustomTraps : BaseEntity
        {
			...
			private void Awake()
            {
                ...
                InvokeHandler.InvokeRepeating(this, RandomTrap, time, time); // запуск таймера с привязкой к классу, а не ловушке, как было раньше, т.к. после перезагрузки старый таймер оставался и спавн производился чаще
            }            
			...
			// метод с фиксом исключающим хавание рыбок ловушкой вместо обычной еды
		...
		// фиксанутый MoveToContainer с учетом скинов, это нужно что бы стакались ракушки в кормушке
					...
					{						
						...
						{							
							...
							{						
								...
								//ins.Puts($"Новый предмет {itemBase.info.shortname} {itemBase.amount}, добавляем к {item1.info.shortname} {item1.amount}");
        ...
        [ChatCommand("riba")]
        void cmdTopOysters(BasePlayer player, string command, string[] args )
        {
            if (player == null || args == null || args.Length <= 0 || args.Length < 1) return;

            if (args[0] == "top")
            {
                int i = 1;
                var top = PlayersList.Where(x=> x.Value != null && covalence.Players.FindPlayerById(x.Key.ToString()) != null).ToList().OrderByDescending(p => p.Value.Opened).Select(p => $"{i++} <color=#8ABB50>{covalence.Players.FindPlayerById(p.Key.ToString()).Name}</color>: {p.Value.Opened}").Take(10);
                SendReply(player, $"ТОП Ловцов устриц за текущий вайп:\n{string.Join("\n", top)}");
            }           
        }
