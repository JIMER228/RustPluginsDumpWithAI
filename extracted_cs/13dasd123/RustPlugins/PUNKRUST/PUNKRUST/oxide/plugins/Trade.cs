...
{
    [Info("Trade", "discord.gg/9vyTXsJyKR", "1.2.14")]
    public class Trade : RustPlugin
    {
        ...
        private static readonly Dictionary<uint, TradeController> Trades = new Dictionary<uint, TradeController>();
        ...
        private object CanMoveItem(Item item, PlayerInventory playerLoot, uint targetContainer)
        {
        ...
        private static TradeController GetTrade(uint shopFrontId)
        {
            TradeController tradeCtrl;
            return !Trades.TryGetValue(shopFrontId, out tradeCtrl) ? null : tradeCtrl;
        }

        private static ShopFront GetShopFront(uint id)
        {
            TradeController tradeCtrl;
        ...
        private static void StartLooting(uint shopId, BasePlayer player)
        {
