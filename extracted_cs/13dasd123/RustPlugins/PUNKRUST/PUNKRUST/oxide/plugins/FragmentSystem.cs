...
namespace Oxide.Plugins
{
    ...
    public class FragmentSystem : RustPlugin
    {
        ...
        {
            ImageLibrary.Call("AddImage", "https://i.ibb.co/LdwfCmB/fbg.png","fragsmall");
            ImageLibrary.Call("AddImage", "https://i.ibb.co/0QZxXh8/Back-Panel.png","fragfull");
        ...
        void FragmentUI(BasePlayer player,int page = 0)
        {
            ...
            container.Add(new CuiPanel
            {
                Image = {Color = HexToRustFormat("f8db7d")},
            ...
            /*for (int i = 0; i < 4; i++)
            {
                container.Add(new CuiPanel
                {
                    Image = {Color = HexToRustFormat("f8db7d97")},
            ...
            {
                Button = {Color = page>0?HexToRustFormat("#f8db7d"):HexToRustFormat("#f8db7db7"),Command = page>0?$"fragments {page-1}":$"fragments {page}"},
                RectTransform = {AnchorMin = "0 0.5",AnchorMax = "0 1",OffsetMin = "10 0",OffsetMax = "25 0"},
                Text = {Text = "↑",Align = TextAnchor.MiddleCenter}
            }, _layer);
            container.Add(new CuiButton
            {
                Button = {Color = config.Settings.Count>(page+1)*4?HexToRustFormat("#f8db7d"):HexToRustFormat("#f8db7db7"),Command = config.Settings.Count>(page +1)*4?$"fragments {page+1}":$"fragments {page}"},
            ...
            foreach (var CompleteItem in config.Settings.Skip(page*4).Take(4))
            {
                ...
                container.Add(new CuiLabel
                {
                    ...
                    Text = {Text = CompleteItem.Value.DisplayName,Align = TextAnchor.MiddleCenter,FontSize = 21,Color = "0 0 0 1"}
                }, $"{_layer}{CompleteItem.Key}");
                container.Add(new CuiLabel
                {
                    RectTransform = {AnchorMin = "0 0",AnchorMax = "0.8 1",OffsetMin = "132.2 3",OffsetMax = "0 -28"},
                    Text = {Text = CompleteItem.Value.Description,Align = TextAnchor.MiddleLeft,FontSize = 18,Color = "0 0 0 1"}
                }, $"{_layer}{CompleteItem.Key}");

                container.Add(new CuiButton
                {
                    RectTransform = {AnchorMin = "1 0",AnchorMax = "1 1",OffsetMin = "-105 5",OffsetMax = "-5 -5"},
                    Button =
                    {
                        Color = "0 0 0 1", Command = $"choosefragment {CompleteItem.Key}"
                    },
                    Text = {Color = HexToRustFormat("#f8db7de6"),Text = "ПОДРОБНЕЕ",FontSize = 18,Align = TextAnchor.MiddleCenter},
                }, $"{_layer}{CompleteItem.Key}");
        ...
        void CraftFragmentUI(BasePlayer player, string fragKey)
        {
            ...
            container.Add(new CuiButton
            {
                Button = {Color = "0 0 0 1",Command = $"fragmentcraft {fragKey}"},
                Text = {Text = "СОБРАТЬ",Color = HexToRustFormat("#f8db7d"),FontSize = 18,Align = TextAnchor.MiddleCenter},
