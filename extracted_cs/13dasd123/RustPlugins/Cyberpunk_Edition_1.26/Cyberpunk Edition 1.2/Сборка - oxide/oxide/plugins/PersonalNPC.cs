...
{
    [Info("PersonalNPC", "Netrunner", "1.0")]
    public class PersonalNPC : RustPlugin 
    {
        public static PersonalNPC Instance;

        [PluginReference] private Plugin ImageLibrary, VehicleDeployedLocks, MarkerManager, PNPCAddonHeli;
            ...
            if(IsBot(player))
            {
                ...
                if(botComponent != null)
                {
                    ...
                    if(controller != null)
                    {
                        ...
                        if(controller.botSetup.dropActiveItem && controller.botSetup.functions.enableBotInventory)
                        {
        ...
        private void DropLoot(BasePlayer player, PlayerBotController controller, HitInfo info)
        {
            ...
            if(_config.enableBackpackOnDeath)
            {
                ...
                droppedContainer.TakeFrom(new ItemContainer[] { player.inventory.containerMain, player.inventory.containerWear, player.inventory.containerBelt });
                        ...
                        foreach(var collider in allDetected)
                        {
                            if(collider != null)
                            {
                                ...
                                if(player != null)
                                {
                                    ...
                                    if(botSetup.functions.pvpSetup.ignorePersonalNPC && plugin.GetOwnerComponent(player.net.ID)) continue;
