...
{
    [Info("Skill Tree", "Netrunner", "1.0.0")]
    [Description("Скилы")]
    class SkillTree : RustPlugin
    {
        ...
        void SetPicker(BasePlayer player)
        {
            ...
            {
                PrintToChat(player, "You still have your Lock Pick ability active. Attempt to open a locked door that you are not authorized on before the timer runs out.");
                return;
            }
            ...
            if (buffDetails.TryGetValue(player.userID, out bd) && bd.buff_values.TryGetValue(Buff.Lock_Picker, out value))
            {
                if (LockPickCooldowns.ContainsKey(player.userID))
                {
                    ...
                    {
                        PrintToChat(player, "Your lock pick ability is still on cooldown.");
                        return;
                    }
                ...
                PrintToChat(player, $"You have activated your Lock Pick ability. It will remain active for {config.buff_settings.raid_perk_settings.Lock_Picker_settings.time} seconds or until you attempt to open a locked entity that you do not have access to.");

                pickerData.timer = timer.Once(config.buff_settings.raid_perk_settings.Lock_Picker_settings.time, () =>
                {
        ...
        object CanUseLockedEntity(BasePlayer player, BaseLock baseLock)
        {
            ...
            {
                PrintToChat(player, $"You manage to pick the lock and break into the door. Ability is on cooldown for {config.buff_settings.raid_perk_settings.Lock_Picker_settings.use_delay} seconds.");
                DestroyPicker(player, pickerData);
                return true;
            }
            PrintToChat(player, $"You failed to pick the lock. You can attempt another lock pick in {config.buff_settings.raid_perk_settings.Lock_Picker_settings.use_delay} seconds.");
            DestroyPicker(player, pickerData);
            return null;
        }
        ...
        object OnEntityTakeDamage(BaseCombatEntity entity, HitInfo info)
        {
            ...
            if (lootContainer != null && AttackerIsRealPlayer)
            {
                ...
                if (buffDetails.TryGetValue(info.InitiatorPlayer.userID, out bd))
                {
                    if (bd.buff_values.ContainsKey(Buff.Barrel_Smasher) && !IsRoadsign(lootContainer.ShortPrefabName)) info?.damageTypes?.ScaleAll(100f);
        ...
        object HandleDispenser(ResourceDispenser dispenser, BasePlayer player, Item item, bool bonus = false)
        {
            ...
            if (dispenser.gatherType == ResourceDispenser.GatherType.Tree)
            {
                ...
                if (!bonus)
                {
                    ...
                    {
                        if (tool != null && config.tools_black_white_list_settings.wc_tools.Contains(tool.info.shortname)) AwardXP(player, config.xp_settings.xp_sources.TreeHit * xp_modifier, dispenser.baseEntity);
                    }
                    else AwardXP(player, config.xp_settings.xp_sources.TreeHit * xp_modifier, dispenser.baseEntity);
                }
            ...
            if (dispenser.gatherType == ResourceDispenser.GatherType.Ore)
            {
                ...
                if (!bonus)
                {
                    ...
                    {
                        if (tool != null && config.tools_black_white_list_settings.mining_tools.Contains(tool.info.shortname)) AwardXP(player, config.xp_settings.xp_sources.NodeHit * xp_modifier, dispenser.baseEntity);
                    }
                    else AwardXP(player, config.xp_settings.xp_sources.NodeHit * xp_modifier, dispenser.baseEntity);
                }
            ...
            if (dispenser.gatherType == ResourceDispenser.GatherType.Flesh)
            {
                ...
                if (!bonus)
                {
                    ...
                    {
                        if (tool != null && config.tools_black_white_list_settings.skinning_tools.Contains(tool.info.shortname)) AwardXP(player, (isFinalHit ? config.xp_settings.xp_sources.SkinHitFinal : config.xp_settings.xp_sources.SkinHit) * xp_modifier, dispenser.baseEntity);
                    }
                    else AwardXP(player, (isFinalHit ? config.xp_settings.xp_sources.SkinHitFinal : config.xp_settings.xp_sources.SkinHit) * xp_modifier, dispenser.baseEntity);
                }
                ...
                if (!bonus && bd.buff_values.ContainsKey(Buff.Instant_Skin) && RollSuccessful((bd.buff_values[Buff.Instant_Skin] * luck_modifier)))
                {
                    foreach (var r in dispenser.containedItems)
                    {
                        if (r.amount < 1) continue;
                ...
                if (bonus && bd.buff_values.ContainsKey(Buff.Skinning_Luck) && RollSuccessful((bd.buff_values[Buff.Skinning_Luck] * luck_modifier)) && config.loot_settings.skinning_loot_table.Count > 0)
                {
        ...
        object OnDispenserBonus(ResourceDispenser dispenser, BasePlayer player, Item item)
        {
            ...
            if (dispenser.gatherType == ResourceDispenser.GatherType.Tree)
            {
                ...
                float value;
                if (!UltimateTriggered || (config.ultimate_settings.ultimate_woodcutting.award_xp)) AwardXP(player, config.xp_settings.xp_sources.TreeHitFinal * xp_modifier, dispenser.baseEntity);
            ...
            else if (dispenser.gatherType == ResourceDispenser.GatherType.Ore)
            {
                ...
                AwardXP(player, config.xp_settings.xp_sources.NodeHitFinal * luck_modifier, dispenser.baseEntity);
        ...
        void OnEntityDeath(BaseEntity entity, HitInfo info)
        {
            ...
            switch (entity.ShortPrefabName)
            {
                ...
                case "stone-ore":
                    ...
                    {
                        var heldEntity = player.GetHeldEntity();
                        if (bd.buff_values.ContainsKey(Buff.Node_Spawn_Chance) && RollSuccessful(bd.buff_values[Buff.Node_Spawn_Chance] * (heldEntity != null && heldEntity is Jackhammer ? config.tools_black_white_list_settings.power_tool_modifier.mining_luck_modifier : 1f)))
                        {
            ...
            if (pcdData.pEntity.TryGetValue(player.userID, out playerData))
            {
                ...
                if (xp_loss > 0 && (config.xp_settings.xp_loss_settings.no_xp_loss_time == 0 || playerData.last_xp_loss.AddSeconds(config.xp_settings.xp_loss_settings.no_xp_loss_time) < DateTime.Now))
                {
                    ...
                    if (playerData.xp - LevelStartXP > xp_loss)
                    {
                        ...
                        PrintToChat(player, string.Format(lang.GetMessage("LostXP", this, player.UserIDString), Math.Round(xp_loss, config.xp_settings.xp_rounding)));
                    }
                    else
                    {
                        if (config.xp_settings.xp_loss_settings.allow_xp_debt)
                        {
                            ...
                            PrintToChat(player, string.Format(lang.GetMessage("AccumulatedXPDebt", this, player.UserIDString), Math.Round(excess, config.xp_settings.xp_rounding), Math.Round(playerData.xp_debt, config.xp_settings.xp_rounding)));
        ...
        void OnServerInitialized(bool initial)
        {
            ...
            cmd.AddChatCommand(config.buff_settings.raid_perk_settings.Lock_Picker_settings.pick_command.Trim(), this, "SetPicker");
            ...
            Puts($"Added {BaseYieldOverrides.Count} resource types to modify the base yields for.");
            ...
            cmd.AddChatCommand(config.ultimate_settings.ultimate_raiding.command, this, "CallRocketStrike");
            ...
            if (playerData.current_level < level)
            {
                ...
                if (config.general_settings.max_player_level == 0 || level <= config.general_settings.max_player_level)
                {
                    ...
                    if (playerData.achieved_level < level)
                    {
                        ...
                        if (max_skill_points == 0 || (playerData.current_level * config.general_settings.points_per_level + config.wipe_update_settings.starting_skill_points < max_skill_points)) playerData.available_points += config.general_settings.points_per_level * (level - playerData.current_level);
        ...
        {
            if (Interface.CallHook("STCanGainXP", player, plugin) != null) return;
            AwardXP(player, value, null, noMod, true);
        }

        [HookMethod("AwardXP")]
        public void AwardXP(BasePlayer player, double value, BaseEntity source = null, bool noMod = false, bool hookAlreadyCalled = false)
        {
            ...
            if (!hookAlreadyCalled && Interface.CallHook("STCanGainXP", player, source ?? null) != null) return;
            ...
            foreach (var cfg in config.trees)
            {
                ...
                foreach (var node in cfg.Value.nodes)
                {
                    ...
                    if (nodeData.tier == 4)
                    {
                        ...
                        {
                            string ult_str = config.ultimate_settings.ultimate_skinning.enabled_buffs.FirstOrDefault(x => x.Value > 0).Value > 0 ? string.Format(string.Join("</color>, <color=#42f105>", config.ultimate_settings.ultimate_skinning.enabled_buffs.Where(x => x.Value > 0).Select(x => x.Key.ToString()))) : "";
                            nodeData.description = string.Format(lang.GetMessage(nodeData.buffInfo.Key.ToString(), this, player.UserIDString), ult_str);
                        }
                        ...
                        else if (nodeData.buffInfo.Key == Buff.Combat_Ultimate)
                        {
                            ...
                            {
                                formattedString += "<color=#42f105>Scientists</color>";
                                if (active == 2) formattedString += " and ";
                                if (active == 3) formattedString += ", ";
                            }
                            if (config.ultimate_settings.ultimate_combat.animals_enabled)
                            {
                                formattedString += "<color=#42f105>Animals</color>";
                                if (active == 3) formattedString += " and ";
                            }
                            if (config.ultimate_settings.ultimate_combat.players_enabled) formattedString += "<color=#42f105>Players</color>";
        ...
        string AddAdditionalDescription(Buff buff, string userid)
        {
            switch (buff) 
            {
                ...
                case Buff.Raiding_Ultimate: return string.Format(lang.GetMessage("Buff.Raiding_Ultimate_Extended.Description", this, userid), config.ultimate_settings.ultimate_raiding.command, config.ultimate_settings.ultimate_raiding.cooldown);
                ...
                case Buff.Animal_Damage_Resist: return string.Format(lang.GetMessage("Buff.Animal_Damage_Resist.Animals", this, userid), string.Join(", ", config.buff_settings.animals));
        ...
        void MoveBar(BasePlayer player)
        {
            ...
            {
                PrintToChat(player, lang.GetMessage("MoveXPDisabled", this, player.UserIDString));
                return;
            }
        ...
        void OnEntityKill(BaseEntity entity)
        {
            ...
            if (container != null)
            {
                ...
                if (!containers.ContainsKey(container.inventory.uid)) return;
        ...
        private object OnExplosiveDud(DudTimedExplosive dudTimedExplosive)
        {
            ...
            {
                PlayerInfo pi;
                if (pcdData.pEntity.TryGetValue(player.userID, out pi) && pi.notifications) PrintToChat(player, $"You explosive was a dud, but it exploded anyway!");
                return false;
            }
        ...
        private void SkillTree_UltimateMenu(BasePlayer player)
        {
            ...
            container.Add(new CuiElement
            {
                ...
                Components = {
                    new CuiTextComponent { Text = "АКТИВНЫЕ СПОСОБНОСТИ", Font = "robotocondensed-bold.ttf", FontSize = 26, Align = TextAnchor.MiddleCenter, Color = "1 1 1 1" },
                    new CuiOutlineComponent { Color = "0 0 0 0.5", Distance = "1 -1" },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-495.24 190.4", OffsetMax = "-180.18 250.4" }
                }
            });
            ...
            if (pi.ultimate_settings.Count == 0)
            {
                container.Add(new CuiElement
                {
                    ...
                    Parent = "SkillTree_UltimateMenu",
                    Components = {
                    new CuiTextComponent { Text = "У ВАС НЕТ АКТИВНЫХ НАВЫКОВ.", Font = "robotocondensed-regular.ttf", FontSize = 14, Align = TextAnchor.MiddleCenter, Color = "1 0.9397521 0 1" },
                    new CuiOutlineComponent { Color = "0 0 0 0.5", Distance = "1 -1" },
        ...
        void HandleUltimateToggle(BasePlayer player, Buff buff, PlayerInfo pi)
        {
            ...
            {
                if (ups.enabled)
                    PrintToChat(player, string.Format(lang.GetMessage("UltimateToggleOnRaiding", this, player.UserIDString), config.ultimate_settings.ultimate_raiding.command, config.ultimate_settings.ultimate_raiding.cooldown));
        ...
        void ResetMLRS(BasePlayer player)
        {
            ...
            PrintToChat(player, "Your MLRS timer has been reset.");
        }
        ...
        void CallRocketStrike(BasePlayer player)
        {
            ...
            {
                PrintToChat(player, "This ability is already active.");
                return;
            }
            ...
            {
                PrintToChat(player, "This command requires the Raiding Ultimate to be unlocked.");
                return;
            }
            ...
            {
                PrintToChat(player, "You must have 1 free inventory slot available in your belt.");
                return;
            }
            ...
            if (pcdData.pEntity.TryGetValue(player.userID, out pi))
            {
                if (pi.raiding_ultimate_used_time.AddMinutes(config.ultimate_settings.ultimate_raiding.cooldown) > DateTime.Now)
                {
                    ...
                    //PrintToChat(player, $"You cannot use this ultimate again until <color=#ffae00>{pi.raiding_ultimate_used_time.AddMinutes(config.ultimate_settings.ultimate_raiding.cooldown).ToString("hh:mm:ss tt")}</color>.");
                    PrintToChat(player, $"You cannot use this ultimate again for another <color=#ffae00>{(span.TotalMinutes > 1 ? Math.Round(span.TotalMinutes, 0) + " minutes" : Math.Round(span.TotalSeconds, 0) + " seconds")}</color>.");
                    return;
                }
            ...
            {
                PrintToChat(player, "Failed to give binoculars due to stacking issues.");
                item.Remove();
                return;
            }            
            ...
            PrintToChat(player, "You have received some binoculars. Use them to set your target.");
        ...
        static private void SendInstructions(BasePlayer player)
        {
            var container = new CuiElementContainer();
            container.Add(new CuiElement
            {
                ...
                Parent = "ContentUI",
                Components = {
                    new CuiTextComponent { Text = "LOOK THROUGH YOUR <color=#ffb600>BINOCULARS</color> AND HOLD <color=#ffb600>E</color> TO SET YOUR TARGET", Font = "robotocondensed-regular.ttf", FontSize = 20, Align = TextAnchor.MiddleCenter, Color = "1 1 1 1" },
                    new CuiOutlineComponent { Color = "0 0 0 0.5", Distance = "1 -1" },
        ...
        void ResetStrikeCooldown(ulong userid, BasePlayer player = null)
        {
            ...
            if (player != null) PrintToChat(player, "Your strike cooldown has been reset as it timed out.");
        }
