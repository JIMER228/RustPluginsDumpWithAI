...
namespace Oxide.Plugins
{
    ...
    public class Expedition : RustPlugin
    {
        ...
        void OnServerInitialized()
        {
            ...
            LoadData();
            LoadImages();
            ImageLibrary.Call("AddImage", "https://lc-crb.ru/CyberPunk/CJZ923P.png","eitem");
            ImageLibrary.Call("AddImage", "https://lc-crb.ru/CyberPunk/naPbhDD.png","eback"); 
            ImageLibrary.Call("AddImage", "https://lc-crb.ru/CyberPunk/CvjN8DD.png","elock");
        ...
        {
            EconomicsEvo.Call("RemoveBalance", userid, config.CurrencyKey, config.Expeditions[key].Price);
        ...
        {
            if (!data.IsOpen) return "ЗАКЛЮЧИТЬ КОНТРАКТ";

            if (data.IsActive)
                return "СНАРЯЖЕНИЕ ОТПРАВЛЕНО";
            if (data.Inventory[0] != null || data.Inventory[1] != null || data.Inventory[2] != null ||
                data.Inventory[3] != null)
                return "ЗАБРАТЬ СНАРЯЖЕНИЕ";
            else
                return "ЗАПРОСИТЬ ПОСТАВКУ";
        }
        ...
        void ChooseUI(BasePlayer player)
        {
            ...
            foreach (var expedition in config.Expeditions)
            {
                ...
                {
                    Button = {Color = "0 0 0 1",Command = $"expchoose {expedition.Key}"},
                ...
                container.Add(new CuiButton
                {
                    ...
                    RectTransform = {AnchorMin = "0 1",AnchorMax = "1 1",OffsetMin = "5 -20",OffsetMax = "-5 0"}
                }, _layer + expedition.Key);
        ...
        void ExpeditionUI(BasePlayer player, string key)
        {
            ...
            {
                RectTransform = {AnchorMin = "0 1", AnchorMax = "0 1", OffsetMin = "220 -165", OffsetMax = "640 -45"},
            ...
            container.Add(new CuiLabel
            {
                RectTransform = {AnchorMin = "0 1",AnchorMax = "0 1",OffsetMin = "220 -280",OffsetMax = "640 -160"},
            ...
            container.Add(new CuiButton
            {
                Button = {Command =GetButtonCommand(data,key),Color = HexToRustFormat("#952F2D97")},
                Text = {Text = GetButtonText(data),Align = TextAnchor.MiddleCenter,Color = HexToRustFormat("#FB6F69FF")},
            ...
            if (data.IsActive)
            {
                container.Add(new CuiLabel
                {
                    ...
                    Text = {Text = new TimeSpan(data.EndTime.Ticks-DateTime.UtcNow.Ticks).ToShortString().Replace('-',' '),Align = TextAnchor.LowerCenter,Color = "0 0 0 0"}
                }, "ExBtn");
            }
            container.Add(new CuiLabel
            {
                RectTransform = {AnchorMin = "0 1",AnchorMax = "0 1",OffsetMin = "370 -420",OffsetMax = "800 -360"},
                Text = {Align = TextAnchor.MiddleLeft,Text = $"СТОИМОСТЬ ПАКЕТА СНАРЯЖЕНИЯ: <color=#EBD4AE>{info.Price} РЕПУТАЦИИ</color>",Color = HexToRustFormat("#5FF6FE95")},
            }, _layer,"Price");
            container.Add(new CuiButton
            {
                Button = {Command = $"ereward {key}",Color = HexToRustFormat("#952F2D97")},
                Text = {Text = "СПИСОК СНАРЯЖЕНИЯ",Align = TextAnchor.MiddleCenter,Color = HexToRustFormat("#FB6F69FF")},
        ...
        void RewardUI(BasePlayer player, string key)
        {
            ...
            container.Add(new CuiPanel
            {
                Image = {Color = HexToRustFormat("#2B1717")},
            ...
            container.Add(new CuiLabel
            {
                ...
                Text = {Text = $"СНАРЯЖЕНИЕ ФИКСЕРА",Align = TextAnchor.LowerLeft,FontSize = 24}
            }, "RewardContainer");

            container.Add(new CuiButton
            {
                Button = {Close = "ExpReward",Color = HexToRustFormat("#992E2E97")},
                Text = {Text = "ЗАКРЫТЬ",Align = TextAnchor.MiddleCenter,FontSize = 18,Color = HexToRustFormat("#FB6F69FF")},
