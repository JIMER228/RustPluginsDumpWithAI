...
{
    [Info("VKBot", "SkiTles | Ernieleo Mod", "1.7.5")]
    class VKBot : RustPlugin
    {
        ...
        private class ConfigData
        {
            ...
            public class AdminNotify
            {
                ...
                [JsonProperty(PropertyName = "SteamID администратора для Rust+(пример 76561198368556237)")]
                public string STID { get; set; } = "Заполните и выполните o.reload VKBot";
            }
            public class ChatNotify
            {
                ...
                [JsonProperty(PropertyName = "Дополнительная отправка оповещений в RUST+")]
                public bool RUSTPLUS { get; set; } = false;
        ...
        private void OnPlayerBanned(string name, ulong id, string address, string reason, string msg2 = null)
        {
            ...
            if (config.AdmNotify.UserBannedMsg)
            {
                ...
                else
                {
                    ...
                    if (config.ChNotify.RUSTPLUS)
                        NotificationList.SendNotificationTo(Convert.ToUInt64(config.AdmNotify.STID), NotificationChannel.SmartAlarm, "BAN PLAYER", msg2, new Dictionary<string, string>());
                }
            }
        }
        #endregion
        ...
        private void CheckPlugins()
        {
            ...
            if (unloadedPluginErrors.Count > 0)
            {
                ...
                else
                {
                    SendVkMessage(config.AdmNotify.VkID, text);
                    if (config.ChNotify.RUSTPLUS)
                        NotificationList.SendNotificationTo(Convert.ToUInt64(config.AdmNotify.STID), NotificationChannel.SmartAlarm, "ERROR PLUGINS", text, new Dictionary<string, string>());
                } 
            }
        }
        ...
        private void CheckOxideUpdate()
        {
            ...
            webrequest.Enqueue("https://umod.org/games/rust.json", null, (code, response) => 
            {
                if (code == 200 || response != null)
                {
                    ...
                    { 
                        SendVkMessage(config.AdmNotify.VkID, $"Доступно новое обновление Oxide {latestver}. https://umod.org/games/rust");
                        if (config.ChNotify.RUSTPLUS)
                            NotificationList.SendNotificationTo(Convert.ToUInt64(config.AdmNotify.STID), NotificationChannel.SmartAlarm, "OXIDE UPDATE", $"Доступно новое обновление Oxide {latestver}. https://umod.org/games/rust", new Dictionary<string, string>());
        ...
        void UnloadGUI(BasePlayer player)
        {
                CuiHelper.DestroyUi(player, "MainUI");
                CuiHelper.DestroyUi(player, "HelpUI");
                CuiHelper.DestroyUi(player, "AddVKUI");
        ...
        private void StartVKBotMainGUI(BasePlayer player)
        {
            ...
            container.Add(Text("MainUI", "1 1 1 1", $"Группа сервера в ВК:\n<color=#0390fc>{config.GrGifts.VKGroupUrl}</color>", TextAnchor.MiddleCenter, 18, "0.01 0.06", "0.99 0.17"));         
