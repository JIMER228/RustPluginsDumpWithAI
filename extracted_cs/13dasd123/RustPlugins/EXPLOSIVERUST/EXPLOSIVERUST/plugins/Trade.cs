...
{
    [Info("Trade", "https://topplugin.ru / https://discord.com/invite/5DPTsRmd3G", "1.0.3")]
    public class Trade : RustPlugin
    {
        ...
        void Unload()
        {
            foreach (var trade in m_Boxes.ToList())
            {
                OnTradeCanceled(trade.Value.guid);
                Destroy(trade.Key);
        ...
        private void OnShopCompleteTrade(ShopFront shop)
        {
            ...
            if (trade != null)
            {
                SendReply(trade.player1, "<color=#ee3e61>Система обмена</color>\nTrade успешно завершён!");
                SendReply(trade.player2, "<color=#ee3e61>Система обмена</color>\nTrade успешно завершён!");
        ...
        void CmdChatTrade(BasePlayer player, string command, string[] args)
        {
            ...
            {
                SendReply(player, "Используйте:\n<color=#ee3e61>/trade ник игрока</color> для обмена\nЧто бы принять обмен, введите: <color=#ee3e61>/trade accept</color>\nЧто бы отказаться от обмена введите: <color=#ee3e61>/trade cancel </color>");
                return;
            }
            if (UsePermission && !permission.UserHasPermission(player.UserIDString, Permission))
            {
                SendReply(player, "<color=#ee3e61>Система обмена</color>\nНедоступно, у Вас нету прав на использование трейда!");
                return;
            }
            if (Cooldowns.ContainsKey(player))
            {
                ...
                if (seconds >= 0)
                {
                    SendReply(player, $"<color=#ee3e61>Система обмена</color>\nВы только недавно обменивались, подождите - <color=#ee3e61>{seconds}</color> сек.");
                    return;
                }
            }
            switch (args[0])
            {
                default:
                    ...
                    if (target == null)
                    {
                        SendReply(player, $"<color=#ee3e61>Система обмена</color>\nИгрок <color=#ee3e61>{name}</color> не найден!");
                        return;
                    }
                    if (target == player)
                    {
                    ...
                    {
                        SendReply(player, $"<color=#ee3e61>Система обмена</color>\nНедоступно, у игрока {target.displayName} нет прав на использование трейда!");
                        return;
                    }
                    if (getCupSend)
                    {
                    ...
                    {
                        SendReply(player, $"<color=#ee3e61>Система обмена</color>\nНевозможно! Игрок <color=#ee3e61>{anotherTarget.displayName}</color> уже отправил вам предложение обмена!");
                        return;
                    }
                    pendings[target] = player;
                    SendReply(player, $"Предложение обмена игроку <color=#ee3e61>{target.displayName}</color> успешно отправлено, ожидайте...");
                    SendReply(target, $"Игрок <color=#ee3e61>{player.displayName}</color> отправил вам предложние обмена\nДля принятия обмена используйте команду <color=#ee3e61>/trade accept</color>\nЧто бы отказаться введите <color=#ee3e61>/trade cancel</color>");
                    mytimer = timer.Once(getTime, () =>
                    {
                        if (target != null && player != null && player.IsConnected && target.IsConnected && pendings.ContainsKey(target))
                        {
                            ...
                            SendReply(target, "<color=#ee3e61>Система обмена</color>\nTrade отменён! Причина: вы вовремя не приняли запрос.");
                        }
                    }
                    );
                    break;
                ...
                case "cancel":
                    ...
                    pendings.Remove(player);
                    if (player2?.IsConnected == true) SendReply(player2, $"<color=#ee3e61>Система обмена</color>\nTrade отменён! Причина: игрок <color=#ee3e61>{player.displayName}</color> отказался");
                    SendReply(player, "<color=#ee3e61>Система обмена</color>\nTrade отменен!");
                    break;
            }
        }
        ...
        public static string Create(BasePlayer player1, BasePlayer player2, int getInt)
        {
            ...
            m_Instance.timer.Once(0.1f, (Action)(() => StartLooting(guid, player1)));
            m_Instance.timer.Once(0.5f, (Action)(() => StartLooting(guid, player2)));
            return guid;
        }
        ...
        public static T AddComponent<T>(string a) where T : Component
        {
            ...
            {
                throw new InvalidOperationException($"AddBehaviour: TradeBox for {b} not Found");
            }
        ...
        {
            if (item == null || item?.uid == null) return;
            ...
            if (container != null) if (boxes.FirstOrDefault(p => p.Value == container).Key != null)
        ...
        private void OpenBox(BasePlayer player1, BasePlayer player2, ulong playerid = 1)
        {
        ...
        private void OnTradeCanceled(string guid)
        {
            ...
            if (m_Boxes.TryGetValue(guid, out trade))
            {
                ...
                {
                    for (int i = trade.shop.customerInventory.itemList.Count - 1;
                    i >= 0;
                    i--)
                    {
                ...
                {
                    for (int i = trade.shop.vendorInventory.itemList.Count - 1;
                    i >= 0;
                    i--)
                    {
        ...
        public static void StartLooting(string guid, BasePlayer player)
        {
            ...
            player.inventory.loot.SendImmediate();
            if (shopFront.customerPlayer == null) shopFront.customerPlayer = player;
