...
{
    [Info("Timed Permissions", "LaserHydra", "1.3.2")]
    ...
    internal class TimedPermissions : CovalencePlugin
    {
        ...
        private IPlayer GetPlayer(string nameOrID, IPlayer player)
        {
            ...
            switch (foundPlayers.Count)
            {
                ...
                default:
                    ...
                    //player.Reply("<color=#F5DA81>НАЙДЕНО НЕСКОЛЬКО ИГРОКОВ:</color> \n" + string.Join(", ", names));
                    break;
            }
        ...
        private void CmdPlayerInfo(IPlayer player, string cmd, string[] args)
        {
            ...
            {
				string GetPrivilage = "Информация об игроке:\n\n";
				foreach (var g in pl.Permissions)
                {
                    string msg1 = GetMessage("Привилегия: <color=#CF1E1E>{groups}</color>\nОставшееся время: <color=#CF1E1E>{timeleft}</color>\n", player.Id);
                ...
                {
                    string msg1 = GetMessage("Группа: <color=#CF1E1E>{groups}</color>\nОставшееся время: <color=#CF1E1E>{timeleft}</color>\n", player.Id);
                    TimeSpan result = (g.ExpireDate).Subtract(DateTime.UtcNow);
                    msg1 = msg1.Replace("{groups}", $"{g.Value}");
                    msg1 = msg1.Replace("{timeleft}", $"{FormatTime(TimeSpan.FromDays(result.TotalDays))}");
                    GetPrivilage = GetPrivilage + msg1 + "\n";
        ...
        private void CmdServiceInfo(IPlayer player, string cmd, string[] args)
        {
            ...
            {
                string GetPrivilage = "Время оставшихся услуг:\n\n";
                foreach (var g in pl.Permissions)
                {
					string msg1 = GetMessage("PermissionsInfo", player.Id);
                    //string msg1 = GetMessage("<size=16>Привилегия:</size> <color=#CF1E1E>{groups}</color>\n<size=16>Оставшееся время:</size> <color=#CF1E1E>{timeleft}</color>\n", player.Id);
                    TimeSpan result = (g.ExpireDate).Subtract(DateTime.UtcNow);
                    msg1 = msg1.Replace("{groups}", $"{g.Value}");
                    msg1 = msg1.Replace("{timeleft}", $"{FormatTime(TimeSpan.FromDays(result.TotalDays))}");
                    GetPrivilage = GetPrivilage + msg1 + "\n";
                ...
                foreach (var g in pl.Groups)
                {
                    ...
                    //string msg1 = GetMessage("<size=16>Группа:</size> <color=#CF1E1E>{groups}</color>\n<size=16>Оставшееся время:</size> <color=#CF1E1E>{timeleft}</color>\n", player.Id);
                    TimeSpan result = (g.ExpireDate).Subtract(DateTime.UtcNow);
                    msg1 = msg1.Replace("{groups}", $"{g.Value}");
                    msg1 = msg1.Replace("{timeleft}", $"{FormatTime(TimeSpan.FromDays(result.TotalDays))}");
                    GetPrivilage = GetPrivilage + msg1 + "\n";
		...
		private void LoadMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>
                {
                    {"NoPermission", "<color=#CF1E1E>•</color> Ошибка:\nУ Вас нет прав на использование этой команды"},
                    {"InvalidTimeFormat", "<color=#CF1E1E>•</color> Ошибка:\nНеверный формат времени:\nПример: 1d12h30m || d = дни, h = часы, m = минуты."},
                    {"UserDoesn'tHavePermission", "<color=#CF1E1E>•</color> Ошибка:\nУ игрока {target} нет привилегии {permission}"},
                    {"UserIsn'tInGroup", "<color=#CF1E1E>•</color> Ошибка:\nУ игрока {target} нет группы {group}"},
					{"MultiplePlayersFound", "<color=#F5DA81>НАЙДЕНО НЕСКОЛЬКО ИГРОКОВ:</color> \n"},
					{"PlayerNotFoundName", "<color=#CF1E1E>•</color> Ошибка:\nНе удалось найти игрока с таким именем: {nameOrID}"},
					{"PlayerNotFoundSteam", "<color=#CF1E1E>•</color> Ошибка:\nНе удалось найти игрока с таким SteamID: {nameOrID}"},
					{"NotService", "<color=#CF1E1E>•</color> Информация:\nУ вас нет временных услуг!"},
					{"PlayerNotService", "<color=#CF1E1E>•</color> Информация:\nУ игрока нет временных услуг!"},
					{"PermissionsInfo", "Привилегия: <color=#CF1E1E>{groups}</color>\nОставшееся время: <color=#CF1E1E>{timeleft}</color>\n"},
					{"GroupsInfo", "Группа: <color=#CF1E1E>{groups}</color>\nОставшееся время: <color=#CF1E1E>{timeleft}</color>\n"},
                    {"Days", "д."},
                    {"Hours", "ч."},
                    {"Minutes", "м."},
                }, this);
