...
namespace Oxide.Plugins
{
    ...
    public class NoEscape : RustPlugin
    {
            ...
            if (alreadyBlock)
            {
                ...
                if (entity.GetBuildingPrivilege() != null && config.BlockSettings.EnabledBlockAutCupboard)
                {
                    foreach (var aplayer in entity.GetBuildingPrivilege().authorizedPlayers)
                    {
                        ...
                        if (AuthPlayer != null && AuthPlayer != info.InitiatorPlayer && AuthPlayer.IsConnected) PlayerBlockStatus.Get(AuthPlayer).BlockPlayer(alreadyBlock, false);
						SendOfflineMEssages(entity.transform.position, info.InitiatorPlayer.displayName, aplayer.userid);
                    }
                }
            ...
            else
            {
                ...
                else if (OwnerPlayer != null && OwnerPlayer != info.InitiatorPlayer)
                {
                    ...
                    if (config.BlockSettings.BlockOwnersIfNotInZone)
                    {
                        ...
                        if (OwnerPlayer != info?.InitiatorPlayer) OwnerPlayer.ChatMessage(string.Format(Messages["blockactive"], GetGridString(entity.transform.position), info.InitiatorPlayer.displayName, NumericalFormatter.FormatTime(config.BlockSettings.BlockLength)));
                    }
                    else
                        OwnerPlayer.ChatMessage(string.Format(Messages["blockactiveOwner"], GetGridString(entity.transform.position), info?.InitiatorPlayer.displayName));
                }
                var col = Vis.colBuffer;
                ...
                if (entity.GetBuildingPrivilege() != null && config.BlockSettings.EnabledBlockAutCupboard)
                {
                    ...
                    {
                        var AuthPlayer = BasePlayer.Find(aplayer.userid.ToString());
                        if (AuthPlayer != null && AuthPlayer != info.InitiatorPlayer) PlayerBlockStatus.Get(AuthPlayer).BlockPlayer(sphere, false);
                        SendOfflineMEssages(entity.transform.position, info.InitiatorPlayer.displayName, aplayer.userid);
                    }
                }
            }
        ...
        private static void SendOfflineMEssages(Vector3 pos, string name = "", ulong playerid = 2996224)
        {
        ...
        Dictionary<string, string> Messages = new Dictionary<string, string>() {
            ...
            , {
             ...
             , {
                "blockactiveOwner", "Внимание! Ваше строение в квадрате <color=#ECBE13>{0}</color> разрушено игроком <color=#ECBE13>{1}</color>"
            }
             , {
