...
{
    [Info("RemoveUpgradeV2", "https://topplugin.ru / https://discord.com/invite/5DPTsRmd3G", "1.0.3")]
    ...
    class RemoveUpgradeV2 : RustPlugin
    {
        ...
        IEnumerator TryUpgrade(BasePlayer player, BuildingBlock block, UpgradeConstruction gr)
        {
            ...
            if (block.IsDestroyed) yield break;
			if (plugins.Find("ClassSystem")){
				//Если опыт будет засчитываться дважды то закоментируйте нижнюю строку
				plugins.Find("ClassSystem").Call("GetXPUpgrade",player,block,@enum);
			}
            block.SetGrade(@enum);
