...
{
    [Info("GameStoresRUST", "https://topplugin.ru / https://discord.com/invite/5DPTsRmd3G", "1.0.3")]
    public class GameStoresRUST : RustPlugin
    {
        ...
        private static class Delays
        {
            ...
            public static bool CanRequest(BasePlayer player)
            {
                ...
                {
                    player.ChatMessage($"<size=12>Пожалуйста, дождитесь <<окончания>> предыдущего действия!</size>");
                    return false;
                }
                return true;
            }
        ...
        private void CmdConsoleCommand(ConsoleSystem.Arg args)
        {
            ...
            {
                player.ChatMessage("<size=12>Вы сделали слишком много запросов. Отдохните немного, скоро возможность выполнять действие к вам вернется.</size>");
                return;
            }
                ...
                case "take":
                    {
                        ...
                        {
                            player.ChatMessage("<size=12>Предмет не найден, попробуйте перезапустить корзину</size>");
                            return;
                        ...
                        Request($"&item=true&steam_id={player.UserIDString}&id={id}", (i, s) =>
                        {
                            switch (i)
                            {
                                case 0:
                                    ...
                                    if (player != null)
                                    {
                                        player.ChatMessage($"<size=12>Непредвиденная ошибка со стороны сервера, просим прощения!</size>");
        ...
        private void InitializeStore(BasePlayer player, int page, bool first = true)
        {
            ...
            if (first)
            {
                ...
                container.Add(new CuiLabel
                {
                    ...
                    Text = { Text = $"<size=25><b>Корзина сервера</b></size>\nЭто ваша корзина с покупками, вы можете забрать их в любой момент.", Color = "1 1 1 0.5", Align = TextAnchor.MiddleLeft, FontSize = 12, Font = "robotocondensed-regular.ttf" }
                }, StoreLayer);
            }
                    ...
                    case 200:
                        {
                            ...
                            if (firstInfo.ContainsKey("result"))
                            {
                                ...
                                else
                                {
                                    ...
                                    foreach (var check in data.Skip(page * 12).Take(12))
                                    {
                                    ...
                                    }, StoreLayer + ".BlockPanel", "Page");

                                    secondContainer.Add(new CuiLabel
                                    {
                                        RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1", OffsetMax = "0 0" },
                                        Text = { Text = (page + 1).ToString(), Align = TextAnchor.MiddleCenter, FontSize = 12, Font = "robotocondensed-regular.ttf" }
                                    }, "Page");

                                    secondContainer.Add(new CuiButton
                                    {
                                        RectTransform = { AnchorMin = $"0.407 0.05", AnchorMax = $"0.467 0.11", OffsetMax = "0 0" },
                                        Button = { Color = "0.10 0.13 0.19 1", Material = "", Command = page > 0 ? $"UI_GameStoresRUST page {page - 1}" : "" },
                                        Text = { Text = "<", Color = page > 0 ? "1 1 1 1" : "1 1 1 0.2", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", FontSize = 12 }
                                    }, StoreLayer + ".BlockPanel");

                                    secondContainer.Add(new CuiButton
                                    {
                                        RectTransform = { AnchorMin = "0.535 0.05", AnchorMax = "0.595 0.11", OffsetMax = "0 0" },
                                        Button = { Color = "0.10 0.13 0.19 1", Material = "", Command = (page + 1) * 12 < data.Count ? $"UI_GameStoresRUST page {page + 1}" : "" },
                                        Text = { Text = ">", Color = (page + 1) * 12 < data.Count ? "1 1 1 1" : "1 1 1 0.2", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", FontSize = 12 }
                                    }, StoreLayer + ".BlockPanel");
                                    ...
                                    for (int i = 0; i < Settings.InterfaceSettings.ItemOnString * Settings.InterfaceSettings.StringAmount; i++)
                                    {
                                        ...
                                        secondContainer.Add(new CuiButton
                                        {
                                            ...
                                            Button = { Color = "0.10 0.13 0.19 1", Command = $"" },
                                            Text = { Text = "" }
