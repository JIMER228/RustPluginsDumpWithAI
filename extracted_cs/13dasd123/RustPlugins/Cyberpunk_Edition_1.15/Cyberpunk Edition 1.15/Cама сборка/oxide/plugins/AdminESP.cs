            ...
            if (PlayerSettings.ContainsKey(player.userID))
            {
                ...
                switch (type)
                {
                    case "updatetime":
                        ...
                        break;
                    case "playerdistance":
                        if (int.Parse(value) >= 50 && int.Parse(value) <= 500)
                    ...
                    foreach (var target in BasePlayer.activePlayerList)
                    {
                        ...
                        if (currDistance < data.PlayerDistance)
                        {
                            if (currDistance > 2 && data.DrawNames)
                                DDraw("text", target, Color.yellow, $"{target.displayName} ({currDistance.ToString("#0")} м.)\nHealth: {Math.Floor(target.health)}");
                            if (currDistance > 2 && data.DrawBoxes)
                                DDraw("box", target, Color.yellow);
                            if (data.DrawEyeLine)
                                DDraw("line", target, Color.green);
