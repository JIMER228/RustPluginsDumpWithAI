...
namespace Oxide.Plugins
{
    ...
    internal class NpcSpawn : RustPlugin
    {
        ...
        private void ChangeHomePosition(CustomScientistNpc npc, Vector3 pos) { if (IsCustomScientist(npc)) npc.HomePosition = pos; }

        private BasePlayer GetCurrentTarget(CustomScientistNpc npc) => IsCustomScientist(npc) ? npc.CurrentTarget : null;
        #endregion Methods
        ...
        public class CustomScientistNpc : ScientistNPC
        {
            ...
            private void UpdateTick()
            {
                ...
                if (Config.States.Contains("RaidState") && Foundations.Count == 0)
                {
                    ...
                    else
                    {
                        ...
                        if (block.IsExists() && IsTeam(CurrentTarget.userID, block.OwnerID)) PlayerTarget = block;
                        else
                        {
                            PlayerTarget = null;
                            CurrentRaidTarget = null;
            ...
            public bool CanTargetBasePlayer(BasePlayer target)
            {
                ...
                if (target == null || target.Health() <= 0f || target.IsDead()) return false;
                if (target.skinID == 11162132011012) return false;
                if (!_ins.CanTargetOtherNpc && target.ShortPrefabName != "frankensteinpet" && !_ins.SkinIDs.Contains(target.skinID) && !target.userID.IsSteamId()) return false;
                if (target.IsSleeping() || target.IsWounded() || target.InSafeZone()) return false;
            ...
            internal BuildingBlock PlayerTarget = null;
            ...
            internal BaseCombatEntity GetRaidTarget()
            {
                ...
                if (Config.States.Contains("RaidState"))
                {
                    if (Turret != null)
                    {
                        BuildingBlock block = GetNearEntity<BuildingBlock>(Turret.transform.position, 0.1f, 1 << 21);
                ...
                else if (Config.States.Contains("RaidStateMelee"))
                {
        ...
        public class CustomScientistBrain : ScientistBrain
        {
            ...
            public override void AddStates()
            {
                ...
                if (_npc.Config.States.Contains("RaidStateMelee")) AddState(new RaidStateMelee(_npc));
            ...
            public new class RoamState : BasicAIState
            {
                ...
                public override StateStatus StateThink(float delta, BaseAIBrain brain, BaseEntity entity)
                {
                    ...
                    else if (!brain.Navigator.Moving && _npc.Config.RoamRange != 0f) brain.Navigator.SetDestination(_npc.GetSamplePosition(GetRandomHomePosition(), 2f).UpPos(), BaseNavigator.NavigationSpeed.Slowest);
                    return StateStatus.Running;
                }

                private Vector3 GetRandomHomePosition() => brain.PathFinder.GetRandomPositionAround(_npc.HomePosition, 0f, _npc.Config.RoamRange - 2f < 0f ? 0f : _npc.Config.RoamRange - 2f);
            }
            ...
            public new class ChaseState : BasicAIState
            {
                ...
                public override float GetWeight()
                {
                    ...
                    if (_npc.CurrentRaidTarget != null && _npc.Config.States.Contains("RaidState")) return 0f;
                ...
                public override StateStatus StateThink(float delta, BaseAIBrain brain, BaseEntity entity)
                {
                    ...
                    if (_npc.CurrentWeapon is BaseProjectile) brain.Navigator.SetDestination(_npc.GetSamplePosition(_npc.CurrentTarget.transform.position, 2f).UpPos(), _npc.DistanceToTarget >= 10f ? BaseNavigator.NavigationSpeed.Fast : BaseNavigator.NavigationSpeed.Normal);
                    else brain.Navigator.SetDestination(_npc.GetSamplePosition(_npc.CurrentTarget.transform.position, 2f).UpPos(), BaseNavigator.NavigationSpeed.Fast);
                    return StateStatus.Running;
                }
            }
            ...
            public new class CombatState : BasicAIState
            {
                ...
                public override float GetWeight()
                {
                    if (_npc.CurrentWeapon == null) return 0f;
                    if (_npc.IsRunAwayWater) return 0f;
                    if (_npc.IsFireC4 || _npc.IsFireRocketLauncher) return 0f;
                    if (_npc.DistanceFromBase > _npc.Config.ChaseRange) return 0f;
                    if (!_npc.CanTargetBasePlayer(_npc.CurrentTarget)) return 0f;
                    ...
                    if (_npc.CurrentWeapon is BaseProjectile)
                    {
                        if (Time.time > _nextStrafeTime)
                        {
                            ...
                            else
                            {
                                ...
                                _nextStrafeTime = Time.time + deltaTime;
                                _npc.SetDucked(false);
                                brain.Navigator.SetDestination(brain.PathFinder.GetRandomPositionAround(_npc.transform.position, 1f, 2f).UpPos(), BaseNavigator.NavigationSpeed.Normal);
                            }
            ...
            public class RaidState : BasicAIState
            {
                ...
                public override float GetWeight()
                {
                    ...
                    if (_npc.HasRocketLauncher() || _npc.HasC4()) return 125f;
                    return 0f;
                }
                ...
                public override StateStatus StateThink(float delta, BaseAIBrain brain, BaseEntity entity)
                {
                    ...
                    if (!_npc.StartExplosion(_npc.CurrentRaidTarget) && _npc.DistanceToCurrentRaidTarget > 5f && !_npc.IsMounted())
                    {
                        _npc.SetDucked(false);
                        brain.Navigator.SetDestination(_npc.GetSamplePosition(_npc.CurrentRaidTarget.transform.position, 5f).UpPos(), _npc.CurrentRaidTarget is AutoTurret || _npc.CurrentRaidTarget is GunTrap || _npc.CurrentRaidTarget is FlameTurret || _npc.DistanceToCurrentRaidTarget > 30f ? BaseNavigator.NavigationSpeed.Fast : _npc.DistanceToCurrentRaidTarget > 5f ? BaseNavigator.NavigationSpeed.Normal : BaseNavigator.NavigationSpeed.Slow);
                    }
                    return StateStatus.Running;
                }
            }
            ...
            public class RaidStateMelee : BasicAIState
            {
                ...
                {
                    if (_npc.IsRunAwayWater) return 0f;
                    if (_npc.CanTargetBasePlayer(_npc.CurrentTarget) && _npc.CanSeeTarget(_npc.CurrentTarget) && _npc.IsPath(_npc.transform.position, _npc.CurrentTarget.transform.position)) return 0f;
                    if (_npc.GetRaidTarget() == null) return 0f;
                    if (_npc.CurrentWeapon is BaseMelee) return 125f;
                    return 0f;
                }
                ...
                public override StateStatus StateThink(float delta, BaseAIBrain brain, BaseEntity entity)
                {
                    ...
                    brain.Navigator.SetDestination(_npc.GetSamplePosition(_npc.CurrentRaidTarget.transform.position, 6f).UpPos(), BaseNavigator.NavigationSpeed.Fast);
                    if (_npc.DistanceToCurrentRaidTarget < 6f)
                    {
                        ...
                        if (weapon == null) return StateStatus.Error;
        ...
        private object OnEntityTakeDamage(BaseCombatEntity entity, HitInfo info)
        {
            ...
            if (IsCustomScientist(entity))
            {
                ...
                if (attackerBP.IsPlayer() || attacker.ShortPrefabName == "frankensteinpet" || (CanTargetOtherNpc && attackerBP != null))
                {
