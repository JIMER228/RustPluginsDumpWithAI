...
{
    [Info("RecyclerHome", "MarinWebCode","1.0.0")]
    [Description("NICE RUST")] 
    public class RecyclerHome : RustPlugin
    {
        ...
        {
            [JsonProperty("Название переработчика")] public string RecyclerName = "Переработчик";
            ...
            [JsonProperty("Разрешить поднимать чужой переработчик?")] public bool canupnotowner = false;
            [JsonProperty("Разрешить подбирать переработчик в зоне запрета строительства?")] public bool canupbuildingblock = false;
