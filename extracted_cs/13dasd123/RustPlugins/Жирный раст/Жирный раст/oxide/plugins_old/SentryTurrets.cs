...
{
    [Info("Sentry Turrets", "Orange", "2.3.7")]
    ...
    public class SentryTurrets : RustPlugin
    {
        ...
        private void SetupTurret(NPCAutoTurret turret)
        {
            ...
            turret.inventory.onlyAllowedItem = ItemManager.FindItemDefinition(ammoShortname);
        ...
        private static void CheckSwitch(BaseEntity turret)
        {
            ...
            if (entity == null)
            {
                entity = GameManager.server.CreateEntity(prefabSwitch)?.GetComponent<ElectricSwitch>();
                if (entity == null)
                {
                    return;
                }
                
                entity.SetParent(turret);
        ...
        private void AuthorizeOthers(ulong userID, NPCAutoTurret entity)
        {
            ...
            {
                var team = RelationshipManager.Instance.teams.FirstOrDefault(x => x.Value.members.Contains(userID)).Value;
            ...
            entity.authorizedPlayers = entity.authorizedPlayers.Distinct().ToList();
        }
