...
namespace Oxide.Plugins
{
    ...
    public class BuildingUpgrade : RustPlugin
    {
        ...
        private string CanRemove(BasePlayer player, BaseEntity entity)
        {
            ...
            if (!player.CanBuild())
            {
                return "Вы не можете удалять в зоне действия</color><color=#F24525>чужого шкафа</color>!"; 
            }
                ...
                if (CanRemove(player, ent) == string.Empty)
                {
                    ...
                    if (block != null && HasUpgradePrivilege(player, block))
                    {
                        ...
                        if (removeTimers.TryGetValue(block.net.ID, out maxRemoveTime) && player.SecondsSinceAttacked > 10) 
                        {
                            ...
                            if (leftRemove < 0) 
                            {
                                SendReply(player, "Время удаления данного объекта</color><color=#D500C3>закончилось</color>!");
                            }
                            else
                            {
                                SendReply(player, $"Через</color><color=#F24525>{FormatTime(TimeSpan.FromSeconds(leftRemove), maxSubstr:2)}</color> вы не сможете удалить этот объект!</color>");
                            }
        ...
        object OnStructureUpgrade(BaseCombatEntity entity, BasePlayer player, BuildingGrade.Enum grade)
        {
            ...
            { 
                SendReply(player, $"Вы можете удалить построенный объект в течении </color><color=#F24525>{FormatTime(TimeSpan.FromSeconds(CONF_RemoveTime), maxSubstr:2)}</color>");
                player.Hurt(0);
            }
            return null;
        }
