...
{
    [Info("Secure Admin", "discord.gg/9vyTXsJyKR", "1.1.8", ResourceId = 1449)]
    ...
    public class SecureAdmin : CovalencePlugin
    {
		...
		private const string permKill = "secureadmin.kill"; // global.kick, global.kickall
        ...
        protected override void LoadDefaultConfig()
        {
			...
			Config["Оповещение о киле (true/false)"] = broadcastKicks = GetConfig("Broadcast Kill (true/false)", true);
			...
			Config["Включить команду Kill (true/false)"] = commandKill = GetConfig("Enable Kill Command (true/false)", true);
        ...
        private void Init()
        {
			...
			permission.RegisterPermission(permKill, this);
			...
			if (commandKill) AddCovalenceCommand("kill", "KillCommand");
        ...
        protected override void LoadDefaultMessages()
        {
            ...
            lang.RegisterMessages(new Dictionary<string, string>
            {
				...
				["UsageKill"] = "Введите: <color=#FFB841>{0} <name or id>  </color>",
        ...
        {
            if (!player.HasPermission(permKill))
            {
                player.Reply(Lang("NotAllowed", player.Id, command));
                return;
            }
            ...
            if (args.Length < 1)
            {
                player.Reply(Lang("UsageKill", player.Id, command));
                return;
            }
