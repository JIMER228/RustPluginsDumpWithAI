...
{
    [Info("Automatic Authorization", "discord.gg/9vyTXsJyKR", "1.3.0", ResourceId = 2063)]
    public class AutomaticAuthorization : RustPlugin
    {
        ...
        private object CanUseLockedEntity(BasePlayer player, BaseLock baseLock)
        {
            ...
            if (shareData.teamsShare.enabled && SameTeam(ownerID, player.userID))
            {
                if (baseLock is KeyLock && CanUnlockEntity(parentEntity, shareData.teamsShare.keyLock))
                {
                    return True;
                }
                var codeLock = baseLock as CodeLock;
                if (codeLock != null && CanUnlockEntity(parentEntity, shareData.teamsShare.codeLock))
                {
            ...
            {
                if (baseLock is KeyLock && CanUnlockEntity(parentEntity, shareData.friendsShare.keyLock))
                {
                    return True;
                }
                var codeLock = baseLock as CodeLock;
                if (codeLock != null && CanUnlockEntity(parentEntity, shareData.friendsShare.codeLock))
                {
                    SendUnlockedEffect(codeLock);
                    return True;
                }
            }
            if (shareData.clansShare.enabled && SameClan(ownerID, player.userID))
            {
                if (baseLock is KeyLock && CanUnlockEntity(parentEntity, shareData.clansShare.keyLock))
                {
                    return True;
                }
                var codeLock = baseLock as CodeLock;
                if (codeLock != null && CanUnlockEntity(parentEntity, shareData.clansShare.codeLock))
                {
                    SendUnlockedEffect(codeLock);
                    return True;
                }
