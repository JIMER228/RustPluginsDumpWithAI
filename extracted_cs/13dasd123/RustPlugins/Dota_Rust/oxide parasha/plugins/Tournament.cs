...
{
    [Info("Tournament", "discord.gg/9vyTXsJyKR", "1.0.0")]
    public class Tournament : RustPlugin
    {

        [PluginReference] private Plugin Clans, RustMap;
        ...
        void OnEntityDeath(BaseCombatEntity entity, HitInfo info)
        {
            ...
            if (entity as BuildingPrivlidge && info != null && info.InitiatorPlayer != null)
            {
                ...
                {
                    string clans = Clans?.Call("ClanAlready", find.ownerclan) as string;
                    p.ChatMessage($"Клан {clans} вылетел с турнира!");
                }
            }
        }
        ...
        private void OnLootEntity(BasePlayer player, BaseEntity entity)
        {
            ...
            if (entity as BuildingPrivlidge)
            {
                ...
                object Points = Clans?.CallHook("ClanPoint", player.userID);
                ...
                if (obj < 228)
                {
                    container.Add(new CuiButton
                    {
                        ...
                        Text =
                    {
                        Text = $"Нужно 228 объектов!", Align = TextAnchor.MiddleCenter,
                        Font = "robotocondensed-regular.ttf", FontSize = 24
                    }
                    }, CupLayer);
                    CuiHelper.AddUi(player, container);
                ...
                if (Points is int && (int)Points < 227)
                {
                    container.Add(new CuiButton
                    {
                        RectTransform = { AnchorMin = "0.08181816 0.3208335", AnchorMax = "0.9454545 0.7166667" },
                        ...
                        Text =
                    {
                        Text = $"Нужно 227 очков!", Align = TextAnchor.MiddleCenter,
                        Font = "robotocondensed-regular.ttf", FontSize = 24
                    }
                    }, CupLayer);
                    CuiHelper.AddUi(player, container);
