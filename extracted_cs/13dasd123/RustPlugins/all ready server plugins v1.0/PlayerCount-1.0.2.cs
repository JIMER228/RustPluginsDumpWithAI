...
namespace Oxide.Plugins
{
    ...
    class PlayerCount : RustPlugin
    {
        ...
        void Init()
        {
			...
			lang.RegisterMessages(new Dictionary<string, string> { ["Chat Message"] = "<color=orange>Игроки онлайн: {0}</color> | <color=orange>Спящих игроков: {1}</color>" }, this);
            ...
            timer.Every(messageTime * 60, () =>
            {
                ...
                PrintToChat(lang.GetMessage("Chat Message", this),String.Format("{0:n0}", playerCount), String.Format("{0:n0}", sleeperCount));
            });
        }
        #endregion
