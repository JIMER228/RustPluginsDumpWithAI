...
{
    [Info("XDCasino", "DezLife", "1.5.0")]
    [Description("Казино, играем на ресурсы :3")]
    public class XDCasino : RustPlugin
    {
        /// <summary>
        /// Фикс камеры
        /// Исправлен формат конфига
        /// Исправлен Lang файл
        /// Некоторые изменения
        /// Исправил проблему с гниением
        /// </summary>
        #region Var
        ...
        private void HelpUiNottice(BasePlayer player, string msg, string sprite = "assets/icons/info.png")
        {
            ...
            {
                FadeOut = 0.30f,
                RectTransform = { AnchorMin = "0.1139241 0.089991959", AnchorMax = "0.9423349 0.8999914", OffsetMax = "0 0" },
        ...
        void OnPasteFinished(List<BaseEntity> pastedEntities, string fileName)
        {
            try
            {
                ...
                foreach (BaseEntity item in CasinoEnt)
                {
                    ...
                    {
                        (item as DecayEntity).decay = null;
                        (item as DecayEntity).upkeepTimer = float.MinValue;
                    }
                    if (item is CCTV_RC)
                    {
                ...
                {
                    {
                        ...
                        else
                        {
                            if (!repeat)
                            {
                                PrintError(GetLang("CASINO_FileNotLoad", null, "CasinoRoomNew.json"));
                                Log(i.ToString(), "LogError");
                                timer.Once(10f, () => LoadDataCopyPaste());
                            }
                            else
                            {
                                PrintError(GetLang("CASINO_ServerNotResponse", null, "CasinoRoomNew.json", i));
                            }
                            return;
                        }
                    }
