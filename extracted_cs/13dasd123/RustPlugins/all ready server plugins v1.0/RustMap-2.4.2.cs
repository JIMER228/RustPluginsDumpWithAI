...
namespace Oxide.Plugins
{
    ...
    class RustMap : RustPlugin
    {
        ...
        [ChatCommand("addfarm")]
        private void asdsad(BasePlayer player)
        {
           AddMarkerApi("https://cdn.discordapp.com/attachments/907668879982080040/938853091019726948/island.png", false, 12, 1f, player.transform, "farm");
        }
        
        Dictionary<BasePlayer, MapMarker> FarmMarkers = new Dictionary<BasePlayer, MapMarker>();
        
        void AddFarmMarker(BasePlayer player)
        {
            var transform = player.transform;
            var farm = MapMarker.Create(transform);
            farm.png = "island";
            farm.rotSupport = false;
            farm.name = "Фарм";
            farm.alpha = 1f;
            farm.size = 0.01f;
            farm.position = transform.position;
            FarmMarkers[player] = farm;

            foreach (var sub in subscribers)
                DrawMapMarker(sub.Key, farm);
        }
        ...
        void OnServerInitialized()
        {
            PrintWarning($"-----------------------------------");
            PrintError($"                RustMap            ");
            PrintError($"            Site  =  Top Plugin  ");
            PrintError($"           Discord =  https://discord.com/invite/5DPTsRmd3G      ");
            PrintError($"          Version =  {Version}     ");
            PrintWarning($"------------------------------------");
            PrintError("Передача плагина 3м лицам и выдача доступа к вашему серверу лишает вас обновлений!\n");
