...
namespace Oxide.Plugins
{
    ...
    class RadHouse : RustPlugin
    {
        ...
        public string TextGUI = "Радиактивный домик:";

        public string ChatPrefix = "<color=#00FF7F>FOX RUST: </color>";
        ...
        public int RadiationIntensity = 50;
        #endregion
        ...
        private void LoadConfigValues()
        {
            ...
            GetConfig("[Лут]", "Список обычного лута", ref ItemList_Common);
            GetConfig("[Лут]", "Список редкого лута", ref ItemList_Rare);
            GetConfig("[Лут]", "Список топового лута", ref ItemList_Top);
        ...
        [ChatCommand("createrad")]
        void CreateRadHouseChat(BasePlayer player)
        {
            if(player.IsAdmin())
            {
                if (BaseEntityList != null) DestroyRadHouse();
        ...
        [ChatCommand("destroyrad")]
        void DestroyRadHouseChat(BasePlayer player)
        {
            if (player.IsAdmin())
            {
                DestroyRadHouse();
	...
	//		//bId = BindDecay(Box, //bId);
		...
		//	//bId = BindDecay(bbFS2, //bId);
            ...
            PrintToChat($"{ChatPrefix}<color=#1E90FF>Радиактивный домик появился на карте <color=#FFB400>{pos.ToString()}</color>.</color>");
            foreach (var player in BasePlayer.activePlayerList)
            {
                CreateGui(player);
            }
            if (plugins.Exists("LustyMap"))
            {
				LustyMap?.Call("AddMarker", pos.x , pos.z +4.5f  , "Радиактивный домик", "rad.png", 0);
        ...
        void DestroyRadHouse()
        {
            if(BaseEntityList != null)
            {
                ...
                if (plugins.Exists("LustyMap"))
                {
					  LustyMap?.Call("RemoveMarker", "Радиактивный домик");
                    ...
                    if (entityID.net.ID == entity.net.ID)
                    {
                        if (CanLoot)
                        {
                            if (PlayerAuth.Contains(player.userID))
                            {
                                ...
                                {
                                    NowLooted = true;
                                    PrintToChat($"{ChatPrefix} <color=#FFE19B><color=#FFB400>{player.displayName}</color> залутал ящик в радиактивном доме. Постройка самоуничтожится через <color=#FFB400>{TimerDestroyHouse}</color> секунд.</color>");
