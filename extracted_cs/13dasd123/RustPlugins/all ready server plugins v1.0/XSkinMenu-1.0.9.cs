...
{
	[Info("XSkinMenu", "[https://info-hacks.ru/] Skuli Dropek", "1.0.9")]
	class XSkinMenu : RustPlugin
	{
		...
		private class SkinConfig
		{
			...
			{
				[JsonProperty("Which API to use to upload images - [ True - regular images from the Steam Workshop (almost all existing skins) | False - beautiful images (all accepted skins by the developers, plus half from the Steam Workshop) ]")] public bool APIOption;
				[JsonProperty("Disable check for presence of image in ImageLibrary date (the image will be reloaded after every plugin load/reload) - If you don't know/understand how images work in ImageLibrary or if you have problems with images, then it's useful to disable validation (true)")] public bool HasImage;
			}
			...
			internal class GeneralSetting
			{
				...
				[JsonProperty("Forbid changing the skin of an item that is not in the config")] public bool ReskinConfig;
				[JsonProperty("Change skins to items after player respawns")] public bool ReskinRespawn;
				[JsonProperty("Logs to the image upload console")] public bool LogLoadIMG;
				[JsonProperty("Logs to image reload console")] public bool LogReloadIMG;
			...
			[JsonProperty("API/image settings")]
				...
				case "setskin":
					{
						...
						Cooldowns[player] = DateTime.Now.AddSeconds(1.5f); // Don't touch here!!! =)
						break;
					}
				case "clear":
					{
					...
					case "add":
						{
							...
							else
							{
								...
								if (config.API.APIOption)
									...
									ImageLibrary.Call("AddImage", $"https://api.skyplugins.ru/api/getskin/v1/{key}/{skinID}/150", $"{skinID}" + 152);
							}
		...
		private void ccmdReloadIMG(ConsoleSystem.Arg args)
		{
			if (args.Player() == null || args.Player().IsAdmin)
				if (_coroutine == null)
					...
					PrintWarning("Images loading/reloading continues. Wait!");
		}
		...
		private void ccmdIMGStop(ConsoleSystem.Arg args)
		{
			if (args.Player() == null || args.Player().IsAdmin)
				if (_coroutine == null)
					PrintWarning("There is no active loading/reloading of images at the moment!");
				else
				{
					ServerMgr.Instance.StopCoroutine(_coroutine);
					_coroutine = null;

					PrintWarning("The current loading/reloading of images has been interrupted!");
				}
		}
		...
		private void OnServerInitialized()
		{
			...
			if (_coroutine == null && ImageLibrary && key != "")
			...
			if (key == "")
			{
				timer.Once(2, () =>
				{
					PrintError("The code is missing the key for the API. If you don't have it, contact the developer.\nIf you have a key, paste it into the code, code line - 183 - string key = '';");
			...
			if (!ImageLibrary)
				timer.Once(2, () =>
				{
					PrintError("You don't have the plugin installed - ImageLibrary!");
					Interface.Oxide.UnloadPlugin(Name);
				});
		}
		...
		private IEnumerator LoadImage()
		{
			...
			PrintWarning("Category images have started loading!");

			foreach (var category in config.Category)
			{
				foreach (var item in category.Value)
				{
					if (config.API.HasImage || !ImageLibrary.Call<bool>("HasImage", item + 150))
					{
				...
				if (config.Setting.LogLoadIMG)
					PrintWarning($"[ Category loaded {x} out of {config.Category.Count} ] - [ Category images loaded {y} out of {category.Value.Count} ]");

				y = 0;
			}

			PrintWarning("Skin images have started loading!");
			...
			foreach (var item in StoredDataSkins)
			{
				foreach (var skin in item.Value)
				{
					if (config.API.HasImage || !ImageLibrary.Call<bool>("HasImage", $"{skin}" + 152))
					{
						if (config.API.APIOption)
							...
							ImageLibrary.Call("AddImage", $"https://api.skyplugins.ru/api/getskin/v1/{key}/{skin}/150", $"{skin}" + 152);
				...
				if (config.Setting.LogLoadIMG)
					PrintWarning($"[ Item loaded {item.Key} | {xx} out of {StoredDataSkins.Count} ] - [ Skin images loaded {yy} out of {item.Value.Count} ]");
		...
		private IEnumerator ReloadImage()
		{
			...
			PrintWarning("Category images have started reloading!");
			...
			foreach (var category in config.Category)
			{
				...
				if (config.Setting.LogReloadIMG)
					PrintWarning($"[ Category reloaded {x} out of {config.Category.Count} ] - [ Category images reloaded {y} out of {category.Value.Count} ]");

				y = 0;
			}

			PrintWarning("Skin images have started reloading!");
			...
			foreach (var item in StoredDataSkins)
			{
				...
				if (config.Setting.LogReloadIMG)
					PrintWarning($"[ Item reloaded {item.Key} | {xx} out of {StoredDataSkins.Count} ] - [ Skin images reloaded {yy} out of {item.Value.Count} ]");
