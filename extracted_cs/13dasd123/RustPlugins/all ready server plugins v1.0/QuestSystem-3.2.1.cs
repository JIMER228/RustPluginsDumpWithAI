...
{
    [Info("QuestSystem", "https://topplugin.ru/", "3.2.1")]
    public class QuestSystem : RustPlugin
    {
            ...
            for(int i = 0; i < QuestList.Count; i++)
            {
                for(int u = 0; u < QuestList[i].PrizeList.Count; u++)
                {
                    ...
                    if (QuestList[i].PrizeList[u].Skin != 0)  
                        ImageLibrary.Call("AddImage", $"http://rust.skyplugins.ru/getskin/{QuestList[i].PrizeList[u].Skin}/", QuestList[i].PrizeList[u].Shortname, QuestList[i].PrizeList[u].Skin);                
                }
            }
        ...
        private bool TryPlaceSign()
        {
            ...
            Vis.Entities<BaseEntity>(resultVector, 3f, baseEntities);
            if (baseEntities.Count > 0)
            {
                foreach (var fdsf in baseEntities)
                {
                    if (fdsf.PrefabName.Contains("sign.post.town.roof"))
                        fdsf.Kill();               
                }
            }
        ...
        private void OnEntityDeath(BaseCombatEntity entity, HitInfo info)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (entity.PrefabName.Contains("servergibs")) return;
                    ...
                    case "get":
                        {
                            ...
                            if (args.HasArgs(2) && int.TryParse(args.Args[1], out questIndex))
                            {
                                ...
                                if (currentQuest != null)
                                {
                                    ...
                                    {
                                        if (!storedData.players[player.userID].PlayerQuestsCooldown.ContainsKey(currentQuest.DisplayName)) { storedData.players[player.userID].PlayerQuestsCooldown.Add(currentQuest.DisplayName, 0); }
                                    }
        ...
        private void UI_DrawInterface(BasePlayer player, int page = 0, string questUpdate = "")
        {
            ...
            if (questUpdate == "")
            {
                ...
                {
                    CursorEnabled = true,
                    RectTransform = { AnchorMin = "0.09687495 0.093518295", AnchorMax = "0.903125 0.9064815", OffsetMax = "0 0" },
                    Image = { Color = "0 0 0 0" }
                }, "Overlay", Layer);
                ...
                for (int i = 0; i < QuestList.Count(); i++)
                {
                    ...
                    container.Add(new CuiButton
                    {
                        RectTransform = { AnchorMin = "0.78122341 0.802927", AnchorMax = "0.9858913 0.966369", OffsetMax = "0 0" },
            ...
            using (WWW www = new WWW(url))
            {
				...
				if (sign.textureIDs != null){
					for (int num = 0; num < sign.textureIDs.Length; num++)
					{
						...
						/*if (sign.textureIDs = new uint[] { id };);
						{
							FileStorage.server.Remove(textureId, FileStorage.Type.png, sign.net.ID);
						}*/

						sign.textureIDs[num]= FileStorage.server.Store(resizedImageBytes, FileStorage.Type.png, sign.net.ID);
					}
