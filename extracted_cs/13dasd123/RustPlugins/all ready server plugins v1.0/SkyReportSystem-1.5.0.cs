...
{
    [Info("SkyReportSystem", "Работает", "1.5.0")]
    [Description("Система репортов для игроков")]
    public class SkyReportSystem : RustPlugin
    {
        ...
        public class Configuration
        {
            ...
            public Dictionary<string, string> Reasonsforcomplaint;
        }
        ...
        public class IsAfk
        {
            ...
            [JsonProperty(PropertyName = "Снимать проверку с игрока если он стоит в AFK ?", Order = 1)]
            public bool usecheckingfalse;
        ...
        {
            [JsonProperty(PropertyName = "Аватар для сообщений", Order = 0)]
            public ulong avatarid;

            [JsonProperty(PropertyName = "Префикс", Order = 1)]
            public string prefix;

            [JsonProperty(PropertyName = "Цвет префикса", Order = 2)]
            public string coloprefix;
        ...
        protected override void LoadDefaultConfig()
        {
            config = new Configuration()
            {
                vKontakte = new VKontakte()
                {
                    UseVK = true,
                    VK_ChatID = "c259",
                    VK_Token = "2190e93c1916eedda9519f0d0708e774075d64f2fcab35680e07b9fddba2696836c1180917d39186dfaaf",
                },
                discord = new Discord()
                {
                ...
                setting = new Setting()
                {
                    ...
                    prefix = "СИСТЕМА РЕПОРТОВ",
                    coloprefix = "#FFFFFF",
                    maxreportcall = 4,
                    usemodercall = true,
                    uselog = true,
                    Cooldown = 360,
                    servername = "STALKER RUST",
                    comandplayer = "report",
                    comandmoder = "reportm",
                    teatletxt = "СИСТЕМА РЕПОРТОВ"

                },
                isafk = new IsAfk()
                {
                    usecheckafk = true,
                    usecheckingfalse = false,
                    timecheckisafk = 15f,
                },
                ...
                Reasonsforcomplaint = new Dictionary<string, string>
                {
                    ["reason_1"] = "Игра 3+",
                    ["reason_2"] = "Подозрения в ПО",
                    ["reason_3"] = "Макросы",
                    ["reason_4"] = "Реклама в нике",
                    ["reason_5"] = "Спам в чат",
                    ["reason_6"] = "Реклама в чате",
                    ["reason_7"] = "Багоюз",
                    ["reason_8"] = "Нецинзурная лексика",
                }
        ...
        private new void LoadDefaultMessages()
        {
            ...
            timer.In(2.5f, () => {
                ...
                foreach (var reason in config.Reasonsforcomplaint)
                {
                    Lang.Add(reason.Key, reason.Value);
                }
        ...
        private void OnServerInitialized()
        {
            ...
            ReportData = Oxide.Core.Interface.Oxide.DataFileSystem.ReadObject<Dictionary<ulong, ReportUser>>("SkyReportSystem/SkyData");
        ...
        {
            Oxide.Core.Interface.Oxide.DataFileSystem.WriteObject("SkyReportSystem/SkyData", ReportData);
            BasePlayer.activePlayerList.ForEach(p => CuiHelper.DestroyUi(p, mainskymenu));
            BasePlayer.activePlayerList.ForEach(p => CuiHelper.DestroyUi(p, ModerMenuSky));
            BasePlayer.activePlayerList.ForEach(p => CuiHelper.DestroyUi(p, ModerAlert));
        ...
        private void ReportActivity(BasePlayer reportplayer, BasePlayer target, string reason)
        {
            ...
            {
                ConVar.Server.Log("SkyReportSystemLOG", $"На игрока <color=#816AD0>{target.displayName}</color> пожаловался <color=#816AD0>{reportplayer.displayName}</color>\n" +
        ...
        private void cmdreportplayerConsole(ConsoleSystem.Arg args)
        {
            ...
            {
                PrintWarning($"Неверный синтаксис, используйте player check/uncheck StemId64");
                return;
            }
        ...
        private void modermenu(BasePlayer player)
        {
            ...
            {
                ReplyWithHelper(player, String.Format(lang.GetMessage("SKY_PLAYER_PERMISSION_IS_FOUND", this)));
            }
        }

        [ConsoleCommand("mreport")]
        private void cmdreportmenu(ConsoleSystem.Arg args)
        {
            ...
            {
                ReplyWithHelper(args.Player(), String.Format(lang.GetMessage("SKY_PLAYER_PERMISSION_IS_FOUND", this)));
            }
        }
        ...
        void ReportDeath(BasePlayer player, BasePlayer Target, string Weapon, string Distance)
        {
            ...
            container.Add(new CuiButton
            {
                ...
                Button = { Command = $"ReportDeath {player.userID} {Target.userID} {Distance} {Weapon}", Color = "0 0 0 0" },
                Text = { Text = "Жалоба на игрока", FontSize = 15, Align = TextAnchor.MiddleCenter, FadeIn = 0.5f }
            }, "DeathGui");

            CuiHelper.DestroyUi(player, "DeathGui");
            CuiHelper.AddUi(player, container);
        }

        [ChatCommand("skype")]
        void skypeacess(BasePlayer player, string cmd, string[] Args)
        {
            if (Args == null || Args.Length == 0)
            {
                ReplyWithHelper(player, String.Format(lang.GetMessage("SKY_PLAYER_NOTHING_MESSAGE_SKYPE_DISCORD", this)));
                return;
            }
            ...
            if (CheckPlayerModeration.ContainsKey(player.userID))
            {
                ...
                SendChatMessage("SKY_SKYPE_MESSAGE", config.setting.servername, player.displayName, player.UserIDString, Skype);
                SendDiscordMsg("SKY_SKYPE_MESSAGE", config.setting.servername, player.displayName, player.UserIDString, Skype);
                ...
                {
                    ReplyWithHelper(moderator, String.Format(lang.GetMessage("SKY_SKYPE_MESSAGE", this), config.setting.servername, player.displayName, player.UserIDString, Skype));
                    ReplyWithHelper(player, String.Format(lang.GetMessage("SKY_MODER_CHECK_P_SKYPE", this), Skype));
                }

            }
            else { ReplyWithHelper(player, String.Format(lang.GetMessage("SKY_SKYPE_DISCORD_ERROR", this))); }
        ...
        void discordaccess(BasePlayer player, string cmd, string[] Args)
        {
            ...
            if (Args == null || Args.Length == 0)
            {
                ReplyWithHelper(player, lang.GetMessage("SKY_PLAYER_NOTHING_MESSAGE_SKYPE_DISCORD", this));
                return;
            }
            ...
            if (CheckPlayerModeration.ContainsKey(player.userID))
            {
                ...
                {
                    ReplyWithHelper(moderator, String.Format(lang.GetMessage("SKY_DISCORD_MESSAGE", this), config.setting.servername, player.displayName, player.UserIDString, Discord));
                    ReplyWithHelper(player, String.Format(lang.GetMessage("SKY_MODER_CHECK_P_DISCORD", this), Discord));
                }

            }

            else { ReplyWithHelper(player, lang.GetMessage("SKY_SKYPE_DISCORD_ERROR", this)); }
        }
        ...
        void consolego(ConsoleSystem.Arg args)
        {
            ...
            BasePlayer player = args.Player();
            bool CanUse = permission.UserHasPermission(player.UserIDString, "SkyReportSystem.moderator");
            if (!CanUse) return;
            ...
            if (args.Args[0].ToLower() == "checkinplayer")
            {
                ...
                {
                    ReplyWithHelper(player, String.Format(lang.GetMessage("SKY_NO_CHECK_MODERATION_OR_CHECKING", this), targetPlayer.displayName));
                }
                else
                {
                    if (CanUse)
                    {
                        ...
                        ReplyWithHelper(player, String.Format(lang.GetMessage("SKY_ACESS_CHECKING", this), targetPlayer.displayName));
                        ...
                        if (config.isafk.usecheckafk == true)
                        {
                            timer.Repeat(config.isafk.timecheckisafk, 3, () =>
                            {
                                if (!IsPlayerAfk(targetPlayer))
                                {
                                    ...
                                    ReplyWithHelper(args.Player(), String.Format(lang.GetMessage("SKY_PLAYER_IS_AFK_OK", this), targetPlayer.displayName));
                                }
                                else
                                {
                                    ...
                                    ReplyWithHelper(args.Player(), String.Format(lang.GetMessage("SKY_PLAYER_IS_AFK_NO", this), targetPlayer.displayName));
                                }
                            });
                        }
                        AlertPlayerCheck(BasePlayer.FindByID(targetPlayer.userID), player);
                        Game.Rust.Cui.CuiHelper.DestroyUi(player, ModerMenuSky);
                    }
                    else
                    {
                        ReplyWithHelper(player, String.Format(lang.GetMessage("SKY_PLAYER_PERMISSION_IS_FOUND", this)));
            ...
            if (args.Args[0] == "stopcheckingplayers")
            {
                if (CanUse)
                {
                    ...
                    CuiHelper.DestroyUi(player, ModerMenuSky);
                    ReplyWithHelper(player, String.Format(lang.GetMessage("SKY_MODERATOR_STOPID_CHEKING", this), targetPlayer.displayName));
                    ReplyWithHelper(targetPlayer, String.Format(lang.GetMessage("SKY_PLAYER_STOPID_CHEKING", this), args.Player().displayName));
        ...
        void ReportSystemBanReason(ConsoleSystem.Arg arg)
        {
            ...
            SendChatMessage("", player.displayName, player.UserIDString, config.setting.servername, BanReason, moderator.displayName);
        ...
        {
            if (!CooldownPC.ContainsKey(ID) || Math.Max(0, CooldownPC[ID]) < 1)
                return false;
            else return true;
        }
        ...
        private void PlayerReportSystem(ConsoleSystem.Arg args)
        {
            ...
            if (args.Args[0].Length != 17)
            {
                for (int t = 0; t < 44; t++)
                {
                    Game.Rust.Cui.CuiHelper.DestroyUi(args.Player(), "Button" + $".ChoosePlayer.{t}");
                    Game.Rust.Cui.CuiHelper.DestroyUi(args.Player(), "Button" + $".ChoosePlayer.{t}.Text");
                }
            ...
            {
                ReportActivity(args.Player(), BasePlayer.Find(args.Args[0]), lang.GetMessage(args.Args[1], this));
                ...
                if (args.Player() != target)
                {
                    ...
                    {
                        ReplyWithHelper(args.Player(), lang.GetMessage("SKY_COLDOWN", this));
                    ...
                    ReportData[target.userID].complaint.Insert(0, lang.GetMessage(args.Args[1], this));
                    ReplyWithHelper(args.Player(), String.Format(lang.GetMessage("SKY_REPORT_REPORTING", this), target.displayName));
                    SendDiscordMsg("SKY_REPORT_REPORTING_VK_DISCORD", config.setting.servername, args.Player().displayName, target.displayName, target.userID, lang.GetMessage(args.Args[1], this));
                    SendChatMessage("SKY_REPORT_REPORTING_VK_DISCORD", config.setting.servername, args.Player().displayName, target.displayName, target.userID, lang.GetMessage(args.Args[1], this));

                    if (ReportData[target.userID].ReportCount > config.setting.maxreportcall)
                    {
                        foreach (BasePlayer modelist in BasePlayer.activePlayerList.Where(p => permission.UserHasPermission(p.UserIDString, "SkyReportSystem.moderator")))
                        {
                            if (config.setting.usemodercall == true)
                            {
                                ReplyWithHelper(modelist, String.Format(lang.GetMessage("SKY_REPORT_MAXIMUM_REPORT", this), target, ReportData[target.userID].ReportCount));
                                ModerAlertCheck(modelist, target);
                                timer.Once(5f, () => { Game.Rust.Cui.CuiHelper.DestroyUi(modelist, ModerAlert); });
                            }
                        }
                        if (config.setting.uselog == true)
                        {
                            ConVar.Server.Log("SkyReportSystemLOG", $"Игрок {target} превысил максимальное количество репортов! [{ReportData[target.userID].ReportCount}]");
                        }
                ...
                {
                    ReplyWithHelper(args.Player(), lang.GetMessage("SKY_ERROR_PLAYER", this));
                }
                return;
            }
        }
        ...
        private void PlayerBanIPlayerModeration(BasePlayer player, BasePlayer targetinfoban)
        {
            ...
            container.Add(new CuiPanel
            {
                ...
                Image = { FadeIn = 1f, Color = HexToCuiColor("#F7EBE109") }
            ...
            CuiHelper.DestroyUi(player, "InfoPlayerSky");
        ...
        private void ModerAlertCheck(BasePlayer player, BasePlayer target)
        {
            ...
            CuiHelper.DestroyUi(player, ModerAlert);
            CuiHelper.DestroyUi(player, "AlerModer");
            CuiHelper.AddUi(player, container);
        ...
        private void AlertPlayerCheck(BasePlayer player, BasePlayer moderinformation)
        {
            ...
            CuiHelper.DestroyUi(player, PlayerAlert);
            CuiHelper.DestroyUi(player, "MainAlert");
            CuiHelper.AddUi(player, container);
        }

        private void SkyModerMenu(BasePlayer player, string target = "")
        {
            CuiElementContainer container = new CuiElementContainer();
			CuiHelper.DestroyUi(player, ModerMenuSky);
            ...
            container.Add(new CuiElement
            {
                ...
                Components =
                    {
                        new CuiImageComponent {  Color = "0 0 0 0.75", Sprite = "assets/content/ui/ui.background.tiletex.psd", Material = "assets/content/ui/uibackgroundblur-ingamemenu.mat" },
                        new CuiRectTransformComponent { AnchorMin = "0 0", AnchorMax = "1 1" }
                    }
            });
            ...
            container.Add(new CuiPanel
            {
                ...
                Image = { Color = HexToCuiColor("#F7EBE109") }
            ...
            container.Add(new CuiPanel
            {
                ...
                Image = { Material = "assets/content/ui/uibackgroundblur.mat", Color = HexToCuiColor("#F7EBE109") }
            ...
            foreach (var mplayer in BasePlayer.activePlayerList)
            {
                if (ReportData[mplayer.userID].ReportCount >= config.setting.maxreportcall)
                {
                    if (mplayer.userID != player.userID)
                    {
                        container.Add(new CuiButton
                        {
                            RectTransform = { AnchorMin = $"{0.01333263 + (x * 0.25)} {0.9033457 - (y * 0.090)}", AnchorMax = $"{0.252307 + (x * 0.25)} {0.9758366 - (y * 0.090)}" },
                            Button = { Command = $"openinfoplayer chooseplayers {mplayer.userID}", Color = HexToCuiColor("#483D8B") },
			...
			CuiHelper.DestroyUi(player, "TitlePanel");
        ...
        private void playerinfomenu(BasePlayer player, string target, BasePlayer targetinfocheck)
        {
            ...
            int i = 0;
            foreach (var captainlist in ReportData[targets.userID].complaint)
            {
                if (i <= 7)
                {
                    container.Add(new CuiElement
                    {
                        ...
                        Components =
                        {
                            new CuiImageComponent { Color = HexToCuiColor("#17D0833E") },
            ...
            if (!CheckPlayerModeration.ContainsKey(targetinfocheck.userID))
            {
                ...
                container.Add(new CuiButton
                {
                    ...
                    Button = { Material = "assets/content/ui/uibackgroundblur.mat", Color = "0 0 0 0.8", Command = $"openinfoplayer checkinplayer {targetinfocheck.userID}" },
            ...
            else if (CheckPlayerModeration.ContainsKey(targetinfocheck.userID) && CheckPlayerModeration.ContainsValue(player.userID) || player.IsAdmin())
            {
                ...
                container.Add(new CuiButton
                {
                    ...
                    Button = { Material = "assets/content/ui/uibackgroundblur.mat", Color = "0 0 0 0.8", Command = $"openinfoplayer stopcheckingplayers {targetinfocheck.userID}" },
                ...
                container.Add(new CuiButton
                {
                    ...
                    Button = { Material = "assets/content/ui/uibackgroundblur.mat", Color = "0 0 0 0.8", Command = $"openinfoplayer moderatorcheckbanreason {targetinfocheck.userID}" },
            ...
            CuiHelper.DestroyUi(player, "InfoPlayerSky");
            CuiHelper.DestroyUi(player, "captain");
            CuiHelper.DestroyUi(player, "listcap");
            CuiHelper.DestroyUi(player, "CheckPlayerBtn");
            CuiHelper.DestroyUi(player, "GoBanned");
            CuiHelper.AddUi(player, container);
        }

        private void SkyReportPlayers(BasePlayer player, string target = "", string reason = "", string time = "")
        {
            CuiElementContainer container = new CuiElementContainer();
            int x = 0; int y = 0;
            if (target == "" && reason == "")
            {
				...
				container.Add(new CuiElement
                {
                    ...
                    {
						new CuiImageComponent {Color = "0 0 0 0"},
                        new CuiRectTransformComponent { AnchorMin = "0.2322917 0.8472127", AnchorMax = "0.7520834 0.9027551" }
                    }
                });
                ...
                {
                    RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1" },
                    Text = { Text = "В данном окне вам нужно выбрать игрока из списка что бы оставить на него жалобу!", FontSize = 17, Font = "robotocondensed-bold.ttf", Align = TextAnchor.MiddleCenter }
                }, mainskymenu + ".tt");
            ...
            if (target.Length != 17 && reason == "")
            {
                ...
                {
                    int t = 0;
                    CuiElementContainer containers = new CuiElementContainer();
                    foreach (var check in BasePlayer.activePlayerList.Where(p => p.displayName.ToLower().Contains(target.ToLower())).Take(44))
                    {
                        string command = $"SkyReportGo chooseplayers {check.userID}";
                        containers.Add(new CuiButton
                        {
                            FadeOut = 0.1f,
                            RectTransform = { AnchorMin = $"{0.01333263 + (x * 0.244)} {0.9033457 - (y * 0.089)}", AnchorMax = $"{0.252307 + (x * 0.244)} {0.9758366 - (y * 0.089)}" },
                            Button = { Command = command, Color = HexToCuiColor("#483D8B"), Material = "assets/content/ui/uibackgroundblur.mat", FadeIn = 0.4f },
                    ...
                    CuiHelper.AddUi(player, containers);
                });
            }
            else
            {
				...
				CuiHelper.DestroyUi(player, mainskymenu + "tt");

                if (reason == "")
                {
                    container.Add(new CuiPanel
                    {
                        RectTransform = { AnchorMin = "0.2463542 0.4439815", AnchorMax = "0.7536458 0.5560185" },
                        Image = { Material = "assets/content/ui/uibackgroundblur.mat", Color = HexToCuiColor("#F7EBE109") }
                    }, mainskymenu, "buttoncap");

                    container.Add(new CuiLabel
                    {
                        RectTransform = { AnchorMin = "0.2416663 0.5564798", AnchorMax = "0.7583337 0.6009237" },
                        Text = { Text = "Далее вам нужно выбрать причину жалобы на данного игрока!", FontSize = 17, Font = "robotocondensed-bold.ttf", Align = TextAnchor.MiddleCenter }
                    }, mainskymenu, "infotxt2");
                    foreach (var check in config.Reasonsforcomplaint)
                    {
                        container.Add(new CuiButton
                        {
                            RectTransform = { AnchorMin = $"{0.009240265 + (x * 0.245)} {0.5289283 - (y * 0.45)}", AnchorMax = $"{0.2505133 + (x * 0.245)} {0.9173553 - (y * 0.45)}" },
                            Button = { Command = $"SkyReportGo {target.Replace(" ", "").Replace(" ", "").Replace(" ", "")} {check.Key}", Color = HexToCuiColor("#483D8B"), Close = "buttoncap" + "infotxt2", Material = "assets/content/ui/uibackgroundblur.mat", FadeIn = 0.4f },
                            Text = { Text = check.Value, Align = TextAnchor.MiddleCenter, Font = "robotocondensed-bold.ttf", FontSize = 16 }
                        }, "buttoncap");
			...
			CuiHelper.DestroyUi(player, mainskymenu + ".tt");
            CuiHelper.DestroyUi(player, "TitlePanel");
            CuiHelper.DestroyUi(player, mainskymenu + ".Input");
            CuiHelper.DestroyUi(player, mainskymenu + ".Input.Current");
            CuiHelper.DestroyUi(player, "playerslist");
            CuiHelper.AddUi(player, container);
        }
        #endregion
        ...
        public bool IsPlayerAfk(BasePlayer player)
        {
            ...
            if (Vector3.Distance(last, current) > 0.1) return true;
        ...
        public void ReplyWithHelper(BasePlayer player, string message, string[] args = null)
        {
            ...
            {
                config.setting.avatarid,
                string.Format("<size=16><color={2}>{0}</color>:</size>\n{1}", config.setting.prefix, message, config.setting.coloprefix)
            });
        }
        #endregion
        ...
        private void SendChatMessage(string msg, params object[] args)
        {
            ...
            webrequest.EnqueuePost($"https://api.vk.com/method/messages.send?chat_id={config.vKontakte.VK_ChatID}&random_id={randomId}&message={vkchat}&access_token={config.vKontakte.VK_Token}&v=5.92", null, (code, response) => { }, this);
            randomId++;
        }
        #endregion
        ...
        void SendDiscordMsg(string key, params object[] args)
        {
            ...
            FancyMessage newMessage = new FancyMessage(null, false, new FancyMessage.Embeds[1] { new FancyMessage.Embeds($"SkyReportSystem", 15427466, fields) });
