...
{
    [Info("Clans", "RustFame", "1.0.0", ResourceId = 14)]
    public class Clans : RustPlugin
    {
        ...
        [ChatCommand("fast")]
        void cmdfast(BasePlayer player, string[] args)
        {
        ...
        void NewClanUI(BasePlayer player, bool leadermode = false)
        {
            ...
            {
                CursorEnabled = true,
                Image = { Color = "0.14 0.14 0.14 1", Sprite = "assets/content/ui/ui.background.tile.psd" },
        ...
        void ClanTOP(BasePlayer player, int page)
        {
            ...
            foreach (KeyValuePair<string, Clan> key in ClanMembers.Skip(10 * page).Take(ClanMembers.ToList().Count >= 10 ? 10 : ClanMembers.ToList().Count))
            {
                ...
                if ((bool)ClanTournament?.Call<bool>("CheckTournament", ulong.Parse(key.Value.owner)) == true)
                {
                    elements.Add(new CuiLabel
                    {
                        Text = { Text = "Участвуют", Font = "robotocondensed-bold.ttf", FontSize = 14, Color = "0.14 0.93 0.00 0.50", Align = TextAnchor.MiddleCenter },
                        RectTransform = { AnchorMin = $"0.5485205 0", AnchorMax = $"0.6885565 1" },
                    }, "clans_player" + key.Key);
                }
                else
                {
                    elements.Add(new CuiLabel
                    {
                        Text = { Text = "-", Font = "robotocondensed-bold.ttf", FontSize = 14, Color = "255 0 0 0.5", Align = TextAnchor.MiddleCenter },
                        RectTransform = { AnchorMin = $"0.5485205 0", AnchorMax = $"0.6885565 1" },
                    }, "clans_player" + key.Key);
                }
                elements.Add(new CuiLabel
        ...
        public class Clan
        {
            ...
            public static Clan Create(string tag, string description, string ownerId, string owName, string URL)
            {
                var clan = new Clan()
                {
                    ...
                    owner = ownerId,
                    ClanAvatar = "https://cdn.discordapp.com/attachments/900395177057865738/957349074158374982/logo4.png",
                    ownerName = owName,
