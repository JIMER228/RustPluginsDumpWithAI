...
{
    [Info("DynamicCupShare", "FuJiCuRa", "2.7.9", ResourceId = 20)]
    ...
    internal class DynamicCupShare : RustPlugin
    {
        ...
        internal class PlayerInfo
        {
            ...
            [JsonIgnore] [ProtoIgnore] public bool HasTeamShare;
        ...
        private string permGetTeamShares;
        private bool usePermGetTeamShares;
        ...
        private bool TeamShareEnabled;
        private bool TeamCupShare;
        private bool TeamDoorShare;
        private bool TeamTurretShare;
        private bool TeamBoxShare;
        private bool TeamLockerShare;
        private bool TeamQuarryShare;
        ...
        private bool toggleTeamCupShare;
        private bool toggleTeamDoorShare;
        private bool toggleTeamTurretShare;
        private bool toggleTeamBoxShare;
        private bool toggleTeamLockerShare;
        private bool toggleTeamQuarryShare;
        ...
        private void LoadVariables()
        {
            ...
            useClanAlliances = Convert.ToBoolean(GetConfig("Options", "useClanAlliances", true));
            ...
            permGetTeamShares = Convert.ToString(GetConfig("Permission", "permGetTeamShares", "dynamiccupshare.getteamshares"));
            usePermGetTeamShares = Convert.ToBoolean(GetConfig("Permission", "usePermGetTeamShares", false));
            ...
            //TeamCupShare = Convert.ToBoolean(GetConfig("PlayerDefaults", "TeamCupShare", false));
            //TeamDoorShare = Convert.ToBoolean(GetConfig("PlayerDefaults", "TeamDoorShare", false));
            //TeamTurretShare = Convert.ToBoolean(GetConfig("PlayerDefaults", "TeamTurretShare", false));
            //TeamBoxShare = Convert.ToBoolean(GetConfig("PlayerDefaults", "TeamBoxShare", false));
            //TeamLockerShare = Convert.ToBoolean(GetConfig("PlayerDefaults", "TeamLockerShare", false));
            //TeamQuarryShare = Convert.ToBoolean(GetConfig("PlayerDefaults", "TeamQuarryShare", false));
            ...
            //toggleTeamCupShare = Convert.ToBoolean(GetConfig("PlayerToggles", "TeamCupShare", true));
            //toggleTeamDoorShare = Convert.ToBoolean(GetConfig("PlayerToggles", "TeamDoorShare", true));
            //toggleTeamTurretShare = Convert.ToBoolean(GetConfig("PlayerToggles", "TeamTurretShare", true));
            //toggleTeamBoxShare = Convert.ToBoolean(GetConfig("PlayerToggles", "TeamBoxShare", true));
            //toggleTeamLockerShare = Convert.ToBoolean(GetConfig("PlayerToggles", "TeamLockerShare", true));
            //toggleTeamQuarryShare = Convert.ToBoolean(GetConfig("PlayerToggles", "TeamQuarryShare", true));

            //TeamShareEnabled = Convert.ToBoolean(GetConfig("PlayerToggles", "TeamShareEnabled", true));
        ...
        private PlayerInfo SetPlayer(BasePlayer player)
        {
            ...
            if (!playerPrefs.PlayerInfo.TryGetValue(player.userID, out p))
            {
                ...
                info.HasTeamShare = !usePermGetTeamShares || usePermGetTeamShares && permission.UserHasPermission(player.UserIDString, permGetTeamShares);
            ...
            else
            {
                ...
                p.HasTeamShare = !usePermGetTeamShares || usePermGetTeamShares && permission.UserHasPermission(player.UserIDString, permGetTeamShares);
        ...
        private IEnumerator ClnMmbrChng(List<ulong> clanMembers, bool wasAdded, bool isCup = true, bool isTurret = true)
        {
            if (isCup)
            {
                List<uint> playercups;
        ...
        private void OnClanMemberJoined(ulong playerId, List<ulong> clanMembers)
        {            
            if (!clanMembers.Contains(playerId))
                clanMembers.Add(playerId);

            PlayerInfo pInfo;
            foreach (ulong member in clanMembers)
            {
                if (!playerPrefs.PlayerInfo.TryGetValue(member, out pInfo))
                    AddPlayerData(member);
            }
            ServerMgr.Instance.StartCoroutine(ClnMmbrChng(clanMembers, true));
        }
        ...
        private void OnClanMemberGone(ulong playerId, List<ulong> clanMembers)
        {
            ...
            ServerMgr.Instance.StartCoroutine(ClnMmbrChng(clanMembers, false));
        }
        ...
        private void CpSttsTeam(BasePlayer player, bool IsOn)
        {
            RelationshipManager.PlayerTeam playerTeam = player.Team;
            if (playerTeam == null)
                return;

            if (playerTeam.members.Count == 0) return;
            if (IsOn)
            {
                foreach (ulong friendId in playerTeam.members)
                    OnFriendAdded(player.UserIDString, friendId.ToString(), true, false);
            }
            else
            {
                foreach (ulong friendId in playerTeam.members)
                    OnFriendRemoved(player.UserIDString, friendId.ToString(), true, false);
            }
        }

        private void TrrtSttsTeam(BasePlayer player, bool IsOn)
        {
            RelationshipManager.PlayerTeam playerTeam = player.Team;
            if (playerTeam == null)
                return;
            ...
            if (IsOn)
                foreach (ulong friendId in playerTeam.members)
                    OnFriendAdded(player.UserIDString, friendId.ToString(), false, true);
            else
                foreach (ulong friendId in playerTeam.members)
                    OnFriendRemoved(player.UserIDString, friendId.ToString(), false, true);
        }
        ...
        private void CpSttsCln(BasePlayer player, bool IsOn)
        {
            ...
            if (Clan == null) return;
            List<ulong> members = Clan["members"].ToObject<List<ulong>>();
            PlayerInfo pInfo;
            foreach (ulong member in members)
                if (!playerPrefs.PlayerInfo.TryGetValue(member, out pInfo))
                    AddPlayerData(member);
            if (IsOn) ServerMgr.Instance.StartCoroutine(ClnMmbrChng(members, true, true, false));
            else ServerMgr.Instance.StartCoroutine(ClnMmbrChng(members, false, true, false));
        }
        ...
        private void TrrtSttsCln(BasePlayer player, bool IsOn)
        {
            ...
            if (IsOn) ServerMgr.Instance.StartCoroutine(ClnMmbrChng(members, true, false, true));
            else ServerMgr.Instance.StartCoroutine(ClnMmbrChng(members, false, false, true));
        }
        ...
        {
            if (entity == null || !turret.IsValid() || turret.OwnerID == 0uL)
                return null;
            ...
            if (player != null && player.IsConnected && !player.IsDead())
            {
                ...
                if (playerPrefs.PlayerInfo.TryGetValue(userID, out playerInfo))
                {
                    ...
                    if (playerPrefs.PlayerInfo.TryGetValue(turret.OwnerID, out otherInfo))
                    {
                        ...
                        if (TeamShareEnabled && otherInfo.TTS && AreTeammates(turret.OwnerID, userID) && (usePermGetTeamShares && permission.UserHasPermission(userID.ToString(), permGetTeamShares) || !usePermGetTeamShares))
                        {
                            turret.authorizedPlayers.Add(new PlayerNameID { userid = userID, username = displayName });
                            turret.SendNetworkUpdate(BasePlayer.NetworkQueue.Update);
                            return true;
        ...
        private object CanBeTurretTarget(ulong userID, uint netID, ulong OwnerID)
        {
            ...
            if (playerPrefs.PlayerInfo.TryGetValue(OwnerID, out oInfo))
            {
                ...
                if (playerPrefs.PlayerInfo.TryGetValue(userID, out pInfo))
                {
                    ...
                    if ((clansEnabled && (clanTurretShareOverride || oInfo.CTS) && SameClan(OwnerID, userID) && pInfo.HasClanShare && (pInfo.CTS || clanTurretShareOverride)) || 
                        (friendsEnabled && oInfo.TS && HasFriend(OwnerID, userID) && pInfo.HasFriendShare) ||
                        (TeamShareEnabled && oInfo.TTS && AreTeammates(OwnerID, userID) && pInfo.HasTeamShare))
                    {
        ...
        private bool SamVerificationInternal(ulong ownerId, ulong playerId, Action action)
        {
            ...
            if (playerPrefs.PlayerInfo.TryGetValue(ownerId, out oInfo))
            {
                ...
                {
                    if ((clansEnabled && (clanTurretShareOverride || oInfo.CTS) && SameClan(ownerId, playerId) && pInfo.HasClanShare && (pInfo.CTS || clanTurretShareOverride)) || 
                        (friendsEnabled && oInfo.TS && HasFriend(ownerId, playerId) && pInfo.HasFriendShare) ||
                        (TeamShareEnabled && oInfo.TTS && AreTeammates(ownerId, playerId) && pInfo.HasTeamShare))
                    {
                        action?.Invoke();
                        return true;
                    }
        ...
        private void OnEntityBuilt(Planner planner, GameObject obj)
        {
            ...
            if (entity is BuildingPrivlidge)
            {
                ...
                if (enableCupSharing)
                {
                    ...
                    if (TeamShareEnabled && pInfo.TCS) CpSttsTeam(player, true);
                }
            }
            else if (entity is AutoTurret)
            {
                ...
                if (enableTurretSharing)
                {
                    ...
                    if (TeamShareEnabled && pInfo.TTS) TrrtSttsTeam(player, true);
                }
            }
        }

        private void OnEntitySpawned(NPCAutoTurret autoTurret)
        {
            if (autoTurret == null || autoTurret.OwnerID == 0UL)
                return;

            BasePlayer player = BasePlayer.FindByID(autoTurret.OwnerID) ?? BasePlayer.FindSleeping(autoTurret.OwnerID);
            if (player == null)
                return;
            ...
            if (notifyAuthTurret)
                PrintToChat(player, string.Format(prefixFormat, prefixColor, pluginPrefix) + $"<color={colorTextMsg}>" + string.Format(lang.GetMessage("TurretAuth", this, player.UserIDString)) + $"</color>");

            PlayerInfo pInfo;
            if (!playerPrefs.PlayerInfo.TryGetValue(player.userID, out pInfo))
                pInfo = SetPlayer(player);

            playerTurrets[player.UserIDString].Add(autoTurret.net.ID);

            if (enableTurretSharing)
            {
                if (clansEnabled && (pInfo.CTS || clanTurretShareOverride))
                    TrrtSttsCln(player, true);

                if (friendsEnabled && pInfo.TS)
                    TrrtSttsFrnd(player, true);

                if (TeamShareEnabled && pInfo.TTS)
                    TrrtSttsTeam(player, true);
            }
        }
        ...
        private void ShareCommand(BasePlayer player, string command, string[] args)
        {
            ...
            if (args.Length == 0)
            {
                ...
                {
                    if (enableCupSharing)
                        sb.AppendLine($"<color={colorCmdUsage}>teamcup | tc</color> - " +
                                      lang.GetMessage("CommandTeamCup", this, UserIDString) + " " +
                                      (pInfo.TCS ? $"<color={colorON}>ON</color>" : $"<color={colorOFF}>OFF</color>"));
                    if (enableDoorSharing)
                        sb.AppendLine($"<color={colorCmdUsage}>teamdoor | td</color> - " +
                                      lang.GetMessage("CommandTeamDoor", this, UserIDString) + " " +
                                      (pInfo.TDS ? $"<color={colorON}>ON</color>" : $"<color={colorOFF}>OFF</color>"));
                    if (enableBoxSharing)
                        sb.AppendLine($"<color={colorCmdUsage}>teambox | tb</color> - " +
                                      lang.GetMessage("CommandTeamBox", this, UserIDString) + " " +
                                      (pInfo.TBS ? $"<color={colorON}>ON</color>" : $"<color={colorOFF}>OFF</color>"));
                    if (enableLockerSharing)
                        sb.AppendLine($"<color={colorCmdUsage}>teamlocker | tl</color> - " +
                                      lang.GetMessage("CommandTeamLocker", this, UserIDString) + " " +
                                      (pInfo.TLS ? $"<color={colorON}>ON</color>" : $"<color={colorOFF}>OFF</color>"));
                    if (enableQuarrySharing)
                        sb.AppendLine($"<color={colorCmdUsage}>teamquarry | tq</color> - " +
                                      lang.GetMessage("CommandTeamQuarry", this, UserIDString) + " " +
                                      (pInfo.TQS ? $"<color={colorON}>ON</color>" : $"<color={colorOFF}>OFF</color>"));
                    if (enableTurretSharing)
                        sb.AppendLine($"<color={colorCmdUsage}>teamturret | tt</color> - " +
                                      lang.GetMessage("CommandTeamTurret", this, player.UserIDString) + " " +
                                      (pInfo.TTS ? $"<color={colorON}>ON</color>" : $"<color={colorOFF}>OFF</color>"));
            ...
            switch (args[0])
            {
                ...
                case "teamcup":
                case "tc":
                    if (!TeamShareEnabled || !enableCupSharing) goto case "disabled";
                    if (!toggleCupShare) goto case "blocked";
                    pInfo.TCS = !pInfo.TCS;
                    CpSttsFrnd(player, pInfo.TCS);
                    PrintChat(player,
                        string.Format(prefixFormat, prefixColor, pluginPrefix) + $"<color={colorTextMsg}>" +
                        (pInfo.TCS
                            ? lang.GetMessage("ShareEnabled", this, UserIDString)
                ...
                case "teamdoor":
                case "td":
                    if (!TeamShareEnabled || !enableDoorSharing) goto case "disabled";
                    if (!toggleDoorShare) goto case "blocked";
                    pInfo.TDS = !pInfo.TDS;
                    PrintChat(player,
                        string.Format(prefixFormat, prefixColor, pluginPrefix) + $"<color={colorTextMsg}>" +
                ...
                case "teambox":
                case "tb":
                    if (!TeamShareEnabled || !enableBoxSharing) goto case "disabled";
                    if (!toggleBoxShare) goto case "blocked";
                    pInfo.TBS = !pInfo.TBS;
                    PrintChat(player,
                        string.Format(prefixFormat, prefixColor, pluginPrefix) + $"<color={colorTextMsg}>" +
                ...
                case "teamlocker":
                case "tl":
                    if (!TeamShareEnabled || !enableLockerSharing) goto case "disabled";
                    if (!toggleLockerShare) goto case "blocked";
                    pInfo.TLS = !pInfo.TLS;
                    PrintChat(player,
                        string.Format(prefixFormat, prefixColor, pluginPrefix) + $"<color={colorTextMsg}>" +
                ...
                case "teamquarry":
                case "tq":
                    if (!TeamShareEnabled || !enableQuarrySharing) goto case "disabled";
                    if (!toggleQuarryShare) goto case "blocked";
                    pInfo.TQS = !pInfo.TQS;
                    PrintChat(player,
                        string.Format(prefixFormat, prefixColor, pluginPrefix) + $"<color={colorTextMsg}>" +
                        (playerPrefs.PlayerInfo[userID].TQS
                            ? lang.GetMessage("QuarriesEnabled", this, UserIDString)
                            : lang.GetMessage("QuarriesDisabled", this, UserIDString)) + "</color>");
                    break;
                case "teamturret":
                case "tt":
                    if (!TeamShareEnabled || !enableTurretSharing) goto case "disabled";
                    if (!toggleTurretShare) goto case "blocked";
                    pInfo.TTS = !pInfo.TTS;
                    TrrtSttsFrnd(player, pInfo.TS);
                    PrintChat(player,
        ...
        private void OnPlayerRespawned(BasePlayer player)
        {
            ...
            {
                if (!playerPrefs.PlayerInfo.ContainsKey(player.userID))
                    SetPlayer(player);
        ...
        private bool AreTeammates(ulong owner = 0uL, ulong friend = 0uL)
        {
            RelationshipManager.PlayerTeam ownerTeam = RelationshipManager.ServerInstance.FindPlayersTeam(owner);
            RelationshipManager.PlayerTeam friendTeam = RelationshipManager.ServerInstance.FindPlayersTeam(friend);

            if (ownerTeam != null && friendTeam != null)
            {
                if (ownerTeam == friendTeam)
                    return true;
            }
        ...
        private bool SameClan(ulong owner, ulong member)
        {
            ...
            if (useClanAlliances)
            {
                object a = Clans.Call("IsAllyPlayer", owner, member);
                if (a != null && (bool)a)
                    return true;
            }
