...
namespace Oxide.Plugins
{
    ...
    public class Friends : CovalencePlugin
    {
        ...
        private readonly Dictionary<ulong, IPlayer> PlayersRequests = new Dictionary<ulong, IPlayer>();
        ...
        protected override void LoadDefaultMessages()
        {
            ...
            {
                {"List", "<color=#ffe100>RUST LAND:</color> {1}"},
                {"NoFriends", "<color=#ffe100>RUST LAND:</color> У вас нет друзей"},
                {"NotOnFriendlist", "<color=#ffe100>RUST LAND:</color><color=#ffe100> {0}</color> не ваш друг"},
                {"FriendRemoved", "<color=#ffe100>RUST LAND:</color><color=#ffe100> {0}</color> больше не ваш друг"},
                {"PlayerNotFound", "<color=#ffe100>RUST LAND:</color> Игрок <color=#ffe100>{0}</color> не найден"},
                {"CantAddSelf", "<color=#ffe100>RUST LAND:</color> Нельзя добавить себя в друзья"},
                {"AlreadyOnList", "<color=#ffe100>RUST LAND:</color> <color=#ffe100>{0}</color> уже ваш друг"},
                {"FriendlistFull", "<color=#ffe100>RUST LAND:</color> Вы достигли лимита друзей"},
                {"Syntax", "<color=#ffe100>RUST LAND:</color> Список команд\n<color=#ffe100>/friend add Ник</color> - отправить запрос на дружбу\n<color=#ffe100>/friend remove Ник</color> - удалить игрока из друзей"},
                {"AcceptFriend", "<color=#ffe100>RUST LAND:</color> <color=#ffe100>{0}</color> теперь ваш друг"},
                {"MultiplePlayers", "<color=#ffe100>RUST LAND:</color> Найдено несколько игроков с похожим именем: {0}"},
                {"Request", "<color=#ffe100>RUST LAND:</color> Игроку <color=#ffe100>{0}</color> был отправлен ваш запрос на дружбу"},
                {"RequestTarget", "<color=#ffe100>RUST LAND:</color> Игрок <color=#ffe100>{0}</color> отправил вам запрос на дружбу \nИспользуйте команду <color=#ffe100>/friend accept Ник</color> чтобы принять запрос"},
                {"TimedOut", "<color=#ffe100>RUST LAND:</color> Игрок <color=#ffe100>{0}</color> не ответил на ваш запрос"},
                {"TimedOutTarget", "<color=#ffe100>RUST LAND:</color> Вы не ответили на запрос игрока <color=#ffe100>{0}</color>"},
                {"RequestTargetOff", "<color=#ffe100>RUST LAND:</color> Игрок <color=#ffe100>{0}</color> отключился. Запрос отменен"},
                {"NoPendingRequest", "<color=#ffe100>RUST LAND:</color> Нет активных запросов"},
                {"PendingRequest", "<color=#ffe100>RUST LAND:</color> У вас уже имеется активный запрос на дружбу"},
                {"PendingRequestTarget", "<color=#ffe100>RUST LAND:</color> Игрок <color=#ffe100>{0}</color> уже имеет активный запрос на дружбу"}
            }, this);
        ...
        private void FriendCommand(IPlayer player, string command, string[] args)
        {
            ...
            switch (args[0].ToLower())
            {
                ...
                case "+":
                    ...
                    if (PlayersRequests.ContainsKey(Convert.ToUInt64(friendPlayer.Id)))
                    {
                        Reply(player, "PendingRequest");
                        return;
                    }
                    if (PlayersRequests.ContainsKey(Convert.ToUInt64(friendPlayer.Id)))
                    {
                        Reply(player, "PendingRequestTarget", friendPlayer.Name);
                        return;
                    }
                    PlayersRequests[Convert.ToUInt64(player.Id)] = friendPlayer;
                    PlayersRequests[Convert.ToUInt64(friendPlayer.Id)] = player;
                    PendingRequests[Convert.ToUInt64(friendPlayer.Id)] = timer.Once(60, () => {
                    RequestTimedOut(player, friendPlayer);
                    });
                    Reply(player, "Request", friendPlayer.Name);
                    Reply(friendPlayer, "RequestTarget", player.Name);
                    return;
                ...
                case "-":
                    ...
                    if (removed) RemoveFriendS(friend.ToString(), player.Id);
                    return;
                case "accept":
                    Timer reqTimer;
                    if (!PendingRequests.TryGetValue(Convert.ToUInt64(player.Id), out reqTimer))
                    {
                        Reply(player, "NoPendingRequest");
                        return;
                    }
                    var playerDataReq = GetPlayerData(Convert.ToUInt64(player.Id));
                    if (playerDataReq.Friends.Count >= configData.MaxFriends)
                    {
                        Reply(player, "FriendlistFull");
                        return;
                    }
                    var friendPlayerReq = PlayersRequests[Convert.ToUInt64(player.Id)];
                    AddFriend(Convert.ToUInt64(player.Id), Convert.ToUInt64(friendPlayerReq.Id));
                    AddFriend(Convert.ToUInt64(friendPlayerReq.Id), Convert.ToUInt64(player.Id));
                    Reply(player, "AcceptFriend", friendPlayerReq.Name);
                    Reply(friendPlayerReq, "AcceptFriend", player.Name);
                    reqTimer.Destroy();
                    PendingRequests.Remove(Convert.ToUInt64(player.Id));
                    PlayersRequests.Remove(Convert.ToUInt64(player.Id));
                    PlayersRequests.Remove(Convert.ToUInt64(friendPlayerReq.Id));
                    break;
            }
        }

        private void RequestTimedOut(IPlayer player, IPlayer friendPlayer)
        {
            PlayersRequests.Remove(Convert.ToUInt64(player.Id));
            PlayersRequests.Remove(Convert.ToUInt64(friendPlayer.Id));
            PendingRequests.Remove(Convert.ToUInt64(friendPlayer.Id));
            Reply(player, "TimedOut", friendPlayer.Name);
            Reply(friendPlayer, "TimedOutTarget", player.Name);
        }
