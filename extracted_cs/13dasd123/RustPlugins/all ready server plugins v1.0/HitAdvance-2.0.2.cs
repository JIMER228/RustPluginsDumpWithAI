...
namespace Oxide.Plugins
{
    ...
    public class HitAdvance : RustPlugin
    {
                ...
                switch (GetSettings().CurrentType)
                {
                    ...
                    case HitType.Text:
                    {
                        ...
                        if (target is BasePlayer)
                        {
                            ...
                            {
                                textDamage = "<color=#DC143C>Bayıltıldı</color>";
                                if (info.isHeadshot)
                                    textDamage += " <color=#DC143C>Kafa vuruşu!</color>";
                            }
        
                            if (Player.currentTeam == targetPlayer.currentTeam && Player.currentTeam != 0)
                            {
                                textDamage = "<color=#32915a>Arkadaşına vurdun!</color>";
                                division = 1;
                            }
                        }
                        ...
                        container.Add(new CuiElement()
                        {
                            ...
                            Components =
                            {
                                new CuiTextComponent { Text = $"<b>{textDamage}</b>", Color = HexToRustFormat("#FFFFFF"), Font = "robotocondensed-bold.ttf", FontSize = (int) Mathf.Lerp(15, 15, division), Align = TextAnchor.MiddleCenter, },
        ...
        {
            [JsonProperty("İşaret Adı")]
            public string DisplayName;
            [JsonProperty("Çözünürlük")]
            public string Permission;

            [JsonProperty("Başlangıçta oyuncular tarafından etkin olsun mu?")]
            public bool IsDefault;
        }
        
        private class Configuration
        {
            [JsonProperty("Farklı işaret ayarları")]
            public Hash<HitType, HitSetting> HitSettings = new Hash<HitType, HitSetting>();

            [JsonProperty("İlk başta bina hasarı etkin olsun mu?")]
            public bool DefaultBuildingDamage = true; 
            [JsonProperty("NPC hasarı gösterilsin mi?")]
            public bool ShowNPCDamage = true;
            [JsonProperty("Hayvan hasarı gösterilsin mi?")]
            public bool ShowAnimalDamage = false;
            [JsonProperty("Hiç bir şeye hasar verilmezse işaretin otomatik kapatılma süresi.")]
            public float DestroyTime = 0.25f;
            [JsonProperty("İşaretçinin değiştirilme menüsünün komutu.")]
            ...
            public static Configuration LoadDefault()
            {
                return new Configuration
                {
                    HitSettings = new Hash<HitType, HitSetting>
                    {
                        ...
                        {
                            DisplayName = "<b><size=20>İşaret tamamen devre dışı</size></b>\n" +
                                    "Düşmana saldırdığında hasarı göremezsin.",
                        ...
                        {
                            DisplayName = "<b><size=20>Can şeridi</size></b>\n" +
                                    "Ekranda bir şerit belirir." +
                                    "\nDüşmanın kalan canını gösterir.",
                        ...
                        {
                            DisplayName = "<b><size=20>Hasar Metni</size></b>\nEkranın ortasında aç\nHasar numaraları",
                        ...
                        {
                            DisplayName = "<b><size=20>Metin ve Şerit</size></b>\nİki önceki belirteç aynı anda ateş ediyor</b>!",
                        ...
                        {
                            DisplayName = "İsabet İşareti\nİsabet işaretini değiştir\nKafasına vurulduğunda gözüken renk</b>!",
        ...
        private void UI_DrawInterface(BasePlayer player)
        {
            ...
            container.Add(new CuiLabel
            {
                ...
                Text          = {Text      = "Bu menüden istediğiniz gibi hasar işaretine ayar çekebilirsiniz.", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", Color = "1 1 1 1", FontSize = 28}
            }, Layer);
            ...
            for (int i = 0; i < 6; i += 3)
            {
                ...
                foreach (var check in Settings.HitSettings.Skip(i).Take(3))
                {
                    ...
                    {
                        text = "Etkin";
                        color = "0.70418 1 0.7 0.5";  
                    }
                    else if (!check.Value.Permission.IsNullOrEmpty() && !permission.UserHasPermission(player.UserIDString, check.Value.Permission))
                    {
                        color = "1 0.7 0.7 0.50";
                        text = "Yetkiniz bulunmamakta.";
                    }
                    
                    container.Add(new CuiPanel
                    {
                        RectTransform = {AnchorMin = $"0.5 {0.65 - i * 0.08}", AnchorMax = $"0.5 {0.65 - i * 0.08}", OffsetMin = $"{leftPosition} -50", OffsetMax = $"{leftPosition + 300} 50"},
                    ...
                    container.Add(new CuiButton
                    {
                        ...
                        Text          = {Text      = text, Align = TextAnchor.MiddleCenter, Font = "robotocondensed-bold.ttf", FontSize = 22, Color = "1 1 1 1"}
                    }, Layer + check.Key);
            ...
            string toggleText  = "Binaların hasarı kapatıldı";
            string toggleColor = "1 0.6 0.6 0.6";
            if (obj.GetSettings().BuildingHit)
            {
                toggleText  = "Binaların hasarı aktif edildi";
                toggleColor = "0.70418 1 0.7 0.6";
            }
            container.Add(new CuiButton
            {
                RectTransform = {AnchorMin = "0.5 0.2", AnchorMax = "0.5 0.2", OffsetMin = "-210 10", OffsetMax = "190 50"},
                Button        = {Color     = toggleColor, Command = $"UI_HitAdvance toggle" },
                Text          = {Text      = toggleText, Align    = TextAnchor.MiddleCenter, Font = "robotocondensed-bold.ttf", FontSize = 22, Color = "1 1 1 0.5"}
            }, Layer);
