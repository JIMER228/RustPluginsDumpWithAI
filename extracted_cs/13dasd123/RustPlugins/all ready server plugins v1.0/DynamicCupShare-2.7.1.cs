...
{
    [Info("DynamicCupShare", "FuJiCuRa", "2.7.1", ResourceId = 20)]
    ...
    internal class DynamicCupShare : RustPlugin
    {
        ...
        {
            if (!includeSamSites || sam.OwnerID == 0) return null;
            BaseVehicleSeat[] seats = null;
            BasePlayer[] childs = target.GetComponentsInChildren<BasePlayer>();
            if (childs.Length == 0)
            {
                seats = target.GetComponentsInChildren<BaseVehicleSeat>();
                if (seats.Length > 0)
                {
                    for (int i = 0; i < seats.Length; i++)
                    {
                        if (seats[i]._mounted != null)
                        {
                            ulong userID = seats[i]._mounted.userID;
                            if (userID == sam.OwnerID || AccessOn(userID))
                            {
                            ...
                            PlayerInfo oInfo;
                            if (playerPrefs.PlayerInfo.TryGetValue(sam.OwnerID, out oInfo))
                            {
                                PlayerInfo pInfo;
                                if (playerPrefs.PlayerInfo.TryGetValue(userID, out pInfo))
                                {
                                    if (clansEnabled && (clanTurretShareOverride || oInfo.CTS) && SameClan(sam.OwnerID, userID) && pInfo.HasClanShare && (pInfo.CTS || clanTurretShareOverride) || friendsEnabled && oInfo.TS && HasFriend(sam.OwnerID, userID) && pInfo.HasFriendShare)
                                    {
                                        sam.CancelInvoke(sam.WeaponTick);
                                        return false;
                                    }
            ...
            if (!samSiteShootEmptyVehicles && childs.Length == 0)
            {
                if (seats.Length == 0 || seats.Length > 0 && seats[0]._mounted == null)
                {
                    sam.CancelInvoke(sam.WeaponTick);
                    return false;
                }
        ...
        {
            if (privilege == null || player == null || privilege.OwnerID == player.userID || !privilege.AnyAuthed() ||
                !privilege.authorizedPlayers.Any((PlayerNameID x) => x.userid == privilege.OwnerID)) return null;
