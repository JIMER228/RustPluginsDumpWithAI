...
{
    [Info("WipeDetection", "Admin", "0.1.3")]
    ...
    class WipeDetection : RustPlugin
    {
        ...
        void OnNewSave()
        {
            configData.LastWipe = DateTime.Now.Date.ToString("MM/dd/yyyy/h:m:s");
            configData.NextWipe = DateTime.Now.AddDays(configData.DaysBetweenWipes).ToString("MM/dd/yyyy/h:m:s");
        ...
        #region Functions        
        private DateTime ParseTime(string time) => DateTime.ParseExact(time, "MM/dd/yyyy/h:m:s", CultureInfo.InvariantCulture);
      //  Слив плагинов server-rust by Apolo YouGame
        ...
        class ConfigData
        {
            ...
            [JsonProperty("Последний вайп был 'MM/dd/yyyy/h:m:s'")]
            public string LastWipe { get; set; }
            [JsonProperty("Следующий вайп будет 'MM/dd/yyyy/h:m:s'")]
        ...
        private void LoadVariables()
        {
            ...
            if (string.IsNullOrEmpty(configData.LastWipe))
                configData.LastWipe = DateTime.Now.ToString("MM/dd/yyyy/h:m:s");
