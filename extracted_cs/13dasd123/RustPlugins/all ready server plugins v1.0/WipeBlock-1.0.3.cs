...
namespace Oxide.Plugins
{
    ...
    class WipeBlock : RustPlugin
    {
        ...
        private object OnMagazineReload(BasePlayer player, BaseProjectile projectile)
        {
            if (!player.userID.IsSteamId())
            {
                return null;
        ...
        {
            if (plugins.Find("AimTrain") && (bool)plugins.Find("AimTrain").Call("IsAimTraining", player.userID)) return true;
            return false;
        }
        ...
        object CanMountEntity(BasePlayer player, MLRS entity)
        {
            ...
            var isBlocked = IsBlocked("mlrs") > 0 ? false : (bool?)null;
            if (isBlocked == false)
            {
                MessBlockUi(player, "mlrs", "МЛРС");
                timer.Once(3.8f, () =>
                {
                    CuiHelper.DestroyUi(player, Layer);
                });
        ...
        private void OnServerInitialized()
        {
            ...
            {
                if (check.Key == "pistol.prototype17")
                {
                    Puts("Popusk");
                    ImageLibrary.Call("AddImage", $"https://github.com/rostov114/rust-items/raw/main/pistol.prototype17.png", check.Key);
                }
                else
                {
                    if (check.Key == "mlrs")
                    {
                        Puts("Popusk");
                        ImageLibrary.Call("AddImage", $"https://rustlabs.com/img/screenshots/mlrs.png", check.Key);
                    }
                    else
                    {
        ...
        private void BlockUi(BasePlayer player)
        {
            ...
            container.Add(new CuiButton
            {
                ...
                Button =
                {
                    Color = "0.235 0.227 0.180 0.90", Material = "assets/icons/greyout.mat", FadeIn = 0.3f,
                    Close = Layer
                },
                Text =
                {
            ...
            container.Add(new CuiElement
            {
                ...
                Components =
                {
                    new CuiImageComponent {Color = "0.36 0.34 0.32 0.75", Material = "assets/icons/greyout.mat", FadeIn = 0.3f},
                ...
                container.Add(new CuiElement
                {
                    ...
                    Components =
                    {
                        new CuiTextComponent()
                        {
                            ...
                            Text =
                                $"{text}",
                            FontSize = 14, Align = TextAnchor.LowerCenter, Font = "robotocondensed-regular.ttf"
                        },
                        new CuiRectTransformComponent {AnchorMin = "-0.1 0.02", AnchorMax = "1.1 1"},
                    }
                });
            }
            container.Add(new CuiButton
