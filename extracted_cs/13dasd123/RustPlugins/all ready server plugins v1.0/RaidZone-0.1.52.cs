...
{
    [Info("RaidZone", "TopPlugin.ru", "0.1.52")]
    ...
    public class RaidZone : RustPlugin
    {
        ...
        private void SendNotify(BasePlayer player, string message, int type, params object[] obj)
        {
            if (Notify != null || UINotify != null)
                Interface.Oxide.CallHook("SendNotify", player, type, Msg(player, message, obj));
            else
                Reply(player, message, obj);
        }
        ...
        [PluginReference] private Plugin DiscordCore, HaxBot, Notify, UINotify;
        ...
        private IEnumerator GORAID(BaseCombatEntity entity, BasePlayer player, int tt = 0)
        {
            ...
            if (priv != null && !priv.authorizedPlayers.Any(x => x.userid == player.userID))
            {
                ...
                string connect = "217.114.43.78:35555";
        ...
        private void ALERTPLAYER(ulong ID, string name, string quad, string connect, string destroy)
        {
            ...
            if (config.GAME.enable && alert.gamecooldown < DateTime.Now)
            {
                ...
                {
                    SendNotify(player, config.GAME.text.Replace("{name}", name).Replace("{quad}", quad).Replace("{destroy}", destroy), 1);
        ...
        private void GetRequest(string reciverID, string msg, BasePlayer player = null, string num = null) => webrequest.Enqueue("https://api.vk.com/method/messages.send?domain=" + reciverID + "&message=" + msg.Replace("#", "%23") + "&v=5.86&access_token=" + config.vk.api, null, (code2, response2) => ServerMgr.Instance.StartCoroutine(GetCallback(code2, response2, reciverID, player, num)), this);
        ...
        #region HEADER
        private const string permvk = "vkraidalert.online";
