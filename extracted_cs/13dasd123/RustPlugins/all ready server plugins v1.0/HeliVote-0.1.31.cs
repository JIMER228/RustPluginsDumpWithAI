...
namespace Oxide.Plugins
{
    ...
    class HeliVote : RustPlugin
    {
        ...
        {
            Puts("Создается новый файл конфигурации..");
            Config.Clear();
            LoadVariables();
        }
        void Unload()
        ...
        private void CallHeli(int amount)
        {
            ...
            while (i < amount)
            {
                ...
                {
                    if (initiator != null)
                    entity.GetComponent<PatrolHelicopterAI>().State_Move_Enter(initiator.transform.position + new Vector3(91.6f, 88.7f, -212.9f));
                }
                i++;
            }
        }
        private void VoteTimer(int amount)
        {
            ...
            {                
                ...
                if (time == 180)
                {
                    msgAll(string.Format(lang.GetMessage("timeLeft", this), 2, "минуты"));
                }
                if (time == 120)
                {
                    msgAll(string.Format(lang.GetMessage("timeLeft", this), 2, "минуты"));
                ...
                if (time == 60)
                {
                    msgAll(string.Format(lang.GetMessage("timeLeft", this), 1, "минута"));
                }
                if (time == 30)
                {
                    msgAll(string.Format(lang.GetMessage("timeLeft", this), 30, "секунд"));
                }
                if (time == 10)
                {
                    msgAll(string.Format(lang.GetMessage("timeLeft", this), 10, "секунд"));
                }
            });
        }
        ...
        Dictionary<string, string> messages = new Dictionary<string, string>()
        {
            ...
            {"badSyn", "<color=#ff704d>/helivote open ##</color> - Откройте голосование, чтобы породить ## количество вертолетов. Максимум <color=#ff704d>{0}</color>" },
            {"voteOpen", "Голосование уже открыто." },
            {"noOpen", "Сейчас нет открытого голосования" },
            {"yesVote", "Ваш голос: Да"},
            {"noVote", "Ваш голос: Нет" },
            {"alreadyVoted", "Вы уже проголосовали." },
            {"opened", "Голосование по вызову <color=#ff704d>{0}</color> вертолета(ов) открыто!\nПроголосовать <color=#ff704d>/helivote yes</color> или <color=#ff704d>/helivote no</color>" },
            {"required", "Минимальное количество голосов за ответ: Да <color=#ff704d>{0}</color>" },
            {"invAmount", "Вы ввели неверный номер.." },
            {"timeLeft", "Голосование заканчивается заканчивается через {0} {1},\nПроголосовать <color=#ff704d>/helivote yes</color> или <color=#ff704d>/helivote no</color>" },
            {"cooldown", "Перед открытием голосования необходимо дождаться окончания периода восстановления." },
            {"voteSuccess", "Голосование прошло успешно, вызвано <color=#ff704d>{0}</color> верталет(ов)!" },
            {"voteFail", "Голосование не соответствует требованиям, повторите попытку через <color=#ff704d>{0}</color> минут" },
            {"heliActive", "Есть еще активные вертолеты с последнего голосования.\nНельзя открыть новое голосование, пока они не будут уничтожены" },
