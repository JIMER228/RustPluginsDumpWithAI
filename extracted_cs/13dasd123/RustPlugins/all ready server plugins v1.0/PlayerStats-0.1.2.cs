...
{
    [Info("Player Stats", "Server-rust.ru", "0.1.2")]
    ...
    class PlayerStats : RustPlugin
    {
        ...
        private void LoadConfigValues()
        {
            ...
            var _Quarrys = new List<object>()
            {
                {"usergroup add {player} quarry"},
                {"usergroup add {player} quarry"}
            };
            ...
            if (GetConfig("Награда игроков", "Награда для ТОП-1 в добыче ресурсов карьерами", ref _Quarrys))
                changed = true;
            Quarrys = _Quarrys.Select(p => p.ToString()).ToList();
      ...
      //  Слив плагинов server-rust by Apolo YouGame
        ...
        Dictionary<string, string> Images = new Dictionary<string, string>()
        {
            ...
            { "imagesgather1", "http://i.imgur.com/LZfTlv4.png" },
        };
        ...
        void GenerateLastWipeTopPlayers()
        {
            Dictionary<topSkill, ulong> topUsers = new Dictionary<topSkill, ulong>
            {
                ...
                { topSkill.quarry, data.OrderByDescending(x => x.Value.quarry.stone + x.Value.quarry.metal + x.Value.quarry.metalOre + x.Value.quarry.sulfurOre + x.Value.quarry.hqmetal + x.Value.quarry.hqmetalOre).Select(x => x.Key).First() },
        ...
        void cmdChat_topGift(BasePlayer player, string cmd, string[] args)
        {
            ...
            if (args != null && args.Length > 0)
            {
                ...
                if (args[0] == "quarry")
                {
                    foreach (string command in Quarrys)
                    {
                        rust.RunServerCommand(command.Replace("{player}", player.userID.ToString()));
                    }
                }
        ...
        void CreateUINext(BasePlayer player)
        {
            ...
            items = from pair in data orderby (pair.Value.quarry.stone + pair.Value.quarry.metal + pair.Value.quarry.metalOre + pair.Value.quarry.sulfurOre + pair.Value.quarry.hqmetal + pair.Value.quarry.hqmetalOre) descending select pair;
            i = 1;
            string topGve1 = "";
            foreach (KeyValuePair<ulong, PlayerData> pair in items)
            {
                string name = pair.Value.name;
                if (name == null) name = "Неизвестный";
                topGve1 = topGve1 + i.ToString() + ". " + name + " - " + (pair.Value.quarry.stone + pair.Value.quarry.metal + pair.Value.quarry.metalOre + pair.Value.quarry.sulfurOre + pair.Value.quarry.hqmetal + pair.Value.quarry.hqmetalOre).ToString() + "\n";
                i++;
                if (i > 10) break;
            }
        ...
        void CreateUI(BasePlayer player)
        {
            ...
            CuiHelper.AddUi(player,
                ...
                .Replace("{победDuel}", data[player.userID].pvp.duelWin.ToString())
                ...
                .Replace("{каменьк}", data[player.userID].quarry.stone.ToString())
                .Replace("{рудак}", data[player.userID].quarry.metalOre.ToString())
                .Replace("{серная рудак}", data[player.userID].quarry.sulfurOre.ToString())
                .Replace("{металк}", data[player.userID].quarry.metal.ToString())
                .Replace("{МВКк}", data[player.userID].quarry.hqmetal.ToString())
                .Replace("{МВКкк}", data[player.userID].quarry.hqmetalOre.ToString())
      ...
      //  Слив плагинов server-rust by Apolo YouGame
            ...
            DynamicConfigFile dataFile = Interface.Oxide.DataFileSystem.GetFile("PlayerStats/PlayerStatsImages");
