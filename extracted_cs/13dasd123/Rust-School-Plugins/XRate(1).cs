...
{
    /*ПЛАГИН БЫЛ ПОФИКШЕН С ПОМОЩЬЮ ПРОГРАММЫ СКАЧАНОЙ С https://discord.gg/dNGbxafuJn */ [Info("XRate", "https://discord.gg/dNGbxafuJn", "2.0.3")]
    public class XRate : RustPlugin
    {
        const bool fermensEN = false; // true - ENGLISH
        ...
        protected override void LoadConfig()
        {
            ...
            {
                if (!x.Value.CollectibleExceptions.Any())
                {
                    x.Value.CollectibleExceptions = new()
                    {
                        ["diesel_barrel"] = 5,
                        ["cloth"] = 5
                    };
            ...
            if (!config.rates.CollectibleExceptions.Any())
            {
                config.rates.CollectibleExceptions = new()
                {
                    ["diesel_barrel"] = 1,
                    ["cloth"] = 1
                };
        ...
        class rateset
        {
            ...
            [JsonProperty(fermensEN ? "Exceptions of collectible objects" : "Исключения подбираемых объектов")]
            public Dictionary<string, int> CollectibleExceptions;
        }
        ...
        private class PluginConfig
        {
            ...
            public static PluginConfig DefaultConfig()
            {
                return new PluginConfig()
                {
                    ...
                    {
                        { "xrate.x3", new rateset{ CollectibleExceptions = new()
                        {
                            ["diesel_barrel"] = 5,
                            ["cloth"] = 5
                        }, carier = 3f, gather = 3f, grab = 3f, npc = 3f, speed = 4f, sulfur = 2.5f, containers = new boxes{ tank = 3f, simp = 3f, loke = 3f, heli = 3f, elit =3f, bare = 3f,aird = 3f } } },
                        { "xrate.x4", new rateset{ CollectibleExceptions = new()
                        {
                            ["diesel_barrel"] = 10,
                            ["cloth"] = 10
                        }, carier = 4f, gather = 4f, grab = 4f, npc = 4f, speed = 4f, sulfur = 2.5f, containers = new boxes{ tank = 4f, simp = 4f, loke = 4f, heli = 4f, elit =4f, bare = 4f,aird = 4f } } }
                    },
                    rates = new rateset {CollectibleExceptions = new()
                    {
                        ["diesel_barrel"] = 1,
                        ["cloth"] = 1
                    }, carier = 2f, gather = 2f, grab = 2f, npc = 2f, speed = 2f, sulfur = 2f, containers = new boxes { tank = 2f, simp = 2f, loke = 2f, heli = 2f, elit = 2f, bare = 2f, aird = 2f } },
        ...
        Dictionary<NetworkableId, float> cashcariers = new Dictionary<NetworkableId, float>();
        static XRate ins;
        void Init()
        {
            ins = this;
        ...
        rateset getuserrate(string id, float bonus = 0f)
        {
            ...
            cash[id].CollectibleExceptions = rate.CollectibleExceptions;
        ...
        void OnCollectiblePickup(CollectibleEntity collectible, BasePlayer player)
        {
            ...
            foreach (var item in collectible.itemList)
            {
                if (item == null) 
                    continue;
                if (config.blacklist.Contains(item.itemDef.shortname)) 
                    return;

                var rateset = GetRate(player.UserIDString);

                if (rateset.CollectibleExceptions.TryGetValue(item.itemDef.shortname, out var amount))
                {
                    item.amount = amount;
                    continue;
                }
                
                item.amount = (int)(item.amount * rateset.grab);
            }
        }
        ...
        void OnGrowableGather(GrowableEntity plant, Item item, BasePlayer player)
        {
            ...
            if (rateset.CollectibleExceptions.TryGetValue(item.info.shortname, out var amount))
            {
                item.amount = amount;
                return;
            }
        ...
        private void CashCarier(ulong id, NetworkableId netid)
        {
        ...
        {
            CashCarier(player.userID, entity.net.ID);
        }
        ...
        void OnQuarryGather(MiningQuarry quarry, Item item)
        {
            ...
            float rate;
            if (!cashcariers.TryGetValue(quarry.net.ID, out rate))
            {
        ...
        void OnContainerDropItems(ItemContainer container)
        {
            ...
            NetworkableId ID = lootcont.net.ID;
        ...
        List<NetworkableId> CHECKED = new List<NetworkableId>();
        ...
        private void OnLootEntity(BasePlayer player, object entity)
        {
            ...
            if (entity is NPCPlayerCorpse)
            {
                ...
                NetworkableId ID = nPCPlayerCorpse.net.ID;
                if (CHECKED.Contains(ID)) return;
                rateset rateset;
        ...
        private void OnOvenCook(ElectricOven oven)
        {
            if (oven.LastLootedBy == 0)
                return;
            
            if (DefaultSmeltSpeed.TryGetValue(oven.ShortPrefabName, out var dspeed))
            {
                int number = (int)GetRate(oven.LastLootedBy.ToString()).speed;
