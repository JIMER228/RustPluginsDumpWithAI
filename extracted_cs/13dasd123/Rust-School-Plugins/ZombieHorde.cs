...
{
    [Info("ZombieHorde", "k1lly0u", "0.6.25")]
    class ZombieHorde : RustPlugin, IChaosNPCPlugin
    {
        ...
        private void CreateMonumentHordeOrders()
        {
            ...
            foreach (GameObject gobject in allObjects)
            {
                ...
                if (gobject.name.Contains("autospawn/monument"))
                {
                    ...
                    if (gobject.name.Contains("ferry_terminal_1") && Configuration.Monument.Ferry.Enabled)
                    {
                        Horde.SpawnOrder.Create(tr.TransformPoint(new Vector3(-6.9f, 5.3f, 6.2f)), Configuration.Monument.Ferry);
                        count++;
                        continue;
                    }
        ...
        public class ZombieNPCBrain : CustomScientistBrain
        {
            ...
            public class ZombieChaseState : BasicAIState
            {
                ...
                public override StateStatus StateThink(float delta, BaseAIBrain brain, BaseEntity entity)
                {
                    ...
                    if (attackEntity is BaseMelee or BaseProjectile && baseEntity is BasePlayer)
                    {
        ...
        private void UpdateConfigValues()
        {
            ...
            {
                Configuration.Monument.Ferry = new ConfigData.MonumentSpawn.MonumentSettings
                {
                    Enabled = false,
                    RoamDistance = 90,
                    HordeSize = 10,
        ...
        private static class TravellingVendorIsInvalidPlayerPatch
        {
            ...
            {
                if (player && player is global::HumanNPC or NPCPlayer)
                {
                    __result = true;
                    return false;
                }
