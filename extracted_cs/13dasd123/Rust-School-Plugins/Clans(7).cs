	...
	public class Clans : RustPlugin
	{
		...
		private class ResourceStandart
		{
				...
				string name = null)
			{
				if (_image == null)
					...
					{
						Png = _instance.ImageLibrary?.Call<string>("GetImage", itemId)
					};
...
#endif
			{
				if (container == null || item == null || item.uid != 0) return;
				...
				if (_looters.TryAdd(item.uid, id))
				{
#if TESTING
					Puts(
						$"Added item {item.info.shortname} UID {item.uid} to looters dictionary with owner ID {id}");
#endif
				}
				else
				{
...
#endif
			{
				if (item == null || player == null || item.uid != 0) return;
...
#if TESTING
						Puts(
							$"Added {item.amount} {item.info.shortname} to player {player.userID} stats and removed looter for item UID {item.uid}");
#endif
					}
				}
				else
...
#if TESTING
					Puts($"Added looter for item UID {item.uid} with player ID {player.userID}");
#endif
				}
			}
		}
...
#if TESTING
						Puts(
							$"Adding last attacker {info.InitiatorPlayer.UserIDString} to heli {helicopter.net.ID}");
#endif
		...
		private void OnEntityDeath(BaseCombatEntity entity, HitInfo info)
		{
...
#endif
			{
				...
				{
					if (_lastHeli.TryGetValue(entity.net.ID, out player))
					{
						if (player != null)
						{
#if TESTING
							...
							for (var i = 0; i < items.Count; i++)
							{
								...
								container.Add(new CuiElement
								{
									...
									Components =
									{
										new CuiRawImageComponent
										{
											Png = ImageLibrary?.Call<string>("GetImage", FindItemID(item))
										},
										new CuiRectTransformComponent
										{
				...
				for (var i = 0; i < items.Count; i++)
				{
					...
					container.Add(new CuiElement
					{
						...
						Components =
						{
							new CuiRawImageComponent
							{
								Png = ImageLibrary?.Call<string>("GetImage", def.itemid)
							},
							new CuiRectTransformComponent
							{
								AnchorMin = "0.5 1", AnchorMax = "0.5 1",
				...
				for (var i = 0; i < skins.Count; i++)
				{
					...
					container.Add(new CuiElement
					{
						...
						Components =
						{
							new CuiRawImageComponent
							{
								Png = ImageLibrary?.Call<string>("GetImage", FindItemID(shortName))
							},
							new CuiRectTransformComponent
							{
								AnchorMin = "0.5 1", AnchorMax = "0.5 1",
