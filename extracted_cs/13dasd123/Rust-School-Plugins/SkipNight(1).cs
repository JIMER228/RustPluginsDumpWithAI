	...
	public class SkipNight : RustPlugin
	{
		...
		private class Configuration
		{
			...
			public InterfaceSettings UI = new InterfaceSettings
			{
				...
				DestroyTime = 5,
				ShowImage = true,
				Image = "https://i.ibb.co/cJh7Ytx/image.png",
				ImageWidth = 42,
				ImageHeight = 33,
				ImageUpIndent = 16,
				LeftIndent = 212,
		...
		{
			if (arg.cmd.FullName == "env.time" && arg.Args.Length > 0)
				_engine.OnExternalSetTime();
		...
		private class SkipEngine : FacepunchBehaviour
		{
			...
			private bool _isSkippedVoting = false;
...
#if TESTING
				Debug.Log($"[StartVote] MissedNights={_config.Time.MissedNights}, _skippedNight={_skippedNight}, result: {!(_config.Time.MissedNights > 0 && _config.Time.MissedNights <= _skippedNight)}");
#endif
			...
			private void StopVote()
			{
				...
				{
#if TESTING
					Debug.Log("[StopVote] set skipped to zero");
#endif
					_skippedNight = 0;
			...
			{
#if TESTING
				Debug.Log($"[OnDay] init with skippedNight={_skippedNight}, MissedNights={_config.Time.MissedNights}");
#endif

				if (_skippedNight == _config.Time.MissedNights)
					_skippedNight = 0;
			}
			...
			{
				return _config.Time.MissedNights > 0 &&
				       _config.Time.MissedNights <= _skippedNight;
			}
		...
		{
			return ImageLibrary?.Call<string>("GetImage", image, skin) ?? string.Empty;
		}
