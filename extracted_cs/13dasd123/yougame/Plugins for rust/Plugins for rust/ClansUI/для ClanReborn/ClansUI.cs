...
{
    [Info("ClansUI", "k1lly0u", "0.1.4", ResourceId = 0)]
    class ClansUI : RustPlugin
    {
        ...
        {
            LoadVariables();
            if (Clans.Author != "Fujikura")
            {
                PrintError("This UI menu can only be used with Clans REBORN!\nYou can purchase a copy of it here : http://www.chaoscode.io/resources/clans-reborn.14/");
        ...
        void MembersMenu(BasePlayer player, Clans.Clan clan, int page = 0)
        {
            ...
            UI.CreateLabel(ref container, ClanUI, "", $"{msg("Название клана: ", player.UserIDString)} {clan.tag}\n\n{msg("Описание: ", player.UserIDString)} {clan.description}", 16, "0.02 0.72", "0.25 0.93", TextAnchor.UpperLeft);
            ...
            UI.CreateButton(ref container, ClanUI, UI.Color(configData.UIColors.ButtonColor.HexColor, configData.UIColors.ButtonColor.Opacity), $"{msg("Дружественный огонь", player.UserIDString)}  :  {(Clans.HasFFEnabled(player.userID) ? msg("<color=green>ВКЛ</color>", player.UserIDString) : !canToggleFF ? msg("<color=red>Отключен</color>", player.UserIDString) : msg("<color=red>ВЫКЛ</color>", player.UserIDString))}", 15, "0.26 0.805", "0.51 0.86", canToggleFF ? $"ClansUI toggleff {page}" : "");
            ...
            UI.CreateLabel(ref container, ClanUI, "", $"{msg("Союз между кланами: ",player.UserIDString)} {(alliesEnabled ? clan.clanAlliances.ToSentence() : msg("<color=red>Отключен</color>", player.UserIDString))}", 15, "0.27 0.735", "0.75 0.79", TextAnchor.MiddleLeft);
            UI.CreateButton(ref container, ClanUI, UI.Color(configData.UIColors.ButtonColor.HexColor, configData.UIColors.ButtonColor.Opacity), msg("Управление альянсами", player.UserIDString), 15, "0.76 0.735", "0.99 0.79", alliesEnabled ? "ClansUI alliances 0" : "");
            ...
            for (int i = (10 * page); i < clan.members.Count; i++)
            {
                ...
                AddClanMember(ref container, player.UserIDString, memberId, i - (10 * page), page, clan.IsOwner(memberId) ? msg("ВЛАДЕЛЕЦ", player.UserIDString) : clan.IsCouncil(memberId) ? msg("СОВЕТ", player.UserIDString) : clan.IsModerator(memberId) ? msg("МОДЕР", player.UserIDString) : "", isOwner, isPlayer);
            }
        ...
        void AddClanMember(ref CuiElementContainer container, string playerId, string memberId, int number, int page, string status, bool isOwner, bool isPlayer)
        {
            ...
            UI.CreatePanel(ref container, ClanUI, UI.Color(configData.UIColors.TextPanel.HexColor, configData.UIColors.TextPanel.Opacity), $"0.26 {yPos}", $"0.345 {yPos + 0.05f}");
            UI.CreateLabel(ref container, ClanUI, "", status, 14, $"0.265 {yPos}", $"0.34 {yPos + 0.05f}");
            for (int i = 0; i < configData.Commands.Count; i++)
            {
                float xPos = 0.35f + (0.075f * i);
            ...
            if (isOwner)
            {
                ...
                UI.CreateButton(ref container, ClanUI, UI.Color(configData.UIColors.ButtonColor.HexColor, configData.UIColors.ButtonColor.Opacity), msg("РАСПУСТИТЬ", playerId), 14, $"0.74 {yPos}", $"0.85 {yPos + 0.05f}", $"ClansUI predisband {page} {memberId}");
            }
            else if (isPlayer)
            {
                UI.CreateButton(ref container, ClanUI, UI.Color(configData.UIColors.ButtonColor.HexColor, configData.UIColors.ButtonColor.Opacity), msg("ВЫЙТИ", playerId), 14, $"0.855 {yPos}", $"0.925 {yPos + 0.05f}", $"ClansUI leave {page} {memberId}");
            ...
            {
                UI.CreateButton(ref container, ClanUI, UI.Color(configData.UIColors.ButtonColor.HexColor, configData.UIColors.ButtonColor.Opacity), msg("ПОВЫСИТЬ", playerId), 14, $"0.745 {yPos}", $"0.85 {yPos + 0.05f}", $"ClansUI promote {page} {memberId}");
                UI.CreateButton(ref container, ClanUI, UI.Color(configData.UIColors.ButtonColor.HexColor, configData.UIColors.ButtonColor.Opacity), msg("ПОНИЗИТЬ", playerId), 14, $"0.855 {yPos}", $"0.925 {yPos + 0.05f}", $"ClansUI demote {page} {memberId}");
                UI.CreateButton(ref container, ClanUI, UI.Color(configData.UIColors.ButtonColor.HexColor, configData.UIColors.ButtonColor.Opacity), msg("ВЫГНАТЬ", playerId), 14, $"0.93 {yPos}", $"0.99 {yPos + 0.05f}", $"ClansUI kick {page} {memberId}");
            }
        }
        ...
        void AllianceMenu(BasePlayer player, Clans.Clan clan, AllianceType type = AllianceType.Current, int page = 0)
        {
            ...
            UI.CreateLabel(ref container, ClanUI, "", msg("Администрирование кланом", player.UserIDString), 18, "0.38 0.94", "0.7 1", TextAnchor.MiddleLeft);
            UI.CreatePanel(ref container, ClanUI, UI.Color(configData.UIColors.Background.HexColor, configData.UIColors.Background.Opacity), "0.0001 0.0001", "0.9999 0.94");
            UI.CreateLabel(ref container, ClanUI, "", $"{msg("Название клана: ", player.UserIDString)} {clan.tag}", 17, "0.02 0.72", "0.25 0.93", TextAnchor.UpperLeft);
        ...
        void AddAlliance(ref CuiElementContainer container, AllianceType type, string playerId, string clanTag, int number, int page)
        {
            ...
            UI.CreateLabel(ref container, ClanUI, "", $"{msg("Участники", playerId)}: {Clans.findClan(clanTag).members.Count}", 14, $"0.27 {yPos}", $"0.391 {yPos + 0.05f}", TextAnchor.MiddleLeft);
            if (type == AllianceType.Current)
        ...
        protected override void LoadDefaultConfig()
        {
            var config = new ConfigData
            {
                ...
                {
                    Background = new UIColor { HexColor = "#000000", Opacity = 0.55f },
                    ButtonColor = new UIColor { HexColor = "#000000", Opacity = 0.9f },
                    TextPanel = new UIColor { HexColor = "#000000", Opacity = 0.6f },
                    TitlePanel = new UIColor { HexColor = "#000000", Opacity = 1 },
                    CloseButton = new UIColor { HexColor = "#ce422b", Opacity = 1 }
                },
                UISize = new UISize
                {                    
                    X_Position = 0.15f,
        ...
        Dictionary<string, string> Messages = new Dictionary<string, string>()
        {
            ...
            {"disabled", "<color=red>Отключен</color>" },
            {"leave", "ВЫЙТИ" },
            {"disband", "РАСПУСТИТЬ" },
