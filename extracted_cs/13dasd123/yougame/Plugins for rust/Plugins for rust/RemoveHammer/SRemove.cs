...
namespace Oxide.Plugins
{
    ...
    public class SRemove : RustPlugin
    {
            ...
            if (victim)
            {
                if (Removers.ContainsKey(victim.userID))
                {
                    ...
                    if (AmountEntities[victim.userID] > 0)
                        SendReply(victim, $"Вы заремували <color=#FF3232>{AmountEntities[victim.userID]}</color> объ.");
            ...
            if (arg.Connection?.player != null)
            {
                ...
                if (Removers.ContainsKey(player.userID))
                {
                    ...
                    if (AmountEntities[player.userID] > 0)
                        SendReply(player, $"Вы заремували <color=#FF3232>{AmountEntities[player.userID]}</color> объ.");
                    return;
                }
                goact(player, 30);
            }
        ...
        void RemoveGui(BasePlayer player, int sec)
        {
            ...
            {
                Text = { Text = $"<b>Режим удаления выключится через {checktime(player, sec) + 1} сек.</b>", FontSize = 16, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
