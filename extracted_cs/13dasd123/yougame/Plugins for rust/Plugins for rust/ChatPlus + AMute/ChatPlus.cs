...
{
    [Info("ChatPlus", "GrazyCat", "1.1.0")]
    [Description("Все в одном для вашего чата!")]
    public class ChatPlus : CovalencePlugin
    {
        ...
        public class Player
        {
            ...
            internal string nameWithClanTag => Group.GetPrimaryGroup(steamID).PlayerName.Replace("{Name}", name);
            ...
            {
                Player pl = null;
                Plugin.LoadData(ref pl, $"ChatPlus/Players/{id}");
            ...
            internal void Updated() => Plugin.SaveData(this, $"ChatPlus/Players/{steamID}");
        ...
        class Group
        {
            ...
            internal static void Updated() => Plugin.SaveData(Plugin.Groups, "ChatPlus/Groups");
        ...
        static ChatPlus Plugin = new ChatPlus();
...
#region Cached Variables
        ...
        void Loaded()
        {
            ...
            LoadData(ref Groups, "ChatPlus/Groups");
            LoadData(ref fixedDefaultGroup, "ChatPlus/DefaultGroupFixed");
            ...
            if (!fixedDefaultGroup)
            {
                ...
                if (playerGroup != null)
                {
                    ...
                    SaveData(Groups, "ChatPlus/Groups");
                }

                fixedDefaultGroup = true;
                SaveData(fixedDefaultGroup, "ChatPlus/DefaultGroupFixed");
            }
        ...
        new void LoadConfig()
        {
            SetConfig("Основные", "Включить теги игроков", false);
            SetConfig("Основные", "Обратный порядок названий", false);
            SetConfig("Основные", "Минимальные символы", 2);

            SetConfig("Антифлуд", "Включено", true);
            SetConfig("Антифлуд", "Время", 1.5f);

            SetConfig("Антимат", "Включено", false);
            SetConfig("Антимат", "Замена", "*");
            SetConfig("Антимат", "Индивидуальная замена", "Unicorn");
            SetConfig("Антимат", "Использовать пользовательскую замену", false);
            SetConfig("Антимат", "Фразы", new List<object> {
                "bitch",
                "faggot",
                "fuck"
            });
            ...
            General_AllowPlayerTagging = GetConfig(false, "Основные", "Включить теги игроков");
            General_ReverseTitleOrder = GetConfig(false, "Основные", "Обратный порядок названий");
            General_MinimalChars = GetConfig(2, "Основные", "Минимальные символы");

            AntiFlood_Enabled = GetConfig(true, "Антифлуд", "Включено");
            AntiFlood_Seconds = GetConfig(1.5f, "Антифлуд", "Время");

            WordFilter_Enabled = GetConfig(false, "Антимат", "Включено");
            WordFilter_Replacement = GetConfig("*", "Антимат", "Замена");
            WordFilter_UseCustomReplacement = GetConfig(false, "Антимат", "Использовать пользовательскую замену");
            WordFilter_CustomReplacement = GetConfig("Unicorn", "Антимат", "Индивидуальная замена");
        ...
        void LoadMessages()
        {
            ...
            {
                {"No Permission", "У вас нет прав на данную команду"},
                {"Group Created", "'{group}' создана"},
                {"Group Removed", "'{group}' удалена."},
                {"Group Already Exists", "'{group}' уже создана!"},
                {"Group Does Not Exist", "'{group}' не существует!"},
                {"Player Added To Group", "Игрок {player} добавлен в группу '{group}'."},
                {"Player Removed From Group", "Игрок {player} удален из группы '{group}'."},
                {"Invalid Type", "Не удалось преобразовать значение: {Message}"},
                {"Failed To Set Group Value", "Не удалось задать значение группы: {Ошибка}"},
                {"Muted Player", "{player} выдан мут!"},
                {"Time Muted Player", "{player} выдан мут на {time}!"},
                {"Unmuted Player", "{player} мут снят!"},
                {"Player Is Not Muted", "{player} мута нет."},
                {"Player Already Muted", "{player} уже выдан мут ."},
                {"You Are Muted", "У вас мут. Вы не можете общаться в чате."},
                {"You Are Time Muted", "Вам выдан мут на {время}. Вы не можете общаться в чате."},
                {"Ignoring Player", "Теперь вы игнорируете {player}."},
                {"No Longer Ignoring Player", "Вы больше не игнорируете {player}."},
                {"Not Ignoring Player", "Вы не игнорируете {player}."},
                {"Already Ignoring Player", "Вы уже игнорируете {player}."},
                {"Player Group List", "Группы {player}: {groups}"},
                {"Chatting Too Fast", "Э не так быстро ! Повтори через  {time} секунд"},
                {"Time Muted Global", "Все игроки были отключены на {time}!"},
                {"Muted Global", "Всем игрокам был выдан  мут!"},
                {"Unmuted Global", "Все игроки были размучены!"}
            }, this);
        }

        protected override void LoadDefaultConfig() => PrintWarning("Создаеться новая конфигурация...");
        ...
        [Command("muteglobal"), Permission("chatplus.mute")]
        ...
        [Command("unmuteglobal"), Permission("chatplus.mute")]
        ...
        [Command("mute"), Permission("chatplus.mute")]
        ...
        [Command("unmute"), Permission("chatplus.mute")]
        ...
        [Command("chat"), Permission("chatplus.admin")]
        void cmdChatPlus(IPlayer player, string cmd, string[] args)
        {
            if (args.Length == 0)
            {
        ...
        string API_GetFormatedMessage(string id, string message, bool console = false)
        {
            ...
            if (GetConfig(false, "Антимат", "Включено"))
        ...
        object OnUserChat(IPlayer player, string message)
        {
            ...
            var hookResult = plugins.CallHook("OnChatPlus", player, message);
...
public static class Extend
{
    ...
    public static BasePlayer GetBasePlayer(this ChatPlus.Player player) => BasePlayer.Find(player.steamID);
#endif
