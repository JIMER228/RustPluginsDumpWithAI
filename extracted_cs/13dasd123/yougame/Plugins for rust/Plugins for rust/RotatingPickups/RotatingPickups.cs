...
{
    [Info("RotatingPickups", "k1lly0u", "0.2.4", ResourceId = 0)]
    class RotatingPickups : RustPlugin
    {
        ...
        private static FieldInfo containerInv = typeof(DroppedItemContainer).GetField("inventory", (BindingFlags.Instance | BindingFlags.NonPublic));
        #endregion
        ...
        {            
            ...
            {
                if (!instance.configData.CollisionPickup) return;
                if (col?.gameObject?.layer != 17) return;
                var player = col.GetComponentInParent<BasePlayer>();
                if (player != null)
                {
                    if (instance.disabledPickup.Contains(player.userID)) return;
            ...
            {
                RaycastHit hit;
                if (Physics.Raycast(new Ray(entity.transform.position, Vector3.down), out hit, 500f, LayerMask.GetMask("Terrain", "Construction", "Deployed", "Water", "Debris")))
                {
                    var point = hit.point;
        ...
        {
            ItemContainer inventory = (ItemContainer)containerInv.GetValue(source);
            if (inventory != null)
            {                
        ...
        void cmdDropZone(BasePlayer player, string command, string[] args)
        {
            ...
            if (!ZoneManager)
            {
                SendReply(player, "ZoneManager is not installed! Unable to register zones for item drops");
                return;
            }
            if (args.Length == 1 && args[0].ToLower() == "list")
            {
                string zoneList = "--- Registered Zone IDs ---";
                foreach (var zoneId in configData.Zones)
                    zoneList += $"\n{zoneId}";
            ...
            if (args.Length != 2)
            {
                ...
                SendReply(player, "/dropzone add <zoneid> - Adds the zone ID to the list zones");
                SendReply(player, "/dropzone remove <zoneid> - Removes the zone ID from the list of zones");
                SendReply(player, "/dropzone list - List all registered zone IDs");
                return;
            }
            switch (args[0].ToLower())
            {
                case "add":
                    ...
                    else SendReply(player, "That is a invalid Zone ID");
                    return;
                case "remove":
                    ...
                    else SendReply(player, "The specified zone ID is not in the list of registered zones");
                    return;
                default:
                    break;
            }
        ...
        void ccmdDropZone(ConsoleSystem.Arg arg)
        {
            ...
            if (!ZoneManager)
            {
                SendReply(arg, "ZoneManager is not installed! Unable to register zones for item drops");
                return;
            }
            ...
            if (arg.Args.Length != 2)
            {
                ...
                SendReply(arg, "dropzone add <zoneid> - Adds the zone ID to the list zones");
                SendReply(arg, "dropzone remove <zoneid> - Removes the zone ID from the list of zones");
                SendReply(arg, "dropzone list - List all registered zone IDs");
                return;
            }
            switch (arg.Args[0].ToLower())
            {
                case "add":
                    ...
                    else SendReply(arg, "That is a invalid Zone ID");
                    return;
                case "remove":
                    ...
                    else SendReply(arg, "The specified zone ID is not in the list of registered zones");
                    return;
                default:
                    break;
            }
        ...
        {
            ["apDisabled"] = "You have disabled auto-pickup",
            ["apEnabled"] = "You have enabled auto-pickup"
        };
        #endregion
