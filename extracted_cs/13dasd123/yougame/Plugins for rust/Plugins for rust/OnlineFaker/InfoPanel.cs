...
namespace Oxide.Plugins
{
    ...
    public class InfoPanel : RustPlugin
    {
        ...
        void OnServerInitialized()
        {
            ...
            if (Settings.CheckPanelAvailability("OPlayers"))
            {
                timer.Repeat(15f, 0, RefreshOnlinePlayers);
            }
        ...
        private void RefreshOnlinePlayers()
        {
            foreach (var panel in PlayerPanels)
            {
                if (Settings.GetPanelSettingsValue("OPlayers", "Available", true) && panel.Value.ContainsKey("OPlayersText"))
                {
                    ...
                    var fakeonline = OnlineFaker?.Call("GetFakeOnline");
                    if(fakeonline != null & fakeonline is int)
                    {
                        panelText.Content = $"{(int)fakeonline}/{ConVar.Server.maxplayers}";
                    }else
                    {
