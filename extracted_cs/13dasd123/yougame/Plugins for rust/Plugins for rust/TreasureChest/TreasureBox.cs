...
{
    [Info("TreasureBox", "k1lly0u", "0.2.0", ResourceId = 0)]
    class TreasureBox : RustPlugin
    {
        ...
        void SpawnContainer(object spawnLoc = null)
        {
            if (BasePlayer.activePlayerList.Count >= configData.Options.MinimumPlayersRequired)
            {
                ...
                cupboard = GameManager.server.CreateEntity(cupboardEnt, location + (Vector3.down * 2), new Quaternion(), true);                
                cupboard.Spawn();
                cupboard.enableSaving = false;
        ...
        void CreateNewZone()
        {
            if (ZoneManager && configData.Options.ZoneSettings.CreateTemporaryZone)
            {
                ZoneManager.Call("CreateOrUpdateZone", "TreasureBox Zone", configData.Options.ZoneSettings.ZoneFlags, container.transform.position);
            }
        }
        void DestroyZone()
        {
        ...
        private void FillLootContainer(BaseEntity entity)
        {
            ...
            List<LootTables.LootItem> lootItems = new List<LootTables.LootItem>(configData.LootTable.LootItems);
        ...
        {            
            ...
            {                
                ...
                else timer.In(configData.Options.ZoneSettings.ZoneActiveAfter, DestroyZone);
                if (configData.LustyMap.ShowOnLustyMap)
                    RemoveMapMarker();
            }
            StartTimers();
        ...
        void cmdTH(BasePlayer player, string command, string[] args)
        {
            ...
            if (args.Length == 0)
            {
                ...
                SendReply(player, "<color=#ffd479>/th info</color><color=white> - Показывает время, оставшееся до следующего появления сундука сокровищ, и его место положения (если включена функция)</color>");
                if (player.IsAdmin)
                {
            ...
            else
            {
                switch (args[0].ToLower())
                {
                    ...
                    case "start":
                        ...
                        if (container != null)
                        {
                            SendReply(player, "Ивент уже проходит!");
                            return;
                        }
                        else
                        {
            ...
            else
            {
                ...
                {                   
                    case "start":
                        if (container != null)
                        {
                            SendReply(arg, "Ивент уже проходит!");
                            return;
                        }
                        else
                        {
                    ...
                    case "cancel":
                        if (container == null)
                        {
                            SendReply(arg, "В настоящее время ивент не проходит!");
                            return;
                        }
                        else
                        {
        ...
        void TryDownloadImage()
        {
            ...
            else
            {
                ...
                treasureIcon = FileStorage.server.Store(stream, FileStorage.Type.png, CommunityEntity.ServerInstance.net.ID).ToString();
                stream = null;
            }
        }
        #endregion
        ...
        class Options
        {
            ...
            {
                public bool CreateTemporaryZone { get; set; }
                public int ZoneActiveAfter { get; set; }
        ...
        protected override void LoadDefaultConfig()
        {
            ...
            {       
                ...
                LootTable = new LootTables
                {
                    ...
                    MaximumItems = 4,
                    MinimumItems = 1,
                    LootItems = new List<LootTables.LootItem>
                        {
                            new LootTables.LootItem {Shortname = "metal.refined", MaximumAmount = 100, MinimumAmount = 10 },
                            new LootTables.LootItem {Shortname = "explosive.timed", MaximumAmount = 2, MinimumAmount = 1 },
                            new LootTables.LootItem {Shortname = "grenade.f1", MaximumAmount = 3, MinimumAmount = 1 },
                            new LootTables.LootItem {Shortname = "supply.signal", MaximumAmount = 1, MinimumAmount = 1 },
                            new LootTables.LootItem {Shortname = "cctv.camera", MaximumAmount = 1, MinimumAmount = 1 },
                            new LootTables.LootItem {Shortname = "targeting.computer", MaximumAmount = 1, MinimumAmount = 1 },
                            new LootTables.LootItem {Shortname = "ammo.rifle", MaximumAmount = 60, MinimumAmount = 20 },
                            new LootTables.LootItem {Shortname = "ammo.pistol", MaximumAmount = 60, MinimumAmount = 20 }
                        }
                },
                Options = new Options
                {
                    ...
                    UISettings = new Options.UIOptions
                    {
                    ...
                    ZoneSettings = new Options.ZoneOptions
                    {
                        CreateTemporaryZone = true,
                        ZoneActiveAfter = 300,
                        ZoneFlags = new string[] { "notp", "true", "notrade", "true", "radius", "75" }
                    }
                },
