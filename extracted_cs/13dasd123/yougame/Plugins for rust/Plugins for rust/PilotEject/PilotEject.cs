...
{
    [Info("PilotEject", "redBDGR", "1.0.10")]
    public class PilotEject : RustPlugin
    {
        ...
        public static LayerMask collLayers = UnityEngine.LayerMask.GetMask(new string[] { "Construction", "Clutter", "Deployed", "Tree", "Terrain", "World", "Water", "Default" });
        ...
        void Loaded()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                ["Broken Heli Inbound"] = "Прибывает поврежденный вертолёт",
        ...
        void DoEvent(BaseNetworkable entity)
        {
            ...
            if (inboundMessage)
                rust.BroadcastChat(msg("Broken Heli Inbound"));
        ...
        {
            RaycastHit hitInfo;
            if (Physics.Raycast(sourcePos, Vector3.down, out hitInfo, collLayers))
            {
        ...
        void CallBrokenHeli()
        {
            var ent = GameManager.server.CreateEntity(heliprefab, new Vector3(Convert.ToSingle(ConVar.Server.worldsize), 50f, Convert.ToSingle(ConVar.Server.worldsize)), new Quaternion(), true);
            if (!ent) return;
            ent.Spawn();
