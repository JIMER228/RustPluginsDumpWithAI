...
{
    [Info("NoEscape", "RustPlugin.ru", "0.4.3", ResourceId = 1394)]
    ...
    class NoEscape : RustPlugin
    {
        ...
        protected override void LoadDefaultConfig()
        {
            ...
            Config["combatDuration"] = 300f; // 3 minutes
        ...
        void LoadMessages()
        {
            ...
            {
                {"Raid Blocked Message", "Вы не можете делать это во время рейда. До окончания {time}"},
                {"Combat Blocked Message", "Вы не можете делать это во время рейда. До окончания {time}"},
                {"Raid Block Complete", "Время блокировки истекло!"},
                {"Combat Block Complete", "Время блокировки истекло!"},
                {"Raid Block Notifier", "Строение разрушено, активирован рейд блок! Некоторые функции временно недоступны."},
                {"Combat Block Notifier", "Строение разрушено, активирован рейд блок! Некоторые функции временно недоступны."},
                {"Unit Seconds", "секунд(ы)"},
                {"Unit Minutes", "минут(ы)"},
                {"Prefix", ""}
            }, this);
        }
