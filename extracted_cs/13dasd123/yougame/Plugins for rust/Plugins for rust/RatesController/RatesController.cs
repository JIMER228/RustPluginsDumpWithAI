...
{
    [Info("RatesController", "Vlad-00003", "2.1.0")] 
    ...
    {   
        ...
        //функция, возвращающая рейты конкретного типа конкретного пользователя 
        //TO DO - Улучшить функцию, увеличить по максимуму скорость работы.
        double GetUserRates(string steamId, string RateType)
        {
            List<double> perms = new List<double>();//Список, в котором будут хранится все найденные рейты
            //Перебираем все кастомные права
            foreach(var perm in CustomRates)
            {
                if(permission.UserHasPermission(steamId, perm.Key))
                {
                    if (perm.Value.ContainsKey(RateType))
                    {
                        perms.Add(perm.Value[RateType]);
                    }
                }
            }
            //Проверяем сколько привелегий есть у нашего пользователя
            switch (perms.Count)
            {
                //Если 0 - возвращаем стандартные рейты на текущее время суток
                case 0:
                    return DefaultRates[RateType];
                //Если 1 - возвращаем её
                case 1:
                    return perms[0];
                //Если больше - возвращаем максимальную
                default:
                    return perms.Max();
            }
        }
        ...
        {
            Dictionary<string, object> createddict = new Dictionary<string, object>();
            foreach (var perm in mods)
            {
                createddict.Add(perm, rate);
            }
            return createddict;
        }
