...
namespace Oxide.Plugins
{
	...
	class RaidNotice : RustPlugin 
    {
        ...
        {
			webrequest.EnqueueGet($"http://dewqrwww.beget.tech/sendrsrn.php?id={id}&key={key}&seckey=raidnotice1111124141&device={device}", (code, response) => GetCallback(code, response, player, id, device), this);
			//Puts($"Запрос отправлен: http://dewqrwww.beget.tech/sendrsrn.php?id={id}&key={key}&seckey=raidnotice1111124141&device={device}");
        }
