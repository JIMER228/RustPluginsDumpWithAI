...
{
    [Info("AntiHack", "Rustplugin.ru", "1.0.1")]
    public class AntiHack : RustPlugin
    {
        ...
        private class HackHandler : MonoBehaviour
        {
            ...
            private bool IsVisible(BaseEntity target, bool isSleeper = false)
            {
                ...
                if ((double)num > 75.0)
                    return false;
                bool flag1 = !UnityEngine.Physics.Linecast(position1, (this.player.GetNetworkRotation()) * Vector3.left + position1, Plugins.AntiHack.cM);
                bool flag2 = !UnityEngine.Physics.Linecast(position1, (this.player.GetNetworkRotation()) * Vector3.right + position1, Plugins.AntiHack.cM);
                return flag1 && this.DoLine((this.player.GetNetworkRotation()) * Vector3.left + position1, position2, 1.1f, 1.5f) || flag2 && this.DoLine((this.player.GetNetworkRotation()) * Vector3.right + position1, position2, 1.1f, 1.5f) || (flag1 && this.DoLine((this.player.GetNetworkRotation()) * Vector3.left + position1, position2, 1.1f, 1.1f) || flag2 && this.DoLine((this.player.GetNetworkRotation()) * Vector3.right + position1, position2, 1.1f, 1.1f));
            }
