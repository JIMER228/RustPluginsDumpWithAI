        ...
        private void OnServerInitialized()
        {
            ...
            if (Settings.MonthID != now.Month)
            {
                ...
                for (int i = 1; i <= DateTime.DaysInMonth(now.Year, now.Month); i++)
                {
                    ...
                    int curWipe = date.DayOfWeek == DayOfWeek.Monday ? currentFull ? 2 : 1 : 0;
            ...
            string nextWipeText = $"Следующий вайп <b>{(nextWipeDate.ToString().Length == 1 ? "0" + nextWipeDate.ToString() : nextWipeDateinfo.Key.ToString())}.{(DateTime.Now.Month.ToString().Length == 1 ? "0" + (nextWipeDateinfo.Key < (int) DateTime.Now.Day ? ((int) DateTime.Now.Month + 1).ToString() : DateTime.Now.Month.ToString()) : DateTime.Now.Month.ToString())}</b> в <b>13:00</b>";
             
            webrequest.Enqueue($"http://185.200.242.130:2008/wipe/{Settings.ServerName}/{(nextWipeDate.ToString().Length == 1 ? "0" + nextWipeDate.ToString() : nextWipeDateinfo.Key.ToString())}.{(DateTime.Now.Month.ToString().Length == 1 ? "0" + (nextWipeDateinfo.Key < (int) DateTime.Now.Day ? ((int) DateTime.Now.Month + 1).ToString() : DateTime.Now.Month.ToString()) : DateTime.Now.Month.ToString())}.{DateTime.Now.Year.ToString().Substring(0, 2)}/{(nextWipeDateinfo.Value == 1 ? "MAP" : "GLOBAL")}", "", (i, s) => { }, this);
