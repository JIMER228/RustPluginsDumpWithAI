            ...
            public static void DrawPrivileges(BasePlayer player)
            {
                ...
                foreach (var point in fetch.Where(p => Settings.GroupImages.ContainsKey(p.Key)))
                {
                    ...
                    container.Add(new CuiElement
                    {
                        ...
                        Components =
                        {
                            new CuiRawImageComponent {Png = (string) _.ImageLibrary.Call("GetImage", "H.Image"), Material = "", Color = TimeSpan.FromSeconds(point.Value).Days < 2 ? "1 0.5 0.5 0.5" : "1 1 1 0.3"},
                    ...
                    container.Add(new CuiLabel
                    {
                        ...
                        Text = {Text = $"ОСТАЛОСЬ: {GetTimeFromSecs(point.Value)}", Align = TextAnchor.MiddleLeft, Font = "robotocondensed-regular.ttf", FontSize = 14, Color = "0.81 0.77 0.74 1"}
                    }, PrivPoint + point.Key);
