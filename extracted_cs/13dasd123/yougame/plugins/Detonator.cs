...
namespace Oxide.Plugins
{
    ...
    public class Detonator : RustPlugin 
    {
        ...
        private void CmdConsoleHandler(ConsoleSystem.Arg arg)
        {
            ...
            switch (arg.Args[0].ToLower())
            {
                case "sethome":
                {
                    ...
                    string frequency = Oxide.Core.Random.Range(10, 100).ToString();
                    while (obj.Homes.Any(p => p.Value.Frequency == frequency))
                        frequency = Oxide.Core.Random.Range(10, 100).ToString();
        ...
        {
            player.ChatMessage(info.HitEntity.net.ID.ToString());
        }
...
//                });
            ...
            container.Add(new CuiLabel
            {
                ...
                if (cHome.Value != null) subText = "ТИП: " + (cHome.Value.IsBed() ? "КРОВАТЬ" : (cHome.Value.IsBag() ? "СПАЛЬНИК" : "ПОЛОТЕНЦЕ")) + $" — <b>{GetGrid(cHome.Value.Entity.transform.position)}</b>";
