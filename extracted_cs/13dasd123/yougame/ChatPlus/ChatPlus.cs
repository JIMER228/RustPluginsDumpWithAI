...
{
    [Info("ChatPlus", "Vlad-00003, Update Robin Hood", "1.4.0")]
    public class ChatPlus : CovalencePlugin
    {
        ...
        object OnUserChat(IPlayer sender, string message)
        {
            if (GlobalMute)
            {
            ...
            {
                LogToFile("Chat", $"[ADMIN MODE] -> [{DateTime.Now.ToShortDateString()} | {DateTime.Now.ToShortTimeString()}] -> Администратор {sender.Name} написал сообщение: {message}", this);
                bool outer;
            ...
            {
                LogToFile("Chat", $"[MODERATOR MODE] -> [{DateTime.Now.ToShortDateString()} | {DateTime.Now.ToShortTimeString()}] -> Модератор: {sender.Name} написал сообщение: {message}", this);
                bool outer;
                var cens = CensorBadWords(message, out outer);
            ...
            Log(LogType.Chat, $"{sender.Name}({sender.Id}): {message}");
			...
			LogToFile("Chat", $"[{DateTime.Now.ToShortDateString()} | {DateTime.Now.ToShortTimeString()}] -> Игрок: {sender.Name} написал сообщение: {message}", this);
            return false;
        }
        ...
        #region Broadcasting
        void SendChat(string name, string message, string censorMessage = "", string userId = "0")
        {
            ...
            {
                string msg = GetPlayerData(player).Censor ? censorMessage : message;
                player.Message(string.IsNullOrEmpty(name) ? $"{msg}" : $"{name}: {msg}");
            }
#endif
        }
        ...
        private void RustChat(IPlayer player, string langkey, string avatar, params object[] args)
        {
            ...
            player.Command("chat.add", PmAvatar() ? avatar : "0", covalence.FormatText(msg));
        }
#endif
...
#endif
            LogToFile("Chat", string.Format(GetMsg("PM.LOG"), $"[{DateTime.Now.ToShortDateString()} | {DateTime.Now.ToShortTimeString()}] -> Игрок {player.Name} отправил сообщение игроку {receiver.Name}: {message}"), this);
        }
        [Command("r")]
        void cmdChatR(IPlayer player, string cmd, string[] args)
        {
            ...
            if (recieverUserId== "server_console")
            {
                ...
                LogToFile("Chat", string.Format(GetMsg("PM.LOG"), $"[{DateTime.Now.ToShortDateString()} | {DateTime.Now.ToShortTimeString()}] -> Игрок {player.Name} Server console: {message}"), this);
                return;
            }
...
#endif
            LogToFile("Chat", string.Format(GetMsg("PM.LOG"), $"[{DateTime.Now.ToShortDateString()} | {DateTime.Now.ToShortTimeString()}] -> Игрок {player.Name} ответил игроку: {receiver.Name} на сообщение: {message}"), this);
        }
        #endregion
