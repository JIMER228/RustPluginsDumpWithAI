...
{
    [Info("Clans", "FuJiCuRa", "2.14.8", ResourceId = 14)]
    ...
    public class Clans : RustPlugin
    {
        ...
        [HookMethod("GetClanOf")]
        private string GetClanOf(BasePlayer player) => GetClanOf(player?.UserIDString);
        ...
        private string GetClanOf(string playerId)
        {
            ...
            Clan clan = findClanByUser(playerId);
            if (clan == null)
                return null;
        ...
        {
            Clan clanOwner = findClanByUser(ownerId);
            if (clanOwner == null)
                return null;

            Clan clanFriend = findClanByUser(playerId);
            if (clanFriend == null)
                return null;
        ...
        private object IsModerator(ulong playerId) => IsModerator(playerId.ToString());

        [HookMethod("IsModerator")]
        private object IsModerator(string playerId)
        {
            Clan clan = findClanByUser(playerId);
            if (clan == null)
                return null;

            if (setHomeOwner && clan.IsOwner(playerId) || setHomeModerator && (clan.IsModerator(playerId) || clan.IsCouncil(playerId)) || setHomeMember)
                return true;

            return false;
        }

        [HookMethod("GetClanAlliances")]
        ...
        private List<string> GetClanAlliances(string playerId)
        {
            ...
            return new List<string>(clan.clanAlliances);
        }
