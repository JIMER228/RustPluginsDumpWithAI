...
{
/*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ 
	[Info("IQPermissions", "https://discord.gg/dNGbxafuJn", "1.3.2")]
	...
	public class IQPermissions : RustPlugin
	{
		...
		/// <summary>
		/// Обновление 1.3.2
		/// - Убрано форматирование сообщений с временем в лэнге (из-за некорректного отображения тэгов в консоли)
		/// - Мелкие корректировки
		/// - Добавлена возможность установить время уведомления об окончании привилегии в конфигурацию
		/// </summary>
	    ...
	    private class InterfaceBuilder
        {
            ...
            public static void AddInterface(String name, String json)
            {
                ...
                {
                    _.PrintError($"Error #32212535! Tried to add existing cui elements! -> {name}");
                    return;
                }
            			...
            			private void BuildingPlayer_PopUp()
			{
				...
				container.Add(new CuiElement
				{
					...
					Components = {
						...
						new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-84.9 -2.062125", OffsetMax = "125.33 19.462125" } 
					}
				});
        ...
        private IEnumerator JoinedAlertPlayer(BasePlayer player)
        {
	        if (config.AlertConfigurations.UseAlertedPermission)
	        {
			                 ...
			                 Where(userPermission => ((userPermission.Value[player.userID] - DateTime.Now).Days + 1) <= config.AlertConfigurations.DayAlerted))
		        {
			        ...
			        yield return CoroutineEx.waitForSeconds(config.AlertConfigurations.TimeAlert);
		        }
	        }
