...
{
	[Info("SkinBox", "Fujikura", "1.8.3", ResourceId=17)] class SkinBox: RustPlugin 
	{
		...
		static Dictionary<string, object>missingWorkshopNames() 
		{
			var dp=new Dictionary<string, object>();
			dp.Add("rug", "Rug");
			dp.Add("chair", "Chair");
			dp.Add("rug.bear", "Bearskin Rug");
			dp.Add("bandana", "Bandana");
			dp.Add("table", "Table");
			return dp;
		}
		...
		Dictionary<string, object>missingSkinNames=new Dictionary<string, object>();
		bool enableCooldown;
		int cooldownBox;
		bool cooldownOverrideAdmin;
		bool activateAfterSkinTaken;
		bool useKeyBindLogin;
		string keyToBind;
		...
		void LoadVariables() {
			...
			missingSkinNames=(Dictionary<string, object>)GetConfig("AvailableSkins", "MissingSkinNames", missingWorkshopNames());
			...
			hideQuickSort=Convert.ToBoolean(GetConfig("Settings", "hideQuickSort", false));
			boxUseDistance=Convert.ToSingle(GetConfig("Settings", "boxUseDistance", 10.0));
			...
			useKeyBindLogin=Convert.ToBoolean(GetConfig("KeyBind", "useKeyBindLogin", false));
			keyToBind=Convert.ToString(GetConfig("KeyBind", "keyToBind", "c"));
		...
		void Loaded() {
			...
			skinsCache=new Dictionary<string, List<ulong>>();
		...
		void OnServerInit() {
			foreach (var itemtype in Rust.Workshop.Approved.All.ToList()) {
				...
				if (!ItemNameToName.ContainsKey(itemtype.Skinnable.ItemName.ToLower())) ItemNameToName.Add(itemtype.Skinnable.ItemName.ToLower(), itemtype.Skinnable.Name.ToLower());
			}
			foreach (var skin in missingSkinNames) {
				...
				if (!NameToItemName.ContainsKey(workshopname)) NameToItemName.Add(workshopname, itemname);
				if (!ItemNameToName.ContainsKey(itemname)) ItemNameToName.Add(itemname, workshopname);
			}
		...
		void Unload() {
			_unloaded=true;
			ServerMgr.Instance.StopCoroutine("GetRankedWorkshopSkins");
			ServerMgr.Instance.StopCoroutine("GetAcceptedWorkshopSkins");
			ServerMgr.Instance.StopCoroutine("SearchWorkshopSkin");
		...
		void GetItemSkins() {
			...
			if (showLoadedSkinCounts && useInbuiltSkins) {
				Puts($"Loaded {countInbuilt} inbuilt skins");
				Interface.CallHook("OnSkinCacheUpdate", skinsCache);
			}
			if (useApprovedSkins) {
				int countApproved=0;
				foreach (var shopskin in Rust.Workshop.Approved.All.Where(skin=> skin.Skinnable.ItemName !=null)) {
					if (!skinsCache.ContainsKey(shopskin.Skinnable.ItemName)) skinsCache[shopskin.Skinnable.ItemName]=new List<ulong>();
				...
				if (showLoadedSkinCounts) Puts($"Loaded {countApproved} inbuilt approved Workshop skins");
			}
				...
				foreach (var manualskins in manualAddedSkins) {
					...
					foreach (var fileid in fileids) {
						if (!skinsCache[shortname].Contains(fileid)) {
							skinsCache[shortname].Add(fileid);
							countManual++;
						}
					}
				}
				if (showLoadedSkinCounts && countManual > 0) {
					Puts($"Loaded {countManual} manual added skins");
					Interface.CallHook("OnSkinCacheUpdate", skinsCache);
				}
			}
			skinsLoaded=true;
			if (useApprovedSkins) ServerMgr.Instance.StartCoroutine(GetAcceptedWorkshopSkins());
			else if (useWebskinsRankedByTrend) ServerMgr.Instance.StartCoroutine(GetRankedWorkshopSkins());
		}
		void RunQuery(Facepunch.Steamworks.Workshop.Query query) {
			FieldInfo Callback=typeof(Facepunch.Steamworks.Workshop.Query).GetField("Callback", (BindingFlags.Instance | BindingFlags.NonPublic));
			FieldInfo _resultSkip=typeof(Facepunch.Steamworks.Workshop.Query).GetField("_resultSkip", (BindingFlags.Instance | BindingFlags.NonPublic));
			FieldInfo _resultsRemain=typeof(Facepunch.Steamworks.Workshop.Query).GetField("_resultsRemain", (BindingFlags.Instance | BindingFlags.NonPublic));
			FieldInfo _resultPage=typeof(Facepunch.Steamworks.Workshop.Query).GetField("_resultPage", (BindingFlags.Instance | BindingFlags.NonPublic));
			FieldInfo _results=typeof(Facepunch.Steamworks.Workshop.Query).GetField("_results", (BindingFlags.Instance | BindingFlags.NonPublic));
			MethodInfo runInternal=typeof(Facepunch.Steamworks.Workshop.Query).GetMethod("RunInternal", BindingFlags.NonPublic | BindingFlags.Instance);
			bool flag=Callback.GetValue(query) !=null;
			if (!flag) {
				bool flag2=query.Page <=0;
				if (flag2) {
					PrintWarning("Page should be 1 or above");
					return;
				}
				int num=(query.Page - 1) * query.PerPage;
				query.TotalResults=0;
				_resultSkip.SetValue(query, num % 50);
				_resultsRemain.SetValue(query, query.PerPage);
				_resultPage.SetValue(query, (int)Math.Floor((double)((float)num / 50f)));
				_results.SetValue(query, new List<Workshop.Item>());
				runInternal.Invoke(query, null);
			}
		}
		public IEnumerator GetAcceptedWorkshopSkins(bool timed=false) {
			yield return new WaitForSeconds ( 2);
			var workshopQuery=Global.SteamServer.Workshop.CreateQuery();
			workshopQuery.Page=1;
			workshopQuery.PerPage=500;
			workshopQuery.RequireTags.Add("Version3");
			workshopQuery.RequireTags.Add("Skin");
			workshopQuery.RequireAllTags=true;
			workshopQuery.Order=Workshop.Order.AcceptedForGameRankedByAcceptanceDate;
			if (showLoadedSkinCounts && !timed) Puts($"Checking Workshop for new accepted skins");
			RunQuery(workshopQuery);
			...
			foreach (var item in workshopQuery.Items) {
				foreach (var tag in item.Tags.Where(t=> !t.Contains("Version") && !t.Contains("Skin"))) {
					if (NameToItemName.ContainsKey(tag)) {
						string shortname=NameToItemName[tag];
						if (!skinsCache[shortname].Contains(item.Id)) {
							skinsCache[shortname].Add(item.Id);
							countQueried++;
						}
					}
				}
			...
			workshopQuery.Dispose();
			if (showLoadedSkinCounts && countQueried > 0) {
				Puts($"Loaded {countQueried} new accepted Workshop skins");
				Interface.CallHook("OnSkinCacheUpdate", skinsCache);
			}
			else if (showLoadedSkinCounts && countQueried==0 && !timed) Puts($"No recently accepted Workshop skins found");
			if (useWebskinsRankedByTrend) ServerMgr.Instance.StartCoroutine(GetRankedWorkshopSkins());
		}
		public IEnumerator GetRankedWorkshopSkins() {
			var workshopQuery=Global.SteamServer.Workshop.CreateQuery();
			workshopQuery.Page=1;
			...
			workshopQuery.RequireTags.Add("Skin");
			workshopQuery.RequireAllTags=true;
			workshopQuery.Order=Workshop.Order.RankedByTrend;
			workshopQuery.RankedByTrendDays=usedRankedByTrendDays;
			if (showLoadedSkinCounts) Puts($"Calling Workshop to fill up the SkinCache. This can take a few moments...");
			RunQuery(workshopQuery);
			yield return new WaitWhile(new System.Func<bool>(()=> workshopQuery.IsRunning));
			if (_unloaded) {
				workshopQuery.Dispose();
			...
			foreach (var item in workshopQuery.Items) {
				foreach (var tag in item.Tags.Where(t=> !t.Contains("Version") && !t.Contains("Skin"))) {
					if (NameToItemName.ContainsKey(tag)) {
						string shortname=NameToItemName[tag];
						if (!skinsCache[shortname].Contains(item.Id) && skinsCache[shortname].Count < 31) {
							skinsCache[shortname].Add(item.Id);
							countQueried++;
						}
					}
			...
			workshopQuery.Dispose();
			if (showLoadedSkinCounts && countQueried > 0) {
				Puts($"Filled up SkinCache with {countQueried} new Workshop skins");
				Interface.CallHook("OnSkinCacheUpdate", skinsCache);
			}
		}
		[ConsoleCommand("skinbox.addexcluded")] void consoleAddExcluded(ConsoleSystem.Arg arg) {
			...
			i++) {
				...
				if (!ulong.TryParse(arg.Args[i], out fileId)) {
					...
					if (arg.Args[i].Length !=9) {
						SendReply(arg, $"Ignored '{arg.Args[i]}' as of not 9-Digits");
						continue;
					}
					fileIds.Add(fileId);
				}
		...
		[ConsoleCommand("skinbox.removeexcluded")] void consoleRemoveExcluded(ConsoleSystem.Arg arg) {
			if (arg.Connection !=null && arg.Connection.authLevel < 2) return;
			if (arg.Args==null || arg.Args.Length < 1) {
				SendReply(arg, "You need to type in one or more Workshop FileId's");
				return;
			...
			if (countRemoved > 0) {
				Config["SkinsExcluded", "SkinList"]=manualAddedSkins;
				Config.Save();
				Puts($"Removed {countRemoved} skins from exclusion");
			}
		}
		[ConsoleCommand("skinbox.addskin")] void consoleAddSkin(ConsoleSystem.Arg arg) {
			if (arg.Connection !=null && arg.Connection.authLevel < 2) return;
			if (arg.Args==null || arg.Args.Length < 1) {
				SendReply(arg, "You need to type in one or more Workshop FileId's");
				return;
			...
			ServerMgr.Instance.StartCoroutine(SearchWorkshopSkin(fileIds));
		}
		[ConsoleCommand("skinbox.removeskin")] void consoleRemoveSkin(ConsoleSystem.Arg arg) {
			if (arg.Connection !=null && arg.Connection.authLevel < 2) return;
			if (arg.Args==null || arg.Args.Length < 1) {
				SendReply(arg, "You need to type in one or more Workshop FileId's");
				return;
			...
			if (doSave) {
				...
				Config.Save();
				Puts($"Removed {removed} FileId's");
			}
		}
		public IEnumerator SearchWorkshopSkin(List<ulong> fileIds) {
			var workshopQuery=Global.SteamServer.Workshop.CreateQuery();
			workshopQuery.Page=1;
			workshopQuery.PerPage=1;
			workshopQuery.RequireTags.Add("Version3");
			workshopQuery.FileId=new List<ulong>(fileIds);
			Puts($"Calling Workshop for '{fileIds.Count}' FileId's");
			RunQuery(workshopQuery);
			yield return new WaitWhile(new System.Func<bool>(()=> workshopQuery.IsRunning));
			bool doSave=false;
			foreach ( var item in workshopQuery.Items) {
				bool matchedAny=false;
				foreach (var tag in item.Tags.Where(t=> !t.Contains("Version") && !t.Contains("Skin") && !t.Contains("Weapon") && !t.Contains("Playerwearable") && !t.Contains("Deployable"))) {
					if (NameToItemName.ContainsKey(tag)) {
						string shortname=NameToItemName[tag];
						if (manualAddedSkins.ContainsKey(shortname)) {
							if (manualAddedSkins[shortname].Contains(item.Id)) {
								Puts($"The FileId '{item.Id}' was already added");
								matchedAny=true;
								continue;
							}
						}
						if (skinsCache.ContainsKey(shortname)) if ((skinsCache[shortname] as List<ulong>).Contains(item.Id)) {
							Puts($"The FileId '{item.Id}' belongs already to the default loaded skins");
							matchedAny=true;
							continue;
						}
						...
						if (!skinsCache.ContainsKey(shortname)) skinsCache.Add(shortname, new List<ulong>());
						skinsCache[shortname].Add(item.Id);
						Puts($"Added '{item.Title}' to the manual skins list for '{shortname}'");
						matchedAny=true;
						doSave=true;
					}
				}
				if (!matchedAny) Puts($"No matching item shortname found to assign the skin '{item.Id}'");
			}
			if (doSave) {
				Config["SkinsAdded", "SkinList"]=manualAddedSkins;
				Config.Save();
		...
		void OnPlayerInit(BasePlayer player) {
			...
			if(useKeyBindLogin) {
				player.Command($"bind {keyToBind} skinbox.open");
			}
			if (player==null || !forceClientPreload || (checkedPlayers.Contains(player.userID) && !forceClientAlways)) return;
		...
		void InitPreload(BasePlayer player) {
			...
			foreach ( var skinCache in skinsCache) {
				if ((skinCache.Value as List<ulong>).Count <=1) continue;
				foreach (var skin in (List<ulong>)skinCache.Value) {
					if (skin==0) continue;
					Item item=ItemManager.Create(ItemManager.FindItemDefinition(skinCache.Key), 1, skin);
		...
		[ConsoleCommand("skinbox.open")] void consoleSkinboxOpen(ConsoleSystem.Arg arg) {
			if (!useKeyBindLogin) return;
			if(arg !=null && arg.Connection !=null && arg.Connection.player !=null) {
				...
				if (enableCooldown && !(cooldownOverrideAdmin && player.IsAdmin)) {
		...
		Boolean CheckOpenBalance(BasePlayer player) {
			if (!activePointSystem || costBoxOpen <=0 || (player.IsAdmin && costExcludeAdmins)) return true;
			object getMoney=null;
			if (activeServerRewards) getMoney=ServerRewards?.Call("CheckPoints", player.userID);
			if (activeEconomics) getMoney=Economics?.CallHook("GetPlayerMoney", player.userID);
			int playerMoney=0;
			...
			if (activeServerRewards) ServerRewards?.Call("TakePoints", player.userID, costBoxOpen);
			if (activeEconomics) Economics?.CallHook("Withdraw", player.userID, Convert.ToDouble(costBoxOpen));
			return true;
		}
		Boolean CheckSkinBalance(BasePlayer player, Item item) {
			if (!activePointSystem) return true;
			object getMoney=null;
			if (activeServerRewards) getMoney=ServerRewards?.Call("CheckPoints", player.userID);
			if (activeEconomics) getMoney=Economics?.CallHook("GetPlayerMoney", player.userID);
			int playerMoney=0;
			...
			switch (item.info.category.ToString()) {
				case "Weapon": case "Tool": if (costWeapon <=0 || playerMoney > costWeapon) hasBalance=true;
				getCost=costWeapon;
				break;
				case "Attire": if (costPlayerwearable <=0 || playerMoney > costPlayerwearable) hasBalance=true;
				getCost=costPlayerwearable;
				break;
				case "Items": case "Construction": if (costDeployable <=0 || playerMoney > costDeployable) hasBalance=true;
				getCost=costDeployable;
				break;
				default: hasBalance=true;
				break;
			}
			if (!hasBalance) {
		...
		Boolean WithdrawBalance(BasePlayer player, Item item) {
			...
			int getCost=0;
			switch (item.info.category.ToString()) {
				case "Weapon": case "Tool": getCost=costWeapon;
				break;
				case "Attire": getCost=costPlayerwearable;
				break;
				case "Items": case "Construction": getCost=costDeployable;
				break;
				default: break;
			}
			bool hadMoney=false;
			if (activeServerRewards && (bool)ServerRewards?.Call("TakePoints", player.userID, getCost)) hadMoney=true;
			if (activeEconomics && (bool)Economics?.CallHook("Withdraw", player.userID, Convert.ToDouble(getCost))) hadMoney=true;
			if (!hadMoney) {
		...
		sealed class SkinBoxHandler: MonoBehaviour {
			...
			void Awake() {
				...
				isCleaning=false;
				isEmptied=false;
				loot=GetComponent<StorageContainer>().inventory;
		...
		Boolean CheckDistance(BasePlayer player) {
			...
			foreach (var id in activeBoxUsers) {
		...
		void OpenSkinBox(BasePlayer player) {
			var skinBox=GameManager.server.CreateEntity("assets/prefabs/deployable/large wood storage/box.wooden.large.prefab", player.transform.position - new Vector3(UnityEngine.Random.Range(-5f, 5f), 200, UnityEngine.Random.Range(-5f, 5f)));
			skinBox.UpdateNetworkGroup();
			skinBox.SendNetworkUpdateImmediate();
			skinBox.globalBroadcast=true;
			(skinBox as BaseNetworkable).limitNetworking=true;
			skinBox.Spawn();
			skinBox.gameObject.AddComponent<SkinBoxHandler>().looter=player;
		...
		{
			FieldInfo _PositionChecks=typeof(PlayerLoot).GetField("PositionChecks", (BindingFlags.Instance | BindingFlags.NonPublic));
			MethodInfo _MarkDirty=typeof(PlayerLoot).GetMethod("MarkDirty", BindingFlags.NonPublic | BindingFlags.Instance);
			loot.Clear();
			if (!targetEntity) return;			
			_PositionChecks.SetValue(loot, false);
			loot.entitySource=targetEntity;
			loot.itemSource=null;
			_MarkDirty.Invoke(loot, null);
		}
		...
		object CanAcceptItem(ItemContainer container, Item item) {
			if (container==null || item==null || container.entityOwner==null || container.entityOwner.GetComponent<SkinBoxHandler>()==null || container.entityOwner !=container.entityOwner.GetComponent<SkinBoxHandler>().entityOwner || container.entityOwner.GetComponent<SkinBoxHandler>().isCreating) return null;
			if (container.entityOwner.GetComponent<SkinBoxHandler>().isBlocked || (enableCustomPerms && !CheckItemPerms(container.entityOwner.GetComponent<SkinBoxHandler>().looter, item)) || item.amount > 1 || item.isBroken || !skinsCache.ContainsKey(item.info.shortname) || (skinsCache[item.info.shortname] as List <ulong>).Count <=1 || !CheckSkinBalance(container.entityOwner.GetComponent<SkinBoxHandler>().looter, item)) return ItemContainer.CanAcceptResult.CannotAccept;
			return null;
		}
		bool CheckItemPerms(BasePlayer player, Item item) {
			string category=item.info.category.ToString();
			switch (category) {
				case "Weapon": if (permission.UserHasPermission(player.UserIDString, permCustomWeapon)) return true;
				break;
				case "Tool": if (permission.UserHasPermission(player.UserIDString, permCustomWeapon)) return true;
				break;
				case "Attire": if (permission.UserHasPermission(player.UserIDString, permCustomPlayerwearable)) return true;
				break;
				case "Items": if (permission.UserHasPermission(player.UserIDString, permCustomDeployable)) return true;
				break;
				case "Construction": if (permission.UserHasPermission(player.UserIDString, permCustomDeployable)) return true;
				break;
				default: return true;
			}
			return false;
		...
		void OnItemAddedToContainer(ItemContainer container, Item item) {
			if (container==null || item==null || container.entityOwner==null || container.entityOwner.GetComponent<SkinBoxHandler>()==null || container.entityOwner !=container.entityOwner.GetComponent<SkinBoxHandler>().entityOwner || container.entityOwner.GetComponent<SkinBoxHandler>().isCreating) return;
			...
			List <ulong>itemSkins=skinsCache[shortname] as List <ulong>;
			if (isWeapon) {
			...
			foreach (var skin in itemSkins) {
				if (container.itemList.Count==30) break;
				if (excludedSkins.Contains(skin) && lootHandler.looter.net.connection.authLevel < exludedSkinsAuthLevel) continue;
		...
		void OnItemRemovedFromContainer(ItemContainer container, Item item) {
			if (container==null || item==null || container.entityOwner==null || container.entityOwner.GetComponent<SkinBoxHandler>()==null || container.entityOwner !=container.entityOwner.GetComponent<SkinBoxHandler>().entityOwner || container.entityOwner.GetComponent<SkinBoxHandler>().isCreating || container.entityOwner.GetComponent<SkinBoxHandler>().isCleaning) return;
			var loothandler=container.entityOwner.GetComponent<SkinBoxHandler>();
			...
			if (enableCooldown && activateAfterSkinTaken && !(cooldownOverrideAdmin && loothandler.looter.IsAdmin) && item.skin !=loothandler.skinId) {
