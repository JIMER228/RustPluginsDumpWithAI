...
{
    [Info("Timed Permissions", "LaserHydra", "1.3.3", ResourceId = 1926)]
    [Description("Позволяет выдавать игрокам разрешения или группы на необходимое количество времени")]
    internal class TimedPermissions : CovalencePlugin
    {
		#region Variables
		private const string PermUse = "timedpermissions.use";
        ...
        {
			if (!permission.UserHasPermission(player.Id, PermUse) && !player.IsAdmin)
			{
			...
			player.Reply($"У игрока {target.Name} забрана привилегия {args[1].ToLower()}");
        }
        ...
        private void CmdGrantPerm(IPlayer player, string cmd, string[] args)
        {
			...
			player.Reply($"Игроку {target.Name} выдана привилегия {args[1].ToLower()}");
        }
        ...
        private void CmdRemoveGroup(IPlayer player, string cmd, string[] args)
        {
			...
			player.Reply($"Игрок {target.Name} удален из группы {args[1].ToLower()}");
        }
        ...
        private void CmdAddGroup(IPlayer player, string cmd, string[] args)
        {
			...
			player.Reply($"Игрок {target.Name} добавлен в группу {args[1].ToLower()}");
        }
        ...
        private void CmdPlayerInfo(IPlayer player, string cmd, string[] args)
        {
            ...
            else
            {
                ...
                {
                    string msg1 = GetMessage("<size=16>Привилегия:</size> <color=#C4FF00>{groups}</color>\n<size=16>Оставшееся время:</size> <color=#C4FF00>{timeleft}</color>\n", player.Id);
                ...
                {
                    string msg1 = GetMessage("<size=16>Группа:</size> <color=#C4FF00>{groups}</color>\n<size=16>Оставшееся время:</size> <color=#C4FF00>{timeleft}</color>\n", player.Id);
                    TimeSpan result = (g.ExpireDate).Subtract(DateTime.UtcNow);
                    msg1 = msg1.Replace("{groups}", $"{g.Value}");
                    msg1 = msg1.Replace("{timeleft}", $"{FormatTime(TimeSpan.FromDays(result.TotalDays))}");
                    GetPrivilage = GetPrivilage + msg1 + "\n";
        ...
        private void CmdServiceInfo(IPlayer player, string cmd, string[] args)
        {
            ...
            {
                player.Reply($"Команда '{cmd}' может использоваться только игроками!", cmd);
                return;
            }
            ...
            lang.RegisterMessages(new Dictionary<string, string>
                {
                    ...
                    {"UserDoesn'tHavePermission", "У игрока {target} нет привилегии: '{permission}'."},
                    {"UserIsn'tInGroup", "Игрок {target} не состоит в группе: '{group}'."},
					...
					{"PermissionsInfo", "<size=16>Привилегия:</size> <color=#C4FF00>{groups}</color>\n<size=16>Оставшееся время:</size> <color=#C4FF00>{timeleft}</color>\n"},
					{"GroupsInfo", "<size=16>Группа:</size> <color=#C4FF00>{groups}</color>\n<size=16>Оставшееся время:</size> <color=#C4FF00>{timeleft}</color>\n"},
                    {"Days", "Д"},
                    {"Hours", "Ч"},
                    {"Minutes", "М"},
                }, this);
