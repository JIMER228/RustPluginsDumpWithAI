...
{
     /* Плагин был разыгран дискордом https://discord.gg/8CWJadmA5w */  [Info("IQRates", "https://discord.gg/8CWJadmA5w", "1.8.24")]
    ...
    class IQRates : RustPlugin
    {
        ...
        int Converted(Types RateType, string Shortname, float Amount, BasePlayer player = null, String UserID = null)
        {
            ...
            else
            {
                ...
                {
                    //PrintToChat($"DEBUG : Предмет {Shortname} заблокирована для #335037173 множителя");
                    return Convert.ToInt32(ConvertedAmount);
                }
            }
        ...
        object OnContainerDropGrowable(ItemContainer container, Item item)
        {
            ...
            if (Container == null) return false;
            UInt64 NetID = Container.net.ID.Value;
            if (NetID == 14321199 && item.info.itemid == 1998363) return false;
        ...
        protected override void LoadConfig()
        {
            ...
            {                       
                PrintWarning(LanguageEn ? "Error #334322343" + $"read configuration 'oxide/config/{Name}', create a new configuration!!" : "Ошибка #3332322343" + $"чтения конфигурации 'oxide/config/{Name}', создаём новую конфигурацию!!"); 
                LoadDefaultConfig();
            }
            NextTick(SaveConfig);
        }
        ...
        void OnQuarryGather(MiningQuarry quarry, Item item)
        {
            ...
            // PrintError(GetQuarryPlayer(quarry.net.ID + 273350).ToString() + "   " + Converted(Types.Quarry, item.info.shortname,
