...
{
    [Info("RFTeleports", "/* https://discord.gg/k3hXsVua7Q */", "1.0.9")]
    public class RFTeleports : RustPlugin
    {
        ...
        private bool IsRaid(BasePlayer player)
        {
            if (NoEscape)
            {
                return NoEscape.CallHook("IsRaidBlocked", player) is bool ? (bool)NoEscape.CallHook("IsRaidBlocked", player) : false;
            }
        ...
        void AcceptHouse(BasePlayer player, ulong numid )
        { 
            ...
            cont.Add(new CuiButton()
            {
                Text =
                {
                    FadeIn = 1f, Text = "<size=14>Телепортация</size>\nЧтобы сохранить точку телепорта на этом спальнике/кровати, <color=#00838a>нажмите сюда</color>.", Align = TextAnchor.MiddleLeft, FontSize = 12
                },
                RectTransform =
                {
        ...
        void HomeGenerate(ulong nument, BasePlayer player)
        {
            if(!cfg.hometeleport) return;
            var entity = BaseNetworkable.serverEntities.Find(new NetworkableId(nument)) as SleepingBag;
            ...
            if (_homeList.TryGetValue(player.userID, out f))
            {
                ...
                {
                    HomeGenerate(entity.net.ID.Value, player);
                    return;
                }
            ...
            if(player.IsBuildingAuthed())
                if(_homeList.TryGetValue(player.userID, out playerData))
                {
                    if (GetHomeLimit(player.userID) > playerData._homeList.Count)
                        AcceptHouse(player, entity.net.ID.Value);
                }
                else
                {
                    AcceptHouse(player, entity.net.ID.Value);
                ...
                { 
                    ...
                    else if (homes.TryGetValue(racia.frequency, out f))
                    {
                        ...
                        { 
                            ...
                            if(f.ShortName == "sleepingbag_leather_deployed") procent += ins.cfg.sleepbeg * 0.1;
                            else procent += ins.cfg.bad * 0.1;
                            DrawUi((int) procent, 100);
                            if (procent >= 100)
                            {
                                ...
                                else if (hit.GetEntity().net.ID.Value == f.netId)
                                {
                                    if (!CheckPlayer(player))
                                        return;
        ...
        void SetHome(BasePlayer player)
        {
            ...
            if(_homeList.TryGetValue(player.userID, out PlayerData playerData))
            {
                var exsistHome = playerData._homeList.FirstOrDefault(p => p.Value.netId == hit.GetEntity().net.ID.Value);
       
                if (exsistHome.Value != null)
                {
                    ReplySend(player, "Вы уже используете этот спальник");
                    
                    return;
                }
                if (GetHomeLimit(player.userID) > playerData._homeList.Count)
                    HomeGenerate(hit.GetEntity().net.ID.Value, player);
