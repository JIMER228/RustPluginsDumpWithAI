...
{
    [Info("LockOnRockets", "NoName", "0.2.0", ResourceId = 0)]
    ...
    class LockOnRockets : RustPlugin
    {
        ...
        {            
            ...
            lang.RegisterMessages(new Dictionary<string, string> 
			{
				...
				{"aquiring", ">><color=#E50000> Capture Target </color><<"},
            ...
            lang.RegisterMessages(new Dictionary<string, string> 
			{
				...
				{"aquiring", ">><color=#E50000> Захват цели </color><<"},
				{"unloaded", "<color=#939393>Вы запустили </color><color=#C4FF00>ракету с наведением</color>"},
				{"loaded", "<color=#939393></color><color=#C4FF00>Ракета с наведением</color><color=#939393> заряжена в ракетницу!</color>"},
				{"inventory", "<color=#939393>У вас есть </color><color=#C4FF00>ракеты с наведением</color><color=#939393> в инвентаре! Для использования выбирите дымовую ракету</color>"}
            }, this, "ru"); 
        }
        ...
        void OnRocketLaunched(BasePlayer player, BaseNetworkable entity)
        {
            ...
            if (entity.ShortPrefabName == "rocket_smoke")
            {
				...
				rocketPrefabId = StringPool.Get(rocketPrefab);
				if (entity.prefabID == rocketPrefabId)
				{
						entity.Kill();
				}
                    ...
                    if (newTarget != null)
                    {
                        switch (newTarget.GetType().ToString())
                        {
                            ...
                            case "PatrolHelicopterAI":
                                ...
                                isHelicopter = true;
                                break;
							case "CH47HelicopterAIController":
                                if (!lockTypes[LockTypes.CH47Helicopter])
                                    return;
								isHelicopter = true;
                                break;
							case "BradleyAPC":
                                if (!lockTypes[LockTypes.Tank])
                                    return;
                                break;
                            case "BasePlayer":
                                ...
                                break;
                            case "GunTrap":
                                if (!lockTypes[LockTypes.GunTrap])
                                    return;
                                break;	
                            case "BaseNPC":
        ...
        class HomingRocket : MonoBehaviour
        {
            ...
            void OnDestroy()
            {
                if (isHelicopter && Vector3.Distance(target.transform.position, rocket.transform.position) < 5)
                {
                    ...
                    var сh47helicopter = target as CH47HelicopterAIController;
                    if (сh47helicopter != null)
                    {
                        сh47helicopter.Hurt(ins.configData.RocketDamage * ins.configData.HelicopterLockModifiers.DamageModifier, Rust.DamageType.Explosion, player);
                    }
                }
        ...
        #region Config 
        enum LockTypes { Car, Plane, Helicopter, CH47Helicopter, Tank, Player, GunTrap, Animal, Structure, Resource, Loot }       
        ...
        protected override void LoadDefaultConfig()
        {
            ...
            {               
                ...
                LockOnTypes = new Dictionary<LockTypes, bool>
                {
                    ...
                    {LockTypes.CH47Helicopter, true },
                    {LockTypes.Tank, true },
                    {LockTypes.GunTrap, true },
