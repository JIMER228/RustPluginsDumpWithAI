...
{
    [Info("OilCrate", "Server-Rust", "0.6.5", ResourceId = 2339)]
    ...
    class OilCrate : RustPlugin
    {
        ...
        {
            quarryLiquid = GetConfig("1. Анализ", "1. Разрешить карьеру собирать нефть?", false);
            pumpjackSolid = GetConfig("1. Анализ", "2. Разрешить нефтекачке собирать руду?", false);
            effectStaticPumpjacks = GetConfig("1. Анализ", "3. Изменение скорости сбора статических насосных гнезд (у памятников)", true);
            missingPermission = GetConfig("1. Анализ", "4. Показать сообщение (отсутствует разрешение)", true);
            oilCrateChance = GetConfig("2. Настройки нефтекачки", "1. Шанс в % (0.0 - 100.0)", 50f);
            oilCrateDespawn = GetConfig("2. Настройки нефтекачки", "2. Таймер удаления в секундах", 300f);
            dryOnly = GetConfig("2. Настройки нефтекачки", "3. Только на сухом биоме", false);
            oilGatherWorkNeededMin = 10 / GetConfig("3. Скорость сбора сырой нефти", "1. Минимальное время сбора сырой нефти ", 0.525f);
            oilGatherWorkNeededMax = 10 / GetConfig("3. Скорость сбора сырой нефти", "2. Максимальное время сбора сырой нефти", 0.725f);
            dropFreePumpjack = GetConfig("4. Получить гнездо насоса", "1. Получить нефтекачку с гнезда", false);
            craftViaPicaxe = GetConfig("4. Получить гнездо насоса", "2. Ремесло через хит с киркой", true);
            pumpJackResources = GetConfig("4. Получить гнездо насоса", "3. Ресурсы для создания гнезда насоса", pumpJackResourcesDefault());
            SaveConfig();
        }
        ...
        private void OnSurveyGather(SurveyCharge survey, Item item)
        {
            ...
            if (!IsAllowed((BasePlayer)survey.creatorEntity, true))
            {
                ...
                if (missingPermission) SendReply((BasePlayer)survey.creatorEntity, "У вас нет привилегий для выполнения нефтяного бурения");
                return;
            }
            if (dryOnly && TerrainMeta.BiomeMap.GetBiome(pos, TerrainBiome.ARID) < 0.5f)
            {
                item.Remove();
                DeSpawn("survey_crater", pos, 0f);
                if (missingPermission) SendReply((BasePlayer)survey.creatorEntity, "Вы можете найти масло только в южной пустыне");
                return;
            }
      ...
      //  Слив плагинов server-rust by Apolo YouGame
        {
            ...
            foreach (var resourcePair in pumpJackResources)
            {
                ...
                {
                    SendReply(attacker, "Вы не можете собрать гнездо насоса. Вам не хватает {0}.", resource.displayName.english.ToLower());
                    return;
                }
        ...
        void cmdOilCrate(BasePlayer player, string command, string[] args)
        {
            ...
            if (args == null || args.Length <= 0 || args.Length > 2)
            {
                ...
                SendReply(player, "Вы можете использовать следующие команды:"
                    + "\n\t- regenerate\t|- <color=yellow>Генерировать каждый источник сырой нефти</color>"
                    + "\n\t- clear\t\t\t\t|- <color=red>Удалять сырую нефть из каждого источника</color>"
                    + "\n\t\t\t\t\t\t\t|  (не может быть отменено)"
                    + "\n\t- destroy\t\t\t|-  <color=red>Разрушает пустые домкраты и карьеры</color>");
                return;
            }
            switch (args[0].ToLower())
            {
                case "regenerate":
                    ...
                    {
                        if (quarryLiquid)
                            Puts("Добыча сырой нефти в каждом насосном гнезде и карьере была успешно обновлена.");
                        else
                            Puts("Нефть сырой нефти в каждом насосном гнезде была успешно обновлена.");
                    }
                    if (quarryLiquid)
                        SendReply(player, "Добыча сырой нефти в каждом насосном гнезде и карьере была успешно обновлена.");
                    else
                        SendReply(player, "Сырая нефть в каждом насосном гнезде была успешно обновлена.");
                    break;
                case "clear":
                    ...
                    {
                        if (quarryLiquid)
                            Puts("Сырая нефть каждого гнезда и карьера насоса была успешно удалена.");
                        else
                            Puts("Сырая нефть каждого гнезда насоса была успешно удалена.");
                    }
                    if (quarryLiquid)
                        SendReply(player, "Сырая нефть каждого гнезда и карьера насоса было успешно удалено.");
                    else
                        SendReply(player, "Сырая нефть каждого гнезда насоса было успешно удалено.");
                    break;
                case "destroy":
                    ...
                    {
                        if (quarryLiquid)
                            Puts("Сырая нефть каждого гнезда насоса и карьера было успешно удалено. И гнёзда были уничтожены.");
                        else
                            Puts("Сырая нефть каждого гнезда насоса было успешно удалено. И гнёзда были уничтожены.");
                    }
                    if (quarryLiquid)
                        SendReply(player, "Сырая нефть каждого гнезда насоса и карьера была успешно удалена. И гнёзда были уничтожены.");
                    else
                        SendReply(player, "Сырая нефть каждого гнезда насоса было успешно удалено. И гнёзда были уничтожены.");
                    break;
                default:
                    SendReply(player, "Введи: /oilcrate {0}", args[0]);
                    break;
            }
        }
