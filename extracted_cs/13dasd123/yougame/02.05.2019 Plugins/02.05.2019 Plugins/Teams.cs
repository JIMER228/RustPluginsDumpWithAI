...
{
    [Info("Teams", "Fartus/A1M41K", "1.0.1")]
        ...
        private void cmdChatFriends(BasePlayer player, string command, string[] args)
        {
			if (args == null || args.Length <= 0 || args.Length == 1 && args[0].ToLower() != "accept" && args[0].ToLower() != "cancel")
			{
                SendReply(player, "Используйте:\n" +
										...
										"<color=#FFA500>/friend remove 'НИК'</color> - выгнать игрока из команды");
                return;
            }
			...
			switch (args[0].ToLower())
            {
                case "add":
				    if (playerTeam == null)
                    {
                        SendReply(player, "У вас нету команды, сначала создайте её!\n<size=12>Создать её можно в левом нижнем углу инвентаря</size>");
                        return;
                    }

                    if (playerTeam.GetLeader() != player)
                    {
                        SendReply(player, "Вы не лидер этой команды, вы не можете приглашать игроков!");
                        return;
                    }
                    ...
                    if (target == player)
                    {
                        SendReply(player, "Вы не можете добавить себя в команду!");
                        return;
                    }
                    ...
                    if (targetTeam != null)
                    {
                        SendReply(player, $"Игрок <color=#BEF781>{target.displayName}</color> уже в вашей команде!");
                        return;
                    }
                    playerTeam.SendInvite(target);

                    SendReply(player, $"Вы успешно пригласили игрока <color=#BEF781>{target.displayName}</color> в свою команду!");
                    //SendReply(target, $"Игрок <color=#BEF781>{player.displayName}</color> пригласил вас в команду\nПримите приглашение в инвентаре или набрав в чате <color=#FFA500>/friend accept</color>");
					SendReply(target, $"Игрок <color=#BEF781>{player.displayName}</color> пригласил вас в команду\nПримите приглашение в инвентаре!");
                    break;

                /*case "accept":
                    playerTeam.AcceptInvite(player);
                    SendReply(player, "Вы приняли запрос в команду!");
                    break;

                case "cancel":
                    playerTeam.RejectInvite(player);
                    SendReply(player, "Вы отклонили запрос в команду!");
                    break;*/
				...
				case "remove":
				    ...
				    if (target2 == null)
					{
						SendReply(player, $"Игрока с именем <color=#BEF781>{playerArg2}</color> нет в вашей команде!");
						return;
					}
					if (target2 == player)
                    {
						SendReply(player, "Вы не можете выгнать себя из команды!");
						return;
					}
					if (playerTeam == null)
				    {
					...
					if (playerTeam.GetLeader() != player)
				    {
				        SendReply(player, "Вы не лидер этой команды, вы не можете выгонять игроков!");
				        return;
				    }
                    playerTeam.RemovePlayer(target2.userID);
                    SendReply(player, $"Вы успешно выгнали игрока <color=#BEF781>{target2.displayName}</color> из вашей команды!");
					SendReply(target2, $"Игрок <color=#BEF781>{player.displayName}</color> выгнал вас из команды!");
				    break;
            }
        }
      ...
      //  Слив плагинов server-rust by Apolo YouGame
        {
            ...
            if (API_AreTeamMates(entity.GetComponent<BasePlayer>(), info.InitiatorPlayer) && !enableDamage.Contains(info.InitiatorPlayer.userID))
            {
                if (!disableMessage.Contains(info.InitiatorPlayer.userID))
                {
                    ...
                    SendReply(info.InitiatorPlayer, "Вы не можете нанести урон своему другу!");
                    timer.Once(1, () =>
                    {
                        if (disableMessage.Contains(info.InitiatorPlayer.userID))
        ...
        private bool API_AreTeamMates(ulong playerId, ulong targetId)
        {
            ...
            ulong teamId1 = RelationshipManager._instance.playerTeams[playerId].teamID;
            if (!RelationshipManager._instance.cachedPlayers.ContainsKey(targetId))
                return false;
            
            ulong teamId2 = RelationshipManager._instance.playerTeams[targetId].teamID;
