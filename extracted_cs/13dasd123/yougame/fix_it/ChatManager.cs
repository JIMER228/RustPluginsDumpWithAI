...
{
    [Info("ChatManager", "bazuka5801", "1.0.4")]
    public class ChatManager : RustPlugin
    {
        ...
        bool? OnPlayerChat(ConsoleSystem.Arg arg)
        {
            ...
            if (prefix.Length > 0)
                name = $"{prefix} " + GetClanTag(sender.userID) + $"{name}" ;
            else
        ...
        void cmdchatCommand(BasePlayer player, string command, string[] args)
        {
            ...
            if (args.Length > 1)
            {
				...
				Server.Broadcast(line, "<color=#5af>" + author + "</color>", 76561197960279927 + (ulong)(UnityEngine.Random.Range(1, 1000)));
            }
        }
        ...
        {
            Server.Broadcast(Format(Messages[langKey]));
        }
        ...
        private Dictionary<string, string> Messages = new Dictionary<string, string>()
        {
            ...
            {"MUTE.ALL.ACTIVE", "Общий чат отключен, Вы не можете общаться в чате"},
            ...
            {"PM.SENDER.FORMAT", "<color=#DAA520>ЛС для {0}</color>: {1}"},
            {"PM.RECEIVER.FORMAT",  "<color=#DAA520>ЛС от {0}</color>: {1}"},
