...
namespace Oxide.Plugins
{
    ...
    class EventQuiz : RustPlugin
    {
        ...
        private void OnPlayerChat(ConsoleSystem.Arg arg)
        {
            ...
            if (player == null || _eventData == null || !arg.HasArgs())
                return;

            var answer = string.Join(" ", arg.Args).ToLower().Trim();
            ...
            {
                SendReply(p, lang.GetMessage("End", this).Replace("{name}", player.displayName));
            });
        ...
        private string CanStartEvent()
        {
            ...
            if (_questions.Where(x => x.IsEnabled).Count() <= 0) return lang.GetMessage("No Question", this);
        ...
        {
            _eventData.Prize.ForEach(prize => player.GiveItem(ItemManager.CreateByName(prize.Name, prize.Amount, prize.Skin)));
        }
