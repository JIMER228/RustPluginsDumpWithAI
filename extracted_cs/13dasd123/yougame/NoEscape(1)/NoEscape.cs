...
namespace Oxide.Plugins
{
    ...
    class NoEscape : RustPlugin
    {
        ...
        private void LoadDefaultConfig()
        {
            ...
            GetConfig("Основное", "Включить отключение блокировки при разрушении шкафа (BuildingPrivilage), если функция будет включена игроку не будет давать блокировку если разрушенный объект будет вне билдинг зоны",
        ...
        void BlockPlayer(BasePlayer player, BuildingPrivlidge building = null, string mode = "", bool owner = false)
        {
            ...
            if (mode == "attack")
            {
                ...
                if (!timers.ContainsKey(player.userID))
                {
                    player.ChatMessage(string.Format(Messages[owner ? "ownerhome" : "blockattackactive"], FormatTime(TimeSpan.FromSeconds(blockAttackTime))));
                }

                if (!owner || !timers.ContainsKey(player.userID))
                {
                    var secs = blockAttackTime;
                    
                    timers[player.userID] = new RaidBlock() {building = null, Seconds = secs, Attack = building == null,};
        ...
        public static string FormatTime(TimeSpan time)
        {
            ...
            if (time.Hours   != 0) result += $"{Format(time.Hours,   "час",  "час",    "час")} ";
        ...
        void OnPlayerInit(BasePlayer player)
        {
            ...
            {
                foreach (var raid in raids)
                    if (raid.Value.owners.Contains(player.userID) && raid.Value.owners.Count(p => BasePlayer.FindByID(p) != null) <= 1 && raid.Value.owners.Any(o => damageTimers.All(t => !t.owners.Contains(o))))
                    {
        ...
        void RaidZoneTimerHandle()
        {
            ...
            foreach (var raid in raids)
            {
                ...
                if (raid.Value.raiders.Count <= 0)
                {
                    ...
                    {
                        timers[owner] = new RaidBlock() {building = raid.Key, Seconds = ownerBlockTime, Attack = raid.Key == null};
        ...
        List<ulong> GetOwnersByOwner(ulong owner, Vector3 position, ulong pIayerid = 2281488) 
        {
            var coliders = new List<BaseEntity>();
        ...
        string DrawGUIAnno =
                "[{\"name\":\"noescape_background\",\"parent\":\"Overlay\",\"components\":[{\"type\":\"UnityEngine.UI.Button\",\"color\":\"0.526714458 0.1478982719 0.14798782819 0.4666667\"},{\"type\":\"RectTransform\",\"anchormin\":\"{min}\",\"anchormax\":\"{max}\",\"offsetmin\":\"0 0\",\"offsetmax\":\"1 1\"}]},{\"name\":\"noescape_text\",\"parent\":\"noescape_background\",\"components\":[{\"type\":\"UnityEngine.UI.Text\",\"text\":\"{0}\",\"fontSize\":18,\"align\":\"MiddleCenter\"},{\"type\":\"UnityEngine.UI.Outline\",\"color\":\"0.35817730573 0 0 0.54355824246\",\"distance\":\"1 -1\"},{\"type\":\"RectTransform\",\"anchormin\":\"0 0\",\"anchormax\":\"1 1\"}]}]";
        ...
        string UI =
                "[{\"name\":\"noescape_main2\",\"parent\":\"Overlay\",\"components\":[{\"type\":\"UnityEngine.UI.Image\",\"color\":\"1 1 1 0.1151957809\"},{\"type\":\"RectTransform\",\"anchormin\":\"{min}\",\"anchormax\":\"{max}\",\"offsetmax\":\"0 0\"}]},{\"name\":\"noescape_main3\",\"parent\":\"noescape_main2\",\"components\":[{\"type\":\"UnityEngine.UI.Image\",\"color\":\"0 0.59537785081 1 0.74875516586\"},{\"type\":\"RectTransform\",\"anchormin\":\"0 0\",\"anchormax\":\"{amax} 1\",\"offsetmax\":\"0 0\"}]},{\"name\":\"noescape_main4\",\"parent\":\"noescape_main2\",\"components\":[{\"type\":\"UnityEngine.UI.Text\",\"text\":\"{0}\",\"align\":\"MiddleCenter\"},{\"type\":\"UnityEngine.UI.Outline\",\"color\":\"0 0 0 1\",\"distance\":\"0.5 0\"},{\"type\":\"RectTransform\",\"anchormin\":\"0.001725851534183 0\",\"anchormax\":\"1.001879571 1\",\"offsetmax\":\"0 0\"}]}]";
        ...
        public static void SetCooldown(BasePlayer player, string key, int seconds, Action onExpired = null)
        {
            ...
            cooldownList.Add(new Cooldown() {UserId = player.userID, Expired = GrabCurrentTime() + (double) seconds, OnExpired = onExpired});
        }
        ...
        {
            {"blocksuccess", "Блок деактивирован. Функции разблокированы"}, {"guitimertext", "<b>Блокировка:</b> Осталось {0}"}, {"noblockowner", "Ваше строение разрушено! Летите на защиту"},
            ...
            {"blockattackactive", "Включен режим боя, активирован блок на {0}! Некоторые функции временно недоступны."}, {"blockrepair", "Вы не можете ремонтировать строения во время рейда, подождите {0} сек."},
            ...
            {"noblock", "NoEscape by <color=#ECBE13><size=15>RustPlugin.ru</size></color>\nУ вас нету рейд блока :)"}, {"blocktp", "Вы не можете использовать телепорт во время рейда, подождите {0} сек."},
            {"blockremove", "Вы не можете удалить постройки во время рейда, подождите {0} сек."}, {"blockupgrade", "Вы не можете использовать автоулучшение построек во время рейда, подождите {0} сек."},
            {"blockKits", "Вы не можете использовать киты во время рейда, подождите {0} сек."}, {"blockbuld", "Вы не можете строить во время рейда, подождите {0} сек."},
            {"yourbuildingdestroy", "Вас, или дом Вашего соклана рейдят! Добавлена отметка на карту"}, {"yourbuildingdestroyOwner", "Внимание! Ваш дом рейдит игрок {1}! Добавлена отметка на карту"},
            {"isClanMember", "Внимание! Данное строение пренадлежит Вашему соклановцу! Урон уменьшен"}, {"isFriendMember", "Внимание! Данное строение пренадлежит Вашему другу! Урон уменьшен"}, {"removerestrict", "Хозяев нет в сети! Ремув запрещён!"},
            {"ownerhome", "Вы рядом со своим домом, который рейдят!\nРейдблок активирован на {0}!"}, {"raidremove", "Вы не можете удалять обьекты во время рейда, подождите {0} сек."},
            {"blockrec", "Вы не можете использовать переработчик во время рейда, подождите {0} сек."}, {"ownernotonline", "Владельцов нет в сети, ремув недоступен!"},
            {"DamageOnlineOwner", "Один их хозяев постройки сейчас в сети.\nУрон по объектам владельца стандартный"}, {"DamageNotOnlineOwner", "Не одного владельца постройки нет в сети. \nУрон по объектам владельца уменьшен в 2 раза!"},
            {"OwnerEnterOnline", "Хозяин постройки зашел в игру.\nУрон по объектам владельца стандартный!"}, {"blocktrade", "Вы не можете использовать обмен во время рейда, подождите {0} сек."},
