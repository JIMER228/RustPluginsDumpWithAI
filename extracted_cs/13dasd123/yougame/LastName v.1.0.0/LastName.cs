...
/* --- Не редактируйте ничего здесь, если не знаете, что делаете --- */

namespace Oxide.Plugins
{
	[Info("LastName", "A0001", "1.0.0", ResourceId = 1227)]
	...
	class LastName : RustPlugin
	{
		...
		{
			CheckConfig();
			Puts("Конфигурация по умолчанию сохранена и загружена");
		}
		...
		void LoadDefaultMessages()
		{
			lang.RegisterMessages(new Dictionary<string, string>()
			{
				...
				{ "NoPlayerFound", "Нет игроков с этим name/steamID" },
				{ "PlayerWasFound", "{name}({id}) был также известен как: " }
			}, this);
		}
