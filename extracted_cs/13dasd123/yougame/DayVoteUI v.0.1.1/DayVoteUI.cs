...
{
    [Info("DayVoteUI", "RustPlugin.Ru", "0.1.1", ResourceId = 0)]
    class DayVoteUI : RustPlugin
    {
        #region Fields
        ...
        {
            var MainCont = UI.CreateElementContainer(Main, UI.Color(configData.Colors.UIBackgroundColor, configData.Colors.UIBackgroundAlpha), "0.325 0.90", "0.625 0.95");
            UI.CreateLabel(ref MainCont, Main, "", $"<color={configData.Colors.MainColor}>{msg("skipNight", player.UserIDString)}</color>", 14, "0.02 0", "1 1", TextAnchor.MiddleLeft);
            ...
            UI.CreatePanel(ref MainCont, Main, UI.Color(configData.Colors.UIBackgroundColor, configData.Colors.UIBackgroundAlpha), $"0.25 0.15", $"0.8 0.85");
            UI.CreatePanel(ref MainCont, Main, UI.Color(configData.Colors.ProgressBarColor, 1), $"0.25 0.15", $"{yMaxVotes} 0.85");            
            UI.CreateLabel(ref MainCont, Main, "", $"{votesReceived.Count} / {requiredVotes}", 14, "0.25 0.15", "0.8 0.85");

            UI.CreateLabel(ref MainCont, Main, "", GetFormatTime(), 14, "0.8 0.1", "0.95 0.9", TextAnchor.MiddleRight);
        ...
        void Loaded()
        {
            ...
            permission.RegisterPermission("dayvoteui.admin", this);           
        }
        ...
        private void cmdVoteDay(BasePlayer player, string command, string[] args)
        {
            ...
            {
                if (!permission.UserHasPermission(player.UserIDString, "dayvoteui.admin")) return;
        ...
        {
            {"voteSuccess", "{msg}Вы проголосовали, чтобы пропустить ночь!</color>" },
            {"noVote", "{msg}В настоящее время голосование не открыто!</color>" },
            {"alreadyOpen", "{msg}Голосование продолжаеться!</color>" },
            {"invalidSyntax", "{msg}Неправильно!</color> {main}/voteday open</color>{msg} || </color> {main}/voteday close</color>" },
            {"votingSuccessful", "{main}Голосование прошло успешно!</color>{msg} Пропускаем ночь!</color>" } ,
            {"votingUnsuccessful", "{main}Голосование провалено!</color>{msg} Да настанет ночь!</color>" },
            {"commandSyn", "{msg}Введите </color>{main}/voteday</color>{msg} что бы пропустить ночь!\n-- Требуеться: </color>{main}{percent}%</color>{msg} голосов что бы пропустить.</color>" },
            {"skipNight", "Голосование" }        
        };
        #endregion
    }
}
