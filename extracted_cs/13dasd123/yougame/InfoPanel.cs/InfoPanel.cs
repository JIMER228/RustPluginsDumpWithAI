...
{
    [Info("InfoPanel", "A0001", "1.0.0", ResourceId = 1356)]
    ...
    public class InfoPanel : RustPlugin
    {
                    ...
                    { "ChinukEvent", new PanelConfig
                        {
                            ...
                            Image = new PanelImageConfig
                            {
                                ...
                                Height = 0.8f,
                                Margin = "0 0.15 0.1 0.1",
                                Url = "https://i.imgur.com/BsE9yMr.png",
                            },
                            PanelSettings = new Dictionary<string,object>
                            {
                                { "InactiveColor", "1 1 1 0.1" },
                    ...
                    { "ShipEvent", new PanelConfig
                        {
                            ...
                            Image = new PanelImageConfig
                            {
                                ...
                                Height = 0.8f,
                                Margin = "0 0.15 0.1 0.1",
                                Url = "https://i.imgur.com/VFvc8pT.png",
                            },
                            PanelSettings = new Dictionary<string,object>
                            {
                                { "InactiveColor", "1 1 1 0.1" },
        ...
        void OnServerInitialized()
        {
            ...
            ActiveChinuk = UnityEngine.Object.FindObjectsOfType<CH47Helicopter>().ToList();
            ...
            {
                Chinuk.Refresh(storedData, PlayerPanels);
            }

            ActiveShip = UnityEngine.Object.FindObjectsOfType<CargoShip>().ToList();

            if (ActiveShip.Count > 0)
            {
                CheckShip();
            }
            else
            {
                Ship.Refresh(storedData, PlayerPanels);
            }
        }
        ...
        private void OnEntitySpawned(BaseEntity Entity)
        {
            ...
            if (Entity is CH47Helicopter && Settings.Panels["ChinukEvent"].Available)
            {
                ActiveChinuk.Add((CH47Helicopter)Entity);

                if (ChinukTimer == false)
                {
                    CheckChinuk();
                }
            }

            if (Entity is CargoShip && Settings.Panels["ShipEvent"].Available)
            {
                ActiveShip.Add((CargoShip)Entity);
        ...
        private List<string> Messages = new List<string> { "Welcome!", "Beware! You Are Not Alone!", "A0001" };
        ...
        private ChinukEvent Chinuk;
        private List<CH47Helicopter> ActiveChinuk;
        private bool ChinukTimer = false;

        private ShipEvent Ship;
        private List<CargoShip> ActiveShip;
        private bool ShipTimer = false;
        ...
        public class ChinukEvent
        {
            ...
            public ChinukEvent()
            {
                ImageColor = ColorEx.Parse(Settings.GetPanelSettingsValue("ChinukEvent", "InactiveColor", "1 1 1 0.1"));
            }
            ...
            public void SetActivity(bool active)
            {
                ...
                if (isActive)
                {
                    ImageColor = ColorEx.Parse(Settings.GetPanelSettingsValue("ChinukEvent", "ActiveColor", "0.08 1 0 1"));
                    return;
                }
            ...
            public void Refresh(StoredData storedData, Dictionary<string, Dictionary<string, IPanel>> panels)
            {
                if (!Settings.CheckPanelAvailability("ChinukEvent"))
                    return;

                foreach (var panel in panels)
                {
                    IPanel iPanel;
                    if (!panel.Value.TryGetValue("ChinukEventImage", out iPanel)) continue;
                    var panelRawImage = (IPanelRawImage)iPanel;
                    if (panelRawImage.Color != ImageColor)
                    {
                        panelRawImage.Color = ImageColor;
        ...
        public class ShipEvent
        {
            ...
            public ShipEvent()
            {
                ImageColor = ColorEx.Parse(Settings.GetPanelSettingsValue("ShipEvent", "InactiveColor", "1 1 1 0.1"));
            }
            ...
            public void SetActivity(bool active)
            {
                ...
                if (isActive)
                {
                    ImageColor = ColorEx.Parse(Settings.GetPanelSettingsValue("ShipEvent", "ActiveColor", "0.08 1 0 1"));
                    return;
                }
            ...
            public void Refresh(StoredData storedData, Dictionary<string, Dictionary<string, IPanel>> panels)
            {
                if (!Settings.CheckPanelAvailability("ShipEvent"))
                    return;

                foreach (var panel in panels)
                {
                    IPanel iPanel;
                    if (!panel.Value.TryGetValue("ShipEventImage", out iPanel)) continue;
                    var panelRawImage = (IPanelRawImage)iPanel;
                    if (panelRawImage.Color != ImageColor)
                    {
                        panelRawImage.Color = ImageColor;
        ...
        public void CheckChinuk()
        {
            ...
            if (ActiveChinuk.Count > 0)
            {
                ...
                if (Chinuk.isActive == false)
                {
                    Chinuk.SetActivity(true);
                    Chinuk.Refresh(storedData, PlayerPanels);
                }

                ChinukTimer = true;
                timer.In(5, CheckChinuk);
                return;
            }

            Chinuk.SetActivity(false);
            Chinuk.Refresh(storedData, PlayerPanels);
            ChinukTimer = false;
        }
        ...
        {
            ActiveShip.RemoveAll(p => !p.IsValid() || !p.gameObject.activeInHierarchy);
            ...
            if (ActiveShip.Count > 0)
            {
                ...
                ShipTimer = true;
                timer.In(5, CheckShip);
                return;
            }

            Ship.SetActivity(false);
            Ship.Refresh(storedData, PlayerPanels);
            ShipTimer = false;
        }
            ...
            foreach (var Panel in PlayerPanels[player.UserIDString])
            {
                switch (Panel.Key)
                {
                    ...
                    case "HelicopterEventImage":
                        ...
                        break;
                    case "ChinukEventImage":
                        ((IPanelRawImage)Panel.Value).Color = Chinuk.ImageColor;
                        break;
                    case "ShipEventImage":
                        ((IPanelRawImage)Panel.Value).Color = Ship.ImageColor;
                        break;
                    case "CompassText":
