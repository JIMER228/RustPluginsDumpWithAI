...
/*  TODO:
              ...
              - Grenade turrets?
              - Make custom permissions at least for per player limits, please.
              - Stop turrets from shooting animals

    BUGS:
            1. anti-air turrets don't lose their health on projectile impact/splash damage.     (idk if this is (efficently) possible)
            2. make custom permissions at least for per player limits, please.          (WIP)
            3. heat seeking rocket feature don't work or i don't understand how it should to work =]                              ✓

    UPDATE LOG:
            
            - Optimized plugin loading (using CoRoutine to initialize all rocket turrets)
*/

namespace Oxide.Plugins
{
    [Info("RocketTurrets", "redBDGR", "1.3.7")]
    [Description("Create rocket shooting turrets")]
    class RocketTurrets : RustPlugin
    {
        private bool changed;

        private List<AutoTurret> rocketTurrets = new List<AutoTurret>();
        private List<AutoTurret> antiairTurrets = new List<AutoTurret>();
        private List<AutoTurret> javelinTurrets = new List<AutoTurret>();
        private Dictionary<string, PlayerData> turretNumbers = new Dictionary<string, PlayerData>();
        private List<ulong> turretIDs = new List<ulong>();
        private List<ulong> antiAirIDs = new List<ulong>();
        private List<ulong> javelinIDs = new List<ulong>();
        ...
        private int maxNumberAllowed = 5;
        ...
        private int aaMaxNumberAllowed = 5;
        private bool aaRequireBuildingRightsToAccessInventory = true;

        // Javelin Turrets
        private float jrocketFireRate = 1.2f;
        private float jrocketLockonTime = 5f;
        private float jminHitDistance = 10f;
        private float jrocketLockonRadius = 40f;
        private float jrocketSpeed = 20f;
        private uint jrocketLauncherSkin;
        private float jexplosionRadius = 4f;
        private int jlockonSoundEveryxUpdate = 100;
        private bool jrequiresAmmo = true;
        private int jmaxNumberAllowed = 5;
        private bool jSeekTarget = true;
        private float jHeightBeforeCurve = 40f;
        ...
        {
            var at = new Dictionary<string, object> { { "rocket.launcher", 1 } };
            return at;
        }

        private static Dictionary<string, object> AntiAirUpgradeCost()
        {
            var at = new Dictionary<string, object> { { "targeting.computer", 1 }, { "cctv.camera", 2 }, { "rocket.launcher", 1 } };
            return at;
        }
        ...
        private const string permissionNameJAVELIN = "rocketturrets.javelin";
        private const string permissionNameUNLIMITED = "rocketturrets.unlimited";
        ...
        private class StoredData
        {
            ...
            public List<ulong> javelinIDs = new List<ulong>();
        }

        private class StoredData2
        {
            public Dictionary<string, PlayerData> turretNumbers = new Dictionary<string, PlayerData>();
        }
        ...
        private void SaveData()
        {
            ...
            storedData.javelinIDs = javelinIDs;
            storedData2.turretNumbers = turretNumbers;
            data.WriteObject(storedData);
            data2.WriteObject(storedData2);
        }
        ...
        private void LoadData()
        {
            try
            {
                ...
                storedData2 = data2.ReadObject<StoredData2>();
                turretNumbers = storedData2.turretNumbers;
                turretIDs = storedData.turretIDs;
                antiAirIDs = storedData.antiAirIDs;
                javelinIDs = storedData.javelinIDs;
            }
            catch
            {
        ...
        private void LoadVariables()
        {
            ...
            maxNumberAllowed = Convert.ToInt16(GetConfig("Turret Settings", "Maximum Allowed Per Player", 5));
            rocketHeatSeeking = Convert.ToBoolean(GetConfig("Turret Settings", "Heatseeking Rockets", false));
            ...
            aaMaxNumberAllowed = Convert.ToInt16(GetConfig("AntiAir Settings", "Maximum Allowed Per Player", 5));
            aaRequireBuildingRightsToAccessInventory = Convert.ToBoolean(GetConfig("AntiAir Settings", "Require Build Acces to Loot", true));

            // Javelin turret settings
            jrocketFireRate = Convert.ToSingle(GetConfig("Javelin Settings", "Rocket fire rate", 2f));
            jrocketLockonTime = Convert.ToSingle(GetConfig("Javelin Settings", "Lockon Length", 5f));
            jminHitDistance = Convert.ToSingle(GetConfig("Javelin Settings", "Min lockon distance", 10f));
            jrocketLockonRadius = Convert.ToSingle(GetConfig("Javelin Settings", "Lockon radius", 80f));
            jrocketSpeed = Convert.ToSingle(GetConfig("Javelin Settings", "Rocket speed", 15f));
            jrocketLauncherSkin = Convert.ToUInt32(GetConfig("Javelin Settings", "Default rocket model skin", 0));
            jexplosionRadius = Convert.ToSingle(GetConfig("Javelin Settings", "Explosion radius", 4f));
            jlockonSoundEveryxUpdate = Convert.ToInt16(GetConfig("Javelin Settings", "Lockon sound every x updates", 100));
            jrequiresAmmo = Convert.ToBoolean(GetConfig("Javelin Settings", "Requires ammo", true));
            jmaxNumberAllowed = Convert.ToInt16(GetConfig("Javelin Settings", "Max number allowed per player", 5));
            jSeekTarget = Convert.ToBoolean(GetConfig("Javelin Settings", "Seek target", true));
            jHeightBeforeCurve = Convert.ToSingle(GetConfig("Javelin Settings", "Height before curve starts", 40f));
            ...
            javelinUpgradeCost = (Dictionary<string, object>) GetConfig("Javelin Settings", "[ Cost to upgrade - Javelin Tier ]", JavelinUpgradeCost());
        ...
        [HookMethod("IsRocketTurret")]
        public bool IsRocketTurret(AutoTurret turret) => turret.GetComponent<RocketTurret>();

        [HookMethod("ToggleTurretAutomation")]
        public void ToggleTurretAutomation(AutoTurret turret, bool enabled)
        {
            if (turret.GetComponent<RocketTurret>())
                turret.GetComponent<RocketTurret>().enabled = true;
        }

        [HookMethod("TryFireRocket")]
        public void TryFireRocket(AutoTurret turret)
        {
            if (turret.GetComponent<RocketTurret>())
                turret.GetComponent<RocketTurret>().TryFireRocket();
        }
        ...
        #region Turret Classes

        private class RocketTurret : MonoBehaviour
        {
            private float aimRadius = plugin.rocketLockonRadius;
            private float fireRate = plugin.rocketFireRate;
            private float rocketSpeed = plugin.rocketSpeed;
            private float minHitDistance = plugin.minHitDistance;
            private float lockonTime = plugin.rocketLockonTime;
            private uint rocketLauncherSkin = plugin.rocketLauncherSkin;
            private float explosionRadius = plugin.explosionRadius;
            private bool explodeOnTargetPos = plugin.explodeAtTarget;
            private int soundEveryxUpdates = plugin.lockonSoundEveryxUpdate;
            private bool requiresAmmo = plugin.requiresAmmo;
            private bool heatSeek = plugin.rocketHeatSeeking;
            ...
            private void Update()
            {
                ...
                if (!isLockedOn)
                {
                    if (!isLockingOn)
                    {
                        lockonFinish = Time.time + lockonTime;
                        isLockingOn = true;
                    }
                    else if (isLockingOn && Time.time > lockonFinish)
                ...
                else
                {
                    if (Time.time < nextShoot)
                        return;
                    if (Vector3.Distance(turret.muzzlePos.transform.position, turret.target.transform.position) < minHitDistance)
                        return;
                    var rocketItem = HasAmmo();
                    if (rocketItem == null)
                        return;
                    FireRocket(rocketItem);
            ...
            public void Destroy()
            {
                ...
                OnDestroy();
                //turret.Kill(BaseNetworkable.DestroyMode.Gib);
                if (this)
                    Destroy(this);
            }
            ...
            {
                return !requiresAmmo ? ItemManager.CreateByItemID(-742865266) : turret.inventory.itemList.FirstOrDefault(item => item.info.itemid == -742865266 || item.info.itemid == 1638322904 || item.info.itemid == -1841918730);
            }
            ...
            {
                // Rocket Launcher
                launcher_1 = ItemManager.CreateByItemID(442886268, 1, rocketLauncherSkin).Drop(turret.transform.position, Vector3.zero).GetComponent<DroppedItem>();
                ...
                // rocket_1
                rocket_1 = ItemManager.CreateByItemID(-742865266).Drop(turret.transform.position, Vector3.zero).GetComponent<DroppedItem>();
                ...
                // rocket_2
                rocket_2 = ItemManager.CreateByItemID(-742865266).Drop(turret.transform.position, Vector3.zero).GetComponent<DroppedItem>();
            ...
            private ServerProjectile CreateRocket(Vector3 launchPos, Item rocketItem)
            {
                ...
                BaseEntity rocket;
                switch (rocketItem.info.shortname)
                {
                ...
                if (target != null)
                {
                    ...
                    rocket.GetComponent<ServerProjectile>()?.InitializeVelocity(target.transform.position);
                }
                else
                {
                    rocket.transform.LookAt(turret.muzzlePos.position + turret.muzzlePos.forward.normalized * 10);
                    rocket.GetComponent<ServerProjectile>()?.InitializeVelocity(turret.muzzlePos.position + turret.muzzlePos.forward * 10);
                }
        ...
        {
            private float aimRadius = plugin.aaLockonRadius;
            private float fireRate = plugin.aaFireRate;
            private float rocketSpeed = plugin.aarocketSpeed;
            private float minHitDistance = plugin.aaminHitDistance;
            private float lockonTime = plugin.aaLockonTime;
            private uint rocketLauncherSkin = plugin.aaLauncherSkin;
            private float explosionRadius = plugin.aaexplosionRadius;
            private int soundEveryxUpdates = plugin.aalockonSoundEveryxUpdate;
            private float damage = plugin.damageToHeli;
            private bool requiresAmmo = plugin.aaRequiresAmmo;
            ...
            */
            ...
            private void InitAesthetics()
            {
                ...
                // computer
                computer = ItemManager.CreateByItemID(1523195708).Drop(turret.transform.position, Vector3.zero).GetComponent<DroppedItem>();
                ...
                // camera_1
                camera_1 = ItemManager.CreateByItemID(634478325).Drop(turret.transform.position, Vector3.zero).GetComponent<DroppedItem>();
                ...
                // camera_2
                camera_2 = ItemManager.CreateByItemID(634478325).Drop(turret.transform.position, Vector3.zero).GetComponent<DroppedItem>();
            ...
            private void FireRocket(Item rocketItem)
            {
                ...
                rocket.InitializeVelocity(turret.muzzlePos.transform.forward);
                Effect.server.Run("assets/prefabs/npc/patrol helicopter/effects/rocket_fire.prefab", turret.muzzlePos.position);
                rocket.gameObject.AddComponent<HeatRocket>().target = turret.target;
            }
        ...
        {
            private float aimRadius = plugin.jrocketLockonRadius;
            private float fireRate = plugin.jrocketFireRate;
            private float rocketSpeed = plugin.jrocketSpeed;
            private float minHitDistance = plugin.jminHitDistance;
            private float lockonTime = plugin.jrocketLockonTime;
            private uint rocketLauncherSkin = plugin.jrocketLauncherSkin;
            private float explosionRadius = plugin.jexplosionRadius;
            private int soundEveryxUpdates = plugin.jlockonSoundEveryxUpdate;
            private bool requiresAmmo = plugin.jrequiresAmmo;
            private bool seekTarget = plugin.jSeekTarget;
            ...
            private void Awake()
            {
                turret = gameObject.GetComponent<AutoTurret>();
                InitAesthetics();
                if (!plugin.javelinTurrets.Contains(turret))
                    plugin.javelinTurrets.Add(turret);
                turret.sightRange = aimRadius;
                if (!plugin.javelinIDs.Contains(turret.net.ID))
                    plugin.javelinIDs.Add(turret.net.ID);
                ...
                // Collider add
                SphereCollider collider = turret.gameObject.AddComponent<SphereCollider>();
                collider.gameObject.layer = (int)Layer.Reserved1;
                collider.radius = aimRadius;
                collider.isTrigger = true;
            }
            ...
            private void Update()
            {
                if (externalTarget)
                {
                    turret.SetTarget(externalTarget);
                    turret.UpdateAiming();
                }
                if (!turret.HasTarget())
                {
            ...
            private void OnTriggerEnter(Collider col)
            {
                if (col.name != "assets/prefabs/npc/m2bradley/bradleyapc.prefab")
                    return;
                turret.SetTarget(col.GetComponent<BaseCombatEntity>());
                externalTarget = col.GetComponent<BaseCombatEntity>();
                turret.UpdateAiming();
            }
            ...
            private void FireRocket(Item rocketItem)
            {
                ...
                rocket.InitializeVelocity(turret.muzzlePos.transform.forward);
                Effect.server.Run("assets/prefabs/npc/patrol helicopter/effects/rocket_fire.prefab", turret.muzzlePos.position);
                rocket.gameObject.AddComponent<JavelinMissile>().targetEntity = target;
            }
        ...
        private class HeatRocket : MonoBehaviour
        {
            ...
            private void Awake()
            {
                ...
                expl = rocket.GetComponent<TimedExplosive>();
                if (expl)
                    expl.CancelInvoke("Explode");
                expl.SetFuse(float.PositiveInfinity);
            }
            ...
            private void FixedUpdate()
            {
                ...
                if (Vector3.Distance(rocket.transform.position, target.transform.position) < 0.2f)
                    expl.Explode();
            }
        }
        ...
        private class JavelinMissile : MonoBehaviour
        {
            ...
            // Config Shit
            private float verticality = plugin.jHeightBeforeCurve;
            private bool seekTarget = plugin.jSeekTarget;
            ...
            private void Awake()
            {
                ...
                //rocket.speed = rocket.speed / 2;
                expl = rocket.GetComponent<TimedExplosive>();
                expl.SetFuse(float.PositiveInfinity);
                if (expl)
                    expl.CancelInvoke("Explode");
                vertPos = transform.position + new Vector3(0, verticality, 0);
            }
            ...
            private void FixedUpdate()
            {
                ...
                {
                    if (targetEntity)
                        cachedPosition = targetEntity.transform.position;
                    if (Vector3.Distance(rocket.transform.position, vertPos) < 2f)
                    {
                        startTime = Time.time;
                        finishedVertical = true;
                        return;
                    }
                    rocket.InitializeVelocity(vertPos - rocket.transform.position);
                    return;
                }
                if (targetEntity && fracComplete > 1f && seekTarget)
                    cachedPosition = targetEntity.transform.position;
                if (start == false)
                {
                    startPos = rocket.transform.position;
                    endPos = cachedPosition + new Vector3(0, verticality, 0);
                    center = (startPos + endPos) * 0.5f - new Vector3(0, 0.1f, 0);
                    journeyTime = Vector3.Distance(startPos, endPos) / rocket.speed;
                    lastPos = transform.position;
                    start = true;
                }
                fracComplete = (Time.time - startTime) / journeyTime;
                if (fracComplete < 1.1f)
                {
                    Vector3 launcherRelCenter = startPos - center;
                    Vector3 targetRelCenter = endPos - center;
                    lastPos = nextPos;
                    nextPos = Vector3.Slerp(launcherRelCenter, targetRelCenter, fracComplete) + center;
                }
                else
                {
                    lastPos = nextPos;
                    nextPos = cachedPosition;
                }
                //Vector3 velocityPos = lastPos - (lastPos - nextPos).normalized * 2f;
                rocket.InitializeVelocity(nextPos - rocket.transform.position);
                /*
                foreach (BasePlayer player in BasePlayer.activePlayerList)
                {
                    player.SendConsoleCommand("ddraw.sphere", 10f, Color.red, velocityPos, 0.05f);
                    player.SendConsoleCommand("ddraw.line", 10f, Color.red, lastPos, nextPos, 1f);
                }
                */
                if (targetEntity != null)
                {
                    if (Vector3.Distance(transform.position, targetEntity.transform.position) < 0.1f)
                        expl.Explode();
                }
                else
                {
        ...
        private void Init()
        {
            ...
            permission.RegisterPermission(permissionNameJAVELIN, this);
            permission.RegisterPermission(permissionNameUNLIMITED, this);
            LoadVariables();
            //var filter = Oxide.Game.Rust.RustExtension.Filter.ToList();
            //filter.Add("Calling hook CanAcceptItem resulted in a conflict between the following plugins: AlphaLoot - CannotAccept (CanAcceptResult), RocketTurrets (CanAccept (CanAcceptResult))");
        ...
        private void Loaded()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                ["Help Menu line4"] = "- <color=#008B45>/turret javelin</color> (upgrades the turret you are looking at to a javelin turret",
                ["Help Menu line5"] = "- <color=#008B45>/turret cost</color> (displays the cost of upgrading to a rocket turret)",
                ...
                ["Cost Menu Header (javelin)"] = "You will need the following items to upgrade to a javelin turret",
                ["Cost Menu Entry"] = "- <color=#00CD66>{0}</color> x <color=#00CD66>{1}</color>",
                ["Max Turrets Allowed"] = "You already have the maximum number of this turret type that is allowed ({0})",
                ["Downgrade First"] = "You need to downgrade this turret before you change its type!",
                ["AntiAir Toggle Warning"] = "AntiAir turrets do not need to be turned on, they are always scanning!",

            }, this);

            data = Interface.Oxide.DataFileSystem.GetFile("RocketTurrets/TurretData");
            data2 = Interface.Oxide.DataFileSystem.GetFile("RocketTurrets/PlayerData");
        }

        private void Unload()
        {
            foreach (var entry in UnityEngine.Object.FindObjectsOfType<AutoTurret>())
            {
                var rt = entry.GetComponent<RocketTurret>();
                if (rt)
                    rt.UnloadDestroy();
                var at = entry.GetComponent<AntiAirTurret>();
                if (at)
                    at.UnloadDestroy();
                var jt = entry.GetComponent<JavelinTurret>();
                if (jt)
                    jt.UnloadDestroy();
        ...
        {
            LoadData();
            ServerMgr.Instance.StartCoroutine(InitTurrets());
        }
        ...
        private IEnumerator InitTurrets()
        {
            foreach (var entry in UnityEngine.Object.FindObjectsOfType<AutoTurret>())
            {
                ...
                else if (javelinIDs.Contains(entry.net.ID))
                {
                    var jt = entry.GetComponent<JavelinTurret>();
                    if (jt)
                        jt.Destroy();
                    entry.gameObject.AddComponent<JavelinTurret>();
                    javelinTurrets.Add(entry);
                    yield return new WaitForSeconds(0.1f);
                }
            }
        }
        ...
        {
            if (!aaRequireBuildingRightsToAccessInventory) return;
            if (!(entity is AutoTurret)) return;
            AutoTurret aa = (AutoTurret)entity;
            if (!antiairTurrets.Contains(aa)) return;
            if (!player.CanBuild())
        ...
        {
            if (!antiairTurrets.Contains(turret)) return null;
            Collider[] x = Physics.OverlapSphere(turret.transform.position, 3f);
            foreach(Collider entry in x)
            {
                if (entry.GetComponent<BasePlayer>() == null) continue;
                BasePlayer player = entry.GetComponent<BasePlayer>();
                player.ChatMessage(msg("AntiAir Toggle Warning", player.UserIDString));
            }
            return false;
        }
        ...
        private object CanAcceptItem(ItemContainer container, Item item)
        {
            ...
            if (entity.ShortPrefabName != "autoturret_deployed")
                return null;
            RocketTurret rt = entity.GetComponent<RocketTurret>(); AntiAirTurret at = entity.GetComponent<AntiAirTurret>();
            if (rt != null)
            {
            ...
            if (container.entityOwner.GetComponent<LootContainer>() == null && item.info.shortname != "ammo.rocket.basic" && item.info.shortname != "ammo.rocket.fire" && item.info.shortname != "ammo.rocket.hv")
                return ItemContainer.CanAcceptResult.CannotAcceptRightNow;
            return null;
        }
        ...
        {
            if (!returnCostOnPickup)
                return;
            AutoTurret turret = entity.GetComponent<AutoTurret>();
            if (!turret)
                return;
            RocketTurret rt = turret.GetComponent<RocketTurret>();
            AntiAirTurret at = turret.GetComponent<AntiAirTurret>();
            if (rt != null)
                ReturnItems(entity.transform.position + new Vector3(0, 0.5f, 0), rocketUpgradeCost);
            else if (at != null)
                ReturnItems(entity.transform.position + new Vector3(0, 0.5f, 0), antiAirUpgradeCost);
        }
        ...
        private void OnEntityKill(BaseNetworkable entity)
        {
            ...
            RocketTurret rt = turret.GetComponent<RocketTurret>();
            AntiAirTurret at = turret.GetComponent<AntiAirTurret>();
            JavelinTurret jt = turret.GetComponent<JavelinTurret>();
            if (!rt && !at && !jt)
                return;
            if (rt)
            {
                if (turretNumbers.ContainsKey(turret.OwnerID.ToString()))
                    turretNumbers[turret.OwnerID.ToString()].RocketTurrets--;
                rocketTurrets.Remove(turret);
            }
            else if (at)
            {
                if (turretNumbers.ContainsKey(turret.OwnerID.ToString()))
                    turretNumbers[turret.OwnerID.ToString()].AntiAirTurrets--;
                antiairTurrets.Remove(turret);
            }
            if (jt)
            {
                if (turretNumbers.ContainsKey(turret.OwnerID.ToString()))
                    turretNumbers[turret.OwnerID.ToString()].JavelinTurrets--;
                javelinTurrets.Remove(turret);
            }
        ...
        private void RocketTurretCMD(BasePlayer player, string command, string[] args)
        {
            ...
            if (!turretNumbers.ContainsKey(player.UserIDString))
                turretNumbers.Add(player.UserIDString, new PlayerData { AntiAirTurrets = 0, RocketTurrets = 0, JavelinTurrets = 0});
            if (args.Length == 0)
            {
                DoHelpMenu(player);
                return;
            ...
            switch (args[0])
            {
                ...
                case "rocket":
                    {
                        RaycastHit hit;
                        if (!Physics.Raycast(player.eyes.HeadRay(), out hit, Mathf.Infinity, collLayers))
                        {
                        ...
                        if (at || jt)
                        {
                            player.ChatMessage(msg("Downgrade First", player.UserIDString));
                            return;
                        }
                        if (turretNumbers[player.UserIDString].RocketTurrets >= maxNumberAllowed)
                        {
                            player.ChatMessage(string.Format(msg("Max Turrets Allowed", player.UserIDString), maxNumberAllowed));
                            return;
                        }
                        ...
                        if (!permission.UserHasPermission(player.UserIDString, permissionNameUNLIMITED))
                            turretNumbers[player.UserIDString].RocketTurrets++;
                        break;
                    }
                case "normal":
                    {
                        ...
                        AutoTurret turret = entity.GetComponent<AutoTurret>();
                        if (!rocketTurrets.Contains(turret) && !antiairTurrets.Contains(turret) && !javelinTurrets.Contains(turret))
                        {
                        ...
                        JavelinTurret jt = turret.gameObject.GetComponent<JavelinTurret>();
                        Effect.server.Run("assets/prefabs/locks/keypad/effects/lock.code.updated.prefab", turret.transform.position);

                        if (returnCostOnDowngrade)
                        {
                            if (rt != null)
                                ReturnItems(entity.transform.position + new Vector3(0, 1, 0), rocketUpgradeCost);
                            else if (at != null)
                                ReturnItems(entity.transform.position + new Vector3(0, 1, 0), antiAirUpgradeCost);
                            else if (jt != null)
                                ReturnItems(entity.transform.position + new Vector3(0, 1, 0), javelinUpgradeCost);
                        }
                        ...
                        if (rocketTurrets.Contains(turret))
                            ...
                            antiairTurrets.Remove(turret);
                        else if (javelinTurrets.Contains(turret))
                            javelinTurrets.Remove(turret);
                ...
                case "antiair":
                    {
                        ...
                        if (turretNumbers[player.UserIDString].AntiAirTurrets >= aaMaxNumberAllowed)
                        {
                            player.ChatMessage(string.Format(msg("Max Turrets Allowed", player.UserIDString), aaMaxNumberAllowed));
                            return;
                        }
                        ...
                        turret.inventory.onlyAllowedItem = null;
                        if (!permission.UserHasPermission(player.UserIDString, permissionNameUNLIMITED))
                            turretNumbers[player.UserIDString].AntiAirTurrets++;
                        break;
                    }
                case "javelin":
                {
                    ...
                    if (!permission.UserHasPermission(player.UserIDString, permissionNameJAVELIN))
                    {
                        player.ChatMessage(msg("No Permission", player.UserIDString));
                        return;
                    }
                    ...
                    if (turretNumbers[player.UserIDString].JavelinTurrets >= jmaxNumberAllowed)
                    {
                        player.ChatMessage(string.Format(msg("Max Turrets Allowed", player.UserIDString), jmaxNumberAllowed));
                        return;
                    }
                    if (javelinTurrets.Contains(turret))
                    {
                        player.ChatMessage(msg("Already A Rocket Turret"));
                        return;
                    }
                    if (!CanUpgade(player.inventory.containerMain, player.inventory.containerBelt, "javelin"))
                    {
                        player.ChatMessage(msg("Invalid Items", player.UserIDString));
                        return;
                    }
                    javelinTurrets.Add(turret);
                    turret.gameObject.AddComponent<JavelinTurret>();
                    Effect.server.Run("assets/prefabs/locks/keypad/effects/lock.code.updated.prefab", turret.transform.position);
                    DropAmmo(turret);
                    turret.inventory.onlyAllowedItem = null;
                    if (!permission.UserHasPermission(player.UserIDString, permissionNameUNLIMITED))
                            turretNumbers[player.UserIDString].JavelinTurrets++;
                    return;
                }
            }
        }
        ...
        {
            foreach (var _entry in dic)
                ItemManager.CreateByName(_entry.Key, (int)_entry.Value).Drop(dropPos, Vector3.zero, UnityEngine.Random.rotation);
        }

        private static void DropAmmo(AutoTurret turret)
        {
            List<Item> items = turret.inventory.itemList.ToList();
            foreach (var entry in items)
                entry?.Drop(turret.transform.position + new Vector3(0f, 1f, 0f), Vector3.zero);
        }
        ...
        private void DoHelpMenu(BasePlayer player)
        {
            ...
            x.AppendLine(msg("Help Menu line5", player.UserIDString));
        ...
        private void DoItemMenu(BasePlayer player)
        {
                ...
                foreach (var entry in rocketUpgradeCost)
                {
                    foreach (ItemDefinition item in ItemManager.itemList)
                        if (item.shortname == entry.Key)
                            x.AppendLine(string.Format(msg("Cost Menu Entry", player.UserIDString), item.displayName.english, entry.Value));
                }
                x.AppendLine();
            }
            if (permission.UserHasPermission(player.UserIDString, permissionNameANTIAIR))
            ...
            if (permission.UserHasPermission(player.UserIDString, permissionNameJAVELIN))
            {
                x.AppendLine(msg("Cost Menu Header (javelin)", player.UserIDString));
                foreach (var entry in javelinUpgradeCost)
                {
                    foreach(ItemDefinition item in ItemManager.itemList)
                        if (item.shortname == entry.Key)
                            x.AppendLine(string.Format(msg("Cost Menu Entry", player.UserIDString), item.displayName.english, entry.Value));
        ...
        private bool CanUpgade(ItemContainer container, ItemContainer container2, string type)
        {
            ...
            if (itemsNeeded.Any(entry => entry.Value > 0))
                return false;
            RemoveItems(cache);
            return true;
        }
