...
{
    [Info("Friends", "OxideBro/NT", "1.1.01", ResourceId = 6)]
    public class Friends : CovalencePlugin
    {
        ...
        void OnEntityTakeDamage(BaseCombatEntity vic, HitInfo info)
        {
            try
            {
                if (vic is BasePlayer && info?.Initiator != null && info.Initiator is BasePlayer && vic != info.Initiator)
                {
                    BasePlayer VicPlayer = vic as BasePlayer;
                    if (VicPlayer == null) return;
                    BasePlayer AttPlayer = info.Initiator as BasePlayer;
                    if (AttPlayer == null) return;
                    if (HasFriend(AttPlayer.userID, VicPlayer.userID))
                    {
                        if (!FriendsData[AttPlayer.userID].AttackFriend)
                        {
                            info.damageTypes?.ScaleAll(0f);
                            AttPlayer.ChatMessage(string.Format(Messages["Attack"], VicPlayer.displayName));
                        }
                    }
                }
            }
