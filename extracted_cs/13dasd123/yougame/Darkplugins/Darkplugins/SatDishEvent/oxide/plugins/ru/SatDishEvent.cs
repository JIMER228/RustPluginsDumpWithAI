...
{
    [Info("SatDishEvent", "Sempai#3239", "2.0.4")]
    internal class SatDishEvent : RustPlugin
    {
                        ...
                        new PresetConfig
                        {
                            ...
                            Config = new NpcConfig
                            {
                                ...
                                BeltItems = new HashSet<NpcBelt>
                                {
                                    ...
                                    new NpcBelt { ShortName = "grenade.f1", Amount = 1, SkinID = 0, Mods = new HashSet<string>() }
                                },
                                Kit = ""
                            },
                            TypeLootTable = 5,
        ...
        protected override void LoadDefaultMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                ["GUI"] = "The bioweapons transaction will end in {0} sec.",
            ...
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                ["GUI"] = "Сделка по продаже биологического оружия закончится через {0} сек.",
            ...
            if (_sound != null && _sound.Count != 0)
            {
                ...
                foreach (byte[] data in _sound.ContainsKey(language) ? _sound[language] : _sound["en"])
                {
                    if (Network.Net.sv.write.Start())
                    {
                        ...
                        Network.Net.sv.write.UInt32(_controller.Dummy.net.ID);
        ...
        private void OnEntityKill(Telephone phone) { if (phone != null && phone == _controller.Phone) _controller.SpawnPhone(); }
        ...
        private void Start()
        {
            ...
            {
                Interface.Oxide.CallHook("OnSatDishEventStart");
                Subscribes();
                ...
                if (_config.PveMode.Pve && plugins.Exists("PveMode"))
                {
                    ...
                    HashSet<uint> tanks = _config.IsAdditionalBradley ? new HashSet<uint> { _controller.AddBradley.net.ID } : new HashSet<uint>();
                    PveMode.Call("EventAddPveMode", Name, config, _controller.transform.position, Radius, _controller.Crates.Select(x => x.net.ID), _controller.Scientists.Select(x => x.net.ID), tanks, new HashSet<ulong>(), null);
                }
        ...
        internal class ControllerSatDishEvent : FacepunchBehaviour
        {
            ...
            internal void UpdateCh47()
            {
                ...
                else if (StageCh47 == 5)
                {
                    ...
                    if (Ch47.transform.position.y - (LandingCh47Pos.y + 7.5f) < 1f)
                    {
                        ...
                        if (_ins._config.PveMode.Pve && _ins.plugins.Exists("PveMode")) _ins.PveMode.Call("EventAddScientists", _ins.Name, Zombies.Select(x => x.net.ID));
                        StageCh47++;
                    }
                }
                else if (StageCh47 == 6)
            ...
            private void UpdateBradley()
            {
                ...
                if (Bradley.transform.position.y - LandingBradleyPos.y < 1f)
                {
                    ...
                    foreach (BaseEntity parachute in _parachutes) if (parachute.IsExists()) parachute.Kill();
                    if (_ins._config.PveMode.Pve && _ins.plugins.Exists("PveMode")) _ins.PveMode.Call("EventAddTanks", _ins.Name, new HashSet<uint> { Bradley.net.ID });
                    CancelInvoke(UpdateBradley);
                }
            }
            ...
            {
                Phone = GameManager.server.CreateEntity("assets/prefabs/voiceaudio/telephone/telephone.deployed.prefab", GetGlobalPosition(new Vector3(56.195f, 16.855f, -6.724f))) as Telephone;
        ...
        private object OnCustomLootNPC(uint netID)
        {
            ...
            if (_controller.Zombies.Any(x => x.IsExists() && x.net.ID == netID))
            {
        ...
        private void OnEntitySpawned(HackableLockedCrate crate)
        {
            if (crate != null && Vector2.Distance(new Vector2(crate.transform.position.x, crate.transform.position.z), new Vector2(_controller.DropCratePos.x, _controller.DropCratePos.z)) < 1f)
            {
                ...
                if (_config.PveMode.Pve && plugins.Exists("PveMode")) PveMode.Call("EventAddCrates", Name, new HashSet<uint> { crate.net.ID });
        ...
        private object CanCh47SpawnNpc(HackableLockedCrate crate)
        {
            if (_controller == null) return null;
            if (Vector3.Distance(crate.transform.position, _controller.transform.position) < Radius) return true;
            else return null;
        }
        #endregion BetterNpc
