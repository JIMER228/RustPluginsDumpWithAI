...
{
    [Info("Raidable Bases", "nivex", "2.6.6")]
    [Description("Create fully automated raidable bases with npcs.")]
    class RaidableBases : RustPlugin
    {
        [PluginReference]
        private Plugin
            DangerousTreasures, ZoneManager, IQEconomic, Economics, ServerRewards, GUIAnnouncements, AdvancedAlerts, Archery,
        ...
        private List<string> arguments { get; set; } = new List<string> { "add", "remove", "list", "clean", "easy", "med", "medium", "hard", "expert", "nightmare", "0", "1", "2", "3", "4", "maintained", "scheduled", "buyable", "toggle" };
        ...
        private List<PasteLoadInfo> loadCoroutines = new List<PasteLoadInfo>();
        private static Dictionary<string, PasteData> _pasteData = new Dictionary<string, PasteData>();
        ...
        public class PasteData
        {
            ...
            {
                PasteData pasteData;
                if (!_pasteData.TryGetValue(baseName, out pasteData))
                {
                    _pasteData[baseName] = pasteData = new PasteData();
                }
                return pasteData;
            }
        }

        public class PasteLoadInfo
        {
            public Coroutine coroutine;
            public RandomBase rb;
            public PasteLoadInfo(Coroutine coroutine, RandomBase rb)
            {
        ...
        {
            public List<uint> UID { get; set; } = new List<uint>();
            public TemporaryData() { }
            public void CleanupTemporaryFiles(List<uint> uids)
            {
                var entities = new List<BaseEntity>();
                foreach (uint uid in uids)
                {
                    var entity = BaseNetworkable.serverEntities.Find(uid) as BaseEntity;
                    if (entity.IsKilled()) UID.Remove(uid);
                    else entities.Add(entity);
                }
                if (entities.Count > 0)
                {
                    Instance.UndoLoop(entities, 10, true, false, true);
                }
        ...
        public class StoredData
        {
            ...
            public Dictionary<ulong, BuyableInfo> BuyableCooldowns { get; set; } = new Dictionary<ulong, BuyableInfo>();
        ...
        public class RandomBase
        {
            ...
            public RandomBase(string BaseName, BaseProfile Profile, Vector3 Position, RaidableType Type, RaidableSpawns spawns, Payments payments)
            {
            ...
            public bool isBuyableEvent => payments != null && payments.position != Vector3.zero;
            ...
            {
                if (!Sorted && isBuyableEvent && config.Settings.Buyable.Closest)
                {
                    Message(owner, "BuyBaseLocate");
                    var positions = spawns.GetLocations(options.Water.CacheType).ToList();
                    positions.Sort(new SpawnComparer(payments.position));
                    if (options.Water.CacheType == CacheType.Seabed)
                    {
                        spawns.Seabed = new HashSet<RaidableSpawnLocation>(positions);
                    }
                    else spawns.Spawns = new HashSet<RaidableSpawnLocation>(positions);
                    Sorted = true;
                }
                else Sorted = false;
            }
        ...
        public class BuyableInfo
        {
            ...
            private static double GetTimeRemaining(ulong userid, RaidableMode mode)
            {
                var diff = 
                    mode == RaidableMode.Easy ? config.Settings.Buyable.Cooldowns.Easy : 
                    mode == RaidableMode.Medium ? config.Settings.Buyable.Cooldowns.Medium : 
                    mode == RaidableMode.Hard ? config.Settings.Buyable.Cooldowns.Hard : 
                    mode == RaidableMode.Expert ? config.Settings.Buyable.Cooldowns.Expert :
                    config.Settings.Buyable.Cooldowns.Nightmare;
                if (diff.Cooldown <= 0) return 0;
                BuyableInfo bi;
                if (!data.BuyableCooldowns.TryGetValue(userid, out bi)) return 0f;
                var dateString =
                    mode == RaidableMode.Easy ? bi.Easy :
                    mode == RaidableMode.Medium ? bi.Medium :
                    mode == RaidableMode.Hard ? bi.Hard :
                    mode == RaidableMode.Expert ? bi.Expert : bi.Nightmare;
                DateTime date;
                if (!DateTime.TryParse(dateString, out date)) return 0;
                double time = date.Subtract(DateTime.Now).TotalSeconds;
                return time > 0 ? time : 0;
            }
            public static double GetTimeRemaining(BasePlayer buyer, RaidableMode mode)
            {
                double time = GetTimeRemaining(buyer.userID, mode);
                if (time > 0)
                {
                    Message(buyer, "BuyCooldown", Math.Ceiling(time));
                    return time;
                }
                return 0;
            }
            public static double GetTimeRemaining(ulong userid)
            {
                foreach (RaidableMode mode in GetRaidableModes())
                {
                    double time = GetTimeRemaining(userid, mode);
                    if (time > 0)
                    {
                        return time;
                    }
                }
                data.BuyableCooldowns.Remove(userid);
                return 0;
            }
        }
        ...
        {
            internal static Dictionary<RaidableType, RaidableSpawns> Spawns { get; set; } = new Dictionary<RaidableType, RaidableSpawns>();
            internal static Coroutine gridCoroutine { get; set; }
            internal static Coroutine fileCoroutine { get; set; }
            ...
            public static void Clear()
            {
                ...
                {
                    RaidableSpawns rs;
                    if (Spawns.TryGetValue(type, out rs))
                    {
                        rs.Seabed.Clear();
                        rs.Spawns.Clear();
                        rs.Cached.Clear();
            ...
            private static IEnumerator LoadFiles()
            {
            ...
            private static IEnumerator GenerateGrid() // Credits to Jake_Rich for helping me with this!
            {
                ...
                {
                    Puts("Grid has failed initialization. Your configuration does not have any spawn options for beaches, inland or seabed enabled!");
                    gridCoroutine = null;
                    gridStopwatch.Stop();
                    yield break;
                }

                float zeroDist = Mathf.Clamp(config.Settings.Management.Vector3ZeroDistance, 0f, World.Size / 2f);
                ...
                for (float x = minPos; x < maxPos; x += CELL_SIZE)
                {
                    for (float z = minPos; z < maxPos; z += CELL_SIZE)
                    {
                        ...
                        if (InRange(position, Vector3.zero, zeroDist))
                        {
                            continue;
                        }
                ...
                if (spawns.Seabed.Count > 0) Puts(mx("Initialized Grid Sea", null, spawns.Seabed.Count));
            }
        ...
        {
            internal const float spawnDelay = 0.1f;
            internal const float pauseDelay = 1.0f;
            internal const int elementLimit = 20;
            internal const int valueLimit = 5;
            ...
            internal const int layers = Layers.Mask.Player_Server | Layers.Mask.Construction | Layers.Mask.Deployed | Layers.Mask.Ragdoll;
            ...
            public class DebugMessages
            {
                ...
                public string Add(string element, object obj = null)
                {
                    ...
                    {
                        if (elementLimit > 0 && _elements.Count >= elementLimit)
                        {
                    ...
                    {
                        if (valueLimit > 0 && info.values.Count >= valueLimit)
                        {
            ...
            private void CompletePurchase(RandomBase rb)
            {
                if (rb.type == RaidableType.Purchased)
                {
                    if (rb.owner != null)
                    {
                        var grid = FormatGridReference(rb.Position);

                        Message(rb.owner, "BuyBaseSpawnedAt", rb.Position, grid);

                        if (config.EventMessages.AnnounceBuy)
                        {
                            foreach (var target in BasePlayer.activePlayerList)
                            {
                                QueueNotification(target, "BuyBaseAnnouncement", rb.owner.displayName, rb.Position, grid);
                            }
                        }

                        Puts(mx("BuyBaseAnnouncementConsole", null, rb.owner.displayName, rb.Profile.Options.Mode, rb.BaseName, rb.Position, grid));
                    }

                    config.Settings.Buyable.Cooldowns.Set(rb.userid, rb.options.Mode);
                }
            }
            ...
            private IEnumerator FindEventPosition()
            {
                ...
                while (true)
                {
                    ...
                    {
                        yield return CoroutineEx.waitForSeconds(pauseDelay);
                        continue;
                    }
                    ...
                    while (spq.hasSpawns)
                    {
                        ...
                        if (++checks >= spawnChecks)
                        {
                            if (spq.type != RaidableType.Purchased && queue.Exists(sp => sp.type == RaidableType.Purchased))
                            {
                                break;
                            }
                            checks = 0;
                        ...
                        if (CanSpawnCustomBuyable(spq, vector))
                        {
                            yield return CoroutineEx.waitForSeconds(pauseDelay);
                            break;
                        }
            ...
            private void CheckSpawner(RandomBase spq)
            {
                if (!RaidableBase.IsSpawnerBusy)
                {
                    ...
                    {
                        spq.payments?.Refund();
                        QueueNotification(spq.owner, "CannotFindPosition");
                    }
            ...
            {
                if (!spq.spawns.IsCustomSpawn && SpawnsController.IsObstructed(vector, spq.pasteData.radius, Mathf.Clamp(spq.options.LandLevel, -3f, 3f), spq.options.Setup.ForcedHeight, spq.options.Water.SpawnOnSeabed))
                {
                    Messages.Add("Area is obstructed", vector);
                    spq.spawns.RemoveNear(vector, spq.protectionRadius / 2f, spq.options.Water.SpawnOnSeabed ? CacheType.Seabed : CacheType.Temporary, spq.type);
                    return true;
                }
                return false;
            }
            ...
            {
                yield return Instance.SetupCopyPasteObstructionRadius(spq.BaseName, spq.options.ProtectionRadii.Obstruction == -1 ? 0f : GetObstructionRadii(spq.options.ProtectionRadii, RaidableType.None));
            }

            private bool IsAreaSafe(RandomBase spq, RaidableSpawnLocation rsl, Vector3 vector, out CacheType cacheType)
            {
                if (!SpawnsController.IsAreaSafe(vector, spq.safeRadius, spq.buildRadius, layers, out cacheType, spq.type, spq.options.CustomSpawns))
                {
            ...
            {
                if (!spq.spawns.IsCustomSpawn && spq.options.Setup.ForcedHeight == -1f && spq.options.Water.Seabed <= 0f && SpawnsController.IsSubmerged(spq.options.Water, rsl))
                {
            ...
            private bool CanSpawnCustomBuyable(RandomBase spq, Vector3 vector)
            {
                if (spq.type == RaidableType.Purchased && spq.spawns.IsCustomSpawn && (config.Settings.Buyable.Ignore || config.Settings.Buyable.SafeRadius > 0f))
                {
                    if (config.Settings.Buyable.SafeRadius <= 0f)
                    {
                        Messages.Add("Ignoring safe checks and spawning Buyable base", vector);
                        Spawn(spq, vector);
                        return true;
                    }
                    else spq.safeRadius = config.Settings.Buyable.SafeRadius;
                }
                return false;
            }
            ...
            private bool CanSpawnCustom(RandomBase spq, Vector3 vector)
            {
                if (spq.spawns.IsCustomSpawn && (spq.options.CustomSpawns.Ignore || spq.options.CustomSpawns.SafeRadius > 0f))
                {
                    if (spq.options.CustomSpawns.SafeRadius <= 0f)
                    {
                        Messages.Add($"Ignoring safe checks and spawning {spq.options.Mode} base", vector);
                        Spawn(spq, vector);
                        return true;
                    }
                    else spq.safeRadius = spq.options.CustomSpawns.SafeRadius;
                }
                return false;
            }
            ...
            {
                if (!spq.spawns.IsCustomSpawn && !SpawnsController.TopologyChecks(vector, spq.protectionRadius, spq.options.Water.SpawnOnSeabed, out topology))
                {
        ...
        private static float GetObstructionRadii(BuildingOptionsProtectionRadius radii, RaidableType type)
        {
        ...
        public class MaintainedController
        {
            ...
            public void StartCoroutine()
            {
                ...
                Instance.timer.Once(0.2f, () => _coroutine = ServerMgr.Instance.StartCoroutine(MaintainCoroutine()));
            }
            ...
            private IEnumerator MaintainCoroutine()
            {
                ...
                while (IsInstanceValid)
                {
                    if (Enabled)
                    {
                        ...
                        if (onlinePlayers < config.Settings.Maintained.PlayerLimit)
                        {
                            Instance.Queues.Messages.Print($"Maintained: Insufficient amount of players online {onlinePlayers}/{config.Settings.Maintained.PlayerLimit}");
                            yield return CoroutineEx.waitForSeconds(5f);
                        }
                        else if (onlinePlayers > config.Settings.Maintained.PlayerLimitMax)
                        {
                            Instance.Queues.Messages.Print($"Maintained: Too many players online {onlinePlayers}/{config.Settings.Maintained.PlayerLimitMax}");
                            yield return CoroutineEx.waitForSeconds(5f);
                        }
                        else if (RaidableBase.IsSpawnerBusy || Instance.Raids.Exists(raid => raid.IsDespawning || raid.IsLoading))
                        {
                            Instance.Queues.Messages.Print("Maintained: Waiting for a base to finish its task");
                            yield return CoroutineEx.waitForSeconds(5f);
                        }
                        else if (config.Settings.Maintained.Max > 0 && RaidableBase.Get(RaidableType.Maintained) >= config.Settings.Maintained.Max)
                        {
                            Instance.Queues.Messages.Print("Maintained: The max amount of maintained events are spawned");
                            yield return CoroutineEx.waitForSeconds(5f);
                        }
                        else if (!Instance.IsCopyPasteLoaded(null))
                        {
                            yield return CoroutineEx.waitForSeconds(60f);
                        }
                        else if (SaveRestore.IsSaving)
                        {
                            Instance.Queues.Messages.Print("Maintained: Server saving");
                        ...
                        {
                            if (!IsModeValid(mode = Instance.GetRandomDifficulty(RaidableType.Maintained)))
                            {
                            ...
                            Instance.SpawnRandomBase(RaidableType.Maintained, mode);
                            yield return CoroutineEx.waitForSeconds(1f); 
                            ...
                            if (Instance.IsBusy(out pastedLocation))
                            {
                                Instance.Queues.Messages.Print("Maintained: Waiting for base to be setup", pastedLocation);
                            }

                            yield return new WaitWhile(() => RaidableBase.IsSpawnerBusy);
                            Instance.Queues.Messages.Print("Maintained: Base has been setup");

                            Instance.Queues.Messages.Print($"Maintained: Waiting {timeBetweenSpawns} seconds");
        ...
        public class ScheduledController
        {
            ...
            public void StartCoroutine()
            {
                ...
                Instance.timer.Once(0.2f, () => _coroutine = ServerMgr.Instance.StartCoroutine(ScheduleCoroutine()));
            }
            ...
            private IEnumerator ScheduleCoroutine()
            {
                ...
                while (IsInstanceValid)
                {
                    if (Enabled)
                    {
                        ...
                        if (onlinePlayers < config.Settings.Schedule.PlayerLimit)
                        {
                            Instance.Queues.Messages.Print($"Scheduled: Insufficient amount of players online {onlinePlayers}/{config.Settings.Schedule.PlayerLimit}");
                            yield return CoroutineEx.waitForSeconds(5f);
                        }
                        else if (onlinePlayers > config.Settings.Schedule.PlayerLimitMax)
                        {
                            Instance.Queues.Messages.Print($"Scheduled: Too many players online {onlinePlayers}/{config.Settings.Schedule.PlayerLimitMax}");
                            yield return CoroutineEx.waitForSeconds(5f);
                        }
                        else if (RaidableBase.IsSpawnerBusy || Instance.Raids.Exists(raid => raid.IsDespawning || raid.IsLoading))
                        {
                            Instance.Queues.Messages.Print($"Scheduled: Waiting for a base to finish its task");
                            yield return CoroutineEx.waitForSeconds(5f);
                        }
                        else if (config.Settings.Schedule.Max > 0 && RaidableBase.Get(RaidableType.Scheduled) >= config.Settings.Schedule.Max)
                        {
                            Instance.Queues.Messages.Print($"Scheduled: The max amount of scheduled events are spawned");
                            yield return CoroutineEx.waitForSeconds(5f);
                        }
                        else if (GridController.GetRaidTime() > 0)
                        {
                            Instance.Queues.Messages.Print($"Scheduled: {FormatTime(GridController.GetRaidTime())} before next event");
                            yield return CoroutineEx.waitForSeconds(5f);
                        }
                        else if (Instance.Queues.queue.Exists(sp => sp.type == RaidableType.Purchased))
                        {
                        ...
                        else
                        {
                            while (RaidableBase.Get(RaidableType.Scheduled) < config.Settings.Schedule.Max && MaxOnce())
                            {
                                ...
                                {
                                    if (!IsModeValid(mode = Instance.GetRandomDifficulty(RaidableType.Scheduled)))
                                    {
                                        Instance.Queues.Messages.PrintLast();
                                        yield return CoroutineEx.waitForSeconds(1f);
                                        continue;
                                    ...
                                    Instance.SpawnRandomBase(RaidableType.Scheduled, mode);
                                    yield return CoroutineEx.waitForSeconds(1f); 
                                    yield return new WaitWhile(() => Instance.Queues.Any);
                                    ...
                                    if (Instance.IsBusy(out pastedLocation))
                                    {
                                        Instance.Queues.Messages.Print("Scheduled: Waiting for base to be setup", pastedLocation);
                                    }

                                    yield return new WaitWhile(() => RaidableBase.IsSpawnerBusy);
                                    Instance.Queues.Messages.Print("Scheduled: Base has been setup");

                                    Instance.Queues.Messages.Print($"Scheduled: Waiting {timeBetweenSpawns} seconds");
                                    yield return CoroutineEx.waitForSeconds(timeBetweenSpawns);
                                }
                                else if (Instance.Queues.Messages.Any())
                                {
        ...
        {
            public HashSet<RaidableSpawnLocation> Seabed { get; set; } = new HashSet<RaidableSpawnLocation>();
            ...
            public RaidableSpawnLocation GetRandom(BuildingWaterOptions options)
            {
                ...
                if (Seabed.Count > 0 && options.Seabed > 0f && UnityEngine.Random.Range(0f, 100f) <= options.Seabed && (rsl = GetSeabed(options, config.Settings.Buyable.Closest)) != null)
                {
                    options.SpawnOnSeabed = true;
                }
                else
                {
                    rsl = config.Settings.Buyable.Closest ? Spawns.ElementAt(0) : Spawns.GetRandom();
            ...
            public float RemoveNear(Vector3 target, float radius, CacheType cacheType, RaidableType type)
            {
                ...
                remove.ForEach(rsl => locations.Remove(rsl));
        ...
        public class HumanoidBrain : ScientistBrain
        {
            ...
            public void Init()
            {
                ...
                if (isStationary)
                {
                    Senses.nextKnownPlayersLOSUpdateTime = float.PositiveInfinity;
                    Senses.nextUpdateTime = float.PositiveInfinity;
                    Senses.knownPlayersLOSUpdateInterval = 0.35f;
                    InvokeRepeating(UpdateKnownPlayers, 0.35f, 0.35f);
                }
                IdentifyWeapon();
            ...
            private bool IsVisibleTest(BasePlayer player)
            {
                if (GamePhysics.LineOfSight(player.eyes.center, player.eyes.position, 1218519041)) //10551296 2162688
                {
                    if (!npc.IsVisible(player.eyes.HeadRay(), 1218519041, Senses.TargetLostRange))
                    {
                        return npc.IsVisible(player.eyes.position, Senses.TargetLostRange);
                    }
                    return true;
                }
                return false;
            ...
            private void TrySetKnown(BasePlayer player)
            {
                if (Senses.ownerAttack != null && !Senses.Memory.IsPlayerKnown(player) && !Senses.Memory.Targets.Contains(player))
                {
                    if (isStationary)
                    {
                        bool flag = IsVisibleTest(player);
                        Senses.Memory.SetLOS(player, flag);
                    }
            ...
            private void UpdateKnownPlayers()
            {
                ...
                {
                    if (raid.intruders.Contains(x.userid) && !ShouldForgetTarget(x.player))
                    {
                        TrySetKnown(x.player);
                    }
                }
                Forget(Senses.MemoryDuration);
            }

            internal void Forget(float secondsOld)
            {
                for (int i = 0; i < Senses.Memory.All.Count; i++)
                {
                    if (!(Time.realtimeSinceStartup - Senses.Memory.All[i].Timestamp > secondsOld))
                    {
                        continue;
                    }
                    BaseEntity entity = Senses.Memory.All[i].Entity;
                    if (entity != null)
                    {
                        if (entity is BasePlayer)
                        {
                            Senses.Memory.Players.Remove(entity);
                        }
                        Senses.Memory.Targets.Remove(entity);
                        Senses.Memory.Threats.Remove(entity);
                        Senses.Memory.Friendlies.Remove(entity);
                        Senses.Memory.LOS.Remove(entity);
                    }
                    Senses.Memory.All.RemoveAt(i);
                    i--;
                }
            }
        ...
        public class RaidableBase : FacepunchBehaviour
        {
            ...
            private List<BaseCombatEntity> _rugs { get; set; } = Pool.GetList<BaseCombatEntity>();
            ...
            private object[] hookObjects
            {
                get
                {
                    return new object[] { Location, (int)Options.Mode, AllowPVP, ID, spawnTime, despawnTime, loadTime, ownerId, GetOwner(), GetRaiders(), GetIntruders(), Entities.ToList(), BaseName, spawnDateTime, despawnDateTime, ProtectionRadius };
                }
            }
            ...
            public void AddEntity(BaseEntity entity)
            {
                ...
                if (!MyInstance.temporaryData.UID.Contains(entity.net.ID))
                {
                    MyInstance.temporaryData.UID.Add(entity.net.ID);
                }
                Entities.Add(entity);
            }
            ...
            internal void CheckEntitiesInSphere()
            {
                ...
                FindEntities(Location, ProtectionRadius, 163848); //174088
            ...
            public void OnEnterRaid(BasePlayer target, bool checkUnderground = true)
            {
                ...
                Interface.CallHook("OnPlayerEnteredRaidableBase", new object[] { target, Location, AllowPVP, (int)Options.Mode, ID, spawnTime, despawnTime, loadTime, ownerId, BaseName, spawnDateTime, despawnDateTime });

                if (config.Settings.Management.PVPDelay > 0)
                {
                    Interface.CallHook("OnPlayerPvpDelayEntry", new object[] { target, (int)Options.Mode, Location, AllowPVP, ID, spawnTime, despawnTime, loadTime, ownerId, BaseName, spawnDateTime, despawnDateTime });
                }
            ...
            public void OnPlayerExit(BasePlayer target, bool skipDelay = true)
            {
                ...
                Interface.CallHook("OnPlayerExitedRaidableBase", new object[] { target, Location, AllowPVP, (int)Options.Mode, ID, spawnTime, despawnTime, loadTime, ownerId, BaseName, spawnDateTime, despawnDateTime });
                ...
                if (config.Settings.Management.PVPDelay > 0)
                {
                    ...
                    if (!Instance.PvpDelay.TryGetValue(id, out ds))
                    {
                        Instance.PvpDelay[id] = ds = new DelaySettings
                        {
                            ...
                            {
                                Interface.CallHook("OnPlayerPvpDelayExpired", new object[] { target, (int)Options.Mode, Location, AllowPVP, ID, spawnTime, despawnTime, loadTime, ownerId, BaseName, spawnDateTime, despawnDateTime });
            ...
            private bool IsHogging(BasePlayer player, bool shouldMessage = true)
            {
                ...
                foreach (var raid in Instance.Raids)
                {
                    ...
                    if (!raid.IsPayLocked && raid.IsOpened && raid.Location != Location && raid.Any(player.userID, false))
                    {
                        if (shouldMessage)
                        {
            ...
            private void CheckRads(BasePlayer player)
            {
                ...
                if (player.RadiationProtection() < Options.Radiation.Protection)
                {
            ...
            private void Protector()
            {
                ...
                foreach (var ri in raiders.Values.ToList())
                {
                    ...
                    if (IsBanned(ri.player))
                    {
                        ...
                        RemovePlayer(ri.player, EjectCode.Banned, Location, ProtectionRadius, Type);
                        continue;
                    }

                    if (IsWearingJetpack(ri.player))
                    {
                        RemovePlayer(ri.player, EjectCode.Mounted, Location, ProtectionRadius, Type, true);
                        continue;
                    }

                    CheckRads(ri.player);

                    if (ri.player == owner || ri.IsAllowed || CanBypass(ri.player))
                    {
                        continue;
                    }
            ...
            public void DestroyEntities()
            {
                if (!IsShuttingDown)
                {
                    MyInstance.UndoLoop(Entities.ToList(), _undoLimit, _undoMounts, _undoStructures, _undoDeployables, hookObjects);
                }
            }
            ...
            {
                return owner ?? GetRaiders().FirstOrDefault();
            }
            ...
            public BasePlayer GetInitiatorPlayer(HitInfo hitInfo)
            {
                if (!(hitInfo.Initiator is BasePlayer))
                {
                    hitInfo.Initiator = hitInfo.Weapon?.creatorEntity ?? hitInfo.WeaponPrefab?.creatorEntity;
                }

                if (!(hitInfo.Initiator is BasePlayer))
                {
                    hitInfo.Initiator = hitInfo.Weapon?.GetParentEntity() ?? hitInfo.WeaponPrefab?.GetParentEntity();
                }
            ...
            //{
            //    if (config.Settings.Management.Restart > 0 && IsShuttingDown)
            //    {
            //        return DateTime.Now.Subtract(spawnDateTime).Minutes <= config.Settings.Management.Restart;
            //    }
            //    return false;
            //}
            ...
            public void AwardRaiders()
            {
                ...
                string con = mx(IsEligible ? "Thieves" : "ThievesDespawn", null, mx($"Mode{Options.Mode}"), posStr, thieves);
            ...
            private int GetRankedLadderPointsForDifficulty(ulong id)
            {
            ...
            private void HandleAwards()
            {
                foreach (var ri in raiders.Values)
                {
                    ...
                    if (config.RankedLadder.Enabled)
                    {
                        ...
                        switch (Options.Mode)
                        {
                            case RaidableMode.Easy:
                                ...
                                if (config.RankedLadder.Assign.Easy > 0 && playerInfo.Easy >= config.RankedLadder.Assign.Easy && CanAssignTo(ri.userid, config.RankedLadder.Assign.Owner))
                                {
                            ...
                            case RaidableMode.Medium:
                                ...
                                if (config.RankedLadder.Assign.Medium > 0 && playerInfo.Medium >= config.RankedLadder.Assign.Medium && CanAssignTo(ri.userid, config.RankedLadder.Assign.Owner))
                                {
                            ...
                            case RaidableMode.Hard:
                                ...
                                if (config.RankedLadder.Assign.Hard > 0 && playerInfo.Hard >= config.RankedLadder.Assign.Hard && CanAssignTo(ri.userid, config.RankedLadder.Assign.Owner))
                                {
                            ...
                            case RaidableMode.Expert:
                                ...
                                if (config.RankedLadder.Assign.Expert > 0 && playerInfo.Expert >= config.RankedLadder.Assign.Expert && CanAssignTo(ri.userid, config.RankedLadder.Assign.Owner))
                                {
                            ...
                            case RaidableMode.Nightmare:
                                ...
                                if (config.RankedLadder.Assign.Nightmare > 0 && playerInfo.Nightmare >= config.RankedLadder.Assign.Nightmare && CanAssignTo(ri.userid, config.RankedLadder.Assign.Owner))
                                {
                    ...
                    if (Options.Rewards.NoBuyableRewards && payments?.valid == true)
                    {
                        return;
                    }
            ...
            public bool TryMessage(BasePlayer player, string key, params object[] args)
            {
                ...
                ActionIn(10f, () => messagesSent.Remove(userid));
            ...
            public bool HasLockout(BasePlayer player)
            {
                ...
                if (data.Lockouts.TryGetValue(player.UserIDString, out lo))
                {
                    ...
                    else time = GetLockoutTime(Options.Mode, lo);
            ...
            public static double GetLockoutTime(RaidableMode mode, Lockout lo)
            {
                switch (mode)
                {
            ...
            public string Mode(string userid = null, bool forceShowName = false)
            {
                ...
                if (GetRaidableMode(difficultyMode) != RaidableMode.Random)
                {
                    difficultyMode = difficultyMode.SentenceCase();
                }
            ...
            {
                if (!IsOpened || payments?.valid != true)
                {
                    ResetLock();
                    return true;
                }

                if (payments.owner == null)
                {
                    return false;
                }
                ...
                if (config.Settings.Buyable.UsePayLock || forced)
                {
                    ...
                    this.payments = payments;
                    IsPayLocked = true;
                    SetOwnerInternal(payments.owner);
                    ClearEnemies();
                    Interface.CallHook("OnRaidableBasePurchased", new object[] { owner.displayName, Location, PositionToGrid(Location), (int)Options.Mode, AllowPVP, spawnTime, loadTime, BaseName, spawnDateTime, despawnDateTime });
                }
            ...
            {
                if (IsInstanceValid && permOwnerId.IsSteamId())
                {
                    config.Settings.Buyable.Cooldowns.Set(permOwnerId, Options.Mode);
                }
            }

            public void Refund(BasePlayer player)
            {
                if (config.Settings.Buyable.Refunds.Percentage == 0 || !config.Settings.Buyable.Refunds.Refund || payments == null)
                {
                    return;
                }
                ...
                if (payments.Custom?.Options?.Count > 0)
                {
                    foreach (var option in payments.Custom.Options)
                    {
                        ...
                        QueueNotification(player, "Refunded", $"{item.amount}x {item.info.displayName.english}");
                    }
                }
                ...
                if (payments.ServerRewards?.RP > 0)
                {
                    ...
                    QueueNotification(player, "Refunded", $"{points} RP");
                }
                ...
                if (payments.Economics?.money > 0)
                {
                    ...
                    QueueNotification(player, "Refunded", $"${money}");
                }
            }
            ...
            {
                RemoveNearDistance = rb.spawns == null ? rb.Profile.Options.ProtectionRadius(rb.type) : rb.spawns.RemoveNear(rb.Position, rb.Profile.Options.ProtectionRadius(rb.type), rb.Profile.Options.Water.CacheType, rb.type);

                Instance.Cycle.Add(rb.type, rb.Profile.Options.Mode, rb.BaseName);

                if (rb.type == RaidableType.Purchased && rb.payments != null)
                {
            ...
            public void SetCenterFromMultiplePoints()
            {
                ...
                else vector.y = SpawnsController.GetSpawnHeight(vector, !Options.Water.SpawnOnSeabed);
            ...
            private void TrySetupEntity(BaseEntity e, ref float invokeTime)
            {
                ...
                if (e is IOEntity)
                {
                    ...
                    if (e is AutoTurret)
                    {
                        ...
                        ActionIn(invokeTime += 0.1f, () => SetupTurret(turret));
                    }
            ...
            internal void SetupCollider()
            {
                ...
                {
                    Interface.Oxide.NextTick(SetupCollider);
                }
            }
            ...
            private bool SetupLoot()
            {
                ...
                int max = Type == RaidableType.Purchased && Options.MaxBuyableTreasure > 0 ? Options.MaxBuyableTreasure : Options.MaxTreasure;

                treasureAmount = Options.MinTreasure > 0 ? UnityEngine.Random.Range(Options.MinTreasure, max + 1) : max;
            ...
            public void SetupEntity(BaseEntity e, bool skipCheck = true)
            {
                ...
                else if (!MyInstance.temporaryData.UID.Contains(e.net.ID))
                {
                    MyInstance.temporaryData.UID.Add(e.net.ID);
                }
            ...
            private List<string> blockPrefabs = new List<string> { "foundation.triangle", "foundation", "floor.triangle", "floor", "roof", "roof.triangle" };
            ...
            private void SetupTurret(AutoTurret turret)
            {
                ...
                if (Options.AutoTurret.Shortnames.Count > 0)
                {
                    ActionIn(0.1f, () =>
                    {
                        if (!turret.IsKilled() && turret.AttachedWeapon.IsNull())
                        {
                ...
                ActionIn(2.5f, () => FillAmmoTurret(turret));
                ...
                if (!Options.AutoTurret.RequiresPower)
                {
                    ...
                    else ActionIn(3f, turret.InitiateStartup);
                }
            ...
            private Timer ActionIn(float time, Action action)
            {
                return MyInstance.timer.Once(time, () =>
                {
                    if (IsUnloading || IsDespawning) return;
            ...
            private void Subscribe()
            {
                ...
                if (config.Weapons.SamSiteRange > 0f && samsites.Count > 0)
                {
            ...
            private void MakeAnnouncements()
            {
                ...
                foreach (var target in BasePlayer.activePlayerList)
                {
                    ...
                    string difficultyMode = mx($"Mode{Options.Mode}", target.UserIDString);
            ...
            {
                if (!IsOpened || IsPayLocked || !ownerId.IsSteamId() || IsPlayerActive(ownerId))
                {
                    Invoke(ResetOwner, config.Settings.Management.LockTime * 60f);
                    return;
                }
            ...
            {
                if (IsPlayerActive(ownerId) || owner.IsReallyValid())
                {
                    Invoke(ResetPayLock, config.Settings.Buyable.ResetDuration * 60f);
                    return;
                }
            ...
            private void TakeLootFrom(RaidableMode mode)
            {
                List<LootItem> lootList;
                if (Instance.Buildings.DifficultyLootLists.TryGetValue(mode, out lootList))
                {
                    TakeLootFrom(lootList, DifficultyLoot, config.Treasure.UniqueDifficultyLoot);
                }
            }
            ...
            private void TakeLootFrom(List<LootItem> source, List<LootItem> to, bool isUnique)
            {
                ...
                if (permOwnerId.IsSteamId())
                {
                    if (permOwnerId.HasPermission("raidablebases.buyable.vip.pve"))
                    {
                        m = Mathf.Clamp(Options.MultiplierPVE, 0f, 999f);
                    }
                    else if (permOwnerId.HasPermission("raidablebases.buyable.vip.pvp"))
                    {
                        m = Mathf.Clamp(Options.MultiplierPVP, 0f, 999f);
                    }
                }
            ...
            private void TakeLootFromBaseLoot()
            {
                ...
                TakeLootFrom(baseLoot, BaseLoot, config.Treasure.UniqueBaseLoot);
            ...
            private void TakeLootFromDifficultyLoot()
            {
                if (BaseLoot.Count < treasureAmount)
                {
                    switch (Options.Mode)
                    {
                        case RaidableMode.Easy: TakeLootFrom(RaidableMode.Easy); break;
                        case RaidableMode.Medium: TakeLootFrom(RaidableMode.Medium); break;
                        case RaidableMode.Hard: TakeLootFrom(RaidableMode.Hard); break;
                        case RaidableMode.Expert: TakeLootFrom(RaidableMode.Expert); break;
                        case RaidableMode.Nightmare: TakeLootFrom(RaidableMode.Nightmare); break;
                    }
                }
            }
            ...
            public static int Get(RaidableMode mode, bool isPurchased)
            {
                int sum = 0;
                foreach (var sp in Instance.Queues.queue)
                {
                    if (isPurchased && sp.type != RaidableType.Purchased)
                    {
                        continue;
                    }
                    if (sp.options.Mode == mode)
                    {
                        sum++;
                    }
                }
                foreach (var raid in Instance.Raids)
                {
                    if (isPurchased && raid.Type != RaidableType.Purchased)
                    {
                        continue;
                    }
            ...
            public bool EjectBackpack(uint key, BackpackData data, bool bypass)
            {
                ...
                if (player.IsReallyConnected())
                {
                    ...
                    try
                    {
                        ...
                        DrawText(player, config.Settings.Management.DrawTime, Color.red, data.backpack.transform.position, message);
                    }
                    catch (Exception ex)
                    {
                        Puts("{0}", ex);
            ...
            public static bool RemovePlayer(BasePlayer player, EjectCode code, Vector3 a, float radius, RaidableType type, bool isJetpack = false)
            {
                ...
                //Instance.Queues.Messages.Print($"Ejected {player.displayName} ({player.userID}) with code {code} at {a} in {PositionToGrid(a)}");
            ...
            private bool CannotEnter(BasePlayer target, bool justEntered)
            {
                if (GetRaider(target).IsAllowed)
                {
                    if (IsBanned(target))
                    {
                        RemovePlayer(target, EjectCode.Banned, Location, ProtectionRadius, Type);
                        return true;
                    }
                    return false;
                }
                ...
                return code != EjectCode.None && RemovePlayer(target, code, Location, ProtectionRadius, Type);
            }
            ...
            private bool IsWearingJetpack(BasePlayer player)
            {
                if (config.Settings.Management.Mounts.Jetpacks && !player.IsKilled())
                {
                    var m = player.GetMounted();

                    if (m.IsValid() && (m.ShortPrefabName == "testseat" || m.ShortPrefabName == "standingdriver") && m.GetParentEntity() is DroppedItemContainer)
                    {
                        return true;
                    }
                }
            ...
            private void TryEjectMountable(BaseEntity e)
            {
                if (e is BaseMountable)
                {
                    ...
                    if (players.Count == 0)
                    {
                        EjectMountable(m, EjectCode.Mounted, players, Location, ProtectionRadius);
                    }
                }
                else if (e is HotAirBalloon)
                {
                    ...
                    if (players.Count == 0)
                    {
                        EjectMountable(hab, EjectCode.Mounted, players, Location, ProtectionRadius);
                    }
                }
            }
            ...
            public bool CanSetupEntity(BaseEntity e)
            {
                ...
                {
                    MyInstance.temporaryData.UID.Remove(e.net.ID);
                    Entities.Remove(e);
                    Interface.Oxide.NextTick(e.SafelyKill);
                    return false;
                }
            ...
            private void SetupNpc(ScientistNPC npc, HumanoidBrain brain, bool isMurderer, List<Vector3> positions)
            {
                ...
                if (Options.NPC.AlternateScientistLoot.Enabled && Options.NPC.AlternateScientistLoot.IDs.Count > 0)
                {
                    ...
                    if (StringPool.toString.ContainsKey(id))
                    {
            ...
            private void CopyInventory(ScientistNPC npc)
            {
                if (Options.NPC.CopyInventory && npc.loadouts.Length != 0)
                {
                    int loadoutSize = npc.loadouts.Sum(x => x.wear.Count + x.belt.Count);
                    int newSize = npc.LootSpawnSlots.Length + loadoutSize;
                    Array.Resize(ref npc.LootSpawnSlots, npc.LootSpawnSlots.Length + newSize);
                    for (int j = 0; j < npc.loadouts.Length; j++)
                    {
                        for (int k = 0; k < npc.loadouts[j].wear.Count; k++)
                        {
                            SetLootSpawnSlot(npc, npc.loadouts[j].wear[k], npc.LootSpawnSlots.Length - k - 1);
                        }
                        for (int k = 0; k < npc.loadouts[j].belt.Count; k++)
                        {
                            SetLootSpawnSlot(npc, npc.loadouts[j].belt[k], npc.LootSpawnSlots.Length - npc.loadouts[j].wear.Count - k - 1);
                        }
                    }
                }
            }

            private void SetLootSpawnSlot(ScientistNPC npc, PlayerInventoryProperties.ItemAmountSkinned ias, int index)
            {
                var lootSpawn = ScriptableObject.CreateInstance<LootSpawn>();
                lootSpawn.items = new ItemAmountRanged[1]
                {
                    new ItemAmountRanged(ias.itemDef, ias.amount, ias.amount)
                };
                lootSpawn.subSpawn = new LootSpawn.Entry[0];
                npc.LootSpawnSlots[index] = new LootContainer.LootSpawnSlot
                {
            ...
            {
                List<string> platforms = new List<string> { "floor", "floor.triangle", "roof", "roof.triangle" };
                for (int i = 0; i < blocks.Count; i++)
                {
                    ...
                    var point = block.CenterPoint() + new Vector3(0f, block.bounds.extents.y + 0.45f, 0f);
                    if (Physics.Raycast(point, Vector3.up, 1.75f, Layers.Mask.Construction | Layers.Mask.Deployed))
                    {
                        continue;
                    }
                    if (GamePhysics.CheckSphere<BaseEntity>(point, 0.3f, Layers.Mask.Deployed))
                    {
                        continue;
                    }
                    var destination = block.transform.position + Vector3.up * 1.25f;
                    var entities = FindEntitiesOfType<BaseEntity>(destination, 1.5f, blockLayers);
                    var walls = entities.Sum(e => e.ShortPrefabName == "wall" ? 1 : 0);
                    if (walls < 3 && block.ShortPrefabName == "foundation.triangle" || walls < 4 && block.ShortPrefabName == "foundation")
                    {
                        _randomSpots.Add(block.CenterPoint() + new Vector3(0f, block.bounds.extents.y + 0.1125f, 0f));
                    }
            ...
            private void CreateGenericMarker()
            {
                if (IsMarkerAllowed() && (config.Settings.Markers.UseExplosionMarker || config.Settings.Markers.UseVendingMarker))
                {
                    ...
                    if (!genericMarker.IsNull())
                    {
                        ...
                        if (World.Size <= 3600)
                        {
                            genericMarker.radius = Mathf.Min(2.5f, config.Settings.Markers.SubRadius);
                        }
                        else genericMarker.radius = Mathf.Min(2.5f, config.Settings.Markers.Radius);
                        genericMarker.Spawn();
                        genericMarker.SendUpdate();
                    }
                }
            ...
            private Color GetMarkerColor1()
            {
                ...
                switch (Options.Mode)
                {
                    case RaidableMode.Easy: return TryParseHtmlString(config.Settings.Management.Colors1.Easy, out color) ? color : Color.green;
                    case RaidableMode.Medium: return TryParseHtmlString(config.Settings.Management.Colors1.Medium, out color) ? color : Color.yellow;
                    case RaidableMode.Hard: return TryParseHtmlString(config.Settings.Management.Colors1.Hard, out color) ? color : Color.red;
                    case RaidableMode.Expert: return TryParseHtmlString(config.Settings.Management.Colors1.Expert, out color) ? color : Color.blue;
                    default: return TryParseHtmlString(config.Settings.Management.Colors1.Nightmare, out color) ? color : Color.black;
                }
            }
            ...
            private Color GetMarkerColor2()
            {
                ...
                switch (Options.Mode)
                {
                    case RaidableMode.Easy: return TryParseHtmlString(config.Settings.Management.Colors2.Easy, out color) ? color : Color.green;
                    case RaidableMode.Medium: return TryParseHtmlString(config.Settings.Management.Colors2.Medium, out color) ? color : Color.yellow;
                    case RaidableMode.Hard: return TryParseHtmlString(config.Settings.Management.Colors2.Hard, out color) ? color : Color.red;
                    case RaidableMode.Expert: return TryParseHtmlString(config.Settings.Management.Colors2.Expert, out color) ? color : Color.blue;
                    default: return TryParseHtmlString(config.Settings.Management.Colors2.Nightmare, out color) ? color : Color.black;
                }
            }
        ...
        public static class SpawnsController
        {
            ...
            public static void Initialize()
            {
                ...
                assets = new List<string> { "perimeter_wall", "/props/", "/structures/", "/building/", "train_", "powerline_", "dune", "candy-cane", "assets/content/nature/", "walkway", "invisible_collider", "train_track", "module_", "junkpile" };
            ...
            public static IEnumerator SetupMonuments()
            {
                ...
                if (!hasMonuments)
                {
                    Puts("Skipping monument setup; no monuments found.");
                    yield break;
                }
            ...
            public static LandLevel GetLandLevel(Vector3 from, float radius, bool spawnOnSeabed, BasePlayer player = null)
            {
                float maxY = -1000;
                float minY = 1000;
            ...
            private static bool IsLocationBlocked(Vector3 vector)
            {
                string grid = PositionToGrid(vector);
                bool flag = config.Settings.Management.BlockedGrids.Exists(blockedGrid => grid.Equals(blockedGrid, StringComparison.OrdinalIgnoreCase)) || HasZone(vector);
                return config.Settings.UseZoneManagerOnly ? !flag : flag;
            }

            private static bool HasZone(Vector3 vector)
            {
            ...
            private static bool IsValidLocation(Vector3 vector, float safeRadius, float minProtectionRadius, bool spawnOnSeabed)
            {
                ...
                if (!spawnOnSeabed && InDeepWater(vector, false, 5f, 5f))
                {
                    return false;
                }
                ...
                string topology;
                return TopologyChecks(vector, minProtectionRadius, spawnOnSeabed, out topology);
            }

            internal static bool TopologyChecks(Vector3 vector, float radius, bool spawnOnSeabed, out string topology)
            {
            ...
            public static void ExtractLocation(RaidableSpawns spawns, Vector3 position, float maxLandLevel, float minProtectionRadius, float maxProtectionRadius, float minWaterDepthSeabed, float maxWaterDepthSeabed, float maxWaterDepth, bool spawnOnSeabed)
            {
            ...
            public static bool IsAreaSafe(Vector3 area, float protectionRadius, float cupboardRadius, int layers, out CacheType cacheType, RaidableType type = RaidableType.None, BuildingOptionsDifficultySpawns spawns = null)
            {
                ...
                for (int i = 0; i < hits; i++)
                {
                    ...
                    float dist = position.Distance(position);

                    if (dist > protectionRadius && dist <= cupboardRadius)
                    {
                        goto next;
                    }
            ...
            public static bool InDeepWater(Vector3 vector, bool seabed, float minDepth, float maxDepth)
            {
                ...
                float waterDepth = WaterLevel.GetWaterDepth(vector, true, null);
            ...
            public static void SetupZones(Plugin ZoneManager)
            {
                ...
                foreach (string zoneId in zoneIds)
                {
                    ...
                    if (!config.Settings.UseZoneManagerOnly)
                    {
                ...
                {
                    Puts(mx(config.Settings.UseZoneManagerOnly ? "AllowedZones" : "BlockedZones", null, managedZones.Count));
                }
            }

            public static bool IsObstructed(Vector3 from, float radius, float landLevel, float forcedHeight, bool spawnOnSeabed, BasePlayer player = null)
            {
                int n = 5;
                float f = radius * 0.2f;
                bool flag = false;
        ...
        {
            LoadTemporaryData();
            if (temporaryData.UID.Count > 0)
            {
                var uids = temporaryData.UID.ToList();
                timer.Once(30f, () => temporaryData.CleanupTemporaryFiles(uids));
            }
        ...
        public void SavePlayerData()
        {
            if (data != null)
            {
                ...
                data.BuyableCooldowns.RemoveAll((userid, bi) => BuyableInfo.GetTimeRemaining(userid) <= 0);
        ...
        internal void StartEntityCleanup()
        {
            ...
            else UndoLoop(entities, despawnLimit, true, false, false);
        }
        ...
        {
            if (EventTerritory(ss.transform.position) && RaidableBase.Has(ss) && !MountEntities.ContainsKey(entity))
            {
        ...
        private bool IsProtectedScientist(BasePlayer player, TriggerBase trigger)
        {
            ...
            if (raid == null || !raid.Options.NPC.IgnoreTrapsTurrets || !InRange(raid.Location, npc.spawnPos, raid.ProtectionRadius))
            {
                return false;
            }
            ...
            {
                var priv = npc.GetBuildingPrivilege();

                if (priv != null && priv.OwnerID == 0)
                {
        ...
        private object OnEntityEnter(TriggerBase trigger, BaseEntity entity)
        {
            ...
            if (player.GetType().Name.Contains("CustomScientist", CompareOptions.OrdinalIgnoreCase) && IsProtectedScientist(player, trigger))
            {
                return true;
            }
        ...
        private object OnServerCommand(ConsoleSystem.Arg arg)
        {
            ...
            return player.IsReallyValid() ? OnPlayerCommand(player, arg.cmd.FullName, arg.Args) : null;
        }
        ...
        private void DropIOLoot(IOEntity io, bool isAutoTurret)
        {
            ...
            {
                var dropPosition = isAutoTurret ? io.transform.position.WithY(io.transform.position.y + io.bounds.extents.y) : io.GetDropPosition();
                var prefab = StringPool.Get(raid.Options.BuoyantBox ? 146366564u : 545786656u);
                ice.inventory.Drop(prefab, dropPosition, io.transform.rotation);
            }
        ...
        private void EntityHandler(StorageContainer container, HitInfo hitInfo)
        {
            ...
            {
                var player = raid.GetInitiatorPlayer(hitInfo);
        ...
        {
            if (config.Settings.Management.PreventFireFromSpreading && fire.ShortPrefabName == "flamethrower_fireball" && fire.creatorEntity is BasePlayer && !fire.creatorEntity.ToPlayer().userID.IsSteamId())
            {
        ...
        {
            if (!fireBall.IsKilled() && hitInfo != null && EventTerritory(fireBall.transform.position) && !(hitInfo.Initiator is BasePlayer))
            {
        ...
        private void OnEntitySpawned(PlayerCorpse corpse)
        {
            ...
            if (corpse.playerSteamID.IsSteamId())
            {
                ...
                if (raid.Options.EjectBackpacks && !playerSteamID.HasPermission("reviveplayer.use"))
                {
                    ...
                    bool canEjectBackpack = Interface.CallHook("OnRaidableBaseEjectBackpack", new object[] { corpse, raid.Location, raid.AllowPVP, (int)raid.Options.Mode, raid.GetOwner(), raid.GetRaiders(), raid.BaseName }) == null;
        ...
        private object HandlePlayerDamage(BasePlayer victim, HitInfo hitInfo)
        {
            ...
            var attacker = raid.GetInitiatorPlayer(hitInfo);
        ...
        private object HandleEntityDamage(BaseCombatEntity entity, HitInfo hitInfo)
        {
            ...
            if (raid.IsLoading || entity is DroppedItemContainer)
            {
                return false;
            }
            ...
            if (hasEntity && hitInfo.WeaponPrefab?.ShortPrefabName == "torpedostraight")
            {
        ...
        public RaidableBase OpenEvent(RandomBase rb)
        {
            ...
            raid.SetAllowPVP(rb.type, rb.Profile.Options.AllowPVP);
        ...
        private Payment TryBuyRaidServerRewards(int cost, BasePlayer buyer, BasePlayer player, out bool isValid)
        {
            if (cost > 0 && ServerRewards.CanCall())
            {
                isValid = true;

                var obj = ServerRewards?.Call("CheckPoints", buyer.userID);
                ...
                int points = Convert.ToInt32(obj);

                if (points > 0 && points - cost >= 0)
                {
                    return new Payment(cost, 0, buyer, player);
                }
        ...
        private Payment TryBuyRaidEconomics(double cost, BasePlayer buyer, BasePlayer player, out bool isValid)
        {
            if (cost > 0)
            {
                isValid = true;
                ...
                if ((obj = Economics?.Call("Balance", buyer.UserIDString)) != null)
                {
                    var balance = Convert.ToDouble(obj);

                    if (balance > 0 && balance - cost >= 0)
                    {
                        return new Payment(0, cost, buyer, player);
                    }

                    Message(buyer, "EconomicsWithdrawFailed", cost);
                }

                if ((obj = IQEconomic?.Call("API_GET_BALANCE", buyer.userID)) != null)
                {
                    var balance = Convert.ToInt32(obj);
        ...
        private Payment TryBuyRaidCustom(List<CustomCostOptions> options, BasePlayer buyer, BasePlayer player, out bool isValid)
        {
            if (options.All(o => o.isValid))
            {
                isValid = true;
                ...
                return new Payment(options, buyer, player);
            }
        ...
        {
            public Payment Custom { get; set; }
            public Payment Economics { get; set; }
            public Payment ServerRewards { get; set; }
            public BasePlayer owner { get; set; }
        ...
        {
            public Payment(List<CustomCostOptions> options, BasePlayer buyer, BasePlayer owner)
            {
            ...
            public void RefundItems()
            {
                ...
                foreach (var option in Options)
                {
                    ...
                    _sb.Append(string.Format("{0} {1}", option.Amount, string.IsNullOrEmpty(option.Name) ? item.info.displayName.english : item.name = option.Name)).Append(", ");
                ...
                if (_sb.Length > 2)
                {
                    ...
                    Message(target, "Refunded", _sb.ToString());
                }
            }
            ...
            public void RefundMoney()
            {
                if (money > 0)
                {
                    ...
                    QueueNotification(buyer, "Refunded", $"${money}");
                }
            }
        ...
        private void BuyRaid(RaidableMode mode, Payments payments, BasePlayer owner, string baseName)
        {
            if (SpawnRandomBase(RaidableType.Purchased, mode, baseName, false, payments, owner))
            {
                QueueNotification(owner, "BaseQueued", Queues.queue.Count);
            }
        }
        ...
        private bool IsDifficultyAvailable(RaidableMode mode, bool checkAllowPVP)
        {
            ...
            foreach (var profile in Buildings.Profiles.Values)
            {
                if (profile.Options.Mode != mode || (checkAllowPVP && !config.Settings.Buyable.BuyPVP && profile.Options.AllowPVP && !config.Settings.Buyable.ConvertPVP))
                {
                    continue;
                }
        ...
        private bool PasteBuilding(RandomBase rb, bool bypass = false)
        {
            ...
            timer.Once(1f, () =>
            {
                if (IsInstanceValid)
                {
                    rb.loadTime = Time.time;

                    StartPasteCoroutine(rb);
                }
            });
        ...
        private void StartPasteCoroutine(RandomBase rb)
        {
            var loadCoroutine = ServerMgr.Instance.StartCoroutine(LoadCopyPasteFile(rb));
            loadCoroutines.Add(new PasteLoadInfo(loadCoroutine, rb));
        }
        ...
        private void StopLoadCoroutines()
        {
            ...
            foreach (var info in loadCoroutines.ToList())
            {
                if (info.coroutine != null)
                {
                    ServerMgr.Instance.StopCoroutine(info.coroutine);
                    info.coroutine = null;
                }
            }
        ...
        private IEnumerator SetupCopyPasteObstructionRadius()
        {
            ...
            {
                var radius = profile.Value.Options.ProtectionRadii.Obstruction == -1 ? 0f : GetObstructionRadii(profile.Value.Options.ProtectionRadii, RaidableType.None);
        ...
        private IEnumerator SetupCopyPasteObstructionRadius(string baseName, float radius)
        {
            ...
            {
                Puts("SetupCopyPasteObstructionRadius {0}: {1}", filename, ex);
                Queues.Messages.Print($"{baseName} could not be read from the disk #1: {ex}");
                yield break;
            }
        ...
        private IEnumerator LoadCopyPasteFile(RandomBase rb)
        {
            ...
            {
                Queues.Messages.Print($"{rb.BaseName} could not be read from the disk #2: {ex}");
                yield break;
            }
        ...
        private IEnumerator ApplyStartPositionAdjustment(RandomBase rb, DynamicConfigFile data)
        {
            ...
            if (!pasteData.valid)
            {
                yield return Instance.SetupCopyPasteObstructionRadius(rb.BaseName, rb.Profile.Options.ProtectionRadii.Obstruction == -1 ? 0f : GetObstructionRadii(rb.Profile.Options.ProtectionRadii, RaidableType.None));
            }
            ...
            center.y = GetSpawnHeight(center, !rb.Profile.Options.Water.SpawnOnSeabed);

            rb.Position = rb.Position + (rb.Position - center);

            if (rb.Profile.Options.Setup.ForcedHeight == -1)
            {
                rb.Position.y = GetSpawnHeight(rb.Position, !rb.Profile.Options.Water.SpawnOnSeabed);
        ...
        private IEnumerator ParseListedOptions(RandomBase rb)
        {
            ...
            rb.stability = rb.Profile.Options.Stability;

            yield return CoroutineEx.waitForFixedUpdate;
        }

        private object SpawnRandomBase(string b = null, Vector3 a = default(Vector3), int m = -1, int t = 1)
        {
            lastSpawnRequestTime = Time.realtimeSinceStartup;

            var type = (RaidableType)t; 
            var mode = GetRaidableMode(m.ToString());
            var profile = GetBuilding(type, mode, b);
            ...
            if (spawns == null || spawns.Spawns.Count == 0)
            {
                return "API_INVALID_SPAWNS";
            }

            if (a == Vector3.zero)
            {
                return SpawnRandomBase(type, mode, profile.Key);
            }
            
            return AddSpawnToQueue(profile, checkTerrain, type, spawns, null, null, null, a);
        }

        private bool SpawnRandomBase(RaidableType type, RaidableMode mode, string baseName = null, bool isAdmin = false, Payments payments = null, BasePlayer owner = null, IPlayer user = null)
        {
            lastSpawnRequestTime = Time.realtimeSinceStartup;
            ...
            {
                return AddSpawnToQueue(profile, checkTerrain, type, spawns, payments, owner, user, Vector3.zero);
            }
            else if (type == RaidableType.Maintained || type == RaidableType.Scheduled)
            {
                Queues.Messages.PrintAll();
            }
            else Queues.Messages.Add(GetDebugMessage(mode, validProfile, isAdmin, owner?.UserIDString, baseName, profile.Value?.Options), null);

            if (!validProfile && payments != null)
            {
                if (owner?.IsAdmin ?? false)
                {
                    owner.ChatMessage(Queues.Messages.GetLast());
                }
        ...
        private bool AddSpawnToQueue(KeyValuePair<string, BaseProfile> profile, bool checkTerrain, RaidableType type, RaidableSpawns spawns, Payments payments = null, BasePlayer owner = null, IPlayer user = null, Vector3 a = default(Vector3))
        {
        ...
        private KeyValuePair<string, BaseProfile> GetBuilding(RaidableType type, RaidableMode mode, string baseName)
        {
            ...
            foreach (var profile in Buildings.Profiles)
            {
                ...
                if (!IsBuildingAllowed(type, mode, profile.Value.Options.Mode, profile.Value.Options.AllowPVP))
                {                    
                    continue;
                }
        ...
        private RaidableMode GetRandomDifficulty(RaidableType type)
        {
            ...
            foreach (RaidableMode mode in GetRaidableModes())
            {
                ...
                if (max < 0 || max > 0 && RaidableBase.Get(mode, false) >= max)
                {
        ...
        {
            var elements = modes.ToDictionary(x => x, x => config.Settings.Management.Chances.Get(x));
            decimal chance = Convert.ToDecimal(Core.Random.Range(0.0, 100.0));
                ...
                case RaidableType.Purchased:
                    {
                        ...
                        {
                            Instance.Queues.Messages.Add("Cannot spawn difficulty today", buildingMode);
                            return false;
                        }
                        if (!config.Settings.Buyable.BuyPVP && allowPVP)
                        {
                            Instance.Queues.Messages.Add("Buyable Events is configured to block PVP purchases.");
                            return false;
                        }
                        break;
                    }
        ...
        private void CommandReloadConfig(IPlayer user, string command, string[] args)
        {
            if (user.IsServer || user.ToPlayer().IsAdmin)
            {
                ...
                if (Maintained._coroutine != null)
                {
                    Maintained.StopCoroutine();
                    user.Reply(mx("ReloadMaintainCo", user.Id));
                }

                if (Scheduled._coroutine != null)
                {
                    Scheduled.StopCoroutine();
        ...
        private void Initialize()
        {
            ...
            {
                config.Settings.Buyable.Cooldowns = new RaidableBaseSettingsBuyableCooldowns();
        ...
        private void CommandBuyRaid(IPlayer user, string command, string[] args)
        {
            ...
            {
                TryResetCooldown(buyer, player);
                return;
            }

            string value = args[0].ToLower();
            RaidableMode mode = GetRaidableMode(value, buyer.HasPermission("raidablebases.allow"));
            ...
            if (config.Settings.Buyable.Limits.Any)
            {
                ...
                if (max < 0 || max > 0 && RaidableBase.Get(mode, true) >= max)
                {
            ...
            if (RaidableBase.IsOwner(player, true))
            {
                Message(buyer, "BuyableAlreadyOwner");
                return;
            }

            if (BuyableInfo.GetTimeRemaining(buyer, mode) > 0)
            {
                return;
            }

            if (IsQueued(player, GetMembers(buyer)))
            {
                return;
            }
            ...
            List<CustomCostOptions> options;
            bool isValid;

            if (config.Settings.Include.Custom && config.Settings.Custom.TryGetValue(mode, out options) && (payments.Custom = TryBuyRaidCustom(options, buyer, player, out isValid)) == null && isValid)
            {
                return;
            }

            if (config.Settings.Include.Economics && (payments.Economics = TryBuyRaidEconomics(config.Settings.Economics.Get(mode), buyer, player, out isValid)) == null && isValid)
            {
                return;
            }

            if (config.Settings.Include.ServerRewards && (payments.ServerRewards = TryBuyRaidServerRewards(config.Settings.ServerRewards.Get(mode), buyer, player, out isValid)) == null && isValid)
            {
                return;
            }
            ...
            if (payments.valid)
            {
                payments.Custom?.TakeItems();
                payments.Economics?.TakeMoney();
                payments.ServerRewards?.TakePoints();

                BuyRaid(mode, payments, player, GetFileMode(value, buyer.HasPermission("raidablebases.allow")) != RaidableMode.Random ? value : null);
            }
            else Message(buyer, "BuyablePaymentUnconfigured");
        }

        private void TryResetCooldown(BasePlayer buyer, BasePlayer player)
        {
            bool isValid;
            var payments = new Payments(buyer);

            if (config.Settings.Buyable.Cooldowns.Costs.Custom.isValid && (payments.Custom = TryBuyRaidCustom(new List<CustomCostOptions> { config.Settings.Buyable.Cooldowns.Costs.Custom }, buyer, player, out isValid)) == null && isValid)
            {
                return;
            }

            if (config.Settings.Buyable.Cooldowns.Costs.Money > 0 && (payments.Economics = TryBuyRaidEconomics(config.Settings.Buyable.Cooldowns.Costs.Money, buyer, player, out isValid)) == null && isValid)
            {
                return;
            }

            if (config.Settings.Buyable.Cooldowns.Costs.Points > 0 && (payments.ServerRewards = TryBuyRaidServerRewards(config.Settings.Buyable.Cooldowns.Costs.Points, buyer, player, out isValid)) == null && isValid)
            {
                return;
            }
        ...
        public bool IsQueued(BasePlayer player, List<ulong> members)
        {
            foreach (ulong member in members)
            {
                foreach (var sp in Queues.queue)
                {
                    ...
                    {
                        if (player.userID == sp.userid)
                        {
                            Message(player, "BuyableAlreadyQueued");
                        }
                        else Message(player, "BuyableAlreadyQueuedAllied");
                ...
                case "queue":
                    {
                        ...
                        if (Queues.queue.Count > 0)
                        {
                            ...
                            {
                                user.Reply($"{spq.type} ({spq.options.Mode}) with {spq.attempts} attempts");
                            }
                        }
                        return;
                    }
                case "queue_clear":
                    {
                        if (isAdmin)
                        {
                            ...
                            Queues.queue.ToList().ForEach(queue => queue.payments?.Refund());
                            Queues.queue.Clear();
        ...
        protected void DrawRaidLocations(BasePlayer player, bool hasPerm)
        {
            ...
            try
            {
                ...
                foreach (var raid in Raids)
                {
                    ...
                    DrawText(player, 15f, Color.yellow, raid.Location, string.Format("{0}{1} {2} [{3} {4}] {5}", raid.BaseName, flag, raid.Mode(player.UserIDString, true), message, PositionToGrid(raid.Location), raid.Location));

                    foreach (var ri in raid.raiders.Values.Where(x => x.IsAlly && x.player.IsReallyValid()))
                    {
                        DrawText(player, 15f, Color.yellow, ri.player.transform.position, mx("Ally", player.UserIDString).Replace(":", string.Empty));
                    }

                    if (raid.owner.IsReallyValid())
                    {
                        DrawText(player, 15f, Color.yellow, raid.owner.transform.position, mx("Owner", player.UserIDString).Replace(":", string.Empty));
                    }
                }
            }
            catch (Exception ex)
        ...
        private static RaidableMode GetRaidableMode(string value, bool fileMode = false)
        {
            ...
            return GetFileMode(value, fileMode);
        }

        private static RaidableMode GetFileMode(string value, bool fileMode)
        {
            if (fileMode || config.Settings.Buyable.FileMode)
            {
                foreach (var profile in Instance.Buildings.Profiles)
                {
                    if (profile.Key.Equals(value, StringComparison.OrdinalIgnoreCase))
                    {
                        return profile.Value.Options.Mode;
                    }

                    foreach (var extra in profile.Value.Options.AdditionalBases)
                    {
                        if (extra.Key.Equals(value, StringComparison.OrdinalIgnoreCase))
                        {
                            return profile.Value.Options.Mode;
                        }
                    }
        ...
        protected void ShowGrid(BasePlayer player, bool showAll)
        {
            ...
            try
            {
                ...
                {
                    var color = cacheType == CacheType.Generic ? Color.red : cacheType == CacheType.Temporary ? Color.cyan : cacheType == CacheType.Privilege ? Color.yellow : cacheType == CacheType.Seabed || cacheType == CacheType.Submerged ? Color.blue : Color.red;
                    var text = cacheType == CacheType.Generic ? "X" : cacheType == CacheType.Temporary ? "C" : cacheType == CacheType.Privilege ? "TC" : cacheType == CacheType.Seabed || cacheType == CacheType.Submerged ? "W" : "X";
        ...
        protected void ProcessEventCommand(IPlayer user, BasePlayer player, bool isAllowed, string[] args) // rbe
        {
            ...
            var mode = GetRaidableMode(args.FirstOrDefault(value => GetRaidableMode(value) != RaidableMode.Random));
            var profile = GetBuilding(RaidableType.Manual, mode, baseName);

            if (!IsProfileValid(profile))
            {
                user.Reply(profile.Value == null ? m("BuildingNotConfigured", user.Id) : GetDebugMessage(mode, false, true, user.Id, profile.Key, profile.Value.Options));
                return;
            }
            ...
            var safe = player.IsAdmin || SpawnsController.IsAreaSafe(hit.point, safeRadius, safeRadius, manualMask, out cacheType, RaidableType.Manual, profile.Value.Options.CustomSpawns);
            ...
            if (safe && (isAllowed || !SpawnsController.IsMonumentPosition(hit.point, profile.Value.Options.ProtectionRadius(RaidableType.Manual))))
            {
                ...
                var rb = new RandomBase(profile.Key, profile.Value, hit.point, RaidableType.Manual, spawns, null);
        ...
        protected void ProcessConsoleCommand(IPlayer user, BasePlayer player, bool isAllowed, string[] args) // rbevent
        {
            ...
            SpawnRandomBase(RaidableType.Manual, mode, baseName, isAllowed, null, null, isAllowed && user.IsConnected ? user : null);
        }
        ...
        private bool HandledCommandArguments(BasePlayer player, IPlayer user, bool isAllowed, string[] args)
        {
            ...
            switch (args[0].ToLower())
            {
                ...
                case "checkflat":
                    {
                        ...
                        float radius;
                        if (args.Length != 2 || !float.TryParse(args[1], out radius)) radius = 50f;
                        user.Reply(SpawnsController.IsObstructed(player.transform.position, radius, 2.5f, -1f, player.IsHeadUnderwater(), player) ? "Obstruction test failed" : "Obstruction test passed");
                        var landLevel = SpawnsController.GetLandLevel(player.transform.position, radius, player.IsHeadUnderwater(), player);
                        DrawText(player, 30f, Color.red, player.transform.position, $"{landLevel.Max - landLevel.Min:N01}");
                        return true;
                    }
                case "debug":
                    {
                        ...
                        if (DebugMode)
                        {
                            user.Reply(string.Format("Scheduled Events Running: {0}", Scheduled._coroutine != null));
                            user.Reply(string.Format("Maintained Events Running: {0}", Maintained._coroutine != null));
                ...
                case "kill_cleanup":
                    {
                        if (!isAllowed) return false;
                        foreach (var entity in BaseNetworkable.serverEntities.OfType<BaseEntity>().ToList())
                        {
                            if (entity.IsDestroyed || entity.OwnerID != 0 || entity.Distance(player) > 100f) continue;
                            if (IsKillableEntity(entity)) entity.SafelyKill();
                        };
                ...
                case "lockraid":
                    {
                        if (args.Length >= 2 && (isAllowed || user.HasPermission("raidablebases.setowner")))
                        {
                            ...
                            if (target.IsReallyValid())
                            {
                                ...
                                else if (raid.TrySetPayLock(new Payments(target) { Economics = new Payment(0, 0, target, target) }, true))
                                {
                                    var payments = new Payments(target) { Economics = new Payment(0, 0, target, target) };
                                    raid.TrySetPayLock(new Payments(target) { Economics = new Payment(0, 0, target, target) }, true);
                ...
                case "clearowner":
                    {
                        if (player.IsReallyValid() && (isAllowed || user.HasPermission("raidablebases.setowner")))
                        {
                            ...
                            else if (raid.TrySetPayLock(null))
                            {
        ...
        private void CommandToggle(IPlayer user, string command, string[] args)
        {
            ...
            Queues.Paused = !buyableEnabled && !Scheduled.Enabled && !Maintained.Enabled;
        ...
        private void CommandToggleType(IPlayer user, string command, string[] args)
        {
            switch (args[0].ToLower())
            {
                case "maintained":
                    {
                        Maintained.Enabled = config.Settings.Maintained.Enabled = !config.Settings.Maintained.Enabled;
                        Maintained.StartCoroutine();
                        SaveConfig();
                        user.Reply($"Toggled maintained events {(Maintained.Enabled ? "on" : "off")}");
                        break;
                ...
                case "scheduled":
                    {
                        Scheduled.Enabled = config.Settings.Schedule.Enabled = !config.Settings.Schedule.Enabled;
                        Scheduled.StartCoroutine();
                        SaveConfig();
                        user.Reply($"Toggled scheduled events {(Scheduled.Enabled ? "on" : "off")}");
                        break;
        ...
        private void CommandConfig(IPlayer user, string command, string[] args)
        {
            ...
            switch (arg)
            {
                ...
                case "maintained": case "scheduled": case "buyable": CommandToggleType(user, command, args); return;
        ...
        private bool KeepEntity(BaseEntity entity, bool mounts, bool deployables, bool structures)
        {
            if (!mounts && KeepMountable(entity) || entity.OwnerID.IsSteamId() && KeepPlayerEntity(entity, deployables, structures))
            {
                return true;
            }
        ...
        private bool KeepPlayerEntity(BaseEntity entity, bool deployables, bool structures)
        {
        ...
        public void DespawnAll(bool inactiveOnly)
        {
            ...
            if (entities.Count > 0)
            {
                bool mounts = config?.Settings?.Management?.DespawnMounts ?? false;
                bool structures = config?.Settings?.Management?.DoNotDestroyStructures ?? false;
                bool deployables = config?.Settings?.Management?.DoNotDestroyDeployables ?? false;
                UndoLoop(entities.ToList(), undoLimit, mounts, structures, deployables);
            }
        }
        ...
        public void UndoLoop(List<BaseEntity> entities, int limit, bool mounts, bool structures, bool deployables, object[] hookObjects = null, int count = 0)
        {
            if (count == 0)
            {
                entities.RemoveAll(e => e.IsKilled() || KeepEntity(e, mounts, deployables, structures) || e.ShortPrefabName == "item_drop_backpack");

                entities.Sort(undoComparer);
            }

            entities.Take(limit).ToList().ForEach(entity =>
            {
                ...
                uint uid = entity.net == null ? 0 : entity.net.ID;

                entity.SafelyKill();

                if (uid != 0 && temporaryData != null && entity.IsKilled())
                {
                    temporaryData.UID.Remove(uid);
                }
            });
        ...
        {
            return new List<RaidableMode> { RaidableMode.Easy, RaidableMode.Medium, RaidableMode.Hard, RaidableMode.Expert, RaidableMode.Nightmare };
        }
        ...
        private List<ulong> GetMembers(BasePlayer buyer)
        {
            var members = new HashSet<ulong>();

            if (buyer.currentTeam == 0uL)
            {
                members.Add(buyer.userID);
            }
            else buyer.Team.members.ForEach(member => members.Add(member));
        ...
        public static float GetDistance(RaidableType type)
        {
            switch (type)
            {
                ...
                case RaidableType.Purchased: return Mathf.Clamp(config.Settings.Buyable.Distance, CELL_SIZE, 9000f);
        ...
        private void UpdateUI()
        {
            if (config.UI.Enabled)
            {
                BasePlayer.activePlayerList.ForEach(UI.UpdateLockoutUI);
            }
        }
        ...
        private void ConfigListBases(IPlayer user)
        {
            ...
            foreach (var entry in Buildings.Profiles)
            {
                ...
                {
                    _sb.AppendLine(entry.Key).Append(" ").Append(entry.Value.Options.Mode);
                    validBase = true;
                }
                ...
                if (entry.Value.Options.AdditionalBases.Count > 0)
                {
                    ...
                    foreach (var ab in entry.Value.Options.AdditionalBases)
                    {
                        ...
                        {
                            _sb.AppendLine(ab.Key).Append(" ").Append(entry.Value.Options.Mode);
                            validBase = true;
                        }
        ...
        private static void DropOrRemoveItems(StorageContainer container, RaidableBase raid)
        {
            ...
            {
                var dropPosition = container is GunTrap ? container.GetDropPosition() : container.transform.position.WithY(container.transform.position.y + container.bounds.size.y);
                ...
                var drop = container.inventory.Drop(prefab, dropPosition, container.transform.rotation);
                if (buoyant)
                {
                    drop.maxItemCount = 48;
                    drop.lootPanelName = "generic_resizable";
                }
                if (raid.Options.DespawnGreyBags)
                {
                    raid.AddEntity(drop);
                }
            }
        ...
        protected bool DespawnBase(BasePlayer player, bool isAllowed)
        {
            ...
            if (!isAllowed && raid.IsDamaged && config.Settings.Buyable.Refunds.Despawn)
            {
                Message(player, "DespawnBaseDamaged");
                return false;
            }

            if (!isAllowed && raid.IsAnyLooted && config.Settings.Buyable.Refunds.AnyLooted)
            {
        ...
        private int[] GetPlayerAmount(string userid, int mode)
        {
            foreach (var user in data.Players)
            {
                if (user.Key == userid)
                {
                    switch (mode)
                    {
                        case 0: return new int[2] { user.Value.Easy, user.Value.TotalEasy };
                        case 1: return new int[2] { user.Value.Medium, user.Value.TotalMedium };
                        case 2: return new int[2] { user.Value.Hard, user.Value.TotalHard };
                        case 3: return new int[2] { user.Value.Expert, user.Value.TotalExpert };
                        case 4: return new int[2] { user.Value.Nightmare, user.Value.TotalNightmare };
                    }
                }
            }
            return new int[0];
        ...
        private void AssignTreasureHunters(RankedRecord record, List<KeyValuePair<string, PlayerInfo>> players)
        {
            ...
            foreach (var entry in players)
            {
                switch (record.Mode)
                {
                    case RaidableMode.Points: if (entry.Value.Points > 0) ladder.Add(new KeyValuePair<string, int>(entry.Key, entry.Value.Points)); break;
                    case RaidableMode.Easy: if (entry.Value.Easy > 0) ladder.Add(new KeyValuePair<string, int>(entry.Key, entry.Value.Easy)); break;
                    case RaidableMode.Medium: if (entry.Value.Medium > 0) ladder.Add(new KeyValuePair<string, int>(entry.Key, entry.Value.Medium)); break;
                    case RaidableMode.Hard: if (entry.Value.Hard > 0) ladder.Add(new KeyValuePair<string, int>(entry.Key, entry.Value.Hard)); break;
                    case RaidableMode.Expert: if (entry.Value.Expert > 0) ladder.Add(new KeyValuePair<string, int>(entry.Key, entry.Value.Expert)); break;
                    case RaidableMode.Nightmare: if (entry.Value.Nightmare > 0) ladder.Add(new KeyValuePair<string, int>(entry.Key, entry.Value.Nightmare)); break;
                }
            }
        ...
        protected IEnumerator LoadProfiles()
        {
            ...
            foreach (string file in files)
            {
                ...
                try
                {
                    ...
                    {
                        double accuracy = options.Mode == RaidableMode.Easy || options.Mode == RaidableMode.Medium ? 15.0 : options.Mode == RaidableMode.Hard ? 20.0 : options.Mode == RaidableMode.Expert ? 25.0 : 30.0;

                        options.NPC.Accuracy = new NpcSettingsAccuracyDifficulty(accuracy);
                    }
            ...
            foreach (var profile in Buildings.Profiles.ToList())
            {
                if (GridController.SpawnsFileValid(profile.Value.Options.CustomSpawns.SpawnsFile))
                {
                    ...
                    if (spawns?.Count > 0)
                    {
                        ...
                        {
                            Instance.Subscribe(nameof(CanBuild));
                        }
                    }
        ...
        private void LoadTable(string file, List<LootItem> lootList)
        {
            ...
            lootList.RemoveAll(ti => ti == null || ti.amount == 0 && ti.amountMin == 0 || BlacklistedItems.Contains(ti.shortname));
        ...
        protected override void LoadDefaultMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                ["Difficulty Not Buyable"] = "Difficulty <color=#FF0000>{0}</color> is not available.",
                ...
                ["BuyRaidNotConfiguredProperly"] = "<color=#FFFF00>Purchases are not configured properly. <color=#FF0000>Allow Players To Buy PVP Raids</color> is disabled, and all profiles have <color=#FF0000>Allow PVP</color> enabled. You must enable <color=#FFFF00>Convert PVP To PVE</color>, or create PVE profiles if you insist that players cannot buy PVP raids.</color>",
                ["BuyRaidsDisabled"] = "<color=#FFFF00>No raids can be bought at this time.</color>",
                ["BuyBaseLocate"] = "<color=#FFFF00>Trying to find your base a spawn location now.</color>",
                ...
                ["DespawnBaseDamaged"] = "<color=#C0C0C0>You cannot despawn a base that has been damaged.</color>",
                ...
                ["BuyableAlreadyQueued"] = "You must wait until your previous queue request is completed!",
                ["BuyableAlreadyQueuedAllied"] = "You must wait until your allies previous queue request is completed!",
                ["BuyablePaymentUnconfigured"] = "Buyable events cannot be purchased until a payment is configured.",
        ...
        private static Dictionary<RaidableMode, List<CustomCostOptions>> DefaultCustomCosts
        {
            get
            {
                return new Dictionary<RaidableMode, List<CustomCostOptions>>
                {
                    [RaidableMode.Easy] = new List<CustomCostOptions> { new CustomCostOptions(50) },
                    [RaidableMode.Medium] = new List<CustomCostOptions> { new CustomCostOptions(100) },
                    [RaidableMode.Hard] = new List<CustomCostOptions> { new CustomCostOptions(150) },
                    [RaidableMode.Expert] = new List<CustomCostOptions> { new CustomCostOptions(200) },
                    [RaidableMode.Nightmare] = new List<CustomCostOptions> { new CustomCostOptions(250) }
                };
            }
        }
        ...
        public class LimitsDaySettings
        {
        ...
        public class BaseLockoutSettings
        {
            ...
            public bool AllyExploit { get; set; }
        ...
        public class BaseAmountSettings : DifficultyModesInt
        {
            ...
            public int Get(RaidableMode a)
            {
                ...
                if (t <= 0 || !Merge || Merge && CanSpawnDifficultyToday(a))
                {
                    return t;
                }
        ...
        {
            [JsonProperty(PropertyName = "Easy")]
            public PlayerAmountsEventTypeSettings Easy { get; set; } = new PlayerAmountsEventTypeSettings();

            [JsonProperty(PropertyName = "Medium")]
            public PlayerAmountsEventTypeSettings Medium { get; set; } = new PlayerAmountsEventTypeSettings();

            [JsonProperty(PropertyName = "Hard")]
            public PlayerAmountsEventTypeSettings Hard { get; set; } = new PlayerAmountsEventTypeSettings();

            [JsonProperty(PropertyName = "Expert")]
            public PlayerAmountsEventTypeSettings Expert { get; set; } = new PlayerAmountsEventTypeSettings();

            [JsonProperty(PropertyName = "Nightmare")]
            public PlayerAmountsEventTypeSettings Nightmare { get; set; } = new PlayerAmountsEventTypeSettings();
        ...
        public class ManagementSettings
        {
            ...
            public BaseAmountSettings Amounts { get; set; } = new BaseAmountSettings();

            [JsonProperty(PropertyName = "Chance To Automatically Spawn Each Difficulty (-1 = ignore)")]
            public BaseChanceSettings Chances { get; set; } = new BaseChanceSettings();

            [JsonProperty(PropertyName = "Player Lockouts (0 = ignore)")]
            public BaseLockoutSettings Lockout { get; set; } = new BaseLockoutSettings();

            [JsonProperty(PropertyName = "Easy Raids Can Spawn On")]
            public LimitsDaySettings Easy { get; set; } = new LimitsDaySettings();

            [JsonProperty(PropertyName = "Medium Raids Can Spawn On")]
            public LimitsDaySettings Medium { get; set; } = new LimitsDaySettings();

            [JsonProperty(PropertyName = "Hard Raids Can Spawn On")]
            public LimitsDaySettings Hard { get; set; } = new LimitsDaySettings();

            [JsonProperty(PropertyName = "Expert Raids Can Spawn On")]
            public LimitsDaySettings Expert { get; set; } = new LimitsDaySettings();

            [JsonProperty(PropertyName = "Nightmare Raids Can Spawn On")]
            public LimitsDaySettings Nightmare { get; set; } = new LimitsDaySettings();
            ...
            public List<string> AdditionalBlockedColliders { get; set; } = new List<string> { "cubes" };
        ...
        public class PluginSettings
        {
            ...
            public RaidableBaseCostOptions Include { get; set; } = new RaidableBaseCostOptions();

            [JsonProperty(PropertyName = "Economics Buy Raid Costs (0 = disabled)")]
            public EconomicsOptions Economics { get; set; } = new EconomicsOptions();

            [JsonProperty(PropertyName = "ServerRewards Buy Raid Costs (0 = disabled)")]
            public ServerRewardsOptions ServerRewards { get; set; } = new ServerRewardsOptions(0, 0, 0, 0, 0);

            [JsonProperty(PropertyName = "Custom Buy Raid Cost", ObjectCreationHandling = ObjectCreationHandling.Replace)]
            public Dictionary<RaidableMode, List<CustomCostOptions>> Custom { get; set; } = DefaultCustomCosts;
        ...
        public class NpcSettingsInsideBase
        {
            ...
            [JsonProperty(PropertyName = "Bed Health Multiplier")]
            public float BedHealthMultiplier { get; set; } = 1f;

            [JsonProperty(PropertyName = "Rug Health Multiplier")]
            public float RugHealthMultiplier { get; set; } = 1f;
        ...
        public class NpcSettings
        {
            ...
            [JsonProperty(PropertyName = "Spawn Loadout In Corpses Inventory")]
            ...
            [JsonProperty(PropertyName = "Kits Are Unique When Applicable")]
            public bool UniqueKits { get; set; }
        ...
        public class BuildingWaterOptions
        {
            ...
            [JsonProperty(PropertyName = "Minimum Water Depth Level Used For Seabed Option")]
            public float MinimumSeabedWaterDepth { get; set; } = -20f;

            [JsonProperty(PropertyName = "Maximum Water Depth Level Used For Seabed Option")]
            public float MaximumSeabedWaterDepth { get; set; } = -35f;

            [JsonProperty(PropertyName = "Torpedo Damage Multiplier (Min)")]
            public float TorpedoMin { get; set; } = 5f;

            [JsonProperty(PropertyName = "Torpedo Damage Multiplier (Max)")]
            public float TorpedoMax { get; set; } = 10f;

            [JsonIgnore]
            public bool SpawnOnSeabed { get; set; }
        ...
        public class BuildingOptionsDifficultySpawns
        {
            ...
            [JsonProperty(PropertyName = "Prevent Building Until Base Spawns")]
            public bool PreventBuilding { get; set; }
        ...
        public class BuildingOptionsRadiation
        {
            ...
            [JsonProperty(PropertyName = "Rads")]
            public float Rads { get; set; } = 2f;

            [JsonProperty(PropertyName = "Protection Required")]
            public float Protection { get; set; } = 6f;
        }
        ...
        public class BuildingOptions
        {
            ...
            public BuildingOptionsRadiation Radiation { get; set; } = new BuildingOptionsRadiation();
            ...
            [JsonProperty(PropertyName = "Amount Of Items To Spawn For Buyable Events (0 = Use Default Value)")]
            public int MaxBuyableTreasure { get; set; } = 0;
            ...
            [JsonProperty(PropertyName = "Empty All Containers (Exemptions)")]
            ...
            [JsonProperty(PropertyName = "Loot Amount Multiplier (raidablebases.buyable.vip.pve)")]
            public float MultiplierPVE { get; set; } = 1f;

            [JsonProperty(PropertyName = "Loot Amount Multiplier (raidablebases.buyable.vip.pvp)")]
            public float MultiplierPVP { get; set; } = 1f;
        ...
        public class RaidableBaseSettingsBuyableCooldownsDifficulty
        {
            [JsonProperty(PropertyName = "VIP Permission: raidablebases.vipcooldown")]
            public double VIP { get; set; } = 1800f;
            ...
            public double Cooldown { get; set; } = 1800f;
        }

        public class PluginBuyableResetCosts
        {
            [JsonProperty(PropertyName = "Custom Currency")]
            public CustomCostOptions Custom { get; set; } = new CustomCostOptions(0);
        ...
        {
            [JsonProperty(PropertyName = "Reset Cooldown Costs")]
            public PluginBuyableResetCosts Costs { get; set; } = new PluginBuyableResetCosts();

            [JsonProperty(PropertyName = "Easy")]
            public RaidableBaseSettingsBuyableCooldownsDifficulty Easy { get; set; } = new RaidableBaseSettingsBuyableCooldownsDifficulty();

            [JsonProperty(PropertyName = "Medium")]
            public RaidableBaseSettingsBuyableCooldownsDifficulty Medium { get; set; } = new RaidableBaseSettingsBuyableCooldownsDifficulty();

            [JsonProperty(PropertyName = "Hard")]
            public RaidableBaseSettingsBuyableCooldownsDifficulty Hard { get; set; } = new RaidableBaseSettingsBuyableCooldownsDifficulty();

            [JsonProperty(PropertyName = "Expert")]
            public RaidableBaseSettingsBuyableCooldownsDifficulty Expert { get; set; } = new RaidableBaseSettingsBuyableCooldownsDifficulty();

            [JsonProperty(PropertyName = "Nightmare")]
            public RaidableBaseSettingsBuyableCooldownsDifficulty Nightmare { get; set; } = new RaidableBaseSettingsBuyableCooldownsDifficulty();
        ...
        public class RaidableBaseSettingsBuyableRefunds
        {
            ...
            [JsonProperty(PropertyName = "Base Becomes Ineligible For Rewards On Despawn")]
            public bool Ineligible { get; set; } = true;

            [JsonProperty(PropertyName = "Block Refund If Base Is Damaged")]
            public bool Damaged { get; set; } = true;

            [JsonProperty(PropertyName = "Block Despawn If Base Is Damaged")]
            public bool Despawn { get; set; } = true;

            [JsonProperty(PropertyName = "Block Despawn If Anything Is Looted")]
            public bool AnyLooted { get; set; } = true;
        ...
        public class RaidableBaseSettingsBuyable : RaidableBaseSettingsEventTypeBase
        {
            [JsonProperty(PropertyName = "Max Amount Purchasable Per Difficulty (0 = infinite, -1 = disabled)")]
            public ServerRewardsOptions Limits { get; set; } = new ServerRewardsOptions(0, 0, 0, 0, 0);

            [JsonProperty(PropertyName = "Cooldowns (0 = No Cooldown)")]
            public RaidableBaseSettingsBuyableCooldowns Cooldowns { get; set; } 
            ...
            [JsonProperty(PropertyName = "Allow Players To Spawn Specified Base Files")]
            ...
            [JsonProperty(PropertyName = "Spawn At Closest Position From Player")]
            public bool Closest { get; set; } = true;
        }
        ...
        public class RaidableBaseCostOptions
        {
            ...
            public bool Economics { get; set; } = true;

            [JsonProperty(PropertyName = "Require Server Rewards Costs")]
            public bool ServerRewards { get; set; } = true;
        }
        ...
        public class ServerRewardsOptions : DifficultyModesInt
        {
            [JsonIgnore]
            public bool Any
            {
            ...
            public ServerRewardsOptions(int a, int b, int c, int d, int e)
            {
                Easy = a;
                Medium = b;
                Hard = c;
        ...
        public class RankedLadderSettings
        {
            ...
            public RaidableBaseSettingsRankedLadderPointOptions Assign { get; set; } = new RaidableBaseSettingsRankedLadderPointOptions(0, 0, 0, 0, 0, false);

            [JsonProperty(PropertyName = "Difficulty Points")]
            public RaidableBaseSettingsRankedLadderPointOptions Points { get; set; } = new RaidableBaseSettingsRankedLadderPointOptions(1, 2, 3, 4, 5, false);
        }
        ...
        public class UIBuyableSettings : UIBaseSettings
        {
            ...
            {
                AnchorMin = "0.522 0.136";
                AnchorMax = "0.639 0.372";
                PanelAlpha = 0f;
            }

            [JsonProperty(PropertyName = "Cursor Enabled", Order = 5)]
            public bool CursorEnabled { get; set; }

            [JsonProperty(PropertyName = "Button Alpha", Order = 6)]
            public float ButtonAlpha { get; set; } = 1f;

            [JsonProperty(PropertyName = "X Text Color", Order = 7)]
            public string XTextColor { get; set; } = "#FFFFFF";

            [JsonProperty(PropertyName = "Font Size", Order = 8)]
            public int FontSize { get; set; } = 14;

            [JsonProperty(PropertyName = "Use Contrast Colors For Text Color", Order = 9)]
            public bool Contrast { get; set; }

            [JsonProperty(PropertyName = "Use Difficulty Colors For Buttons", Order = 10)]
            public bool Difficulty { get; set; }

            [JsonProperty(PropertyName = "X Button Color", Order = 11)]
            public string CloseColor { get; set; } = "#497CAF";

            [JsonProperty(PropertyName = "Easy Button Color", Order = 12)]
            public string EasyColor { get; set; } = "#497CAF";

            [JsonProperty(PropertyName = "Easy Text Color", Order = 13)]
            public string EasyTextColor { get; set; } = "#FFFFFF";

            [JsonProperty(PropertyName = "Medium Button Color", Order = 14)]
            public string MediumColor { get; set; } = "#497CAF";

            [JsonProperty(PropertyName = "Medium Text Color", Order = 15)]
            public string MediumTextColor { get; set; } = "#FFFFFF";

            [JsonProperty(PropertyName = "Hard Button Color", Order = 16)]
            public string HardColor { get; set; } = "#497CAF";

            [JsonProperty(PropertyName = "Hard Text Color", Order = 17)]
            public string HardTextColor { get; set; } = "#FFFFFF";

            [JsonProperty(PropertyName = "Expert Button Color", Order = 18)]
            public string ExpertColor { get; set; } = "#497CAF";

            [JsonProperty(PropertyName = "Expert Text Color", Order = 19)]
            public string ExpertTextColor { get; set; } = "#FFFFFF";

            [JsonProperty(PropertyName = "Nightmare Button Color", Order = 20)]
            public string NightmareColor { get; set; } = "#497CAF";

            [JsonProperty(PropertyName = "Nightmare Text Color", Order = 21)]
            public string NightmareTextColor { get; set; } = "#FFFFFF";

            public string GetButton(RaidableMode mode)
            {
                switch (mode)
                {
            ...
            public string GetText(RaidableMode mode)
            {
                switch (mode)
                {
                    case RaidableMode.Easy: return EasyTextColor;
                    case RaidableMode.Medium: return MediumTextColor;
                    case RaidableMode.Hard: return HardTextColor;
                    case RaidableMode.Expert: return ExpertTextColor;
                    default: return NightmareTextColor;
                }
            }
        }
        ...
        public class UI // Credits: Absolut & k1lly0u
        {
            ...
            public static void CreateButton(ref CuiElementContainer container, string panel, string btnColor, string text, int size, string aMin, string aMax, string command, TextAnchor align, string labelColor)
            {
                var name = Guid.NewGuid().ToString();

                container.Add(new CuiElement
                {
                    Name = name, Parent = panel,
                    Components = {
                        new CuiButtonComponent { Color = btnColor, Command = command },
                ...
                container.Add(new CuiElement
                {
                    Name = "BUYRAID_LABEL", Parent = name,
                    Components =
                    {
                        new CuiTextComponent { Align = align, Color = labelColor, Font = "robotocondensed-bold.ttf", FontSize = size, Text = text },
            ...
            private static string GetContrast(string hex)
            {
                return ((p(hex, 0, 2) * 299) + (p(hex, 2, 2) * 587) + (p(hex, 4, 2) * 114)) / 1000 >= 128 ? "0 0 0 1" : "1 1 1 1";
            }
            ...
            {
                var split = hex.Split(' ');
                return split.Length != 4 || !split.All(x => x.IsNumeric()) ? "FFFFFFFF" : ColorUtility.ToHtmlStringRGBA(new Color(float.Parse(split[0]), float.Parse(split[1]), float.Parse(split[2]), float.Parse(split[3])));
            }
            ...
            {
                if (player.IsReallyConnected() && Lockouts.Remove(player))
                {
                    GetRaidableModes().ForEach(mode => CuiHelper.DestroyUi(player, $"RB_UI_{mode}"));
            ...
            private static bool IsEnabled(RaidableMode mode)
            {
                if (CanSpawnDifficultyToday(mode))
                {
                    ...
                    return config.Settings.Economics.Get(mode) > 0 || config.Settings.ServerRewards.Get(mode) > 0; 
                }
                return false;
            }
            ...
            private static string GetPurchasePrice(RaidableMode mode, string langMode, string userid)
            {
                if (CanSpawnDifficultyToday(mode))
                {
                    List<CustomCostOptions> o;
                    if (config.Settings.Custom.TryGetValue(mode, out o) && o.Count > 0 && o.All(x => x.isValid))
                    {
                        return o.Count == 1 ? string.Format("{0} ({1} {2})", langMode, o[0].Amount, string.IsNullOrEmpty(o[0].Name) ? o[0].Shortname : o[0].Name) : langMode;
                    }
                    if (config.Settings.ServerRewards.Get(mode) > 0)
                    {
                        return string.Format("{0} ({1} {2})", langMode, config.Settings.ServerRewards.Get(mode), mx("RP", userid));
                    }
                    if (config.Settings.Economics.Get(mode) > 0)
                    {
                        return string.Format("{0} (${1})", langMode, config.Settings.Economics.Get(mode));
                    }
                }
                return null;
            }
            ...
            public static void CreateBuyableUI(BasePlayer player)
            {
                ...
                var element = CreateElementContainer(BuyablePanelName, Color(ui.PanelColor, ui.PanelAlpha.Value), ui.AnchorMin, ui.AnchorMax, ui.CursorEnabled, "Hud");
                var created = false;

                CreateLabel(ref element, BuyablePanelName, "1 1 1 1", mx("Buy Raids", player.UserIDString), 14, "0.02 0.865", "0.447 0.959");
                CreateButton(ref element, BuyablePanelName, Color(ui.CloseColor, ui.ButtonAlpha), "ⓧ", 14, "0.833 0.835", "1 0.982", "ui_buyraid closeui", TextAnchor.MiddleCenter, ui.XTextColor);
                ...
                foreach (RaidableMode mode in GetRaidableModes())
                {
                    ...
                    var modeText = mx($"Mode{mode}", player.UserIDString);

                    if (GetRaidableMode(modeText) != RaidableMode.Random)
                    {
                        modeText = modeText.SentenceCase();
                    }

                    var modeColor = config.Settings.Management.Colors2.Get(mode);
                    var priceText = GetPurchasePrice(mode, modeText, player.UserIDString);                    
                    var command = priceText == null ? "ui_buyraid closeui" : $"ui_buyraid {(int)mode}";
                    var labelColor = ui.Contrast ? GetContrast(priceText == null ? "#808080" : ui.Difficulty ? modeColor : ui.GetButton(mode)) : Color(ui.GetText(mode));
                    var btnColor = Color(priceText == null ? "#808080" : ui.Difficulty ? modeColor : ui.GetButton(mode), ui.ButtonAlpha);
                    CreateButton(ref element, BuyablePanelName, btnColor, priceText ?? modeText, ui.FontSize, $"0 {min}", $"1 {max}", command, TextAnchor.MiddleCenter, labelColor);
                    created = true;
                    min -= 0.165;
                    max -= 0.165;
                }
                ...
                else
                {
                    ...
                    if (!GetRaidableModes().Exists(mode => CanSpawnDifficultyToday(mode)))
                    {
            ...
            private static void ShowDelayUI(BasePlayer player)
            {
                ...
                CreateLockout(player, DelayPanelName, ds.time.ToString(), ui.Foreground, Color(ui.PanelColor, ui.PanelAlpha.Value), ui.AnchorMin, ui.AnchorMax);
            ...
            private static void ShowLockouts(BasePlayer player)
            {
                ...
                foreach (RaidableMode mode in GetRaidableModes())
                {
                    var key = Math.Floor(TimeSpan.FromSeconds(Math.Max(0, RaidableBase.GetLockoutTime(mode, lo))).TotalMinutes).ToString();
                    var color2H = config.Settings.Management.Colors2.Get(mode);

                    CreateLockout(player, $"RB_UI_{mode}", mx("UIFormatLockoutMinutes", player.UserIDString, key), RGBToHex(GetContrast(color2H)), Color(color2H, config.UI.Lockout.Alpha), config.UI.Lockout.Min(mode), config.UI.Lockout.Max(mode));
                }
            ...
            public static void UpdateLockoutUI(BasePlayer player)
            {
                ...
                if (config == null || !config.UI.Lockout.Enabled)
                {
                    return;
                }
            ...
            {
                Timers timers;
                if (InvokeTimers.TryGetValue(player.userID, out timers) && timers.Lockout != null && !timers.Lockout.Destroyed)
                {
            ...
            public static List<BasePlayer> Lockouts = new List<BasePlayer>();
            public static List<BasePlayer> Buyables = new List<BasePlayer>();
...
namespace Oxide.Plugins.RaidableBasesExtensionMethods
{
    public static class ExtensionMethods
    {
        ...
        public static IEnumerable<T> Union<T>(this IEnumerable<T> a, IEnumerable<T> b, IEqualityComparer<T> c = null) { HashSet<T> d = new HashSet<T>(c); foreach (T e in a) { if (d.Add(e)) { yield return e; } } foreach (T f in b) { if (d.Add(f)) { yield return f; } } }
