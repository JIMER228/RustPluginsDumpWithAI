...
{
    /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ [Info("HitAdvance", "https://discord.gg/dNGbxafuJn", "2.0.3")]
    [Description("Уникальный маркер для вашего сервера. Куплено на whiteplugins.ru")]
    public class HitAdvance : RustPlugin
    {
                    ...
                    case HitType.Text:
                        {
                                ...
                                Components =
                            {
                                ...
                                new CuiOutlineComponent() {Color = "0 0 0 1", Distance = "0.15500292 0.1550502912"},
        ...
        private void OnServerInitialized()
        {
            ...
            foreach (var check in Settings.HitSettings.Where(p => !string.IsNullOrEmpty(p.Value.Permission)))
                ...
                case "choose":
                    {
                        ...
                        if (!string.IsNullOrEmpty(Settings.HitSettings[type].Permission) && !permission.UserHasPermission(player.UserIDString, Settings.HitSettings[type].Permission)) return;
        ...
        private void UI_DrawInterface(BasePlayer player)
        {
            ...
            for (int i = 0; i < 6; i += 3)
            {
                ...
                foreach (var check in Settings.HitSettings.Skip(i).Take(3))
                {
                    ...
                    {
                        text = "МАРКЕР ВЫБРАН";
                        color = "0.7029 1 0.7 0.12";
                    }
                    else if (!string.IsNullOrEmpty(check.Value.Permission) && !permission.UserHasPermission(player.UserIDString, check.Value.Permission))
                    {
