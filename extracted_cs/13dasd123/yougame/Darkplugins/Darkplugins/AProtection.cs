...
{
    /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ [Info("AProtection", "https://discord.gg/dNGbxafuJn", "1.5.6")]
    public class AProtection : RustPlugin
    {
        ...
        private void OnServerInitialized()
        {
            ...
            timer.Every(300, Unload);
        }

        private void Unload() => Interface.Oxide.DataFileSystem.WriteObject("AProtection", StoredData);
        ...
        private void ForceAuthorization(BasePlayer player)
        {
            ...
            if (!StoredData.ContainsKey(player.userID))
                StoredData.Add(player.userID, new IAuthorize());
            IAuthorize buffer = StoredData[player.userID];
            if (buffer.IsAuthed)
                return;
