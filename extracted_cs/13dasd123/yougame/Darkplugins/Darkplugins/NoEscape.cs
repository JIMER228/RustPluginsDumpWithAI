...
{
    /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ [Info("NoEscape", "https://discord.gg/dNGbxafuJn", "0.1.71")]
    public class NoEscape : RustPlugin
    {
        ...
        private class PlayerBlocked : MonoBehaviour
        {
            ...
            private void Awake()
            {
                ...
                if (!instance.permission.UserHasPermission(Player.UserIDString, Settings.NotificationSettings.PermissionToIgnore))
                    InvokeRepeating(nameof(ControllerUpdate), 1f, 1f);
            }
        ...
        private class Blocker : MonoBehaviour
        {
            ...
            public void Awake()
            {
                ...
                if (Cupboard == null)
                {
                    Cupboard = GetComponent<BaseEntity>().GetBuildingPrivilege();
                }
                else
                {
            ...
            public bool IsInBlocker(BaseEntity player) => Vector3.Distance(player.transform.position, transform.position) < Settings.BlockSettings.BlockerDistance;
        }
        ...
        private class Configuration
        {
            internal class BlockSetting
            {
                ...
                [JsonProperty("ЭКСПЕРИМЕНТАЛЬНО. Отвязать РБ от шкафа (сохранив звуки и мигалку)")]
                public bool ExperimentalFoundation;
        ...
        private void OnLootEntity(BasePlayer player, BaseEntity entity)
        {
            ...
            if (!permission.UserHasPermission(player.UserIDString, Settings.NotificationSettings.PermissionToInstall) && !isOn)
            {
                container.Add(new CuiButton
                {
                    ...
                    Button = { Color = isOn ? "0.501486 0.60850789 0.504524 0" : "0.70465 0.50488 0.50655 0", Command = "UI_NoEscape switch" },
        ...
        private void OnEntityDeath(BaseCombatEntity entity, HitInfo info)
        {
            ...
            if (alreadyBlock)
            {
                BlockerList.ForEach(p =>
                {
                    if (p.IsInBlocker(entity)) p.CurrentTime = 0;
                });
            }
            else
            {
                if (Settings.BlockSettings.ExperimentalFoundation)
                {
                    var newBlock = buildingPrivlidge.GetBuilding()?.buildingBlocks.ToList().GetRandom();
                    if (newBlock != null)
                    {
                        var obj = newBlock.gameObject.AddComponent<Blocker>();
                        BlockerList.Add(obj);
                    }
                }
        ...
        private void OnServerInitialized()
        {
            ...
            permission.RegisterPermission(Settings.NotificationSettings.PermissionToIgnore, this);
            timer.Once(1, () =>
            {
                BasePlayer.activePlayerList.ToList().ForEach(OnPlayerInit);
                PrintError("NoEscape initialized");
            });
        ...
        {
            if (IsBlocked(player)) return "Вы не можете удалять объекты во время рейда!";
            return null;
        }

        private string canTeleport(BasePlayer player) // ALIAS FOR MagicTeleportation
        {
            if (IsBlocked(player)) return "Вы не можете телепортироваться во время рейда!";
            return null;
        }
