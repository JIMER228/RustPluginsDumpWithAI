...
{
    [Info("PveMode", "Sempai#3239", "1.0.3")]
    internal class PveMode : RustPlugin
    {
        ...
        #region Scientists
        private void ScientistAddPveMode(ScientistNPC npc) => _scientists.Add(npc.net.ID, npc.gameObject.AddComponent<ControllerScientist>());
        ...
        private void EventAddPveMode(string shortname, JObject configJson, Vector3 position, float radius, HashSet<uint> crates, HashSet<uint> scientists, HashSet<uint> tanks, HashSet<ulong> owners, BasePlayer owner = null)
        {
        ...
        private object CanLootEntity(BasePlayer player, StorageContainer container)
        {
            ...
            ControllerEvent controllerEvent = _events.FirstOrDefault(x => x.Owner != 0 && x.Crates.Contains(container.net.ID));
            if (controllerEvent != null)
            {
        ...
        private object CanHackCrate(BasePlayer player, HackableLockedCrate crate)
        {
            ...
            ControllerEvent controllerEvent = _events.FirstOrDefault(x => x.Owner != 0 && x.Crates.Contains(crate.net.ID));
            if (controllerEvent != null)
            {
        ...
        private object OnEntityTakeDamage(BradleyAPC bradley, HitInfo info)
        {
            ...
            ControllerEvent controllerEvent = _events.FirstOrDefault(x => x.Tanks.Contains(bradley.net.ID));
            if (controllerEvent != null)
            {
                if (controllerEvent.Owner == 0)
                {
        ...
        private object CanEntityTakeDamage(ScientistNPC npc, HitInfo info)
        {
            ...
            ControllerEvent controllerEvent = _events.FirstOrDefault(x => x.Owner != 0 && x.Scientists.Contains(npc.net.ID));
            if (controllerEvent != null)
            {
                if (controllerEvent.Config.DamageNpc || IsTeam(attacker, controllerEvent.Owner)) return null;
                else return false;
        ...
        private object CanEntityTakeDamage(BradleyAPC bradley, HitInfo info)
        {
            ...
            ControllerEvent controllerEvent = _events.FirstOrDefault(x => x.Owner != 0 && x.Scientists.Contains(bradley.net.ID));
            if (controllerEvent != null)
            {
                if (controllerEvent.Config.DamageTank || IsTeam(attacker, controllerEvent.Owner)) return null;
                else return false;
        ...
        private object OnNpcTarget(ScientistNPC attacker, BasePlayer player)
        {
            ...
            ControllerEvent controllerEvent = _events.FirstOrDefault(x => x.Owner != 0 && x.Scientists.Contains(attacker.net.ID));
            if (controllerEvent != null)
            {
        ...
        private object CanBradleyApcTarget(BradleyAPC bradley, BasePlayer player)
        {
            ...
            ControllerEvent controllerEvent = _events.FirstOrDefault(x => x.Owner != 0 && x.Tanks.Contains(bradley.net.ID));
            if (controllerEvent != null)
            {
