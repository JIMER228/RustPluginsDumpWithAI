...
{
    /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ [Info("EggsLoot", "https://discord.gg/dNGbxafuJn", "1.0.7")]
    [Description("Modify the loot tables for the eggs")]

    class EggsLoot : RustPlugin
    {
        ...
        {
            smallMinItems = Convert.ToInt16(GetConfig("Bronze Eggs", "Min Items", 1));
            smallMaxItems = Convert.ToInt16(GetConfig("Bronze Eggs", "Max Items", 2));
            smallPresentsToUse = Convert.ToInt16(GetConfig("Bronze Eggs", "Num Needed To Unwrap", 1));

            mediumMinItems = Convert.ToInt16(GetConfig("Silver Eggs", "Min Items", 1));
            mediumMaxItems = Convert.ToInt16(GetConfig("Silver Eggs", "Max Items", 2));
            mediumPresentsToUse = Convert.ToInt16(GetConfig("Silver Eggs", "Num Needed To Unwrap", 1));

            largeMinItems = Convert.ToInt16(GetConfig("Gold Eggs", "Min Items", 1));
            largeMaxItems = Convert.ToInt16(GetConfig("Gold Eggs", "Max Items", 2));
            largePresentsToUse = Convert.ToInt16(GetConfig("Gold Eggs", "Num Needed To Unwrap", 1));
        ...
        private void Init()
        {
            PresentData = Interface.Oxide.DataFileSystem.GetFile("EggsLoot/BronzeEggs");
            PresentData2 = Interface.Oxide.DataFileSystem.GetFile("EggsLoot/SilverEggs");
            PresentData3 = Interface.Oxide.DataFileSystem.GetFile("EggsLoot/GoldEggs");
            LoadData();
            AddDefaultItems();
            LoadVariables();
            amountNeededDic.Add("easter.goldegg", largePresentsToUse); amountNeededDic.Add("easter.silveregg", mediumPresentsToUse); amountNeededDic.Add("easter.bronzeegg", smallPresentsToUse);
        ...
        private object OnItemAction(Item item, string action)
        {
            if (item == null || action == null || action == "")
                return null;
            if (item.info.shortname != "easter.goldegg" && item.info.shortname != "easter.silveregg" && item.info.shortname != "easter.bronzeegg")
                return null;
            if (action != "unwrap")
                return null;
            ...
            Effect.server.Run("assets/prefabs/misc/easter/ui/bunnyentry.prefab", player.transform.position);
            return false;
        }
