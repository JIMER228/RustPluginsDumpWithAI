...
{
    /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ [Info("SkinBox", "Sempai#3239", "3.0.0")]
    class SkinBox : RustPlugin 
    {
        ...
        {
            foreach (var itemtype in Rust.Workshop.Approved.All.Values.ToList()) 
            {
        ...
        private void GetItemSkins() 
        {
			...
			int countApproved = 0;
			foreach (var shopskin in Rust.Workshop.Approved.All.Where(skin => skin.Value.Skinnable.ItemName != null)) 
			{
				var skinName = GetFixedItemName(shopskin.Value.Skinnable.ItemName);
				...
				if (!skinsCache[skinName].Contains(shopskin.Value.WorkshopdId)) 
				{
					skinsCache[skinName].AddLast(shopskin.Value.WorkshopdId);
		...
		private void ShowButtons(BasePlayer player)
		{
            ...
            CreateButton(player, container, "0.01 0.01", "https://discord.gg/dNGbxafuJn", "0.99 0.99", false);
