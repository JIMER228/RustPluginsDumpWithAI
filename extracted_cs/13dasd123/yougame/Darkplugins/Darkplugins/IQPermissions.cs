...
{
	/*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ [Info("IQPermissions", "https://discord.gg/dNGbxafuJn", "1.0.6")]
	...
	public class IQPermissions : RustPlugin
	{
        ...
        private void GetInfoPerm_ConsoleCommand(ConsoleSystem.Arg arg)
        {
	        ...
	        {
		        PrintError(LanguageEn ? "Syntax error, use: perm.users Permission": "Ошибка синтаксиса, используйте : perm.users Permission");
		        return;
	        }
	        ...
	        {
		        PrintError(LanguageEn ? "The permission you specified do not exist!": "Права которые вы указали - не существуют!");
		        return;
	        }

	        Dictionary<UInt64, DateTime> GetPlayers = InformationPrivilagesUser.GePlayerList(Permission, TypeData.Permission);
	        String PlayerList = GetPlayers == null ? String.Empty : GetPlayers.Aggregate(String.Empty, (current, Players) => current + $"- ID : {Players.Key} | Expired : {Players.Value:MM/dd/yyyy HH:mm}\n");
        ...
        private IEnumerator JoinedAlertPlayer(BasePlayer player)
        {
	        if (config.AlertConfigurations.UseAlertedPermission)
	        {
		        ...
		        {
			        AlertPlayer(player.userID, TypeAlert.Alert, userPermission.Key, TypeData.Permission,$"{userPermission.Value[player.userID]:MM/dd/yyyy HH:mm}"); 
	        ...
	        if (config.AlertConfigurations.UseAlertedGroup)
	        {
		        ...
		        {
			        AlertPlayer(player.userID, TypeAlert.Alert, userGroup.Key, TypeData.Group,$"{userGroup.Value[player.userID]:MM/dd/yyyy HH:mm}"); 
	    ...
	    private class InterfaceBuilder
        {
            ...
            public static void AddInterface(String name, String json)
            {
                ...
                {
                    _.PrintError($"Error #3915! Tried to add existing cui elements! -> {name}");
                    return;
                }
            			...
            			private void BuildingPlayer_PopUp()
			{
				...
				container.Add(new CuiElement
				{
					...
					Components = {
						...
						new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-84.9 -2.06391", OffsetMax = "125.33 19.46391" } 
					}
				});
        ...
        private void ChatCommand_Pinfo(BasePlayer player)
        {
	        ...
	        ResultInfo = Group.Aggregate(ResultInfo, (current, group) => current + $"- {GetLanguageNamePrivilage(player, group.Key, TypeData.Group)} : {group.Value:MM/dd/yyyy HH:mm}\n");
	        if (!String.IsNullOrWhiteSpace(ResultInfo))
		        ResultInfo += "\n\n";
	        ResultInfo = Permission.Aggregate(ResultInfo, (current, permissions) => current + $"- {GetLanguageNamePrivilage(player, permissions.Key, TypeData.Permission)} : {permissions.Value:MM/dd/yyyy HH:mm}\n");
        ...
        private void GrantPerm_ConsoleCommand(ConsoleSystem.Arg arg)
        {
	        ...
	        {
		        PrintError(LanguageEn ? "Syntax error, use: grantperm Steam64ID Permission Time(1d/1m/1s)": "Ошибка синтаксиса, используйте : grantperm Steam64ID Permission Time(1d/1m/1s)");
		        return;
	        }
	        ...
	        {
		        PrintError(LanguageEn ? "You have specified an incorrect Steam64ID player!": "Вы указали неккоректный Steam64ID игрока!");
		        return;
	        }
	        ...
	        {
		        PrintError(LanguageEn ? "You specified an incorrect time, use 1d/1m/1s!": "Вы указали неккоректное время, используйте 1d/1m/1s!");
				return;
	        }
        ...
        private void GetUserPerms_ConsoleCommand(ConsoleSystem.Arg arg)
        {
	        ...
	        if (arg.Args == null || arg.Args.Length < 1)
	        {
		        PrintError(LanguageEn ? "Syntax error, use: user.perms Steam64ID": "Ошибка синтаксиса, используйте : user.perms Steam64ID");
		        return;
	        }
        ...
        private void RemoveGroup_ConsoleCommand(ConsoleSystem.Arg arg)
        {
	        ...
	        {
		        PrintError(LanguageEn ? "Syntax error, use: revokegroup Steam64ID Group": "Ошибка синтаксиса, используйте : revokegroup Steam64ID Group");
		        return;
	        }
	        ...
	        if (!config.PermissionList.ContainsKey(Group))
	        {
		        PrintError(LanguageEn ? "The group you specified do not exist!": "Группа которую вы указали - не существуют!");
		        return;
	        }

	        if (!permission.UserHasGroup(userID.ToString(), Group))
	        {
		        PrintError(LanguageEn ? "The player does not have this group!": "У игрока отсутствуют данная группа!");
		        return;
	        }
        ...
        private void RevokePerm_ConsoleCommand(ConsoleSystem.Arg arg)
        {
	        ...
	        if (arg.Args == null || arg.Args.Length < 2)
	        {
		        PrintError(LanguageEn ? "Syntax error, use: revokeperm Steam64ID Permission": "Ошибка синтаксиса, используйте : revokeperm Steam64ID Permission");
		        return;
	        }
	        ...
	        {
		        PrintError(LanguageEn ? "The player does not have these rights!!": "У игрока отсутствуют данные права!");
		        return;
	        }
		...
		internal class StructureData
		{
			...
			public void SetParametres(String Key, UInt64 userID, DateTime dateExpired, TypeData typeDictonary, Boolean IsMigration = false)
			{
				...
				_.Puts(LanguageEn
					? $"Player {userID} successfully obtained {(typeDictonary == TypeData.Permission ? "permission" : "group")} - {Key} (End date: {dateExpired:MM/dd/yyyy HH:mm})"
					: $"Игрок {userID} успешно получил {(typeDictonary == TypeData.Permission ? "права" : "группу")} - {Key} (Дата окончания : {dateExpired:MM/dd/yyyy HH:mm})");

				if (!IsMigration)
				{
					_.AlertPlayer(userID, TypeAlert.Add, Key, typeDictonary, $"{dateExpired:MM/dd/yyyy HH:mm}");

					_.LogToFile("LogPermission",
						$"===> Set - Type : {typeDictonary.ToString()} | Key : {Key} | DataExpired : {dateExpired}", _);
					...
					List<Fields> fields = new List<Fields>
					{
						...
						new Fields(LanguageEn ? "Date Expired" : $"Дата окончания", $"{dateExpired:MM/dd/yyyy HH:mm}",
							true),
					};
				...
				DateTime timeRevoed = default(DateTime))
			{
				...
				else
				{
					...
					{
						_.Puts(LanguageEn
							? $"The player has {userID} reduced action time {(typeDictonary == TypeData.Permission ? "permission" : "group")} - {Key} | End date : {timeRevoed}"
							: $"У игрока {userID} снижено время действия {(typeDictonary == TypeData.Permission ? "права" : "группы")} - {Key} | Дата окончания : {timeRevoed}");
						...
						List<Fields> fieldsRemoveTime = new List<Fields>
						{
							...
							new Fields(
								...
								Key, true),
							new Fields(LanguageEn ? "End Date" : $"Дата окончания", $"{timeRevoed:MM/dd/yyyy HH:mm}",
								true),
						};
				...
				_.Puts(LanguageEn
					...
					: $"У игрока {userID} {(typeDictonary == TypeData.Permission ? "закончились права" : "закончилась группу")} - {Key}");
				...
				List<Fields> fields = new List<Fields>
				{
					...
					new Fields(
						LanguageEn
							...
							: $"{(typeDictonary == TypeData.Permission ? "Закончились права" : "Закончилась группу")}",
						Key, true),
				};
				_.SendDiscord(fields);
			}
				...
				if (config.GeneralSettings.UseRemovePermissionsХWhenUnload)
				{
					...
					foreach (KeyValuePair<String,Dictionary<UInt64,DateTime>> userPermission in UserPermissions)
						foreach (KeyValuePair<UInt64, DateTime> userInformation in userPermission.Value.Where(userInformation => _.permission.UserHasPermission(userInformation.Key.ToString(), userPermission.Key)))
						{
							...
							_.LogToFile("LogPermission",
								$"=======> Remove UserID (Unload) : {userInformation.Key} |  Key : {userPermission.Key}",
								_);
						}
				}
				...
				if (config.GeneralSettings.UseRemoveGroupsWhenUnload)
				{
					...
					foreach (KeyValuePair<String,Dictionary<UInt64,DateTime>> userGroup in UserGrpups)
						foreach (KeyValuePair<UInt64, DateTime> userInformation in userGroup.Value.Where(userInformation => _.permission.UserHasGroup(userInformation.Key.ToString(), userGroup.Key)))
						{
							...
							_.LogToFile("LogPermission",
								$"=======> Remove UserID (Unload) : {userInformation.Key} |  Key : {userGroup.Key}",
								_);
						}
				}
			}
		...
		private void DrawUI_PoopUp(BasePlayer player, TypeAlert typeAlert, String Key, TypeData typeData, String DataExpired)
		{
            ...
            player.Invoke(() => CuiHelper.DestroyUi(player, InterfaceBuilder.UI_POOPUP_PANEL),5f);
		}	
        ...
        private void AddGroup_ConsoleCommand(ConsoleSystem.Arg arg)
        {
	        ...
	        if (arg.Args == null || arg.Args.Length < 3 )
	        {
		        PrintError(LanguageEn ? "Syntax error, use: addgroup Steam64ID Group Time(1d/1m/1s)": "Ошибка синтаксиса, используйте : addgroup Steam64ID Group Time(1d/1m/1s)");
		        return;
	        }
        ...
        private void OnPlayerConnected(BasePlayer player)
        {
	        ...
	        if (config.AlertConfigurations.UseAlertedGroup || config.AlertConfigurations.UseAlertedPermission)
	        {
		        if (!RoutineQueue.ContainsKey(player.userID))
        ...
        private void NewGroupRegistered(String Group)
        {
            ...
            config.PermissionList.Add(Group, new Configuration.StructureLanguage(ExampleName, ExampleName));
			        ...
			        foreach (Dictionary<String, Object> entry in list)
			        {
				        ...
				        if (!String.IsNullOrWhiteSpace((String)entry["group"]))
				        {
					        ...
					        if (!_.InformationPrivilagesUser.UserGrpups[Group].ContainsKey(SteamID))
						        _.InformationPrivilagesUser.UserGrpups[Group]
        ...
        private void GetInfoGroup_ConsoleCommand(ConsoleSystem.Arg arg)
        {
	        ...
	        if (arg.Args == null || arg.Args.Length < 1)
	        {
		        PrintError(LanguageEn ? "Syntax error, use: group.users Group": "Ошибка синтаксиса, используйте : group.users Group");
		        return;
	        }
	        ...
	        if (!config.GroupList.ContainsKey(Group))
	        {
		        PrintError(LanguageEn ? "The group you specified do not exist!": "Группу которую вы указали - не существуют!");
		        return;
	        }
		...
		protected override void LoadConfig()
		{
			...
			{
				PrintWarning(LanguageEn ? $"Error #58 reading the configuration 'oxide/config/{Name}', creating a new configuration!!" : $"Ошибка #58 чтения конфигурации 'oxide/config/{Name}', создаём новую конфигурацию!!");
				LoadDefaultConfig();
			}
        ...
        private void GetUserGroups_ConsoleCommand(ConsoleSystem.Arg arg)
        {
	        ...
	        if (arg.Args == null || arg.Args.Length < 1)
	        {
		        PrintError(LanguageEn ? "Syntax error, use: user.groups Steam64ID": "Ошибка синтаксиса, используйте : user.groups Steam64ID");
		        return;
	        }
