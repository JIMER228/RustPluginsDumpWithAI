...
{
    /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ [Info("IQAlcoholFarm", "https://discord.gg/dNGbxafuJn", "1.0.2")]
    ...
    internal class IQAlcoholFarm : RustPlugin
    {
        ...
        private class ImageUi
        {
            ...
            private static IEnumerator AddImage()
            {
                ...
                foreach (ImageStruct Icons in KeyImages)
                {
                    ...
                    {
                        if (Icons.SkinID != 0)
                            URL = $"http://api.skyplugins.ru/api/getskin/{Icons.SkinID}/64";
                        else URL = $"https://api.skyplugins.ru/api/getimage/{Icons.URL}/64";
                    }
        ...
        private IEnumerator PlayersUpdate(Boolean afterReload = false)
        {
            ...
            yield break;
                ...
                void ReadData()
        {
            try { RenderSounds = Oxide.Core.Interface.GetMod().DataFileSystem.ReadObject<Dictionary<String, List<Byte[]>>>("IQSystem/IQAlcoholFarm/Sounds"); }
                ...
                CuiElementContainer container = new CuiElementContainer
                {
                    ...
                    new CuiElement
                    {
                        ...
                        Components = {
                            ...
                            new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-181.062 216.907", OffsetMax = "-123.298 235.8930949" }
                        }
                    },
                    ...
                    new CuiElement
                    {
                        ...
                        Components = {
                            ...
                            new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-50.482 216.907", OffsetMax = "7.282 235.8930949" }
                        }
                    },
            ...
            private void BuildingTrader_ItemPanel()
            {
                CuiElementContainer container = new CuiElementContainer
                {
                    ...
                    new CuiElement
                    {
                        ...
                        Components = {
                            new CuiRawImageComponent { Color = "1 1 1 1", Png = "%BUY_ITEM_IMG%" },
                    ...
                    new CuiElement
                    {
                        ...
                        Components = {
                            new CuiRawImageComponent { Color = "1 1 1 1", Png = "%PRICE_ITEM_IMG%" },
                            new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-24 -21.3", OffsetMax = "24 26.7" }
                        }
                    },
        ...
        private void GetSounds()
        {
            ...
            timer.Once(3f, () =>
            {
                ...
                try
                {
                    webrequest.Enqueue($"http://iqsystem.skyplugins.ru/iqalcohol/getsound/W5fgSD3dfVBxc", null, (code, response) =>
                    {
                        switch (code)
                        {
                            case 404:
                                {
                                    PrintError($"ERROR #694976  {response} | ERROR #:545445  (Discord - Mercury#5212)"); ///
                                _initiated = false;
                                    break;
                                }
                            case 503:
                                {
                                    PrintError($"ERROR #45494945 Your plugin version is outdated!Upgrade to the latest version! (Discord - Mercury#5212)");
                                    _initiated = false;
                                    break;
                                }
                            case 200:
                ...
                {
                    PrintError($"ERROR #8594973 An error occurred while connecting, please inform the developer Discord - Mercury#5212\nError : {e}");
                }
        ...
        private void TakeItems(BasePlayer player, PluginConfig.ShopStore.CategorySettings.ItemsPair BuyItem, ItemContainer container = null, Int32 Amount = 1)
        {
            ...
            {
                if (ItemPlayer.info.shortname.Equals(BuyItem.PriceItems.Shortname) && ItemPlayer.skin == BuyItem.PriceItems.SkinID)
                {
                    if (TargetAmount <= 0) continue;
                    TargetAmount -= ItemPlayer.amount;
                    ItemPlayer.UseItem(TargetAmount > 0 ? TargetAmount : BuyItem.PriceItems.Amount * Amount);
                }
                else
                {
                    if (ItemPlayer.contents != null)
                    {
                        foreach (Item item1 in ItemPlayer.contents.itemList)
                        {
                            if (item1.info.shortname.Equals(BuyItem.PriceItems.Shortname) && item1.skin == BuyItem.PriceItems.SkinID)
                            {
                                ...
                                item1.UseItem(TargetAmount > 0 ? TargetAmount : BuyItem.PriceItems.Amount * Amount);
                            }
                        }
                    }
                }
        ...
        private System.Object CanHideStash(BasePlayer player, StashContainer stash)
        {
            ...
            if (BarrelController.ActiveControllers.Exists(x => x.GetStashId() == stash.net.ID))
                return false;
        ...
        /// <summary>
        /// Обновление 1.0.1
        /// - Если у ягод в магазине установлен SkinID их будет невозможно съесть
        /// Обновление 1.0.2
        /// - Исправлен проблема с покупкой или обменом у торговца из-за стаков в инвентаре
        /// </summary>
        ...
        private System.Object OnWireConnect(BasePlayer player, IOEntity entity1, Int32 inputs, IOEntity entity2, Int32 outputs)
        {
            if (_initiated == false)
                return null;
            ...
            if (BarrelController.ActiveControllersIds.Exists(x => x == entity1.net.ID || x == entity2.net.ID))
                return false;
        ...
        private class CustomNpcController : MonoBehaviour
        {
            ...
            {
                if (BotAlerts.Contains(_npcPlayer.net.ID))
                    return;
                else
                    BotAlerts.Add(_npcPlayer.net.ID);
            ...
            public IEnumerator API_NPC_SendToAll()
            {
                ...
                {
                    SoundRoutine = null;
                    BotAlerts.Remove(_npcPlayer.net.ID);
                    yield break;
                }
                ...
                foreach (var data in Data)
                {
                    if (_npcPlayer == null)
                        break;
                    SendSound(_npcPlayer.net.ID, data);
            ...
            public void SendSound(uint netId, byte[] data)
            {
        ...
        {
            foreach (KeyValuePair<UInt32, BaseNetworkable> keyValuePair in BaseNetworkable.serverEntities.entityList)
            {
        ...
        void WriteData() => Oxide.Core.Interface.Oxide.DataFileSystem.WriteObject("IQSystem/IQAlcoholFarm/Sounds", RenderSounds, true);
        ...
        private class BarrelController : MonoBehaviour
        {
            ...
            public static List<UInt32> ActiveControllersIds = new List<UInt32>();
            ...
            public UInt32 GetStashId()
            {
                if (_stash == null)
                    return 0;

                return _stash.net.ID;
            }
            ...
            private void Awake()
            {
                ...
                if (ActiveControllersIds.Contains(_barrel.net.ID) == false)
                    ActiveControllersIds.Add(_barrel.net.ID);
            ...
            private void OnDestroy()
            {
                ...
                ActiveControllersIds.Remove(_barrel.net.ID);
            }
            ...
            private void ProcessTick()
            {
                ...
                if (item == null)
                {
                    Item water = ItemManager.CreateByName("water", tickOutput, _config.itemSettings.WortItemSkinId);
                    if (water == null)
                        return;
        ...
        private void DrawUI_Items(BasePlayer player, CategoryType categoryType = CategoryType.Equipment, Int32 Page = 0)
        {
            ...
            foreach (PluginConfig.ShopStore.CategorySettings.ItemsPair Category in CategoryItems.Skip(Page * 5).Take(5))
            {
                ...
                Interface = Interface.Replace("%BUY_ITEM_IMG%", ImageUi.GetImage($"{Category.BuyItems.Shortname}_{Category.BuyItems.SkinID}"));
                Interface = Interface.Replace("%PRICE_ITEM_IMG%", ImageUi.GetImage($"{Category.PriceItems.Shortname}_{Category.PriceItems.SkinID}"));
        ...
        public void InitializeNPC(Vector3 pos, Quaternion rot)
        {
            ...
            {
                npc.inventory.Strip();
                for (Int32 i = 0; i < _config.storeNpc.NPCShopSetting.Wear.Count; i++)
                    ItemManager.Create(ItemManager.FindItemDefinition(_config.storeNpc.NPCShopSetting.Wear[i].ShortName), 1, _config.storeNpc.NPCShopSetting.Wear[i].SkinId).MoveToContainer(npc.inventory.containerWear);
            }
