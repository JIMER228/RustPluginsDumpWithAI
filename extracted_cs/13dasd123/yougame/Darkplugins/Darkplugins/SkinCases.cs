...
{
    /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ [Info("SkinCases", "https://discord.gg/dNGbxafuJn", "0.1.0")]
    public class SkinCases : RustPlugin
    {
        ...
        private class Configuration
        {
            ...
            public class Drops
            {
                public class Drop
                {
                    ...
                    [JsonProperty("Дата следующего выпадения бесплатного кейса")]
                    public string NextDropTime;
                }
                ...
                return new Configuration
                {
                    CaseSettings = new Cases
                    {
                        CaseLits = new List<Case>
                        {
                            ...
                            }, "https://mycsgo.gg/public/img/cases/bomj.png"),
                            ...
                            }, "https://mycsgo.gg/public/img/cases/fry.png"),
                            ...
                            }, "https://mycsgo.gg/public/img/cases/pikachu.png"),
                            ...
                            new Case("Пися кейс", new List<CaseItem>
                            {
                            ...
                            }, "https://mycsgo.gg/public/img/cases/brazil_777.png"),
                            ...
                            }, "https://mycsgo.gg/public/img/cases/boomboom.png"),
                        }
                    },
                    ChancesSettings = new Chances
                    {
                        ...
                        {
                            new Rarity(35, "1.00 1.00 1.00 0.3"),
                            new Rarity(30, "0.68 0.87 1.00 0.3"),
                            new Rarity(25, "0.77 0.65 1.00 0.3"),
                            new Rarity(5,  "1.00 0.68 0.17 0.3"),
                        },
                    ...
                    DropSettrings = new Drops
                    {
                        DropList = new List<Drops.Drop>
                        {
                            new Drops.Drop
                            {
                                ...
                                Interval = 86400,
                                MaxCaseAmount = 10,
                                NextDropTime = DateTime.Now.ToLocalTime().AddHours(24).ToString("dd/MM/yyyy H:mm"),
                            },
                            new Drops.Drop
                            {
        ...
        private void OnServerInitialized()
        {
            ...
            CaseHandler.CaseSettings.CaseLits.ForEach(c =>
            {
                ...
                c.Items.ForEach(i =>
                {
                    if (!string.IsNullOrEmpty(i.ShortName))
                    {
                        ImageLibrary.Call("AddImage", $"http://api.rustplugin.ru/store/100x100/{i.ShortName}.png", i.ShortName + 100, i.SkinID);
                    }

                    if (!string.IsNullOrEmpty(i.PictureURL))
                    {
        ...
        private IEnumerator DrawLine(BasePlayer player, List<CaseItem> skinList, Case cCase, bool really = false)
        {
                ...
                foreach (var check in localList.Select((i, t) => new { A = i, B = t }))
                {
                    try
                    {
                        ...
                        container.Add(new CuiButton
                        {
                            RectTransform =
                            {
                                AnchorMin = $"{0.3375946984 + check.B * kitMargin} 0.7",
                                AnchorMax = $"{0.3375946984 + check.B * kitMargin} 0.7",
                                OffsetMin = "-50 -50", OffsetMax = "50 50",
                            },
                            Button = { Color = CaseHandler.ChancesSettings.Rarities.ElementAtOrDefault(check.A.RarityType).Color.Remove(6, 2) + 23 },
                            Text = { Text = "" }
                        }, Layer, Layer + $".Prize.{check.B}");

                        string img = !string.IsNullOrEmpty(check.A.PictureURL) ? GetNameFromURI(check.A.PictureURL) : check.A.ShortName;
                        string imageName = check.A.SkinID == 0 && string.IsNullOrEmpty(check.A.PictureURL) ? check.A.ShortName + 100 : img;
            ...
            if (!really)
            {
                ...
                {
                    RectTransform = { AnchorMin = "0.5 0.52", AnchorMax = "0.5 0.52", OffsetMin = "-150 -20", OffsetMax = "150 20" },
                    Button = { Color = "1 1 1 0.4", Command = HasCase(player, cCase) ? $"UI_SkinCases open {caseIndex}" : "" },
            ...
            else
            {
                ...
                newCont.Add(new CuiButton
                {
                    ...
                    Button = { Color = "1 1 1 0.4", Command = $"UI_SkinCases show {caseIndex}" },
                ...
                newCont.Add(new CuiButton
                {
                    ...
                    Button = { Color = "1 1 1 0.4", Command = HasCase(player, cCase) ? $"UI_SkinCases open {caseIndex}" : "UI_SkinCases shop" },
                    Text = { Text = openCase, Align = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", FontSize = 20 }
                }, Layer, Layer + ".Btn1");
        ...
        private void UI_DrawCaseInventory(BasePlayer player, int page = 0)
        {
            ...
            foreach (var check in list.Select((i, t) => new { A = i, B = t }))
            {
                ...
                container.Add(new CuiButton
                {
                    RectTransform = { AnchorMin = $"0.5 0.5", AnchorMax = $"0.5 0.5", OffsetMin = $"{currentX} {currentY - 200 - 20}", OffsetMax = $"{currentX + 200} {currentY - 50}" },
            ...
            container.Add(new CuiLabel
            {
                ...
                Text = {Text = "Вы получаете <b>бесплатные кейсы</b>, просто играя на сервере!\n", Align = TextAnchor.UpperCenter, Font = "robotocondensed-regular.ttf", Color = "1 1 1 0.7"}
            }, Layer);
        ...
        private void UI_DrawInventory(BasePlayer player, int page = 0)
        {
            ...
            container.Add(new CuiButton
            {
                ...
                Button = { Color = "1 1 1 0.4", Command = $"UI_SkinCases shop" },
            ...
            if (PlayerInventories[player.userID].Inventory.Count == 0)
            {
                ...
                container.Add(new CuiButton
                {
                    ...
                    Text = { Text = $"У ВАС НЕТУ ПРЕДМЕТОВ\n" +
                                    ...
                                    $"<size=16>КУПИТЕ VIP, ЧТОБЫ ПОЛУЧАТЬ БОЛЬШЕ ПРЕДМЕТОВ</size>", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", FontSize = 34 }
                }, Layer + ".SkinsHandler");
            ...
            if (page > 0)
            {
                container.Add(new CuiButton
                {
                    ...
                    Button = { Color = "1 1 1 0.4", Command = $"UI_SkinCases invpage {page - 1}" },
            ...
            float currentX = -600;
            float currentY = Mathf.CeilToInt((list.Count() / 11f) - 0.010469f) / 2f * 106f;
            ...
            foreach (var check in list.Select((i, t) => new { A = i, B = t }))
            {
                ...
                container.Add(new CuiButton
                {
                    ...
                    Button = { Color = "1 1 1 0.4", Command = $"UI_SkinCases take {check.B + page * 44}" },
                    Text = { Text = "" }
        ...
        private void UI_DrawCase(BasePlayer player, Case currentCase)
        {
            ...
            container.Add(new CuiButton
            {
                RectTransform = { AnchorMin = "0.3375946984 0.7", AnchorMax = "0.65759839284694 0.7", OffsetMin = "-50 0", OffsetMax = "50 60" },
                Button = { Color = "1 1 1 0.4" },
                Text = { Text = "" }
            }, Layer);
            ...
            container.Add(new CuiButton
            {
                RectTransform = { AnchorMin = "0.3375946984 0.7", AnchorMax = "0.65759839469284 0.7", OffsetMin = "-50 -60", OffsetMax = "50 0" },
                Button = { Color = "1 1 1 0.4" },
                Text = { Text = "" }
            }, Layer);
            ...
            {
                RectTransform = { AnchorMin = "0.5 0.52", AnchorMax = "0.5 0.52", OffsetMin = "-150 -20", OffsetMax = "150 20" },
                Button = { Color = "1 1 1 0.4", Command = $"UI_SkinCases open {CaseHandler.CaseSettings.CaseLits.IndexOf(currentCase)}" },
            ...
            container.Add(new CuiButton
            {
                ...
                Button = { Color = "1 1 1 0.4469", Command = $"UI_SkinCases inv" },
            ...
            container.Add(new CuiButton
            {
                ...
                Button = { Color = "1 1 1 0.4", Command = $"UI_SkinCases shop 0" },
                Text = { Text = $"МОИ КЕЙСЫ", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", FontSize = 20 }
            }, Layer, Layer + ".Shop");
            ...
            foreach (var check in currentCase.Items.OrderBy(p => p.RarityType).Select((i, t) => new { A = i, B = t }))
            {
                ...
                container.Add(new CuiButton
                {
                    ...
                    Button = { Color =CaseHandler.ChancesSettings.Rarities.ElementAtOrDefault(check.A.RarityType).Color},
                    Text = { Text = "" }
                }, Layer + ".SkinsHandler", Layer + $".{check.B}");
        ...
        private IEnumerator GiveCases()
        {
            while (true)
            {
                ...
                {
                    if (CurrentTime() > TimeTotalSeconds(p.NextDropTime))
                    {
                        try
                        {
                            p.NextDropTime = DateTime.Now.ToLocalTime().AddSeconds(p.Interval).ToString("dd/MM/yyyy H:mm");
                ...
                x.Stop();
                PrintWarning($"Giving free cases finish. Elapsed time: {x.Elapsed.ToShortString()}:{x.Elapsed.Milliseconds}. Affected: {affected}");
                SaveConfig();
        ...
        private double CurrentTime() => DateTime.UtcNow.ToLocalTime().Subtract(new DateTime(1970, 1, 1, 0, 0, 0)).TotalSeconds;

        private DateTime ParseTime(string time) => DateTime.ParseExact(time, "dd/MM/yyyy H:m", CultureInfo.InvariantCulture);
        private double TimeTotalSeconds(string type) => ParseTime(type).Subtract(new DateTime(1970, 1, 1, 0, 0, 0)).TotalSeconds;
