...
{
    /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ [Info("IQBackpack", "https://discord.gg/dNGbxafuJn", "1.0.15")]
    ...
    class IQBackpack : RustPlugin
    {
        ...
        private class Configuration
        {
            ...
            internal class Turneds
            {
                ...
                [JsonProperty("Автоматически очищать инвентарь рюкзаков игроков после вайпа (true - да/false - нет)")]
...
//            shopFront.inventory.entityOwner = shopFront;
            ...
            shopFront.inventory.allowedContents = ItemContainer.ContentsType.Generic;
        ...
        public static StorageContainer CreateContainer(BasePlayer player)
        {
            ...
            StorageContainer containerEntity = storage as StorageContainer;
            ...
            foreach (Collider collider in storage.GetComponentsInChildren<Collider>())
                UnityEngine.Object.DestroyImmediate(collider);
       ...
       ItemContainer.CanAcceptResult? CanAcceptItem(ItemContainer container, Item item, int targetPos)
       {
                   ...
                   if (config.TurnedsSetting.Types == TypeBackpack.Wear)
                   {
                       NextTick(() =>
                       {
                           ...
                           if (item.parent == null || item.parent.entityOwner == null ||
                               item.parent.entityOwner.net == null) return;
                           if (item.skin != config.BackpackItem.SkinID) return;
                           UInt64 NetID = item.parent.entityOwner.net.ID;
                           if (NetID == 0) return;
                           if (ContainerGetPlayer.ContainsKey(NetID))
                               CommunityEntity.ServerInstance.ClientRPCEx(new SendInfo(ContainerGetPlayer[NetID]), null,
                                   "DestroyUI", "UI_BACKPACK_VISUAL");
                       });
                   }
           ...
           {
               LogToFile("IQBackpack/Debug/Logs", $"===== START ===== \n {e.Message}\n\n{e}\n ===== END =====", this);
               PrintWarning(
                   "Вышло неизвестное действие!Сообщите разработчику об этом и прикрепите .log (IQBackpack/Debug/Logs)");
               throw;
           }
        ...
        /// <summary>
        /// Обновление 1.0.х
        /// - Исправлено уведомление при выдаче прав на рюкзак
        /// - Исправлено удаление рюкзака когда игрок оффлайн и у него кончились права на рюкзак
        /// - Добавлен дебаг-лог
        /// - В описание ресурса добавил инструкцию, как передвинуть UI рюкзака направо
        /// - Добавлена возможность включить автоматическую очистку рюкзаков игроков после вайпа
        /// </summary>
        /// 
