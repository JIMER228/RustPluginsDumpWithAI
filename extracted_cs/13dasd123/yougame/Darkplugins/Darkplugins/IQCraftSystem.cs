...
{
    /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ [Info("IQCraftSystem", "https://discord.gg/dNGbxafuJn", "1.1.7")]
    ...
    internal class IQCraftSystem : RustPlugin
    {
        ...
        private void OnPlayerAttack(BasePlayer player, HitInfo info)
        {
            ...
            if (entity == null || player == null)
                return;
            if (!HealthCustom.ContainsKey(entity.net.ID)) return;

            HealthCustom[entity.net.ID] -= (Int32)info.damageTypes.Total();

            if (HealthCustom[entity.net.ID] <= 0)
                entity.Kill();
        }
        ...
        private void OnHammerHit(BasePlayer player, HitInfo info)
        {
            ...
            Single Health = 0;
            if (HealthCustom.ContainsKey(Entity.net.ID))
            {
                Health = HealthCustom[Entity.net.ID];
                HealthCustom.Remove(Entity.net.ID);
            }
        ...
        private void SpawnItem(BaseEntity entity, UInt64 OwnerID)
        {
            ...
            if (ItemSpawn.CategoryItems == CategoryItem.Entity || ItemSpawn.CategoryItems == CategoryItem.Transport)
            {
                ...
                if (CombatEntity != null && ItemSpawn.HealthItem != 0)
                {
                    ...
                    if (ItemSpawn.CategoryItems == CategoryItem.Entity)
                    {
                        if (!HealthCustom.ContainsKey(SpawnedEntity.net.ID))
                            HealthCustom.Add(SpawnedEntity.net.ID, ItemSpawn.HealthItem);
                    }
                }
