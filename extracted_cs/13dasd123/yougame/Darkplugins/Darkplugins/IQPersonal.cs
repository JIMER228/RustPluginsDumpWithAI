...
{
    /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ [Info("IQPersonal", "https://discord.gg/dNGbxafuJn", "0.0.8")]
    ...
    class IQPersonal : RustPlugin
    {
        ...
        {
            if (HasPermission(player.userID, PermissionUse))
            {
                var User = DataInformation[player.userID];
        ...
        void BalanceSet(ulong UserID)
        {
            ...
            if(Store.GameStores)
            {
                ...
                {
                    PrintWarning("Ошибка #401 : Магазин не настроен! Он будет работать неккоректно!"); 
                    return;
                }
            ...
            if (More.GiveMorePermission)
                foreach (var PermList in More.Permissions)
                    if (HasPermission(userID, PermList))
                        permission.RevokeUserPermission(userID.ToString(), PermList);

            if (HasPermission(userID, PermissionUse))
                permission.RevokeUserPermission(userID.ToString(), PermissionUse);
		   		 		  						  	   		  		 			  	 	 		  	 	 		  	  	
            if (DataInformation[userID].VKLink == null || String.IsNullOrWhiteSpace(DataInformation[userID].VKLink))
            {
                VKSendMessage(String.Format(lang.GetMessage("VK_REMOVE_PERSONAL", this), $"{DataInformation[userID].DisplayName}(Данные ВК неизестны)", userID), VK.VKChatIDPersonal);
                DiscordSendMessage(String.Format(lang.GetMessage("VK_REMOVE_PERSONAL", this), $"{DataInformation[userID].DisplayName}(Данные ВК неизестны)", userID), userID);
        ...
        void DayNormalCheck()
        {
            ...
            foreach (var Data in DataInformation.Where(x => x.Key != 0))
            {
                var User = Data.Value;
        ...
        {
            var User = DataInformation[UserID];
            if (User.DisplayName == null || String.IsNullOrWhiteSpace(User.DisplayName)) return;
            var Controller = config.ControllerSetting;
        ...
        void VKSendMessage(string Message, string CustomChatID = "")
        {
            ...
            {
                PrintWarning("Ошибка #34401: Вы не настроили конфигурацию,в пункте с ВК"); 
                return;
            }
