...
{
    /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ [Info("AbsolutSorter", "https://discord.gg/dNGbxafuJn", "2.0.17")]
    ...
    class AbsolutSorter : RustPlugin
    {
        ...
        private const string PERMISSION_NEARBY = "absolutsorter.nearby";
        ...
        private void Loaded()
        {
            ...
            permission.RegisterPermission(PERMISSION_NEARBY, this);
        ...
        private void OnLootEntity(BasePlayer player, StorageContainer container)
        {
            ...
            if (configData.UseRaidBlocked && (NoEscape && (bool)NoEscape.Call("IsRaidBlocked", player)))
                return;
        ...
        private void ThisSort(BasePlayer player, uint id)
        {
            ...
            {
                StoredData.BoxData data;
                storedData.IsSortingBox(container.net.ID, out data);

                for (int y = items.Count - 1; y >= 0; y--)
                {
                    Item item = items[y];

                    if ((data?.AcceptsItem(item.info) ?? (container.onlyAcceptCategory == ItemCategory.All ? true : item.info.category == container.onlyAcceptCategory)) && item.MoveToContainer(container.inventory))                    
                        items.Remove(item);                    
                }

                if (items.Count == itemCount)
                    player.ChatMessage(msg("Error.NoThisItems", player.userID));
        ...
        private void NearbySort(BasePlayer player, uint id)
        {
            ...
            if (itemCount > 0)
            {
                ...
                if (validSortingContainers == 0)
                    player.ChatMessage(msg("Error.NoNearbyContainers", player.userID));

                else if (items.Count == itemCount)                
                    player.ChatMessage(msg("Error.NoNearbyItems", player.userID));                            
        ...
        private void CreateSortingPanel(BasePlayer player, uint id)
        {
            ...
            if (!IsHidden(player))
            {
                container = UI.Container(SORTING_UI, BG_COLOR, configData.ScreenPosition, true);

                StoredData.BoxData data;
                storedData.boxes.TryGetValue(id, out data);

                UI.Button(ref container, SORTING_UI, BUTTON_COLOR, msg("UI.SelectCategories", player.userID), 10, new UI4(0f, 0.824f, 0.49f, 1f), $"asui.opencategories {id} 0");
                ...
                if (!configData.UseNearbySortPerm || (configData.UseNearbySortPerm && permission.UserHasPermission(player.UserIDString, PERMISSION_NEARBY)))
            ...
            {
                container = UI.Container(SORTING_UI, BG_COLOR, new UI4(configData.ScreenPosition.xMax - 0.0592f, configData.ScreenPosition.yMin + 0.0026f, configData.ScreenPosition.xMax, configData.ScreenPosition.yMin + 0.02548f), true); //new UI4(0.7718f, 0.0126f, 0.831f, 0.03548f)
        ...
        private void SelectCategories(BasePlayer player, uint id, int page)
        {
            CuiHelper.DestroyUi(player, SORTING_UI);
            ...
            CuiElementContainer container = UI.Container(SORTING_UI, BG_COLOR, configData.ScreenPosition, true);//new UI4(0.646f, 0.01f, 0.831f, 0.14f)

            int count = 0;
            int limit = page == 0 ? 14 : itemCategories.Length;

            for (int i = 14 * page; i < limit; i++)
            {
                ...
                UI.Button(ref container, SORTING_UI, data?.HasCategory(category) ?? false ? BUTTON_SELECTED_COLOR : BUTTON_COLOR, category.ToString(), 10, GetPosition(count), $"asui.selectcategory {id} {(int)category} {page}");
                count++;
            }
            if (page == 0)                           
                UI.Button(ref container, SORTING_UI, BUTTON_EXIT_COLOR, msg("UI.NextPage", player.userID), 10, GetPosition(13), $"asui.opencategories {id} 1");
            else UI.Button(ref container, SORTING_UI, BUTTON_EXIT_COLOR, msg("UI.PreviousPage", player.userID), 10, GetPosition(13), $"asui.opencategories {id} 0");
        ...
        private void SelectItems(BasePlayer player, uint id, int page = 0)
        {
            ...
            if (storedData.IsSortingBox(id, out data))
            {
                CuiElementContainer container = UI.Container(SORTING_UI, BG_COLOR, configData.ScreenPosition, true);
        ...
        private void ccmdOpenCategories(ConsoleSystem.Arg arg)
        {
            ...
            SelectCategories(player, id, page);
        }
        ...
        private void ccmdSelectCategory(ConsoleSystem.Arg arg)
        {
            ...
            SelectCategories(player, id, arg.GetInt(2));
        }
        ...
        private class ConfigData
        {
            ...
            [JsonProperty(PropertyName = "GUI screen position")]
            public UI4 ScreenPosition { get; set; }
        ...
        private ConfigData GetBaseConfig()
        {
            return new ConfigData
            {
                ...
                ScreenPosition = new UI4(0.646f, 0.01f, 0.831f, 0.14f),
                UseRaidBlocked = true,
        ...
        private void UpdateConfigValues()
        {
            ...
            {
                configData.ScreenPosition = baseConfig.ScreenPosition;
                configData.UseRaidBlocked = baseConfig.UseRaidBlocked;
            }
