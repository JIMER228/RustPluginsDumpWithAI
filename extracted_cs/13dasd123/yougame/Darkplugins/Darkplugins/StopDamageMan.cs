...
{
    /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ [Info("StopDamageMan", "https://discord.gg/dNGbxafuJn", "1.1.0")]
    class StopDamageMan : RustPlugin
    {
        ...
        void AddPlayerNoDamageList(BasePlayer player, string command, string[] args)
        {
            ...
            {
                SendReply(player, lang.GetMessage("NOT_FOUND", this));
                return;
            }
            ...
            if (!PlayerStopDamage.Contains(targetPlayer.userID))
            {
                ...
                SendReply(player, String.Format(lang.GetMessage("PLAYER_ACCEPT", this), targetPlayer.displayName));
            }
            else { SendReply(player, String.Format(lang.GetMessage("PLAYER_NDM", this), targetPlayer.displayName)); }
        ...
        void RemovePlayerNoDamageList(BasePlayer player, string command, string[] args)
        {
            ...
            if (PlayerStopDamage.Contains(targetPlayer.userID))
            {
                ...
                SendReply(player, String.Format(lang.GetMessage("PLAYER_DICLINE", this), targetPlayer.displayName));
            }
            else { SendReply(player, String.Format(lang.GetMessage("PLAYER_YDM", this), targetPlayer.displayName)); }
