...
{
    /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ [Info("UniversalItems", "https://discord.gg/dNGbxafuJn", "1.0.5")]
    ...
    public class UniversalItems : RustPlugin
    {
        ...
        void OnServerInitialized()
        {
            ...
            foreach (var cfg in config.ItemsSetings.Values)
            {
                ...
                {
                    ImageLibrary.Call("AddImage", $"http://api.hougan.space/rust/item/getImage/{cfg.ShortName}/128", cfg.ShortName + 128, cfg.SkinIDI);
                }
                if (cfg.SkinIDI != 0)
                {
                    ImageLibrary.Call("AddImage", $"http://api.hougan.space/rust/skin/getImage/{cfg.SkinIDI}.png", cfg.ShortName, cfg.SkinIDI);
                }
            }
        ...
        void Unload()
        {
            BasePlayer.activePlayerList.ForEach(p => CuiHelper.DestroyUi(p, MAINMENU));
        ...
        object CanRecycle(Recycler recycler, Item item)
        {
            ...
            if (item.info.shortname == cfg.ShortName && item.skin != cfg.SkinIDI) return false;
            ...
            itemInfo.Blueprint.ingredients = new List<ItemAmount> { new ItemAmount { itemDef = ItemManager.FindItemDefinition(cfg.ItemsDrop[RandomItem].Shortname), amount = UnityEngine.Random.Range(cfg.ItemsDrop[RandomItem].MinimalAmount, cfg.ItemsDrop[RandomItem].MaximumAmount) } };
        ...
        {
            var customItem = config.ItemsSetings.FirstOrDefault(p => p.Value.ShortName == item.info.shortname && item.skin == p.Value.SkinIDI);
        ...
        public class FurnaceBurn
        {
            ...
            public void wxecrvtbynu(BaseOven oven)
            {
                this.oven = oven;
        ...
        private void GUI_MAIN(BasePlayer player, Type type)
        {
            ...
            if (type == Type.None)
            {
                ...
                Gui.Add(new CuiPanel
                {
                    ...
                    Image = { Color = "0 0 0 0.8", Material = "assets/content/ui/uibackgroundblur.mat", Sprite = "assets/content/ui/ui.background.transparent.radial.psd" }
                }, "Overlay", MAINMENU);
                ...
                Gui.Add(new CuiLabel
                {
                    ...
                    Text = { Text = lang.GetMessage("INFO_GUI", this, player.UserIDString), FontSize = 23, Font = "robotocondensed-bold.ttf", Align = TextAnchor.MiddleCenter }
                }, "main");

                Gui.Add(new CuiLabel
                {
                    RectTransform = { AnchorMin = "0 0.8589257", AnchorMax = "1 1", OffsetMax = "0 0" },
                    Text = { Text = lang.GetMessage("TITLE_GUI", this, player.UserIDString), FontSize = 23, Font = "robotocondensed-bold.ttf", Align = TextAnchor.MiddleCenter }
                }, "main");
        ...
        private static string HexToRustFormat(string hex)
        {
            ...
            if (str.Length != 8)
            {
                ...
                throw new InvalidOperationException("Cannot convert a wrong format #{DarkPluginsID}.");
            }
