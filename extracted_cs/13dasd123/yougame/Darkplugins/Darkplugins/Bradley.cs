...
{
    /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ [Info("Bradley", "https://discord.gg/dNGbxafuJn", "1.3.4")]
    ...
    public class Bradley : RustPlugin
    {
        ...
        void Unload()
        {
            ...
            var Controller = UnityEngine.Object.FindObjectsOfType<PathMaker>();
            foreach (var insp in Controller)
            {
                insp.OnDestroy();
            }
        }
        ...
        public void spawnroad()
        {
            ...
            BradleyAPC bradly = GameManager.server.CreateEntity("assets/prefabs/npc/m2bradley/bradleyapc.prefab", position) as BradleyAPC;
        ...
        public void getpath(string name)
        {
            foreach (var monument in UnityEngine.Object.FindObjectsOfType<MonumentInfo>())
            {
                ...
                if (canSpawn && name.ToLower() == monumentName.ToString().ToLower() && name.ToLower() == "airfield")
                {
                    Vector3 position = monument.transform.TransformPoint(new Vector3(-86.86411f, 0.3000298f, 38.17391f));
                    bradly = GameManager.server.CreateEntity("assets/prefabs/npc/m2bradley/bradleyapc.prefab", position) as BradleyAPC;
                    bradly.enableSaving = false;
                    bradly.Spawn();
                ...
                else if (canSpawn && name.ToLower() == monumentName.ToString().ToLower() && name.ToLower() == "watertreatment")
                {
                    Vector3 position = monument.transform.TransformPoint(new Vector3(96.74454f, 0.1126404f, 31.32247f));
                    bradly = GameManager.server.CreateEntity("assets/prefabs/npc/m2bradley/bradleyapc.prefab", position) as BradleyAPC;
                    bradly.enableSaving = false;
                    bradly.Spawn();
                    bradly.ClearPath();
                ...
                else if (canSpawn && name.ToLower() == monumentName.ToString().ToLower() && name.ToLower() == "trainyard")
                {
                    Vector3 position = monument.transform.TransformPoint(new Vector3(66.8533f, 0.1389904f, -2.566289f));
                    bradly = GameManager.server.CreateEntity("assets/prefabs/npc/m2bradley/bradleyapc.prefab", position) as BradleyAPC;
                    bradly.enableSaving = false;
                    bradly.Spawn();
                    bradly.ClearPath();
                ...
                else if (canSpawn && name.ToLower() == monumentName.ToString().ToLower() && name.ToLower() == "powerplant")
                {
                    Vector3 position = monument.transform.TransformPoint(new Vector3(18.3474f, 0.1845f, 67.7239f));
                    bradly = GameManager.server.CreateEntity("assets/prefabs/npc/m2bradley/bradleyapc.prefab", position) as BradleyAPC;
                    bradly.enableSaving = false;
                    bradly.Spawn();
                    BradleySaveData.Add(bradly, monumentName);
                ...
                else if (canSpawn && name.ToLower() == monumentName.ToString().ToLower() && name.ToLower() == "harbor_b")
                {
                    Vector3 position = monument.transform.TransformPoint(new Vector3(-30.7f, 4.9f, -77.9f));
                    bradly = GameManager.server.CreateEntity("assets/prefabs/npc/m2bradley/bradleyapc.prefab", position) as BradleyAPC;
                    bradly.enableSaving = false;
                    bradly.Spawn();
                    BradleySaveData.Add(bradly, monumentName);
                ...
                else if (canSpawn && name.ToLower() == monumentName.ToString().ToLower() && name.ToLower() == "harbor_a")
                {
                    Vector3 position = monument.transform.TransformPoint(new Vector3(100.5f, 5.1f, -21.75f));
                    bradly = GameManager.server.CreateEntity("assets/prefabs/npc/m2bradley/bradleyapc.prefab", position) as BradleyAPC;
                    bradly.enableSaving = false;
                    bradly.Spawn();
                    BradleySaveData.Add(bradly, monumentName);
        ...
        private void thePath(BasePlayer player, string cmd, string[] args)
        {
            ...
            switch (args[0].ToLower())
            {
                case "activate":
                    ...
                    if (Checking == null)
                    {
                        player.gameObject.AddComponent<PathMaker>();
        ...
        public class BradlyAI : MonoBehaviour
        {
            ...
            private Vector3 stuckPath { get; set; }
            private bool IsStuck()
            {
                nextStuckTime = Time.time + 1.5f;
                if (stopped) return false;
                float distance = Vector3.Distance(stuckPath, bradly.transform.position);
                if (distance > 0.01f)
                {
                    stuckPath = bradly.transform.position;
                    isStuck = 0;
                }
                ...
                {
                    isStuck = 0;
                    bradly.transform.position = Path[bradly.currentPathIndex];                  
                    return true;                  
                }
                return false;
            }
            ...
            private void MakePath(bool start = false)
            {
                Path.Clear();
                if (RoadRing)
                {
                ...
                else if (!Custom)
                {
                    ...
                    {
                        Path.Add(monument.transform.TransformPoint(new Vector3(-30.7f, 4.9f, -77.9f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-31.25f, 4.9f, -84.0f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-31.8f, 4.9f, -84.0f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-83.1f, 4.9f, -84.0f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-82.6f, 4.9f, -26.1f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-78.6f, 4.9f, -24.1f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-72.7f, 4.9f, -23.8f)));
                        // new turn
                        Path.Add(monument.transform.TransformPoint(new Vector3(-14.29669f, 4.888798f, -23.8f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-13.29669f, 5.000001f, -2.145452f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-5.221963f, 5.000001f, 2.050046f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(5.981122f, 5.000001f, 2.12549f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(11.6275f, 5.000001f, -7.164688f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(26.82205f, 5.000001f, -7.164688f))); //stop
                        Path.Add(monument.transform.TransformPoint(new Vector3(11.6275f, 5.000001f, -7.164688f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(5.981122f, 5.000001f, 2.12549f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-5.221963f, 5.000001f, 2.050046f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-13.29669f, 5.000001f, -2.145452f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-13.29669f, 4.888798f, -23.8f)));
                        // end road and turn
                        Path.Add(monument.transform.TransformPoint(new Vector3(39.9f, 4.9f, -23.8f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(39.9f, 4.9f, 16.1f)));
                        // stop and turn up
                        // new turn
                        Path.Add(monument.transform.TransformPoint(new Vector3(-28.809f, 4.888929f, 16.1f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-28.809f, 5.000001f, 45.82645f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-26.847f, 5.000007f, 48.14343f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-26.847f, 5.000028f, 60.56243f))); //stop
                        Path.Add(monument.transform.TransformPoint(new Vector3(-26.847f, 5.000007f, 48.14343f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-28.809f, 5.000001f, 45.82645f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-28.809f, 4.888929f, 16.1f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-65.0f, 4.9f, 16.1f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-65.0f, 4.9f, 94.2f)));
                        // end new turn
                        // start new turns                        
                        Path.Add(monument.transform.TransformPoint(new Vector3(-25.40976f, 5f, 94.2f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-25.40976f, 5f, 74.37666f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-20.49991f, 5f, 69.07885f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(28.74104f, 5f, 69.07885f))); //stop
                        Path.Add(monument.transform.TransformPoint(new Vector3(-20.49991f, 5f, 69.07885f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-25.40976f, 5f, 74.37666f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-25.40976f, 5f, 103.0489f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-7.722996f, 5f, 103.0489f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-7.835934f, 5f, 93.81862f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-4.288837f, 5f, 90.97488f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(3.753239f, 5f, 90.97488f))); //stop
                        Path.Add(monument.transform.TransformPoint(new Vector3(-4.288837f, 5f, 90.97488f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-7.835934f, 5f, 93.81862f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-7.722996f, 5f, 103.0489f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-36.49657f, 5f, 103.0489f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-40.19431f, 5f, 97.15047f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-40.61494f, 5f, 66.39552f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-40.68375f, 5f, 94.2f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-65.0f, 4f, 94.2f)));
                        //end new
                        //end run return to begin 
                        Path.Add(monument.transform.TransformPoint(new Vector3(-65.0f, 4.9f, 65.0f))); // stop
                        Path.Add(monument.transform.TransformPoint(new Vector3(-65.0f, 4.9f, 17.7f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-65.0f, 4.9f, -19.9f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-83.3f, 4.9f, -30.7f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-83.1f, 4.9f, -84.0f)));
                        Path.Add(monument.transform.TransformPoint(new Vector3(-30.7f, 4.9f, -83.1f)));
                        if (stopPause.Count <= 0)
                        {
                            stopPause.Add(12, 5);
                            stopPause.Add(23, 5);
            ...
            private void shouldFireRocket()
            {
                ...
                foreach (var target in bradly.targetList.ToList())
                {
                    ...
                    if (target.entity is BasePlayer)
                    {
                        ...
                        if (targetPlayer == null || targetPlayer.IsDead() || targetPlayer.IsSleeping()) continue;
            ...
            private void OnCollisionEnter(Collision collision)
            {
                ...
                ReclaimBackpack backpack = gObject.GetComponentInParent<ReclaimBackpack>();
                ...
                {
                    _instance.Puts(containers.name);
