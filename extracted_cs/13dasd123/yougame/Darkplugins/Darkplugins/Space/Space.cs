...
{
    [Info("Space", "Sempai#3239", "1.0.8")]
    class Space : RustPlugin
    {
        #region Variables
        const bool en = false;
        static Space ins;
        [PluginReference] Plugin NpcSpawn, PveMode, GUIAnnouncements, DiscordMessages, Economics, ServerRewards, IQEconomic, NightVision;
        ...
        void OnPlayerDeath(BasePlayer player)
        {
            ...
            GuiManager.DestroyAllGuiForPlayer(player);
        }
        ...
        void OnEntityDismounted(BaseMountable entity, BasePlayer player)
        {
            ...
            {
                SpaceShuttle spaceShuttle = SpaceShuttle.GetSpaceShuttleByMainSubmarineNetId(submarineDuo.net.ID);
            ...
            NextTick(() => Astronaut.TryAddComponentToPlayer(player));
        }

        object OnPlayerWantsMount(BasePlayer player, SubmarineDuo submarineDuo)
        {
            if (submarineDuo == null) return null;
            SpaceShuttle controlledSpaceShuttle = SpaceShuttle.GetSpaceShuttleByMainSubmarineNetId(submarineDuo.net.ID);
        ...
        {
            if (baseCombatEntity == null) return null;
            if (baseCombatEntity.ShortPrefabName == "submarinesolo.entity")
            {
                SpaceShuttle controlledSpaceShuttle = SpaceShuttle.GetSpaceShuttleByDecorSubmarineNetId(baseCombatEntity.net.ID);
        ...
        void OnHotAirBalloonToggled(HotAirBalloon balloon, BasePlayer player)
        {
            ...
            Aerostat aerostat = Aerostat.GetAetostatByBallonNetId(balloon.net.ID);
        ...
        void OnCardSwipe(CardReader cardReader, Keycard card, BasePlayer player)
        {
            ...
            if (SpaceClass.IsInSpace(cardReader.transform.position)) SpaceStation.OnStationCardReaderSwipe(cardReader, card, player);
        }

        void OnButtonPress(PressButton button, BasePlayer player)
        {
            if (button == null) return;
            if (SpaceClass.IsInSpace(button.transform.position)) SpaceStation.OnStationButtonPress(button);
        }
        ...
        {
            if (_config.aerostatConfig.allBallons && !SpaceClass.IsInSpace(entity.transform.position) && entity.skinID == 0) Aerostat.AttachAerostatClass(entity);
        }

        void OnEntityTakeDamage(SubmarineDuo submarine, HitInfo info)
        {
            if (!submarine.IsExists() || submarine.net == null) return;
            SpaceShuttle controlledSpaceShuttle = SpaceShuttle.GetSpaceShuttleByMainSubmarineNetId(submarine.net.ID);
        ...
        {
            if (!_config.supportedPluginsConfig.economy.enable || !EventManager.isEventActive || entity == null || info == null) return;
        ...
        void OnEntityDeath(Door entity, HitInfo info)
        {
            ...
            {
                if (_config.supportedPluginsConfig.economy.enable) EconomyManager.ActionEconomy(info.InitiatorPlayer.userID, "Door");
            }
        }
        ...
        object CanEntityBeTargeted(BasePlayer target, AutoTurret autoTurret)
        {
            ...
            if (SpaceClass.IsInSpace(autoTurret.transform.position)) return true;
            return null;
        }
        ...
        {
            if (!EventManager.isEventActive || bradley == null) return null;
            if (SpaceClass.IsInSpace(bradley.transform.position)) return true;
            return null;
        }
        ...
        void GivePurpleCardCommand(ConsoleSystem.Arg arg)
        {
            ...
            Item spaceCardItem = LootManager.CreateItem(_config.spaceCardConfig, 1);
        ...
        void GiveSuitCommand(ConsoleSystem.Arg arg)
        {
            ...
            {
                PrintToConsole(player, "Space Suit not found");
                return;
            }
            int amount = 1;
        ...
        void UpdateConfig()
        {
            foreach (CrateConfig crateConfig in ins._config.crates)
            {
        ...
        static class EventManager
        {
            ...
            internal static void StopEvent(bool unload = false)
            {
                ...
                if (isEventActive)
                {
                    ...
                    if (ins._config.supportedPluginsConfig.economy.enable) EconomyManager.SendBalance();
                    eventCorountine = null;
                    eventConfig = null;
                    eventTime = 0;
                    if (ins._config.supportedPluginsConfig.pveMode.pve && ins.plugins.Exists("PveMode")) ins.PveMode.Call("EventRemovePveMode", ins.Name, false);
                ...
                if (!unload) TryStartEvent(immediately: false);
            ...
            internal static void TryStartEvent(bool immediately, string presetName = "", BasePlayer initiator = null)
            {
                ...
                if (eventCorountine != null) ServerMgr.Instance.StopCoroutine(eventCorountine);
                if (immediately) StartPeriodicEvent(true);
                else DelayedStartPeriodicEvent();
                if (immediately && initiator == null) ins.Puts("The event is activated!");
            }
            ...
            static void StartPeriodicEvent(bool over = false)
            {
                ...
                if (eventCorountine != null) ServerMgr.Instance.StopCoroutine(eventCorountine);
                eventCorountine = ServerMgr.Instance.StartCoroutine(StartPeriodicEventCoroutine());
            }
            ...
            {
                NotifyManager.SendMessageToAll("StartEvent", ins._config.prefix, eventConfig.displayName, PhoneController.PositionToGridCoord(SpaceClass.GetSpaceTransform().position));
            ...
            static IEnumerator PeriodicEventCorountine()
            {
                ...
                {
                    if (ins._config.mainConfig.remainTimeNotifications.Contains(eventTime)) NotifyManager.SendMessageToAll("RemainTime", ins._config.prefix, eventConfig.displayName, eventTime);
                    if (ins._config.mainConfig.descriptionNotifications.Contains(eventTime)) NotifyManager.SendMessageToAll("EventDescription", ins._config.prefix, eventConfig.displayName, PhoneController.PositionToGridCoord(SpaceClass.GetSpaceTransform().position));
                    GuiManager.UpdateCountdownGui(eventTime);
                    MapMarker.UpdateMapMarkerTime(eventTime);
                    eventTime--;
        ...
        sealed class SpaceClass : FacepunchBehaviour
        {
            ...
            HashSet<SpaceShuttle> spaceShuttles = new HashSet<SpaceShuttle>();
            HashSet<Aerostat> aerostats = new HashSet<Aerostat>();
            ...
            internal static void CreateSpaceFirstStage()
            {
                ...
                if (ins._config.mainConfig.onlyOceanSpawn)
                {
                    ...
                    gameObject.transform.position = new Vector3(randomx, ins._config.mainConfig.spaceHeight, randomz);
                }
                else gameObject.transform.position = new Vector3(UnityEngine.Random.Range(-TerrainMeta.Size.x / 2, TerrainMeta.Size.x / 2), ins._config.mainConfig.spaceHeight, UnityEngine.Random.Range(-TerrainMeta.Size.x / 2, TerrainMeta.Size.x / 2));
                ...
                if (EventManager.eventConfig.spaceStationConfig.enable) SpaceStation.SpawnSpaceStation(EventManager.eventConfig.spaceStationConfig.presetName);
                else CreateSpaceSecondStage();
            }
            ...
            {
                if (spaceClass == null) return;
                if (spaceClass.spaceZone == null) spaceClass.spaceZone = EventZone.AddZoneComponrntToGameObject(spaceClass);
                MapMarker.CreateMapMarker(spaceClass.transform.position);
            ...
            {
                if (spaceClass != null) UnityEngine.Object.Destroy(spaceClass.gameObject);
            }
            ...
            internal static string GetContainerPrefabNameBynetId(uint netId)
            {
                if (spaceClass == null) return null;
                BaseEntity crate = spaceClass.crates.FirstOrDefault(x => x != null && x.net.ID == netId);
                ...
                class RoamSpaceNpc : FacepunchBehaviour
                {
                    ...
                    class AutoPilotZeroGravity : ZeroGravity
                    {
                        ...
                        void UpdatePosition()
                        {
                            ...
                            if (spaceNpcComponent.currentTarget != null && spaceNpcComponent.scientistNPC.IsPlayerVisibleToUs(spaceNpcComponent.currentTarget, 1218519041))
                            {
        ...
        sealed class EventZone : FacepunchBehaviour
        {
            ...
            void Init(SpaceClass spaceClass)
            {
                ...
                if (ins._config.supportedPluginsConfig.pveMode.pve && ins.plugins.Exists("PveMode")) CreatePveModeZone();
            ...
            void CreatePveModeZone()
            {
                ...
                foreach (ScientistNPC scientistNPC in SpaceStation.GetSpaceStationNPCs()) if (scientistNPC.IsExists()) npcs.Add(scientistNPC.net.ID);
                foreach (ScientistNPC scientistNPC in SpaceClass.GetSpaceNPCs()) if (scientistNPC.IsExists()) npcs.Add(scientistNPC.net.ID);

                foreach (BaseEntity lootContainer in SpaceStation.GetSpaceStationCrates()) if (lootContainer.IsExists()) crates.Add(lootContainer.net.ID);
                foreach (BaseEntity lootContainer in SpaceClass.GetSpaceCrates()) if (lootContainer.IsExists()) crates.Add(lootContainer.net.ID);

                foreach (BradleyAPC bradleyAPC in SpaceStation.GetSpaceStationBradleys()) if (bradleyAPC.IsExists()) bradleys.Add(bradleyAPC.net.ID);

                ins.PveMode.Call("EventAddPveMode", ins.Name, config, SpaceClass.GetSpaceTransform().position, ins._config.mainConfig.spaceRadius, crates, npcs, bradleys, new HashSet<ulong>(), null);
            }
            ...
            void OnTriggerExit(Collider other)
            {
                ...
                if (player.IsRealPlayer())
                {
                    ...
                    GuiManager.DestroyCountdownGui(player);
        ...
        {
            internal static MapMarker mapMarker;
            MapMarkerGenericRadius mapmarker;
            VendingMachineMapMarker vendingMarker;

            internal static void CreateMapMarker(Vector3 position)
            {
                if (mapMarker != null) DeleteMapMarker();
            ...
            {
                SpawnMapMarker(position);
                SpawnVendingMarker(position);
            }

            void SpawnMapMarker(Vector3 position)
            {
                mapmarker = GameManager.server.CreateEntity("assets/prefabs/tools/map/genericradiusmarker.prefab", new Vector3(position.x, 10, position.z)) as MapMarkerGenericRadius;
            ...
            void SpawnVendingMarker(Vector3 position)
            {
                ...
                vendingMarker.markerShopName = $"{EventManager.eventConfig.displayName}";
                vendingMarker.Spawn();
            }

            internal static void UpdateMapMarkerTime(int eventTime)
            {
                if (eventTime < 0) eventTime = 0;
                if (mapMarker == null) return;
                if (mapMarker.vendingMarker != null)
                {
                    mapMarker.vendingMarker.markerShopName = $"{EventManager.eventConfig.displayName} {NotifyManager.GetTimeMessage(null, eventTime)}";
                    mapMarker.vendingMarker.SendNetworkUpdate();
                }
                if (mapMarker.mapmarker != null)
                {
                    mapMarker.mapmarker.SendUpdate();
                    mapMarker.mapmarker.SendNetworkUpdate();
                }
            ...
            {
                if (mapMarker == null) return;
                if (mapMarker.mapmarker.IsExists()) mapMarker.mapmarker.Kill();
                if (mapMarker.vendingMarker.IsExists()) mapMarker.vendingMarker.Kill();
                mapMarker = null;
            }
        }
        ...
        sealed class SpaceStation
        {
            ...
            internal static void OnStationCardReaderSwipe(CardReader cardReader, Keycard card, BasePlayer player)
            {
                if (spaceStation == null) return;
                CardDoor cardDoor = spaceStation.cardDoors.FirstOrDefault(x => x.IsOwnCardReader(cardReader.net.ID));
                if (cardDoor != null) cardDoor.OnCardSwipe(card, player);
            }

            internal static void OnStationButtonPress(PressButton pressButton)
            {
                if (spaceStation == null) return;
                CardDoor cardDoor = spaceStation.cardDoors.FirstOrDefault(x => x.IsOwnButton(pressButton.net.ID));
                if (cardDoor != null) cardDoor.OnButtonPress();
            }

            internal static void OnDoorOpened(Door door)
            {
                if (spaceStation == null) return;
                RegularDoor regularDoor = spaceStation.cardDoors.FirstOrDefault(x => x != null && !x.isDoorOpen && x.IsOwnDoor(door.net.ID));
                if (regularDoor == null)
                {
                    regularDoor = spaceStation.regularDoors.FirstOrDefault(x => x != null && !x.isDoorOpen && x.IsOwnDoor(door.net.ID));
                }
            ...
            {
                if (spaceStation == null) return null;
                BaseEntity crate = spaceStation.crates.FirstOrDefault(x => x != null && x.net.ID == netId);
                if (crate != null) return crate.PrefabName;
                return null;
            }
            ...
            void SpawnDoorNpcs(RegularDoor regularDoor)
            {
                if (NpcManager.CheckNPCSpawn())
                {
                    ...
                    if (ins._config.supportedPluginsConfig.pveMode.pve && ins.plugins.Exists("PveMode")) ins.PveMode.Call("EventAddScientists", ins.Name, stationNpcs.Where(x => x.IsExists()).Select(x => x.net.ID));
                }
            }
            ...
            SpaceStation(string stationPrefabName)
            {
                ...
                mainDoor = MainDoor.CreateMainDoor();
            }
            ...
            void DespawnStationEntities()
            {
                ...
                foreach (BaseEntity entity in crates) if (entity.IsExists()) entity.Kill();
                foreach (AutoTurret entity in turrets) if (entity.IsExists()) entity.Kill();
            ...
            sealed class MainDoor : FacepunchBehaviour
            {
                ...
                internal static MainDoor CreateMainDoor()
                {
                    GameObject gameObject = new GameObject();
                    gameObject.layer = (int)Layer.Reserved1;
                ...
                {
                    if (doorOpenCorountine != null) ServerMgr.Instance.StopCoroutine(doorOpenCorountine);
                ...
                void OnTriggerEnter(Collider other)
                {
                    ...
                    else if (entity is BaseSubmarine) OpenMainDoor();
                }
            ...
            sealed class CardDoor : RegularDoor
            {
                ...
                internal bool IsOwnCardReader(uint cardReaderNetId)
                {
                    return cardReader != null && cardReaderNetId == cardReader.net.ID;
                }

                internal bool IsOwnButton(uint buttonNetId)
                {
                    return pressButton != null && pressButton.net.ID == buttonNetId;
                }
                ...
                internal void OnCardSwipe(Keycard card, BasePlayer player)
                {
                    ...
                    {
                        cardName = "GreenCard";
                        if (card.accessLevel != 1) return;
                    }
                    else if (cardReader.accessLevel == 2)
                    {
                        cardName = "BlueCard";
                        if (card.accessLevel != 2) return;
                    }
                    else if (cardReader.accessLevel == 3)
                    {
                        cardName = "RedCard";
                        if (card.accessLevel != 3) return;
                    }
                    ...
                    if (ins.plugins.Exists("PveMode") && ins.PveMode.Call("CanActionEvent", ins.Name, player) != null) return;
                    if (ins._config.supportedPluginsConfig.economy.enable) EconomyManager.ActionEconomy(player.userID, cardName);
                    card.GetItem().LoseCondition(1f);
                    OpenDoor();
                ...
                void OpenDoor()
                {
                    if (ins._config.spaceStaionConfig.autoCloseCardDoors)
                    {
                        openTime = ins._config.spaceStaionConfig.doorCloseTime;
                        if (openCorountine != null) return;
                        foreach (Door door in doors)
                        {
                            if (door != null) door.SetOpen(true);
                        }
                        openCorountine = ServerMgr.Instance.StartCoroutine(OpenDoorCorountine());
                    }
                    else
                    {
                        foreach (Door door in doors)
                ...
                IEnumerator OpenDoorCorountine()
                {
                    while (openTime > 0)
                    {
                        openTime -= 1;
                ...
                {
                    if (openCorountine != null) ServerMgr.Instance.StopCoroutine(openCorountine);
                    DestroyDoor();
            ...
            class RegularDoor
            {
                ...
                internal bool IsOwnDoor(uint doorNetId)
                {
                    return doors.Any(x => x != null && x.net.ID == doorNetId);
                }
        ...
        sealed class Astronaut : FacepunchBehaviour
        {
            ...
            {
                if (!ins._config.mainConfig.enableZeroGravity || player == null || !EventManager.isEventActive || !SpaceClass.IsInSpace(player.transform.position, checkMaxHeight: false) || (player.IsAdmin && player.IsFlying)) return;
            ...
            static void RemoveComponent(Astronaut astronaut)
            {
                if (astronaut != null)
                {
            ...
            IEnumerator CheckPLayerHeightCorountine()
            {
                ...
                while (true)
                {
                    ...
                    {
                        if (time == -1) time = 10;
                        GuiManager.MessageGUI(player, GetMessage("GravityTakeEffectGUI", player.UserIDString, NotifyManager.GetTimeMessage(player.UserIDString, time)));
                        time--;
                        if (time == -1) RemoveComponent(this);
                    }
                    else if (time != -1)
                    {
                        GuiManager.DestroyMessageGUIForPlayer(player);
                        time = -1;
                    }
                    yield return CoroutineEx.waitForSeconds(1f);
                }
            ...
            void CheckNearShuttle()
            {
                ...
                {
                    SpaceShuttle controlledSpaceShuttle = SpaceShuttle.GetSpaceShuttleByMainSubmarineNetId(entity.net.ID);
            ...
            class PlayerZeroGravity : ZeroGravity
            {
                ...
                void FixedUpdate()
                {
                    if (movableBaseMountable._mounted == null) return;
        ...
        sealed class Aerostat : FacepunchBehaviour
        {
            ...
            internal static Aerostat GetAetostatByBallonNetId(uint netId)
            {
                return aerostats.FirstOrDefault(x => x != null && x.hotAirBalloon != null && x.hotAirBalloon.net.ID == netId);
            }
            ...
            internal static void CheckAllBallons()
            {
                foreach (HotAirBalloon hotAirBalloon in BaseNetworkable.serverEntities.OfType<HotAirBalloon>())
                {
                    ...
                    if (ins._config.aerostatConfig.allBallons || hotAirBalloon.skinID == aerostatSkin) AttachAerostatClass(hotAirBalloon);
                }
            }
            ...
            void Init(HotAirBalloon hotAirBalloon)
            {
                ...
                if (EventManager.isEventActive) FlyingToSpaceControl();
            }
        ...
        sealed class SpaceShuttle : FacepunchBehaviour
        {
            ...
            static void RespawnRoadShuttles()
            {
                ...
                {
                    if (junkPile == null) continue;
                    if (junkPile.PlayersNearby()) continue;
            ...
            internal static void OnEventStop()
            {
                ...
                {
                    if (spaceShuttle != null && !spaceShuttle.used) spaceShuttle.KillSubmarine();
                }
            }

            internal static SpaceShuttle SpawnSpaceShuttle(Vector3 position, Quaternion rotation)
            {
                SubmarineDuo submarineDuo = BuildManager.CreateRegularEntity("assets/content/vehicles/submarine/submarineduo.entity.prefab", position + new Vector3(0, 0.75f, 0), rotation, enableSaving: true) as SubmarineDuo;
            ...
            internal static void CheckAllDuoSubmarine()
            {
                foreach (SubmarineDuo submarineDuo in BaseNetworkable.serverEntities.OfType<SubmarineDuo>())
                {
                    ...
                    if (submarineDuo.skinID == shuttleSkin) AttachClassToDuoSubmarine(submarineDuo);
                }
            }
            ...
            internal static void AttachClassToDuoSubmarine(SubmarineDuo submarineDuo)
            {
                ...
                submarineDuo.skinID = shuttleSkin;
            ...
            {
                foreach (SpaceShuttle controlledSpaceShuttle in spaceShuttles) controlledSpaceShuttle.DestroyShuttle();
            }

            internal static SpaceShuttle GetSpaceShuttleByMainSubmarineNetId(uint netId)
            {
                return spaceShuttles.FirstOrDefault(x => x != null && x.submarineDuo != null && x.submarineDuo.net != null && x.submarineDuo.net.ID == netId);
            }

            internal static SpaceShuttle GetSpaceShuttleByDecorSubmarineNetId(uint netId)
            {
                return spaceShuttles.FirstOrDefault(x => x != null && x.decorSubmarines.Any(y => y.IsExists() && y.net != null && y.net.ID == netId));
            }
            ...
            {
                if (fuelAmount <= 0) return;
                StorageContainer fuelContainer = submarineDuo.GetFuelSystem().GetFuelContainer();
                fuelContainer.inventory.AddItem(fuelContainer.allowedItem, fuelAmount);
            }
            ...
            internal void DisactivateShuttle()
            {
                ...
                if (SpaceStation.IsPositionInSpaceStation(submarineDuo.transform.position)) EnterInAtmosphere();
            }
            ...
            void FireTorpedo(BasePlayer driver)
            {
                ...
                if (submarineDuo.timeSinceTorpedoFired >= submarineDuo.maxFireRate && BaseMountable.TryFireProjectile(submarineDuo.GetTorpedoContainer(), AmmoTypes.TORPEDO, submarineDuo.torpedoFiringPoint.position + submarineDuo.torpedoFiringPoint.up + submarineDuo.torpedoFiringPoint.forward * 2, submarineDuo.torpedoFiringPoint.forward * 3, driver, 1f, 0, out serverProjectile))
                {
            ...
            {
                if (submarineDuo.IsExists()) submarineDuo.Kill();
            }
        ...
        static class LootManager
        {
            ...
            internal static void UpdateLootContainerLootTable(ItemContainer itemContainer, OwnLootTableConfig lootTableConfig, int typeOfLootTable)
            {
                ...
                {
                    foreach (LootItemConfig item in lootTableConfig.items)
                    {
                        if (UnityEngine.Random.Range(0.0f, 100.0f) <= item.chance)
                        {
                            if (itemContainer.itemList.Any(x => x.info.shortname == item.shortname)) continue;
                            int amount = UnityEngine.Random.Range(item.minAmount, item.maxAmount);
                            Item newItem = item.isBlueprint ? ItemManager.CreateByName("blueprintbase") : ItemManager.CreateByName(item.shortname, amount, item.skin);
                            if (item.isBlueprint) newItem.blueprintTarget = ItemManager.FindItemDefinition(item.shortname).itemid;
                            if (item.name != "") newItem.name = item.name;
                            if (!newItem.MoveToContainer(itemContainer)) newItem.Remove();
                            else
                            {
        ...
        static class GuiManager
        {
            ...
            {
                new ImageName { name = "Gui_1", path = "Space/Images/Gui_1.png" }
            };
            ...
            {
                if (time < 0) time = 0;
                if (!ins._config.notifyConfig.guiConfig.isCountdownGUI) return;
                for (int i = 0; i < EventZone.playersInSpace.Count; i++)
                {
                    BasePlayer player = EventZone.playersInSpace[i];
                    if (player == null) EventZone.playersInSpace.RemoveAt(i);
                    if (!SpaceClass.IsInSpace(player.transform.position))
                    {
                        CuiHelper.DestroyUi(player, "CountdownGUI");
                        EventZone.playersInSpace.RemoveAt(i);
                        continue;
                    }
            ...
            static void CountdownGUI(BasePlayer player, int time)
            {
                ...
                {
                    Image = { Color = "0 0 0 0" },
                    RectTransform = { AnchorMin = ins._config.notifyConfig.guiConfig.anchorMin, AnchorMax = ins._config.notifyConfig.guiConfig.anchorMax },
                    CursorEnabled = false,
        ...
        protected override void LoadDefaultMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                ["GravityTakeEffectGUI"] = "Гравитация начнёт действовать через <color=#8f43d8>{0}</color>",
            ...
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                ["GravityTakeEffectGUI"] = "Gravity will take effect in <color=#ce3f27>{0}</color>",
        ...
        public class SpaceStationBaseConfig
        {
            ...
            [JsonProperty(en ? "Automatically close doors that have been opened with a card [true/false]" : "Автоматически закрывать двери, которые были открыты карточкой [true/false]")] public bool autoCloseCardDoors { get; set; }
            [JsonProperty(en ? "Time until the door is automatically closed [sec]" : "Время до автоматического закрытия двери [sec]")] public int doorCloseTime { get; set; }
        ...
        {
            [JsonProperty(en ? "GUIAnnouncements plugin in use? [true/false]" : "Использовать ли GUI Announcements? [true/false]")] public bool isGUIAnnouncements { get; set; }
        ...
        {
            [JsonProperty(en ? "Notify plugin in use? [true/false]" : "Использовать ли Notify? [true/false]")] public bool isNotify { get; set; }
            [JsonProperty(en ? "Type" : "Тип")] public string type { get; set; }
        }

        public class DiscordConfig
        {
            [JsonProperty(en ? "DiscordMessages plugin in use? [true/false]" : "Использовать ли Discord? [true/false]")] public bool isDiscord { get; set; }
