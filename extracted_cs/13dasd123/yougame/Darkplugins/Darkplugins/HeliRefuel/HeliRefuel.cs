...
{
    [Info("HeliRefuel", "Sempai#3239", "2.1.8")]
    ...
    class HeliRefuel : RustPlugin, IChaosNPCPlugin
    {
        ...
        private void OnEntityTakeDamage(BaseNetworkable networkable, HitInfo info)
        {
            ...
            RefuelHelicopter refuelHelicopter = networkable is BaseHelicopter ? networkable.GetComponent<RefuelHelicopter>() : null;
            if (refuelHelicopter != null)
                refuelHelicopter.OnDamage(info);
        }

        private void OnEntityKill(BaseHelicopter helicopter)
        {
        ...
        private void OnPlayerCorpseSpawned(CustomScientistNPC customNpc, BaseCorpse corpse)
        {
            ...
            if (velocityInheritance.TryGetValue(customNpc, out velocity))
            {
        ...
        {
            BaseHelicopter helicopter = GameManager.server.CreateEntity(HELICOPTER_PREFAB) as BaseHelicopter;
        ...
        private Vector3 RandomPointOnWorldBounds()
        {
            ...
            if (UnityEngine.Random.value > 0.5f)
                position = new Vector3(UnityEngine.Random.value > 0.5f ? size : -size, 0, UnityEngine.Random.Range(-size, size));
            else position = new Vector3(UnityEngine.Random.Range(-size, size), 0, UnityEngine.Random.value > 0.5f ? size : -size);

            position.y = TerrainMeta.HeightMap.GetHeight(position) + 30f;
        ...
        private static void PopulateLoot(LootContainer container)
        {
            ...
            List<ConfigData.LootOptions.LootItem> list = Pool.GetList<ConfigData.LootOptions.LootItem>();
        ...
        private bool IsRefuelHelicopter(BaseHelicopter helicopter) => helicopter.GetComponent<RefuelHelicopter>() != null;
        #endregion
        ...
        public class RefuelHelicopter : MonoBehaviour
        {
            ...
            protected BaseHelicopter Helicopter { get; private set; }
            ...
            private void OnDestroy()
            {
                ...
                if (!isDying)
                {
                    for (int i = 0; i < NPCs?.Count; i++)
                    {
                        ...
                        if (customNpc != null && !customNpc.IsDestroyed)
                            customNpc.Kill();
                    }
                }
            ...
            internal void ResetStateToPatrol()
            {
                ...
                if (configData.NPC.Amount > 0 && NPCs.Count == 0)
                {
                    if (configData.Helicopter.SelfDestruct)
                    {
                        ...
                        if (!InvokeHandler.IsInvoking(this, Helicopter.DieInstantly))
                        {
                            Broadcast("Notification.SelfDestruct", configData.Helicopter.SelfDestructTime);
                            InvokeHandler.Invoke(this, Helicopter.DieInstantly, configData.Helicopter.SelfDestructTime);
                        }
                        return;
                    }
                    else
            ...
            private void ResetState()
            {
                ...
                {
                    CustomScientistNPC customNpc = NPCs[i];
                    if (customNpc != null && !customNpc.isMounted)
                    {
                        MountAny(customNpc);
                    }
                }
            ...
            internal void OnDeath(HitInfo info)
            {
                ...
                {
                    AI.moveSpeed = 0;
                    typeof(PatrolHelicopterAI).GetField("_lastMoveDir", BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance).SetValue(AI, Vector3.zero);
                }
            ...
            private void CreateMountPoints()
            {
                for (int i = 0; i < Mathf.Clamp(configData.NPC.Amount, 0, 6); i++)
                {
                    ...
                    mountable.SetParent(Helicopter);
            ...
            private void MountObject(CustomScientistNPC customNpc, BaseMountable mountable)
            {
                ...
                customNpc.MountObject(mountable, 0);
