...
{
    [Info("NpcRaiders", "Sempai#3239", "1.4.6", ResourceId = 107)]
    ...
    public class NpcRaiders : RustPlugin
    {
        ...
        public class PCDInfo
        {
            ...
            public Dictionary<uint, long> tcCooldown = new Dictionary<uint, long>();
        }
        ...
        private void OnCupboardAuthorize(BuildingPrivlidge privilege, BasePlayer player)
        {
            ...
            if (!pcdData.pEntity[player.userID].tcCooldown.ContainsKey(privilege.net.ID))
                pcdData.pEntity[player.userID].tcCooldown.Add(privilege.net.ID, 0);
            ...
            pcdData.pEntity[player.userID].tcCooldown[privilege.net.ID] = coolTime;
            SaveData();
        }
        ...
        private bool canBuyRaid(BasePlayer player, string type)
        {
            ...
            if (pcdData.pEntity.ContainsKey(player.userID) && !permission.UserHasPermission(player.UserIDString, coolPerm))
            {
                if (pcdData.pEntity.ContainsKey(player.userID))
                {
                    ...
                    if (priv != null)
                    {
                        if (pcdData.pEntity[player.userID].tcCooldown.ContainsKey(priv.net.ID))
                        {
                            var cdTime = pcdData.pEntity[player.userID].tcCooldown[priv.net.ID];
                            if (cdTime > timeStamp)
                            {
