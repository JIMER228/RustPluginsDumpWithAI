...
{
    [Info("BotReSpawn", "Sempai#3239", "1.1.7", ResourceId = 0)]
    [Description("Spawn tailored AI with kits at monuments, custom locations, or randomly.")]

    class BotReSpawn : RustPlugin
    {
        [PluginReference] Plugin NoSash, Kits, CustomLoot, RustRewards, XPerience;
        ...
        object OnEntityTakeDamage(BasePlayer player, HitInfo info)
        {
            ...
            if (bData1 != null)
            {
                ...
                if (info.InitiatorPlayer != null)
                {
                    ...
                    if (brain != null)
                    {
                        if (bData1.inAir && bData1.profile.Other.Invincible_Whilst_Chuting)
                            return true;
        ...
        void SelectWeapon(ScientistNPC npc, BotData bData, BaseAIBrain brain) 
        {
            ...
            Range enemyrange = bData.CurrentTarget == null ? TargetRange(41) : TargetRange(Vector3.Distance(npc.transform.position, bData.CurrentTarget.transform.position));
        ...
        void TidyUp(ScientistNPC npc, BotData bData)
        {
            if (npc != null && !npc.IsDestroyed)
                npc.Kill();
            if (bData != null)
            ...
            else
            {
                if (!t && profile.Spawn.UseCustomSpawns)
                {
                    ...
                    foreach (var entry in SpawnsData)
                    {
                        ...
                        if (!profile.Spawn.Stationary && !HasNav(loc))
                        {
                            skipped++; 
                            continue;
                        }
                ...
                if (Points.Count < maxPopulation)
                {
                    ...
                    while (Points.Count() < maxPopulation)
                    {
                        ...
                        Vector3 loc = TryGetSpawn(gameObject.transform.position, profile.Spawn.Radius, profile.type == ProfileType.Event, profile.Other.OverWater);
                        if (loc != Vector3.zero)
        ...
        {
            bool canChute = profile.Other.Chute && gameObject.transform.position.y > TerrainMeta.HeightMap.GetHeight(gameObject.transform.position);
            int counter = 0;
            ...
            gameObject.GetComponent<CustomGroup>().Setup(profile, name, t, q, g, sps, canChute);
        }
        ...
        public class CustomGroup : SpawnGroup
        {
            ...
            public void Setup(Profile p, string n, bool t, int q, string g, List<GameObject> sps, bool canChute)
            {
            ...
            protected override void PostSpawnProcess(BaseEntity entity, BaseSpawnPoint spawnPoint)
            {
                ...
                bs.timer.Once(1.0f, () => 
                {
                    ...
                    brain.Senses.Init(npc, npc.Brain, bs.configData.Global.Deaggro_Memory_Duration, 400, 400, brain.VisionCone, brain.CheckVisionCone, brain.CheckLOS, brain.IgnoreNonVisionSneakers, brain.ListenRange, bData.profile.Behaviour.Peace_Keeper, brain.MaxGroupSize > 0, brain.IgnoreSafeZonePlayers, brain.SenseTypes, true);
        ...
        public static Vector3 CalculateGroundPos(Vector3 pos, bool e, bool OverWater)
        {
            if (!OverWater && (!e || TerrainMeta.HeightMap.GetHeight(pos) <= pos.y))
                pos.y = TerrainMeta.HeightMap.GetHeight(pos); 

            NavMeshHit navMeshHit;
            if (!NavMesh.SamplePosition(pos, out navMeshHit, 2, 1))
                pos = Vector3.zero;
            else if (!e && WaterLevel.GetWaterDepth(pos, true) > 0)
                pos = Vector3.zero;
            else if (!e && Physics.RaycastAll(navMeshHit.position + new Vector3(0, 100, 0), Vector3.down, 99f, 1235288065).Any())
                pos = Vector3.zero;
        ...
        Vector3 TryGetSpawn(Vector3 pos, int radius, bool e, bool OverWater)
        {
            int attempts = 0;
            ...
            while (attempts < 50 && spawnPoint == Vector3.zero)
            {
                ...
                spawnPoint = CalculateGroundPos(pos + new Vector3(rand.x, 0, rand.y), e, OverWater);
        ...
        void GiveKit(ScientistNPC npc, string kit)
        {
            ...
            {
                if (bData.profile.Spawn.Keep_Default_Loadout == false)
                    npc?.inventory?.Strip();
        ...
        public class BotData : MonoBehaviour
        {
            ...
            public bool canFire = true, throwing, healing, melee, temporary, noweapon, hasHeadLamp, stationary, inAir, abletoChute;
            ...
            public void Think()
            {
                ...
                {
                    if (profile.Behaviour.Peace_Keeper && !CurrentTarget.IsHostile() || SafeIgnore(CurrentTarget, npc, profile.Behaviour.Respect_Safe_Zones))
            ...
            {
                var pos = bs.TryGetSpawn(CurrentTarget == null || currentRange == Range.Long ? npc.transform.position : CurrentTarget.transform.position, radius, profile.type == ProfileType.Event, profile.Other.OverWater);
            ...
            public bool WantsAttack(BasePlayer player, bool hurt)
            {
                ...
                if (profile.Behaviour.Respect_Safe_Zones && player.InSafeZone() || npc.InSafeZone())  
                    return false;
            ...
            bool FactionAllies(int aa, int ab, int ba, int bb) => (aa == 0 && ab == 0) || (ba == 0 && bb == 0) || (aa != 0 && aa == ba) || (aa != 0 && aa == bb) || (ab != 0 && ab == ba) || (ab != 0 && ab == bb);
            ...
            public void AttackTarget(BasePlayer target, bool targetIsLOS)
            {
                ...
                if (melee)
                {
                    ...
                    if (target != null && bs.HasNav(target.transform.position))
                    {
                        ...
                        if (!stationary && npc.Brain.Navigator.CanUseNavMesh) 
        ...
        void BSMainUI(BasePlayer player, string tab, string profile = "", string sub = "", int page = 1)
        {
            ...
            else
            {
                ...
                {
                    elements.Add(new CuiPanel { Image = { Color = $"0 0 0 0.8" }, RectTransform = { AnchorMin = $"0.06 0.12", AnchorMax = $"0.34 0.25" }, CursorEnabled = true }, mainName);
                    elements.Add(new CuiButton { Button = { Command = $"BSUITPPlayer {RS(profile)}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.1 0.21", AnchorMax = $"0.3 0.24" }, Text = { Text = "TP me there", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter } }, mainName);
                ...
                elements.Add(new CuiButton { Button = { Command = $"BSCopy {tab} {RS(profile)} {sub}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.68 0.17", AnchorMax = $"0.79 0.20" }, Text = { Text = "Copy profile", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter } }, mainName);
                if (Copy.ContainsKey(player.userID))
                    elements.Add(new CuiButton { Button = { Command = $"BSPaste {tab} {RS(profile)} {sub}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.68 0.13", AnchorMax = $"0.79 0.16" }, Text = { Text = $"Paste {Copy[player.userID]}", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter } }, mainName);

                elements.Add(new CuiButton { Button = { Command = $"BSUISaveTemplate {RS(profile)}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.81 0.17", AnchorMax = $"0.92 0.20" }, Text = { Text = "Save template", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter } }, mainName);
                if (templateData.Templates.Count > 0)
                    elements.Add(new CuiButton { Button = { Command = $"BSUIViewTemplates {tab} {RS(profile)} {sub}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.81 0.13", AnchorMax = $"0.92 0.16" }, Text = { Text = "Load template", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter } }, mainName);
        ...
        private void BSUIShowSpawns(ConsoleSystem.Arg arg)
        {
            ...
            for (int i = 0; i < s.Count; i++)
            {
                ...
                ShowSpawn(arg.Player(), t.TransformPoint(s[i].loc), i, 20);
            }
        ...
        private void BSUISetEditing(ConsoleSystem.Arg arg)
        {
            ...
            MsgUI(arg.Player(), $"You can add spawnpoints to {RD(arg.Args[0])} by console command 'addspawn'", 5);
        }
...
//  Assist_Sense_Range.
//  Roam_Pause_Length.
//  OverWater default false. Allows roaming on custom monuments at sea.
...
//  Added Allow_AlphaLoot false - Global config option
