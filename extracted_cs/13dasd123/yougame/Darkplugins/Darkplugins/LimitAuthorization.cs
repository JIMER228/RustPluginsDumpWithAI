...
{
    /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ [Info("LimitAuthorization", "https://discord.gg/dNGbxafuJn", "1.1.4")]
    class LimitAuthorization : RustPlugin
    {
        ...
        public List<ulong> GetFriendsRP(BasePlayer player)
        {
            var result = (ulong[])Friends.CallHook("GetFriends", player.userID);
            return result?.ToList();
        }
        ...
        public List<string> GetFriends(BasePlayer player)
        {
            if (config.FriendsAllow && plugins.Exists("Friends"))
            {
                ...
                //Friends by RustPlugin.ru
                if (Friends.Author.Contains("OxideBro"))
                {
                    return GetFriendsRP(player).Select(p => p.ToString()).ToList();
                }
                return null;
            }
