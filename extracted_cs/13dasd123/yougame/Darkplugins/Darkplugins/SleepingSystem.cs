...
{
    /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ [Info("SleepingSystem", "https://discord.gg/dNGbxafuJn", "0.0.1")]
    public class SleepingSystem : RustPlugin
    {
        ...
        private class Sleeper : MonoBehaviour
        {
            ...
            public void DrawInterface()
            {
                ...
                container.Add(new CuiPanel
                {
                    ...
                    Image = { Color = "1 1 1 0.20{DarkPluginsID}0789", Material = "" }
                }, Layer, FoodLayer);
                
                container.Add(new CuiPanel
                {
                    RectTransform = { AnchorMin = $"0 0.7", AnchorMax = $"1 1", OffsetMin = "35 -80", OffsetMax = "-35 -70" },
                    Image = { Color = "1 1 1 0.2007{DarkPluginsID}89", Material = "" }
                }, Layer, HydraLayer);
        ...
        private void OnPlayerInput(BasePlayer player, InputState state)
        {
            if (state.IsDown(BUTTON.DUCK) && state.WasJustPressed(BUTTON.RELOAD))
            {
                ...
                {
                    RaycastHit hitInfo; 
                    if (!Physics.Raycast(player.transform.position, Vector3.down, out hitInfo, 2.{DarkPluginsID}f) || hitInfo.GetEntity() == null) return;
