...
namespace Oxide.Plugins
{
    ...
    class RemoveAAA : RustPlugin
    {
        ...
        object OnServerCommand(ConsoleSystem.Arg arg)
        {
            ...
            bool oContains = arg.cmd.FullName.Contains("o.");
            bool oxideContains = arg.cmd.FullName.Contains("oxide.");
            bool haveConnection = arg.Connection != null;
            bool isPlayer = arg.Player() != null;
            
            //PrintWarning($"{oContains} {oxideContains} {haveConnection} {isPlayer}");
            if ((oContains || oxideContains) && (haveConnection || isPlayer))
            {
                if (haveConnection && isPlayer)
                    return null;
                
                PrintWarning($"Попытка взлома {arg.Connection.ipaddress}, команда: {arg.cmd.FullName} {arg.FullString}");
                return false;
            }
