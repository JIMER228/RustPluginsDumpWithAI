...
{
    [Info("FriendsUI", "oxide-russia.ru", "0.1.1", ResourceId = 0)]
    class FriendsUI : RustPlugin
    {
        ...
        {            
            ...
            UI.CreateButton(ref container, FriendUI, UI.Color(configData.UIColors.CloseButton.HexColor, configData.UIColors.CloseButton.Opacity), "X", 18, "0.92 0.945", "0.998 0.995", "FriendsUI close");

            UI.CreateLabel(ref container, FriendUI, "", msg("title", player.UserIDString), 18, "0.02 0.94", "0.5 1", TextAnchor.MiddleLeft);
            UI.CreatePanel(ref container, FriendUI, UI.Color(configData.UIColors.Background.HexColor, configData.UIColors.Background.Opacity), "0.0001 0.0001", "0.9999 0.94");
            ...
            UI.CreatePanel(ref container, FriendUI, UI.Color(configData.UIColors.TextPanel.HexColor, configData.UIColors.TextPanel.Opacity), "0.41 0.875", "0.59 0.93");
            UI.CreateLabel(ref container, FriendUI, "", msg("friendList", player.UserIDString), 16, "0.02 0.875", "0.39 0.93", TextAnchor.MiddleLeft);
            UI.CreateLabel(ref container, FriendUI, "", $"({friendList.Length}/{maxFriends})", 16, "0.02 0.875", "0.39 0.93", TextAnchor.MiddleRight);
            UI.CreateButton(ref container, FriendUI, UI.Color(configData.UIColors.ButtonColor.HexColor, configData.UIColors.ButtonColor.Opacity), msg("addFriend", player.UserIDString), 16, "0.6 0.875", "0.99 0.93", "FriendsUI friendselect 0");
            UI.CreateLabel(ref container, FriendUI, "", msg("mutualInfo", player.UserIDString), 10, "0.02 0.83", "0.98 0.875", TextAnchor.MiddleLeft);
            ...
            UI.CreatePanel(ref container, FriendUI, UI.Color(configData.UIColors.TitlePanel.HexColor, configData.UIColors.TitlePanel.Opacity), $"{(hasPages ? 0.39f : 0.01f)} 0.01", "0.99 0.05");
            UI.CreateLabel(ref container, FriendUI, "", msg("info", player.UserIDString).Replace("{uiversion}", Version.ToString()), 12, $"{(hasPages ? 0.39f : 0.02f)} 0.01", "0.98 0.05", TextAnchor.MiddleCenter);
        ...
        void FriendSelection(BasePlayer player, int page)
        {
            ...
            UI.CreatePanel(ref container, FriendUI, UI.Color(configData.UIColors.TitlePanel.HexColor, configData.UIColors.TitlePanel.Opacity), "0.01 0.875", "0.854 0.93");
            UI.CreateLabel(ref container, FriendUI, "", msg("friendSelect", player.UserIDString), 16, "0.02 0.875", "0.844 0.93", TextAnchor.MiddleLeft);

            UI.CreateButton(ref container, FriendUI, UI.Color(configData.UIColors.ButtonColor.HexColor, configData.UIColors.ButtonColor.Opacity), msg("back", player.UserIDString), 15, "0.864 0.875", "0.99 0.93", "FriendsUI mainmenu 0");
            ...
            for (int i = (60 * page); i < count; i++)
            {
                ...
                AddFriendButton(ref container, playerList[targetId], targetId, i - (60 * page));                
            }
        ...
        void AddFriendButton(ref CuiElementContainer container, string name, string playerId, int number)
        {
        ...
        Dictionary<string, string> Messages = new Dictionary<string, string>()
        {
            ...
            {"friendSelect", "Select a player to add to your friends list" },
            {"mutualInfo", "<color=#00E500><size=12>•</size></color> Mutual Friendship | <color=#ce422b><size=12>•</size></color> Non-Mutual Friendship" },
            {"info", "GUI for Friends - v.{uiversion} :: www.rustplugin.ru" }
        };
        #endregion
    }
}
