...
{
    [Info("AntiCheat", "OxideBro/RustPlugin.ru", "2.1.51")]
    class AntiCheat : RustPlugin
    {
        ...
        private void OnEntityTakeDamage(BaseCombatEntity entity, HitInfo info, BasePlayer player)
        {
            if (entity is BasePlayer && info.Initiator is BasePlayer)
            {
                ...
                if (IsNPC(init) || IsNPC(player)) return;
                if (init == null) return;
                ...
                if (distance > 10 && !IsNPC(init))
                {
                    ...
                    if (bodylock.Ticks > 5)
                    {
                        ...
                        LogToFile("log", $"[{DateTime.Now.ToShortTimeString()}] - (АимЛок) {init.displayName}({init.UserIDString})| обнаружений {bodylock.Ticks} |  {bodylock?.Body ?? ""} | {distance} м.", this, true);
                        bodylock.Ticks = 1;
                    }
                    ...
                    if (con.Попаданий > AimPercentOverCount && aim > AimPercent && kdr > 2 && AIMEnabled)
                    {
                        ...
                        LogToFile("ban", $"[{DateTime.Now.ToShortTimeString()}] - {init.displayName}({init.UserIDString}) забанен! Соотношение попаданий в голову {aim}% и КДР - ({kdr}) аномальные!", this, true);
                        return;
                    }
                    if (con.Попаданий > AimPercentOverCount && aim > AimPercent && AIMEnabled)
                    {
                        ...
                        LogToFile("ban", $"[{DateTime.Now.ToShortTimeString()}] - {init.displayName}({init.UserIDString}) забанен! Процент попаданий в голову слишком большой {aim}%", this, true);
                    }
                ...
                if (instance.SHEnable)
                {
                    if (hack.player.IsOnGround())
                    {
                        ...
                        else
                        {
                            ...
                            SendDetection(hack.player, string.Format($"<color=#ffa500>[Античит детект]</color> (SPEEDHack) Игрок {hack.player.displayName}({hack.player.UserIDString}) двигаеться со скоростью выше нормы. Предупреждений: {b + 1}"));
