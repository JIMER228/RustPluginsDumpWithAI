    ...
    class MultiEvents : RustPlugin
    {
                ...
                case "KingMountain":
                    {
                        ...
                        DestroyTimer = timer.Once(config.KingMountain.TimeDelay, () =>
                        {
                            DestroyEvent(nowEvent);
                        });
                    }
                ...
                case "HuntAnimal":
                    {
                        ...
                        DestroyTimer = timer.Once(config.HuntAnimal.TimeDelay, () =>
                        {
                            DestroyEvent(nowEvent);
                        });
                    }
                ...
                case "HelicopterPet":
                    {
                        ...
                        DestroyTimer = timer.Once(config.HelicopterPet.TimeDelay, () =>
                        {
                            DestroyEvent(nowEvent);
                        });
                    }
                ...
                case "LookingLoot":
                    {
                        ...
                        DestroyTimer = timer.Once(config.LookingLoot.TimeDelay, () =>
                        {
                            DestroyEvent(nowEvent);
                        });
                    }
                ...
                case "SpecialCargo":
                    {
                        ...
                        for (int i = 0; i < BasePlayer.activePlayerList.Count; i++)
                            UI_Notification(BasePlayer.activePlayerList[i], GetMessage("SpecialCargo.New", BasePlayer.activePlayerList[i].UserIDString, beginning_mission.displayPhrase.english), ".Notification.New");
                        ...
                        Main = timer.Repeat(1, (int)(time - GrabCurrentTime()), () =>
                        {
                            for (int i = 0; i < BasePlayer.activePlayerList.Count; i++)
                            {
                                ...
                                if (runner == null)
                                    if (Vector3.Distance(beginning_mission.transform.position, player.transform.position) < 100f)
                                    {
                                        runner = player;
                                        UI_Notification(runner, GetMessage("SpecialCargo.CreateRunner", runner.UserIDString, end_mission.displayPhrase.english, GetGridString(end_mission.transform.position)), ".Notification.CreateRunner");
                                        timer.Once(config.SpecialCargo.timeNewRunner, () => CuiHelper.DestroyUi(runner, Layer + ".Notification.CreateRunner"));
                                ...
                                if (runner == player && Vector3.Distance(end_mission.transform.position, player.transform.position) < 100f)
                                    DestroyEvent(nowEvent);
                            }
                        });

                        DestroyTimer = timer.Once(config.SpecialCargo.TimeDelay, () =>
                        {
                            DestroyEvent(nowEvent);
                        });
                    }
        ...
        void DestroyEvent(string type)
        {
            ...
            switch (type)
            {
                case "KingMountain":
                    ...
                    {
                        var list = PlayersTop.OrderByDescending(p => p.Value).Take(config.KingMountain.loot.Count).ToList();
                ...
                case "HuntAnimal":
                    ...
                    if (PlayersTop.Count != 0)
                    {
                        var list = PlayersTop.OrderByDescending(p => p.Value).Take(config.HuntAnimal.loot.Count).ToList();
                        for (int i = 0; i < list.Count; i++)
                        {
                            var winner = list[i];
                            DropItem(winner.Key, nowEvent, i);
                ...
                case "HelicopterPet":
                    ...
                    if (PlayersTop.Count != 0)
                    {
                        var list = PlayersTop.OrderByDescending(p => p.Value).Take(config.HelicopterPet.loot.Count).ToList();
                        for (int i = 0; i < list.Count; i++)
                        {
                            var winner = list[i];
                            DropItem(winner.Key, nowEvent, i);
                ...
                case "LookingLoot":
                    ...
                    if (PlayersTop.Count != 0)
                    {
                        var list = PlayersTop.OrderByDescending(p => p.Value).Take(config.LookingLoot.loot.Count).ToList();
                        for (int i = 0; i < list.Count; i++)
                        {
                            var winner = list[i];
                            DropItem(winner.Key, nowEvent, i);
        ...
        private void DropItem(BasePlayer player, string type, int temp = 0)
        {
            ...
            if (item != null)
            {
                ...
                var newItem = item.blueprint ? ItemManager.CreateByName("blueprintbase") : ItemManager.CreateByName(item.shortname, amount, item.skinid);
        ...
        void OnPlayerDie(BasePlayer player, HitInfo info)
        {
            ...
            {
                runner = BasePlayer.activePlayerList.ToList().GetRandom();
                for (int i = 0; i < BasePlayer.activePlayerList.Count; i++)
        ...
        {
            if (entity == null || player == null || entity as LootContainer == null || LookingLoot.Contains(entity as LootContainer)) return;
        ...
        void TopUI(BasePlayer player, string Type, string TopName = "", string description = "", string oMin = "", string oMax = "", string color = "", List<KeyValuePair<BasePlayer, float>> list = null)
        {
            ...
            switch (Type)
            {
                ...
                case "refresh":
                    ...
                    if (list != null)
                    {
                        for (int i = 0; i < list.Count; i++)
                        {
                            ...
                            {
                                RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1", OffsetMin = "24 0", OffsetMax = "0 0" },
                                Text = { Text = $"<b>{check.Key.displayName}</b>", FontSize = 16, Align = TextAnchor.MiddleLeft }
                            ...
                            container.Add(new CuiLabel
                            {
                                ...
                                Text = { Text = nowEvent == "KingMountain" ? $"{check.Value.ToString("0.0")} м" : nowEvent == "CollectionResources" ? $"{check.Value} шт" : $"{check.Value}", FontSize = 16, Align = TextAnchor.MiddleRight }
                ...
                case "SpecialCargo":
                    ...
                    {
                        RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1" },
                        Text = { Text = GetMessage("SpecialCargo.Purpose", player.UserIDString, end_mission.displayPhrase.english, GetGridString(end_mission.transform.position)), FontSize = 15, Align = TextAnchor.MiddleCenter }
                    }, Layer + ".SpecialCargo");
                    break;
            }
        ...
        protected override void LoadDefaultMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                {"FOUNDROP.Naked", "<size=16>Clear inventory! The event can be reached only <color=#FF9494><b>COMPLETELY NAKED</b></color></size>" },
