...
namespace Oxide.Plugins
{
    ...
    class BuildingUpgrade : RustPlugin
    {
        ...
        private bool EnabledRemove = true;
        ...
        {
            EnabledRemove = GetConfig(true, "Включить выключение удаления построек при включении Upgrade (Поддержка плагина Remove, только с сайта yougame!)"); 
        ...
        void DesBuidingUpgrade(BasePlayer player, string command, string[] args)
        {
            if (args == null || args.Length == 0)
            {
                if (grades.ContainsKey(player))
                {
                    ...
                    timers.Remove(player);
                    DestroyUI(player);
                    SendReply(player, Messages["UpOFF"]);
                    return;
                }
                else
                {
        ...
        Dictionary<string, string> Messages = new Dictionary<string, string>()
        {
            ...
            {"UpOFF", "Режим улучшения построек отключен!"},
        };
