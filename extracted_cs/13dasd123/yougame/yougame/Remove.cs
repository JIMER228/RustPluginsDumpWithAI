...
namespace Oxide.Plugins
{
    ...
    class Remove : RustPlugin
    {
        ...
        private bool EnabledBuildingUpgrade = true; 
		...
		private void LoadDefaultConfig()
        {
            ...
            GetConfig("Основные настройки", "Включить выключение авто-улучшения при включении режима удаления (Поддержка плагина BuildingUpgrade, только с сайта yougame!)", ref EnabledBuildingUpgrade); 
        ...
        void cmdRemove(BasePlayer player, string command, string[] args)
        {
            ...
            {
                BuildingUpgrade?.Call("DesBuidingUpgrade", player);
            } 
      ...
      //  Слив плагинов server-rust by Apolo YouGame
        ...
        void DesRemove(BasePlayer player, string command, string[] args)
        {
            if (args == null || args.Length == 0)
            {
                if (activePlayers.ContainsKey(player.userID))
                {
                    ...
                    DeactivateRemove(player.userID);
                    DestroyUI(player);
                    SendReply(player, Messages["REOFF"]);
                    return;
                }
                else
                {
        ...
        Dictionary<string, string> Messages = new Dictionary<string, string>()
        {
            ...
            {"REOFF", "Режим удаления построек отключен!" },
        };
        #endregion
