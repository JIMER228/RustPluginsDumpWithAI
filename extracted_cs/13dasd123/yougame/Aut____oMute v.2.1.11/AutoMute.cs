...
namespace Oxide.Plugins
{
    ...
    class AutoMute : CovalencePlugin
    {
        ...
        object OnUserChat(IPlayer player, string message)
        {
            ...
            if (mutes.ContainsKey(player.Id))
            {
                player.Command("chat.add", new object[] { "0", $"<color=RED>Чат заблокирован!</color> До окончания: <color=RED>{mutes[player.Id]} сек.</color>" });
                return false;
            }
            foreach (var word in Chat.Слова)
            {
                if (message.ToLower().Contains(word))
                {
                    ...
                    {
                        Mute(bplayer);
                        ConsoleNetwork.BroadcastToAllClients("chat.add", new object[] { 0, $"Игроку <color=#FF6347>{Nickname}</color> отключен чат! \nДлительность: <color=#FF6347>{Chat.MuteTime} сек.</color>  \nПричина: <color=#FF6347>Нецензурная лексика!</color>" });
