...
{
    [Info("IQCraftSystem", "anfunny", "2.0")]
    class IQCraftSystem : RustPlugin
    {
        ...
        private void OnServerInitialized()
        {
            ...
            foreach (var Icon in Items.Where(NIDROLDZVJKXCKNJAMTYQDBSLXKYYSDXHQZPZMMIBYXYOFTMK => NIDROLDZVJKXCKNJAMTYQDBSLXKYYSDXHQZPZMMIBYXYOFTMK.Value.DFYXMDOQITSQFCFNDCDZTRQXYRTUKJRKVVIBMIYOHBAXYL == TSBPHBMHELFQQDFGBMCRTDMFVEWPUHDDAUMJCPWWBAVHCDQLC.OALASODIGTPCWBYWXNCPANUIRGDDIZAPATKWIGXEPFZYQF || NIDROLDZVJKXCKNJAMTYQDBSLXKYYSDXHQZPZMMIBYXYOFTMK.Value.DFYXMDOQITSQFCFNDCDZTRQXYRTUKJRKVVIBMIYOHBAXYL == TSBPHBMHELFQQDFGBMCRTDMFVEWPUHDDAUMJCPWWBAVHCDQLC.RGPOFMTEINCGGTFPHWNXCWDDDLLKKVJOUYQVQYFTODMF || NIDROLDZVJKXCKNJAMTYQDBSLXKYYSDXHQZPZMMIBYXYOFTMK.Value.DFYXMDOQITSQFCFNDCDZTRQXYRTUKJRKVVIBMIYOHBAXYL == TSBPHBMHELFQQDFGBMCRTDMFVEWPUHDDAUMJCPWWBAVHCDQLC.Entity))
            {
                ...
                {
                    ImageLibrary.Call("AddImage", $"https://api.skyplugins.ru/api/getimage/{Icon.Value.Shortname}/128", Icon.Value.Shortname);
                }
                else
                {
            ...
            {
                foreach (var ItemCraft in Item.Value.MSOGFCEDYPLGIQMYWWYEQPIGGIICOZTJYGPJGPPTMHYR)
                {
                    ImageLibrary.Call("AddImage", ItemCraft.LJEEOVBWQPXGTEUSFGHUWCMKRWBGGRYNXSPLZLDVPWIX, ItemCraft.LJEEOVBWQPXGTEUSFGHUWCMKRWBGGRYNXSPLZLDVPWIX);
                }
            }
            foreach (var Item in Items)
            {
                ...
                {
                    ImageLibrary.Call("AddImage", $"https://api.skyplugins.ru/api/getskin/{Item.Value.SkinID}/128", Item.Value.SkinID.ToString());
                }
                else
                {
                    if (!String.IsNullOrWhiteSpace(Item.Value.Shortname))
                    {
                        ImageLibrary.Call("AddImage", $"https://api.skyplugins.ru/api/getimage/{Item.Value.Shortname}/128", Item.Value.Shortname);
                    }
                }
                foreach (var ItemCraft in Item.Value.MSOGFCEDYPLGIQMYWWYEQPIGGIICOZTJYGPJGPPTMHYR.Where(NIDROLDZVJKXCKNJAMTYQDBSLXKYYSDXHQZPZMMIBYXYOFTMK => !String.IsNullOrEmpty(NIDROLDZVJKXCKNJAMTYQDBSLXKYYSDXHQZPZMMIBYXYOFTMK.Shortname)))
                {
                    if (ItemCraft.SkinID != 0)
                    {
                        ImageLibrary.Call("AddImage", $"https://api.skyplugins.ru/api/getskin/{ItemCraft.SkinID}/128", ItemCraft.SkinID.ToString());
                    }
                    else
                    {
                        ImageLibrary.Call("AddImage", $"https://api.skyplugins.ru/api/getimage/{ItemCraft.Shortname}/128", ItemCraft.Shortname);
                    }
                }
            }
        ...
        void INJEGEPRCOJXHVIJVMGKYBNRMKYXYYYFCSUHXWZSUVSGFOYXO(BasePlayer player, TSBPHBMHELFQQDFGBMCRTDMFVEWPUHDDAUMJCPWWBAVHCDQLC VPWRENLSFIZAOZGMAAKYQQKJFKRWFJKMKLPSHSUVF = TSBPHBMHELFQQDFGBMCRTDMFVEWPUHDDAUMJCPWWBAVHCDQLC.ZPFDVOHMXJQPVRGDCPHVYBFRSDYBCCCYOYNFZQMXGFMLWWE, int DQLXMTQJZJQUMFEMLUWPMVXNPDJOEHFOTNNUYIYYF = 0)
        {
            ...
            foreach (var Item in Items)
            {
                ...
                YWOAREEHJETPXTMQRDLQWDIYPQQWKNAMSUDVTOPVTBE.Add(new CuiElement
                {
                    ...
                    Components = {
                                                new CuiRawImageComponent {
                                                        FadeIn = FadeIn, Png = EJEEDQFWUCEWMWHBQWQVDMQGQTHFCSOKNMDTYWQWDGV
                                                },
                                                new CuiRectTransformComponent {
