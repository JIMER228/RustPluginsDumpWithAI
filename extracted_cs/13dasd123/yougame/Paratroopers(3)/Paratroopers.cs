...
// GetComponent is expensive, store your components in a dictionary
// Added proper static cleanup and usage
// Proper subscription of hooks
// Fixed server not saving
// Fixed OnEntitySpawned cleanup
// Fixed bradley not spawning due to invalid hackable crate prefab
// Removed reflection as it's not allowed - this will make npcs act a bit differently
// Fixed config not loading or saving
// Changed AI to Apex
// Fixed language API usage
// Removed useless and unused code

namespace Oxide.Plugins
{
    [Info("Paratroopers", "FastBurst", "3.0.2")]
    ...
    class Paratroopers : RustPlugin
    {
        ...
        private const uint SCIENTIST_PREFABID = 4223875851;
        ...
        private Dictionary<Scientist, MapMarkerGenericRadius> botMarkerList = new Dictionary<Scientist, MapMarkerGenericRadius>();
        private Dictionary<Scientist, NPCParatrooper> paratrooperList = new Dictionary<Scientist, NPCParatrooper>();
        private List<Scientist> jumperList = new List<Scientist>();
        ...
        private object OnDeathNotice(Dictionary<string, object> data, string message) // this shouldn't be required ever
        {
            ...
            var entity = data["VictimEntity"] as Scientist;
        ...
        private object OnNpcResume(Scientist npc)
        {
            if (npc == null)
            {
                return null;
        ...
        private void OnEntityKill(Scientist victim) => OnPlayerDeath(victim, null);

        private void OnPlayerDeath(Scientist victim, HitInfo hitInfo)
        {
            if (victim == null)
            {
                return;
        ...
        private object OnTurretTarget(NPCAutoTurret turret, Scientist scientist) => HookHandler(scientist);

        private object CanHelicopterTarget(PatrolHelicopterAI heliAi, Scientist scientist) => HookHandler(scientist);

        private object CanHelicopterStrafeTarget(PatrolHelicopterAI heliAi, Scientist scientist) => HookHandler(scientist);

        private object OnNpcTarget(BaseNpc npc, Scientist scientist) => HookHandler(scientist);

        private object HookHandler(Scientist scientist) => scientist != null && paratrooperList.ContainsKey(scientist) ? true : (object)null;
        ...
        private object CanBradleyApcTarget(BradleyAPC apc, BasePlayer player)
        {
            ...
            {
                return HookHandler(player as Scientist);
            }
        ...
        private void OnEntityTakeDamage(Scientist scientist, HitInfo hitInfo)
        {
            if (scientist == null || hitInfo == null || !(hitInfo.Initiator is BasePlayer))
            {
                return;
            }

            NPCParatrooper paratrooper;
            if (!paratrooperList.TryGetValue(scientist, out paratrooper))
            {
                return;
            }
        ...
        private object CanPopulateLoot(Scientist scientist, NPCPlayerCorpse corpse) => HookHandler(scientist);

        private object OnCorpsePopulate(Scientist scientist, NPCPlayerCorpse corpse)
        {
            ...
            paratrooperList.Remove(scientist);
        ...
        private object CanLootPlayer(Scientist scientist, BasePlayer player)
        {
            if (scientist != null && paratrooperList.ContainsKey(scientist) && scientist.IsWounded())
            {
                return false;
            }
        ...
        private object OnPlayerAssist(Scientist scientist, BasePlayer player) => HookHandler(scientist);
        ...
        private void CreateBotMarker(Scientist npc)
        {
        ...
        private List<string> GetParatrooperPositions()
        {
            ...
            foreach (var scientist in paratrooperList.Keys)
            {
                ...
                var gridPos = GetGridString(scientist.transform.position);
        ...
        private class JumpEventManager
        {
            ...
            private static Scientist InstantiateEntity(Vector3 position)
            {
                var prefab = GameManager.server.FindPrefab(StringPool.Get(SCIENTIST_PREFABID));
                var go = Facepunch.Instantiate.GameObject(prefab, position, default(Quaternion));

                go.name = StringPool.Get(SCIENTIST_PREFABID);
                ...
                return go.GetComponent<Scientist>();
            }
            ...
            private IEnumerator SpawnNPCs(Vector3 position, string jumperKit, int botHealth, bool randomKit)
            {
                ...
                var npc = InstantiateEntity(GetRandomPoint(position));
        ...
        {
            internal Scientist npc { get; private set; }
            ...
            private bool isWaitingAtDestination { get; set; }
            ...
            private void InitializeNPC()
            {
                ...
                {
                    StripInventory(npc);
                    npc.Invoke(() => Interface.CallHook("GiveKit", npc, ParatrooperRandomKit ? configData.npc.Kits.GetRandom() : ParatrooperKit), 1f);
            ...
            private void WaterCheck(Scientist npc)
            {
            ...
            {
                parachute = GameManager.server.CreateEntity(StringPool.Get(CHUTE_PREFABID), npc.transform.position);
            ...
            public void OnReceivedDamage(BasePlayer attacker, HitInfo hitInfo)
            {
                ...
                npc.AttackTarget = attacker;

                if (distance > npc.Stats.AggressionRange && npc.Stats.AggressionRange < 200)
                {
                    npc.Stats.AggressionRange += 200;
                    npc.Stats.DeaggroRange += 200;
                }

                if (distance <= npc.Stats.AggressionRange && distance > 15f && npc.AttackTarget == attacker)
                {
            ...
            private void AttackTarget(Scientist npc, BasePlayer attacker)
            {
                ...
                if (attacker.IsVisible(npc.eyes.position, attacker.eyes.position, npc.Stats.AggressionRange))
                {
                ...
                if (++attacks >= 20)
                {
                    npc.Stats.AggressionRange = configData.npc.Aggro_Range;
                    npc.Stats.DeaggroRange = configData.npc.Aggro_Range * 1.25f;
                    attacks = 0;
                }
        ...
        private Vector3 GetAirfieldPosition()
        {
            ...
            return _airfieldPositions.GetRandom();
        }
