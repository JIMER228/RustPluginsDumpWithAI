...
namespace Oxide.Plugins
{
    ...
    public class ChatSystem : RustPlugin
    {
        private class PluginSettings
        {
            ...
            [JsonProperty("Стандартные префиксы игроков")]
        ...
        {
            [JsonProperty("Текущий префикс игрока")]
            public string ChatPrefix;        
            [JsonProperty("Текущий цвет имени игрока")]
        ...
        private string ModerPermission = "chatsystem.moder";
        ...
        private void HandleMessage(Chat.ChatChannel channel, BasePlayer player, string message)
        {
            ...
            var DataPlayer = playerSettings[player.userID];
            ...
            if (HookResult != null)
            {
                ...
                if (channel == Chat.ChatChannel.Team)
					FormatMessage = $"<color=#a5e664>[Team]</color> {ChatTags["Prefixes"].ToString().Replace("— ", "")} <color={playerChat.NickColor}>{player.displayName}</color>: {ChatTags["Message"]}";
				else FormatMessage = $"{ChatTags["Prefixes"].ToString().Replace("— ", "")} <color={playerChat.NickColor}>{player.displayName}</color>: {ChatTags["Message"]}";
            }
            else 
				if (channel == Chat.ChatChannel.Team)
					FormatMessage = $"<color=#a5e664>[Team]</color> {DataPlayer.ChatPrefix.Replace("—", "")} <color={playerChat.NickColor}>{player.displayName}</color>: {message}";
				else FormatMessage = $"{DataPlayer.ChatPrefix.Replace("—", "")} <color={playerChat.NickColor}>{player.displayName}</color>: {message}";

            
            if (permission.UserHasPermission(player.UserIDString, ModerPermission))
            {
				if (channel == Chat.ChatChannel.Team)
					FormatMessage = $"<color=#a5e664>[Team]</color> <color=#FF7514>MODER</color> <color={playerChat.NickColor}>{player.displayName}</color>: {message}";
			    else FormatMessage = $"<color=#FF7514>MODER</color> <color={playerChat.NickColor}>{player.displayName}</color>: {message}";
            }
            ...
            {
                check.SendConsoleCommand("chat.add", channel, player.userID, FormatMessage);
            }
        }
        ...
        void OnUserPermissionRevoked(string id, string perm)
        {
            ...
            if (playerSettings[ulong.Parse(id)].ChatPrefix == tag)
            {
                playerSettings[ulong.Parse(id)].ChatPrefix = Settings.Tags["chatsystem.default"];
        ...
        private void OnServerInitialized()
        {
            ...
            else
            {
                Settings = new PluginSettings
                {
                    ...
                    {
                        "Добро пожаловать на наш сервер! Приятной игры! :)",
                        "Для получения информации о сервере, используйте <color=#FFC040>/info</color>",
                        "У нас есть наборы для игроков, используйте <color=#FFC040>/kit</color>",
                        "Не забывайте посетить наш магазин: <color=#FFC040>http://rustserver.ru</color>"
                    },
                    Colors = new Dictionary<string, string>
                    {
                        ["#55aaff"] = "chatsystem.blue",
                        ["#FFFF00"] = "chatsystem.yellow",
                        ["#800080"] = "chatsystem.purple",
                        ["#00FF00"] = "chatsystem.lime",
                        ["#00FFFF"] = "chatsystem.cyan",
                        ["#FFA500"] = "chatsystem.orange",
                        ["#FF0000"] = "chatsystem.red",
                        ["#808000"] = "chatsystem.olive",
                    },
                    Tags =
                    {
                        ["chatsystem.default"] = "—",
                        ["chatsystem.vip"] = "<color=#ADFF2F>VIP</color>",
                        ["chatsystem.elite"] = "<color=#FCF777>ELITE</color>",
                        ["chatsystem.gold"] = "<color=#FFFF00>GOLD</color>",
                        ["chatsystem.youtube"] = "<color=#DC143C>YOUTUBE</color>",
                        ["chatsystem.admin"] = "<color=#FF4500>ADMIN</color>"
                    },
        ...
        private void OnPlayerConnected(BasePlayer player)
        {
            if (!playerSettings.ContainsKey(player.userID))
                playerSettings.Add(player.userID, new Chatter { ChatPrefix = Settings.Tags["chatsystem.default"], NickColor = "#4286f4"});
        }

        [ChatCommand("mute")]
        private void cmdMuteChat(BasePlayer player)
        {
            if (!player.IsAdmin && !permission.UserHasPermission(player.UserIDString, ModerPermission))
			{
        ...
        private void cmdChatPM(BasePlayer player, string command, string[] args)
        {
            ...
            SendReply(player, $"<color=#DAA520>ЛС для {reciever.displayName}</color>:\n" + message);
            SendReply(reciever, $"<color=#DAA520>ЛС от {player.displayName}</color>:\n" + message);
        ...
        private void consoleUIHandler(ConsoleSystem.Arg args)
        {
            ...
            if (args.Args[0].StartsWith("#"))
            {
                ...
                {
                    playerSettings[player.userID].NickColor = args.Args[0];
                    ChatSettUp(player, "updatecolor");
                    player.ChatMessage($"Вы успешно изменили цвет ника!");
                }
                else
                {
                    player.ChatMessage($"Вы не можете изменить цвет ника, у вас недостаточно прав!");
                    return;
                }
            }
            ...
            switch (args.Args[0].ToLower())
            {
                ...
                case "settag":
                {
                    ...
                    {
                        player.ChatMessage($"Вы не можете изменить ваш префикс, у вас недостаточно прав!");
                        return;
                    }
                    playerSettings[args.Player().userID].ChatPrefix = args.Args[1];
                    ChatSettUp(args.Player(), $"updatetag");
                    player.ChatMessage($"Вы успешно изменили свой префикс!");
                    break;
                }
            }
        }
        ...
        private void ChatSettUp(BasePlayer player, string change = "")
        {
            ...
            if (string.IsNullOrEmpty(change) || change == "updatetag")
            {
                ...
                foreach (var check in Settings.Tags)
                {
                    string color = playerSettings[player.userID].ChatPrefix == check.Value ? HexToRustFormat("#8eb9ff3c") : "0 0 0 0";
            ...
            if (string.IsNullOrEmpty(change) || change == "updatecolor")
            {
                ...
                {
                    string text = "";
                    if (check.Key == playerSettings[player.userID].NickColor)
                        text = "ТЕКУЩИЙ";
