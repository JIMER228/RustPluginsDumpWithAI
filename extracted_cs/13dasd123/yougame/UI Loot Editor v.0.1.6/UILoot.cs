...
namespace Oxide.Plugins
{
    ...
    public class UILoot : RustPlugin
    {
        ...
        private class RateHandler
        {
            public class Rates
            {
                ...
                public Rates(BaseEntity entity)
                {
                    ...
                    if (obj != null)
                    {
                        foreach (var check in entity.GetComponent<LootContainer>()?.LootSpawnSlots?.Select(p => p.definition))
                        {
                            foreach (var drop in check?.subSpawn?.Select(p => p.category))
                            {
                                foreach (var item in drop?.subSpawn?.SelectMany(p => p.category.items))
                                ...
                                if (p.category?.subSpawn != null)
                                {
                                    p.category.subSpawn.ToList().ForEach(t =>
                                    {
                                        if (t.category?.items != null)
                                        {
                                            ...
                                            { 
                                                if (!ItemsList.Any(x => x.ShortName == z.itemDef.shortname && x.SkinID == 0 && x.IsBlueprint == z.itemDef.spawnAsBlueprint))
                                                    itemList.Add(new Items(z.itemDef.shortname, (int) z.startAmount, z.maxAmount == -1 || z.maxAmount == 0 ? (int) z.startAmount : (int) z.maxAmount, 0, z.itemDef.spawnAsBlueprint)); 
                                            });
                                        }  
                                    });
                                }  
        ...
        private void Unload() => BasePlayer.activePlayerList.ForEach(p => CuiHelper.DestroyUi(p, MainLayer));
        ...
        private void UI_DrawRate(BasePlayer player, RateHandler.Rates rate, int page)
        {
            ...
            { 
                ...
                Text = { Text = "ПЕРЕНАПОЛНИТЬ", Align = TextAnchor.UpperLeft, Font = "robotocondensed-regular.ttf", FontSize = 24 }
            }, ShowRate);
            ...
            { 
                ...
                Text = { Text = rate.DropAmount.ToString(), Align = TextAnchor.UpperCenter, Font = "robotocondensed-regular.ttf", FontSize = 24 }
            ...
            container.Add(new CuiButton
            {
                ...
                Text = { Text = "-", Color = rate.DropAmount > 0 ? "1 1 1 1" : "1 1 1 0.2", Align = TextAnchor.UpperRight, Font = "robotocondensed-regular.ttf", FontSize = 40 }
            }, ShowRate);
            ...
            container.Add(new CuiButton
            {
                ...
                Text = { Text = "ВОССТАНОВИТЬ", Color =  "1 1 1 1", Align = TextAnchor.UpperRight, Font = "robotocondensed-regular.ttf", FontSize = 24 } 
            }, ShowRate); 
            ...
            container.Add(new CuiButton
            {
                ...
                Text = { Text = "СКОПИРОВАТЬ <b>В</b> ИНВЕНТАРЬ", Align = TextAnchor.LowerLeft, Font = "robotocondensed-regular.ttf", FontSize = 24 }
            }, ShowRate);
            ...
            { 
                ...
                Text = { Text = "СКОПИРОВАТЬ <b>ИЗ</b> ИНВЕНТАРЯ", Align = TextAnchor.LowerRight, Font = "robotocondensed-regular.ttf", FontSize = 24 }
            }, ShowRate);
            ...
            container.Add(new CuiButton
            {
                ...
                Text = { Text = "ДОБАВИТЬ <b>ИЗ</b> ИНВЕНТАРЯ", Align = TextAnchor.LowerCenter, Font = "robotocondensed-regular.ttf", FontSize = 24 }
            }, ShowRate);
        ...
        private void UI_DrawItem(BasePlayer player, RateHandler.Rates rate, RateHandler.Rates.Items item, float leftPos, float topPos, bool firstPass = true)
        {
            ...
            container.Add(new CuiButton 
            {
                ...
                Text = { Text = "УДАЛИТЬ", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", FontSize = 14 }
            }, ShowRate + $".{rate.ItemsList.IndexOf(item)}.BG");
         
            container.Add(new CuiLabel
            {
                RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1", OffsetMin = "0 25", OffsetMax = "0 0" }, 
                Text = { Text = "МАКСИМУМ", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", Color = "1 1 1 0.4" }
            }, ShowRate + $".{rate.ItemsList.IndexOf(item)}.Up");
            
            container.Add(new CuiLabel
            {
                RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1", OffsetMin = "0 0", OffsetMax = "0 -25" }, 
                Text = { Text = "МИНИМУМ", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", Color = "1 1 1 0.4" }
            ...
            { 
                ...
                Button = { Color = $"0.801007 0.4 0.4 {(item.Amounts.Min - 1 >= 0 ? 0.6 : 0.1)}", Command = $"UI_UILootHandler min {rate.PrefabName.Replace(" ", "+")} -1 {rate.ItemsList.IndexOf(item)}" },
            ...
            container.Add(new CuiButton
            {
                ...
                Button = { Color = "0.4054100787 0.8010074584 0.401007879 0.6", Command = $"UI_UILootHandler max {rate.PrefabName.Replace(" ", "+")} 1 {rate.ItemsList.IndexOf(item)}" }, 
                Text = { Text = "+", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", FontSize = 20 }
        ...
        private void CmdConsoleHandler(ConsoleSystem.Arg args)
        {
            ...
            switch (args.Args[0].ToLower())
            {
                case "to":
                {
                    ...
                    NextTick(() =>
                    {
                       foreach (var check in rate.ItemsList)
                       {
                           ...
                           if (!player.inventory.GiveItem(create)) create.Drop(player.transform.position, Vector3.down);
                       }
                ...
                case "from":
                {
                    ...
                    {
                        if (!rate.ItemsList.Any(p => p.ShortName == check.info.shortname && p.SkinID == check.skin && p.IsBlueprint == check.IsBlueprint()))
        ...
        private void PopulateLoot(BaseEntity entity)
        {
            ...
            NextTick(() =>
            {
                ...
                {
                    check.ToItem().MoveToContainer(obj.inventory);
                }
