...
namespace Oxide.Plugins
{
    ...
    class Paratroopers : RustPlugin
    {
        ...
        private const uint NPCPlayer_PREFABID = 1256759028; //
        ...
        private Dictionary<NPCPlayer, MapMarkerGenericRadius> botMarkerList = new Dictionary<NPCPlayer, MapMarkerGenericRadius>();
        private Dictionary<NPCPlayer, NPCParatrooper> paratrooperList = new Dictionary<NPCPlayer, NPCParatrooper>();
        private List<NPCPlayer> jumperList = new List<NPCPlayer>();
        ...
        private object OnDeathNotice(Dictionary<string, object> data, string message) // this shouldn't be required ever
        {
            ...
            var entity = data["VictimEntity"] as NPCPlayer;
        ...
        private void OnEntityKill(NPCPlayer victim) => OnPlayerDeath(victim, null);

        private void OnPlayerDeath(NPCPlayer victim, HitInfo hitInfo)
        {
            if (victim == null)
            {
                return;
        ...
        private object OnTurretTarget(NPCAutoTurret turret, NPCPlayer NPCPlayer) => HookHandler(NPCPlayer);

        private object CanHelicopterTarget(PatrolHelicopterAI heliAi, NPCPlayer NPCPlayer) => HookHandler(NPCPlayer);

        private object CanHelicopterStrafeTarget(PatrolHelicopterAI heliAi, NPCPlayer NPCPlayer) => HookHandler(NPCPlayer);

        private object OnNpcTarget(BaseNpc npc, NPCPlayer NPCPlayer) => HookHandler(NPCPlayer);

        private object HookHandler(NPCPlayer NPCPlayer) => NPCPlayer != null && paratrooperList.ContainsKey(NPCPlayer) ? true : (object)null;
        ...
        private object CanBradleyApcTarget(BradleyAPC apc, BasePlayer player)
        {
            ...
            {
                return HookHandler(player as NPCPlayer);
            }
        ...
        private void OnEntityTakeDamage(NPCPlayer NPCPlayer, HitInfo hitInfo)
        {
            if (NPCPlayer == null || hitInfo == null || !(hitInfo.Initiator is BasePlayer))
            {
                return;
            }

            NPCParatrooper paratrooper;
            if (!paratrooperList.TryGetValue(NPCPlayer, out paratrooper))
            {
                return;
            }
        ...
        private object CanPopulateLoot(NPCPlayer NPCPlayer, NPCPlayerCorpse corpse) => HookHandler(NPCPlayer);

        private object OnCorpsePopulate(NPCPlayer NPCPlayer, NPCPlayerCorpse corpse)
        {
            ...
            paratrooperList.Remove(NPCPlayer);
        ...
        private object CanLootPlayer(NPCPlayer NPCPlayer, BasePlayer player)
        {
            if (NPCPlayer != null && paratrooperList.ContainsKey(NPCPlayer) && NPCPlayer.IsWounded())
            {
                return false;
            }
        ...
        private object OnPlayerAssist(NPCPlayer NPCPlayer, BasePlayer player) => HookHandler(NPCPlayer);
        ...
        private void CreateBotMarker(NPCPlayer npc)
        {
        ...
        private List<string> GetParatrooperPositions()
        {
            ...
            foreach (var NPCPlayer in paratrooperList.Keys)
            {
                ...
                var gridPos = GetGridString(NPCPlayer.transform.position);
        ...
        private class JumpEventManager
        {
            ...
            private static NPCPlayer InstantiateEntity(Vector3 position)
            {
                ...
                return go.GetComponent<NPCPlayer>();
            }
            ...
            private IEnumerator SpawnNPCs(Vector3 position, string jumperKit, int botHealth, bool randomKit)
            {
                ...
                var spawner = npc.GetComponent<ScientistSpawner>();
                if(spawner != null)
                    spawner.NeverMove = true;
        ...
        {
            internal NPCPlayer npc { get; private set; }
            ...
            private void WaterCheck(NPCPlayer npc)
            {
            ...
            private void AttackTarget(NPCPlayer npc, BasePlayer attacker)
            {
