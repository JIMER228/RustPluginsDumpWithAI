...
{
	[Info("GameStoresRUST", "Forum: https://topplugin.ru Ds: alone_sempai Vk: https://vk.com/rustnastroika", "1.0.5")]
	public class GameStoresRUST : RustPlugin
	{
		...
		private const string
			...
			CFApiLink = "https://ac.gamestores.app/v1/",
		...
		[ConsoleCommand("mb.store.open")]
		private void cmdOpenStore(ConsoleSystem.Arg arg)
		{
			if (arg.Player() == null)
				return;
		...
		[ConsoleCommand("mb.store")]
		private void cmdOpenStoreAlias(ConsoleSystem.Arg arg)
		{
			// alias to open the store from MenuBase or other plugins
			cmdOpenStore(arg);
		}
		...
		private void ShowHelpUI(BasePlayer player)
		{
			...
			container.Add(new CuiLabel
			{
				...
				Text =
				{
					...
					Font = "robotocondensed-regular.ttf", FontSize = 14, Align = TextAnchor.MiddleCenter
				}
			}, HelpLayer);
			...
			int page)
		{
			...
			var wItems = playerBasket.GetItems(0, 500000);

			var secondContainer = new CuiElementContainer();
			
			secondContainer.Add(new CuiElement()
			{
				Name = Layer + ".main.div" + ".store" + ".items.div",
				DestroyUi = Layer + ".main.div" + ".store" + ".items.div",
				Parent = Layer + ".main.div" + ".store",
				Components =
				{
					new CuiScrollViewComponent
					{
						...
						VerticalScrollbar = new()
						{
							...
							HandleSprite = null,
							Size = 2,
							HandleColor = "0.9490196 0.5019608 0.05490196 1",
							HighlightColor = "0.9490196 0.5019608 0.05490196 1",
							PressedColor = "0.9490196 0.5019608 0.05490196 1",
							TrackSprite = null,
					...
					new CuiRectTransformComponent()
					{
						AnchorMin = "0 1", AnchorMax = "0 1", OffsetMin = "11.44 -344.256", OffsetMax = "615.551 -4.4"
					}
				}
			});
			secondContainer.Add(new CuiPanel()
			{
				Image = { Color = "1 1 1 0.3" },
				RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1", OffsetMin = "-1000 -10000", OffsetMax = "1000 0"}
			}, Layer + ".main.div" + ".store" + ".items.div");
			//
			// secondContainer.Add(new CuiPanel
			// {
			// 	CursorEnabled = false,
			// 	Image = { Color = "1 1 1 0.4" },
			// 	RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-303.56 -141.256", OffsetMax = "300.551 198.6" }
			// }, Layer + ".main.div" + ".store", Layer + ".main.div" + ".store" + ".items.div", Layer + ".main.div" + ".store" + ".items.div");

			float minx = 16.80002f;
			float maxx = 95.60751f;
			float miny = -95.05719f;
			float maxy = -16.1875f;
			int i = 0;
			foreach (var x in wItems)
			{
				if (i % 6 == 0 && i != 0)
				{
					minx = 16.80002f;
				...
				{
					Button = { Color = "0 0 0 0.4", Command = $"mb.store.selectitem {x.basketId}"},
					RectTransform = { AnchorMin = "0 1", AnchorMax = "0 1", OffsetMin = $"{minx} {miny}", OffsetMax = $"{maxx} {maxy}" }
				}, Layer + ".main.div" + ".store" + ".items.div", Layer + ".main.div" + ".store" + ".items.div" + $".{x.basketId}");
				
				if (x.IsBlueprint)
				{
					secondContainer.Add(new CuiElement
					{
						Parent = Layer + ".main.div" + ".store" + ".items.div" + $".{x.basketId}",
						Components =
						{
				...
				if (_config.Plugin.LoadSpriteImages && x.IsValid && x.itemDefinition != null)
				{
					secondContainer.Add(new CuiElement
					{
						Parent = Layer + ".main.div" + ".store" + ".items.div" + $".{x.basketId}",
						Components =
						{
							new CuiImageComponent {ItemId = x.itemDefinition.itemid},
							new CuiRectTransformComponent
								{AnchorMin = "0 0", AnchorMax = "1 1", OffsetMin = "5 5", OffsetMax = "-5 -5"}
						}
					});
				...
				{
					if (!x.ImageUrl.Contains("http"))
						secondContainer.Add(new CuiElement
						{
							Parent = Layer + ".main.div" + ".store" + ".items.div" + $".{x.basketId}",
							Components =
							{
								new CuiRawImageComponent {Png = x.ImageUrl},
								new CuiRectTransformComponent
									{AnchorMin = "0 0", AnchorMax = "1 1", OffsetMin = "5 5", OffsetMax = "-5 -5"}
							}
						});
						...
						secondContainer.Add(new CuiElement
						{
							...
							Components =
							{
								new CuiRawImageComponent { Url = x.ImageUrl },
								new CuiRectTransformComponent
									{AnchorMin = "0 0", AnchorMax = "1 1", OffsetMin = "5 5", OffsetMax = "-5 -5"}
							}
						});
				...
				// }, Layer + ".main.div" + ".store" + ".items.div" + $".{x.basketId}", Layer + ".main.div" + ".store" + ".items.div" + $".{x.basketId}" + ".img");

				secondContainer.Add(new CuiElement
				{
					Name = Layer + ".main.div" + ".store" + ".items.div" + $".{x.basketId}" + ".text",
					Parent = Layer + ".main.div" + ".store" + ".items.div" + $".{x.basketId}",
					Components = {
						new CuiTextComponent { Text = x.Name, Font = "robotocondensed-regular.ttf", FontSize = 9, Align = TextAnchor.LowerCenter, Color = "1 1 1 1" },
						new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-42.344 -37.269", OffsetMax = "42.344 -21.331" }
					}
				});
			...
			(secondContainer[0].Components[0] as CuiScrollViewComponent).ContentTransform.OffsetMin = $"0 {Mathf.Min(miny - 14, -339.856f)}";
			if (miny > -339.856f)
			{
				(secondContainer[0].Components[0] as CuiScrollViewComponent).VerticalScrollbar = null;
			}
			// for (int i = 0; i < option.Count; i++) {
			// 	if (i != 0) {
			// 		int[] resetIndexs = {6,12};
			...
			// 		Text = { Text = "VIP на 7 дней", Font = "robotocondensed-regular.ttf", FontSize = 9, Align = TextAnchor.LowerCenter, Color = "1 1 1 1" },
		...
		[ConsoleCommand("mb.store.selectitem")]
		private void cmdSelectItem(ConsoleSystem.Arg arg)
		{
			if (arg.Player() == null || arg.Args.IsNullOrEmpty())
				return;

			string basketId = arg.Args[0];
			
			UI_UpdateTakeButton(arg.Player(), basketId);
		}
		
		private void UI_UpdateTakeButton(BasePlayer player, string basketId)
		{
			var container = new CuiElementContainer();
			...
			{
				CuiHelper.DestroyUi(player, Layer + ".main.div" + ".store" + ".itemname");
				CuiHelper.DestroyUi(player, Layer + ".main.div" + ".store" + ".take.btn");
				return;
			}

			if (!_playerBaskets.TryGetValue(player.userID, out var basket))
				return;
			if (!basket.BasketItems.TryGetValue(int.Parse(basketId), out var item))
				return;
			

			container.Add(new CuiElement
			{
				Name = Layer + ".main.div" + ".store" + ".itemname",
				DestroyUi = Layer + ".main.div" + ".store" + ".itemname",
				Parent = Layer + ".main.div" + ".store",
				Components = {
					new CuiTextComponent { Text = item.Name + (item.Amount <= 1 || item.IsCommand ? "" : $" x{item.Amount}"), Font = "robotocondensed-regular.ttf", FontSize = 16, Align = TextAnchor.MiddleLeft, Color = "1 1 1 1" },
					new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-303.25 -192.104", OffsetMax = "-76.49 -150.576" }
				}
			});
			// container.Add(new CuiPanel()
			// {
			// 	Image = { Sprite = "assets/icons/square.png", Color = "0.9490196 0.5019608 0.05490196 1" },
			// 	RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1", OffsetMin = "-11 -11", OffsetMax = "11 11"}
			// }, Layer + ".main.div" + ".store" + ".items.div" + $".{basketId}", Layer + ".main.div" + ".store" + ".selectedItem", Layer + ".main.div" + ".store" + ".selectedItem");
			container.Add(new CuiButton
			{
				Button = { Color = "0.247 0.933 0.0 0.26", Command = $"UI_GameStoresRUST take {basketId} {basketId}" },
				Text = { Text = "ЗАБРАТЬ", Font = "robotocondensed-bold.ttf", FontSize = 15, Align = TextAnchor.MiddleCenter, Color = "1 1 1 0.8" },
				RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-67.79 -186.1", OffsetMax = "67.79 -156.573" }
			}, Layer + ".main.div" + ".store", Layer + ".main.div" + ".store" + ".take.btn", Layer + ".main.div" + ".store" + ".take.btn");
			CuiHelper.AddUi(player, container);
		}
			...
			if (!CanRequestGetItem(Convert.ToInt32(item.basketId)))
			{
				...
				secondContainer.Add(new CuiButton
				{
					...
					Text =
					{
						...
						Font = "robotocondensed-bold.ttf", Color = "1 1 1 0.4", FontSize = 14
					}
				}, StoreLayer + ".BlockPanel." + index, StoreLayer + ".BlockPanel." + index + ".Open");
			}
		}
		...
		private void CreateStoreUiContainer(ref CuiElementContainer container, BasePlayer player)
		{
			container.Add(new CuiPanel
			{
				...
				RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-314.691 -229.228", OffsetMax = "314.689 176.992" }
			}, Layer + ".main.div", Layer + ".main.div" + ".store");
			container.Add(new CuiElement
			{
				Name = Layer + ".main.div" + ".label",
				Parent = Layer + ".main.div" + ".store",
				Components = {
					new CuiTextComponent { Text = "КОРЗИНА", Font = "robotocondensed-bold.ttf", FontSize = 27, Align = TextAnchor.MiddleLeft, Color = "1 1 1 1" },
					new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-303.556 203.112", OffsetMax = "40.556 255.344" }
				}
			});
		...
		private void HandleBasketUnavailable(BasePlayer player)
		{
			UpdateUI(player, secondContainer =>
			{
				...
				}, Layer + ".main.div" + ".store", StoreLayer + ".BlockPanel.Text", StoreLayer + ".BlockPanel.Text");
			});
		}
				...
				if ((firstInfo.ContainsKey("playerNotFound") && Convert.ToBoolean(firstInfo["playerNotFound"])) ||
					...
					UpdateUI(player, secondContainer =>
					{
						secondContainer.Add(new CuiLabel
							...
							}, Layer + ".main.div" + ".store", StoreLayer + ".BlockPanel.Text",
						...
						container.Add(new CuiButton
							{
								...
								Button =
								{
									...
									Close = Layer + ".main.div" + ".store" + ".items.div" + $".{basketID}" + ".Open",
									Command = $"mb.store.selectitem {basketID}"
								},
								Text =
								{
									Text = Msg(player, MsgTakeGiveError), Align = TextAnchor.MiddleCenter,
									Font = "robotocondensed-bold.ttf", Color = "1 0.7 0.7 1", FontSize = 14
								}
							}, Layer + ".main.div" + ".store" + ".items.div" + $".{basketID}",
							Layer + ".main.div" + ".store" + ".items.div" + $".{basketID}" + ".Open",
							Layer + ".main.div" + ".store" + ".items.div" + $".{basketID}" + ".Open");
					});
				}
					...
					container.Add(new CuiButton
						{
							...
							Button =
							{
								Color = "1 1 1 0.2", Close = Layer + ".main.div" + ".store" + ".items.div" + $".{basketID}" + ".Open",
								Command = $"mb.store.selectitem {basketID}"
							},
							Text =
							{
						...
						container.Add(new CuiButton
							{
								...
								Text =
								{
									...
									Font = "robotocondensed-bold.ttf", Color = "0.7 1 0.7 1", FontSize = 14
								}
							}, Layer + ".main.div" + ".store" + ".items.div" + $".{basketID}", Layer + ".main.div" + ".store" + ".items.div" + $".{basketID}" + ".Open",
							Layer + ".main.div" + ".store" + ".items.div" + $".{basketID}" + ".Open");
					});
		...
		private void FetchShopUrl()
		{
			...
			Request($"store.pluginInfo?store_id={_config.API.ShopID}&server_id={_config.API.ServerID}", null, HandleFetchShopUrlResponse);
		}

		#region Networking

		private const int timeout = 3; // 3 seconds
			...
			BasePlayer player = null)
		{
			...
			_activeCoroutines.Add(ServerMgr.Instance.StartCoroutine(WebPostRequestAsync(BestApiLink + method, args,
				(code, response) =>
				{
		...
		{
			_activeCoroutines.Add(ServerMgr.Instance.StartCoroutine(WebPostRequestAsync(url + method, null,
		...
		private void HandlePlayerBalanceChangeSuccess(ulong steamId, string response, Action<bool, string> callback)
		{
			try
			{
				...
				switch (result)
				{
					case "success":
					{
						...
						if (int.TryParse(balance, out var balanceInt))
						{
							callback?.Invoke(true, balanceInt.ToString());
							LogAction(null, $"Баланс игрока {steamId} изменен: {balanceInt}");
						}
						break;
					}
					case "fail" when message.Contains("Игрок не найден") &&
					                 json["playerNotFound"]?.ToString() == "true":
			...
			{
				LogAction(null,
					$"Ошибка выполнения запроса для изменения баланса игрока {steamId}: ошибка при сериализации ответа: {ex.Message}");
			}
		}
		...
		private void HandlePlayerBalanceGetSuccess(ulong steamId, string response, Action<bool, string> callback)
		{
			try
			{
				...
				switch (result)
				{
					...
					{
						var balance = json["data"]?["balance"]?.ToString();
						if (balance != null && int.TryParse(balance, out var balanceInt))
						{
							callback?.Invoke(true, balanceInt.ToString());

							LogAction(null, $"Баланс игрока {steamId} получен: {balanceInt}");
						}
					...
					case "fail" when message.Contains("Игрок не найден") &&
					                 json["playerNotFound"]?.ToString() == "true":
						callback?.Invoke(false, "player_not_found");

						LogAction(null,
							$"Ошибка выполнения запроса для получения баланса игрока {steamId}: Игрок не найден");
						break;
					default:
						callback?.Invoke(false, "request_error");

						LogAction(null,
							$"Ошибка выполнения запроса для получения баланса игрока {steamId}. Неизвестная ошибка при сериализации ответа: {response}");
						break;
				}
			}
			catch (JsonException ex)
			{
				LogAction(null,
					$"Ошибка выполнения запроса для получения баланса игрока {steamId}: ошибка при сериализации ответа: {ex.Message}");
			}
		}
