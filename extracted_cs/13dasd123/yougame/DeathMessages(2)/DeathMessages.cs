...
{
    [Info("DeathMessages", "RustPlugin.ru", "2.2.3")]
    class DeathMessages : RustPlugin
    {
        ...
        class PluginConfig
        {
            ...
            public Dictionary<string, ColorsPrivilage> ColorPrivilage { get; set; }
        ...
        class Attacker
        {
            ...
            private AttackerType InitializeType()
            {
                ...
                if (Entity.GetComponent<NPCPlayer>() != null)
                    return AttackerType.NPC;
        ...
        protected override void LoadDefaultConfig()
        {
            ...
            Config.WriteObject(new PluginConfig
            {
                ...
                Weapons = new Dictionary<string, string>
                {
                    ...
                    {"Crossbow", "Арбалет"},
                    {"Compound Bow", "Композитный лук"},
                    {"Eoka Pistol", "Самодельный пистолет"},
                    {"Nailgun", "Гвоздомет"},
                    {"Revolver", "Револьвер"},
                    {"Semi-Automatic Pistol", "Полуавтоматический пистолет"},
                    {"Python Revolver", "Револьвер питон"},
                    {"M92 Pistol", "Беретта"},
                    {"Waterpipe Shotgun", "Самодельный дробовик"},
                    {"Bolt Action Rifle", "Винтовка"},
                    {"Pump Shotgun", "Помповый дробовик"},
                    {"Thompson", "Пистолет-пулемёт Томпсона"},
                    {"M39 Rifle", "Винтовка М39"},
                    {"Assault Rifle", "Автомат Калашникова"},
                    {"Custom SMG", "Самодельный пистолет-пулемёт"},
                    {"MP5A4", "MP5A4"},
                    {"M249", "Пулемет М249"},
                    {"Semi-Automatic Rifle", "Полуавтоматическая винтовка"},
                    {"Flame Thrower", "Огнемет"},
                    {"Double Barrel Shotgun", "Двухствольный дробовик"},
                    {"LR-300 Assault Rifle", "LR-300 штурмовая винтовка"},
                    {"L96 Rifle", "Винтовка L96"},
                    ...
                    {"Snowball", "Снежок"},
                    {"Candy Cane Club", "Леденец-дубинка"},
					{"Birthday Cake", "Праздничный торт"},
                    {"Timed Explosive Charge", "Взрывчатка с таймером С4"},
                    {"F1 Grenade", "Граната Ф1"},
                    {"Beancan Grenade)", "Бобовая граната"},
                    {"40mm Shotgun Round", "40мм гранатомёт"},
                ...
                Turrets = new Dictionary<string, string>
                {
                    ...
                    {"Static Turret", "Автоматическая турель"},
					{"Sentry Turret", "Турель безопасной зоны"},
					{"SAM Site", "Турель ПВО"}
                },
        ...
        private void OnEntityDeath(BaseCombatEntity victim, HitInfo info)
        {
            try
            {
                ...
                if ((_victim.Type == VictimType.Zombie && _attacker.Type == AttackerType.NPC))
                    return;
                if (!_config.ShowDeathAnimals && _victim.Type == VictimType.Animal) return;
                if (!_config.ShowDeathAnimals && _attacker.Type == AttackerType.Animal) return;
                if (!_config.ShowDeathNPC && _victim.Type == VictimType.NPC)
                    return;
                if (!_config.ShowDeathNPC && _attacker.Type == AttackerType.NPC)
                    return;
