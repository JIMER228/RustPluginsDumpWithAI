                ...
                },
{
    ...
    private Item BuildItem(int itemid, int amount, ulong skin)
        ...
        void CreateKitEntry(BasePlayer player, ref CuiElementContainer element, string panel, float[] pos, string entry)
        {
            ...
            UI.LoadImage(ref element, PanelKits, image, $"{pos[2] - 0.18f} {pos[1] + 0.09f}", $"{pos[2] - .010f} {pos[1] + .365f}");
            UI.CreateLabel(ref element, PanelKits, UIColors["white"], kit.name, 12, $"{pos[0] + .005f} {pos[3] - .034f}", $"{pos[2] - .005f} {pos[3] - .01f}");
            UI.CreateLabel(ref element, PanelKits, UIColors["white"], kit.description ?? string.Empty, 12, $"{pos[0] + .5f} {pos[3] - .18f}", $"{pos[2] - .005f} {pos[3] - .07f}", TextAnchor.UpperLeft);
            UI.CreateLabel(ref element, PanelKits, UIColors["white"], string.Format(GetMsg("KitCooldown", player.userID), kit.cooldown <= 0 ? GetMsg("None", player.userID) : CurrentTime() > kitData.cooldown ? MinuteFormat(kit.cooldown / 60).ToString() : "<color=red> -" + MinuteFormat((double)Math.Abs(Math.Ceiling(CurrentTime() - kitData.cooldown)) / 60) + "</color>"), 12, $"{pos[0] + .005f} {pos[3] - .46f}", $"{pos[2] - .005f} {pos[3] - .21f}", TextAnchor.MiddleLeft);
            UI.CreateLabel(ref element, PanelKits, UIColors["white"], string.Format(GetMsg("KitUses", player.userID), kit.max <= 0 ? GetMsg("Unlimited") : kit.max - kitData.max < kit.max ? $"<color=red>{kit.max - kitData.max }</color>/{kit.max}" : $"{kit.max - kitData.max }/{kit.max}"), 12, $"{pos[0] + .005f} {pos[3] - .392f}", $"{pos[2] - .005f} {pos[3] - .24f}", TextAnchor.MiddleLeft);
            if (player.net.connection.authLevel == 2 || permission.UserHasPermission(player.UserIDString, this.Title + ".admin"))
                UI.CreateButton(ref element, PanelKits, UIColors["buttonred"], GetMsg("RemoveKit", player.userID), 14, $"{pos[0] + .12f} {pos[1] + 0.365f}", $"{pos[0] + .19f} {pos[1] + .4f}", $"UI_RemoveKit {entry.ToLower()}");
            if (!ShowUnavailableKits)
                UI.CreateButton(ref element, PanelKits, UIColors["green"], GetMsg("Redeem"), 12, $"{pos[0] + .005f} {pos[1] + 0.01f}", $"{pos[0] + .06f} {pos[1] + .07f}", $"kit.gui {entry.ToLower()}");
            else
            {
                string reason;
                ...
                {
                    UI.CreatePanel(ref element, PanelKits, UIColors["red"], $"{pos[0] + .005f} {pos[1] + 0.01f}", $"{pos[0] + .185f} {pos[1] + .05f}");
                    UI.CreateLabel(ref element, PanelKits, UIColors["red"],"<color=black>"+GetMsg("Unavailable", player.userID)+"</color>", 12, $"{pos[0] + .005f} {pos[1] + 0.01f}", $"{pos[0] + .185f} {pos[1] + .05f}");
                }
                else
                    UI.CreateButton(ref element, PanelKits, UIColors["buttongreen"], GetMsg("Redeem"), 12, $"{pos[0] + .005f} {pos[1] + 0.01f}", $"{pos[0] + .185f} {pos[1] + .05f}", $"kit.gui {entry.ToLower()}");
            }
        }
