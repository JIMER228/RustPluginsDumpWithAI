...
{
    [Info("Auto Chat Mute", "AlexOne", "1.0")]
    [Description("Плагин дает мут игрокам за нарушение правил чата")]
    internal class AutoChatMute : CovalencePlugin
    {
        ...
        private void Loaded()
        {
            permission.RegisterPermission("autochatmute.permanent", this);
            ...
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                ["Muted"] = "{player} получил мут {initiator}.",
                ["Muted Time"] = "Игроку <color=#F27660>{player}</color> отключен чат!\nДлительность: <color=#F27660>{time}</color>\nПричина: <color=#F27660>Нецензурная лексика!</color>",
                ["Unmuted"] = "{player} разблокирован {initiator}.",
                ["Not Muted"] = "{player} в настоящее время нет мута",
                ["Mute Expired"] = "Блокировка чата снята с игрока <color=#F27660>{player}</color>",
                ["Invalid Time Format"] = "Неверный Формат времени. Пример: 1d2h3m4s = 1 день, 2 часа, 3 минут(ы), 4 секунд(ы)",
                ["Nobody Muted"] = "Никто не в муте сейчас.",
                ["Invalid Syntax Mute"] = "/mute <player|steamid> [время: 1d1h1m1s]",
                ["Invalid Syntax Unmute"] = "/unmute <player|steamid>",
                ["Player Name Not Found"] = "Игрок с именем '{name}' не найден",
                ["Player ID Not Found"] = "Игрока не найден с ID '{id}'",
                ["Multiple Players Found"] = "Multiple matching players found: \n{matches}",
                ["Time Muted Player Joined"] = "{player} временно в муте. До окончания: {time}",
                ["Time Muted Player Chat"] = "<color=#FF0000>Чат для вас заблокирован!</color>\nДо окончания: <color=#FF0000>{time}</color>",
                ["Muted Player Joined"] = "{player} в муте навсегда.",
                ["Muted Player Chat"] = "Вы больше не можете общаться, вы получили перманентний мут.",
            ...
            timer.Repeat(10, 0, () =>
            {
                ...
                foreach (string id in expired)
                {
                    ...
                    Interface.CallHook("OnAutoChatMuteExpired", player);
        ...
        private void API_Mute(IPlayer target, IPlayer player, bool callHook = true, bool broadcast = true)
        {
            ...
            if (callHook)
                Interface.CallHook("OnAutoChatMuted", target, player);
        ...
        private bool API_Unmute(IPlayer target, IPlayer player, bool callHook = true, bool broadcast = true)
        {
            ...
            if (callHook)
                Interface.CallHook("OnAutoChatUnmuted", target, player);
        ...
        [Command("toggleglobalmute"), Permission("autochatmute.use.global")]
        ...
        [Command("mutelist"), Permission("autochatmute.use")]
        ...
        [Command("mute"), Permission("autochatmute.use")]
        private void CmdMute(IPlayer player, string cmd, string[] args)
        {
            ...
            {
                case 1:
                    if (!permission.UserHasPermission(player.Id, "autochatmute.permanent") && player.Id != "server_console")
                    {
                ...
                case 2:
                    ...
                    Interface.CallHook("OnAutoChatTimeMuted", target, player, timeSpan);
        ...
        [Command("unmute"), Permission("autochatmute.use")]
        ...
        private object HandleChat(IPlayer player)
        {
            ...
            var result = Interface.CallHook("OnAutoChatMuteHandle", player, MuteInfo.IsMuted(player) ? JObject.FromObject(mutes[player.Id]) : null);
            ...
            if (globalMute && !permission.UserHasPermission(player.Id, "autochatmute.use.global"))
            {
