...
namespace Oxide.Plugins
{
    ...
    public partial class WarMode : CovalencePlugin
    {
        ...
        public static void DebugAction(string action, BaseEntity initiator, BaseEntity target, HitInfo info = null, TargetType? ttoverride = null)
        {
            if (!DEBUGGING) { return; }
            if (!(initiator is BasePlayer)) { return; }
            var basePlayer = initiator as BasePlayer;
            var privInfo = target.GetPrivInfo();
            var tt = ttoverride ?? target.GetTargetType();
            Debug($"{basePlayer.displayName} {action} {initiator.GetTargetType()}({initiator.GetMode().Name()}) -> {tt}({target.GetMode().Name()}){(privInfo.NotExists ? "" : $"(base is {privInfo.GetMode().Name()})")}");
        }
        ...
        public void SubscribeAll()
        {
            foreach (var method in _subscriptions)
            {
                switch(method)
                {
                    // Methods that are dynamically subscribed can go here
                    default:
                        Subscribe(method);
                        break;
                }
        ...
        public void Notify(Plugin plugin, BasePlayer basePlayer, string loc, bool positive, float cooldown, params object[] args)
        {
            ...
            if (config.Notifications.Chat)
            {
                ...
                {
                    chatText = $"<color={config.Chat.PluginPrefixColor}>[WarMode]</color> " + localizedText;
                }
...
namespace Oxide.Plugins
{
    public partial class WarMode
    {
        ...
        */
        ...
        private string GetEntityMode(BaseEntity entity)
        {
            return entity?.GetMode()?.Name ?? string.Empty;
        }

        // Returns the group name for the given mode. For example if given 'pve' will return 'warmodepve'
        private string GetModeGroup(string modeId)
        {
            return Modes.GetValueOrDefault(modeId)?.Group ?? string.Empty;
        }
        ...
        // return 'horses'.
        private string GetEntityTargetType(BaseEntity entity)
        {
            return entity?.GetTargetType().ToString() ?? string.Empty;
        }
    }
}
    ...
    public partial class WarMode
    {
        ...
        private void CmdWarmode(IPlayer player, string command, string[] args)
        {
            var commands = new[]
            {
                ...
                {
                    Enabled = true,
                    Commands = new string[]  { "warmode.getmode" },
                    Args = new string[] { },
                    Description = "Display the mode for the entity or player you are looking at. Does not work for every entity."
                }
            };
            ...
            public ModeConfig[] Modes = new ModeConfig[]
{
        ...
        public class ChatConfig
        {
            ...
            [ConfigDescription("Only matters if ShowPluginPrefix is true.")]
            public string PluginPrefixColor = "#aaee32";
        ...
        public class FlaggingConfig
        {
            ...
            [ConfigDescription("The amount of seconds before a player can use the flag command again. Set to 0 for no cooldown.")]
        ...
        public class ModeConfig
        {
            ...
            [ConfigDescription("If true then the fire damage over time effect will always deal damage to this player and anything they own. This will always allow fire damage regardless of what mode caused the fire.")]
            public bool AlwaysAllowFireDamage = false;

            [ConfigDescription("If true then patrol helicopters will be able to damage players and bases of this mode. If false, then they will be protected from the damage of these helicopters.")]
            public bool CanTakePatrolHeliDamage = true;
...
namespace Oxide.Plugins.WarModeMethods
{
    public static partial class ExtensionMethods
    {
        ...
        public static WarMode.ModeConfig GetMode(this BaseEntity entity)
        {
            ...
            var isplayer = entity is BasePlayer;
            if (parent != null && !isplayer && parent is BaseVehicle)
            {
                entity = parent;
            }
        ...
        public static WarMode.PrivInfo GetPrivInfo(this BaseEntity baseEntity)
        {
            ...
            if (parent is Tugboat) { return new WarMode.PrivInfo(parent); }
            // ToolCupboard
        ...
        public static bool IsValidVehicle(this BaseEntity entity)
        {
            return entity is BaseVehicle && !(entity is TrainCar);
        }
    }
}
    ...
    public partial class WarMode
    {
        ...
        protected override void LoadDefaultMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                ["cannot loot publiccontainers"] = "Your mode does not have permission to public containers within bases owned by {0} players.",
                ["cannot loot tugboats"] = "Your mode does not have permission to loot tugboats owned by {0} players.",
...
namespace Oxide.Plugins
{
    public partial class WarMode
    {
        public bool? _markerExists = null;

        public bool MarkerExists
        {
            get
            {
                if (_markerExists == null)
                {
                    _markerExists = config.Marker.Show && config.Modes.Any(x => x.ShowMarkerWhenAimedAt);
                }
                return _markerExists.Value;
            }
        }
...
namespace Oxide.Plugins
{
    public partial class WarMode
    {
        ...
        {
            if (Interface.CallHook("CanEntityTakeDamage", target, info) is bool val)
            {
                Debug($"OVERRIDE BY PLUGIN: VALUE={val}");
                if (!val)
                {
            ...
            try
            {
                ...
                var isPatrolHeliAttack = (initiator is PatrolHelicopter || (info.WeaponPrefab != null && (info.WeaponPrefab.name == "rocket_heli" || info.WeaponPrefab.name == "rocket_heli_napalm")));

                if (initiator == null && !isPatrolHeliAttack) { return; }
                
                // Patrol Heli Attack
                if (isPatrolHeliAttack)
                {
                    if ((target.GetMode()?.CanTakePatrolHeliDamage ?? true) == false)
                    {
                        info.PreventAllDamage();
                    }
                    return;
                ...
                {
                    // AUTO TURRET -> PLAYER
                    if (initiator is AutoTurret turret)
                    {
                        if (turret.IsBeingControlled)
                        {
                            var controllerId = turret.ControllingViewerId?.SteamId;
                            if (controllerId != null)
                            {
                                var attackerPlayer = FindBasePlayer(controllerId.Value);
                    ...
                    // NPC -> PLAYER
                    else if (initiator is NPCPlayer attackerNpc)
                    {
        ...
        private void HandleFireDamage(BaseCombatEntity target, HitInfo info, bool requireBuildingPriv = true)
        {
            ...
            if (target?.GetMode()?.AlwaysAllowFireDamage == false)
            {
                info.PreventAllDamage();
                target.PreventAttackLog();
            }
        ...
        {
            return CanLootPlayerEntity(looter, target, target.UserIDString, 1, hook: "CanLootPlayer");
        }
        ...
        private bool CanPickupDroppedBackpack(DroppedItem droppedItem, BasePlayer looter)
        {
            ...
            if (Interface.CallHook("CanEntityLoot", looter, droppedItem, false) is bool val)
            {
                Debug($"OVERRIDE BY PLUGIN: VALUE={val}");
                return val;
            }
            ...
            // Backpack is in forced Zone
            if (IsUsingZoneManager && droppedItem != null)
            {
                var zoneids = GetEntityZoneIDs(droppedItem);
                var forcedMode = GetForcedModeForZones(zoneids);
                if (forcedMode != null)
                {
                    if (!looter.GetMode().CanLootMode(forcedMode, TargetType.droppedbackpacks))
                    {
        ...
        public object CanLootPlayerEntity(BasePlayer looter, BasePlayer target, string targetUserid, int from, BaseCombatEntity container = null, string hook = null)
        {
            ...
            //Try Override
            if (Interface.CallHook("CanEntityLoot", looter, target, false) is bool val)
            {
                Debug($"OVERRIDE BY PLUGIN: VALUE={val}");
                if (hook == "CanLootPlayer")
                {
                    return val;
                }
                else
                {
                    return val ? (object)null : true; // Any non-null will prevent
                }
            }
        ...
        {
            // Try Override
            if (Interface.CallHook("CanEntityLoot", looter, target, skipVendingCheck) is bool val)
            {
                Debug($"OVERRIDE BY PLUGIN: VALUE={val}");
                return val ? (object)null : true; // Any non-null will prevent
            }
            ...
            if (looter.HasBypass()) { return null; }
            var isPublic = false;
            if (target is DropBox dropBox)
            {
                isPublic = dropBox.PlayerInfront(looter);
            }
            else if (target is Mailbox mailbox)
            {
                isPublic = true;
            }
            else if (target is VendingMachine vendingMachine)
            {
                if (skipVendingCheck) { return null; }
                isPublic = !vendingMachine.CanPlayerAdmin(looter);
            }
            var ttype = target.GetTargetType(isPublic: isPublic);
            if (!TargetTypesForCategory.Looting.Contains(ttype)) { return null; } // Not a tracked looting ttype, its allowed
            DebugAction($"LOOTING", looter, target, ttoverride: ttype);
        ...
        public const float CanBeTargetedCacheTimeSeconds = 10f;
        public struct CanBeTargetedResult
        {
            ...
            public CanBeTargetedResult(bool? canTarget)
            {
                IsNull = canTarget == null;
                CanTarget = canTarget.HasValue ? canTarget.Value : false;
                ExpireTime = UnityEngine.Time.realtimeSinceStartup + CanBeTargetedCacheTimeSeconds;
            }
        }
        

        Dictionary<string, CanBeTargetedResult> CachedCanBeTargeted = new Dictionary<string, CanBeTargetedResult>(); // Key is entityNetId|userIdString
        ...
        object CanBeTargeted(BasePlayer targetPlayer, MonoBehaviour behaviour)
        {
            ...
            // Try to use a cached result for better performance
            var cpk = CompoundKey(turret.net.ID.Value, targetPlayer.UserIDString);
            if (CachedCanBeTargeted.ContainsKey(cpk))
            {
                var cached = CachedCanBeTargeted.GetValueOrDefault(cpk);
                if (UnityEngine.Time.realtimeSinceStartup >= cached.ExpireTime)
                {
                    CachedCanBeTargeted.Remove(cpk);
                }
                return cached.IsNull ? (bool?)null : cached.CanTarget;
            }

            // Try Override
            if (Interface.CallHook("CanEntityBeTargeted", targetPlayer, turret) is bool val)
            {
                Debug($"OVERRIDE BY PLUGIN: VALUE={val}");
                CachedCanBeTargeted[cpk] = new CanBeTargetedResult(val);
                return val;
            }
            ...
            if (isNpcOwnedTurret)
            {
                ...
                {
                    CachedCanBeTargeted[cpk] = new CanBeTargetedResult(false);
                    return false;
                }
            }
            ...
            CachedCanBeTargeted[cpk] = new CanBeTargetedResult(null);
            return null;
        }

        Dictionary<string, CanBeTargetedResult> CachedOnSamSiteTarget = new Dictionary<string, CanBeTargetedResult>(); // Key is entityNetId|targetNetId
        object OnSamSiteTarget(SamSite samSite, BaseCombatEntity target)
        {
            if (samSite == null || target == null) { return null; }

            // Try to use a cached result for better performance
            var cpk = CompoundKey(samSite.net.ID.Value, target.net.ID.Value);
            if (CachedOnSamSiteTarget.ContainsKey(cpk))
            {
                var cached = CachedOnSamSiteTarget.GetValueOrDefault(cpk);
                if (UnityEngine.Time.realtimeSinceStartup >= cached.ExpireTime)
                {
                    CachedOnSamSiteTarget.Remove(cpk);
                }
                return cached.IsNull ? (bool?)null : cached.CanTarget;
            }

            // Try Override
            if (Interface.CallHook("CanEntityBeTargeted", target, samSite) is bool val)
            {
                Debug($"OVERRIDE BY PLUGIN: VALUE={val}");
                if (val)
                {
                    CachedOnSamSiteTarget[cpk] = new CanBeTargetedResult(null);
                    return null; 
                }
                CachedOnSamSiteTarget[cpk] = new CanBeTargetedResult(true);
                samSite.CancelInvoke(samSite.WeaponTick);
                return true; // Any non-null will prevent
            }
            ...
            if (isNpcOwnedSamSite)
            {
                ...
                if (target is BaseVehicle targetVehicle)
                {
                    ...
                    {
                        CachedOnSamSiteTarget[cpk] = new CanBeTargetedResult(false);
                        return false;
                    }
                }
            }
        ...
        object OnTrapTrigger(BaseTrap trap, GameObject go)
        {
            ...
            // Try Override
            if (Interface.CallHook("CanEntityTrapTrigger", trap, target) is bool val)
            {
                Debug($"OVERRIDE BY PLUGIN: VALUE={val}");
                return val ? (object)null : true; // Any non-null will prevent
            }
            ...
            if (isNpcOwnedTrap)
            {
                ...
                {
                    if (!NpcMode.CanTargetMode(targetPlayer.GetMode(), trap.GetTargetType()))
                    {
                        return false;
                    }
                }
                ...
                {
                    if (!NpcMode.CanTargetMode(targetVehicle.GetMode(), trap.GetTargetType()))
                    {
                        return false;
                    }
                }
            ...
            else
            {
                ...
                if (target is BasePlayer targetPlayer)
                {
                    ...
                    if (trapMode != null && !trapMode.CanTargetMode(targetPlayer.GetMode(), trap.GetTargetType()))
                    {
                        return false;
                    }
                }
                ...
                {
                    ModeConfig trapMode = priv?.GetMode() ?? (trap.OwnerID != 0 ? trap.OwnerID.GetMode() : null);
                    if (trapMode != null && !trapMode.CanTargetMode(targetVehicle.GetMode(), trap.GetTargetType()))
                    {
                        return false;
                    }
                }
        ...
        {
            // Try Override
            if (Interface.CallHook("CanEntityMount", mounter, target) is bool val)
            {
                Debug($"OVERRIDE BY PLUGIN: VALUE={val}");
                return val ? (object)null : true; // Any non-null will prevent
            }
        ...
        {
            if (basePlayer == null || !basePlayer.IsRealPlayer()) { return; }
...
namespace Oxide.Plugins.WarModeMethods
{
    public static partial class ExtensionMethods
    {
        public static WarMode.TargetType GetTargetType(this BaseEntity entity, bool isPublic = false)
        {
            // ADD TARGET TYPE
            ...
            if (WarMode.CONFIG.Settings.AllowVehicleModeOwnership && ((entity.IsValidVehicle() && entity.OwnerID != 0) || (parent.IsValidVehicle() && parent.OwnerID != 0)))
            {
                return WarMode.TargetType.claimedvehicles;
            }
            if (entity.IsValidVehicle() || parent.IsValidVehicle())
            {
            ...
            if (entity is BuildingBlock || entity is StabilityEntity || entity is Signage || entity is DecorDeployable || entity is IOEntity)
            {
                return WarMode.TargetType.buildings;
            }
            if (isPublic && (entity is DropBox || entity is Mailbox || entity is VendingMachine))
            {
                return parent?.GetTargetType() ?? WarMode.TargetType.publiccontainers;
            }
