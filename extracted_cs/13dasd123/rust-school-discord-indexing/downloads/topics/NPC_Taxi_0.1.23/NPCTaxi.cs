    ...
    class NPCTaxi : RustPlugin, IPathFinder
    {
        ...
        private static void SendMapMarker(BasePlayer player, Vector3 position, string text)
        {
            if (player.State.pointsOfInterest == null)
                player.State.pointsOfInterest = Pool.Get<List<MapNote>>();
        ...
        private static bool ContainsTopology(int topology, int mask) => (topology & mask) != 0;
        #endregion
        ...
        public void EvaluateNode(GridNode gridNode, Vector3 position)
        {
            ...
            float waterDepth = WaterLevel.GetWaterDepth(position, false, false);
        ...
        private class VehicleAI : MonoBehaviour
        {
            ...
            private void OnCollisionEnter(Collision collision)
            {
                ...
                if (resourceEntity != null)
                    resourceEntity.OnDied(new HitInfo(Entity, resourceEntity, Rust.DamageType.Collision, 1000f));
            }
            ...
            private void OnDestroy()
            {
                ...
                if (shouldDestroyVehicle)
                {
                    ...
                    {
                        StorageContainer fuelContainer = (Entity.GetFuelSystem() as EntityFuelSystem).GetFuelContainer();
            ...
            private void Update()
            {
                ...
                if (State is TaxiState.WaitingForInput or TaxiState.WaitingForExit)
                {
            ...
            internal void OnMarkerPlaced(Vector3 position, MapNote mapNote)
            {
                ...
                FareCost = 0;
                PathFinder.EvaluateGrid(new PathRequest(_tr.position, position, Caller, Pool.Get<List<Vector3>>(), OnPathGenerated, OnPathFailed));                
            }
            ...
            private class Sensor
            {
                ...
                internal bool IsOverWater()
                {
                    float waterDepth = WaterLevel.GetWaterDepth(_sensorEnd, false, false);
                    if (waterDepth > 0.7f)
                        return true;

                    const int WATER_LAYERS = (int)(TerrainTopology.Enum.Ocean | TerrainTopology.Enum.River | TerrainTopology.Enum.Lake);
                    const int ROAD_LAYER = (int)TerrainTopology.Enum.Road;
                    
                    int topology = TerrainMeta.TopologyMap.GetTopology(_sensorEnd);
                    
                    if (ContainsTopology(WATER_LAYERS, topology) && !ContainsTopology(ROAD_LAYER, topology))
                        return true;
        ...
        private void RequestTaxi(BasePlayer player, bool isTelephone)
        {
            ...
            if (Cooldowns.TryGetValue(player.userID, out float cooldown))
            {
            ...
            PathFinder.EvaluateGrid(new PathRequest(spawnPoint, closestNode, player, Pool.Get<List<Vector3>>(), OnPathGenerated, OnPathFailed));
        }
        ...
        private void CellDebug(BasePlayer player, string command, string[] args)
        {
            ...
            List<GridNode> list = Pool.Get<List<GridNode>>();
