...
{
    [Info("Personal Builder", "walkinrey & Max39ru", "1.0.8")] 
    public class PersonalBuilder : RustPlugin
    {
        ...
        private Dictionary<ulong, PlayerBuilderController> _controllersByBot = new Dictionary<ulong, PlayerBuilderController>();//nullHit.HitPositionWorld, 1f, 97185f);
        ...
        public void DropLoot(BasePlayer player, PlayerBuilderController controller, HitInfo info) 
        {
            ...
            droppedContainer.TakeFrom(containers.ToArray());
        ...
        public class PlayerBuilderController : FacepunchBehaviour
        {
            ...
            private List<Dictionary<string, object>> foundationsObjects = new List<Dictionary<string, object>>(), floorObjects = new List<Dictionary<string, object>>(), rampObjects = new List<Dictionary<string, object>>(),
                roofObjects = new List<Dictionary<string, object>>(), stairsObjects = new List<Dictionary<string, object>>(), wallObjects = new List<Dictionary<string, object>>(), doorObjects = new List<Dictionary<string, object>>(), cupObjects = new List<Dictionary<string, object>>();
            ...
            private IEnumerator InitializationFile()
            {
                ...
                foreach (Dictionary<string, object> data in ents)
                {
                    ...
                    BuildingGrade.Enum? grade = !data.ContainsKey("grade") ? (BuildingGrade.Enum?)null : (BuildingGrade.Enum)Convert.ToInt32(data["grade"]);
            ...
            private IEnumerator SpawnObjects(List<Dictionary<string, object>> objects, bool isBuildBlock = false)
            {
                foreach (var data in objects)
                {
                    ...
                    BuildingGrade.Enum? grade = isBuildBlock && data.ContainsKey("grade") ? (BuildingGrade.Enum?)Convert.ToInt32(data["grade"]) : (BuildingGrade.Enum?)null;
