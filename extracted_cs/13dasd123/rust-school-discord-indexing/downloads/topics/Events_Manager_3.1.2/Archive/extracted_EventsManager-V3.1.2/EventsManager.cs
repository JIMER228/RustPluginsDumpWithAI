...
{
    [Info("EventsManager", "Mevent & Qbis", "3.1.2")]
    internal class EventsManager : RustPlugin
    {
        ...
        private void UpdateConfigValues()
        {
            ...
            if (config.Version < new VersionNumber(3, 0, 0))
            {
        ...
        {
            if (!config.DisableVanillaAutoEvents) return;
            ...
            Puts("Vanilla events are disabled");
        }

        private void EnableVanillaEvents()
        {
            if (config.DisableVanillaAutoEvents)
        ...
        private class EventController : FacepunchBehaviour
        {
            ...
            private void EventTimer()
            {
                ...
                foreach (var @event in instance.data.eventTimes)
                {
                    if (@event.eventsId.IsEmpty() || @event.minPlayers > BasePlayer.activePlayerList.Count) continue;

                    switch (@event.type)
                    {
                        case EventType.Static:
                            {
                                if (!@event.activeDays.IsEmpty() && !@event.activeDays.Contains(DateTime.Now.DayOfWeek)) continue;

                                var startHour = @event.hour + (@event.nextOffset / 60);
                                var startMinute = @event.minute + (@event.nextOffset % 60);
                                if (startMinute >= 60)
                                {
                                    startHour++;
                                    startMinute -= 60;
                                ...
                                if (@event.lastRun.DayOfWeek == day && @event.lastRun.Hour == hour && @event.lastRun.Minute == minute) continue;

                                if (hour == startHour && minute == startMinute)
                                {
                                    var id = @event.eventsId[UnityEngine.Random.Range(0, @event.eventsId.Count)];
                        ...
                        case EventType.Random:
                            {
                                ...
                                var startHour = @event.lastRun.Hour + ((@event.interval + @event.nextOffset) / 60);
                                var startMinute = @event.lastRun.Minute + ((@event.interval + @event.nextOffset) % 60);
                                ...
                                if (@event.lastRun.Hour == hour && @event.lastRun.Minute == minute) continue;
        ...
        #region Commands
        [ChatCommand("em")]
        private void CmdNewEventManager(BasePlayer player)
        {
        ...
        {
            if (!_interface.TryGetValue(player.userID, out var @interface)) return;
            var times = data.eventTimes.FindAll(t => t.eventsId.Count > 1 || t.eventsId.Count == 0);
        ...
        private void DrawRandomTime(BasePlayer player, CuiElementContainer container)
        {
            ...
            container.Add(new CuiElement
            {
                Parent = panelName,
                Components = {
                    new CuiTextComponent { Text = "MIN PLAYERS", Font = "robotocondensed-regular.ttf", FontSize = 14, Align = TextAnchor.UpperLeft, Color = HexToCuiColor("#E2DBD3", 90) },
                    new CuiRectTransformComponent
                    {
                        AnchorMin = "0 1",
                        AnchorMax = "0 1",
        ...
        private void DrawModalTimes(BasePlayer player, CuiElementContainer container)
        {
            ...
            #region Scroll View
            var times = data.eventTimes.FindAll(t => t.eventsId.Count == 1 && t.eventsId.Contains(@interface.editEvent.eventId));
        ...
        private void DrawModalTime(BasePlayer player)
        {
            ...
            container.Add(new CuiElement
            {
                Parent = panelName,
                Components = {
                    new CuiTextComponent { Text = "MIN PLAYERS", Font = "robotocondensed-regular.ttf", FontSize = 10, Align = TextAnchor.UpperLeft, Color = HexToCuiColor("#E2DBD3", 90) },
                    new CuiRectTransformComponent
                    {
                        AnchorMin = "0 1",
                        AnchorMax = "0 1",
        ...
        private void DrawModalCreator(BasePlayer player, CuiElementContainer container, string creatorName)
        {
