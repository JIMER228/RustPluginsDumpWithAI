...
namespace Oxide.Plugins
{
    ...
    class MapVoter : CovalencePlugin
    {
        ...
        {
            [JsonProperty(PropertyName = "Path to server identity folder")]
            public string  ServerIdentityPath { get; set; }
        ...
        private ConfigData GetDefaultSettings()
        {
            ...
            {
                ServerIdentityPath = "/server/my_server_identity",
        ...
        private void Init()
        {
            WipeData = Interface.Oxide.DataFileSystem.ReadObject<AutoWipeConfig>("wipe");
            if (WipeData.IsWipeDay)
            {
                string[] arrLine = File.ReadAllLines(Directory.GetCurrentDirectory() + config.ServerIdentityPath +"/cfg/server.cfg");
                if (!WipeData.iSCustomMap)
                {
                    for (int i = 0; i < arrLine.Length; i++)
                    {
                        if (arrLine[i].Contains("server.seed"))
                            arrLine[i] = "server.seed " + WipeData.MapSeed;
                        if (arrLine[i].Contains("server.worldsize"))
                            arrLine[i] = "server.worldsize " + WipeData.MapSize;
                        if (arrLine[i].Contains("server.levelurl"))
                            arrLine[i] = "//" + arrLine[i];
                    }
                }
                else
                {
                    for (int i = 0; i < arrLine.Length; i++)
                    {
                        ...
                        if (arrLine[i].Contains("server.worldsize"))
                            arrLine[i] = "//" + arrLine[i];
                        if (arrLine[i].Contains("server.levelurl"))
                            arrLine[i] = "server.levelurl " + WipeData.LevelUrl;
                    }
                }
                if (WipeData.FullWipe)
                {
                    System.IO.DirectoryInfo di = new DirectoryInfo(Directory.GetCurrentDirectory() + config.ServerIdentityPath + "/");

                    foreach (FileInfo file in di.GetFiles())
                    {
                        if (file.Extension.Contains("db") || file.Extension.Contains("db-journal") || file.Extension.Contains("sav") || file.FullName.Contains("sav"))
                        {
                            file.Delete();
                        }
                    }
                }
                File.WriteAllLines(Directory.GetCurrentDirectory() + config.ServerIdentityPath + "/cfg/server.cfg", arrLine);
            }
        }
        private void OnServerInitialized()
        {
            ...
            Debug.Log("Next wipe: " + DateTime.ParseExact(config._AutoWipe.WipeTime, "HH:mm", null).AddDays(daysTillNextWipe).ToString() + " current time: " + DateTime.Now);
            GetWipeSchedule();
