...
namespace Oxide.Plugins
{
    ...
    internal class BetterTC : RustPlugin
    {
        ...
        private Dictionary<int, string> WallPrefabs = new Dictionary<int, string>
        {
            ...
            { 10303, "assets/prefabs/building/wall.external.high.legacy/wall.external.high.legacy.prefab" },
        ...
        private Dictionary<int, string> GatePrefabs = new Dictionary<int, string>
        {
            ...
            { 10303, "assets/prefabs/building/gates.external.high.legacy/gates.external.high.legacy.prefab" },
        ...
        private void OnEntityBuilt(Planner plan, GameObject go){  
            ...
            string expectedShortName = GetShortNameForSkin(selectedSkin);
			if (tc.ShortPrefabName != expectedShortName) TCSkinReplace(tc, player, selectedSkin);
		}
        ...
        private string GetShortNameForSkin(TCSkin skin) => skin switch
        {
            TCSkin.Retro => "cupboard.tool.retro.deployed",
            TCSkin.Shockbyte => "cupboard.tool.shockbyte.deployed",
            _ => "cupboard.tool.deployed"
        };
            ...
            if (cup != null){
                ...
                foreach (ProtoBuf.PlayerNameID pnid in cup.authorizedPlayers) {
                    ...
                    if(foundPlayer != null){
                        ...
                        if(config.alertnotify && (Notify != null || UINotify != null)) Interface.Oxide.CallHook("SendNotify", foundPlayer, type, text);//rBody.AddExplosionForce(Mathf.Min(10 * 650f, 137040f);
        ...
        private void ShowMenu(BasePlayer player, BuildingPrivlidge cup, int page = 0){
            ...
            foreach (var list_entry in BuildingItems){
				...
				bool colour = list_entry.color && unlock2;
                if (colour){
                ...
                if (config.reskin){
                    ...
                    UI.Button(ref container, upgrade_3, "0 0 0 0", "", 10, $"0.82 {yMin}", $"0.95 {yMin + 0.13f}", unlock2 ? $"SENDCMD RESKIN {ID} {list_entry.grade} {list_entry.skinid} {page} {list_entry.color}" : $"SENDCMD ERROR");
                    yMin -= yStep;
				}
                ...
                if(config.wallpaper){
                    ...
                    UI.Button(ref container, upgrade_3, "0 0 0 0", "", 10, $"0.82 {yMin}", $"0.95 {yMin + 0.13f}", unlock3 ? $"SENDCMD WALLPAPER {ID} {list_entry.grade} {list_entry.skinid} {page}" : $"SENDCMD ERROR");
                    yMin -= yStep;
                }
                
                if (config.reskinwall && list_entry.wall != -1){ //&& (!config.samewallgrade || list_entry.itemID2 != -335089230)
                    ...
                    UI.Button(ref container, upgrade_3, "0 0 0 0", "", 10, $"0.82 {yMin}", $"0.95 {yMin + 0.13f}", unlock3 ? $"SENDCMD UPWALL {ID} {list_entry.grade} {list_entry.wall} {page}" : $"SENDCMD ERROR");
                    yMin -= yStep;
                }
                ...
                if (!HasPermission(player.UserIDString, permupgrade) && HasPermission(player.UserIDString, permreskin)) UI.Button(ref container, upgrade_2, "0.80 1.00 0.50 0.10", Lang("Reskin", player.UserIDString), 10, "0.6 0", "0.993 0.15", $"SENDCMD RESKIN {ID} {list_entry.grade} {list_entry.skinid} {page} {list_entry.color}");
        ...
        public static class RPC_PickupWallpaperStart_Patch
        {
            ...
            public static bool Prefix(BuildingBlock __instance, BaseEntity.RPCMessage msg){
                try
                {
                    ...
                    if (!msg.player.CanInteract() || !__instance.CanPickup(msg.player)) return false;
