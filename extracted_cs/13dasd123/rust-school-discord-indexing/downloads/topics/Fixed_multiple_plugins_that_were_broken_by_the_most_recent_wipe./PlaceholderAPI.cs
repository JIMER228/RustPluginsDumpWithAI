...
internal class PlaceholderAPI : CovalencePlugin
{
    ...
    private void InitSteamTypes()
    {
        ...
        steamServerType = Type.GetType($"Steamworks.SteamServer, {assemblyName}");
        if (steamServerType == null)
        {
            PrintError($"Unable to find Steamworks.SteamServer in assembly {assemblyName}");
        }
    }
    ...
    private static readonly Regex PlaceholderValidNameRegex = new Regex(@"^[a-z0-9_.-]+$", RegexOptions.Compiled);
    ...
    private IPAddress GetServerAddress()
    {
        ...
        if (steamServerType != null)
        {
            PropertyInfo publicIpPI = steamServerType.GetProperty("PublicIp", BindingFlags.Public | BindingFlags.Static);
            if (publicIpPI != null)
            {
                return (System.Net.IPAddress)publicIpPI.GetValue(null);
            }
            else
            {
                PrintError("PublicIp property not found");
            }
        }
        return null;
#else
