...
{
    [Info("VirtualQuarries", "ThePitereq", "2.5.0")]
    public class VirtualQuarries : RustPlugin
    {
        ...
        private class VirtualQuarry : FacepunchBehaviour
        {
            ...
            private bool CalculateFuel(bool takeFuel = true)
            {
                ...
                if (TakeFuel(fuelStorage, requiredFuel, amount, takeFuel)) return true;
                return false;
            }

            private bool TakeFuel(BoxStorage storage, FuelItem fuel, float amount, bool takeFuel = true)
            {
                int fuelToTakeCeiling = Mathf.CeilToInt(fuel.amount);
                ...
                if (takeFuel)
                    foreach (var item in storage.inventory.itemList.ToList())
                    {
                        if (item.skin == fuel.skin && item.info.shortname == fuel.shortname)
                        {
                            ...
                            {
                                nonIntOutput["fuel"] += needToTake - amount;
                            ...
                            {
                                takenItems += item.amount;
                                nonIntOutput["fuel"] += item.amount - amount;
                                item.GetHeldEntity()?.Kill();
