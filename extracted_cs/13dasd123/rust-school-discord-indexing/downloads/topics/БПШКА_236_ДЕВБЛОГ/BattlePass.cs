...
{
    [Info("BattlePass", "ShadowDecrypt", "0.0.1")]
        ...
        private void ShowUIMain(BasePlayer player, int page)
        {
            ...
            if (settings != null)
            {
                ...
                container.Add(new CuiLabel
                {
                    ...
                    Text =
                    {
                        Text = $"SKL EXP: <color=white>{d.Score}/{settings.Exp}</color>", Font = "robotocondensed-bold.ttf", FontSize = 15, Align = TextAnchor.MiddleLeft,
                        Color = "0.93 0.77 0.58 1.00"
                    }
                }, Layer);
            ...
            else
            {
                container.Add(new CuiLabel
                {
                    ...
                    Text =
                    {
                        Text = "Maks. Exp <color=green>- TEBRİKLER</color>", Font = "robotocondensed-bold.ttf", FontSize = 15, Align = TextAnchor.MiddleLeft,
                        Color = "0.93 0.77 0.58 1.00"
                    }
                }, Layer);
            ...
            foreach (var check in levels)
            {
                ...
                if (lvl % 10f != 0)
                {
                    ...
                    container.Add(new CuiLabel
                    {
                        ...
                        Text =
                        {
                            Text = $"Level: {lvl}", Font = "robotocondensed-bold.ttf", FontSize = 20, Align = TextAnchor.MiddleCenter,
                    ...
                    var image = !string.IsNullOrEmpty(check.Image) ? check.Image : check.Reward.ShortName; 
                    container.Add(new CuiElement
                    {
                        Parent = Layer + ".rewards" + lvl,
                        Components =
                ...
                else
                {
                    ...
                    var donateImgae = !string.IsNullOrEmpty(check.Image)
                        ? check.Image
            ...
            foreach (var check in donate)
            {
                ...
                if (lvl % 10f != 0)
                {
                    ...
                    var img = !string.IsNullOrEmpty(check.Image)
                        ? check.Image
                        : check.Reward.ShortName;
                    container.Add(new CuiElement
                    {
                        ...
                        Components =
                        {
                            new CuiRawImageComponent {Png = (String) ImageLibrary.Call("GetImage", img), FadeIn = 0.5f * i},
