...
{
    [Info("GameStoresRUST", "/https://blackplugin.ru/", "11.0.1")]
    public class GameStoresRUST : RustPlugin
    {
        ...
        private void LoadDefaultMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>() 
            {
                ...
                { "EXIT", "ВЫХОД" },
                { "BASKET.DESCRIPTION", "Это ваша корзина с покупками,  забрать их в любой момент" },
                ...
                { "BASKET.NO.AUTH", "Вы не авторизованы в магазине!\n" +
                                    "Ссылку на авторизацию  найти в разделе 'ПОМОЩЬ'" },


                { "USER.MANUAL", "ИНСТРУКЦИЯ ПОЛЬЗОВАТЕЛЯ" },
                { "USER.MANUAL.DESCRIPTION", "Здесь  забрать товары приобретенные у нас в магазине!\n" +
        ...
        private void OnServerInitialized()
        {
            ...
            ImageLibrary.Call("AddImage", "https://gspics.org/images/2024/07/01/0zFyMw.png", "backgroundpanel");
			if (plugins.Find("ImageLibrary") != null && ImageLibrary != null && !(bool)ImageLibrary.Call("HasImage", $"blueprintbase"))
				ImageLibrary.Call("AddImage", "https://i.ibb.co/2SM4fh4/blueprintbase.png", "blueprintbase");
        ...
        private void ShowNotify(BasePlayer player, string text)
        {
            ...
            container.Add(new CuiLabel
            {
                RectTransform = { AnchorMin = "0.185 0.835", AnchorMax = "0.822 0.90", OffsetMax = "0 0" },
