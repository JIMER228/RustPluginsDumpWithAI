...
{
    [Info("SkinBox", "k1lly0u", "2.1.19"), Description("Allows you to reskin item's by placing it in the SkinBox and selecting a new skin")]
    class SkinBox : RustPlugin
    {
        ...
        protected override void LoadDefaultMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                ["ClearFavourites"] = "Clear favourites for {0}"
            }, this);
        }
        ...
        private void OnServerInitialized()
        {
            ...
            if ((Steamworks.SteamInventory.Definitions?.Length ?? 0) == 0 && Configuration.Skins.UseApproved)
            {
                ...
                if (Configuration.Skins.ApprovedTimeout > 0)
        ...
        private void GetSkinsFor(BasePlayer player, string shortname, ref List<ulong> list)
        {
            ...
            SORT_FAVOURITES:
            if (Configuration.Favourites.Enabled)            
                Favourites.SortForPlayer(player, shortname, ref list);            
        }
        ...
        private class DeployableHandler : LootHandler
        {
            ...
            internal void Populate()
            {
                ...
                if (Configuration.Favourites.Enabled && Favourites.UserHasFavouritesFor(Looter, InputShortname))
                    CreateClearFavouritesButton();
            ...
            internal override void OnItemRemoved(Item item)
            {
                ...
                CuiHelper.DestroyUi(Looter, UI_CLEAR_FAVOURITES);
                ...
                if (wasSuccess && Configuration.Favourites.Enabled)
                    Favourites.OnSkinClaimed(Looter, item);
            ...
            internal static bool CanEntityBeRespawned(BaseEntity targetEntity, out string reason)
            {
                ...
                if (baseMountable != null && baseMountable.AnyMounted())
                {
        ...
        private class LootHandler : MonoBehaviour
        {
            ...
            internal void OnItemAdded(Item item)
            {
                ...
                if (Configuration.Favourites.Enabled)
                {
                    PopupMessage(GetMessage("FavouritesEnabled", Looter.userID));
                    
                    if (Favourites.UserHasFavouritesFor(Looter, InputShortname))
                        CreateClearFavouritesButton();
                }
            ...
            private IEnumerator RefillContainer()
            {
                ...
                if (HasItem && (_currentPage > 0 || _maximumPages > 1))
                    CreateOverlay();
            }
            ...
            protected const string UI_CLEAR_FAVOURITES = "SkinBox_Clear_Favourites_UI";
            ...
            private static UI.Offset OFFSET_ROOT = new UI.Offset(0, 16, -16, -16);
            private static UI.Offset OFFSET_INV_MENU = new UI.Offset(-622.5f, 0, 622.5f, 0);
            private static UI.Offset OFFSET_CONTAINER = new UI.Offset(-580.5f, 64f, 580.5f, 635f);
            private static UI.Offset OFFSET_RIGHT = new UI.Offset(-380, 28, 0, 5);

            private static UI.Offset OFFSET_BUTTONS = new UI.Offset(1f, -272f, 60f, 10f);
            private static UI.Offset OFFSET_BUTTONS_BACK = new UI.Offset(0f, -90f, 0f, 0f);
            private static UI.Offset OFFSET_BUTTONS_FORWARD = new UI.Offset(0f, 0f, 0f, 90f);
            private static UI.Offset OFFSET_BUTTONS_LABEL = new UI.Offset(0f, -45f, 0f, 45f);
            
            private static UI.Offset OFFSET_SEARCH = new UI.Offset(-184f, -640f, 36f, -614f);
            private static UI.Offset OFFSET_SEARCH_IMAGE = new UI.Offset(0, -13, 26, 13);
            private static UI.Offset OFFSET_SEARCH_INPUT = new UI.Offset(-82, -13, 82, 13);
            private static UI.Offset OFFSET_SEARCH_CLEAR = new UI.Offset(-26, -13, 0, 13);

            private static UI.Offset OFFSET_FAVOURITES = new UI.Offset(-184f, -612f, 36f, -586.5f);
            
            protected void CreateOverlay()
            {
                CuiElementContainer root = UI.Container(UI_PANEL, UI.Anchor.FullStretch, OFFSET_ROOT);
                CuiElement inventoryMenu = UI.Empty(root, UI_PANEL, UI.Anchor.VerticalCenterStretch, OFFSET_INV_MENU);
                CuiElement container = UI.Empty(root, inventoryMenu.Name, UI.Anchor.BottomCenter, OFFSET_CONTAINER);
                CuiElement right = UI.Empty(root, container.Name, UI.Anchor.RightStretch, OFFSET_RIGHT);

                CuiElement buttonHolder = UI.Empty(root, right.Name, UI.Anchor.CenterRight, OFFSET_BUTTONS);
                
                UI.Button(root, buttonHolder.Name, BUTTON_COLOR, BUTTON_TEXT_COLOR, "◀", 50, UI.Anchor.TopStretch, OFFSET_BUTTONS_BACK, _currentPage > 0 ? "skinbox.pageprev" : "");
                UI.Button(root, buttonHolder.Name, BUTTON_COLOR, BUTTON_TEXT_COLOR, "▶", 50, UI.Anchor.BottomStretch, OFFSET_BUTTONS_FORWARD, (_currentPage + 1) < _maximumPages ? "skinbox.pagenext" : "");
                
                CuiElement labelPanel = UI.Panel(root, buttonHolder.Name, PAGE_COLOR, UI.Anchor.HoriztonalCenterStretch, OFFSET_BUTTONS_LABEL);
                UI.Label(root, labelPanel.Name, PAGE_TEXT_COLOR, $"{_currentPage + 1}\nof\n{_maximumPages}", 20, UI.Anchor.FullStretch, UI.Offset.zero);
                
                CuiHelper.DestroyUi(Looter, UI_PANEL);
                CuiHelper.AddUi(Looter, root);
            }

            protected void CreateSearchBar()
            {
                CuiElementContainer root = UI.Container(UI_SEARCH, UI.Anchor.FullStretch, OFFSET_ROOT);
                CuiElement inventoryMenu = UI.Empty(root, UI_SEARCH, UI.Anchor.VerticalCenterStretch, OFFSET_INV_MENU);
                CuiElement container = UI.Empty(root, inventoryMenu.Name, UI.Anchor.BottomCenter, OFFSET_CONTAINER);
                CuiElement right = UI.Empty(root, container.Name, UI.Anchor.RightStretch, OFFSET_RIGHT);

                CuiElement searchHolder = UI.Panel(root, right.Name, BUTTON_COLOR, UI.Anchor.TopCenter, OFFSET_SEARCH);
                
                const string MAGNIFY_ICON = "https://chaoscode.io/oxide/Images/magnifyingglass_sb.png";
               
                UI.Image(root, searchHolder.Name, MAGNIFY_ICON, UI.Anchor.CenterLeft, OFFSET_SEARCH_IMAGE);
                UI.InputField(root, searchHolder.Name, BUTTON_TEXT_COLOR, string.Empty, 15, UI.Anchor.Center, OFFSET_SEARCH_INPUT, "skinbox.updatesearch");
                UI.Button(root, searchHolder.Name, BUTTON_COLOR, BUTTON_TEXT_COLOR, "<b>×</b>", 20, UI.Anchor.CenterRight, OFFSET_SEARCH_CLEAR, "skinbox.updatesearch");
                
                CuiHelper.DestroyUi(Looter, UI_SEARCH);
                CuiHelper.AddUi(Looter, root);
            }

            protected void CreateClearFavouritesButton()
            {
                CuiElementContainer root = UI.Container(UI_CLEAR_FAVOURITES, UI.Anchor.FullStretch, OFFSET_ROOT);
                CuiElement inventoryMenu = UI.Empty(root, UI_CLEAR_FAVOURITES, UI.Anchor.VerticalCenterStretch, OFFSET_INV_MENU);
                CuiElement container = UI.Empty(root, inventoryMenu.Name, UI.Anchor.BottomCenter, OFFSET_CONTAINER);
                CuiElement right = UI.Empty(root, container.Name, UI.Anchor.RightStretch, OFFSET_RIGHT);

                UI.Button(root, right.Name, BUTTON_COLOR, BUTTON_TEXT_COLOR, 
                    string.Format(GetMessage("ClearFavourites", Looter.userID), Definition.displayName.english), 12, 
                    UI.Anchor.TopCenter, OFFSET_FAVOURITES, "skinbox.clearfavourites");
            ...
            {
                CuiElementContainer container = UI.Container(UI_POPUP, new UI.Anchor(0.65f, 0.8f, 0.99f, 0.99f), UI.Offset.zero);
             
                UI.Label(container, UI_POPUP, BUTTON_TEXT_COLOR, message, 15, UI.Anchor.FullStretch, UI.Offset.zero, TextAnchor.UpperRight);
        ...
        public static class UI
        {
            ...
            public static CuiElement Empty(CuiElementContainer container, string parent, Anchor anchor, Offset offset)
            {
                CuiElement c = new CuiElement
                {
            ...
            public static CuiElement Panel(CuiElementContainer container, string parent, string color, Anchor anchor, Offset offset)
            {
            ...
            public static void Label(CuiElementContainer container, string panel, string color, string text, int size, Anchor anchor, Offset offset, TextAnchor align = TextAnchor.MiddleCenter)
            {
            ...
            public static void Button(CuiElementContainer container, string panel, string buttonColor, string textColor, string text, int fontSize, Anchor anchor, Offset offset, string command)
            {
                container.Add(new CuiElement
                {
                    Name = CuiHelper.GetGuid(),
                    ...
                    Components =
                    {
                        new CuiImageComponent { Color = buttonColor, Sprite = "assets/content/ui/ui.background.tile.psd", Material = "assets/icons/iconmaterial.mat" },
                        new CuiRectTransformComponent {AnchorMin = anchor.Min.ToString(), AnchorMax = anchor.Max.ToString(), OffsetMin = offset.Min.ToString(), OffsetMax = offset.Max.ToString()},
                    }
                });
                ...
                container.Add(new CuiElement
                {
                    ...
                    {
                        new CuiRectTransformComponent { AnchorMin = anchor.Min.ToString(), AnchorMax = anchor.Max.ToString(), OffsetMin = offset.Min.ToString(), OffsetMax = offset.Max.ToString() },
                        new CuiTextComponent {Color = textColor, Text = text, FontSize = fontSize, Align = TextAnchor.MiddleCenter}
                    }
                });
            ...
            public static void Image(CuiElementContainer container, string panel, string url, Anchor anchor, Offset offset)
            {
                container.Add(new CuiElement
                {
                    Name = CuiHelper.GetGuid(),
            ...
            public static void InputField(CuiElementContainer container, string panel, string textColor, string text, int size, Anchor anchor, Offset offset, string command)
            {
                container.Add(new CuiElement
                {
                    Name = CuiHelper.GetGuid(),
            ...
            public class HudMenuInutField : CuiInputFieldComponent
            {
                [JsonProperty("hudMenuInput")]
                public bool NeedsKeyboardHud { get; set; }
            }
            ...
            public struct Anchor
            {
                ...
                {
                    this.Min = new Bounds(x, y);
                    this.Max = new Bounds(x, y);
                }
        ...
        [ConsoleCommand("skinbox.clearfavourites")]
        private void cmdClearFavourites(ConsoleSystem.Arg arg)
        {
            if (arg == null || arg.Connection == null)
                return;
            ...
            if (_activeSkinBoxes.TryGetValue(player.userID, out lootHandler))
            {
                Favourites.ClearForPlayer(player, lootHandler.InputShortname);
                lootHandler.ResetSkinOrder();
            }
        }
        #endregion
        ...
        private void UpdateConfigValues()
        {
            ...
            if (Configuration.Version < new VersionNumber(2, 1, 14))
            {
                Configuration.Favourites = baseConfig.Favourites;
            }
        ...
        private static StoredData Favourites;
        ...
        private class StoredData
        {
            ...
            public void SortForPlayer(BasePlayer player, string shortname, ref List<ulong> list)
            {
                ...
                if (!Users.TryGetValue(player.userID, out userFavourites))
                    return;

                Hash<ulong, int> usage;
                if (!userFavourites.Usage.TryGetValue(shortname, out usage))
                    return;
            ...
            {
                UserFavourites userFavourites;
                if (!Users.TryGetValue(looter.userID, out userFavourites))
            ...
            public class UserFavourites
            {
                ...
                public void OnSkinClaimed(string shortname, ulong skinId)
                {
                    ...
                    Hash<ulong, int> usage;
                    if (!Usage.TryGetValue(shortname, out usage))
            ...
            public static void Save() => data.WriteObject(Favourites);

            public static void Load()
            {
                data = Interface.Oxide.DataFileSystem.GetFile("SkinBox/user_favourites");

                Favourites = data.ReadObject<StoredData>();

                if (Favourites == null)
                    Favourites = new StoredData();

                Favourites.PurgeOldEntries();
            }
        }
        #endregion
    }
