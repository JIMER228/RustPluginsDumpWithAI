...
namespace Oxide.Plugins
{
    ...
    class TsunHorse : RustPlugin
    {
        ...
        {            
            ...
            {
                string str = configData.Settings.AllowedTypes[i].ToLower();

                if (_animals.Contains(str))
                {
                    _allowedTypes.Add(str);
                    permission.RegisterPermission($"tsunhorse.ridehorse.{str}", this);
                    permission.RegisterPermission($"tsunhorse.spawnhorse.{str}", this);
                }
                else Debug.Log($"[TsunHorse] Invalid animal specified in config : {str}");
            }            
        ...
        private void OnPlayerInput(BasePlayer player, InputState input)
        {
            ...
            {
                if (Physics.SphereCast(player.eyes.position, 0.5f, Quaternion.Euler(player.serverInput.current.aimAngles) * Vector3.forward, out RaycastHit hit, 1.5f))
                {
                    ...
                    if (!_allowedTypes.Any(x => x.Contains(baseNpc.ShortPrefabName, CompareOptions.IgnoreCase)))
                    {
        ...
        private string FormatTime(double time)
        {
            ...
            if (days > 0)
                return $"{days:00}d:{hours:00}h:{mins:00}m:{secs:00}s";
            if (hours > 0)
        ...
        {
            if (FindPointOnNavmesh(position, 1, out Vector3 point))
            {
        ...
        private bool FindPointOnNavmesh(Vector3 center, float range, out Vector3 result)
        {
            for (int i = 0; i < 30; i++)
            {
                ...
                if (UnityEngine.AI.NavMesh.SamplePosition(randomPoint, out NavMeshHit hit, 5f, UnityEngine.AI.NavMesh.AllAreas))
                {
                    result = hit.position;
                    return true;
                }
        ...
        {            
            ...
            public void MountToNpc(BaseAnimalNPC baseNpc)
            {
                ...
                Player.SetMounted(Animal.Mountable);
                Animal.Npc.Resume();
                enabled = true;
        ...
        private class AnimalController : MonoBehaviour
        {
            ...
            public void CreateMountable(Controller controller)
            {
                ...
                if (ins.mountingPositions.TryGetValue(Npc.ShortPrefabName, out KeyValuePair<Vector3, Vector3> offset))
                {
            ...
            private void OnDestroy()
            {
                ...
                if (Npc != null && !Npc.IsDestroyed)
                {
                    ...
                    Npc.Invoke(() =>
                    {
                        ...
                        if (NavMesh.SamplePosition(Npc.ServerPosition, out NavMeshHit navHit, 5f, NavMesh.AllAreas))
                        {
        ...
        {            
            if (args.Length == 0)
            {
                ...
                SendReply(player, string.Format(msg("Help.AvailableTypes", player.UserIDString), _allowedTypes.ToSentence()));
                return;
            }
            ...
            {
                SendReply(player, msg("Error.InvalidAnimal", player.UserIDString) + " " + string.Format(msg("Help.AvailableTypes", player.UserIDString), _allowedTypes.ToSentence()));
                return;
            }
            ...
            if (storedData.cooldowns.TryGetValue(player.userID, out StoredData.Cooldown cooldown))
            {
            ...
            if (type == "horse")
            {
                if (!SpawnRideableHorse(player.transform.position + (Quaternion.Euler(0, player.serverInput.current.aimAngles.y, 0) * Vector3.forward), out RidableHorse ridableHorse))
                {
            ...
            {
                if (!SpawnAnimal(type, player.transform.position, out BaseNpc baseNpc))
                {
                    SendReply(player, msg("Error.InvalidPosition", player.UserIDString));
                    return;
                }
        ...
        private void cmdStop(BasePlayer player, string command, string[] args)
        {
            ...
            List<BaseNpc> entities = Pool.Get<List<BaseNpc>>();
        ...
        private void ccmdSpawn(ConsoleSystem.Arg arg)
        {
            ...
            if (!_allowedTypes.Contains(type))
            {
                SendReply(arg, "Invalid animal type selected. Options are: " + _allowedTypes.ToSentence());
                return;
            }
            ...
            {
                if (!float.TryParse(arg.Args[1], out float x) || !float.TryParse(arg.Args[2], out float z))
                {
            ...
            if (type == "horse")
            {
                if (!SpawnRideableHorse(position, out RidableHorse ridableHorse))
                {
            ...
            {
                if (!SpawnAnimal(type, position, out BaseNpc baseNpc))
                {
                    SendReply(arg, "Unable to spawn a animal at the specified position");
                    return;
                }
        ...
        private List<BasePlayer> FindPlayer(string arg)
        {
            ...
            ulong.TryParse(arg, out ulong steamid);
