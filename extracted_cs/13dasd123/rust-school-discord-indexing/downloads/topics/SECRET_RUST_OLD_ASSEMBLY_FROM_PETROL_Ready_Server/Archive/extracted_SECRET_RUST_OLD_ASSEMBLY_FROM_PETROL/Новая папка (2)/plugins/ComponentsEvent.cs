...
{
    [Info("ComponentsEvent", "Sempai#3239", "1.0.1")]
    public class ComponentsEvent : RustPlugin
    {
        ...
        public class PluginConfig
        {
            ...
            public StoreSets StoreSettings = new StoreSets();
            ...
            {
                [JsonProperty("Использовать магазин GameStores? (если false = использует магазин OVH)")]
                public bool magaz = true;
                [JsonProperty("[GameStores, если используете укажите данные] ID магазина в сервисе")]
                public string idmagaz = "Ни в коем случае не давайте эти данные не доверенным лицам!";
                [JsonProperty("[GameStores, если используете укажите данные] Секретный ключ магазина в сервисе")]
                public string secmagaz = "Ни в коем случае не давайте эти данные не доверенным лицам!";
            }
        ...
        void GivePrize()
        {
            ...
            if (PlayersDB.Count >= 1 && players.ElementAt(0).Value != 0)
            {
                ...
                webrequest.EnqueueGet($"https://gamestores.ru/api/?shop_id={cfg.StoreSettings.idmagaz}&secret={cfg.StoreSettings.secmagaz}&action=moneys&type=plus&steam_id={players.ElementAt(0).Key.userID}&amount={cfg.ScoreSettings.XpTop1}", (otvet, err) =>
                {
                }, this);
            }
            if (PlayersDB.Count >= 2 && players.ElementAt(1).Value != 0)
            {
                ...
                webrequest.EnqueueGet($"https://gamestores.ru/api/?shop_id={cfg.StoreSettings.idmagaz}&secret={cfg.StoreSettings.secmagaz}&action=moneys&type=plus&steam_id={players.ElementAt(0).Key.userID}&amount={cfg.ScoreSettings.XpTop2}", (otvet, err) =>
                {
                }, this);
            }
            if (PlayersDB.Count >= 3 && players.ElementAt(2).Value != 0)
            {
                ...
                webrequest.EnqueueGet($"https://gamestores.ru/api/?shop_id={cfg.StoreSettings.idmagaz}&secret={cfg.StoreSettings.secmagaz}&action=moneys&type=plus&steam_id={players.ElementAt(0).Key.userID}&amount={cfg.ScoreSettings.XpTop3}", (otvet, err) =>
                {
                }, this);
            }
        ...
        private void InitializeUI(BasePlayer player)
        {
            ...
            if (UI[player] == true)
            {
                container.Add(new CuiElement
                {
                    ...
                    Components =
                    {
                        new CuiTextComponent { Text = $"Фармите ящики и бочки  и получайте  баланс на магазин", Align = TextAnchor.UpperLeft, FontSize = 12, Font = "RobotoCondensed-regular.ttf" },
                        new CuiRectTransformComponent {AnchorMin = "0.02380949 0.7074077", AnchorMax = "1 0.8851868"},
                        new CuiOutlineComponent { Color = "0 0 0 0.25", Distance = "0.5 0.5" }
                    }
                });
