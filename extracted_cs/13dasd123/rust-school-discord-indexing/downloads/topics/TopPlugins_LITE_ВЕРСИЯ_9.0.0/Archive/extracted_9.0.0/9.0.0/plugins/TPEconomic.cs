...
{
    [Info("TPEconomic", "ds:alone_sempai / https://topplugin.ru/", "9.0.0")]
    class TPEconomic : RustPlugin
    {
        //апи пополнить забрать взять
        [ConsoleCommand("ec.give")]
        private void cmdMoneyGive(ConsoleSystem.Arg args)
        {
            if(args.Player != null) return;
            var player = args.Args[0];
            var much = args.Args[1];

            API_PUT_BALANCE_PLUS(ulong.Parse(player), int.Parse(much));
            Puts($"Баланс выдан {player} - {much} exp.");
        }
