...
{
    [Info("TPReportSystem", "ds:alone_sempai / https://topplugin.ru/", "9.0.0")]
    class TPReportSystem : RustPlugin
    {
        ...
        public class Configuration
        {
            ...
            public static Configuration GetNewCong()
            {
                return new Configuration
                {
                    ...
                    {
                        ["Reason_1"] = "Багоюз игрового процесса",
                        ["Reason_2"] = "Читы / макросы",
                        ["Reason_3"] = "Игра 3+"
                    },
                    Ban = new List<string>()
                    {
        ...
        void OnServerInitialized() {
            ...
            ImageLibrary.Call("AddImage", "https://i.ibb.co/zVrkzrt/b6wyl5i.png", "b6wyl5i");
            ImageLibrary.Call("AddImage", "https://i.ibb.co/DYcqfpp/lamC17G.png", "lamC17G");
            ImageLibrary.Call("AddImage", "https://i.ibb.co/ThGqcvC/HoGCxnN.png", "HoGCxnN");
            ImageLibrary.Call("AddImage", "https://i.ibb.co/ZzY0b88/DA7OETZ.png", "DA7OETZ");
            ImageLibrary.Call("AddImage", "https://i.ibb.co/F6n891m/aAm4ZHw.png", "aAm4ZHw");
            ImageLibrary.Call("AddImage", "https://i.ibb.co/S3CnyYC/xclemZi.png", "xclemZi");
        ...
        void ConsoleReport(ConsoleSystem.Arg args) {
            ...
            if (player != null && args.HasArgs(1))
            {
                ...
                {
                    if (!permission.UserHasPermission(player.UserIDString, config.Perm)) return;
                    var target = BasePlayer.FindByID(ulong.Parse(args.Args[1]));
                    var target_ulong = ulong.Parse(args.Args[1]);
                    var db = DB[target_ulong];
                    if (db.Enable == false)
                    {
                        CuiHelper.DestroyUi(player, "Menu_UI");
                    ...
                    else
                    {
                        ...
                        db.Enable = false;
                        db.DS = null;
                        List<Fields> fields = DT_PlayerCheckRemove(player, target_ulong);
		                SendDiscord(config.WebhookNotify, fields, new Authors(player.displayName, "", "", ""), config.Color);
        ...
        void ModerUI(BasePlayer player, string name = "", int page = 0) {
            ...
            CuiHelper.DestroyUi(player, "Moder");
             Dictionary<ulong, DataBase> target1 = new Dictionary<ulong, DataBase>();
            List<ulong> onlineUsr = new List<ulong>();

            foreach(var d in DB)
            {
                if(d.Value.SteamID != 0)
                {
                    target1.Add(d.Key, d.Value);
                }
            }

            foreach(var plO in BasePlayer.activePlayerList)
            {
                if(target1.ContainsKey(plO.userID))
                {
                    onlineUsr.Add(plO.userID);
                }
            }
            var container = new CuiElementContainer();
            container.Add(new CuiPanel
            ...
            {
                RectTransform = { AnchorMin = "0.91 0.38", AnchorMax = "0.962 0.645", OffsetMax = "0 0" },
                Button = { Color = "0 0 0 0", Command = target1.Count() > (page + 1) * 18 ? $"report skip {page + 1}" : "" },
                Text = { Text = "", Color = "0 0 0 0", Align = TextAnchor.MiddleCenter, FontSize = 20, Font = "robotocondensed-regular.ttf" }
            }, "Reports");


            float width = 0.132f, height = 0.31f, startxBox = 0.101f, startyBox = 0.985f - height, xmin = startxBox, ymin = startyBox;
            foreach (var check in target1.Skip(page * 18).Take(18))
            {
                if (check.Value.DisplayName.Contains(name) && onlineUsr.Contains(check.Value.SteamID)) {
                    container.Add(new CuiButton
                    {  
                        RectTransform = { AnchorMin = xmin + " " + ymin, AnchorMax = (xmin + width) + " " + (ymin + height * 1), OffsetMin = "4 4", OffsetMax = "-4 -4" },
        ...
        public class Fields
        {
            ...
            public Fields(String name, String values, bool inline)
            {
                this.name = name;
                this.values = values;
                this.inline = inline;
