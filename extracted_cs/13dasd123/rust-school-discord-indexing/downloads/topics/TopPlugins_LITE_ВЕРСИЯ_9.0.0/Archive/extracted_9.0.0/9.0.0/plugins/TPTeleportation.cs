...
{
    [Info("TPTeleportation", "ds:alone_sempai / https://topplugin.ru/", "9.0.0")]
	...
	Dictionary<string, int> TeleportSecsTownsPerms;
            ...
            foreach (MonumentInfo monument in UnityEngine.Object.FindObjectsOfType<MonumentInfo>())
            {
                if (monument.name.ToLower().Contains("compound"))
                {
                    ...
                    foreach (BaseEntity entity in list)
                    {
                        ...
                        {
                            PrintWarning("Позиция города нпс установленна!");
                            Vector3 chairPos = entity.transform.position + entity.transform.rotation * new Vector3(0, 0, 2);
                ...
                else if (monument.name.Contains("bandit"))
                {
                    ...
                    foreach (BaseEntity entity in list)
                    {
                        ...
                        {
                            PrintWarning("Позиция города бандитов установленна!");
                            Vector3 chairPos = entity.transform.position + entity.transform.rotation * new Vector3(0, 0, 2);
        ...
        private void DDrawMenu(BasePlayer player)
        {
            ...
            if (player.currentTeam != 0 && playerTeam.members.Count >= 2)
            {
                ...
                {
                    IPlayer friend = covalence.Players.FindPlayerById(check.ToString());
                    if (friend.Name != player.displayName) {
                        container.Add(new CuiButton
                        {
                            RectTransform = { AnchorMin = xmin + " " + ymin, AnchorMax = (xmin + width) + " " + (ymin + height * 1), OffsetMax = "0 0" },
                        ...
                        container.Add(new CuiButton
                        {
                            ...
                            Text = { Text = $"\n\n{friend.Name}", Color = "1 1 1 0.3", Align = TextAnchor.UpperCenter, FontSize = 12, Font = "robotocondensed-bold.ttf" }
                        }, "Users");

                        container.Add(new CuiButton
                        {
                            RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1", OffsetMax = "0 0" },
                            Button = { Color = "0 0 0 0", Command = $"tp.cmd tpr {check}" },
                            Text = { Text = "" }
                        }, "Users");
        ...
        void ChatBanditcamp(BasePlayer player)
        {
            ...
            if (lastTp != null)
            {
            	...
            	tpQueue.Add(new TP(player, BanditSpawns[new System.Random().Next(BanditSpawns.Count)], GetKDSecsToTpTown(player.userID), false, false, townTp: true));
            Effect.server.Run(EffectPrefab1, player, 0, Vector3.zero, Vector3.forward);
        ...
        {
            ImageLibrary.Call("AddImage", "https://i.ibb.co/zN6NSB2/bPDniMj.png", "fontp");
            ImageLibrary.Call("AddImage", "https://i.ibb.co/2P2C77m/qJhQdgd.png", "fonhome");
            ImageLibrary.Call("AddImage", "https://i.ibb.co/R3P1xqv/1VmpIaU.png", "fonavatar");
            ImageLibrary.Call("AddImage", "https://i.ibb.co/1XxnvQG/Jkcta4l.png", "fononline");
            ImageLibrary.Call("AddImage", "https://i.ibb.co/zHzMYj3/egv8dwC.png", "fonoffline");
            ImageLibrary.Call("AddImage", "https://i.ibb.co/VDW333j/5543435453453543.png", "fonDescription");
            LoadData();
            LoadDefaultConfig();
            FindTowns();
    ...
    ""components"": [
      ...
      },
      {
        ...
        Dictionary<string, string> Messages = new Dictionary<string, string>() {
            ...
            , {
                "sleepingbugbuildblock", "Спальный мешок находится в зоне чужого шкафа"
            }
            , {
