...
{
    [Info("Payback", "1928Tommygun", "2.0.17")]
    ...
    class Payback : RustPlugin
    {
        ...
        //| 2.0.17
        //| - update for Jan 12 2025, prevent players with sit from attacking and becoming hostile
        ...
        void DoPinyataEffect(BasePlayer player)
        {
            ...
            int itemCount = GetAllItems(player).Count();
            List<Item> items = new List<Item>(GetAllItems(player));
            ...
            int i = 0;
            foreach (var item in GetAllItems(player))
            {
        ...
        void DoDana(BasePlayer player, BasePlayer admin)
        {
            if (player != null && admin != null)
            {
                ...
                foreach (var item in new List<Item>(GetAllItems(player)))
                {
                    GiveItemOrDrop(admin, item);
                }
            }
        ...
        IEnumerator NakedOverTime(BasePlayer targetPlayer)
        {
            ...
            foreach (var item in GetAllItems(targetPlayer))
            {
                if (item != null)
                {
        ...
        {
            if (HasCard(attacker.userID, Card.Sit))
            {
                return false;
            }
            return null;
