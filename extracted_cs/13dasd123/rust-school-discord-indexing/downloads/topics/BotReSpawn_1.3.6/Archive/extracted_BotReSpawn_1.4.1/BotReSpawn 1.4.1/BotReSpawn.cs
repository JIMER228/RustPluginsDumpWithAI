...
namespace Oxide.Plugins
{
    ...
    //  Map markers option.
    //  Make melee npcs retreat if player is mounted. 
    ...
    //  Add Only_throw_In_Building priv 
    //  Parachuters flying away at heli kill profile - can't replicate

    //  Dont_Spawn_For_X_Hours_After_Wipe  




    //  Add proper NPC shield use. Maybe immediately after player reload, if player has bullet weapon?
    //  If not, randomise.

    //  Add additional damage types, such as radiation. Amount per hit and % chance.

    //  Adjustable melee attack speed? MelodyZE

    //  Suppressed weapons and quiet weapons - Ai_Assist

    //  Add console command to reload one or all profiles - No longer needed. Guy had NPCLoadouts, fighting with BRS.

    //  Make rocket npcs continue shooting even if building block is in the way.

    //  Support Jungle Biome. 



    //  Changes in 1.4.1
    //  Reverted to 1.3.9 navmesh behaviour. Roaming in weird areas shouldn't happen anymore.
    //  Revisited navmesh spam (server log file) issue. BotReSpawn now catches and handles issues with minimal server log output.
    //  Fixed prevent death messages issue.
    //  Added autofocus = false to input fields.
    //  Ignore commands from input fields where content didn't change (people couldn't access behaviour tab)
    //  Fixed Instant_Death_From_Headshot_Allowed_Weapons 'Manage' taking you to edit BotNames page.
    //  Added valid-shortname check when adding Instant_Death_From_Headshot_Allowed_Weapons.
    //  Added various missing descriptions from UI (when setting name is clicked)
    //  Improved /botrespawn info command. Now shows some detail on screen for any+all BotReSpawn npcs in close proximity.
    //  Reduced the load-spreading delay when initially spawning profiles. Full population will be reached 5X faster now.

    [Info("BotReSpawn", "Steenamaroo", "1.4.1", ResourceId = 0)] 
    [Description("Spawn tailored AI with kits at monuments, custom locations, or randomly.")]

    class BotReSpawn : RustPlugin
    {
        [PluginReference] Plugin Convoy, NoSash, Kits, CustomLoot, RustRewards, XPerience, AlphaLoot;
        int no_of_AI;
        bool loaded;
        ...
        {
            if (bs?.configData?.Global == null || bs.Unloading)
                return true;
            ...
            if (bs.configData.Global.NightStartHour > bs.configData.Global.DayStartHour)
                return hour >= bs.configData.Global.NightStartHour || hour < bs.configData.Global.DayStartHour;
            else
                return hour >= bs.configData.Global.NightStartHour && hour < bs.configData.Global.DayStartHour;
        }
        ...
        PlayerInventoryProperties[] scloadouts;
        ...
        public Dictionary<Gender, Dictionary<SkinTone, List<ulong>>> Models = new Dictionary<Gender, Dictionary<SkinTone, List<ulong>>>();
        ...
        void Unload()
        {
            ...
            harmony.UnpatchAll("BotReSpawn");
        }

        List<string> WeaponShortNames = new List<string>();

        void OnServerInitialized()
        {
            bs = this;
            scrap = ItemManager.FindItemDefinition("scrap");
            var crow = (ScarecrowNPC)GameManager.server.FindPrefab("assets/prefabs/npc/scarecrow/scarecrow.prefab")?.GetComponent<ScarecrowNPC>();
            if (crow != null)
            {
                scloadouts = crow.loadouts;
                sc = crow.LootSpawnSlots;
            }
            ...
            ApplyPatches();
            GetModels();
            WeaponShortNames = ItemManager.itemList.Select(x => x.shortname).ToList();
        }
        ...
        public void GetPlayerRandomBodyDetails(ulong userID, out int skinType, out float skinNumber, out float meshNumber)
        {
            skinType = (GetRandomFloatBasedOnUserID(userID, (ulong)4332) > 0.5f ? 1 : 0);
            meshNumber = GetRandomFloatBasedOnUserID(userID, (ulong)2647);
            //skinColor = GetRandomFloatBasedOnUserID(userID, (ulong)5977);
            skinNumber = GetRandomFloatBasedOnUserID(userID, (ulong)3975);
        }

        public float GetRandomFloatBasedOnUserID(ulong steamid, ulong seed)
        {
        ...
        void CheckAPCSafe()
        {
            ...
            foreach (var profile in Profiles)
                if (profile.Value.Spawn.AutoSpawn && profile.Value.Other.APC_Safe)
                    APCSafe = true;
        ...
        void CheckGunTrapSafe()
        {
            bool GunTrapSafe = false;
            foreach (var profile in Profiles)
                if (profile.Value.Spawn.AutoSpawn && profile.Value.Other.WeaponTrap_Safe)
                    GunTrapSafe = true;
            if (GunTrapSafe)
        ...
        void GetBiomePoints()
        {
            ...
            foreach (var tree in trees.ToList())
            {
                ...
                point = CalculateGroundPos(tree.transform.position + Vector3.forward / 2f, false, false, true);
        ...
        void CheckMonuments(bool add)
        {
            ...
            foreach (var monumentInfo in TerrainMeta.Path.Monuments.OrderBy(x => x.displayPhrase.english))
            {
                ...
                if (add)
                {
                    if (displayPhrase.Contains("Water Well") && configData.Global.Disable_WaterWells)
                        continue;
                    if (displayPhrase.Contains("Substation") && configData.Global.Disable_Substations)
                        continue;
                    if (displayPhrase.Contains("Underground Cave") && configData.Global.Disable_UndergroundCaves)
                        continue;
                    if (displayPhrase.Contains("Underwater Lab") && configData.Global.Disable_UnderwaterLabs)
                        continue;
                    if (displayPhrase.Contains("Oil Rig") && !configData.Global.Allow_Oilrigs)
                        continue;
                }
        ...
        string ProcessName(string name, string displayPhrase)
        {
            ...
            else if (displayPhrase == "Fishing Village" || displayPhrase == "Wild Swamp" || displayPhrase == "Abandoned Military Base")
                displayPhrase += name.Contains("_a.") ? "_A" : name.Contains("_b.") ? "_B" : name.Contains("_c.") ? "_C" : "_D";
            ...
            foreach (var kit in ValidKits.ToList())
            {
                ...
                if (kitContents != null)
                {
                    ...
                    foreach (var weap in items)
                    {
                        ...
                        if (Isweapon(Convert.ToInt32(item["itemid"]), null))
                        {
                            weaponInKit = true;
                            break;
                        }
        ...
        public void PopulateLoot(LootContainer source, ItemContainer container, bool doscrap)
        {
            ...
            if (doscrap && source != null && source.scrapAmount > 0)
            {
        ...
        public bool Isweapon(int i, Item item)
        {
            if (i != 0)
                item = ItemManager.CreateByItemID(i, 1);
        ...
        private object CanBeTargeted(NPCPlayer npc, BaseEntity entity)
        {
            BotData bData;
            if (NPCPlayers.TryGetValue(npc.userID, out bData))
                if (bData.profile.Other.WeaponTrap_Safe)
                    return false;
        ...
        private object CanBradleyApcTarget(BradleyAPC bradley, NPCPlayer npc)
        {
            ...
            BotData bData;
            if (NPCPlayers.TryGetValue(npc.userID, out bData))
                return !bData.profile.Other.APC_Safe;
        ...
        void SetupAPC(BradleyAPC apc)
        {
            if (apc != null)
                apc.InvokeRepeating(() => UpdateTargetList(apc, Convoy && (bool)Convoy?.Call("IsConvoyVehicle", apc)), 0f, 2f);
        }

        public void UpdateTargetList(BradleyAPC apc, bool convoy)
        {
            List<BaseEntity> list = Pool.Get<List<BaseEntity>>();
            Vis.Entities<BaseEntity>(apc.transform.position, apc.searchRange, list, 133120, QueryTriggerInteraction.Collide);
            BotData bData;
            foreach (BaseEntity baseEntity in list)
            {
                if ((ConVar.AI.ignoreplayers && !baseEntity.IsNpc) || !(baseEntity is BasePlayer))
                    continue;

                BasePlayer basePlayer = baseEntity as BasePlayer;
                if (basePlayer is HumanNPC || basePlayer is NPCPlayer)
                {
                    bData = null;
                    if (NPCPlayers.TryGetValue(basePlayer.userID, out bData))
                    {
                        if (!convoy && bData.profile.Other.APC_Safe)
                            continue;
                    }
                    else
                        continue;
                }
                if (basePlayer.IsDead() || basePlayer.InSafeZone() && !basePlayer.IsHostile() || !apc.VisibilityTest(baseEntity))
                {
                    continue;
                }
                bool flag = false;
                ...
                BradleyAPC.TargetInfo targetInfo1 = new BradleyAPC.TargetInfo();
                targetInfo1.Setup(baseEntity, Time.time);
                apc.targetList.Add(targetInfo1);
            }
            for (int i = apc.targetList.Count - 1; i >= 0; i--)
            {
                BradleyAPC.TargetInfo item = apc.targetList[i];
                BasePlayer basePlayer1 = item.entity as BasePlayer;
                if (item.entity == null || Time.time - item.lastSeenTime > apc.memoryDuration || basePlayer1.IsDead() || (basePlayer1.InSafeZone() && !basePlayer1.IsHostile()) || (ConVar.AI.ignoreplayers && !basePlayer1.IsNpc))
                    apc.targetList.Remove(item);
            }
            list.Clear();
            Pool.FreeUnmanaged<BaseEntity>(ref list);
            //Pool.FreeList<BaseEntity>(ref list);
            apc.targetList.Sort(new Comparison<BradleyAPC.TargetInfo>(apc.SortTargets));
        }

        List<BaseEntity> LiveGrenades = new List<BaseEntity>();
        object CanWaterBallSplash(ItemDefinition liquidDef, Vector3 position, float radius, int amount)
        {
            foreach (var entity in LiveGrenades)
                if (entity != null && !entity.IsDestroyed && entity?.transform != null)
                    if (Vector3.Distance(entity.transform.position, position) < radius + 1)
                        return false;
            return null;
        }
        ...
        object OnEntityTakeDamage(ScientistNPC player, HitInfo info)
        {
            ...
            if (info?.Initiator == player || (!configData.Global.Animal_Safe && info?.Initiator is BaseAnimalNPC))
                return true;

            if (bData1.invincible || (bData1.profile.Other.Fire_Safe && info.damageTypes.GetMajorityDamageType() == Rust.DamageType.Heat))
                return true;

            if (bData1.profile.Behaviour.NPCs_Defend_Against_Drones)
            {
                if (info.Initiator is Drone)
                {
                    bData1.CurrentEntity = info.Initiator as Drone;
                    return null;
                }
                if (info.Initiator is AutoTurret)
                {
                    var sphere = info.Initiator.GetParentEntity() as SphereEntity;
                    if (sphere != null)
                    {
                        bData1.CurrentEntity = sphere.GetParentEntity() as Drone;
                        return null;
                    }
                }
            }
            ...
            if (info.InitiatorPlayer != null)
            {
                ...
                {
                    info.damageTypes.ScaleAll((float)bData1.profile.Spawn.Bot_Take_Damage_Percent / 100f);
                    ...
                    if (player.lastAttacker == info.InitiatorPlayer && (info.InitiatorPlayer == bData1.CurrentTarget || bData1.Targets.ContainsKey(info.InitiatorPlayer) && (!player.limitNetworking && !player._limitedNetworking)))
                    ...
                    else if (bData1.WantsAttack(info.InitiatorPlayer, true))
                    {
                        ...
                        {
                            // if (check weapon is quiet or silenced)

                            List<BotData> bDatas = Pool.Get<List<BotData>>();
                            Vis.Components<BotData>(player.transform.position, Mathf.Min(300, bData1.profile.Behaviour.Assist_Sense_Range), bDatas);
                            foreach (var bData in bDatas)
                            {
                                ...
                                {
                                    timer.Once(Mathf.Min(5, Vector3.Distance(bData.transform.position, bData1.transform.position) / 20f), () =>
                                    {
                                        if (bData?.profile != null && bData.brain?.Senses != null && info.InitiatorPlayer != null && !info.InitiatorPlayer.IsDead())
                                        {
                                            ...
                                            bData.npc.Brain.Senses.Memory.SetKnown(info.InitiatorPlayer, bData.npc, bData.npc.Brain.Senses); // added not tested
                                        }
                                    });
                                }
                                //else
                            ...
                            bDatas.Clear();
                            Pool.FreeUnmanaged<BotData>(ref bDatas);
            ...
            if (configData.Global.Pve_Safe)
            {
                ...
                if (att == null || att.Contains("cactus") || att.Contains("barricade"))
                    return true;
                if (info.damageTypes.GetMajorityDamageType() == Rust.DamageType.ElectricShock)
                    return true;
            }
        ...
        private void OnTimedExplosiveExplode(TimedExplosive exp, Vector3 pos)
        {
            if (exp?.ShortPrefabName != "grenade.flashbang.deployed")
                return;

            List<BotData> brs = Pool.Get<List<BotData>>();
            Vis.Components<BotData>(pos, 10f, brs);
            foreach (var npc in brs)
            {
                if (npc?.BlindedTime != null && Vector2.Angle(npc.npc.eyes.transform.forward, pos - npc.transform.position) < 90) // Vector3.Angle(pos - npc.transform.position, npc.npc.eyes.BodyForward()) < 90
                    npc.BlindedTime = Time.time + 5f;
            }

            brs.Clear();
            Pool.FreeUnmanaged<BotData>(ref brs);
        }
        ...
        Dictionary<ulong, ulong> BSCrates = new Dictionary<ulong, ulong>();
        ...
        void OnEntityKill(ScientistNPC npc, HitInfo info, bool killed)
        {
            if (npc?.userID != null && NPCPlayers.ContainsKey(npc.userID) && !botInventories.ContainsKey(npc.userID))
            {
                ...
                if (!bData.temporary)
                    timer.Once(Mathf.Max(5, bData.profile.Death.Respawn_Timer * 60), () => bData.sg?.SpawnBot(1, bData.profile.type == ProfileType.Biome || bData.profile.Spawn.Change_SpawnPoint_On_Death ? null : bData.sp));
                    ...
                    if (bData.profile.Death.Spawn_Hackable_Death_Crate_Percent > 0 && bData.profile.Death.Spawn_Hackable_Death_Crate_Percent > GetRand(1, 101) && npc.WaterFactor() < 0.1f)
                    {
                        ...
                        timer.Once(2f, () =>
                        {
                            ...
                            if (bData?.profile == null)
                                return;
                            var Crate = (HackableLockedCrate)GameManager.server.CreateEntity(LockedCrate, pos + new Vector3(2, 3, 2), Quaternion.Euler(0, 0, 0));
                            Crate.Spawn();
                            BSCrates.Add(Crate.net.ID.Value, info?.InitiatorPlayer != null && !info.InitiatorPlayer.IsNpc && bData.profile.Death.Lock_Death_Crate_To_Killer_Player ? info.InitiatorPlayer.userID : 0);
                ...
                botInv.sacr.ServerInitialize(null, 36);
        ...
        object CanHackCrate(BasePlayer player, HackableLockedCrate crate) => CanLootEntity(player, crate);

        object CanLootEntity(BasePlayer player, HackableLockedCrate crate)
        {
            if (crate?.net?.ID.Value == null)
                return null;
            ulong owner = 0;
            if (BSCrates.TryGetValue(crate.net.ID.Value, out owner))
        ...
        void OnEntitySpawned(HackableLockedCrate crate)
        {
            ...
            NextTick(() =>
            {
                if (crate?.net?.ID == null || !loaded || BSCrates.ContainsKey(crate.net.ID.Value))
                    return;
        ...
        void DoEvent(string name, Vector3 pos)
        {
            ...
            if (profile.Spawn.AutoSpawn == true && GetPop(profile) > 0)
            {
                ...
                if (profile.Spawn.AutoSpawn == true && quantity > 0)
                {
                    ...
                    CreateTempSpawnGroup(null, pos, name, profile, string.Empty, quantity);
                }
            }
        }
        #endregion
        ...
        //{
        //    if (!loaded || !configData.Global.Supply_Enabled || (configData.Global.Ignore_Skinned_Supply_Grenades && signal.skinID != 0))
        //        Ignore.Add(plane.net.ID.Value);
        //}

        //void OnSupplyDropDropped(SupplyDrop drop, CargoPlane plane)
        //{
        //    if (Ignore.Contains(plane.net.ID.Value))
        //    {
        //        Ignore.Remove(plane.net.ID.Value);
        //        Ignore.Add(drop.net.ID.Value);
        //        return;
        //    }    
        //    DoDrop(drop, false);
        //}
        ...
        //{
        //    if (Ignore.Contains(drop.net.ID.Value))
        //    {
        //        Ignore.Remove(drop.net.ID.Value);
        //        return;
        //    }
        //    DoDrop(drop, true);
        //}

        //void DoDrop(SupplyDrop drop, bool landed)
        //{
        //    if (landed != configData.Global.Spawn_Airdrop_On_Landing)
        //        return;
        ...
        void OnEntitySpawned(SupplySignal signal) // Change to OnExplosiveThrown (Dropped?)
        {
            if (!loaded || signal == null || (configData.Global.Ignore_Skinned_Supply_Grenades && signal.skinID != 0))
                return;
        ...
        void OnEntitySpawned(SupplyDrop drop) // Change to OnSupplyDropDropped
        {
            if (!loaded || drop == null || (!drop.name.Contains("supply_drop") && !drop.name.Contains("sleigh/presentdrop")))
                return;
        ...
        object CanPopulateLoot(ScientistNPC entity, NPCPlayerCorpse corpse) => corpse?.playerSteamID != null && botInventories.ContainsKey(corpse.playerSteamID) ? true : (object)null;
        ...
        void OnEntitySpawned(NPCPlayerCorpse corpse)
        {
            ...
            timer.Once(0.1f, () =>
            {
                ...
                timer.Once((profile.Death.Corpse_Duration * 60) + 0.1f, () => { if (corpse != null && !corpse.IsDestroyed) corpse?.Kill(); });
                ...
                else
                {
                    ...
                    if (AlphaLoot || profile.Death.Rust_Loot_Source != "Default NPC")
                        corpse.containers[0].Clear();

                    if (AlphaLoot)
                    {
                        AlphaLoot.Call("PopulateLoot", botInv.sacr, profile.Death.Rust_Loot_Source == "Default NPC" ? "scientistnpc_roam" : profile.Death.Rust_Loot_Source == "ScarecrowNPC" ? "scarecrow" : container.ShortPrefabName);
                        for (int i = 0; i < botInv.sacr.itemList.Count(); i++)
                            botInv.sacr.itemList[i].MoveToContainer(corpse.containers[0]);
                    }
                    else if (profile.Death.Rust_Loot_Source != "Default NPC")
                        PopulateLoot(container, corpse.containers[0], !profile.Death.Remove_Vanilla_Scrap);
                ...
                if (profile.Death.AlphaLoot_Profile != string.Empty)
                {
                    botInv.sacr.capacity = 36;
                    AlphaLoot?.Call("PopulateLoot", botInv.sacr, profile.Death.AlphaLoot_Profile);

                    for (int i = 0; i < botInv.sacr.itemList.Count(); i++)
                        botInv.sacr.itemList[i].MoveToContainer(corpse.containers[0]);
                } 

                if (profile.Death.Corpse_Has_Skull)
                {
                ...
                for (int i = 0; i < botInv.inventory.Length; i++)
                {
                    ...
                    foreach (var item in botInv.inventory[i])
                    {
                        ...
                        giveItem.name = item.name;
                        giveItem.text = item.text;
                ...
                timer.Once(1f, () =>
                {
                    if (profile != null && corpse?.containers != null)
                    {
                        corpse.ResetRemovalTime((profile.Death.Corpse_Duration * 60) + 0.1f);
                ...
                if (profile.Spawn.Give_Scarecrow_Loadout || profile.Death.Wipe_Belt_Percent >= GetRand(1, 101))
                    corpse.containers[2].Clear();
                if (profile.Spawn.Give_Scarecrow_Loadout || profile.Death.Wipe_Clothing_Percent >= GetRand(1, 101))
        ...
        {
            if (configData == null || bData == null || bData.throwing || bData.healing || brain?.Senses == null || npc?.inventory?.containerBelt == null)
                return;
            ...
            if (bData.inAir && bData.profile.Other.Invincible_Whilst_Chuting)
                bData.canFire = false;
        ...
        void UpdateActiveItem(ScientistNPC npc, HeldEntity held, BotData bData)
        {
            ...
            held.GetItem().MoveToContainer(npc.inventory.containerBelt, 0, false, false, null, true); //// Added to ensure current weapon is in slot 0 
            if (held != null)
                held.SetHeld(true);
                    ...
                    if (IsSpawner(entry.Key) && entry.Value.Spawn.AutoSpawn && Mathf.Max(configData.Global.DayStartHour, configData.Global.NightStartHour) > 0)
                    {
                        ...
                        timer.Once(delay, () =>
                        {
                            ...
                            foreach (var go in Spawners[entry.Key])
                                SetUpSpawnGroup(null, go.go, go.profile, entry.Key, false, 0, String.Empty);
                        });
                    }
                }
            }
        ...
        void CreateTempSpawnGroup(BasePlayer MainTarget, Vector3 pos, string name, Profile profile, string group, int quantity)
        {
            ...
            SetUpSpawnGroup(MainTarget, gameObject, profile, name, true, quantity, group);
        }
        ...
        public void SetUpSpawnGroup(BasePlayer MainTarget, GameObject gameObject, Profile profile, string name, bool t, int q, string g)
        {
            int skipped = 0;
            ...
            comp.MainTarget = MainTarget;
            if (profile.Other.Marker_Enabled)
            {
                ...
                comp.marker.alpha = profile.Other.Marker_Opacity_Percent / 100f;
                comp.marker.radius = profile.Other.Marker_Radius_Percent / 300f;
                ColorUtility.TryParseHtmlString(profile.Other.Marker_Main_Colour, out comp.marker.color1);
                ColorUtility.TryParseHtmlString(profile.Other.Marker_Outer_Colour, out comp.marker.color2);
                comp.marker.Spawn();
            ...
            else
            {
                ...
                int multiplier = profile.Spawn.UseCustomSpawns ? 1 : 2;
                if (Points.Count < maxPopulation * multiplier)
                {
                    int safety = 0;
                    while (Points.Count < maxPopulation * multiplier)
                    {
                        safety++;
                        if (safety > maxPopulation * 2 * multiplier)
                        {
                            if (profile.type != ProfileType.Event)
                                bs.Puts($"Failed to get enough spawnpoints for profile {name}.");
                            return;
                        }
                        Vector3 loc = TryGetSpawn(gameObject.transform.position, profile.Spawn.Radius, profile.type == ProfileType.Event, profile.Other.Off_Terrain, true);
                        if (loc != Vector3.zero)
        ...
        void FinaliseSpawnGroup(CustomGroup comp, GameObject gameObject, Profile profile, string name, List<SpawnInfo> Points, bool t, int q, string g)
        {
            ...
            foreach (var entry in Points)
            {
                ...
                if (canChute)
                    canChute = profile.Other.Chute && point.location.y + 1f >= TerrainMeta.HeightMap.GetHeight(point.location) && WaterLevel.GetWaterDepth(point.location, true, true) <= 0;
                counter++;
            }
        ...
        //  Wodger request + Viktorvill
        //  Add JsonIgnore properties to profile class
        //  current population % decrease , user specified maximum % decrease
        //  and maybe DateTime last randomised?

        //  Alter ScalePop() to randomise the result in accordance with these options
        //  and re-randomise if DateTime total seconds elapsed is greater than some value.
        ...
        public class CustomGroup : SpawnGroup
        {
                ...
                for (int i = 0; i < numToSpawn; i++)
                {
                    ...
                    if (prefab != null && !string.IsNullOrEmpty(prefab.guid))
                    {
                        ...
                        if (spawnpoint)
                        {
                            ...
                            if (npc)
                            {
                                if (profile.Spawn.Gender != Gender.Random && profile.Spawn.SkinTone != SkinTone.Random)
                                {
                                    var g = profile.Spawn.Gender == Gender.Random ? bs.Models[(Gender)random.Next(1, 3)] : bs.Models[profile.Spawn.Gender]; 
                                    npc.userID = profile.Spawn.SkinTone == SkinTone.Random ? g[(SkinTone)random.Next(1, 5)].GetRandom() : g[profile.Spawn.SkinTone].GetRandom();
                                    npc.UserIDString = npc.userID.Get().ToString();
                                    npc.displayName = npc.UserIDString;
                                ...
                                npc.DeathIconOverride = new GameObjectRef();
                                if (marker != null)
                                {
                                    marker.radius = profile.Other.Marker_Radius_Percent / 300f;
                                    marker.SendUpdate();
                                }
            ...
            public override BaseSpawnPoint GetSpawnPoint(GameObjectRef prefabRef, out Vector3 pos, out Quaternion rot)
            {
                spawnPoints = spawnPoints.Where(x => x.GetType() == typeof(NewSpawnPoint)).ToArray();

                //var test = spawnPoints.Where(x => x is NewSpawnPoint).ToArray();                                  //// REMOVE
                //bs.Puts($"There are {spawnPoints.Count()} via typeof and {test.Count()} via is"); 
            ...
            public override void PostSpawnProcess(BaseEntity entity, BaseSpawnPoint spawnPoint)
            {
                ...
                { 
                    ...
                    npc.Brain.CancelInvoke(new Action(npc.Brain.TickMovement)); ////TRYING
                    ...
                    if (!bData.stationary && !ableToChute)
                    {
                        ...
                        nav.CanUseBaseNav = false; //bs.configData.Global.Use_Base_Nav;
                    ...
                    if (profile.Spawn.Give_Scarecrow_Loadout && bs.scloadouts != null)
                    {
                        npc.inventory.Strip();
                        bs.scloadouts.GetRandom().GiveToPlayer(npc);
                    ...
                    Interface.CallHook("OnBotReSpawnNPCSpawned", npc, profilename, group, MainTarget);
                });
            }
            ...
            private void OnDestroy()
            {
                ...
                for (int i = spawnInstances.Count - 1; i >= 0; i--)
                {
                    ...
                    else if (baseEntity)
                    {
                        ...
                        ScientistNPC npc = baseEntity as ScientistNPC;
                        npc.lifestate = BaseCombatEntity.LifeState.Dead;
                        npc.Invoke(new Action(npc.KillMessage), 0.5f);
                    } 
                }
        ...
        public bool ValidPoint(Vector3 pos) => NavMesh.SamplePosition(pos, out navMeshHit, 2, 1) && WaterLevel.GetWaterDepth(pos, true, true) <= 0;

        public static Vector3 CalculateGroundPos(Vector3 pos, bool e, bool Off_Terrain, bool initial)
        {
            if (!Off_Terrain && (!e || TerrainMeta.HeightMap.GetHeight(pos) <= pos.y))
                pos.y = TerrainMeta.HeightMap.GetHeight(pos);

            NavMeshHit navMeshHit;
            if (!NavMesh.SamplePosition(pos, out navMeshHit, 2, 1) || WaterLevel.GetWaterDepth(pos, true, true) > 0 || (initial && (!e || TerrainMeta.HeightMap.GetHeight(pos) - 100 <= pos.y) && Physics.RaycastAll(navMeshHit.position + new Vector3(0, 100, 0), Vector3.down, 99f, 1235288065).Any()))
                pos = Vector3.zero;
            else
        ...
        Vector3 TryGetSpawn(Vector3 pos, int radius, bool e, bool Off_Terrain, bool initial)
        {
            int attempts = 0;
            ...
            while (attempts < 50 && spawnPoint == Vector3.zero)
            {
                ...
                spawnPoint = CalculateGroundPos(pos + new Vector3(rand.x, 0, rand.y), e, Off_Terrain, initial);
        ...
        {
            string displayName = "BotReSpawn NPC"; 
            if (p.Spawn.BotNames.Count == 0)
            {
                displayName = Get(npc.userID);
                displayName = char.ToUpper(displayName[0]) + displayName.Substring(1);
            }
            else 
            {
                if (num != -1 && p.Spawn.BotNames.Count > num)
                    displayName = p.Spawn.BotNames[num];
                else
                    displayName = p.Spawn.BotNames.GetRandom();
            }

            if (p.Spawn.BotNamePrefix != String.Empty)
                displayName = p.Spawn.BotNamePrefix + " " + displayName;

            DisplayNames[npc.userID] = displayName;
        }

        Timer PatchTimer;
        private Harmony harmony = new Harmony("BotReSpawn");
        private void OnPluginUnloaded(Plugin plugin) => OnPluginLoaded(plugin);
        ...
        private void OnPluginLoaded(Plugin plugin)
        {
            if (bs?.harmony == null)
                return;
            PatchTimer?.Destroy();
            PatchTimer = timer.Once(2f, () => ApplyPatches());
        ...
        {
            [HarmonyPrefix]
            public static bool DisplayNameFix(ref string __result, ScientistNPC __instance)
            {
                if (__instance != null && bs != null && bs.DisplayNames.ContainsKey(__instance.userID))
                {
                    __result = bs.DisplayNames[__instance.userID];
                    return false;
                }
                return true;  
            }
        ...
        {
            [HarmonyPrefix]
            public static bool BlockDeathMessage(Debug __instance, ref string message)
            {
                try
                {
                    if (bs?.configData == null || !bs.configData.Global.Block_Console_Kill_Messages || !message.Contains("] was killed by"))
                        return true;
                    var ID = string.Concat(message.Substring(0, message.IndexOf("]")).Where(Char.IsDigit)); 
                    if (bs?.NPCPlayers == null || ID == null)
                        return true;

                    if (bs.NPCPlayers.ContainsKey(Convert.ToUInt64(ID)))
                        return false;
                }
                catch { }
                return true;
        ...
        //public static class NavSpam
        //{
        //    [HarmonyPrefix]
        //    public static bool PrintNavSpam(ref string log)
        //    {
        //        if (log.Contains("Failed to create agent"))
        //        {
        ...
        void ApplyPatches()
        {
            if (!Harmony.HasAnyPatches("BotReSpawn"))
            {
                var displaynameTarget = typeof(ScientistNPC).GetMethod("get_displayName");
                var predisplaynamePatch = typeof(DisplayName).GetMethod("DisplayNameFix");
                harmony.Patch(displaynameTarget, new HarmonyMethod(predisplaynamePatch));

                var KillLogTarget = typeof(Debug).GetMethods().FirstOrDefault(x => x.Name == "Log");
                var KillLogPatch = typeof(DeathMessage).GetMethod("BlockDeathMessage");
                harmony.Patch(KillLogTarget, new HarmonyMethod(KillLogPatch));

                //var NavSpam = typeof(Output).GetMethod("LogHandler");
                //var NavSpamPatch = typeof(NavSpam).GetMethod("PrintNavSpam");
                //harmony.Patch(NavSpam, new HarmonyMethod(NavSpamPatch));
            }
        }
        ...
        void SortWeapons(ScientistNPC npc, BotData bData)
        {
            ...
            if (npc?.inventory?.containerBelt?.itemList != null)
                foreach (Item item in npc.inventory.containerBelt.itemList)
                {
                    ...
                    {
                        if (gun.ShortPrefabName == "smg.entity" || gun.ShortPrefabName == "thompson.entity" || gun.ShortPrefabName == "nailgun.entity")
                        {
                            gun.attackLengthMin = 0.1f;
                            gun.attackLengthMax = 0.5f;
                            gun.attackSpacing = 1f;
                        }
                        ...
                        if (held.ShortPrefabName.Contains("pistol") || held.ShortPrefabName.Contains("shotgun") || held.ShortPrefabName.Contains("spas12") || held.ShortPrefabName.Contains("bow"))
                            bData.Weaps[Range.Close].Add(held);
                        else if (held.ShortPrefabName.Contains("bolt") || held.ShortPrefabName.Contains("l96"))
        ...
        void CmdBotsCount(ConsoleSystem.Arg arg)
        {
            ...
            foreach (var entry in records.Where(x => Profiles[x.Key].Spawn.AutoSpawn == true || x.Value.Count > 0))
            {
        ...
        bool CanInfo = true;
        public bool ShowNearInfo(BasePlayer player)
        {
            bool found = false;
            foreach (var npc in NPCPlayers)
            {
                if (npc.Value?.npc?.transform == null)
                    continue;
                if (Vector3.Distance(npc.Value.npc.transform.position, player.transform.position) < 100)
                {
                    var nav = npc.Value.npc.GetComponent<BaseNavigator>();
                    if (nav == null)
                        continue;
                    found = true;
                    CanInfo = false;
                    bool WasDormant = npc.Value.npc.IsDormant;
                    npc.Value.npc.IsDormant = true;
                    nav.Stop();

                    player.SendConsoleCommand("ddraw.text", 5f, Color.green, npc.Value.npc.transform.position + Vector3.up*2f, $"Profile :{npc.Value.profilename}\nName :{npc.Value.npc.displayName}\nHealth :{npc.Value.npc.health}/{npc.Value.npc.MaxHealth()}\nKit: {(string.IsNullOrEmpty(npc.Value.kit) ? "None" : npc.Value.kit)}");
                    timer.Once(5f, () =>
                    {
                        if (npc.Value?.npc != null)
                            npc.Value.npc.IsDormant = WasDormant;
                        if (nav != null)
                            nav.Resume();
                        CanInfo = true;
                    });
        ...
        void botrespawn(BasePlayer player, string command, string[] args)
        {
            ...
            if (args != null && args.Length == 1)
            {
                ...
                {
                    case "info":
                        var bData = ShowNearInfo(player);
                        if (!bData)
                            SendReply(player, TitleText + lang.GetMessage("nonpcfound", this));
                        return;
                }
            }
        ...
        public class Inv
        {
            ...
            public ItemContainer sacr = new ItemContainer();
        }
        ...
        public Dictionary<string, int> Biomes = new Dictionary<string, int> { { "BiomeArid", 1 }, { "BiomeTemperate", 2 }, { "BiomeTundra", 4 }, { "BiomeArctic", 8 }, { "BiomeJungle", 16 } };
        ...
        {
            public CustomSeekerTarget cstarget = new CustomSeekerTarget(null);
            public BasePlayer MainTarget;
            public string kit;
            ...
            public Single BlindedTime = Time.time;
            ...
            public BaseCombatEntity CurrentAnimal;
            ...
            void Start()
            {
                ...
                InvokeRepeating("TickMove", 1f, 0.1f); ////TRYING
            }

            void TickMove() => brain?.TickMovement(); 
            ...
            public bool Hostile(BasePlayer player) => bs.NPCPlayers.ContainsKey(player.userID) ? true : player.userID != 0 && player.State.unHostileTimestamp > Network.TimeEx.currentTimestamp; //// player.userID != 0 && 
            ...
            public void Think()
            {
                ...
                if (profile.Behaviour.NPCs_Defend_Against_Drones && CurrentEntity != null)
                {
                    Fire(CurrentEntity, npc.GetHeldEntity() as AttackEntity, true);
                    relaxing = false;
                    return;
                }
            ...
            public void Move()
            {
                if (!stationary)
                {
                    ...
                    else
                    {
                        ...
                        var loc = GetNearNavPoint(profile.Other.Short_Roam_Vision ? 5 : 20);
                        ...
                        if (npc.Brain.Navigator.StuckOffNavmesh)
                        {
                            npc.transform.position = roampoint;
                            brain.Navigator.SetNavMeshEnabled(true);
                            brain.Navigator.PlaceOnNavMesh(2); 
                        }
                    }
                }
            }
            ...
            Dictionary<ulong, int> times = new Dictionary<ulong, int>();
            public bool GoTo(Vector3 pos)
            {
                //if (brain.Navigator.Agent.isOnNavMesh)
                //    if (!brain.Navigator.Agent.CalculatePath(pos, brain.Navigator.Agent.path))
                //        return false;

                if (!brain.Navigator.CanUseNavMesh || npc.IsDormant) //// dormant
                    return false;
                brain.Navigator.Destination = pos;
                return brain.Navigator.SetDestination(pos, (CurrentTarget || CurrentAnimal) ? BaseNavigator.NavigationSpeed.Fast : BaseNavigator.NavigationSpeed.Slow, 0f, 0f);
            }
            ...
            public void ServerThrow(ScientistNPC npc, BasePlayer target)
            {
                ...
                var rand = UnityEngine.Random.insideUnitCircle * (15 - ((float)profile.Other.Grenade_Precision_Percent / 9));
                ...
                BaseEntity baseEntity = GameManager.server.CreateEntity(weap.prefabToThrow.resourcePath, npc.eyes.position, Quaternion.LookRotation((weap.overrideAngle == Vector3.zero ? -vector31 : weap.overrideAngle)), true);
                if (baseEntity == null)
                    return;

                baseEntity.SetCreatorEntity(npc);
                Vector3 vector32 = vector31 + (Quaternion.AngleAxis(10f, Vector3.zero) * Vector3.up);
                float throwVelocity = GetThrowVelocity(npc.eyes.position, target.transform.position, vector32);
                if (float.IsNaN(throwVelocity))
                {
                    vector32 = vector31 + (Quaternion.AngleAxis(20f, Vector3.zero) * Vector3.up);
                    throwVelocity = GetThrowVelocity(npc.eyes.position, target.transform.position, vector32);
                    if (float.IsNaN(throwVelocity))
                        throwVelocity = 5f;
                }
                ...
                baseEntity.Spawn();
                bs.LiveGrenades.Add(baseEntity);
            ...
            {
                ////  Prioritised MainTarget if not null.
                var pos = bs.TryGetSpawn(MainTarget != null ? MainTarget.transform.position : CurrentTarget == null || currentRange == Range.Long ? npc.transform.position : CurrentTarget.transform.position, radius, profile.type == ProfileType.Event, profile.Other.Off_Terrain, false);
            ...
            public void TryHeal()
            {
                ...
                healing = true;
                foreach (var item in (meds.OrderBy(x => Guid.NewGuid()).ToList()))  ////  Check health % from max, and choose best healing tool
                {                                                                   ////  Take scale_meds_to_health into account
                    if (item.ShortPrefabName.Contains("syringe_medical") || item.ShortPrefabName.Contains("bandage"))
                    {
                ...
                bs.timer.Once(1f, () =>
                {
                    if (bs?.configData == null || npc == null || npc.IsDestroyed)
                        return;

                    var newActive = npc.GetHeldEntity();
                    if (newActive?.ShortPrefabName != null && (newActive.ShortPrefabName.Contains("syringe_medical") || newActive.ShortPrefabName.Contains("bandage")))
                    {
                        ItemModConsumable component = newActive?.GetOwnerItemDefinition()?.GetComponent<ItemModConsumable>();
                        if (component?.effects == null) 
                            return;
                        foreach (var effect in component.effects.Where(effect => effect?.type != null && (effect.type == MetabolismAttribute.Type.Health || effect.type == MetabolismAttribute.Type.HealthOverTime)))
            ...
            List<BaseCombatEntity> Animals = new List<BaseCombatEntity>();
            public void DoThink()
            {
                if (npc?.transform == null || npc?.Brain?.Navigator?.transform == null)
                    return;

                if (npc?.Brain?.Senses?.Memory == null || bs?.configData == null)
                {
                ...
                if (profile.Behaviour.NPCs_Attack_Animals && CurrentAnimal == null)
                {
                    Animals = new List<BaseCombatEntity>();
                    Vis.Entities<BaseCombatEntity>(npc.transform.position, 10f, Animals);
                    if (Animals.Count > 0)
                    {
                        Animals = Animals.Where(x => x != null && !x.IsDead() && !x.IsDestroyed && (x is BaseAnimalNPC || x is BaseNPC2)).ToList();
                        if (Animals.Count > 0)
                ...
                if (bs.configData.Global.Allow_Ai_Dormant && (profile.type != ProfileType.Biome || !bs.configData.Global.Prevent_Biome_Ai_Dormant))
                {
                    ...
                    if (playersnear == 0)
                    {
                        ...
                        npc.Brain.Navigator.Stop(); //// Dormant
                    }
                    else
                        npc.IsDormant = false;
                }
            ...
            public void GetNearest()
            {
                ...
                ////  Check memory.threats for drones here?
            }
            ...
            public bool IsPlayer(BasePlayer player) => player?.net?.connection != null && player.userID.IsSteamId() && !player.IsNpc;
            ...
            public bool WantsAttack(BasePlayer player, bool hurt)
            {
                ...
                if (profile.Behaviour.Ignore_Wounded && player.IsWounded())
                    return false;
            ...
            public void KILL()
            {
                if (npc == null || npc.IsDead() || npc.IsDestroyed || profile == null || sg?.currentPopulation == null)
                {
                    string message = "";
                    if (bs.debug)
                    {
            ...
            public void OnDestroy()
            {
                ...
                CancelInvoke("Attack");
                CancelInvoke("TickMove");

                if (sg?.marker != null && sg.currentPopulation == 1 && !profile.Other.Marker_Visible_When_NPCs_Are_Dead)
                {
                    sg.marker.radius = 0f;
                ...
                if (cstarget != null)
                    SeekerTarget.SetSeekerTarget(cstarget, SeekerTarget.SeekerStrength.OFF);
            }
            ...
            private void OnTriggerEnter(Collider col)
            {
                ...
                {
                    npc.playerCollider.isTrigger = false;
                    npc.playerCollider.radius -= 2f;
                }
                ...
                //npc.Brain.Navigator.CanUseBaseNav = true;  
                npc.NavAgent.enabled = true;
                npc.Brain.Navigator.PlaceOnNavMesh(2f);

                if (!npc.NavAgent.isOnNavMesh)
                    npc.BecomeWounded(null);
            ...
            public void AttackTarget(BasePlayer target, bool targetIsLOS)
            {
                ...
                if (target != null && target.IsWounded() && profile.Behaviour.Ignore_Wounded)
                    return;
                ...
                if (melee)
                {
                    ...
                    if (target != null && bs.ValidPoint(target.transform.position))
                    {
                        ...
                        if (!stationary)
                            GoTo(target.transform.position + (target.transform.position - npc.transform.position).normalized);
                    }
                    else
                        if (!stationary)
                        GoTo(sp.transform.position);
            ...
            public void Fire(BaseEntity target, AttackEntity heldEntity, bool targetIsLOS)
            {
                ...
                if (CurrentTarget == null)
                {
                    if (target == CurrentAnimal && profile.Behaviour.NPCs_Attack_Animals)
                    {
                    ...
                    if (target == CurrentEntity && profile.Behaviour.NPCs_Defend_Against_Drones)
                        brain.Navigator.SetFacingDirectionOverride(GetAim());
                }
                ...
                else
                {
                    ...
                    if (Mathf.Approximately(heldEntity.attackLengthMin, -1f))
                    {
                        ...
                        if (target == CurrentAnimal)
                            CurrentAnimal.Hurt(CurrentAnimal.MaxHealth() / 4f);
                        return;
                    }
            ...
            public void LauncherUse(BaseLauncher launcher)
            {
                ...
                if (dist < 7 && launcher.primaryMagazine.ammoType.itemid != 1055319033)
                    return;

                if (dist > 100 && launcher.primaryMagazine.ammoType.itemid != -1841918730)
                    return;
                ...
                if (serverProjectile)
                {
                    ...
                    var Seeker = baseEntity.GetComponent<SeekingServerProjectile>();
                    if (Seeker != null)
                    {
                        Seeker.autoSeek = false;
                        cstarget.target = CurrentTarget;
                        SeekerTarget.SetSeekerTarget(cstarget, SeekerTarget.SeekerStrength.HIGHEST);
                        Seeker.UpdateTarget();
                    }
                }
            ...
            public class CustomSeekerTarget : SeekerTarget.ISeekerTargetOwner
            {
                public CustomSeekerTarget(BasePlayer player) { target = player; }
                public BasePlayer target;
                public Vector3 CenterPoint()
                {
                    if (target == null || target.transform == null)
                        return Vector3.zero;
                    return target?.transform?.position == null ? Vector3.zero : target.transform.position;
                }
                public bool InSafeZone() => false;
                public bool IsValidHomingTarget() => true;
                public bool IsVisible(Vector3 position, float maxDistance) => true;
            ...
            {
                BaseEntity target = CurrentTarget != null ? CurrentTarget : CurrentAnimal != null ? CurrentAnimal : CurrentEntity != null ? CurrentEntity : null;
            ...
            public void ServerUse(HeldEntity held, float damageModifier)
            {
                ...
                if (att == null || att.HasAttackCooldown())
                    return;
                damageModifier *= (held.ShortPrefabName.Contains("bolt") || held.ShortPrefabName.Contains("l96")) ? (float)profile.Behaviour.RangeWeapon_DamageScale : 1;
                ...
                //npc.SetAimDirection(npc.GetAimDirection()); // GetAim()
                npc.SetAimDirection(GetAim());
                ...
                var dir = npc.eyes.BodyForward();
                BasePlayer target = null;
                BaseEntity entity = null;

                for (int i = 0; i < component.numProjectiles; i++)
                {
                    var spread = i > 1 ? AimConeUtil.GetModifiedAimConeDirection(component.projectileSpread + att.GetAimCone() / 2f * 1f, npc.eyes.BodyForward(), true) : npc.GetAimDirection();

                    List<RaycastHit> list = Pool.Get<List<RaycastHit>>();
                    GamePhysics.TraceAll(new Ray(npc.eyes.position, spread), 0f, list, 400f, 1219701505, QueryTriggerInteraction.UseGlobal);
                    ...
                    for (int j = 0; j < list.Count; j++)
                    {
                        ...
                        if (target != null && (entity is AutoTurret || !npc.IsVisibleAndCanSee(target.eyes.position)))
                            break;
                        ...
                        if (entity != held && !entity.EqualNetID(held))
                        {
                            ...
                            hitInfo.DidHit = true;
                            hitInfo.ProjectileVelocity = npc.eyes.BodyForward() * 300f;
                            ...
                            if (target != null)
                            {
                                if (!hitInfo.damageTypes.Has(Rust.DamageType.Bleeding) && hitInfo.damageTypes.IsBleedCausing())
                                    target.metabolism?.bleeding.Add(-(hitInfo.damageTypes.Total() * 0.2f));

                                if (profile.Behaviour.Victim_Bleed_Amount_Per_Hit == 0)
                                    hitInfo.damageTypes.types[6] = 0f;
                                else
                                {
                                    var max = profile.Behaviour.Victim_Bleed_Amount_Max - target.metabolism.bleeding.value;
                                    if (max > 0)
                                    {
                                        if (profile.Behaviour.Bleed_Amount_Is_Percent_Of_Damage)
                                            target.metabolism?.bleeding.Add(Mathf.Min(max, hitInfo.damageTypes.Total() / 100f * profile.Behaviour.Victim_Bleed_Amount_Per_Hit));
                                        else
                                            target.metabolism?.bleeding.Add(Mathf.Min(max, profile.Behaviour.Victim_Bleed_Amount_Per_Hit));
                                    }
                                }
                            }
                            ...
                            if (target != null || entity is BaseNpc)
                            {
                                hitInfo.HitMaterial = StringPool.Get("Flesh");
                                Effect.server.ImpactEffect(hitInfo);
                            }

                            if (entity.ShouldBlockProjectiles() || entity is Shield)
                                break;
                        }
                    }
                    list.Clear();
                    ...
                    var cone = AimConeUtil.GetModifiedAimConeDirection(component.projectileSpread + att.GetAimCone() + att.GetAIAimcone() * 1f, npc.eyes.BodyForward(), true);
                    att.CreateProjectileEffectClientside(component.projectileObject.resourcePath, npc.eyes.position + npc.eyes.BodyForward() * 3f, cone * component.projectileVelocity, UnityEngine.Random.Range(1, 100), null, att.IsSilenced(), false);
                    att.CreateProjectileEffectClientside(component.projectileObject.resourcePath, npc.eyes.position + npc.eyes.BodyForward() * 3f, cone * component.projectileVelocity, UnityEngine.Random.Range(1, 100), target?.net?.connection, att.IsSilenced(), true);
                }
            }
            ...
            void MeleeAttack(BasePlayer t, BaseMelee melee)
            {
                ...
                delay = bs.timer.Once(0.2f, () =>
                {
                    ...
                    {
                        List<RaycastHit> list = Pool.Get<List<RaycastHit>>();
                        GamePhysics.TraceAll(new Ray(position - direction * (i == 0 ? 0.0f : 0.2f), direction), i == 0 ? 0.0f : melee.attackRadius, list, melee.effectiveRange + 0.2f, 1219701521, QueryTriggerInteraction.UseGlobal); // 1220225809 
                        bool flag = false;
                        bool shield = false;
                        for (int j = 0; j < list.Count; ++j)
                        {
                            e = list[j].GetEntity();
                            if (e != null && e is Shield)
                                shield = true;  
                        }
                        for (int j = 0; j < list.Count; ++j)
                        {
                                ...
                                if (p != null)
                                {
                                    ...
                                    else
                                    {
                                        if (profile.Behaviour.Victim_Bleed_Amount_Max == 100)
                                            p.metabolism?.bleeding.Add(profile.Behaviour.Bleed_Amount_Is_Percent_Of_Damage ? (attinfo.damageTypes.Total() / 100f * profile.Behaviour.Victim_Bleed_Amount_Per_Hit) : profile.Behaviour.Victim_Bleed_Amount_Per_Hit);
                                        else
                                        {
        ...
        public class Global
        {
            ...
            public bool Enable_Targeting_Hook = false, Animal_Safe = true, Supply_Enabled, Ignore_Skinned_Supply_Grenades, Staggered_Despawn = false;
            public bool Suicide_Boom = true, Remove_Frankenstein_Parts = true, Remove_KeyCard = true, Pve_Safe = true, Disable_WaterWells = true, Disable_Substations = false, Disable_UndergroundCaves = true, Disable_UnderwaterLabs;
        ...
        class DefaultData
        {
            ...
            public Dictionary<string, Profile> Biomes = new Dictionary<string, Profile>() { { "BiomeArid", new Profile(ProfileType.Biome) }, { "BiomeTemperate", new Profile(ProfileType.Biome) }, { "BiomeTundra", new Profile(ProfileType.Biome) }, { "BiomeArctic", new Profile(ProfileType.Biome) }, { "BiomeJungle", new Profile(ProfileType.Biome) } };
        }
        ...
        void ReloadData(string profile, bool UI, object AutoSpawn)
        {
            ...
            LastReloads[profile] = DateTime.Now;
        ...
        public class Profile
        {
            ...
            public class _Spawn : Base
            {
                ...
                public SkinTone SkinTone = SkinTone.Random;
                ...
                public int Bot_Take_Damage_Percent = 100;
                public bool Stationary;
                public bool UseCustomSpawns;
                public bool Change_SpawnPoint_On_Death;
                public bool Scale_NPC_Count_To_Player_Count;
                public bool Give_Scarecrow_Loadout = false;
                ...
                public bool ShouldSerializeChange_SpawnPoint_On_Death() => type != ProfileType.Event && type != ProfileType.Biome;
            }
            ...
            public class _Behaviour : Base
            {
                ...
                public bool Ignore_Wounded = true;
                public bool Target_Noobs = false;
                public bool NPCs_Attack_Animals = false;
                public bool NPCs_Defend_Against_Drones = false;
            ...
            public class _Death : Base
            {
                ...
                public bool Lock_Death_Crate_To_Killer_Player = false;
                public int Corpse_Duration = 1;
                public bool Corpse_Has_Skull = true;
                ...
                public bool Remove_Vanilla_Scrap = true;
                public string AlphaLoot_Profile = "";
            ...
            public class _Other : Base
            {
                ...
                public bool Marker_Enabled = false;
                public bool Marker_Visible_When_NPCs_Are_Dead = true;
                public string Marker_Main_Colour = "#ffffff";
                public string Marker_Outer_Colour = "#000000";
                public int Marker_Opacity_Percent = 50;
                public int Marker_Radius_Percent = 20;
                ...
                public bool APC_Safe = false;
                public bool WeaponTrap_Safe = false;
        ...
        readonly Dictionary<string, string> Messages = new Dictionary<string, string>()
        {
            ...
            {"NoWeaponShortName", "Weapon shortname {0} not found." },
            {"ParentSelected", "Parent Monument {0} set for profile {1}." },
            {"nonpcfound", "No BotReSpawn npcs were found near you." },
        ...
        void DestroyMenu(BasePlayer player, bool all)
        {
            ...
            CuiHelper.DestroyUi(player, "BSManageListUI");
        ...
        void BSDocs(BasePlayer player, string docname, string doc)
        {
            ...
            elements.Add(new CuiPanel { Image = { Color = "0.1 0.1 0.1 0.99" }, RectTransform = { AnchorMin = "0.1 0.05", AnchorMax = "0.9 0.95" }, CursorEnabled = true }, "Overlay", "BSDocs");
            elements.Add(new CuiButton { Button = { Color = "0 0 0 1" }, RectTransform = { AnchorMin = $"0 0.95", AnchorMax = $"0.999 1" }, Text = { Text = string.Empty } }, "BSDocs");
            elements.Add(new CuiButton { Button = { Color = "0 0 0 1" }, RectTransform = { AnchorMin = $"0 0", AnchorMax = $"0.999 0.05" }, Text = { Text = string.Empty } }, "BSDocs");
            elements.Add(new CuiLabel { Text = { Text = "BotReSpawn", FontSize = 20, Font = Font, Align = TextAnchor.MiddleCenter }, RectTransform = { AnchorMin = "0 0.95", AnchorMax = "1 1" } }, "BSDocs");
            elements.Add(new CuiLabel { Text = { Text = $"{docname.Replace("_", " ")}", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter }, RectTransform = { AnchorMin = "0 0.75", AnchorMax = "1 0.8" } }, "BSDocs");
            elements.Add(new CuiLabel { Text = { Text = $"{doc}", FontSize = 14, Font = Font, Align = TextAnchor.MiddleCenter }, RectTransform = { AnchorMin = "0 0.05", AnchorMax = "1 0.95" } }, "BSDocs");
            elements.Add(new CuiButton { Button = { Command = "CloseDocs", Color = "0 0 0 0" }, RectTransform = { AnchorMin = $"0 0", AnchorMax = $"1 1" }, Text = { Text = "", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSDocs");
        ...
        void BSBGUI(BasePlayer player)
        {
            ...
            var elements = new CuiElementContainer();
            elements.Add(new CuiPanel { Image = { Color = "0.1 0.1 0.1 0.94" }, RectTransform = { AnchorMin = "0.1 0.05", AnchorMax = "0.9 0.95" }, CursorEnabled = true }, "Overlay", "BSBGUI");
            elements.Add(new CuiButton { Button = { Color = "0 0 0 1" }, RectTransform = { AnchorMin = $"0 0.95", AnchorMax = $"0.999 1" }, Text = { Text = string.Empty } }, "BSBGUI");
            elements.Add(new CuiButton { Button = { Color = "0 0 0 1" }, RectTransform = { AnchorMin = $"0 0", AnchorMax = $"0.999 0.05" }, Text = { Text = string.Empty } }, "BSBGUI");
            elements.Add(new CuiButton { Button = { Command = "CloseBS", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = "0.955 0.96", AnchorMax = "0.99 0.99" }, Text = { Text = "X", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSBGUI");
            elements.Add(new CuiLabel { Text = { Text = "BotReSpawn", FontSize = 20, Font = Font, Align = TextAnchor.MiddleCenter }, RectTransform = { AnchorMin = "0.2 0.95", AnchorMax = "0.8 1" } }, "BSBGUI");
            CuiHelper.AddUi(player, elements);
        }
        ...
        void BSMainUI(BasePlayer player, string tab, string profile = "", string sub = "", int page = 1)
        {
            ...
            var elements = new CuiElementContainer();
            elements.Add(new CuiPanel { Image = { Color = "0 0 0 0" }, RectTransform = { AnchorMin = "0.1 0", AnchorMax = "0.9 0.9" }, CursorEnabled = true }, "Overlay", "BSMainUI");
            ...
            if (tab == string.Empty)
            {
                elements.Add(new CuiButton { Button = { Command = "", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0 0.95", AnchorMax = $"0.999 0.99" }, Text = { Text = $"BotReSpawn Settings", FontSize = 18, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");

                elements.Add(new CuiButton { Button = { Command = $"BSUIMain 0", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.45 0.775", AnchorMax = $"0.55 0.8" }, Text = { Text = $"Global settings", FontSize = 14, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                elements.Add(new CuiButton { Button = { Command = $"BSUIMain 1", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.45 0.725", AnchorMax = $"0.55 0.750" }, Text = { Text = $"Default profiles", FontSize = 14, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                elements.Add(new CuiButton { Button = { Command = $"BSUIMain 2", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.45 0.675", AnchorMax = $"0.55 0.700" }, Text = { Text = $"Custom profiles", FontSize = 14, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                elements.Add(new CuiButton { Button = { Command = $"BSUIShowAll", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.45 0.575", AnchorMax = $"0.55 0.600" }, Text = { Text = $"Show all profiles", FontSize = 14, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                ...
                if (Editing.ContainsKey(player.userID))
                {
                    ...
                    {
                        elements.Add(new CuiLabel { Text = { Text = $"Addspawn command is enabled for profile {name}", FontSize = 14, Font = Font, Align = TextAnchor.MiddleCenter }, RectTransform = { AnchorMin = $"0.35 0.425", AnchorMax = $"0.65 0.450" } }, "BSMainUI");
                        elements.Add(new CuiButton { Button = { Command = $"BSUIStopEdit", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.35 0.375", AnchorMax = $"0.65 0.400" }, Text = { Text = $"Stop editing {name}", FontSize = 14, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                        elements.Add(new CuiButton { Button = { Command = $"BSGotoProfile {(Profiles[name].type == ProfileType.Custom ? "2" : "1")} {RS(name)}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.35 0.325", AnchorMax = $"0.65 0.350" }, Text = { Text = $"Go to settings for {name}", FontSize = 14, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                    }
                    else 
                    {
            ...
            else if (profile == string.Empty)
            {
                if (tab == "0")
                {
                    ...
                    elements.Add(new CuiButton { Button = { Command = "", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0 0.95", AnchorMax = $"0.999 0.99" }, Text = { Text = $"Global settings", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                    ...
                    foreach (var setting in typeof(Global).GetFields().OrderBy(x => x.Name))
                    {
                        ...
                        if (setting.FieldType == typeof(bool))
                        {
                            ...
                            bottom -= 0.0235f;
                            if (odd)
                                elements.Add(new CuiPanel { Image = { Color = $"0 0 0 0.8" }, RectTransform = { AnchorMin = $"0 {top}", AnchorMax = $"0.999 {bottom}" }, CursorEnabled = true }, "BSMainUI");

                            elements.Add(new CuiLabel { Text = { Text = $"{setting.Name.Replace("_", " ")}", FontSize = 11, Font = Font, Align = TextAnchor.MiddleLeft }, RectTransform = { AnchorMin = $"0.05 {top}", AnchorMax = $"0.3 {bottom}" } }, "BSMainUI");
                            elements.Add(new CuiButton { Button = { Command = $"BSDocs {RS(setting.Name)}", Color = "0 0 0 0" }, RectTransform = { AnchorMin = $"0.05 {top}", AnchorMax = $"0.3 {bottom}" }, Text = { Text = "", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                            elements.Add(new CuiButton { Button = { Command = $"BSConfChangeBool {RS(setting.Name)}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.35 {top + 0.003}", AnchorMax = $"0.45 {bottom - 0.003}" }, Text = { Text = $"{cat}", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                            odd = !odd;
                        }
                    }
                    ...
                    foreach (var setting in typeof(Global).GetFields().OrderBy(x => x.Name).OrderByDescending(y => y.Name.Contains("Start")))
                    {
                        ...
                        if (setting.FieldType == typeof(int))
                        {
                            top -= 0.0235f;
                            bottom -= 0.0235f;
                            elements.Add(new CuiLabel { Text = { Text = $"{setting.Name.Replace("_", " ")}", FontSize = 11, Font = Font, Align = TextAnchor.MiddleLeft }, RectTransform = { AnchorMin = $"0.5 {top}", AnchorMax = $"0.8 {bottom}" } }, "BSMainUI");
                            elements.Add(new CuiButton { Button = { Command = $"BSDocs {RS(setting.Name)}", Color = "0 0 0 0" }, RectTransform = { AnchorMin = $"0.5 {top}", AnchorMax = $"0.8 {bottom}" }, Text = { Text = "", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                            elements.Add(new CuiButton { Button = { Command = $"BSConfChangeNum {RS(setting.Name)} false", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.85 {top + 0.003}", AnchorMax = $"0.87 {bottom - 0.003}" }, Text = { Text = "<", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                            elements.Add(new CuiLabel { Text = { Text = $"{cat}", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter }, RectTransform = { AnchorMin = $"0.875 {top + 0.003}", AnchorMax = $"0.925 {bottom - 0.003}" } }, "BSMainUI");
                            elements.Add(new CuiButton { Button = { Command = $"BSConfChangeNum {RS(setting.Name)} true", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.93 {top + 0.003}", AnchorMax = $"0.95 {bottom - 0.003}" }, Text = { Text = ">", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                        }
                    }
                    ...
                    elements.Add(new CuiLabel { Text = { Text = $"NPC Pop %", FontSize = 11, Font = Font, Align = TextAnchor.MiddleLeft }, RectTransform = { AnchorMin = $"0.5 {top}", AnchorMax = $"0.8 {bottom}" } }, "BSMainUI");
                    elements.Add(new CuiButton { Button = { Command = $"BSDocs NPCPop%", Color = "0 0 0 0" }, RectTransform = { AnchorMin = $"0.5 {top}", AnchorMax = $"0.8 {bottom}" }, Text = { Text = "", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                    elements.Add(new CuiLabel { Text = { Text = $"Server Pop", FontSize = 11, Font = Font, Align = TextAnchor.MiddleLeft }, RectTransform = { AnchorMin = $"0.7 {top}", AnchorMax = $"1 {bottom}" } }, "BSMainUI");
                    foreach (var entry in configData.Global.ScaleRules)
                    {
                        top -= 0.0235f;
                        bottom -= 0.0235f;
                        elements.Add(new CuiLabel { Text = { Text = $"{entry.Key}%", FontSize = 11, Font = Font, Align = TextAnchor.MiddleLeft }, RectTransform = { AnchorMin = $"0.5 {top + 0.003}", AnchorMax = $"0.525 {bottom - 0.003}" } }, "BSMainUI");
                        elements.Add(new CuiLabel { Text = { Text = $"of npcs will spawn if pop is at least", FontSize = 11, Font = Font, Align = TextAnchor.MiddleLeft }, RectTransform = { AnchorMin = $"0.53 {top + 0.003}", AnchorMax = $"0.7 {bottom - 0.003}" } }, "BSMainUI");

                        elements.Add(new CuiButton { Button = { Command = $"BSChangePop false {entry.Key}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.7 {top + 0.003}", AnchorMax = $"0.72 {bottom - 0.003}" }, Text = { Text = "<", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                        elements.Add(new CuiLabel { Text = { Text = $"{entry.Value}", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter }, RectTransform = { AnchorMin = $"0.725 {top + 0.003}", AnchorMax = $"0.775 {bottom - 0.003}" } }, "BSMainUI");
                        elements.Add(new CuiButton { Button = { Command = $"BSChangePop true {entry.Key}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.78 {top + 0.003}", AnchorMax = $"0.8 {bottom - 0.003}" }, Text = { Text = ">", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                    }
                }
                else
                {
                    elements.Add(new CuiButton { Button = { Command = "", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0 0.95", AnchorMax = $"0.999 0.99" }, Text = { Text = tab == "1" ? "Default Profiles" : "Custom Profiles", FontSize = 18, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                    ...
                    foreach (var entry in data)
                    {
                        ...
                        if ((counter != (page * 120) - 120) && counter > 0 && counter % 30 == 0)
                        {
                            top = 0.895f;
                            bottom = 0.92f;
                            left += 0.25;
                        ...
                        elements.Add(new CuiButton { Button = { Command = $"BSUI {tab} {RS(entry.Key)} 0 0", Color = "0 0 0 0" }, RectTransform = { AnchorMin = $"{left + 0.05} {top}", AnchorMax = $"{left + 0.3} {bottom}" }, Text = { Text = $"{entry.Key}", Color = entry.Value.Spawn.AutoSpawn ? "0 1 0 1" : "1 1 1 1", FontSize = 11, Font = Font, Align = TextAnchor.MiddleLeft } }, "BSMainUI");
                ...
                elements.Add(new CuiButton { Button = { Command = "BSUIMain", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.4 0.065", AnchorMax = $"0.6 0.095" }, Text = { Text = $"Back", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");

                if (page > 1)
                    elements.Add(new CuiButton { Button = { Command = $"BSUIPage {tab} {page - 1}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.15 0.065", AnchorMax = $"0.3 0.095" }, Text = { Text = $"<-", FontSize = 18, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");

                if (data.Count > page * 120)
                    elements.Add(new CuiButton { Button = { Command = $"BSUIPage {tab} {page + 1}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.7 0.065", AnchorMax = $"0.85 0.095" }, Text = { Text = $"->", FontSize = 18, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
            }
            else
            {
                var entry = data[RD(profile)];
                elements.Add(new CuiButton { Button = { Command = "", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0 0.95", AnchorMax = $"0.999 0.99" }, Text = { Text = $"{RD(profile)}", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                ...
                foreach (var category in typeof(Profile).GetFields())
                {
                    ...
                    if (sub == "0")
                        sub = category.Name;
                    elements.Add(new CuiButton { Button = { Command = $"BSUI {tab} {RS(profile)} {category.Name} 0", Color = configData.UI.ButtonColour2 }, RectTransform = { AnchorMin = $"{l} 0.91", AnchorMax = $"{r} 0.935" }, Text = { Text = $"{category.Name}", FontSize = 14, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                    ...
                    foreach (var setting in category.FieldType.GetFields())
                    {
                        ...
                        if (setting.Name == "type" || category.Name == "NextProfileThrow" || setting.Name == "Kit")
                            continue;
                        ...
                        if (entry.type != ProfileType.Custom && setting.Name == "Parent_Monument")
                            continue;
                        ...
                        elements.Add(new CuiLabel { Text = { Text = $"{setting.Name.Replace("_", " ")}", FontSize = 11, Font = Font, Align = TextAnchor.MiddleLeft }, RectTransform = { AnchorMin = $"0.02 {top}", AnchorMax = $"0.3 {bottom}" } }, "BSMainUI");
                        elements.Add(new CuiButton { Button = { Command = $"BSDocs {RS(setting.Name)}", Color = "0 0 0 0" }, RectTransform = { AnchorMin = $"0.02 {top}", AnchorMax = $"0.3 {bottom}" }, Text = { Text = "", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");

                        if (setting.FieldType == typeof(int))
                        {
                            elements.Add(new CuiButton { Button = { Command = $"BSChangeNum {tab} {RS(profile)} {RS(category.Name)} {RS(setting.Name)} false", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.25 {top + 0.003}", AnchorMax = $"0.27 {bottom - 0.003}" }, Text = { Text = "<", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                            elements.Add(new CuiLabel { Text = { Text = $"{setting.GetValue(cat)}", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter }, RectTransform = { AnchorMin = $"0.275 {top + 0.003}", AnchorMax = $"0.325 {bottom - 0.003}" } }, "BSMainUI");
                            elements.Add(new CuiButton { Button = { Command = $"BSChangeNum {tab} {RS(profile)} {RS(category.Name)} {RS(setting.Name)} true", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.33 {top + 0.003}", AnchorMax = $"0.35 {bottom - 0.003}" }, Text = { Text = ">", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                        }
                        else if (setting.FieldType == typeof(ShouldAttack) || setting.FieldType == typeof(FStein) || setting.FieldType == typeof(Gender) || setting.FieldType == typeof(SkinTone))
                        {
                            elements.Add(new CuiButton { Button = { Command = $"BSChangeEnum {tab} {RS(profile)} {RS(category.Name)} {RS(setting.Name)} false {Enum.GetNames(setting.FieldType).Length}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.25 {top + 0.003}", AnchorMax = $"0.27 {bottom - 0.003}" }, Text = { Text = "<", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                            elements.Add(new CuiLabel { Text = { Text = $"{setting.GetValue(cat)}", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter }, RectTransform = { AnchorMin = $"0.275 {top + 0.003}", AnchorMax = $"0.325 {bottom - 0.003}" } }, "BSMainUI");
                            elements.Add(new CuiButton { Button = { Command = $"BSChangeEnum {tab} {RS(profile)} {RS(category.Name)} {RS(setting.Name)} true {Enum.GetNames(setting.FieldType).Length}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.33 {top + 0.003}", AnchorMax = $"0.35 {bottom - 0.003}" }, Text = { Text = ">", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                        }
                        else if (setting.FieldType == typeof(double))
                        {
                            elements.Add(new CuiButton { Button = { Command = $"BSChangeDouble {tab} {RS(profile)} {RS(category.Name)} {RS(setting.Name)} false", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.25 {top + 0.003}", AnchorMax = $"0.27 {bottom - 0.003}" }, Text = { Text = "<", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                            elements.Add(new CuiLabel { Text = { Text = $"{setting.GetValue(cat)}", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter }, RectTransform = { AnchorMin = $"0.275 {top + 0.003}", AnchorMax = $"0.325 {bottom - 0.003}" } }, "BSMainUI");
                            elements.Add(new CuiButton { Button = { Command = $"BSChangeDouble {tab} {RS(profile)} {RS(category.Name)} {RS(setting.Name)} true", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.33 {top + 0.003}", AnchorMax = $"0.35 {bottom - 0.003}" }, Text = { Text = ">", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                        }
                        else if (setting.FieldType == typeof(bool))
                            elements.Add(new CuiButton { Button = { Command = $"BSChangeBool {tab} {RS(profile)} {RS(category.Name)} {RS(setting.Name)}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.25 {top + 0.003}", AnchorMax = $"0.35 {bottom - 0.003}" }, Text = { Text = $"{setting.GetValue(cat)}", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                        else if (setting.FieldType == typeof(List<string>))
                        {
                            elements.Add(new CuiButton { Button = { Command = $"BSManageList {RS(profile)} {RS(category.Name)} {RS(setting.Name)}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.25 {top + 0.003}", AnchorMax = $"0.35 {bottom - 0.003}" }, Text = { Text = "Manage", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                        }
                        else if (setting.FieldType == typeof(string))
                        {
                            if (setting.Name == "Parent_Monument")
                                elements.Add(new CuiButton { Button = { Command = $"BSShowParents {tab} {RS(profile)} {RS(category.Name)} {RS(setting.Name)}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.25 {top + 0.003}", AnchorMax = $"0.35 {bottom - 0.003}" }, Text = { Text = $"{(setting.GetValue(cat).ToString() == string.Empty ? "Select" : setting.GetValue(cat))}", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                            ...
                            else
                            {
                                elements.Add(new CuiElement
                                {
                                    ...
                                    Components =
                                    {
                                        new CuiInputFieldComponent { Align = TextAnchor.MiddleLeft, CharsLimit = 300, Command = $"BSChangeString {tab} {RS(profile)} {RS(category.Name)} {RS(setting.Name)}", FontSize = 11, IsPassword = false, Text = setting.GetValue(cat).ToString(), NeedsKeyboard = true, Autofocus = false },
                                        new CuiRectTransformComponent { AnchorMin = $"0.25 {top + 0.003}", AnchorMax = $"0.55 {bottom - 0.003}" }
                                    }
                                });
                            }
                        }
                        else
                            elements.Add(new CuiLabel { Text = { Text = $"{(setting.Name == "Location" ? setting.GetValue(cat) : "Edit in json")}", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter }, RectTransform = { AnchorMin = $"0.25 {top + 0.003}", AnchorMax = $"0.35 {bottom - 0.003}" } }, "BSMainUI");
                        odd = !odd;
                    }
                }
                ...
                {
                    elements.Add(new CuiButton { Button = { Command = $"BSUITPPlayer {RS(profile)}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.08 0.14", AnchorMax = $"0.19 0.16" }, Text = { Text = "TP me there", FontSize = 12, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                    elements.Add(new CuiButton { Button = { Command = $"BSUIEditSpawns {RS(profile)} 0", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.21 0.14", AnchorMax = $"0.32 0.16" }, Text = { Text = "Edit Spawnpoints", FontSize = 12, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                    if (data[RD(profile)].type == ProfileType.Custom)
                        elements.Add(new CuiButton { Button = { Command = $"BSUIMoveProfile {RS(profile)}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.21 0.11", AnchorMax = $"0.32 0.13" }, Text = { Text = "Move profile here", FontSize = 12, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                }

                elements.Add(new CuiButton { Button = { Command = $"BSUIReload {RS(profile)}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.4 0.17", AnchorMax = $"0.6 0.19" }, Text = { Text = "Reload profile", FontSize = 12, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                elements.Add(new CuiButton { Button = { Command = $"BSUIEditKits {RS(profile)} -1 0", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.4 0.14", AnchorMax = $"0.6 0.16" }, Text = { Text = "Edit Kits", FontSize = 12, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                elements.Add(new CuiButton { Button = { Command = $"BSUIToPlayerSelect {RS(profile)}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.4 0.11", AnchorMax = $"0.6 0.13" }, Text = { Text = "Send to player", FontSize = 12, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");

                elements.Add(new CuiButton { Button = { Command = $"BSCopy {tab} {RS(profile)} {sub}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.68 0.14", AnchorMax = $"0.79 0.16" }, Text = { Text = "Copy profile", FontSize = 12, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                if (Copy.ContainsKey(player.userID))
                    elements.Add(new CuiButton { Button = { Command = $"BSPaste {tab} {RS(profile)} {sub}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.68 0.11", AnchorMax = $"0.79 0.13" }, Text = { Text = $"Paste {Copy[player.userID]}", FontSize = 12, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");

                elements.Add(new CuiButton { Button = { Command = $"BSUISaveTemplate {RS(profile)}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.81 0.14", AnchorMax = $"0.92 0.16" }, Text = { Text = "Save template", FontSize = 12, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
                if (templateData.Templates.Count > 0)
                    elements.Add(new CuiButton { Button = { Command = $"BSUIViewTemplates {tab} {RS(profile)} {sub}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.81 0.11", AnchorMax = $"0.92 0.13" }, Text = { Text = "Load template", FontSize = 12, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");

                if (data[RD(profile)].type == ProfileType.Custom)
                    elements.Add(new CuiButton { Button = { Command = $"BSUIRemoveProfile {RS(profile)}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.01 0.065", AnchorMax = $"0.16 0.095" }, Text = { Text = "Delete Profile", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");

                elements.Add(new CuiButton { Button = { Command = $"BSUIMain {tab}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.4 0.065", AnchorMax = $"0.6 0.095" }, Text = { Text = "Back", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSMainUI");
        ...
        void BSKitsUI(BasePlayer player, string profile = "", int spawnpoint = -1, int page = 0)
        {
            DestroyMenu(player, false);
            var elements = new CuiElementContainer();
            elements.Add(new CuiPanel { Image = { Color = "0 0 0 0" }, RectTransform = { AnchorMin = "0.1 0", AnchorMax = "0.9 0.9" }, CursorEnabled = true }, "Overlay", "BSKitsUI");
            ...
            elements.Add(new CuiLabel { Text = { Text = text, FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter }, RectTransform = { AnchorMin = $"0 0.9", AnchorMax = $"1 1" } }, "BSKitsUI");
            ...
            for (int i = from; i <= to; i++)
            {
                ...
                if (spawnpoint > -1)
                {
                    elements.Add(new CuiLabel { Text = { Text = $"{ValidKits[i]}", FontSize = 11, Font = Font, Align = TextAnchor.MiddleLeft }, RectTransform = { AnchorMin = $"{left + 0.05} {top}", AnchorMax = $"{left + 0.25} {bottom}" } }, "BSKitsUI");
                    elements.Add(new CuiButton { Button = { Command = $"BSChangeSPKit {RS(profile)} {i} {spawnpoint} {page} false", Color = num > 0 ? configData.UI.ButtonColour2 : configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"{left + 0.25} {top + 0.003}", AnchorMax = $"{left + 0.27} {bottom - 0.003}" }, Text = { Text = "<", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSKitsUI");
                    elements.Add(new CuiLabel { Text = { Text = $"{k.Where(x => x == ValidKits[i]).Count()}", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter }, RectTransform = { AnchorMin = $"{left + 0.275} {top + 0.003}", AnchorMax = $"{left + 0.285} {bottom - 0.003}" } }, "BSKitsUI");
                    elements.Add(new CuiButton { Button = { Command = $"BSChangeSPKit {RS(profile)} {i} {spawnpoint} {page} true", Color = num > 0 ? configData.UI.ButtonColour2 : configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"{left + 0.29} {top + 0.003}", AnchorMax = $"{left + 0.31} {bottom - 0.003}" }, Text = { Text = ">", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSKitsUI");
                }
                else
                {
                    elements.Add(new CuiLabel { Text = { Text = $"{ValidKits[i]}", FontSize = 11, Font = Font, Align = TextAnchor.MiddleLeft }, RectTransform = { AnchorMin = $"{left + 0.05} {top}", AnchorMax = $"{left + 0.25} {bottom}" } }, "BSKitsUI");
                    elements.Add(new CuiButton { Button = { Command = $"BSChangeKit {RS(profile)} {i} -1 {page} false", Color = num > 0 ? configData.UI.ButtonColour2 : configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"{left + 0.25} {top + 0.003}", AnchorMax = $"{left + 0.27} {bottom - 0.003}" }, Text = { Text = "<", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSKitsUI");
                    elements.Add(new CuiLabel { Text = { Text = $"{num}", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter }, RectTransform = { AnchorMin = $"{left + 0.275} {top + 0.003}", AnchorMax = $"{left + 0.285} {bottom - 0.003}" } }, "BSKitsUI");
                    elements.Add(new CuiButton { Button = { Command = $"BSChangeKit {RS(profile)} {i} -1 {page} true", Color = num > 0 ? configData.UI.ButtonColour2 : configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"{left + 0.29} {top + 0.003}", AnchorMax = $"{left + 0.31} {bottom - 0.003}" }, Text = { Text = ">", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSKitsUI");
                }
            }

            if (page > 0)
                elements.Add(new CuiButton { Button = { Command = $"BSUIEditKits {RS(profile)} {spawnpoint} {page - 1}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.2 0.065", AnchorMax = $"0.3 0.095" }, Text = { Text = "<-", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSKitsUI");
            if (ValidKits.Count > to)
                elements.Add(new CuiButton { Button = { Command = $"BSUIEditKits {RS(profile)} {spawnpoint} {page + 1}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.7 0.065", AnchorMax = $"0.8 0.095" }, Text = { Text = "->", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSKitsUI");


            elements.Add(new CuiButton { Button = { Command = $"CloseExtra BSKitsUI {RS(profile)} {spawnpoint} {page}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.4 0.065", AnchorMax = $"0.6 0.095" }, Text = { Text = "Back", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSKitsUI");
            CuiHelper.AddUi(player, elements);
        }
        ...
        void BSSpawnsUI(BasePlayer player, string profile = "", int page = 0)
        {
            DestroyMenu(player, false);
            var elements = new CuiElementContainer();
            elements.Add(new CuiPanel { Image = { Color = "0 0 0 0" }, RectTransform = { AnchorMin = "0.1 0", AnchorMax = "0.9 0.9" }, CursorEnabled = true }, "Overlay", "BSSpawnsUI");
            elements.Add(new CuiLabel { Text = { Text = $"Spawn points for {profile}", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter }, RectTransform = { AnchorMin = $"0 0.9", AnchorMax = $"1 1" } }, "BSSpawnsUI");
            ...
            for (int i = 0; i < s.Count; i++)
            {
                ...
                top -= 0.0235f;
                bottom -= 0.0235f;
                elements.Add(new CuiLabel { Text = { Text = $"{i + (page * 54)}", FontSize = 11, Font = Font, Align = TextAnchor.MiddleLeft }, RectTransform = { AnchorMin = $"{left + 0.08} {top}", AnchorMax = $"{left + 0.10} {bottom}" } }, "BSSpawnsUI");
                elements.Add(new CuiButton { Button = { Command = $"BSUIMoveSpawn {RS(profile)} {i + (page * 54)} true {page}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"{left + 0.11} {top + 0.003}", AnchorMax = $"{left + 0.16} {bottom - 0.003}" }, Text = { Text = "Remove", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSSpawnsUI");
                elements.Add(new CuiButton { Button = { Command = $"BSUIMoveSpawn {RS(profile)} {i + (page * 54)} false {page}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"{left + 0.17} {top + 0.003}", AnchorMax = $"{left + 0.23} {bottom - 0.003}" }, Text = { Text = "Move here", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSSpawnsUI");
                elements.Add(new CuiButton { Button = { Command = $"BSEditOverRides {RS(profile)} {i + (page * 54)}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"{left + 0.24} {top + 0.003}", AnchorMax = $"{left + 0.31} {bottom - 0.003}" }, Text = { Text = "Edit overrides", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSSpawnsUI");
            }

            elements.Add(new CuiButton { Button = { Command = $"BSUISetEditing {RS(profile)}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.3 0.16", AnchorMax = $"0.49 0.19" }, Text = { Text = "Edit with console commands", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSSpawnsUI");
            elements.Add(new CuiButton { Button = { Command = $"BSUIAddSpawn {RS(profile)} {page}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.3 0.12", AnchorMax = $"0.49 0.15" }, Text = { Text = "Add spawnpoint here", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSSpawnsUI");
            elements.Add(new CuiButton { Button = { Command = $"BSUIShowSpawns {RS(profile)}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.51 0.16", AnchorMax = $"0.7 0.19" }, Text = { Text = "Show all spawnpoints", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSSpawnsUI");
            elements.Add(new CuiButton { Button = { Command = $"BSUICheckNav", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.51 0.12", AnchorMax = $"0.7 0.15" }, Text = { Text = "Check for Navmesh", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSSpawnsUI");

            if (page > 0)
                elements.Add(new CuiButton { Button = { Command = $"BSUIEditSpawns {RS(profile)} {page - 1}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.2 0.065", AnchorMax = $"0.3 0.095" }, Text = { Text = "<-", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSSpawnsUI");
            if (spawnsData.CustomSpawnLocations[profile].Count() > (page + 1) * 54)
                elements.Add(new CuiButton { Button = { Command = $"BSUIEditSpawns {RS(profile)} {page + 1}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.7 0.065", AnchorMax = $"0.8 0.095" }, Text = { Text = "->", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSSpawnsUI");

            elements.Add(new CuiButton { Button = { Command = $"CloseExtra BSSpawnsUI {RS(profile)} -1", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.4 0.065", AnchorMax = $"0.6 0.095" }, Text = { Text = "Back", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSSpawnsUI");
        ...
        void BSOverridesUI(BasePlayer player, string profile = "", int spawnpoint = 0)
        {
            CuiHelper.DestroyUi(player, "BSBSOverridesUI");
            var elements = new CuiElementContainer();
            elements.Add(new CuiPanel { Image = { Color = "0 0 0 0.96" }, RectTransform = { AnchorMin = "0.2 0.1", AnchorMax = "0.8 0.8" }, CursorEnabled = true }, "Overlay", "BSBSOverridesUI");
            elements.Add(new CuiLabel { Text = { Text = $"Overrides for spawn point {spawnpoint}", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter }, RectTransform = { AnchorMin = $"0 0.9", AnchorMax = $"1 1" } }, "BSBSOverridesUI");

            var s = spawnsData.CustomSpawnLocations[profile][spawnpoint];
            elements.Add(new CuiLabel { Text = { Text = $"Enable overrides", FontSize = 11, Font = Font, Align = TextAnchor.MiddleLeft }, RectTransform = { AnchorMin = $"0.4 0.76", AnchorMax = $"0.5 0.8" } }, "BSBSOverridesUI");
            elements.Add(new CuiButton { Button = { Command = $"BSChangeOverRides {RS(profile)} {spawnpoint} 0 true", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.5 0.76", AnchorMax = $"0.6 0.8" }, Text = { Text = s.UseOverrides.ToString(), FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSBSOverridesUI");

            if (s.UseOverrides)
            {
                elements.Add(new CuiLabel { Text = { Text = $"Stationary", FontSize = 11, Font = Font, Align = TextAnchor.MiddleLeft }, RectTransform = { AnchorMin = $"0.4 0.7", AnchorMax = $"0.5 0.74" } }, "BSBSOverridesUI");
                elements.Add(new CuiButton { Button = { Command = $"BSChangeOverRides {RS(profile)} {spawnpoint} 1 true", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.5 0.7", AnchorMax = $"0.6 0.74" }, Text = { Text = s.Stationary.ToString(), FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSBSOverridesUI");

                elements.Add(new CuiLabel { Text = { Text = $"Health", FontSize = 11, Font = Font, Align = TextAnchor.MiddleLeft }, RectTransform = { AnchorMin = $"0.4 0.64", AnchorMax = $"0.5 0.68" } }, "BSBSOverridesUI");
                elements.Add(new CuiButton { Button = { Command = $"BSChangeOverRides {RS(profile)} {spawnpoint} 2 false", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.5 0.64", AnchorMax = $"0.53 0.68" }, Text = { Text = "<", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSBSOverridesUI");
                elements.Add(new CuiLabel { Text = { Text = s.Health.ToString(), FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter }, RectTransform = { AnchorMin = $"0.54 0.64", AnchorMax = $"0.56 0.68" } }, "BSBSOverridesUI");
                elements.Add(new CuiButton { Button = { Command = $"BSChangeOverRides {RS(profile)} {spawnpoint} 2 true", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.57 0.64", AnchorMax = $"0.6 0.68" }, Text = { Text = ">", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSBSOverridesUI");

                elements.Add(new CuiLabel { Text = { Text = $"Roam range", FontSize = 11, Font = Font, Align = TextAnchor.MiddleLeft }, RectTransform = { AnchorMin = $"0.4 0.58", AnchorMax = $"0.5 0.62" } }, "BSBSOverridesUI");
                elements.Add(new CuiButton { Button = { Command = $"BSChangeOverRides {RS(profile)} {spawnpoint} 3 false", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.5 0.58", AnchorMax = $"0.53 0.62" }, Text = { Text = "<", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSBSOverridesUI");
                elements.Add(new CuiLabel { Text = { Text = s.RoamRange.ToString(), FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter }, RectTransform = { AnchorMin = $"0.54 0.58", AnchorMax = $"0.56 0.62" } }, "BSBSOverridesUI");
                elements.Add(new CuiButton { Button = { Command = $"BSChangeOverRides {RS(profile)} {spawnpoint} 3 true", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.57 0.58", AnchorMax = $"0.6 0.62" }, Text = { Text = ">", FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSBSOverridesUI");

                elements.Add(new CuiButton { Button = { Command = $"BSUIEditSPKits {RS(profile)} {spawnpoint} 0", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.4 0.52", AnchorMax = $"0.6 0.56" }, Text = { Text = "Kits", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSBSOverridesUI");
            }

            elements.Add(new CuiButton { Button = { Command = $"BSCloseOverrides", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.4 0.015", AnchorMax = $"0.6 0.05" }, Text = { Text = "Save", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSBSOverridesUI");
            CuiHelper.AddUi(player, elements);
        }
        ...
        void BSToPlayerUI(BasePlayer player, string profile)
        {
            CuiHelper.DestroyUi(player, "BSUIToPlayerSelect");
            var elements = new CuiElementContainer();
            elements.Add(new CuiPanel { Image = { Color = "0 0 0 0.98" }, RectTransform = { AnchorMin = "0.1 0.05", AnchorMax = "0.9 0.9" }, CursorEnabled = true }, "Overlay", "BSUIToPlayerSelect");
            elements.Add(new CuiLabel { Text = { Text = $"Spawn {profile} npcs near a player.", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter }, RectTransform = { AnchorMin = $"0 0.9", AnchorMax = $"1 1" } }, "BSUIToPlayerSelect");
            ...
            for (int i = 0; i < players.Count; i++)
            {
                ...
                bottom -= 0.0235f;
                if (odd && left == 0)
                    elements.Add(new CuiPanel { Image = { Color = $"0 0 0 0.8" }, RectTransform = { AnchorMin = $"0 {top}", AnchorMax = $"0.999 {bottom - 0.005}" }, CursorEnabled = true }, "BSUIToPlayerSelect");

                elements.Add(new CuiButton { Button = { Command = $"BSUIToPlayer {RS(profile)} {players[i].userID} ", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"{left + 0.02} {top}", AnchorMax = $"{left + 0.18} {bottom - 0.005}" }, Text = { Text = players[i].displayName, FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSUIToPlayerSelect");
            }
            elements.Add(new CuiButton { Button = { Command = $"BSUICloseToPlayer", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.4 0.015", AnchorMax = $"0.6 0.05" }, Text = { Text = "Close", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSUIToPlayerSelect");
            CuiHelper.AddUi(player, elements);
        }

        void BSManageListUI(BasePlayer player, string profile, string tab, string setting)
        {
            CuiHelper.DestroyUi(player, "BSManageListUI");

            var record = GetProfile(RD(profile));
            bool names = setting == "BotNames";
            var data = record.Spawn.BotNames;
            if (!names)
                data = record.Other.Instant_Death_From_Headshot_Allowed_Weapons;

            var elements = new CuiElementContainer();
            elements.Add(new CuiPanel { Image = { Color = "0 0 0 0.98" }, RectTransform = { AnchorMin = "0.1 0.05", AnchorMax = "0.9 0.9" }, CursorEnabled = true }, "Overlay", "BSManageListUI");
            if (names)
                elements.Add(new CuiLabel { Text = { Text = $"Manage bot names for {RD(profile)}.\nClick a name to remove it.", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter }, RectTransform = { AnchorMin = $"0 0.9", AnchorMax = $"1 1" } }, "BSManageListUI");
            else
                elements.Add(new CuiLabel { Text = { Text = $"Manage instant-death headshot weapons for {RD(profile)}.\nClick a weapon name to remove it.", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter }, RectTransform = { AnchorMin = $"0 0.9", AnchorMax = $"1 1" } }, "BSManageListUI");
            ...
            for (int i = 0; i < data.Count(); i++)
            {
                ...
                elements.Add(new CuiButton { Button = { Command = $"BSRemoveListEntry {RS(profile)} {tab} {setting} {data[i]}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"{left + 0.02} {top}", AnchorMax = $"{left + 0.18} {bottom - 0.005}" }, Text = { Text = data[i], FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSManageListUI");
            }

            elements.Add(new CuiButton { Button = { Color = "1 1 1 0.5" }, RectTransform = { AnchorMin = $"0.35 0.1", AnchorMax = $"0.65 0.2" } }, "BSManageListUI");
            elements.Add(new CuiElement
            {
                Name = CuiHelper.GetGuid(),
                Parent = "BSManageListUI",
                Components =
                {
                    new CuiInputFieldComponent { Align = TextAnchor.MiddleCenter, CharsLimit = 300, Command = $"BSAddListEntry {profile} {tab} {setting}", FontSize = 18, IsPassword = false, Text = $"Type {(names ? "":"weapon short")} names here to add", NeedsKeyboard = true, Autofocus = false, Color = "1 1 1 1" },
                    new CuiRectTransformComponent { AnchorMin = $"0.35 0.1", AnchorMax = $"0.65 0.2" }
                }
            });


            elements.Add(new CuiButton { Button = { Command = $"BSUICloseManageList", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.4 0.015", AnchorMax = $"0.6 0.05" }, Text = { Text = "Close", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSManageListUI");
            CuiHelper.AddUi(player, elements);
        }
        ...
        void BSShowParentsUI(BasePlayer player, string tab, string profile, string category)
        {
            CuiHelper.DestroyUi(player, "BSShowParentsUI");
            var elements = new CuiElementContainer();
            elements.Add(new CuiPanel { Image = { Color = "0 0 0 0.98" }, RectTransform = { AnchorMin = "0.1 0.05", AnchorMax = "0.9 0.9" }, CursorEnabled = true }, "Overlay", "BSShowParentsUI");
            elements.Add(new CuiLabel { Text = { Text = $"Select parent monument for {profile}.", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter }, RectTransform = { AnchorMin = $"0 0.9", AnchorMax = $"1 1" } }, "BSShowParentsUI");
            ...
            for (int i = 0; i < mons.Count(); i++)
            {
                ...
                elements.Add(new CuiButton { Button = { Command = $"BSSelectParent {tab} {RS(profile)} {RS(category)} {RS(mons[i].Key)} ", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"{left + 0.02} {top}", AnchorMax = $"{left + 0.18} {bottom - 0.005}" }, Text = { Text = mons[i].Key, FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSShowParentsUI");
            }
            elements.Add(new CuiButton { Button = { Command = $"BSUICloseParentMonument", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.4 0.015", AnchorMax = $"0.6 0.05" }, Text = { Text = "Close", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSShowParentsUI");
            CuiHelper.AddUi(player, elements);
        }
        ...
        void BSShowLootUI(BasePlayer player, string tab, string profile, string category)
        {
            CuiHelper.DestroyUi(player, "BSShowLootUI");
            var elements = new CuiElementContainer();
            elements.Add(new CuiPanel { Image = { Color = "0 0 0 0.98" }, RectTransform = { AnchorMin = "0.1 0.05", AnchorMax = "0.9 0.9" }, CursorEnabled = true }, "Overlay", "BSShowLootUI");
            elements.Add(new CuiLabel { Text = { Text = $"Select vanilla loot table for {profile}.", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter }, RectTransform = { AnchorMin = $"0 0.9", AnchorMax = $"1 1" } }, "BSShowLootUI");
            ...
            for (int i = 0; i < containers.Count(); i++)
            {
                ...
                elements.Add(new CuiButton { Button = { Command = $"BSSelectLoot {tab} {RS(profile)} {RS(category)} {RS(containers[i].Key)} ", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"{left + 0.02} {top}", AnchorMax = $"{left + 0.18} {bottom - 0.005}" }, Text = { Text = containers[i].Key, FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSShowLootUI");
            }
            elements.Add(new CuiButton { Button = { Command = $"BSUICloseLoot", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.4 0.015", AnchorMax = $"0.6 0.05" }, Text = { Text = "Close", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSShowLootUI");
            CuiHelper.AddUi(player, elements);
        }
        ...
        void BSTemplatesUI(BasePlayer player, string tab, string profile = "", string sub = "", int page = 1)
        {
            DestroyMenu(player, false);
            var elements = new CuiElementContainer();
            elements.Add(new CuiPanel { Image = { Color = "0 0 0 0" }, RectTransform = { AnchorMin = "0.1 0", AnchorMax = "0.9 0.9" }, CursorEnabled = true }, "Overlay", "BSTemplatesUI");
            elements.Add(new CuiLabel { Text = { Text = "Profile templates", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter }, RectTransform = { AnchorMin = $"0 0.9", AnchorMax = $"1 1" } }, "BSTemplatesUI");
            ...
            foreach (var entry in templateData.Templates)
            {
                ...
                top -= 0.023f;
                bottom -= 0.023f;
                elements.Add(new CuiButton { Button = { Command = $"BSUILoadTemplate {tab} {RS(profile)} {sub} {RS(entry.Key)} ", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"{left + 0.05} {top + 0.003}", AnchorMax = $"{left + 0.2} {bottom - 0.003}" }, Text = { Text = entry.Key, FontSize = 11, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSTemplatesUI");
                i++;
            }

            elements.Add(new CuiButton { Button = { Command = $"BSUI {tab} {RS(profile)} {sub}", Color = configData.UI.ButtonColour }, RectTransform = { AnchorMin = $"0.4 0.065", AnchorMax = $"0.6 0.095" }, Text = { Text = "Back", FontSize = 16, Font = Font, Align = TextAnchor.MiddleCenter } }, "BSTemplatesUI");
            CuiHelper.AddUi(player, elements);
        }
        #endregion
        ...
        void MsgUI(BasePlayer player, string message, float duration = 1.5f)
        {
            ...
            var elements = new CuiElementContainer();
            elements.Add(new CuiPanel { Image = { FadeIn = 0.3f, Color = $"0.1 0.1 0.1 0.9" }, RectTransform = { AnchorMin = "0.1 0.63", AnchorMax = "0.9 0.77" }, CursorEnabled = false, FadeOut = 0.3f }, "Overlay", "msgui");
            elements.Add(new CuiLabel { FadeOut = 0.5f, Text = { FadeIn = 0.5f, Text = message, Color = "1 1 1 1", FontSize = 26, Align = TextAnchor.MiddleCenter }, RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1" } }, "msgui");
        ...
        private void botrespawn(ConsoleSystem.Arg arg)
        {
            ...
            if (arg.Args.Length == 2)
            {
                ...
                if (arg.Args[0] == "tempspawn")
                {
                    ...
                    Puts($"Spawn temporary command received for profile {arg.Args[1]}.");
                }
        ...
        bool SpawnToPlayer(BasePlayer target, string profile, int num)
        {
            ...
            {
                CreateTempSpawnGroup(target, target.transform.position, entry.Key, entry.Value, null, num == -1 ? IsNight() ? entry.Value.Spawn.Night_Time_Spawn_Amount : entry.Value.Spawn.Day_Time_Spawn_Amount : num);
        ...
        private void BSChangeBool(ConsoleSystem.Arg arg)
        {
            ...
            if (arg.Args[3] == "AutoSpawn")
            {
                CheckAPCSafe();
                CheckGunTrapSafe();
            }

            if (arg.Args[3] == "APC_Safe")
                CheckAPCSafe();
            if (arg.Args[3] == "WeaponTrap_Safe")
                CheckGunTrapSafe();

            BSMainUI(arg.Player(), arg.Args[0], RD(arg.Args[1]), arg.Args[2], 0);
        }

        [ConsoleCommand("BSChangeString")]
        private void CmdChangeString(ConsoleSystem.Arg arg)
        {
            //// Check marker formatting
            //// and ensure no spaces in CustomLoot / Alphaloot

            string fulltext = "";
            if (arg.Args.Length >= 5 && (arg.Args[4] != null && arg.Args[4] != string.Empty))
                for (int i = 4; i < arg.Args.Length; i++)
                    fulltext += (i == 4 ? "" : " ") + arg.Args[i];
            ...
            var prop = subobj.GetType().GetField(RD(arg.Args[3]));
            var propobj = prop.GetValue(subobj);
            prop.SetValue(sub.GetValue(record), fulltext);
            sub.SetValue(record, subobj);

            if ((string)propobj == fulltext)
                return;
        ...
        [ConsoleCommand("BSUICloseManageList")] 
        private void BSUICloseManageList(ConsoleSystem.Arg arg)
        {
            if (arg.Player() == null)
                return;
            CuiHelper.DestroyUi(arg.Player(), "BSManageListUI");
        }

        [ConsoleCommand("BSAddListEntry")]
        private void BSAddListEntry(ConsoleSystem.Arg arg)
        {
            if (arg.Player() == null)
                return;
            string fulltext = "";
            for (int i = 3; i < arg.Args.Length; i++)
                fulltext += (i == 3 ? "" : " ") + arg.Args[i];

            var record = GetProfile(RD(arg.Args[0]));
            bool names = arg.Args[2] == "BotNames";
            var data = record.Spawn.BotNames;
            if (!names)
            {
                data = record.Other.Instant_Death_From_Headshot_Allowed_Weapons;
                if (!WeaponShortNames.Contains(fulltext))
                {
                    MsgUI(arg.Player(), String.Format(lang.GetMessage("NoWeaponShortName", this), fulltext));
                    return;
                }
            }
            ...
            CuiHelper.DestroyUi(arg.Player(), "BSManageListUI");
            BSManageListUI(arg.Player(), arg.Args[0], arg.Args[1], arg.Args[2]);
        }

        [ConsoleCommand("BSRemoveListEntry")]
        private void BSRemoveListEntry(ConsoleSystem.Arg arg) 
        {
            if (arg.Player() == null)
                return;
            string fulltext = "";
        ...
        [ConsoleCommand("BSManageList")]
        private void BSManageLIit(ConsoleSystem.Arg arg)
        {
            if (arg.Player() == null)
                return;
            BSManageListUI(arg.Player(), arg.Args[0], arg.Args[1], arg.Args[2]);
        ...
        private void BSUIRemoveProfile(ConsoleSystem.Arg arg)
        {
            if (arg.Player() == null)
                return;
        ...
        {
            if (NPCPlayers.ContainsKey(npc.userID))
                return NPCPlayers[npc.userID].profilename;
            return "No Name";
        }

        private List<Vector3> BotReSpawnLocations() => Profiles.Where(x => x.Value.Spawn.AutoSpawn).Select(x => x.Value.Other.Location).ToList<Vector3>();
        ...
        private string[] AddGroupSpawn(Vector3 location, string profileName, string group, int quantity)
        {
            ...
            foreach (var entry in bs.Profiles.Where(entry => entry.Key.ToLower() == profileName.ToLower() && IsSpawner(entry.Key)))
            {
                if (entry.Key.ToLower() == profileName.ToLower())
                {
                    ...
                    CreateTempSpawnGroup(null, location, entry.Key, entry.Value, group.ToLower(), quantity);
        ...
        private string[] RemoveGroupSpawn(string group)
        {
            ...
            List<global::HumanNPC> toDestroy = new List<global::HumanNPC>();
            bool flag = false;
        ...
        {
            //{ "Use_Base_Nav", "Experimental - This should be left true for the majority of users.\nIf you're seeing huge quantities of 'failed to create agent' spam in your server log you may want to set this to false\nbut be aware it can have unintended consequences, such as npcs walking on unexpected objects or areas, particularly on custom maps.\n\nIf you have either of these issues please let me know!"},
            ...
            { "Block_Console_Kill_Messages", "Disables console output X was killed by Y, where X is a BotReSpawn npc."},
            ...
            { "Animal_Safe", "Setting this to true will prevent BotReSpawn npcs from being damaged by animals which attack." },
            ...
            { "WeaponTrap_Safe", "Setting this to true will prevent turrets, shotgun traps, flame traps etc from targeting BotReSpawn npcs." },
            ...
            { "Disable_WaterWells", "Setting this to true will disable the profiles and hide them from the UI. A reload of the plugin is required."},
            { "Disable_Substations", "Setting this to true will disable the profiles and hide them from the UI. A reload of the plugin is required."},
            { "Disable_UndergroundCaves", "Setting this to true will disable the profiles and hide them from the UI. A reload of the plugin is required."},
            { "Disable_UnderwaterLabs", "Setting this to true will disable the profiles and hide them from the UI. A reload of the plugin is required."},
            ...
            { "BotNames", "Click 'Manage' to curate your list of names for the npcs in this profile.\n A name will be chosen randomly so you can enter a name more than once if you want it to be more common.\n\nIf the number of kits matches the number of names then the name is not chosen randomly.\nInstead the name will always be chosen to match the kit.\n\nkit one + name one, kit two and name two, etc." },
            ...
            { "Gender", "Use the Left/Right arrows to choose between Random, Male, or Female for npcs in this profile." },
            { "SkinTone", "Use the Left/Right arrows to choose between Random, Lightest, Light, Dark, Darkest skintone for npcs in this profile" },
            ...
            { "Bot_Take_Damage_Percent", "Adjust the % of damage that npcs take per hit. Decreasing is essentially the same as increasing health\nbut this option can be useful in making npcs' armour last longer."},
            ...
            { "Change_SpawnPoint_On_Death", "If this is set to true then an npc who has been killed will not respawn at its previous spawnpoint.\nInstead a new spawnpoint will be chosen when it respawns.\n\nFor cases where you're using custom spawn points, you should provide more than maximum npc population, otherwise randomly found points will be used." },
            ...
            { "Give_Scarecrow_Loadout", "Gives one of the four scarecrow loadouts instead of trying to give a kit." },
            ...
            { "Ignore_Wounded", "If this is set to true then npcs from this profile will not kill or attack wounded players." },
            ...
            { "NPCs_Defend_Against_Drones", "If this is true NPCs will return fire when attacked by drones" },
            ...
            { "Lock_Death_Crate_To_Killer_Player", "If this is true and Spawn Hackable Death Crate Percent is greater than zero,\nlocked-crates spawned upon the death of an npc will only be lootable by the player who killed the npc." },
            { "Corpse_Duration", "The length of time, in minutes, that the corpse of a dead npc from this profile will remain." },
            { "Corpse_Has_Skull", "If this is set to false then harvesting the corpse of an npc from this profile will not yeild a skull."},
            ...
            { "Remove_Vanilla_Scrap", "If this is set to true then scrap will be removed from any vanilla loot given to the npc."},
            { "AlphaLoot_Profile", "If Alphaloot is installed and a profile of this name exists withing it, BotReSpawn will call Alphaloot and give its npcs this profile of loot upon death." },
            ...
            { "Instant_Death_From_Headshot_Allowed_Weapons", "If Die_Instantly_From_Headshot is enabled, adding weapons here will mean only those weapons will be able to instant headshot.\nLeaving this list empty menas all weapons can instant kill the npcs by headshot."},
            ...
            { "Marker_Enabled", "If this is set to true a basic map marker will show on the in game map, to indicate a BotReSpawn profile location.\n\nIt does not show at the location of specific npcs - It shows at the location of the profile." },
            { "Marker_Visible_When_NPCs_Are_Dead", "By default the map marker does not show if all npcs from that profile are dead.\n\nSet this to true to show the marker regardless." },
...
//  Changes in 1.2.8
//  Fix for FP mountable changes (parachute), and Pooling changes.
...
//  Changes in 1.3.0
//  Added this != null in displaynamepatches
//  Added player.userID != 0 in Hostile check
//  added corpse steamid null check in CanPopulateLoot


//  Changes in 1.3.1
//  Added Give_Scarecrow_Loadout option


//  Changes in 1.3.2
//  Scarecrow names fix.
//  Prevented "Parent Monument option from showing for non-custom profiles.
//  Added per-profile marker control options.
...
//  Changes in 1.3.3
//  Onattacked error patch


//  Changes in 1.3.4
//  Updated for 2nd Jan game update.


//  Changes in 1.3.5
//  Patched for Feb '24 forced wipe.

//  Changes in V1.3.8
//  Removed `Ai_Weapon_Safe` and replaced with per profile `APC_Safe` and `WeaponTrap_Safe` due to unforeseen issues.
//  Added skintone and gender per profile.
//  Added global option `Block_Console_Kill_Messages` to suppress console 'was killed by' messages where victim is BotReSpawn npc.
//  Improved underground tunnel roam.

//  Bleed_Amount_Is_Percent_Of_Damage division patch.
//  Fixed shields not protecting players from npc damage.
//  Fixed gap in profile reload cooldown logic.
//  Fixed "Default NPC" setting for Rust loot not automatically getting Alphaloot version.  
//  Fixed inability to set empty text fields.
//  Dormancy fixes
//  TryHeal null checks.
//  Fixes for SeekerTarget related errors.
//  Fixed 1s timer in OnEntitySpawned - (corpse). 
//  Fixed unload/reload NRE 