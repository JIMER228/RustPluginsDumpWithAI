...
namespace Oxide.Plugins
{
	...
	public class Shop : RustPlugin
	{
		...
		private class Configuration
		{
			...
			[JsonProperty(PropertyName = LangRu ? "Разрешение для использования плагина (прим: shop.use)" : "Permission to use plugin (ex: shop.use)")]
		...
		public class SellContainers
		{
			...
			public int AllItems(BasePlayer player, List<Item> items)
			{
				...
				foreach (var itemContainer in GetContainers(player))
				{
					items.AddRange(itemContainer.itemList);
				}
						...
						switch (SortType)
						{
							...
							default:
								...
								return _sortedItems?.FindAll(item => player.HasPermission(item.Permission) && (!item.Currencies.Enabled ||
									item.CanBuy && item.Currencies.HasCurrency(true, selectedEconomy) || item.CanBeSold() &&
									item.Currencies.HasCurrency(false, selectedEconomy)));
						}
					}
				}
		...
		public class ShopItem
		{
			...
			{
				return !item.isBroken && item.IsSameItem(ShortName, Skin) && (!Genes.Enabled || Genes.IsSameGenes(item));
			}
			...
			if (_config.Version != default)
			{
				...
				if (_config.Version >= new VersionNumber(2, 0, 0))
				{
					...
					if (_config.Version < new VersionNumber(2, 1, 1))
					{
						...
						{
							if (economyEntry != null && string.IsNullOrEmpty(economyEntry.EconomyFooterPrice.Message) && !string.IsNullOrEmpty(economyEntry.EconomyPrice.Message))
							{
		...
		private void RegisterImagesFromUI(ref Dictionary<string,string> imagesList)
		{
			...
			{
				_uiData.FullscreenUI?.RegisterImages(ref imagesList);
			}
			
			if (_uiData.IsInMenuUISet)
			{
				_uiData.InMenuUI?.RegisterImages(ref imagesList);
			}
		}
		...
		public class ShopUI
		{
			...
			public void RegisterImages(ref Dictionary<string, string> imagesList)
			{
				RegisterImage(ShopBackground?.Background?.Image, ref imagesList);
				...
				switch (arg.Args[0])
				{
					...
					case "sellitem":
					{
						...
						try
						{
							...
							{
								var playerEconomy = GetPlayerEconomy(player);
								Log(LogType.Sell, LogSellItem, player.displayName, player.UserIDString, playerEconomy.GetPriceTitle(player, (item.GetSellPrice(player, selectedEconomy) * amount).ToString(_config.Formatting.SellPriceFormat)), item.ToString());
			...
			CuiElementContainer container)
		{
			...
			{				
				...
				if (IsAdmin(player) && shop.canShowAddItemButton)
		...
		private void BuyButtonUI(BasePlayer player, ref CuiElementContainer container, ShopItem shopItem, int selectedEconomy)
		{
			...
			if (cooldownTime > 0)
			{
				...
				container.Add(buttonTemplate.Cooldown.LeftTime.GetText("%TIME_LEFT%",
					Layer + $".Item.{shopItem.ID}.Buy", components: new CuiCountdownComponent
					{
						EndTime = 0,
						...
						DestroyIfDone = true,
						Command = $"{CMD_Main_Console} refresh_shop_item {shopItem.ID}"
					}));
			}
			else
			{
		...
		private void SellButtonUI(BasePlayer player, ref CuiElementContainer container, ShopItem shopItem, int selectedEconomy)
		{
			...
			if (cooldownTime > 0)
			{
				...
				container.Add(buttonTemplate.Cooldown.LeftTime.GetText("%TIME_LEFT%",
					Layer + $".Item.{shopItem.ID}.Sell", components: new CuiCountdownComponent
					{
						EndTime = 0,
						StartTime = cooldownTime,
						Step = 1,
		...
		{
			_enabledServerPanel = ServerPanel is {IsLoaded: true};
		...
		private string GetItemDisplayNameFromLangAPI(string shortName, string displayName, string userID)
		{
			if (Convert.ToBoolean(LangAPI?.Call("IsDefaultDisplayName", shortName)))
				return Convert.ToString(LangAPI.Call("GetItemDisplayName", shortName, displayName, userID)) ?? displayName;
		...
		private void TryBuyItems(BasePlayer player, bool again = false)
		{
			...
			if (items.Any(x =>
			    {
				    if (x.Key.BuyBlockDurationAfterWipe > 0)
				    {
					    var timeLeft = Mathf.RoundToInt(x.Key.BuyBlockDurationAfterWipe - secondsFromWipe);
					    if (timeLeft > 0)
					    {
						    ErrorUi(player,
							    Msg(player, BuyWipeCooldown,
		...
		{
			return _config.SellContainers.Enabled ? _config.SellContainers.AllItems(player, items) : player.inventory.GetAllItems(items);
		}
		...
		{
			return items.FindAll(item => item.IsSameItem(shortname, skin)).Sum(item => item.amount);
		}

		private static int ItemCount(List<Item> items, ShopItem shopItem)
		{
			return items.FindAll(shopItem.CanTake).Sum(item => item.amount);
		}
		...
		private CuiElementContainer API_OpenPlugin(BasePlayer player)
		{
			...
			if (_uiData is not {IsInMenuUISet: true})
			{
				...
				PrintError(msg);
				_config?.Notifications?.ShowNotify(player, player.IsAdmin ? msg : "The UI is currently unavailable. Please try again later.", 1);
				return container;
			}
			...
			container.Add(shopUI.ShopContent.Background.GetImage(Layer + ".Background", Layer + ".Main", Layer + ".Main"));
		...
		private class OpenedShop
		{
			...
			private void UpdateAvailableShowCategoriesMoveButtons()
			{
				...
				canShowCategoriesMoveButtons = !HasSearch() && shopCategory is {SortType: Configuration.SortType.None} and not {CategoryType: ShopCategory.Type.Favorite};
		...
		{
			_instance?.webrequest.Enqueue("https://gitlab.com/TheMevent/PluginsStorage/raw/main/ce7bd40bd7affedef6b91e0146f3d2ef_Codefling.json", null, (code, response) =>
			{
				if (code != 200)
				{
		...
		private void CmdChatShopInstaller(BasePlayer player, string command, string[] args)
		{
			...
			if (!IsAdmin(player))
			{
				...
				var adminMSG = $"Player {player.UserIDString} tried to run the installer, but he doesn't have {PermAdmin} permission";
			...
			{
				SendReply(player, $"Shop has already been installed! To run the installer again, you need to reset Shop. To reset, use the 'shop.reset' command (only console)!");
				return;
			}
						...
						case "list":
						{
                            ...
                            try
                            {
	                            ...
	                            foreach (var additionalEconomy in _config.AdditionalEconomics) 
		                            sb.AppendLine($"{additionalEconomy.ID}: {_config.Economy.GetDebugInfo()} ({(additionalEconomy.Enabled ? "Enabled" : "Disabled")})");
...
namespace Oxide.Plugins.ShopExtensionMethods
{
	...
	public static class ExtensionMethods
	{
		...
		{
			if (item == null || item.info == null || item.info.shortname != shortname || item.isBroken)
			{
				return false;
			}
