...
{
    [Info("extendedindustrial", "bmgjet", "1.2.0")]
    ...
    class extendedindustrial : RustPlugin
    {
        ...
        private BaseEntity FindEntity(Ray ray)
        {
            ...
            RaycastHit hit;
            var raycast = UnityEngine.Physics.Raycast(ray, out hit, 6, -1);
        ...
        private bool Attach(string pid, BaseEntity baseent, bool protect, bool checkonly)
        {
            ...
            if (baseent is PlanterBox && HasPermission(pid, permPlanterBox))
            {
                ...
                {
                    case 467313155: if (!checkonly) { SpawnPart(baseent, UnityEngine.Quaternion.Euler(0, 0, 0), new UnityEngine.Vector3(1.3f, .28f, .35f), protect); } return true; //planter.small
                    case 1162882237: if (!checkonly) { SpawnPart(baseent, UnityEngine.Quaternion.Euler(0, 0, 0), new UnityEngine.Vector3(1.3f, .28f, 1.28f), protect); } return true; //planter.large
                    case 47518702: if (!checkonly) { SpawnPart(baseent, UnityEngine.Quaternion.Euler(90, 180, 0), new UnityEngine.Vector3(0f, .66f, -.49f), protect); } return true; //minecart
                    case 115096413: if (!checkonly) { SpawnPart(baseent, UnityEngine.Quaternion.Euler(0, 0, 0), new UnityEngine.Vector3(1.3f, .28f, 1.28f), protect); } return true; //railroadplanter
                    case 2846319393: if (!checkonly) { SpawnPart(baseent, UnityEngine.Quaternion.Euler(0, 0, 0), new UnityEngine.Vector3(0f, .66f, -.49f), protect); } return true; //bathtub
                }
            }
            else if (baseent is StashContainer && HasPermission(pid, permStash)) { if (!checkonly) { SpawnPart(baseent, UnityEngine.Quaternion.Euler(0, 0, 0), new UnityEngine.Vector3(0f, 0f, 0f), protect); } baseent.skinID = 1111111111; return true; }
            ...
            else if (baseent is SnowMachine && HasPermission(pid, permSnowMachine)) { if (!checkonly) { SpawnPart(baseent, UnityEngine.Quaternion.Euler(0, 90, 0), new UnityEngine.Vector3(0f, 0f, -.5f), protect); } return true; }
            ...
            else if (baseent is SamSite && HasPermission(pid, permSamSite)) { if (!checkonly) { SpawnPart(baseent, UnityEngine.Quaternion.Euler(90, 90, 180), new UnityEngine.Vector3(-.90f, .48f, 0f), protect); } return true; }
            else if (baseent is FuelGenerator && HasPermission(pid, permFuelGenerator)) { if (!checkonly) { SpawnPart(baseent, UnityEngine.Quaternion.Euler(0, 0, 90), new UnityEngine.Vector3(0f, .52f, 0f), protect); } return true; }
            else if (baseent is AutoTurret && HasPermission(pid, permAutoTurret)) { if (!checkonly) { SpawnPart(baseent, UnityEngine.Quaternion.Euler(0, 0, 180), new UnityEngine.Vector3(0f, .20f, 0f), protect); } return true; }
            else if (baseent is DropBox && HasPermission(pid, permDropBox)) { if (!checkonly) { SpawnPart(baseent, UnityEngine.Quaternion.Euler(270, 0, 0), new UnityEngine.Vector3(0f, -.22f, -.31f), protect); } return true; }
            ...
            else if (baseent is HitchTrough && HasPermission(pid, permHitchTrough)) { if (!checkonly) { SpawnPart(baseent, UnityEngine.Quaternion.Euler(270, 90, 180), new UnityEngine.Vector3(1.02f, .48f, .11f), protect); } return true; }
            else if (baseent is Recycler && HasPermission(pid, permRecycler)) { if (!checkonly) { SpawnPart(baseent, UnityEngine.Quaternion.Euler(0, 0, 180), new UnityEngine.Vector3(-0.25f, .42f, 0.1f), protect); } return true; }
            else if (baseent is MixingTable && HasPermission(pid, permMixingTable)) { if (!checkonly) { SpawnPart(baseent, UnityEngine.Quaternion.Euler(0, 0, 180), new UnityEngine.Vector3(0f, .82f, .1f), protect); } return true; }
            else if (baseent is BaseOven)
            {
                switch (baseent.prefabID)
                {
                    case 2013224025: //Legecy furnace
                    case 2931042549: if (HasPermission(pid, permFurnace)) { if (!checkonly) { SpawnPart(baseent, UnityEngine.Quaternion.Euler(270, 180, 0), new UnityEngine.Vector3(-0.05f, 0.15f, 0.5f), protect); } return true; } break; //furnace
                    case 2409469892: if (HasPermission(pid, permBBQ)) { if (!checkonly) { SpawnPart(baseent, UnityEngine.Quaternion.Euler(0, 90, 180), new UnityEngine.Vector3(0f, .62f, -.3f), protect); } return true; } break; //BBQ
                    case 1392608348: if (HasPermission(pid, permLight)) { if (!checkonly) { SpawnPart(baseent, UnityEngine.Quaternion.Euler(270, 180, 0), new UnityEngine.Vector3(0f, .0f, .1f), protect); } return true; } break; //TunaLight
                    case 1748062128: if (HasPermission(pid, permHobo)) { if (!checkonly) { SpawnPart(baseent, UnityEngine.Quaternion.Euler(270, 180, 0), new UnityEngine.Vector3(0f, .2f, .3f), protect); } return true; } break; //Hobo
                    case 1906669538: if (HasPermission(pid, permFirePit)) { if (!checkonly) { SpawnPart(baseent, UnityEngine.Quaternion.Euler(0, 0, 0), new UnityEngine.Vector3(0f, .0f, .6f), protect); } return true; } break; //skullfirepit
                    case 1348425051: if (HasPermission(pid, permFirePit)) { if (!checkonly) { SpawnPart(baseent, UnityEngine.Quaternion.Euler(0, 0, 0), new UnityEngine.Vector3(0f, 0f, .6f), protect); } return true; } break; //cursedcauldron
                    case 4160694184: if (HasPermission(pid, permCampfire)) { if (!checkonly) { SpawnPart(baseent, UnityEngine.Quaternion.Euler(0, 0, 0), new UnityEngine.Vector3(0f, 0f, .5f), protect); } return true; } break; //campfire
                    case 110576239: if (HasPermission(pid, permFireplace)) { if (!checkonly) { SpawnPart(baseent, UnityEngine.Quaternion.Euler(270, 180, 0), new UnityEngine.Vector3(0f, .28f, .45f), protect); } return true; } break; //fireplace
                }
            }
            else if (baseent is SurvivalFishTrap && HasPermission(pid, permFishTrap))
            {
                if (!checkonly) { SpawnPart(baseent, UnityEngine.Quaternion.Euler(90, 270, 0), new UnityEngine.Vector3(-0.79f, .2f, .42f), protect); }
                return true;
            }
        ...
        private Vector2i SlotsMod(BaseEntity entity, ItemContainer itemContainer, Vector2i defaultvalue, bool input)
        {
        ...
        public static class IndustrialStorageAdaptor_InputSlotRange
        {
            ...
            public static void Postfix(ref Vector2i __result, IndustrialStorageAdaptor __instance)
            {
                try
                {
                    if (__instance.cachedContainer != null && __instance.cachedParent != null)
                    {
                        __result = plugin.SlotsMod(__instance.cachedParent, __instance.cachedContainer, __result, true);
                    }
                }
                catch { }
            }
        ...
        public static class IndustrialStorageAdaptor_OutputSlotRange
        {
            ...
            public static void Postfix(ref Vector2i __result, IndustrialStorageAdaptor __instance)
            {
                try
                {
                    if (__instance.cachedContainer != null && __instance.cachedParent != null)
                    {
                        __result = plugin.SlotsMod(__instance.cachedParent, __instance.cachedContainer, __result, false);
                    }
                }
                catch { }
            }
