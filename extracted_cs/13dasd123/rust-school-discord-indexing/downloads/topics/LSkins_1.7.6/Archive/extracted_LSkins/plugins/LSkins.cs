...
[Info("LSkins", "https://discord.gg/TrJ7jnS233", "1.7.6")]
public class LSkins : RustPlugin
{
        ...
        for (var i = 0; i < 12; i++)
        {
            ...
            cont.Add(new CuiElement()
            {
                ...
                Components =
                {
                    ...
                    {
                        Png = GetImage(weapon + "_" + tSkinsList.ToList()[i].Key)
                    },
                     ...
                     .Select((i, t) => new { A = i, B = t - (page - 1) * 12 }).Skip((page - 1) * 12).Take(12))
        {
            ...
            cont.Add(new CuiElement()
            {
                ...
                Components =
                {
                    new CuiRawImageComponent()
                    {
                        Png = GetImage(weapon + "_" + skinItem.A.Key)
                    },
                    new CuiRectTransformComponent()
                    {
                        AnchorMin = "0.5 0.5",
    ...
    private bool ChangeSkin(BasePlayer player)
    {
        ...
        switch (Cfg.Market.WhoMarket)
        {
            case "Scrap":
            {
                var scrap = Enumerable.Concat(player.inventory.containerMain?.itemList ?? Enumerable.Empty<Item>(), Enumerable.Concat(player.inventory.containerBelt?.itemList ?? Enumerable.Empty<Item>(), player.inventory.containerWear?.itemList ?? Enumerable.Empty<Item>())).FirstOrDefault(p =>
    ...
    private void LoadDataSkins()
    {
        foreach (var cfgWeaponSkin in Interface.Oxide.DataFileSystem.GetFiles("LSkins/Skins/"))
        {
            ...
            {
                if (ImageLibrary.Call<bool>("HasImage", $"{text2}_{keyValuePair.Key}", 0ul) ||
    ...
    private void OnServerInitialized()
    {
        ...
        if (Cfg.IsHazmat)
        {
            ...
            AddImage($"https://rustapi.top/items/{_apiKey}/hazmatsuit/128.png", $"hazmatsuit_0");
            AddImage($"https://rustapi.top/items/{_apiKey}/hazmatsuit.spacesuit/128.png",
                $"hazmatsuit_{1}");
            AddImage($"https://rustapi.top/items/{_apiKey}/hazmatsuit.nomadsuit/128.png",
                $"hazmatsuit_{2}");
