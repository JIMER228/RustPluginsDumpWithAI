...
/* 1.0.19
 * Fixed issue with enhancement kits not being recognised.
 * Added config option to disable chat message for UncannyDodge.
 * 
 */

namespace Oxide.Plugins
{
    [Info("Item Perks", "imthenewguy", "1.0.19")]
    ...
    class ItemPerks : RustPlugin
    {
        ...
        {
            [JsonProperty("Allow the plugin to handle stacking of Item Perk items? [set to false if using StackModifier]")]
            public bool handleStacking = true;

            [JsonProperty("Allow the plugin to handle splitting of Item Perk items? [set to false if using StackModifier]")]
        ...
        public class ThirdpartyPluginSettings
        {
            ...
            {
                [JsonProperty("Allow max repair perk from Skill Tree to be used on enhanced items?")]
        ...
        protected override void LoadDefaultConfig()
        {
            ...
            config.enhancementSettings.lootSettings.corpse = DefaultCorpses;
            ...
            config.enhancementSettings.enhancement_kit_settings.prefab_menu = new List<string>() { "furnace", "legacy_furnace" };
        }
        ...
        Dictionary<string, float> DefaultCorpses
        {
            get
            {
                return new Dictionary<string, float>()
                {
                    ["scientistnpc_heavy"] = 15,
                    ["scientistnpc_cargo"] = 10,
                    ["scientistnpc_cargo_turret_any"] = 10,
                    ["scientistnpc_cargo_turret_lr300"] = 10,
                    ["scientistnpc_ch47_gunner"] = 10,
                    ["scientistnpc_excavator"] = 10,
                    ["scientistnpc_full_any"] = 10,
                    ["scientistnpc_full_lr300"] = 10,
                    ["scientistnpc_full_mp5"] = 10,
                    ["scientistnpc_full_pistol"] = 10,
                    ["scientistnpc_full_shotgun"] = 10,
                    ["scientistnpc_junkpile_pistol"] = 10,
                    ["scientistnpc_oilrig"] = 10,
                    ["scientistnpc_patrol"] = 10,
                    ["scientistnpc_peacekeeper"] = 10,
                    ["scientistnpc_roam"] = 10,
                    ["scientistnpc_roamtethered"] = 10
                };
            }
        }
        ...
        public class PlayerSettings
        {
            ...
            [JsonProperty("Enhance item button position")]
            public AnchorSettings enhance_button_settings = new AnchorSettings("0.5 0", "0.5 0", "192.7 81", "294.7 106");
            ...
            [JsonProperty("Settings for chat messages")]
            public ChatNotificationSettings chatMessageSettings = new ChatNotificationSettings();
        }

        public class ChatNotificationSettings
        {
            [JsonProperty("Send a message when a players regen is disabled?")]
            public bool regen_disabled = true;
            
            [JsonProperty("Send a message when the fishing luck perk triggers?")]
            public bool fishing_luck = true;


            [JsonProperty("Send a message when the UncannyDodge perk triggers?")]
        ...
        public class EnhancementKitSettings
        {
            ...
            [JsonProperty("List of prefab shortnames that will show the enhancement menu when selected")]
            public List<string> prefab_menu = new List<string>();
        ...
        public class LootSettings
        {
            ...
            [JsonProperty("Chance for a player to receive an enhanced item when looting a corpse [out of 100]")]
            public Dictionary<string, float> corpse;
        ...
        public class PerkConfigs
        {
            ...
            [JsonProperty("Settings for RockCycler perk")]
            public RockCyclerSettings RockCyclerSettings = new RockCyclerSettings();
        }

        #region Perk configs     

        public class RockCyclerSettings
        {
            [JsonProperty("Distance in front of player for nodes to spawn when the buff procs?")]
            public float spawn_dist = 2f;
        }
        ...
        {
            [JsonProperty("Delay after receiving damage before the regen buff buff continues to heal [set to 0 to ignore damage]")]
        ...
        Dictionary<ulong, PlayerPerks> Player_perks = new Dictionary<ulong, PlayerPerks>();
        ...
        void Init()
        {
            ...
            if (!config.miscSettings.handleSplitting) Unsubscribe(nameof(OnItemSplit));
        }
        ...
        void Unload()
        {
            ...
            Pool.FreeUnmanaged(ref PerksList);
        ...
        void OnServerInitialized(bool initial)
        {
            ...
            if (config.enhancementSettings.lootSettings.corpse == null || config.enhancementSettings.lootSettings.corpse.Count == 0)
            {
                config.enhancementSettings.lootSettings.corpse = DefaultCorpses;
                DoSave = true;
            }
            ...
            List<Perk> perks = Pool.Get<List<Perk>>();
            ...
            if (config.enhancementSettings.enhancement_kit_settings.prefab_menu.Count == 0)
            {
                config.enhancementSettings.enhancement_kit_settings.prefab_menu = new List<string>() { "furnace", "legacy_furnace" };
                DoSave = true;
            }
        ...
        void OnItemSkinChange(int num, Item item, RepairBench bench, BasePlayer player)
        {
            if (item.text == null || !HasPerks(item.text)) return;
            ...
            NextTick(() =>
            {
                if (bench != null && bench.inventory != null && bench.inventory.itemList != null && bench.inventory.itemList.Count > 0)
                {
                    ...
                    if (!string.IsNullOrEmpty(name)) _item.name = name;
                    _item.text = text;
                    _item.MarkDirty();
                }
            });
        ...
        void OnFishCatch(Item item, BaseFishingRod rod, BasePlayer player)
        {
            ...
            if (RollSuccessful(value))
            {
                ...
                if (comp != null)
                {
                    ...
                    if (config.playerSettings.chatMessageSettings.fishing_luck) PrintToChat(player, message);
        ...
        object OnPlayerHealthChange(BasePlayer player, float oldValue, float newValue)
        {
            ...
            {                
                ...
                Unsubscribe(nameof(OnPlayerHealthChange));
                player._health += additionalHealth;
                player.SendNetworkUpdate();
            ...
            if (perks.TryGetValue(Perk.HealShare, out value))
            {
                ...
                List<BasePlayer> teamMembers = Pool.Get<List<BasePlayer>>();
                ...
                foreach (var hit in teamMembers)
                {
                    ...
                    if (!config.enhancementSettings.perk_config.healShareSettings.team_only)
                    {
                        //hit.Heal(healthToShare);
                        hit._health += healthToShare;
                        hit.SendNetworkUpdate();
                    }
                    else if (hit.Team != null && player.Team != null && hit.Team != null && player.Team.teamID == hit.Team.teamID)
                    {
                        //hit.Heal(healthToShare);
                        hit._health += healthToShare;
                        hit.SendNetworkUpdate();
        ...
        void OnPlayerDeath(BasePlayer player, HitInfo info)
        {
            ...
            {
                if (player is global::HumanNPC && !LootedCorpses.ContainsKey(player.userID)) LootedCorpses.Add(player.userID, new LootedCorpse(player as global::HumanNPC));
                return;
            }
        ...
        public Vector3 GetPositionInLookDirection(Vector3 playerPosition, Vector3 playerForward, float dist)
        {
            if (dist == 0) return playerPosition;
            Vector3 offset = playerForward.normalized * dist;
        ...
        void HandleDispenser(ResourceDispenser dispenser, BasePlayer player, Item item, bool finalHit)
        {
            ...
            if (dispenser.gatherType == ResourceDispenser.GatherType.Tree)
            {
                ...
                if (finalHit && perks.TryGetValue(Perk.TreePlanter, out value) && RollSuccessful(value))
                {
                    RespawnResource(dispenser.baseEntity.PrefabName, dispenser.baseEntity.transform.position, dispenser.baseEntity.transform.rotation, player.eyes.MovementForward(), false);
                }
                if (!DoingClearOut && finalHit && perks.TryGetValue(Perk.Deforest, out value) && RollSuccessful(value))
                {
                    ...
                    List<ResourceDispenser> dispensers = Pool.Get<List<ResourceDispenser>>();
                    ...
                    Unsubscribe(nameof(STCanGainXP));
                    Pool.FreeUnmanaged(ref dispensers);
                    DoingClearOut = false;
                }
            ...
            else if (dispenser.gatherType == ResourceDispenser.GatherType.Ore)
            {
                ...
                if (finalHit && perks.TryGetValue(Perk.RockCycler, out value) && RollSuccessful(value)) RespawnResource(dispenser.baseEntity.PrefabName, dispenser.baseEntity.transform.position, dispenser.baseEntity.transform.rotation, Vector3.zero, true);
        ...
        Item CreateRandomComponent(BasePlayer player, Perk perk)
        {
            ...
            List<KeyValuePair<string, DropInfo>> keys = Pool.Get<List<KeyValuePair<string, DropInfo>>>();
            keys.AddRange(dict);
            ...
            if (result != null)
            {
                if (!string.IsNullOrEmpty(chosen.Value.display_name)) result.name = chosen.Value.display_name;
                result.MarkDirty();
            }
        ...
        void RespawnResource(string prefab, Vector3 pos, Quaternion rot, Vector3 eyesDir, bool isRock)
        {
            var spawnPos = isRock ? GetPositionInLookDirection(pos, eyesDir, config.enhancementSettings.perk_config.RockCyclerSettings.spawn_dist) : pos;
            NextTick(() =>
            {
                NextTick(() =>
                {
        ...
        static bool IsEnhancementKit(Item item)
        {
            return item.skin == Instance.config.enhancementSettings.enhancement_kit_settings.skin;
        }
        ...
        void OnEntityTakeDamage(BasePlayer victim, HitInfo info)
        {
            ...
            if (defenderPerks != null && defenderPerks.TryGetValue(Perk.UncannyDodge, out value) && RollSuccessful(value))
            {
                ...
                if (config.playerSettings.chatMessageSettings.dodge) PrintToChat(victim, message);
                modifier = 0;
            }
        ...
        void OnEntityTakeDamage(LootContainer entity, HitInfo info)
        {
            ...
            if (entityWillDie && entity.inventory != null)
            {
        ...
        void OnEntityKill(LootContainer entity)
        {
            ...
            LootedContainers.Remove(entity);
        }

        void OnEntityKill(LootableCorpse entity)
        {
            if (entity == null) return;
            LootedCorpses.Remove(entity.playerSteamID);
        }
        ...
        class LootedCorpse
        {
            ...
            public LootedCorpse(global::HumanNPC npc)
            {
                this.npc = npc;
            }
        }

        Dictionary<ulong, LootedCorpse> LootedCorpses = new Dictionary<ulong, LootedCorpse>();
        
        void CanLootEntity(BasePlayer player, LootableCorpse corpse)
        {
            if (player == null || corpse == null) return;

            LootedCorpse corpseData;
            if (!LootedCorpses.TryGetValue(corpse.playerSteamID, out corpseData) || corpseData.looted) return;
            corpseData.looted = true;
            //corpse.lootde
            
            //LootableCorpse.Containers [0= Main, 1= Worn, 2= Belt]
            HandleItemRoll(player, corpse, corpseData.npc);
        }

        List<BasePlayer> workbench_looters = new List<BasePlayer>();
        void OnLootEntity(BasePlayer player, StorageContainer entity)
        {
            if (!config.enhancementSettings.enhancement_kit_settings.prefab_menu.Contains(entity.ShortPrefabName)) return;
        ...
        {
            if (!workbench_looters.Contains(player)) return;
        ...
        void HandleItemRoll(BasePlayer player, LootContainer container)
        {
            ...
            {
                lootProfile = Pool.Get<List<string>>();
                ...
                {
                    Pool.FreeUnmanaged(ref lootProfile);
                    return;
                }
        ...
        void HandleItemRoll(BasePlayer player, LootableCorpse corpse, global::HumanNPC npc)
        {
            if (!permission.UserHasPermission(player.UserIDString, perm_loot) || corpse == null) return;

            float target;
            if (!config.enhancementSettings.lootSettings.corpse.TryGetValue(npc.ShortPrefabName, out target) || target == 0) return;
            ...
            if (!config.enhancementSettings.lootSettings.use_default_loot_profiles)
            {
                RollRandomItem(player, corpse);
                return;
            }

            List<string> lootProfile;

            if (!LootTables.TryGetValue(npc.ShortPrefabName, out lootProfile) || lootProfile == null)
            {
                lootProfile = Pool.Get<List<string>>();
                int i = 0;
                foreach (var slot in npc.LootSpawnSlots)
                {
                    if (slot.definition != null) lootProfile.AddRange(GetLootFromTable(slot.definition));
                    i++;
                    if (lootProfile.Count == 0)
                    {
                        Pool.FreeUnmanaged(ref lootProfile);
                ...
                AddTable(npc.ShortPrefabName, lootProfile);
            }
            List<string> profile;
            if (!LootTables.TryGetValue(npc.ShortPrefabName, out profile)) return;

            string randomShortname = profile.GetRandom();
            var item = CreateItem(randomShortname);            
            if (item != null) AddEnhancedItemToContainer(player, item, corpse);
        }
        ...
        void RollRandomItem(BasePlayer player, LootableCorpse corpse)
        {
            var item = CreateItem();
            if (item != null) AddEnhancedItemToContainer(player, item, corpse);
        }

        void AddEnhancedItemToContainer(BasePlayer player, Item item, LootableCorpse corpse)
        {
            foreach (var container in corpse.containers)
            {
                if (container.IsFull()) continue;
                if (!item.MoveToContainer(container)) continue;
                NotifyPlayer(player, item);
                return;
            }
        ...
        void AddTable(string prefab, List<string> profile)
        {
            ...
            Pool.FreeUnmanaged(ref profile);
        }
        ...
        void NotifyPlayer(BasePlayer player, Item item, bool found = true)
        {
            if (config.playerSettings.uinotify_settings.enabled && UINotify != null && UINotify.IsLoaded && permission.UserHasPermission(player.UserIDString, "uinotify.see") && ((config.playerSettings.uinotify_settings.notify_Crafted && false) || (config.playerSettings.uinotify_settings.notify_ItemDrops && true)))
                UINotify.Call("SendNotify", player.userID.Get(), config.playerSettings.uinotify_settings.messageType, lang.GetMessage("Notify_FirstFindLoot", this, player.UserIDString));
        ...
        void SendUINotify(BasePlayer player, string message)
        {
            ...
            UINotify.Call("SendNotify", player.userID.Get(), config.playerSettings.uinotify_settings.messageType, message);
        }
        ...
        void GetPlayerPerks(BasePlayer player)
        {
            ...
            {
                if (item.text == null || item.isBroken) continue;
                var perks = GetMods(item.text);
                if (perks != null)
                {
                    foreach (var perk in perks)
            ...
            if (config.playerSettings.only_show_when_equipped)
            {
                ...
                {
                    if (activeItem == null || string.IsNullOrEmpty(activeItem.text) || activeItem.isBroken || !HasPerks(activeItem.text))
                        CuiHelper.DestroyUi(player, "InspectorButtonImg");
                    else SendInspectMenuButton(player);
                }
                else SendInspectMenuButton(player);
            }
            List<Perk> subscriptionCheck = Pool.Get<List<Perk>>();
            ...
            Pool.FreeUnmanaged(ref subscriptionCheck);
        }
        ...
        void HandleDispenserClearOut(BasePlayer player, ResourceDispenser dispenser)
        {
            foreach (var _item in dispenser.containedItems)
            {
                ...
                var item = ItemManager.CreateByItemID(_item.itemid, Convert.ToInt32(_item.amount + (_item.amount * CalculateTeaBonus(player, dispenser))));
                _item.amount = 0;
                HandleDispenser(dispenser, player, item, false);
                item.MarkDirty();
                player.GiveItem(item);
        ...
        float CalculateTeaBonus(BasePlayer player, ResourceDispenser dispenser)
        {
            if (player == null || player.modifiers.All.Count == 0) return 0;
            switch (dispenser.gatherType)
            {
                case ResourceDispenser.GatherType.Tree:
                    return player.modifiers.GetValue(Modifier.ModifierType.Wood_Yield);

                case ResourceDispenser.GatherType.Ore:
                    return player.modifiers.GetValue(Modifier.ModifierType.Ore_Yield);
        ...
        {
            bool WhitelistPopulated = config.enhancementSettings.lootSettings.enhanceable_whitelist?.Count > 0;
            foreach (var itemDef in ItemManager.GetItemDefinitions())
            {
                ...
                if (itemDef.category == ItemCategory.Attire && itemDef.isWearable && !itemDef.shortname.StartsWith("frankensteins.monster") && !EnhanceableItems.Contains(itemDef.shortname) && (WhitelistPopulated ? config.enhancementSettings.lootSettings.enhanceable_whitelist.Contains(itemDef.shortname) : !config.enhancementSettings.lootSettings.enhanceable_blacklist.Contains(itemDef.shortname)))
                {
                    ItemIDs.Add(itemDef.shortname, itemDef.itemid);
                    EnhanceableItems.Add(itemDef.shortname);
                }

                if ((itemDef.category == ItemCategory.Weapon || itemDef.category == ItemCategory.Tool) && itemDef.isHoldable && (itemDef.occupySlots == ItemSlot.None || itemDef.occupySlots == 0) && !EnhanceableItems.Contains(itemDef.shortname) && (WhitelistPopulated ? config.enhancementSettings.lootSettings.enhanceable_whitelist.Contains(itemDef.shortname) : !config.enhancementSettings.lootSettings.enhanceable_blacklist.Contains(itemDef.shortname)))
                {
                    ItemIDs.Add(itemDef.shortname, itemDef.itemid);
                    EnhanceableItems.Add(itemDef.shortname);
                }
        ...
        [HookMethod("IPAPI_GetDropChanceModifier")]
        float IPAPI_GetDropChanceModifier(string userid)
        {
            var result = 0f;
            foreach (var kvp in config.enhancementSettings.chance_modifier_settings.loot_chance_permissions)
            {
                if (permission.UserHasPermission(userid, kvp.Key) && result < kvp.Value) result = kvp.Value;
            }
            return result;
        }
        ...
        {
            List<KeyValuePair<Perk, float>> perks = Pool.Get<List<KeyValuePair<Perk, float>>>();
        ...
        string GetShortnameForPerk(List<Perk> perks)
        {
            ...
            List<string> whitelistShortnames = Pool.Get<List<string>>();
            List<string> blacklistShortnames = Pool.Get<List<string>>();
            foreach (var perk in perks)
            {
            ...
            {
                List<string> validshortnames = Pool.Get<List<string>>();
                validshortnames.AddRange(EnhanceableItems.Where(x => !blacklistShortnames.Contains(x)));
                shortname = validshortnames.GetRandom();
                Pool.FreeUnmanaged(ref validshortnames);
            }
            else shortname = EnhanceableItems.GetRandom();

            Pool.FreeUnmanaged(ref whitelistShortnames);
            Pool.FreeUnmanaged(ref blacklistShortnames);
        ...
        Item CreateItem(string shortname = null, ulong skin = 0, List<KeyValuePair<Perk, float>> perks = null)
        {
            ...
            {
                List<Perk> _perks = Pool.Get<List<Perk>>();
            ...
            {
                if (perks == null) perks = Pool.Get<List<KeyValuePair<Perk, float>>>();
            ...
            if (handleListClear) Pool.FreeUnmanaged(ref perks);           
        ...
        Perk RollPerk(string shortname, List<KeyValuePair<Perk, float>> existing_perks)
        {
            ...
            List<KeyValuePair<Perk, PerkSettings>> perks = Pool.Get<List<KeyValuePair<Perk, PerkSettings>>>();
            List<Perk> exclusions = Pool.Get<List<Perk>>();
            ...
            {
                List<KeyValuePair<Perk, PerkSettings>> duplicateRemove = Pool.Get<List<KeyValuePair<Perk, PerkSettings>>>();
                ...
                Pool.FreeUnmanaged(ref duplicateRemove);
            }
        ...
        Perk GetRandomPerkIgnoreCriteria(List<KeyValuePair<Perk, float>> existing_perks)
        {
            ...
            List<string> perkstrings = Pool.Get<List<string>>();
            if (existing_perks != null && existing_perks.Count > 0)
            {
                forbiddenPerks = Pool.Get<List<string>>();
            ...
            Pool.FreeUnmanaged(ref perkstrings);
        ...
        private void SendWorkbenchButton(BasePlayer player)
        {
            ...
            container.Add(new CuiPanel
            {
                ...
                RectTransform = { AnchorMin = config.playerSettings.enhance_button_settings.AnchorMin, AnchorMax = config.playerSettings.enhance_button_settings.AnchorMax, OffsetMin = config.playerSettings.enhance_button_settings.OffsetMin, OffsetMax = config.playerSettings.enhance_button_settings.OffsetMax }
        ...
        private void EnhancementMenu(BasePlayer player)
        {
            ...
            foreach (var item in allItems)
            {
                ...
                if (IsEnhancementKit(item)) items.Add(item);
            }
            ...
            else
            {
                ...
                foreach (var item in items)
                {
                    ...
                    container.Add(new CuiButton
                    {
                        ...
                        Text = { Text = string.Format(lang.GetMessage("UIEnhancementChooseKitTitle", this, player.UserIDString), item.name.Split(':').Last()), Font = "robotocondensed-bold.ttf", FontSize = 16, Align = TextAnchor.MiddleCenter, Color = "0 0 0 1" },
        ...
        private void ItemSelectMenu(BasePlayer player, ulong perk_uid)
        {
            ...
            var perk = GetEnhancementKitPerk(perkItem);
        ...
        {
            if (!EnhanceableItems.Contains(item.info.shortname) || config.enhancementSettings.lootSettings.enhanceable_blacklist.Contains(item.info.shortname) || (config.enhancementSettings.lootSettings.enhanceable_whitelist?.Count > 0 && !config.enhancementSettings.lootSettings.enhanceable_whitelist.Contains(item.info.shortname))) return false;
            if (string.IsNullOrEmpty(item.text)) return true;
            if (item.text.Contains("[named]")) return false;
            ...
            if (config.enhancementSettings.enhancement_kit_settings.perks_capped)
            {
                ...
                foreach (var kvp in perks)
                {
                    if (config.enhancementSettings.add_duplicates_to_buff_count) perkCount += kvp.Value;
                    else if (kvp.Key != perk) perkCount++;
                }
                    ...
                    switch (config.enhancementSettings.enhancement_kit_settings.upgrade_settings.upgrade_cost_type)
                    {
                        ...
                        case "serverrewards":
                            if (ServerRewards != null && ServerRewards.IsLoaded)
                            {
                                ...
                                if (!Convert.ToBoolean(ServerRewards.Call("TakePoints", player.userID.Get(), Convert.ToInt32(cost))))
                                {
                        ...
                        default:
                            ...
                            if (allItems.Where(x => x.info.shortname == "scrap").Sum(x => x.amount) < scrapCost)
                            {
        ...
        Perk GetEnhancementKitPerk(Item item)
        {
            if (item == null || string.IsNullOrEmpty(item.name)) return Perk.None;
            if (!Enum.TryParse<Perk>(item.name.Replace(config.enhancementSettings.enhancement_kit_settings.displayName, "").Trim(), out var perk)) return Perk.None;
            return perk;
        }
        ...
        void GiveItemCMD(BasePlayer player, string[] args, bool console = false)
        {
            ...
            if (perks != null) Pool.FreeUnmanaged(ref perks);
        }
        ...
        {
            List<KeyValuePair<Perk, float>> result = Pool.Get<List<KeyValuePair<Perk, float>>>();
            
            List<string> parsedArgs = Pool.Get<List<string>>();
            ...
            Pool.FreeUnmanaged(ref parsedArgs);
            return result;
        }
        ...
        private Dictionary<BasePlayer, float> RegenAmount;
        public Dictionary<ulong, float> took_damage;
        public static ItemPerks Instance;
        ...
        {
            if (player == null) return;
            if (Instance.RegenAmount != null && Instance.RegenAmount.ContainsKey(player)) Instance.RegenAmount.Remove(player);
        ...
        void AddRegenDelay(BasePlayer player)
        {
            ...
            if (took_damage == null || !took_damage.ContainsKey(player.userID))
            {
                ...
                if (config.playerSettings.chatMessageSettings.regen_disabled) PrintToChat(player, string.Format(lang.GetMessage("DisabledRegen", this, player.UserIDString), config.enhancementSettings.perk_config.regenerationSettings.damage_delay));
            }
            else
            {
        ...
        public class Regen : MonoBehaviour
        {
            ...
            private void Awake()
            {
                ...
                _regenAmount = Instance.RegenAmount[player];
            }
            ...
            public void FixedUpdate()
            {
                ...
                {
                    if (Instance.took_damage.ContainsKey(player.userID))
                    {
                        if (Instance.took_damage[player.userID] > Time.time) return;
                        else
                        {
                            Instance.took_damage.Remove(player.userID);
        ...
        {
            PlayerPerks perks;
            if (Instance.Player_perks.TryGetValue(player.userID, out perks) && perks.perk_modifiers.ContainsKey(Perk.Regeneration)) return true;
            return false;
        }
        ...
        {
            List<Perk> perks_to_remove = Pool.Get<List<Perk>>();
            foreach (var perk in perks)
            {
        ...
        private void Loaded()
        {
            ...
            if (config.enhancementSettings.prevent_max_condition_loss)
            {
            ...
            if (config.miscSettings.handleStacking)
            {
            ...
            if (config.miscSettings.handleSplitting)
            {
        ...
        void Loaded()
        {
            ...
            if (config.miscSettings.handleStacking)
            {
                ...
                HarmonyInstance.Patch(AccessTools.Method(typeof(DroppedItem), "OnDroppedOn"), new HarmonyMethod(typeof(CombineDroppedItem_Patch), "Prefix"));
            }
        ...
        internal class Item_CanStack_Patch
        {
            ...
            {
                return CanStackResult(item, __instance);
            }
        }

        [HarmonyPatch(typeof(DroppedItem), "OnDroppedOn", typeof(DroppedItem))]
        internal class CombineDroppedItem_Patch
        {
            [HarmonyPrefix]
            private static bool Prefix(DroppedItem __instance, DroppedItem di)
            {
                return CanStackResult(__instance.item, di.item);
            }
        }

        static bool CanStackResult(Item item, Item __instance)
        {
            // Handles enhancement kits
            if (IsEnhancementKit(item) || IsEnhancementKit(__instance))
            {
                return item.name == __instance.name;
            }

            // Returns default behaviour if item.text fields or perks are null or empty
            if (string.IsNullOrEmpty(item.text) && string.IsNullOrEmpty(__instance.text)) return true;
            if (!HasPerks(item.text) && !HasPerks(__instance.text)) return true;

            // Returns true if both item.text fields are the same.
            return item.text == __instance.text;
        }
