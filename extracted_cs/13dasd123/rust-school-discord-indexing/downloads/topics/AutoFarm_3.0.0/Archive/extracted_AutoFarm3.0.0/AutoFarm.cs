    ...
    public class AutoFarm : RustPlugin
    {
        ...
        private void OnServerInitialized()
        {
            if (configData.vipInformation == null)
            {
                ...
                {
                    { "autofarm.allow", new vipInfo() { AddSprinkler = false, AddStorageAdapter = false, BoxStorageSlots = 6, heatExposure = false, lightExposure = false, SprinklerNeedsWater = true, totalFarms = 5, seedStorageSlots = 6, cloneList = new Dictionary<string, string>() { { "blue_berry", "Sapling" }, { "white_berry", "Sapling" }, { "red_berry", "Sapling" }, { "green_berry", "Sapling" }, { "black_berry", "Sapling" }, { "yellow_berry", "Sapling" }, { "pumpkin", "Sapling" }, { "potato", "Sapling" }, { "hemp", "Sapling" }, { "corn", "Sapling" } }, knifeList = new Dictionary<int, int>() { { 1814288539, 0 }, { -194509282, 0 }, { 2040726127, 0 }, { -2073432256, 0 } }, seedsAllowedAndMultiplier = new Dictionary<int, int>() { { 803954639, 1 }, { 998894949, 1 }, { 1911552868, 1 }, { -1776128552, 1 }, { -237809779, 1 }, { -2084071424, 1 }, { -1511285251, 1 }, { 830839496, 1 }, { -992286106, 1 }, { -520133715, 1 }, { 838831151, 1 }, { -778875547, 1 }, { -1305326964, 1 }, { -886280491, 1 }, { 1512054436, 1 }, { 1898094925, 1 }, { 2133269020, 1 }, { 1533551194, 1 }, { 390728933, 1 } } } },
                    { "autofarm.vip", new vipInfo() { AddSprinkler = false, AddStorageAdapter = false, BoxStorageSlots = 6, heatExposure = false, lightExposure = false, SprinklerNeedsWater = true, totalFarms = 5, seedStorageSlots = 6, cloneList = new Dictionary<string, string>() { { "blue_berry", "Sapling" }, { "white_berry", "Sapling" }, { "red_berry", "Sapling" }, { "green_berry", "Sapling" }, { "black_berry", "Sapling" }, { "yellow_berry", "Sapling" }, { "pumpkin", "Sapling" }, { "potato", "Sapling" }, { "hemp", "Sapling" }, { "corn", "Sapling" } }, knifeList = new Dictionary<int, int>() { { 1814288539, 0 }, { -194509282, 0 }, { 2040726127, 0 }, { -2073432256, 0 } }, seedsAllowedAndMultiplier = new Dictionary<int, int>() { { 803954639, 1 }, { 998894949, 1 }, { 1911552868, 1 }, { -1776128552, 1 }, { -237809779, 1 }, { -2084071424, 1 }, { -1511285251, 1 }, { 830839496, 1 }, { -992286106, 1 }, { -520133715, 1 }, { 838831151, 1 }, { -778875547, 1 }, { -1305326964, 1 }, { -886280491, 1 }, { 1512054436, 1 }, { 1898094925, 1 }, { 2133269020, 1 }, { 1533551194, 1 }, { 390728933, 1 } } } },
                    { "autofarm.advanced", new vipInfo() { AddSprinkler = true, AddStorageAdapter = true, BoxStorageSlots = 12, heatExposure = false, lightExposure = false, SprinklerNeedsWater = false, totalFarms = 10, seedStorageSlots = 12, cloneList = new Dictionary<string, string>() { { "blue_berry", "Sapling" }, { "white_berry", "Sapling" }, { "red_berry", "Sapling" }, { "green_berry", "Sapling" }, { "black_berry", "Sapling" }, { "yellow_berry", "Sapling" }, { "pumpkin", "Sapling" }, { "potato", "Sapling" }, { "hemp", "Sapling" }, { "corn", "Sapling" } }, knifeList = new Dictionary<int, int>() { { 1814288539, 0 }, { -194509282, 0 }, { 2040726127, 0 }, { -2073432256, 0 } }, seedsAllowedAndMultiplier = new Dictionary<int, int>() { { 803954639, 1 }, { 998894949, 1 }, { 1911552868, 1 }, { -1776128552, 1 }, { -237809779, 1 }, { -2084071424, 1 }, { -1511285251, 1 }, { 830839496, 1 }, { -992286106, 1 }, { -520133715, 1 }, { 838831151, 1 }, { -778875547, 1 }, { -1305326964, 1 }, { -886280491, 1 }, { 1512054436, 1 }, { 1898094925, 1 }, { 2133269020, 1 }, { 1533551194, 1 }, { 390728933, 1 } } } }
                };
                SaveConfig();
            }
        ...
        class planterBoxBehavior : FacepunchBehaviour
        {
            ...
            private float CalculateExposureLight() => 1f;
            ...
            public void ForceLightUpdate()
            {
                planterBox.artificialLightExposure = new TimeCachedValue<float>()
                {
                    ...
                    updateValue = new Func<float>(CalculateExposureLight)
                };
            }
            ...
            private void generateStorage()
            {
                ...
                if (config.SprinklerNeedsWater && config.AddSprinkler)
                {
                    ...
                    if (waterSourceOld != null && waterSourceOld.GetComponentInParent<PlanterBox>() == null)
                    {
                        ...
                        UpdateLocalPositionAndRotation(transform, waterSourceOld, planterConfig.SplitterPos, Quaternion.identity);
                    }
                }
                ...
                if (config.SprinklerNeedsWater && waterSource == null)
                {
                ...
                {
                    FertilizerAdapter = AddStorageAdaptor(planterBox, planterBox, planterConfig.FertilizerAdapterPos, Quaternion.identity);
            ...
            {
                if (growableEntity == null || container == null) return;
                ...
                if (config.enableCloning && cloningItem != null && config.cloneList.ContainsKey(sName))
                {
                    ...
                    else if (growableEntity.CanClone() && growableEntity.State.ToString().ToLower().Contains(config.cloneList[sName].ToLower()))
                    {
                ...
                if (obj != null)
                {
                    ...
                    if (_ != null && _.Ganja != null && _.configData.settings.GanjaPluginEnable)
                    {
                        if (growableEntity.prefabID == 3006540952)
                            ...
                            Interface.CallHook("OnAutoFarmGather", planterBox.OwnerID.ToString(), container, growableEntity.transform.position, false, growableEntity.Genes.GetGeneTypeCount(GrowableGenetics.GeneType.Hardiness), growableEntity.Genes.GetGeneTypeCount(GrowableGenetics.GeneType.GrowthSpeed));
                    }
