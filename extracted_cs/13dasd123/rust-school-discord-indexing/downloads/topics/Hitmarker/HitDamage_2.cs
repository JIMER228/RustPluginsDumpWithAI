...
{
    [Info("HitDamage", "rustmods.ru", "1.0.5")]
        ...
        void OnEntityTakeDamage(BasePlayer victim, HitInfo info)
        {
            ...
            try {
                ...
                if (!victim.IsNpc && config.players)
                {
                    ...
                    if (victim.IsWounded())
                    {
                        victimEyes = victim.transform.position + new Vector3(0, 0.2f, 0);
                    }
                    ...
                    NextTick(() => {
                        ...
                        {   
                            SetFlag(info.InitiatorPlayer, true);
                            info.InitiatorPlayer.SendConsoleCommand("ddraw.text", 1f, Color.black, victimEyes, $"<size=23>УПАЛ</size>");
                            info.InitiatorPlayer.SendConsoleCommand("ddraw.text", 1f, Color.white, victimEyes, $"<size=23>УПАЛ</size>");
                            SetFlag(info.InitiatorPlayer, false);
                        }
                        if (victim.IsDead())
                        {   
                            SetFlag(info.InitiatorPlayer, true);
                            info.InitiatorPlayer.SendConsoleCommand("ddraw.text", 1f, Color.black, victimEyes, $"<size=23.5>УМЕР</size>");
                            info.InitiatorPlayer.SendConsoleCommand("ddraw.text", 1f, Color.white, victimEyes, $"<size=23>УМЕР</size>");
                            SetFlag(info.InitiatorPlayer, false);
                        }
                    });
                }
