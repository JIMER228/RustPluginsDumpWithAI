...
{
    [Info("HitMarker", "thouxanbanfauni", "1.1.1")]
	[Description("GUI маркер попаданий в игрока.")]
    class HitMarker : RustPlugin
    {
        ...
        {
            enablesound = Convert.ToBoolean( GetConfig( "Sound", "Включить звуковой эффект", true ) );
            soundeffect =
                Convert.ToString( GetConfig( "Sound", "Звуковой эффект хита", "assets/bundled/prefabs/fx/takedamage_hit.prefab" ) );
            headshotsoundeffect =
                Convert.ToString( GetConfig( "Sound", "Звуковой эффект попадания в голову", "assets/bundled/prefabs/fx/headshot.prefab" ) );
            GetVariable(Config, "Через сколько будет пропадать хитмаркер", out damageTimeout, 0.5f );
            SaveConfig();
        }
		...
		string GetDamageText(string action)
		{
			switch (action)
			{
				case "wound": return "<color=#00e7ff>УПАЛ!</color>";							  
				case "kill":  return "<color=purple>УБИТ!</color>";							  
			}
			
			return "<color=blue>ПОПАЛ!</color>";
		}
        ...
        {						
            ...
            for (var i = damages.Count-1; i >= 0; i--)
            {
                ...
                {
                    string color = item.Value.isFriend ? "green" : (item.Value.isHead ? "purple" : "green");
					CuiHelper.AddUi( player, HandleArgs( DamageGUI, item.Value.num, $"<color={color}>-{item.Value.damage}</color>", item.Value.xs, item.Value.ys, item.Value.xe, item.Value.ye ));
                }
