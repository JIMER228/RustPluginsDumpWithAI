...
/* 2.0.15 Change Log
 * Added console commannd: setglobalmarketquantity <amount> - Sets the market quantity for all items to amount.
 * Fixed an exception caused by 0.10 timer.
 * Added failsafe to ensure bag is closed.
 * Added color option for recipe title menu in recipes menu to be different when the meal is learnt vs unlearnt.
 * Fixed an exception when adding ingredients to crates that had null networkables.
 * Patched for Decembers update
*/

namespace Oxide.Plugins
{
    [Info("Cooking 2.0", "imthenewguy", "2.0.15")]
    ...
    class Cooking : RustPlugin
    {
        ...
        public class CookbookSettings
        {
            ...
            [JsonProperty("Should meals that a player has not learny be visible in the menu?")]
        ...
        void RollForRecipe(BasePlayer player, LootContainer container)
        {
            ...
            List<string> learnable = Pool.Get<List<string>>();

            foreach (var meal in config.mealSettings.meals)
            {
                if (!meal.Value.enabled) continue;
                if (config.mealSettings.cookbookSettings.defaultMeals.Contains(meal.Key)) continue;
        ...
        List<NPCMissionProvider> MarketNPCs = new List<NPCMissionProvider>();
        object OnNpcConversationStart(NPCMissionProvider NPCMissionProvider, BasePlayer player, ConversationData conversationData)
        {
            if (!MarketNPCs.Contains(NPCMissionProvider)) return null;
        ...
        void RemoveAllMarketNPCs(BasePlayer player)
        {
            ...
            List<NPCMissionProvider> marketNPCs = Pool.Get<List<NPCMissionProvider>>();
        ...
        void RemoveNPCCMD(BasePlayer player)
        {
            ...
            var npc = GetTargetEntity(player) as NPCMissionProvider;
            if (npc == null)
            {
        ...
        bool RemoveNPC(NPCMissionProvider npc)
        {
            foreach (var npcData in pcdData.npcs)
            {
        ...
        NPCMissionProvider IsNPCSpawned(Vector3 pos)
        {
            var npcs = FindEntitiesOfType<NPCMissionProvider>(pos, 5);
            NPCMissionProvider result = null;
        ...
        {
            var npc = GameManager.server.CreateEntity("assets/prefabs/npc/bandit/missionproviders/missionprovider_bandit_a.prefab", pos, rot) as NPCMissionProvider;
        ...
        void OnEntitySpawned(MapMarkerMissionProvider marker)
        {
            timer.Once(1f, () =>
            {
        ...
        void SendMealDescription(ConsoleSystem.Arg arg)
        {
            ...
            List<string> buffStrings = Pool.Get<List<string>>();
            foreach (var buff in mealData.buffs)
            ...
            Pool.FreeUnmanaged(ref buffStrings);
        }
        ...
        void GiveRandomMeal(ConsoleSystem.Arg arg)
        {
            ...
            GiveMeal(target, meal.Value.shortname, meal.Value.skin, meal.Key, 1);
            Pool.FreeUnmanaged(ref meals);
        }
        ...
        void GiveRandomIngredients(ConsoleSystem.Arg arg)
        {
            ...
            GiveMeal(target, ingredient.Value.shortname, ingredient.Value.skin, ingredient.Key, amount);
        ...
        internal class OreResourceEntity_OnAttacked
        {
            [HarmonyPrefix]
            private static bool Prefix(HitInfo info, OreResourceEntity __instance, ref OreHotSpot ____hotSpot)
            {
                try
                {
                    if (info != null && __instance != null && info.InitiatorPlayer != null && info.InitiatorPlayer.userID.IsSteamId())
                    {
                        var buffManager = Instance.GetBuffManager(info.InitiatorPlayer, false);
                        if (buffManager == null) return true;

                        if (!buffManager.GetBuffModifiers.ContainsKey(Buff.Mining_Hotspot)) return true;
                        if (____hotSpot == null)
                        {
                            ____hotSpot = __instance.SpawnBonusSpot(info.HitPositionWorld);
                        }
                        ____hotSpot.transform.position = info.HitPositionWorld;
                        ____hotSpot.SendNetworkUpdateImmediate(false);
                    }
                }
                catch { }
                return true;
