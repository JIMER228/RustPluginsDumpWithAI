    ...
    public class AutoFarm : RustPlugin
    {
        ...
        private void OnServerInitialized()
        {
            ...
            NextTick(() =>
            {
                ...
                for (int i = 0; i < pcdData.Planters.Count; i++)
                {
                    var element = pcdData.Planters.ElementAt(i);
                    var networkable = FindEntity(element.Key);
                   if (networkable == null || !(networkable is PlanterBox)) { pcdData.Planters.Remove(element.Key); removeCount++; }
                    else
                    {
                        totalPlanters++;
        ...
        {
            _.Puts(planterBox.ShortPrefabName);
        ...
        void SaveData(string filename = "12549")
        {
