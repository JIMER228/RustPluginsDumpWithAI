...
{
    [Info("RemoveUpgradeV2", "fermens", "1.1.0")]
    ...
    class RemoveUpgradeV2 : RustPlugin
    {
        ...
        object OnHammerHit(BasePlayer player, HitInfo info)
        {
            ...
            else if (info.HitEntity is BuildingBlock)
            {
                ...
                TryUpgrade(player, info.HitEntity.GetComponent<BuildingBlock>(), gr);
                return true;
            }
            return null;
        }
		...
		private static void GiveRefund(BaseCombatEntity entity, BasePlayer player)
		{
			...
			{
				var amount = Convert.ToInt32(100f < 100
					? value.amount * (100f / 100f)
					: value.amount);
        ...
        private void TryRemove(BasePlayer player, BaseCombatEntity removeObject, RemoveType removeType)
        {
            ...
            else if (removeType.Equals(RemoveType.Refund)) GiveRefund(removeObject, player);
        ...
        bool CanRemoveEntity(BasePlayer player, BaseEntity entity, RemoveType removeType)
        {
            ...
            DateTime time;
            if (entity is DecayEntity && !permission.UserHasPermission(player.UserIDString, "RemoveUpgradeV2.vip") && config.blocktime > 0 && (!canremove.TryGetValue(entity.net.ID.Value, out time) || time < DateTime.Now))
            {
