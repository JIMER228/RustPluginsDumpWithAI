...
namespace Oxide.Plugins
{
    ...
    {   
        ...
        {   
            ...
            {   
                ...
                if ((int) Kits.Call("GetKitMaxUses", kitname) != 0)
                    usesleft = $"{(int) Kits.Call("GetKitMaxUses", kitname) - (int) Kits.Call("GetPlayerKitUses", player.userID, kitname)}";

                string cooldown = gl("cooldown_title_none");
                if ((int) Kits.Call("GetKitCooldown", kitname) != 0)
                    cooldown = $"{TimeFormated(Convert.ToDouble(Kits.Call("GetKitCooldown", kitname)))}";
        ...
        private string Button_Cooldown_Text(BasePlayer player, string kitname)
        {
            ...
            int kitTimesUsed = Convert.ToInt32(Kits?.CallHook("GetPlayerKitUses", player.userID, kitname));
