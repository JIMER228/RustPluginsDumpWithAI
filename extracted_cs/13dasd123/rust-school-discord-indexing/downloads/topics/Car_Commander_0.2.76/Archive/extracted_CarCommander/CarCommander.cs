...
{
    [Info("CarCommander", "k1lly0u", "0.2.76")]
    ...
    class CarCommander : RustPlugin
    {
        ...
        private void RestoreVehicleInventories()
        {
            if (storedData.restoreData.Count > 0)
            {
                ...
                if (objects != null)
                {
                    foreach (BaseEntity obj in objects)
                    {
        ...
        private void ToggleController(BasicCar BasicCar, bool enabled)
        {
            ...
            if (controller != null)
            {
                ...
                if (!enabled)
                {
                    foreach (BasicCar.VehicleWheel wheel in controller.entity.wheels)
        ...
        public class CarController : MonoBehaviour
        {
            ...
            private void SetWheelColliders()
            {
                ...
                ConfigData.MovementSettings movement = ins.configData.Movement;
            ...
            {
                ConfigData.FuelOptions fuel = ins.configData.Fuel;
                fuelId = ins.fuelType;
                ...
                if (fuelEnabled)
                {
                    ...
                    if (fuelTank.uid == default(ItemContainerId))
                        fuelTank.GiveUID();
            ...
            private void InitializeInventory()
            {
                if (ins.configData.Inventory.Enabled)
                {
                    ...
                    if (container.inventory.uid == default(ItemContainerId))
            ...
            {               
                if (WaterLevel.Factor(entity.WorldSpaceBounds().ToBounds(), true, true, entity) > 0.7f)
                {
                    isDieing = true;
                    enabled = false;
                ...
                if (useDefaultHandling)
                {
                    ...
                    entity.SetFlag(BaseEntity.Flags.Reserved1, entity.AnyMounted() && hasFuel && ignitionOn, false);
                    entity.SetFlag(BaseEntity.Flags.Reserved2, (!entity.AnyMounted() ? false : lightsOn), false);
            ...
            private void ApplyForceAtWheels()
            {
                ...
                entity.SetFlag(BaseEntity.Flags.Reserved3, (single1 < 100f ? false : entity.AnyMounted()), false);
            }
            ...
            {               
                float steerAngle = Mathf.Lerp(maxSteeringAngle, maxSteeringAngleSpeed, (speed / maxSpeed)) * steerInput;
            ...
            public bool HasVehicleKey(BasePlayer keyHolder)
            {
                ...
                List<Item> items = keyHolder.inventory.FindItemsByItemID(946662961);
            ...
            public void StopToDie()
            {
                ...
                if (entity != null)
                {
                    ...
                    foreach (BasicCar.VehicleWheel wheel in entity.wheels)
                    {
            ...
            private void OnDeath()
            {
                ...
                {
                    if (container != null)
                        container.inventory.Drop("assets/prefabs/misc/item drop/item_drop.prefab", transform.position + Vector3.up + (UnityEngine.Random.onUnitSphere * UnityEngine.Random.Range(2f, 3f)), Quaternion.identity, 0f);
                    if (fuelTank != null)
                        fuelTank.Drop("assets/prefabs/misc/item drop/item_drop.prefab", transform.position + Vector3.up + (UnityEngine.Random.onUnitSphere * UnityEngine.Random.Range(2f, 3f)), Quaternion.identity, 0f);
                }
        ...
        {
            ConfigData.UIOptions.UICounter opt = configData.UI.Health;
            if (!opt.Enabled)
                return;

            CuiElementContainer container = UI.ElementContainer(uiHealth, UI.Color(opt.Color1, opt.Color1A), new UI4(opt.Xmin, opt.YMin, opt.XMax, opt.YMax));
        ...
        private void CreateFuelUI(BasePlayer player, CarController controller)
        {
            ...
            {
                ConfigData.UIOptions.UICounter opt = configData.UI.Fuel;
                if (!opt.Enabled)
                    return;

                int fuelAmount = controller.fuelTank.GetAmount(fuelType, false);
                CuiElementContainer container = UI.ElementContainer(uiFuel, UI.Color(opt.Color1, opt.Color1A), new UI4(opt.Xmin, opt.YMin, opt.XMax, opt.YMax));
        ...
        private void cmdClearCars(BasePlayer player, string command, string[] args)
        {
            ...
            for (int i = saveableCars.Count - 1; i >= 0; i--)
            {
                CarController car = saveableCars[i];
            ...
            {
                CarController car = temporaryCars[i];
                if (car != null && car.entity != null && !car.entity.IsDestroyed)
                    car.StopToDie();
            }
        ...
        {
            public Hash<ulong, InventoryData> restoreData = new Hash<ulong, InventoryData>();
            public Hash<ulong, SecurityData> securityData = new Hash<ulong, SecurityData>();

            public void AddData(CarController controller)
            {
                restoreData[controller.entity.net.ID.Value] = new InventoryData(controller);
                securityData[controller.entity.net.ID.Value] = new SecurityData(controller);
            }

            public void RemoveData(NetworkableId netId)
            {
                if (HasRestoreData(netId))
                    restoreData.Remove(netId.Value);

                if (HasSecurityData(netId))
                    securityData.Remove(netId.Value);
            }

            public bool HasRestoreData(NetworkableId netId) => restoreData.ContainsKey(netId.Value);

            public bool HasSecurityData(NetworkableId netId) => securityData.ContainsKey(netId.Value);
            ...
            public void RestoreVehicle(CarController controller)
            {
                ...
                if (restoreData.TryGetValue(controller.entity.net.ID.Value, out inventoryData))
                {
                ...
                if (securityData.TryGetValue(controller.entity.net.ID.Value, out securityDat))
                {
                ...
                public void RestoreVehicleSecurity(CarController controller)
                {
                    ...
                    if (hasCodeLock)
                    {
                        ...
                        if (lockEntity != null)
                        {
                            ...
                            lockEntity.OnDeployed(controller.container, null, null);
                            lockEntity.Spawn();
