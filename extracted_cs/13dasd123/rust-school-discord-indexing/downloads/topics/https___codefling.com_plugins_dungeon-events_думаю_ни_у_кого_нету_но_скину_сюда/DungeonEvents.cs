...
namespace Oxide.Plugins
{
    ...
    public class DungeonEvents : RustPlugin
    {
        ...
        private Dictionary<ulong, DateTime> _buyCooldowns = new Dictionary<ulong, DateTime>();
        ...
        private const bool ForceDefaultConfig = false;
        ...
        private readonly string[] filesToDelete = new string[] { "Dungeon_01.json", "Dungeon_02.json", "Dungeon_03.json", "Dungeon_001.json", "Dungeon_002.json", "Dungeon_003.json" };
        private static readonly WaitForSeconds SmallDelay = new WaitForSeconds(0.15f);
        private static readonly WaitForSeconds BigDelay = new WaitForSeconds(0.50f);

        private class SpawnedEntitiesInfo
        {
        ...
        {
            if (scientistNPC == null || info?.Initiator == null)
                return null;
        ...
        private object CanEntityTakeDamage(AutoTurret autoTurret, HitInfo hitinfo)
        {
        ...
        private object CanEntityTakeDamage(StorageContainer storageContainer, HitInfo hitinfo)
        {
        ...
        private void OnCorpsePopulate(ScientistNPC npc, NPCPlayerCorpse corpse)
        {
            ...
            var lootConfig = isBoss ? activeDungeon.TierConfig?.BossNpcConfig : activeDungeon.TierConfig?.NpcConfig;
            var corpseData = corpse.gameObject.AddComponent<CorpseLootData>();
            corpseData.IsBoss = isBoss;
            corpseData.LootConfig = lootConfig;
            NextTick(() =>
            {
                if (corpse == null)
                    return;
                var storedData = corpse.GetComponent<CorpseLootData>();
                if (storedData == null || storedData.LootConfig == null)
                    return;
                ...
                container.Clear();
                var items = storedData.LootConfig.LootDropConfig?.Items;
                if (items != null && items.Count > 0)
                {
                    ...
                    foreach (var itemCfg in lootItems)
                    {
                        if (Random.value * 100f <= itemCfg.InclusionChancePercentage)
                        {
                            ...
                            if (def == null)
                                continue;
                            int amount = Random.Range(itemCfg.MinimumAmount, itemCfg.MaximumAmount + 1);
        ...
        private void OnPlayerDeath(BasePlayer player, HitInfo info)
        {
            ...
            if (dungeon == null)
                return;
            var basePortal = dungeon.BasePortal;
            var insidePortal = dungeon.InsidePortal;
            if (basePortal == null || insidePortal == null)
                return;
            if (basePortal.transform == null || insidePortal.transform == null)
                return;
            try
            {
            ...
            try
            {
                exitPos = insidePortal.transform.position;
            }
            catch
            {
                return;
            ...
            foreach (var door in dungeon.Entities.OfType<Door>())
            {
                ...
                try
                {
                    if (Vector3.Distance(door.transform.position, exitPos) <= 3f)
                    {
                        door.SetOpen(false);
        ...
        {
            var file = Interface.Oxide.DataFileSystem.GetFile($"{Name}/{dungeonId}");
        ...
        {
            string folder = Path.Combine(Interface.Oxide.DataFileSystem.Directory, Name);
        ...
        private void DeleteDungeonFiles()
        {
            string pluginDataFolder = Path.Combine(Interface.Oxide.DataDirectory, Name);
            ...
            if (dungeon.OwnerID != 0UL)
            {
                ...
                if (owner != null && owner.IsConnected)
                {
                    try
                    {
                        ...
                        {
                            SkillTree.Call("AwardXP", owner.userID, xpReward, "DungeonEvents", false);
                            owner.ChatMessage(Msg(owner.UserIDString, "DungeonClearXpReward", xpReward));
                        }
                        _buyCooldowns[owner.userID] = DateTime.UtcNow.AddSeconds(_config.DungeonSpawn.BuyCooldown);
                        rewarded = true;
                    }
                    catch
                    {
                ...
                foreach (ulong uid in dungeon.PlayersInside.ToList())
                {
                    ...
                    if (participant != null && participant.IsConnected)
                    {
                        try
                        {
                            ...
                            if (xpReward > 0.0 && SkillTree != null)
                            {
                                SkillTree.Call("AwardXP", participant.userID, xpReward, "DungeonEvents", false);
                                participant.ChatMessage(Msg(participant.UserIDString, "DungeonClearXpReward", xpReward));
                            }
                            _buyCooldowns[participant.userID] = DateTime.UtcNow.AddSeconds(_config.DungeonSpawn.BuyCooldown);
                            break;
                        }
                        catch { }
                    }
        ...
        private void TeleportPlayerToOutsidePortal(BasePlayer player, ActiveDungeon dungeon)
        {
            ...
            List<ulong> playersInside = dungeon.PlayersInside.ToList();
            playersInside.Sort();
            ...
            Vector3 verticalOffset = Vector3.up * 5f;
            Vector3 newPos = exitPos + horizontalOffset + verticalOffset;
        ...
        [ConsoleCommand("buydungeon")]
        private void BuyDungeonConsoleCommand(ConsoleSystem.Arg arg)
        {
        ...
        private void CloseCommandUi(ConsoleSystem.Arg arg)
        {
        ...
        [ChatCommand("buydungeon")]
        private void BuyDungeonChatCommandUi(BasePlayer player, string command, string[] args)
        {
            if (player == null)
                return;
        ...
        private void CreateDungeonCommand(BasePlayer player, string command, string[] args)
        {
            ...
            string tierName = args[0].ToLower();
            var prop = _config.Tiers.GetType().GetProperties().FirstOrDefault(p => p.Name.Equals(tierName, StringComparison.OrdinalIgnoreCase));
            var tier = prop?.GetValue(_config.Tiers) as DungeonTierConfig;
            if (tier == null)
            {
        ...
        private void ReloadConfigCommand(BasePlayer player, string command, string[] args)
        {
            ...
            LoadConfig();
            dungeonTimer?.Destroy();
            dungeonTimer = timer.Every(TimeTimers, HandleDungeonTimers);
            UpdateMarkers();
            _dungeonQueue.Clear();
            NotifyPlayer(player, "ConfigReloaded", 0);
        }

        [ChatCommand("de.toggle")]
        private void TogglePurchasesCommand(BasePlayer player, string command, string[] args)
        {
            if (!IsAdmin(player))
                return;
        ...
        [ChatCommand("de.removenearest")]
        private void RemoveNearestDungeonCommand(BasePlayer player, string command, string[] args)
        {
            if (!IsAdmin(player))
                return;
        ...
        [ChatCommand("removedungeon")]
        private void RemoveOwnDungeonCommand(BasePlayer player, string command, string[] args)
        {
            if (player == null)
                return;
        ...
        [ChatCommand("spawdungeonnentity")]
        private void SpawnDungeonEntityCommand(BasePlayer player, string command, string[] args)
        {
            if (!IsAdmin(player))
            {
                NotifyPlayer(player, "NoAccess", 0);
            ...
            if (args.Length < 2)
            {
                player.ChatMessage("Correct Usage: /spawdungeonnentity <npc|boss|turret> <easy|normal|medium|hard|nightmare>");
                return;
            }
            var entityType = args[0].ToLower();
            var difficultyName = args[1].ToLower();
            DungeonTierConfig tierConfig = null;
            var allTiersProps = _config.Tiers.GetType().GetProperties();
            var foundTierProp = allTiersProps.FirstOrDefault(p => p.Name.Equals(difficultyName, StringComparison.OrdinalIgnoreCase));
            if (foundTierProp == null)
            {
                player.ChatMessage("Invalid difficulty. Try: easy, normal, medium, hard, nightmare.");
                return;
            }
            tierConfig = foundTierProp.GetValue(_config.Tiers) as DungeonTierConfig;
            if (tierConfig == null)
            {
                player.ChatMessage("Failed to retrieve difficulty settings.");
                return;
            }
            var spawnPos = player.transform.position + (player.eyes.BodyForward() * 2f);
            var spawnRot = Quaternion.Euler(0f, player.eyes.rotation.eulerAngles.y, 0f);
            var dummyDungeon = new ActiveDungeon
            {
            ...
            {
                case "npc":
                    SpawnNpc(spawnPos, dummyDungeon, boss: false);
                    player.ChatMessage($"NPC ({difficultyName}) spawned in front of you!");
                    break;
                case "boss":
                    SpawnNpc(spawnPos, dummyDungeon, boss: true);
                    player.ChatMessage($"Boss ({difficultyName}) spawned in front of you!");
                    break;
                case "turret":
                    SpawnTurret(spawnPos, spawnRot, dummyDungeon.TierConfig.AutoTurretConfig, dummyDungeon);
                    player.ChatMessage($"Turret ({difficultyName}) spawned in front of you!");
                    break;
                default:
                    player.ChatMessage("Invalid type! Use /spawdungeonnentity <npc|boss|turret> <easy|normal|medium|hard|nightmare>");
                    break;
            }
        }

        [ConsoleCommand("spawnrandomdungeon")]
        private void SpawnRandomDungeonConsoleCommand(ConsoleSystem.Arg arg)
        {
        ...
        [ConsoleCommand("spawnfixeddungeon")]
        private void SpawnFixedDungeonConsoleCommand(ConsoleSystem.Arg arg)
        {
            if (arg.Player() != null && !IsAdmin(arg.Player()))
            {
                arg.ReplyWith("You do not have permission to run this command.");
            ...
            if (!arg.HasArgs(1))
            {
                arg.ReplyWith("Usage: spawnfixeddungeon <tierName>");
                return;
            }
            var tierName = arg.GetString(0).ToLower();
            var tier = _config.Tiers.GetType().GetProperties().FirstOrDefault(prop => prop.Name.ToLower() == tierName)?.GetValue(_config.Tiers) as DungeonTierConfig;
            if (tier == null)
            {
                arg.ReplyWith($"Invalid tier '{tierName}'. Available tiers: {GetAvailableTiers()}");
                return;
            }
            if (!_tierInfoMap.ContainsKey(tier))
            {
                arg.ReplyWith("This tier is disabled.");
                return;
            }
            EnqueueDungeonRequest(null, tier, false);
        ...
        private IEnumerator CreateDungeon(DungeonRequest request)
        {
            ...
            if (NpcSpawn == null)
            {
                LogDebug("NpcSpawn plugin is not loaded.", 2);
                player?.ChatMessage(Msg(player?.UserIDString, "NpcSpawnNotLoaded"));
        ...
        private void AutoSpawnDungeon()
        {
            ...
            LogDebug($"Enqueued Dungeon Auto Spawn of tier {_tierInfoMap[tier].Name}.");
        }
        ...
        private void DestroyDungeon(ActiveDungeon dungeon)
        {
            ...
            CleanupDungeonArea(dungeon.DungeonPosition);
        ...
        private void CleanupDungeonArea(Vector3 position)
        {
            float radius = 80f;
            var corpses = Pool.GetList<LootableCorpse>();
            Vis.Entities(position, radius, corpses);
            ...
            Vis.Entities(position, radius, droppedItems);
            foreach (var di in droppedItems)
            {
                if (di.IsValid() && !di.IsDestroyed)
                    di.Kill();
            }
            Pool.FreeList(ref droppedItems);
            var droppedContainers = Pool.GetList<DroppedItemContainer>();
            Vis.Entities(position, radius, droppedContainers);
        ...
        private bool HasDungeonExceededMaxLifetime(ActiveDungeon dungeon)
        {
            var timeElapsed = DateTime.UtcNow - dungeon.CreationTime;
            return timeElapsed.TotalSeconds >= dungeon.TierConfig.MaxDungeonLifetime;
        }
        ...
        private void SpawnLootBox(Vector3 position, Quaternion rotation, DungeonTierConfig tierConfig, ActiveDungeon activeDungeon)
        {
            ...
            lootBoxEntity.Spawn();
            if (_config.TierSpecificLoot)
                PopulateStorage(lootBoxEntity.inventory, tierConfig, tierConfig.LootBoxItems);
            else
                PopulateStorage(lootBoxEntity.inventory, tierConfig, _config.LootBoxConfig.LootItems);
        ...
        {
            var configObject = NpcSpawnObjectConfig(activeDungeon, boss);
            var spawnedNpc = NpcSpawn?.Call("SpawnNpc", position, configObject) as ScientistNPC;
            if (spawnedNpc != null)
            {
                NextTick(() =>
                {
                    spawnedNpc.Brain.Navigator.CanUseNavMesh = false;
                });
                spawnedNpc.OwnerID = OwnerID;
                spawnedNpc.gameObject.AddComponent<NpcMovement>();
                if (boss)
                    spawnedNpc.gameObject.AddComponent<BossMonoBehaviour>();
                activeDungeon.Entities.Add(spawnedNpc);
                activeDungeon.Npcs.Add(spawnedNpc);
                RegisterSpawnedEntityForDungeon(spawnedNpc, activeDungeon.UniqueId);
                LogDebug($"NPC spawned at {position}.", 0);
            }
        }
        ...
        public class NpcMovement : MonoBehaviour
        {
            ...
            public bool contact;
            public float range;
            private ScientistNPC scientistNpc;
            private ScientistBrain scientistBrain;
            private int ignoreLayersMask;
            private Vector3 randomOffset;
            private Vector3 initialPosition;
            private float movementLimit = 15f;
            private float moveSpeed = 0.6f;
            private float obstacleCheckDistance = 1.5f;
            private float sphereCheckRadius = 0.3f;

            private void Awake()
            {
                scientistNpc = GetComponent<ScientistNPC>();
                scientistBrain = GetComponent<ScientistBrain>();
                ignoreLayersMask = ~(Layers.Mask.Ignore_Raycast | Layers.Mask.Reserved1 | Layers.Mask.Invisible | Layers.Mask.Trigger | Layers.PreventBuilding | Layers.Mask.Prevent_Movement);
                initialPosition = transform.position;
                moveTarget = transform.position;
                mainTarget = scientistNpc;
                contact = false;
                range = 0.5f;
                InvokeRepeating(nameof(UpdateAi), 1f, 1f);
                InvokeRepeating(nameof(UpdateMovement), 0.28f, 0.28f);
            }

            private BaseEntity GetFirstPlayer() => scientistBrain.Senses.Players.FirstOrDefault();

            private Vector3 GenerateRandomOffset() => new Vector3(Random.Range(-0.2f, 0.2f), 0, Random.Range(-0.2f, 0.2f));
            ...
            {
                scientistNpc.transform.rotation = Quaternion.identity;
                ...
                if (mainTarget != player)
                    contact = false;
                float distanceToPlayer = Vector3.Distance(scientistNpc.transform.position, player.transform.position);
                Vector3 directionToPlayer = (player.transform.position - scientistNpc.transform.position) + Vector3.up * 0.5f;
                if (Physics.Raycast(scientistNpc.transform.position + Vector3.up * 0.2f, directionToPlayer, out RaycastHit hit, distanceToPlayer, ignoreLayersMask))
                {
                    var hitEntity = hit.GetEntity();
                    if (hitEntity != null && hitEntity == player)
                    {
                        contact = true;
                    ...
                    else
                    {
                        ...
                        range = distanceToPlayer < 1f ? 1f : 0.5f;
                    }
                }
            }
            ...
            private void UpdateMovement()
            {
                if (scientistNpc == null)
                    return;
                var player = GetFirstPlayer();
                if (player == null || Mathf.Abs(player.transform.position.y - scientistNpc.transform.position.y) > 2.5f)
                    contact = false;
                if (!contact)
                {
                    if (Vector3.Distance(scientistNpc.transform.position, initialPosition) > 0.1f)
                        MoveTowards(initialPosition);
                    return;
                }
                if (Vector3.Distance(scientistNpc.transform.position, player.transform.position) <= 1f)
                    return;
                wayPoints.Add(player.transform.position + randomOffset);
                if (wayPoints.Count > 99)
                    wayPoints.RemoveAt(0);
                if (moveTarget == scientistNpc.transform.position && wayPoints.Count > 0)
                    moveTarget = wayPoints[0];
                float distanceToMoveTarget = Vector3.Distance(scientistNpc.transform.position, moveTarget);
                if (distanceToMoveTarget < range)
                {
                    if (range < 0.6f && wayPoints.Count > 0)
                        wayPoints.RemoveAt(0);
                    moveTarget = wayPoints.Count > 0 ? wayPoints[0] : scientistNpc.transform.position;
                }
                if (distanceToMoveTarget > 0.2f)
                {
                    Vector3 desiredDirection = (new Vector3(moveTarget.x, 0, moveTarget.z) - new Vector3(scientistNpc.transform.position.x, 0, scientistNpc.transform.position.z)).normalized;
                    Vector3 rayOrigin = scientistNpc.transform.position + Vector3.up * 0.5f;
                    if (Physics.Raycast(rayOrigin, desiredDirection, out RaycastHit hit, obstacleCheckDistance, ignoreLayersMask))
                        desiredDirection = (desiredDirection + hit.normal).normalized;
                    desiredDirection.y = 0;
                    desiredDirection.Normalize();
                    Vector3 tentativePosition = scientistNpc.transform.position + desiredDirection * moveSpeed;
                    tentativePosition.y = scientistNpc.transform.position.y;
                    Vector3 newPosition = ConstrainToCircle(tentativePosition, initialPosition, movementLimit);
                    var dungeon = DungeonEvents._instance.GetActiveDungeonByEntity(scientistNpc);
                    Vector3 exclusionCenter = dungeon?.InsidePortal?.transform.position ?? Vector3.zero;
                    float exclusionRadius = 3f;
                    if (exclusionCenter != Vector3.zero && Vector3.Distance(newPosition, exclusionCenter) <= exclusionRadius)
                        return;
                    if (!IsPositionFree(newPosition))
                    {
                        ...
                        for (int i = 0; i < maxAttempts; i++)
                        {
                            ...
                            Vector3 alternativeDirection = Quaternion.Euler(0, randomAngle, 0) * desiredDirection;
                            Vector3 altTentative = scientistNpc.transform.position + alternativeDirection * moveSpeed;
                            altTentative.y = scientistNpc.transform.position.y;
                            Vector3 altPosition = ConstrainToCircle(altTentative, initialPosition, movementLimit);
                            if (IsPositionFree(altPosition) && !(exclusionCenter != Vector3.zero && Vector3.Distance(altPosition, exclusionCenter) <= exclusionRadius))
                            {
                    ...
                    scientistNpc.transform.position = newPosition;
                }
            }

            private void MoveTowards(Vector3 target)
            {
                Vector3 desiredDirection = (new Vector3(target.x, 0, target.z) - new Vector3(scientistNpc.transform.position.x, 0, scientistNpc.transform.position.z)).normalized;
                Vector3 rayOrigin = scientistNpc.transform.position + Vector3.up * 0.5f;
                Debug.DrawRay(rayOrigin, desiredDirection * obstacleCheckDistance, Color.red, 0.2f);
                if (Physics.Raycast(rayOrigin, desiredDirection, out RaycastHit hit, obstacleCheckDistance, ignoreLayersMask))
                    desiredDirection = (desiredDirection + hit.normal).normalized;
                desiredDirection.y = 0;
                desiredDirection.Normalize();
            ...
            {
                Collider[] colliders = Physics.OverlapSphere(position, sphereCheckRadius, ~0);
                foreach (Collider col in colliders)
                {
                    if (col.gameObject == scientistNpc.gameObject || col.transform.IsChildOf(scientistNpc.transform))
                        continue;
                    if (col.bounds.max.y < scientistNpc.transform.position.y + 0.2f)
                        continue;
                    if (col.gameObject.name.Contains("TargetDetection"))
                        continue;
                    if (col.gameObject.name.Contains("foundation"))
                        continue;
                    if (col.gameObject.name.Contains("sentry"))
                        continue;
                    if (col.gameObject.name.Contains("helipad_desert_1500x1500"))
                        continue;
                    if (col.gameObject.name.Contains("autoturret_deployed"))
                        continue;
                    if (col.gameObject.name.Contains("preventBuilding"))
                        continue;
                    if (col.gameObject.name.Contains("DungeonEventsTrigger"))
                        continue;
                    return false;
                }
                return true;
        ...
        private JObject NpcSpawnObjectConfig(ActiveDungeon activeDungeon, bool boss = false)
        {
            ...
            return new JObject
            {
                ...
                ["BeltItems"] = new JArray(
                    npcConfig.BeltItems.Items.Select(item => new JObject
                    {
        ...
        private void ProcessDungeonQueue()
        {
            ...
            if (req == null || req.Tier == null)
                return;
            if (req.Origin == DungeonOrigin.AutoSpawn && _activeDungeons.Count(d => d.Origin == DungeonOrigin.AutoSpawn) >= _config.DungeonSpawn.MaxTotalActiveDungeonsAutoSpawn)
            {
                _dungeonFailedAutoSpawnQueue.Enqueue(req);
                return;
            }
            if (req.Origin == DungeonOrigin.Buy && _activeDungeons.Count(d => d.Origin == DungeonOrigin.Buy) >= _config.DungeonSpawn.MaxTotalActiveDungeonsBuy)
            {
        ...
        {
            if (_buyCooldowns.TryGetValue(player.userID, out DateTime until) && DateTime.UtcNow < until)
            {
            ...
            var tierProp = _config.Tiers.GetType().GetProperties().FirstOrDefault(p => p.Name.Equals(tierName, StringComparison.OrdinalIgnoreCase));
            DungeonTierConfig tier = tierProp != null ? tierProp.GetValue(_config.Tiers) as DungeonTierConfig : null;
            if (tier == null || !tier.Enabled)
            {
                NotifyPlayer(player, "InvalidTierMessage", 0, tierName, GetAvailableTiers());
                return;
        ...
        private void InitializeTierInfoMap()
        {
            ...
            if (_config?.Tiers == null)
                return;
            _tierInfoMap[_config.Tiers.Easy] = new TierInfo
            {
                Name = "Easy",
                MapName = _config.Tiers.Easy.MapName,
                BuyCost = _config.Tiers.Easy.BuyCost,
                MarkerColor = new Color(0.2f, 0.8f, 0.2f, 1f),
                ColorString = "0.2 0.8 0.2 1",
                Difficulty = Difficulty.EASY,
                Enabled = _config.Tiers.Easy.Enabled,
            };
            _tierInfoMap[_config.Tiers.Normal] = new TierInfo
            {
                Name = "Normal",
                MapName = _config.Tiers.Normal.MapName,
                BuyCost = _config.Tiers.Normal.BuyCost,
                MarkerColor = new Color(1f, 0.8f, 0f, 1f),
                ColorString = "1 0.8 0 1",
                Difficulty = Difficulty.NORMAL,
                Enabled = _config.Tiers.Normal.Enabled,
            };
            _tierInfoMap[_config.Tiers.Medium] = new TierInfo
            {
                Name = "Medium",
                MapName = _config.Tiers.Medium.MapName,
                BuyCost = _config.Tiers.Medium.BuyCost,
                MarkerColor = new Color(1f, 0.5f, 0f, 1f),
                ColorString = "1 0.5 0 1",
                Difficulty = Difficulty.MEDIUM,
                Enabled = _config.Tiers.Medium.Enabled,
            };
            _tierInfoMap[_config.Tiers.Hard] = new TierInfo
            {
                Name = "Hard",
                MapName = _config.Tiers.Hard.MapName,
                BuyCost = _config.Tiers.Hard.BuyCost,
                MarkerColor = new Color(0.8f, 0.1f, 0.1f, 1f),
                ColorString = "0.8 0.1 0.1 1",
                Difficulty = Difficulty.HARD,
                Enabled = _config.Tiers.Hard.Enabled,
            };
            _tierInfoMap[_config.Tiers.Nightmare] = new TierInfo
            {
                Name = "Nightmare",
                MapName = _config.Tiers.Nightmare.MapName,
                BuyCost = _config.Tiers.Nightmare.BuyCost,
                MarkerColor = new Color(0.6f, 0.3f, 1f, 1f),
                ColorString = "0.6 0.3 1 1",
                Difficulty = Difficulty.NIGHTMARE,
                Enabled = _config.Tiers.Nightmare.Enabled,
            };
        }
        ...
        private string GetAvailableTiers()
        {
            if (_config?.Tiers == null)
                return "None";
            var tierProps = _config.Tiers.GetType().GetProperties().Where(prop => ((DungeonTierConfig)prop.GetValue(_config.Tiers)).Enabled).Select(prop => prop.Name);
            return string.Join(", ", tierProps);
        }
        ...
        {
            var availableTiers = _tierInfoMap.Keys.Where(t => t.Enabled).ToList();
            if (availableTiers.Count == 0)
                return null;
            var counts = availableTiers.ToDictionary(tier => tier, tier => _activeDungeons.Count(d => d.TierConfig == tier));
            int minCount = counts.Values.Min();
            var candidates = counts.Where(kv => kv.Value == minCount).Select(kv => kv.Key).ToList();
        ...
        private void PopulateStorage(ItemContainer container, DungeonTierConfig tierConfig, List<ItemConfig> lootItems)
        {
            ...
            foreach (var itemConfig in shuffled)
            {
                ...
                if (def == null)
                    continue;
                int amount = Random.Range(itemConfig.MinimumAmount, itemConfig.MaximumAmount + 1);
                if (container.itemList.Count >= container.capacity)
                    break;
        ...
        private void OpenEventUI(BasePlayer player, ActiveDungeon dungeon)
        {
            ...
            var infos = new List<IconInfo>
            {
                new IconInfo
                {
                    Sprite = "assets/icons/farming/icon_age.png",
                ...
                elements.Add(
                    new CuiButton
                    {
                        Button =
                        {
                            ...
                            Command = $"buydungeon {tierInfo.Name} {player.UserIDString}",
                        ...
                        RectTransform = { AnchorMin = $"0.1 {0.7 - index * 0.15}", AnchorMax = $"0.9 {0.85 - index * 0.15}" },
                    },
                    UiMainPanel
                );
                index++;
        ...
        {
            VersionNumber PreviousVersion = Version;
            base.LoadConfig();
            try
            {
                ...
                {
                    PreviousVersion = _config.Version;
        ...
        protected override void LoadDefaultMessages()
        {
            if (ForceDefaultConfig)
            {
                ...
                lang.RegisterMessages(new Dictionary<string, string> { }, this, "pt-BR");
                lang.RegisterMessages(new Dictionary<string, string> { }, this, "de");
                lang.RegisterMessages(new Dictionary<string, string> { }, this, "es");
        ...
        private class Configuration
        {
            ...
            [JsonProperty("Blocked commands inside a dungeon")]
            ...
            [JsonProperty("Unified vs. Tiered Loot (If true, uses tier-specific loot; if false, uses a single loot table)")]
            public bool TierSpecificLoot { get; set; } = false;

            [JsonProperty("Loot Box Config")]
                ...
                new DungeonTierConfig
                {
                    ...
                    NpcConfig = new NpcConfig
                    {
                        ...
                        WearItems = new WearConfig
                        {
                            ...
                            {
                                new() { ShortName = "tshirt.long", SkinID = 10118 },
                                new() { ShortName = "burlap.gloves", SkinID = 949616124 },
                                new() { ShortName = "attire.snowman.helmet", SkinID = 0 },
                                new() { ShortName = "pants", SkinID = 2352962213 },
                                new() { ShortName = "shoes.boots", SkinID = 2380731293 },
                            },
                        },
                        ...
                        LootDropConfig = new LootDropConfig
                        {
                    ...
                    BossNpcConfig = new NpcConfig
                    {
                        ...
                        WearItems = new WearConfig
                        {
                            Items = new List<WearItemConfig>
                            {
                                new() { ShortName = "riot.helmet", SkinID = 1087995729 },
                                new() { ShortName = "hoodie", SkinID = 1087990973 },
                                new() { ShortName = "burlap.gloves", SkinID = 1087998101 },
                                new() { ShortName = "pants", SkinID = 1087992342 },
                                new() { ShortName = "shoes.boots", SkinID = 1088000573 },
                            },
                        },
                        BeltItems = new BeltConfig
                        {
                    ...
                    LootBoxItems = new List<ItemConfig>
                    {
                        new()
                        {
                            ShortName = "ammo.rifle",
                ...
                new DungeonTierConfig
                {
                    ...
                    NpcConfig = new NpcConfig
                    {
                        ...
                        WearItems = new WearConfig
                        {
                            Items = new List<WearItemConfig>
                            {
                                new() { ShortName = "bucket.helmet", SkinID = 1073904216 },
                                new() { ShortName = "jacket", SkinID = 2350426469 },
                                new() { ShortName = "tshirt", SkinID = 10039 },
                                new() { ShortName = "burlap.gloves.new", SkinID = 0 },
                                new() { ShortName = "pants", SkinID = 1441311938 },
                                new() { ShortName = "shoes.boots", SkinID = 2075527039 },
                            },
                        },
                        BeltItems = new BeltConfig
                        {
                ...
                new DungeonTierConfig
                {
                    ...
                    NpcConfig = new NpcConfig
                    {
                        ...
                        WearItems = new WearConfig
                        {
                            Items = new List<WearItemConfig>
                            {
                                new() { ShortName = "coffeecan.helmet", SkinID = 1727561127 },
                                new() { ShortName = "roadsign.jacket", SkinID = 1727562915 },
                                new() { ShortName = "roadsign.gloves", SkinID = 2799639349 },
                                new() { ShortName = "pants", SkinID = 2814837980 },
                                new() { ShortName = "shoes.boots", SkinID = 1839313604 },
                                new() { ShortName = "tshirt.long", SkinID = 566893368 },
                            },
                        },
                        BeltItems = new BeltConfig
                        {
                ...
                new DungeonTierConfig
                {
                    ...
                    NpcConfig = new NpcConfig
                    {
                        ...
                        WearItems = new WearConfig
                        {
                            Items = new List<WearItemConfig>
                            {
                                new() { ShortName = "metal.facemask", SkinID = 3284864766 },
                                new() { ShortName = "metal.plate.torso", SkinID = 2105505757 },
                                new() { ShortName = "burlap.gloves", SkinID = 2090790324 },
                                new() { ShortName = "hoodie", SkinID = 2080975449 },
                                new() { ShortName = "shoes.boots", SkinID = 10023 },
                                new() { ShortName = "pants", SkinID = 2080977144 },
                                new() { ShortName = "roadsign.kilt", SkinID = 2120628865 },
                            },
                        },
                        BeltItems = new BeltConfig
                        {
                ...
                new DungeonTierConfig
                {
                    ...
                    NpcConfig = new NpcConfig
                    {
                        ...
                        WearItems = new WearConfig
                        {
                            Items = new List<WearItemConfig>
                            {
                                new() { ShortName = "metal.facemask", SkinID = 3343860599 },
                                new() { ShortName = "metal.plate.torso", SkinID = 3343861569 },
                                new() { ShortName = "hoodie", SkinID = 1369835131 },
                                new() { ShortName = "roadsign.kilt", SkinID = 1727564168 },
                                new() { ShortName = "burlap.gloves", SkinID = 1210780157 },
                                new() { ShortName = "pants", SkinID = 810745264 },
                                new() { ShortName = "shoes.boots", SkinID = 1210771348 },
                            },
                        },
                        BeltItems = new BeltConfig
                        {
        ...
        public class DungeonTierConfig
        {
            ...
            [JsonProperty("Loot Box Items", ObjectCreationHandling = ObjectCreationHandling.Replace)]
        ...
        public class TurretBehaviour : FacepunchBehaviour
        {
            ...
            public void CustomTargetTick()
            {
                ...
                {
                    autoTurret.nextVisCheck = nowRT + UnityEngine.Random.Range(0.2f, 0.3f);
            ...
            private void Perform360IdleSweep()
            {
                if (!autoTurret.HasTarget())
                {
                    autoTurret.targetAimDir = Quaternion.Euler(0, UnityEngine.Random.Range(0f, 360f), 0) * Vector3.forward;
        ...
        public class DungeonEventsTrigger : FacepunchBehaviour
        {
            ...
            private void UpdateDungeonPlayersUI()
            {
                ...
                else if (_dungeon.PlayersInside.Count == 0)
                    CancelInvoke(nameof(UpdateDungeonPlayersUI));
            }
        ...
        private readonly WaitForSeconds _shortDelay = new WaitForSeconds(0.15f);
        ...
        private void CreateLight(Vector3 centre, ActiveDungeon room)
        {
            ...
            var lightEntity = GameManager.server.CreateEntity(LightPrefab, lightPos, rotation);
            if (lightEntity == null)
                return;
        ...
        private void BuildCorridor(Vector3 start, Vector3 end, ActiveDungeon room, bool spawnEntities, bool placePortal)
        {
            ...
            if (PlaceLights)
            {
                ...
                var cellCount = Mathf.Max(1, Mathf.CeilToInt(totalLength / CellSpacing));
                for (var c = 0; c <= cellCount; c++)
                {
                    var offset = c * CellSpacing + CellSpacing * 0.5f;
        ...
        {
            var count = 0;
            bool duelistLoaded = Duelist != null && Duelist.IsLoaded;
            bool raidableBasesLoaded = RaidableBases != null && RaidableBases.IsLoaded;
            bool abandonedBasesLoaded = AbandonedBases != null && AbandonedBases.IsLoaded;
        ...
        private Vector3 CheckCandidate(Vector3 position, GeneratorConfig cfg, bool duelistLoaded, bool raidableBasesLoaded, bool abandonedBasesLoaded)
        {
            ...
            if (cfg.AvoidTerrainTopology && InAvoidedTopology(position, cfg))
                return Vector3.zero;
            if (Physics.CheckSphere(position, cfg.CheckSphereRadius, cfg.CheckSphereMask))
                return Vector3.zero;
            if (HasEntities(position, cfg.CheckSphereRadius, cfg.CheckSphereMask))
                return Vector3.zero;
