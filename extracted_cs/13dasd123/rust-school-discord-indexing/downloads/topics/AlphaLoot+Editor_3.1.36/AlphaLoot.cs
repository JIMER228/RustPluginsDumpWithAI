...
namespace Oxide.Plugins
{
    ...
    class AlphaLoot : RustPlugin
    {
        ...
        private bool PopulateLoot(LootContainer container) => PopulateLoot(container, ToProfileName(container));
            ...
            foreach (int itemId in items)
            {
                ...
                foreach (KeyValuePair<string, AdvancedLootContainerProfile> kvp in source.loot_advanced)
                {
                    ...
                    if (newLootItem.HasItems)
                    {
                        ...
                        {
                            InsertItemAmountWeighted(kvp.Key, shortname, newLootItem, ref simpleLootProfile.Items, ref additions);
                        }
                    }
                }
        ...
        private void InsertItemAmountWeighted(string container, string shortname, NewLootItem newLootItem, ref ItemAmountWeighted[] dest, ref int additions)
        {
        ...
        private void FindItemRemoveItemByNameRecursive(string container, string[] shortnames, ref ItemAmountWeighted[] items, ref bool shouldSave, ref int count)
        {
            bool shouldUpdate = false;
            for (int i = 0; i < items.Length; i++)
            {
                ItemAmountWeighted itemAmountWeighted = items[i];
                if (!shortnames.Contains(itemAmountWeighted.Shortname))
                    keepItemsWeighted.Add(itemAmountWeighted);
                else
                {
                    count++;
        ...
        public class SimpleNPCLootProfile : BaseLootProfile
        {
            ...
            public override void PopulateLoot(ItemContainer container)
            {
                ...
                List<ItemAmountWeighted> items = Pool.Get<List<ItemAmountWeighted>>();
                items.AddRange(Items);
