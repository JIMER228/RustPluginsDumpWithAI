...
{
    [Info("Quests", "k1lly0u modet GRAZYCAT", "2.2.7", ResourceId = 1084)]
    ...
    public class Quests : RustPlugin
    {
        ...
        private Dictionary<ulong, Dictionary<ulong, int>> HeliAttackers = new Dictionary<ulong, Dictionary<ulong, int>>();

        private Dictionary<ulong, List<string>> OpenUI = new Dictionary<ulong, List<string>>();
        private Dictionary<ulong, ulong> Looters = new Dictionary<ulong, ulong>();
        ...
        void OnEntityTakeDamage(BaseCombatEntity victim, HitInfo info)
        {
            if (victim.GetComponent<BaseHelicopter>() != null && info?.Initiator?.ToPlayer() != null)
            {
                ...
                {
                    if (heli == null) return;
                    if (!HeliAttackers.ContainsKey(heli.net.ID.Value))
                        HeliAttackers.Add(heli.net.ID.Value, new Dictionary<ulong, int>());
                    if (!HeliAttackers[heli.net.ID.Value].ContainsKey(player.userID))
                        HeliAttackers[heli.net.ID.Value].Add(player.userID, 0);
                    HeliAttackers[heli.net.ID.Value][player.userID]++;
                });
            }
        }
        // Gather
    ...
    if (player == null)
        ...
        {
            if (Looters.ContainsKey(item.uid.Value))
            {
                if (container.playerOwner != null)
                {
                    if (Looters[item.uid.Value] != container.playerOwner.userID)
                    {
                        if (hasQuests(container.playerOwner.userID) && isQuestItem(container.playerOwner.userID, item.info.shortname, QuestType.Loot))
                        {
                            ...
                            Looters.Remove(item.uid.Value);
                        }
                    }
                }
            }
            else if (container.playerOwner != null) Looters.Add(item.uid.Value, container.playerOwner.userID);
        }
