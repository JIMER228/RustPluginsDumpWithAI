...
{
    [Info("AAlertRaidEN", "fermens", "2.0.3")]
    public class AAlertRaidEN : RustPlugin, IDiscordPlugin
    {
        ...
        private void OnEntityDeath(BaseCombatEntity entity, HitInfo info)
        {
            ...
            else if (config.extralist && (entity is DecayEntity || entity is IOEntity) || entity is AnimatedBuildingBlock || entity is SamSite || entity is AutoTurret || config.spisok != null && entity.ShortPrefabName != null && config.spisok.Contains(entity.ShortPrefabName))
            {
        ...
        private void ALERTPLAYER(ulong ID, string name, string quad, string connect, string destroy, string attackerid)
        {
            ...
            {
                NotificationList.SendNotificationTo(ID, NotificationChannel.SmartAlarm, GetMessage("alertrustplus", IDstring).Replace("{steamid}", attackerid).Replace("{ip}", connect).Replace("{name}", name).Replace("{destroy}", destroy).Replace("{quad}", quad).Replace("{servername}", config.servername), config.servername, Util.TryGetServerPairingData());
