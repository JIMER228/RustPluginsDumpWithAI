...
{
    [Info("LoadoutController", "by вимирающий вид", "2.0.8")]
            ...
            if (loadout.Count > 0)
            {
                ...
                foreach (var item in loadout)
                {
                    ...
                    if (theItem.info.category.ToString() == "Weapon")
                    {
                        ...
                        if (item.AmmoType != null && weapon != null)
                        {
                        ...
                        if (item.Mods != null)
                            foreach (var attachment in item.Mods)
                            {
                                ...
                                if (attachment == 2005491391 && weapon != null)
                                {
                                    weapon.primaryMagazine.contents = item.AmmoAmount;
                                    weapon.primaryMagazine.capacity = Mathf.CeilToInt(ProjectileWeaponMod.Mult(weapon, (x => x.magazineCapacity), (y => y.scalar), 1f) * weapon.primaryMagazine.definition.builtInSize);
                                }
