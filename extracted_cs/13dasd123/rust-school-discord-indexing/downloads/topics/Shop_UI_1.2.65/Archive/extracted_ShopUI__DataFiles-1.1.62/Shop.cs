...
{
    [Info("Shop", "Skuli Dropek", "1.1.62")]
    [Description("In-game store, standalone plugin compatible with WelcomePanel")]
    ...
    public class Shop : RustPlugin
    {
        ...
        {
            if (wipeBlock == null ||
                wipeBlock["Wipe Block"].Enabled == null ||
                !wipeBlock["Wipe Block"].Enabled ||
                !wipeBlock["Wipe Block"].StartOnMapWipe) return;
        ...
        private bool TakeFrom(BasePlayer player, string currencyType, int amount)
        {
            if (currencyType.ToLower() != "rp" && currencyType.ToLower() != "eco")
            {
                var itemDef = ItemManager.FindItemDefinition(currencyType);
                if (itemDef == null)
                { Puts($" <color=#C2291D>!</color> '{currencyType}' <color=#C2291D>is not correct shortname.</color>"); return false; }
        ...
        private bool BuildChecks(BasePlayer player)
        {
            ...
            if (!player.CanBuild() && config.ms.requireBuild)
            {
                ...
                if (config.ns.notifs)
                {
                    ...
                    if (config.ns.msgs["buildingBlocked"] == null)
                    {
            ...
            if (!player.IsBuildingAuthed() && config.ms.requireTC)
            {
                ...
                {
                    //create message if missing
                    if (config.ns.msgs["notTCAuthed"] == null)
                    {
        ...
        private void Sell(BasePlayer player, string category, string itemName, int amount)
        {
            ...
            //take currency
            if (TakeFrom(player, itemName, amount))
            {
        ...
        private int PriceAfterSales(BasePlayer player, string category, string itemName, bool sell = false, bool fullprice = false)
        {
            ...
            foreach (string perm in config.gs.perms.Keys)
            {
                if (permission.UserHasPermission(player.UserIDString, "shop." + perm))
                {
                    if (!availableSales.Contains(config.gs.perms[perm]))
                        availableSales.Append(config.gs.perms[perm]);
        ...
        private void CategorySelection(BasePlayer player, int page = 0)
        {
            ...
            if (config._as.addon)
            {
                ...
                for (int i = 0; i < _lenght; i++)
                {
                    ...
                    CUIClass.CreateButton(ref _cM, "cm_btn", "container", "0.115 0.115 0.115 0.9", $"", 22, anchorSplit[0], anchorSplit[1], $"shop_page {storedCategories[i + startingIndex]} 0", "", "1 1 1 0.7", 0.2f, TextAnchor.MiddleCenter, $"robotocondensed-bold.ttf", "assets/content/ui/uibackgroundblur.mat");
        ...
        private void CreateItemCui(BasePlayer player, int index, string item = "test", string category = "Attire")
        {
            ...
            if (currencyType != "eco" && currencyType != "rp")
            {
                price += "";
                pAnchor = "0.79";
                string currencyImage = $"https://rustlabs.com/img/items180/{currencyType}.png";
        ...
        private void CreatePageBtns(BasePlayer player, string category, int currentPage = 0, bool last = false, bool npc = false, string npcId = "0")
        {
            ...
            if (config._as.addon && categories.Count() > 1)
                CUIClass.CreateButton(ref _btnP, "btn_back", "container", $"{config.us.buttons["Page Button Color"]}", "BACK TO CATEGORIES", 9, "0.0 -0.09", "0.16 0", $"shop_categories", "", "1 1 1 0.7", 0.0f, TextAnchor.MiddleCenter, $"robotocondensed-regular.ttf", "assets/content/ui/uibackgroundblur.mat");

            if (config._as.addon)
            {
                try
                {
                    int template = WelcomePanel.Call<int>("GetLayout_API1");
        ...
        private void CreateSelectionCui(BasePlayer player, int index, string item, string category, string currencyType, bool sell = false, bool onInput = false)
        {
            ...
            if (item.Contains("cmd/") || item.Contains("command/"))
            {
                ...
                string _btnText = $"BUY <size={11 + fontAdjustment}><b>{_buyPrice}{_price}</b></size>";
            ...
            string btnText = $"BUY <size={11 + fontAdjustment}><b>{buyPrice}{price}</b></size>";
            ...
            if (sell)
            {
                btnText = $"SELL <size={11 + fontAdjustment}><b>{sellPrice}{price}</b></size>";
            ...
            //if amount change is disabled 
            string inputDefTextCmd = "shop_cmd default_text";
            ...
            //CUIClass.CreateButton(ref _slc, "amount_btn", $"item{index}", "0 0 0 0.0", $"{amountStorage[player.userID]}", 9, "0.035 0.05", $"0.22 0.23", inputDefTextCmd, "", "1 1 1 0.8", 0.1f, TextAnchor.MiddleCenter, $"robotocondensed-regular.ttf", "assets/icons/iconmaterial.mat");
        ...
        private void CurrencyBtns(BasePlayer player)
        {
            ...
            {
                double playersEco = Economics.Call<double>("Balance", player.UserIDString);
                text += $"<b><color=#F9F9F9>{playersEco}</color></b>$";

                if (ServerRewards != null)
                {
                    var playersRP = ServerRewards?.Call<int>("CheckPoints", player.userID);
                    text += $" <size=11>/</size> <b><color=#F9F9F9>{playersRP}</color></b>RP";
                    anchorMax = "0.325 0";
        ...
        private void DownloadImages()
        {
            ...
            if (config.gl.forceImage != null)
            {
                ...
                if (!imgList.Contains(config.gl.forceImage))
                    imgList.Add(config.gl.forceImage);
            }
            ...
            foreach (string item in items.Keys)
            {
                ...
                if (!items[item].Image.StartsWith("http"))
                {
                    if (!(bool)ImageLibrary.Call("HasImage", prefix + items[item].Image))
                        ImageLibrary.Call("AddImage", prefix + items[item].Image, prefix + items[item].Image);

                    if (!imgList.Contains(prefix + items[item].Image))
                        imgList.Add(prefix + items[item].Image);
                }
                else
                {
                    if (!(bool)ImageLibrary.Call("HasImage", items[item].Image))
                        ImageLibrary.Call("AddImage", items[item].Image, items[item].Image);

                    if (!imgList.Contains(items[item].Image))
                        imgList.Add(items[item].Image);
                }
            }
            ...
            foreach (string category in categories.Keys)
            {
                ...
                imgList.Add(categories[category].Image);
            }

            //add ui images
            ImageLibrary.Call("AddImage", config.us.main["Image"], config.us.main["Image"]); imgList.Add(config.us.main["Image"]);
            ImageLibrary.Call("AddImage", config.us.side["Image"], config.us.side["Image"]); imgList.Add(config.us.side["Image"]);
            ImageLibrary.Call("AddImage", config.us.logo["Image"], config.us.logo["Image"]); imgList.Add(config.us.logo["Image"]);
        ...
        private void ImageQueCheck()
        {
            ...
            if (imgCount > downloaded)
                Puts($"(!) Stored Images ({downloaded}/{imgCount}). Reload ImageLibrary and then Shop plugin to start download order.");
        ...
        private void CreateDefaultItems()
        {
            ...
            foreach (var item in _items)
            {
                ...
                items[$"{item.shortname}"].Image = $"{item.shortname}.png";
        ...
        private void LoadCommandData()
        {
            ...
            else
            {
                ...
                commands["YourCommandName"].Image = $"https://rustlabs.com/img/items180/water.salt.png";
                ...
                commands["YourCommandName2"].Image = $"https://rustlabs.com/img/items180/water.salt.png";
        ...
        {
            if (config._as.autoTemplate && config._as.addon)
            {
                int wpLayout = WelcomePanel.Call<int>("GetLayout_API1");
                // just in case...
                if (wpLayout > 0 && wpLayout < 6)
                {
                    layer = "imagePanel";
                    if (wpLayout == 1)
                    {
                        config._as.offset["AnchorMin"] = "0.03 0.15";
                        config._as.offset["AnchorMax"] = "0.983 0.935";
                        config._as.offset["OffSetMin"] = "0 0";
                        config._as.offset["OffSetMax"] = "0 0";

                    }
                    if (wpLayout == 2)
                    {
                        fontAdjustment = 5;
                        config._as.offset["AnchorMin"] = "0 0.12";
                        config._as.offset["AnchorMax"] = "1 0.93";
                        config._as.offset["OffSetMin"] = "0 0";
                        config._as.offset["OffSetMax"] = "0 0";
                    }
                    if (wpLayout == 3)
                    {
                        config._as.offset["AnchorMin"] = "0 0.15";
                        config._as.offset["AnchorMax"] = "1 0.97";
                        config._as.offset["OffSetMin"] = "0 0";
                        config._as.offset["OffSetMax"] = "0 0";
                    }
                    if (wpLayout == 4)
                    {
                        config._as.offset["AnchorMin"] = "0.075 0.12";
                        config._as.offset["AnchorMax"] = "0.95 1";
                        config._as.offset["OffSetMin"] = "0 0";
                        config._as.offset["OffSetMax"] = "0 0";
                    }
                    if (wpLayout == 5)
                    {
                        fontAdjustment = 2;
                        config._as.offset["AnchorMin"] = "0 0.12";
                        config._as.offset["AnchorMax"] = "1.03 0.93";
                        config._as.offset["OffSetMin"] = "0 0";
                        config._as.offset["OffSetMax"] = "0 0";
                    }
                    SaveConfig();
        ...
        class Configuration
        {
            ...
            public class AS
            {
                ...
                [JsonProperty("» Auto Detect Template")]
