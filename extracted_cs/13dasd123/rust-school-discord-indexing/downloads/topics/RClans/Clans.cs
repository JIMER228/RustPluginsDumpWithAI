...
{
    [Info("Clans", "always work", "1.0.66", ResourceId = 14)]
    public class Clans : RustPlugin
    {
        ...
        void LoadVariables()
        {
            ...
            PointsOfDeath = Convert.ToInt32(GetConfig("ТОП", "Очки: Сколько очков убираем за смерть", 1));
            ...
            PointsOfSuicide = Convert.ToInt32(GetConfig("ТОП", "Очки: Сколько очков забираем за самоубийство", 1));
            PointFarm = Convert.ToInt32(GetConfig("ТОП", "Скок очков ауе за фарм?", 1));
            ...
            limitpvo = Convert.ToInt32(GetConfig("Ограничения объектов", "Лимит на пво", 10));
            limitobj = Convert.ToInt32(GetConfig("Ограничения объектов", "Лимит на объекты", 8000));
            limitcups = Convert.ToInt32(GetConfig("Ограничения объектов", "Лимиты на шкаф", 100));
            limitturret = Convert.ToInt32(GetConfig("Ограничения объектов", "Лимиты на турели", 100));
            ...
            clanTagColorBetterChat = Convert.ToString(GetConfig("BetterChat", "Цвет НЕЙМА кланов в чате", "#aaff55"));
            clanTagSizeBetterChat = Convert.ToInt32(GetConfig("BetterChat", "Размер НЕЙМА клана в чате", 15));
        ...
        string building = "https://i.postimg.cc/brcmQ1kH/EkzSLVm.png";
        string onlinepng = "https://i.postimg.cc/gjzZFJWb/O1T5M2S.png";
        string Closed = "https://i.postimg.cc/dt0y5L68/6xO5e95.png";
        string cupboarding = "https://i.postimg.cc/023bZZbs/vE0gHbR.png";
        string turretka = "https://i.postimg.cc/HxBLM9fF/aF0lwQE.png";
        string SamSite = "https://i.postimg.cc/wTSjDvGx/jinccMR.png";
        string but = "https://i.postimg.cc/tJ8pdSDv/ds.png";
        ...
        void OnServerInitialized()
        {
            ...
            else
            {
                ...
                ImageLibrary?.Call("AddImage", building, building);
                ImageLibrary?.Call("AddImage", Closed, Closed);
                ImageLibrary?.Call("AddImage", SamSite, SamSite);
                ImageLibrary?.Call("AddImage", but, but);
                ImageLibrary?.Call("AddImage", cupboarding, cupboarding);
                ImageLibrary?.Call("AddImage", turretka, turretka);
            ...
            timer.Every(1f, ClanPlayerTime);

            ImageLibrary?.Call("AddImage", "https://i.postimg.cc/nhJM6rYR/MbO0Yhi.png", "loot-barrel");
            ImageLibrary?.Call("AddImage", "https://i.postimg.cc/fTZNs9p9/teamwork.png", "logopng");
            ImageLibrary?.Call("AddImage", "https://i.postimg.cc/DZdxP8g9/0kwHAsx.png", "closepngrr");

            ImageLibrary?.Call("AddImage", "https://i.postimg.cc/N0RDQ5WK/imag111111e.png", "fonercoketcreate");
        }
        ...
        void Unload()
        {
            ...
            foreach (BasePlayer player in BasePlayer.activePlayerList)
            {
                ...
                CuiHelper.DestroyUi(player, "MainSkinLayer");
        ...
        private void ClanPlayerTime()
        {
            foreach (var player in BasePlayer.activePlayerList)
            {
                var clan = findClanByUser(player.UserIDString);
                if (clan == null) continue;
                clan.members[player.UserIDString].PlayerTime += 1;
            }
        }
        ...
        private object CanBuild(Planner builder, Construction prefab, Construction.Target target)
        {
            ...
            if (clan == null)
            {
                player.ChatMessage("Для строительства вам не обходим клан");
                return false;
            }
            if (prefab.fullName.Contains("autoturret_deployed"))
            {
                if (clan.turret >= limitturret )
                {
                    player.ChatMessage($"Лимит вашего клана на автоматическую турель исчерпан.");
                    return false;
                }
                else
                {
                    clan.turret++;
                    player.ChatMessage($"Вы успешно установили автоматическую турель, ваш текущий лимит: {clan.turret}/{limitturret}");
                    return null;
                }
            }
            if (prefab.fullName.Contains("cupboard.tool"))
            {
                if (clan.cups >= limitcups)
                {
                    player.ChatMessage($"Лимит вашего клана на шкаф исчерпан.");
                    return false;
                }
                else
                {
                    clan.cups++;
                    player.ChatMessage($"Вы успешно установили шкаф, ваш текущий лимит: {clan.cups}/{limitcups}");
                    return null;
                }
            }
            if (prefab.fullName.Contains("sam_site_turret_deployed"))
            {
                if (clan.pvo >= limitpvo)
                {
                    player.ChatMessage($"Лимит вашего клана на пво исчерпан.");
                    return false;
                }
                else
                {
                    clan.pvo++;
                    player.ChatMessage($"Вы успешно установили пво, ваш текущий лимит: {clan.pvo}/{limitpvo}");
                    return null;
                }
            }
            if (prefab.fullName.Contains("assets/prefabs/building core"))
            {
                if (clan.obj >= limitobj)
                {
                    player.ChatMessage($"Лимит вашего клана на обьекты исчерпан.");
                    return false;
                }
                else
                {
                    clan.obj++;
                    player.ChatMessage($"Вы успешно установили обьект, ваш текущий лимит: {clan.obj}/{limitobj}");
                    return null;
                }
            }
            return null;
        ...
        private void OnEntityKill(BaseEntity entity)
        {
            ...
            var clan = findClanByUser(entity.OwnerID.ToString());
            if (clan == null) return;
            ...
            if (entity.PrefabName.Contains("sam_site_turret_deployed"))
            {
                clan.pvo--;
            }
        ...
        void cmdChatClan(BasePlayer player, string command, string[] args)
        {
            ...
            if (opt == msg("clanArgCreate", player.UserIDString))
            {
                ...
                if (myClan != null)
                {
                    player.ChatMessage($"Вы уже являетесь участником клана {myClan.tag}");
                    return;
                }

                cmdClanCreate(player);
                return;
            }
        ...
        void NewClanUI(BasePlayer player, int page = 0, bool leadermode = false)
        {
            ...
            int i = 1 + (page * 5);
            float width = 0.455f, height = 0.055f, startxBox = 0.043f, startyBox = 0.53f - height, xmin = startxBox, ymin = startyBox;
            var clan = findClanByUser(player.UserIDString);
            if (clan == null) return;
            #endregion
            ...
            elements.Add(new CuiPanel
            {
                RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-545 -314.5", OffsetMax = "547.5 316" },
                Image = { Color = "0 0 0 0.225", Material = "assets/content/ui/uibackgroundblur.mat" }
            }, "clans_main1", "clans_main1layer");

            elements.Add(new CuiElement
            {
                Parent = "clans_main1layer",
                Components =
                {
                    new CuiImageComponent { Color = "0 0 0 0.99" },
            ...
            {
                Text = { Text = $"Клановая система", Color = "1 1 1 1", FontSize = 31, Font = "robotocondensed-regular.ttf", Align = TextAnchor.UpperLeft },
                RectTransform = { AnchorMin = $"0.045 0", AnchorMax = $"1 0.965" },
            }, "clans_main1layer");
            #endregion
            ...
            elements.Add(new CuiPanel
            {
                RectTransform = { AnchorMin = "0.046 0.585", AnchorMax = "0.495 0.863" },
                Image = { Color = "0 0 0 0.45" }
            }, "clans_main1layer", "clans_main1layer" + ".Info");

            elements.Add(new CuiPanel
            {
                RectTransform = { AnchorMin = "0.5025 0.585", AnchorMax = "0.957 0.863" },
                Image = { Color = "0 0 0 0.45" }
            }, "clans_main1layer", "clans_main1layer" + ".Wear");

            elements.Add(new CuiPanel
            {
                RectTransform = { AnchorMin = "0.5025 0.14", AnchorMax = "0.957 0.527" },
                Image = { Color = "0 0 0 0.45" }
            }, "clans_main1layer", "clans_main1layer" + ".Resourse");
            #endregion

            #region [Text]
            elements.Add(new CuiPanel
            {
                RectTransform = { AnchorMin = "0.046 0.545", AnchorMax = "0.495 0.565" },
                Image = { Color = "0 0 0 0" }
            }, "clans_main1layer", "clans_main1layertext");

            elements.Add(new CuiLabel
            {
                Text = { Text = $"Имя игрока", Color = "1 1 1 0.45", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleLeft },
                RectTransform = { AnchorMin = $"0.112 0", AnchorMax = $"1 1" },
            }, "clans_main1layertext");

            elements.Add(new CuiLabel
            {
                Text = { Text = $"Статус", Color = "1 1 1 0.45", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleLeft },
                RectTransform = { AnchorMin = $"0.35 0", AnchorMax = $"1 1" },
            }, "clans_main1layertext");

            elements.Add(new CuiLabel
            {
                Text = { Text = $"Норма", Color = "1 1 1 0.45", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleLeft },
                RectTransform = { AnchorMin = $"0.555 0", AnchorMax = $"1 1" },
            }, "clans_main1layertext");

            elements.Add(new CuiLabel
            {
                Text = { Text = $"Управление", Color = "1 1 1 0.45", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleLeft },
                RectTransform = { AnchorMin = $"0.785 0", AnchorMax = $"1 1" },
            }, "clans_main1layertext");
            #endregion

            #region [PlayerInfo]
            foreach (var key in clan.members.OrderBy(pair => BasePlayer.FindByID(ulong.Parse(pair.Key)) == null).Skip(5 * page).Take(clan.members.ToList().Count >= 5 ? 5 : clan.members.ToList().Count))
            {
                elements.Add(new CuiPanel
                {
                    RectTransform = { AnchorMin = $"{xmin} {ymin}", AnchorMax = $"{xmin + width} {ymin + height * 1}", OffsetMin = "2 1", OffsetMax = "-2 -1" },
                    Image = { Color = colored }
                }, "clans_main1layer", "PlayerLine");

                elements.Add(new CuiLabel
                {
                    Text = { Text = $"{i}. {covalence.Players.FindPlayerById(key.Key).Name}", Font = "robotocondensed-regular.ttf", FontSize = 12, Color = "1 0.9294118 0.8666667 0.9", Align = TextAnchor.MiddleCenter },
                    RectTransform = { AnchorMin = $"0 0", AnchorMax = $"0.34 1" },
                }, "PlayerLine");

                elements.Add(new CuiLabel
                {
                    Text = { Text = GetPlayerStatus(key.Key, clan), Color = "1 0.9294118 0.8666667 1", Font = "robotocondensed-regular.ttf", FontSize = 12, Align = TextAnchor.MiddleLeft },
                    RectTransform = { AnchorMin = $"0.35 0", AnchorMax = $"0.6526684 1" },
                }, "PlayerLine");

                elements.Add(new CuiLabel
                {
                    Text = { Text = $"{GetFullPercent(key.Key)}%", Color = "1 0.9294118 0.8666667 1", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleLeft },
                    RectTransform = { AnchorMin = $"0.57 0", AnchorMax = $"0.814169 1" },
                }, "PlayerLine");
            ...
            elements.Add(new CuiElement
            {
                Parent = "clans_main1layer" + ".Info",
                Components =
                {
                    new CuiRawImageComponent { Png = (string)ImageLibrary?.Call("GetImage", "logopng") },
                    new CuiRectTransformComponent { AnchorMin = "0.115 0.4", AnchorMax = "0.275 0.9" }
                }
            });

            elements.Add(new CuiLabel
            {
                Text = { Text = $"{clan.tag}", Color = "1 1 1 1", FontSize = 24, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleLeft },
                RectTransform = { AnchorMin = $"0.33 0.615", AnchorMax = $"1 1" },
            }, "clans_main1layer" + ".Info");

            elements.Add(new CuiLabel
            {
                Text = { Text = $"Игроков в клане: <color=orange>{clan.online}/{clan.members.Count}</color>", Color = "1 1 1 0.45", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleLeft },
                RectTransform = { AnchorMin = $"0.327 0.32", AnchorMax = $"1 1" },
            }, "clans_main1layer" + ".Info");

            elements.Add(new CuiLabel
            {
                Text = { Text = $"Место в топе: <color=orange>{GetClanIndex(clan.tag)}</color>", Color = "1 1 1 0.45", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleLeft },
                RectTransform = { AnchorMin = $"0.327 0.09", AnchorMax = $"1 1" },
            }, "clans_main1layer" + ".Info");

            elements.Add(new CuiLabel
            {
                Text = { Text = $"Глава клана: <color=orange>{clan.ownerName}</color>", Color = "1 1 1 0.45", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleLeft },
                RectTransform = { AnchorMin = $"0.327 0", AnchorMax = $"1 0.86" },
            }, "clans_main1layer" + ".Info");
            #endregion
            ...
            elements.Add(new CuiLabel
            {
                Text = { Text = $"Одежда клана", Color = "1 1 1 1", FontSize = 26, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0 0.625", AnchorMax = $"0.42 1" },
            }, "clans_main1layer" + ".Wear");

            elements.Add(new CuiButton
            {
                Button = { Color = "0.13 0.62 0.27 1.00", Command = clan.IsOwner(player.UserIDString) ? $"clan_itemChange" : "" },
                Text = { Text = "Настроить", Color = "1 1 1 1", FontSize = 14, Font = "robotocondensed-bold.ttf", Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0.707 0.75", AnchorMax = $"0.951 0.8625" },
            }, "clans_main1layer" + ".Wear");

            foreach (var check in clan.SkinList["wear"].Select((y, t) => new { A = y, B = t }).Take(7))
            {
                elements.Add(new CuiPanel
                {
                    RectTransform = { AnchorMin = $"{0.051 + check.B * 0.1307 - Math.Floor((float) check.B / 7) * 7 * 0.1307} {0.26 - Math.Floor((float) check.B/ 7) * 0}",
                                        AnchorMax = $"{0.165 + check.B * 0.1307 - Math.Floor((float) check.B / 7) * 7 * 0.1307} {0.575 - Math.Floor((float) check.B / 7) * 0}", },
                    Image = { Color = "0 0 0 0.45" }
                }, "clans_main1layer" + ".Wear", "clans_main1layer" + ".Wear" + $".{check.B}");

                elements.Add(new CuiElement
                {
                    Parent = "clans_main1layer" + ".Wear" + $".{check.B}",
                    Components =
                    {
            ...
            elements.Add(new CuiLabel
            {
                Text = { Text = $"Норма клана", Color = "1 1 1 1", FontSize = 26, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0 0.72", AnchorMax = $"0.39 1" },
            }, "clans_main1layer" + ".Resourse");

            elements.Add(new CuiButton
            {
                Button = { Color = "0.13 0.62 0.27 1.00", Command = clan.IsOwner(player.UserIDString) ? $"clan_setChange" : "" },
                Text = { Text = "Настроить", Color = "1 1 1 1", FontSize = 14, Font = "robotocondensed-bold.ttf", Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0.707 0.8", AnchorMax = $"0.951 0.88" },
            }, "clans_main1layer" + ".Resourse");

            foreach (var check in clan.Change.Select((y, t) => new { A = y, B = t }))
            {
                elements.Add(new CuiPanel
                {
                    RectTransform = { AnchorMin = $"{0.051 + check.B * 0.1307 - Math.Floor((float) check.B / 7) * 7 * 0.1307} {0.47 - Math.Floor((float) check.B/ 7) * 0.27}",
                                        AnchorMax = $"{0.165 + check.B * 0.1307 - Math.Floor((float) check.B / 7) * 7 * 0.1307} {0.7 - Math.Floor((float) check.B / 7) * 0.27}", },
                    Image = { Color = "0 0 0 0.45" }
                }, "clans_main1layer" + ".Resourse", "clans_main1layer" + ".Resourse" + $".{check.B}");

                if (FindItemID(check.A.Key) != 0)
                {
                    elements.Add(new CuiElement
                    {
                        Parent = "clans_main1layer" + ".Resourse" + $".{check.B}",
                        Components =
                        {
                ...
                elements.Add(new CuiElement
                {
                    ...
                    Components =
                    {
                        new CuiTextComponent { Text = $"{check.A.Value.Need}", Color = "1 1 1 1", Align = TextAnchor.MiddleCenter, FontSize = 12, Font = "robotocondensed-regular.ttf" },
                        new CuiRectTransformComponent { AnchorMin = $"0 0", AnchorMax = $"1 0.35" },
                    }
                });
            }
            #endregion
            ...
            elements.Add(new CuiElement
            {
                Name = "clans_main1layer" + ".Button",
                Parent = $"clans_main1layer",
                Components =
                {
                    new CuiRawImageComponent { Png = (string)ImageLibrary?.Call("GetImage", "closepngrr")},
                    new CuiRectTransformComponent { AnchorMin = "0.92 0.91", AnchorMax = "0.957 0.975" }
                }
            });

            elements.Add(new CuiButton
            {
                Button = { Color = "0 0 0 0", Close = "clans_main1" },
                RectTransform = { AnchorMin = $"0 0", AnchorMax = $"1 1" },
            }, "clans_main1layer" + ".Button");
            #endregion
            ...
            elements.Add(new CuiButton
            {
                ...
                Text = { Text = $"{page + 1}", Color = "1 0.9294118 0.8666667 1", Font = "robotocondensed-regular.ttf", FontSize = 22, Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0.265 0.045", AnchorMax = $"0.275 0.105" },
            }, "clans_main1layer");

            elements.Add(new CuiButton
            {
                Button = { Color = "0 0 0 0", Command = page > 0 ? $"clan.page {page - 1}" : "" },
                Text = { Text = "◀", Color = page > 0 ? $"1.00 1.00 0.00 1.00" : "0.20 0.20 0.20 0.98", FontSize = 23, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0.245 0.045", AnchorMax = $"0.265 0.105" },
            }, "clans_main1layer");

            elements.Add(new CuiButton
            {
                Button = { Color = "0 0 0 0", Command = clan.members.Skip(6 * (page + 1)).Count() > 0 ? $"clan.page {page + 1}" : "" },
                Text = { Text = "▶", Color = page > 0 ? $"1.00 1.00 0.00 1.00" : "0.20 0.20 0.20 0.98", FontSize = 23, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0.275 0.045", AnchorMax = $"0.295 0.105" },
            }, "clans_main1layer");
            #endregion
        ...
        [ConsoleCommand("testseta")]
        void ClanSetSkinhed(ConsoleSystem.Arg arg)
        {
            ...
            {
                player.ChatMessage("Вы не выбрали сэт который хотите!\nЧтобы выбрать сэт пропишите:\n<color=#8ebd2e>/clanset</color> Набор..\nДоступные наборы:\nЧерный - black\nБелый - white\nЗленый - green\nПесочный - pesok\nКомикс - komicks\nТрап - Коровий");
                return;
            }
            string Set = arg.Args[0];
            if (Set == "raduga")
            {
                clan.SkinList["wear"]["metal.facemask"] = 2375037928;
                clan.SkinList["wear"]["metal.plate.torso"] = 2375036472;
                clan.SkinList["wear"]["roadsign.kilt"] = 2429521544;
                clan.SkinList["wear"]["hoodie"] = 2375033766;
                clan.SkinList["wear"]["pants"] = 2375034694;
                clan.SkinList["wear"]["shoes.boots"] = 2375042259;
                clan.SkinList["weapon"]["rifle.ak"] = 2360689888;
                player.ChatMessage("Вы выбрали Радужный сэт!");
            }
            if (Set == "red")
            {
            ...
            if (Set == "ender")
            {
                clan.SkinList["wear"]["metal.facemask"] = 2800771290;
                clan.SkinList["wear"]["metal.plate.torso"] = 2800771470;
                clan.SkinList["wear"]["roadsign.kilt"] = 2800771660;
                clan.SkinList["wear"]["hoodie"] = 2800765949;
                clan.SkinList["wear"]["pants"] = 2800766264;
                clan.SkinList["wear"]["shoes.boots"] = 2800766795;
                clan.SkinList["weapon"]["rifle.ak"] = 2800768824;
                player.ChatMessage("Вы выбрали Эндер сэт!");
            }
            if (Set == "uzor")
            {
                clan.SkinList["wear"]["metal.facemask"] = 2372284024;
                clan.SkinList["wear"]["metal.plate.torso"] = 2372279687;
                clan.SkinList["wear"]["roadsign.kilt"] = 2372313526;
                clan.SkinList["wear"]["hoodie"] = 2372357343;
                clan.SkinList["wear"]["pants"] = 2372299533;
                clan.SkinList["wear"]["shoes.boots"] = 237232780;
                clan.SkinList["weapon"]["rifle.ak"] = 2817939150;
                player.ChatMessage("Вы выбрали Черный сэт!");
            }
            if (Set == "white")
            {
                clan.SkinList["wear"]["metal.facemask"] = 2366303225;
                clan.SkinList["wear"]["metal.plate.torso"] = 2365802374;
                ...
                clan.SkinList["weapon"]["rifle.ak"] = 2365578339;
                player.ChatMessage("Вы выбрали Сакура сэт!");
            }
            if (Set == "blue")
            {
                clan.SkinList["wear"]["metal.facemask"] = 1342380888;
                clan.SkinList["wear"]["metal.plate.torso"] = 1342389802;
                clan.SkinList["wear"]["roadsign.kilt"] = 2799910737;
                clan.SkinList["wear"]["hoodie"] = 1342381804;
                clan.SkinList["wear"]["pants"] = 1342382423;
                clan.SkinList["wear"]["shoes.boots"] = 1342388900;
                clan.SkinList["weapon"]["rifle.ak"] = 2799907679;
                player.ChatMessage("Вы выбрали Голубой сэт!");
            }
            if (Set == "snow")
            {
                clan.SkinList["wear"]["metal.facemask"] = 2799341233;
                clan.SkinList["wear"]["metal.plate.torso"] = 2799340445;
                clan.SkinList["wear"]["roadsign.kilt"] = 2799342952;
                clan.SkinList["wear"]["hoodie"] = 2799342491;
                clan.SkinList["wear"]["pants"] = 2799341855;
                clan.SkinList["wear"]["shoes.boots"] = 2799344361;
                clan.SkinList["weapon"]["rifle.ak"] = 2799337106;
                player.ChatMessage("Вы выбрали Снежный сэт!");
            }
            if (Set == "lightning")
            {
                clan.SkinList["wear"]["metal.facemask"] = 2905493124;
                clan.SkinList["wear"]["metal.plate.torso"] = 2905492479;
                clan.SkinList["wear"]["roadsign.kilt"] = 2905494067;
                clan.SkinList["wear"]["hoodie"] = 2905490200;
                clan.SkinList["wear"]["pants"] = 2905493516;
                clan.SkinList["wear"]["shoes.boots"] = 2905495802;
                clan.SkinList["weapon"]["rifle.ak"] = 2905489710;
                player.ChatMessage("Вы выбрали Штормовой сэт");
            }
            player.ChatMessage("Если вы не нашли нужный сэт пишите нам группу в вк!\nМы обязательно добавим этот сэт!");
        }

        [ChatCommand("2626464643")]
        ...
        void cmdClanOverview(BasePlayer player)
        {
            ...
            if (myClan == null)
            {
                player.ChatMessage("Вы не являетесь участником клана. Больше информации /clan help");
                return;
            }
            NewClanUI(player, 0);
        }

        [ConsoleCommand("UI_CREATE")]
        private void cmdCreateClan(ConsoleSystem.Arg args)
        {
            ...
            {
                player.ChatMessage("У вас нету разрешения для создания клана");
                return;
            }

            if (tagReExt.IsMatch(args.Args[0]))
            {
                PrintChat(player, string.Format(msg("hintchars", current.Id), allowedSpecialChars));
                return;
            }

            if (enableWordFilter && FilterText(args.Args[0]))
            {
            ...
            if (clanKeys.Contains(args.Args[0].ToLower()))
            {
                PrintChat(player, string.Format(msg("tagblocked", current.Id)));
                return;
            }

            myClan = Clan.Create(args.Args[0], string.Empty, current.Id, current.Name, "https://www.guilded.gg/asset/GameIcons/Rust-lg.png");
            ...
            myClan.OnCreate();
            myClan.total++;
            CuiHelper.DestroyUi(player, "CreateClanLayer");
            player.ChatMessage($"Вы успешно создали клан <color=#9ACD32>{myClan.tag}</color>.");
        }

        private void cmdClanCreate(BasePlayer player)
        {
            CuiHelper.DestroyUi(player, "CreateClanLayer");
            ...
            container.Add(new CuiElement
            {
                Name = "CreateClanLayer" + ".Main" + ".Close",
                Parent = "CreateClanLayer" + ".Main",
                Components =
                {
                    new CuiRawImageComponent { Png = (string)ImageLibrary?.Call("GetImage", "closepngrr")},
                    new CuiRectTransformComponent { AnchorMin = $"0.905 0.825", AnchorMax = $"0.99 0.99" }
                }
            });

            container.Add(new CuiButton
            {
                Button = { Color = "0 0 0 0", Close = "CreateClanLayer" },
                RectTransform = { AnchorMin = $"0 0", AnchorMax = $"1 1" },
            }, "CreateClanLayer" + ".Main" + ".Close");

            container.Add(new CuiButton
            {
                RectTransform = { AnchorMin = "0.295 0.323", AnchorMax = "0.704 0.461" },
                Button = { Color = "0.13 0.62 0.27 1.00", Command = $"" },
                Text = { Text = $"Создать", Color = "1 1 1 0.65", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-bold.ttf", FontSize = 14 }
            }, "CreateClanLayer" + ".Main");

            container.Add(new CuiElement
            {
                Name = "CreateClanLayer" + ".Main" + ".Input",
                Parent = "CreateClanLayer" + ".Main",
                Components =
                {
                    new CuiImageComponent { Color = "0 0 0 0.55" },
                    new CuiRectTransformComponent { AnchorMin = "0.295 0.53", AnchorMax = "0.704 0.671" }
                }
            });

            container.Add(new CuiElement()
            {
                Parent = "CreateClanLayer" + ".Main" + ".Input",
                Components =
                {
            ...
            container.Add(new CuiElement
            {
                ...
                Components =
                {
                    new CuiTextComponent { Text = $"Название клана", Color = "1 1 1 0.35", FontSize = 15, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter},
                    new CuiRectTransformComponent { AnchorMin = "0.295 0.68", AnchorMax = "0.704 0.771" },
                }
            });
        ...
        string ButtonListed = "[{\"name\":\"clans_player{id}\",\"parent\":\"clans_main7\",\"components\":[{\"type\":\"UnityEngine.UI.Image\",\"color\":\"{color}\"},{\"type\":\"RectTransform\",\"anchormin\":\"0.09 {amin}\",\"anchormax\":\"0.9975 {amax}\",\"offsetmin\":\"0 0\",\"offsetmax\":\"1 1\"}]},{\"name\":\"CuiElement\",\"parent\":\"clans_player{id}\",\"components\":[{\"type\":\"UnityEngine.UI.Text\",\"text\":\"\",\"fontSize\":16,\"font\":\"robotocondensed-bold.ttf\",\"align\":\"MiddleCenter\"},{\"type\":\"UnityEngine.UI.Outline\",\"color\":\"0 0 0 0.2784314\",\"distance\":\"1 -1\"},{\"type\":\"RectTransform\",\"anchormin\":\"0 0\",\"anchormax\":\"1 1\",\"offsetmin\":\"0 0\",\"offsetmax\":\"1 1\"}]},{\"name\":\"CuiElement\",\"parent\":\"clans_player{id}\",\"components\":[{\"type\":\"UnityEngine.UI.Button\",\"command\":\"{command}\",\"color\":\"1 1 1 0\"},{\"type\":\"RectTransform\",\"anchormin\":\"0 0\",\"anchormax\":\"1 1\",\"offsetmin\":\"0 0\",\"offsetmax\":\"1 1\"}]}]";
            ...
            if (clan != null && clan.members[player.UserIDString].GatherInfo.ContainsKey(item.info.shortname))
            {
                ...
                switch (item.info.shortname)
                {
                    case "stones":
                        ...
                        clan.members[player.UserIDString].FarmPlayer += item.amount;
                        clan.Farm += item.amount;
                        break;
                    case "wood":
            ...
            {
                {
                    ...
                    switch (item.info.shortname)
                    {
                        case "stones":
                            ...
                            {
                                clan.ClanPoints += 2;
                                clan.members[player.UserIDString].PlayerPoints += 2;
                            }
                            break;
                        case "wood":
                            clan.members[player.UserIDString].GatherInfo["wood"] = clan.members[player.UserIDString].GatherInfo["wood"] + item.amount;
        ...
        private void SelectSkinUi(BasePlayer player, string shortName, int page = 0)
        {
            ...
            elements.Add(new CuiPanel
            {
                CursorEnabled = true,
                RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1" },
                Image = { Color = "0 0 0 0.45", Material = "assets/content/ui/uibackgroundblur-ingamemenu.mat" }
            }, "Overlay", "MainSkinLayer");
            #endregion

            #region [Main-Gui]
            elements.Add(new CuiElement
            {
                Name = "MainSkinLayerStartSet",
                Parent = "MainSkinLayer",
                Components =
                {
                    new CuiImageComponent { Color = "0 0 0 0.45" },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-255 -235.5", OffsetMax = "260 180"}
                }
            });

            elements.Add(new CuiElement
            {
                Name = "MainSkinLayerStarHad",
                Parent = "MainSkinLayer",
                Components =
                {
                    new CuiImageComponent { Color = "0 0 0 0.65" },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-255 180", OffsetMax = "260 220" }
                }
            });
            #endregion
            ...
            elements.Add(new CuiElement
            {
                Parent = $"MainSkinLayerStarHad",
                Components =
                {
                    new CuiTextComponent { Text = $"Скины для <color=yellow>{ItemManager.FindItemDefinition(shortName)?.displayName?.english}</color>", Color = "1 0.9294118 0.8666667 1", FontSize = 28, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter},
                    new CuiRectTransformComponent { AnchorMin = "0.035 0", AnchorMax = "1 1" },
                }
            });
            #endregion
            ...
            elements.Add(new CuiElement
            {
                Name = "ImgButtonsClancc",
                Parent = $"MainSkinLayerStarHad",
                Components =
                {
                    new CuiRawImageComponent { Png = (string)ImageLibrary?.Call("GetImage", "closepngrr") },
                    new CuiRectTransformComponent { AnchorMin = "0.925 0.1", AnchorMax = "0.9955 0.97425" }
                }
            });

            elements.Add(new CuiButton
            {
                Button = { Color = "0 0 0 0", Command = "UI_CLAN close MainSkinLayer UICLAN_edit" },
                RectTransform = { AnchorMin = $"0 0", AnchorMax = $"1 1" },
            }, "ImgButtonsClancc");
            #endregion
            ...
            foreach (var check in SkinList[shortName].Select((i, t) => new { A = i, B = t - page * totalAmount }).Skip(page * totalAmount).Take(totalAmount))
            {
                elements.Add(new CuiPanel
                {
                    RectTransform = { AnchorMin = $"{0.015 + check.B * 0.2 - Math.Floor((float) check.B / 5) * 5 * 0.2} {0.755 - Math.Floor((float) check.B/ 5) * 0.24}",
                                        AnchorMax = $"{0.187 + check.B * 0.2 - Math.Floor((float) check.B / 5) * 5 * 0.2} {0.969 - Math.Floor((float) check.B / 5) * 0.24}", },
                    Image = { Color = "0 0 0 0.45" }
                }, "MainSkinLayerStartSet", "MainSkinLayerStartSet" + ".Wear" + $".{check.B}");

                elements.Add(new CuiElement
                {
                    Parent = "MainSkinLayerStartSet" + ".Wear" + $".{check.B}",
                    Components =
                    {
                ...
                elements.Add(new CuiButton
                {
                    ...
                    Text = { Text = "" },
                    RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1" },
                }, "MainSkinLayerStartSet" + ".Wear" + $".{check.B}");
            }
            ...
            elements.Add(new CuiElement
            {
                ...
                Components =
                {
                    new CuiTextComponent { Text = $"Указать скин для <color=yellow>{ItemManager.FindItemDefinition(shortName)?.displayName?.english}</color>", Color = "1 1 1 1", FontSize = 16, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter},
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-145 -317.5", OffsetMax = "145 -217" },
                }
            });

            elements.Add(new CuiElement
            {
                Name = "MainSkinLayerSet_input",
                Parent = "MainSkinLayer",
                Components =
                    {
                        new CuiImageComponent {Color = "0 0 0 0.65"},
                        new CuiRectTransformComponent {AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-100 -310.5", OffsetMax = "100 -284"}
                    }
            });

            elements.Add(new CuiElement()
            {
                Parent = "MainSkinLayerSet_input",
                Components =
                {
                    new CuiInputFieldComponent
                    {
                        ...
                        CharsLimit = 64,
                        FontSize = 16,
                        Command = $"clan_changeskin {shortName} ",Font = "robotocondensed-bold.ttf",
                        Text = "",
                        Color = "1 1 1 1"
                    },
                    new CuiRectTransformComponent
            ...
            elements.Add(new CuiButton
            {
                Button = { Color = "0 0 0 0.15" },
                Text = { Text = $"{page + 1}", Color = "1 0.9294118 0.8666667 1", Font = "robotocondensed-bold.ttf", FontSize = 26, Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0.485 0.1", AnchorMax = $"0.51 0.22" },
            }, "MainSkinLayer");

            elements.Add(new CuiButton
            {
                Button = { Color = "0 0 0 0.15", Command = page != 0 ? $"UI_CLAN editskin {shortName} {page - 1}" : "" },
                Text = { Text = "◀", Color = page != 0 ? $"1.00 1.00 0.00 1.00" : "0.20 0.20 0.20 0.98", FontSize = 26, Font = "robotocondensed-bold.ttf", Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0.47 0.1", AnchorMax = $"0.49 0.22" },
            }, "MainSkinLayer");

            elements.Add(new CuiButton
            {
                Button = { Color = "0 0 0 0.15", Command = SkinList[shortName].Count > (page + 1) * totalAmount ? $"UI_CLAN editskin {shortName} {page + 1}" : "" },
                Text = { Text = "▶", Color = SkinList[shortName].Count > (page + 1) * totalAmount ? $"1.00 1.00 0.00 1.00" : "0.20 0.20 0.20 0.98", FontSize = 26, Font = "robotocondensed-bold.ttf", Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0.505 0.1", AnchorMax = $"0.525 0.22" },
            }, "MainSkinLayer");
            #endregion
        ...
        void InterfaceUI(BasePlayer player, string name)
        {
            ...
            container.Add(new CuiButton
            {
                ...
                Button = { Color = "0.13 0.62 0.27 1.00", Command = $"join {name}" },
            ...
            container.Add(new CuiButton
            {
                ...
                Button = { Color = "1 0 0 1", Close = Layer },
        ...
        void cmdItemChangeOfClan(ConsoleSystem.Arg args)
        {
            ...
            elements.Add(new CuiPanel
            {
                RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-545 -314.5", OffsetMax = "547.5 316" },
                Image = { Color = "0 0 0 0.225", Material = "assets/content/ui/uibackgroundblur.mat" }
            }, "clan_itemchangemainlayer", "MainLayerItemChange");
            ...
            elements.Add(new CuiElement
            {
                Name = "MainLayerItemChange" + ".Button",
                Parent = $"MainLayerItemChange",
                Components =
                {
                    new CuiRawImageComponent { Png = (string)ImageLibrary?.Call("GetImage", "closepngrr")},
            ...
            elements.Add(new CuiButton
            {
                Button = { Color = "0 0 0 0", Close = "clan_itemchangemainlayer" },
                RectTransform = { AnchorMin = $"0 0", AnchorMax = $"1 1" },
            }, "MainLayerItemChange" + ".Button");
            #endregion

            #region [Cloth]
            elements.Add(new CuiElement
            {
                Name = "clan_setitemchange",
                Parent = "MainLayerItemChange",
                Components =
                    {
                        new CuiImageComponent {Color = "0 0 0 0.45" },
                        new CuiRectTransformComponent {AnchorMin = "0.0765 0.58", AnchorMax = "0.96 0.86"}
                    }
            });

            elements.Add(new CuiElement
            {
                Parent = $"clan_setitemchange",
                Components =
                {
                    new CuiTextComponent { Text = $"<color=yellow>Одежда</color> клана", Color = "1 1 1 1", FontSize = 27, Font = "robotocondensed-bold.ttf", Align = TextAnchor.MiddleLeft},
                    new CuiRectTransformComponent { AnchorMin = "0.034 0.65", AnchorMax = "1 1" },
                }
            });

            foreach (var check in clan.SkinList["wear"].Select((y, t) => new { A = y, B = t }).Take(8))
            {
                elements.Add(new CuiPanel
                {
                    RectTransform = { AnchorMin = $"{0.035 + check.B * 0.107 - Math.Floor((float) check.B / 8) * 8 * 0.107} 0.14",
                                        AnchorMax = $"{0.1265 + check.B * 0.107 - Math.Floor((float) check.B / 8) * 8 * 0.107} 0.629", },
                    Image = { Color = "0 0 0 0.45" }
                }, "clan_setitemchange", "clan_setitemchange" + ".Wear" + $".{check.B}");

                elements.Add(new CuiElement
                {
                    Parent = "clan_setitemchange" + ".Wear" + $".{check.B}",
                    Components =
                    {
                        new CuiImageComponent { ItemId = FindItemID(check.A.Key), SkinId = check.A.Value },
                        new CuiRectTransformComponent { AnchorMin = "0.025 0.025", AnchorMax = "0.975 0.975" }
                ...
                elements.Add(new CuiButton
                {
                    Button = { Color = "0.43 0.68 0.42 0", Command = $"UI_CLAN editskin {check.A.Key} 0" },
                    Text = { Text = $"" },
                    RectTransform = { AnchorMin = "0 0", AnchorMax = $"1 1" },
                }, "clan_setitemchange" + ".Wear" + $".{check.B}");
            }
            #endregion
            ...
            elements.Add(new CuiElement
            {
                Name = "clan_setitemchange1",
                Parent = "MainLayerItemChange",
                Components =
                    {
                        new CuiImageComponent {Color = "0 0 0 0.45" },
                        new CuiRectTransformComponent {AnchorMin = "0.0765 0.03", AnchorMax = "0.96 0.495"}
                    }
            });

            elements.Add(new CuiElement
            {
                Parent = $"clan_setitemchange1",
                Components =
                {
                    new CuiTextComponent { Text = $"<color=yellow>Оружие</color> клана", Color = "1 1 1 1", FontSize = 27, Font = "robotocondensed-bold.ttf", Align = TextAnchor.MiddleLeft},
                    new CuiRectTransformComponent { AnchorMin = "0.034 0.78", AnchorMax = "1 1" },
                }
            });

            foreach (var check in clan.SkinList["weapon"].Select((y, t) => new { A = y, B = t }).Take(12))
            {
                elements.Add(new CuiPanel
                {
                    RectTransform = { AnchorMin = $"{0.035 + check.B * 0.107 - Math.Floor((float) check.B / 8) * 8 * 0.107} {0.465 - Math.Floor((float) check.B/ 8) * 0.34}",
                                        AnchorMax = $"{0.1265 + check.B * 0.107 - Math.Floor((float) check.B / 8) * 8 * 0.107} {0.77 - Math.Floor((float) check.B / 8) * 0.34}", },
                    Image = { Color = "0 0 0 0.45" }
                }, "clan_setitemchange1", "clan_setitemchange1" + ".Wear" + $".{check.B}");

                elements.Add(new CuiElement
                {
                    Parent = "clan_setitemchange1" + ".Wear" + $".{check.B}",
                    Components =
                    {
                        new CuiImageComponent { ItemId = FindItemID(check.A.Key), SkinId = check.A.Value },
                        new CuiRectTransformComponent { AnchorMin = "0.025 0.025", AnchorMax = "0.975 0.975" }
                ...
                elements.Add(new CuiButton
                {
                    ...
                    Text = { Text = $"" },
                    RectTransform = { AnchorMin = "0 0", AnchorMax = $"1 1" },
                }, "clan_setitemchange1" + ".Wear" + $".{check.B}");
            }
            #endregion
        ...
        void cmdSetChangeOfClan(ConsoleSystem.Arg args)
        {
            ...
            #region [DestroyUi]
            CuiHelper.DestroyUi(player, "clans_main1");
            CuiHelper.DestroyUi(player, "MainLayerSetRes");
            #endregion
            ...
            elements.Add(new CuiPanel
            {
                RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-545 -314.5", OffsetMax = "547.5 316" },
                Image = { Color = "0 0 0 0.225", Material = "assets/content/ui/uibackgroundblur.mat" }
            }, "MainLayerSetRes", "MainLayerSetResource");

            elements.Add(new CuiElement
            {
                Parent = "MainLayerSetResource",
                Components =
                {
                    new CuiImageComponent { Color = "0 0 0 0.97" },
                    new CuiRectTransformComponent { AnchorMin = "0 0", AnchorMax = "1 1"}
                }
            });
            #endregion
            ...
            elements.Add(new CuiLabel
            {
                Text = { Text = $"Клановая система", Color = "1 1 1 1", FontSize = 31, Font = "robotocondensed-regular.ttf", Align = TextAnchor.UpperLeft },
                RectTransform = { AnchorMin = $"0.045 0", AnchorMax = $"1 0.965" },
            }, "MainLayerSetResource");
            #endregion

            #region [Close]
            elements.Add(new CuiElement
            {
                Name = "MainLayerSetResource" + ".Button",
                Parent = $"MainLayerSetResource",
                Components =
                {
                    new CuiRawImageComponent { Png = (string)ImageLibrary?.Call("GetImage", "closepngrr")},
            ...
            elements.Add(new CuiButton
            {
                Button = { Color = "0 0 0 0", Close = "MainLayerSetRes" },
                RectTransform = { AnchorMin = $"0 0", AnchorMax = $"1 1" },
            }, "MainLayerSetResource" + ".Button");
            #endregion
            ...
            elements.Add(new CuiElement
            {
                ...
                Components =
                    {
                        new CuiImageComponent {Color = "0 0 0 0.65" },
                        new CuiRectTransformComponent {AnchorMin = "0.0475 0.13", AnchorMax = "0.9625 0.865"}
                    }
            });
            ...
            elements.Add(new CuiElement
            {
                ...
                Components =
                {
                    new CuiTextComponent { Text = $"<color=yellow>Норма</color> клана", Color = "1 1 1 1", FontSize = 27, Font = "robotocondensed-bold.ttf", Align = TextAnchor.MiddleCenter},
                    new CuiRectTransformComponent { AnchorMin = "0 0.855", AnchorMax = "0.21 1" },
                }
            });

            foreach (var check in clan.Change.Select((y, t) => new { A = y, B = t }).Take(8))
            {
                elements.Add(new CuiPanel
                {
                    RectTransform = { AnchorMin = $"{0.0325 + check.B * 0.426 - Math.Floor((float) check.B / 2) * 2 * 0.426} {0.665 - Math.Floor((float) check.B/ 2) * 0.221}",
                                        AnchorMax = $"{0.445 + check.B * 0.426 - Math.Floor((float) check.B / 2) * 2 * 0.426} {0.853 - Math.Floor((float) check.B / 2) * 0.221}", },
                    Image = { Color = "0 0 0 0" }
                }, "clan_setChange1", "clan_setChange1" + ".Resourse" + $".{check.B}");

                elements.Add(new CuiPanel
                {
                    RectTransform = { AnchorMin = "0 0", AnchorMax = "0.22 1" },
                    Image = { Color = "0 0 0 0.65" }
                }, "clan_setChange1" + ".Resourse" + $".{check.B}", "clan_setChange1" + ".Resourse" + $".{check.B}" + ".Image");

                if (FindItemID(check.A.Key) != 0)
                {
                    elements.Add(new CuiElement
                    {
                        Parent = "clan_setChange1" + ".Resourse" + $".{check.B}" + ".Image",
                        Components =
                        {
                            new CuiImageComponent { ItemId = FindItemID(check.A.Key), SkinId = 0 },
                            new CuiRectTransformComponent {AnchorMin = "0.1 0.1", AnchorMax = "0.9 0.9"}
                ...
                elements.Add(new CuiElement
                {
                    Name = "clan_setChange1" + ".Resourse" + $".{check.B}" + ".Field",
                    Parent = "clan_setChange1" + ".Resourse" + $".{check.B}",
                    Components =
                        {
                            new CuiImageComponent { Color = "0 0 0 0.65" },
                            new CuiRectTransformComponent {AnchorMin = $"0.25 0.3", AnchorMax = $"0.667 0.7"}
                        }
                });

                elements.Add(new CuiElement()
                {
                    Parent = "clan_setChange1" + ".Resourse" + $".{check.B}" + ".Field",
                    Components =
                    {
                        new CuiInputFieldComponent { Align = TextAnchor.MiddleCenter,CharsLimit = 7,FontSize = 14, Command = $"clan_Change {check.A.Key} ",Font = "robotocondensed-regular.ttf",Text = $"{check.A.Value.Need}", Color = "1 1 1 0.45"},
                        new CuiRectTransformComponent { AnchorMin = "0 0", AnchorMax = "1 1" }
                    }
                });
                ...
                {
                    Button = { Color = "0.13 0.62 0.27 1.00", Command = "" },
                    Text = { Text = "Применить", Color = "1 1 1 1", FontSize = 14, Font = "robotocondensed-bold.ttf", Align = TextAnchor.MiddleCenter },
                    RectTransform = { AnchorMin = $"0.695 0.377", AnchorMax = $"0.995 0.615" },
                }, "clan_setChange1" + ".Resourse" + $".{check.B}");
            }
            #endregion
        ...
        void cmdChatSkinOfClan(ConsoleSystem.Arg args)
        {
            ...
            CuiHelper.DestroyUi(player, "MainLayerSkinLayer");
        ...
        void OnItemDropped(Item item, BaseEntity entity)
        {
            ...
            if (!IgnoreList.Contains(item.uid.Value))
                IgnoreList.Add(item.uid.Value);
        }
        ...
        {
            if (!_looters.ContainsKey(@from.uid.Value))
            {
                _looters[@from.uid.Value] = new HashSet<ulong>();
            }
            if (!_looters.ContainsKey(to.uid.Value))
            {
                _looters[to.uid.Value] = new HashSet<ulong>();
            }
            _looters[to.uid.Value].UnionWith(_looters[@from.uid.Value]);
        }
        ...
        private void SetSeenPlayer(ulong ownerId, Item item)
        {
            if (!_looters.ContainsKey(item.uid.Value))
            {
                _looters[item.uid.Value] = new HashSet<ulong> { ownerId };
            }

            if (_looters[item.uid.Value].Contains(ownerId)) return;
            _looters[item.uid.Value].Add(ownerId);
        }

        private Dictionary<ulong, HashSet<ulong>> _looters = new Dictionary<ulong, HashSet<ulong>>();


        private bool HasSeenPlayer(ulong ownerId, Item item)
        {
            if (item == null) return false;
            return _looters.ContainsKey(item.uid.Value) && _looters[item.uid.Value].Contains(ownerId);
        }
        ...
        [ChatCommand("limit")]
        void CommandClanLimits(BasePlayer player)
        {
            ClanLimit(player);
        }

        private void ClanLimit(BasePlayer player)
        {
            CuiElementContainer container = new CuiElementContainer();
            ...
            container.Add(new CuiElement
            {
                ...
                Components =
                {
                    new CuiTextComponent() { Color = "1 1 1 1", Text = $"Ограничения вашего клана: {clan.tag}", FontSize = 25, Align = TextAnchor.MiddleCenter },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-252 160", OffsetMax = "252 205"},
                    new CuiOutlineComponent{Color = "0 0 0 0"},
                }
            });


            container.Add(new CuiElement//Картинка на закрывание
            {
                Parent = Layer,
                Components =
                     {
                         ...
                         {
                             Png = (string) ImageLibrary.Call("GetImage", but),
                             Color = "1 1 1 1"
                         },
                         new CuiRectTransformComponent
                             { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-145 110", OffsetMax = "267 140" }
                     }
            });
            ...
            container.Add(new CuiElement
            {
                ...
                Components =
                {
                    new CuiTextComponent() { Color = "1 1 1 1", Text = $"Всего доступно:", FontSize = 12, Align = TextAnchor.MiddleLeft },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-130 110", OffsetMax = "255 140"},
                    new CuiOutlineComponent{Color = "0 0 0 0"},
                }
            });
            ...
            container.Add(new CuiElement
            {
                ...
                Components =
                {
                    new CuiTextComponent() { Color = "1 1 1 1", Text = $"{limitobj - clan.obj}", FontSize = 12, Align = TextAnchor.MiddleRight },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-115 110", OffsetMax = "240 140"},
                    new CuiOutlineComponent{Color = "0 0 0 0"},
                }
            });
                ...
                Components =
                     {
                         ...
                         new CuiRectTransformComponent
                             { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-145 73", OffsetMax = "267 103" }
                     }
            });
            ...
            container.Add(new CuiElement
            {
                ...
                Components =
                {
                    new CuiTextComponent() { Color = "1 1 1 1", Text = $"Всего установлено:", FontSize = 12, Align = TextAnchor.MiddleLeft },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-130 73", OffsetMax = "245 103"},
                    new CuiOutlineComponent{Color = "0 0 0 0"},
                }
            });
            ...
            container.Add(new CuiElement
            {
                ...
                Components =
                {
                    new CuiTextComponent() { Color = "1 1 1 1", Text = $"{clan.obj}", FontSize = 12, Align = TextAnchor.MiddleRight },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-115 73", OffsetMax = "240 103"},
                    new CuiOutlineComponent{Color = "0 0 0 0"},
                }
            });
                ...
                Components =
                     {
                         new CuiRawImageComponent
                         {
                             Png = (string) ImageLibrary.Call("GetImage", building),
                             Color = "1 1 1 1"
                         },
                         new CuiRectTransformComponent
                             { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-238 65", OffsetMax = "-151 148" }
                     }
            });
                ...
                Components =
                     {
                         new CuiRawImageComponent
                         {
                             Png = (string) ImageLibrary.Call("GetImage", cupboarding),
                             Color = "1 1 1 1"
                         },
                         new CuiRectTransformComponent
                             { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-238 -28", OffsetMax = "-151 55" }
                     }
            });
                ...
                Components =
                     {
                         ...
                         new CuiRectTransformComponent
                             { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-145 -78", OffsetMax = "267 -48" }
                     }
            });
            ...
            container.Add(new CuiElement
            {
                ...
                {
                    new CuiTextComponent() { Color = "1 1 1 1", Text = $"Всего доступно:", FontSize = 12, Align = TextAnchor.MiddleLeft },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-130 -78", OffsetMax = "245 -48"},
                    new CuiOutlineComponent{Color = "0 0 0 0"},
                }
            });
            ...
            container.Add(new CuiElement
            {
                ...
                Components =
                {
                    new CuiTextComponent() { Color = "1 1 1 1", Text = $"{limitturret - clan.turret}", FontSize = 12, Align = TextAnchor.MiddleRight },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-110 -78", OffsetMax = "240 -48"},
                    new CuiOutlineComponent{Color = "0 0 0 0"},
                }
            });
                ...
                Components =
                     {
                         ...
                         new CuiRectTransformComponent
                             { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-145 -115", OffsetMax = "267 -85" }
                     }
            });
            ...
            container.Add(new CuiElement
            {
                ...
                {
                    new CuiTextComponent() { Color = "1 1 1 1", Text = $"Всего установлено:", FontSize = 12, Align = TextAnchor.MiddleLeft },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-130 -110", OffsetMax = "245 -85"},
                    new CuiOutlineComponent{Color = "0 0 0 0"},
                }
            });
            ...
            container.Add(new CuiElement
            {
                ...
                Components =
                {
                    new CuiTextComponent() { Color = "1 1 1 1", Text = $"{clan.turret}", FontSize = 12, Align = TextAnchor.MiddleRight },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-115 -110", OffsetMax = "240 -85"},
                    new CuiOutlineComponent{Color = "0 0 0 0"},
                }
            });
                ...
                Components =
                     {
                         new CuiRawImageComponent
                         {
                             Png = (string) ImageLibrary.Call("GetImage", turretka),
                             Color = "1 1 1 1"
                         },
                         new CuiRectTransformComponent
                             { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-238 -121", OffsetMax = "-151 -38" }
                     }
            });
                ...
                Components =
                     {
                         ...
                         new CuiRectTransformComponent
                             { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-145 16", OffsetMax = "267 46" }
                     }
            });
            ...
            container.Add(new CuiElement
            {
                ...
                {
                    new CuiTextComponent() { Color = "1 1 1 1", Text = $"Всего доступно:", FontSize = 12, Align = TextAnchor.MiddleLeft },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-130 16", OffsetMax = "240 46" },
                    new CuiOutlineComponent{Color = "0 0 0 0"},
                }
            });
            ...
            container.Add(new CuiElement
            {
                ...
                Components =
                {
                    new CuiTextComponent() { Color = "1 1 1 1", Text = $"{limitcups - clan.cups}", FontSize = 12, Align = TextAnchor.MiddleRight },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-115 16", OffsetMax = "240 46"},
                    new CuiOutlineComponent{Color = "0 0 0 0"},
                }
            });
                ...
                Components =
                     {
                         ...
                         new CuiRectTransformComponent
                             { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-145 -21", OffsetMax = "267 9" }
                     }
            });
            ...
            container.Add(new CuiElement
            {
                ...
                {
                    new CuiTextComponent() { Color = "1 1 1 1", Text = $"Всего установлено:", FontSize = 12, Align = TextAnchor.MiddleLeft },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-130 -21", OffsetMax = "245 9" },
                    new CuiOutlineComponent{Color = "0 0 0 0"},
                }
            });
            ...
            container.Add(new CuiElement
            {
                ...
                Components =
                {
                    new CuiTextComponent() { Color = "1 1 1 1", Text = $"{clan.cups}", FontSize = 12, Align = TextAnchor.MiddleRight },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-110 -21", OffsetMax = "240 9"},
                    new CuiOutlineComponent{Color = "0 0 0 0"},
                }
            });
                ...
                Components =
                     {
                         new CuiRawImageComponent
                         {
                             Png = (string) ImageLibrary.Call("GetImage", SamSite),
                             Color = "1 1 1 1"
                         },
                         new CuiRectTransformComponent
                             { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-238 -214", OffsetMax = "-151 -131" }
                     }
            });
                ...
                Components =
                     {
                         ...
                         new CuiRectTransformComponent
                             { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-145 -170", OffsetMax = "267 -141" }
                     }
            });
            ...
            container.Add(new CuiElement
            {
                ...
                {
                    new CuiTextComponent() { Color = "1 1 1 1", Text = $"Всего доступно:", FontSize = 12, Align = TextAnchor.MiddleLeft },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-130 -170", OffsetMax = "245 -141" },
                    new CuiOutlineComponent{Color = "0 0 0 0"},
                }
            });
            ...
            container.Add(new CuiElement
            {
                ...
                Components =
                {
                    new CuiTextComponent() { Color = "1 1 1 1", Text = $"{limitpvo - clan.pvo}", FontSize = 12, Align = TextAnchor.MiddleRight },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-110 -170", OffsetMax = "240 -141"},
                    new CuiOutlineComponent{Color = "0 0 0 0"},
                }
            });
                ...
                Components =
                     {
                         ...
                         new CuiRectTransformComponent
                             { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-145 -207", OffsetMax = "267 -178" }
                     }
            });
            ...
            container.Add(new CuiElement
            {
                ...
                {
                    new CuiTextComponent() { Color = "1 1 1 1", Text = $"Всего установлено:", FontSize = 12, Align = TextAnchor.MiddleLeft },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-130 -207", OffsetMax = "245 -178" },
                    new CuiOutlineComponent{Color = "0 0 0 0"},
                }
            });
            ...
            container.Add(new CuiElement
            {
                ...
                Components =
                {
                    new CuiTextComponent() { Color = "1 1 1 1", Text = $"{clan.pvo}", FontSize = 12, Align = TextAnchor.MiddleRight },
                    new CuiRectTransformComponent {  AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-110 -207", OffsetMax = "240 -178"},
                    new CuiOutlineComponent{Color = "0 0 0 0"},
                }
            });
            ...
            container.Add(new CuiButton//Кнопка закрывания
            {
                Button = { Color = "0.9 0 0 0", Material = "assets/icons/greyout.mat", Close = Layer },
                RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "205 170", OffsetMax = "250 215" }
            }, Layer, "Closes");
                ...
                Components =
                     {
                         new CuiRawImageComponent
                         {
                             Png = (string) ImageLibrary.Call("GetImage", Closed),
                             Color = "1 1 1 0.65"
                         },
                         new CuiRectTransformComponent
        ...
        string GetPlayerStatus(string player, Clan clan)
        {
            ...
            if (clan.owner.Contains(player)) status = "Глава";
            return status;
        }
        ...
        void ClanTOPInfo(BasePlayer player, Clan clan, int page)
        {
            ...
            elements.Add(new CuiElement
            {
                Name = "Clanstop_infolayer" + ".Button",
                Parent = $"Clanstop_infolayer",
                Components =
                {
                    new CuiRawImageComponent { Png = (string)ImageLibrary?.Call("GetImage", "closepngrr")},
            ...
            elements.Add(new CuiButton
            {
                Button = { Color = "0 0 0 0", Close = "Clanstop_info2" },
                RectTransform = { AnchorMin = $"0 0", AnchorMax = $"1 1" },
            }, "Clanstop_infolayer" + ".Button");
            #endregion

            #region [InfoClan]
            elements.Add(new CuiElement
            {
                Name = "Clanstop_infolayer" + ".Info",
                Parent = "Clanstop_infolayer",
                Components =
                {
                    new CuiImageComponent { Color = "0 0 0 0.45" },
                    new CuiRectTransformComponent { AnchorMin = "0.046 0.642", AnchorMax = "0.957 0.863"}
                }
            });

            elements.Add(new CuiElement
            {
                Parent = "Clanstop_infolayer" + ".Info",
                Components =
                {
                    new CuiRawImageComponent { Png = (string)ImageLibrary?.Call("GetImage", "logopng"), Color = "1 1 1 1"},
                    new CuiRectTransformComponent { AnchorMin = "0.0565 0.25", AnchorMax = "0.135 0.85" }
                }
            });

            elements.Add(new CuiLabel
            {
                Text = { Text = $"{clan.tag}", Color = "1 1 1 1", FontSize = 22, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleLeft },
                RectTransform = { AnchorMin = $"0.1625 0.5", AnchorMax = $"1 1" },
            }, "Clanstop_infolayer" + ".Info");

            elements.Add(new CuiLabel
            {
                Text = { Text = $"Игроков в клане: <color=orange>{clan.online}/{clan.members.Count}</color>", Color = "1 1 1 0.45", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleLeft },
                RectTransform = { AnchorMin = $"0.1625 0.255", AnchorMax = $"1 0.88" },
            }, "Clanstop_infolayer" + ".Info");

            elements.Add(new CuiLabel
            {
                Text = { Text = $"Лидер клана: <color=orange>{clan.ownerName}</color>", Color = "1 1 1 0.45", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleLeft },
                RectTransform = { AnchorMin = $"0.1625 0", AnchorMax = $"1 0.87" },
            }, "Clanstop_infolayer" + ".Info");

            elements.Add(new CuiLabel
            {
                Text = { Text = $"Очки: <color=orange>{clan.ClanPoints}</color>", Color = "1 1 1 0.45", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleLeft },
                RectTransform = { AnchorMin = $"0.1625 0", AnchorMax = $"1 0.562" },
            }, "Clanstop_infolayer" + ".Info");

            if (TournamentBolotoRocket)
            {
                var IsReg = (bool)TournamentBolotoRocket?.Call<bool>("CheckRegistration", clan.tag) == true ? "Участвует" : "Не участвует";
                elements.Add(new CuiLabel
                {
                    Text = { Text = $"Турнир: <color=orange>{IsReg}</color>", Color = "1 1 1 0.45", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleLeft },
                    RectTransform = { AnchorMin = $"0.341 0.255", AnchorMax = $"1 0.88" },
                }, "Clanstop_infolayer" + ".Info");
            }
            ...
            {
                Text = { Text = $"Место в топе: <color=orange>{GetClanIndex(clan.tag)}</color>", Color = "1 1 1 0.45", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleLeft },
                RectTransform = { AnchorMin = $"0.341 0", AnchorMax = $"1 0.87" },
            }, "Clanstop_infolayer" + ".Info");

            elements.Add(new CuiLabel
            {
                Text = { Text = $"Захваты: <color=orange>{clan.CupCount}</color>", Color = "1 1 1 0.45", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleLeft },
                RectTransform = { AnchorMin = $"0.341 0", AnchorMax = $"1 0.562" },
            }, "Clanstop_infolayer" + ".Info");
            #endregion
            ...
            elements.Add(new CuiButton
            {
                Button = { Color = "0 0 0 0", Command = "clanstop_main" },
                Text = { Text = "< Назад", Color = "1.00 1.00 0.00 1.00", FontSize = 20, Font = "robotocondensed-bold.ttf", Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0.01 0.08", AnchorMax = $"0.15 0.125" },
            }, "Clanstop_infolayer");
            #endregion
            ...
            elements.Add(new CuiButton
            {
                Button = { Color = "0 0 0 0", Command = page > 0 ? $"clanstop_info {clan.tag} {page - 1}" : "" },
                Text = { Text = "◀", Color = page > 0 ? "1.00 1.00 0.00 1.00" : "0.2 0.2 0.2 0.98", FontSize = 24, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0.465 0.085", AnchorMax = $"0.49 0.145" },
            }, "Clanstop_infolayer");
            ...
            elements.Add(new CuiButton
            {
                Button = { Color = "0 0 0 0" },
                Text = { Text = $"{page + 1}", Color = "1 0.9294118 0.8666667 1", Font = "robotocondensed-regular.ttf", FontSize = 22, Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0.49 0.085", AnchorMax = $"0.505 0.145" },
            }, "Clanstop_infolayer");

            elements.Add(new CuiButton
            {
                Button = { Color = "0 0 0 0", Command = ClanMembers.Skip(6 * (page + 1)).Count() > 0 ? $"clanstop_info {clan.tag} {page + 1}" : "" },
                Text = { Text = "▶", Color = page > 0 ? "1.00 1.00 0.00 1.00" : "0.2 0.2 0.2 0.98", FontSize = 24, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0.505 0.085", AnchorMax = $"0.53 0.145" },
            }, "Clanstop_infolayer");
            #endregion
            ...
            elements.Add(new CuiPanel
            {
                RectTransform = { AnchorMin = "0 0.575", AnchorMax = "1 0.605" },
                Image = { Color = "0 0 0 0" }
            }, "Clanstop_infolayer", "Clanstop_infolayertext");

            elements.Add(new CuiLabel
            {
                Text = { Text = $"Имя игрока", Color = "1 1 1 0.45", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0.0175 0", AnchorMax = $"0.255 1" },
            }, "Clanstop_infolayertext");

            elements.Add(new CuiLabel
            {
                Text = { Text = $"Статус", Color = "1 1 1 0.45", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0.095 0", AnchorMax = $"0.4625 1" },
            }, "Clanstop_infolayertext");

            elements.Add(new CuiLabel
            {
                Text = { Text = $"Очки", Color = "1 1 1 0.45", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0.26 0", AnchorMax = $"0.6 1" },
            }, "Clanstop_infolayertext");

            elements.Add(new CuiLabel
            {
                Text = { Text = $"Фарм", Color = "1 1 1 0.45", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0.451 0", AnchorMax = $"0.7 1" },
            }, "Clanstop_infolayertext");

            elements.Add(new CuiLabel
            {
                Text = { Text = $"К/Д", Color = "1 1 1 0.45", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0.626 0", AnchorMax = $"0.8 1" },
            }, "Clanstop_infolayertext");

            elements.Add(new CuiLabel
            {
                Text = { Text = $"Активность", Color = "1 1 1 0.45", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0.735 0", AnchorMax = $"1 1" },
            }, "Clanstop_infolayertext");
            #endregion

            #region [MemberInfo]
            float width = 0.916f, height = 0.055f, startxBox = 0.0435f, startyBox = 0.57f - height, xmin = startxBox, ymin = startyBox;
            foreach (KeyValuePair<string, PlayerStats> key in ClanMembers.Skip(7 * page).Take(ClanMembers.ToList().Count >= 6 ? 6 : ClanMembers.ToList().Count))
            {
                elements.Add(new CuiPanel
                {
                    RectTransform = { AnchorMin = $"{xmin} {ymin}", AnchorMax = $"{xmin + width} {ymin + height * 1}", OffsetMin = "2 1", OffsetMax = "-2 -1" },
                    Image = { Color = colored }
                }, "Clanstop_infolayer", "MemberTopLine");
                ...
                elements.Add(new CuiLabel
                {
                    Text = { Text = $"{page * 5 + i}. {covalence.Players.FindPlayerById(key.Key).Name}", Font = "robotocondensed-regular.ttf", FontSize = 12, Color = "1 0.9294118 0.8666667 0.9", Align = TextAnchor.MiddleCenter },
                    RectTransform = { AnchorMin = $"0.012 0", AnchorMax = $"0.17528636 1" },
                }, "MemberTopLine");

                elements.Add(new CuiLabel
                {
                    Text = { Text = GetPlayerStatus(key.Key, clan), Color = "1 0.9294118 0.8666667 0.9", Font = "robotocondensed-regular.ttf", FontSize = 12, Align = TextAnchor.MiddleCenter },
                    RectTransform = { AnchorMin = $"0.16 0", AnchorMax = $"0.35 1" },
                }, "MemberTopLine");

                elements.Add(new CuiLabel
                {
                    Text = { Text = $"{key.Value.PlayerPoints}", Color = "1 0.9294118 0.8666667 0.9", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
                    RectTransform = { AnchorMin = $"0.34 0", AnchorMax = $"0.5 1" },
                }, "MemberTopLine");
                ...
                elements.Add(new CuiLabel
                {
                    Text = { Text = $"{key.Value.FarmPlayer}", Color = "1 0.9294118 0.8666667 0.9", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
                    RectTransform = { AnchorMin = $"0.465 0", AnchorMax = $"0.7 1" },
                }, "MemberTopLine");

                elements.Add(new CuiLabel
                {
                    Text = { Text = $"{key.Value.Killed}/{key.Value.Death + key.Value.Suicide}", Color = "1 0.9294118 0.8666667 0.9", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
                    RectTransform = { AnchorMin = $"0.665 0", AnchorMax = $"0.8 1" },
                }, "MemberTopLine");

                elements.Add(new CuiLabel
                {
                    Text = { Text = $"{FormatShortTime(TimeSpan.FromSeconds(key.Value.PlayerTime))}", Color = "1 0.9294118 0.8666667 1", Font = "robotocondensed-regular.ttf", FontSize = 14, Align = TextAnchor.MiddleCenter },
                    RectTransform = { AnchorMin = $"0.81 0", AnchorMax = $"1 1" },
                }, "MemberTopLine");
        ...
        [PluginReference] private Plugin TournamentBolotoRocket;
        ...
        void ClanTOP(BasePlayer player, int page)
        {
            ...
            elements.Add(new CuiElement
            {
                Name = "Clanstop_mainlayer" + ".Button",
                Parent = $"Clanstop_mainlayer",
                Components =
                {
                    new CuiRawImageComponent { Png = (string)ImageLibrary?.Call("GetImage", "closepngrr")},
            ...
            elements.Add(new CuiButton
            {
                Button = { Color = "0 0 0 0", Close = "Clanstop_main2" },
                RectTransform = { AnchorMin = $"0 0", AnchorMax = $"1 1" },
            }, "Clanstop_mainlayer" + ".Button");
            #endregion

            #region [Page]
            elements.Add(new CuiButton
            {
                Button = { Color = "0 0 0 0", Command = page > 0 ? $"clanstop_main {page - 1}" : "" },
                Text = { Text = "◀", Color = page > 0 ? "1.00 1.00 0.00 1.00" : "0.2 0.2 0.2 0.98", FontSize = 24, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0.465 0.085", AnchorMax = $"0.49 0.145" },
            }, "Clanstop_mainlayer");
            ...
            elements.Add(new CuiButton
            {
                Button = { Color = "0 0 0 0", Command = clans.Keys.Skip(10 * (page + 1)).Count() > 0 ? $"clanstop_main {page + 1}" : "" },
                Text = { Text = "▶", Color = clans.Keys.Skip(10 * (page + 1)).Count() > 0 ? "1.00 1.00 0.00 1.00" : "0.2 0.2 0.2 0.98", FontSize = 24, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0.505 0.085", AnchorMax = $"0.53 0.145" },
            }, "Clanstop_mainlayer");
            #endregion
            ...
            elements.Add(new CuiPanel
            {
                RectTransform = { AnchorMin = "0 0.84", AnchorMax = "1 0.865" },
                Image = { Color = "0 0 0 0" }
            }, "Clanstop_mainlayer", "Clanstop_mainlayertext");

            elements.Add(new CuiLabel
            {
                Text = { Text = $"Место", Color = "1 1 1 0.45", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0.03 0", AnchorMax = $"0.15 1" },
            }, "Clanstop_mainlayertext");

            elements.Add(new CuiLabel
            {
                Text = { Text = $"Название клана", Color = "1 1 1 0.45", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleLeft },
                RectTransform = { AnchorMin = $"0.190 0", AnchorMax = $"0.6283538 1" },
            }, "Clanstop_mainlayertext");

            elements.Add(new CuiLabel
            {
                Text = { Text = $"Шкаф", Color = "1 1 1 0.45", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0.34 0", AnchorMax = $"0.41 1" },
            }, "Clanstop_mainlayertext");

            elements.Add(new CuiLabel
            {
                Text = { Text = $"Очки", Color = "1 1 1 0.45", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0.058 0", AnchorMax = $"0.895 1" },
            }, "Clanstop_mainlayertext");

            elements.Add(new CuiLabel
            {
                Text = { Text = $"Игроков", Color = "1 1 1 0.45", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0.171 0", AnchorMax = $"0.98 1" },
            }, "Clanstop_mainlayertext");

            elements.Add(new CuiLabel
            {
                Text = { Text = $"К/Д", Color = "1 1 1 0.45", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0.575 0", AnchorMax = $"0.79 1" },
            }, "Clanstop_mainlayertext");

            elements.Add(new CuiLabel
            {
                Text = { Text = $"Фарм", Color = "1 1 1 0.45", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0.695 0", AnchorMax = $"0.89 1" },
            }, "Clanstop_mainlayertext");

            elements.Add(new CuiLabel
            {
                Text = { Text = $"Захваты", Color = "1 1 1 0.45", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = $"0.81 0", AnchorMax = $"1 1" },
            }, "Clanstop_mainlayertext");
            #endregion

            #region [ClanInfo]
            float width = 0.916f, height = 0.055f, startxBox = 0.0435f, startyBox = 0.834f - height, xmin = startxBox, ymin = startyBox;
            foreach (KeyValuePair<string, Clan> key in ClanMembers.Skip(10 * page).Take(ClanMembers.ToList().Count >= 10 ? 10 : ClanMembers.ToList().Count))
            {
                elements.Add(new CuiPanel
                {
                    RectTransform = { AnchorMin = $"{xmin} {ymin}", AnchorMax = $"{xmin + width} {ymin + height * 1}", OffsetMin = "2 1", OffsetMax = "-2 -1" },
                    Image = { Color = colored }
                }, "Clanstop_mainlayer", "ClanTopLine");

                elements.Add(new CuiLabel
                {
                    Text = { Text = $"{i}.", Color = "1 0.9294118 0.8666667 1", FontSize = 12, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
                    RectTransform = { AnchorMin = $"0.0205 0", AnchorMax = $"0.077 1" },
                }, "ClanTopLine");

                elements.Add(new CuiLabel
                {
                    Text = { Text = $"{key.Value.tag}", Font = "robotocondensed-regular.ttf", FontSize = 12, Color = "1 0.9294118 0.8666667 1", Align = TextAnchor.MiddleCenter },
                    RectTransform = { AnchorMin = $"0 0", AnchorMax = $"0.4 1" },
                }, "ClanTopLine");

                if (TournamentBolotoRocket)
                {
                    if ((bool)TournamentBolotoRocket?.Call<bool>("CheckRegistration", key.Value.tag) == true)
                    {
                        elements.Add(new CuiLabel
                        {
                            Text = { Text = "Участвуют", Font = "robotocondensed-regular.ttf", FontSize = 12, Color = "1.00 1.00 0.00 1.00", Align = TextAnchor.MiddleCenter },
                            RectTransform = { AnchorMin = $"0.32 0", AnchorMax = $"0.4 1" },
                        }, "ClanTopLine");
                    }
                    else
                    {
                        elements.Add(new CuiLabel
                        {
                            Text = { Text = "Не участвуют", Font = "robotocondensed-regular.ttf", FontSize = 12, Color = "1 0 0 1", Align = TextAnchor.MiddleCenter },
                            RectTransform = { AnchorMin = $"0.32 0", AnchorMax = $"0.4 1" },
                        }, "ClanTopLine");
                    }
                }
                ...
                elements.Add(new CuiLabel
                {
                    Text = { Text = $"{key.Value.ClanPoints}", Font = "robotocondensed-regular.ttf", FontSize = 12, Color = "1 0.9294118 0.8666667 1", Align = TextAnchor.MiddleCenter },
                    RectTransform = { AnchorMin = $"0.044 0", AnchorMax = $"0.9 1" },
                }, "ClanTopLine");

                elements.Add(new CuiLabel
                {
                    Text = { Text = $"{key.Value.online}/{key.Value.members.Count}".ToUpper(), Font = "robotocondensed-regular.ttf", FontSize = 12, Color = "1 0.9294118 0.8666667 1", Align = TextAnchor.MiddleCenter },
                    RectTransform = { AnchorMin = $"0.162 0", AnchorMax = $"1 1" },
                }, "ClanTopLine");

                elements.Add(new CuiLabel
                {
                    Text = { Text = $"{key.Value.KillC}/{key.Value.DeathC}", Font = "robotocondensed-regular.ttf", FontSize = 12, Color = "1 0.9294118 0.8666667 1", Align = TextAnchor.MiddleCenter },
                    RectTransform = { AnchorMin = $"0.497 0", AnchorMax = $"0.9 1" },
                }, "ClanTopLine");

                elements.Add(new CuiLabel
                {
                    Text = { Text = $"{key.Value.Farm}", Font = "robotocondensed-regular.ttf", FontSize = 12, Color = "1 0.9294118 0.8666667 1", Align = TextAnchor.MiddleCenter },
                    RectTransform = { AnchorMin = $"0.635 0", AnchorMax = $"1 1" },
                }, "ClanTopLine");

                elements.Add(new CuiLabel
                {
                    Text = { Text = $"{key.Value.CupCount}", Font = "robotocondensed-regular.ttf", FontSize = 12, Color = "1 0.9294118 0.8666667 1", Align = TextAnchor.MiddleCenter },
                    RectTransform = { AnchorMin = $"0.884 0", AnchorMax = $"1 1" },
                }, "ClanTopLine");
        ...
        string GetFullPercent(string id)
        {
            ...
            return $"{((float)playerCurrent * 100 / Need * 9)}";
        }

        string GetFullClanPercent(string tag)
        {
            var clan = findClan(tag);
            var Need = clan.Change.Sum(x => x.Value.Need);
            var Current = clan.Change.Sum(x => x.Value.Complete);
            return $"{((float)Current * 100 / Need * 9)}";
        }
...
"leave"
        ...
        void cmdClanLeave(BasePlayer player, string[] args)
        {
            ...
            if (myClan.IsOwner(player.UserIDString))
            {
                player.ChatMessage("Вы не можете покинуть клан. Вы можете удалить свой клан при помощи команды: /clan disband");
                return;
            }
...
"disband", "forever"
        ...
        void cmdClanDisband(BasePlayer player, string[] args)
        {
            ...
            player.ChatMessage("Клан успешно распущен");
            foreach (var ally in clans)
            {
                Clan allyClan = clans[ally.Key];
...
type, targetClan
        ...
        void cmdChatClanHelp(BasePlayer player)
        {
            if (player == null) return;
            player.ChatMessage("<color=#FF6c6c>Доступные команды:</color>\n<color=#2394cb>/clan</color> - открыть меню клана\n<color=#2394cb>/clan create</color> - открыть меню создания клана\n<color=#2394cb>/clan invite</color> <НикИгрока> - пригласить игрока в клан\n<color=#2394cb>/clan ff</color> - включить/выключить FriendlyFire\n<color=#2394cb>/clan promote</color> - Повысить участника\n<color=#2394cb>/clan leave</color> - покинуть клан\n<color=#2394cb>/clan disband</color> - удалить клан");
        }
                ...
                var clan = new Clan()
                {
                    ...
                    SkinList = new Dictionary<string, Dictionary<string, ulong>>()
                    {
                        ...
                        ["weapon"] = new Dictionary<string, ulong>()
                        {
                            ...
                            {"pistol.python", 0 },
                            {"rocket.launcher", 0 },
                            {"door.double.hinged.toptier", 2739422880 },
                            {"door.hinged.toptier", 1640341902 },
                        },
                    },
        ...
        private List<string> RTeleportMenuHook(ulong playerID)
        {
