...
{
    [Info("Epic Loot", "imthenewguy", "1.2.5")]
    ...
    class EpicLoot : RustPlugin
    {
        ...
        {
#if CARBON
            if (Patched) _harmony.UnpatchAll(Name + "Patch");
#endif
            SaveData();
        ...
        void RemovePerms(ulong id)
        {
            ...
            {
                List<string> perms_to_remove = Pool.Get<List<string>>();
                ...
                Pool.FreeUnmanaged(ref perms_to_remove);
            }
        }
        ...
        void OnItemCraftFinished(ItemCraftTask task, Item item, ItemCrafter crafter)
        {
            ...
            {
                ReplaceItem(player, item, out replaced);
            }
        ...
        void ReplaceItem(BasePlayer player, Item oldItem, out bool replaced)
        {
            replaced = false;
            ...
            if (potentialBuffs.Count > 0)
            {
                ...
                if (config.enhancements.TryGetValue(buff, out ei))
                {
                    ...
                    if (tier.Key != null)
                    {
                        ...
                        var newItem = ItemManager.CreateByName(oldItem.info.shortname, 1, skin);
                        ...
                        if (oldItem.amount > 1) oldItem.amount--;
                        else
                        {
        ...
        void GenerateSet(BasePlayer player, string command, string[] args)
        {
            ...
            {
                shortname.Add(_item);
                GenerateItem(target, args[1], shortname, "s");
                shortname.Clear();
            }
            Pool.FreeUnmanaged(ref shortname);
        }
