...
{
    [Info("VirtualQuarries", "ThePitereq", "2.4.9")]
    public class VirtualQuarries : RustPlugin
    {
        ...
        private readonly Dictionary<ulong, float> commandCooldowns = new Dictionary<ulong, float>();
            ...
            if (config.excavatorQuarry)
            {
                ...
                if (data.staticQuarries.ContainsKey(uint.MaxValue))
                {
                    foreach (var user in data.staticQuarries[uint.MaxValue])
                    {
                        ...
                        if (data.excavatorOutputs.ContainsKey(user.Key))
                            vQuarry.mineType = data.excavatorOutputs[user.Key];
                        NextTick(() => vQuarry.SetupQuarry());
                    }
                }
            }
        ...
        private object CanLootEntity(BasePlayer player, StorageContainer storage)
        {
            ...
            else if (config.excavatorQuarry && storage is DieselEngine dieselStorage)
            {
                if (config.excavatorPerm && !permission.UserHasPermission(player.UserIDString, "virtualquarries.static.excavator"))
                {
                    PopUpAPI?.Call("ShowPopUp", player, config.popUpPreset, Lang("NoPermissionQuarry", player.UserIDString));
            ...
            else if (config.staticQuarries && storage is ResourceExtractorFuelStorage quarryStorage)
            {
        ...
        private void TryUpgradeQuarry(BasePlayer player, int quarryId, string type)
        {
            ...
            if (upgrades.Count <= quarry.level) return;
        ...
        private void TryOpenFuelContainer(BasePlayer player, int quarryId)
        {
            ...
            if (config.lockAccesNoPerm)
            {
                ...
                foreach (var configPerm in config.permissions)
                    if (permission.UserHasPermission(player.UserIDString, configPerm.Key))
                        playerPerm = configPerm.Value;
                if (!playerPerm.ContainsKey("*") && !playerPerm.ContainsKey(data.quarries[quarryId].profile))
                {
                ...
                if (config.checkQuarryAmount)
                {
                    ...
                    {
                        if (quarryCheck.Value.owner == player.userID && quarryCheck.Value.profile == data.quarries[quarryId].profile)
                            quarryCount++;
                    }
                    if (playerPerm.ContainsKey(data.quarries[quarryId].profile) && quarryCount > playerPerm[data.quarries[quarryId].profile])
                    {
                        PopUpAPI?.Call("ShowPopUp", player, config.popUpPreset, Lang("TooManyQuarriesPermMissing", player.UserIDString));
                        return;
                    }
            ...
            player.inventory.loot.entitySource = fuelStorage;
        ...
        private class VirtualQuarry : FacepunchBehaviour
        {
            ...
            private void ConfigureStaticOutput()
            {
                output = new List<OutputInfo>();
                output.Clear();
                List<StaticQuarryOutput> quarryOutput = new List<StaticQuarryOutput>();
            ...
            {
                if (config.checkForFull)
                {
                    bool isFull = true;
                    for (int i = 0; i < storage.inventory.capacity; i++)
                    {
                        if (storage.inventory.GetSlot(i) == null)
                        {
                            isFull = false;
                            break;
                        }
        ...
        private class PluginConfig
        {
            ...
            [JsonProperty("Static Quarries - Check For Running Before Fuel Consume")]
            public bool checkForFull = false;
