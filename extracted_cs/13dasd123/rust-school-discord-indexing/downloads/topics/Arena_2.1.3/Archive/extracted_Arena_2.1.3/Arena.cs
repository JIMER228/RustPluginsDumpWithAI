...
namespace Oxide.Plugins
{
    ...
    public class Arena : ChaosPlugin
    {
        ...
        private Hash<string, IEventPlugin> EventModes { get; set; } = new Hash<string, IEventPlugin>();

        private Hash<string, BaseEventGame> ActiveEvents { get; set; } = new Hash<string, BaseEventGame>();
        ...
        private static readonly NetworkableId LOBBY_BAG_ID = new NetworkableId(113);
        #endregion
        ...
        private void Loaded()
        {
            ...
            Restore.Data.SetFlags(Restoration.Flags.RestoreHealth | Restoration.Flags.RestoreInventory | 
                                  Restoration.Flags.RestoreMetabolism | Restoration.Flags.RestorePosition | Restoration.Flags.RestoreTeam);
        ...
        public abstract class BaseEventGame : MonoBehaviour
        {
            ...
            private EventResults LastEventResult { get; set; } = new EventResults();
            ...
            public virtual void OnPlayerTakeDamage(BaseEventPlayer eventPlayer, HitInfo hitInfo)
            {
                ...
                {
                    (hitInfo.Initiator as BaseCombatEntity).Die(new HitInfo(eventPlayer.Player, hitInfo.Initiator, DamageType.Suicide, 1000f));
                    ClearDamage(hitInfo);
                    return;
                }
        ...
        #region API
        private readonly Hash<Plugin, Hash<string, EventTeleporter>> _pluginTeleporters = new Hash<Plugin, Hash<string, EventTeleporter>>();
        ...
        public class SpawnSelector
        {
            ...
            {
                _defaultSpawns = Spawns.IsLoaded ? Spawns.LoadSpawnFile(spawnFile) as List<Vector3> : new List<Vector3>();
        ...
        private static BaseEventPlayer GetUser(BasePlayer player) => player?.GetComponent<BaseEventPlayer>();
        ...
        private class EventResults
        {
            ...
            private List<ScoreEntry> Scores { get; set; } = new List<ScoreEntry>();
        ...
        private void CreateEventEntry(BasePlayer player, CuiElementContainer container, BaseEventGame eventGame, int index)
        {
            ...
            UI.Label(container, UI_MENU, string.Format(Message("Info.Event.Status", player.userID), eventGame.Status), 14, new UI4(position.xMin + 0.005f, position.yMin, position.xMax, position.yMin + 0.035f), TextAnchor.MiddleLeft);
        ...
        private Hash<ulong, EventConfig> _eventCreators = new Hash<ulong, EventConfig>();
        ...
        private void CmdEventMenu(BasePlayer player, string command, string[] args)
        {
            ...
            {
                if (permission.UserHasPermission(player.UserIDString, BLACKLISTED_PERMISSION))
                {
                    player.ChatMessage(Message("Error.Blacklisted", player.userID));
                    return;
                }
