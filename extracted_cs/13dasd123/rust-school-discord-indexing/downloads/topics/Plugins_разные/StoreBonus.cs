...
namespace Oxide.Plugins
{
    ...
    class StoreBonus : RustPlugin
    {
        ...
        private void LoadDefaultConfig()
        {
            ...
            GetConfig("Сообщения", "Формат сообщения о выдаче бонуса игроку в чат", ref ChatSms);

            GetConfig("GUI Баланс", "Включить панель баланса под чатом (Если отключено, будет появляться лишь кнопка Забрать бонус)", ref EnableGUIPlayer);
            GetConfig("GUI Баланс", "Цвет кнопки 'Забрать бонус'", ref GUIEnabledColor);
            GetConfig("GUI Баланс", "Текст кнопки 'Забрать бонус'", ref GUIEnabledText);
            ...
            CuiElement BonusIcon = new CuiElement
            {
                ...
                Components = {
                        new CuiRawImageComponent {
                            Url = "https://i.ibb.co/WfMNBsV/shopping-basket.png",
        ...
        {
            string url = $"http://gamestores.ru/api?shop_id={shopId}&secret={secret}" +
