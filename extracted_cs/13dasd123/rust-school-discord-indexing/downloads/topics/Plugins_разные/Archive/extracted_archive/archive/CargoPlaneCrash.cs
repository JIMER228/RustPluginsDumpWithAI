...
{
    [Info("CargoPlaneCrash", "Fruster", "1.4.0")]
    ...
    class CargoPlaneCrash : CovalencePlugin
    {
        ...
        void crashSite()
        {
            ...
            if (Configuration.coordMessage != "")
                foreach (BasePlayer player in BasePlayer.activePlayerList)
                    player.ChatMessage(Configuration.coordMessage + PhoneController.PositionToGridCoord(entity.transform.position).ToString());
        ...
        {
            CargoPlaneCrash cargoPlaneCrash = new CargoPlaneCrash();
