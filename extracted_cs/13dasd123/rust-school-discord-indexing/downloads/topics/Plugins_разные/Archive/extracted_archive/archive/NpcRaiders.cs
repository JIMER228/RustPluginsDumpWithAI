...
namespace Oxide.Plugins
{
    ...
    public class NpcRaiders : RustPlugin
    {
        ...
        [PluginReference]
        private Plugin Kits, TruePVE, ServerRewards, Economics, LifeSupport, AlphaLoot;
        ...
        public class raidConfig
        {
            ...
            [JsonProperty(PropertyName = "AlphaLoot profile names For Npc Loot")]
        ...
        public class EventRaidManager
        {
                ...
                else if (def == null)
                {
                    ...
                    if (reward.rewardAll && totalPlayers > 1)
                    {
                        foreach (var RewardPlayer in onlinePlayer.ToList())
                        {
                            ...
                            if (reward.RewardType.ToLower() == "serverrewards")
                            {
                                ...
                                _.ServerRewards?.Call("AddPoints", RewardPlayer.userID.Get(), Convert.ToInt32(totalsSplit));
                            ...
                            {
                                totalsSplit = ((Convert.ToInt32(totals)) / totalPlayers);
                                _.Economics?.Call("Deposit", RewardPlayer.userID.Get(), Convert.ToDouble(totalsSplit));
                    ...
                    else
                    {
                        if (player != null)
                        {
                            if (reward.RewardType.ToLower() == "serverrewards")
                            {
                                _.ServerRewards?.Call("AddPoints", player.userID.Get(), Convert.ToInt32(totals));
                                _.SendReply(player, String.Format(_.lang.GetMessage("MoneyIssued", _), Convert.ToInt32(totals), player.displayName));
                            }
                            else if (reward.RewardType.ToLower() == "economics")
                            {
                                _.Economics?.Call("Deposit", player.userID.Get(), Convert.ToDouble(totals));
            ...
            internal static List<string> _prefabs = new List<string> { "rock", "formation", "cliff", "dunes", "prevent_movement"};
