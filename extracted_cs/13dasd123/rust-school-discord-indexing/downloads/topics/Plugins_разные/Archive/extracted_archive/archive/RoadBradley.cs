...
{
    [Info("RoadBradley", "ThePitereq", "1.7.3")]
#if CARBON
...
#endif
    {
        ...
        private static void FindRegularRoads()
        {
            ...
            foreach (var road in TerrainMeta.Path.Roads)
            {
                ...
                string editedKey = $"{counter}_Edited";
                ...
                Dictionary<Vector3, string> obstacles = config.checkObstacles ? FindObstaclePrefabs() : null;
                int checkpointCount = road.Path.Points.Length;
                bool isLooped = (checkpointCount > 0 && Vector3.Distance(road.Path.Points[0], road.Path.Points[checkpointCount - 1]) < 20f);
                int checkpointCounter = 0;
                bool failedCheck = false;
                foreach (var checkpoint in road.Path.Points)
                {
                    checkpointCounter++;
        ...
        private void StartRouteEdit(BasePlayer player, string roadName)
        {
            ...
            activeEditors.Add(player.userID, new EditorCache()
            {
        ...
        private void SpawnRoadBradley(string profile, BasePlayer owner = null, string spawnProfile = "")
        {
            ...
            if (spawnProfile != "" && config.spawns[spawnProfile].announceSpawn)
            {
                ...
                foreach (var player in BasePlayer.activePlayerList)
                    SendReply(player, Lang("BradleySpawned", player.UserIDString, profile, PhoneController.PositionToGridCoord(startPosition)));
            }
            else if (owner != null)
            {
                Puts($"Road Bradley purchased by {owner.displayName} ({owner.UserIDString}) of profile '{profile}' has spawned at {startPosition}!");
                StringBuilder sb = Pool.Get<StringBuilder>();
                SendReply(owner, Lang("BradleyPurchased", owner.UserIDString, profile, PhoneController.PositionToGridCoord(startPosition), FormatTime(config.purchases[profile].killTime, sb), FormatTime(config.purchases[profile].comingTime, sb)));
                Pool.FreeUnmanaged(ref sb);
            }
