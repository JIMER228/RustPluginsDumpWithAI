		...
		private class TemplateData
		{
				...
				if (categoryPage != null)
				{
					switch (categoryPage.Type)
					{
						...
						case CategoryPage.PageType.UI:
						{
							...
							// categoryPage.Elements?.ForEach(element => element?.Get(ref container, player, LayerContent, element.Name));
				...
				CuiElementContainer container)
			{
				...
				foreach (var menuCategory in _categoriesData.Categories)
				{
					...
					UI.Categories.CategoryTitle.Get(player, container, categoryButton, text: Instance.Msg(player, menuCategory.Title),
							...
							switch (arg.GetString(2))
							{
								...
								case "line":
								{
									...
									switch (textAction)
									{
										case "set":
										{
											var val = string.Join(" ", arg.Args.Skip(5)).FormatEscapedRichText();
