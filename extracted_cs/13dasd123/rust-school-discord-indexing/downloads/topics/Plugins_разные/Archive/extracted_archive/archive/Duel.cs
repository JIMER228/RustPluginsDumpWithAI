...
namespace Oxide.Plugins
{
    ...
    class Duel : RustPlugin
    {
        ...
        string notificationAboutWin = "[Дуэль] <color=#6ecc68>{0}</color> vs <color=#cc6868>{1}</color>\nПобедитель: <color=#68a7cc>{2}</color>";
        ...
        void RemoveGarbage(BaseEntity entity)
        {
            ...
            if (!isIni) return;
            if (entity is CargoPlane || entity is SupplyDrop || entity is CH47Helicopter ||
                entity is BaseHelicopter) return;
...
//                    mods = new List<SavedItem>(),
                ...
                if(item.contents != null)
                {
                    ...
                    {
                        iItem.content.Add(new ItemContent()
                        {
                            shortname = cont.info.shortname,
                            itemid = cont.info.itemid,
                            amount = cont.amount,
                            condition = cont.condition,
                            maxcondition = cont.maxCondition
                        });
                    }
                }
                return iItem;
            ...
            Item BuildItem(SavedItem sItem)
            {
                ...
                Chainsaw chainsaw = item.GetHeldEntity()?.GetComponent<Chainsaw>();
                if (chainsaw)
                    chainsaw.ammo = sItem.flamefuel;
                if(sItem.content != null)
                {
                    foreach(var cont in sItem.content)
                    {
                        Item new_cont = ItemManager.CreateByItemID(cont.itemid, cont.amount);
                        if (new_cont.hasCondition)
                        {
                            new_cont.condition = cont.condition;
                            new_cont.maxCondition = cont.maxcondition;
                        }
...
//            public List<SavedItem> mods;
        ...
        void Unload()
        {
			foreach (var check in UnityEngine.Object.FindObjectsOfType<TeamDuel>())
            ...
            foreach (var data in entities)
            {
                try
                {
                    ...
                    if (entity != null)
                    {
                        ...
                        var ladd = entity.GetComponentInParent<BaseLadder>();
                        if (ladd != null)
                            ladd.pickup.enabled = false;
						var doors = entity.GetComponentInParent<Door>();
                        if (doors != null)
                            doors.pickup.enabled = false;
        ...
        {
            public DuelStavki(string SteamId, string Win, int item, int kolvo, bool weapon, int patron, string ammotype, int flamefuel, float maxCondition, float condition)
            {
        ...
        void Loaded()
        {
            timer.Repeat(1f, 0, delegate
            {
                ...
                if (check > 0)
                {
					...
					if (item.hasCondition)
					{
						item.condition = con.condition;
						item.maxCondition = con.maxCondition;
					}
                    FlameThrower flameThrower = item.GetHeldEntity()?.GetComponent<FlameThrower>();
                    if (flameThrower)
                ...
                foreach (Item item in view.inventory.itemList.ToArray())
                {
                    if (item.position != -1)
                    {
                        ...
                        if (weapon != null)
                        {
                            ...
                            // Puts("1");
                            Tops.Add(new DuelStavki(player.UserIDString, "", item.info.itemid, item.amount, true, weapon.primaryMagazine != null ? weapon.primaryMagazine.contents : 0, weapon.primaryMagazine != null ? weapon.primaryMagazine.ammoType.shortname : "", 0, item.maxCondition, item.condition));
                        }
                        else
                        {
                            // Puts("2");
                            ...
                            //    iItem.flamefuel = flameThrower.ammo;
                            FlameThrower flameThrower = item.GetHeldEntity()?.GetComponent<FlameThrower>();
                            Tops.Add(new DuelStavki(player.UserIDString, "", item.info.itemid, item.amount, false, 0, "", flameThrower != null ? flameThrower.ammo : 0, item.maxCondition, item.condition));
                        }
