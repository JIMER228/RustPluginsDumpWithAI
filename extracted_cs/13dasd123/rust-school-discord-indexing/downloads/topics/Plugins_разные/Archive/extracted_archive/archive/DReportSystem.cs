...
{
    [Info("DReportSystem", "TopPlugin.ru", "1.0.19")]
    public class DReportSystem : RustPlugin
    {
        ...
        {
            if (!cfg.plugininterface.english) NotificationList.SendNotificationTo(userid, NotificationChannel.SmartAlarm, "ReportSystem", $"Игрок {reportplayername}/{reportplayerid} отправил жалобу на игрока {suspectname}/{suspectid}. Заголовок жалобы: {subject}, текст жалобы: {message}", new Dictionary<string, string>());
            else NotificationList.SendNotificationTo(userid, NotificationChannel.SmartAlarm, "ReportSystem", $"Player {reportplayername}/{reportplayerid} sent a report to the player {suspectname}/{suspectid}. Report subject: {subject}, report message: {message}", new Dictionary<string, string>());
        }
            ...
            private void Send()
            {
                ...
                _ins.webrequest.Enqueue( _url, _payload, ( code, rawResponse ) =>
                {
                    ...
                    else
                    {
                        ...
                        {
                            Interface.Oxide.LogException( "[DReportSystem] Request callback raised an exception!", ex );
                        }
                    }
        ...
        public class FancyMessage
        {
            ...
            {
                return JsonConvert.SerializeObject( this, _ins.jsonSettings );
            }
        }
        ...
        {
            if (cfg.alerts.webhook == "Если не собираетесь использовать оповещения в канал Discord, не изменяйте это значение") return;
        ...
        {
            if (cfg.alerts.logwebhook == "Если не собираетесь использовать логи в канал Discord, не изменяйте это значение") return;
        ...
        void SendReport(BasePlayer player, BasePlayer suspect, string subject, string message)
        {
            ...
            {
                player.ChatMessage(GetMsg("Repeated Report"));
                return;
            }
            ...
            if (!cfg.plugininterface.english) Log($"Игрок {player.displayName}/{player.userID} отправил жалобу на игрока: {suspect.displayName}/{suspect.userID}. Заголовок жалобы: {subject}, текст: {message}");
            else Log($"Player {player.displayName}/{player.userID} filed a complaint against player: {suspect.displayName}/{suspect.userID}. Complaint title: {subject}, text: {message}");
            foreach (var players in BasePlayer.activePlayerList.Where(x => permission.UserHasPermission(x.UserIDString, cfg.access.perm)))
            {
                if (cfg.alerts.rustplusalert) RustPlusAlert(players.userID, player.displayName, suspect.displayName, player.userID, suspect.userID, subject, message);
                if (cfg.alerts.chatalert) players.ChatMessage(GetMsg("New Report").Replace("{0}", $"{player.displayName}").Replace("{1}", $"{player.userID}").Replace("{2}", $"{suspect.displayName}").Replace("{3}", $"{suspect.userID}").Replace("{4}", $"{subject}").Replace("{5}", $"{message}"));
            }
            if (cfg.alerts.vkalert && cfg.alerts.token != "Если не собираетесь использовать оповещения в беседу VK, не изменяйте это значение" 
            && cfg.alerts.chatid != "Если не собираетесь использовать оповещения в беседу VK, не изменяйте это значение") SendVKChatMessage(GetMsg("New Report").Replace("{0}", $"{player.displayName}").Replace("{1}", $"{player.userID}").Replace("{2}", $"{suspect.displayName}").Replace("{3}", $"{suspect.userID}").Replace("{4}", $"{subject}").Replace("{5}", $"{message}"));
            if (cfg.alerts.discordalert) DiscordMessage(GetMsg("New Report").Replace("{0}", $"``{player.displayName}``").Replace("{1}", $"``{player.userID}``").Replace("{2}", $"``{suspect.displayName}``").Replace("{3}", $"``{suspect.userID}``").Replace("{4}", $"``{subject}``").Replace("{5}", $"``{message}``"));
        }
        ...
        void AlarmCheckSuspect(BasePlayer suspect)
        {
            ...
            DrawNotify(suspect, GetMsg("Player Check Notify"));
        }
        ...
        void AlarmCheckModerator(BasePlayer moderator)
        {
            ...
            DrawNotify(moderator, GetMsg("Moderator Check Notify"));
        }

        void CancelCheck(BasePlayer suspect)
        {
        ...
        private void CheckCommand(BasePlayer player, string command, string[] args)
        {
            ...
            if (args.Length < 1)
            {
                player.ChatMessage(GetMsg("Verdict Help"));
                return;
            }
            if (args.Length > 2)
            {
                player.ChatMessage(GetMsg("Verdict Error"));
                return;
            }
            ...
            if (args[0] == "cancel")
            {
                ...
                player.ChatMessage(GetMsg("Success Vedict"));

                if (!cfg.plugininterface.english) Log($"{player.displayName}/{player.userID} закончил проверку игрока и вынес свой вердикт: {args[1]}");
                else Log($"{player.displayName}/{player.userID} finished checking the player and delivered his verdict: {args[1]}");
                ...
                DiscordLog($"``{player.displayName}/{player.userID}`` закончил проверку игрока и вынес свой вердикт: ``{args[1]}``");
                if (cfg.plugininterface.english && cfg.alerts.discordalert && cfg.alerts.logwebhook != "Если не собираетесь использовать логи в канал Discord, не изменяйте это значение") 
                DiscordLog($"``{player.displayName}/{player.userID}`` finished checking the player and delivered his verdict: ``{args[1]}``");
        ...
        private void SendDiscordCommand(BasePlayer player, string command, string[] args)
        {
            ...
            if (args.Length < 1)
            {
                player.ChatMessage(GetMsg("Send Discord Help"));
                return;
            }
            if (args.Length > 1)
            {
                player.ChatMessage(GetMsg("Send Discord Error"));
                return;
            }
            ...
            if (moderator == null)
            {
                player.ChatMessage(GetMsg("Moderator Leave"));
                CancelCheck(player);
                return;
            }
            moderator.ChatMessage(GetMsg("Player Send Discord").Replace("{0}", args[0].Replace("\"", string.Empty)) + "\n" + GetMsg("Discord Double"));
            moderator.SendConsoleCommand($"echo {GetMsg("Player Send Discord").Replace("{0}", args[0].Replace("\"", string.Empty))}");

            if (!cfg.plugininterface.english) Log($"{player.displayName}/{player.userID} отправил свой Discord модератору {moderator.displayName}/{moderator.userID}");
            else Log($"{player.displayName}/{player.userID} sent Discord to a moderator {moderator.displayName}/{moderator.userID}");

            if (!cfg.plugininterface.english && cfg.alerts.discordalert && cfg.alerts.logwebhook != "Если не собираетесь использовать логи в канал Discord, не изменяйте это значение") 
                DiscordLog($"``{player.displayName}/{player.userID}`` отправил свой Discord модератору ``{moderator.displayName}/{moderator.userID}``");
            if (cfg.plugininterface.english && cfg.alerts.discordalert && cfg.alerts.logwebhook != "Если не собираетесь использовать логи в канал Discord, не изменяйте это значение") 
                DiscordLog($"``{player.displayName}/{player.userID}`` sent Discord to a moderator ``{moderator.displayName}/{moderator.userID}``");
        }
        ...
        {
            if (player == null) return;
            if (cfg.pluginsettings.alert) player.ChatMessage(GetMsg("Report Command"));
        }
        ...
        private void ReportsCommand(BasePlayer player)
        {
            ...
            {
                player.ChatMessage(GetMsg("No Access"));
                return;
            }
            DrawUI(player, 0);
        }
        ...
        {
            DiscordLog("``Test/76561111111`` отправил свой Discord модератору ``Test/76561111111``");
        }
        ...
        private void DeletePlayer(ConsoleSystem.Arg args)
        {
            ...
            if (!cfg.plugininterface.english) Log($"{player.displayName}/{player.userID} очистил количество репортов игроку {name}/{id}");
            else Log($"{player.displayName}/{player.userID} cleared the number of reports to the player {name}/{id}");

            if (!cfg.plugininterface.english && cfg.alerts.discordalert && cfg.alerts.logwebhook != "Если не собираетесь использовать логи в канал Discord, не изменяйте это значение") 
                DiscordLog($"``{player.displayName}/{player.userID}`` очистил количество репортов игроку ``{name}/{id}``");
            if (cfg.plugininterface.english && cfg.alerts.discordalert && cfg.alerts.logwebhook != "Если не собираетесь использовать логи в канал Discord, не изменяйте это значение") 
                DiscordLog($"``{player.displayName}/{player.userID}`` cleared the number of reports to the player ``{name}/{id}``");
        }
        ...
        private void CheckPlayer(ConsoleSystem.Arg args)
        {
            ...
            if (suspect == null)
            {
                player.ChatMessage(GetMsg("Player Not Found"));
                return;
            }
            if (Check.ContainsValue(player.userID))
            {
                player.ChatMessage(GetMsg("You On Check"));
                return;
            }
            if (Check.ContainsKey(suspect.userID))
            {
                player.ChatMessage(GetMsg("Player On Check"));
                return;
            }
            ...
            if (!cfg.plugininterface.english) Log($"{player.displayName}/{player.userID} начал проверку игрока {suspect.displayName}/{suspect.userID}");
            else Log($"{player.displayName}/{player.userID} started checking the player {suspect.displayName}/{suspect.userID}");
            
            if (!cfg.plugininterface.english && cfg.alerts.discordalert && cfg.alerts.logwebhook != "Если не собираетесь использовать логи в канал Discord, не изменяйте это значение") 
                DiscordLog($"``{player.displayName}/{player.userID}`` начал проверку игрока ``{suspect.displayName}/{suspect.userID}``");
            if (cfg.plugininterface.english && cfg.alerts.discordalert && cfg.alerts.logwebhook != "Если не собираетесь использовать логи в канал Discord, не изменяйте это значение") 
                DiscordLog($"``{player.displayName}/{player.userID}`` started checking the player ``{suspect.displayName}/{suspect.userID}``");
        }
        ...
        private void OpenPlayerInfo(ConsoleSystem.Arg args)
        {
            BasePlayer player = args.Player();
            if (player == null || !permission.UserHasPermission(player.UserIDString, cfg.access.perm) || !args.HasArgs(1)) return;
            DrawPlayerInfo(player, ulong.Parse(args.Args[0]), 0);
        }
        ...
        private void DrawUI(BasePlayer player, int page)
        {
            ...
            container.Add(new CuiElement
            {
                ...
                Components =
                {
                    new CuiImageComponent {Color = HexToRustFormat("#181B18DB"), Material = ""},
            ...
            for (int i = 0; i < dict.Count(); i++)
            {
                ...
                if (Players[suspectid].Reports.Count > 0)
                {
                    container.Add(new CuiButton
                    {
                        RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1" },
        ...
        private void DrawPlayerInfo(BasePlayer player, ulong steamid, int pages)
        {
            ...
            container.Add(new CuiElement
            {
                ...
                Components =
                {
                    new CuiTextComponent { Color = HexToRustFormat("#DDDDDDFF"), Text = $"{Players[steamid].name.ToUpper()}", Align = TextAnchor.UpperLeft, FontSize = 18, Font = "RobotoCondensed-bold.ttf" },
                    new CuiRectTransformComponent {AnchorMin = "0.05333334 0.1979178", AnchorMax = "0.3711112 0.7187503"}
                }
            });
            container.Add(new CuiElement
