...
namespace Oxide.Plugins
{
    ...
    [Info("BBank", "https://topplugin.ru/", "1.0.2")]
    public class BBank : RustPlugin
    {
        ...
        void OnServerInitialized()
        {
            ...
            BasePlayer.activePlayerList.ToList().ForEach(ShowGUI_Icon);
        }
        ...
        void DeleteNPC()
        {
            if (Data.NPCID > 0)
            {
                var noc = BaseNetworkable.serverEntities.Find(Data.NPCID);
                if (noc && noc.IsDestroyed == false)
                {
                    noc.Kill();
                    PrintWarning($"BOT DELETE");
                }
            }
        }
        ...
        void SpawnNPC(BasePlayer player)
        {
            ...
            {
                Item bot = ItemManager.CreateByName("shirt.collared", 1, 0);
                Item grev = ItemManager.CreateByName("pants", 1, 0);
                Item grev1 = ItemManager.CreateByName("shoes.boots", 1, 0);

                var ent = GameManager.server.CreateEntity("assets/prefabs/player/player.prefab", hit.point) as BasePlayer;
                ent.Spawn();
                ent.displayName = "Банкир";
                if (6 - ent.inventory.containerWear.itemList.Count > 0)
                {
                    bot.MoveToContainer(ent.inventory.containerWear);
                    grev.MoveToContainer(ent.inventory.containerWear);
                    grev1.MoveToContainer(ent.inventory.containerWear);
                }
