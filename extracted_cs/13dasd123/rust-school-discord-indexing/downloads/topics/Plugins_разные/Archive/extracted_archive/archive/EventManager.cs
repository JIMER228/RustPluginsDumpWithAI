...
{
    [Info("EventManager", "senyaa & M&B-Studios", "2.2.5")]
  ...
  private Dictionary<ulong, NewCreatorData> _creatingCreators = new Dictionary<ulong, NewCreatorData>();
    ...
    foreach (var entry in config.Entries.Values)
    {
        ...
        if (entry.Hour == hour && entry.Minute == minute)
        {
            ...
            if (entry.Name == "RANDOM START" && entry.RandomStartEvents.Count > 0)
            {
                ...
                Instance.rust.RunServerCommand(eventSettings.Command);
                Instance.CallStartedHook(eventSettings.displayName);
                continue;
            }
            ...
            Instance.rust.RunServerCommand(eventSettingsEntry.Command);
            Instance.CallStartedHook(eventSettingsEntry.displayName);
        }
    }
            ...
            timer.Once(3f, () =>
            {
                ...
                foreach (var x in EventList.ToArray())
                    foreach (var y in x.Value)
                        if (string.IsNullOrEmpty(y.CommandType))
                        {
                            EventList[x.Key][EventList[x.Key].IndexOf(y)].CommandType = "console";
                            replacedType = true;
                        }
...
private void cmdCreateConfirm(ConsoleSystem.Arg arg)
{
    ...
    if (e.CommandType == "chat")
    {
        // Simulate chat command execution
        SimulateChatCommand(arg.Player(), e.Command);
    }
    else
    {
...
private void SimulateChatCommand(BasePlayer player, string command)
{
    ...
    if (hookResult != null)
    {
        player.ChatMessage(hookResult.ToString());
    }
    else
    {
        // If no hook handles the command, send an error message
        player.ChatMessage($"Unknown command: {cmd}");
    }
}
...
private void StartEvent(ConsoleSystem.Arg arg)
{
    ...
    switch (eventSettings.CommandType)
    {
        ...
        case "chat":
            ...
            arg.Player().SendConsoleCommand($"chat.say \"{eventSettings.Command}\"");
            break;
        default:
            return;
    }
...
private void UI_DrawCreateNewCreator(BasePlayer player)
{
    ...
    string nameInput = container.Add(new CuiPanel
    {
    ...
    AddButton(container, mainPanel, "0.068 0.332", "0.9 0.52", "cmdShowColorOptions", "choose a colour", 14, "0.2 0.2 0.2 1");
    AddButton(container, mainPanel, "0.06 0.052", "0.416 0.248", "cmdCancelCreation", "Cancel", 14, "0.8156863 0.5568628 0.4745098 1");
    AddButton(container, mainPanel, "0.544 0.06", "0.9 0.256", "cmdConfirmCreator", "Create", 14, "0.8078431 0.8431373 0.5490196 1");
...
private void AddButton(CuiElementContainer container, string parent, string anchorMin, string anchorMax, string command, string text, int fontSize, string color)
{
    string buttonId = container.Add(new CuiButton
    {
...
private void cmdPage(ConsoleSystem.Arg arg)
{
    ...
    UI_DrawCreators(arg.Player(), page);
}
        ...
        private void UI_DrawCreators(BasePlayer player, int page = 0)
        {
            ...
            container.Add(new CuiButton
            {
                ...
                Text = { Text = "<", Font = "robotocondensed-bold.ttf", FontSize = 25, Align = TextAnchor.MiddleCenter, Color = "0.8078431 0.8431373 0.5490196 1" },
                RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "120.6 19.5", OffsetMax = "155.6 54.5" }
            }, Layer + ".eventcreator.div", Layer + ".eventmanager" + ".eventcreator.div" + ".btn.prev");

            container.Add(new CuiButton
            {
                Button = { Color = "0.6470588 0.6235294 0.6 1", Command = EventList.Where(x => x.Value.Count < 8).Skip(8 *
                    (page + 1)).Any() ? $"em_page {page + 1}" : "" },
                Text = { Text = ">", Font = "robotocondensed-bold.ttf", FontSize = 25, Align = TextAnchor.MiddleCenter, Color = "0.8078431 0.8431373 0.5490196 1" },
                RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "158.7 19.5", OffsetMax = "193.7 54.5" }
...
private void UI_DrawCreateNewEvent(BasePlayer player)
{
    ...
    container.Add(new CuiButton
    {
        ...
        Text =
        {
            Text = "Console Command", Font = "robotocondensed-bold.ttf", FontSize = 18, Align = TextAnchor.MiddleCenter,
            Color = "1 1 1 1"
        },
        RectTransform =
        {
...
private void ToggleCommandType(ConsoleSystem.Arg arg)
{
    ...
    container.Add(new CuiButton
    {
        ...
        Text =
        {
            Text = ev.CommandType == "console" ? "Console Command" : "Chat Command", Font = "robotocondensed-bold.ttf", FontSize = 18, Align = TextAnchor.MiddleCenter,
            Color = "1 1 1 1"
        },
        RectTransform =
        {
...
private void cmdShowColorOptions(ConsoleSystem.Arg arg)
{
    ...
    string[] colors = { "1 1 1 1", "0.76 0.42 0.36 1", "0.76 0.27 0.19 1", "1 0.14 0 1", "1 0.77 0.57 1", "1 0.65 0.35 1", "1 0.47 0 1", "0.79 0.44 0.13 1", "0.27 0.79 0.13 1", "0.23 0.73 0.09 1", "0.22 1 0 1", "0.15 0.69 0 1", "0 0.69 0.68 1", "0 1 0.98 1", "0.23 0.8 1 1", "0 0.73 1 1", "0.27 0.62 1 1", "0 0.48 1 1", "0.31 0.37 1 1", "0 0.08 1 1" };
    float yPos = 0.8f;
    float xPos = 0.067f;
    int counter = 0;
...
private void SetCreatorColor(ConsoleSystem.Arg arg) {
    ...
    string color = string.Join(" ", arg.Args);
    ulong userId = player.userID;

    Puts($"Color set attempt by {player.displayName} [{userId}]: {color}"); // Logging the color and user info

    if (_creatingCreators.ContainsKey(userId)) {
        _creatingCreators[userId].Color = color;
...
private void cmdCancelCreation(ConsoleSystem.Arg arg)
{
    ...
    CuiHelper.DestroyUi(player, "Hud");  // "Hud" muss mit der ID übereinstimmen, die beim Erstellen verwendet wurde.
}
...
private void cmdConfirmCreator(ConsoleSystem.Arg arg)
{
    ...
    AuthorName author = new AuthorName { Name = newCreator.Name, Color = newCreator.Color }; 
