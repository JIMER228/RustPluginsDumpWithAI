...
namespace Oxide.Plugins
{
    ...
    class IQChat : RustPlugin
    {
            ...
            if (target == null)
            {
                ...
                if (UInt64.TryParse(NameOrID, out Steam64ID))
                {
                    if (UserInformation.ContainsKey(Steam64ID))
                    {
                        ...
                        if (!Moderator.IsAdmin && Moderator.userID.Get() == Steam64ID)
                        {
        ...
        private void ChatCommandRename(BasePlayer Renamer, string command, string[] args)
        {
            ...
            if (General.RenameList.ContainsKey(Renamer.userID.Get()))
            {
                General.RenameList[Renamer.userID.Get()].RenameNick = Name;
                General.RenameList[Renamer.userID.Get()].RenameID = ID;
            }
            else General.RenameList.Add(Renamer.userID.Get(), new GeneralInformation.RenameInfo { RenameNick = Name, RenameID = ID });
                ...
                private void DrawUI_IQChat_Update_Check_Box(BasePlayer player, ElementsSettingsType Type, String OffsetMin, String OffsetMax, Boolean StatusCheckBox)
        {
            ...
            User Info = UserInformation[player.userID.Get()];
                ...
                private void MutePlayer(BasePlayer Target, MuteType Type, Int32 ReasonIndex, BasePlayer Moderator = null, String ReasonCustom = null, Int32 TimeCustom = 0, Boolean HideMute = false, Boolean Command = false, String fakeUserId = "")
        {
            ...
            if (!UserInformation.ContainsKey(Target.userID.Get())) return;
            User Info = UserInformation[Target.userID.Get()];
            ...
            if (Moderator != null)
            {
                ...
                {
                    GeneralInformation.RenameInfo ModeratorRename = GeneralInfo.GetInfoRename(Moderator.userID.Get());
                    NameModerator = ModeratorRename != null ? $"{ModeratorRename.RenameNick ?? Moderator.displayName}" : Moderator.displayName;
                }
            }

            GeneralInformation.RenameInfo TagetRename = GeneralInfo.GetInfoRename(Target.userID.Get());
        ...
        private class InterfaceBuilder
        {
                        ...
                        private void BuildingStaticContext()
            {
                ...
                container.Add(new CuiElement
                {
                    ...
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-149.429 112.0213406", OffsetMax = "152.881 131.787" }
                }
                });
            ...
            if (target == null)
            {
                ...
                if (UInt64.TryParse(NameOrID, out Steam64ID))
                {
                    if (UserInformation.ContainsKey(Steam64ID))
                    {
                        ...
                        if (arg.Player()!= null)
                        {
                            if (!arg.Player().IsAdmin && arg.Player().userID.Get() == Steam64ID)
                            {
                                ConsoleOrPrintMessage(arg.Player(),
                                    LanguageEn
        ...
        public Object IsGradientColorValue(string value)
        {
            ...
            List<String> colors = new List<string>(value.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries));
        ...
        private IOrderedEnumerable<BasePlayer> GetPlayerList(String searchName, SelectedAction action)
        {
            ...
            if (searchName != null)
                basePlayerList = basePlayerList.Where(p => UserInformation.ContainsKey(p.userID.Get()) && p.displayName.ToLower().Contains(searchName.ToLower())).ToList();

            return action == SelectedAction.Mute ?
                basePlayerList.OrderBy(p => UserInformation[p.userID.Get()].MuteInfo.IsMute(MuteType.Chat) || UserInformation[p.userID.Get()].MuteInfo.IsMute(MuteType.Voice)) :
                basePlayerList.OrderBy(p => UserInformation[p.userID.Get()].Settings.IgnoreUsers.Contains(p.userID.Get()));
        }
        ...
        private void UserConnecteionData(BasePlayer player)
        {
            ...
            {
                if (!UserInformationConnection.ContainsKey(player.userID.Get()))
                    UserInformationConnection.Add(player.userID.Get(), new AntiNoob());
            }

            Configuration.ControllerConnection ControllerConntect = config.ControllerConnect;
            Configuration.ControllerParameters ControllerParameter = config.ControllerParameter;
            if (ControllerConntect == null || ControllerParameter == null || UserInformation.ContainsKey(player.userID.Get())) return;
            ...
            UserInformation.Add(player.userID.Get(), Info);
        }
            ...
            if (IQRankSystem)
            {
                ...
                if (IQRank.UseRankSystem)
                {
                    if (IQRank.UseTimeStandart)
                        RankTime = String.IsNullOrWhiteSpace(IQRankGetTimeGame(player.userID.Get())) ? String.Empty : String.Format(IQRank.FormatRankTime, IQRankGetTimeGame(player.userID.Get()));
                    Rank = String.IsNullOrWhiteSpace(IQRankGetRank(player.userID.Get())) ? String.Empty : String.Format(IQRank.FormatRank, IQRankGetRank(player.userID.Get()));
        ...
        private void UnmutePlayer(BasePlayer Target, MuteType Type, BasePlayer Moderator = null, Boolean HideUnmute = false, Boolean Command = false, String fakeUserId = "")
        {
            ...
            GeneralInformation.RenameInfo TargetRename = GeneralInfo.GetInfoRename(Target.userID.Get());
            GeneralInformation.RenameInfo ModeratorRename = Moderator != null ? GeneralInfo.GetInfoRename(Moderator.userID.Get()) : null;
            if (!Info.MuteInfo.IsMute(Type))
            {
                if (Moderator != null)
            ...
            {
                if (Moderator != null)
                    if (!Moderator.IsAdmin && Moderator.userID.Get() == Target.userID.Get())
                    {
                        ReplySystem(Moderator, GetLang("FUNC_MESSAGE_UNMUTE_YOURSELF", Moderator.UserIDString));
                        return;
                    }
        ...
        {
            User MyInfo = UserInformation[player.userID.Get()];
            ...
            else
            {
                foreach (var playerInList in PlayerList)
                {
                    String Interface = InterfaceBuilder.GetInterface("UI_Chat_Mute_And_Ignore_Player");
                    if (Interface == null) return;
                    User Info = UserInformation[playerInList.userID.Get()];
                    if (Info == null) continue;

                    String DisplayName = playerInList.displayName;
                    if (GeneralInfo.RenameList.ContainsKey(playerInList.userID.Get()))
                        if (!String.IsNullOrWhiteSpace(GeneralInfo.RenameList[playerInList.userID.Get()].RenameNick))
                            DisplayName = GeneralInfo.RenameList[playerInList.userID.Get()].RenameNick;
		   		 		  						  	   		  	 	 		  	  			  			 		  				
                    Interface = Interface.Replace("%OFFSET_MIN%", $"{-385.795 - (-281.17 * X)} {97.54 - (46.185 * Y)}");
                    Interface = Interface.Replace("%OFFSET_MAX%", $"{-186.345 - (-281.17 * X)} {132.03 - (46.185 * Y)}");
                    Interface = Interface.Replace("%DISPLAY_NAME%", $"{DisplayName}");
                    Interface = Interface.Replace("%COMMAND_ACTION%", $"newui.cmd action.mute.ignore ignore.and.mute.controller {Action} confirm.alert {playerInList.userID.Get()}");
                    ...
                    switch (Action)
                    {
                        case SelectedAction.Mute:
                            ...
                            break;
                        case SelectedAction.Ignore:
                            if (MyInfo.Settings.IsIgnored(playerInList.userID.Get()))
                                Color = ColorRed;
                            else Color = ColorGreen;
                            break;
                        default:
        ...
        private void SeparatorChat(Chat.ChatChannel channel, BasePlayer player, String Message)
        {
            ...
            if (antiNoob.AntiNoobActivate)
                if (IsNoob(player.userID.Get(), antiNoob.TimeBlocked))
                {
                    ReplySystem(player, GetLang("IQCHAT_INFO_ANTI_NOOB", player.UserIDString, FormatTime(UserInformationConnection[player.userID.Get()].LeftTime(antiNoob.TimeBlocked), player.UserIDString)));
                    return;
                }
            ...
            if (ControllerMessage.TurnedFunc.AntiSpamSetting.AntiSpamActivate)
                if (!permission.UserHasPermission(player.UserIDString, PermissionAntiSpam))
                {
                    if (!Info.MuteInfo.IsMute(MuteType.Chat))
                    {
                        if (!Flooders.ContainsKey(player.userID.Get()))
                            Flooders.Add(player.userID.Get(), new FlooderInfo { Time = CurrentTime + ControllerMessage.TurnedFunc.AntiSpamSetting.FloodTime, LastMessage = Message });
                        else
                        {
                            if (Flooders[player.userID.Get()].Time > CurrentTime)
                            {
                                ReplySystem(player, GetLang("FLOODERS_MESSAGE", player.UserIDString, Convert.ToInt32(Flooders[player.userID.Get()].Time - CurrentTime)));
                                return;
                            }

                            if (ControllerMessage.TurnedFunc.AntiSpamSetting.AntiSpamDuplesSetting.AntiSpamDuplesActivate)
                            {
                                if (Flooders[player.userID.Get()].LastMessage == Message)
                                {
                                    if (Flooders[player.userID.Get()].TryFlood >= ControllerMessage.TurnedFunc.AntiSpamSetting.AntiSpamDuplesSetting.TryDuples)
                                    {
                                        ...
                                        Flooders[player.userID.Get()].TryFlood = 0;
                                        return;
                                    }
                                    Flooders[player.userID.Get()].TryFlood++;
                                }
                            }
                        }
                        Flooders[player.userID.Get()].Time = ControllerMessage.TurnedFunc.AntiSpamSetting.FloodTime + CurrentTime;
                        Flooders[player.userID.Get()].LastMessage = Message;
                    }
                }

            GeneralInformation General = GeneralInfo;
            GeneralInformation.RenameInfo RenameInformation = General.GetInfoRename(player.userID.Get());
            ...
            if (config.RustPlusSettings.UseRustPlus)
                if (channel == Chat.ChatChannel.Team)
                {
                    ...
                    if (Team == null) return;
                    Util.BroadcastTeamChat(player.Team, player.userID.Get(), player.displayName, FormattingMessage, Info.Info.ColorMessage);
                }
        ...
        {
            if (UserInformation[player.userID.Get()].MuteInfo.IsMute(MuteType.Voice))
                return false;
            return null;
        }
                ...
                private void DrawUI_IQChat_Update_DisplayName(BasePlayer player)
        {
            ...
            String DisplayName = player.displayName;
            String ColorNickPlayer = Info.Info.ColorNick ?? "#ffffff";
            ...
            DisplayNick += $"{DisplayName}: <color={Info.Info.ColorMessage ?? "#ffffff"}>{ChatMessage}</color>";
        ...
        {
            GeneralInformation.RenameInfo RenameInfo = GeneralInfo.GetInfoRename(player.userID.Get());
            UInt64 RenameID = RenameInfo != null ? RenameInfo.RenameID != 0 ? RenameInfo.RenameID : player.userID.Get() : player.userID;
        ...
        private void ChatCommandOpenedUI(BasePlayer player)
        {
            ...
            if (IQRankSystem && config.ReferenceSetting.IQRankSystems.UseRankSystem)
            {
                ...
                foreach (String Rank in IQRankListKey(player.userID.Get()))
        ...
        {
            foreach (BasePlayer p in !AdminAlert ? BasePlayer.activePlayerList.Where(p => UserInformation[p.userID.Get()].Settings.TurnBroadcast) : BasePlayer.activePlayerList)
        ...
        private void AlertController(BasePlayer player)
        {
            ...
            GeneralInformation.RenameInfo RenameInformation = GeneralInfo.GetInfoRename(player.userID.Get());
            ...
            if (AlertSessionPlayer.ConnectedAlert)
            {
                ...
                if (AlertSessionPlayer.ConnectedWorld)
                {
                    ...
                    webrequest.Enqueue("http://ip-api.com/json/" + ipPlayer, null, (code, response) =>
                    {
                        ...
                        Log($"[{player.userID.Get()}] {GetLang("WELCOME_PLAYER_WORLD", "", DisplayName, country ?? "none")}");
                    }, this);
                }
                else
                {
                    ...
                    Log($"[{player.userID.Get()}] {GetLang("WELCOME_PLAYER", "", DisplayName)}");
                }
            }
        ...
        void Alert(BasePlayer Sender, string[] arg, Boolean IsAdmin)
        {
            ...
            if (config.RustPlusSettings.UseRustPlus)
                foreach (BasePlayer playerList in BasePlayer.activePlayerList)
                    NotificationList.SendNotificationTo(playerList.userID.Get(), NotificationChannel.SmartAlarm, config.RustPlusSettings.DisplayNameAlert, Message, Util.TryGetServerPairingData());
        }
        ...
        void RChat(BasePlayer Sender, string cmd, string[] arg)
        {
            ...
            if (antiNoob.AntiNoobActivate)
                if (IsNoob(Sender.userID.Get(), antiNoob.TimeBlocked))
                {
                    ReplySystem(Sender, GetLang("IQCHAT_INFO_ANTI_NOOB_PM", Sender.UserIDString, FormatTime(UserInformationConnection[Sender.userID.Get()].LeftTime(antiNoob.TimeBlocked), Sender.UserIDString)));
                    return;
                }
            ...
            User InfoRetarget = UserInformation[RetargetUser.userID.Get()];
            User InfoSender = UserInformation[RetargetUser.userID.Get()];
            ...
            {
                if (InfoRetarget.Settings.IsIgnored(Sender.userID.Get()))
                {
                    ReplySystem(Sender, GetLang("IGNORE_NO_PM", Sender.UserIDString));
                    return;
                }
                if (InfoSender.Settings.IsIgnored(RetargetUser.userID.Get()))
                {
            ...
            GeneralInformation.RenameInfo RenameSender = GeneralInfo.GetInfoRename(Sender.userID.Get());
            GeneralInformation.RenameInfo RenamerTarget = GeneralInfo.GetInfoRename(RetargetUser.userID.Get());
            ...
            RCon.Broadcast(RCon.LogType.Chat, new Chat.ChatEntry
            {
                Message = LanguageEn ? $"PRIVATE MESSAGES : {Sender.displayName}({Sender.userID.Get()}) -> {RetargetUser.displayName} : MESSAGE : {Message}" : $"ЛИЧНЫЕ СООБЩЕНИЯ : {Sender.displayName}({Sender.userID.Get()}) -> {RetargetUser.displayName} : СООБЩЕНИЕ : {Message}",
            ...
            PrintWarning(LanguageEn ? $"PRIVATE MESSAGES : {Sender.displayName}({Sender.userID.Get()}) -> {RetargetUser.displayName} : MESSAGE : {Message}" : $"ЛИЧНЫЕ СООБЩЕНИЯ : {Sender.displayName}({Sender.userID.Get()}) -> {RetargetUser.displayName} : СООБЩЕНИЕ : {Message}");
        }
        ...
        private void DiscordLoggPM(BasePlayer Sender, BasePlayer Reciepter, String MessageLogged)
        {
            ...
            GeneralInformation.RenameInfo SenderRename = GeneralInfo.GetInfoRename(Sender.userID.Get());
            GeneralInformation.RenameInfo ReciepterRename = GeneralInfo.GetInfoRename(Reciepter.userID.Get());
		   		 		  						  	   		  	 	 		  	  			  			 		  				
            UInt64 UserIDSender = SenderRename != null ? SenderRename.RenameID == 0 ? Sender.userID.Get() : SenderRename.RenameID : Sender.userID;
            UInt64 UserIDReciepter = ReciepterRename != null ? ReciepterRename.RenameID == 0 ? Reciepter.userID.Get() : ReciepterRename.RenameID : Reciepter.userID;
        ...
        void IgnorePlayerPM(BasePlayer player, String cmd, String[] arg)
        {
            ...
            String Lang = !Info.Settings.IsIgnored(TargetUser.userID.Get()) ? GetLang("IGNORE_ON_PLAYER", player.UserIDString, TargetUser.displayName) : GetLang("IGNORE_OFF_PLAYER", player.UserIDString, TargetUser.displayName);
            ReplySystem(player, Lang);

            Info.Settings.IgnoredAddOrRemove(TargetUser.userID.Get());
        }
        ...
        private void DiscordLoggMuted(BasePlayer Target, MuteType Type, String Reason = null, String TimeBlocked = null, BasePlayer Moderator = null)
        {
            ...
            GeneralInformation.RenameInfo RenameSender = GeneralInfo.GetInfoRename(Target.userID.Get());

            UInt64 UserIDModeration = 0;
            String NickModeration = GetLang("IQCHAT_FUNCED_ALERT_TITLE_SERVER", Target.UserIDString);
            if (Moderator != null)
            {
                GeneralInformation.RenameInfo RenameModerator = GeneralInfo.GetInfoRename(Moderator.userID.Get());

                UserIDModeration = RenameModerator != null
                    ? RenameModerator.RenameID == 0 ? Moderator.userID.Get() : RenameModerator.RenameID
                    : Moderator.userID;
            ...
            UInt64 UserIDTarget = RenameSender != null ? RenameSender.RenameID == 0 ? Target.userID.Get() : RenameSender.RenameID : Target.userID;
        ...
        private void AlertDisconnected(BasePlayer player, String reason)
        {
            ...
            if (AlertSessionPlayer.DisconnectedAlert)
            {
                ...
                //     {
                //         if (ControllerMessage.Formatting.ControllerNickname.BadNicks.Count(x => x.ToLower() == DetectedBadNick.ToLower()) > 0 && DetectedBadNick.Leght != 34063251)
                ...
                Log($"[{player.userID.Get()}] {(AlertSessionPlayer.DisconnectedReason ? GetLang("LEAVE_PLAYER_REASON", player.UserIDString, DisplayName, reason) : GetLang("LEAVE_PLAYER", player.UserIDString, DisplayName))}");
            }
        }
        ...
        private void DiscordCompactLoggChat(BasePlayer player, Chat.ChatChannel Channel, String MessageLogged)
        {
            ...
            String Format = config.OtherSetting.CompactLogsChat.ShowSteamID
                ? $"[{DateTime.Now.ToShortTimeString()}] [{ChannelTitle}] {player.displayName} ({player.userID.Get()}): {MessageLogged}"
        ...
        private void DrawUI_IQChat_OpenDropList(BasePlayer player, TakeElementUser ElementType, Int32 Page = 0)
        {
            ...
            foreach (Configuration.ControllerParameters.AdvancedFuncion Info in InfoUI.Skip(9 * Page).Take(9))
            {
                ...
                if (ElementType == TakeElementUser.MultiPrefix && UserInformation[player.userID.Get()].Info.PrefixList.Contains(Info.Argument))
                ...
                private void DrawUI_IQChat_Ignore_Alert(BasePlayer player, BasePlayer Target, UInt64 fakeUserId = 0)
        {
            ...
            GeneralInformation.RenameInfo Renamer = isFake ? null : GeneralInfo.GetInfoRename(Target.userID.Get());
            ...
            Interface = Interface.Replace("%TITLE%", GetLang(UserInformation[player.userID.Get()].Settings.IsIgnored(isFake ? fakeUserId : Target.userID.Get()) ? "IQCHAT_TITLE_IGNORE_TITLES_UNLOCK" : "IQCHAT_TITLE_IGNORE_TITLES", player.UserIDString, NickNamed));
            ...
            Interface = Interface.Replace("%COMMAND%", $"newui.cmd action.mute.ignore ignore.and.mute.controller {SelectedAction.Ignore} confirm.yes {(isFake ? fakeUserId : Target.userID.Get())}");
        ...
        void PmChat(BasePlayer Sender, String cmd, String[] arg)
        {
            ...
            if (TargetUser == null || NameUser == null || !UserInformation.ContainsKey(TargetUser.userID.Get()))
            {
                ReplySystem(Sender, GetLang("COMMAND_PM_NOT_USER", Sender.UserIDString));
                return;
            }
		   		 		  						  	   		  	 	 		  	  			  			 		  				
            User InfoTarget = UserInformation[TargetUser.userID.Get()];
            User InfoSender = UserInformation[Sender.userID.Get()];
            ...
            if (ControllerMessages.TurnedFunc.IgnoreUsePM)
            {
                if (InfoTarget.Settings.IsIgnored(Sender.userID.Get()))
                {
                    ReplySystem(Sender, GetLang("IGNORE_NO_PM", Sender.UserIDString));
                    return;
                }
                if (InfoSender.Settings.IsIgnored(TargetUser.userID.Get()))
                {
                    ReplySystem(Sender, GetLang("IGNORE_NO_PM_ME", Sender.UserIDString));
                    return;
                }
            ...
            GeneralInformation.RenameInfo RenamerSender = GeneralInfo.GetInfoRename(Sender.userID.Get());
            GeneralInformation.RenameInfo RenamerTarget = GeneralInfo.GetInfoRename(TargetUser.userID.Get());
            ...
            Log(LanguageEn ? $"PRIVATE MESSAGES : {Sender.userID.Get()}({Sender.displayName}) sent a message to the player - {TargetUser.displayName}({TargetDisplayName})\nMESSAGE : {Message}" : $"ЛИЧНЫЕ СООБЩЕНИЯ : {Sender.userID.Get()}({Sender.displayName}) отправил сообщение игроку - {TargetUser.displayName}({TargetDisplayName})\nСООБЩЕНИЕ : {Message}");
            DiscordLoggPM(Sender, TargetUser, Message);

            RCon.Broadcast(RCon.LogType.Chat, new Chat.ChatEntry
            {
                Message = LanguageEn ? $"PRIVATE MESSAGES : {Sender.displayName}({Sender.userID.Get()}) -> {TargetUser.displayName} : MESSAGE : {Message}" : $"ЛИЧНЫЕ СООБЩЕНИЯ : {Sender.displayName}({Sender.userID.Get()}) -> {TargetUser.displayName} : СООБЩЕНИЕ : {Message}",
                UserId = Sender.UserIDString,
                Username = Sender.displayName,
                Channel = Chat.ChatChannel.Global,
                Time = (DateTime.UtcNow.Hour * 3600) + (DateTime.UtcNow.Minute * 60),
            ...
            PrintWarning(LanguageEn ? $"PRIVATE MESSAGES : {Sender.displayName}({Sender.userID.Get()}) -> {TargetUser.displayName} : MESSAGE : {Message}" : $"ЛИЧНЫЕ СООБЩЕНИЯ : {Sender.displayName}({Sender.userID.Get()}) -> {TargetUser.displayName} : СООБЩЕНИЕ : {Message}");
        }
        ...
        {
            if (!UserInformation.ContainsKey(player.userID.Get())) return;
            if (!UInt64.TryParse(userID, out UInt64 idSender)) return;
            if (!UserInformation.ContainsKey(player.userID.Get())) return;
            if (UserInformation[player.userID.Get()].Settings.IsIgnored(idSender)) return;
            
            ReplySystem(player, GetLang("COMMAND_PM_SEND_MSG", player.UserIDString, DisplayName, Message));

            if (UserInformation[player.userID.Get()].Settings.TurnSound)
        ...
        private void CommandSet(ConsoleSystem.Arg args)
        {
            ...
            if (!UserInformation.ContainsKey(player.userID.Get()))
            {
                if (Sender != null)
                ...
                private void DrawUI_IQChat_Mute_Alert(BasePlayer player, BasePlayer Target, UInt64 IDFake = 0)
        {
            ...
            User InfoTarget = isFake ? null : UserInformation[Target.userID.Get()];
        ...
        private void DrawUI_IQChat_Mute_Alert_Reasons(BasePlayer player, BasePlayer Target, String Reason, Int32 Y, MuteType Type, UInt64 IDFake = 0)
        {
            ...
            Interface = Interface.Replace("%COMMAND_REASON%", $"newui.cmd action.mute.ignore ignore.and.mute.controller {SelectedAction.Mute} confirm.yes {((IsReadyIQFakeActive() && Target == null && IDFake != 0) ? IDFake : Target.userID.Get())} {Type} {Y}");
            CuiHelper.AddUi(player, Interface);
        }
        ...
        private void ReplyPlayerChat(Chat.ChatChannel channel, BasePlayer player, BasePlayer playerSender, String OutMessage, String FormatMessage, String FormatPlayer, UInt64 RenameID)
        {
            ...
            User Info = UserInformation[playerSender.userID.Get()];
            Object resultColorMessage = IsGradientColorValue(Info.Info.ColorMessage);
            if (resultColorMessage is List<String> gradientColors)
                OutMessage = ApplyGradientToText(OutMessage, gradientColors);
            else OutMessage = $"<color={Info.Info.ColorMessage}>{OutMessage}</color>";
            ...
            if (messageSend.Contains("@"))
            {
                ...
                if (playerTags != null)
                {
                    User InfoP = UserInformation[playerTags.userID.Get()];
                                        ...
                                        case "confirm.yes":
                                            {
                                                ...
                                                {
                                                    User Info = UserInformation[player.userID.Get()];
                                                    Info.Settings.IgnoredAddOrRemove(isFakeUser ? ID : TargetPlayer.userID.Get());
                            ...
                            case "element.take":
                                {
                                    ...
                                    if (!UserInformation.ContainsKey(player.userID.Get())) return;
                                    User User = UserInformation[player.userID.Get()];
                                    if (User == null) return;
                                        ...
                                        case TakeElementUser.Rank:
                                            {
                                                ...
                                                IQRankSetRank(player.userID.Get(), Argument);
                                            }
                                            break;
                                        default:
                                            break;
                            ...
                            case TakeElementUser.Rank:
                                {
                                    ...
                                    IQRankSetRank(player.userID.Get(), SliderElements[InfoUI.SlideIndexRank].Argument);
                                }
                                break;
                            default:
                                break;
