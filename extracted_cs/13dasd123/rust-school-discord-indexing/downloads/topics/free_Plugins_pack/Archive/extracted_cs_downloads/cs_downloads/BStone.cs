	...
	public class BStone : RustPlugin
	{
        ...
        private void OnDispenserBonus(ResourceDispenser dispenser, BasePlayer player, Item item)
        {
            ...
            if (LuckyChance)
            {
                ...
                // Добавляем описание к предмету
                if (!string.IsNullOrEmpty(config._MainSettings._BolotoStone.Description))
                {
                    player.ChatMessage($"Вы нашли: {config._MainSettings._BolotoStone.Description}");
                }
                ...
                return new PluginConfig()
                {
                    _MainSettings = new MainSettings()
                    {
                        _BolotoStone = new BolotoStone()
                        {
                            ...
                            Description = "Магический камень из болот",
                            SkinID = 9863321,
                        },
                        ItemAmount = 3,
                        Rare = 50
