...
[Info("Friends", "LAGZYA", "4.2.4")]
public class Friends : RustPlugin
{
    #region [DATA&CONFIG]

    private Dictionary<ulong, List<CodeLock>> _cdPlayers = new Dictionary<ulong, List<CodeLock>>();
    private Dictionary<ulong, List<BuildingPrivlidge>> _cbPlayers =
        new Dictionary<ulong, List<BuildingPrivlidge>>();
    ...
    {
        [JsonProperty(
            Eng ? "Enable save during map save?" : "Включить сохранение во время сейва карты?"
        )]
        ...
        [JsonProperty(
            Eng
                ? "Enable turret auto-authorization setting?"
                : "Включить настройку авто авторизации турелей?"
        )]
        public bool Turret;

        [JsonProperty(
            Eng ? "Enable friendly damage setting?" : "Включить настройку урона по своим?"
        )]
        public bool Damage;
        ...
        [JsonProperty(
            Eng
                ...
                : "Урон по человеку(По стандрату у игрока включена?)"
        )]
        public bool SDamage;

        [JsonProperty(
            Eng
                ? "Default turret-auth setting"
                : "Авторизациия в турелях(По стандрату у игрока включена?)"
        )]
        public bool STurret;

        [JsonProperty(
            Eng
                ? "Default door-auth setting"
                : "Авторизациия в дверях(По стандрату у игрока включена?)"
        )]
        public bool SDoor;

        [JsonProperty(
            Eng
                ? "Default air defense setting"
                : "Авторизациия в пво(По стандрату у игрока включена?)"
        )]
        public bool SSam;

        [JsonProperty(
            Eng ? "Default TC auth" : "Авторизациия в шкафу(По стандрату у игрока включена?)"
        )]
        public bool bp;
    ...
    protected override void LoadDefaultMessages()
    {
        ...
        foreach (
            var rus in new Dictionary<string, string>()
            {
                ["SYNTAX"] =
        ...
        var eu = new Dictionary<string, string>()
        {
            ...
            ["LIST2"] = "Friend list",
            ["SYNTAXSET"] =
                "/f(riend) set damage [Name] - Damage per person\n"
            ...
            ["SYNTAXSETALL"] =
                "/f(riend) setall damage 0/1 - Damage on all friends\n"
        ...
        switch (arg[0])
        {
            case "add":
                ...
                if (cfg.fake && IQFakeActive && (bool)IQFakeActive.CallHook("IsFakeUser", name))
                {
                    SendReply(
                        player,
                        $"{PlugName}{lang.GetMessage("SENDADD", this, player.UserIDString)}"
                    );
                    return;
                }
                if (target == null || !friendData.TryGetValue(target.userID, out targetPlayer))
                {
                    SendReply(
                        player,
                        $"{PlugName}{lang.GetMessage("NPLAYER", this, player.UserIDString)}"
                    );
                    return;
                }
                ...
                if (target.userID == player.userID)
                {
                    SendReply(
                        player,
                        $"{PlugName}{lang.GetMessage("CANTADDME", this, player.UserIDString)}"
                    );
                    return;
                }
                ...
                if (playerAccept.ContainsValue(player.userID))
                {
                    SendReply(
                        player,
                        PlugName + lang.GetMessage("ALREADYADD", this, player.UserIDString)
                    );
                    return;
                }
                ...
                if (player1.friendList.Count >= cfg.MaxFriends)
                {
                    SendReply(
                        player,
                        $"{PlugName}{lang.GetMessage("MAXFRIENDYOU", this, player.UserIDString)}"
                    );
                    return;
                }
                ...
                if (player1.friendList.ContainsKey(target.userID))
                {
                    SendReply(
                        player,
                        $"{PlugName}{lang.GetMessage("ONFRIENDS", this, player.UserIDString)}"
                    );
                    return;
                }
                ...
                if (targetPlayer.friendList.Count >= cfg.MaxFriends)
                {
                    SendReply(
                        player,
                        $"{PlugName}{lang.GetMessage("MAXFRIENDSPLAYERS", this, player.UserIDString)}"
                    );
                    return;
                }
                ...
                if (playerAccept.ContainsKey(target.userID))
                {
                    SendReply(
                        player,
                        $"{PlugName}{lang.GetMessage("HAVEINVITE", this, player.UserIDString)}"
                    );
                    return;
                }
                ...
                if (targetPlayer.friendList.ContainsKey(player.userID))
                    return;
                ...
                SendReply(
                    target,
                    $"{PlugName}{lang.GetMessage("YOUHAVEINVITE", this, target.UserIDString)}"
                );
                ...
                timer.Once(
                    ...
                    {
                        if (
                            !playerAccept.ContainsKey(target.userID)
                            || !playerAccept.ContainsValue(player.userID)
                        )
                            return;
                        if (target != null)
                        {
                            ...
                            SendReply(
                                target,
                                $"{PlugName}{lang.GetMessage("TIMELEFT", this, target.UserIDString)}"
                            );
                        }

                        SendReply(
                            player,
                            $"{PlugName}{lang.GetMessage("HETIMELEFT", this, player.UserIDString)}"
                        );
            ...
            case "accept":
                ...
                if (!playerAccept.TryGetValue(player.userID, out ss))
                {
                    SendReply(
                        player,
                        $"{PlugName}{lang.GetMessage("DONTHAVE", this, player.UserIDString)}"
                    );
                    return;
                }
                ...
                SendReply(
                    player,
                    $"{PlugName}{lang.GetMessage("ADDFRIEND", this, player.UserIDString)}"
                );
                playerAccept.Remove(player.userID);
                SendReply(
                    target,
                    $"{PlugName}{lang.GetMessage("ADDFRIEND", this, target.UserIDString)}"
                );
                if (cfg.bp)
                    AuthBuild(target.userID, player.userID);
                if (cfg.SDoor)
                    AuthDoor(target.userID, player.userID);
                if (cfg.bp)
                    AuthBuild(player.userID, target.userID);
                if (cfg.SDoor)
                    AuthDoor(player.userID, target.userID);
            ...
            case "deny":
                ...
                SendReply(
                    player,
                    $"{PlugName}{lang.GetMessage("DENYADD", this, player.UserIDString)}"
                );
                SendReply(
                    target,
                    $"{PlugName}{lang.GetMessage("DENYADD", this, target.UserIDString)}"
                );
                CuiHelper.DestroyUi(player, LayerInvite);
                break;
            case "remove":
                ...
                if (ulong.TryParse(arg[1], out tt)) { }
                    ...
                    tt = player1
                        .friendList.FirstOrDefault(p => p.Value.name.ToLower().Contains(name))
                        .Key;
                ...
                if (!player1.friendList.ContainsKey(tt))
                {
                    SendReply(
                        player,
                        $"{PlugName}{lang.GetMessage("PLAYERDHAVE", this, player.UserIDString)}"
                    );
                    return;
                }
                ...
                SendReply(
                    player,
                    $"{PlugName}{lang.GetMessage("REMOVEFRIEND", this, player.UserIDString)}"
                );
                target = tt.IsSteamId()
                    ? BasePlayer.FindByID(tt)
                    : BasePlayer.Find(arg[1].ToLower());
                if (target != null)
                    SendReply(
                        target,
                        $"{PlugName}{lang.GetMessage("REMOVEFRIEND", this, player.UserIDString)}"
            ...
            case "list":
                if (player1.friendList.Count < 1)
                {
                    SendReply(
                        player,
                        $"{PlugName}{lang.GetMessage("LIST", this, player.UserIDString)}"
                    );
                    return;
                }
                ...
                var friendlist =
                    $"{PlugName}{lang.GetMessage("LIST2", this, player.UserIDString)}\n";
                ...
                switch (arg[1])
                {
                    case "damage":
                        if (!cfg.Damage)
                        {
                            SendReply(
                                player,
                                $"{PlugName}{lang.GetMessage("SETOFF", this, player.UserIDString)}"
                            );
                            return;
                        }
                ...
                switch (arg[1])
                {
                    ...
                    case "damage":
                        ...
                        if (arg[2] == "1")
                        {
                            ...
                            SendReply(
                                player,
                                $"{PlugName}{lang.GetMessage("DAMAGEONALL", this, player.UserIDString)}"
                            );
                        }
                        else
                        {
            ...
            case "team":
                ...
                var text =
                    $"{PlugName}{lang.GetMessage("SENDINVITETEAM", this, player.UserIDString)}";
    ...
    {
        if (arg.Args == null || arg.Args.Length < 1 || arg.Player() == null)
            return;
    ...
    {
        FriendData fData;
        if (!friendData.TryGetValue(entity.OwnerID, out fData))
            return;
        if (_cbPlayers.ContainsKey(entity.OwnerID))
            _cbPlayers[entity.OwnerID].Add(entity);
        foreach (var ids in fData.friendList.Where(p => p.Value.bp == true))
        {
            entity.authorizedPlayers.Add(
                new PlayerNameID()
                {
                    ShouldPool = true,
                    userid = ids.Key,
    ...
    private void OnEntitySpawned(CodeLock entity)
    {
        ...
        if (!friendData.TryGetValue(entity.OwnerID, out fData))
            return;
        if (_cdPlayers.ContainsKey(entity.OwnerID))
            _cdPlayers[entity.OwnerID].Add(entity);
        else
            _cdPlayers.Add(entity.OwnerID, new List<CodeLock>() { entity });
        foreach (var ids in fData.friendList.Where(p => p.Value.Door == true))
        {
            if (!entity.guestPlayers.Contains(ids.Key))
                entity.guestPlayers.Add(ids.Key);
        }
    ...
    private void CanChangeCode(BasePlayer player, CodeLock codeLock, string code, bool isGuest)
    {
        NextFrame(() =>
        {
            ...
            foreach (
                var friend in friendData[player.userID].friendList.Where(p => p.Value.Door == true)
            )
            {
    ...
    private object CanEntityTakeDamage(BaseEntity entity, HitInfo info)
    {
        ...
        if (attackerplayer == null || targetplayer == null)
            return null;
        if (IsOnTournament(attackerplayer.userID))
            return null;
        if (!friendData.TryGetValue(attackerplayer.userID, out player1))
            return null;
        FriendData.FriendAcces ss;
        if (!player1.friendList.TryGetValue(targetplayer.userID, out ss))
            return null;
        if (ss.Damage)
            return null;
        ...
        SendReply(
            attackerplayer,
            string.Format(
                lang.GetMessage("DAMAGE", this, attackerplayer.UserIDString),
    ...
    private object OnTurretTarget(AutoTurret turret, BaseCombatEntity entity)
    {
        ...
        if (targetplayer == null)
            return null;
        if (!friendData.TryGetValue(turret.OwnerID, out targetPlayer))
            return null;
        FriendData.FriendAcces ss;
        var owner = turret.authorizedPlayers.Any(p => p.userid == turret.OwnerID);
        if (!owner)
            return null;
        if (!targetPlayer.friendList.TryGetValue(targetplayer.userID, out ss))
            return null;
        if (!ss.Turret)
            return null;
        return false;
    ...
    private object CanUseLockedEntity(BasePlayer player, BaseLock baseLock)
    {
        ...
        if (baseLock.ShortPrefabName == "lock.key" && !cfg.odinlock)
            return null;
        if (!friendData.TryGetValue(baseLock.OwnerID, out targetPlayer2))
            return null;
        FriendData.FriendAcces ss;
        if (!targetPlayer2.friendList.TryGetValue(player.userID, out ss))
            return null;
        if (!ss.Door)
            return null;
        return true;
    ...
    private object OnSamSiteTarget(SamSite entity, BaseCombatEntity target)
    {
        if (!cfg.SSamOn)
            return null;
        if (cfg.targetPilot && !TargetPilot(entity, target))
            return true;
        ...
        if (targetpcopter != null)
        {
            ...
            if (build == null)
                return null;
            if (!build.authorizedPlayers.Any(p => p.userid == entity.OwnerID))
                return null;
            BasePlayer targePlayer = null;
            if (targetpcopter != null)
                targePlayer = targetpcopter.GetDriver();
            if (targePlayer == null)
                return true;
            if (entity.OwnerID == targePlayer.userID)
                return true;
            if (!friendData.TryGetValue(entity.OwnerID, out targetPlayer))
                return null;
            FriendData.FriendAcces ss;
            if (!targetPlayer.friendList.TryGetValue(targePlayer.userID, out ss))
                return null;
            if (!ss.Sam)
                return null;
        }
    ...
    {
        FriendData t;
        if (friendData.TryGetValue(player.userID, out t))
            return;
        friendData.Add(
            player.userID,
            new FriendData() { Name = player.displayName, friendList = { } }
        );
    }
    ...
    private List<FakePlayer> GetFakePlayerList()
    {
        ...
        if (!jsonData.TryGetValue("players", out JToken playersToken))
            return null;
    ...
    private void OnServerInitialized()
    {
        ...
        permission.RegisterPermission("friends.remove", this);
        ServerConsole.PrintColoured(
            ConsoleColor.Blue,
            (object)$"{Name} [{Version}] ",
            (object)ConsoleColor.Blue,
            (object)"B",
            (object)ConsoleColor.Cyan,
            (object)"Y ",
            (object)ConsoleColor.Green,
            (object)"L",
            (object)ConsoleColor.Magenta,
            (object)"A",
            (object)ConsoleColor.Red,
            (object)"G",
            (object)ConsoleColor.Yellow,
            (object)"Z",
            (object)ConsoleColor.Cyan,
            (object)"Y",
            (object)ConsoleColor.DarkCyan,
            (object)"A"
        );
        ...
        AddImage(
            $"https://rustapi.top/cartinki/givecart.php?token={API_KEY}&image=buttonfriend.png",
            $"button_friend"
        );
        AddImage(
            $"https://rustapi.top/cartinki/givecart.php?token={API_KEY}&image=f-setting.png",
            $"setting_friend"
        );
        AddImage(
            $"https://rustapi.top/cartinki/givecart.php?token={API_KEY}&image=add-group.png",
            $"addfriend_friend"
        );
        AddImage(
            $"https://rustapi.top/cartinki/givecart.php?token={API_KEY}&image=pageright.png",
            $"pageright_friend"
        );
        AddImage(
            $"https://rustapi.top/cartinki/givecart.php?token={API_KEY}&image=pageleft.png",
            $"pageleft_friend"
        );
        AddImage(
            $"https://rustapi.top/cartinki/givecart.php?token={API_KEY}&image=fsetting.png",
            $"settingback_friend"
        );
        AddImage(
            $"https://rustapi.top/cartinki/givecart.php?token={API_KEY}&image=fremove.png",
            $"remove_friend"
        );
        AddImage(
            $"https://rustapi.top/cartinki/givecart.php?token={API_KEY}&image=foff.png",
            $"off_friend"
        );
        AddImage(
            $"https://rustapi.top/cartinki/givecart.php?token={API_KEY}&image=fon.png",
            $"on_friend"
        );

        if (!cfg.serversave)
            Unsubscribe("OnServerSave");
        friendData = Interface.Oxide.DataFileSystem.ReadObject<Dictionary<ulong, FriendData>>(
            "Friends/FriendData"
        );
        ...
        {
            foreach (
                var baseNetworkable in BaseNetworkable.serverEntities.Where(p =>
                    p.ShortPrefabName.Contains("lock.code")
                    || p.ShortPrefabName.Contains("cupboard")
                )
            )
            {
                if (baseNetworkable.ShortPrefabName.Contains("cupboard"))
                {
                    var cb = baseNetworkable as BuildingPrivlidge;
                    if (cb == null || cb.OwnerID == 0 || !cb.OwnerID.IsSteamId())
                        continue;
                    ...
                    {
                        _cbPlayers.Add(cb.OwnerID, new List<BuildingPrivlidge>() { cb });
                    }
                }

                if (baseNetworkable.ShortPrefabName.Contains("lock.code"))
                {
                    var cd = baseNetworkable as CodeLock;
                    if (cd == null || cd.OwnerID == 0 || !cd.OwnerID.IsSteamId())
                        continue;
                    ...
                    {
                        _cdPlayers.Add(cd.OwnerID, new List<CodeLock>() { cd });
                    }
                }
            }
        });
        ...
        cont.Add(
            new CuiElement()
            {
                ...
                Components =
                {
                    ...
                    {
                        Text = String.Format(
                            lang.GetMessage("SENDACCEPTFRIENDS", this, player.UserIDString),
    ...
    void TeamCommand(BasePlayer player, string command, string[] arg)
    {
        ...
        foreach (var keyValuePair in player1.friendList)
        {
            ...
            if (target == null || target.Team != null || !target.IsConnected)
                continue;
            team.SendInvite(target);
        ...
        cont.Add(
            new CuiElement()
            {
                ...
                Components =
                {
                    ...
                    {
                        Text = String.Format(
                            lang.GetMessage("SYSTEMFRIENDS", this, player.UserIDString)
                        ),
                    ...
                    new CuiRectTransformComponent()
                    {
                        AnchorMin = "0.3442708 0.6361111",
                        AnchorMax = "0.41875 0.6549382",
                    },
                },
            }
        );
    ...
    void CheckPlayer(ConsoleSystem.Arg arg)
    {
        ...
        if (arg.Player() == null)
        {
            ServerConsole.PrintColoured(
                ...
                (object)$"{CheckFriends(steamId)}"
            );
            return;
        }

        var admin = arg.Player();
        if (!permission.UserHasPermission(admin.UserIDString, "friends.checkplayer"))
            return;
    ...
    private void SettingInit(BasePlayer player, ulong steamdIdTarget, string b)
    {
        ...
        if (!friendData.TryGetValue(player.userID, out target))
            return;
        if (!target.friendList.TryGetValue(steamdIdTarget, out access))
            return;
        ...
        if (b.ToInt() <= 47)
        {
            cont.Add(
                new CuiElement()
                {
                    ...
                    Components =
                    {
                        new CuiRawImageComponent { Png = GetImage($"settingback_friend") },
                        new CuiRectTransformComponent
                        {
            ...
            new CuiElement()
            {
                ...
                Components =
                {
                    new CuiTextComponent()
                    {
                        ...
                        Font = regular,
                        FontSize = 12,
                        Color = HexToRustFormat("#52eb80"),
                    },
                    new CuiRectTransformComponent()
                    {
                        AnchorMin = "0.02648985 0.8941861",
                        AnchorMax = "0.9420606 0.9694828",
                    },
                },
            }
        );
        ...
        cont.Add(
            new CuiElement()
            {
                ...
                Components =
                {
                    new CuiRawImageComponent { Png = GetImage($"remove_friend") },
                    new CuiRectTransformComponent
                    {
                        AnchorMin = "0.15 0.01144091",
                        AnchorMax = "0.85 0.12",
                    },
                },
            }
        ...
        cont.Add(
            new CuiButton()
            {
                ...
                {
                    Color = "0 0 0 0",
                    Command = $"friendui2 remove {steamdIdTarget}",
                    Close = Layer,
                },
                Text =
                {
                    Text = lang.GetMessage("UIREMOVEFRIENDv2", this, player.UserIDString),
                    Align = TextAnchor.MiddleCenter,
                    FontSize = 10,
                },
            },
        ...
        cont.Add(
            new CuiElement()
            {
                ...
                Components =
                {
                    ...
                    new CuiRectTransformComponent
                    {
                        AnchorMin = "0.02648985 0.7407911",
                        AnchorMax = "0.9699578 0.8606074",
                    },
                },
            }
        );
        if (access.Damage)
        {
            cont.Add(
                new CuiButton()
                {
                    ...
                    {
                        Color = "0 0 0 0",
                        Command = $"friendui2 set damage {steamdIdTarget} {b}",
                    },
                    Text =
                    {
            ...
            cont.Add(
                new CuiElement()
                {
                    ...
                    Components =
                    {
                        new CuiRawImageComponent() { Png = GetImage("on_friend") },
                        new CuiRectTransformComponent()
                        {
        ...
        else
        {
            ...
            cont.Add(
                new CuiElement()
                {
                    ...
                    Components =
                    {
                        new CuiRawImageComponent() { Png = GetImage("off_friend") },
                        new CuiRectTransformComponent()
                        {
                            AnchorMin = "0.7990546 0.1258519",
                            AnchorMax = "0.9492701 0.8832379",
        ...
        cont.Add(
            new CuiElement()
            {
                ...
                Components =
                {
                    ...
                    new CuiRectTransformComponent
                    {
                        AnchorMin = "0.02648985 0.603502",
                        AnchorMax = "0.9699578 0.7233183",
                    },
                },
            }
        );
        if (access.Door)
        {
            cont.Add(
                new CuiButton()
                {
                    ...
                    {
                        Color = "0 0 0 0",
                        Command = $"friendui2 set door {steamdIdTarget} {b}",
                    },
                    Text =
                    {
        ...
        cont.Add(
            new CuiElement()
            {
                ...
                Components =
                {
                    ...
                    new CuiRectTransformComponent
                    {
                        AnchorMin = "0.02648985 0.466213",
                        AnchorMax = "0.9699578 0.5860293",
                    },
                },
            }
        );
        if (access.Turret)
        {
            cont.Add(
                new CuiButton()
                {
                    ...
                    {
                        Color = "0 0 0 0",
                        Command = $"friendui2 set turret {steamdIdTarget} {b}",
                    },
                    Text =
                    {
        ...
        cont.Add(
            new CuiElement()
            {
                ...
                Components =
                {
                    ...
                    new CuiRectTransformComponent
                    {
                        AnchorMin = "0.02648985 0.3289239",
                        AnchorMax = "0.9699578 0.4487402",
                    },
                },
            }
        );
        if (access.bp)
        {
            cont.Add(
                new CuiButton()
                {
                    ...
                    {
                        Color = "0 0 0 0",
                        Command = $"friendui2 set build {steamdIdTarget} {b}",
                    },
                    Text =
                    {
            ...
            cont.Add(
                new CuiElement()
                {
                    ...
                    Components =
                    {
                        ...
                        new CuiRectTransformComponent
                        {
                            AnchorMin = "0.02648985 0.1916349",
                            AnchorMax = "0.9699578 0.3114512",
                        },
                    },
                }
            );
            if (access.Sam)
            {
                cont.Add(
                    new CuiButton()
                    {
                        ...
                        {
                            Color = "0 0 0 0",
                            Command = $"friendui2 set sam {steamdIdTarget} {b}",
                        },
                        Text =
                        {
    ...
    private void RemoveDoor(ulong player, ulong friendId)
    {
        if (_cdPlayers.ContainsKey(player))
        {
            foreach (CodeLock bp in _cdPlayers[player])
            {
                ...
                {
                    if (bp.guestPlayers.Contains(friendId))
                        bp.guestPlayers.Remove(friendId);
                    bp.SendNetworkUpdate();
                }
            }
        }
    ...
    private void AuthBuild(ulong player, ulong friendId)
    {
        ...
        if (_cbPlayers.ContainsKey(player))
        {
            foreach (BuildingPrivlidge bp in _cbPlayers[player])
            {
                ...
                {
                    if (bp.authorizedPlayers.Any(p => p.userid == friendId))
                        continue;
                    bp.authorizedPlayers.Add(
                        new PlayerNameID()
                        {
                            userid = friendId,
    ...
    private void FriendsInit(BasePlayer player, int page, string find = "")
    {
        ...
        cont.Add(
            new CuiButton()
            {
                RectTransform = { AnchorMin = "0.973 0.48", AnchorMax = "0.9979069 0.53" },
                Text =
                {
                    Text = "",
                    Align = TextAnchor.MiddleCenter,
        ...
        cont.Add(
            new CuiButton()
            {
                RectTransform = { AnchorMin = "0.003 0.48", AnchorMax = "0.028 0.53" },
                Text =
                {
                    Text = "",
                    Align = TextAnchor.MiddleCenter,
        ...
        foreach (
            var basePlayer in BasePlayer
                .activePlayerList.OrderBy(s => s.displayName)
                .Where(p =>
                    !playerList.Any(f => f.Key.SteamId == p.userID)
        ...
        foreach (
            var sellItem in playerList
                .Where(p =>
                    find == ""
                    || find != ""
                        && (
                            NormalizeString(p.Key.DisplayName).ToLower().Contains(find.ToLower())
                            || p.Key.SteamId.ToString().Contains(find)
                        )
                )
                .Select((i, t) => new { A = i, B = t - (page - 1) * 30 })
                .Skip((page - 1) * 30)
                .Take(30)
        )
        {
            ...
            cont.Add(
                new CuiElement()
                {
                    ...
                    Components =
                    {
                        new CuiImageComponent() { Color = "1 1 1 0.7" },
                        new CuiRectTransformComponent()
                        {
                ...
                new CuiElement()
                {
                    ...
                    Components =
                    {
                        new CuiTextComponent()
                        {
                            ...
                            Align = TextAnchor.MiddleLeft,
                            Color = sellItem.A.Value
                                ? HexToRustFormat("#52eb80")
                                : "0.8 0.8 0.8 0.86",
                            FontSize = 12,
                        },
                        new CuiRectTransformComponent()
            ...
            if (sellItem.A.Value)
            {
                ...
                cont.Add(
                    new CuiButton()
                    {
                        ...
                        {
                            Color = "0 0 0 0",
                            Command = $"friendui setting {sellItem.A.Key.SteamId} {sellItem.B}",
                        },
                    },
    ...
    private bool HasFriend(string playerId, string friendId)
    {
        ...
        ulong pId = 0;
        ulong fId = 0;
        if (!ulong.TryParse(playerId, out pId) || !ulong.TryParse(friendId, out fId))
            return false;
    ...
    private bool AreFriends(ulong playerId, ulong friendId)
    {
        ...
        if (
            !friendData.TryGetValue(playerId, out playerData)
            || !friendData.TryGetValue(friendId, out playerData2)
        )
            return false;
        return playerData.friendList.ContainsKey(friendId)
            && playerData2.friendList.ContainsKey(playerId);
        ;
    }
    ...
    private bool AddFriend(ulong playerId, ulong friendId)
    {
        ...
        if (
            ...
            return false;
        if (playerData.friendList.ContainsKey(friendId))
            return false;
        playerData.friendList.Add(
            friendId,
            new FriendData.FriendAcces()
            {
                name =
                    BasePlayer.FindByID(friendId) ? BasePlayer.FindByID(friendId).displayName
                    : Eng ? "NOT FOUND"
                    : "НЕИЗВЕСТНЫЙ",
                Damage = cfg.SDamage,
                Door = cfg.SDoor,
        ...
        playerData2.friendList.Add(
            playerId,
            new FriendData.FriendAcces()
            {
                name =
                    BasePlayer.FindByID(playerId) ? BasePlayer.FindByID(playerId).displayName
                    : Eng ? "NOT FOUND"
                    : "НЕИЗВЕСТНЫЙ",
                Damage = cfg.SDamage,
                Door = cfg.SDoor,
    ...
    private List<TakePlayers> FindFriendsData(ulong playerId)
    {
        FriendData playerData;
        if (!friendData.TryGetValue(playerId, out playerData))
            return new List<TakePlayers>();
        var test = new List<TakePlayers>();
        foreach (var friendId in playerData.friendList)
        {
            var t = new TakePlayers() { SteamId = friendId.Key, DisplayName = friendId.Value.name };
            test.Add(t);
        }
    ...
    private ulong[] GetFriendList(string playerId)
    {
        if (string.IsNullOrEmpty(playerId))
            return new ulong[0];
        FriendData playerData;
        if (!friendData.TryGetValue(ulong.Parse(playerId), out playerData))
            return new ulong[0];
    ...
    {
        FriendData friend;
        return friendData.TryGetValue(playerId, out friend)
            ? friend.friendList.Keys.ToArray()
            : new ulong[0];
    }

    #endregion

    [ConsoleCommand("fremove")]
    private void RemoveFriendConsole(ConsoleSystem.Arg arg)
    {
        ...
        {
            SendReply(player, $"{PlugName}Invalid Steam ID format");
            return;
        }
        ...
        if (!player1.friendList.ContainsKey(targetId))
        {
            SendReply(
                player,
                $"{PlugName}{lang.GetMessage("PLAYERDHAVE", this, player.UserIDString)}"
        ...
        FriendData targetPlayer;
        if (!friendData.TryGetValue(targetId, out targetPlayer))
        {
            SendReply(player, $"{PlugName}{lang.GetMessage("NPLAYER", this, player.UserIDString)}");
            return;
        }

        player1.friendList.Remove(targetId);
        targetPlayer.friendList.Remove(player.userID);
        SendReply(
            player,
            $"{PlugName}{lang.GetMessage("REMOVEFRIEND", this, player.UserIDString)}"
        ...
        if (cfg.build)
        {
            RemoveBuild(player.userID, targetId);
            RemoveBuild(targetId, player.userID);
        }

        RemoveDoor(player.userID, targetId);
        RemoveDoor(targetId, player.userID);
    }
}
