...
{
	[Info("BuildCost", "Lincoln & ignignokt84", "1.0.3")]
	...
	class BuildCost : CovalencePlugin
	{
		...
		void CalculateCost(IPlayer player, string command, string[] args)
		{
			...
			if (closestEntity is BaseEntity initialBlock)
			{
				...
				{
					CalculateBuildingBlockCost(buildingBlocks);
					player.Reply("<color=#ffa500>【 Building Block Costs 】</color>");
					player.Reply(aggregator.GetFormattedCost());

					aggregator.Reset();
					CalculateDeployableCost(deployables);
					player.Reply("\n<color=#ffa500>【 Deployable Costs 】</color>");
					player.Reply(aggregator.GetFormattedCost());
				}
			}
		}
		...
		void CalculateDeployableCost(HashSet<BaseEntity> deployables)
		{
			...
			{
				if (!deployableLookup.TryGetValue(deployable.prefabID, out int itemId)) continue;
