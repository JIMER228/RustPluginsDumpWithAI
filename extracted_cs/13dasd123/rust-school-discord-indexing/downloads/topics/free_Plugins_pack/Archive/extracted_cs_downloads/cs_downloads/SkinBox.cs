...
namespace Oxide.Plugins
{
    ...
    class SkinBox : ChaosPlugin
    {
        ...
        private class SprayCanWatcher : MonoBehaviour
        {
            ...
            private const int LAYER_MASK = 1 << (int)Rust.Layer.Reserved1 | 1 << (int) Rust.Layer.Construction | 1 << (int) Rust.Layer.Deployed | 1 << (int)Rust.Layer.Physics_Debris; //20360
            private void Awake()
            {
            ...
            private static IEnumerator ProcessQueue()
            {
                ...
                try
                {
                    while (QueueList.Count > 0)
                    {
                        ...
                        string details = string.Format(ITEMS_BODY, Configuration.Skins.APIKey, queueItem.RequestedIds.Count);

                        for (int i = 0; i < queueItem.RequestedIds.Count; i++)
                            details += string.Format(ITEM_ENTRY, i, queueItem.RequestedIds[i]);

                        using UnityWebRequest www = UnityWebRequest.Post(PUBLISHED_FILE_DETAILS, FormFields);
                        ...
                        if (ValidItems.Count > 0)
                        {
                            ...
                            foreach ((string itemName, string skinName, ulong skinId, string previewUrl) in ValidItems)
                            {
                                discordMessage.WithEmbed(DiscordEmbed.Create()
                                    .WithTitle(skinName)
                                    .WithColor(DiscordColor.Blurple)
                                    .WithUrl($"https://steamcommunity.com/sharedfiles/filedetails/?id={skinId}")
                                    .WithDescription($"**Requester Name**: *{queueItem.RequesterName}*\r**Requester ID**: *{queueItem.RequesterId}*\r**Item**: *{itemName}*\r```skinbox.addskin {skinId}```")
