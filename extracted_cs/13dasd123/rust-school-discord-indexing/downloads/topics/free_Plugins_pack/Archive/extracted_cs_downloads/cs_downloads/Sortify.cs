...
{
    [Info("Sortify", "0xF [dsc.gg/0xf-plugins]", "1.0.1")]
    partial class Sortify : RustPlugin
    {
        ...
        public static class HarmonyClass
        {
            ...
            {
                Name = name;
                Instance = new Harmony(name);
            }

            public static void Patch()
            {
                Instance.Patch(AccessTools.Method(typeof(BaseEntity), "ClientRPCSend"), new HarmonyMethod(typeof(Patches), "ClientRPCSend"));
                Instance.Patch(AccessTools.Method(typeof(Item), "MoveToContainer"), new HarmonyMethod(typeof(Patches), "MoveToContainer"));
                //Instance.Patch(typeof(BaseEntity).GetMethods().First(m => m.IsGenericMethod && m.Name == "ClientRPCEx" && m.GetParameters().Length == 4).MakeGenericMethod(typeof(PlayerUpdateLoot)), transpiler: new HarmonyMethod(typeof(Patches), "ClientRPCEx"));
            }
