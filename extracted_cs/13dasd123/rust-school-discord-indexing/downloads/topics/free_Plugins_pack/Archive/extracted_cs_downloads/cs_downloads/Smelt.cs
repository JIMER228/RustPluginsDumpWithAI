...
namespace Oxide.Plugins
{
    ...
    class Smelt : RustPlugin
    {
        ...
        private class PluginConfig
        {
            ...
            public static PluginConfig DefaultConfig()
            {
                return new PluginConfig()
                {
                    ...
                    {
                        {"M.COOLDOWN", "<color=#8888ff>[CLASH RUST]</color>  Переплавка инвентаря находится на перезарядке подождите:<color=#ff8888> {0} </color> сек."},
                        {"M.PERM", "<color=#8888ff>[CLASH RUST]</color> У вас нет разрешения на использование <color=#ff8888>smelt</color>." },
                        {"M.INV", "<color=#8888ff>[CLASH RUST]</color> Твой инвентарь переплавлен <color=#ff8888>успешно</color>." }
                    }
                };
            }
        }
        ...
        private void SmeltCmd(BasePlayer player, string command, string[] args)
        {
            ...
            if (cooldowns.ContainsKey(player.userID) && cooldowns[player.userID] > (int)UnityEngine.Time.realtimeSinceStartup)
            {
                SendReply(player, config.messages["M.COOLDOWN"], cooldowns[player.userID] - (int)UnityEngine.Time.realtimeSinceStartup);
                return;
            }

            var mainItems = player.inventory.containerMain?.itemList;
