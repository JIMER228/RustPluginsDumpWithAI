...
{
    [Info("UpdateChecker", "tofurahie", "4.2.10")]
    ...
    {  
        ...
        private void console_uc_check_config(ConsoleSystem.Arg arg)
        {
            ...
            foreach (var check in Directory.GetFiles(Interface.Oxide.ConfigDirectory, "*.json").Select(Path.GetFileNameWithoutExtension))
                if (_config.ListOfPlugins.All(x => !string.Equals(x.Name, check, StringComparison.CurrentCultureIgnoreCase)) && !ConfigIgnoreList.Contains(check, StringComparer.CurrentCultureIgnoreCase))
                    configsNames += check + ", ";
        ...
        private void console_uc_clean_config(ConsoleSystem.Arg arg)
        {
            ...
            foreach (var check in Directory.GetFiles(Interface.Oxide.ConfigDirectory, "*.json"))
                if (_config.ListOfPlugins.All(x => !string.Equals(x.Name, Path.GetFileNameWithoutExtension(check), StringComparison.CurrentCultureIgnoreCase)) && !ConfigIgnoreList.Contains(Path.GetFileNameWithoutExtension(check), StringComparer.CurrentCultureIgnoreCase))
                {
        ...
        private void cmdDiscordTest(BasePlayer player, string command, string[] args)
        {
            ...
            var updateList = _config.AutoSearch.ToDictionary(check => check.Key, check => (new List<string>(), new List<string> { "Your discord webhook is working" }));
        ...
        private void cmdConsoleUI_UC(ConsoleSystem.Arg arg)
        {
            ...
            switch (arg.GetString(0)) 
            {
                ...
                case "PAGE":
                    ...
                    break;
                case "CHECK":
                    ShowUICurrentPluginInfo(player, _config.ListOfPlugins.FirstOrDefault(x => string.Equals(x.Name, string.Join(" ", arg.Args.Skip(1)), StringComparison.CurrentCultureIgnoreCase)));
                    break;       
                case "CHANGEURL":
                    ...
                    break;      
                case "CHANGEIGNORE":
                    _config.ListOfPlugins.FirstOrDefault(x => string.Equals(x.Name, arg.GetString(1))).Ignore = !_config.ListOfPlugins.FirstOrDefault(x => string.Equals(x.Name, arg.GetString(1))).Ignore;
                    ShowUICurrentPluginInfo(player, _config.ListOfPlugins.FirstOrDefault(x => string.Equals(x.Name, string.Join(" ", arg.Args.Skip(1)), StringComparison.CurrentCultureIgnoreCase)));
                    SaveConfig(); 
                    break;
            }
        ...
        private IEnumerator SendMessageDiscord(Dictionary<string, (List<string>, List<string>)> infoList)
        {
            ...
            var message = new
            {
                ...
                embeds = new[]
                {
                    new
                    {
                        ...
                        color = updateList.Contains("All plugins have the latest version") ? 65280 : int.Parse(_config.EmbedLineColor.Substring(1), System.Globalization.NumberStyles.HexNumber),
                    }
                }
            };
            ...
            {
                using var request = string.IsNullOrEmpty(_config.MessageID) ? new UnityWebRequest(_config.DiscordWebHook, "POST") : new UnityWebRequest($"{_config.DiscordWebHook}/messages/{_config.MessageID}", "PATCH");
                ...
                if (request.result != UnityWebRequest.Result.Success)
                    PrintError(request.responseCode == 404 ? "Failed to find the message with the specified ID in the config, please check it and try again." : "The discord hook is borken, please check it and try again.");
        ...
        private List<AboutPlugin> GetAllPlugins() 
        {
            ...
            foreach (var fileSystemInfo in new DirectoryInfo(Interface.Oxide.PluginDirectory).GetFiles("*" + "cs").Where(f => (f.Attributes & FileAttributes.Hidden) != FileAttributes.Hidden))
                if (allFoundedPlugins.All(x => x.Name != Path.GetFileNameWithoutExtension(fileSystemInfo.Name)))
                {
                    ...
                    var indexOfRustPlugins = readAllText.IndexOf(":RustPlugin") == -1 ? readAllText.IndexOf(":CarbonPlugin") == -1 ? readAllText.IndexOf(":CovalencePlugin") : readAllText.IndexOf(":CarbonPlugin") : readAllText.IndexOf(":RustPlugin");
        ...
        private IEnumerator CheckNewVersion()
        {
            ...
            updateList["uMod"] = (new List<string> {$"{DateTime.UtcNow.AddMinutes(_config.UTC).ToString(_config.TimeFormat ? "d MMM yyyy HH:mm" : "d MMM yyyy hh:mm tt")}"}, new List<string> {$"### {DateTime.UtcNow.AddMinutes(_config.UTC).ToString(_config.TimeFormat ? "d MMM yyyy HH:mm" : "d MMM yyyy hh:mm tt")}"});
            ...
            for (var index = 0; index < _config.ListOfPlugins.Count; index++)
            {
                ...
                var request = UnityWebRequest.Get($"https://serverarmour.com/api/v1/marketplace/search?plugin={(string.IsNullOrEmpty(check.Url) ? check.Name : check.Url)}");
                ...
                var foundedPlugins = JsonConvert.DeserializeObject<List<PluginData>>(request.downloadHandler.text);
                if (foundedPlugins.Count == 0)
                {  
        ...
        private void ShowUICurrentPluginInfo(BasePlayer player, PluginInfo pluginInfo)
        {
            ...
            UI.Input(ref container, ".current.plugin.name", ".current.plugin.name.input", ".current.plugin.name.input", oMin:"5 0", oMax:"-5 0", text:$"{pluginInfo.Name}", fontSize:18, color:"1 1 1 1", align:TextAnchor.MiddleLeft, readOnly:true); 
            
            UI.Input(ref container, ".current.plugin.author", ".current.plugin.author.input", ".current.plugin.author.input", oMin:"5 0", oMax:"-5 0", text:$"{pluginInfo.Author}", fontSize:18, color:"1 1 1 1", align:TextAnchor.MiddleLeft,  readOnly:true); 
            
            UI.Input(ref container, ".current.plugin.version", ".current.plugin.version.input", ".current.plugin.version.input", oMin:"5 0", oMax:"-5 0", text:$"{pluginInfo.Version}", fontSize:18, color:"1 1 1 1", align:TextAnchor.MiddleLeft,  readOnly:true); 
            
            UI.Input(ref container, ".current.plugin.url", ".current.plugin.url.input", ".current.plugin.url.input", oMin:"5 0", oMax:"-5 0", text:$"{pluginInfo.Url}", fontSize:18, color:"1 1 1 1", align:TextAnchor.MiddleLeft,  command:$"UI_UC CHANGEURL {pluginInfo.Name}", limit:200); 
            
            UI.Input(ref container, ".current.plugin.marketplace", ".current.plugin.marketplace.input", ".current.plugin.marketplace.input", oMin:"5 0", oMax:"-5 0", text:$"{pluginInfo.Marketplace}", fontSize:18, color:"1 1 1 1", align:TextAnchor.MiddleLeft,  readOnly:true); 
            
            UI.Button(ref container, ".current.plugin.bg", ".current.plugin.ignore", ".current.plugin.ignore", aMin:"0 1", aMax:"1 1", oMin:"5 -175", oMax:"-5 -155",command:$"UI_UC CHANGEIGNORE {pluginInfo.Name}", text:$"Ignore the plugin <color={(pluginInfo.Ignore ? "green" : "red")}>{pluginInfo.Ignore}</color>",align:TextAnchor.MiddleLeft);
        ...
        private void ShowUIPlugins(BasePlayer player, string search = "", int page = 0)
        {
            ...
            var searchSort = (!string.IsNullOrEmpty(search) ? _config.ListOfPlugins.Where(x => x.Name.ToLower().Contains(search.ToLower()) || x.Author.ToLower().Contains(search.ToLower())) : _config.ListOfPlugins).OrderByDescending(x => x.IsFounded).ThenByDescending(x => x.Ignore);
            
            var posX = 8;
            var posY = -70;
            foreach (var check in searchSort.Skip(page * 63).Take(63))
            {
                UI.Button(ref container, ".plugins", ".plugin" + posX + posY, ".plugin" + posX + posY, "0 1", "0 1", $"{posX} {posY}", $"{posX + 108} {posY + 45}", $"<color=#{(check.Ignore ? "FFA500" : !check.IsFounded ? "FF0000" : "06c258")}>{check.Name}</color>\n<size=8>[{check.Author}]</size>", 12, command:$"UI_UC CHECK {check.Name}");
            ...
            if (page > 0)
                UI.Button(ref container, ".plugins", ".previous", ".previous",  "0.5 0", "0.5 0", "-25 5", "-5 25", "<", command:$"UI_UC PAGE {page - 1} {search}");
        ...
        private void ShowUIBG(BasePlayer player)
        {
            ...
            UI.Button(ref container, ".bg", ".bg",  oMin:"-640 -360", oMax:"640 360", material:"assets/content/ui/uibackgroundblur-ingamemenu.mat", bgColor:"0.13 0.13 0.13 0.85");
            
            UI.Panel(ref container, ".bg", ".plugins.bg", oMin:"-400 -325", oMax:"400 125", material:"assets/content/ui/binocular_overlay.mat", bgColor:"0.25 0.25 0.25 0.95");
            
            
            UI.Panel(ref container, ".bg", ".search.bg", oMin:"-250 105", oMax:"250 145", material:"assets/content/ui/binocular_overlay.mat", bgColor:"0.15 0.15 0.15 0.95");
            
            UI.Label(ref container, ".search.bg", oMin:"10 0", text:"Search:", color:"0.85 0.85 0.85 0.95", align:TextAnchor.MiddleLeft);
            
            UI.Panel(ref container, ".search.bg", ".search.input", oMin:"65 5", oMax:"-10 -5", material:"assets/content/ui/uibackgroundblur-notice.mat", bgColor:"0.75 0.75 0.75 0.6");
            
            UI.Input(ref container, ".search.input", oMin:"5 0", oMax:"-5 0", text:"", fontSize:18, color:"1 1 1 1", autoFocus:true, command:"UI_UC SEARCH"); 
            
            
            UI.Panel(ref container, ".bg", ".current.plugin.bg", oMin:"-250 160", oMax:"250 339", material:"assets/content/ui/binocular_overlay.mat", bgColor:"0.25 0.25 0.25 0.95");
            
            UI.Label(ref container, ".current.plugin.bg", aMin:"0 1", aMax:"1 1", oMin:"5 -30", oMax:"0 -5", text:"Name:",align:TextAnchor.MiddleLeft);
            
            UI.Panel(ref container, ".current.plugin.bg", ".current.plugin.name", aMin:"0 1", aMax:"1 1", oMin:"95 -30", oMax:"-5 -5", bgColor:"0.85 0.85 0.85 0.25");
            
            UI.Label(ref container, ".current.plugin.bg", aMin:"0 1", aMax:"1 1", oMin:"5 -60", oMax:"0 -35", text:"Author:",align:TextAnchor.MiddleLeft);
            
            UI.Panel(ref container, ".current.plugin.bg", ".current.plugin.author", aMin:"0 1", aMax:"1 1", oMin:"95 -60", oMax:"-5 -35", bgColor:"0.85 0.85 0.85 0.25");  
            
            UI.Label(ref container, ".current.plugin.bg", aMin:"0 1", aMax:"1 1", oMin:"5 -90", oMax:"0 -65", text:"Version:",align:TextAnchor.MiddleLeft);
            
            UI.Panel(ref container, ".current.plugin.bg", ".current.plugin.version", aMin:"0 1", aMax:"1 1", oMin:"95 -90", oMax:"-5 -65", bgColor:"0.85 0.85 0.85 0.25");   
            
            UI.Label(ref container, ".current.plugin.bg", aMin:"0 1", aMax:"1 1", oMin:"5 -120", oMax:"0 -95", text:"URL*:",align:TextAnchor.MiddleLeft);
            
            UI.Panel(ref container, ".current.plugin.bg", ".current.plugin.url", aMin:"0 1", aMax:"1 1", oMin:"95 -120", oMax:"-5 -95", bgColor:"0.85 0.85 0.85 0.65");    
            
            UI.Label(ref container, ".current.plugin.bg", aMin:"0 1", aMax:"1 1", oMin:"5 -150", oMax:"0 -125", text:"Marketplace:",align:TextAnchor.MiddleLeft);
            
            UI.Panel(ref container, ".current.plugin.bg", ".current.plugin.marketplace", aMin:"0 1", aMax:"1 1", oMin:"95 -150", oMax:"-5 -125", bgColor:"0.85 0.85 0.85 0.25");   
        ...
        private class UI
        {
            ...
            public static void Panel(ref CuiElementContainer container, string parent, string name = null, string destroy = null, string aMin = "0 0", string aMax = "1 1", string oMin = "0 0", string oMax = "0 0", string bgColor = "0.33 0.33 0.33 1", string material = null, string sprite = null, int itemID = 0, ulong skinID = 0) =>
            ...
            public static void Icon(ref CuiElementContainer container, string parent, string name = null, string destroy = null, string aMin = "0 0", string aMax = "1 1", string oMin = "0 0", string oMax = "0 0", int itemID = 0, ulong skinID = 0) =>
                container.Add(new CuiElement
                {
                    Parent = Layer + parent,
                    Name = Layer + name,
            ...
            public static void Outline(ref CuiElementContainer container, string layer, string size = "1 1 1 1", string color = "0 0 0 1", bool external = false)
            {
                var borders = size.Split(' ');

                if (borders[0] != "0")
                    Panel(ref container, layer, aMin: "0 1", aMax: "1 1", oMin: $"-{borders[0]} {(external ? "0" : "-" + borders[0])}", oMax: $"{borders[0]} {(external ? borders[0] : "0")}", bgColor: color);
                if (borders[1] != "0")
                    Panel(ref container, layer, aMin: "1 0", aMax: "1 1", oMin: $"{(external ? "0" : "-" + borders[1])} -{borders[1]}", oMax: $"{(external ? borders[1] : "0")} {borders[1]}", bgColor: color);
                if (borders[2] != "0")
                    Panel(ref container, layer, aMin: "0 0", aMax: "1 0", oMin: $"-{borders[2]} {(external ? "-" + borders[2] : "0")}", oMax: $"{borders[2]} {(external ? "0" : borders[2])}", bgColor: color);
                if (borders[3] != "0")
                    Panel(ref container, layer, aMin: "0 0", aMax: "0 1", oMin: $"{(external ? "-" + borders[3] : "0")} -{borders[3]}", oMax: $"{(external ? "0" : borders[3])} {borders[3]}", bgColor: color);
            }
