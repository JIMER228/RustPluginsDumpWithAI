 ...
 * Fix for replies not sent to eligible players if trigger owner was not eligible
 * Fix for broadcast to all replies not always showing same message for all players
 ...
 * Starts fetching country earlier + webrequest exception handling
 * Get grid using MapHelper.PositionToString
 * Code cleanup
*/

namespace Oxide.Plugins
{
    [Info("AutomatedMessages", "beee", "2.2.1")]
    ...
    class AutomatedMessages : RustPlugin
    {
        ...
        private void InitTimers()
        {
            ...
            foreach (var action in _config.Actions.FindAll(s => s.IsEnabled() && s.Type == "Timed"))
            {
                ...
                var repeatTimer = _plugin.timer.Repeat(action.Interval * 60, 0, () => ProcessReply(action, defaultLangAction: defaultLangAction));
        ...
        private void ProcessReply(PluginConfig.ConfigAction action, Lang_Action defaultLangAction, BasePlayer triggerOwner = null, bool onlySendToTeam = false)
        {
            ...
            foreach (var player in BasePlayer.activePlayerList)
            {
                ...
                if (!cachedLangsReplies.TryGetValue(playerLanguage, out string reply))
                {
                    ...
                    if (!Lang_TryGetAction(player, action.Type, action.Id, playerLanguage, out Lang_Action playerlangAction) || playerlangAction.Replies.Count == 0)
                    {
                        ...
                        if (cachedLangsReplies.TryGetValue(_config.DefaultLang, out reply))
                            retrievedCached = true;
                        else
                        {
                            playerLanguage = _config.DefaultLang;
        ...
        private ulong GetActionIcon(PluginConfig.ConfigAction action, BasePlayer triggerOwner, string defaultIconSteamId)
        {
            ulong iconSteamId;
        ...
        private string DefineVariables(BasePlayer player, string message, TriggerDefinition triggerDefinition)
        {
            ...
            {
                var found = pData.Players.TryGetValue(player.userID, out var pinfo);
            ...
            if (message.Contains("{wipetimeremaining}") && WipeTimer.serverinstance != null)
            {
                ...
                nowTime = nowTime.AddDays(WipeTimer.daysToAddTest);
                nowTime = nowTime.AddHours(WipeTimer.hoursToAddTest);
            ...
            {
                case "CrateHacked":
                    if (pData.Players.TryGetValue(player.userID, out var pinfo) && !string.IsNullOrEmpty(pinfo.HackLocation))
                        builder.Replace("{hacklocation}", pinfo.HackLocation);
                    else
                        builder.Replace("{hacklocation}", MapHelper.PositionToString(player.transform.position));
                    break;
            }
        ...
        private void CanHackCrate(BasePlayer player, HackableLockedCrate crate)
        {
            ...
            string Location = MapHelper.PositionToString(crate.transform.position);
            ...
            if (pData.Players.TryGetValue(player.userID, out var pinfo))
                ...
                pData.Players.Add(player.userID, new PlayerInfo { TipsActive = true, HackLocation = Location });
        ...
        private void TipsToggleChatCMD(IPlayer user, string command, string[] args)
        {
            ...
            var isActive = pData.ToggleTipsActive(player.userID);
        ...
        private void InitTriggerDefinitions()
        {
            ...
            TriggerDefinitions.Add("ChatCommand", new() { Key = "ChatCommand", Hooks = "OnPlayerChat", RequiresTarget = true, UsesGenericCooldown = true });
            TriggerDefinitions.Add("AutoReply", new() { Key = "AutoReply", Hooks = "OnPlayerChat", RequiresTarget = true, UsesGenericCooldown = true });
            TriggerDefinitions.Add("NewPlayerJoined", new() { Key = "NewPlayerJoined", Hooks = "OnPlayerSleepEnded", RequiresTarget = false });
            TriggerDefinitions.Add("PlayerConnected", new() { Key = "PlayerConnected", Hooks = "OnPlayerSleepEnded", RequiresTarget = false });
            TriggerDefinitions.Add("PlayerDisconnected", new() { Key = "PlayerDisconnected", Hooks = "OnPlayerDisconnected", RequiresTarget = false });
            TriggerDefinitions.Add("PermissionGranted", new() { Key = "PermissionGranted", Hooks = "OnUserPermissionGranted", RequiresTarget = true });
            TriggerDefinitions.Add("PermissionRevoked", new() { Key = "PermissionRevoked", Hooks = "OnUserPermissionRevoked", RequiresTarget = true });
            TriggerDefinitions.Add("AddedToGroup", new() { Key = "AddedToGroup", Hooks = "OnUserGroupAdded", RequiresTarget = true });
            TriggerDefinitions.Add("RemovedFromGroup", new() { Key = "RemovedFromGroup", Hooks = "OnUserGroupRemoved", RequiresTarget = true });
            TriggerDefinitions.Add("PlayerDead", new() { Key = "PlayerDead", Hooks = "OnEntityDeath,OnPlayerSleepEnded", RequiresTarget = false });
            TriggerDefinitions.Add("CrateHacked", new() { Key = "CrateHacked", Hooks = "CanHackCrate", RequiresTarget = false, Variables = new() {"{hacklocation}"}});
            TriggerDefinitions.Add("EnteredZone", new() { Key = "EnteredZone", Hooks = "OnEnterZone", RequiresTarget = true, UsesPlayerCooldown = true });
            TriggerDefinitions.Add("LeftZone", new() { Key = "LeftZone", Hooks = "OnExitZone", RequiresTarget = true, UsesPlayerCooldown = true });
            TriggerDefinitions.Add("EnteredMonument", new() { Key = "EnteredMonument", Hooks = "OnPlayerEnteredMonument", RequiresTarget = true, UsesPlayerCooldown = true });
            TriggerDefinitions.Add("LeftMonument", new() { Key = "LeftMonument", Hooks = "OnPlayerExitedMonument", RequiresTarget = true, UsesPlayerCooldown = true });
        }
        ...
        private class PluginConfig
        {
            ...
            public List<ConfigAction> _ObsMessages;
            ...
            internal class ConfigAction
            {
                ...
                public List<string> _ObsMessages;

                [JsonProperty("Replies")]
                public List<string> _ObsReplies;
        ...
        private void CheckForConfigUpdates()
        {
            ...
            if(string.IsNullOrEmpty(_config.DefaultLang) || !AvailableLangs.ContainsKey(_config.DefaultLang))
            {
                ...
                if (!string.IsNullOrEmpty(_config.DefaultLang) && CorrectedLangNames.TryGetValue(_config.DefaultLang, out string changedTo))
                {
                    PrintWarning($"Config default language {_config.DefaultLang} has been changed to '{changedTo}'");
                    _config.DefaultLang = changedTo;
                }
                else
                    _config.DefaultLang = GetOxideLanguage();
                changes = true;
            }
            ...
            else
            {
                ...
                {
                    _config.ServerLangs = new() {GetOxideLanguage()};
                    changes = true;
                }
            
                List<string> langsRemoved = new();
                for (int i = _config.ServerLangs.Count - 1; i >= 0; i--)
                {
                    if (!AvailableLangs.ContainsKey(_config.ServerLangs[i]))
                    {
                        if (CorrectedLangNames.TryGetValue(_config.ServerLangs[i], out string changedTo))
                        {
                            PrintWarning($"Config reply language {_config.ServerLangs[i]} has been changed to '{changedTo}'");
                            _config.ServerLangs[i] = changedTo;
                            changes = true;
                        }
                        else
                        {
                            langsRemoved.Add(_config.ServerLangs[i]);
                            _config.ServerLangs.RemoveAt(i);
                            changes = true;
                        }
                    }
                }
        ...
        private void SaveData() => pData?.Save();
        ...
        public class PlayersData
        {
            ...
            {
                if (!Players.TryGetValue(playerid, out var playerInfo))
                    Players.Add(playerid, new PlayerInfo { TipsActive = active, isNew = true });
                else
            ...
            public bool IsActive(ulong playerid) => !Players.TryGetValue(playerid, out var playerInfo) || playerInfo.TipsActive;
        }
        ...
        private static readonly Dictionary<string, MonumentRef> MonumentsRadius = new Dictionary<string, MonumentRef>()
        {
        ...
        private void UpdateConfigFromUISession(UISession uiSession)
        {
            ...
            _config.AutoReplyCooldown = uiSession.Settings.AutoReplyCooldown;
            _config.ChatCommandCooldown = uiSession.Settings.ChatCommandCooldown;
            _config.ZoneManagerCooldown = uiSession.Settings.ZoneManagerCooldown;
            _config.MonumentWatcherCooldown = uiSession.Settings.MonumentWatcherCooldown;
        ...
        private void UI_ShowEditor(BasePlayer player)
        {
            if (!UISessions.TryGetValue(player.userID, out var session))
            {
                ...
                session.Settings.AutoReplyCooldown = _config.AutoReplyCooldown;
                session.Settings.ChatCommandCooldown = _config.ChatCommandCooldown;
                session.Settings.ZoneManagerCooldown = _config.ZoneManagerCooldown;
                session.Settings.MonumentWatcherCooldown = _config.MonumentWatcherCooldown;
                session.Settings.DefaultLang = _config.DefaultLang;
                session.Settings.ServerLangs = new(_config.ServerLangs);
        ...
        private void UI_ShowSettingsView(BasePlayer player, UISession session = null, int panelWidth = 1000, int panelHeight = 500, int panelInnerMargin = 30)
        {
            ...
            UI_TextBox(
                session.Settings.AutoReplyCooldown.ToString(), lang.GetMessage($"UI_Edit_Settings_AutoReplyCooldown", this, player.UserIDString), $"{PREFIX_SHORT}.ui.texteditor Settings SettingsTextbox {nameof(PluginConfig.AutoReplyCooldown)}", scrollContent, $"{editPanel}.scrollviewer",
                new CuiRectTransformComponent { AnchorMin = "0 1", AnchorMax = "0.49 1", OffsetMin = $"{formPanelInnerMargin} -{yStart + 30}", OffsetMax = $"-{formPanelInnerMargin - 5} -{yStart}" }
            );
            //yStart += 30 + fieldsGap;
            ...
            UI_TextBox(
                session.Settings.ChatCommandCooldown.ToString(), lang.GetMessage($"UI_Edit_Settings_ChatCommandCooldown", this, player.UserIDString), $"{PREFIX_SHORT}.ui.texteditor Settings SettingsTextbox {nameof(PluginConfig.ChatCommandCooldown)}", scrollContent, $"{editPanel}.scrollviewer",
                new CuiRectTransformComponent { AnchorMin = "0.51 1", AnchorMax = "1 1", OffsetMin = $"0 -{yStart + 30}", OffsetMax = $"-{formPanelInnerMargin - 5} -{yStart}" }
            );
            yStart += 30 + fieldsGap;
            ...
            UI_TextBox(
                session.Settings.ZoneManagerCooldown.ToString(), lang.GetMessage($"UI_Edit_Settings_ZoneManagerCooldown", this, player.UserIDString), $"{PREFIX_SHORT}.ui.texteditor Settings SettingsTextbox {nameof(PluginConfig.ZoneManagerCooldown)}", scrollContent, $"{editPanel}.scrollviewer",
                new CuiRectTransformComponent { AnchorMin = "0 1", AnchorMax = "0.49 1", OffsetMin = $"{formPanelInnerMargin} -{yStart + 30}", OffsetMax = $"-{formPanelInnerMargin - 5} -{yStart}" }
            );
            //yStart += 30 + fieldsGap;
            ...
            UI_TextBox(
                session.Settings.MonumentWatcherCooldown.ToString(), lang.GetMessage($"UI_Edit_Settings_MonumentWatcherCooldown", this, player.UserIDString), $"{PREFIX_SHORT}.ui.texteditor Settings SettingsTextbox {nameof(PluginConfig.MonumentWatcherCooldown)}", scrollContent, $"{editPanel}.scrollviewer",
                new CuiRectTransformComponent { AnchorMin = "0.51 1", AnchorMax = "1 1", OffsetMin = $"0 -{yStart + 30}", OffsetMax = $"-{formPanelInnerMargin - 5} -{yStart}" }
            );
            yStart += 30 + fieldsGap;
        ...
        private void UI_UpdateSelectedActionInList(BasePlayer player, UITrigger uiTrigger, int actionIndex)
        {
            ...
            if (uiTrigger.TriggerDefinition.RequiresTarget && !string.IsNullOrEmpty(uiTrigger.ConfigActions[actionIndex].Target))
        ...
        private void UI_ShowSelectedAction(BasePlayer player, UITrigger uiTrigger, int actionIndex, string selectedRepliesLang, int titleHeight = 30, int panelInnerMargin = 30)
        {
            ...
            //Checkbox
            UI_CheckBox(
                uiTrigger.ConfigActions[actionIndex].Enabled, nameof(PluginConfig.ConfigAction.Enabled), lang.GetMessage("UI_Edit_Action_Active", this, player.UserIDString), $"{PREFIX_SHORT}.ui.cmd ActionCB {uiTrigger.TriggerDefinition.Key} {actionIndex}", scrollContent, $"{editPanel}.scrollviewer",
                new CuiRectTransformComponent { AnchorMin = "0 1", AnchorMax = "1 1", OffsetMin = $"{formPanelInnerMargin} -{yStart + 20}", OffsetMax = $"-{formPanelInnerMargin - 5} -{yStart}" },
            ...
            //Replies
            UI_Listbox(
                player, "Action", nameof(Lang_Action.Replies), uiTrigger.LangsTriggers[selectedRepliesLang].Actions[configAction.Id].Replies, lang.GetMessage("UI_Edit_Action_Replies", this, player.UserIDString).ToUpper(), $"{PREFIX_SHORT}.ui.cmd %cmdname% {uiTrigger.TriggerDefinition.Key} {actionIndex} %index%", scrollContent, $"{editPanel}.scrollviewer",
            ...
            if (uiTrigger.TriggerDefinition.Key == "Timed")
            {
                //Textbox
                UI_TextBox(
                    uiTrigger.ConfigActions[actionIndex].Interval.ToString(), $"Interval in minutes", $"{PREFIX_SHORT}.ui.texteditor Action ActionTextbox {uiTrigger.TriggerDefinition.Key} {actionIndex} {nameof(PluginConfig.ConfigAction.Interval)}", scrollContent, $"{editPanel}.scrollviewer",
            ...
            if (uiTrigger.TriggerDefinition.RequiresTarget)
            {
                //Textbox
                UI_TextBox(
                    uiTrigger.ConfigActions[actionIndex].Target, lang.GetMessage($"UI_Edit_Action_Target_{uiTrigger.TriggerDefinition.Key}", this, player.UserIDString), $"{PREFIX_SHORT}.ui.texteditor Action ActionTextbox {uiTrigger.TriggerDefinition.Key} {actionIndex} {nameof(PluginConfig.ConfigAction.Target)}", scrollContent, $"{editPanel}.scrollviewer",
                    new CuiRectTransformComponent { AnchorMin = "0 1", AnchorMax = "1 1", OffsetMin = $"{formPanelInnerMargin} -{yStart + 30}", OffsetMax = $"-{formPanelInnerMargin - 5} -{yStart}" }
                );
                yStart += 30 + fieldsGap;
            }
            ...
            //SendInChat Checkbox
            UI_CheckBox(
                uiTrigger.ConfigActions[actionIndex].SendInChat, nameof(PluginConfig.ConfigAction.SendInChat), lang.GetMessage("UI_Edit_Action_SendInChat", this, player.UserIDString), $"{PREFIX_SHORT}.ui.cmd ActionCB {uiTrigger.TriggerDefinition.Key} {actionIndex}", scrollContent, $"{editPanel}.scrollviewer",
            ...
            UI_CheckBox(
                uiTrigger.ConfigActions[actionIndex].PlayerCanDisable, nameof(PluginConfig.ConfigAction.PlayerCanDisable), string.Format(lang.GetMessage("UI_Edit_Action_CanDisable", this, player.UserIDString), _config.ToggleCommand), $"{PREFIX_SHORT}.ui.cmd ActionCB {uiTrigger.TriggerDefinition.Key} {actionIndex}", scrollContent, $"{editPanel}.scrollviewer",
            ...
            UI_CheckBox(
                uiTrigger.ConfigActions[actionIndex].SendAsGameTip, nameof(PluginConfig.ConfigAction.SendAsGameTip), lang.GetMessage("UI_Edit_Action_SendAsGameTip", this, player.UserIDString), $"{PREFIX_SHORT}.ui.cmd ActionCB {uiTrigger.TriggerDefinition.Key} {actionIndex}", scrollContent, $"{editPanel}.scrollviewer",
                new CuiRectTransformComponent { AnchorMin = "0 1", AnchorMax = "0.49 1", OffsetMin = $"{formPanelInnerMargin} -{yStart + 20}", OffsetMax = $"-{formPanelInnerMargin - 5} -{yStart}" }
            );
            
            //IsBlueGameTip Checkbox
            UI_CheckBox(
                uiTrigger.ConfigActions[actionIndex].IsBlueGameTip, nameof(PluginConfig.ConfigAction.IsBlueGameTip), lang.GetMessage("UI_Edit_Action_IsBlueGameTip", this, player.UserIDString), $"{PREFIX_SHORT}.ui.cmd ActionCB {uiTrigger.TriggerDefinition.Key} {actionIndex}", scrollContent, $"{editPanel}.scrollviewer",
                new CuiRectTransformComponent { AnchorMin = "0.51 1", AnchorMax = "1 1", OffsetMin = $"0 -{yStart + 20}", OffsetMax = $"-{formPanelInnerMargin - 5} -{yStart}" },
            ...
            if (uiTrigger.TriggerDefinition.UsesIsGlobalBroadcast)
            {
                ...
                UI_CheckBox(
                    uiTrigger.ConfigActions[actionIndex].IsGlobalBroadcast, nameof(PluginConfig.ConfigAction.IsGlobalBroadcast), lang.GetMessage("UI_Edit_Action_BroadcastToAll", this, player.UserIDString), $"{PREFIX_SHORT}.ui.cmd ActionCB {uiTrigger.TriggerDefinition.Key} {actionIndex}", scrollContent, $"{editPanel}.scrollviewer",
                    new CuiRectTransformComponent { AnchorMin = "0 1", AnchorMax = "0.49 1", OffsetMin = $"{formPanelInnerMargin} -{yStart + 20}", OffsetMax = $"-{formPanelInnerMargin - 5} -{yStart}" }
                );
                ...
                if (uiTrigger.TriggerDefinition.UsesDontTriggerAdmin)
                {
                    ...
                    UI_CheckBox(
                        uiTrigger.ConfigActions[actionIndex].DontTriggerAdmin, nameof(PluginConfig.ConfigAction.DontTriggerAdmin), lang.GetMessage("UI_Edit_Action_ExcludeAdmin", this, player.UserIDString), $"{PREFIX_SHORT}.ui.cmd ActionCB {uiTrigger.TriggerDefinition.Key} {actionIndex}", scrollContent, $"{editPanel}.scrollviewer",
                        new CuiRectTransformComponent { AnchorMin = "0.51 1", AnchorMax = "1 1", OffsetMin = $"0 -{yStart + 20}", OffsetMax = $"-{formPanelInnerMargin - 5} -{yStart}" }
                    );
                }
                yStart += 20 + fieldsGap;
            ...
            if (uiTrigger.TriggerDefinition.HasTriggerOwner)
            {
                ...
                UI_CheckBox(
                    uiTrigger.ConfigActions[actionIndex].UseTriggerOwnerIcon, nameof(PluginConfig.ConfigAction.UseTriggerOwnerIcon), lang.GetMessage("UI_Edit_Action_UseTriggerOwnerIcon", this, player.UserIDString), $"{PREFIX_SHORT}.ui.cmd ActionCB {uiTrigger.TriggerDefinition.Key} {actionIndex}", scrollContent, $"{editPanel}.scrollviewer",
                    new CuiRectTransformComponent { AnchorMin = "0 1", AnchorMax = "0.49 1", OffsetMin = $"{formPanelInnerMargin} -{yStart + 20}", OffsetMax = $"-{formPanelInnerMargin - 5} -{yStart}" }
                );
                yStart += 20 + fieldsGap;
            }
            ...
            //CustomIconSteamId Textbox
            UI_TextBox(
                uiTrigger.ConfigActions[actionIndex].CustomIconSteamId, lang.GetMessage($"UI_Edit_Action_CustomIconSteamId", this, player.UserIDString), $"{PREFIX_SHORT}.ui.texteditor Action ActionTextbox {uiTrigger.TriggerDefinition.Key} {actionIndex} {nameof(PluginConfig.ConfigAction.CustomIconSteamId)}", scrollContent, $"{editPanel}.scrollviewer",
                new CuiRectTransformComponent { AnchorMin = "0 1", AnchorMax = "1 1", OffsetMin = $"{formPanelInnerMargin} -{yStart + 30}", OffsetMax = $"-{formPanelInnerMargin - 5} -{yStart}" }
            );
            yStart += 30 + fieldsGap;
            ...
            //Permissions
            UI_Listbox(
                player, "Action", nameof(PluginConfig.ConfigAction.Permissions), uiTrigger.ConfigActions[actionIndex].Permissions, lang.GetMessage("UI_Edit_Action_Permissions", this, player.UserIDString).ToUpper(), $"{PREFIX_SHORT}.ui.cmd %cmdname% {uiTrigger.TriggerDefinition.Key} {actionIndex} %index%", scrollContent, $"{editPanel}.scrollviewer",
            ...
            //Groups
            UI_Listbox(
                player, "Action", nameof(PluginConfig.ConfigAction.Groups), uiTrigger.ConfigActions[actionIndex].Groups, lang.GetMessage("UI_Edit_Action_Groups", this, player.UserIDString).ToUpper(), $"{PREFIX_SHORT}.ui.cmd %cmdname% {uiTrigger.TriggerDefinition.Key} {actionIndex} %index%", scrollContent, $"{editPanel}.scrollviewer",
            ...
            UI_Listbox(
                player, "Action", nameof(PluginConfig.ConfigAction.BlacklistedPerms), uiTrigger.ConfigActions[actionIndex].BlacklistedPerms, lang.GetMessage("UI_Edit_Action_ExcludedPermissions", this, player.UserIDString).ToUpper(), $"{PREFIX_SHORT}.ui.cmd %cmdname% {uiTrigger.TriggerDefinition.Key} {actionIndex} %index%", scrollContent, $"{editPanel}.scrollviewer",
                new CuiRectTransformComponent { AnchorMin = "0 1", AnchorMax = "0.49 1", OffsetMin = $"{formPanelInnerMargin} -{yStart + 110}", OffsetMax = $"0 -{yStart}" }
            );
            //yStart += 115 + fieldsGap;
            ...
            //Blacklisted Groups
            UI_Listbox(
                player, "Action", nameof(PluginConfig.ConfigAction.BlacklistedGroups), uiTrigger.ConfigActions[actionIndex].BlacklistedGroups, lang.GetMessage("UI_Edit_Action_ExcludedGroups", this, player.UserIDString).ToUpper(), $"{PREFIX_SHORT}.ui.cmd %cmdname% {uiTrigger.TriggerDefinition.Key} {actionIndex} %index%", scrollContent, $"{editPanel}.scrollviewer",
                new CuiRectTransformComponent { AnchorMin = "0.51 1", AnchorMax = "1 1", OffsetMin = $"0 -{yStart + 110}", OffsetMax = $"-{formPanelInnerMargin - 5} -{yStart}" }
            );
        ...
        private void UI_Listbox(BasePlayer player, string view, string key, List<string> list, string label, string command, CuiElementContainer cont, string parent, CuiRectTransformComponent RectTransform, int minItemsInView = 2, bool previewButton = false)
        {
            ...
            UI_Listbox_Scrollviewer(player, view, key, listPanel, list, listInnerMargin, subtitleHeight, command, previewButton, minItemsInView, ref cont);
        }

        private void UI_Listbox_Scrollviewer(BasePlayer player, string view, string key, string listPanel, List<string> list, int listInnerMargin, int subtitleHeight, string command, bool previewButton, int minItemsInView, ref CuiElementContainer cont, bool scrollToBottom = false)
        {
        ...
        private void UI_Listbox_Update(BasePlayer player, string view, string key, string command, List<string> list, CuiElementContainer cont, string label = null, bool scrollToBottom = false, int minItemsInView = 2, bool previewButton = false)
        {
            string listPanel = $"{uimodal}.list.{key}";
            ...
            UI_Listbox_Scrollviewer(player, view, key, listPanel, list, listInnerMargin, subtitleHeight, command, previewButton, minItemsInView, ref cont, scrollToBottom);
        }
        ...
        void AMEditorSelectViewConsole(ConsoleSystem.Arg arg)
        {
            ...
            switch (arg.Args[0])
            {
                ...
                case "AddNewAction":
                    ...
                    UI_ShowActionsList(player, uiTrigger, uiSession.SelectedRepliesLang, uiTrigger.ConfigActions.Count - 1);
                    break;
                ...
                case "ActionCB":
                    ...
                    switch (arg.Args[3])
                    {
                        case "Enabled":
                            ...
                            UI_CheckBox_Update(uiAction.Enabled, nameof(PluginConfig.ConfigAction.Enabled), cont, lang.GetMessage("UI_Edit_Action_Active", this, player.UserIDString), lang.GetMessage("UI_Edit_Action_Inactive", this, player.UserIDString));
                            CuiHelper.AddUi(player, cont);
                            break;
                        case "SendInChat":
                        ...
                        case "IsBlueGameTip":
                            ...
                            UI_CheckBox_Update(uiAction.IsBlueGameTip, nameof(PluginConfig.ConfigAction.IsBlueGameTip), cont, lang.GetMessage("UI_Edit_Action_IsBlueGameTip", this, player.UserIDString), lang.GetMessage("UI_Edit_Action_IsRedGameTip", this, player.UserIDString), enabledHex: "#376E92");
                            CuiHelper.AddUi(player, cont);
                            break;
                        ...
                        case "UseTriggerOwnerIcon":
                            ...
                            UI_CheckBox_Update(uiAction.UseTriggerOwnerIcon, nameof(PluginConfig.ConfigAction.UseTriggerOwnerIcon), cont);
                            CuiHelper.AddUi(player, cont);
                            break;
                    }
                    break;
                case "ActionDeleteListItem":
                    ...
                    switch (arg.Args[3])
                    {
                        case "Replies":
                            ...
                            UI_Listbox_Update(player, "Action", nameof(Lang_Action.Replies), $"{PREFIX_SHORT}.ui.cmd %cmdname% {uiTrigger.TriggerDefinition.Key} {actionIndex} %index%", langAction.Replies, cont, minItemsInView: 3, previewButton: true);
                            CuiHelper.AddUi(player, cont);
                            break;
                        case "Permissions":
                            listItemIndex = int.Parse(arg.Args[4]);
                            ...
                            UI_Listbox_Update(player, "Action", nameof(PluginConfig.ConfigAction.Permissions), $"{PREFIX_SHORT}.ui.cmd %cmdname% {uiTrigger.TriggerDefinition.Key} {actionIndex} %index%", uiAction.Permissions, cont);
                            CuiHelper.AddUi(player, cont);
                            break;
                        case "Groups":
                            listItemIndex = int.Parse(arg.Args[4]);
                            ...
                            UI_Listbox_Update(player, "Action", nameof(PluginConfig.ConfigAction.Groups), $"{PREFIX_SHORT}.ui.cmd %cmdname% {uiTrigger.TriggerDefinition.Key} {actionIndex} %index%", uiAction.Groups, cont);
                            CuiHelper.AddUi(player, cont);
                            break;
                        case "BlacklistedPerms":
                            ...
                            UI_Listbox_Update(player, "Action", nameof(PluginConfig.ConfigAction.BlacklistedPerms), $"{PREFIX_SHORT}.ui.cmd %cmdname% {uiTrigger.TriggerDefinition.Key} {actionIndex} %index%", uiAction.BlacklistedPerms, cont);
                            CuiHelper.AddUi(player, cont);
                            break;
                        case "BlacklistedGroups":
                            ...
                            UI_Listbox_Update(player, "Action", nameof(PluginConfig.ConfigAction.BlacklistedGroups), $"{PREFIX_SHORT}.ui.cmd %cmdname% {uiTrigger.TriggerDefinition.Key} {actionIndex} %index%", uiAction.BlacklistedGroups, cont);
                            CuiHelper.AddUi(player, cont);
                            break;
                    }
                    break;
                case "ActionAddListItem":
                    ...
                    switch (arg.Args[3])
                    {
                        case "Replies":
                            ...
                            UI_Listbox_Update(player, "Action", nameof(Lang_Action.Replies), $"{PREFIX_SHORT}.ui.cmd %cmdname% {uiTrigger.TriggerDefinition.Key} {actionIndex} %index%", langAction.Replies, cont, scrollToBottom: true, minItemsInView: 3, previewButton: true);
                            CuiHelper.AddUi(player, cont);
                        ...
                        case "Permissions":
                            ...
                            UI_Listbox_Update(player, "Action", nameof(PluginConfig.ConfigAction.Permissions), $"{PREFIX_SHORT}.ui.cmd %cmdname% {uiTrigger.TriggerDefinition.Key} {actionIndex} %index%", uiAction.Permissions, cont, scrollToBottom: true);
                            CuiHelper.AddUi(player, cont);
                            listItemIndex = uiAction.Permissions.Count - 1;
                            UI_ShowTextEditor(player, "Action", uiAction.Permissions.Last(), "Permission Name", $"{uiTrigger.TriggerDefinition.Key} {actionIndex}", nameof(PluginConfig.ConfigAction.Permissions), listItemIndex);
                            break;
                        case "Groups":
                            uiAction.Groups.Add("");

                            UI_Listbox_Update(player, "Action", nameof(PluginConfig.ConfigAction.Groups), $"{PREFIX_SHORT}.ui.cmd %cmdname% {uiTrigger.TriggerDefinition.Key} {actionIndex} %index%", uiAction.Groups, cont, scrollToBottom: true);
                            CuiHelper.AddUi(player, cont);
                            listItemIndex = uiAction.Groups.Count - 1;
                            UI_ShowTextEditor(player, "Action", uiAction.Groups.Last(), "Group Name", $"{uiTrigger.TriggerDefinition.Key} {actionIndex}", nameof(PluginConfig.ConfigAction.Groups), listItemIndex);
                            break;
                        case "BlacklistedPerms":
                            uiAction.BlacklistedPerms.Add("pluginname.permission");

                            UI_Listbox_Update(player, "Action", nameof(PluginConfig.ConfigAction.BlacklistedPerms), $"{PREFIX_SHORT}.ui.cmd %cmdname% {uiTrigger.TriggerDefinition.Key} {actionIndex} %index%", uiAction.BlacklistedPerms, cont, scrollToBottom: true);
                            CuiHelper.AddUi(player, cont);
                            listItemIndex = uiAction.BlacklistedPerms.Count - 1;
                            UI_ShowTextEditor(player, "Action", uiAction.BlacklistedPerms.Last(), "Permission Name", $"{uiTrigger.TriggerDefinition.Key} {actionIndex}", nameof(PluginConfig.ConfigAction.BlacklistedPerms), listItemIndex);
                            break;
                        case "BlacklistedGroups":
                            uiAction.BlacklistedGroups.Add("");

                            UI_Listbox_Update(player, "Action", nameof(PluginConfig.ConfigAction.BlacklistedGroups), $"{PREFIX_SHORT}.ui.cmd %cmdname% {uiTrigger.TriggerDefinition.Key} {actionIndex} %index%", uiAction.BlacklistedGroups, cont, scrollToBottom: true);
                            CuiHelper.AddUi(player, cont);
                            listItemIndex = uiAction.BlacklistedGroups.Count - 1;
                            UI_ShowTextEditor(player, "Action", uiAction.BlacklistedGroups.Last(), "Group Name", $"{uiTrigger.TriggerDefinition.Key} {actionIndex}", nameof(PluginConfig.ConfigAction.BlacklistedGroups), listItemIndex);
                            break;
                    }
                    break;
                case "ActionSelectListItem":
                    ...
                    switch (arg.Args[3])
                    {
                        ...
                        case "Permissions":
                            listItemIndex = int.Parse(arg.Args[4]);
                            UI_ShowTextEditor(player, "Action", uiAction.Permissions[listItemIndex], "Permission Name", $"{uiTrigger.TriggerDefinition.Key} {actionIndex}", nameof(PluginConfig.ConfigAction.Permissions), listItemIndex);
                            break;
                        case "Groups":
                            listItemIndex = int.Parse(arg.Args[4]);
                            UI_ShowTextEditor(player, "Action", uiAction.Groups[listItemIndex], "Group Name", $"{uiTrigger.TriggerDefinition.Key} {actionIndex}", nameof(PluginConfig.ConfigAction.Groups), listItemIndex);
                            break;
                        case "BlacklistedPerms":
                            listItemIndex = int.Parse(arg.Args[4]);
                            UI_ShowTextEditor(player, "Action", uiAction.BlacklistedPerms[listItemIndex], "Permission Name", $"{uiTrigger.TriggerDefinition.Key} {actionIndex}", nameof(PluginConfig.ConfigAction.BlacklistedPerms), listItemIndex);
                            break;
                        case "BlacklistedGroups":
                            listItemIndex = int.Parse(arg.Args[4]);
                            UI_ShowTextEditor(player, "Action", uiAction.BlacklistedGroups[listItemIndex], "Group Name", $"{uiTrigger.TriggerDefinition.Key} {actionIndex}", nameof(PluginConfig.ConfigAction.BlacklistedGroups), listItemIndex);
                            break;
                    }
                    break;
