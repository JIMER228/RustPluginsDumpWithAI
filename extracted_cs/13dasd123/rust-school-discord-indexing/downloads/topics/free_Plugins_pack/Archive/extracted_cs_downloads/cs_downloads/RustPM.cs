...
namespace Oxide.Plugins
{
    ...
    class RustPM : RustPlugin
    {
        ...
        private void Init()
        {
            ...
            {
                {"PMTo", "<size=18>Личное сообщение для <color=orange>{0}</color></size>\nСообщение: <color=orange>{1}</color>"},
                {"PMFrom", "<size=18>Личное сообщение от <color=orange>{0}</color></size>\nСообщение: <color=orange>{1}</color>"},
                {"PLAYER.NOT.FOUND", "Игрок <color=orange>{0}</color> не найден"},
