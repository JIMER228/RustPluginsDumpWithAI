 ...
 *
 * 2.7.1
 * Added code to yell at you for setting in-correct stack size values & un-supported stack sizes, to prevent support ticket problems.
 *
 * 2.7.2 - 2.7.4
 * Rust updates
 *
 * 2.7.5
 * Fixed hover / bug with stacking items to max stack size.
 *
 * 2.7.6
 * Fixed weapon attachments unloading if stack size was set to 1.
 * Due to stack-modifier already supporting weapon stacks.
 *
 * 2.7.7
 * Preached for rust update.
 * Added 46? ish new items
 * Patched new config generation bug with staging builds, when defaults where missing.
 *
 * 2.7.8
 * Patched crafted armor bugs with armor slots.
 *

 * ⚡ **Version 2.7.9 – Food Spoilage Fixes & Changes** ⚡
 * 
 * 🛠 **Bug Fixes**:
 * - Fixed food spoil times when stored in fridges or stacked.
 * - Patched Vanilla Rust bug causing spoil timer overwrite on stack or split.
 * 
 * 📏 **New Stacking Rules**:
 * - 🍖 Food can only stack if spoil times are within 1 hour.
 * - ⏳ If spoil times differ by more than 1 hour, they won’t stack (prevents spoil timer resets!).
 * - 💀 Spoiled food only stacks with other spoiled food.
 *
 * update 2.8.0
 * Rewrote conveyor's harmony patch to be exactly like the canstack logic.
 *
 * beta update 2.8.4
 * Harmony Patched: Industrial Crafter bugs / exploits.
 *
 * Update 2.8.5
 * Stable
 *
 * update 2.8.7
 * Stripped all remaining code for industrial out.
 * Cleaned code
 * Updated for forcewipe
*/
namespace Oxide.Plugins
{
    [Info("Stack Modifier", "Khan", "2.8.9")]
    ...
    public class StackModifier : RustPlugin
    {
        ...
        */
        ...
        private const float Tolerance = 3600f;
        ...
        private void GenerateConfig()
        {
            ...
            foreach (ItemDefinition item in ItemManager.itemList)
            {
                ...
                int stored = 1;
                if (_defaults.TryGetValue(id, out stored))
                {
                    if (stack != stored)
                ...
                stackCategory.TryAdd(shortName, new _Items
                {
                    ShortName = shortName,
                    ItemId = id,
                    DisplayName = _corrections.TryGetValue(shortName, out var correction) ? correction : item.displayName.english,
        ...
        private IEnumerator CheckForDiscrepanciesWithGameItems()
        {
            ...
            {
                if (item.Value.condition.enabled)
                {
                    counter++;
                    if (item.Value.condition.max > 0.0)
                        counter2++;
                }
            ...
            Puts($"{counter} items have condition enabled but only: {counter2} actually use it.");
            ...
            StringBuilder categorysizetoobig = new StringBuilder();
            List<string> categories = new List<string>();
            foreach (var size in _config.StackCategoryMultipliers)
            {
                int cat = size.Value;
                if (cat * 5 <= -1)
                {
                    categorysizetoobig.AppendLine($"Category: {size.Key} multiplier of {size.Value} is too large, resetting to 0.");
                    categories.Add(size.Key);
                }
            }
            ...
            if (categorysizetoobig.Length > 0)
                PrintError(categorysizetoobig.ToString());

            SaveConfig();
            Puts("100%");
            yield return ModifyStacks();
        }
        ...
        private IEnumerator ModifyStacks()
        {
            ...
            foreach (ItemDefinition item in ItemManager.itemList)
            {
                ...
                else if (multiplier > 0 && update.Modified == stock)
                {
                    ...
                    if (math < -1)
                    {
                        ...
                        //PrintError($"reset {categoryName} category multiplier to 0 due to multiplier exceeding max supported value.");
                        reload = true;
                    }
                    item.stackable = math;
            ...
            {
                SaveConfig();
                PrintError("Corrected an invalid stack-size problem and reloading plugin.");
                yield return CoroutineEx.waitForSeconds(0.30f);
                Server.Command("o.reload StackModifier");
                Updating = null;
                yield return null;
            }
        ...
        private void OnServerShutdown()
        {
            ...
            var check = BasePlayer.allPlayerList.ToList();
            if (check.Any())
                foreach (var player in check)
                {
                    ulong user = (ulong)player.userID;
                    if (!user.IsSteamId() || !permission.UserHasPermission(player.UserIDString, Admin)) continue;
                    DestroyUi(player, true);
                }
            _defaults = null;
        }
        ...
        //ItemModOpenWrapped
        //ItemModUnwrap
        // possible issue stacking armor can lead to unloading armor plates, lol
        private object CanStackItem(Item item, Item targetItem)
        {
            ...
            if (item.removeTime > 0 ||
                ...
                (item.blueprintTarget != 0 && item.blueprintTarget != targetItem.blueprintTarget) ||
            ...
            ProtoBuf.Item.InstanceData a = item.instanceData;
            ProtoBuf.Item.InstanceData b = targetItem.instanceData;
            if ((a != null) != (b != null)) return false;

            if (a != null)
            {
                if (b.dataInt != a.dataInt || (a?.subEntity.Value ?? 0) != (b?.subEntity.Value ?? 0)) return false;

                bool sA = (bool)id.GetComponent<ItemModFoodSpoiling>();
                bool sB = (bool)td.GetComponent<ItemModFoodSpoiling>();
                if (sA != sB) return false;

                if (sA)
                {
                    float tA = a.dataFloat;
                    float tB = b.dataFloat;
                    if (Math.Abs(tA - tB) > Tolerance) return false;
                }
            }

            ItemContainer tc = targetItem.contents;
            ItemContainer ic = item.contents;
            if ((ic?.capacity ?? 0) != (tc?.capacity ?? 0)) return false;

            List<Item> tci = tc?.itemList;
            List<Item> ici = ic?.itemList;
            bool tcc = (tci?.Count ?? 0) > 0;
            bool icc = (ici?.Count ?? 0) > 0;
            if ((tcc && !HasVanillaContainer(td)) || (icc && !HasVanillaContainer(id))) return false;

            ItemContainer fix = p?.inventory.containerMain ?? (item.parent ?? targetItem.parent);
            BaseEntity held = targetItem.GetHeldEntity();
            if (!(ic?.allowedContents == ItemContainer.ContentsType.Liquid && held?.GetComponentInChildren<BaseLiquidVessel>() != null))
            {
                if (tcc)
            ...
            if (held is BaseProjectile weapon)
            {
                if (weapon.primaryMagazine.contents <= 0) return true;
                fix.GiveItem(ItemManager.Create(weapon.primaryMagazine.ammoType, weapon.primaryMagazine.contents));
        ...
        {
            if (itemDefinition.itemMods == null)
                return true;

            ItemMod[] mods = itemDefinition.itemMods;
            int count = mods.Length;
            for (int i = 0; i < count; i++)
            {
                if (mods[i] is ItemModContainer)
                    return true;
            }
        ...
        */
        ...
        private Item OnItemSplit(Item item, int amount)
        {
            ...
            if (container != null)
            {
                ItemModContainerArmorSlot armor = i.info.GetComponent<ItemModContainerArmorSlot>();
                if (armor != null)
                    armor.CreateAtCapacity(container.capacity, i);
                else if (i.contents == null)
                {
            ...
            item.MigrateItemOwnership(i, item.amount);
            if (item.IsBlueprint())
                i.blueprintTarget = item.blueprintTarget;
            if (item.instanceData != null)
            {
                i.instanceData = new ProtoBuf.Item.InstanceData();
                i.instanceData.ShouldPool = false;
                item.instanceData.CopyTo(i.instanceData);
            }
        ...
        {
            if (player?.userID.Get().IsSteamId() != true) return;
            List<Item> mods = weapon.GetCachedItem().contents?.itemList;
            if (mods == null || mods.Count == 0) return;
            PlayerInventory send = player.inventory;
            ItemContainer to = player.inventory.containerMain;
            foreach (Item i in mods)
            {
                if (i?.isBroken != false || i.amount <= 1) continue;
        ...
        {
            if (item == null || item.GetOwnerPlayer()?.userID.Get().IsSteamId() != true) return null;
            BaseEntity held = item.GetHeldEntity();
            if (!(held is ProjectileWeaponMod)) return null;
            Item thingy = playerLoot.FindContainer(targetContainer)?.parent;
            BaseEntity projectile = thingy?.GetHeldEntity();
            if (!(projectile is BaseProjectile)) return null;
        ...
        private void OnItemAddedToContainer(ItemContainer container, Item item)
        {
            ...
            if (player?.userID.Get().IsSteamId() != true || player.IsAdmin) return;
        ...
        private object OnCardSwipe(CardReader cardReader, Keycard card, BasePlayer player)
        {
            ...
            if (item?.isBroken != false || item.amount <= 1 || cardReader.accessLevel != card.accessLevel) return null;
            Sort(item, player);
            return null;
        }
        ...
        {
            if (item?.isBroken != false || item.amount <= 1) return null;
        ...
        private void Sort(Item item, BasePlayer player)
        {
            ...
            foreach (int value in results)
            {
                ...
                if (x != null && !x.MoveToContainer(container) && (item.parent == null || !x.MoveToContainer(item.parent)))
        ...
        */
        ...
        {
            _editorPageSM[player.userID.Get()] = from;
        ...
        private void ConsoleEditorShow(ConsoleSystem.Arg arg)
        {
            ...
            if (catid.Equals("close"))
            {
                ...
                _editorPageSM.Remove(targetPlayer.userID.Get());
                Interface.CallHook("OnStackSizeUpdated");
                return;
            }
        ...
        private void ConsoleEditSet(ConsoleSystem.Arg arg)
        {
            ...
            ShowEditor(player, catName, _editorPageSM[player.userID.Get()], false);
        }
        ...
        private void ConsoleEditClose(ConsoleSystem.Arg arg)
        {
            ...
            BasePlayer player = arg.Player();
            DestroyUi(player, true);
            _editorPageSM.Remove(player.userID.Get());
            Interface.CallHook("OnStackSizeUpdated");
        }
        ...
        public class Color
        {
            ...
            [JsonIgnore]
            public string c = "ZwtlAwD= 1746132106 Khan";
        }

        #endregion

        #region API ( pls don't make me regret adding this.. )
