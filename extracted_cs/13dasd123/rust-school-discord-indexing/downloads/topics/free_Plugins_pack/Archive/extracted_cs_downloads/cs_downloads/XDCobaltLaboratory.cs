...
{
    [Info("XDCobaltLaboratory", "DezLife", "2.5.7")]
    internal class XDCobaltLaboratory : RustPlugin
    {
		...
		private IEnumerator GenerateSpawnPoints() //cywka
		{
			int maxRadius = 1500; 
			int checks = 0;
			...
			for (int i = 0; i < 100; i++)
			{
				...
				float distance = Random.Range(500f, maxRadius);
				...
				// Проверка, чтобы точка была на суше
				if (TerrainMeta.HeightMap.GetHeight(pos) > TerrainMeta.WaterMap.GetHeight(pos))
			{
					PosValidation(pos);
			}
        ...
        void CMDHideUi(ConsoleSystem.Arg arg)
        {
            ...
            if (Player != null)
            {
                if (!HideUIUser.Contains(player.userID.Get()))
                {
                    HideUIUser.Add(player.userID.Get());
                    CuiHelper.DestroyUi(player, "CobaltPanel");
                }
                else
                {
                    HideUIUser.Remove(player.userID.Get());
                    Cui.MainUI(player);
                }
            }
        }
            ...
            foreach (BaseEntity ent in HouseCobaltLab.ToArray())
            {
                ...
                if (ent is Minicopter)
                {
                    if (Random.Range(0, 100) >= (100 - labIndex.copterChance))
                    {
                        ...
                        copter.GetFuelSystem().AddFuel(50);
        ...
        private void OnServerInitialized()
        {
            ...
            else if (CopyPaste.Version < new VersionNumber(4, 1, 37))
            {
                NextTick(() =>
                {
            ...
            else if (NpcSpawn.Version < new VersionNumber(2, 5, 6))
            {
                NextTick(() =>
                {
                    PrintError("You have an old version of NpcSpawn!\nplease update the plugin to the latest version (2.5.6 or higher) - ReadMe.txt");
                    Interface.Oxide.UnloadPlugin(Name);
                });
                return;
            }
        ...
        private class Configuration
        {
            ...
            internal class NotiferSettings
            {
                ...
                public string colorBtnCloseUi = "0.67 0.25 0.18 0.8";
            }
                ...
                public static class Cui
        {
            public static void CreateUIAllPlayer()
            {
                ...
                {
                    ButtonClose(player);
                    if (_.HideUIUser.Contains(player.userID.Get()))
                    {
                        continue;
                    }
                ...
                {
                    {
                        new CuiPanel
                        {
                            ...
                            RectTransform = { AnchorMin = "1 0.5", AnchorMax = "1 0.5", OffsetMin = "-342.19 -15.97", OffsetMax = "-13.80 59.66" }
                        },
                        "Overlay",
                        "CobaltPanel"
                    },
                    ...
                    new CuiElement
                    {
                        ...
                        Parent = "CobaltPanel",
                        Components = {
                    new CuiRawImageComponent { Color = "0.95 0.72 0 1", Material = "assets/icons/iconmaterial.mat", Sprite = "assets/icons/radiation.png", FadeIn = 0.2f },
                    ...
                    new CuiRectTransformComponent { AnchorMin = "0.5 1", AnchorMax = "0.5 1", OffsetMin = "-103.80 -23.93", OffsetMax = "103.80 -2.86" }
                }
                    },
                    ...
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-118.84 -33.07", OffsetMax = "151.44 13.88" }
                }
                    }
                };
                ...
                {
                    {
                        new CuiPanel
                        {
                            ...
                            RectTransform = { AnchorMin = "1 0.5", AnchorMax = "1 0.5", OffsetMin = "-13.85 -15.97", OffsetMax = "0 59.66" }
                        },
                        "Overlay",
                        "CobaltClosePanel"
                    },
                    ...
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-8.73 -11.99", OffsetMax = "8.74 11.99" }
                }
                    },

                    {
                        new CuiButton
                        {
                            ...
                            RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-8.73 -37.82", OffsetMax = "8.74 37.82" }
                        },
                        "CobaltClosePanel",
                        "Closed"
                    }
