...
{
    [Info("IQWipeBlock", "Mercury", "1.12.15")]
    ...
    class IQWipeBlock : RustPlugin
    {
        ...
        void OnItemAddedToContainer(ItemContainer container, Item item)
        {
            ...
            if (IQRecycler && IQRecycler.Call<Boolean>("IsBlackList", player.userID.Get(), item) && item.HasFlag(global::Item.Flag.IsLocked) && !Status)
                return;
        ...
        Boolean Block_Skin_Controller(Configuration.Blocks.BlockElement BlockInfo, Dictionary<String, Configuration.Blocks.BlockElement> BlockAll, Boolean BlockStatusShortname, UInt64 SkinID = 0)
        {
            ...
            else if(SkinID == 2908022597) return true;
            else return false;
        }
        ...
        {
            if (alternativeNames.TryGetValue(shortname, out String alternativeName))
                shortname = alternativeName;
            else
            {
                foreach (String suffix in suffixes)
                {
                    Int32 index = shortname.IndexOf(suffix, StringComparison.Ordinal);
                    if (index <= 0) continue;
        ...
        void Interface_Button_Panel(BasePlayer player)
        {
            ...
            {
                FadeOut = 0.2f,
                RectTransform = { AnchorMin = $"0.2075757 0.3444443", AnchorMax = $"1 0.62668022" }, 
                Button = { Command = "block", Color = "0 0 0 0" },
            ...
            if (General.UseHidePanelButton)
            {
                ...
                {
                    FadeOut = 0.2f,
                    RectTransform = { AnchorMin = $"0.7 0.06666651", AnchorMax = $"1 0.348888022" },  
        ...
        ItemContainer.CanAcceptResult? CanAcceptItem(ItemContainer container, Item item, int targetPos)
        {
            ...
            if (container.availableSlots.Count != 0)
            {
                ...
                {
                    if (IsDuel(ContainerPlayer.userID.Get()))
                        return null;
        ...
        public Boolean HasImage(String imageName) => (Boolean)ImageLibrary?.Call("HasImage", imageName);
                ...
                private readonly List<String> suffixes = new() { ".ice", ".diver", ".dragon", ".driver" };
        ...
        private Object OnRackedWeaponLoad(Item weapon, ItemDefinition itemDefinition, BasePlayer player, WeaponRack instance)
        {
            if (!Interface_UI_Block(player, itemDefinition.shortname)) return false;
            if (Interface_UI_Block(player, weapon.info.shortname, itemDefinition.shortname))
                return null;
            return false;
        }
        ...
        Boolean Interface_UI_Block(BasePlayer player, String Shortname, String ShortnameModule = "", UInt64 SkinID = 0, List<Connection> playerList = null, Boolean IsCheckConnect = false)
        {
            ...
            if (!IsCheckConnect)
            {
                ...
                {
                    FadeOut = FadeOut,
                    RectTransform = { AnchorMin = "0.1125769 0.041802243", AnchorMax = "1 0.5480229" },
                    Text =
                    {
        ...
        //TODO: Отключать вайпблок в рейд базах (в зоне ее)
        /// <summary>
        /// Обновление 1.
        ...
        private readonly Dictionary<String, String> alternativeNames = new Dictionary<String, String>
        {
            { "blunderbuss", "shotgun.double" }, 
            { "rifle.sks", "rifle.semiauto" }    
        };
