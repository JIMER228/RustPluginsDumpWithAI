...
{
    [Info("StatsSystem", "Scrooge", "1.1.0")]
    [Description("Система статистики для игроков")]
    class StatsSystem : RustPlugin
    {
        #region Вар
        private const string Layer = "Stats_UI";
        private const string PermissionAdmin = "statssystem.admin";

        [PluginReference] private Plugin ImageLibrary, RustStore;

        private Dictionary<ulong, DBSettings> DB = new Dictionary<ulong, DBSettings>();
        ...
        #region Хуки
        private Dictionary<BaseEntity, BasePlayer> lastDamagePlayer = new Dictionary<BaseEntity, BasePlayer>();
        ...
        void OnServerInitialized()
        {
            ...
            if (Interface.Oxide.DataFileSystem.ExistsDatafile("StatsSystem/PlayerList"))
                DB = Interface.Oxide.DataFileSystem.ReadObject<Dictionary<ulong, DBSettings>>("StatsSystem/PlayerList");
            ...
            // Очистка словаря каждые 5 минут
            timer.Every(300f, () => 
            {
                lastDamagePlayer.Clear();
                Puts("Cleared lastDamagePlayer dictionary");
            });
        }
        ...
        {
            if (player == null || !DB.ContainsKey(player.userID)) return;
        ...
        void OnDispenserGather(ResourceDispenser dispenser, BasePlayer player, Item item)
        {
            ...
            if (DB[player.userID].Res.ContainsKey(item.info.shortname) && dispenser.fractionRemaining <= 0)
            {
        ...
        void OnEntityTakeDamage(BaseCombatEntity entity, HitInfo info)
        {
            ...
            var prefabName = entity.ShortPrefabName.ToLower();
            if (prefabName.Contains("barrel") || prefabName.Contains("loot") || prefabName.Contains("crate") || prefabName.Contains("patrolhelicopter"))
            {
                lastDamagePlayer[entity] = info.InitiatorPlayer;
            }
        }
        ...
        void OnEntityDeath(BaseCombatEntity entity, HitInfo info)
        {
            ...
            var prefabName = entity.ShortPrefabName.ToLower();
            if (prefabName.Contains("barrel") || prefabName.Contains("loot") || prefabName.Contains("crate"))
            {
                BasePlayer player = null;

                // Пытаемся получить игрока из разных источников
                if (info?.InitiatorPlayer != null)
                    player = info.InitiatorPlayer;
                else if (lastDamagePlayer.ContainsKey(entity))
                    player = lastDamagePlayer[entity];

                if (player != null && DB.ContainsKey(player.userID))
                {
                ...
                // Очищаем запись о последнем ударившем игроке
                lastDamagePlayer.Remove(entity);
            }
            else if (prefabName.Contains("patrolhelicopter"))
            {
                BasePlayer player = null;
                if (info?.InitiatorPlayer != null)
                    player = info.InitiatorPlayer;
                ...
                if (player != null && DB.ContainsKey(player.userID))
                {
                    const int helicopterPoints = 500; // Количество очков за сбитие вертолета
                    DB[player.userID].Points += helicopterPoints;
                    SaveDataBase();
                    Puts($"Player {player.displayName} destroyed helicopter and got {helicopterPoints} points");
                    SendReply(player, $"Вы сбили вертолет и получили {helicopterPoints} очков!");
                }
        ...
        void ApiChangeGameStoresBalance(ulong userId, int amount)
        {
            ...
            if (player == null)
            {
                PrintError($"ApiChangeGameStoresBalance: Player not found for ID {userId}");
                return;
            }
        ...
        void APIChangeUserBalance(ulong steam, int balanceChange)
        {
            if (RustStore == null)
            {
                PrintError("RustStore plugin not found!");
                return;
            }

            RustStore.Call("APIChangeUserBalance", steam, balanceChange, new Action<string>((result) =>
            {
                if (result == "SUCCESS")
                {
        ...
        {
            if (string.IsNullOrEmpty(config.ShopID) || string.IsNullOrEmpty(config.Secret))
            {
                PrintError("Shop ID or Secret key is not configured!");
                return;
            }

            string url = $"https://gamestores.ru/api/?shop_id={config.ShopID}&secret={config.Secret}" + 
            ...
            webrequest.EnqueueGet(url, (i, s) =>
            {
                if (i != 200)
                {
                    PrintError($"Ошибка соединения с сайтом! Код: {i}");
                    return;
                }
                ...
                {
                    PrintError($"Error parsing API response: {ex.Message}");
                    LogToFile("logError", $"API Error: {ex}", this);
                }
            }, this);
        }
        #endregion
        ...
        private void CmdAdmin(BasePlayer player, string command, string[] args)
        {
            ...
            if (args.Length == 0)
            {
                SendReply(player, "Использование команд администратора:");
                SendReply(player, "/stats.admin points <steamid> <amount> - установить количество очков");
                SendReply(player, "/stats.admin reset <steamid> - сбросить статистику игрока");
                SendReply(player, "/stats.admin resetall - сбросить статистику всех игроков");
                return;
            }
            ...
            switch (args[0].ToLower())
            {
                case "points":
                    if (args.Length != 3)
                    {
                        SendReply(player, "Использование: /stats.admin points <steamid> <amount>");
                        return;
                    }
                    if (!ulong.TryParse(args[1], out ulong targetId) || !int.TryParse(args[2], out int amount))
                    {
                        SendReply(player, "Неверный формат steamid или количества очков");
                        return;
                    }
                    if (!DB.ContainsKey(targetId))
                    {
                        SendReply(player, "Игрок не найден в базе данных");
                        return;
                    }
                    DB[targetId].Points = amount;
                    SaveDataBase();
                    SendReply(player, $"Установлено {amount} очков для игрока {DB[targetId].DisplayName}");
                    break;

                case "reset":
                    if (args.Length != 2)
                    {
                        SendReply(player, "Использование: /stats.admin reset <steamid>");
                        return;
                    }
                    if (!ulong.TryParse(args[1], out ulong resetId))
                    {
                        SendReply(player, "Неверный формат steamid");
                        return;
                    }
                    if (!DB.ContainsKey(resetId))
                    {
                        SendReply(player, "Игрок не найден в базе данных");
                        return;
                    }
                    ResetPlayerStats(resetId);
                    SendReply(player, $"Статистика игрока {DB[resetId].DisplayName} сброшена");
                    break;
                ...
                case "resetall":
                    ...
                    foreach (var userId in DB.Keys.ToList())
                    {
                        ResetPlayerStats(userId);
                    }
                    SaveDataBase();
                    SendReply(player, "Статистика всех игроков сброшена");
                    break;

                default:
                    SendReply(player, "Неизвестная команда. Используйте /stats.admin для просмотра списка команд");
                    break;
            }
        }

        private void ResetPlayerStats(ulong userId)
        {
            if (!DB.ContainsKey(userId)) return;

            var displayName = DB[userId].DisplayName;
            var isConnected = DB[userId].IsConnected;
            DB[userId] = new DBSettings
            {
