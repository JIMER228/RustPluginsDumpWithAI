...
{
    [Info("IQControllerSpawnCars", "Mercury", "1.0.3")]
    ...
    class IQControllerSpawnCars : RustPlugin
    {
        ...
        private void OnServerInitialized()
        {
            ...
            foreach (BaseEntity entity in UnityEngine.GameObject.FindObjectsOfType<BaseEntity>())
            {
                if (entity != null && 
                    entity.ShortPrefabName.Contains("module_car") && 
                    entity.GetComponentInChildren<VehicleModuleEngine>()?.GetContainer()?.inventory.itemList.Count == 0)
                {
        ...
        #region Metods
        private void FuelSystemSpawned(IFuelSystem FuelSystem)
        {
            ...
            NextTick(() =>
            {
                var fuelEntity = FuelSystem as BaseEntity;
                if (fuelEntity == null) return;
                
                var storageEntity = fuelEntity.GetComponent<StorageContainer>();
                if (storageEntity == null || storageEntity.inventory == null) return;
                
                Item FuelItem = ItemManager.CreateByName("lowgradefuel", FuelAmount);
                if (FuelItem == null) return;
                FuelItem.MoveToContainer(storageEntity.inventory);
            });
        }
