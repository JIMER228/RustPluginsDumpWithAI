...
{
    [Info("HackCrateSettings", "CRACED", "1.0.1")]
    public class HackCrateSettings : RustPlugin
    {
        ...
        {
            if (hackList.ContainsKey((uint)entity.net.ID.Value))
                hackList.Remove((uint)entity.net.ID.Value);
        }
        #endregion
        Dictionary<uint, ulong> hackList = new Dictionary<uint, ulong>();
        object CanHackCrate(BasePlayer player, HackableLockedCrate crate)
        {
            if (player == null || crate == null) return null;
            crate.hackSeconds = 900 - GetTime(player.userID);
            if(cfg.hackOwner) if(!hackList.ContainsKey((uint)crate.net.ID.Value)) hackList.Add((uint)crate.net.ID.Value, player.userID);
            return null;
        }  
        object CanLootEntity(BasePlayer player, StorageContainer container)
        {
            ...
            if (!hackList.TryGetValue((uint)container.GetEntity().net.ID.Value, out ownerId)) return null;
            var owner = BasePlayer.FindByID(ownerId);
            if(owner == null || !owner.IsConnected)
            {
                hackList.Remove((uint)container.GetEntity().net.ID.Value);
                return null;
            }
