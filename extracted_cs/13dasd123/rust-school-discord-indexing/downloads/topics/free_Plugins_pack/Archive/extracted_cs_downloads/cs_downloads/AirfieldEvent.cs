...
{
    [Info("AirfieldEvent", "Fruster", "1.4.6")]
    ...
    class AirfieldEvent : CovalencePlugin
    {
        ...
        [PluginReference] Plugin Kits, SimpleLootTable, ZoneManager, Notify;
        ...
        class ConfigData
        {
            ...
            [JsonProperty("BradleyAPC spawn chance (0 - 100%)")]
        ...
        private void AddBradleyAPC(int radius)
        {
            if (Configuration.customLanding)
            {
                Debug.LogWarning("Adding bradleyAPC is not available for custom places at the moment");
                return;
            }
        ...
        {
            if (!chinookDelivery) return;

            BaseMountable[] seats = chinookDelivery.GetComponentsInChildren<BaseMountable>();
            ...
            chinookDelivery.transform.position = new Vector3(0, -1500, 0);

            string prefab = "assets/prefabs/tools/c4/explosive.timed.deployed.prefab";
            var c4 = GameManager.server.CreateEntity(prefab, chinookDelivery.transform.position);
            c4.Spawn();
            var explosive = c4 as TimedExplosive;
            explosive.SetDamageScale(10);
            explosive.SetFuse(0.01f);

            timer.Once(1f, () => { if (chinookDelivery) chinookDelivery.Kill(); });
        }
        ...
        public class CollisionListener : MonoBehaviour
        {
            ...
            private void OnTriggerEnter(Collider collider)
            {
                ...
                if (!useNotify)
                    ...
                    airfieldEvent.NotifyMessage(player, typeEnterMessage, enterMessage);
