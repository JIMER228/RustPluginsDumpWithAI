...
namespace Oxide.Plugins
{
    ...
    internal class Weather : RustPlugin
    {
        ...
        private void StartPreset(string name)
        {
            ...
            PresetCoroutine = SetPreset(name.Contains(",") ? GetPreset(name) : _config.GetPresetByName(name));
        ...
        {
            if (_config.SunriseTime <= Env.time && _config.SunsetTime > Env.time)
                OnSunrise();
            else
                OnSunset();  
        }
        ...
        {
            if (_config.ConsoleNewWeatherNotify)
        ...
        private void ShowUIPreset(BasePlayer player, string name)
        {
            ...
            UI.Panel(ref container, ".bg", ".preset.bg", ".preset.bg", aMin: "0.5 0.5", aMax: "0.5 0.5", oMin: "-90 -280", oMax: "190 280", bgColor: "0.15 0.15 0.15 0.65", material: "assets/content/ui/binocular_overlay.mat");
        ...
        private void ShowUIPresets(BasePlayer player, string selectedName)
        {
            ...
            UI.Panel(ref container, ".setup.bg", ".presets.bg", ".presets.bg", oMax: $"0 {(_config.UseTimeControle ? -130 : -80)}", bgColor: "0 0 0 0");
            ...
            {
                UI.Label(ref container, ".scroll.bg", aMin: "0 1", aMax: "1 1", oMin: $"10 {posY}", oMax: $"-10 {posY + 24}", text: check.Name, color: _data.PresetName == check.Name ? "0.451 0.5529 0.2706 1" : selectedName == check.Name ? "1 1 1 1" : "1 1 1 0.35");
                
                UI.Button(ref container, ".scroll.bg", aMin: "0 1", aMax: "1 1", oMin: $"10 {posY}", oMax: $"-10 {posY + 24}", text: "", color: _data.PresetName == check.Name ? "0.451 0.5529 0.2706 1" : selectedName == check.Name ? "1 1 1 1" : "1 1 1 0.35", command: selectedName == check.Name ? "" : $"UI_W SELECTPRESET {check.Name}");

                UI.Button(ref container, ".scroll.bg", aMin: "0 1", aMax: "0 1", oMin: $"7 {posY + 7}", oMax: $"17 {posY + 17}", sprite: "assets/icons/close.png", bgColor: "1 0 0 1", command: $"UI_W REMOVEPRESET {selectedName} {check.Name}");

                if (_data.PresetName != check.Name)
                    UI.Button(ref container, ".scroll.bg", aMin: "1 1", aMax: "1 1", oMin: $"-23 {posY + 5}", oMax: $"-9 {posY + 19}", sprite: "assets/icons/check.png", bgColor: "0 1 0 1", command: $"UI_W SETPRESET {selectedName} {check.Name}");
        ...
        { 
            ...
            UI.Panel(ref container, ".bg", ".setup.bg", ".setup.bg", aMin: "0.5 0.5", aMax: "0.5 0.5", oMin: "-300 -250", oMax: "-100 250", bgColor: "0.15 0.15 0.15 0.65", material: "assets/content/ui/binocular_overlay.mat");

            UI.Label(ref container, ".setup.bg", aMin: "0 1", aMax: "1 1", oMin: "5 -25", oMax: "-5 -5", text:"Use custom schedule:", align:TextAnchor.MiddleLeft);
            UI.Label(ref container, ".setup.bg", aMin: "0 1", aMax: "1 1", oMin: "5 -25", oMax: "-5 -5", text:_config.UseCustomSchedule ? "<color=green>ON</color>" : "<color=red>OFF</color>", align:TextAnchor.MiddleRight);
            UI.Button(ref container, ".setup.bg", aMin: "0 1", aMax: "1 1", oMin: "5 -25", oMax: "-5 -5", text: $"", command: "UI_W CHANGECUSTOMSCHEDULE");
            
            UI.Label(ref container, ".setup.bg", aMin: "0 1", aMax: "1 1", oMin: "5 -50", oMax: "-5 -30", text:"Use time control:", align:TextAnchor.MiddleLeft);
            UI.Label(ref container, ".setup.bg", aMin: "0 1", aMax: "1 1", oMin: "5 -50", oMax: "-5 -30", text:_config.UseTimeControle ? "<color=green>ON</color>" : "<color=red>OFF</color>", align:TextAnchor.MiddleRight);
            UI.Button(ref container, ".setup.bg",aMin: "0 1", aMax: "1 1", oMin: "5 -50", oMax: "-5 -30", text: $"", command: "UI_W CHANGEUSETIMECONTROLE");
  
            UI.Label(ref container, ".setup.bg",aMin: "0 1", aMax: "1 1", oMin: "5 -75", oMax: "-5 -55",text:"Use skip night:", align:TextAnchor.MiddleLeft);
            UI.Label(ref container, ".setup.bg",aMin: "0 1", aMax: "1 1", oMin: "5 -75", oMax: "-5 -55", text:_config.UseSkipNight ? "<color=green>ON</color>" : "<color=red>OFF</color>", align:TextAnchor.MiddleRight);
            UI.Button(ref container, ".setup.bg",aMin: "0 1", aMax: "1 1", oMin: "5 -75", oMax: "-5 -55", text: $"", command: "UI_W CHANGESKIPNIGHT");
        ...
        private void ShowUIBG(BasePlayer player)
        {
            ...
            UI.Panel(ref container, ".bg", bgColor: "0 0 0 0", material: "assets/content/ui/uibackgroundblur-ingamemenu.mat");
        ...
        private void ShowUICustomSchedule(BasePlayer player)
        {
            ...
            UI.Panel(ref container, ".bg", ".custom.schedule.bg", ".custom.schedule.bg", aMin: "0.5 0.5", aMax: "0.5 0.5", oMin: "-460 -250", oMax: "-310 250", bgColor: "0.15 0.15 0.15 0.65", material: "assets/content/ui/binocular_overlay.mat");
        ...
        private class Configuration
        {
            ...
            [JsonProperty("New weather present notify in the console [true/false]")]
            public bool ConsoleNewWeatherNotify = true;
            
            [JsonProperty("Sunrise time")]
            public float SunriseTime = 7.33f;

            [JsonProperty("Sunset time")]
            public float SunsetTime = 19.7f;
            ...
            public static void Label(ref CuiElementContainer container, string parent, string name = null, string destroy = null, string aMin = "0 0", string aMax = "1 1", string oMin = "0 0", string oMax = "0 0", string text = null, int fontSize = 16, string color = "1 1 1 1", TextAnchor align = TextAnchor.MiddleCenter, string outlineDistance = null, string outlineColor = "0 0 0 1", VerticalWrapMode wrapMode = VerticalWrapMode.Truncate, string font = "robotocondensed-regular.ttf") =>
                container.Add(new CuiElement
                {
                    ...
                    Components =
                    {
                        ...
                        outlineDistance == null ? new CuiOutlineComponent { Distance = "-0.5 -0.5", Color = "0 0 0 1" } : new CuiOutlineComponent { Distance = outlineDistance, Color = HexToRustFormat(outlineColor) },
                    },
                });
