...
public class LSkins : RustPlugin
{
    ...
    {
        if (!_playerData.TryGetValue(player.userID.Get(), out var weapons)) return;
        ...
        for (var i = 0; i < 12; i++)
        {
            ...
            cont.Add(new CuiElement()
            {
                ...
                Components =
                {
                    ...
                    {
                        Png = GetImage($"LSKINLOAD({Version})" + weapon + "_" + tSkinsList.ToList()[i].Key)
                    },
            ...
            if (_playerData.TryGetValue(player.userID.Get(), out weapons) && weapons._defaultSkins.ContainsKey(weapon) &&
    ...
    private void ConsoleCommand(ConsoleSystem.Arg arg)
    {
        ...
        switch (arg.Args[0])
        {
            ...
            case "setizbranoe":
                EffectNetwork.Send(sound1, player.Connection);
                if (!_playerData.TryGetValue(player.userID.Get(), out weapons)) return;
                weapon = arg.Args[1];
    ...
    private bool ChangeSkin(BasePlayer player)
    {
        ...
        switch (Cfg.Market.WhoMarket)
        {
            case "Scrap":
            {
               var scrap = player.inventory.containerMain.itemList.Concat(player.inventory.containerBelt.itemList).FirstOrDefault(p =>
            ...
            case "ServerRewards":
            {
                var checkPoint = ServerRewards?.Call("CheckPoints", player.userID.Get());
                if (checkPoint == null)
                {
                    return false;
                }
                ...
                {
                    ServerRewards?.Call("TakePoints", player.userID.Get(), Cfg.Market.costChange);
                }
            ...
            case "Economics":
            {
                double checkPoint = (double)Economics?.Call("Balance", player.userID.Get())!;
                ...
                else
                {
                    ...
                    Economics?.Call("Withdraw", player.userID.Get(), (double)Cfg.Market.costChange);
                }
    ...
    private bool OpenPay(BasePlayer player)
    {
        ...
        switch (Cfg.Market.WhoMarket)
        {
            ...
            case "ServerRewards":
            {
                ...
                {
                    ServerRewards?.Call("TakePoints", player.userID.Get(), Cfg.Market.costOpen);
                }
            ...
            case "Economics":
            {
                ...
                {
                    Economics?.Call("Withdraw", player.userID.Get(), (double)Cfg.Market.costOpen);
                }
    ...
    private void OnItemAddedToContainer(ItemContainer container, Item item)
    {
        ...
        if (!_playerData.TryGetValue(player.userID.Get(), out var playerData)) return;
    ...
    private void OnItemCraftFinished(ItemCraftTask task, Item item, ItemCrafter craft)
    {
        ...
        if (!this._playerData.TryGetValue(craft.owner.userID.Get(), out var playerData)) return;
        if (!playerData._defaultSkins.TryGetValue(shortname, out var skinData)) return;
        if (item.skin == skinData.skinId) return;
        if (shortname.Contains("hazmatsuit"))
        {
    ...
    {
        if (_openSkins.ContainsKey(player.userID.Get())) return false;
        return null;
    }
    ...
    private void LoadDataSkins()
    {
        foreach (var cfgWeaponSkin in Interface.Oxide.DataFileSystem.GetFiles("LSkins/Skins/"))
        {
            ...
            if (text2 == "hazmatsuit")
            {
                AddImage($"https://rustapi.top/items/{_apiKey}/hazmatsuit/128.png",
                    $"LSKINLOAD({Version})hazmatsuit_0");
                AddImage($"https://rustapi.top/items/{_apiKey}/hazmatsuit.spacesuit/128.png",
                    $"LSKINLOAD({Version})hazmatsuit_{1}");
                AddImage($"https://rustapi.top/items/{_apiKey}/hazmatsuit.nomadsuit/128.png",
                    $"LSKINLOAD({Version})hazmatsuit_{2}");
                AddImage($"https://rustapi.top/items/{_apiKey}/hazmatsuit.arcticsuit/128.png",
                    $"LSKINLOAD({Version})hazmatsuit_{3}");
                AddImage($"https://rustapi.top/items/{_apiKey}/hazmatsuit.lumberjack/128.png",
                    $"LSKINLOAD({Version})hazmatsuit_{4}");
                AddImage($"https://rustapi.top/items/{_apiKey}/hazmatsuit.diver/128.png",
                    $"LSKINLOAD({Version})hazmatsuit_{5}");
                continue;
            }

            foreach (var keyValuePair in skins)
            {
                if (ImageLibrary.Call<bool>("HasImage", $"LSKINLOAD({Version}){text2}_{keyValuePair.Key}", 0ul) ||
                    _skinIdsRust.Exists(p => p.Contains($"{text2}_{keyValuePair.Key}"))) continue;
                _skinIdsRust.Add($"{text2}_{keyValuePair.Key}");
            }
        }
    }
    ...
    private void OnServerInitialized()
    {
        ...
        if (Cfg.IsHazmat)
        {
            ...
            AddImage($"https://rustapi.top/items/{_apiKey}/hazmatsuit/128.png", $"LSKINLOAD({Version})hazmatsuit_0");
            AddImage($"https://rustapi.top/items/{_apiKey}/hazmatsuit.spacesuit/128.png",
                $"LSKINLOAD({Version})hazmatsuit_{1}");
            AddImage($"https://rustapi.top/items/{_apiKey}/hazmatsuit.nomadsuit/128.png",
                $"LSKINLOAD({Version})hazmatsuit_{2}");
        ...
        timer.Once(1f, () =>
        {
            if (_skinIdsRust.Count <= 0) return;
            _start = Global.Runner.StartCoroutine(LoadSkin());
        });
    }
    ...
    private void Unload()
    {
        ...
        if (_start != null) Global.Runner.StopCoroutine(_start);
        foreach (var basePlayer in BasePlayer.activePlayerList.Where(p => _openSkins.ContainsKey(p.userID.Get())))
        {
                ...
                foreach (var publishedFileQueryDetail in query.response.publishedfiledetails)
                {
                    foreach (var tag in publishedFileQueryDetail.tags)
                    {
                        ...
                        if (!_loadData.ContainsKey(weapon))
                        {
                            ...
                            AddImage($"https://rustapi.top/items/{_apiKey}/{weapon}/128.png",
                                $"LSKINLOAD({Version}){weapon}_{0}");
                            ...
                            AddImage($"https://rustapi.top/skins/{_apiKey}/{skinId}/128.png",
                                $"LSKINLOAD({Version}){weapon}_{skinId}");
    ...
    private void CommandEntity(IPlayer user, string command, string[] args)
    {
        ...
        player.EndLooting();
        if (_openSkins.ContainsKey(player.userID.Get())) return;
        timer.Once(0.5f, () =>
        {
            StartLoot(player);
    ...
    private object CanLootPlayer(BasePlayer looted, BasePlayer looter)
    {
        ...
        if (_openSkins.ContainsKey(looter.userID.Get()))
        {
            return true;
        }
    ...
    private void StartLoot(BasePlayer player)
    {
        ...
        container.GiveUID();
        _openSkins.Add(player.userID.Get(), container);
    ...
    {
        if (!_playerData.TryGetValue(player.userID.Get(), out DataPlayer dataPlayer)) return;
        CuiHelper.DestroyUi(player, Layer);
        CuiHelper.DestroyUi(player, Layer + "-Izbranoe");
    ...
    private void OnPlayerLootEnd(PlayerLoot inventory)
    {
        ...
        if (player == null || !_openSkins.ContainsKey(player.userID.Get())) return;
        var cont = _openSkins[player.userID.Get()];
        var itemc = inventory.containers.Find(p => p.uid == cont.uid);
        if (itemc == null) return;
        itemc.Clear();
        _openSkins.Remove(player.userID.Get());
        itemc.Kill();
        CuiHelper.DestroyUi(player, Layer);
        CuiHelper.DestroyUi(player, Layer + "-Izbranoe");
        CuiHelper.DestroyUi(player, Layer + "LableIzb");
    ...
    {
        if (!_playerData.TryGetValue(player.userID.Get(), out DataPlayer playerDatas))
            _playerData.Add(player.userID.Get(), new DataPlayer());
    }
    ...
    {
        Puts($"Loads skins: " + _skinIdsRust.Count);
        foreach (var args in _skinIdsRust.Select(keyValuePair => keyValuePair.Split('_')))
        {
            if (args[1] == "0")
            {
                AddImage($"https://rustapi.top/items/{_apiKey}/{args[0]}/128.png",
                    $"LSKINLOAD({Version})" + args[0] + "_" + args[1]);
                continue;
            }

            AddImage($"https://rustapi.top/skins/{_apiKey}/{args[1]}/128.png",
                $"LSKINLOAD({Version})" + args[0] + "_" + args[1]);
        }
