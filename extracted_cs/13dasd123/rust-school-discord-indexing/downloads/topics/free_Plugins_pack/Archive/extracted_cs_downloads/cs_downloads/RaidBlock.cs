...
{
	[Info("RaidBlock", "DezLife & Mercury", "1.3.5")]
	public class RaidBlock : RustPlugin
	{
        ...
        private bool FindItemInInventory(BasePlayer player, ulong skinId)
        {
            ...
            try
            {
                ...
                return items.Any(item => item.skin == skinId);
            }
            finally
            {
                Pool.Free(ref items);
        ...
        private void SubscribeHook(bool main, bool raidActions)
        {
            ...
            if (raidActions)
            {
                ...
                if (config.ActionsBlocked.CanUseTeleportHome)
                {
                    Subscribe(nameof(CanTeleportHome));
                }
        ...
        private object CanTeleportHome(BasePlayer player, Vector3 to)
        {
            RaidableZone isRbZone = GetRbZone(to);
            if (isRbZone != null)
            {
                RunEffect(player, "assets/prefabs/locks/keypad/effects/lock.code.denied.prefab");
                return "RAIDBLOCK_ACTION_BLOCKED_ZONE".GetAdaptedMessage(player.UserIDString, isRbZone.UnblockTimeLeft.ToTimeFormat());
            }
            return null;
        }
        ...
        private void UnsubscribeHook(bool main, bool raidActions)
        {
            ...
            if (raidActions)
            {
                ...
                Unsubscribe(nameof(CanTeleport));
                Unsubscribe(nameof(CanTeleportHome));
        ...
        private class Configuration
        {
            public class RaidBlockActionsBlocked
            {
                ...
                [JsonProperty(LanguageEn ? "Forbid teleporting home if he is within the radius of the raid block" : "Запретить телепортироваться домой, если он в радиусе рейдблока")]
                public bool CanUseTeleportHome = true;
            ...
            public class RaidBlockUi
            {
                ...
                public RaidBlockUiSettings InterfaceSettingsVariant1 = new()
                {
                    ...
                    MainTextColor = "0.1921569 0.1928762 0.1921569 1",
                ...
                {
                    BackgroundColor = "0.1921569 0.1921569 0.1921569 1",
                    IconColor = "0.9411765 0.3137255 0.2868762 1",
                    AdditionalElementsColor = "0.9568627 0.3607843 0.2628762 1",
                    MainTextColor = "1 1 1 1",
                    SecondaryTextColor = "1 1 1 0.5019608",
        ...
        // С нивеском что-то
        /// <summary>
        /// - Исправлена выдача рейдблока при убийстве NPC с плагином LootDefender
        /// - Добавлен новый пункт в конфигурацию "Запретить телепортироваться домой, если он в радиусе рейдблока" (Работает только с последней с версией 1.8.7 NTeleportation и выше)
        /// </summary>
		...
		protected override void LoadDefaultMessages()
		{
			lang.RegisterMessages(new Dictionary<string, string>
			{
                ...
                ["RAIDBLOCK_TOOLGUN_REMOVED_FROM_BLACKLIST"] = "The item '<color=#FF8762>{0} ({1})</color>' has been removed from the blacklist",
            ...
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                ["RAIDBLOCK_TOOLGUN_REMOVED_FROM_BLACKLIST"] = "Предмет '<color=#FF8762>{0} ({1})</color>' был удален из черного списка",
