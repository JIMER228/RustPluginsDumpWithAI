...
namespace Oxide.Plugins
{
    ...
    internal class Hud : RustPlugin
    {
        ...
        private class Data
        {
            ...
            public int CustomPlayerCount = -1;
            public bool ManualPlayerCount = false;
            public int PlayerCountOffset = 0;
        ...
        {
            PrintWarning("OnServerInitialized started...");
            ...
            if (_data.FirstRun)
            {
                PrintWarning("First run detected...");
                _data.FirstRun = false;
                SaveData();
            }

            if (_data.CustomPlayerCount == -1)
            {
                PrintWarning("Initializing player count...");
                _data.CustomPlayerCount = BasePlayer.activePlayerList?.Count ?? 0;
                SaveData();
            }

            if (BasePlayer.activePlayerList != null)
            {
                PrintWarning($"Processing {BasePlayer.activePlayerList.Count} active players...");
                foreach (var player in BasePlayer.activePlayerList)
                {
                    if (player != null && !_data.PlayersState.ContainsKey(player.UserIDString))
                    {
                        _data.PlayersState.Add(player.UserIDString, States.Full);
                        PrintWarning($"Added player {player.displayName} to PlayersState");
                    }
                }
            }
            ...
            EventsInit();
            PrintWarning("Events initialized...");

            permission.RegisterPermission(PERM_STREAMER, this);
            permission.RegisterPermission(PERM_ADMIN, this);
            PrintWarning("Permissions registered...");

            if (_config?.MainSetup != null)
            {
                UI.Size = _config.MainSetup.Size / 100f;
                PrintWarning($"UI Size set to {UI.Size}");

                if (!_config.MainSetup.QueuePlayersAppearance?.IsEnable == true)
                    Unsubscribe(nameof(OnConnectionQueue));

                if (!_config.MainSetup.SleepPlayersAppearance?.IsEnable == true)
                {
            ...
            if (BasePlayer.activePlayerList != null)
            {
                PrintWarning("Showing UI to all active players...");
                foreach (var player in BasePlayer.activePlayerList)
                    if (player != null)
                    {
                        PrintWarning($"Showing UI to {player.displayName}");
                        OnPlayerConnected(player);
                    }
            }

            PrintWarning("OnServerInitialized completed.");
        }
        ...
        {
            PrintWarning($"OnPlayerConnected called for {player.displayName}");
            
            if (!_data.PlayersState.ContainsKey(player.UserIDString))
            {
                PrintWarning($"Adding {player.displayName} to PlayersState");
                _data.PlayersState.Add(player.UserIDString, States.Full);
            }

            if (!_data.ManualPlayerCount)
            {
                _data.CustomPlayerCount = BasePlayer.activePlayerList.Count;
                SaveData();
            }
        ...
        {
            if (_data.PlayersState.ContainsKey(player.UserIDString))
                _data.PlayersState.Remove(player.UserIDString);
        ...
        private void PlayerCommand(BasePlayer player, string command, string[] args)
        {
            ...
            if (args.Length == 0)
            {
                SendReply(player, "Usage: /pset [count]");
                return;
            }

            if (int.TryParse(args[0], out int count))
            {
                _data.CustomPlayerCount = count;
                _data.ManualPlayerCount = true;
                SaveData();
                ...
                if (BasePlayer.activePlayerList != null)
                {
                    foreach (var p in BasePlayer.activePlayerList)
                        if (p != null)
                            ShowUIPlayersCounts(p);
                }
                
                SendReply(player, $"Player count changed to {count}.");
            }
            else
            {
                SendReply(player, "Please enter a valid number!");
            }
        }

        [ChatCommand("resetplayer")]
        private void ResetPlayerCommand(BasePlayer player, string command, string[] args)
        {
            if (!permission.UserHasPermission(player.UserIDString, PERM_ADMIN))
            {
                SendReply(player, "You don't have permission to use this command!");
            ...
            SendReply(player, "Player count has been reset to automatic mode.");
        }

        [ChatCommand("pup")]
        private void PlayerUpCommand(BasePlayer player, string command, string[] args)
        {
            if (!permission.UserHasPermission(player.UserIDString, PERM_ADMIN))
            {
                SendReply(player, "You don't have permission to use this command!");
            ...
            if (args.Length == 0)
            {
                SendReply(player, "Usage: /pup [count]");
                return;
            }

            if (int.TryParse(args[0], out int count))
            {
                _data.PlayerCountOffset += count;
                SaveData();
                ...
                SendReply(player, $"Added {count} to displayed player count.");
            }
            else
            {
                SendReply(player, "Please enter a valid number!");
        ...
        [ChatCommand("pdown")]
        private void PlayerDownCommand(BasePlayer player, string command, string[] args)
        {
            if (!permission.UserHasPermission(player.UserIDString, PERM_ADMIN))
            {
                SendReply(player, "You don't have permission to use this command!");
            ...
            if (args.Length == 0)
            {
                SendReply(player, "Usage: /pdown [count]");
                return;
            }
            ...
            if (int.TryParse(args[0], out int count))
            {
                ...
                SendReply(player, $"Subtracted {count} from displayed player count.");
            }
            else
            {
                SendReply(player, "Please enter a valid number!");
        ...
        [ChatCommand("resetoffset")]
        private void ResetOffsetCommand(BasePlayer player, string command, string[] args)
        {
            if (!permission.UserHasPermission(player.UserIDString, PERM_ADMIN))
            {
                SendReply(player, "You don't have permission to use this command!");
            ...
            SendReply(player, "Player count offset has been reset to 0.");
        }

        [ChatCommand("pr")]
        private void PlayerResetCommand(BasePlayer player, string command, string[] args)
        {
            if (!permission.UserHasPermission(player.UserIDString, PERM_ADMIN))
            {
                SendReply(player, "You don't have permission to use this command!");
            ...
            SendReply(player, "Player count has been reset to default mode.");
        }

        [ChatCommand("playercount")]
        private void PlayerCountCommand(BasePlayer player, string command, string[] args)
        {
            int realPlayers = BasePlayer.activePlayerList.Count;
            int fakePlayers = _data.ManualPlayerCount ? _data.CustomPlayerCount : (realPlayers + _data.PlayerCountOffset);
            int fakeCount = fakePlayers - realPlayers;

            SendReply(player, $"Real Players: {realPlayers}\nFake Players: {fakeCount}\nTotal Players: {fakePlayers}");
        }
        ...
        private void ShowUISettings(BasePlayer player)
        {
            ...
            UI.Panel(ref container, ".bgSettings", ".bgSettings.Main", ".bgSettings.Main", "0.5 0.5", "0.5 0.5", -198, -238, 198, 268, "0.9686 0.9216 0.8824 0.0392", "assets/icons/greyout.mat", "assets/content/ui/ui.background.tile.psd", ignoreSize: true);
        ...
        private void ShowUIPlayersCounts(BasePlayer player = null)
        {
            ...
            if (_config.MainSetup.ActivePlayersAppearance.IsEnable)
            {
                int displayCount;
                if (_data.ManualPlayerCount)
                    displayCount = _data.CustomPlayerCount;
                else
                    displayCount = BasePlayer.activePlayerList.Count(x => _config.MainSetup.ActivePlayersAppearance.IsAdminsCount ? !x.IsAdmin : true) + _data.PlayerCountOffset;

                UI.Label(ref container, ".Main.bg", ".ActivePlayers.count", ".ActivePlayers.count", "0 1", "1 1", oXMin: 44, oYMin: -70, oXMax: 0, oYMax: -50, text: displayCount.ToString(), align:TextAnchor.MiddleLeft);
            }
