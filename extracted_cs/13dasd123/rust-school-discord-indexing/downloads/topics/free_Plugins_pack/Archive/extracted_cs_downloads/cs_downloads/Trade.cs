...
namespace Oxide.Plugins
{
    ...
    public class Trade : RustPlugin
    {
        ...
        Dictionary<string, string> Messages = new Dictionary<string, string>() {
            ...
            , {
                "TRADE.HELP", "Используйте комманду <color=purple>/trade \"НИК\"</color> для обмена\nЧто бы принять обмен, введите: <color=purple>/trade yes</color> (или /trade accept)\nЧто бы отказаться от обмена введите: <color=purple>/trade no </color> (или /trade cancel)"
            }
            , {
        ...
        private void cmdTradeAccept(ConsoleSystem.Arg arg)
        {
            var player = arg.Player();
            if (player == null) return;
            CmdChatTrade(player, string.Empty, new string[] { "yes" });
        }

        [ConsoleCommand("trade.cancel")]
        private void cmdTradeCancel(ConsoleSystem.Arg arg)
        {
            var player = arg.Player();
            if (player == null) return;
            CmdChatTrade(player, string.Empty, new string[] { "no" });
        }
