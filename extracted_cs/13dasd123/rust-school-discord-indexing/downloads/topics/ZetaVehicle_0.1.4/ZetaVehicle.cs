...
{
    [Info("ZetaVehicle", "fermens", "0.1.4")]
    ...
    class ZetaVehicle : RustPlugin
    {
        ...
        private class PluginConfig
        {
            ...
            public static PluginConfig DefaultConfig()
            {
                return new PluginConfig()
                {
                    ...
                    {
                        { "background2", "https://gspics.org/images/2025/02/18/Ii0OCJ.png" },
                        { "fon5", "https://gspics.org/images/2025/02/18/Ii0ZoX.png"}
                    },
        ...
        void AddImage(string url, string shortname, ulong skin = 0)
        {
            try
            {
                ...
                {
                    PrintError($"URL изображения для {shortname} пустой");
                    return;
                }

                ImageLibrary?.Call("AddImage", url, shortname, skin);
                PrintWarning($"Запрос на загрузку изображения {shortname} отправлен");
                gettimage(shortname);
            }
            catch (Exception ex)
            {
                PrintError($"Ошибка при добавлении изображения {shortname}: {ex.Message}");
            }
        }
        ...
        void OnServerInitialized()
        {
            ...
            {
                config.prefabs.Add("testridablehorse", new vehicle { prefab = "assets/content/vehicles/horse/ridablehorse2.prefab", displayname = "ЕЗДОВАЯ ЛОШАДЬ", img = "https://gspics.org/images/2020/01/24/5MNpI.png", cooldown = 60, distancespawn = 2f, infinitefuel = false, fuel = 0 });
        ...
        object CanMountEntity(BasePlayer player, BaseMountable baseMountable)
        {
            ...
            {
                if (baseVehicle.OwnerID == player.userID.Get() || player.Team != null && player.Team.members.Any(x => x == baseVehicle.OwnerID)) return null;
        ...
        void gettimage(string name)
        {
            try
            {
                ...
                if (!img.Equals("39274839"))
                {
                    ...
                    {
                        images.Add(name, img);
                        PrintWarning($"Изображение {name} успешно загружено");
                    }
                    return;
                }

                PrintWarning($"Ожидание загрузки изображения {name}...");
                timer.Once(1f, () => gettimage(name));
            }
            catch (Exception ex)
            {
                PrintError($"Ошибка при загрузке изображения {name}: {ex.Message}");
            }
        }
        ...
        string CanSpawn(BasePlayer player, string name)
        {
            ...
            if (vehicles[player.userID.Get()].vehicles.ContainsKey(name)) return config.messages[3];
            if (vehicles[player.userID.Get()].cooldowns.ContainsKey(name))
            {
                DateTime col = vehicles[player.userID.Get()].cooldowns[name];
        ...
        void remove(BaseEntity ent)
        {
            ...
            if (prefab == "ch47.entity") prefab = "ch47";
        ...
        void spawn(BasePlayer player, string prefab)
        {
            ...
            {
                AttackHelicopter helicopter = entity as AttackHelicopter;
                if (helicopter != null)
                {
                    if (che.infinitefuel) helicopter.fuelPerSec = 0;
                    StorageContainer str = GetFuelSystemContainerFromVehicle(helicopter);
                    if (str != null)
                    {
            ...
            if (entity is Minicopter)
            {
                ...
                StorageContainer str = GetFuelSystemContainerFromVehicle(entity.GetComponent<Minicopter>());
                givefuel(str.inventory, che.fuel, che.infinitefuel);
            }
            ...
            vehicles[player.userID.Get()].vehicles.Add(prefab, entity);
            DestroyUI(player);
            UiMSG(player, config.messages[5].Replace("{name}", che.displayname));
        }
        private StorageContainer GetFuelSystemContainerFromVehicle(BaseVehicle baseVehicle)
        {
            var fuelSystem = baseVehicle.GetFuelSystem() as EntityFuelSystem;
            if (fuelSystem == null)
            {
                return null;
            }
...
=> (time.Days == 0 ? string.Empty : FormatDays(time.Days)) + (time.Hours == 0 ? string.Empty : FormatHours(time.Hours)) + (time.Minutes == 0 ? string.Empty : FormatMinutes(time.Minutes)) + ((time.Seconds == 0 || time.Days != 0 || time.Hours != 0) ? string.Empty : FormatSeconds(time.Seconds));
        ...
        {
            if (!active.Contains(player)) opengui(player, vehicles.ContainsKey(player.userID.Get()) ? vehicles[player.userID.Get()].lastpage : 0);
        ...
        private void cmdzetaspawn(ConsoleSystem.Arg arg)
        {
            ...
            if (arg.HasArgs() && !int.TryParse(arg.Args[0], out page) && !arg.Args[0].Equals("True"))
            {
                ...
                {
                    if (!vehicles.ContainsKey(player.userID.Get()) || !vehicles[player.userID.Get()].vehicles.ContainsKey(arg.Args[1]))
                    {
                        UiMSG(player, config.messages[6]);
                        return;
                    }
                    BaseEntity ent = vehicles[player.userID.Get()].vehicles[arg.Args[1]];
            ...
            {
                if (arg.HasArgs() && !arg.Args[0].Equals("True") || !active.Contains(player)) opengui(player, (!arg.HasArgs() || arg.Args[0].Equals("True")) ? vehicles[player.userID.Get()].lastpage : page);
                else DestroyUI(player);
            }
        }
        ...
        {
            if (!vehicles.ContainsKey(player.userID.Get())) vehicles.Add(player.userID.Get(), new MyVehicles());
        }
        ...
        {
            check(player);
            vehicles[player.userID.Get()].lastpage = page;
            destroyUIMSG(player);
            ...
            try
            {
                ...
                if (count > 0)
                {
                    ...
                    foreach (var z in allowed.Skip(max * page).Take(max * (page + 1)))
                    {
                        ...
                        if (images.ContainsKey("fon5"))
                        {
                            backgroundImage = images["fon5"];
                        }
                        
                        string vehicleImage = null;
                        if (images.ContainsKey(z.Key))
                        {
                            vehicleImage = images[z.Key];
                        }

                        if (backgroundImage != null)
                        {
                            LMUI.LoadImage(ref nocontainer, nomainUI, backgroundImage, $"{x} 0.3", $"{x + 0.16f} 0.68", "1 1 1 0.5");
                        }
                        
                        if (vehicleImage != null)
                        {
                            LMUI.LoadImage(ref nocontainer, nomainUI, vehicleImage, $"{x + 0.02f} 0.35", $"{x + 0.14f} 0.6");
                        }

                        LMUI.CreateLabel(ref nocontainer, nomainUI, "1 1 1 0.5", z.Value.displayname, 24, $"{x} 0.59", $"{x + 0.16f} 0.68", TextAnchor.MiddleCenter);
                        if (vehicles[player.userID.Get()].cooldowns.ContainsKey(z.Key) && vehicles[player.userID.Get()].cooldowns[z.Key] > DateTime.Now)
                        {
                            LMUI.CreatePanel(ref nocontainer, nomainUI, "0 0 0 0.8", $"{x} 0.3", $"{x + 0.1602f} 0.681");
                            LMUI.CreateLabel(ref nocontainer, nomainUI, "1 1 1 0.5", FormatTime(vehicles[player.userID.Get()].cooldowns[z.Key] - DateTime.Now), 24, $"{x} 0.34", $"{x + 0.16f} 0.68", TextAnchor.MiddleCenter);
                            LMUI.CreateButton(ref nocontainer, nomainUI, "0 0 0 0.98", config.messages[11], 20, $"{x} 0.3", $"{x + 0.1606f} 0.34", "");
                        }
                        else if (vehicles.ContainsKey(player.userID.Get()) && vehicles[player.userID.Get()].vehicles.ContainsKey(z.Key)) 
                            LMUI.CreateButton(ref nocontainer, nomainUI, "0 0 0 0.98", config.messages[13], 20, $"{x} 0.3", $"{x + 0.1606f} 0.34", "zeta.open remove " + z.Key);
                        else 
                            LMUI.CreateButton(ref nocontainer, nomainUI, "0 0 0 0.98", config.messages[12], 20, $"{x} 0.3", $"{x + 0.1606f} 0.34", "zeta.open " + z.Key);
                        x += 0.18f;
                    }
                }
            ...
            {
                PrintError($"Ошибка при создании UI: {ex.Message}");
            }
        }
