...
{
    [Info("ZetaVehicle", "fermens", "0.2.3")]
            ...
            if (count > 0)
            {
                ...
                foreach (var z in allowed.Skip(max * page).Take(max * (page + 1)))
                {
                    ...
                    if (vehicles[player.userID.Get()].cooldowns.ContainsKey(z.Key) && vehicles[player.userID.Get()].cooldowns[z.Key] > DateTime.Now)
                    {
                        ...
                        //vehicles[player.userID.Get()].cooldowns[z.Key]
                    }
                    else if (vehicles.ContainsKey(player.userID.Get()) && vehicles[player.userID.Get()].vehicles.ContainsKey(z.Key)) LMUI.CreateButton(ref nocontainer, nomainUI, "0 0 0 0.98", config.messages[13], 20, $"{x} 0.3", $"{x + 0.1606f} 0.34", "zeta.open remove " + z.Key);
