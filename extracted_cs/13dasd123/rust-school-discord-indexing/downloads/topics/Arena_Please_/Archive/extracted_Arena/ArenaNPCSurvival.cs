...
{
    [Info("Arena NPC Survival", "k1lly0u", "2.0.12"), Description("NPC Survival event mode for Arena")]
    class ArenaNPCSurvival : RustPlugin, IEventPlugin, IChaosNPCPlugin
    {
        ...
        internal class NPCSpawnSelector : Arena.SpawnSelector
        {
            ...
            internal NPCSpawnSelector(string spawnFile)
            {
               List<Vector3> _spawns = Arena.Instance.Spawns.Call("LoadSpawnFile", spawnFile) as List<Vector3>;
                ...
                {
                    if (NavmeshSpawnPoint.Find(_spawns[i], 20f, out Vector3 position))
        ...
        public class NPCSurvivalEvent : Arena.BaseEventGame
        {
            ...
            {
                if (baseCombatEntity == null || baseCombatEntity is NPCSurvivalBot)
                    return false;
