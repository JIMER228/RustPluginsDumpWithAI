...
namespace Oxide.Plugins
{
    ...
    public class ArenaUI : RustPlugin
    {
        ...
        {
            List<ArenaEx.IEventPlugin> list = Facepunch.Pool.Get<List<ArenaEx.IEventPlugin>>();
        ...
        {
            List<Arena.BaseEventGame> list = Facepunch.Pool.Get<List<Arena.BaseEventGame>>();
        ...
        {
            if (Arena.Instance.ActiveEvents.TryGetValue(eventName, out Arena.BaseEventGame eventGame))
            {
        ...
        {
            _eventCreators.TryGetValue(player.userID, out Arena.EventConfig eventConfig);
        ...
        private void ccmdSaveEvent(ConsoleSystem.Arg arg)
        {
            ...
            if (player.IsAdmin || permission.UserHasPermission(player.UserIDString, Arena.ADMIN_PERMISSION))
            {
                if (!_eventCreators.TryGetValue(player.userID, out Arena.EventConfig eventConfig))
                    return;
