...
{
    [Info("CargoPlaneCrash", "Fruster", "1.3.0")]
    ...
    class CargoPlaneCrash : CovalencePlugin
    {
        ...
        private const string botPrefab = "assets/rust.ai/agents/npcplayer/npcplayertest.prefab";
        ...
        class ConfigData
        {
            ...
            [JsonProperty("Crates amount(spawn after crash)")]
            ...
            [JsonProperty("Airdrops amount(spawn after crash)")]
            ...
            [JsonProperty("NPCs amount(spawn after crash)")]
        ...
        private void OnServerInitialized()
        {
            ...
            airdropsAmount = Configuration.airdropsAmount;
            ...
            if (npcsAmount > 15) npcsAmount = 15;
        ...
        void crashSite()
        {
            if (Configuration.crashMessage != null)
                foreach (BasePlayer player in BasePlayer.activePlayerList)
                    player.ChatMessage(Configuration.crashMessage);

            //Puts(PhoneController.PositionToGridCoord(crashPosition).ToString());
            ...
            spawnItem(botPrefab, passengersAmount, false);
                ...
                if (WaterLevel.GetWaterDepth(hit.point, false, false) < Configuration.spawnDepth)
                {
                    ...
                    if (i < Configuration.displayNamesListNPC.Count)
                        if (Configuration.displayNamesListNPC[i] != null)
                            player.displayName = Configuration.displayNamesListNPC[i];
                    ...
                    ScientistNPC npcBrain = npc.gameObject.GetComponent<ScientistNPC>();

                    if (npcBrain)
                    {
                        npcBrain.damageScale = npcDamageMultiplier;
                        npcBrain.aimConeScale = Configuration.npcAccuracy;
                    }
                    else
                        PrintWarning("This type of npc is no longer supported(it is recommended to choose type number 8)");
        ...
        void spawnItem(string str, int amount, bool effect)
        {
            RaycastHit hit = new RaycastHit();
            ...
            if (buildingBlock)
                if (buildingBlockTimer > 0)
                    if (entity is BuildingBlock)
                        if (Vector3.Distance(player.transform.position, crashPosition) < crashRadius * 2)
                        {
                            NextFrame(() => { if (entity) entity.AdminKill(); });
                            player.ChatMessage("You can't build near the crash site");
                        }
        ...
        void drawDot(BasePlayer player, Vector3 targetPoint, string str)
        {
            RaycastHit hit = new RaycastHit();
        ...
        private void setcrashzonecenter(IPlayer iplayer)
        {
            ...
            if (player && player.IsAdmin)
            {
                ...
                SaveConfig(Configuration);
                player.ChatMessage("Сenter successfully saved " + crashZoneCenter);
        ...
        private void cpc_add_crashzone(IPlayer iplayer, string command, string[] args)
        {
            ...
            if (player && player.IsAdmin)
            {
                ...
                SaveConfig(Configuration);
                player.ChatMessage("Zone was added successfully " + zoneName);
                if (Configuration.useMultipleZones)
                    showcrashzone(player.IPlayer);
                else
