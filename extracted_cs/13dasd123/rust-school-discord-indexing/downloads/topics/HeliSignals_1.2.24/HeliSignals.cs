...
{
    /*ПЛАГИН БЫЛ ПОФИКШЕН С ПОМОЩЬЮ ПРОГРАММЫ СКАЧАНОЙ С https://discord.gg/dNGbxafuJn */ [Info("Heli Signals", "https://discord.gg/dNGbxafuJn", "1.2.19")]
    [Description("Call Patrol Helicopters to your location with custom supply signals.")]
    public class HeliSignals: RustPlugin
    {
        #region Plugin References
        
        [PluginReference] Plugin Friends, Clans, FancyDrop, RewardPlugin, ServerRewards, Economics, Vanish, NoEscape, BotReSpawn, DynamicPVP, XPerience, SkillTree, NpcSpawn;
        ...
        private object CanStackItem(Item item, Item targetItem)
        {
            ...
            if (HeliSkins.Contains(item.skin))
            {
        ...
        private object CanCombineDroppedItem(DroppedItem droppedItem, DroppedItem targetItem)
        {
            ...
            if (HeliSkins.Contains(droppedItem.item.skin))
            {
                // Only act on Heli Signal supply drops
            ...
            if (!isVanilla)
            {
                ...
                if (config.heli.heliConfig.ContainsKey(heliProfile))
                {
                    if (Initiator is AutoTurret)
                    {
                        ...
                        if (config.heli.heliTargetTurret && (UnityEngine.Time.realtimeSinceStartup - heliComp.turretCooldown > config.heli.turretCooldown))
                        {
                            if (!heliComp.isStrafingTurret && (HeliSignalData[heliId].TurretDamage > HeliSignalData[heliId].PlayerDamage))
                            {
                                heliComp.turretCooldown = UnityEngine.Time.realtimeSinceStartup;
        ...
        private object CanHelicopterStrafe(PatrolHelicopterAI heliAI)
        {
            ...
            if (heliComp != null)
            {
                if (heliComp.isRetiring || heliComp.isDying)
                    ...
                    return false;
                else if (UnityEngine.Time.realtimeSinceStartup - heliAI.lastStrafeTime < config.heli.heliConfig[heliComp.heliProfile].StrafeCooldown)
                    return false;
                else
                    return true;
            }
        ...
        private class HeliComponent : MonoBehaviour
        {
            ...
            void Start()
            {
                ...
                turretCooldown = UnityEngine.Time.realtimeSinceStartup;
            ...
            public void PositionCheck()
            {
                ...
                if (UnityEngine.Time.realtimeSinceStartup - lastPositionCheck > 0.2f)
                {
                    lastPositionCheck = UnityEngine.Time.realtimeSinceStartup;
                    if (heli != null || heliAI != null)
                    {
            ...
            public void UpdateTargetList()
            {
                ...
                if (UnityEngine.Time.realtimeSinceStartup - lastUpdateTargets > 1.0f)
                {
                    lastUpdateTargets = UnityEngine.Time.realtimeSinceStartup;
                    if (heliAI.isRetiring)
                    {
                        isRetiring = true;
                        return;
            ...
            public void StrafeThink()
            {
                ...
                if (UnityEngine.Time.realtimeSinceStartup - lastStrafeThink > (config.heli.heliConfig[heliProfile].TimeBetweenRockets / 2))
                {
                    lastStrafeThink = UnityEngine.Time.realtimeSinceStartup;
                    switch (heliAI._currentState)
                    {
                        ...
                        default:
                        {
                            ...
                            if (isStrafingTurret) turretCooldown = UnityEngine.Time.realtimeSinceStartup;
            ...
            public void ReturnHeliToPlayer()
            {
                ...
                if (UnityEngine.Time.realtimeSinceStartup - lastReturnCheck > 5.0f)
                {
                    lastReturnCheck = UnityEngine.Time.realtimeSinceStartup;
