...
{
    [Info("AntiHack", "fox rust by VAMPIR", "3.5.1")]
    public class AntiHack : RustPlugin
    {
        ...
        private static float minObjectsWallHackDistanceCheck = 30.0f;
        private static float maxPlayersWallHackDistanceCheck = 350f;
        private static float maxObjectsWallHackDistanceCheck = 150f;
        private static float tickRate = 0.02f;
        ...
        private void ShootingThroughWallHanlder(BasePlayer attacker, HitInfo info, float timeNow)
        {
            ...
            else
            {
                ...
                ConVar.Server.Log("Log", strMessage);
        ...
        private class HackHandler : MonoBehaviour
        {
            ...
            private void SpeedHackHandler()
            {
                ...
                {
                    if (Plugins.AntiHack.enableFlyHackCar != null) return;
            ...
            private void CrimeHandler(string reason)
            {
                if (Plugins.AntiHack.needBan)
                    ConsoleSystem.Run.Server.Quiet(string.Format("ban {0} {1}", (object)this.player.userID, (object)reason));
                if (!Plugins.AntiHack.needKick)
                    return;
                ConsoleSystem.Run.Server.Quiet(string.Format("kick {0} {1}", (object)this.player.userID, (object)reason));
            }
