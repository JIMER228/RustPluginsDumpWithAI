...
{
    [Info("PlaceAnything", "David", "1.0.63")]
    ...
    {   
        ...
        Dictionary<ulong, string> lastPlaced = new Dictionary<ulong, string>();
        ...
        void OnEntityBuilt(Planner plan, GameObject go)
        {
            ...
            if (!lastPlaced.ContainsKey(player.userID))
            {
                lastPlaced.Add(player.userID, item.name);
            }
            else
            {
                lastPlaced[player.userID] = item.name;
            } 
            ...
            if (item.name != null) 
            {
                if (placeable.ContainsKey(item.name))
                {
                    ...
                    NextTick(() =>
                    {
                        ...
                        if(!placeable.ContainsKey($"{item.name}"))
                        {       
                            //Puts("grabbing item name");
                            item.name = lastPlaced[player.userID];
                        }
                        ...
                        if (EntityScaleManager != null && config.main.resize.ContainsKey(item.name))
                           EntityScaleManager.CallHook("API_ScaleEntity", entity, config.main.resize[item.name]);
                            ...
                            RefundItem(player, item.name);
                            SendReply(player, "<b>Placement Error!</b> Item was placed back to your inventory.");
                        }
        ...
        [ConsoleCommand("placeanything")]
        private void placeanything_cmd(ConsoleSystem.Arg arg)
        {
            ...
            var args = arg.Args;
            if (args.Length < 2) { Puts("wrong syntax (placeanything <steamid> \"<item display name>\")");
                return; }
            ...
            try{
                ...
                var item = ItemManager.CreateByName(placeable[args[1]].BaseItem, 1, placeable[args[1]].SkinID);
                if (item != null)
                {   
                    item.name = args[1];
                    target.GiveItem(item);
                    Puts($"{args[0]} received {args[1]}");
                    return;
                }
            }
            catch{
                Puts("wrong syntax (placeanything steamid \"item display name>\")");
            }
        }
        ...
        {   
            ...
            ulong[] skinId = {
                ...
                2685224593,//tree
                2685224593,
                2906148311, //minicopter
                2783365006, //scrap heli
                2783365542, //RHIB
                2783365060, //Sedan
                2783365250, //rowboat
