...
namespace Oxide.Plugins
{
    ...
    class Convoy : RustPlugin
    {
        ...
        {
            if (baseHelicopter == null || baseHelicopter.net == null) return false;
            return convoyController != null && convoyController.IsConvoyHeli(baseHelicopter.net.ID.Value);
        }
        #endregion API
        ...
        object OnEntityTakeDamage(BaseVehicleModule baseVehicleModule, HitInfo info)
        {
            ...
            if (convoyController.CanConvoyEntityTakeDamage(info, baseVehicleModule))
            {
                ...
                convoyController.StopConvoy(info.InitiatorPlayer);
                return null;
            }
            else return true;
        }
        ...
        object OnEntityTakeDamage(BradleyAPC entity, HitInfo info)
        {
            ...
            if (convoyController.CanConvoyEntityTakeDamage(info, entity))
            {
                if (info.InitiatorPlayer.IsRealPlayer()) convoyController.StopConvoy(info.InitiatorPlayer);
                return null;
            }
            else return true;
        }
        ...
        object OnEntityTakeDamage(ScientistNPC entity, HitInfo info)
        {
            ...
            if (!NpcManager.IsConvoyNpc(entity.displayName)) return null;

            if (convoyController.CanConvoyNpcTakeDamage(info))
            {
                if (!info.InitiatorPlayer.IsRealPlayer()) return null;
                if (entity.isMounted)
                {
                    BaseVehicle baseVehicle = entity.GetMountedVehicle();
                    if (baseVehicle != null && entity.GetMountedVehicle().IsDriver(entity)) return true;
                }
                convoyController.StopConvoy(info.InitiatorPlayer);
                return null;
            }
        ...
        void OnEntityTakeDamage(BaseHelicopter entity, HitInfo info)
        {
            ...
            if (IsConvoyHeli(entity) && info.InitiatorPlayer != null) convoyController.StopConvoy();
        }
        ...
        object OnTurretTarget(AutoTurret turret, ScientistNPC scientistNPC)
        {
            ...
            if (NpcManager.IsConvoyNpc(scientistNPC.displayName) && scientistNPC.isMounted) return true;
            return null;
        }
        ...
        void OnExplosiveThrown(BasePlayer player, BaseEntity entity, ThrownWeapon item)
        {
            ...
            if (convoyController.convoyVehicles.Any(x => x != null && x.baseEntity != null && Vector3.Distance(player.transform.position, x.baseEntity.transform.position) < 25f))
                convoyController.StopConvoy(player);
        }

        void OnCorpsePopulate(ScientistNPC scientistNPC, NPCPlayerCorpse corpse)
        {
            if (scientistNPC == null || corpse == null) return;
        ...
        sealed class ConvoyController : FacepunchBehaviour
        {
            ...
            {
                return convoyHeli != null && convoyHeli.baseHelicopter != null && convoyHeli.baseHelicopter.net != null && convoyHeli.baseHelicopter.net.ID.Value == heliNetId;
            }
            ...
            {
                if (convoyHeli != null && Vector3.Distance(position, convoyHeli.baseHelicopter.transform.position) < 10f) return convoyHeli;
                return null;
            }
            ...
            void CreateConvoy()
            {
                ...
                if (convoySetting.heliOn)
                {
                    ...
                    if (heliConfig != null) ConvoyHeli.CreateHelicopter(heliConfig, convoyPath.currentPath[firstpoint]);
                }
            }
            ...
            void StartConvoy()
            {
                if (IsAnyCrateHacking() || !ConvoyHaveUnlootedCrate()) return;
            ...
            internal void StopConvoy(BasePlayer initiator = null)
            {
                ...
                if (initiator != null && convoyHeli != null)
                    convoyHeli.SetTarget(initiator);
                ...
                Invoke(() =>
                {
                    if (ins.convoyController != null) ZoneController.CreateZone(GetConvoyPosition());
            ...
            void OnDestroy()
            {
                ...
                if (convoyHeli != null) convoyHeli.KillHeli();


                if (convoyMarker != null) convoyMarker.DeleteMarker();
        ...
        sealed class ConvoySedan : ConvoyVehicle
        {
            ...
            internal void InitSedan(SedanConfig sedanConfig, ConvoyPath convoyPath)
            {
                ...
                base.InitVehicle(sedanConfig.npcName, sedanConfig.coordinates, convoyPath);
            ...
            void FixedUpdate()
            {
                ...
                if (currentPoint == convoyPath.currentPath.Count - 1)
                    currentPoint = 0;
                if (convoyPath.round)
                {
                    if (currentPoint >= convoyPath.currentPath.Count - 2) currentPoint = 0;
                }
                else if (currentPoint >= convoyPath.currentPath.Count)
                {
            ...
            void ControlTrottle()
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        SetSpeed(GetSpeed(distance, 100, 11f));
                    }
                }
            }
            ...
            void ControlTurn()
            {
                ...
                Vector3 lhs = global::BradleyAPC.Direction2D(convoyPath.currentPath[currentPoint + 1], basicCar.transform.position);
        ...
        class ConvoyModular : ConvoyVehicle
        {
            ...
            {
                base.InitVehicle(supportModularConfig.npcName, supportModularConfig.coordinates, convoyPath);
            ...
            void ControlTrottle()
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        SetSpeed(GetSpeed(distance, 1.5f, 0.315f));
                    }
                }
            }
        ...
        sealed class ConvoyBradley : ConvoyVehicle
        {
            ...
            internal static ConvoyBradley CreateBradley(int firstPoint, int secondPoint, BradleyConfig bradleyConfig, ConvoyPath convoyPath)
            {
                Vector3 vector3 = convoyPath.currentPath[firstPoint];
                DeleteEntitiesInSpawnPosition(vector3);
                BradleyAPC bradley = GameManager.server.CreateEntity("assets/prefabs/npc/m2bradley/bradleyapc.prefab", vector3, Quaternion.LookRotation(convoyPath.currentPath[secondPoint] - convoyPath.currentPath[firstPoint])) as BradleyAPC;
            ...
            void InitBradley(BradleyConfig bradleyConfig, ConvoyPath convoyPath, int currentPathIndex)
            {
                ...
                base.InitVehicle(bradleyConfig.npcName, bradleyConfig.coordinates, convoyPath);
            ...
            {
                if (followVehicle != null) return;
                if (bradley.currentPathIndex >= convoyPath.currentPath.Count - 3) ins.convoyController.ReverseConvoy();
            ...
            void FixedUpdate()
            {
                if (convoyPath.round && bradley.currentPathIndex >= bradley.currentPath.Count - 1) bradley.currentPathIndex = 1;
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        SetSpeed(GetSpeed(distance, 2000, 200));
                    }
                }
            }
        ...
        abstract class ConvoyVehicle : FacepunchBehaviour
        {
            ...
            protected bool allConvoyStop = true;
            ...
            protected static float GetSpeed(float distance, float maxSpeed, float multiplicator)
            {
                float speed;
                float stopDistance = 10;
                if (distance <= stopDistance) return -1;
                speed = (distance - 5 - stopDistance) * multiplicator;
                if (speed < 0) return 0;
                if (speed > maxSpeed) return maxSpeed;
                return speed;
            }

            protected void InitVehicle(string npcConfigPresetName, List<CoordConfig> coordNPC, ConvoyPath convoyPath)
            {
                baseEntity = GetComponent<BaseEntity>();
                npcConfig = ins._config.NPC.FirstOrDefault(x => x.name == npcConfigPresetName);
                this.coordNPC = coordNPC;
                this.convoyPath = convoyPath;
            ...
            internal void CheckBarriers()
            {
                ...
                foreach (Collider collider in UnityEngine.Physics.OverlapSphere(checkPosition, 3f))
                {
                    ...
                    if (entity is TreeEntity) entity.Kill();
                    else if (ins._config.barriers.Contains(entity.ShortPrefabName) && !ins.convoyController.convoyVehicles.Any(x => x != null && x.baseEntity != null && x.baseEntity.net.ID == entity.net.ID))
                    {
            ...
            internal void StartMoving()
            {
                ...
                if (allConvoyStop) CreatePassengers();
            ...
            protected virtual void CreatePassengers()
            {
                ...
                JObject passengerNPCConfigJObject = NpcManager.GetNpcJObjectConfig(npcConfig, false, true);
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    bool takeWeaponFromNpc = mountPointInfo.isDriver;
                    ScientistNPC scientist = takeWeaponFromNpc ? NpcManager.CreateNpc(driverNPCConfigJObject, baseEntity.transform.position) : NpcManager.CreateNpc(passengerNPCConfigJObject, baseEntity.transform.position);
            ...
            void CreateRoamNpc()
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    Vector3 globalPosition = GlobalPositionDefiner.GetGlobalPosition(baseEntity.transform, location.position.ToVector3());
        ...
        sealed class ConvoyHeli : FacepunchBehaviour
        {
            ...
            internal static void CreateHelicopter(HeliConfig heliConfig, Vector3 position)
            {
                BaseHelicopter heli = GameManager.server.CreateEntity("assets/prefabs/npc/patrol helicopter/patrolhelicopter.prefab", position + new Vector3(0, heliConfig.height, 0), Quaternion.identity) as BaseHelicopter;
                heli.enableSaving = false;
                heli.skinID = 755446;
                heli.Spawn();
                heli.transform.position = position;
                ins.convoyController.convoyHeli = heli.gameObject.AddComponent<ConvoyHeli>();
                heli._maxHealth = heliConfig.hp;
                heli.startHealth = heliConfig.hp;
                ins.convoyController.convoyHeli.InitHelicopter(heliConfig.hp, heliConfig);
            }

            internal void InitHelicopter(float hp, HeliConfig heliConfig)
            {
            ...
            void FixedUpdate()
            {
                ...
                else if (targetEntity.Distance(baseHelicopter.transform.position) > heliConfig.distance)
                {
                    patrolHelicopterAI.SetTargetDestination(targetEntity.transform.position + new Vector3(0, heliConfig.height, 0));
                }
            }
            ...
            {
                ConvoyVehicle convoyVehicle = ins.convoyController.trucks.FirstOrDefault(x => x != null && x.baseEntity != null);
                if (convoyVehicle == null) convoyVehicle = ins.convoyController.convoyVehicles.FirstOrDefault(x => x != null && x.baseEntity != null);
                if (convoyVehicle != null) targetEntity = convoyVehicle.baseEntity;
            }

            internal void KillHeli()
            {
                if (baseHelicopter.IsExists())
                    baseHelicopter.Kill();
            }
        ...
        sealed class ConvoyPath
        {
            ...
            internal static ConvoyPath DefineConvoyPath(int vehicleCount)
            {
                ...
                ConvoyPath convoyPath = new ConvoyPath
                {
                    ...
                    spawnPointIndex = GetConvoySpawnPoint(convoyPathArray, isRound)
                };
            ...
            static int GetConvoySpawnPoint(Vector3[] path, bool round)
            {
                if (round)
                    return UnityEngine.Random.Range(0, path.Length - 1);
                else
                    return 0;
            }
        ...
        sealed class ZoneController : FacepunchBehaviour
        {
            ...
            void CreatePveModeZone()
            {
                ...
                if (ins.convoyController.convoyHeli != null && ins.convoyController.convoyHeli.baseHelicopter != null)
                    helicopters.Add(ins.convoyController.convoyHeli.baseHelicopter.net.ID.Value);
            ...
            IEnumerator MarkerUpdateCounter()
            {
                while (true)
                {
                    ...
                    if (vendingMarker.IsExists())
                    {
                        ...
                        vendingMarker.markerShopName = $"{ins.convoyController.convoySetting.displayName} ({NotifyManager.GetTimeMessage(null, ins.convoyController.GetEventTime())})";
        ...
        public class SupportModularConfig
        {
            ...
            [JsonProperty(en ? "NPC preset" : "Пресет НПС", Order = 4)] public string npcName { get; set; }
            [JsonProperty(en ? "Location of NPCs" : "Расположение NPC", Order = 5)] public List<CoordConfig> coordinates { get; set; }
        }
