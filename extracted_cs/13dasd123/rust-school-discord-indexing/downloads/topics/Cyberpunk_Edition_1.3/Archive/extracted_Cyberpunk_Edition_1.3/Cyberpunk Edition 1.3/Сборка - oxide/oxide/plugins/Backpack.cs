...
{
    [Info("Backpack", "Netrunner", "1.0.1")]
    public class Backpack : RustPlugin
    {
        [PluginReference] private Plugin ImageLibrary;
        static Backpack ins;
        private string _textColor = HexToCuiColor("#ece1d7");
        ...
        public class BackpackBox : MonoBehaviour
        {
            ...
            private static StorageContainer SpawnContainer(BasePlayer player, int size, Vector3 position, ulong ownerid, ulong playerid = 294912)
            {
                ...
                if (container.uid == null) container.GiveUID();
            ...
            if (openedBackpacks.ContainsKey(player.userID))
            {
                ...
                if (target != null && targetContainer != item.GetRootContainer().uid)
                {
                    if (!PermissionService.HasPermission(player.UserIDString, BPIgnoreBlackListed)) if (BlackListed.Contains(target.info.shortname))
                        {
                            SendReply(player, $"<size=12>Данный предмет <color=#B1D6F1>{target.info.displayName.english}</color> не может быть помещен в сундук.</size>");
                            return false;
                        }
                }
                if (openedBackpacks[player.userID].storage.inventory.uid == targetContainer)
                {
                    if (!PermissionService.HasPermission(player.UserIDString, BPIgnoreBlackListed)) if (BlackListed.Contains(item.info.shortname))
                        {
                            SendReply(player, $"<size=12>Данный предмет <color=#B1D6F1>{item.info.displayName.english}</color> не может быть помещен в сундук.</size>");
                            return false;
                        }
                }
        ...
        private void LoadConfigValues()
        {
            ...
            if (GetConfig("Основные настройки", "Какой вид заполнения использовать в UI (Примеры есть в описании плагина на сайте TopPlugin.ru)", ref Type))
            {
                PrintWarning("Конфигурация обновлена! Добавлен пункт: Какой вид заполнения использовать в UI (Примеры есть в описании плагина на сайте TopPlugin.ru)");
                changed = true;
            }
        ...
        {
            ImageLibrary.Call("AddImage", "https://lc-crb.ru/CyberPunk/JBKYIIp.png", "backpackpng");
            LoadConfig();
            LoadConfigValues();
            InitFileManager();
        ...
        void BackpackShow(BasePlayer player, ulong target = 0)
        {
            ...
            CuiHelper.DestroyUi(player, "TextBox");
            ...
            if (backpackSize<7)
            {
                ...
                {
                    Text = {Text = "ХРАНИЛИЩЕ",FontSize = 21,Color = _textColor},
                    RectTransform = {AnchorMin = "1 0",AnchorMax = "1 0",OffsetMin = "-450 325",OffsetMax = "-10 355"}
                }, "Overlay", "Title");
                container.Add(new CuiElement
                {
                    ...
                    Components =
                    {
                        ...
                        {
                            Png = (string) ImageLibrary.Call("GetImage","backpackpng")
                        },
                        new CuiRectTransformComponent
                        {
                            AnchorMin = "0.5 0",AnchorMax = "0.5 0",OffsetMin = "-100 60",OffsetMax = "100 260"
                        }
                    }
                });
                ...
                {
                    Image = {Color = HexToCuiColor("#54534d80"),Material = "assets/icons/greyout.mat"},
                    RectTransform = {AnchorMin = "1 0",AnchorMax = "1 0",OffsetMin = "-450 230",OffsetMax = "-10 320"}
                }, "Overlay", "TextBox");
                container.Add(new CuiLabel
                {
                    Text = {Text = "Хранилище - специальное вампирское место, которое сохранит ваши вещи, даже после смерти, кроме взрывоопасных веществ и компонентов.",Color = _textColor,FontSize = 16},
            ...
            else
            {
                container.Add(new CuiLabel
            {
                Text = {Text = "ХРАНИЛИЩЕ",FontSize = 21,Color = _textColor,Align = TextAnchor.LowerLeft},
                RectTransform = {AnchorMin = "1 0",AnchorMax = "1 0",OffsetMin = "-450 375",OffsetMax = "-10 405"}
            }, "Overlay", "Title");
            container.Add(new CuiPanel
            {
                Image = {Color = HexToCuiColor("#54534d80"),Material = "assets/icons/greyout.mat"},
                RectTransform = {AnchorMin = "1 0",AnchorMax = "1 0",OffsetMin = "-450 290",OffsetMax = "-10 370"}
            }, "Overlay", "TextBox");
            container.Add(new CuiLabel
            {
                Text = {Text = "Тайник - специальный защищенное место, которое сохраняет ваши вещи, даже после смерти, кроме взрывоопасных веществ и компонентов.",Color = _textColor,FontSize = 16},
                RectTransform = {AnchorMin = "0.05 0.15",AnchorMax = "0.95 0.85"}
            }, "TextBox", "Text");
            container.Add(new CuiElement
            {
        ...
        void OnPlayerLootEnd(PlayerLoot inventory)
        {
            ...
            if (player != null) CuiHelper.DestroyUi(player, "TextBox");
             CuiHelper.DestroyUi(player, "Title");
        }
        ...
        void ShowUIPlayer(BasePlayer player, BasePlayer target)
        {
            ...
            if (!permission.UserHasPermission(player.UserIDString, BPPrivilageMainLoot)) return;
       ...
       public bool BackpackHide(ulong userId)
        ...
        Item BuildItem(SavedItem sItem)
        {
            ...
            if (sItem.mods != null) foreach (var mod in sItem.mods) item.contents.AddItem(ItemManager.FindItemDefinition(mod.itemid), mod.amount);
            return item;
        }
        Item BuildWeapon(SavedItem sItem)
        {
            ...
            if (sItem.mods != null) foreach (var mod in sItem.mods) item.contents.AddItem(ItemManager.FindItemDefinition(mod.itemid), 1);
            return item;
        }
        [PluginReference] Plugin Duel;
        bool InDuel(BasePlayer player){
			if (plugins.Find("Duel")) return (bool)Duel?.Call<bool>("IsPlayerOnActiveDuel", player);
