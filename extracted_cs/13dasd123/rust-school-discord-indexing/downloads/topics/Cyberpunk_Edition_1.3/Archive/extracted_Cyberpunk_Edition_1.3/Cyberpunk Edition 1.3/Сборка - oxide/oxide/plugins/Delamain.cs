...
{
    [Info("Delamain", "Netrunner", "1.0.0")]
    public class Delamain : RustPlugin
    {
        #region [Vars]
        [PluginReference] Plugin ImageLibrary, Economics, IQEconomic, ServerRewards;
        private static Delamain plugin;
            ...
            public static PluginConfig DefaultConfig()
            {
                return new PluginConfig()
                {
                    taxi = new TaxiSettings()
                    {
                        ...
                        monument = "compound",
                        permmission = "Delamain.use",
                        discount = new Dictionary<string, int>()
                        {
                            ["Delamain.discount10"] = 10,
                            ["Delamain.discount20"] = 20,
                            ["Delamain.discount30"] = 30
                        }
                    },
        ...
        {
            if (WaterLevel.Test(randomPos + new Vector3(0, 1.3f, 0), false, false)) return false;
        ...
        private object CanMountEntity(BasePlayer player, BaseMountable entity)
        {
            ...
            {
                CreatePayDelamainUI(player, "", 0, GetMsg("AlreadyCall", player));
                return false;
            }
        ...
        private void OnMapMarkerAdded(BasePlayer player, MapNote note)
        {
            ...
            {
                CreatePayDelamainUI(player, "", 0, GetMsg("Cant", player));
                return;
            }

            if (Vector3.Distance(copter.transform.position, hitInfo.point) < 50f)
            {
                CreatePayDelamainUI(player, "", 0, GetMsg("Near", player));
                return;
            }
            ...
            CreatePayDelamainUI(player, (metre / 1000).ToString("0.00"), cost);
        ...
        #region [UI]
        private void CreateCallDelamainUI(BasePlayer player)
        {
        ...
        private void CreatePayDelamainUI(BasePlayer player, string km, int cost, string text = "")
        {
            CuiElementContainer container = new CuiElementContainer();
            container.Add(new CuiElement { Parent = "Overlay", Name = "TaxiMain", Components = { new CuiImageComponent { Color = "0 0 0 0", Material = "assets/content/ui/uibackgroundblur-ingamemenu.mat" }, new CuiRectTransformComponent { AnchorMin = "0 0", AnchorMax = "1 1" }, new CuiNeedsCursorComponent() } });
            UI.CreateButton(ref container, "TaxiMain", "0 0 0 0", "", 0, "0 0", "1 1", "UI_CLOSE_TAXI");
        ...
        private void cmd_UI_PAY_TAXI(ConsoleSystem.Arg arg)
        {
            ...
            if(result != "paid")
            {
                CreatePayDelamainUI(player, "", 0, result);
                return;
            }
