...
namespace Oxide.Plugins
{
    ...
    internal class NpcSpawn : RustPlugin
    {
        ...
        private void AddParentEntity(CustomScientistNpc npc, BaseEntity parent, Vector3 pos) { if (IsCustomScientist(npc) && parent != null) npc.AddParentEntity(parent, pos); }
        ...
        public class CustomScientistNpc : ScientistNPC
        {
            ...
            private void UpdateTick()
            {
                ...
                if (IsRaidState && Foundations.Count == 0)
                {
                    ...
                    else if (IsBasePlayerTarget)
                    {
                        ...
                        if (block.IsExists() && IsTeam((CurrentTarget as BasePlayer).userID, block.OwnerID)) PlayerTarget = block;
                        else
                        {
                            PlayerTarget = null;
                            CurrentRaidTarget = null;
                ...
                if (Config.CanSleep)
                {
                    bool sleep = Query.Server.PlayerGrid.Query(transform.position.x, transform.position.z, 100f, AIBrainSenses.playerQueryResults, x => x.IsPlayer() && !x.IsSleeping()) == 0;
            ...
            internal bool CanTargetPlayer(BasePlayer target) => !(target.IsSleeping() || target.IsWounded() || target._limitedNetworking);
            ...
            {
                if (_isReloadGrenade || CurrentTarget == null || !IsBasePlayerTarget) return false;
            ...
            internal BuildingBlock PlayerTarget { get; set; } = null;
            ...
            internal void AddParentEntity(BaseEntity parent, Vector3 pos)
            {
            ...
            internal bool CanRaidStateMelee()
            {
                ...
                if (CurrentWeapon is BaseMelee) return true;
                return false;
            }
            #endregion States
        }
        ...
        public class CustomScientistBrain : ScientistBrain
        {
            ...
            public override void InitializeAI()
            {
                ...
                SenseRange = Math.Abs(_npc.Config.SenseRange);
                ...
                HostileTargetsOnly = _npc.Config.SenseRange < 0f;
            ...
            public new class RoamState : BasicAIState
            {
                ...
                public override StateStatus StateThink(float delta, BaseAIBrain brain, BaseEntity entity)
                {
                    ...
                    else if (!brain.Navigator.Moving && _npc.Config.RoamRange > 2f) _npc.SetDestination(_npc.GetRandomPos(_npc.HomePosition, _npc.Config.RoamRange - 2f), 2f, BaseNavigator.NavigationSpeed.Slowest);
            ...
            public class RaidState : BasicAIState
            {
                ...
                public override StateStatus StateThink(float delta, BaseAIBrain brain, BaseEntity entity)
                {
                    ...
                    if (distance > 5f && !_npc.IsMounted() && !_npc.StartExplosion(_npc.CurrentRaidTarget))
                    {
                        _npc.SetDucked(false);
        ...
        private object CanEntityTakeDamage(BaseCombatEntity entity, HitInfo info)
        {
            ...
            if (IsCustomScientist(attacker))
            {
                ...
                if (entity is Drone) return true;
                if (entity.OwnerID.IsSteamId())
                {
                    BaseEntity weaponPrefab = info.WeaponPrefab;
