...
namespace Oxide.Plugins
{
    ...
    public class Trade : RustPlugin
    {
        ...
        private static readonly Dictionary<ulong, TradeController> Trades = new Dictionary<ulong, TradeController>();
        ...
        private object OnEntityVisibilityCheck(BaseEntity entity, BasePlayer player, uint rpcId, string debugName, float _)
        {
            if (!MonitoredVisChecks.Contains(rpcId))
                return null;
            var trade = GetTrade(entity.net.ID.Value);
            if (!trade)
                return null;
        ...
        {
            var trade = GetTrade(shop.net.ID.Value);
            if (trade == null)
                return;
        ...
        private void OnPlayerLootEnd(PlayerLoot inventory)
        {
            if (!inventory.entitySource.IsValid() || inventory.entitySource.prefabID != ShopFrontPrefabId)
                return;
            var trade = GetTrade(inventory.entitySource.net.ID.Value);
            if (trade == null)
                return;
            InterruptTrade(trade);
        }
        ...
        private bool IsTradeBox(BaseNetworkable bn) => bn.net != null && Trades.ContainsKey(bn.net.ID.Value);
        ...
        private static TradeController GetTrade(ulong shopFrontId)
        {
            TradeController tradeCtrl;
            return !Trades.TryGetValue(shopFrontId, out tradeCtrl) ? null : tradeCtrl;
        }

        private static ShopFront GetShopFront(ulong id)
        {
            TradeController tradeCtrl;
        ...
        private static ShopFront CreateShopFront(BasePlayer vendorPlayer, BasePlayer customerPlayer)
        {
            ...
            GetTimer().Once(.2f, () =>
            {
                StartLooting(netId.Value, vendorPlayer);
                StartLooting(netId.Value, customerPlayer);
            });
            return shopFront;
        }
        ...
        private static void StartLooting(ulong shopId, BasePlayer player)
        {
