 ...
 * 
 * Team Skill Tree
 * - Ultimate: mobile respawn point. Will function effectively as a sleeping bag with a global cooldown time for teams.
 *      - Configureable cooldown time
 *      - Delayed unlock - a cooldown after the player dies before it can be used by them
 * 
 * Add prestige icons to the scoreboard.
 * 
 * Fishing bait skill?
 * 
 * Damage to bradley 
...
/* 1.5.8
 * Added new permission: skilltree.respec.bypass - removes the cost for respeccing.
 * Fixed an issue with OnWeaponModChange throwing an exception.
 * Added check for virtual recycler.
 * Added command: sttogglebuffs <true/false> - False: Disables buffs globally for all players.
 * Added scroll element to prestige description.
 * Fixed an issue with NextTick and GiveItem exceptions.
 * Added permission requirement for toggling better chat: skilltree.togglebc
 * Added config options for givexp and givesp commands to give xp/sp to offline players when run in console (default false).
 * Added a config option to prestige settings to allow excess accumulated points to persist when ranking up (default false).
 * Added support for wolf2.
 * Added a config option to adjust the spawn position of a new node with Node_Spawn_Chance.
 * Fixed the resurrection ability to prevent button spamming causing more rolls than intended.
 * Fixed an exception from GiveItem.
 */

namespace Oxide.Plugins
{
    [Info("Skill Tree", "imthenewguy", "1.5.8")]
    ...
    class SkillTree : RustPlugin
    {
        ...
        public class Configuration
        {
            ...
            public class WipeUpdate
            {
                ...
                [JsonProperty("Remove players prestige?")]
                public bool remove_prestige = false;
            ...
            public class MiscSettings
            {
                ...
                public class ZoneManagerSettings
                {
                    ...
                    public List<string> NoXPLossZones = new List<string>();

                    [JsonProperty("List of zone ids that will prevent player skills from functioning")]
                    public List<string> NoSkillZones = new List<string>();
                }
            }
        ...
        protected override void LoadDefaultConfig()
        {
            ...
            config.tools_black_white_list_settings.electricalComponentSettings.comp_blacklist = new List<string>() { "generic", "chassis", "glue", "bleach", "ducttape", "sticks", "vehicle.chassis", "vehicle.module", "vehicle.chassis.4mod", "vehicle.chassis.3mod", "vehicle.chassis.2mod", "electric.generator.small" };

            config.buff_settings.primitive_weapons = new List<string>() { "spear.stone", "spear.wooden", "bone.club", "bow.hunting" };

            config.buff_settings.animals = new List<string>() { "boar", "horse", "stag", "chicken", "wolf", "wolf2", "bear", "scarecrow", "polarbear" };
        ...
        const string FileName = "SkillTree.json";
        const string SharedXP = "Shared_XP";
        const string perm_admin = "skilltree.admin";
        const string perm_no_scoreboard = "skilltree.noscoreboard";
        const string perm_teaBonus = "skilltree.teabonus";
        const string perm_respec_bypass = "skilltree.respec.bypass";
        const string perm_toggle_bc = "skilltree.togglebc";
        ...
        void Unload()
        {
            ...
            foreach (var recycler in ModifiedRecyclers)
            {
                try
                {
        ...
        IEnumerator HandlePlayerSaves()
        {
            ...
            Puts($"Saving player data for {pcdData.pEntity.Count} players.");
            var count = 0;
            Saving.Clear();
            ...
            Puts($"Finished saving player data.");
        }
        ...
        public class ScoreboardInfo
        {
            ...
            {
                orderedList.Clear();
                orderedList.AddRange(data.OrderByDescending(x => x.Value.prestige).ThenByDescending(x => x.Value.xp));
            }
        ...
        public bool GetBuffDetails(ulong id, out BuffDetails buffData)
        {
            if (BuffsDisabledGlobally || (BuffsDisabled && DisabledPlayers.Contains(id)))
            {
                buffData = null;
                return false;
            }
        ...
        protected override void LoadDefaultMessages()
        {
            ...
            foreach (var entry in config.trees)
            {
                foreach (var node in entry.Value.nodes)
                {
                    ...
                    permission_descriptions.Add(node.Key + ".permission", node.Value.permissions.description);
                }
            }
            ...
            foreach (var level in config.prestige_settings.levels)
            {
                ...
                if (level.Value.perms == null) continue;
        ...
        {
            AwardXP(player, config.xp_settings.xp_sources.detect_diggable, detector, false, false, "metal_detector");
        }
        ...
        object OnPlayerViolation(BasePlayer player, AntiHackType type, float amount)
        {
            ...
            BuffDetails bd;
            if (!GetBuffDetails(player.userID, out bd) || bd.buff_values.ContainsKey(Buff.Fall_Damage_Reduction)) return null;
            return true;
        }
        ...
        object CanTakeCutting(BasePlayer player, GrowableEntity plant)
        {
            BuffDetails bd;
            float value;
            if (!GetBuffDetails(player.userID, out bd) || !bd.buff_values.TryGetValue(Buff.Harvest_Grown_Yield, out value) || Interface.CallHook("STCanReceiveYield", player, plant) != null) return null;
        ...
        void SetPicker(BasePlayer player)
        {
            ...
            float value;
            BuffDetails bd;
            if (GetBuffDetails(player.userID, out bd) && bd.buff_values.TryGetValue(Buff.Lock_Picker, out value))
            {
        ...
        object OnEntityTakeDamage(BaseCombatEntity entity, HitInfo info)
        {
            ...
            try
            {
                ...
                if (IsTrap(entity) && AttackerIsRealPlayer)
                {
                    errorAt = "TRAP 2";
                    float value;
                    if (!GetBuffDetails(info.InitiatorPlayer.userID, out bd) || !bd.buff_values.TryGetValue(Buff.Trap_Damage_Increase, out value) || config.buff_settings.raid_perk_settings.trap_damage_increase_settings.blacklist.Contains(entity.ShortPrefabName) || !PassRaidableBasesCheck(entity, Buff.Trap_Damage_Increase)) return null;
                ...
                if (player != null)
                {
                    ...
                    if ((!player.userID.IsSteamId() || ConsiderAsNPC(player)) && info.InitiatorPlayer != null)
                    {
                        ...
                        errorAt = "BasePlayer 9";
                        return null;
                    }
                    errorAt = "BasePlayer 10";
                    if (!GetBuffDetails(player.userID, out bd))
                    {
                        return null;
                    }
                    errorAt = "BasePlayer 11";
                    if (bd == null) return null;
                    errorAt = "BasePlayer 12";
                    if (IsTrap(info.Initiator) && bd.buff_values.TryGetValue(Buff.Trap_Damage_Reduction, out value) && !config.buff_settings.raid_perk_settings.trap_damage_reduction_settings.blacklist.Contains(info.Initiator.ShortPrefabName) && PassRaidableBasesCheck(info.Initiator, Buff.Trap_Damage_Reduction))
                    {
                        errorAt = "BasePlayer 13";
                        var reducedValue = 1f - value;
                        if (reducedValue < 0) reducedValue = 0;
                        info.damageTypes.ScaleAll(reducedValue);
                        errorAt = "BasePlayer 14";
                        if (reducedValue > 0) AddRegenDelay(player);
                        HandleBearBuff(player, info, damageType);
                        errorAt = "BasePlayer 15";
                        return null;
                    }
                    errorAt = "BasePlayer 16";
                    var damage = info.damageTypes.GetMajorityDamageType();
                    errorAt = "BasePlayer 17";
                    switch (damage)
                    {
                        ...
                        case Rust.DamageType.Collision:
                            ...
                            if (vehicle == null || !bd.buff_values.TryGetValue(Buff.DriverCollisionResistance, out value)) break;

                            info.damageTypes.Scale(Rust.DamageType.Collision, MathF.Max(0, 1f - value));
                            return null;
                    }
                    errorAt = "BasePlayer 18";
                    var attacker = info.Initiator;
                    if (attacker == null) return null;
                    errorAt = "BasePlayer 19";
                ...
                {
                    errorAt = "BasePlayer 42";
                    if (lootContainer.ShortPrefabName == "trash-pile-1") return null;
                    if (GetBuffDetails(info.InitiatorPlayer.userID, out bd))
                    {
                        errorAt = "BasePlayer 43";
                        if (bd.buff_values.ContainsKey(Buff.Barrel_Smasher) && IsBarrel(lootContainer.ShortPrefabName, true)) info?.damageTypes?.ScaleAll(100f);
                        if (info?.damageTypes?.Total() >= lootContainer?.health)
                        {
                            errorAt = $"BasePlayer 44 - {lootContainer == null} - {lootContainer?.ShortPrefabName == null}";
                            if (bd.buff_values.TryGetValue(Buff.Extra_Scrap_Barrel, out var value) && IsBarrel(lootContainer.ShortPrefabName) && lootContainer.inventory != null && RollSuccessful(value))
                            {
                                errorAt = "BasePlayer 44 - 1";
                                lootContainer.inventory.capacity++;
                                errorAt = "BasePlayer 44 - 2";
                                ...
                                errorAt = "BasePlayer 44 - 3";
                                if (!item.MoveToContainer(lootContainer.inventory)) item.Remove();
                                errorAt = "BasePlayer 44 - 4";
                            }
                            errorAt = "BasePlayer 45";
                            if (bd.buff_values.TryGetValue(Buff.Component_Barrel, out value) && IsBarrel(lootContainer.ShortPrefabName) && RollSuccessful(value))
                            {
                            ...
                            errorAt = "BasePlayer 46";
                            if (bd.buff_values.TryGetValue(Buff.Electronic_Barrel, out value) && IsBarrel(lootContainer.ShortPrefabName) && RollSuccessful(value))
                            {
                            ...
                            errorAt = "BasePlayer 47";
                            List<Item> _containerItems = Pool.Get<List<Item>>();
                            try
                            {
                                errorAt = $"BasePlayer 47 - 1 - {lootContainer == null} - {lootContainer?.inventory == null} - {lootContainer?.inventory?.itemList == null}";
                                
                                errorAt = "BasePlayer 47 - 2";
                                if (bd.buff_values.ContainsKey(Buff.Scavengers_Ultimate) && IsUltimateEnabled(info.InitiatorPlayer, Buff.Scavengers_Ultimate) && lootContainer != null && lootContainer.inventory != null && lootContainer.inventory.itemList != null)
                                {
                                    _containerItems.AddRange(lootContainer.inventory.itemList);
                                    errorAt = "BasePlayer 47 - 3";
                                    foreach (var item in _containerItems)
                                    {
                                        errorAt = "BasePlayer 47 - 4";
                                        ScrapItems(item, lootContainer);
                                    }
                                }
                                errorAt = "BasePlayer 47 - 5";
                            }
                            finally
                            {
                                errorAt += " - Hit the finally.";
                                Pool.FreeUnmanaged(ref _containerItems);
                            }
                        }
                    }
                    errorAt = "BasePlayer 48";

                    return null;
                }
                errorAt = "BasePlayer 49";
                #endregion

                #region BaseMountable

                errorAt = "BasePlayer 50";
                var mountable = entity as BaseMountable;
                if (mountable != null)
                {
                    #region BaseVehicle

                    errorAt = "BasePlayer 51";
                    var vehicle = mountable as BaseVehicle;
                    if (vehicle != null)
                    {
                        errorAt = "BasePlayer 52";
                        var driver = vehicle.GetDriver();
                        if (driver == null)
                        {
                            errorAt = "BasePlayer 53";
                            if (entity.skinID == 444) info.damageTypes.ScaleAll(1f - config.ultimate_settings.ultimate_vehicle.reduce_by);
                            return null;
                        }
                        errorAt = "BasePlayer 54";
                        if (GetBuffDetails(driver.userID, out bd) && bd.buff_values.ContainsKey(Buff.Vehicle_Ultimate))
                        {
            ...
            {
                Puts($"Error: {errorAt}. Exception: {ex.Message}");
            }
        ...
        object OnItemUse(Item item, int amountToUse)
        {
            ...
            BuffDetails bd;
            if (GetBuffDetails(player.userID, out bd) && bd.buff_values.ContainsKey(Buff.Rationer))
            {
        ...
        void OnPlayerRevive(BasePlayer reviver, BasePlayer player)
        {
            ...
            BuffDetails bd;
            if (GetBuffDetails(reviver.userID, out bd) && bd.buff_values.ContainsKey(Buff.Reviver))
            {
        ...
        void OnActiveItemChanged(BasePlayer player, Item oldItem, Item newItem)
        {
            ...
            try
            {
                ...
                if (newItem == null)
                {
                    error = $"Error 2 - MiningUltimate_AutoTrigger_Timers null? {MiningUltimate_AutoTrigger_Timers == null}";
                    Timer timer;
                ...
                error = $"Error 4 - Null checks for newItem. newItem: {newItem == null}. newItem.info: {newItem?.info == null}. newItem.info.shortname: {newItem?.info?.shortname == null}";
            ...
            {
                LogToFile("OnActiveItemChanged_Error", error, this, false, true);
            }
        }
        ...
        void OnNewSave(string filename)
        {
            ...
            foreach (var file in files)
            {
                try
                {
                    allPlayers.Add(FormatUserIDFromPath(file), JsonConvert.DeserializeObject<PlayerInfo>(File.ReadAllText(file)));
                }
                catch { }
            }
            Pool.FreeUnmanaged(ref files);

            if (config.wipe_update_settings.refund_sp_on_wipe)
            {
                foreach (var kvp in allPlayers)
                {
                    try
                    {
                        ...
                        var pointsPerLevel = GetPointsPerLevel(kvp.Key);
                        if (pointsPerLevel * kvp.Value.current_level > kvp.Value.available_points) kvp.Value.available_points = pointsPerLevel * kvp.Value.current_level;
            ...
            if (config.wipe_update_settings.bonus_skill_points)
            {
            ...
            foreach (var pi in allPlayers)
            {
                ...
                {
                    pi.Value.pending_xp_without_bonus = 0;
                    pi.Value.pending_xp_with_bonus = 0;
                }
            }

            if (config.wipe_update_settings.remove_prestige)
            {
                ClearPrestigeGroup();
                foreach (var pi in allPlayers)
                {
                    pi.Value.prestige_level = 0;
                }
            }
        ...
        void OnPlayerHealthChange(BasePlayer player, float oldValue, float newValue)
        {
            ...
            if (GetBuffDetails(player.userID, out var bd) && bd.buff_values.TryGetValue(Buff.Heal_Share, out var value)) HandleHPShare(player, oldValue, newValue, value);
        }
        ...
        object HandleDispenser(ResourceDispenser dispenser, BasePlayer player, Item item, bool bonus = false)
        {
            ...
            {
                int teaYield = GetModifierAmount(player, dispenser, item);
            ...
            if (dispenser.gatherType == ResourceDispenser.GatherType.Tree)
            {
                ...
                float value;
                if (bd.buff_values.TryGetValue(Buff.Woodcutting_Yield, out value) && Interface.CallHook("STCanReceiveYield", player, dispenser.baseEntity, item) == null)
                {
                    var amount = item.amount * (value * gather_modifier) + (TOD_Sky.Instance.IsNight ? item.amount * ((config.xp_settings.night_settings.night_woodcutting_yield_modifier - 1) * gather_modifier) : 0);
                    if (amount > 0.5)
                    {
                ...
                item.amount += tea_addition;

                if (!bonus && bd.buff_values.TryGetValue(Buff.Instant_Chop, out var buffValue) && RollSuccessful((buffValue * luck_modifier)))
                {
                    HandleInstantChop(dispenser, player, item, bd);
                }

                //if (!bonus && bd.buff_values.TryGetValue(Buff.Instant_Chop, out value) && RollSuccessful((value * luck_modifier)))
                //{
                ...
                //        var bonus_amount = Convert.ToInt32(r.amount + (r.amount * (TOD_Sky.Instance.IsNight ? (config.xp_settings.night_settings.night_woodcutting_yield_modifier - 1) * gather_modifier : 0)) + (r.amount * (bd.buff_values.TryGetValue(Buff.Woodcutting_Yield, out value) ? value * gather_modifier : 0)));
            ...
            if (dispenser.gatherType == ResourceDispenser.GatherType.Ore)
            {
                ...
                if (bd.buff_values.TryGetValue(Buff.Mining_Yield, out var buffValue))
                {
                    HandleMiningYield(dispenser, player, item, bd, gather_modifier, buffValue);
                }

                item.amount += nightAmount;
                item.amount += tea_addition;

                if (!bonus && bd.buff_values.TryGetValue(Buff.Instant_Mine, out buffValue) && RollSuccessful((buffValue * luck_modifier)))
                {
            ...
            if (dispenser.gatherType == ResourceDispenser.GatherType.Flesh)
            {
                ...
                if (bd.buff_values.TryGetValue(Buff.Skinning_Yield, out var buffValue))
                {
                    if (Interface.CallHook("STCanReceiveYield", player, dispenser.baseEntity, item) == null)
                    {
                        var amount = item.amount * (buffValue * gather_modifier) + (TOD_Sky.Instance.IsNight ? item.amount * ((config.xp_settings.night_settings.night_skinning_yield_modifier - 1) * gather_modifier) : 0);
                        if (amount > 0.5)
                        {
                            if (amount < 1) amount = 1;
                            item.amount += Convert.ToInt32(amount);
                ...
                if (!isFinalHit && bd.buff_values.ContainsKey(Buff.Instant_Skin) && RollSuccessful((bd.buff_values[Buff.Instant_Skin] * luck_modifier)))
                {
                    foreach (var r in dispenser.containedItems)
                    {
                        if (r.amount < 1) continue;
                        r.amount = GetMultipliedItemAmount(r.itemDef.shortname, Convert.ToInt32(r.amount));
                        var bonus_amount = Convert.ToInt32(r.amount + (r.amount * (TOD_Sky.Instance.IsNight ? (config.xp_settings.night_settings.night_skinning_yield_modifier - 1) * gather_modifier : 0)) + (r.amount * (bd.buff_values.TryGetValue(Buff.Skinning_Yield, out var skinYield) ? skinYield * gather_modifier : 0)));
                        if (r.itemDef.shortname == item.info.shortname) item.amount += bonus_amount;
                        else
                        {
                            //player.GiveItem(ItemManager.CreateByName(r.itemDef.shortname, amount_to_give));
                ...
                if (PassWhitelistCheck(player, tool, GatherSourceType.Skinning)) AwardXP(player, (isFinalHit ? config.xp_settings.xp_sources.SkinHitFinal : config.xp_settings.xp_sources.SkinHit) * xp_modifier, dispenser.baseEntity, false, false, isFinalHit ? "skinning final" : "skinning");
            }
            return null;
        }

        // Credit Nivex
        private int GetModifierAmount(BasePlayer player, ResourceDispenser dispenser, Item item)
        {
            ...
            return (int)(item.amount - (item.amount / (1f + yieldModifier)));
        }
        ...
        void HandleInstantMining(ResourceDispenser dispenser, BasePlayer player, Item item, BuffDetails bd)
        {
            ...
            foreach (var r in dispenser.containedItems)
            {
                ...
                var bonus_amount = Convert.ToInt32(r.amount + (r.amount * (TOD_Sky.Instance.IsNight ? config.xp_settings.night_settings.night_mining_yield_modifier - 1 : 0)) + (r.amount * (bd.buff_values.TryGetValue(Buff.Mining_Yield, out var buffValue) ? buffValue : 0)));
        ...
        void HandleInstantChop(ResourceDispenser dispenser, BasePlayer player, Item item, BuffDetails bd)
        {
            foreach (var r in dispenser.containedItems)
            {
                if (r.amount < 1) continue;
                ...
                r.amount = GetMultipliedItemAmount(r.itemDef.shortname, r.amount);
                var bonus_amount = Convert.ToInt32(r.amount + (r.amount * (TOD_Sky.Instance.IsNight ? config.xp_settings.night_settings.night_woodcutting_yield_modifier - 1 : 0)) + (r.amount * (bd.buff_values.TryGetValue(Buff.Woodcutting_Yield, out var buffValue) ? buffValue : 0)));
                if (!applyBefore) bonus_amount += teaAmount;
                if (r.itemDef.shortname == item.info.shortname) item.amount += bonus_amount;
                else
                {
        ...
        void HandleMiningYield(ResourceDispenser dispenser, BasePlayer player, Item item, BuffDetails bd, float gather_modifier, float buffValue)
        {
            if (Interface.CallHook("STCanReceiveYield", player, dispenser.baseEntity, item) == null)
            {
                var amount = item.amount * (buffValue * gather_modifier) + (TOD_Sky.Instance.IsNight ? item.amount * ((config.xp_settings.night_settings.night_mining_yield_modifier - 1) * gather_modifier) : 0);
                if (amount > 0.5)
                {
                    if (amount < 1) amount = 1;
                    item.amount += Convert.ToInt32(amount);
            ...
            if (dispenser.gatherType == ResourceDispenser.GatherType.Tree)
            {
                ...
                if (bd.buff_values.TryGetValue(Buff.Regrowth, out value) && RollSuccessful(value * luck_modifier))
                {
                    ...
                    if (!alreadySpawned)
                    {
                        ...
                        newTree.Spawn();
                        if (config.notification_settings.chatMessageNotificationSettings.Regrowth_Proc && NotificationsOn(player)) Player.Message(player, lang.GetMessage("RegrowthProc", this, player.UserIDString), config.misc_settings.ChatID);
                    }
                }
                if (!UltimateTriggered && bd.buff_values.ContainsKey(Buff.Woodcutting_Ultimate) && IsUltimateEnabled(player, Buff.Woodcutting_Ultimate) && luck_modifier > 0 && (config.ultimate_settings.ultimate_woodcutting.cooldown_between_uses == 0 || bd.cooldown_Woodcutting_Ultimate < Time.time))
                {
                    ...
                    List<ResourceDispenser> other_trees = Pool.Get<List<ResourceDispenser>>();
        ...
        void CanCatchFish(BasePlayer player, BaseFishingRod fishingRod, Item fish)
        {
            ...
            float value;
            BuffDetails bd;
            if (GetBuffDetails(player.userID, out bd) && bd.buff_values.TryGetValue(Buff.Extra_Fish, out value))
            {
        ...
        void OnFishCatch(Item item, BaseFishingRod rod, BasePlayer player)
        {
            ...
            float luck;
            if (!bd.buff_values.TryGetValue(Buff.Fishing_Luck, out luck) || !RollSuccessful(luck)) return;
        ...
        void OnCollectiblePickup(CollectibleEntity entity, BasePlayer player)
        {
            ...
            BuffDetails bd;
            float buffValue;
            if (GetBuffDetails(player.userID, out bd) && bd.buff_values.TryGetValue(Buff.Harvest_Wild_Yield, out buffValue))
            {
                foreach (var item in entity.itemList)
                {
                    ...
                    item.amount += Mathf.RoundToInt((buffValue * item.amount) + (TOD_Sky.Instance.IsNight ? (config.xp_settings.night_settings.night_harvesting_yield_modifier - 1) * item.amount : 0));
                }
            }
            else
            {
            ...
            if (bd != null && config.loot_settings.harvesting_loot_table_wild.Count > 0 && bd.buff_values.TryGetValue(Buff.Harvesting_Luck, out buffValue) && RollSuccessful(buffValue) && !config.buff_settings.harvesting_luck_settings.blacklist.Contains(entity.ShortPrefabName))
            {
        ...
        void OnItemCraftFinished(ItemCraftTask task, Item item, ItemCrafter crafter)
        {
            ...
            if (GetBuffDetails(player.userID, out bd))
            {
                ...
                if (bd.buff_values.TryGetValue(Buff.Craft_Duplicate, out value) && RollSuccessful(value) && !config.tools_black_white_list_settings.craft_duplicate_blacklist.Contains(item.info.shortname))
                {
                    ...
                    if (ditem != null)
                    {
                        if (config.notification_settings.chatMessageNotificationSettings.Craft_Duplicate_Proc && NotificationsOn(player)) Player.Message(player, string.Format(lang.GetMessage("DuplicateProc", this, player.UserIDString), item.info.displayName.english), config.misc_settings.ChatID);
        ...
        void OnGrowableGathered(GrowableEntity plant, Item item, BasePlayer player)
        {
            ...
            if (!item.info.shortname.Contains("seed") && (!config.xp_settings.ripe_required || plant.State == PlantProperties.State.Ripe))
            {
                ...
                AwardXP(player, config.xp_settings.xp_sources.CollectGrownPlant, plant, false, false, "grown plant");
            }
                
            if (bd != null && bd.buff_values.TryGetValue(Buff.Harvest_Grown_Yield, out var value) && Interface.CallHook("STCanReceiveYield", player, plant, item) == null)
            {
                var quantity = Convert.ToInt32(Math.Round(value * item.amount + (TOD_Sky.Instance.IsNight && config.xp_settings.night_settings.include_grown_harvesting ? (config.xp_settings.night_settings.night_harvesting_yield_modifier - 1) * item.amount : 0), 0, MidpointRounding.AwayFromZero));
        ...
        void HandleGrownHarvestingLuck(GrowableEntity plant, Item item, BasePlayer player, BuffDetails bd)
        {
            ...
            if (bd == null || config.loot_settings.harvesting_loot_table_grown.Count == 0 || !bd.buff_values.TryGetValue(Buff.Harvesting_Luck, out var value) || !RollSuccessful(value)) return;
        ...
        Dictionary<Recycler, RecyclerEfficiency> ModifiedRecyclers = new Dictionary<Recycler, RecyclerEfficiency>();
        public class RecyclerEfficiency
        {
            public float radtownRecycleEfficiency;
            public float safezoneRecycleEfficiency;
            public RecyclerEfficiency(float radtownRecycleEfficiency, float safezoneRecycleEfficiency)
            {
        ...
        public void ResetRecyclerEfficiency(Recycler recycler, RecyclerEfficiency data = null, bool remove = false)
        {
            if (data == null && !ModifiedRecyclers.TryGetValue(recycler, out data)) return;
            recycler.radtownRecycleEfficiency = data.radtownRecycleEfficiency;
            recycler.safezoneRecycleEfficiency = data.safezoneRecycleEfficiency;
            if (remove) ModifiedRecyclers.Remove(recycler);
        }
        ...
        private void OnRecyclerToggle(Recycler recycler, BasePlayer player)
        {
            ...
            if (bd.buff_values.TryGetValue(Buff.Recycler_Efficiency, out var value))
            {
                if (ModifiedRecyclers.TryGetValue(recycler, out var existingData)) ResetRecyclerEfficiency(recycler, existingData, true);
                ModifiedRecyclers.Add(recycler, new RecyclerEfficiency(recycler.radtownRecycleEfficiency, recycler.safezoneRecycleEfficiency));
            ...
            if (bd.buff_values.TryGetValue(Buff.Recycler_Speed, out var speedDecrease))
            {
        ...
        void OnBonusItemDropped(Item item, BasePlayer player)
        {
            BuffDetails bd;
            bool result;
            if (LastMagnetSuccess.TryGetValue(player, out result) && result && GetBuffDetails(player.userID, out bd) && bd.buff_values.ContainsKey(Buff.Loot_Pickup))
            {
        ...
        {
            BuffDetails bd;
            if (GetBuffDetails(player.userID, out bd) && bd.buff_values.TryGetValue(Buff.Loot_Pickup, out var value) && entity.inventory?.itemList != null && entity.inventory.itemList.Count > 0)
            {
        ...
        object OnPlayerAddModifiers(BasePlayer player, Item item, ItemModConsumable consumable)
        {
            ...
            float value;
            if (bd.buff_values.TryGetValue(Buff.Extra_Food_Water, out value))
            {
            ...
            if (bd.buff_values.TryGetValue(Buff.Metabolism_Share, out value)) HandleMetabolismSharing(player, item, consumable, value);
            if (bd.buff_values.TryGetValue(Buff.Tea_Share, out value) && item.info.shortname.Contains("tea")) HandleTeaSharing(player, item, consumable, value);
        ...
        void HandleMetabolismSharing(BasePlayer player, Item item, ItemModConsumable consumable, float Modifier)
        {
            if (player.Team == null) return;
        ...
        object OnPlayerWound(BasePlayer player, HitInfo info)
        {
            ...
            if (bd.buff_values.ContainsKey(Buff.Wounded_Resist) && RollSuccessful(bd.buff_values[Buff.Wounded_Resist]) && (config.buff_settings.Wounded_Resist_Cooldown == 0 || bd.cooldown_Wounded_Resist < Time.time))
            {
        ...
        void OnEntityMounted(BaseMountable entity, BasePlayer player)
        {
            ...
            else if (vehicle is Bike)
            {
                ...
                if (bd.buff_values.TryGetValue(Buff.BikeEnginePower, out var value))
                {
            ...
            else if (vehicle is MotorRowboat)
            {
                ...
                if (bd.buff_values.TryGetValue(Buff.Boat_Fuel_Rate, out var value))
                {
        ...
        {
            BuffDetails bd;
            if (GetBuffDetails(player.userID, out bd) && bd.buff_values.ContainsKey(Buff.Vehicle_Mechanic) && info?.HitEntity is BaseVehicle)
            {
        ...
        void OnPlayerDeath(BasePlayer player, HitInfo info)
        {
            ...
            if (info?.damageTypes?.GetMajorityDamageType() == Rust.DamageType.Fall)
            {
                ...
                else if (config.xp_settings.xp_loss_settings.suicide_death_penalty > 0) LoseXP(player, DeathType.Suicide);
                return;
            }
            ...
            if (info?.InitiatorPlayer != null && PlayerIsReal(info.InitiatorPlayer))
            {
                switch (player.GetType().Name)
                {
                    ...
                    default:
                        ...
                        if (!IsSameTeam(info.InitiatorPlayer, player)) AwardXP(info.InitiatorPlayer, config.xp_settings.xp_sources.PVP, player, false, false, "pvp");
                        if (config.xp_settings.xp_loss_settings.pvp_death_penalty > 0)
                        {
                            ...
                            else LoseXP(player, DeathType.PVP);
                        }
                        break;
                }
            }
            else if (config.xp_settings.xp_loss_settings.pve_death_penalty > 0 && (attacker.IsNpc || attacker is BaseAnimalNPC))
            {
                ...
                else LoseXP(player, DeathType.PVE);
            }
        }
        ...
        {
            BuffDetails bd;
            if ((!config.ultimate_settings.ultimate_medical.prevent_on_suicide || (attacker == null || attacker != player)) && GetBuffDetails(player.userID, out bd) && bd.buff_values.ContainsKey(Buff.Medical_Ultimate) && IsUltimateEnabled(player, Buff.Medical_Ultimate))
            {
        ...
        void OnPlayerDeath(GingerbreadNPC npc, HitInfo info)
        {
            ...
            AwardXP(attacker, config.xp_settings.xp_sources.GingerbreadNPC, npc, false, false, npc.name ?? npc.ShortPrefabName);
        }
        ...
        void OnEntityDismounted(BaseMountable entity, BasePlayer player)
        {
            ...
            else if (vehicle is BaseBoat)
            {
                ...
                BuffDetails bd;
                if (GetBuffDetails(player.userID, out bd) && bd.buff_values.ContainsKey(Buff.Boat_Fuel_Rate))
                {
            ...
            else if (vehicle is Bike)
            {
                var bike = vehicle as Bike;
                if (!GetBuffDetails(player.userID, out var bd)) return;
                if (bd.buff_values.ContainsKey(Buff.BikeEnginePower))
                {
        ...
        {
            BuffDetails bd;
            if (player != null && GetBuffDetails(player.userID, out bd) && bd.buff_values.ContainsKey(Buff.Upgrade_Refund) && RollSuccessful(bd.buff_values[Buff.Upgrade_Refund]))
            {
        ...
        object OnResearchCostDetermine(Item item)
        {
            ...
            BuffDetails bd;
            float value;
            if (player != null && GetBuffDetails(player.userID, out bd) && bd.buff_values.TryGetValue(Buff.Research_Refund, out value) && RollSuccessful(value))
            {
        ...
        public void HandleNewConnection(BasePlayer player)
        {
            ...
            BuffDetails bd;
            if (GetBuffDetails(player.userID, out bd) && bd.buff_values.ContainsKey(Buff.Metabolism_Boost)) IncreaseCalories(player, bd.buff_values[Buff.Metabolism_Boost]);
        ...
        void CanLootEntity(BasePlayer player, LootContainer container)
        {
            ...
            BuffDetails bd;
            if (player != null && GetBuffDetails(player.userID, out bd))
            {
        ...
        [PluginReference]
        private Plugin ImageLibrary, Economics, ServerRewards, EventManager, BotReSpawn, Cooking, UINotify, ZombieHorde, EventHelper, RaidableBases, LootDefender, SkillTreeXPEvent, ZoneManager, VirtualRecycler;
        ...
        {
            if (config.general_settings.level_rewards.IsNullOrEmpty()) return;
            foreach (var level in config.general_settings.level_rewards)
            {
                if (level.Key > level_achieved || level.Value.reward_commands == null) continue;
        ...
        void LoggingOff(BasePlayer player, bool saveData)
        {
            ...
            if (ScoreBoard.data.TryGetValue(player.userID, out var score))
            {
                score.xp = pi.xp;
        ...
        public void AwardXP(BasePlayer player, double value, BaseEntity source = null, bool noMod = false, bool hookAlreadyCalled = false, string source_string = null)
        {
            ...
            if (IsSkillTreeXPEventLoaded && SkillTreeXPEventRunning)
            {
                ...
                if (modifiedXP > 0) value = modifiedXP;
            }
        ...
        void HandleSharedXP(BasePlayer player, double value, BaseEntity source = null, bool noMod = false, string source_string = null)
        {
            ...
            if (player.Team == null || !GetBuffDetails(player.userID, out var bd) || !bd.buff_values.TryGetValue(Buff.XP_Share, out var shareMod)) return;
        ...
        public void AwardXP(ulong userid, double value, string plugin, bool noMod = false)
        {
            ...
            if (player != null && player.IsConnected) AwardXP(player, value, null, noMod, true);
        ...
        void LevelUpNode(BasePlayer player, string tree, string name)
        {
            ...
            NodeInfo ni;
            if (nsi.nodes.TryGetValue(name, out ni))
            {
                ...
                {
                    Player.Message(player, string.Format(lang.GetMessage("FailMinPrestige", this, player.UserIDString), nsi.min_prestige), config.misc_settings.ChatID);
                    return;
                }

                if (ni.min_prestige > 0 && ni.min_prestige > playerData.prestige_level)
                {
                    Player.Message(player, string.Format(lang.GetMessage("FailMinPrestigeNode", this, player.UserIDString), ni.min_prestige), config.misc_settings.ChatID);
                    return;
                }
                ...
                {
                    Player.Message(player, string.Format(lang.GetMessage("FailPreReqNode", this, player.UserIDString), ni.required_skill), config.misc_settings.ChatID);
                    return;
                }

                if (!string.IsNullOrEmpty(ni.excluded_skill) && (playerData.buff_values.ContainsKey(ni.excluded_skill)))
                {
                    Player.Message(player, string.Format(lang.GetMessage("ExcludedNode", this, player.UserIDString), ni.excluded_skill), config.misc_settings.ChatID);
                    return;
                }
                ...
                switch (ni.buffInfo.Key)
                {
                    ...
                    case Buff.Metabolism_Boost: IncreaseCalories(player, bd.buff_values[ni.buffInfo.Key]); break;
                    case Buff.HealthRegen: UpdateRegen(player, bd.buff_values[ni.buffInfo.Key]); break;
                    case Buff.WaterBreathing: UpdateWaterBreathing(player, bd.buff_values[ni.buffInfo.Key]); break;
                    case Buff.InstantUntie: UpdateInstantUntie(player); break;
                    case Buff.Comfort: AddComfortSkill(player, bd.buff_values[ni.buffInfo.Key]); break;
                    case Buff.Human_Workbench: AddWorkbenchSkill(player, Mathf.RoundToInt(Mathf.Clamp(bd.buff_values[ni.buffInfo.Key], 1, 3))); break;
                }
        ...
        float GetBuffModifier(ulong id, Buff buff)
        {
            ...
            BuffDetails bd;
            if (!GetBuffDetails(id, out bd) || !bd.buff_values.ContainsKey(buff)) return 0;
        ...
        void UpdateInstancedData(BasePlayer player)
        {
            ...
            if (ti == null) return;
            BuffDetails bd;
            if (!GetBuffDetails(player.userID, out bd)) buffDetails.Add(player.userID, bd = new BuffDetails());
            foreach (var cfg in config.trees)
            {
                ...
                if (bd.buff_values.TryGetValue(Buff.Comfort, out value)) AddComfortSkill(player, value);
                if (bd.buff_values.TryGetValue(Buff.Human_Workbench, out value)) AddWorkbenchSkill(player, Mathf.RoundToInt(Mathf.Clamp(value, 1, 3)));
        ...
        void SetupPlayer(ulong id, string name = null, bool create = true)
        {
            ...
            PlayerInfo playerData;
            var startingPoints = GetStartingSkillPoints(id.ToString());
            if (!pcdData.pEntity.TryGetValue(id, out playerData))
            {
                ...
                pcdData.pEntity.Add(id, playerData = new PlayerInfo() { xp_hud_pos = config.general_settings.pump_bar_settings.offset_default, xp_drops = config.xp_settings.enable_xp_drop_by_default, logged_off = DateTime.Now, available_points = startingPoints });
            }

            if (playerData.GetTotalSkillPoints < startingPoints) playerData.available_points += startingPoints;
        ...
        void RespecPlayer(BasePlayer player)
        {
            ...
            if (heldItem != null)
            {
                ...
                if (rod != null && TrackedRods.Contains(rod))
                {
                    ResetRod(rod);
                }
            }
        ...
        void SendSkillTreeMenu(BasePlayer player, string tree = null, string selected_node = null)
        {
            ...
            for (int i = 0; i < T1_Nodes.Count; i++)
            {
                ...
                if (node.Value.min_prestige > 0 && playerData.prestige_level < node.Value.min_prestige)
                {
                    container.Add(new CuiPanel
                    {
                    ...
                    }, "SkillTree_panel_1", "SkillTree_Grey_Box");
                    ...
                    container.Add(new CuiElement
                    {
                        ...
                        Parent = "SkillTree_panel_1",
                        Components = {
                        new CuiTextComponent { Text = string.Format(lang.GetMessage("UIReqPresLvl", this, player.UserIDString), node.Value.min_prestige), Font = "robotocondensed-bold.ttf", FontSize = 10, Align = TextAnchor.MiddleCenter, Color = "1 1 1 1" },
                        new CuiOutlineComponent { Color = "0 0 0 0.5", Distance = "1 -1" },
                        new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-27 -27", OffsetMax = "27 27" }
                    }
                    });
                ...
                if (!string.IsNullOrEmpty(node.Value.required_skill) && (!playerData.buff_values.TryGetValue(node.Value.required_skill, out var nodeLevel) || nodeLevel < GetNodeMaxLevel(node.Value.required_skill)))
                {
                    container.Add(new CuiPanel
                    {
                        Image = { Color = "0.3584906 0.3584906 0.3584906 0.8823529" },
                    ...
                    container.Add(new CuiElement
                    {
                        ...
                        Parent = "SkillTree_panel_1",
                        Components = {
                        new CuiTextComponent { Text = $"<color=#f74a04>Requires skill {node.Value.required_skill}</color>", Font = "robotocondensed-bold.ttf", FontSize = 6, Align = TextAnchor.MiddleCenter, Color = "1 1 1 1" },
                        new CuiOutlineComponent { Color = "0 0 0 0.5", Distance = "1 -1" },
                        new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-27 -27", OffsetMax = "27 27" }
                    }
                    });
                }
                else if (!string.IsNullOrEmpty(node.Value.excluded_skill) && playerData.buff_values.ContainsKey(node.Value.excluded_skill))
                {
                    container.Add(new CuiPanel
                    {
                        Image = { Color = "0.3584906 0.3584906 0.3584906 0.8823529" },
                    ...
                    container.Add(new CuiElement
                    {
                        ...
                        Parent = "SkillTree_panel_1",
                        Components = {
                        new CuiTextComponent { Text = lang.GetMessage("UINodeLocked", this, player.UserIDString), Font = "robotocondensed-bold.ttf", FontSize = 6, Align = TextAnchor.MiddleCenter, Color = "1 1 1 1" },
                        new CuiOutlineComponent { Color = "0 0 0 0.5", Distance = "1 -1" },
                        new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-27 -27", OffsetMax = "27 27" }
                    }
                    });
            ...
            for (int i = 0; i < T2_Nodes.Count; i++)
            {
                ...
                if (node.Value.min_prestige > 0 && playerData.prestige_level < node.Value.min_prestige)
                {
                    container.Add(new CuiPanel
                    {
                        Image = { Color = "0.3584906 0.3584906 0.3584906 0.8823529" },
                        RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-27 -27", OffsetMax = "27 27" }
                    }, "SkillTree_panel_5", "SkillTree_Grey_Box");
                    ...
                    container.Add(new CuiElement
                    {
                        ...
                        Parent = "SkillTree_panel_5",
                        Components = {
                        new CuiTextComponent { Text = $"<color=#f74a04>Requires Prestige {node.Value.min_prestige}</color>", Font = "robotocondensed-bold.ttf", FontSize = 10, Align = TextAnchor.MiddleCenter, Color = "1 1 1 1" },
                        new CuiOutlineComponent { Color = "0 0 0 0.5", Distance = "1 -1" },
                        new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-27 -27", OffsetMax = "27 27" }
                    }
                    });
                ...
                else if (!string.IsNullOrEmpty(node.Value.excluded_skill) && playerData.buff_values.ContainsKey(node.Value.excluded_skill))
                {
                    ...
                    container.Add(new CuiElement
                    {
                        ...
                        Parent = "SkillTree_panel_5",
                        Components = {
                        new CuiTextComponent { Text = $"<color=#f74a04>LOCKED</color>", Font = "robotocondensed-bold.ttf", FontSize = 6, Align = TextAnchor.MiddleCenter, Color = "1 1 1 1" },
                        new CuiOutlineComponent { Color = "0 0 0 0.5", Distance = "1 -1" },
                        new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-27 -27", OffsetMax = "27 27" }
                    }
                    });
            ...
            for (int i = 0; i < T3_Nodes.Count; i++)
            {
                ...
                if (node.Value.min_prestige > 0 && playerData.prestige_level < node.Value.min_prestige)
                {
                    container.Add(new CuiPanel
                    {
                        Image = { Color = "0.3584906 0.3584906 0.3584906 0.8823529" },
                        RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-27 -27", OffsetMax = "27 27" }
                    }, "SkillTree_panel_8", "SkillTree_Grey_Box");
            ...
            if (foundNode != null)
            {
                ...
                {
                    nodeDescription += lang.GetMessage(selected_node + ".permission", this, player.UserIDString);
                }

                nodeDescription += AddAdditionalDescription(foundNode.buffInfo.Key, player.UserIDString);
                if (!string.IsNullOrEmpty(foundNode.excluded_skill)) nodeDescription += string.Format(lang.GetMessage("UIExcludeDescription", this, player.UserIDString), foundNode.excluded_skill);
            ...
            container.Add(new CuiElement
            {
                ...
                Components = {
                    ...
                    new CuiOutlineComponent { Color = "0 0 0 0.5", Distance = "1 -1" },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-450.04 -213", OffsetMax = "-284.04 -191" }
                }
            });

            if (config.prestige_settings.levels.ContainsKey(playerData.prestige_level))
            {
                container.Add(new CuiElement
                {
                    Name = "prestigeImg",
                    Parent = "SkillTree",
                    Components = {
                    new CuiImageComponent { Color = "1 1 1 1", ItemId = 1776460938, SkinId = config.prestige_settings.levels[playerData.prestige_level].RankUpPic },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-476 -213", OffsetMax = "-454 -191" }
                }
                });
            }

            if (config.prestige_settings.enabled && config.general_settings.max_player_level > 0 && (config.prestige_settings.show_button_before_level || playerData.current_level >= config.general_settings.max_player_level) && playerData.prestige_level < GetHighestPrestigeLevel)
            {
                container.Add(new CuiPanel
                {
                    CursorEnabled = false,
                    Image = { Color = "0.2358491 0.2358491 0.2358491 0.6176471" },
                    RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-291.36 -213", OffsetMax = "-233.36 -191" }
                }, "SkillTree", "Panel_3082");

                container.Add(new CuiElement
                {
                    Name = "SkillTree_Prestige",
                    Parent = "SkillTree",
                    Components = {
                    new CuiTextComponent { Text = playerData.current_level >= config.general_settings.max_player_level ? lang.GetMessage("UIPrestigeUpButtonHasLevel", this, player.UserIDString) : lang.GetMessage("UIPrestigeUpButton", this, player.UserIDString), Font = "robotocondensed-bold.ttf", FontSize = 10, Align = TextAnchor.MiddleCenter, Color = "1 1 1 1" },
                    new CuiOutlineComponent { Color = "0 0 0 0.5", Distance = "1 -1" },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-291.36 -213", OffsetMax = "-233.36 -191" }
                    }
                });

                container.Add(new CuiButton
                {
                    Button = { Color = "1 1 1 0", Command = $"stincreaseprestigepage {playerData.prestige_level + 1} {playerData.current_level >= config.general_settings.max_player_level}" },
                    Text = { Text = " ", Font = "robotocondensed-regular.ttf", FontSize = 14, Align = TextAnchor.MiddleCenter, Color = "0 0 0 1" },
                    RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-29 -11", OffsetMax = "29 11" }
                }, "SkillTree_Prestige", "SkillTree_PrestigeButton");
            ...
            if (buffDetails.TryGetValue(player.userID, out bd) && bd.buff_values != null)
            {
                ...
                foreach (var buff in bd.buff_values)
                {
                    ...
                    {
                        switch (BuffBuffType[buff.Key])
                        {
                            case BuffType.Percentage: value = $"+{Math.Round(buff.Value / 1 * 100, 2)}%"; break;
                            case BuffType.Seconds: value = $"-{buff.Value} seconds"; break;
                            case BuffType.PerSecond: value = $"+{buff.Value} / second"; break;
                            case BuffType.Slots: value = $"+{buff.Value} slots"; break;
                            case BuffType.Level: value = $"Level {Mathf.RoundToInt(buff.Value)}"; break;
                            default: value = lang.GetMessage("UIEnabled", this, player.UserIDString); break;
                        }

                    }
                    sb.AppendFormat(lang.GetMessage("BuffUIFormat", this, player.UserIDString), lang.GetMessage("UI" + buff.Key.ToString(), this, player.UserIDString), value);
                    //sb.AppendFormat("{0} - <color=#42f105>{1}</color>\n", lang.GetMessage("UI" + buff.Key.ToString(), this, player.UserIDString), value);
                    loopCount++;
                }
                ...
                {
                    sb.AppendFormat(lang.GetMessage("PermUIFormat", this, player.UserIDString), perm.Value ?? perm.Key);
                    //sb.AppendFormat("Perm - <color=#42f105>{0}</color>\n", perm.Value ?? perm.Key);
                    loopCount++;
                }
                ...
                container.Add(new CuiElement
                {
                    Name = "BuffInfoScrollPanel",
                    Parent = "SkillTree_buff_totals",
                    DestroyUi = "BuffInfoScrollPanel",
                    Components =
                {
                    new CuiNeedsCursorComponent(),
                    new CuiImageComponent{ Color = "1 1 1 0" },
                    new CuiRectTransformComponent{ AnchorMin = "0.5 1", AnchorMax = "0.5 1", OffsetMin = "-83 -358.8", OffsetMax = "83 -42.8" }
                }
                });
                ...
                if (loopCount > 33)
                {
                    container.Add(new CuiElement
                    {
                        ...
                        new CuiScrollViewComponent {
                            ...
                            ContentTransform = new CuiRectTransform { AnchorMin = "0 1", AnchorMax = "1 1", OffsetMin = "0 " + (((10.67f * loopCount) + 5) * -1), OffsetMax = "0 0" },
                ...
                container.Add(new CuiElement
                {
                    ...
                    Parent = loopCount > 33 ? "SB" : "BuffInfoScrollPanel",
                    Components = {
                    ...
                    new CuiOutlineComponent { Color = "0 0 0 0.5", Distance = "1 -1" },
                    new CuiRectTransformComponent { AnchorMin = "0.5 1", AnchorMax = "0.5 1", OffsetMin = $"-71.852 {-10.67 - (loopCount * 10.67)}", OffsetMax = "71.852 0" }
                }
                });
            ...
            if (ni.min_prestige > 0)
            {
                container.Add(new CuiElement
                {
                    ...
                    Parent = "SkillTree",
                    Components = {
                    new CuiTextComponent { Text = $"{(ni.min_prestige <= playerData.prestige_level ? lang.GetMessage("MinLevelColHasLevel", this, player.UserIDString) : lang.GetMessage("MinLevelColUnderLevel", this, player.UserIDString))}{string.Format(lang.GetMessage("MinPrestigeString", this, player.UserIDString), ni.min_prestige)}", Font = "robotocondensed-regular.ttf", FontSize = 14, Align = TextAnchor.MiddleCenter, Color = "1 1 1 1" },
                    new CuiOutlineComponent { Color = "0 0 0 0.5", Distance = "1 -1" },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = ni.min_level > 0 && ni.min_points > 0 ? "-180 120.4" : ni.min_level > 0 || ni.min_points > 0 ? "-180 140.4" : "-180 160.4", OffsetMax = ni.min_level > 0 ? "180 200.4" : "180 220.4" }
                }
                });
            }
            ...
            if (Ultimate_node.Key != null && Ultimate_node.Value != null)
            {
                ...
                if (Ultimate_node.Value.min_prestige > 0 && playerData.prestige_level < Ultimate_node.Value.min_prestige)
                {
                    container.Add(new CuiPanel
                    {
                        Image = { Color = "0.3584906 0.3584906 0.3584906 0.8823529" },
                        RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-27 -27", OffsetMax = "27 27" }
                    }, "SkillTree_panel_ultimate", "SkillTree_Grey_Box");
                    ...
                    container.Add(new CuiElement
                    {
                        ...
                        Parent = "SkillTree_panel_ultimate",
                        Components = {
                        new CuiTextComponent { Text = $"<color=#f74a04>Requires Prestige {Ultimate_node.Value.min_prestige}</color>", Font = "robotocondensed-bold.ttf", FontSize = 10, Align = TextAnchor.MiddleCenter, Color = "1 1 1 1" },
                        new CuiOutlineComponent { Color = "0 0 0 0.5", Distance = "1 -1" },
                        new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-27 -27", OffsetMax = "27 27" }
                    }
                    });
                ...
                if (!string.IsNullOrEmpty(Ultimate_node.Value.required_skill) && (!playerData.buff_values.TryGetValue(Ultimate_node.Value.required_skill, out var nodeLevel) || nodeLevel < GetNodeMaxLevel(Ultimate_node.Value.required_skill)))
                {
                    container.Add(new CuiPanel
                    {
                        Image = { Color = "0.3584906 0.3584906 0.3584906 0.8823529" },
                    ...
                    container.Add(new CuiElement
                    {
                        ...
                        Parent = "SkillTree_panel_ultimate",
                        Components = {
                        new CuiTextComponent { Text = $"<color=#f74a04>Requires skill {Ultimate_node.Value.required_skill}</color>", Font = "robotocondensed-bold.ttf", FontSize = 6, Align = TextAnchor.MiddleCenter, Color = "1 1 1 1" },
                        new CuiOutlineComponent { Color = "0 0 0 0.5", Distance = "1 -1" },
                        new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-27 -27", OffsetMax = "27 27" }
                    }
                    });
        ...
        void ConfirmRespec(BasePlayer player, double cost, string tree, string selected_node)
        {
            ...
            {
                PlayerInfo pi;
                if (pcdData.pEntity.TryGetValue(player.userID, out pi)) text += string.Format(lang.GetMessage("RespecMultiplierMsg", this, player.UserIDString), Mathf.Min(pi.respec_multiplier + config.general_settings.respec_multiplier, config.general_settings.respec_multiplier_max) * 100);
            }
        ...
        void DoRespec(ConsoleSystem.Arg arg)
        {
            ...
            RespecPlayer(player);
            SendSkillTreeMenu(player, tree, name);
            Player.Message(player, string.Format(lang.GetMessage("PaidRespec", this, player.UserIDString), bypassCost ? 0 : cost), config.misc_settings.ChatID);
        }
        ...
        void ResetAllDataCommand(BasePlayer player = null, bool fromConsole = true)
        {
            foreach (var p in BasePlayer.activePlayerList)
            {
                OnPlayerDisconnected(p, string.Empty);
            }
        ...
        void ResetSkillsConsoleSingle(ConsoleSystem.Arg arg)
        {
            ...
            if (pcdData.pEntity.ContainsKey(target.userID))
            {
                RespecPlayer(target);
            ...
            PlayerInfo playerData = LoadPlayerData(target.UserIDString);
            if (playerData == null)
            {
        ...
        //        var obj = GetOfflinePlayerInfo(ID.ToString());
        //        if (obj == null)
        //        {
        //            arg.ReplyWith($"No data found for {ID}");
        //            return;
        //        }
        ...
        //        if (config.general_settings.points_per_level * obj.current_level > obj.available_points) obj.available_points = config.general_settings.points_per_level * obj.current_level;
        //        obj.buff_values.Clear();
        //        RemovePerms(ID.ToString());
        //        //kvp.Value.available_points = config.general_settings.points_per_level * kvp.Value.current_level;
        //        Puts($"Finished respeccing data for {ID}");

        //        WriteJson(ID.ToString(), JsonConvert.SerializeObject(obj));
        ...
        {
            var sTStats = new Dictionary<ulong, (string Name, double XP, int PrestigeLevel, int CurrentLevel)>();
        ...
        float GetBuffValue(BasePlayer player, string skill)
        {
            ...
            if (GetBuffDetails(player.userID, out buffData) && buffData.buff_values.TryGetValue(buff, out result)) return result;
        ...
        object[] GetXPStats(ulong id)
        {
            ...
            var result = new object[] { currentLevel, currentXP, currentLevelStartXP, nextLevelStartXP };
            return result;
        }
        ...
        {
            BuffDetails bd;
            if (GetBuffDetails(player.userID, out bd) && bd.buff_values.ContainsKey(Buff.Rationer) && RollSuccessful(bd.buff_values[Buff.Rationer]))
            {
        ...
        private object OnBetterChat(Dictionary<string, object> data)
        {
            ...
            //    {
            //        if (string.IsNullOrEmpty(kvp.Value.better_title_format)) continue;
            //        titleFound = true;
            //        break;
            //    }
            //    if (!titleFound) return null;
            //}
            //var player = (IPlayer)data["Player"];
            //if (permission.UserHasPermission(player.Id, "skilltree.notitles")) return null;
            ...
            //var levelChat = string.Format(config.betterchat_settings.better_title_format, col, config.general_settings.max_player_level > 0 && playerData.current_level > config.general_settings.max_player_level ? config.general_settings.max_player_level : playerData.current_level);

            //var title = playerData.prestige_level > 0 ? string.Format("{0} {1}", GetPrestigeBetterChatTitle(id), levelChat) : levelChat;
        ...
        void OnOvenToggle(BaseOven oven, BasePlayer player)
        {
            ...
            BuffDetails bd;
            if (GetBuffDetails(player.userID, out bd) && bd.buff_values.ContainsKey(Buff.Smelt_Speed))
            {
        ...
        public Dictionary<string, Subscription> subscriptions = new Dictionary<string, Subscription>()
        {
            ...
            [nameof(OnPlayerAddModifiers)] = new Subscription(false, new List<Buff>() { Buff.Extra_Food_Water, Buff.Iron_Stomach, Buff.Extended_Tea_Duration, Buff.Tea_Share, Buff.Metabolism_Share }),
        ...
        void AddRegenDelay(BasePlayer player)
        {
            ...
            BuffDetails bd;
            if (!GetBuffDetails(player.userID, out bd) || !bd.buff_values.ContainsKey(Buff.HealthRegen)) return;
        ...
        {
            if (!buffDetails.TryGetValue(player.userID, out var bd) || !bd.buff_values.ContainsKey(Buff.HealthRegen)) return;
            var gameObject = player.GetComponent<Regen>();
            if (gameObject == null) return;
        ...
        void TrackAnimal(BasePlayer player)
        {
            ...
            animals.RemoveAll(x => x.skinID > 0);
        ...
        void OpenBag(BasePlayer player)
        {
            ...
            BuffDetails bd;
            if (GetBuffDetails(player.userID, out bd) && bd.buff_values.ContainsKey(Buff.ExtraPockets))
            {
                // Handle cooldown
        ...
        void OnTimedExplosiveExplode(TimedExplosive explosive)
        {
            ...
            BuffDetails bd;
            float value;
            if (!GetBuffDetails(player.userID, out bd) || !bd.buff_values.TryGetValue(Buff.Double_Explosion_Chance, out value)) return;
        ...
        void OnEntityKill(BaseEntity entity)
        {
            ...
            if (RemoveContainer(entity as StorageContainer)) return;
        ...
        void RemoveDuds(BaseEntity entity)
        {
            ...
            {
                Duds.Remove(entity as DudTimedExplosive);
            }
        }
        ...
        bool RemoveContainer(StorageContainer container)
        {
            ...
            if (container.inventory == null || !containers.ContainsKey(container.inventory.uid.Value)) return false;
            var p = BasePlayer.activePlayerList.FirstOrDefault(x => x.userID == container.OwnerID);
        ...
        private object OnExplosiveDud(DudTimedExplosive dudTimedExplosive)
        {
            ...
            BuffDetails bd;
            float value;
            if (GetBuffDetails(player.userID, out bd) && bd.buff_values.TryGetValue(Buff.Dudless_Explosive, out value) && RollSuccessful(value) && PassRaidableBasesCheck(dudTimedExplosive, Buff.Dudless_Explosive))
            {
                PlayerInfo pi;
        ...
        void HandleExplosionRadius(BasePlayer player, TimedExplosive timedExplosive)
        {
            ...
            BuffDetails bd;
            float value;
            if (!GetBuffDetails(player.userID, out bd) || !bd.buff_values.TryGetValue(Buff.Explosion_Radius, out value) || !PassRaidableBasesCheck(timedExplosive, Buff.Explosion_Radius)) return;
        ...
        {
            if (ScoreBoard.lastChecked + 60f < Time.time) UpdateScoreBoard(false);
        ...
        void UpdateScoreBoard(bool initialize)
        {
            if (initialize)
            {
                ...
                foreach (var file in Directory.GetFiles(NewDirectory))
                {
                    ...
                    {
                        var userid = FormatUserIDFromPath(file);
                        if (permission.UserHasPermission(userid, perm_no_scoreboard)) continue;
                        var obj = LoadOfflinePlayerInfo(file, false);
                        data.Add(Convert.ToUInt64(userid), new ScoreboardInfo.ScoreInfo(obj.name ?? userid, obj.xp, obj.prestige_level, obj.prestige_level > 0 ? config.prestige_settings.levels.TryGetValue(obj.prestige_level, out var presData) ? presData.RankUpPic : 0 : 0));
                    }
                    catch
                    {
        ...
        private void ScoreBoardPanel(BasePlayer player, int lastElement = 0)
        {
            ...
            for (int i = lastElement; i < lastElement + 15; i++)
            {
                ...
                if (ScoreBoard.orderedList.Count <= i)
                {
                    break;
                }

                var element = ScoreBoard.orderedList.ElementAt(i).Value;
                ...
                container.Add(new CuiElement
                {
                    ...
                    Parent = "ScoreBoardPanel",
                    Components = {
                    new CuiTextComponent { Text = element.name, Font = "robotocondensed-regular.ttf", FontSize = 14, Align = TextAnchor.UpperCenter, Color = "0.9137255 0.8039216 0 1" },
                    new CuiOutlineComponent { Color = "0 0 0 0.5", Distance = "1 -1" },
                ...
                container.Add(new CuiElement
                {
                    ...
                    Parent = "ScoreBoardPanel",
                    Components = {
                    new CuiTextComponent { Text = $"{element.xp:###,###,##0}", Font = "robotocondensed-regular.ttf", FontSize = 14, Align = TextAnchor.UpperLeft, Color = "0 0.8666667 0.8509804 1" },
                    new CuiOutlineComponent { Color = "0 0 0 0.5", Distance = "1 -1" },
                ...
                if (element.pres_skin > 0)
                {
                    container.Add(new CuiElement
                    {
                        ...
                        Parent = $"ScoreXP_{i}",
                        Components = {
                        new CuiImageComponent { Color = "1 1 1 1", ItemId = 1751045826, SkinId = element.pres_skin },
                        new CuiRectTransformComponent { AnchorMin = "1 0.5", AnchorMax = "1 0.5", OffsetMin = "-19 -7", OffsetMax = "-5 7" }
                    }
                    });
                }                
            ...
            if (ScoreBoard.orderedList.Count - 1 > lastEntry)
            {
                container.Add(new CuiElement
                {
                    Name = "rightarrow",
        ...
        void ChangeBoard(ConsoleSystem.Arg arg)
        {
            ...
            else if (startElement > ScoreBoard.orderedList.Count - 1) ScoreBoardPanel(player, ScoreBoard.orderedList.Count - 16);
        ...
        void TriggerMiningUltimateAction(BasePlayer player, bool from_command = true)
        {
            ...
            BuffDetails bd;
            if (GetBuffDetails(player.userID, out bd) && bd.buff_values.ContainsKey(Buff.Mining_Ultimate) && IsUltimateEnabled(player, Buff.Mining_Ultimate))
            {
        ...
        void AttemptResurrection(ConsoleSystem.Arg arg)
        {
            ...
            timer.Once(0.1f, () => SpamProtection.Remove(id));
        ...
        {
            BuffDetails bd;
            if (!GetBuffDetails(player.userID, out bd) || !bd.buff_values.ContainsKey(Buff.Harvester_Ultimate))
            {
        ...
        private object OnCardSwipe(CardReader cardReader, Keycard card, BasePlayer player)
        {
            ...
            BuffDetails bd;
            if (card.accessLevel != cardReader.accessLevel && GetBuffDetails(player.userID, out bd) && bd.buff_values.ContainsKey(Buff.Build_Craft_Ultimate) && IsUltimateEnabled(player, Buff.Build_Craft_Ultimate))
            {
        ...
        {
            if (!buffDetails.TryGetValue(player.userID, out var bd) || !bd.buff_values.ContainsKey(Buff.WaterBreathing)) return;
        ...
        void CallRocketStrike(BasePlayer player)
        {
            ...
            BuffDetails bd;
            if (!GetBuffDetails(player.userID, out bd) || !bd.buff_values.ContainsKey(Buff.Raiding_Ultimate))
            {
        ...
        void HandleSonarBuff(BasePlayer player)
        {
            ...
            BuffDetails bd;
            if (!GetBuffDetails(player.userID, out bd) || !bd.buff_values.ContainsKey(Buff.Sonar)) return;
        ...
        void HandleForagerBuff(BasePlayer player)
        {
            ...
            BuffDetails bd;
            if (!GetBuffDetails(player.userID, out bd) || !bd.buff_values.ContainsKey(Buff.Forager)) return;
        ...
        {
            if (!buffDetails.TryGetValue(player.userID, out var bd) || !bd.buff_values.ContainsKey(Buff.InstantUntie)) return;
        ...
        {
            BuffDetails bd;
            if (!GetBuffDetails(player.userID, out bd) || !bd.buff_values.ContainsKey(Buff.Cooking_Ultimate))
            {
        ...
        void SearchForTraps(BasePlayer player)
        {
            ...
            BuffDetails bd;
            float value;
            if (!GetBuffDetails(player.userID, out bd) || !bd.buff_values.TryGetValue(Buff.Trap_Spotter, out value))
            {
        ...
        internal class Item_SplitItem_Patch
        {
            ...
            private static bool Prefix(MetalDetectorSource __instance, Vector3 pos)
            {
                ...
                if (!Instance.GetBuffDetails(player.userID, out var buffData) || !buffData.buff_values.TryGetValue(Buff.Efficient_Digger, out var value) || !Instance.RollSuccessful(value)) return true;
        ...
        internal class OnFullyDug_Patch
        {
            ...
            private static bool Prefix(BaseDiggableEntity __instance, BasePlayer player)
            {
                ...
                if (buffData.buff_values.TryGetValue(Buff.Metal_Detector_Luck, out var value) && Instance.RollSuccessful(value))
                {
                ...
                if (buffData.buff_values.TryGetValue(Buff.Magnetic_Digger, out value) && Instance.RollSuccessful(value))
                {
                    var dItem = __instance.SpawnItem() as DroppedItem;
        ...
        internal class StopRecycling_Patch
        {
            ...
            {
                if (__instance == null) return;
                Instance.ResetRecyclerEfficiency(__instance, null, true);
            }
        }
        ...
        internal class RadiationProtection_Patch
        {
            ...
            {
                if (__instance == null || __instance.baseProtection == null || !Instance.GetBuffDetails(__instance.userID, out var buffData)) return;

                var baseRadProtection = __instance.baseProtection.amounts[17] * 100f;
                float result = 0;
                if (buffData.buff_values.TryGetValue(Buff.RadiationBaseValue, out var value)) result += value * 100;
                if (buffData.buff_values.TryGetValue(Buff.RadiationResistModifier, out value)) result += (baseRadProtection * value);
                __result += result;
            }
        }
        ...
        private void PrestigeConfirmation(BasePlayer player, int nextLevel, bool hasLevel)
        {
            ...
            container.Add(new CuiElement
            {
                ...
                Components = {
                    new CuiTextComponent { Text = "PRESTIGE", Font = "robotocondensed-bold.ttf", FontSize = 26, Align = TextAnchor.MiddleCenter, Color = "1 1 1 1" },
                    new CuiOutlineComponent { Color = "0 0 0 0.5", Distance = "1 -1" },
            ...
            {
                benefits += $"<color=#ece209>-</color> {lang.GetMessage(description, this, player.UserIDString)}.\n";
                count++;
            }

            foreach (var benefit in prestigeData.additionalBenfitsDescription)
            {
                benefits += $"<color=#ece209>-</color> {lang.GetMessage(benefit, this, player.UserIDString)}.\n";
                count++;
            }
            ...
            if (count > threshold)
            {
                container.Add(new CuiElement
                {
                    ...
                    new CuiScrollViewComponent {
                        ...
                        DecelerationRate = 0.3f,
                        ScrollSensitivity = 24f,
                        ContentTransform = new CuiRectTransform { AnchorMin = "0.5 1", AnchorMax = "0.5 1", OffsetMin = "0 " + count * 20 * -1 , OffsetMax = "0 0" },
                    },
                    new CuiNeedsCursorComponent()
                }
                });
                ...
                }, "STPrestigeScrollElement", "STPrestigeScrollElementInner");
            }

            container.Add(new CuiElement
            {
                Name = "text",
                Parent = count > threshold ? "STPrestigeScrollElement" : "STPrestigeScrollPanelAnchor",
                Components = {
                    new CuiTextComponent { Text = lang.GetMessage("UIPrestigeExplain", this, player.UserIDString) + benefits + (prestigeData.xp_mod != 1 ? string.Format(lang.GetMessage("UIPrestigeXPMod", this, player.UserIDString), Math.Round(prestigeData.xp_mod * 100, 2)) : ""), Font = "robotocondensed-regular.ttf", FontSize = 12, Align = TextAnchor.UpperLeft, Color = "1 1 1 1" },
                    new CuiOutlineComponent { Color = "0 0 0 0.5", Distance = "1 -1" },
                    new CuiRectTransformComponent { AnchorMin = "0.5 1", AnchorMax = "0.5 1", OffsetMin = $"-158.493 {-253.626 - (count > threshold ? 21 * (count - 12) : 0)}", OffsetMax = "158.491 -0.375" }
                }
            });
        ...
        void ConfirmPrestigeMenu(ConsoleSystem.Arg arg)
        {
            ...
            playerData.available_points += excessPoints;
        ...
        private void AchievedRank(BasePlayer player, ulong skin, string title)
        {
            ...
            container.Add(new CuiPanel
            {
                CursorEnabled = true,
                Image = { Color = "0 0 0 1" },
                RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1", OffsetMin = "0.04 0.342", OffsetMax = "-0.36 -0.338" }
        ...
        {
            if (!buffDetails.TryGetValue(player.userID, out var bd) || !bd.buff_values.ContainsKey(Buff.Comfort)) return;
        ...
        {
            if (!buffDetails.TryGetValue(player.userID, out var bd) || !bd.buff_values.ContainsKey(Buff.Human_Workbench)) return;
        ...
        [ChatCommand("wb")]
        void Workbenches(BasePlayer player)
        {
            var benches = BaseNetworkable.serverEntities.OfType<Workbench>().Count();
            PrintToChat(player, $"Benches: {benches}");
        }
    }
}
