...
{
    [Info("DiscordLinkBot", "Amino & Shady14u", "2.1.2")]
    ...
    class DiscordLinkBot : RustPlugin
    {
        ...
        public class Configuration
        {
            ...
            "https://i.ibb.co/QrRWZDD/1f517.png";
        ...
        {
            public string UIBackgroundColor { get; set; } = "0 0 0 .4";
            public string UIMainColor { get; set; } = "0 0 0 .5";
            public string HowToLinkHeaderColor { get; set; } = "0 0 0 .5";
            public string HowToLinkBodyColor { get; set; } = "0 0 0 .5";
            public string UnlinkButtonColor { get; set; } = "1 1 1 .3";
            public string GenerateCodeButtonColor { get; set; } = "1 1 1 .3";
            public string CodeBoxColor { get; set; } = "0 0 0 .5";
            public string UIHeaderColor { get; set; } = "0 0 0 .5";
            public string LinkedInfoPanelColor { get; set; } = "0 0 0 .5";
            public string SecondaryPanelColor { get; set; } = "0 0 0 .5";
            public string DiscordInfoPanelColor { get; set; } = "0 0 0 .5";
            public string SteamInfoPanelColor { get; set; } = "0 0 0 .5";
            public string TEXTCodeColor { get; set; } = ".43 1 .5 1";
            public string TEXTDiscordInfoColor { get; set; } = ".89 .89 .89 1";
            public string TEXTSteamInfoColor { get; set; } = ".44 .85 1 1";
            public string TEXTBoosterColor { get; set; } = "1 .44 .99 1";
        ...
        private static class PluginMessages
        {
            ...
            public const string NotABooster = "Not a booster";
            ...
            public const string YoureLinked = "YOU'RE LINKED!";
        }
        ...
        protected override void LoadDefaultMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                [PluginMessages.NotABooster] = "Not a booster",
                ...
                [PluginMessages.YoureLinked] = "YOU'RE LINKED!"
            }, this);
        }
        ...
        void Init()
        {
            var isUsingWC = Interface.Call("IsUsingPlugin", "DiscordLinkBot");
        ...
        {
            if (!neededPlugin.Equals("DiscordLinkBot", StringComparison.OrdinalIgnoreCase)) return;
            usingWC = true;
        ...
        void OnPlayerConnected(BasePlayer player)
        {
            ...
            var groupRoles = permission.GetUserGroups(player.UserIDString);
            var groups = _config.RolesToSync.Where(x => groupRoles.Contains(x.Key));
            LogAction("CheckStatus", new { steamId = player.UserIDString, groups = groups });
        }

        void OnPlayerDisconnected(BasePlayer player, string reason)
        {
            if(_openUIs.Contains(player.userID)) _openUIs.RemoveAll(x => x == player.userID);
        }
        ...
        private void UpdatePlayer(ConsoleSystem.Arg arg)
        {
            ...
            bool isLinked = playerInData != null;
        ...
        private void ShowLinkPanel(BasePlayer player)
        {
            ...
            if (!_openUIs.Contains(player.userID)) _openUIs.Add(player.userID);
            ...
            if (!usingWC)
            {
                    ...
                    RectTransform =
                {
                    ...
                    CursorEnabled = true,
                    Image = { Color = _uiColors.UIBackgroundColor, Material = "assets/content/ui/uibackgroundblur.mat" }
                }, "Overlay", "DLMainLinkPanel");

                var mainPanel = CreatePanel(ref container, ".3 .3", ".7 .7", _uiColors.UIMainColor, "DLMainLinkPanel");
                var topLabel = CreateInput(ref container, "0 .85", "1 1", "dl_main title", _uiColors.UIHeaderColor, "1 1 1 1", _config.DiscordUrl, 25, TextAnchor.MiddleCenter, mainPanel);
                ...
                if (playerLink != null)
                {
                    CreateLabel(ref container, "0 .73", ".5 .85", "0 0 0 0", _uiColors.TEXTDiscordInfoColor, $"Discord: {playerLink.DiscordName}", 17, TextAnchor.MiddleCenter, mainPanel);
                    CreateImagePanel(ref container, ".085 .12", ".43 .73", playerLink.DiscordImage.Contains(".gif") ? _config.DefaultAvatarUrl : playerLink.DiscordImage, mainPanel, isUrl: true);
                    CreateLabel(ref container, "0 0", ".5 .12", "0 0 0 0", _uiColors.TEXTDiscordInfoColor, playerLink.DiscordId, 17, TextAnchor.MiddleCenter, mainPanel);

                    CreateLabel(ref container, ".5 .73", "1 .85", "0 0 0 0", _uiColors.TEXTDiscordInfoColor, $"Steam: {playerLink.SteamName}", 17, TextAnchor.MiddleCenter, mainPanel);
                    CreateImagePanel(ref container, ".57 .12", ".915 .73", playerLink.SteamAvatar, mainPanel, isUrl: true);
                    CreateLabel(ref container, ".5 0", "1 .12", "0 0 0 0", _uiColors.TEXTDiscordInfoColor, $"{playerLink.SteamId}", 17, TextAnchor.MiddleCenter, mainPanel);

                    var unlinkButton = CreateButton(ref container, "0 -.13", ".997 -.005", _uiColors.UnlinkButtonColor, "1 1 1 1", GetMsg(PluginMessages.Unlink, player.userID), 30, "dl_main unlink", mainPanel);
                    if (!string.IsNullOrEmpty(_config.UnlinkImageUrl)) CreateImagePanel(ref container, ".005 .05", ".065 .95", _config.UnlinkImageUrl, unlinkButton, isUrl: true);
                }
                else
                {
                    var code = GetAuthCode(player);
                    CreateInput(ref container, ".1 .35", ".9 .75", "dl_main code", _uiColors.GenerateCodeButtonColor, _uiColors.TEXTCodeColor, code, 60, TextAnchor.MiddleCenter, mainPanel);
                    CreateButton(ref container, ".1 .1", ".9 .32", _uiColors.GenerateCodeButtonColor, "1 1 1 1", GetMsg(PluginMessages.LinkButtonText, player.userID), 30, $"dl_main generatecode {code}", mainPanel);
                }
            } else
            {
                var panel = CreatePanel(ref container, "0 0", "1 1", _uiColors.UIMainColor, "WCSourcePanel", "DLMainLinkPanel");
                CreateInput(ref container, "0 .9", ".999 1", "dl_main title", _uiColors.UIHeaderColor, "1 1 1 1", _config.DiscordUrl, 35, TextAnchor.MiddleCenter, panel);

                CreateLabel(ref container, ".015 .82", ".985 .88", _uiColors.HowToLinkHeaderColor, "1 1 1 1", GetMsg(PluginMessages.HowToLink), 30, TextAnchor.MiddleCenter, panel);
                var instPanel = CreatePanel(ref container, ".015 .62", ".985 .81", _uiColors.HowToLinkBodyColor, panel);
                CreateLabel(ref container, ".02 .05", ".98 .95", "0 0 0 0", "1 1 1 1", GetMsg(PluginMessages.Instructions), 25, TextAnchor.MiddleCenter, instPanel);

                var linkedPanel = CreatePanel(ref container, ".015 .02", ".985 .6", _uiColors.SecondaryPanelColor, panel);

                if (playerLink != null)
                {
                    CreateImagePanel(ref container, ".015 .52", ".19 .96", playerLink.DiscordImage.Contains(".gif") ? _config.DefaultAvatarUrl : playerLink.DiscordImage, linkedPanel, isUrl: true);
                    CreateLabel(ref container, ".20 .75", ".6 .96", _uiColors.DiscordInfoPanelColor, _uiColors.TEXTDiscordInfoColor, $"  {playerLink.DiscordName} - <color=#f959ff>{(playerLink.IsBooster ? GetMsg(PluginMessages.Booster) : GetMsg(PluginMessages.NotABooster))}</color>", 23, TextAnchor.MiddleLeft, linkedPanel);
                    CreateLabel(ref container, ".20 .52", ".6 .73", _uiColors.DiscordInfoPanelColor, _uiColors.TEXTDiscordInfoColor, $"  {playerLink.DiscordId}", 23, TextAnchor.MiddleLeft, linkedPanel);

                    CreateImagePanel(ref container, ".015 .04", ".19 .48", playerLink.SteamAvatar, linkedPanel, isUrl: true);
                    CreateLabel(ref container, ".20 .27", ".6 .48", _uiColors.SteamInfoPanelColor, _uiColors.TEXTSteamInfoColor, $"  {playerLink.SteamName}", 23, TextAnchor.MiddleLeft, linkedPanel);
                    CreateLabel(ref container, ".20 .04", ".6 .25", _uiColors.SteamInfoPanelColor, _uiColors.TEXTSteamInfoColor, $"  {playerLink.SteamId}", 23, TextAnchor.MiddleLeft, linkedPanel);

                    // CreateLabel(ref container, ".31 .36", ".69 .41", _config.InstPanelColor, "1 1 1 1", GetMsg(PluginMessages.YoureLinked), 17, TextAnchor.MiddleCenter, linkedPanel);
                    var unlinkButton = CreateButton(ref container, ".62 .375", ".985 .625", _uiColors.UnlinkButtonColor, "1 1 1 1", GetMsg(PluginMessages.Unlink), 25, "dl_main unlink", linkedPanel);
                    if (!string.IsNullOrEmpty(_config.UnlinkImageUrl)) CreateImagePanel(ref container, ".02 .25", ".15 .75", _config.UnlinkImageUrl, unlinkButton, isUrl: true);
                } else
                {
                    var code = GetAuthCode(player);
                    CreateInput(ref container, ".1 .42", ".9 .8", "dl_main code", _uiColors.CodeBoxColor, _uiColors.TEXTCodeColor, code, 80, TextAnchor.MiddleCenter, linkedPanel);
                    CreateButton(ref container, ".1 .2", ".9 .4", _uiColors.GenerateCodeButtonColor, "1 1 1 1", GetMsg(PluginMessages.LinkButtonText, player.userID), 30, $"dl_main generatecode {code}", linkedPanel);
                }
            }
            ...
            if (!usingWC && _config.AutoInstructions) CreateLinkingInfo(player);
        }
        ...
        private void CreateLinkingInfo(BasePlayer player)
        {
            ...
            var mainPanel = CreatePanel(ref container, ".71 .3", ".9 .7", _uiColors.UIMainColor, "DLMainLinkPanel", "DLMainInfoPanel");
            CreateLabel(ref container, "0 .85", "1 1", _uiColors.UIHeaderColor, "1 1 1 1", GetMsg(PluginMessages.HowToLink), 25, TextAnchor.MiddleCenter, mainPanel);
            CreateButton(ref container, ".93 .85", ".98 1", "1 1 1 0", "1 1 1 1", "<", 25, "dl_main infoclose", mainPanel);

            CreateLabel(ref container, ".05 .05", ".95 .84", "1 1 1 0", "1 1 1 1", GetMsg(PluginMessages.Instructions), 15, TextAnchor.UpperLeft, mainPanel);
        ...
        private void GeneratePlayersUI(BasePlayer player)
        {
            ...
            foreach (var link in linkedList.Skip(theUser.Page * 22).Take(22))
            {
                ...
                CreateImagePanel(ref container, ".005 .05", ".075 .93", link.SteamAvatar, linkPanel, isUrl: true);
                CreateLabel(ref container, ".085 .5", ".45 1", "0 0 0 0", _uiColors.TEXTSteamInfoColor, link.SteamName, 15, TextAnchor.MiddleLeft, linkPanel);
                CreateLabel(ref container, ".085 0", ".45 .5", "0 0 0 0", _uiColors.TEXTSteamInfoColor, $"{link.SteamId}", 15, TextAnchor.MiddleLeft, linkPanel);

                CreateImagePanel(ref container, ".46 .05", ".53 .93", link.DiscordImage.Contains(".gif") ? _config.DefaultAvatarUrl : link.DiscordImage, linkPanel, isUrl: true);
                CreateLabel(ref container, ".54 .5", ".905 1", "0 0 0 0", _uiColors.TEXTDiscordInfoColor, link.DiscordName, 15, TextAnchor.MiddleLeft, linkPanel);
                CreateLabel(ref container, ".54 0", ".905 .5", "0 0 0 0", _uiColors.TEXTDiscordInfoColor, $"{link.DiscordId}", 15, TextAnchor.MiddleLeft, linkPanel);
        ...
        private void ShowDetailedPlayer(BasePlayer player)
        {
            ...
            CreateImagePanel(ref container, ".05 .1", ".37 .8", info.LinkedPlayer.SteamAvatar, mainPanel, isUrl: true);
            ...
            if (info.LinkedPlayer.IsBooster) CreateImagePanel(ref container, ".42 .33", ".58 .68", "https://i.ibb.co/5nhXx7s/7791-server-boost.png", mainPanel, isUrl: true);
            ...
            CreateImagePanel(ref container, ".63 .1", ".95 .8", info.LinkedPlayer.DiscordImage.Contains(".gif") ? _config.DefaultAvatarUrl : info.LinkedPlayer.DiscordImage, mainPanel, isUrl: true);
        ...
        {
            if(!usingWC) foreach (var chatCmd in _config.CommandToLink)
