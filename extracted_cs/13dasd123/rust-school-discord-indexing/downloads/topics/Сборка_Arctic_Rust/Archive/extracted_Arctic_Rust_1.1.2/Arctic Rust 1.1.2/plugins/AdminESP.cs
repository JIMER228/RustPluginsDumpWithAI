...
namespace Oxide.Plugins
{
    ...
    public class AdminESP : RustPlugin
    {
        ...
        void cmdAdminESP(BasePlayer player, string command, string[] args)
        {
            ...
            if (args.Length < 3 && args[0] == "help")
            {
                SendReply(player, "ESP Настройки:\n"
                     ...
                     + $"\n/ae settings PlayerDistance {data.PlayerDistance} - максимальная дистанция отображения игроков (1000 метров)"
        ...
        void CrateMainMenu(BasePlayer player)
        {
            ...
            {
                RectTransform = { AnchorMin = "0.85 0", AnchorMax = "1 0.997" },
                Button = { Color = "1 1 1 0.5", Command = $"adminespUI_toggle playerdistance {data.PlayerDistance + 100}" },
                Text = { Text = "+", Align = TextAnchor.MiddleCenter, Font = "robotocondensed-bold.ttf", FontSize = 18 },
            }, $"{Main}CurrentPanel.2");
        ...
        private static List<Position> GetPositions(int colums, int rows, float colPadding = 0, float rowPadding = 0, bool columsFirst = false, ulong playerid = 4854784)
        {
            if (colums == 0)
