...
namespace Oxide.Plugins
{
	...
	internal class WipeBlock : RustPlugin
	{
		...
		private object CanMoveItem(Item item, PlayerInventory inventory, ItemContainerId itemContainer, int num,
			int num2)
		{
		...
		private void CmdConsoleWipeBlock(ConsoleSystem.Arg arg)
		{
			...
			switch (arg.Args[0])
			{
				...
				case "item_update":
				{
					var guid = arg.GetString(1);
					
					var targetItem = ItemsAll.Find(x => x.GUID == guid);
					if (targetItem == null) return;

					UpdateUI(player, container => ItemUi(player, ref container, targetItem));
					break;
				}
			}
		}
				...
				items.ForEach(category =>
				{
					...
					for (var i = 0; i < category.Items.Count; i++)
					{
						...
						container.Add(new CuiPanel
							...
							}, Layer + ".Main", Layer + $".Item.{item.GUID}", Layer + $".Item.{item.GUID}");
						...
						if ((i + 1) % _config.UI.ItemsOnString == 0 && (i + 1) != category.Items.Count)
						{
		...
		private void ItemUi(BasePlayer player, ref CuiElementContainer container, ItemConf item)
		{
			container.Add(new CuiElement()
			{
				...
				Components =
				{
					...
					{
						EndTime = 0,
						StartTime = LeftTime(item),
						Step = 1,
						TimerFormat = TimerFormat.HoursMinutesSeconds,
						DestroyIfDone = true,
						Command = $"UI_WipeBlock item_update {item.GUID}"
					}
				}
			});
		...
		{
			return (container.entityOwner is AutoTurret || IsContainerMLRS(container.entityOwner)) &&
			       IsBlocked(item.info.shortname, item.skin);
		}
		
		private bool IsContainerMLRS(BaseEntity owner)
		{
			return owner is StorageContainer storageContainer && storageContainer.parentEntity.IsValid(true) && storageContainer.parentEntity.Get(true) is MLRS;
		}
