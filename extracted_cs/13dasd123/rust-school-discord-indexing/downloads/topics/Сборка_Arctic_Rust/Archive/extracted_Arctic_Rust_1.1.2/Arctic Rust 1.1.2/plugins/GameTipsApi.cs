...
{
    [Info("GameTipsApi", "MaltrzD", "0.0.1")]
    class GameTipsApi : RustPlugin
    {
        #region [ OTHER ]
        [ChatCommand("gtatest")]
        private void Test(BasePlayer player, string cmd, string[] args)
        {
            ...
            ShowGameTip(player, $"{player.displayName}\n{player.userID}", Convert.ToInt32(args[0]));
        }
        #endregion

        #region [ CONSOLE COMMAND ]
        [ConsoleCommand("gametipsapi.showforall")]
        private void ShowGameTipForAll_ConsoleCommand(ConsoleSystem.Arg arg) //0 = type, 1 = message
        {
            ...
            try
            {
                type = Convert.ToInt32(arg.Args[0]);
        ...
        [ConsoleCommand("gametipsapi.showforplayer")]
