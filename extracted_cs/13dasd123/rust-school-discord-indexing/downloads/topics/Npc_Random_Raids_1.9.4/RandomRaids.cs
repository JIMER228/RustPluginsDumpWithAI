...
namespace Oxide.Plugins
{
    ...
    public class RandomRaids : RustPlugin
    {
		...
		private void CheckNpcKillProgress(BasePlayer player)
		{
			if (player == null) return;

			ulong playerId = player.userID;
			if (!logData.pLogs.TryGetValue(playerId, out var log))
			{
				SendReply(player, "You have not killed any NPCs yet.");
				return;
			}

			int totalKills = log._totalNpcKills;
			int killThreshold = configData.settingsNPCKILLS.CombineProfile.totalRaids;
			int remainingKills = Math.Max(0, killThreshold - totalKills);

			SendReply(player, $"You have killed {totalKills}/{killThreshold} NPCs. You need {remainingKills} more kills before you get raided.");
		}
		
		private void OnNpcKill(ulong playerId, string npcType)
		{
			// Check if the player has a log entry; if not, create one
			if (!logData.pLogs.ContainsKey(playerId))
			{
				logData.pLogs[playerId] = new lData(); // Directly reference lData
			}

			// Access the player's log
			lData playerLog = logData.pLogs[playerId];

			// Check if the NPC type has been logged; if not, initialize it
			if (!playerLog.NPCKILLS.ContainsKey(npcType))
			{
				playerLog.NPCKILLS[npcType] = 0;
			}

			// Increment the kill count
			playerLog.NPCKILLS[npcType]++;
			playerLog._totalNpcKills++;
                        ...
                        foreach (var user in priv.authorizedPlayers)
                        {
							...
							if (configData.settingsNPCKILLS.warnPlayer)
							{
								...
								if (p2 != null)
								{
									...
									// Play the updated C4 explosion sound
									Effect.server.Run("assets/bundled/prefabs/fx/explosions/water_bomb.prefab", p2.transform.position);
								}
							}
						}
