...
{
    [Info("SkyCraftSystem", "Mercury", "1.0.31")]
    class SkyCraftSystem : RustPlugin
    {
        ...
        private class Configuration
        {
            ...
            [JsonProperty("Дополнительные настройки | More Settings")]
            public OtherSetting OtherSettings = new OtherSetting();
            ...
            {
                #region OtherSetting
                [JsonProperty("Разрешить подобрать транспорт | Allow pick up transport")]
                public bool UsePickUpItem;
                [JsonProperty("На транспорт могут садиться только владелец и друзья | Only the owner and friends can board the transport")]
                public bool MountedOnlyFriendsAndOwner;
                #endregion
            }
            ...
            public static Configuration GetNewConfiguration()
            {
                return new Configuration
                {
                    SettingsPlugins = new List<CraftItemsConfiguration>
                    {
                        ...
                        {
                            OnOffCraft = true,
                            Permissions = "SkyCraftSystem.helicraft",
                            Prefab = "",
                            ShortnameReplyPrefab = "bleach",
                            DisplayName = "Helicopter call",
                            WorkBenchLevel = 3,
                            SkinID = 1802775440,
                            PNGLink = "https://i.imgur.com/UUJt4o9.png",
                            CraftingItems = new Dictionary<string, int>
                            {
                                ["metal.refined"] = 600,
                                ["techparts"] = 10,
                        ...
                        {
                            OnOffCraft = true,
                            Permissions = "SkyCraftSystem.chinoockcraft",
                            Prefab = "",
                            ShortnameReplyPrefab = "sticks",
                            DisplayName = "Chinook Call",
                            WorkBenchLevel = 3,
                            SkinID = 1802774090,
                            PNGLink = "https://i.imgur.com/6rJo0GZ.png",
                            CraftingItems = new Dictionary<string, int>
                            {
                                ["metal.refined"] = 1200,
                                ["techparts"] = 10,
                    ...
                    OtherSettings = new OtherSetting
                    {
                        UsePickUpItem = true,
                        MountedOnlyFriendsAndOwner = true,
                    }
                };
            }
        }
        ...
        //{
        //    PrintToChat(info.HitEntity.PrefabName + " " + config.SettingsPlugins.IndexOf(new Configuration.CraftItemsConfiguration
        //    {
        //        Prefab = "assets/content/vehicles/minicopter/minicopter.entity.prefab"
        //    }).ToString());
        ...
        //        GiveItems(player, config.SettingsPlugins.IndexOf(new Configuration.CraftItemsConfiguration { Prefab = info.HitEntity.PrefabName }));
        ...
        //        PrintToChat(info.HitEntity.OwnerID.ToString());
        ...
        {
            if (basemountable is MiniCopter || basemountable is ModularCar || basemountable is BasicCar || basemountable is BaseBoat)
            {
                BaseVehicle veh = basemountable?.VehicleParent();
                if (veh == null || veh.OwnerID.Equals(0) || veh.OwnerID.Equals(player.userID)) return null;
                if (player.Team == null || !player.Team.members.Contains(veh.OwnerID))
                {
                    MessageUI(player, lang.GetMessage("OTHER_FUNC_MOUNTED", this, player.UserIDString));
                    return false;
                }
            }
            return null;
        ...
        //{
        //    var vehicle = entity.OwnerID;
        //    PrintToChat(entity.OwnerID.ToString() + " " + entity.PrefabName + "TEST " + vehicle);
        //    return null;
        //}

        bool CheckFriend(ulong player, ulong target)
        {
            if (player == target || target == 0) return true;
            bool IsFriends = (bool)Friends?.Call("IsFriend", player, target);
            if (IsFriends)
                return true;
            else return false;
        }
        ...
        private void CheckDeploy(BaseEntity entity)
        {
            ...
            if (SpawnItem(entity.transform.position, entity.skinID, entity.OwnerID, entity.transform.rotation))
            {
                ///
            }
            else
        ...
        private bool SpawnItem(Vector3 position, ulong SkinID, ulong ownerID, Quaternion rotation = default(Quaternion))
        {
            Vector3 vector = position + Vector3.up * 2f;
            ...
            for (int i = 0; i < config.SettingsPlugins.Count; i++)
            {
                ...
                {
                    if (cfg.Prefab.Contains("rhib") || cfg.Prefab.Contains("rowboat"))
                    {
                        float height = TerrainMeta.WaterMap.GetHeight(spawnpos);
                        if (spawnpos.y < height) spawnpos.y = TerrainMeta.WaterMap.GetHeight(position) + 2f;
                        else spawnpos.y += 3f;
                    }
                    else spawnpos.y += 2f;

                    BaseEntity Item = GameManager.server.CreateEntity(cfg.Prefab, spawnpos, rotation);
        ...
        private new void LoadDefaultMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                ["OTHER_FUNC_PICK_UP_ONE"] = "You can not pick up someone else's transport",
                ["OTHER_FUNC_PICK_UP_TWO"] = "You have successfully picked up your transport",
                ["OTHER_FUNC_MOUNTED"] = "You cannot take someone else's copter!",
            ...
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                ["OTHER_FUNC_PICK_UP_ONE"] = "Вы не можете подобрать чужой транспорт",
                ["OTHER_FUNC_PICK_UP_TWO"] = "Вы успешно подобрали свой транспорт",
                ["OTHER_FUNC_MOUNTED"] = "Вы не можете сесть на чужой коптер!", //Вы не можете сесть на чужой коптер!
        ...
        public static string PARENT_SHOW_ACTION_MENU = "MenuActionTake";
