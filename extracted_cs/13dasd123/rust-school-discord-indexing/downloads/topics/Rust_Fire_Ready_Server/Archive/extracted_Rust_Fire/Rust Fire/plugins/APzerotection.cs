...
{
    [Info("APzerotection", "Hougan", "1.5.8")]
    public class APzerotection : RustPlugin
    {
        #region Variables
        ...
        private void OnServerInitialized()
        {
            ...
            permission.RegisterPermission("aprotection.use", this);
            ...
            lang.RegisterMessages(new Dictionary<string, string>()
            {
                ...
                ["ПОДСКАЗКА.РЕГИСТРАЦИЯ"] = "Приветствую! Чтобы зарегистрироваться - придумай и впиши пароль в консоль!\n" +
                                            ...
                                            "Bind 'bind key auth ваш_пароль': {0}\n" +
            ...
            if (Interface.Oxide.DataFileSystem.ExistsDatafile("APzerotection"))
                StoredData = Interface.Oxide.DataFileSystem.ReadObject<Dictionary<ulong, IAuthorize>>("APzerotection");
            foreach (BasePlayer player in BasePlayer.activePlayerList) OnPlayerConnected(player);
            timer.Every(3456f, Unload);
        }

        private void Unload() => Interface.Oxide.DataFileSystem.WriteObject("APzerotection", StoredData);
        ...
        {
            return blocker(arg.Player(), arg.cmd.FullName);
        }
        ...
        {
            if (player == null || !permission.UserHasPermission(player.UserIDString, "aprotection.use")) return null;

            IAuthorize authorize;
            if (!StoredData.TryGetValue(player.userID, out authorize)) return false;
            
            if (!authorize.IsAuthed && command.ToLower() != "global.auth") return false;
        ...
        private void OnPlayerConnected(BasePlayer player)
        {
            ...
            if (!permission.UserHasPermission(player.UserIDString, "aprotection.use")) return;
            
            string IP = GetIP(player.net.connection.ipaddress);

            IAuthorize authorize;
            if (!StoredData.TryGetValue(player.userID, out authorize))
            {
            ...
            {
                authorize.LastPosition = player.transform.position;
                authorize.IsAuthed = false;

                if (authorize.AuthedIP.Contains(IP))
                {
                    authorize.IsAuthed = true;
                    player.ChatMessage(FL("АВТОРИЗОВАН.IP", player).Replace("{0}", IP));
                    return;
                }
            }
        ...
        private void cmdAuthConsole(ConsoleSystem.Arg args)
        {
            ...
            if (player == null || !permission.UserHasPermission(player.UserIDString, "aprotection.use")) return;
            ...
            if (!args.HasArgs() || args.FullString.Length == 0)
            {
                args.ReplyWithObject(FL("РЕГИСТРАЦИЯ. НЕ ВВЁЛ ПАРОЛЬ", player));
                return;
            }
            ...
            if (authorize.IsAuthed) return;

            if (string.IsNullOrEmpty(authorize.Password))
            {
                args.ReplyWithObject(FL("РЕГИСТРАЦИЯ.УСПЕШНО", player).Replace("{0}", password));
                authorize.Password = password;
                authorize.IsAuthed = true;
                authorize.Authed(player, ip);
            }
            else if (password == authorize.Password)
            {
                args.ReplyWithObject(FL("АВТОРИЗАЦИЯ.УСПЕШНО", player));
            ...
            {
                args.ReplyWithObject(FL("НЕВЕРНЫЙ ПАРОЛЬ", player));
                return;
            }
        }
        ...
        private void ForceAuthorization(BasePlayer player)
        {
            ...
            if (string.IsNullOrEmpty(authorize.Password))
            {
                player.ChatMessage(FL("ПОМОЩЬ.ЗАРЕГИСТРИРУЙТЕСЬ", player));
                player.SendConsoleCommand($"echo {FL("ПОДСКАЗКА.РЕГИСТРАЦИЯ", player).Replace("{0}", GetIP(player.net.connection.ipaddress))}");
                authorize.Timer = timer.Once(10, () => ForceAuthorization(player));
                return;
            }

            player.SetPlayerFlag(BasePlayer.PlayerFlags.ChatMute, true);
            player.SendConsoleCommand($"echo {FL("ПОДСКАЗКА.АВТОРИЗАЦИЯ", player).Replace("{0}", GetIP(player.net.connection.ipaddress))}");
            player.Teleport(authorize.LastPosition);
            authorize.Timer = timer.Once(2, () => ForceAuthorization(player));
        }
        ...
        private string FL(string key, BasePlayer player) => lang.GetMessage(key, this, player.UserIDString);
        ...
        private void DrawInterface(BasePlayer player)
        {
            ...
            string action = string.IsNullOrEmpty(StoredData[player.userID].Password) ? FL("ЗАРЕГИСТРИРУЙТЕСЬ", player) : FL("АВТОРИЗУЙТЕСЬ", player);
            ...
            container.Add(new CuiLabel
            {
                ...
                Text = { FadeIn = 2f, Text = FL("ИНФО", player), FontSize = 22, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter },
