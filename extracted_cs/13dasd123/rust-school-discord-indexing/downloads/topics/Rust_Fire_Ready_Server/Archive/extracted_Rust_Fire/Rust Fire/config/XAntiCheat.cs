...
{
    [Info("XAntiCheat", "fermens", "0.3.48")]
    public class XAntiCheat : RustPlugin
    {
      ...
      /* Dictionary<BasePlayer, DateTime> lastbowtime = new Dictionary<BasePlayer, DateTime>();
        ...
        private static void BAN(string steamid, string reason, int time, string displayname)
        {
        ...
        class ANTICHEAT : MonoBehaviour
        {
          ...
          //      if (!enablefull) Debug.Log(player.displayName + " добавили в античит.");
            ...
            public void ADDHIT(string hitbone, string weapon, float distance)
            {
                ...
                if (hits == 0) firsthit = DateTime.Now;
                 hits++;
                ...
                if (hits >= 5)
                {
                    ...
                    if (distancehit > 100 && (weaponhit.Contains("bow_hunting.entity") || weaponhit.Contains("crossbow.entity") || weaponhit.Contains("pistol_eoka.entity"))
                        || distancehit > 65 && (weaponhit == "bow_hunting.entity" || weaponhit == "crossbow.entity")
                        || distancehit > 40 && weaponhit == "pistol_eoka.entity"
                        || (DateTime.Now - hitmod).TotalMinutes < 7f && !weapon.Contains("l96.entity"))
                    {
      ...
      /*  private void OnWeaponFired(BaseProjectile projectile, BasePlayer player, ItemModProjectile mod, ProtoBuf.ProjectileShoot projectiles)
        ...
        private static float sizeworldx;
        private static float sizeworldz;
        ...
        private static Vector3 RANDOMPOS() => new Vector3(Random.Range(-sizeworldx, sizeworldx), 400f, Random.Range(-sizeworldz, sizeworldz));
        ...
        private void OnServerInitialized()
        {
            ...
            sizeworldx = TerrainMeta.Size.x / 2f;
            sizeworldz = TerrainMeta.Size.z / 2f;
            ...
            while(i < config.ESPStash.amount)
            {
                ...
                if (OntheMap.Any(x => Vector2.Distance(x, pos) < 120f)) continue;
               // if (stashContainers.Any(x => Vector2.Distance(x.transform.position, pos) < 20f)) continue;
        ...
        private void OnPlayerViolation(BasePlayer player, AntiHackType type, float amount)
        {
            if (type == AntiHackType.FlyHack && !IsBattles(player.userID))
            {
                ...
                Vis.Entities<BaseEntity>(player.transform.position, 2f, list);
                List<TreeEntity> list2 = Pool.GetList<TreeEntity>();
                Vis.Entities<TreeEntity>(player.transform.position, 5f, list2);
                string elements = "";
                bool pl = false;
                ...
                if (raycast)
                {
                    ...
                    if (roof || drop || spiral || tree || pl || distance < 3f || distance > 7f) return;
