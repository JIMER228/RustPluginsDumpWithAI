...
{
    [Info("RemoveUpgradeV2", "fermens", "0.2.61")]
    ...
    class RemoveUpgradeV2 : RustPlugin
    {
        ...
        void OnServerInitialized()
        {
            ...
            permission.RegisterPermission("remove.refund", this);
        ...
        {
            if (!permission.UserHasPermission(player.UserIDString, "remove.use") && !permission.UserHasPermission(player.UserIDString, "removeupgradev2.remove"))
            {
            ...
            RemoveType removetype = permission.UserHasPermission(player.UserIDString, "remove.refund") ? RemoveType.Refund : RemoveType.Normal;
        ...
        {
            if (!permission.UserHasPermission(player.UserIDString, "remove.use") && !permission.UserHasPermission(player.UserIDString, "removeupgradev2.up"))
            {
                SendReply(player, config.messages[13]);
                return;
            }
