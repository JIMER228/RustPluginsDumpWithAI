...
namespace Oxide.Plugins
{
	...
	public sealed class ServerInfo : RustPlugin
	{
		...
		[ChatCommand("help")]
		private void ShowInfo2(BasePlayer player, string command, string[] args)
		{
			if (player == null || _settings == null)
				return;
		...
		private static CuiButton CreateCloseButton(string mainPanelName, string hexColor)
		{
			...
			return new CuiButton
			{
				Button =
				{
					Command = $"infoclose {mainPanelName}",
					Close = mainPanelName,
					Color = $"{(float) 255/255} {(float) 77/255} {(float) 77/255}"
				},
				RectTransform =
				{
					AnchorMin = "1 1",
		...
		public sealed class Settings
		{
			public Settings()
			{
				...
				ShowInfoOnPlayerConnected = true;
			...
			public bool ShowInfoOnPlayerConnected { get; set; }
		...
		public sealed class PlayerInfoState
		{
			public PlayerInfoState(Settings settings)
			{
				...
				PageIndex = 0;
				InfoShownOnLogin = settings.ShowInfoOnPlayerConnected;
