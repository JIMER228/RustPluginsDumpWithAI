...
namespace Oxide.Plugins
{
    ...
    class NTeleportation : RustPlugin
    {
        ...
        [PluginReference]
        private Plugin Clans, Economics, ServerRewards, Friends, CompoundTeleport, ZoneManager, NoEscape, Vanish, RaidZone;
            ...
            TeleportTimers[player.userID] = new TeleportTimer
            {
                ...
                Timer = timer.Once(countdown, () =>
                {
                    ...
                    if (UseEconomy())
                    {
					  ...
					  if (RaidZone != null && RaidZone.Call<bool>("HasBlockTera", location))
                    {
                        player.ChatMessage("<color=yellow>Телепорт отменен. Место телепорта находиться в зоне рейдблока!</color>");
                        return;
                    }
