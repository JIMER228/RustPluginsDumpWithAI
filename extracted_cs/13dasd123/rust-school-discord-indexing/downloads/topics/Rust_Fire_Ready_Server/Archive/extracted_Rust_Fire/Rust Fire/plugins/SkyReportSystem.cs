...
namespace Oxide.Plugins
{
    ...
    public class SkyReportSystem : RustPlugin
    {
        ...
        public static class ImageLibraryConfig
        {
            public class UserImagesManuallydefineimagestobeloaded
            {
            }
            ...
            public class Root
            {
                [JsonProperty("Avatars-Storeplayeravatars")]
                public bool AvatarsStoreplayeravatars { get; set; }

                [JsonProperty("SteamAPIkey(getoneherehttps://steamcommunity.com/dev/apikey)")]
                public string SteamAPIkeyGetoneherehttpsSteamcommunityComDevApikey { get; set; }

                [JsonProperty("Progress-Showdownloadprogressinconsole")]
                public bool ProgressShowdownloadprogressinconsole { get; set; }

                [JsonProperty("Progress-Timebetweenupdatenotifications")]
                public int ProgressTimebetweenupdatenotifications { get; set; }

                [JsonProperty("UserImages-Manuallydefineimagestobeloaded")]
                public UserImagesManuallydefineimagestobeloaded UserImagesManuallydefineimagestobeloaded { get; set; }
        ...
        {
            var cfg = Config.ReadObject<ImageLibraryConfig.Root>(Interface.Oxide.ConfigDirectory + "/ImageLibrary.json");
            if (string.IsNullOrEmpty(userId) || string.IsNullOrEmpty(cfg.SteamAPIkeyGetoneherehttpsSteamcommunityComDevApikey) || (bool)ImageLibrary?.Call("HasImage", userId, 0))
                return;

            webrequest.Enqueue(string.Format(STEAM_AVATAR_URL, cfg.SteamAPIkeyGetoneherehttpsSteamcommunityComDevApikey, userId), null, (code, response) =>
            {
                if (response != null && code == 200)
                {
                    Puts("Loading avatar for id " + userId);
                    try
                    {
                        ...
                        if (rootObject?.response?.players?.Length > 0)
                        {
                            ...
                            if (!string.IsNullOrEmpty(avatarUrl))
                                ImageLibrary?.Call("AddImage", avatarUrl, userId, 0);
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        protected override void LoadDefaultMessages()
        {
            Dictionary<string, string> Lang = new Dictionary<string, string>
            {
                ...
                "Его колличевство {1}",
                ["SKY_COLDOWN"] = "Вы недавно отправляли жалобу,подождите еще {0} секунд",
                ...
                ["SKY_MODER_CHECK_PLAYER_GOING_CONSOLE"] = "=====================================================\nИгрок {0}[{1}] был вызван с помощью консоли",
                ...
                ["SKY_CHECK_STOP_CONSOLE"] =
                                           "=====================================================\n" +
                                           "Сервер : {0}\n" +
                                           "Проверка окончена над игроком {1} [{2}]\n",
                ["BAN_USER_VK_DISCORD"] =
                                           "=====================================================\n" +
                                           "Игрок :{0}({1})\n" +
        ...
        void consolego(ConsoleSystem.Arg args)
        {
            ...
            GetPlayerAvatar(targetPlayer.UserIDString);
            BasePlayer player = args.Player();
        ...
        private void PlayerReportSystem(ConsoleSystem.Arg args)
        {
            ...
            if (args.Args.Length >= 2)
            {
                ...
                if (args.Player() != target)
                {
                    ...
                    {
                        var sec = (int)CurrentTime() - CooldownPC[args.Player().userID];
                        SendChat(args.Player(), string.Format(lang.GetMessage("SKY_COLDOWN", this), sec.ToString()));
                        return;
                    }
        ...
        private void playerinfomenu(BasePlayer player, string target, BasePlayer targetinfocheck)
        {
            ...
            string ImageAvatar = GetImage(targets.UserIDString, 0);
        ...
        private void SendChatMessage(string msg, params object[] args)
        {
            ...
            webrequest.Enqueue($"https://api.vk.com/method/messages.send?peer_id={config.vKontakte.VK_ChatID}&random_id={RandomID}&message={vkchat}&access_token={config.vKontakte.VK_Token}&v=5.120", null, (code, response) => 
            {
