...
namespace Oxide.Plugins
{
    ...
    {
        #region Fields
        [PluginReference] Plugin NoEscape, ArenaTournament;
        ...
        private void OnRespawnInformationGiven(BasePlayer player, List<RespawnInformation.SpawnOptions> list)
        {
            ...
            if (NoEscape)
            {
                if (configData.NoEscape.RaidBlockedRespawn)
                {
                    ...
                    if (success)
                    {
                        SendReply(player, msg("Error.RaidBlocked", player.userID.Get()));
                        return;
                    }
                }
                ...
                if (configData.NoEscape.CombatBlockedRespawn)
                {
                    ...
                    if (success)
                    {
                        SendReply(player, msg("Error.CombatBlocked", player.userID.Get()));
                        return;
                    }
                }
            }
            ...
            Cooldown cooldown;
            cooldowns.TryGetValue(player.userID.Get(), out cooldown);
            ...
            if (Outpost != null && configData.Respawn.AllowOutpost)
            {
                ...
                d.id = OUTPOST_ID;
                d.name = msg("Name.Outpost", player.userID.Get());
            ...
            if (Bandit != null && configData.Respawn.AllowBandit)
            {
                RespawnInformation.SpawnOptions d = Pool.Get<RespawnInformation.SpawnOptions>();
                d.id = BANDIT_ID;
                d.name = msg("Name.Bandit", player.userID.Get());
        ...
        private void RespawnPlayerAt(BasePlayer player, Vector3 position)
        {
            ...
            if (!cooldowns.TryGetValue(player.userID.Get(), out cooldown))
                cooldowns[player.userID.Get()] = cooldown = new Cooldown();

            player.RespawnAt(position, Quaternion.identity);
            cooldown.respawn = Epoch.Current + configData.Respawn.Cooldown;
        }

        private bool HasPendingTeleports(BasePlayer player) => pending.ContainsKey(player.userID.Get());
        ...
        private void CancelPendingTeleports(BasePlayer player)
        {
            ...
            {
                player.CancelInvoke(pending[player.userID.Get()]);
                pending.Remove(player.userID.Get());
            }
        }
        #endregion
        ...
        {
            if (ArenaTournament)
                if (ArenaTournament.Call("isEventPlayer", player) != null)
                {
                    SendReply(player, msg("Notification.Cancelled", player.userID.Get()));
                    return;
                }
            try
            {
        ...
        private void cmdTeleportOutpost(BasePlayer player, string command, string[] args)
        {
            ...
            object o = Interface.Oxide.CallHook("CanTeleport2", player);
            if (o != null)
            {
                player.ChatMessage("Телепортация в город внутри купола запрещена.");
                return;
            }
            InvokeTeleport(player, () => TeleportToOutpost(player));

            SendReply(player, string.Format(msg("Notification.TPPending", player.userID.Get()), msg("Name.Outpost", player.userID.Get()), FormatTime(GetWaitTime(player.UserIDString))));
        }
        ...
        private void cmdTeleportBandit(BasePlayer player, string command, string[] args)
        {
            ...
            if (o != null)
            {
                player.ChatMessage("Телепортация в город бандитов внутри купола запрещена.");
                return;
            }
            InvokeTeleport(player, () => TeleportToBandit(player));

            SendReply(player, string.Format(msg("Notification.TPPending", player.userID.Get()), msg("Name.Bandit", player.userID.Get()), FormatTime(GetWaitTime(player.UserIDString))));
        }
        ...
        private void cmdTeleportCustom(BasePlayer player, string command, string[] args)
        {
            ...
            {
                SendReply(player, msg("Notification.NoAreas", player.userID.Get()));
                return;
            }

            if (args == null || args.Length == 0)
            {
                SendReply(player, msg("Notification.NoAreaSpecified", player.userID.Get()));
                return;
            }
            ...
            {
                SendReply(player, string.Format(msg("Notification.NoAreaWithName", player.userID.Get()), name));
                return;
            }
            ...
            SendReply(player, string.Format(msg("Notification.TPPending", player.userID.Get()), customSpawnPoint.Name, FormatTime(GetWaitTime(player.UserIDString))));
        }
        ...
        private void InvokeTeleport(BasePlayer player, Action action)
        {
            ...
            pending[player.userID.Get()] = action;
        }
        ...
        private bool MeetsAllRequirements(BasePlayer player)
        {
            ...
            {
                SendReply(player, msg("Error.NoPermission", player.userID.Get()));
                return false;
            }

            if (HasPendingTeleports(player))
            {
                SendReply(player, msg("Error.TPPending", player.userID.Get()));
                return false;
            }

            if (configData.Teleport.Hostile && player.IsHostile())
            {
                SendReply(player, msg("Error.Hostile", player.userID.Get()));
                return false;
            }

            if (configData.Teleport.BuildingBlocked && player.IsBuildingBlocked())
            {
                SendReply(player, msg("Error.BuildingBlocked", player.userID.Get()));
                return false;
            }
            ...
            if (cooldowns.TryGetValue(player.userID.Get(), out cooldown))
            {
                ...
                if (cooldownTime > 0)
                {
                    SendReply(player, string.Format(msg("Error.Cooldown", player.userID.Get()), FormatTime(cooldownTime)));
                    return false;
                }
            }
        ...
        private void cmdTeleportCancel(BasePlayer player, string command, string[] args)
        {
            if (!HasPendingTeleports(player))
            {
                SendReply(player, msg("Notification.NonePending", player.userID.Get()));
                return;
            }
