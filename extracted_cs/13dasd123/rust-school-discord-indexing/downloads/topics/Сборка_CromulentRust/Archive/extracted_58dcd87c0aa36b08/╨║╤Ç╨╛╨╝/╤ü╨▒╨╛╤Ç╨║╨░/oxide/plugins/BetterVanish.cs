...
namespace Oxide.Plugins
{
    ...
    public class BetterVanish : RustPlugin
    {
        ...
        private void VanishCommand(IPlayer iplayer, string command, string[] args)
        {
            ...
            {
                _safePoints[player.userID.Get()] = $"{player.transform.position.x} {player.transform.position.y} {player.transform.position.z}";
        ...
        private void Disappear(BasePlayer player, bool showGui = true)
        {
            ...
            {
                if (!_hiddenPlayers.Contains(player.userID.Get()))
                    _hiddenPlayers.Add(player.userID.Get());
                _hiddenPlayersPersist.Remove(player.userID.Get());
            }
            else
        ...
        private void Reappear(BasePlayer player)
        {
            if (!IsPlayerVanished(player) && !IsPlayerPersisted(player))
                return;
            _hiddenPlayers.Remove(player.userID.Get());
            _hiddenPlayersPersist.Remove(player.userID.Get());
        ...
        private object CanUseLockedEntity(BasePlayer player, BaseLock baseLock)
        {
            ...
            if (cl != null)
                return cl.whitelistPlayers.Contains(player.userID.Get()) || cl.guestPlayers.Contains(player.userID.Get()) ? TrueObj : FalseObj;
            return null;
        }
        ...
        {
            if(!_hiddenPlayersPersist.Contains(player.userID.Get()))
                _hiddenPlayersPersist.Add(player.userID.Get());
            _hiddenPlayers.Remove(player.userID.Get());
            ManageHooks();
        }
        ...
        private bool HasSafePoint(BasePlayer player) => _config.SafePoints && _safePoints.ContainsKey(player.userID.Get());
        ...
        private Vector3 GetSafePoint(BasePlayer player)
        {
            try
            {
                if (HasSafePoint(player))
                    return Vector3Ex.Parse(_safePoints[player.userID.Get()]);
            }
            catch { }
        ...
        private bool IsPlayerVanished(BasePlayer player) => player != null && _hiddenPlayers.Contains(player.userID.Get());
        
        private bool IsPlayerVanished(ulong playerId) => _hiddenPlayers.Contains(playerId);

        private bool IsPlayerPersisted(BasePlayer player) => player != null && _hiddenPlayersPersist.Contains(player.userID.Get());
