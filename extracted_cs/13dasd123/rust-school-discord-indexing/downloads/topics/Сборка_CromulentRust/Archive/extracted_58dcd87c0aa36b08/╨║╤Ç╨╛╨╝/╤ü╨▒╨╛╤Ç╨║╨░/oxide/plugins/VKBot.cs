...
namespace Oxide.Plugins
{
    ...
    class VKBot : RustPlugin
    {
        ...
        {
            if (usersdata.VKUsersData.ContainsKey(player.userID.Get()) && usersdata.VKUsersData[player.userID.Get()].Name != player.displayName)
            {
                usersdata.VKUsersData[player.userID.Get()].Name = player.displayName;
        ...
        {
            if (!usersdata.VKUsersData.ContainsKey(player.userID.Get())) return;
            ...
            if (config.BDayGift.BDayEnabled && permission.GroupExists(config.BDayGift.BDayGroup))
            {
                ...
                var bday = usersdata.VKUsersData[player.userID.Get()].Bdate;
                ...
                if (bday == DateTime.Now.ToString("d.M", CultureInfo.InvariantCulture))
                {
                    ...
                    BDayPlayers.Add(player.userID.Get());
        ...
        private void OnPlayerDisconnected(BasePlayer player, string reason)
        {
            if (config.BDayGift.BDayEnabled && permission.GroupExists(config.BDayGift.BDayGroup))
            {
                if (BDayPlayers.Contains(player.userID.Get()))
                {
                    permission.RemoveUserGroup(player.userID.ToString(), config.BDayGift.BDayGroup);
                    BDayPlayers.Remove(player.userID.Get());
                    Log("bday", $"Игрок {player.displayName} удален из группы {config.BDayGift.BDayGroup}");
                }
            }
            if (OpenReportUI.Contains(player)) OpenReportUI.Remove(player);
            if (usersdata.VKUsersData.ContainsKey(player.userID.Get())) { usersdata.VKUsersData[player.userID.Get()].LastSeen = DateTime.Now.ToString("dd.MM.yyyy HH:mm:ss"); VKBData.WriteObject(usersdata); }
        }
        ...
        private void OnDispenserGather(ResourceDispenser dispenser, BaseEntity entity, Item item)
        {
            ...
            if (config.TopWPlayersPromo.TopWPlEnabled)
            {
                ...
                if (usersdata.VKUsersData.ContainsKey(player.userID.Get())) usersdata.VKUsersData[player.userID.Get()].Farm = usersdata.VKUsersData[player.userID.Get()].Farm + item.amount;
            }
        }
        private void OnDispenserBonus(ResourceDispenser dispenser, BasePlayer player, Item item)
        {
            ...
            if (config.TopWPlayersPromo.TopWPlEnabled && usersdata.VKUsersData.ContainsKey(player.userID.Get())) usersdata.VKUsersData[player.userID.Get()].Farm = usersdata.VKUsersData[player.userID.Get()].Farm + item.amount;
        }
        ...
        void OnEntityDeath(BaseCombatEntity entity, HitInfo hitInfo)
        {
            if (config.TopWPlayersPromo.TopWPlEnabled)
            {
                ...
                if (entity is BaseEntity)
                {
                    ...
                    if (attacker.userID.Get() == entity.OwnerID) return;
                    ...
                    if (usersdata.VKUsersData.ContainsKey(attacker.userID.Get())) usersdata.VKUsersData[attacker.userID.Get()].Raids++;
                }
            }
        }
        private void CheckDeath(BasePlayer player, HitInfo info, BasePlayer attacker)
        {
            ...
            if (!usersdata.VKUsersData.ContainsKey(attacker.userID.Get())) return;
            if (!player.IsConnected) return;
            if (Duel && (bool)Duel?.Call("IsDuelPlayer", player)) return;
            usersdata.VKUsersData[attacker.userID.Get()].Kills++;
        }
        #endregion
        ...
        {
            string reportplayer = "";
            if (target != null) reportplayer = reportplayer + "Жалоба на игрока " + target.displayName + " (" + "steamcommunity.com/profiles/" + target.userID.Get() + "/) ";
            string reporttext = "[CROMULENT BOT]";
            statdata.Reports = statdata.Reports + 1;
            int reportid = statdata.Reports;
            StatData.WriteObject(statdata);
            if (config.MltServSet.MSSEnable) reporttext = reporttext;
            reporttext = reporttext + " " + player.displayName + " " + "(" + player.UserIDString + ")";
            if (usersdata.VKUsersData.ContainsKey(player.userID.Get()))
            {
                if (usersdata.VKUsersData[player.userID.Get()].Confirmed) reporttext = reporttext + ". ВК: vk.com/id" + usersdata.VKUsersData[player.userID.Get()].VkID;
                else reporttext = reporttext + ". ВК: vk.com/id" + usersdata.VKUsersData[player.userID.Get()].VkID + " (не подтвержден)";
            }
            ...
            ReportsData.WriteObject(reportsdata);
            Log("Log", $"{player.displayName} ({player.userID.Get()}): написал администратору: {reporttext}");
        ...
        private void CheckVkUser(BasePlayer player, string url)
        {
            ...
            webrequest.Enqueue("https://api.vk.com/method/users.get?user_ids=" + vkname + "&" + apiver + "&fields=bdate&access_token=" + config.VKAPIT.VKToken, null, (code, response) => {
                ...
                {
                    PrintWarning($"Ошибка проверки ВК профиля игрока {player.displayName} ({player.userID.Get()}). URL - {url}");
                    Log("checkresponce", $"Ошибка проверки ВК профиля игрока {player.displayName} ({player.userID.Get()}). URL - {url}. Ответ сервера ВК: {response}");
                }
            }, this);
        }
        private void AddVKUser(BasePlayer player, string Userid, string bdate)
        {
            if (!usersdata.VKUsersData.ContainsKey(player.userID.Get()))
            {
                usersdata.VKUsersData.Add(player.userID.Get(), new VKUDATA()
                {
                    UserID = player.userID.Get(),
                    Name = player.displayName,
                    VkID = Userid,
                ...
                VKBData.WriteObject(usersdata);
                SendConfCode(usersdata.VKUsersData[player.userID.Get()].VkID, $"Для подтверждения вашего ВК профиля введите в игровой чат  команду /vk confirm {usersdata.VKUsersData[player.userID.Get()].ConfirmCode}", player);
            }
            else
            {
                if (Userid == usersdata.VKUsersData[player.userID.Get()].VkID && usersdata.VKUsersData[player.userID.Get()].Confirmed) { PrintToChat(player, string.Format(GetMsg("ПрофильДобавленИПодтвержден"))); return; }
                if (Userid == usersdata.VKUsersData[player.userID.Get()].VkID && !usersdata.VKUsersData[player.userID.Get()].Confirmed) { PrintToChat(player, string.Format(GetMsg("ПрофильДобавлен"))); return; }
                usersdata.VKUsersData[player.userID.Get()].Name = player.displayName;
                usersdata.VKUsersData[player.userID.Get()].VkID = Userid;
                usersdata.VKUsersData[player.userID.Get()].Confirmed = false;
                usersdata.VKUsersData[player.userID.Get()].ConfirmCode = random.Next(1, 9999999);
                usersdata.VKUsersData[player.userID.Get()].Bdate = bdate;
                VKBData.WriteObject(usersdata);
                SendConfCode(usersdata.VKUsersData[player.userID.Get()].VkID, $"Для подтверждения вашего ВК профиля введите в игровой чат команду /vk confirm {usersdata.VKUsersData[player.userID.Get()].ConfirmCode}", player);
            }
        }
            ...
            if (args.Length > 0)
            {
                ...
                if (args[0] == "confirm")
                {
                    if (args.Length >= 2)
                    {
                        if (usersdata.VKUsersData.ContainsKey(player.userID.Get()))
                        {
                            if (usersdata.VKUsersData[player.userID.Get()].Confirmed) { PrintToChat(player, string.Format(GetMsg("ПрофильДобавленИПодтвержден"))); return; }
                            if (args[1] == usersdata.VKUsersData[player.userID.Get()].ConfirmCode.ToString())
                            {
                                usersdata.VKUsersData[player.userID.Get()].Confirmed = true;
                                VKBData.WriteObject(usersdata);
                                PrintToChat(player, string.Format(GetMsg("ПрофильПодтвержден")));
								//ПЕРЕМЕЩАЕМ ИГРОКА В ГРУППУ ВК							

								if (permission.GroupExists(vkgroup))
								{
									permission.AddUserGroup(player.UserIDString, vkgroup);									
								}else {PrintWarning($"Group \"{vkgroup}\" doesn't exist and wasn't added to player \"{player.displayName}\"");}
                    ...
                    {
                        if (!usersdata.VKUsersData.ContainsKey(player.userID.Get())) { PrintToChat(player, string.Format(GetMsg("ПрофильНеДобавлен"))); return; }
                        if (usersdata.VKUsersData[player.userID.Get()].Confirmed) { PrintToChat(player, string.Format(GetMsg("ПрофильДобавленИПодтвержден"))); return; }
                        SendConfCode(usersdata.VKUsersData[player.userID.Get()].VkID, $"Для подтверждения вашего ВК профиля введите в игровой чат команду /vk confirm {usersdata.VKUsersData[player.userID.Get()].ConfirmCode}", player);
                    }
                }
        ...
        {
            if (!usersdata.VKUsersData.ContainsKey(player.userID.Get())) { PrintToChat(player, string.Format(GetMsg("ПрофильНеДобавлен"))); return; }
            if (!usersdata.VKUsersData[player.userID.Get()].Confirmed) { PrintToChat(player, string.Format(GetMsg("ПрофильНеПодтвержден"))); return; }
            if (config.WipeStg.WCMDIgnore) { PrintToChat(player, string.Format(GetMsg("АвтоОповещенияОвайпе"))); return; }
            if (usersdata.VKUsersData[player.userID.Get()].WipeMsg)
            {
                usersdata.VKUsersData[player.userID.Get()].WipeMsg = false;
                VKBData.WriteObject(usersdata);
                PrintToChat(player, string.Format(GetMsg("ПодпискаОтключена")));
            }
            else
            {
                usersdata.VKUsersData[player.userID.Get()].WipeMsg = true;
                VKBData.WriteObject(usersdata);
        ...
        private void VKGift(BasePlayer player)
        {
            if (config.GrGifts.VKGroupGifts)
            {
                if (!usersdata.VKUsersData.ContainsKey(player.userID.Get())) { PrintToChat(player, string.Format(GetMsg("ПрофильНеДобавлен"))); return; }
                if (!usersdata.VKUsersData[player.userID.Get()].Confirmed) { PrintToChat(player, string.Format(GetMsg("ПрофильНеПодтвержден"))); return; }
                if (usersdata.VKUsersData[player.userID.Get()].GiftRecived) { PrintToChat(player, string.Format(GetMsg("НаградаУжеПолучена"))); return; }
                webrequest.Enqueue($"https://api.vk.com/method/groups.isMember?group_id={config.VKAPIT.GroupID}&user_id={usersdata.VKUsersData[player.userID.Get()].VkID}&" + apiver + $"&access_token={config.VKAPIT.VKToken}", null, (code, response) => {
        ...
        private void GetGift(int code, string Result, BasePlayer player)
        {
            if (Result == "1")
            {
                if (config.GrGifts.VKGroupGiftCMD == "none")
                {
                    ...
                    {
                        usersdata.VKUsersData[player.userID.Get()].GiftRecived = true;
                        VKBData.WriteObject(usersdata);
        ...
        private void GiftNotifier()
        {
            if (config.GrGifts.VKGroupGifts)
            {
                ...
                {
                    if (!usersdata.VKUsersData.ContainsKey(pl.userID.Get())) PrintToChat(pl, string.Format(GetMsg("ОповещениеОПодарках"), config.GrGifts.VKGroupUrl));
                    else
                    {
                        if (!usersdata.VKUsersData[pl.userID.Get()].GiftRecived) PrintToChat(pl, string.Format(GetMsg("ОповещениеОПодарках"), config.GrGifts.VKGroupUrl));
                    }
                }
            }
        }
        ...
        private void MsgAdmin(ConsoleSystem.Arg arg)
        {
            ...
            {
                string text = null;
                if (config.MltServSet.MSSEnable) text = $"[VKBot msgadmin] " + string.Join(" ", args.Skip(0).ToArray());
        ...
        private void CheckPlugins()
        {
            ...
            {
                string text = null;
                if (config.MltServSet.MSSEnable) text = $"[WARNING] Произошла ошибка загрузки следующих плагинов:";
                else text = $"[WARNING]  Произошла ошибка загрузки следующих плагинов:";
        ...
        private bool IsNPC(BasePlayer player)
        {
            ...
            if (!(player.userID.Get() >= 76560000000000000L || player.userID.Get() <= 0L))  return true;
            return false;
        }
        ...
        private void SetRandomPromo()
        {
            ...
            Config.WriteObject(config, true);
            string msg = "[CROMULENT BOT]";
            if (config.MltServSet.MSSEnable) msg = msg + "]";
        ...
        #region GUIBuilder
        private CuiElement BPanel(string name, string color, string anMin, string anMax, string parent = "Overlay", bool cursor = false, float fade = 1f)
        {
        ...
        private CuiElement Panel(string name, string color, string anMin, string anMax, string parent = "Overlay", bool cursor = false, float fade = 1f)
        {
            var Element = new CuiElement()
            {
                Name = name,
        ...
        private void StartVKBotAddVKGUI(BasePlayer player)
        {
            ...
            {
                BPanel("AddVKUI", GetColor(config.GUISet.BgColor), config.GUISet.AnchorMin, config.GUISet.AnchorMax, "Overlay", true),
        ...
        private void StartVKBotMainGUI(BasePlayer player)
        {
            ...
            CuiElementContainer container = new CuiElementContainer
            {
                BPanel("MainUI", GetColor(config.GUISet.BgColor), config.GUISet.AnchorMin, config.GUISet.AnchorMax, "Overlay", true),            
            ...
            if (usersdata.VKUsersData.ContainsKey(player.userID.Get()))
            {
                if (!usersdata.VKUsersData[player.userID.Get()].Confirmed) { addvkbuutontext = "Подтвердить профиль"; addvkbuttoncommand = "vk.menugui maingui.confirm"; NeedRemove = true; addvkbuttonanmax = "0.49 0.5"; }
                else { addvkbuutontext = "Профиль добавлен"; addvkbuttoncommand = ""; addvkbuttoncolor = "0 0.115 0 0.65"; }
                if (usersdata.VKUsersData[player.userID.Get()].GiftRecived) { giftvkbuutontext = "Награда за вступление\nв группу ВК получена"; giftvkbuttoncommand = ""; giftvkbuttoncolor = "0 0.115 0 0.65"; }
            }
            ...
            if (!config.WipeStg.WCMDIgnore)
            {
                ...
                if (usersdata.VKUsersData.ContainsKey(player.userID.Get()) && usersdata.VKUsersData[player.userID.Get()].WipeMsg) { text = "Отписаться от\nопвещений о вайпе в вк"; }
        ...
        private void StartVKBotHelpVKGUI(BasePlayer player)
        {
            CuiElementContainer container = new CuiElementContainer
            {
                BPanel("HelpUI", GetColor(config.GUISet.BgColor), config.GUISet.AnchorMin, config.GUISet.AnchorMax, "Overlay", true),
        ...
        private void StartCodeSendedGUI(BasePlayer player)
        {
            CuiElementContainer container = new CuiElementContainer
            {
                BPanel("CodeSendedUI", GetColor(config.GUISet.BgColor), config.GUISet.AnchorMin, config.GUISet.AnchorMax, "Overlay", true),
        ...
        private void CmdChoose(ConsoleSystem.Arg arg)
        {
            ...
            switch (arg.Args[0])
            {
                ...
                case "maingui.removevk":
                    CuiHelper.DestroyUi(player, "MainUI");
                    if (usersdata.VKUsersData.ContainsKey(player.userID.Get())) { usersdata.VKUsersData.Remove(player.userID.Get()); VKBData.WriteObject(usersdata); }
                    break;
                case "maingui.walert":
                    WAlert(player);
                    break;
        ...
        private void ReportGUI(BasePlayer player, BasePlayer target = null)
        {
            ...
            CuiElementContainer container = new CuiElementContainer
            {
                BPanel("ReportGUI", "0 0 0 0.75", "0.2 0.125", "0.8 0.9", "Overlay", true),
            ...
            if (target != null) command = command + target.userID.Get() + " ";
        ...
        private void PListUI(BasePlayer player)
        {
            ...
            CuiElementContainer container = new CuiElementContainer
            {
                BPanel("PListGUI", "0 0 0 0.75", "0.2 0.125", "0.8 0.9", "Overlay", true),
