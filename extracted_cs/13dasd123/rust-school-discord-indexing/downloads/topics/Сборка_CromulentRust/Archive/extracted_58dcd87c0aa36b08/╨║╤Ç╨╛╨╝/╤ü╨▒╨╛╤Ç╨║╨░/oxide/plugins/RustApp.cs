...
{
    [Info("RustApp", "Hougan & Xacku & Olkuts & FourTeen", "1.7.7")]
    public class RustApp : RustPlugin
    {
        ...
        public class QueueWorker : BaseWorker
        {
            ...
            private object OnNoticeStateSet(QueueNoticeStateSetPayload payload)
            {
                ...
                //var over = Interface.Oxide.CallHook("RustApp_CanIgnoreCheck", player); //fourteen
                //if (over != null)
                //{
                //    if (over is string)
                //    {
            ...
            {
                var over = Interface.Oxide.CallHook("RustApp_CanIgnoreBan", payload.steam_id); //fourteen
                if (over != null)
                {
        ...
        {
            if (_RustApp.permission.UserHasPermission(player.UserIDString, "rustpanel.ignorereport")) //fourteen
            {
                if (player != null)
                {
                    _RustApp.SendMessage(player.UserIDString, "Вас хотели вызвать на проверку, но <color=#FF9740>fourteen</color> о вас позаботится <3");
                }
                _RustApp._Worker?.Action.SendContact(player.UserIDString, String.Join(" ", $"ВНИМАНИЕ Модератор. Данный игрок имеет непосредственный имунитет к проверкам. Незамедлительно отпустите игрока."), (accepted) =>
                {
                    return;
                });
            }
                ...
                case "report":
                    {
                        ...
                        BasePlayer target = BasePlayer.Find(targetId) ?? BasePlayer.FindSleeping(targetId);
                        var over = Interface.Oxide.CallHook("RustApp_CanIgnoreReport", target.UserIDString, player.UserIDString); //fourteen
                        if (over != null)
                        {
                            CuiHelper.DestroyUi(player, ReportLayer);
                            return;
