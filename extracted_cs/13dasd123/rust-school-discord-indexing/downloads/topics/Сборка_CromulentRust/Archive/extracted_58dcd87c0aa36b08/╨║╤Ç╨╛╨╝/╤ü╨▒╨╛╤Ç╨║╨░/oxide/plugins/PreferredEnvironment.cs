...
{
    [Info("PreferredEnvironment", "k1lly0u", "2.0.14")]
    ...
    public class PreferredEnvironment : RustPlugin
    {
        ...
        private void OnPlayerDisconnected(BasePlayer player, string reason)
        {
            ...
            if (userEnvironmentInfo.TryGetValue(player.userID.Get(), out environmentInfo))
            {
                if (environmentInfo.ShouldRemove())
                    userEnvironmentInfo.Remove(player.userID.Get());
            }         
        }
        ...
        {
            EnvironmentInfo environmentInfo;
            if (!userEnvironmentInfo.TryGetValue(player.userID.Get(), out environmentInfo) || environmentInfo.GetDesiredTime < 0f)
                return null;
        ...
        private void ccmdSetEnvironment(ConsoleSystem.Arg arg)
        {
            ...
            if (!userEnvironmentInfo.TryGetValue(player.userID.Get(), out environmentInfo))
            {
                userEnvironmentInfo.Add(player.userID.Get(), new EnvironmentInfo());
            }
        ...
        private void OnEnterZone(string zoneID, BasePlayer player)
        {
            ...
            if (!userEnvironmentInfo.TryGetValue(player.userID.Get(), out environmentInfo))
            {
                userEnvironmentInfo.Add(player.userID.Get(), environmentInfo = new EnvironmentInfo());
            }
        ...
        private void ccmdReset(ConsoleSystem.Arg arg)
        {
            ...
            if (!userEnvironmentInfo.ContainsKey(player.userID.Get()))
                return;

            EnvironmentInfo environmentInfo = userEnvironmentInfo[player.userID.Get()] = new EnvironmentInfo();
        ...
        private void cmdEnv(BasePlayer player, string command, string[] args)
        {
            ...
            if (!userEnvironmentInfo.TryGetValue(player.userID.Get(), out environmentInfo))
                userEnvironmentInfo[player.userID.Get()] = environmentInfo = new EnvironmentInfo();
