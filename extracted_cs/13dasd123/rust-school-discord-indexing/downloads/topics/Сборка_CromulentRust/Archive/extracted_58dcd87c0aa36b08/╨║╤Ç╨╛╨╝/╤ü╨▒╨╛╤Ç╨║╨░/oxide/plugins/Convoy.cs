...
{
    [Info("Convoy", "Adem", "2.5.1")]
    class Convoy : RustPlugin
    {
        [PluginReference] Plugin NpcSpawn, GUIAnnouncements, DiscordMessages, PveMode, Economics, ServerRewards, IQEconomic, DynamicPVP, Friends, Clans;
        ...
        ConvoyController eventController;
        ...
        {
            if (entity == null || eventController == null || entity.net == null || entity.net.ID == null)
                return false;

            return eventController.GetConvoyVehicleByEntityNetId(entity.net.ID.Value) != null;
        }

        private bool IsConvoyCrate(HackableLockedCrate crate)
        {
            if (crate == null || eventController == null)
                return false;

            return eventController.GetTruckByCrateNetID(crate.net.ID.Value) != null;
        }

        private bool IsConvoyHeli(PatrolHelicopter patrolHelicopter)
        {
            if (patrolHelicopter == null || eventController == null || patrolHelicopter.net == null || patrolHelicopter.net == null || patrolHelicopter.net.ID == null)
                return false;

            return eventController.IsConvoyHeli(patrolHelicopter.net.ID.Value);
        }
        #endregion API
        ...
        void OnEntitySpawned(LockedByEntCrate crate)
        {
            ...
            {
                ConvoyBradley convoyBradley = eventController.GetConvoyBradleyAtPosition(crate.transform.position);

                if (convoyBradley == null)
                    return;
            ...
            {
                ConvoyHeli convoyHeli = eventController.GetCConvoyHeliAtPosition(crate.transform.position);
        ...
        void OnEntitySpawned(HelicopterDebris entity)
        {
            ...
            if (eventController.GetConvoyBradleyAtPosition(entity.transform.position) != null || eventController.GetCConvoyHeliAtPosition(entity.transform.position) != null)
            {
        ...
        object OnTurretTarget(AutoTurret turret, ScientistNPC scientistNPC)
        {
            ...
            if (NpcManager.IsConvoyNpc(scientistNPC.displayName))
            {
                if (!_config.enablePlayerTurret)
                    ...
                    return true;
                else if (!eventController.IsPlayerTurretCanAttackConvoy(turret))
                    return true;
            }
        ...
        {
            if (baseVehicleModule == null || info == null)
                return null;

            BaseModularVehicle modularVehicle = baseVehicleModule.Vehicle;

            if (modularVehicle == null || modularVehicle.net == null)
                return null;

            ConvoyVehicle convoyVehicle = eventController.GetConvoyVehicleByEntityNetId(modularVehicle.net.ID.Value);

            if (convoyVehicle == null)
                return null;

            if (eventController.CanConvoyEntityTakeDamage(info, baseVehicleModule))
            {
                ...
                if (info.InitiatorPlayer != null)
                    eventController.OnConvoyAttacked(info.InitiatorPlayer);
        ...
        object OnEntityTakeDamage(ModularCar entity, HitInfo info)
        {
            ...
            ConvoyVehicle convoyVehicle = eventController.GetConvoyVehicleByEntityNetId(entity.net.ID.Value);

            if (convoyVehicle == null)
                return null;

            if (eventController.CanConvoyEntityTakeDamage(info, entity))
            {
                ConvoyModular convoyModular = convoyVehicle as ConvoyModular;
        ...
        object OnEntityTakeDamage(BradleyAPC entity, HitInfo info)
        {
            ...
            if (eventController.CanConvoyEntityTakeDamage(info, entity))
            {
                if (info.InitiatorPlayer.IsRealPlayer()) eventController.OnConvoyAttacked(info.InitiatorPlayer);
                return null;
            }
            else
            {
        ...
        object OnEntityTakeDamage(ScientistNPC entity, HitInfo info)
        {
            ...
            if (NpcManager.IsConvoyNpc(entity.displayName) && eventController.CanConvoyNpcTakeDamage(info))
            {
        ...
        {
            if (buildingBlock == null || info.PointStart == null)
            {
                return;
            }
            else if (info.WeaponPrefab != null && info.WeaponPrefab.name == "MainCannonShell")
            {
                if (!eventController.convoyVehicles.Any(x => x != null && Vector3.Distance(info.PointStart, x.baseEntity.transform.position) < _config.maxDamageDistance * 2))
                    return;
            ...
            if (bradley != null)
            {
                ConvoyVehicle convoyVehicle = eventController.GetConvoyVehicleByEntityNetId(bradley.net.ID.Value);
        ...
        void OnEntityKill(ModularCar entity)
        {
            ...
            if (convoyVehicle != null)
            {
                eventController.OnConvoyVehicleDie(convoyVehicle);
                ...
                if (convoyTruck != null)
                {
                    ...
                    ins.NextTick(eventController.CheckConvoyHasNoLootedTruck);
                }
            }
        }
        ...
        void OnEntityKill(BradleyAPC entity)
        {
            ...
            if (convoyVehicle != null)
            {
                ...
                if (ins._config.pveMode.enable && ins.plugins.Exists("PveMode"))
                {
                    timer.In(1f, () =>
                    {
        ...
        void OnEntityKill(PatrolHelicopter entity)
        {
            ...
            if (IsConvoyHeli(entity) && _config.pveMode.enable && plugins.Exists("PveMode"))
            {
                timer.In(1f, () =>
                {
        ...
        {
            if (scientistNPC == null || scientistNPC.net == null || !NpcManager.IsConvoyNpc(scientistNPC.displayName) || scientistNPC.isMounted)
                return;

            if (ins._config.blockSpawnDieNpc)
            {
                ConvoyVehicle convoyVehicle = eventController.convoyVehicles.FirstOrDefault(x => x != null && x.roamNpcsNetIDs.Any(y => y == scientistNPC.net.ID.Value));
                if (convoyVehicle != null)
                    convoyVehicle.countDieNpc++;
            }

            if (eventController.IsConvoyStop() && _config.needKillNpc)
            {
                eventController.roamNpc.Remove(scientistNPC);
                if (eventController.roamNpc.Count == 0)
        ...
        void OnPlayerDeath(BasePlayer player, HitInfo info)
        {
            ...
            if (ZoneController.IsPlayerInZone(player.userID.Get()))
                ZoneController.OnPlayerLeaveZone(player);
        }

        void OnEntityDeath(ModularCar entity, HitInfo info)
        {
            if (entity == null || entity.net == null || info == null || !info.InitiatorPlayer.IsRealPlayer())
                return;
        ...
        void OnEntityDeath(PatrolHelicopter entity, HitInfo info)
        {
            ...
            if (IsConvoyHeli(entity))
                EconomyManager.ActionEconomy(info.InitiatorPlayer.userID.Get(), "Heli");
        }

        void OnEntityDeath(ScientistNPC scientistNPC, HitInfo info)
        {
            if (scientistNPC == null || !NpcManager.IsConvoyNpc(scientistNPC.displayName))
                return;

            if (info != null && info.InitiatorPlayer.IsRealPlayer())
                EconomyManager.ActionEconomy(info.InitiatorPlayer.userID.Get(), "Npc");
        }
        ...
        object CanHackCrate(BasePlayer player, HackableLockedCrate crate)
        {
            ...
            ConvoyTruck convoyTruck = eventController.GetTruckByCrateNetID(crate.net.ID.Value);
            ...
            else if ((!_config.needStopConvoy || eventController.IsConvoyStop()) && (!_config.needKillCars || !eventController.ConvoyHaveAnySecurityVehivcle()) && (!_config.needKillNpc || !eventController.roamNpc.Any(x => x.IsExists())))
            {
                if (_config.pveMode.enable && plugins.Exists("PveMode") && !_config.pveMode.hackCrate && PveMode.Call("CanActionEvent", Name, player) != null)
                    return true;

                NotifyManager.SendMessageToAll("StartHackCrate", _config.prefix, player.displayName);
                eventController.OnConvoyAttacked(player);
                convoyTruck.UpdateCrateHackTimeWithDelay();
                EconomyManager.ActionEconomy(player.userID.Get(), "LockedCrate");
        ...
        void OnLootEntity(BasePlayer player, HackableLockedCrate crate)
        {
            ...
            if (convoyTruck != null)
                eventController.OnCrateLooted(crate.net.ID.Value);
        }
        ...
        object CanMountEntity(BasePlayer player, BaseMountable entity)
        {
            ...
            if (vehicle != null && IsConvoyVehicle(vehicle))
                return true;
        ...
        {
            if (eventController.convoyHeli != null && eventController.convoyHeli.patrolHelicopterAI == ai)
                return true;
            else
                return null;
        }
        ...
        {
            if (heli == null || heli.helicopterBase == null)
                return null;
            if (IsConvoyHeli(heli.helicopterBase))
            {
                if (!player.IsRealPlayer() || player.IsSleeping())
                    return false;
                else if (!_config.isAggressive && !eventController.IsConvoyStop() && !eventController.IsAnyCrateHacking())
                    return false;
            }
        ...
        {
            if (apc == null || entity == null || apc.net == null)
                return null;

            ConvoyVehicle convoyVehicle = eventController.GetConvoyVehicleByEntityNetId(apc.net.ID.Value);

            if (convoyVehicle == null)
                return null;

            if (!_config.isAggressive && !eventController.IsConvoyStop() && !eventController.IsAnyCrateHacking())
                return false;

            BasePlayer player = entity as BasePlayer;
            if (!player.IsRealPlayer() || player.IsSleeping() || player.InSafeZone())
                return false;
        ...
        void OnExplosiveThrown(BasePlayer player, BaseEntity entity, ThrownWeapon item)
        {
            ...
            if (eventController.convoyVehicles.Any(x => x != null && x.baseEntity != null && Vector3.Distance(player.transform.position, x.baseEntity.transform.position) < 25f))
                eventController.OnConvoyAttacked(player);
        }
        ...
        {
            if (eventController == null || npc == null)
                return null;
            else if (NpcManager.IsConvoyNpc(npc) && !eventController.IsConvoyStop() && !_config.isAggressive)
                return false;
        ...
        {
            if (eventController == null || container == null)
                return null;

            if (container is HackableLockedCrate)
            {
                ConvoyTruck convoyTruck = eventController.GetTruckByCrateNetID(container.net.ID.Value);

                if (convoyTruck == null)
                    return null;
                else if (convoyTruck.modularConfig.typeLootTable == 2)
                    return null;
                else
                    return true;
            }
            ...
            {
                ConvoyBradley convoyBradley = eventController.GetConvoyBradleyAtPosition(container.transform.position);
            ...
            {
                ConvoyHeli convoyHeli = eventController.GetCConvoyHeliAtPosition(container.transform.position);
        ...
        {
            if (eventController == null || scientistNPC == null || corpse == null || eventController.convoyVehicles.Count == 0) return null;
            if (eventController.convoyVehicles.Any(x => x != null && x.scientists.Contains(scientistNPC)))
            {
        ...
        {
            if (eventController == null || netID == null) return null;
            ConvoyTruck convoyTruck = eventController.GetTruckByCrateNetID(netID.Value);
        ...
        {
            if (eventController == null || !_config.eventZone.isCreateZonePVP || hitinfo == null || !victim.IsRealPlayer())
                return null;

            if (_config.eventZone.isCreateZonePVP && !_config.pveMode.enable && ZoneController.IsPlayerInZone(victim.userID.Get()) && hitinfo.InitiatorPlayer.IsRealPlayer() && ZoneController.IsPlayerInZone(hitinfo.InitiatorPlayer.userID.Get()))
                return true;
            else
                return null;
        }
        ...
        {
            if (eventController == null || info == null || entity == null || !info.InitiatorPlayer.IsRealPlayer())
                return null;

            if (NpcManager.IsConvoyNpc(entity.displayName))
            {
                BaseVehicle baseVehicle = entity.GetMountedVehicle();

                if (!eventController.CanConvoyNpcTakeDamage(info))
                    return false;
        ...
        {
            if (eventController == null || victim == null || hitinfo == null)
                return null;

            ConvoyVehicle convoyVehicle = eventController.GetConvoyVehicleByEntityNetId(victim.net.ID.Value);

            if (convoyVehicle != null && hitinfo.InitiatorPlayer.IsRealPlayer())
            {
                if (!eventController.CanConvoyEntityTakeDamage(hitinfo, victim))
                    return false;

                if (PveModeController.IsPveModeBlockAction(hitinfo.InitiatorPlayer))
                    return false;
                else
                    return true;
            }
        ...
        {
            if (eventController == null || victim == null || victim.net == null || hitinfo == null || !hitinfo.InitiatorPlayer.IsRealPlayer())
                return null;
        ...
        {
            if (eventController == null || baseVehicleModule == null || baseVehicleModule.Vehicle == null || baseVehicleModule.Vehicle.net == null || hitinfo == null || !hitinfo.InitiatorPlayer.IsRealPlayer())
                return null;

            ConvoyVehicle convoyVehicle = eventController.GetConvoyVehicleByEntityNetId(baseVehicleModule.Vehicle.net.ID.Value);

            if (convoyVehicle != null && hitinfo.InitiatorPlayer.IsRealPlayer())
            {
                if (!eventController.CanConvoyEntityTakeDamage(hitinfo, baseVehicleModule.Vehicle))
                    return false;
        ...
        object CanEntityTakeDamage(PatrolHelicopter entity, HitInfo hitinfo)
        {
            ...
            {
                if (!eventController.CanConvoyEntityTakeDamage(hitinfo, entity))
                    return false;
            }
        ...
        {
            if (eventController == null || entity == null)
                return null;

            if (IsConvoyVehicle(entity))
                return true;
        ...
        {
            if (eventController == null || _config.betterNpcConfig.bradleyNpc || bradley == null || bradley.net == null)
                return null;

            if (eventController.GetConvoyVehicleByEntityNetId(bradley.net.ID.Value) != null)
                return true;
        ...
        {
            if (eventController == null || _config.betterNpcConfig.heliNpc || helicopter == null)
                return null;

            if (IsConvoyHeli(helicopter))
                return true;
        ...
        private object OnRestoreUponDeath(BasePlayer player)
        {
            ...
            if (_config.eventZone.blockRestoreUponDeath && ZoneController.IsPlayerInZone(player.userID.Get()))
                return false;
            ...
            if (_config.version != Version.ToString())
            {
                ...
                if (versionNumber.Major == 2)
                {
                    ...
                    if (versionNumber.Minor == 4)
                    {
                        ...
                        if (versionNumber.Patch <= 4)
                        {
                            _config.barriers.Remove("testridablehorse");
                        }
                        if (versionNumber.Patch <= 5)
                        {
        ...
        static class ConvoyLauncher
        {
            ...
            internal static void StartEvent(BasePlayer initiator = null, string presetName = "")
            {
                ...
                ins.eventController = gameObject.AddComponent<ConvoyController>();
                ins.eventController.Init(convoysetting, convoyPath);
            }
        ...
        {
            internal ConvoySetting eventConfig;
            ...
            {
                this.eventConfig = convoySetting;
            ...
            IEnumerator EventCounter()
            {
                ...
                if (ins._config.enableStartStopLogs)
                    NotifyManager.PrintLogMessage("EventStart_Log", eventConfig.name);
                CreateConvoy();
                ...
                NotifyManager.SendMessageToAll("EventStart", ins._config.prefix, eventConfig.displayName);
            ...
            void CreateConvoy()
            {
                ...
                for (int i = eventConfig.vehiclesOrder.Count - 1; i >= 0; i--)
                {
                    string vehiclePresetName = eventConfig.vehiclesOrder[i];
                    int secondpoint = 0;
                ...
                if (eventConfig.heliOn)
                {
                    HeliConfig heliConfig = ins._config.heliesConfiguration.FirstOrDefault(x => x.presetName == eventConfig.heliConfigurationName);
            ...
            void StartConvoy()
            {
                ...
                roamNpc.Clear();
                Interface.CallHook("OnConvoyStartMoving", GetEventPosition());
            ...
            internal void OnConvoyAttacked(BasePlayer initiator = null)
            {
                ...
                if (initiator != null && !convoyAttackersIds.Contains(initiator.userID.Get()))
                    convoyAttackersIds.Add(initiator.userID.Get());
                ...
                Invoke(() =>
                {
                    if (ins.eventController != null)
                    {
                        ZoneController.CreateZone(GetEventPosition(), initiator);
                        Interface.CallHook("OnConvoyStopMoving", GetEventPosition());
                    }
        ...
        sealed class ConvoySedan : ConvoyVehicle
        {
            ...
            internal override void CheckRotate()
            {
                ...
                if (currentPoint >= convoyPath.currentPath.Count - 2) ins.eventController.ReverseConvoy();
                if (UnityEngine.Physics.RaycastAll(transform.position, basicCar.transform.forward, 7.5f, 1 << 16).Any(x => x.collider != null && x.collider.name.Contains("cliff"))) ins.eventController.ReverseConvoy();
            }
            ...
            void FixedUpdate()
            {
                ...
                {
                    ins.eventController.ReverseConvoy();
                    return;
                }
        ...
        class ConvoyModular : ConvoyVehicle
        {
            ...
            {
                if (followVehicle != null) return;
                if (currentPoint >= convoyPath.currentPath.Count - 3) ins.eventController.ReverseConvoy();

                if (UnityEngine.Physics.RaycastAll(transform.position, modularCar.transform.forward, 7.5f, 1 << 16).Any(x => x.collider != null && x.collider.name.Contains("cliff"))) ins.eventController.ReverseConvoy();
            }
            ...
            void ControlTurn()
            {
                ...
                if (rigidbody.velocity.magnitude < maxSpeed)
                {
                    ...
                    {
                        float distance = Vector3.Distance(modularCar.transform.position, followVehicle.transform.position);
                        float speed = GetSpeed(distance, 1.5f, 0.315f, supportModularConfig.frontVehicleDistance);
                        if (speed > 0)
                ...
                if (driver != null && inputState != null)
                    modularCar.PlayerServerInput(inputState, driver);
            }
        ...
        sealed class ConvoyBradley : ConvoyVehicle
        {
            ...
            {
                if (followVehicle != null) return;
                if (bradley.currentPathIndex >= convoyPath.currentPath.Count - 2) ins.eventController.ReverseConvoy();
                if (UnityEngine.Physics.RaycastAll(transform.position, bradley.transform.forward, 7.5f, 1 << 16).Any(x => x.collider != null && x.collider.name.Contains("cliff"))) ins.eventController.ReverseConvoy();
            }
        ...
        abstract class ConvoyVehicle : FacepunchBehaviour
        {
            ...
            protected static void DeleteEntitiesInSpawnPosition(Vector3 pos)
            {
                ...
                {
                    BaseEntity entity = collider.ToBaseEntity();
                    if (entity.IsExists() && entity.net != null && ins._config.barriers.Contains(entity.ShortPrefabName) && ins.eventController.GetConvoyVehicleByEntityNetId(entity.net.ID.Value) == null) entity.Kill();
                }
            }
            ...
            void OnCollisionEnter(Collision collision)
            {
                ...
                if (entity is JunkPile || entity is TreeEntity || entity is ResourceEntity || entity is TravellingVendor)
                {
                    entity.Kill(BaseNetworkable.DestroyMode.Gib);
                    return;
                }

                if (ins._config.barriers.Contains(entity.ShortPrefabName) && ins.eventController.GetConvoyVehicleByEntityNetId(entity.net.ID.Value) == null)
                {
                    ...
                    if (vehicle != null)
                    {
                        ...
                        if (modularCar != null)
                        {
                            if (modularCar.IsLocked() || modularCar.HasAnyWorkingEngines())
                                return;
                        }
                    }
            ...
            {
                int index = ins.eventController.convoyVehicles.IndexOf(this);

                if (index == 0) followVehicle = null;
                else followVehicle = ins.eventController.convoyVehicles[index - 1];

                if (index >= ins.eventController.convoyVehicles.Count - 1) previusVehicle = null;
                else previusVehicle = ins.eventController.convoyVehicles[index + 1];
            }
            ...
            void CreateRoamNpc()
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    if (scientist != null)
                    {
                        ins.eventController.roamNpc.Add(scientist);
        ...
        class ConvoyHeli : FacepunchBehaviour
        {
            ...
            {
                if (ins.eventController == null || patrolHelicopter.myAI._currentState == PatrolHelicopterAI.aiState.DEATH || patrolHelicopter.myAI._currentState == PatrolHelicopterAI.aiState.STRAFE)
                    return;

                Vector3 targetPosition = ins.eventController.GetEventPosition() + new Vector3(0, heliConfig.height, 0);
                float distanceToTargetPosition = Vector3.Distance(targetPosition, patrolHelicopter.transform.position);

                if (ins.eventController.IsConvoyStop())
                {
        ...
        sealed class ZoneController : FacepunchBehaviour
        {
            ...
            internal static void CreateZone(Vector3 position, BasePlayer initiator)
            {
            ...
            internal static void DeleteZone(bool isEventEnd)
            {
                if (ins._config.pveMode.enable && ins.plugins.Exists("PveMode"))
                {
                    if (!isEventEnd)
                ...
                if (zoneController != null)
                {
                    ...
                    if (ins._config.pveMode.enable && ins.plugins.Exists("PveMode"))
                        ins.PveMode.Call("EventRemovePveMode", ins.Name, ins._config.pveMode.resetOwnerAfterStartMoving || isEventEnd);
                }
                else if (ins._config.pveMode.enable && ins.plugins.Exists("PveMode") && isEventEnd)
                    ins.PveMode.Call("EventAddCooldown", ins.Name, pveModeOwners, ins._config.pveMode.cooldownOwner);
            }

            internal static bool IsPlayerInZone(ulong userID)
            {
                return zoneController != null && zoneController.playersInZone.Any(x => x != null && x.userID.Get() == userID);
            }
            ...
            internal static string GetEventOwnerPlayerName()
            {
                if (zoneController == null)
                {
                    if (pveModeOwner != 0)
                    {
                        if (Time.realtimeSinceStartup - timeScienceLastZoneCreated < ins._config.pveMode.timeExitOwner && !ins._config.pveMode.resetOwnerAfterStartMoving)
                        {
            ...
            void Init(BasePlayer initiator)
            {
                CreateTriggerSphere();

                if (ins._config.pveMode.enable && ins.plugins.Exists("PveMode"))
                    CreatePveModeZone(initiator);
            ...
            void CreatePveModeZone(BasePlayer initiator)
            {
                Dictionary<string, object> config = new Dictionary<string, object>
                {
                    ["Damage"] = ins._config.pveMode.damage,
                    ["ScaleDamage"] = ins._config.pveMode.scaleDamage,
                ...
                if (pveModeOwner != 0 && Time.realtimeSinceStartup - timeScienceLastZoneCreated < ins._config.pveMode.timeExitOwner && !ins._config.pveMode.resetOwnerAfterStartMoving)
                    playerOwner = BasePlayer.FindByID(pveModeOwner);
                else if (ins._config.pveMode.ownerWhoStoppedConvoy && initiator.IsRealPlayer())
                    playerOwner = initiator;

                if (ins.eventController.convoyHeli != null && ins.eventController.convoyHeli.patrolHelicopter != null)
                    helicopters.Add(ins.eventController.convoyHeli.patrolHelicopter.net.ID.Value);

                foreach (ScientistNPC scientistNPC in ins.eventController.roamNpc)
                    if (scientistNPC != null && scientistNPC.net != null)
                        npcs.Add(scientistNPC.net.ID.Value);

                foreach (ConvoyVehicle convoyVehicle in ins.eventController.convoyVehicles)
                    if (convoyVehicle is ConvoyBradley)
                        bradleys.Add(convoyVehicle.baseEntity.net.ID.Value);

                foreach (ConvoyTruck truck in ins.eventController.trucks)
                    if (truck != null && truck.crate != null)
                        crates.Add(truck.crate.net.ID.Value);

                ins.PveMode.Call("EventAddPveMode", ins.Name, config, gameObject.transform.position, ins._config.eventZone.radius, crates, npcs, bradleys, helicopters, new HashSet<ulong>(), pveModeOwners, playerOwner);
            }
            ...
            IEnumerator GuiCoroune()
            {
                while (true)
                {
                    foreach (BasePlayer player in playersInZone) MessageGUI(player, GetMessage("GUI", player.UserIDString, NotifyManager.GetTimeMessage(player.UserIDString, ins.eventController.GetEventTime())));
                    yield return CoroutineEx.waitForSeconds(1f);
                }
            }
            ...
            void OnTriggerEnter(Collider other)
            {
                ...
                if (player.IsRealPlayer())
                {
                    ...
                    if (ins._config.GUI.IsGUI) MessageGUI(player, GetMessage("GUI", player.UserIDString, NotifyManager.GetTimeMessage(player.UserIDString, ins.eventController.GetEventTime())));
            ...
            void OnTriggerExit(Collider other)
            {
                ...
                if (player != null && player.userID.IsSteamId())
                {
                    ...
                    {
                        if (ins.plugins.Exists("DynamicPVP") && (bool)ins.DynamicPVP.Call("IsPlayerInPVPDelay", player.userID.Get())) return;
        ...
        class EventMapMarker : FacepunchBehaviour
        {
            ...
            {
                while (ins.eventController != null)
                {
            ...
            void UpdateVendingMarker(Vector3 position)
            {
                ...
                string displayEventOwnerName = ins._config.pveMode.showEventOwnerNameOnMap && pveModeEventOwner != null ? GetMessage("Marker_EventOwner", null, pveModeEventOwner.displayName) : "";
        ...
        static class NpcManager
        {
            ...
            internal static bool IsConvoyNpc(ScientistNPC scientistNPC)
            {
                return scientistNPC._lastSetName == ins.Name;
            }

            internal static ScientistNPC CreateNpc(JObject npcConfig, Vector3 position)
            {
                ScientistNPC scientistNPC = (ScientistNPC)ins.NpcSpawn.Call("SpawnNpc", position, npcConfig);
                ins.NextTick(() => scientistNPC._lastSetName = ins.Name);
                return scientistNPC;
            }
        ...
        static class EconomyManager
        {
            ...
            {
                ConvoyVehicle convoyVehicle = ins.eventController.GetConvoyVehicleByEntityNetId(vehicleNetId);
                if (convoyVehicle == null || player == null) return;
                ActionEconomy(player.userID.Get(), vehicleType);
            }
            ...
            static void DefineEventWinner()
            {
                ...
                Interface.CallHook("OnConvoyEventWin", winnerPair.Key);

                if (winnerPair.Value > 0 && ins._config.economyConfig.enable)
                {
                    if (winnerPair.Value >= ins._config.economyConfig.minCommandPoint)
                        foreach (string command in ins._config.economyConfig.commands)
        ...
        static class PveModeController
        {
            ...
            {
                if (!ins._config.pveMode.enable)
                    return false;

                if (eventOwner == null)
                    return false;

                if (player.userID.Get() == eventOwner.userID.Get())
                    return false;

                if (IsTeam(player, eventOwner.userID.Get()))
                    return false;
            ...
            static bool IsTeam(BasePlayer player, ulong targetId)
            {
                ...
                if (ins.plugins.Exists("Friends") && (bool)ins.Friends.Call("AreFriends", player.userID.Get(), targetId))
                    return true;
        ...
        protected override void LoadDefaultMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                ["PveMode_NewOwner"] = "{0} <color=#55aaff>{1}</color> est devenu le propriétaire de l'événement!",
            }, this, "fr");
        ...
        public class PveModeConfig
        {
            ...
            [JsonProperty(en ? "Reset the event owner at the beginning of the convoy movement" : "Сбрасывать владельца ивента при начале движения конвоя")] public bool resetOwnerAfterStartMoving { get; set; }
            [JsonProperty(en ? "The owner will immediately be the one who stopped the convoy" : "Владельцем сразу будет становиться тот, кто остановил конвой")] public bool ownerWhoStoppedConvoy { get; set; }
        ...
        class PluginConfig
        {
            ...
            [JsonProperty(en ? "Marker Setting" : "Настройки маркера", Order = 27)] public MarkerConfig markerConfig { get; set; }
