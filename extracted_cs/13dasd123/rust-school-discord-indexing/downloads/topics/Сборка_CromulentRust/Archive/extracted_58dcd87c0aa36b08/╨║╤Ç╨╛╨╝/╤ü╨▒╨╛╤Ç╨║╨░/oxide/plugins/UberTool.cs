...
{
    [Info("UberTool", "FuJiCuRa & FourTeen", "1.4.37", ResourceId = 78)]
    ...
    {
        public const string permhammer = "ubertool.hammer.use";
        public const string permremover = "ubertool.remover.use";
        public const string permplanner = "ubertool.planner.use";
        public const string permbuild = "ubertool.build.everywhere";

        //Dictionary<ulong, Dictionary<ulong, int>> limit = new Dictionary<ulong, Dictionary<ulong, int>>();

        [PluginReference]
        private Plugin Clans, ImageLibrary, AdvancedEntityLimit, BuildProtection;
        ...
        private void PpltBldngSkns()
        {
            ...
            Debug.Log(bldngSkns.Select(x => $"{x.Nm} {x.Skn}").ToSentence());
        }
        ...
        public class EPlanner : MonoBehaviour
        {
            ...
            private void Awake()
            {
                ...
                int grade = Instance.playerPrefs.playerData[player.userID.Get()].DBG;
                ulong skinId = Instance.playerPrefs.playerData[player.userID.Get()].DBS;
            ...
            private void GetTool(object[] tool)
            {
                ...
                if (p1 != null)
                {
                    ...
                    if (!player.HasPermission(permhammer) && p1.name == "UberHammer")
                    {
                        return;
                    }
                    else if (!player.HasPermission(permremover) && p1.name == "UberRemove")
                    {
                        return;
                    }
                    else if (!player.HasPermission(permplanner) && p1.name == "UberTool")
                    {
                        return;
                    }
                ...
                else
                {
                    ...
                    if (p2 != null)
                    {
                        ...
                        if (!player.HasPermission(permhammer) && p2.name == "UberHammer")
                        {
                            return;
                        }
                        else if (!player.HasPermission(permremover) && p2.name == "UberRemove")
                        {
                            return;
                        }
                        else if (!player.HasPermission(permplanner) && p2.name == "UberTool")
                        {
                            return;
                        }
            ...
            private void CrtTls()
            {
                if (Instance.checkExistingPlanner && player.HasPermission(permplanner)) GetTool(Instance.playerTools[0]);
                if (Instance.checkExistingRemover && player.HasPermission(permremover)) GetTool(Instance.playerTools[1]);
                if (Instance.checkExistingHammer && player.HasPermission(permhammer)) GetTool(Instance.playerTools[2]);
            }
            ...
            private bool GetCurrentTool()
            {
                ...
                {
                    if (!player.HasPermission(permplanner))
                    {
                        return false;
                    }
                ...
                {
                    if (!player.HasPermission(permremover))
                    {
                        return false;
                    }
                    sRmvr = true;
                ...
                {
                    if (!player.HasPermission(permhammer))
                    {
                        return false;
                    }
                    isHammering = true;
            ...
            {
                bool hsLsr = false;
                if (!player.HasPermission(permremover) && ctvtmLnk.name == "UberRemove")
                {
                    return;
                }
                ...
                if (uid != ctvtm)
                {
                    ...
                    else if (isPlanner || isHammering)
                    {
                        ...
                        if (isPlanner)
                        {
                            ...
                            else DoPlannerUpdate(PType.Mode, $"{construction.info.name.english} ({bldngSkns[grdIdx].Nm})");
                        }
                        else
                        {
            ...
            public void TickUpdate(PlayerTick tick)
            {
                ...
                if (changedInput && !ctvTrgt)
                {
                    ...
                    else if (isAnotherHeld)
                    {
                        ...
                        else if (heldItem is BaseProjectile && serverInput.WasJustPressed((BTN)8192))
                        {
                            ...
                            {
                                if (!player.HasPermission(permremover) && weapon.name == "UberRemove")
                                {
                                    return;
                                }
                                try
                        ...
                        if (player.serverInput.WasJustPressed(BTN.FIRE_PRIMARY))
                        {
                            ...
                            if (Physics.Raycast(player.eyes.HeadRay(), out raycastHit, 5f))
                            {
                                ...
                                if (lightsDeployer.active != null)
                                {
                                    ...
                                    {
                                        length = item1;
                                        raycastHit.point = position + (UnityEngine.Vector3Ex.Direction(raycastHit.point, position) * length);
                                    }
                                ...
                                lightsDeployer.active.AddPoint(raycastHit.point, raycastHit.normal, 1);
                ...
                if (isPlanner && !sTpDplybl)
                {
                    if (lastSocketBase != null && lastPlacement != null && lastSocketEntity)
                    {
            ...
            public void DoTick()
            {
                ...
                if (isPlanner)
                {
                    ...
                    if (!sTpDplybl)
                    {
                        ...
                        {
                            BldMnUI(Instance.playerPrefs.playerData[player.userID.Get()].SF);
                            return;
                        }
                        ...
                        if (serverInput.IsDown(controlButtons[CmdType.PlannerTierChange]))
                        {
                            if (serverInput.WasJustPressed(controlButtons[CmdType.PlannerTierNext]))
                            {
                                ...
                                Instance.playerPrefs.playerData[player.userID.Get()].DBG = (int)skin.Grd;
                                Instance.playerPrefs.playerData[player.userID.Get()].DBS = skin.Skn;
                ...
                else if (isHammering)
                {
                    ...
                    else if (!ctvTrgt)
                    {
                        ...
                        {
                            if (rayEntity.OwnerID != player.userID.Get()) { return; };
                            BuildingBlock block = rayEntity as BuildingBlock;
                        ...
                        {
                            if (player.IsBuildingBlocked()) return;
                            if (rayEntity.OwnerID == 0) return;
                            if (rayEntity.children != null)
                            {
                                List<BaseEntity> objectsToRemove = new List<BaseEntity>();

                                foreach (var current in rayEntity.children)
                                {
                                    if (current.ShortPrefabName == "explosive.timed.deployed")
                                    {
                            ...
                            if (rayEntity is BaseVehicle) return;
                            if (rayEntity is BaseVehicleSeat) return;
                            if (rayEntity is RockingChair) return;
                            if (mvTrgt.entity != null)
                            {
                                mvTrgt =
                            default(Construction.Target);
            ...
            private void DoRm(bool remAl = false)
            {
                ...
                if (rayEntity.OwnerID == 8767) return;
                if (rayEntity.IsValid())
                {
                    if (Instance.BuildProtection) //aaa
                    {
                        BaseEntity ent = rayEntity;
                        if (ent != null)
                        {
                            if (ent.OwnerID != 0)
                            {
                                BuildingPrivlidge cupboard = ent.GetBuildingPrivilege();
                                if (cupboard != null && cupboard.OwnerID != player.userID.Get())
                                {
                                    //print($"1, {cupboard.OwnerID}, {player.userID.Get()}");
                                    object protectionData = Instance.BuildProtection.CallHook("GetProtectionData", cupboard, player.userID.Get());
                                    if (protectionData != null && protectionData is Dictionary<string, object>)
                                    {
                                        ...
                                        //print($"3, {creator}, {initiatorLvl}, {creatorLvl}");
                                        if (creator != player.userID.Get())
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                if (creatorPlayer != null) Instance.BuildProtection.CallHook("Message", creatorPlayer, initiatorLvl);
                                            }
                                        }
                                    }
                                }
                    ...
                    else
                    {
                        ...
                        else if (rayEntity is BaseNpc || rayEntity is NPCPlayer || rayEntity is BradleyAPC || rayEntity is PatrolHelicopter)
                        {
            ...
            private void CollRm(BaseEntity srcntt, BasePlayer player)
            {
                ...
                if (bldngBlck)
                {
                    ...
                    ServerMgr.Instance.StartCoroutine(DlyRm(building.buildingBlocks.ToList(), building.decayEntities.ToList(), building.buildingPrivileges.ToList(), player));
                }
            }

            private WaitForEndOfFrame wait = new WaitForEndOfFrame();

            private IEnumerator DlyRm(List<BuildingBlock> bLst, List<DecayEntity> dLst, List<BuildingPrivlidge> pLst, BasePlayer player)
            {
                ...
                for (int i = 0; i < pLst.Count; i++)
                    ...
                    {
                        object protectionData = Instance.BuildProtection.CallHook("GetProtectionData", pLst[i], player.userID.Get());
                        if (protectionData != null && protectionData is Dictionary<string, object>)
                        {
                            var data = (Dictionary<string, object>)protectionData;
                            ulong creator = (ulong)data["Creator"];
                        ...
                        if (pLst[i].OwnerID == player.userID.Get() || canremove == true)
                        {
                            if (pLst[i] == rayEntity)
                            {
                                rayEntity = null;
                ...
                for (int i = 0; i < dLst.Count; i++)
                    ...
                    {
                        object protectionData = Instance.BuildProtection.CallHook("GetProtectionData", dLst[i], player.userID.Get());
                        if (protectionData != null && protectionData is Dictionary<string, object>)
                        {
                            var data = (Dictionary<string, object>)protectionData;
                            ulong creator = (ulong)data["Creator"];
                        ...
                        if (dLst[i].OwnerID == player.userID.Get() || canremove == true)
                        {
                            if (dLst[i] == rayEntity)
                            {
                                rayEntity = null;
                ...
                for (int i = 0; i < bLst.Count; i++)
                    ...
                    {
                        object protectionData = Instance.BuildProtection.CallHook("GetProtectionData", bLst[i], player.userID.Get());
                        if (protectionData != null && protectionData is Dictionary<string, object>)
                        {
                            var data = (Dictionary<string, object>)protectionData;
                            ulong creator = (ulong)data["Creator"];
            ...
            public GameObject DoPlaG(Construction.Target p, Construction component)
            {
                ...
                bsntt.Spawn();
                if (Instance.AdvancedEntityLimit != null)
                {
                    var player = BasePlayer.FindByID(bsntt.OwnerID);
                    if (player == null)
                        return null;

                    var res = (KeyValuePair<int, bool>)Instance.AdvancedEntityLimit.Call("CanPlayerBuild", player, bsntt.PrefabName);
                    if (res.Value == false) { bsntt.Kill(); return null; }
                }
                //if (Instance.limit.TryGetValue(bsntt.OwnerID, out Dictionary<ulong, int> ownerDict))
                //{
                //    int last = ownerDict.Values.Any() ? ownerDict.Values.Max() + 1 : 1;
                ...
                //    ownerDict[bsntt.net.ID.Value] = last;
                //    Instance.limit[bsntt.OwnerID] = ownerDict;
                //}
                //else
                //{
                //    ownerDict = new Dictionary<ulong, int>();
                //    ownerDict[bsntt.net.ID.Value] = 1;
                //    Instance.limit[bsntt.OwnerID] = ownerDict;
                //}
                //Instance.SaveData();
            ...
            private bool UpdtPlcmnt(Transform tn, Construction common, ref Construction.Target target)
            {
                ...
                foreach (Socket_Base current in list)
                {
                    ...
                    if (!(target.entity != null) || !(target.socket != null) || !target.entity.IsOccupied(target.socket))
                    {
                        ...
                        if (plcmnt != null)
                        {
                            ...
                            {
                                current.socketMods[i].ModifyPlacement(plcmnt);
                            }
                ...
                if (_errOrr.IsMatch(Construction.lastPlacementError))
                    if (tn.position != Vector3.zero)
                        return true;
                if (player.HasPermission(permbuild))
                {
                    if (tn.position != Vector3.zero) return true;
                }
            ...
            private void DoCrosshair(string cColor = default(string), bool kill = false)
            {
                ...
                element.Add(CustomIconUI(mainName, new Vector2(0.499f, 0.499f), r("nffrgf/vpbaf/gnetrg.cat"), -0.005f, 0.005f, cColor, Instance.playerPrefs.playerData[player.userID.Get()].SF));
            ...
            private void DoWarning(string cColor = default(string), bool kill = false)
            {
                ...
                "Under", r("HgJneavatHV"));
                element.Add(CustomIconUI(mainName, new Vector2(0.499f, 0.35f), r("nffrgf/vpbaf/jneavat_2.cat"), -0.05f, 0.05f, cColor, Instance.playerPrefs.playerData[player.userID.Get()].SF));
                CuiHelper.DestroyUi(player, mainName);
                CuiHelper.AddUi(player, element);
            }
        }
        ...
        private void OnServerInitialized()
        {
            ...
            permission.RegisterPermission(permremover, this);
            permission.RegisterPermission(permplanner, this);
            ...
            {
                Stsr(player);
                ctvUbrTls[player.userID.Get()] = false;
            }
        ...
        private void Stsr(BasePlayer player)
        {
            ...
            Plyrnf p = null;
            if (!playerPrefs.playerData.TryGetValue(player.userID.Get(), out p))
            {
                Plyrnf info = new Plyrnf();
                info.SF = scaleFactorDef;
                info.DBG = 4;
                info.DBS = 0UL;
                playerPrefs.playerData.Add(player.userID.Get(), info);
            }
        }
        ...
        //    try
        //    {
        //        limit = Interface.Oxide.DataFileSystem.ReadObject<Dictionary<ulong, Dictionary<ulong, int>>>(Title + "/Limit");
        //    }
        //    catch (Exception e)
        //    {
        ...
        private object CanBuild(Planner plan, Construction prefab, Construction.Target target)
        {
            if (plan != null)
            {
                ...
                bool exists = false;
                if (p && ctvUbrTls.TryGetValue(p.userID.Get(), out exists) && exists) return false;
            }
        ...
        private void OnItemDeployed(Deployer d)
        {
            if (d != null)
            {
                ...
                bool exists = false;
                if (p && ctvUbrTls.TryGetValue(p.userID.Get(), out exists) && exists)
                {
                    Item i = d.GetItem();
                    i.amount++;
                }
        ...
        {
            bool exists = false;
            if (!permission.UserHasPermission(p.UserIDString, permremover) && bP.name == "UberRemove")
            {
                return null;
            }
            if (p && ctvUbrTls.TryGetValue(p.userID.Get(), out exists) && exists && bP.skinID == Convert.ToUInt64(playerTools[1][2])) return false;
            return null;
        }
        ...
        private void OnLoseCondition(Item item, float amount)
        {
            ...
            if (item != null)
            {
                ...
                if (p && ctvUbrTls.TryGetValue(p.userID.Get(), out exists) && exists) item.condition = item.maxCondition;
            }
        }
        ...
        private void OnPlayerTick(BasePlayer p, PlayerTick msg, bool wasPlayerStalled)
        {
            ...
            if (p && ctvUbrTls.TryGetValue(p.userID.Get(), out exists) && exists)
            {
                ...
                {
                    ctvUbrTls[p.userID.Get()] = false;
                    activeUberObjects[p.userID.Get()].OnDestroy();
                    activeUberObjects.Remove(p.userID.Get());
                    UpdateHooks();
                    return;
                }
                ...
                activeUberObjects[p.userID.Get()].SetHeldItem(msg.activeItem);

                if (msg.activeItem != default(ItemId))
                {
                    Instance.activeUberObjects[p.userID.Get()].TickUpdate(msg);
                    if (msg.inputState != null) // && p.serverInput.current.buttons != p.serverInput.previous.buttons)
                        Instance.activeUberObjects[p.userID.Get()].DoTick();
                }
            }
        }
        ...
        private void cmdPrefab(ConsoleSystem.Arg arg)
        {
            ...
            activeUberObjects[player.userID.Get()].SetBlockPrefab(Instance.constructionIds[id]);
        }
        ...
        private void cmdScale(ConsoleSystem.Arg arg)
        {
            ...
            if (!arg.HasArgs(1))
            {
                SendReply(arg, r("Pheerag fpnyr: ") + playerPrefs.playerData[player.userID.Get()].SF);
                return;
            }
        ...
        private void chatScale(BasePlayer player, string command, string[] args)
        {
            ...
            {
                SendReply(player, string.Format(LangMsg(r("PheeragFpnyr"), player.UserIDString), playerPrefs.playerData[player.userID.Get()].SF));
                return;
            }
            ...
            playerPrefs.playerData[player.userID.Get()].SF = f;
        ...
        {
            bool exists = false;
            if (!ctvUbrTls.TryGetValue(p.userID.Get(), out exists))
            {
                Stsr(p);
                ctvUbrTls[p.userID.Get()] = false;
            }

            if ((bool)ctvUbrTls[p.userID.Get()])
            {
                ctvUbrTls[p.userID.Get()] = false;
                activeUberObjects[p.userID.Get()].OnDestroy();
                activeUberObjects.Remove(p.userID.Get());
            ...
            ctvUbrTls[p.userID.Get()] = true;
            activeUberObjects[p.userID.Get()] = p.gameObject.AddComponent<EPlanner>();
        ...
        {
            bool exists = false;
            if (player && ctvUbrTls.TryGetValue(player.userID.Get(), out exists) && exists)
            {
        ...
        //void OnEntityKill(DecayEntity entity)
        //{
        //    if (entity.OwnerID != 0)
        //    {
        //        if (Instance.limit.TryGetValue(entity.OwnerID, out Dictionary<ulong, int> ownerDict))
        //        {
        //            ownerDict.Remove(entity.net.ID.Value);
        //            int index = 1;
        //            foreach (var key in ownerDict.Keys.ToArray())
        //            {
        //                ownerDict[key] = index++;
        //            }

        //            Instance.limit[entity.OwnerID] = ownerDict;
        //            Instance.SaveData();
        //        }
        //    }
        //}
        ...
        private object OnServerCommand(ConsoleSystem.Arg arg)
        {
            ...
            {
                bool exists = false;
                if (arg.Player() && ctvUbrTls.TryGetValue(arg.Player().userID.Get(), out exists) && exists)
                {
                    NetworkableId targetID = arg.GetEntityID(1);
                    object checkID = activeUberObjects[arg.Player().userID.Get()].GtMvTrgt();
        ...
        {
            bool exists = false;
            if (player && ctvUbrTls.TryGetValue(player.userID.Get(), out exists) && exists) if (message == "Can't afford to place!" || message == "Building is blocked!") return true;
            return null;
        }
    }
}