    ...
    class XScan : RustPlugin
    {
		...
		private void cmdScan(BasePlayer player)
		{
			...
			if (permission.UserHasPermission(player.UserIDString, "xscan.unlimit")) 
				Scan(player);
			else if (StoredDataS.ContainsKey(player.userID.Get()))
			{
				StoredDataS[player.userID.Get()] -= 1; 
				Scan(player);
				
				SendReply(player, string.Format(lang.GetMessage("CHATSC", this, player.UserIDString), StoredDataS[player.userID.Get()]));
				
				if (StoredDataS[player.userID.Get()] == 0)
					StoredDataS.Remove(player.userID.Get());
		...
		private object OnHammerHit(BasePlayer player, HitInfo info)
		{
			...
			{
				if (StoredDataT.ContainsKey(entity.net.ID.Value))
					SendReply(player, string.Format(lang.GetMessage("CupboardInfo", this), StoredDataT[entity.net.ID.Value]));
			}
		...
		private void Scan(BasePlayer player)
		{
			...
			{
			    StoredData.Add($"{DateTime.Now}", $"{player.displayName} | {player.userID.Get()}");	
			    Interface.Oxide.DataFileSystem.WriteObject("XScan/ScanUSE", StoredData);
			}
		}
		...
		private void OnEntityBuilt(Planner plan, GameObject go)
        {
			...
			if (entity is BuildingPrivlidge)
			{
				StoredDataT.Add(entity.net.ID.Value, $"{entity.OwnerID} | {entity.transform.position} | {DateTime.Now}");
