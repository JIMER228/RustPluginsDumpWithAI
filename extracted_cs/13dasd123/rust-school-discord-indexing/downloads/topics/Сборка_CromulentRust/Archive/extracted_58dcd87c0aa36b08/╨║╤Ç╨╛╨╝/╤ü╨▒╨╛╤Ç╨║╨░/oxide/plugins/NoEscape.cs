...
namespace Oxide.Plugins
{
    ...
    public class NoEscape : RustPlugin
    {
        ...
        void Loaded()
        {
            ...
            if (!config.PlayerBlockSettings.CanUpgrade) Unsubscribe(nameof(CanChangeGrade));
            else Subscribe(nameof(CanChangeGrade));
        ...
        private void OnEntityDeath(BaseCombatEntity entity, HitInfo info)
        {
            ...
            if (entity == null || info == null || info.InitiatorPlayer == null || !(entity is StabilityEntity || entity is ShopFront || entity is BuildingPrivlidge)
                || config.BlockSettings.EnabledBuildingBlock && entity.GetBuildingPrivilege() == null || entity.OwnerID == 0 || (config.BlockSettings.EnabledOwner && info.InitiatorPlayer.userID.Get() == entity.OwnerID) || (config.BlockSettings.EnabledTeam && IsTeamate(info.InitiatorPlayer.userID.Get(), entity.OwnerID))) return;
        ...
        private bool? CanChangeGrade(BasePlayer player, BuildingBlock block, BuildingGrade.Enum grade, ulong iSkin)
        {
            if (!IsRaidBlocked(player)) return null;
            var component = PlayerBlockStatus.Get(player);
            if (component == null) return null;
