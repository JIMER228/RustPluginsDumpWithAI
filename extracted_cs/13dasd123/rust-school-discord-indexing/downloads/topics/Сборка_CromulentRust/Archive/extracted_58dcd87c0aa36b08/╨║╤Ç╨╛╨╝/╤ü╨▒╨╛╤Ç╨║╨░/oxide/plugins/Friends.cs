...
{
    [Info("Friends", "xkrystalll / Mevent", "2.0.0")]
    ...
    public class Friends : RustPlugin
    {
        ...
        private const string Layer = "ui.Friends.bg";
        ...
        private void SaveFilesData()
        {
            ...
            if (_data.Players == null)
                _data.Players = new Dictionary<ulong, PlayerData>();
        ...
        private void LoadFilesData()
        {
            ...
            _playersData = _data.Players ?? new Dictionary<ulong, PlayerData>();
        }
        ...
        private static PlayerData GetPlayerData(BasePlayer player)
        {
            ...
            if (!_instance._playersData.ContainsKey(player.userID.Get()))
                _instance._playersData.Add(player.userID.Get(), new PlayerData
                {
                    DisplayName = player.displayName,
                    UserId = player.userID
                });

            return _instance._playersData[player.userID.Get()];
        }
        ...
        private void OnPlayerConnected(BasePlayer player)
        {
            ...
            if (_config.AddTeam && data.Friends.Count > 0)
            {
                ...
                if (friend == null)
                {
                    ...
                    team.SetTeamLeader(player.userID.Get());
            ...
            if (_config.PlayerDatabase.Enabled) LoadPlayerData(player.userID.Get());
        }
        ...
        private void OnPlayerDisconnected(BasePlayer player)
        {
            ...
            SaveData(player.userID.Get(), GetPlayerData(player.userID.Get()));
        }
        ...
        private void OnEntityTakeDamage(BasePlayer player, HitInfo info)
        {
            ...
            if (!IsFriend(player.userID.Get(), initiator.userID.Get()) || player.userID.Get() == initiator.userID.Get())
                return;

            var data = GetPlayerData(initiator);

            var friend = data?.Friends.Find(x => x.UserId == player.userID.Get());
            if (friend == null || friend.FriendlyFire) return;

            info.damageTypes.ScaleAll(0);

            if (IsCd(initiator.userID.Get())) return;
        ...
        private void OnTeamLeave(RelationshipManager.PlayerTeam playerTeam, BasePlayer player)
        {
            ...
            playerTeam.members.ToList().ForEach(user =>
            {
                ...
                GetPlayerData(user)?.RemoveFriend(player.userID.Get());
            });

            UpdateAuthList(player.userID.Get(), AutoAuthType.All);
        ...
        private void OnTeamAcceptInvite(RelationshipManager.PlayerTeam playerTeam, BasePlayer player)
        {
            NextTick(() =>
            {
                ...
                if (invite != null && playerTeam.members.Exists(x => invite.Inviter.userID.Get() == x))
                {
                    AcceptInvite(player);
                    return;
                }
                ...
                if (playerTeam.members.Contains(player.userID.Get()))
                    if (_config.UseTeams)
                    {
                        playerTeam.members.ToList().ForEach(user =>
                        {
                            if (user == player.userID.Get()) return;

                            GetPlayerData(player)?.AddFriend(user);
                            GetPlayerData(user)?.AddFriend(player.userID.Get());
        ...
        private object OnSamSiteTarget(SamSite samSite, BaseVehicle vehicle)
        {
            ...
            {
                if (mounted.userID.Get() == samSite.OwnerID)
                    return true;

                var friendData = data.GetFriend(mounted.userID.Get());
                            ...
                            case "add":
                                {
                                    ...
                                    if (player.userID.Get() == friend.userID.Get())
                                    {
                                        Notify(player, _config.Notify.Delay, Msg(NotifyTitle, player.UserIDString),
                                    ...
                                    if (pData.IsFriend(friend.userID.Get()))
                                    {
                                        Notify(player, _config.Notify.Delay, Msg(NotifyTitle, player.UserIDString),
                            ...
                            case "cupboard":
                                {
                                    ...
                                    UpdateAuthList(player.userID.Get(), AutoAuthType.Cupboard);
                                    break;
                                }
                            case "containers":
                                {
                            ...
                            case "turrets":
                                {
                                    ...
                                    UpdateAuthList(player.userID.Get(), AutoAuthType.Turret);
                                    break;
                                }
        ...
        private object CanUseLockedEntity(BasePlayer player, BaseLock baseLock)
        {
            ...
            var friend = GetPlayerData(baseLock.OwnerID)?.GetFriend(player.userID.Get());
        ...
        private List<PlayerNameID> GetPlayerNameIDs(ulong playerId, AutoAuthType autoAuthType)
        {
            ...
            {
                userid = auth,
                username = covalence.Players.FindPlayer(auth.ToString())?.Name ?? string.Empty,
                ShouldPool = true
            }));
            ...
            toRemove.ForEach(invite =>
            {
                if (_config.AddTeam)
                    if (invite.Inviter != null && invite.Target != null)
                    {
                        ...
                        if (team != null)
                            if (!team.invites.Contains(invite.Target.userID.Get()))
        ...
        private void RemoveFriend(BasePlayer player, ulong friend)
        {
            ...
            fData.RemoveFriend(player.userID.Get());

            if (_config.AddTeam) player.Team?.RemovePlayer(friend);

            if (_config.UseTeams)
                GetTeamList(player.userID.Get())?.ForEach(member =>
                {
        ...
        {
            var team = RelationshipManager.ServerInstance.FindPlayersTeam(player.userID.Get());
            if (team == null)
            {
                team = RelationshipManager.ServerInstance.CreateTeam();
                team.AddPlayer(player);
        ...
        private void SendInvite(BasePlayer inviter, BasePlayer target)
        {
            ...
            if (_config.AddTeam)
            {
                ...
                if (team != null)
                    if (!team.invites.Contains(target.userID.Get()))
        ...
        private void AcceptInvite(BasePlayer target)
        {
            ...
            if (target.userID.Get() == inviter.userID.Get())
            {
                Notify(target, _config.Notify.Delay, Msg(NotifyTitle, target.UserIDString),
            ...
            if (_config.UseTeams)
            {
                GetTeamList(inviter.userID.Get())?.ForEach(player =>
                {
                    GetPlayerData(player)?.AddFriend(target.userID.Get());
                    tData.AddFriend(player);

                    Log("friends", $"Player '{player}' added '{target.userID.Get()}' as a friend");
                });
            }
            else
            {
                GetPlayerData(inviter)?.AddFriend(target.userID.Get());
                tData.AddFriend(inviter.userID.Get());

                Log("friends", $"Player '{inviter.userID.Get()}' added '{target.userID.Get()}' as a friend");
            }
            ...
            NextTick(() =>
            {
                if (_config.AddTeam && inviter.Team != null) UpdateTeamAuthList(GetTeamList(inviter.userID.Get()));
            });
        }
