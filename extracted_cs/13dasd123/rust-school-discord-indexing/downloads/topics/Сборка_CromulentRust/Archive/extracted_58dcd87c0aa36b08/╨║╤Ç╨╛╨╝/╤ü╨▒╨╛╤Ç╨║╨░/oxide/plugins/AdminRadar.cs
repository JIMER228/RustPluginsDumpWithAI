...
namespace Oxide.Plugins
{
    ...
    internal class AdminRadar : RustPlugin
    {
        ...
        private class Radar : FacepunchBehaviour
        {
            ...
            internal int inactiveSeconds, activatedSeconds, checks, intBypassOverride;
            ...
            public void Init(AdminRadar instance)
            {
                ...
                int lvl = this.instance.GetLvl(((ulong)userid.ToLong()));
                canBypassOverride = HasPermission(userid, $"adminradar.lvl{lvl}");
                intBypassOverride = lvl;
            ...
            {
                if (config.Voice.Enabled && dist <= config.Voice.Distance && instance._voices.ContainsKey(target.userID.Get()))
                {
            ...
            {
                currType = EntityType.Active;
                if (target.IsKilled() || !GetBool(EntityType.Active) || exclude.Contains(target.userID.Get()))
                {
                    return;
                }
                foreach (var kvp in this.instance.config.Settings.Lvls)
                {
                    int lvl = this.instance.GetLvl(target.userID.Get());
                    int lvl2 = intBypassOverride;
                    if (lvl2 == 0)
                    {
                        if (lvl != 0)
                        {
            ...
            private void DrawAppendedText(BasePlayer target, Vector3 a, Vector3 offset, Color color)
            {
                ...
                if (config.Additional.BackpackPlugin && API_GetExistingBackpacks(target.userID.Get()))
                {
                    sb.Append("*");
                }
                ...
                if (instance._clanColors.TryGetValue(instance.GetClanOf(target.userID.Get()), out var clan) && !config.Settings.ApplySameColor)
                {
        ...
        private void ccmdMovePosition(BasePlayer player, string command, string[] args)
        {
            ...
            UiOffsets offsets;
            if (!data.Offsets.TryGetValue(player.userID.Get(), out offsets))
            {
                return;
            }
            ...
            data.Offsets[player.userID.Get()] = offsets;
        ...
        private void RadarCommandX(BasePlayer player, string command, string[] args)
        {
            ...
            else
            {
                switch (args[0])
                {
                    ...
                    case "reset":
                        {
                            data.Offsets.Remove(player.userID.Get());
                            args = Array.FindAll(args, x => x != args[0]);
                        }
                        break;
        ...
        private void Init()
        {
            ...
            foreach (var perm in config.Settings.Lvls.Keys)
            {
        ...
        {
            if (!_voices.TryGetValue(player.userID.Get(), out var voice))
            {
        ...
        private bool HasAccess(BasePlayer player)
        {
            ...
            if (DeveloperList.Contains(player.userID.Get()))
                return True;
        ...
        private UiOffsets GetOffsets(BasePlayer player)
        {
            ...
            if (!data.Offsets.TryGetValue(player.userID.Get(), out offsets))
            {
                ...
                data.Offsets[player.userID.Get()] = offsets = new UiOffsets(DefaultOffset.Min, DefaultOffset.Max);
            }
            return offsets;
        }
        ...
        public void ShowMoveUi(BasePlayer player, bool destroyUi)
        {
            ...
            if (destroyUi && isMovingUi.Remove(player.userID.Get()))
            {
                CuiHelper.DestroyUi(player, name);
                SaveOffsetData();
                return;
            }
            else if (!isMovingUi.Contains(player.userID.Get()))
            {
                isMovingUi.Add(player.userID.Get());
            }
        ...
        {
            int lvl = 0;
            foreach (var kvp in config.Settings.Lvls)
            {
                string perm = kvp.Key;
                int i = kvp.Value;
