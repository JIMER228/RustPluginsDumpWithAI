...
namespace Oxide.Plugins
{
    ...
    class PogoStick : RustPlugin
    {
        ...
        void OnActiveItemChanged(BasePlayer player, Item oldItem, Item newItem)
        {
            ...
            if (pogoConfig != null)
            {
                if (!_config.mainConfig.allowInAir && player.isInAir) 
                    return;
        ...
        void OnEntityEnterZone(string ZoneID, MovableDroppedItem entity)
        {
            ...
            if (pogoStickEntity != null && !SupportedPluginsManager.CheckZoneOfZoneManager(ZoneID))
        ...
        {
            if (arg == null || arg.Player() != null || arg.Args.Length < 2) 
                return;
        ...
        void Subscribes()
        {
            foreach (string hook in allHooks)
            {
                if (hook == "CanWearItem" &&_config.mainConfig.itemType == 0) 
                    continue;
                else if (hook == "OnActiveItemChanged" && _config.mainConfig.itemType == 1) 
                    continue;
                else if(hook == "OnLootSpawn" && !_config.pogoConfigs.Any(x => x.itemConfig.inLootSpawn)) 
                    continue;
                else if(hook == "OnEntityEnterZone" && !_config.supportedPluginsConfig.zoneManager.enable) 
                    continue;
        ...
        void RegisterPermissions()
        {
            foreach (PogoConfig pogoConfig in _config.pogoConfigs)
                if (pogoConfig.permission != "")
                    permission.RegisterPermission(pogoConfig.permission, this);
        }
        #endregion Methods
        ...
        sealed class PogoStickEntity : FacepunchBehaviour
        {
            ...
            {
                if (GetPogoByPlayerUserId(player.userID.Get()) != null) 
                    return;

                if (pogoConfig.permission != "" && !CheckPlayerPermission(player, pogoConfig.permission)) 
                    return;
            ...
            {
                foreach (PogoStickEntity pogoStickEntity in pogoSticks) 
                    if (pogoStickEntity != null) 
                        pogoStickEntity.DestroyPogoImmediately();
            }

            internal static PogoStickEntity GetPogoByPlayerUserId(ulong userId)
            {
                pogoSticks.RemoveWhere(x => x == null);
                return pogoSticks.FirstOrDefault(x => x != null && x.player != null && x.player.userID.Get() == userId);
            }
            ...
            internal bool CanPlayerDismount()
            {
                ...
                if (Vector3.Angle(Vector3.up, movableDroppedItem.transform.up) > 45) 
                    return false;

                TimeSince timeNow = 0f;

                if (timeSinceCSpawn - timeNow < 1f) 
                    return false;
            ...
            void Init(BasePlayer player, MovableDroppedItem movableDroppedItem, PogoConfig pogoConfig, Item item)
            {
                ...
                if (ins._config.mainConfig.thirdPersonViewMode) 
                    SwitchPlayerViewMode(true);
            }

            void BuildPogo()
            {
                movableBaseMountable = MovableBaseMountable.CreateMovableBaseMountable(movableDroppedItem, "assets/prefabs/vehicle/seats/testseat.prefab", new Vector3(0, 0f, 0.035f), new Vector3(0f, 0, 0));
                CreateCollider();
                SpawnDecorEntities();
            }
            ...
            void OnCollisionEnter(Collision collision)
            {
                ...
                if (timeSinceLastJump - timeNow < 0.5f) 
                    return;
            ...
            void FixedUpdate()
            {
                ...
                if ((wantDismount || forceDismount) && CanPlayerDismount())
                    DestroyPogoDelay();
            ...
            bool CheckPlayer()
            {
                if (movableBaseMountable._mounted == null || player == null || player.IsSleeping() || !player.isMounted || !player.IsConnected) 
                    return false;
            ...
            bool CheckWater()
            {
                return ins._config.mainConfig.allowInWater || player.transform.position.y > 0;
            }
            ...
            class EntityLocation
            {
                ...
                internal static HashSet<EntityLocation> entityLocations = new HashSet<EntityLocation>
                {
                    ...
                    new EntityLocation("assets/prefabs/tools/lumberjack_tools/lumberjack_pick.entity.prefab", new Vector3(-0.040f, 0.602f, -0.091f), new Vector3(5.387f, 160.000f, 78.379f)),
                    new EntityLocation("assets/prefabs/tools/lumberjack_tools/lumberjack_pick.entity.prefab", new Vector3(0.048f, 0.602f, -0.094f), new Vector3(5.387f, 190.000f, 78.379f)),
                    new EntityLocation("assets/prefabs/tools/lumberjack_tools/lumberjack_pick.entity.prefab", new Vector3(0.012f, 0.265f, -0.172f), new Vector3(79.092f, 40.266f, 300.947f)),
