...
{
	[Info("GameStoresRUST", "HOUGAN & Sstine & rostov114 & Mevent # GAMESTORES", "1.0.21")]
	public class GameStoresRUST : RustPlugin
	{
		...
		private void OnPlayerDisconnected(BasePlayer player)
		{
			...
			DequeueBasketRequest(player.userID.Get());
		}
		...
		private void ShowBasketItemsUI(BasePlayer player,
			int page)
		{
			if (!_playerBaskets.TryGetValue(player.userID.Get(), out var playerBasket))
				return;
		...
		private void HandleBasketResponse(BasePlayer player, string response, int page)
		{
			...
			else
			{
				...
				_playerBaskets.Remove(player.userID.Get());

				_playerBaskets[player.userID.Get()] = new PlayerBasket(data);
		...
		{
			if (ListBannedCommandUserID.Contains(player.userID.Get()))
			{
			...
			if (IsCommandBanned(player)) ListBannedCommandUserID.Add(player.userID.Get());
		...
		{
			if (!ListTimeOutCommand[index].ContainsKey(player.userID.Get()))
				ListTimeOutCommand[index][player.userID.Get()] = 1;
			else
				ListTimeOutCommand[index][player.userID.Get()]++;
		}

		private bool IsCommandBanned(BasePlayer player)
		{
			return ListTimeOutCommand[0][player.userID.Get()] >= 10 ||
			       ListTimeOutCommand[1][player.userID.Get()] >= 20 ||
			       ListTimeOutCommand[2][player.userID.Get()] >= 30;
		}
		...
		private void HandleTakeResponse(BasePlayer player, int index, int basketID, int responseCode, string response)
		{
			switch (responseCode)
			{
				...
				case 200:
					...
					if (_playerBaskets.ContainsKey(player.userID.Get()) &&
					    _playerBaskets[player.userID.Get()].BasketItems.ContainsKey(basketID))
						_playerBaskets[player.userID.Get()].BasketItems.Remove(basketID);
					break;
				case 404:
		...
		private void LogAction(BasePlayer player, string text, bool printToConsole = false, bool printError = false)
		{
			...
			LogToFile("!global",
				$"{DateTime.Now.ToShortTimeString()} {(player != null ? "[" + player.userID.Get() + "]" : " ")}{text}", this);
