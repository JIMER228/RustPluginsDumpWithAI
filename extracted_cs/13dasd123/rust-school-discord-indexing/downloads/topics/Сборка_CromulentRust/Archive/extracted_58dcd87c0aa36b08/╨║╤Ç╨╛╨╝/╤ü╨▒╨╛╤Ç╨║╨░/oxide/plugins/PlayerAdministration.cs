...
#pragma warning restore CS0162
        ...
        private void BuildUserPage(ref Cui aUIObj, UiPage aPageType, ulong aPlayerId)
        {
            ...
            if (aPageType == UiPage.PlayerPage)
            {
                ...
                // Ban, Kick
                if (aPlayerId != aUIObj.PlayerId && VerifyPermission(aUIObj.PlayerId, CPermBan))
                { // No need to check for null, as we ban the ID and don't attempt retrieval
                    aUIObj.AddButton(
                ...
                if ((!isPlayerNull) && aPlayerId != aUIObj.PlayerId && VerifyPermission(aUIObj.PlayerId, CPermTeleport2))
                {
                    aUIObj.AddButton(
        ...
        private const string CPermTeleport2 = "playeradministration.access.teleport2";
...
#pragma warning disable IDE0051, IDE0060 // Remove unused private members, unused parameter
        ...
        void Loaded()
        {
            ...
            permission.RegisterPermission(CPermTeleport2, this);
        ...
        void Unload()
        {
            foreach (BasePlayer player in Player.Players)
            {
                ...
                if (FMainPageBanIdInputText.ContainsKey(player.userID.Get()))
                    FMainPageBanIdInputText.Remove(player.userID.Get());

                if (FUserBtnPageSearchInputText.ContainsKey(player.userID.Get()))
                    FUserBtnPageSearchInputText.Remove(player.userID.Get());

                if (FUserPageReasonInputText.ContainsKey(player.userID.Get()))
                    FUserPageReasonInputText.Remove(player.userID.Get());
            }
        ...
        void OnServerInitialized()
        {
            foreach (BasePlayer user in Player.Players)
            {
                ...
                ServerUsers.User servUser = ServerUsers.Get(user.userID.Get());

                if (servUser == null || servUser?.group != ServerUsers.UserGroup.Banned)
                    FOnlineUserList[user.userID.Get()] = user.displayName;
            }
            ...
            foreach (BasePlayer user in Player.Sleepers)
            {
                ...
                if (servUser == null || servUser?.group != ServerUsers.UserGroup.Banned)
                    FOfflineUserList[user.userID.Get()] = user.displayName;
            }
        }
        ...
        void OnUserConnected(IPlayer aPlayer)
        {
            ...
            if (FOfflineUserList.ContainsKey(user.userID.Get()))
                FOfflineUserList.Remove(user.userID.Get());
        ...
        void OnUserDisconnected(IPlayer aPlayer)
        {
            ...
            if (FMainPageBanIdInputText.ContainsKey(user.userID.Get()))
                FMainPageBanIdInputText.Remove(user.userID.Get());

            if (FUserBtnPageSearchInputText.ContainsKey(user.userID.Get()))
                FUserBtnPageSearchInputText.Remove(user.userID.Get());

            if (FUserPageReasonInputText.ContainsKey(user.userID.Get()))
                FUserPageReasonInputText.Remove(user.userID.Get());

            if (FOnlineUserList.ContainsKey(user.userID.Get()))
                FOnlineUserList.Remove(user.userID.Get());
        ...
        private void PlayerAdministrationBanUserCallback(IPlayer aPlayer, string aCommand, string[] aArgs)
        {
            ...
            banReasonMsg = GetReason(player.userID.Get(), targetId.ToString());
            ...
            SendDiscordKickBanMessage(
                player.displayName, player.userID.Get(), ServerUsers.Get(targetId)?.username ?? targetId.ToString(), targetId, banReasonMsg, true
            );
        ...
        private void PlayerAdministrationMainPageBanByIdCallback(IPlayer aPlayer, string aCommand, string[] aArgs)
        {
            ...
            if (
                !VerifyPermission(ref player, CPermBan, true) || !FMainPageBanIdInputText.ContainsKey(player.userID.Get()) ||
                !ulong.TryParse(FMainPageBanIdInputText[player.userID.Get()], out targetId)
            )
                return;
        ...
        private void PlayerAdministrationKickUserCallback(IPlayer aPlayer, string aCommand, string[] aArgs)
        {
            ...
            kickReasonMsg = GetReason(player.userID.Get(), targetId.ToString(), true);
            targetPlayer?.Kick(kickReasonMsg);
            LogInfo($"{player.displayName}: Kicked user ID {targetId}");
            BroadcastKickBan(aPlayer.Id, targetId, kickReasonMsg, true);
            SendDiscordKickBanMessage(player.displayName, player.userID.Get(), targetPlayer.displayName, targetId, kickReasonMsg, false);
            timer.Once(0.01f, () => BuildUI(player, UiPage.PlayerPage, targetId.ToString()));
        }
        ...
        private void PlayerAdministrationMuteUserCallback(IPlayer aPlayer, string aCommand, string[] aArgs)
        {
            ...
            else
            {
                ...
                if (BetterChatMute != null && target != null)
                {
                    string inputReason = GetReason(player.userID.Get(), targetId.ToString());
        ...
        private void SetCooldown(BasePlayer player)
        {
            ...
            cooldown[userid] = Time.time + 60;
            timer.Once(60, () => cooldown.Remove(userid));
        }
        ...
        private void PlayerAdministrationTeleportUserCallback(IPlayer aPlayer, string aCommand, string[] aArgs)
        {
            ...
            if (!VerifyPermission(ref player, CPermTeleport2, true) || !GetTargetFromArg(aArgs, out targetId))
                return;
        ...
        private void PlayerAdministrationMainPageBanIdInputTextCallback(IPlayer aPlayer, string aCommand, string[] aArgs)
        {
            ...
            if (FMainPageBanIdInputText.ContainsKey(player.userID.Get()))
            {
                FMainPageBanIdInputText[player.userID.Get()] = aArgs[0];
            }
            else
            {
                FMainPageBanIdInputText.Add(player.userID.Get(), aArgs[0]);
            }
        }
        ...
        private void PlayerAdministrationUserBtnPageSearchInputTextCallback(IPlayer aPlayer, string aCommand, string[] aArgs)
        {
            ...
            if (FUserBtnPageSearchInputText.ContainsKey(player.userID.Get()))
            {
                FUserBtnPageSearchInputText[player.userID.Get()] = aArgs[0];
            }
            else
            {
                FUserBtnPageSearchInputText.Add(player.userID.Get(), aArgs[0]);
            }
        }
        ...
        private void PlayerAdministrationUserPageReasonInputTextCallback(IPlayer aPlayer, string aCommand, string[] aArgs)
        {
            ...
            if (FUserPageReasonInputText.ContainsKey(player.userID.Get()))
            {
                FUserPageReasonInputText[player.userID.Get()] = aArgs[0];
            }
            else
            {
                FUserPageReasonInputText.Add(player.userID.Get(), aArgs[0]);
            }
        }
