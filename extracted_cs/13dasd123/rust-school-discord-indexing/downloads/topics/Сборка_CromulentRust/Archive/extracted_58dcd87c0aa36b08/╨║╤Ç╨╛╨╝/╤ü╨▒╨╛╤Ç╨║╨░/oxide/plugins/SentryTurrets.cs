...
namespace Oxide.Plugins
{
    ...
    public class SentryTurrets : RustPlugin
    {
        ...
        private static Vector3 switchPosition = new Vector3(0, -0.8f, 1);
        Dictionary<ulong, int> Alreadyturrets = new Dictionary<ulong, int>();
        ...
        {
            foreach (var perm in config.perm)
            {
        ...
        {
            if (entity is NPCAutoTurret turret)
            {
                if (entity.OwnerID != 0)
                {
                    if (!info.hasDamage)
        ...
        private void OnSwitchToggle(ElectricSwitch entity, BasePlayer player)
        {
            ...
            if (turret.authorizedPlayers.Any(x => x.userid == player.userID.Get()) == false)
            {
        ...
        void OnEntityKill(NPCAutoTurret entity)
        {
            Alreadyturrets[entity.OwnerID]--;
        }
        ...
        private void SetupTurret(NPCAutoTurret turret)
        {
            ...
            foreach (var perm in config.perm)
            {
                if (permission.UserHasPermission(turret.OwnerID.ToString(), perm.Key)) maxturret = Math.Max(maxturret, perm.Value);
            }
            if ((Alreadyturrets.ContainsKey(turret.OwnerID) && Alreadyturrets[turret.OwnerID] >= maxturret) || maxturret == 0)
            {
                Alreadyturrets[turret.OwnerID]++;
                ...
                turret.AdminKill();
                player.ChatMessage($"Вы превысили допустимый лимит в {maxturret} турелей NPC, чтобы увеличить лимит купите одну из привилегий на сайте <color=#8e7cc3>https://rust-crom.ru/ <color=#ffffff>список ограничений на привилегию \n[<color=#ffd966>Vip-10 <color=#3d85c6>Premium-15 <color=#a64d79>Deluxe-20 <color=#6fa8dc>Elite-25 <color=#674ea7>Cezar-50 <color=#9fc5e8>Zeus-100<color=#ffffff>]");
                GiveItem(player);
                return;
            }
            if (Alreadyturrets.ContainsKey(turret.OwnerID)) Alreadyturrets[turret.OwnerID]++;
            else Alreadyturrets.Add(turret.OwnerID, 1);
        ...
        {
            var ent = turret.GetComponentInChildren<ElectricSwitch>();
            var entity = new ElectricSwitch();
            if (ent == null)
            {
                Vector3 PositionSwitch = turret is NPCAutoTurret
                ? new Vector3(0f, 0.35f, 1.12f)
                : new Vector3(0f, 0.35f, 1.75f);
                if (PositionSwitch == null) return;
                entity = GameManager.server.CreateEntity(prefabSwitch,
                turret.transform.TransformPoint(PositionSwitch),
                Quaternion.Euler(turret.transform.rotation.eulerAngles.x,
                    turret.transform.rotation.eulerAngles.y, 0f), true) as ElectricSwitch;
                if (entity == null)
                {
                    return;
                }
        ...
        private void AuthorizeOthers(ulong userID, NPCAutoTurret entity)
        {
            ...
            entity.authorizedPlayers = entity.authorizedPlayers.Distinct().ToList().ToHashSet();
        }
        ...
        private class ConfigData
        {
            ...
            [JsonProperty("Пермишены : Количество")]
            public Dictionary<string, int> perm = new Dictionary<string, int>()
            {
                {"sentryturrets.limit.standard", 5},
                {"sentryturrets.limit.vip", 10},
                {"sentryturrets.limit.premium", 15},
                {"sentryturrets.limit.deluxe", 20},
                {"sentryturrets.limit.elite", 25},
                {"sentryturrets.limit.cezar", 50},
                {"sentryturrets.limit.zeus", 100},
                {"sentryturrets.limit.vipp", 250}
            };
        ...
        {
            return IsFriends(player1.userID.Get(), player2.userID.Get());
        }

        private bool IsFriends(BasePlayer player1, ulong player2)
        {
            return IsFriends(player1.userID.Get(), player2);
        }

        private bool IsFriends(ulong player1, BasePlayer player2)
        {
            return IsFriends(player1, player2.userID.Get());
        }
