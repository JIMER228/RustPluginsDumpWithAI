...
{
    [Info("BulletProjectile", "Karuza", "1.0.1")]
    public class BulletProjectile : RustPlugin
    {
        ...
        public static class GameTrace
        {
            public static void TraceAll(HitTest test, List<TraceInfo> traces, int layerMask = -5)
            {
                ...
                BaseEntity ignoreEntity = test.ignoreEntity;
                if ((layerMask & 16384) != 0)
                {
                    layerMask &= -16385;
                    GamePhysics.TraceAllUnordered(new Ray(origin - direction * 5f, direction), radius, list, maxDistance + 5f, 16384, QueryTriggerInteraction.UseGlobal, ignoreEntity);
                ...
                GamePhysics.TraceAllUnordered(new Ray(origin, direction), radius, list, maxDistance, layerMask, QueryTriggerInteraction.UseGlobal, ignoreEntity);
