    ...
    public class DeathMessages : RustPlugin
    {
        ...
        private void DMCmd(BasePlayer player, string cmd, string[] args)
        {
            if (args.Length > 0)
            {
                switch (args[0])
                {
                    case "on":
                        {
                            uiHide[player.userID.Get()] = new TemporaryBool()
                            {
                                Value = true,
        ...
        public class PluginConfig
        {
            ...
            public Dictionary<string, ColorNickName> ColorsNamePlayer = new Dictionary<string, PluginConfig.ColorNickName>
            {
                ...
                { "deathmessages.godlike", new PluginConfig.ColorNickName { ColorDeath = "#ff0000", ColorKill = "#ff3948" } }
            };
        ...
        public static void UpdateUI(bool onInsert)
        {
            ...
            foreach (var player in BasePlayer.activePlayerList)
            {
                ...
                if (Instance.uiHide.TryGetValue(player.userID.Get(), out showUI) && showUI.Value == false)
                    continue;
