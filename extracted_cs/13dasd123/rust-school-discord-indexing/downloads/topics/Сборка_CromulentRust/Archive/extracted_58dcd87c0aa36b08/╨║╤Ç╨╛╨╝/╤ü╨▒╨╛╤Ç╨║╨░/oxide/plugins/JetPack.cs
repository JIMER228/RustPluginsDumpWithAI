    ...
    class JetPack : RustPlugin
    {
        ...
        void OnItemRemovedFromContainer(ItemContainer container, Item item)
        {
            ...
            if (LootManager.IsJetpackItem(item))
            {
                ...
                if (player.IsRealPlayer() && container == player.inventory.containerWear)
                    JetpackComponent.RemoveJetpackFromPlayer(player.userID.Get());
            }
        }
        ...
        object CanMoveItem(Item item, PlayerInventory playerLoot, ItemContainerId targetContainer, int targetSlot, int amount)
        {
            ...
            if (!_config.isAirTakeOffAllowed)
            {
                ...
                {
                    JetpackComponent jetPack = JetpackComponent.GetJetpackComponentByUserId(player.userID.Get());
        ...
        object OnPlayerCommand(BasePlayer player, string command, string[] args)
        {
            ...
            {
                JetpackComponent jetPackComponent = JetpackComponent.GetJetpackComponentByUserId(player.userID.Get());
        ...
        class JetpackComponent : FacepunchBehaviour
        {
            ...
            internal static bool IsPlayerCanUseJetpack(BasePlayer player)
            {
                return LootManager.IsPlayerHaveJetItem(player) || ins.permission;
            }

            internal static JetpackComponent GetJetpackComponentByUserId(ulong userId)
            {
                return jetpacks.FirstOrDefault(x => x != null && x.player.userID.Get() == userId);
            }
            ...
            static bool IsPlayerCanWearJetpack(BasePlayer player)
            {
                ...
                JetpackComponent jetPackComponent = GetJetpackComponentByUserId(player.userID.Get());
            ...
            void SpawnChair()
            {
                ...
                chair = BuildManager.CreateChildEntity(movableDroppedItem, chairPrefabName, localPosition, localRotation) as BaseMountable;
            ...
            void CheckWater()
            {
                if (ins._config.isTakeOffJetInWater && transform.position.y <= 0)
                    RemoveJetpack();
            }
            ...
            void ControlFuelConsumtion()
            {
                ...
                {
                    Item item = player.inventory.AllItems().FirstOrDefault(x => x != null && LootManager.IsFuelItem(x.info.shortname));
        ...
        static class LootManager
        {
            ...
            {
                return ins._config.allowOpenFromAnyContainer ? player.inventory.AllItems().Any(x => IsJetpackItem(x)) : player.inventory.containerWear.itemList.Any(x => IsJetpackItem(x));
            }
        ...
        static class BuildManager
        {
            ...
            static void SetParent(BaseEntity parrentEntity, BaseEntity childEntity, Vector3 localPosition, Vector3 localRotation)
            {
