...
namespace Oxide.Plugins
{
    ...
    class TimedPermissions : CovalencePlugin 
    {
        ...
        private void Loaded()
        {
            ...
            LoadData(ref command, "TimedPermissionsEmptyPlayers");
        ...
        void OnPlayerConnected(BasePlayer player)
        {
            ...
            {
                string st = command[i];
                if (st.Contains(player.UserIDString))
                {
                    server.Command(st);
                    command.RemoveAt(i);
                }
            }
            SaveData(command, "TimedPermissionsEmptyPlayers");
        }
        ...
        private void CmdGrantPerm(IPlayer player, string cmd, string[] args)
        {
            ...
            IPlayer result = players.All.ToList().Find(p => p.Id == args[0]);
            if (result == null)
            {
                command.Add($"grantperm {args[0]} {args[1]} {args[2]}");
                Puts($"Записано - grantperm {args[0]} {args[1]} {args[2]}");
                SaveData(command, "TimedPermissionsEmptyPlayers");
            }
        ...
        private void CmdAddGroup(IPlayer player, string cmd, string[] args)
        {
            ...
            if (result == null)
            {
                command.Add($"addgroup {args[0]} {args[1]} {args[2]}");
                Puts($"Записано - addgroup {args[0]} {args[1]} {args[2]}");
                SaveData(command, "TimedPermissionsEmptyPlayers");
            }
