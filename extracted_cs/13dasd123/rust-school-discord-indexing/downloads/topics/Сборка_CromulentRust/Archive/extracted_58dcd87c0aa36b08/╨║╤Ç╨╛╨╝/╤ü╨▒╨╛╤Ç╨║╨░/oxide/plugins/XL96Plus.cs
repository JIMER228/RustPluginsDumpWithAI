...
namespace Oxide.Plugins
{
    ...
    {
        [PluginReference] Plugin HealthTick;

        private const string PermUse = "xl96plus.use";
        private const string PermIgnore = "xl96plus.ignore";
        ...
        object OnMagazineReload(BaseProjectile weapon, int desiredAmount, BasePlayer player)
        {
            if (weapon.name.Contains("l96"))
            {
                if (weapon.skinID == config.L96.SkinIDL96)
                {
                    if (weapon.primaryMagazine.ammoType.shortname == "ammo.rifle.incendiary")
                    {
                        player.ChatMessage("Зажигательные патроны запрещены.");
                        return true;
                    }
                }
            }
        ...
        void OnEntityTakeDamage(BaseCombatEntity entity, HitInfo info)
        {
            ...
            if (weapon.skin == config.L96.SkinIDL96 && permission.UserHasPermission(player.UserIDString, PermUse))
            {
                if (permission.UserHasPermission(eplayer.UserIDString, PermIgnore)) return;
                ...
                if (info.isHeadshot)
                {
                    ...
                    {
                        info.damageTypes.Set(info.damageTypes.GetMajorityDamageType(), 99999999999999999f);
                    }
                }
                else
                {
                    if (config.Settings.HealL96 == true)
                    {
                        ...
                        if (config.Settings.regenen == true)
                        {
                            HealthTick.Call("xl96plHook", "off", player);
                            timer.Once(config.Settings.regenensec, () => { HealthTick.Call("xl96plHook", "on", player); });
                        }
                    }
                }
            }
        ...
        private class L96Config
        {
            internal class SettingsSetting
            {
                ...
                [JsonProperty("Выключать реген?(healthtick)")] public bool regenen = false;
                [JsonProperty("На сколько выключать(секунды)")] public int regenensec = 5;
            }
        ...
        object GetL96ConfigValue(string key)
        {
            if (key == "NameL96")
                return config.L96.NameL96;
            return null;
        }
