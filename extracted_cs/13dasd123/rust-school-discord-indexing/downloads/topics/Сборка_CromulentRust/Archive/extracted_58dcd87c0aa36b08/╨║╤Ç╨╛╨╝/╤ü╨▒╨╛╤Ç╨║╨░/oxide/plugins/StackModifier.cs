...
{
    [Info("Stack Modifier", "Khan", "2.3.8")]
    ...
    public class StackModifier : RustPlugin
    {
        ...
        */
            ...
            foreach (ItemDefinition item in ItemManager.itemList)
            {
                ...
                if (!stackCategory.ContainsKey(item.shortname))
                {
                    stackCategory.Add(item.shortname, new _Items
                    {
                        ...
                        /*ContainerSizes = new Dictionary<ContainerTypes, int>
                        {
                            {ContainerTypes.LargeWoodenBox, item.stackable},
                            {ContainerTypes.WoodenBox, item.stackable},
                            {ContainerTypes.Coffin, item.stackable},
                        },*/
                    });

                    /*if (item.category == ItemCategory.Resources)
                        stackCategory[item.shortname].ContainerSizes.Add(ContainerTypes.ToolCupboard, item.stackable);*/
                }

                _Items update = _config.StackCategories[categoryName][item.shortname];

                /*if (update.ContainerSizes.IsNullOrEmpty())
                {
                    update.ContainerSizes = new Dictionary<ContainerTypes, int>
                    {
                        {ContainerTypes.LargeWoodenBox, item.stackable},
                        {ContainerTypes.WoodenBox, item.stackable},
                        {ContainerTypes.Coffin, item.stackable},
                    ...
                    if (item.category == ItemCategory.Resources)
                        update.ContainerSizes.Add(ContainerTypes.ToolCupboard, item.stackable);
                }*/
                ...
                {
                    _defaults.Add(item.itemid, item.stackable);
                    Puts($"Missing {item.shortname}, {item.itemid}, {item.stackable}");
                    continue;
                }
                ...
                if (item.stackable == 0)
                {
                    if (update.Modified <= 0)
                        _config.StackCategories[categoryName][item.shortname].Modified = stock;
        ...
        public class _Items
        {
            ...
            public int Modified;
            public bool Disable;
            //public Dictionary<ContainerTypes, int> ContainerSizes;
        }
        ...
        {
            BasePlayer p = targetItem?.GetOwnerPlayer() ?? item.GetOwnerPlayer();
            if (p.IsUnityNull())
                return null;

            ItemDefinition id = item.info;
            ItemDefinition td = targetItem.info;
            if (id.itemid == td.itemid && !CanWaterItemsStack(item, targetItem))
                return false;
            ...
            if (!(targetItem != item &&
                  ...
                  td.stackable > 1 &&
                  td.itemid == id.itemid &&
                  (!item.hasCondition || (double)item.condition == (double)td.condition.max) &&
                  (!targetItem.hasCondition || (double)targetItem.condition == (double)td.condition.max) &&
                  item.IsValid() &&
                  ...
                  (td.amountType != ItemDefinition.AmountType.Genetics && id.amountType != ItemDefinition.AmountType.Genetics || (targetItem.instanceData != null ? targetItem.instanceData.dataInt : -1) == (item.instanceData != null ? item.instanceData.dataInt : -1)) &&
                  (item.instanceData == null || item.instanceData.subEntity.Value == 0U || !(bool)(UnityEngine.Object)id.GetComponent<ItemModSign>()) && 
                  (targetItem.instanceData == null || targetItem.instanceData.subEntity.Value == 0U || !(bool)(UnityEngine.Object)td.GetComponent<ItemModSign>())))
                return false;

            ItemContainer tc = targetItem.contents;
            if ((item.contents?.capacity ?? 0) != (tc?.capacity ?? 0)) 
                return false;
            ...
            if (projectile != null)
            {
                ...
                {
                    p.GiveItem(ItemManager.CreateByItemID(projectile.primaryMagazine.ammoType.itemid, projectile.primaryMagazine.contents));
            ...
            {
                FlameThrower flameThrower = held as FlameThrower;
                if (flameThrower.ammo > 0)
                {
                    p.GiveItem(ItemManager.CreateByItemID(flameThrower.fuelType.itemid, flameThrower.ammo));
                    flameThrower.ammo = 0;
                }
            }
            ...
            if (held is Chainsaw)
            {
                Chainsaw chainsaw = held as Chainsaw;
                if (chainsaw.ammo > 0)
                {
                    p.GiveItem(ItemManager.CreateByItemID(chainsaw.fuelType.itemid, chainsaw.ammo));
                    chainsaw.ammo = 0;
                }
            }
        ...
        {
            Item i = item.item;
            Item z = targetItem.item;
            if (i.info.itemid != z.info.itemid ||
                i.skin != z.skin ||
                i.name != z.name ||
                i.text != z.text ||
                i.contents?.itemList.Count > 0 ||
                z.contents?.itemList.Count > 0)
                return false;

            if ((i.contents?.capacity ?? 0) != (z.contents?.capacity ?? 0)) 
                return false;
        ...
        private Item OnItemSplit(Item item, int amount)
        {
            ...
            BaseProjectile.Magazine newItemMag = (held as BaseProjectile)?.primaryMagazine;
            ...
            if (held is FlameThrower)
                (held as FlameThrower).ammo = 0;

            if (held is Chainsaw)
                (held as Chainsaw).ammo = 0;

            BaseProjectile.Magazine itemMagDefault = (held as BaseProjectile)?.primaryMagazine;
