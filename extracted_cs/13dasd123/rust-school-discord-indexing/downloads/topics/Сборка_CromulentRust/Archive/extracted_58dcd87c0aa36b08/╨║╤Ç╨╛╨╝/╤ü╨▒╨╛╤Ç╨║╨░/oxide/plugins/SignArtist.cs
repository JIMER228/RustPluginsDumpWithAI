...
namespace Oxide.Plugins
{
    ...
    internal class SignArtist : RustPlugin
    {
                ...
                if (request.Sender != null)
                {
                    ...
                    if (signArtist.Settings.ConsoleLogging)
                    {
                        ...
                        signArtist.Puts(signArtist.GetTranslation("LogEntry"), request.Sender.displayName,
                            request.Sender.userID.Get(), textureId, request.Sign.ShortPrefabName, request.Url);
                    }
                    ...
                    if (signArtist.Settings.FileLogging)
                    {
                        ...
                        signArtist.LogToFile("log",
                            string.Format(signArtist.GetTranslation("LogEntry"), request.Sender.displayName,
                                request.Sender.userID.Get(), textureId, request.Sign.ShortPrefabName,
        ...
        private bool HasCooldown(BasePlayer player)
        {
            ...
            if (!cooldowns.ContainsKey(player.userID.Get()))
            {
                cooldowns.Add(player.userID.Get(), 0);
            }

            // Check if the command is on cooldown or not.
            return Time.realtimeSinceStartup - cooldowns[player.userID.Get()] < Settings.Cooldown;
        }
        ...
        {
            return Settings.Cooldown - (Time.realtimeSinceStartup - cooldowns[player.userID.Get()]);
        }
        ...
        private void SetCooldown(BasePlayer player)
        {
            ...
            // Set the last use
            cooldowns[player.userID.Get()] = Time.realtimeSinceStartup;
        }
