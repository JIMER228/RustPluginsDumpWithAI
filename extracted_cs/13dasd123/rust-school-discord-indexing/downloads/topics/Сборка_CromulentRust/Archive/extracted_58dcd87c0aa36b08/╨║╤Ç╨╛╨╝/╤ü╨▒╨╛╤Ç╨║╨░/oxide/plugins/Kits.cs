...
namespace Oxide.Plugins
{
    ...
    class Kits : RustPlugin
    {
        ...
        private void OnPlayerRespawned(BasePlayer player)
        {
            ...
            if (Configuration.AllowAutoToggle && !playerData[player.userID.Get()].ClaimAutoKits)
            {
                player.ChatMessage(Message("Error.AutoKitDisabled", player.userID.Get()));
                return;
            }
        ...
        private bool TryClaimKit(BasePlayer player, string name, bool usingUI)
        {
            ...
            {
                if (usingUI)
                    CreateMenuPopup(player, Message("Error.EmptyKitName", player.userID.Get()));
                else player.ChatMessage(Message("Error.EmptyKitName", player.userID.Get()));
                return false;
            }
            ...
            {
                if (usingUI)
                    CreateMenuPopup(player, Message("Error.InvalidKitName", player.userID.Get()));
                else player.ChatMessage(Message("Error.InvalidKitName", player.userID.Get()));
                return false;
            }
        ...
        private object CanClaimKit(BasePlayer player, KitData.Kit kit, bool ignoreAuthCost = false)
        {
            ...
            if (Configuration.WipeCooldowns.TryGetValue(kit.Name, out int wipeCooldownTime))
            {
                if (kitData.IsOnWipeCooldown(wipeCooldownTime, out wipeCooldownTime))
                    return string.Format(Message("Error.CanClaim.WipeCooldown", player.userID.Get()), FormatTime(wipeCooldownTime));
            }

            if (playerData.Find(player.userID.Get(), out PlayerData.PlayerUsageData playerUsageData))
            {
                if (kit.Cooldown > 0)
                {
                    double cooldownRemaining = playerUsageData.GetCooldownRemaining(kit.Name);
                    if (cooldownRemaining > 0)
                        return string.Format(Message("Error.CanClaim.Cooldown", player.userID.Get()), FormatTime(cooldownRemaining));
                }
                ...
                if (kit.MaximumUses > 0)
                {
                    ...
                    if (currentUses >= kit.MaximumUses)
                        return Message("Error.CanClaim.MaxUses", player.userID.Get());
                }
            }
            ...
            if (!ignoreAuthCost && kit.Cost > 0)
            {
                if (!ChargePlayer(player, kit.Cost))
                    return string.Format(Message("Error.CanClaim.InsufficientFunds", player.userID.Get()), kit.Cost, Message($"Cost.{_costType}", player.userID.Get()));
            }
        ...
        {
            playerData[player.userID.Get()].OnKitClaimed(kit);

            Interface.CallHook("OnKitRedeemed", player, kit.Name);

            if (Configuration.LogKitsGiven)
                LogToFile("Kits_Received", $"{player.displayName} ({player.userID.Get()}) - Received {kit.Name}", this);
        }
        #endregion
                ...
                case CostType.ServerRewards:
                    {
                        ...
                        return (bool)ServerRewards?.Call("TakePoints", player.userID.Get(), amount);
                    }
        ...
        {
            if (Configuration.NPCKitMenu.ContainsKey(npcPlayer.userID.Get()))            
                OpenKitGrid(player, 0, npcPlayer.userID.Get());            
        }
        #endregion
        ...
        public object GiveKit(BasePlayer player, string name)
        {
            ...
            if (string.IsNullOrEmpty(name))
                return Message("Error.EmptyKitName", player.userID.Get());

            if (!kitData.Find(name, out KitData.Kit kit))
                return Message("Error.InvalidKitName", player.userID.Get());
        ...
        {
            CuiElementContainer container = UI.Container(UI_MENU, "0 0 0 0.9", new UI4(0.2f, 0.15f, 0.8f, 0.85f), true, "Overlay");

            UI.Panel(container, UI_MENU, Configuration.Menu.Panel.Get, new UI4(0.005f, 0.93f, 0.995f, 0.99f));

            UI.Label(container, UI_MENU, Message("UI.Title", player.userID.Get()), 20, new UI4(0.015f, 0.93f, 0.99f, 0.99f), TextAnchor.MiddleLeft);
            ...
            if (IsAdmin(player) && npcId == 0UL)            
                UI.Button(container, UI_MENU, Configuration.Menu.Color2.Get, Message("UI.CreateNew", player.userID.Get()), 14, new UI4(0.85f, 0.9375f, 0.9525f, 0.9825f), "kits.create");
        ...
        private void CreateGridView(BasePlayer player, CuiElementContainer container, int page = 0, ulong npcId = 0UL)
        {
            ...
            if (list.Count == 0)
            {
                UI.Label(container, UI_MENU, Message("UI.NoKitsAvailable", player.userID.Get()), 14, new UI4(0.015f, 0.88f, 0.99f, 0.92f), TextAnchor.MiddleLeft);
                return;
            }

            PlayerData.PlayerUsageData playerUsageData = playerData[player.userID.Get()];
        ...
        {            
            ...
            if (Configuration.AdminIgnoreRestrictions && IsAdmin(player))
            {
                buttonText = Message("UI.Redeem", player.userID.Get());
            ...
            else
            {
                if (!string.IsNullOrEmpty(kit.RequiredPermission) && !permission.UserHasPermission(player.UserIDString, kit.RequiredPermission))
                {
                    buttonText = Message("UI.NeedsPermission", player.userID.Get());
                    buttonColor = Configuration.Menu.Disabled.Get;
                }
                else if (kit.Cooldown > 0 && cooldown > 0)
                {
                    UI.Label(container, UI_MENU, string.Format(Message("UI.Cooldown", player.userID.Get()), FormatTime(cooldown)), 12,
                        new UI4(position.xMin + 0.005f, position.yMin + 0.0475f, position.xMax - 0.005f, position.yMax - 0.3f), TextAnchor.MiddleLeft);

                    buttonText = Message("UI.OnCooldown", player.userID.Get());
                    buttonColor = Configuration.Menu.Disabled.Get;
                }
                else if (kit.MaximumUses > 0 && currentUses >= kit.MaximumUses)
                {
                    buttonText = Message("UI.MaximumUses", player.userID.Get());
                    buttonColor = Configuration.Menu.Disabled.Get;
                }
                else if (kit.Cost > 0)
                {
                    UI.Label(container, UI_MENU, string.Format(Message("UI.Cost", player.userID.Get()), kit.Cost, Message($"Cost.{_costType}", player.userID.Get())), 12,
                        new UI4(position.xMin + 0.005f, position.yMin + 0.0475f, position.xMax - 0.005f, position.yMax - 0.3f), TextAnchor.MiddleLeft);

                    buttonText = Message("UI.Purchase", player.userID.Get());
                    buttonColor = Configuration.Menu.Color2.Get;
                    buttonCommand = $"kits.gridview redeem {CommandSafe(kit.Name)} {page} {npcId}";
                }
                else
        ...
        private void OpenKitView(BasePlayer player, string name, int page, ulong npcId)
        {
            ...
            UI.Label(container, UI_MENU, $"{Message("UI.Title", player.userID.Get())} - {name}", 20, new UI4(0.015f, 0.93f, 0.99f, 0.99f), TextAnchor.MiddleLeft);
            ...
            if (isAdmin)
            {
                UI.Button(container, UI_MENU, Configuration.Menu.Color2.Get, Message("UI.EditKit", player.userID.Get()), 14, new UI4(0.7525f, 0.9375f, 0.845f, 0.9825f), $"kits.edit {CommandSafe(name)}");
                UI.Button(container, UI_MENU, Configuration.Menu.Color2.Get, Message("UI.CreateNew", player.userID.Get()), 14, new UI4(0.85f, 0.9375f, 0.9525f, 0.9825f), "kits.create");
            }
            ...
            AddTitleSperator(container, i += 1, Message("UI.Details", player.userID.Get()));
            AddLabelField(container, i += 1, Message("UI.Name", player.userID.Get()), kit.Name);
            ...
            if (!string.IsNullOrEmpty(kit.Description)) 
            {
                ...
                AddLabelField(container, i += 1, Message("UI.Description", player.userID.Get()), kit.Description, descriptionSlots - 1);
            ...
            {
                AddTitleSperator(container, i += 1, Message("UI.Usage", player.userID.Get()));

                if (kit.MaximumUses != 0)
                {
                    int playerUses = playerUsageData.GetKitUses(kit.Name);

                    AddLabelField(container, i += 1, Message("UI.MaxUses", player.userID.Get()), kit.MaximumUses.ToString());
                    AddLabelField(container, i += 1, Message("UI.YourUses", player.userID.Get()), playerUses.ToString());
                ...
                if (kit.Cooldown != 0)
                {
                    ...
                    AddLabelField(container, i += 1, Message("UI.CooldownTime", player.userID.Get()), FormatTime(kit.Cooldown));
                    AddLabelField(container, i += 1, Message("UI.CooldownRemaining", player.userID.Get()), cooldownRemaining == 0 ? Message("UI.None", player.userID.Get()) : 
                ...
                if (kit.Cost != 0)
                {
                    AddLabelField(container, i += 1, Message("UI.PurchaseCost", player.userID.Get()), $"{kit.Cost} {(Message($"Cost.{_costType}", player.userID.Get()))}");
            ...
            {
                AddTitleSperator(container, i += 1, Message("UI.CopyPaste", player.userID.Get()));
                AddLabelField(container, i += 1, Message("UI.FileName", player.userID.Get()), kit.CopyPasteFile);
            }
        ...
        private void CreateKitLayout(BasePlayer player, CuiElementContainer container, KitData.Kit kit)
        {
            ...
            UI.Label(container, UI_MENU, Message("UI.KitItems", player.userID.Get()), 14, new UI4(0.51f, 0.88f, 0.995f, 0.92f), TextAnchor.MiddleLeft);

            // Main Items
            UI.Panel(container, UI_MENU, Configuration.Menu.Color4.Get, new UI4(0.505f, 0.835f, 0.995f, 0.875f));
            UI.Label(container, UI_MENU, Message("UI.MainItems", player.userID.Get()), 14, new UI4(0.51f, 0.835f, 0.995f, 0.875f), TextAnchor.MiddleLeft);
            ...
            UI.Label(container, UI_MENU, Message("UI.WearItems", player.userID.Get()), 14, new UI4(0.51f, 0.365f, 0.995f, 0.405f), TextAnchor.MiddleLeft);
            ...
            UI.Label(container, UI_MENU, Message("UI.BeltItems", player.userID.Get()), 14, new UI4(0.51f, 0.2225f, 0.995f, 0.2625f), TextAnchor.MiddleLeft);
        ...
        {
            if (!_kitCreators.TryGetValue(player.userID.Get(), out KitData.Kit kit))
                return;
            ...
            UI.Label(container, UI_MENU, Message("UI.Title.Editor", player.userID.Get()), 20, new UI4(0.015f, 0.93f, 0.99f, 0.99f), TextAnchor.MiddleLeft);

            UI.Button(container, UI_MENU, Configuration.Menu.Color3.Get, "<b>×</b>", 20, new UI4(0.9575f, 0.9375f, 0.99f, 0.9825f), "kits.close");

            // Kit Options
            AddTitleSperator(container, 0, Message("UI.Details", player.userID.Get()));
            AddInputField(container, 1, Message("UI.Name", player.userID.Get()), "name", kit.Name);
            AddInputField(container, 2, Message("UI.Description", player.userID.Get()), "description", kit.Description, 3);
            AddInputField(container, 6, Message("UI.IconURL", player.userID.Get()), "image", kit.KitImage);

            AddTitleSperator(container, 7, Message("UI.UsageAuthority", player.userID.Get()));
            AddInputField(container, 8, Message("UI.Permission", player.userID.Get()), "permission", kit.RequiredPermission);
            AddInputField(container, 9, Message("UI.AuthLevel", player.userID.Get()), "authLevel", kit.RequiredAuth);
            AddToggleField(container, 10, Message("UI.IsHidden", player.userID.Get()), "isHidden", kit.IsHidden);

            AddTitleSperator(container, 11, Message("UI.Usage", player.userID.Get()));
            AddInputField(container, 12, Message("UI.MaxUses", player.userID.Get()), "maximumUses", kit.MaximumUses);
            AddInputField(container, 13, Message("UI.CooldownSeconds", player.userID.Get()), "cooldown", kit.Cooldown);
            AddInputField(container, 14, Message("UI.PurchaseCost", player.userID.Get()), "cost", kit.Cost);

            AddTitleSperator(container, 15, Message("UI.CopyPaste", player.userID.Get()));
            AddInputField(container, 16, Message("UI.FileName", player.userID.Get()), "copyPaste", kit.CopyPasteFile);

            // Kit Items
            CreateKitLayout(player, container, kit);

            // Kit Saving            
            UI.Button(container, UI_MENU, Configuration.Menu.Color2.Get, Message("UI.SaveKit", player.userID.Get()), 14, new UI4(0.005f, 0.005f, 0.2475f, 0.045f), $"kits.savekit {overwrite}");
            ...
            UI.Label(container, UI_MENU, Message("UI.Overwrite", player.userID.Get()), 14, new UI4(0.2875f, 0.005f, 0.495f, 0.045f), TextAnchor.MiddleLeft);

            // Item Management            
            UI.Button(container, UI_MENU, Configuration.Menu.Color3.Get, Message("UI.ClearItems", player.userID.Get()), 14, new UI4(0.505f, 0.005f, 0.7475f, 0.045f), $"kits.clearitems {overwrite}");
            UI.Button(container, UI_MENU, Configuration.Menu.Color2.Get, Message("UI.CopyInv", player.userID.Get()), 14, new UI4(0.7525f, 0.005f, 0.995f, 0.045f), $"kits.copyinv {overwrite}");
        ...
        private void ccmdKitsClose(ConsoleSystem.Arg arg)
        {
            ...
            _kitCreators.Remove(player.userID.Get());
                ...
                case "redeem":
                    {
                        ...
                        if (TryClaimKit(player, kit, true))
                        {
                            CuiHelper.DestroyUi(player, UI_MENU);
                            CuiHelper.DestroyUi(player, UI_POPUP);
                            player.ChatMessage(string.Format(Message("Notification.KitReceived", player.userID.Get()), kit));
                        }
        ...
        private void ccmdCreateKit(ConsoleSystem.Arg arg)
        {
            ...
            if (IsAdmin(player))
            {
                _kitCreators[player.userID.Get()] = new KitData.Kit();
        ...
        private void ccmdEditKit(ConsoleSystem.Arg arg)
        {
            ...
            if (IsAdmin(player))
            {
                ...
                {
                    player.ChatMessage(string.Format(Message("Chat.Error.DoesntExist", player.userID.Get()), name));
                    return;
                }

                _kitCreators[player.userID.Get()] = KitData.Kit.CloneOf(editKit);
                OpenKitsEditor(player);
            }
        }
        ...
        private void ccmdSaveKit(ConsoleSystem.Arg arg)
        {
            ...
            {
                CreateMenuPopup(player, Message("SaveKit.Error.NoName", player.userID.Get()));
                return;
            }

            if (kit.ItemCount == 0 && string.IsNullOrEmpty(kit.CopyPasteFile))
            {
                CreateMenuPopup(player, Message("SaveKit.Error.NoContents", player.userID.Get()));
                return;
            }

            if (kitData.Exists(kit.Name) && !arg.GetBool(0))
            {
                CreateMenuPopup(player, Message("SaveKit.Error.Exists", player.userID.Get()));
                return;
            }
            ...
            CreateMenuPopup(player, string.Format(Message("SaveKit.Success", player.userID.Get()), kit.Name));
        }
        ...
        private void SetParameter(BasePlayer player, KitData.Kit kit, string fieldName, object value)
        {
            ...
            switch (fieldName)
            {
                ...
                case "permission":
                    ...
                    {
                        CreateMenuPopup(player, Message("EditKit.PermissionPrefix", player.userID.Get()));
                        return;
                    }
                ...
                case "cost":
                    {
                        if (!TryConvertValue<int>(value, out int intValue))
                            CreateMenuPopup(player, Message("EditKit.Number", player.userID.Get()));
                ...
                case "isHidden":
                    {
                        if (!TryConvertValue<bool>(value, out bool boolValue))
                            CreateMenuPopup(player, Message("EditKit.Bool", player.userID.Get()));
        ...
        private void cmdKit(BasePlayer player, string command, string[] args)
        {
            ...
            switch (args[0].ToLower())
            {
                ...
                case "list":
                    if (isAdmin)                    
                        player.ChatMessage(string.Format(Message("Chat.KitList", player.userID.Get()), kitData.Keys.ToSentence()));
                    else
                    {
                        List<KitData.Kit> kits = Facepunch.Pool.GetList<KitData.Kit>();
                        GetUserValidKits(player, kits);

                        player.ChatMessage(string.Format(Message("Chat.KitList", player.userID.Get()), kits.Select((KitData.Kit kit) => kit.Name).ToSentence()));
                ...
                case "new":
                    if (!isAdmin)
                    {
                        player.ChatMessage(Message("Chat.Error.NotAdmin", player.userID.Get()));
                        return;
                    }
                ...
                case "edit":
                    ...
                    if (args.Length != 2)
                    {
                        player.ChatMessage(Message("Chat.Error.NoKit", player.userID.Get()));
                        return;
                    }

                    if (!kitData.Find(args[1], out KitData.Kit editKit))
                    {
                        player.ChatMessage(string.Format(Message("Chat.Error.DoesntExist", player.userID.Get()), args[1]));
                        return;
                    }
                ...
                case "delete":
                    ...
                    SaveKitData();
                    player.ChatMessage(string.Format(Message("Chat.KitDeleted", player.userID.Get()), args[1]));
                ...
                case "give":
                    ...
                    if (args.Length != 3)
                    {
                        player.ChatMessage(Message("Chat.Error.GiveArgs", player.userID.Get()));
                        return;
                    }
                    ...
                    if (target == null)
                    {
                        player.ChatMessage(Message("Chat.Error.NoPlayer", player.userID.Get()));
                        return;
                    }

                    if (!kitData.Find(args[2], out KitData.Kit giveKit))
                    {
                        player.ChatMessage(Message("Chat.Error.DoesntExist", player.userID.Get()));
                        return;
                    }

                    GiveKit(target, giveKit);
                    player.ChatMessage(string.Format(Message("Chat.KitGiven", player.userID.Get()), target.displayName, args[2]));
                    return;
                ...
                case "givenpc":
                    ...
                    if (args.Length != 2)
                    {
                        player.ChatMessage(Message("Chat.Error.NPCGiveArgs", player.userID.Get()));
                        return;
                    }
                    ...
                    if (npc == null)
                    {
                        player.ChatMessage(Message("Chat.Error.NoNPCTarget", player.userID.Get()));
                        return;
                    }
                    ...
                    player.ChatMessage(string.Format(Message("Chat.KitGiven", player.userID.Get()), npc.displayName, args[1]));
                    return;
                ...
                case "reset":
                    ...
                    playerData.Wipe();
                    SavePlayerData();
                    player.ChatMessage(Message("Chat.ResetPlayers", player.userID.Get()));
                ...
                case "autokit":
                    ...
                    {
                        bool v = playerData[player.userID.Get()].ClaimAutoKits = !playerData[player.userID.Get()].ClaimAutoKits;
                        player.ChatMessage(string.Format(Message("Chat.AutoKit.Toggle", player.userID.Get()), Message($"Chat.AutoKit.{v}", player.userID.Get())));
                    }
                    return;
                ...
                default:
                    ...
                    {
                        player.ChatMessage(string.Format(Message("Chat.Error.DoesntExist", player.userID.Get()), args[0]));
                        return;
                    }

                    if (TryClaimKit(player, args[0], false))
                        player.ChatMessage(string.Format(Message("Notification.KitReceived", player.userID.Get()), args[0]));
        ...
        {
            player.ChatMessage(string.Format(Message("Chat.Help.Title", player.userID.Get()), Version));
            player.ChatMessage(Message("Chat.Help.1", player.userID.Get()));
            player.ChatMessage(Message("Chat.Help.2", player.userID.Get()));
            
            if (Configuration.AllowAutoToggle)
                player.ChatMessage(Message("Chat.Help.9", player.userID.Get()));

            if (IsAdmin(player))
            {
                player.ChatMessage(Message("Chat.Help.3", player.userID.Get()));
                player.ChatMessage(Message("Chat.Help.4", player.userID.Get()));
                player.ChatMessage(Message("Chat.Help.5", player.userID.Get()));
                player.ChatMessage(Message("Chat.Help.6", player.userID.Get()));
                player.ChatMessage(Message("Chat.Help.7", player.userID.Get()));
                player.ChatMessage(Message("Chat.Help.8", player.userID.Get()));
                player.ChatMessage(Message("Chat.Help.10", player.userID.Get()));
            }
        }
        #endregion
        ...
        private class PlayerData
        {
            ...
            internal void OnKitClaimed(BasePlayer player, KitData.Kit kit)
            {
                ...
                if (!_players.TryGetValue(player.userID.Get(), out PlayerUsageData playerUsageData))
                    playerUsageData = _players[player.userID.Get()] = new PlayerUsageData();
