...
{
    [Info("AdvancedEntityLimit", "M&B-Studios", "1.0.7")]
    class AdvancedEntityLimit : RustPlugin
    {
        ...
        private object CanBuild(Planner planner, Construction prefab, Construction.Target target)
        {
            ...
            var entity = planner.GetEntity();
            return res.Value ? (bool?)null : false;
        }
        private void ChatMessageHook(BasePlayer player, KeyValuePair<int, bool> res)
        {
            player.ChatMessage(cfg.Prefix + cfg.MaxObjectsMessage.Replace("{0}", res.Key.ToString()));
        }
        #endregion
        ...
        private int DeployedByPlayer(BasePlayer player, string prefabname) =>
            DeployedByPlayers(new List<ulong>() { player.userID.Get() }, prefabname);
        ...
        private KeyValuePair<int, bool> CanPlayerBuild(BasePlayer player, string prefabname)
        {
            ...
            if (limit == -1)
                return new KeyValuePair<int, bool>(0, true);
            List<ulong> players = new List<ulong>() { player.userID.Get() };
        ...
        private bool PlayerInClan(BasePlayer player)
        {
            ...
            {
                ClansAuthor.Mevent => (string)GetClanPlugin()?.Call("GetClanOf", player.userID.Get()) != null,
                ClansAuthor.k1lly0u => (string)GetClanPlugin()?.Call("GetClanOf", player.userID.Get()) != null,
                _ => false
            };
        }
        ...
        private IReadOnlyCollection<string> GetClanMembers(BasePlayer player)
        {
            ...
            return (List<string>)GetClanPlugin().Call("GetClanMembers", player.userID.Get());
        }
        ...
        private void UI_DrawPrivilegePages(BasePlayer player, int page = 0)
        {
            ...
            {
                Button = { Color = "0.5254902 0.5019608 0.4666667 0.7607843", Command = page - 1 >= 0 ? $"ael.changeprivpage {page - 1}" : "" },
                Text = { Text = "<", Font = "robotocondensed-bold.ttf", FontSize = 21, Align = TextAnchor.MiddleCenter, Color = "0.8078431 0.7803922 0.7411765 0.6588235" },
            ...
            container.Add(new CuiButton
            {
                Button = { Color = "0.5254902 0.5019608 0.4666667 0.7607843", Command = list.Skip((page + 1) * PRIVILEGES_PER_PAGE).Any() ? $"ael.changeprivpage {page + 1}" : "" },
                Text = { Text = ">", Font = "robotocondensed-bold.ttf", FontSize = 21, Align = TextAnchor.MiddleCenter, Color = "0.8078431 0.7803922 0.7411765 0.6588235" },
        ...
        private void UI_DrawEntities(BasePlayer player, string privilege, string search = "", int page = 0)
        {
            ...
            container.Add(new CuiButton
            {
                Button = { Color = "0.6470588 0.6235294 0.6 1", Command = $"ael.opensearch {privilege}" },
            ...
            foreach (var entity in list.Skip(page * ITEMS_PER_PAGE).Take(ITEMS_PER_PAGE))
            {
                ...
                {
                    CursorEnabled = false,
                    Image = { Color =  "0.8078431 0.7803922 0.7411765 0.4",
                        FadeIn = 0.1f },
                    RectTransform =
                    {
                ...
                {
                    CursorEnabled = false,
                    Image = { Color = "0.5254902 0.5019608 0.4666667 0.7607843",
                        FadeIn = 0.1f },
                    RectTransform =
                    {
                ...
                container.Add(new CuiElement
                {
                    ...
                    Components =
                    {
                        new CuiTextComponent
                        {
                            ...
                            Align = TextAnchor.UpperCenter, Color = "0.8078431 0.7803922 0.7411765 1",
                            FadeIn = 0.1f
                        },
        ...
        private void UI_DrawSearch(BasePlayer player, string privilege)
        {
            ...
            {
                CursorEnabled = false,
                Image = { Color = "0.5254902 0.5019608 0.4666667 0.6745098" },
                RectTransform =
                {
        ...
        private void UI_DrawPrivileges(BasePlayer player, string privilege = "", int page = 0)
        {
            ...
            for (var i = 0; i < list.Count(); i++)
            {
                ...
                container.Add(new CuiButton
                {
                    Button =
                    {
                        Color = "0.5254902 0.5019608 0.4666667 1",
