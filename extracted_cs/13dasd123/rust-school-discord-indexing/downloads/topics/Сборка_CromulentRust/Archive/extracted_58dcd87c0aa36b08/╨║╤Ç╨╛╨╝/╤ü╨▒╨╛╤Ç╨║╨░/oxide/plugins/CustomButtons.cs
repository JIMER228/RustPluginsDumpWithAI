...
{
    [Info("CustomButtons", "David", "2.0.6")]
    ...
    {   
        ...
        {   
            if (!_playerData.ContainsKey(player.userID.Get())) return;
            ...
            {       
                ...
                if (_playerData[player.userID.Get()].hidden)
                {   
            ...
            //if (_playerData[player.userID.Get()].hidden) DestroyHidden(player);
        ...
        {
            if (!_playerData.ContainsKey(player.userID.Get()))
            {
                _playerData.Add(player.userID.Get(), new PlayerData());
                _playerData[player.userID.Get()].hidden = false;
                SavePlayerData();
            }
        }
        ...
        private void custombuttonstoggle(BasePlayer player)
        {
            ...
            else { _playerData[player.userID.Get()].hidden = true; }
            SavePlayerData();
            CreateCui(player);
        }
        ...
        private void custombuttons_runbtncmd(ConsoleSystem.Arg arg) 
        {
            ...
            .Replace("{player.steam}", $"{player.userID.Get()}");
