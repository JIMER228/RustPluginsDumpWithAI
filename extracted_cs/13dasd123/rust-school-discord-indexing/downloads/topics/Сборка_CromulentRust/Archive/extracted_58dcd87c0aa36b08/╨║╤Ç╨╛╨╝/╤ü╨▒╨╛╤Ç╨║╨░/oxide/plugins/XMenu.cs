    ...
    class XMenu : RustPlugin
    {
		...
		private void GUIAddButton(BasePlayer player)
		{
			...
			container.Add(new CuiLabel
            {
                ...
                Text = { Text = lang.GetMessage("AddCLOSE", this, player.UserIDString), Align = TextAnchor.MiddleCenter, FontSize = 10, Color = "1 1 1 0.803612" }
            }, ".AddButton_Close");
			...
			foreach(var button in config.ButtonAdd)
			{
                ...
                {
                    RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1", OffsetMax = "0 -1" },
                    Text = { FadeIn = fadein, Text = lang.GetMessage(button.Key, this, player.UserIDString), Align = TextAnchor.MiddleCenter, FontSize = 10, Color = "1 1 1 0.803612" }
                }, ".AddButton_N");
		...
		private void GUIPluginsInfo(BasePlayer player)
		{
            ...
            foreach(var plugininfo in config.PluginsInfo.Where(p => plugins.Find(p.PluginName)))
			{
				...
				if(plugininfo.Parameter == "player")
                    ...
                    {
                        RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1", OffsetMax = "0 0" },
                        Text = { Text = string.Format(lang.GetMessage(plugininfo.PluginName, this, player.UserIDString), plugins.Find(plugininfo.PluginName).CallHook(plugininfo.HookName, player.userID.Get())), Align = TextAnchor.MiddleCenter, FontSize = 11 }
                    }, ".InfoText");
        ...
        {		
			...
			public static MenuConfig GetNewConfiguration()
            {
                return new MenuConfig
                {
                    ...
                    {
						MenuColor = "1 0.2703612 0 0.503612",
						MenuMaterial = "assets/icons/greyout.mat",
						ButtonColor = "0.21703612 0.22103612 0.20903612 0.7503612",
					...
					{
						EMenuColor = "1 0.2703612 0 0.503612",
        ...
        private void InitializeLang()
        {
			...
			{
				["TITLE"] = "WELCOME TO MY COOL SERVER\n<size=9>ONLINE: {0}/{1} | JOIN: {2} | SLEEPING: {3} | TIME: {4}</size>",
			...
			{
				["TITLE"] = "ДОБРО ПОЖАЛОВАТЬ НА МОЙ КРУТОЙ СЕРВЕР\n<size=9>ОНЛАЙН: {0}/{1} | ПОДКЛЮЧАЮТСЯ: {2} | СЛИПЕРЫ: {3} | ВРЕМЯ: {4}</size>",
			...
			{
				["TITLE"] = "ЛАСКАВО ПРОСИМО НА МІЙ КРУТИЙ СЕРВЕР\n<size=9>ОНЛАЙН: {0}/{1} | ПІД'ЄДНУЮТЬСЯ: {2} | СЛІПЕРИ: {3} | ЧАС: {4}</size>",
			...
			{
				["TITLE"] = "BIENVENIDOS A MI SERVIDOR COOL\n<size=9>EN LÍNEA: {0}/{1} | UNIRSE: {2} | DURMIENDO: {3} | TIEMPO: {4}</size>",
				["AddOPEN"] = "MÁS BOTONES",
				["AddCLOSE"] = "CERCA",
		...
		private void GUIAddButtonOpen(BasePlayer player)
		{
            ...
            {
                RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1", OffsetMax = "0 -1" },
                Text = { Text = lang.GetMessage("AddOPEN", this, player.UserIDString), Align = TextAnchor.MiddleCenter, FontSize = 8, Color = "1 1 1 0.803612" }
            }, ".AddButton_Open");
