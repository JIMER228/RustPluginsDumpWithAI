    ...
    public class CopyPaste : CovalencePlugin
    {
        ...
        private void PasteLoop(PasteData pasteData)
        {
            ...
            foreach (var data in todo)
            {
                ...
                var ownerId = pasteData.BasePlayer?.userID.Get() ?? 0;
                ...
                if (autoTurret != null)
                {
                    ...
                    if (pasteData.BasePlayer != null && !authorizedPlayers.Contains(pasteData.BasePlayer.userID.Get()) &&
                        pasteData.Auth)
                        authorizedPlayers.Add(pasteData.BasePlayer.userID.Get());
                ...
                if (slotEntity.GetComponent<CodeLock>())
                {
                    ...
                    if (!string.IsNullOrEmpty(code))
                    {
                        ...
                        if (pasteData.Auth && pasteData.BasePlayer != null)
                            codeLock.whitelistPlayers.Add(pasteData.BasePlayer.userID.Get());
        ...
        private void CmdCopy(IPlayer player, string command, string[] args)
        {
            ...
            var savename = args[0];
            var success =
                TryCopyFromSteamId(basePlayer == null ? 0ul : basePlayer.userID.Get(), savename,
        ...
        private void CmdPaste(IPlayer player, string command, string[] args)
        {
            ...
            var basePlayer = player.Object as BasePlayer;
            var success =
                TryPasteFromSteamId(basePlayer == null ? 0ul : basePlayer.userID.Get(), args[0],
                    args.Skip(1).ToArray()) as string;
