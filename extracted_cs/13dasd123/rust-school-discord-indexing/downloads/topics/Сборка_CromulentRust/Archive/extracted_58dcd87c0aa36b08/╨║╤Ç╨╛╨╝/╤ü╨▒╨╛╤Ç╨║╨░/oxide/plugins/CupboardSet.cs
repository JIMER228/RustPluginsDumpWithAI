    ...
    public class CupboardSet : RustPlugin
    {
        ...
        void OnLootEntity(BasePlayer player, BuildingPrivlidge entity)
        {
            ...
            foreach (var entityAuthorizedPlayer in entity.authorizedPlayers)
            {
                ...
                var findPlayer = covalence.Players.FindPlayerById(entityAuthorizedPlayer.userid.ToString());
                string name = entityAuthorizedPlayer.username.Substring(0, Math.Min(10, entityAuthorizedPlayer.username.Length));
                if (findPlayer != null)
                {
                    name = findPlayer.Name.Substring(0, Math.Min(10, findPlayer.Name.Length));
                }
                cont.Add(new CuiElement()
                {
        ...
        void UICommands(ConsoleSystem.Arg arg)
        {
            ...
            {
                case "remove":
                    var ent = BaseNetworkable.serverEntities.Find(new NetworkableId(ulong.Parse(arg.Args[2]))) as BuildingPrivlidge;
                    if (ent.authorizedPlayers.Any(p => p.userid == arg.Player().userID.Get()))
                    {
                        if(!ent.authorizedPlayers.Any(p => p.userid == ulong.Parse(arg.Args[1]))) return;
                        var friend = ent.authorizedPlayers.FirstOrDefault(p => p.userid == ulong.Parse(arg.Args[1]));
                        ...
                        if(friend.userid == arg.Player().userID.Get())
                        {
