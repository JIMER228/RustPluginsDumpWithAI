	...
	public class Notify : RustPlugin
	{
		...
		private class NotifyComponent : FacepunchBehaviour
		{
			...
			private void Awake()
			{
				...
				_instance._notifications[_player.userID.Get()] = this;
			...
			private void OnDestroy()
			{
				...
				if (_player != null)
				{
					...
					_instance?._notifications.Remove(_player.userID.Get());
				}
			}
		...
		private NotifyComponent GetComponent(BasePlayer player)
		{
			...
			return _notifications.TryGetValue(player.userID.Get(), out component)
				? component
