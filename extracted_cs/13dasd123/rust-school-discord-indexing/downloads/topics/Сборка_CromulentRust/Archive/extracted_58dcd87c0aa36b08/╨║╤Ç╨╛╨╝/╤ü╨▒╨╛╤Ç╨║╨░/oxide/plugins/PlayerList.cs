...
namespace Oxide.Plugins
{
    ...
    {
        public List<ulong> deleteplayer = new List<ulong>();
                ...
                if (_instance != null && _instance.ImageLibrary)
                    container.Add(new CuiElement
                    {
                        ...
                        Components =
                        {
                            ...
                            {
                                Url = "https://gspics.org/images/2023/10/20/02CWTN.png"
                                 /*Png = _instance.ImageLibrary.Call<string>("GetImage", "logoimg")*/
                            },
       ...
       ? BasePlayer.activePlayerList
           .Where(p => p.IsConnected && p.net.connection.authLevel < 1 && deleteplayer.Contains(p.userID.Get()) == false)
           .Skip(panel.AmountOnPage * page)
           .Take(panel.AmountOnPage)
           .ToList()
       : BasePlayer.activePlayerList
           .Where(p =>
               p.IsConnected && p.net.connection.authLevel < 1 && deleteplayer.Contains(p.userID.Get()) == false &&
               (string.Equals(p.UserIDString, target, StringComparison.CurrentCultureIgnoreCase) ||
                p.UserIDString.ToLower().Contains(target.ToLower()) ||
           ...
           .ToList();
            ...
            for (var i = 0; i < list.Count; i++)
            {
                ...
                _config.Panel.Get(ref container, check.userID.Get(), name, Layer, Layer + $".Player.{i}",
        ...
        [ConsoleCommand("listdelete")]
        private void ConsoleCmdListDelete(ConsoleSystem.Arg arg)
        {
            if (arg.Args.Count() == 0 || arg.Args == null)
            {
                PrintWarning("Используй listdelete <SteamId>");
                return;
            }
            if (arg.Args[0] != null)
            {
                ulong id = (ulong)arg.Args[0].ToLong();
                deleteplayer.Add(id);
            }
        }
        private void SaveData()
        {
            Interface.Oxide.DataFileSystem.WriteObject($"{Title}/deletedplayer", deleteplayer);
        }
        private void LoadData()
        {
            try
            {
                deleteplayer = Interface.Oxide.DataFileSystem.ReadObject<List<ulong>>($"{Title}/deletedplayer");
            }
            catch (Exception e)
            {
