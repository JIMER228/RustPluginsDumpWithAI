...
namespace Oxide.Plugins
{
    ...
    class FRPGSystem : RustPlugin
    {
        ...
        private void RegenUI(BasePlayer player)
        {
            ...
            CuiElementContainer container = new CuiElementContainer();
            float rateanimal = 43200;
            if (timesplayer.ContainsKey(player.userID.Get()))
            {
                ulong ftime = timesplayer[player.userID.Get()] + GetSecondsConnected(player.Connection);
                rateanimal = ftime;
            }
            float maxrateanimal = 43200;
            int regen = 0;
                    ...
                    if (timesplayer.ContainsKey(player.userID.Get()) && !needexit.Contains(player.userID.Get()))
                    {
                        if (timesplayer.TryGetValue(player.userID.Get(), out time))
                        {
                            ...
                            if (ftime >= 43200) //36000 //43200
                            {
                                ...
                                //timer.Once(1, () => { player.ChatMessage($"<color=#6A5ACD><b>[</color><color=#6b53f5>CROM</color><color=#6A5ACD>]</b></color> <color=#1E90FF>-</color> Вы успешно получили <color=#FF9740>{hp} xп</color>, <color=#FF9740>x{damage} урон(а)</color>, <color=#FF9740>{regen} реген(а)</color> за наигранные <color=#FF9740>10 часов</color> на сервере."); });
                                timesplayer.Remove(player.userID.Get());
        ...
        private void OnServerInitialized()
        {
            ...
            timer.Every(90, () => { foreach (var player in BasePlayer.activePlayerList) player.ChatMessage($"<color=#6A5ACD><b>[</color><color=#6b53f5>CROM</color><color=#6A5ACD>]</b></color> <color=#1E90FF>-</color> За каждые наигранные <color=#4169E1>12 часов</color> вы будете получать <color=#16C553FF>подарки</color> <color=#4169E1>+ 100хп, + X1 урон и + 1 рег в сек.!</color>."); });
        }
