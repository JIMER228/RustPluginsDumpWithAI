    ...
    public class IQTurret : RustPlugin
    {
        ...
        Object OnSwitchToggle(IOEntity entity, BasePlayer player)
        {
            ...
            if (config.LimitController.UseLimitControll)
            {
                ...
                {
                    SendChat(GetLang(electricSwitch.OwnerID != player.userID.Get() ? "IS_LIMIT_TRUE_OTHER" : "IS_LIMIT_TRUE", player.UserIDString), player);
                    return false;
                }
            }
        ...
        Boolean CanPickupEntity(BasePlayer player, AutoTurret turret)
        {
            ...
            if (turret == null || turret.OwnerID == 0 || !localRepositories.ContainsKey(player.userID.Get())) return canPickUp;
        ...
        /// <summary>
        /// Обновление :
        /// - Изменен и улучшен метод включения туреллей
        /// </summary>
        ///
        ...
        private void SetupTurret(BaseEntity entityTurret, Boolean IsInit = false)
        {
            ...
            if (entityTurret == null || entityTurret is NPCAutoTurret ||
                ...
                entityTurret.skinID == 738583205) return;
        ...
        private void TurretAction(BasePlayer player, String action)
        {
            ...
            Int32 limitPlayer = GetLimitPlayer(player.userID.Get());

            if (localRepositories.TryGetValue(player.userID.Get(), out List<LocalRepository> localRepository))
            {
                ...
                filteredList.Sort((x, y) =>
                {
                    ...
                    Boolean xFlag = IsValidTurret(x.turret, action) && HasValidSwitch(x.electricSwitch, action) && x.turret.OwnerID == player.userID.Get() && GetBuildingID(null, x.turret) == playerBuildingID;
                    Boolean yFlag = IsValidTurret(y.turret, action) && HasValidSwitch(y.electricSwitch, action) && y.turret.OwnerID == player.userID.Get() && GetBuildingID(null, y.turret) == playerBuildingID;
                ...
                foreach (LocalRepository item in filteredList)
                {
                    ...
                    switch (action)
                    {
                        ...
                        case "on" when permission.UserHasPermission(player.UserIDString, PermissionTurnAllTurretsOn) &&
                                       IsLimitPlayer(player.userID.Get(), buildingID):
                            return;
                        case "on" when permission.UserHasPermission(player.UserIDString, PermissionTurnAllTurretsOn):
            ...
            String lang = GetLang("INFORMATION_MY_LIMIT", player.UserIDString, (limitPlayer - GetAmountActiveTurretPlayer(player.userID.Get(), playerBuildingID)));
