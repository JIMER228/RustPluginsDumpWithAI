...
namespace Oxide.Plugins
{
    ...
    public class MapNoteTeleport : CovalencePlugin
    {
        ...
        {
            if (player == null || note == null || _playersOnCooldown.Contains(player.userID.Get()))
            {
                return;
            }
            ...
            var playerData = GetPlayerData(player.userID.Get());
            if (!playerData.Enabled)
            {
                return;
            }
        ...
        private object OnEntityTakeDamage(BasePlayer player, HitInfo info)
        {
            ...
            if (_playersWithGodMode.Contains(player.userID.Get()))
            {
                return true;
            }
