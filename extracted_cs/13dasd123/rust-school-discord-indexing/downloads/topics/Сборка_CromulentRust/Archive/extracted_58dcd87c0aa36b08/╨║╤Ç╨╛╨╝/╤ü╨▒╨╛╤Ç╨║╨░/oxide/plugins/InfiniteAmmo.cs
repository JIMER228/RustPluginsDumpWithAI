...
namespace Oxide.Plugins
{
    ...
    public class InfiniteAmmo : CovalencePlugin
    {
        ...
        {
            if (!IsInfiniteAmmo(player.userID.Get()))
            {
                return;
            }
        ...
        private void CommandToggleAmmo(IPlayer iPlayer, string cmd, string[] args)
        {
            ...
            if (!IsInfiniteAmmo(bPlayer.userID.Get()))
            {
                _storedData.ActiveUsers.Add(bPlayer.userID.Get());
                SendMessage(bPlayer, GetMessage("Enabled", bPlayer));
            }
            else
            {
                _storedData.ActiveUsers.Remove(bPlayer.userID.Get());
