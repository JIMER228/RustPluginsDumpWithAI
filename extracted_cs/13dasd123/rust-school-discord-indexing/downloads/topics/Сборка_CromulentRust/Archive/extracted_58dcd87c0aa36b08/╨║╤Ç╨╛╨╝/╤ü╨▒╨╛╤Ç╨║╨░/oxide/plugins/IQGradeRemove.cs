...
{
    [Info("IQGradeRemove", "Mercury", "2.27.33")]
    ...
    public class IQGradeRemove : RustPlugin
    {
        ...
        private class LocalRepositoryUser
        {
            ...
            private void DeleteTimerUI(BasePlayer player)
            {
        ...
        private void OnEntityBuilt(Planner plan, GameObject go)
        {
            ...
            if(entity.prefabID is 133905323 or 3390532) return;
        ...
        private void UpgradeHit(BasePlayer player, BaseEntity entity, LocalRepositoryUser repositoryUser)
        {
            ...
            if(buildingBlock.grade == BuildingGrade.Enum.Metal && SkinBuilding != 0 && SkinBuilding == 10221)
                buildingBlock.SetCustomColour(player.LastBlockColourChangeId == 0 ? buildingBlock.currentSkin.GetStartingDetailColour(0) : player.LastBlockColourChangeId);
            ...
            internal class RemovePreset
            {
                ...
                internal class ReturnedSettings
                {
                    [JsonProperty(LanguageEn
                        ...
                        : "Настройка возврата ресурсов за удаление строений")]
                    public Building BuildingSettings = new Building();

                    [JsonProperty(LanguageEn
                        ? "Resource/item return settings for item deletion"
                        : "Настройка возврата ресурсов/предметов за удаление предметов")]
        ...
        /// <summary>
        /// - Добавлена ссылка на картинки
        /// - Добавлена возможность отвязать функции плагина от радиального меню RUST
        /// - Добавлено разрешение - iqgraderemove.removeadmin , позволяющее удалять объекты игнорируя ограничения (будьте внимательны, вы сможете удалять объекты сервера, которые невозможно скрафтить (например : переработчики, машины, коптеры))
        /// - Исправлен возврат префаба деревянной баррикады после обновления игры
        /// - Исправлен конфликт с AutoBaseUpgrade
        /// </summary>
        
        
        [PluginReference] Plugin IQTurret, Friends, Clans, IQRecycler, XBuildingSkinMenu, AutoBaseUpgrade;

        private Item CreateItem(BaseEntity entity, String NormalizePrefab)
        {
            return entity switch
            {
                ModularCarGarage when entity.ShortPrefabName.Contains("electrical.modularcarlift") =>
                    ItemManager.CreateByName("modularcarlift", 1, entity.skinID),
                BaseCombatEntity baseCombat when baseCombat.pickup.itemTarget == null => ItemManager.CreateByName(
                    NormalizePrefab, 1, entity.skinID),
                BaseCombatEntity baseCombat => ItemManager.Create(baseCombat.pickup.itemTarget, 1, entity.skinID),
                _ => null
            };
        }
        ...
        private void AllObjectAction(BasePlayer player, BaseEntity entity)
        {
            ...
            buildingBlocks = GetBlocks(player, entity);
        ...
        Object OnStructureUpgrade(BaseCombatEntity entity, BasePlayer player, BuildingGrade.Enum grade)
        {
            ...
            if(entity.prefabID is 133905323 or 3390532) return null;
            ...
            if (String.IsNullOrWhiteSpace(canUpgrade))
            {
                ...
                if (grade == BuildingGrade.Enum.Metal && SkinBuilding == 10221)
                    buildingBlock.SetCustomColour(player.LastBlockColourChangeId); 
            ...
            private void Building_AllObjectButton()
            {
                ...
                container.Add(new CuiElement
                {
                    ...
                    Components =
                    {
                        ...
                        new CuiRectTransformComponent
                        {
                            AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-17.33321 -17.3339054",
        ...
        private List<BuildingBlock> GetBlocks(BasePlayer player, BaseEntity buildingBlock)
        {
            ...
            List<BuildingBlock> buildingBlocks = new();
        ...
        String CanRemove(BasePlayer player, BaseEntity entity)
        {
            ...
            if (RemoveConfig.RemoveOnlyFriends && player.userID.Get() != entity.OwnerID)
            {
                ...
                return GetLang("REMOVE_NOT_REMOVE_ALIEN_OBJECT", player.UserIDString);
            else if (privilege == null && entity.OwnerID != player.userID.Get())
                return GetLang("REMOVE_NOT_REMOVE_ALIEN_OBJECT", player.UserIDString);
