...
namespace Oxide.Plugins 
{
    ...
    class PermissionsManager : RustPlugin
    {
        ...
        {
            foreach (BasePlayer player in BasePlayer.activePlayerList.Where(player => MenuOpen.Contains(player.userID.Get())))
        ...
        {
            if (MenuOpen.Contains(player.userID.Get()))
                DestroyMenu(player, true);
        }
        ...
        void DestroyMenu(BasePlayer player, bool all)
        {
            ...
            if (all)
            {
                ...
                MenuOpen.Remove(player.userID.Get());
            }
        }
        ...
        {
            var path = ActiveAdmins[player.userID.Get()];
            PlugList.Clear();
            path.noOfPlugs = 0;
        ...
        {
            MenuOpen.Add(player.userID.Get());
        ...
        void PMMainUI(BasePlayer player, bool group, int page)
        {
            ...
            { 
                foreach (var plyr in BasePlayer.allPlayerList.OrderBy(x => x.displayName).OrderBy(x => x?.net?.connection == null))
                {
                    ...
                    if (pos1 > 0 && pos1 < 21)
                    {
                        elements.Add(new CuiButton { Button = { Command = $"PMSelected player {plyr.userID.Get()}", Color = config.ButtonColour }, RectTransform = { AnchorMin = $"0.3 {bottom}", AnchorMax = $"0.7 {top}" }, Text = { Text = StripSlashes(plyr.displayName), FontSize = 11, Align = TextAnchor.MiddleCenter, Color = plyr?.net?.connection == null ? "1 1 1 1" : "0.2 1 0.2 1" } }, mainName);
                        top = top - 0.025f;
                        bottom = bottom - 0.025f;
                    }
                }
        ...
        private void PMTogglePlayerGroup(ConsoleSystem.Arg arg, bool group, int page)
        {
            ...
            if (group)
                ActiveAdmins[player.userID.Get()].GPage = page;
            else
                ActiveAdmins[player.userID.Get()].PPage = page;
        ...
        {
            var path = ActiveAdmins[arg.Player().userID.Get()];
        ...
        private void GroupsPM(ConsoleSystem.Arg arg)
        {
            ...
            ActiveAdmins[arg.Player().userID.Get()].GPage = Convert.ToInt32(arg.Args[0]);
            DestroyMenu(arg.Player(), false);
        ...
        private void PlayersPM(ConsoleSystem.Arg arg)
        {
            var path = ActiveAdmins[arg.Player().userID.Get()];
            if (arg.Player() == null || arg.Args == null || arg.Args.Length < 1) return;
            ActiveAdmins[arg.Player().userID.Get()].PPage = Convert.ToInt32(arg.Args[0]);
            DestroyMenu(arg.Player(), false);
        ...
        private void ClosePM(ConsoleSystem.Arg arg)
        {
            ...
            ActiveAdmins.Remove(arg.Player().userID.Get());
        ...
        private void Navigate(ConsoleSystem.Arg arg)
        {
            ...
            ActiveAdmins[arg.Player().userID.Get()].pluginPage = Convert.ToInt32(arg.Args[1]);
            DestroyMenu(arg.Player(), false);
            string[] argsOut;
            if (arg.Args[0] == "true")
            {
        ...
        private void PMSelected(ConsoleSystem.Arg arg)
        {
            ...
            if (arg.Args[0] == "player")
                ActiveAdmins[arg.Player().userID.Get()].subject = FindPlayer(Convert.ToUInt64(arg.Args[1]));
            else
                ActiveAdmins[arg.Player().userID.Get()].subjectGroup = arg.Args[1];
            CmdPerms(arg.Player(), null, argsOut);
            return;
        }
        ...
        void CmdPerms(BasePlayer player, string command, string[] args)
        {
            ...
            if (!ActiveAdmins.ContainsKey(player.userID.Get()))
                ActiveAdmins.Add(player.userID.Get(), new Info());
            ...
            else if (args[0] == "group")
            {
                ...
                if (Groups.Contains($"{args[1]}"))
                {
                    ...
                    ActiveAdmins[player.userID.Get()].subjectGroup = args[1];
                    if (MenuOpen.Contains(player.userID.Get()))
                        DestroyMenu(player, true);
                    PMBgUI(player);
