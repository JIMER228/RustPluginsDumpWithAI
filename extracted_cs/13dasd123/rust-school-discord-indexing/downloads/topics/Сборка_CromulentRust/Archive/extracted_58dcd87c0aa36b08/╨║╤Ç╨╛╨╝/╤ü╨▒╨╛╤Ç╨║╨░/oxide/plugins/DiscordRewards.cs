...
namespace Oxide.Plugins
{
    ...
    class DiscordRewards : ChaosPlugin
    {
        ...
        private void OnDiscordGuildCreated(DiscordGuild guild)
        {
            ...
            Debug.Log($"[Discord Rewards] Connection to {Guild.Name} established! DiscordRewards is now active");

            NitroRole = GetBoosterRole();

            if (NitroRole == null)
                Debug.Log($"[Discord Rewards] No nitro role found");
            else Debug.Log($"[Discord Rewards] Nitro role found {NitroRole.Name} ({NitroRole.Id})");
        ...
        {
            if (userData.Data.FindById(member.Id, out ulong steamId))
            {
        ...
        private void OnDiscordDirectMessageCreated(DiscordMessage message, DiscordChannel channel)
        {
            ...
            message.Author.CreateDirectMessageChannel(Client, (DiscordChannel dmChannel) => dmChannel.CreateMessage(Client, GetString("Discord.InvalidToken", string.Empty)));
        }
        ...
        private bool AttemptTokenValidation(DiscordUser discordUser, int code)
        {
            if (userData.Data.IsValidToken(code, out UserData.DiscordToken token))
            {
                ...
                {
                    discordUser.CreateDirectMessageChannel(Client, (DiscordChannel dmChannel) => dmChannel.CreateMessage(Client, GetString("Discord.TokenExpired", player)));
                    userData.Data.InvalidateToken(code);
                    return true;
                }
                ...
                if (!player)
                {
                    discordUser.CreateDirectMessageChannel(Client, (DiscordChannel dmChannel) => dmChannel.CreateMessage(Client, FormatString("Discord.FailedToFindPlayer", player, token.UserId)));
                    return true;
                }

                if (!player.IsConnected)
                {
                    discordUser.CreateDirectMessageChannel(Client, (DiscordChannel dmChannel) => dmChannel.CreateMessage(Client, GetString("Discord.NotOnServer", player)));
                    return true;
                }

                if (player.IsDead())
                {
                    discordUser.CreateDirectMessageChannel(Client, (DiscordChannel dmChannel) => dmChannel.CreateMessage(Client, GetString("Discord.UserIsDead", player)));
                    return true;
                }
            ...
            public class RewardOptions
            {
                ...
                public void IssueRewards(BasePlayer player, UserData.User user)
                {
                    Guild.GetGuildMember(Client, user.Id, (GuildMember guildMember) =>
                    {
                        ...
                        if (sendUpdate)
                            Guild.ModifyGuildMember(Client, user.Id, guildMemberUpdate);
                    });
                }
