...
{
    [Info("WelcomePanel", "David", "3.2.21")]
    ...
    public class WelcomePanel : RustPlugin
    {
        ...
        void OnPlayerConnected(BasePlayer player)
        {
            if (config.main.open)
            {
                if (config.main.onceWipe)
                {
                    if (playerData.Contains(player.userID.Get()))
                        return;

                    playerData.Add(player.userID.Get());
                }
        ...
        string ReplaceTags(BasePlayer player, string _text)
        {
            ...
            if (ServerRewards != null)
            {
                if (text.Contains("{rp}"))
                {
                    string playersRP = $"{ServerRewards?.Call<int>("CheckPoints", player.userID.Get())}";
