...
using static ProtoBuf.ProjectileShoot;
...
namespace Oxide.Plugins
{
    ...
    public class ExplosiveWeapons : CovalencePlugin
    {
        ...
        string PermIgnoreStun = "ExplosiveWeapons.IgnorStun";
        ...
        private void Init()
        {
            ...
            permission.RegisterPermission(PermIgnoreStun, this);
        ...
        private void JericoGive(IPlayer iplayer, string command, string[] args)
        {
            ...
            Item item = ItemManager.CreateByName("rocket.launcher", 1, jericoSkin);
            item.name = "Jerico";
        ...
        {
            BasePlayer player = iplayer.Object as BasePlayer;
            Item item = ItemManager.CreateByName("rocket.launcher", 1, seekerSkin);
            item.name = "Seeker";
            player.inventory.GiveItem(item);
        }
        ...
        {
            BasePlayer player = iplayer.Object as BasePlayer;
            Item item = ItemManager.CreateByName("rocket.launcher", 1, trackSkin);
            item.name = "Follow";
            player.inventory.GiveItem(item);
        }
        ...
        {
            BasePlayer player = iplayer.Object as BasePlayer;
            Item item = ItemManager.CreateByName("rocket.launcher", 1, javalin);
            item.name = "Javalin";
            player.inventory.GiveItem(item);
        }
        ...
        {
            BasePlayer player = iplayer.Object as BasePlayer;
            Item item = ItemManager.CreateByName("grenade.f1", 1, multiGrenadeSkin);
            item.name = "Multi";
            player.inventory.GiveItem(item);
        }
        ...
        {
            BasePlayer player = iplayer.Object as BasePlayer;
            Item item = ItemManager.CreateByName("grenade.f1", 1, flashBangSkin);
            item.name = "Flash";
            player.inventory.GiveItem(item);
        }
        ...
        {
            BasePlayer player = iplayer.Object as BasePlayer;
            Item item = ItemManager.CreateByName("grenade.f1", 1, impactNadeSkin);
            item.name = "Impact";
            player.inventory.GiveItem(item);
        }
        ...
        {
            BasePlayer player = iplayer.Object as BasePlayer;
            Item item = ItemManager.CreateByName("grenade.f1", 1, stunGrenadeSkin);
            item.name = "Stun";
            player.inventory.GiveItem(item);
        }
        ...
        {
            BasePlayer player = iplayer.Object as BasePlayer;
            Item item = ItemManager.CreateByName("grenade.f1", 1, healingSkin);
            item.name = "Heal";
            player.inventory.GiveItem(item);
        }
        ...
        {
            BasePlayer player = iplayer.Object as BasePlayer;
            JericoGive(iplayer, command, args);
            GiveSeeker(iplayer, command, args);
            GiveFollow(iplayer, command, args);
            givemulti(iplayer, command, args);
            giveflash(iplayer, command, args);
            giveimpact(iplayer, command, args);
            givestun(iplayer, command, args);
            givehealing(iplayer, command, args);
        }
        #endregion
        ...
        void OnRocketLaunched(BasePlayer player, BaseEntity entity)
        {
            ...
            if (invItem.skin == jericoSkin) // jerico
            {
                ...
                if (cd != 0f)
                {
                    var projectile = item as BaseProjectile;
                    entity.Kill();
                    projectile.primaryMagazine.contents = projectile.primaryMagazine.capacity;
                    projectile.SendNetworkUpdateImmediate();
                    player.ChatMessage($"Вы сможете использовать это через <color=#FF9740>{cd.ToString("0")}</color> секунд.");
                    PlayEffect("assets/prefabs/locks/keypad/effects/lock.code.denied.prefab", player);
                    return;
                }
                entity.Kill();
        ...
        private object OnExplosiveFuseSet(TimedExplosive ent, float fuseLength)
        {
            ...
            else if (ent.skinID == stunGrenadeSkin)
            {
                timer.Once(fuseLength - 0.05f, () =>
                {
                    ...
                    {
                        if (permission.UserHasPermission(seeker.UserIDString, PermIgnoreStun)) continue;
