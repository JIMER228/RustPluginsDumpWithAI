...
namespace Oxide.Plugins
{
    ...
    class industrialquarry : RustPlugin
    {
        ...
        [ChatCommand("softcleaniq")]
        private void ChatSoftIndustrialQuarry(BasePlayer player, string command, string[] args)
        {
            if (player.IsAdmin) //limit to admin
            {
                DoCleanUp(true);
                player.ChatMessage("Cleaned IQ Ents. Respawning in 5secs");
                ServerMgr.Instance.StartCoroutine(Startup(2));//restart plugin so it respawns and reconnects
            }
        }
        ...
        private void OnServerInitialized(bool initial)
        {
            ...
            else
            {
                ...
                {
                    DoCleanUp(true);
                    Puts("Fixing QuarryLock Deleting All The Containers");
                    ServerMgr.Instance.StartCoroutine(Startup(2));//restart plugin so it respawns and reconnects
                    return;
                }
        ...
        public void DoCleanUp(bool soft = false)
        {
            foreach (BaseNetworkable bn in BaseNetworkable.serverEntities.entityList.Get().Values) //Scan all server ents
            {
                if (bn.PrefabName == QuarryRock) //Found the rock things mounted to
                {
                    ...
                    Vis.Entities<BaseEntity>(bn.transform.position, 15f, list); //Scan everything within 6f
                    foreach (BaseEntity be in list)
                    {
                        if (!soft && be is IOEntity or IndustrialStorageAdaptor && !be.IsDestroyed && be.skinID == 1234567890) { be.skinID = 0; be.Kill(); }
                        if (soft && be is IndustrialStorageAdaptor && !be.IsDestroyed && be.skinID == 1234567890) { be.skinID = 0; be.Kill(); }
                    } //Kill io ents
                    if (!bn.IsDestroyed && !soft) { bn.Kill(); } //kill rock
                    continue;
                }
            }
        }
