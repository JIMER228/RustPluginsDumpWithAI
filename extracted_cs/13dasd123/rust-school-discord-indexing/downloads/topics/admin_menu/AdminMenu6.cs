...
{
    /*ПЛАГИН БЫЛ ПОФИКШЕН С ПОМОЩЬЮ ПРОГРАММЫ СКАЧАНОЙ С https://discord.gg/dNGbxafuJn */ /*ПЛАГИН БЫЛ ПОФИКШЕН С ПОМОЩЬЮ ПРОГРАММЫ СКАЧАНОЙ С https://discord.gg/dNGbxafuJn */ /*ПЛАГИН БЫЛ ПОФИКШЕН С ПОМОЩЬЮ ПРОГРАММЫ СКАЧАНОЙ С https://discord.gg/dNGbxafuJn */ /*ПЛАГИН БЫЛ ПОФИКШЕН С ПОМОЩЬЮ ПРОГРАММЫ СКАЧАНОЙ С https://discord.gg/dNGbxafuJn */ [Info("AdminMenu", "https://discord.gg/dNGbxafuJn", "1.1.8")]
        ...
        public class CUI
        {
            ...
            private static readonly Dictionary<Font, string> FontToString = new Dictionary<Font, string>
            {
                ...
                },
                {
                    ...
                    "nonenglish/notosansarabic-bold.ttf"
                }
            };
        ...
        public class PluginManagerContent : Content
        {
            ...
            protected override void Render(CUI.Element root, ConnectionData connectionData, Dictionary<string, object> userData)
            {
                ...
                IEnumerable<FileInfo> enumerable =
                    ...
                    select f;
                List<string> pluginNames = enumerable.Select(fileInfo => Path.GetFileNameWithoutExtension(fileInfo.FullName)).Where(name => name.Contains(searchQuery, System.Globalization.CompareOptions.IgnoreCase)).ToList();
                ...
                for (int i = 0; i < pageCount; i++)
                {
                    ...
                    for (int j = 0; j < rows; j++)
                    {
                        ...
                        for (int k = 0; k < buttonsToRender.Count; k++)
                        {
                            ...
                            panel.AddButton(command: $"adminmenu {button.Command} {pluginName}", sprite: "assets/content/ui/ui.background.rounded.png", imageType: UnityEngine.UI.Image.Type.Tiled, color: button.Style.BackgroundColor, anchorMin: "0.99 0.1", anchorMax: "0.99 0.9", offsetMin: $"-{(k + 1) * 100 + k * 5} 0", offsetMax: $"-{k * 100 + k * 5} 0").AddText(text: button.Label.Localize(connectionData.connection), align: TextAnchor.MiddleCenter, offsetMin: "10 0", offsetMax: "-10 0");
                        }

                        panel.AddText(text: $"<b>{(plugin != null ? $"{plugin.Name} v{plugin.Version} by {plugin.Author}" : pluginName)}</b>{(plugin != null && !string.IsNullOrEmpty(plugin.Description) ? $"\n<color=#7A7A7A><size=11>{plugin.Description}</size></color>" : string.Empty)}", align: TextAnchor.MiddleLeft, overflow: VerticalWrapMode.Truncate, anchorMin: "0.01 0.1", anchorMax: "0.99 0.9", offsetMax: $"-{buttonsToRender.Count * 100 + (buttonsToRender.Count - 1) * 5} 0");
                    }
                            ...
                            case "cb.exec":
                                ...
                                switch (qmcb.PlayerReceivers)
                                {
                                    case QMCustomButton.Recievers.None:
                                        commandsToExec.AddRange(qmcb.Commands.Select(cmd => FormatCommand(cmd, admin)));
                                        break;
                                    case QMCustomButton.Recievers.Online:
                                        foreach (BasePlayer activePlayer in BasePlayer.activePlayerList)
                                            commandsToExec.AddRange(qmcb.Commands.Select(cmd => FormatCommandToUser(cmd, activePlayer, admin)));
                                        break;
                                    case QMCustomButton.Recievers.Offline:
                                        foreach (BasePlayer sleeper in BasePlayer.sleepingPlayerList)
                                            commandsToExec.AddRange(qmcb.Commands.Select(cmd => FormatCommandToUser(cmd, sleeper, admin)));
                                        break;
                                    case QMCustomButton.Recievers.Everyone:
                                        foreach (BasePlayer player in BasePlayer.allPlayerList)
                                            commandsToExec.AddRange(qmcb.Commands.Select(cmd => FormatCommandToUser(cmd, player, admin)));
                                        break;
                                }
