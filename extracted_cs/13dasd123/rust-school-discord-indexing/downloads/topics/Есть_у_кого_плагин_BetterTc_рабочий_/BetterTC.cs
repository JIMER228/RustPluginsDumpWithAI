...
namespace Oxide.Plugins
{
    ...
    internal class BetterTC : RustPlugin
    {
		...
		string[] incompatiblePlugins = { "BuildingSkin", "BuildingSkins", "XBuildingSkinMenu" };
        ...
        private IEnumerator UpdateCost(BasePlayer player, BuildingPrivlidge cup){
            ...
            CreateGameTip(cup, totalCost.Count == 0 ? Languaje("NoUpgradeAvailable", player.UserIDString) : Languaje("TotalCostUP", player.UserIDString, costMessage), player, fxfinish, 10);
            //Puts("Cost Upgrade: " + (totalCost.Count == 0 ? Languaje("NoUpgradeAvailable", player.UserIDString) : Languaje("TotalCostUP", player.UserIDString, costMessage)) + " TC: " + player.transform.position);
            playerTeamMembers.Clear();
            totalCost.Clear();
            yield return 0;
        }
...
if (itemAmounts.Sum(x => x.amount) <= 0f){
    ...
    entity.OnRepairFinished(player); // Pass player reference here
    return true;
}
                ...
                using (WebClient client = new WebClient()){
                    ...
                    if (parsedData != null){
                        ...
                        if (configUpdated){
                            ...
                            Puts("CheckUpdate: Updated configuration with new items.");
                        } else {
                            if(player != null) player.ShowToast(GameTip.Styles.Error, "There are no new items.");
                            Puts("CheckUpdate: There are no new items.");
                        }
                    }
                }
            }
        ...
        private void commands(ConsoleSystem.Arg arg){
            ...
            if (cup == null || !BuildingCupboard.ContainsKey(cup)){
                ...
                if (cup == null || !BuildingCupboard.ContainsKey(cup)){
                    CreateGameTip(cup, Languaje("ErrorTC2", player.UserIDString), player, fxerror, 10, "danger");
                    return;
                }
            }
        ...
        private void ShowMenu(BasePlayer player, BuildingPrivlidge cup, int page = 0){
            ...
            foreach (var list_entry in BuildingItems){
                ...
                if (list_entry.color && unlock2){
                	UI.Panel(ref container, upgrade_3, "0.80 1.00 0.50 0.30", "0.82 0.66", "0.95 0.79");
                	UI.Image(ref container, upgrade_3, GetImageLibrary("color_" + BuildingCupboard[cup].colour), "0.83 0.67", "0.94 0.78");
                	UI.Button(ref container, upgrade_3, "0 0 0 0", "", 10, "0.82 0.66", "0.95 0.79", $"SENDCMD COLOR {ID} {list_entry.grade} {list_entry.skinid} {BuildingCupboard[cup].colour} {page}");
                }

                if (config.reskin){
                    bool ups = (BuildingCupboard[cup].reskin && BuildingCupboard[cup].id == ID);
                	UI.Panel(ref container, upgrade_3, ups ? "0.90 0.20 0.20 0.50" : "0.80 1.00 0.50 0.30", (list_entry.color && unlock2) ? "0.82 0.50" : "0.82 0.66", (list_entry.color && unlock2) ? "0.95 0.63" : "0.95 0.79");
                	UI.Image2(ref container, upgrade_3, -596876839, 0, (list_entry.color && unlock2) ? "0.825 0.505" : "0.825 0.665", (list_entry.color && unlock2) ? "0.945 0.615" : "0.94 0.785");
                	UI.Button(ref container, upgrade_3, "0 0 0 0", "", 10, (list_entry.color && unlock2) ? "0.82 0.50" : "0.82 0.66", (list_entry.color && unlock2) ? "0.95 0.63" : "0.95 0.79", unlock2 ? $"SENDCMD RESKIN {ID} {list_entry.grade} {list_entry.skinid} {page} {list_entry.color}" : $"SENDCMD ERROR");

                    bool ups2 = (BuildingCupboard[cup].workwallpaper != null && BuildingCupboard[cup].id == ID);
                    UI.Panel(ref container, upgrade_3, ups2 ? "0.90 0.20 0.20 0.50" : "0.80 1.00 0.50 0.30", (list_entry.color && unlock2) ? "0.82 0.34" : "0.82 0.50", (list_entry.color && unlock2) ? "0.95 0.47" : "0.95 0.63");  
                    UI.Image2(ref container, upgrade_3, -1501434104, 0, (list_entry.color && unlock2) ? "0.825 0.345" : "0.825 0.505", (list_entry.color && unlock2) ? "0.945 0.465" : "0.945 0.625");
                    UI.Button(ref container, upgrade_3, "0 0 0 0", "", 10, (list_entry.color && unlock2) ? "0.82 0.34" : "0.82 0.50", (list_entry.color && unlock2) ? "0.95 0.47" : "0.95 0.63", unlock3 ? $"SENDCMD WALLPAPER {ID} {list_entry.grade} {list_entry.skinid} {page}" : $"SENDCMD ERROR");
                }
		...
		private void ShowMenuAuthlist(BasePlayer player, BuildingPrivlidge cup, int page = 0){
            ...
            foreach (var entry in PlayersTC){
				...
				UI.Label(ref container, authlist_3, entry.username, 13, (showsteamid) ? "0.12 0.40" : "0.12 0.05", "0.8 0.95", "1.00 1.00 1.00 0.9", TextAnchor.MiddleLeft, true);
                
                if (showsteamid) UI.Label(ref container, authlist_3, entry.userid.ToString(), 10, "0.12 0.05", "0.8 0.50", "1.00 1.00 1.00 0.7", TextAnchor.MiddleLeft);
                ...
                UI.Image(ref container, authlist_3, entry.userid.ToString(), "0 0", "0.1 0.95");
        ...
        private void HarmonyPatches(){
            foreach (var nestedType in GetType().GetNestedTypes(BindingFlags.DeclaredOnly | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static)){
                if (nestedType.GetCustomAttributes(typeof(HarmonyPatch), false).Length == 0)
                    continue;

                HarmonyInstance.CreateClassProcessor(nestedType).Patch();
                Puts($"Patched: {nestedType.Name}");
            }
        }
        ...
        protected override void LoadDefaultMessages(){
            lang.RegisterMessages(new Dictionary<string, string> {
                ["title1"] = "АВТОМАТИЧЕСКОЕ ОБНОВЛЕНИЕ ЗДАНИЯ",
                ["title2"] = "ВЫБОР ЦВЕТА",
                ["title3"] = "АВТОРИЗОВАННЫЕ ИГРОКИ",
                ["title4"] = "ВЫБЕРИТЕ СКИН ДЛЯ TC",
                ["title5"] = "ВЫБЕРИТЕ ОБОИ ДЛЯ ОТДЕЛКИ СТЕН",
                ["CLOSE"] = "CLOSE",
                ["STOP"] = "STOP",
                ["UPGRADE"] = "UPGRADE",
                ["ListAuth"] = "LIST AUTH",
                ...
                ["RaidBlocked"] = "Вы не можете этого сделать, пока у вас есть RaidBlock.",
                ["ErrorTC2"] = "К сожалению, что-то пошло не так, откройте TC снова, стоя на строительном блоке.",
                ["UpgradeFinish"] = "Процесс улучшения завершен.",
                ["UpgradeFinishNoPlayer"] = "Обновление ваших зданий завершено. В вашей команде не обнаружено игроков.",
                ["RepairFinish"] = "Процесс ремонта завершен.",
                ["ReskinFinish"]  = "Процесс повторной установки завершен.",
                ["NoResourcesRepair"] = "Ремонт остановлен из-за нехватки ресурсов.",
                ...
                ["UpgradeBlock"] = "Улучите пол под шкафом до уровня, который хотите улучшить дом.",
                ["UpgradeLock"] = "У вас нет прав на улучшение выбранного параметра.",
                ["LOCK"] = "LOCK",
                ["EffectON"] = "EFFECT ON",
                ...
                ["TCSkin"] = "TC SKIN",
                ["WALLPAPER"] = "ОБОИ",
                ["WALLPAPERGRADE"] = "ОЦЕНКА МЕСТА",
                ["WALLPAPERALL"] = "РАЗМЕСТИТЬ ВСЕ",
                ["NoResourcesWallpaper"] = "Поклейка обоев была приостановлена из-за нехватки ткани в ТК.",
                ["WallpaperFinish"] = "Установка обоев завершена.",
                ["WallpaperFinishNoPlayer"] ="Поклейка обоев на ваших зданиях завершена. В вашей команде не обнаружено игроков.",
                ["BOTHSIDES"] = "Обе стороны?",
                ["TotalCostUP"] = "Общая стоимость обновления: {0}",
                ["NoUpgradeAvailable"] = "Улучшать нечего, стоимость равна 0."
            }, this);
        }
