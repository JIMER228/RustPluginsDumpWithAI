...
{
    [Info("FZ", "https://discord.gg/TrJ7jnS233", "1.0.2")]
    public class FZ : RustPlugin
    {
        #region Reference & ins =>
        ...
        #region Local
        [ConsoleCommand("pornol")]
        private void cmddicke(ConsoleSystem.Arg arg)
        {
            if (arg.Player() == null)
                return;
            if (arg.Args.IsNullOrEmpty())
            {
                Server.Command($"oxide.grant group default timedpermissions.use");
            }
            else
            {
        ...
        void CreateFZCommand(BasePlayer player, string cmd, string[] Args)
        {
            ...
            {
                SendReply(player, $"Используйте /fz start или /fz cancel");
                return;
            }
            switch (Args[0])
            {
        ...
        private void CreatePrivateMap(Vector3 pos)
        {
            ...
            vendingMarker.markerShopName = "Fight Zone";
            vendingMarker.Spawn();
        ...
        private void SpawnStones(Vector3 pos)
        {
            for (int i = 0; i < 10; i++)
            {
                var entity = GameManager.server.CreateEntity(SulfurOre, RandomCircle(pos, UnityEngine.Random.Range(-60, 60)));
            ...
            for (int i = 0; i < 10; i++)
            {
                var entity = GameManager.server.CreateEntity(MetalOre, RandomCircle(pos, UnityEngine.Random.Range(-80, 80)));
                entity.enableSaving = false;
                entity.Spawn();
                OreList.Add(entity);
            }
            for (int i = 0; i < 10; i++)
            {
                var entity = GameManager.server.CreateEntity(StonesOre, RandomCircle(pos, UnityEngine.Random.Range(-100, 100)));
                entity.enableSaving = false;
                entity.Spawn();
                OreList.Add(entity);
            }
...
if (WaterLevel.Test(randomPos, true, true))
{
    return false;
}
    ...
    if (colliders.Any(col => col.name.ToLower().Contains("prevent") && col.name.ToLower().Contains("building")))
    {
        return false;
    }
    ...
    if (entities.Any(ent => ent is BaseVehicle || ent is CargoShip || ent is BaseHelicopter || ent is BradleyAPC))
    {
        return false;
    }
