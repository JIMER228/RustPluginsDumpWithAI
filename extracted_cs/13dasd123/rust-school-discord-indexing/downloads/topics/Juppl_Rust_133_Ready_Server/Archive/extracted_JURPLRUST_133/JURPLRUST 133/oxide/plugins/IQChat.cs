        ...
        private class Configuration
        {
            ...
            public static Configuration GetNewConfiguration()
            {
                return new Configuration
                {
                    ...
                    {
                        ["iqchat.default"] = "<color=white><b>[Игрок]</b></color>",
                        ["iqchat.vip"] = "<color=yellow><b>[Vip]</b></color>",
                        ["iqchat.premium"] = "<color=#4040ff><b>[Premium]</b></color>",
						["iqchat.elite"] = "<color=#a600ff><b>[Elite]</b></color>",
						["iqchat.king"] = "<color=#00e8f1><b>[KING]</b></color>",
						["iqchat.moderator"] = "<color=blue><b>[Модератор]</b></color>",
						["iqchat.admin"] = "<color=red><b>[A]</b></color>",
                    },
                    ...
                    {
                        BroadcastTitle = "<color=#007FFF><b>[Уведомление]</b></color>",
                    ...
                    {
                        ["Оскорбление игроков"] = 300,
						["Без причины"] = 120,
                        ["Оскорбление родителей"] = 600,
                    },
                    ...
                    AlertSettings = new AlertSetting
                    {
                        ...
                        {
                            "Добро пожаловать на сервер Jurpl Rust\nРады,что выбрал именно нас!",
...
;
        ...
        private void Message(BasePlayer player, string message)
        {
            ...
            for (int i = 0; i < cfg.BadWords.Count; i++)
            {
                ...
                foreach (var msg in message.Split(' '))
                {
                    if (msg.ToLower() == BadWord.ToLower())
                        message = message.Replace(msg, cfg.ReplaceBadWord);
                }
            }
            ...
            if (IsMuted)
            {
                ReplySystem(player, string.Format(lang.GetMessage("FUNC_MESSAGE_ISMUTED_TRUE", this, player.UserIDString), FormatTime(TimeSpan.FromSeconds(DataPlayer.MuteChatTime - CurrentTime()))));
                return;
            }
            ...
            if (HookResult != null)
            {
                ...
                FormatMessage = $"{ChatTags["Prefixes"].ToString().Replace("— ", "")} <color={DataPlayer.NickColor}>{DisplayNick}</color>:  <color={DataPlayer.MessageColor}>{ChatTags["Message"]}</color>";
            }
            else
                //if (channel == Chat.ChatChannel.Team)
                ...
                FormatMessage = $"{DataPlayer.ChatPrefix.Replace("—", "")} <color={DataPlayer.NickColor}>{DisplayNick}</color>: <color={DataPlayer.MessageColor}>{message}</color>";

            ReplyChat(player, FormatMessage);
            Puts($"{player}: {message}");
        ...
        public void RenameFunc(BasePlayer player, string NewName)
        {
            if (permission.UserHasPermission(player.UserIDString, config.RenamePermission))
            {
                ...
                ReplySystem(player, String.Format(lang.GetMessage("COMMAND_RENAME_SUCCES", this, player.UserIDString), NewName));
            }
            else ReplySystem(player, lang.GetMessage("COMMAND_NOT_PERMISSION", this, player.UserIDString));
        }
        ...
        public void UI_MainMenu(BasePlayer player)
        {
            ...
            CuiHelper.DestroyUi(player, "TITLE_PANEL");
            ...
            CuiHelper.DestroyUi(player, "BTN_CLOSE_PARENT");
            ...
            CuiHelper.DestroyUi(player, "BTN_PREFIX_SETTINGS");
            ...
            CuiHelper.DestroyUi(player, "BTN_NICK_COLOR_SETTINGS");
            ...
            CuiHelper.DestroyUi(player, "BTN_MSG_COLOR_SETTINGS");
            ...
            if (player.IsAdmin())
            {
                ...
                CuiHelper.DestroyUi(player, "AdminPanel_TITLE");
        ...
        public void UI_MuteMenu(BasePlayer player)
        {
            ...
            }, "Hud", MUTE_MENU_PARENT);

            CuiHelper.DestroyUi(player, "TITLE_PANEL_MUTE");
            ...
            foreach (var pList in BasePlayer.activePlayerList)
            {
                ...
                CuiHelper.DestroyUi(player, "BUTTON" + pList.userID);
                ...
                }, MUTE_MENU_PARENT, "BUTTON" + pList.userID);
            ...
            CuiHelper.DestroyUi(player, "BTN_CLOSE_MUTE_MENU_PARENT");
        ...
        public void UI_MuteTakeAction(BasePlayer player, ulong userID)
        {
            ...
            CuiHelper.DestroyUi(player, "TITLE_MENU_MUTE_ACTION");
        ...
        void UI_MuteTakeActionShowListReason(BasePlayer player, ulong userID, string MuteFormat)
        {
            ...
            CuiHelper.DestroyUi(player, "TAKE_ACTION_MUTE_TITLE");
        ...
        public void NewElementSettings(BasePlayer player)
        {
            ...
            CuiHelper.DestroyUi(player, "MySettingPanel");
        ...
        public void NewElementPrefixSetting(BasePlayer player)
        {
            ...
            for (int i = 0; i < PrefixList.Count; i++)
            {
                ...
                CuiHelper.DestroyUi(player, $"BUTTON_{i}");
        ...
        void ChatAlertPlayers(BasePlayer player, string cmd, string[] arg)
        {
            ...
            if (arg.Length == 0 || arg == null)
            {
                ReplySystem(player, lang.GetMessage("FUNC_MESSAGE_NO_ARG_BROADCAST", this, player.UserIDString));
                return;
            }
            string Message = "";
        ...
        void PmChat(BasePlayer player, string cmd, string[] arg)
        {
            ...
            {
                ReplySystem(player, lang.GetMessage("COMMAND_PM_NOT_NULL_MSG", this, player.UserIDString));
                return;
            }
            ...
            ReplySystem(TargetUser, String.Format(lang.GetMessage("COMMAND_PM_SEND_MSG", this, player.UserIDString), DisplayNick, Message));
            ReplySystem(player, String.Format(lang.GetMessage("COMMAND_PM_SUCCESS", this, player.UserIDString), Message));
			...
			Server.Run.Server.Normal($"sendmsgadmin [PM] {player.displayName}({DisplayNick}) отправил сообщение игроку - {TargetUser.displayName}\nСООБЩЕНИЕ : {Message}");
        }
        ...
        void RChat(BasePlayer player, string cmd, string[] arg)
        {
            ...
            if (arg.Length == 0 || arg == null)
            {
                ReplySystem(player, lang.GetMessage("COMMAND_R_NOTARG", this, player.UserIDString));
                return;
            }
            if (!PMHistory.ContainsKey(player))
            {
                ReplySystem(player, lang.GetMessage("COMMAND_R_NOTMSG", this, player.UserIDString));
                return;
            }
            ...
            ReplySystem(RetargetUser, String.Format(lang.GetMessage("COMMAND_PM_SEND_MSG", this, player.UserIDString), DisplayNick, Message));
            ReplySystem(player, String.Format(lang.GetMessage("COMMAND_PM_SUCCESS", this, player.UserIDString), Message));
			...
			Server.Run.Server.Normal($"sendmsgadmin [R] {player.displayName} отправил сообщение игроку - {RetargetUser.displayName}\nСООБЩЕНИЕ : {Message}");
        }
        ...
        public void Log(string LoggedMessage) => ConVar.Server.Log("IQChatLogs.txt", LoggedMessage);
        ...
        void ReplyChat(BasePlayer player, string format)
        {
            ...
            BasePlayer.activePlayerList.ForEach(p => p.SendConsoleCommand("chat.add", player.userID, format));
        ...
        void ReplySystem(BasePlayer player, string Message, string CustomPrefix = "", string CustomAvatar = "")
        {
        ...
        void API_ALERT(string Message, string CustomPrefix = "", string CustomAvatar = "") => BasePlayer.activePlayerList.ForEach(p => { ReplySystem(p, Message, CustomPrefix, CustomAvatar); });
        void API_ALERT_PLAYER(BasePlayer player, string Message, string CustomPrefix = "", string CustomAvatar = "") => ReplySystem(player, Message, CustomPrefix, CustomAvatar);
