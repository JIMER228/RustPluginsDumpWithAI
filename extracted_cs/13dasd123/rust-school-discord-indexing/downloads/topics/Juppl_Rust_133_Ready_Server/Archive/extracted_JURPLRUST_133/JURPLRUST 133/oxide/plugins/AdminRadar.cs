...
{
    [Info("Player Radar", "Austinv900 & Speedy2M", "2.0.1", ResourceId = 978)]
    [Description("Позволяет администраторам иметь радар для обнаружения читеров")]
    ...
    class AdminRadar : RustPlugin
    {
        ...
        class Radar : MonoBehaviour
        {
            ...
            void radar()
            {
                if (players && filter == "player" || filter == "all")
                {
                    string message = (ExtDetails) ? ExtMessages["player"] : Messages["player"];
                    foreach (var target in BasePlayer.activePlayerList)
                    {
                        if (target.transform.position == new Vector3(0, 0, 0) && !SpectateCheck(player, target)) return;
                        var distance = Vector3.Distance(target.transform.position, player.transform.position);
                        if (distance < setdistance && target != player)
                        {
                            ...
                            player.SendConsoleCommand("ddraw.box", RefreshTime, Color.green, target.transform.position + bodyheight, target.GetHeight());
                            player.SendConsoleCommand("ddraw.text", RefreshTime, Color.yellow, target.transform.position + textheight, msg);
                        }
                    }
                }

                if (sleepers && filter == "sleeper" || filter == "all")
                {
                    string message = (ExtDetails) ? ExtMessages["sleeper"] : Messages["sleeper"];
                    foreach (var sleeper in BasePlayer.sleepingPlayerList)
                    {
                        if (sleeper.transform.position == new Vector3(0, 0, 0)) return;
                ...
                if (toolcupboards && filter == "toolcupboard" || filter == "all")
                {
                    ...
                    {
                        if (Cupboard.transform.position == new Vector3(0, 0, 0)) return;
                        ...
                        if (distance < setdistance)
                        {
                            ...
                            player.SendConsoleCommand("ddraw.arrow", RefreshTime, Color.yellow, arrowGround, arrowSky, arrowsize);
                ...
                if (storages && filter == "storage" || filter == "all")
                {
                    string message = (ExtDetails) ? ExtMessages["thing"] : Messages["thing"];
                    ...
                    {
                        if (storage.transform.position == new Vector3(0, 0, 0)) return;
        ...
        void LoadMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>()
                {
                ["RadarOff"] = "РАДАР БЫЛ <color=red>ОТКЛЮЧЕН</color>",
                ["NoAccess"] = "Неизвестная команда: {0}",
                ["RadarOn"] = "РАДАР БЫЛ <color=green>ВКЛЮЧЕН</color> | ФИЛЬТР <color=green>{0}</color>",
                ["InvalidSyntax"] = "Неверный синтаксис команды: /{0} help",
                ["CommandDisabled"] = "Команда /{0} {1} отключена администратором сервера.",
                ["RadarGive"] = "Радар был {0} для {1}",
                ["Enabled"] = "<color=green>Включено</color>",
                ["Disabled"] = "<color=red>Отключено</color>",
                ["SettingUpdate"] = "Настройка {0} {1} изменена с {2} на {3}",
                ["RadarList"] = "------[ Активные радары ]------\n{0}",
                ["NoRadars"] = "Ни один игрок в настоящее время не использует радар"
            }, this);
        }
        #endregion
        ...
        void ccmdRadar(BasePlayer player, string command, string[] args)
        {
            ...
            if (args.Length == 1 && FilterList.Contains(filterValidation(args[0]))) { Puts(args[0] + filterValidation(args[0])); ToggleRadar(player, filterValidation(args[0])); return; }
        ...
        void CreatePlayerData(string id)
        {
            if (!LoadedData.ContainsKey(id))
            {
                ...
                SendMessage(rust.FindPlayer(id), "PlayerData Creates");
            }
        }
        #endregion
