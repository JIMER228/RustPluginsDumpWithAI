...
namespace Oxide.Plugins
{
    ...
    class Teleportation : RustPlugin
    {
        ...
        protected override void LoadDefaultConfig()
        {
            ...
            GetVariable(Config, "Создавать объект при сохранении местоположения в виде Sleeping Bag", out createSleepingBug, false);
            ...
            GetVariable(Config, "Разрешать сохранять местоположение если игрок является другом или соклановцем владельца фундамента ", out foundationOwnerFC, true);
            ...
            Config["Ограничение на количество сохранённых местоположений с привилегией"] =
                homelimitPerms =
            ...
            Config["Длительность задержки перед телепортацией с привилегией (в секундах)"] =
                teleportSecsPerms =
            ...
            Config["Длительность перезарядки телепорта с привилегией (в секундах)"] =
                tpkdPerms =
            ...
            Config["Длительность перезарядки телепорта домой с привилегией (в секундах)"] =
                tpkdhomePerms =
        ...
        void cmdChatHome(BasePlayer player, string command, string[] args)
        {
            ...
            if (homecupboard)
            {
                ...
                if (privilege != null && !player.CanBuild())
                {
                    Effect.server.Run(EffectPrefab, player, 0, Vector3.zero, Vector3.forward);
        ...
        void cmdChatTpr(BasePlayer player, string command, string[] args)
        {
            ...
            if (player.metabolism.temperature.value < -10)
            {
                Effect.server.Run(EffectPrefab, player, 0, Vector3.zero, Vector3.forward);
                SendReply(player, Messages["coldplayer"]);
                return;
        ...
        void SetHome(BasePlayer player, string name)
        {
            ...
            if (player.GetBuildingPrivilege() != null && !player.GetBuildingPrivilege())
            {
                Effect.server.Run(EffectPrefab, player, 0, Vector3.zero, Vector3.forward);
        ...
        void CreateSleepingBag(BasePlayer player, Vector3 pos, string name)
        {
            ...
            sleepingBag.skinID = 10121;
            sleepingBag.deployerUserID = player.userID;
            sleepingBag.niceName = $"ДОМ {name}";
