    ...
    class HitMarker : RustPlugin
    {
        ...
        void cmdHitMarker(BasePlayer player, string cmd, string[] args)
        {
            if (!hitmarkeron.Contains(player))
            {
                ...
                SendReply(player,
                    "<color=cyan>HitMarker</color>:" + " " + "<color=green>Вы включили показ урона.</color>");
            }
            else
            {
                hitmarkeron.Remove(player);
                SendReply(player,
                    "<color=cyan>HitMarker</color>:" + " " + "<color=red>Вы отключили показ урона.</color>");
            }
        }
		...
		string GetDamageText(string action)
		{
			switch (action)
			{
				case "wound": return "<color=red>УПАЛ!</color>";							  
