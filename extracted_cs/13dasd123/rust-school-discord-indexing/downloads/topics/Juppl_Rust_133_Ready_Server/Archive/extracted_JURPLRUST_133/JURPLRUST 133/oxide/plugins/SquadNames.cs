...
{
    [Info("SquadNames", "", "1.0.2")]
    [Description("Shows freinds and clanmates")]

    class SquadNames : RustPlugin
    {
        #region Vars
        [PluginReference]
        ...
        private static SquadNames inst;
        ...
        {
            string state = data.Self.Contains(player.userID) ? GetMsg("Hidden") : GetMsg("Visible");
            if (args.Length <= 0)
            {
                player.ChatMessage(GetMsg("Status").Replace("{0}", state) + GetMsg("Syntax"));
                return;
            }
            if (args.Length == 1 && args[0] == "list")
            {
                ...
                {
                    player.ChatMessage(GetMsg("Status").Replace("{0}", state) + GetMsg("HideListEmpty"));
                    return;
                }
                player.ChatMessage(GetMsg("Status").Replace("{0}", state) + GetMsg("HideList").Replace("{0}", string.Join("\n", data.Ignore[player.userID].Select(p => $"{p.Name} ({p.ID})").ToArray())));
                return;
            }
            if (args.Length == 1 && args[0] == "self")
            {
                ...
                state = data.Self.Contains(player.userID) ? GetMsg("Hidden") : GetMsg("Visible");
                player.ChatMessage(GetMsg("Now").Replace("{0}", state));
                return;
            }
            if (args.Length < 2)
            {
                player.ChatMessage(GetMsg("Syntax").Replace("{0}", state));
                return;
            }
            switch (args[0])
            {
                ...
                case "add":
                    ...
                    if (!founded.Any())
                    {
                        player.ChatMessage(GetMsg("PlayerNotFound").Replace("{0}", args[1]));
                        return;
                    }
                    if (founded.Count() > 1)
                    {
                        player.ChatMessage(GetMsg("MultiplyFound").Replace("{0}", string.Join("\n", founded.Select(p => $"{p.displayName}({p.UserIDString})").ToArray())));
                        return;
                    }
                    BasePlayer adding = founded.First();
                    if (adding == player)
                    {
                        player.ChatMessage(GetMsg("CantAddSelf"));
                        return;
                    }
                    if (!data.Ignore.ContainsKey(player.userID))
                    {
                        ...
                        player.ChatMessage(GetMsg("PlayerAdded").Replace("{0}", adding.displayName));
                        return;
                    }
                    if (data.Ignore[player.userID].Any(p => p.ID == adding.userID))
                    {
                        player.ChatMessage(GetMsg("AlreadyInList").Replace("{0}", adding.displayName));
                        return;
                    }
                ...
                case "remove":
                    if (!data.Ignore.ContainsKey(player.userID) || data.Ignore.ContainsKey(player.userID) && data.Ignore[player.userID].Count == 0)
                    {
                        player.ChatMessage(GetMsg("HideListEmpty"));
                        return;
                    }
                    ...
                    player.ChatMessage(GetMsg("PlayerRemoved").Replace("{0}", removing.Name));
                    return;
            }
        }
        #endregion
        ...
        private class FriendsDrawer : MonoBehaviour
        {
            ...
            private void DisplayFriend(BasePlayer player, BasePlayer friend, string color)
            {
                ...
                string text = inst.config.Format
                    ...
                    .Replace("{Health}", $"<color={health}>{ $"{Math.Floor(friend.health)} ХП"}</color>")
                    .Replace("{Distance}", "Дистанция: " + inst.GetMsg("Distance").Replace("{0}", distance.ToString()))
                    .Replace("{ActiveItem}", friend.GetActiveItem()?.info.displayName.english ?? inst.GetMsg("Nothing"));
