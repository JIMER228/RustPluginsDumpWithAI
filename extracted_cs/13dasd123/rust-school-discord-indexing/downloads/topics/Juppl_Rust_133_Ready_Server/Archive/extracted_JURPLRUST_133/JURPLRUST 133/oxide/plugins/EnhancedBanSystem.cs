...
{
    [Info("Enhanced Ban System", "Rust133", "5.5.4")]
    class EnhancedBanSystem : CovalencePlugin
    {
        ...
        private static string Server = "46.174.54.152:28017";
        ...
        private string MySQL_Host = "37.59.68.6";
        private int MySQL_Port = 3306;
        private string MySQL_DB = "s66_133";
        private string MySQL_User = "u66_fxVWDToMGD";
        private string MySQL_Pass = ".0ZDb@vtqnOOyUwVIye5favO";
        ...
        void InitializeLang()
        {
            ...
            {
                { "PlayerNotFound", "Соответствующий игрок не найден.\n" },
                { "PlayerTempBanned", "Вы временно забанены на этом сервере ({0}). {1}" },
                { "PlayerPermBanned", "Вы навсегда заблокированы на этом сервере ({0})." },
                {"LoadedBans","Загружено {0} банов" },
                {"BanExists","Этот запрет уже существует ({0})." },
                {"BanAdded","Успешно добавлено {0} в список запрещенных." },
                {"MultipleBans","Найдено несколько банов:\n\r" },
                {"BansRemoved","{0} соответствующие баны были сняты" },
                {"PlayerNotBanned","{0} - {1} не забанен.\n" },
                {"Loaded","Загружено" },
                {"NoPermission","У вас нет разрешения использовать эту команду." },
                {"KickSyntax","Синтаксис: kick < Name > < Причина >" },
                {"UnbanSyntax","Синтаксис: unban < Name | SteamID >" },
                {"BanSyntax","Синтаксис: ban < Name | SteamID > < Причина > < Время > " },
                {"BanlistSyntax","Синтаксис: banlist <BanSystem> <startid>" },
                {"BanSystems","Доступные системы бана:\n" },
                {"BanlistSyntax1","Неправильное использование /banlist" },
                {"OutOfRange","Индекс вне допустимого диапазона. Текущие баны зафиксированы {0}" },
                {"Banlist","Banlist - {0}-{1}/{2}\n" },
                {"PlayerPermBan", "{0} был навсегда заблокирован с сервера за {1}" },
                {"PlayerTempBan", "{0} был временно заблокирован на {1} за {2}" },
                {"IPPermBan", "IP-адрес заблокирован с сервера на {0}" },
                {"IPTempBan", "IP был временно заблокирован за {0} за {1}" },
                {"BanUpdated", "Успешно обновлено {0} в банлисте." }
            }, this);
        }
        ...
        {
            webrequest.EnqueueGet(FormatOnlineBansystem(WebAPI_IsBanned_Request, new Dictionary<string, string> { { "{id}", bandata.id.ToString() }, { "{steamid}", bandata.steamid }, { "{name}", bandata.name }, { "{ip}", bandata.ip }, { "{source}", "Auto Ban" }, { "{update}", update.ToString() }, { "{time}", LogTime().ToString() } }), (code, response) =>
            {
                if (response != null || code != 200)
                {
        ...
        bool isBanned_Check(string name, string steamid, string ip)
        {
            ...
            if (BanSystemHasFlag(banSystem, BanSystem.WebAPI))
            {
                count++;
                bool banned = false;
                webrequest.EnqueueGet(FormatOnlineBansystem(WebAPI_IsBanned_Request, new Dictionary<string, string> { { "{id}", null }, { "{steamid}", steamid }, { "{name}", name }, { "{ip}", ip }, { "{source}", "Auto Ban" }, { "{update}", false.ToString() }, { "{time}", LogTime().ToString() } }), (code, response) =>
                {
                    if (response != null || code != 200)
                    {
                        if (response == "true")
