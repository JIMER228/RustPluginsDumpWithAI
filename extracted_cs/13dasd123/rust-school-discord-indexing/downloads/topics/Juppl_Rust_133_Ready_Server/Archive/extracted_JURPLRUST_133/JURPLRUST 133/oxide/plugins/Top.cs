...
namespace Oxide.Plugins
{
    ...
    class Top : RustPlugin
    {
        ...
        void TurboRankCommand(BasePlayer player, string command)
        {
            ...
            player.ChatMessage($"<size=15>Статистика игрока <color=#0091ff>{player.displayName}</color></size>");
            foreach (var top in TopPlayer)
            {
                rust.SendChatMessage(player, $"<size=14><color=#ffffff>Убийств игроков: <color=#0091ff>{top.УбийствPVP}</color> | Смертей: <color=#0091ff>{top.Смертей}</color></color></size>", null, "0");
                rust.SendChatMessage(player, $"<size=14><color=#ffffff>Ракет выпущено: <color=#0091ff>{top.РакетВыпущено}</color> | Взрывчаток использовано: <color=#0091ff>{top.ВзрывчатокИспользовано}</color></color></size>", null, "0");
                rust.SendChatMessage(player, $"<size=14><color=#ffffff>Ресурсов собрано: <color=#0091ff>{top.РесурсовСобрано}</color> | Животных убито: <color=#0091ff>{top.УбийствЖивотных}</color></color></size>", null, "0");
                rust.SendChatMessage(player, $"<size=14><color=#ffffff>Пуль выпущено: <color=#0091ff>{top.ПульВыпущено}</color> | Предметов скрафчено: <color=#0091ff>{top.ПредметовСкрафчено}</color></color></size>", null, "0");
                rust.SendChatMessage(player, $"<size=14><color=#ffffff>NPC убито: <color=#0091ff>{top.NPCУбито}</color> | Вертолетов сбито: <color=#0091ff>{top.ВертолётовУничтожено}</color></color></size>", null, "0");
            }
            return;
        }
        [ChatCommand("top")]
        void TurboTopCommand(BasePlayer player, string command, string[] args)
        {
            if (args.Length == 1)
            {
                ...
                if (args[0] == "reset")
                {
                    if (!player.IsAdmin())
                    {
                        rust.SendChatMessage(player, $"<size=14><color=#0091ff>Ты кто такой? Давай досвиданье!</color></size>", null, "0");
                        return;
                    }
                    ...
                    rust.SendChatMessage(player, $"<size=14><color=#0091ff>Статистика игроков обнулена!</color></size>", null, "0");
                    return;
                }
                if (args[0] == "farm")
                {
                    bool prov = false;
                    player.ChatMessage("<size=14><color=#0091ff>[СТАТИСТИКА]</color> ТОП Фармеров</size>");
                    ...
                    foreach (var top in TopPlayer)
                    {
                        ...
                        if (n <= 5)
                        {
                            rust.SendChatMessage(player, $"<size=14><color=#0091ff>{n}.</color> <color=#0091ff>{top.Ник}</color> ({top.РесурсовСобрано})</size>", null, top.UID);
                    ...
                    if (!prov)
                    {
                        ...
                        foreach (var top in TopPlayer)
                        {
                            ...
                            if (top.UID == player.UserIDString)
                            {
                                rust.SendChatMessage(player, $"<size=14><color=#0091ff>{i}.</color> <color=#0091ff>{top.Ник}</color> ({top.РесурсовСобрано})</size>", null, player.UserIDString);
                            }
                        }
                    }
                    return;
                ...
                {
                    bool prov = false;
                    player.ChatMessage("<size=14><color=#0091ff>[СТАТИСТИКА]</color> ТОП Убийств PVP</size>");
                    ...
                    foreach (var top in TopPlayer)
                    {
                        ...
                        if (n <= 5)
                        {
                            rust.SendChatMessage(player, $"<size=14><color=#0091ff>{n}.</color> <color=#0091ff>{top.Ник}</color> ({top.УбийствPVP})</size>", null, top.UID);
                            if (top.UID == player.UserIDString)
                            {
                                prov = true;
                            }
                    ...
                    if (!prov)
                    {
                        ...
                        foreach (var top in TopPlayer)
                        {
                            ...
                            if (top.UID == player.UserIDString)
                            {
                                rust.SendChatMessage(player, $"<size=14><color=#0091ff>{i}.</color> <color=#0091ff>{top.Ник}</color> ({top.УбийствPVP})</size>", null, player.UserIDString);
                            }
                        }
                    }
                    return;
                ...
                {
                    bool prov = false;
                    player.ChatMessage("<size=14><color=#0091ff>[СТАТИСТИКА]</color> ТОП Рейдеров</size>");
                    ...
                    foreach (var top in TopPlayer)
                    {
                        ...
                        if (n <= 5)
                        {
                            rust.SendChatMessage(player, $"<size=14><color=#0091ff>{n}.</color> <color=#0091ff>{top.Ник}</color> ({top.РакетВыпущено + top.ВзрывчатокИспользовано})</size>", null, top.UID);
                            if (top.UID == player.UserIDString)
                            {
                                prov = true;
                            }
                    ...
                    if (!prov)
                    {
                        ...
                        foreach (var top in TopPlayer)
                        {
                            ...
                            if (top.UID == player.UserIDString)
                            {
                                rust.SendChatMessage(player, $"<size=14><color=#0091ff>{i}.</color> <color=#0091ff>{top.Ник}</color> ({top.РакетВыпущено + top.ВзрывчатокИспользовано})</size>", null, player.UserIDString);
                            }
                        }
                    }
                    return;
        ...
        CuiElementContainer CreatePanel(string number)
        {
            ...
            {
                Text = { Text = "<color=#0091ff>TOP 10 ИГРОКОВ</color>", FontSize = 30, Align = TextAnchor.MiddleCenter },
            ...
            elements.Add(new CuiButton
            {
                ...
                Text = { Text = "<color=#0091ff>X</color>", FontSize = 18, Align = TextAnchor.MiddleCenter }
            }, panel);
            ...
            elements.Add(new CuiLabel
            {
                Text = { Text = "<color=#0091ff>Игрок</color>", FontSize = 20, Align = TextAnchor.MiddleCenter },
                RectTransform = { AnchorMin = "0 0.81", AnchorMax = "0.29 0.8899999" },
            }, panel);
            elements.Add(new CuiButton
            {
                Text = { Text = "<color=#0091ff>Убийства</color>", FontSize = 15, Align = TextAnchor.MiddleCenter },
                Button = { Command = "top.show 1", Color = $"{cvet}" },
                RectTransform = { AnchorMin = "0.30 0.81", AnchorMax = "0.454 0.8899999" }
            }, panel);
            elements.Add(new CuiButton
            {
                Text = { Text = "<color=#0091ff>Смертей</color>", FontSize = 15, Align = TextAnchor.MiddleCenter },
                Button = { Command = "top.show 2", Color = $"{cvet}" },
                RectTransform = { AnchorMin = "0.455 0.81", AnchorMax = "0.578 0.8899999" }
            }, panel);
            elements.Add(new CuiButton
            {
                Text = { Text = "<color=#0091ff>Животные</color>", FontSize = 15, Align = TextAnchor.MiddleCenter },
                Button = { Command = "top.show 3", Color = $"{cvet}" },
                RectTransform = { AnchorMin = "0.579 0.81", AnchorMax = "0.6784 0.8899999" }
            }, panel);
            elements.Add(new CuiButton
            {
                Text = { Text = "<color=#0091ff>Взрывов</color>", FontSize = 15, Align = TextAnchor.MiddleCenter },
                Button = { Command = "top.show 4", Color = $"{cvet}" },
                RectTransform = { AnchorMin = "0.68 0.81", AnchorMax = "0.829 0.8899999" }
            }, panel);
            elements.Add(new CuiButton
            {
                Text = { Text = "<color=#0091ff>Ресурсы</color>", FontSize = 15, Align = TextAnchor.MiddleCenter },
                Button = { Command = "top.show 5", Color = $"{cvet}" },
                RectTransform = { AnchorMin = "0.83 0.81", AnchorMax = "1 0.8899999" }
            }, panel);
            ...
            foreach (var top in TopPlayer)
            {
                ...
                elements.Add(new CuiLabel
                {
                    ...
                    RectTransform = { AnchorMin = $"0.668 {0.72 - (n * 0.08)}", AnchorMax = $"0.85 {0.8 - (n * 0.08)}" },
                }, panel);
                elements.Add(new CuiLabel
                {
                    Text = { Text = Convert.ToString(top.РесурсовСобрано), FontSize = 15, Align = TextAnchor.MiddleCenter },
                    RectTransform = { AnchorMin = $"0.83 {0.72 - (n * 0.08)}", AnchorMax = $"0.99 {0.8 - (n * 0.08)}" },
                }, panel);
                n++;
            }
            return elements;
        ...
        void OnEntityDeath(BaseCombatEntity victim, HitInfo info)
        {
            ...
            if (initiator != null && !IsNPC(initiator))
            {
                ...
                if((bool)victim?.name?.Contains("animals/"))
                {
                    if ((bool)initiator?.name?.Contains("animals/"))
                        return;
