...
{
    /*ПЛАГИН БЫЛ ПОФИКШЕН С ПОМОЩЬЮ ПРОГРАММЫ СКАЧАНОЙ С https://discord.gg/dNGbxafuJn */ [Info("AutoCodeLock", "https://discord.gg/dNGbxafuJn", "3.0.6")]
    ...
    class AutoCodeLock : ChaosPlugin
    {
        ...
        private void OnPlayerConnected(BasePlayer player)
        {
            ...
            {
                StoredData.PlayerData playerData = storedData.FindPlayerData(player.userID.Get());
                if (playerData == null)
                    playerData = storedData.SetupPlayer(player.userID.Get(), Configuration);
        ...
        private void OnItemDeployed(Deployer deployer, BaseEntity entity, BaseEntity param3)
        {
            ...
            {
                StoredData.PlayerData playerData = storedData.FindPlayerData(owner.userID.Get()) ?? storedData.SetupPlayer(owner.userID.Get(), Configuration);
        ...
        private void OnEntityBuilt(Planner planner, GameObject obj)
        {
            ...
            if (!planner || !entity || entity.OwnerID == 0UL)
                return;
            if (!(entity is Door || entity is BoxStorage || entity is Locker || entity is BuildingPrivlidge))
                return;
            BasePlayer player = planner.GetOwnerPlayer();
            if (!player)
            {
                BasePlayer pl = BasePlayer.FindByID(entity.OwnerID);
                if (pl != null)
                {
                    player = pl;
                }
        ...
        private bool CanDeployLock(BasePlayer player, BaseEntity entity)
        {
            if (!(player.IsAdmin && Configuration.Other.AdminBypass))
            {
                if (player.userID.Get() != entity.OwnerID)
                {
        ...
        private void SetCodeLock(CodeLock codelock, BasePlayer owner, StoredData.PlayerData playerData)
        {
            ...
            codelock.whitelistPlayers.Add(owner.userID.Get());
        ...
        private void AutoDoorCommand(BasePlayer player, string command, string[] args)
        {
            ...
            if (Physics.Raycast(player.eyes.HeadRay(), out RaycastHit raycastHit, 3f, 1 << (int)Rust.Layer.Construction, QueryTriggerInteraction.Ignore))
            {
                ...
                if (door)
                {
                    StoredData.PlayerData playerData = storedData.FindPlayerData(player.userID.Get()) ?? storedData.SetupPlayer(player.userID.Get(), Configuration);

                    if (door.OwnerID != player.userID.Get())
                    {
                ...
                ImageContainer.Create(parent, Anchor.FullStretch, new Offset(5f, 5f, -5f, -40f))
                    ...
                    .WithChildren(layout =>
                    {
                        ...
                        if (canAutoLock)
                        {
                            ...
                            if (playerData.IsSet(Options.AutoLock))
                            {
                                CreateInputElement(layout, player, playerData, Options.PinCode, ++elementIndex);
                                CreateToggleElement(layout, player, playerData, Options.EnableGuestCode, ++elementIndex);

                                if (playerData.IsSet(Options.EnableGuestCode))
                                    CreateInputElement(layout, player, playerData, Options.GuestCode, ++elementIndex);
                            }
                        }
        ...
        private void CreateInputElement(BaseContainer layout, BasePlayer player, StoredData.PlayerData playerData, Options option, int index)
        {
            const float ELEMENT_HEIGHT = 30;
        ...
        {
            PlayerEntities playerEntities = PlayerEntities.GetOrCreate(player.userID.Get());
        ...
        private class PlayerEntities
        {
                ...
                foreach (KeyValuePair<ulong, PlayerEntities> kvp in m_PlayerEntities)
                {
                    ...
                    if (playerData != null)
                    {
                        foreach (DoorCloser doorCloser in kvp.Value.m_DoorClosers)
                        {
                            ...
                            bool isHatches = parentEntity.ShortPrefabName == "floor.ladder.hatch" || parentEntity.ShortPrefabName == "floor.triangle.ladder.hatch";
            ...
            public void AddEntity(DoorCloser doorCloser)
            {
                ...
                if (m_DoorClosers == null)
                    m_DoorClosers = Facepunch.Pool.GetList<DoorCloser>();
            ...
            public void RemoveEntity(DoorCloser doorCloser)
            {
                ...
                if (m_DoorClosers.Count == 0)
                    Facepunch.Pool.FreeList(ref m_DoorClosers);
            }
        ...
        protected override void PopulatePhrases()
        {
            ...
            {
                ["Prefix"] = "[<color=#08ffa4>АВТОКОДОВЫЙ ЗАМОК</color>] : ",

                ["Notification.CodelockSecured"] = "Кодовый замок закреплен и заблокирован для настройки пропишите команду <color=#08ffa4>/ad</color>\nКод: <color=#08ffa4>{0}</color>",
                ["Notification.CodelockSecured.Guest"] = "Кодовый замок закреплен и заблокирован для настройки пропишите команду <color=#08ffa4>/ad</color>\nКод: <color=#08ffa4>{0}</color>\nКод гостя: <color=#08ffa4>{1}</color>",
                ["Notification.NotLocked"] = "Кодовый замок не автоблокируется. Вы не являетесь владельцем объекта",
                ["Notification.NotLocked.RaidBlock"] = "Кодовый замок не автоблокируется. В настоящее время вы заблокированы рейдом",
                ["Notification.NotLocked.CombatBlock"] = "Кодовый замок не автоблокируется. В настоящее время вы заблокированы в бою",
                ["Notification.NotLocked.Plugin"] = "Кодовый замок не автоблокируется по причине: {0}",

                ["Notification.NoPermission"] = "У вас нет разрешения использовать эту команду!	",
                ...
                ["Notification.ApplyingCodeChanges"] = "Пин-код был применён ко всем кодовым замкам!",
                ["Notification.ApplyingGuestCodeChanges"] = "Пин-код был применён ко всем кодовым замкам!",
                ["Notification.ApplyingDoorCloserChanges"] = "Кд дверей успешно изменено!",
                ["Notification.ApplyingHatchCloserChanges"] = "Кд люков успешно изменено!",

                ["Label.Title"] = "KAIF CODELOCK",
                ["Label.AutoLock"] = "Автоматический пин-код",
                ["Label.DeployDoor"] = "Установить замок на дверях",
                ["Label.DeployBox"] = "Установить замок на ящиках",
                ["Label.DeployLocker"] = "Установить замок на локере",
                ["Label.DeployCupboard"] = "Установить замок на шкафах",
                ["Label.DeployDoorCloser"] = "Установить дверной доводчик",
                ["Label.EnableGuestCode"] = "становить код гостя",
                ["Label.PinCode"] = "Авто пин-код",
                ["Label.GuestCode"] = "Авто пин-код гостя",
                ["Label.DoorDelay"] = "Кд закрытия - дверей",
                ["Label.HatchDelay"] = "Кд закрытия - люков",
            };
        }
        #endregion
    }
