...
class ServerRewards : RustPlugin
{
    ...
    private void OnServerInitialized()
    {
        ...
        {
            Debug.LogWarning($"[ServerRewards] Steam definitions check failed: {ex.Message}");
        }
        RegisterImages();
        CreateKitsFunctions();
    ...
    private void RegisterImages()
    {
        ...
        {
            Debug.LogWarning($"[ServerRewards] Failed to register images via ImageLibrary: {ex.Message}");
        }
    }
    ...
    private void CheckSteamDefinitionsUpdated()
    {
        ...
        {
            Debug.LogWarning($"[ServerRewards] Steam definitions check exception: {ex.Message}");
            timer.In(5f, CheckSteamDefinitionsUpdated);
        }
    }
