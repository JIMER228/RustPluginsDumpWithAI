...
namespace Oxide.Plugins
{
    ...
    class Space : RustPlugin
    {
        ...
        void OnPlayerSleep(BasePlayer player)
        {
            ...
            if (SpaceClass.spaceClass != null && SpaceClass.spaceClass.eventZone != null)
                SpaceClass.spaceClass.eventZone.OnPlayerLeaveZone(player);
        }
        ...
        static class EventManager
        {
            ...
            {
                NotifyManager.SendMessageToAll("StartEvent", ins._config.prefix, eventConfig.displayName, MapHelper.PositionToString(SpaceClass.GetSpaceTransform().position));
            ...
            static IEnumerator PeriodicEventCorountine()
            {
                while (eventTime > 0 || (ins._config.mainConfig.dontStopIfPlayerInSpace && EventZone.playersInSpace.Any(x => x != null && x.IsConnected && !x.IsSleeping() && x.transform != null && SpaceClass.IsInSpace(x.transform.position, false))))
                {
                    ...
                    if (ins._config.mainConfig.descriptionNotifications.Contains(eventTime))
                        NotifyManager.SendMessageToAll("EventDescription", ins._config.prefix, eventConfig.displayName, MapHelper.PositionToString(SpaceClass.GetSpaceTransform().position));
        ...
        sealed class SpaceClass : FacepunchBehaviour
        {
            ...
            internal static SpaceClass spaceClass;
            internal EventZone eventZone;
