...
{
    [Info("Automatic Authorization", "k1lly0u/Arainrr/ADS", "1.3.6", ResourceId = 2063)]
    ...
    public class AutomaticAuthorization : RustPlugin
    {
        ...
        private void OnCodeChanged(BasePlayer player, CodeLock codeLock, string code, bool isGuest)
        {
            NextFrame(() =>
            {
        ...
        private void CheckEntitySpawned(CodeLock codeLock, bool justCreated = false)
        {
            ...
            if (justCreated && codeLock.hasCode && permission.UserHasPermission(ownerId.ToString(), PERMISSION_USE))
            {
