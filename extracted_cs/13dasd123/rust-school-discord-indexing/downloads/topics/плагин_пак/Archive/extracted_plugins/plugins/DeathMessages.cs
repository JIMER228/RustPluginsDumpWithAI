...
{
    [Info("DeathMessages", "OxideBro", "2.3.2")]
    class DeathMessages : RustPlugin
    {
        ...
        private Dictionary<ulong, BasePlayer> LastHeli = new Dictionary<ulong, BasePlayer>();
        ...
        private void OnEntityTakeDamage(BaseCombatEntity entity, HitInfo info)
        {
            ...
            if (entity is BaseHelicopter && info.InitiatorPlayer != null)
                LastHeli[entity.net.ID.Value] = info.InitiatorPlayer;
        }
        ...
        private void OnEntityDeath(BaseCombatEntity victim, HitInfo info)
        {
            try
            {
                ...
                {
                    if (LastHeli.ContainsKey(victim.net.ID.Value))
                        _attacker = new Attacker(LastHeli[victim.net.ID.Value]);
                }
