...
using VLB;
using AimUI = Oxide.Plugins.AimTrainUI.UIMethods;
using Anchor = Oxide.Plugins.AimTrainUI.Anchor;

namespace Oxide.Plugins
{
    /*Пофикшено вновь 16.07.2025*/ [Info("AimTrain", "https://rust-client.ru", "1.3.2")]
    public class AimTrain : RustPlugin
    {
        ...
        // Block admin/server teleport console commands when any involved player is in AimTrain
        private object OnServerCommand( ConsoleSystem.Arg arg )
        {
            string cmd = string.Empty;
            if( arg?.cmd != null )
            {
                // Use FullName only to avoid missing member errors
                cmd = arg.cmd.FullName ?? string.Empty;
            }
            // Fallback: parse first token from full string if available
            if( string.IsNullOrEmpty( cmd ) && !string.IsNullOrEmpty( arg?.FullString ) )
            {
            ...
            // Built-in Rust admin teleport commands
            if( cmd == "teleport" || cmd == "teleport2me" || cmd == "teleportpos" || cmd == "teleportany" )
            {
                // If any involved player (caller or targets) is in AimTrain, block
                var player = arg.Player();
                if( player != null && IsAimTraining( player.userID ) )
                {
                    arg.ReplyWith( Lang( "CantWhileAimTrain" ) );
                    return _falseObject;
                }

                // Check all args that could be player names/ids
                if( arg.Args != null )
                {
                    foreach( var a in arg.Args )
                    {
                        ...
                        var maybePlayer = BasePlayer.Find( a );
                        if( maybePlayer == null )
                        {
                            // Try by ID
                            if( ulong.TryParse( a, out var uid ) )
                            {
                                maybePlayer = BasePlayer.FindByID( uid );
                            }
                        }

                        if( maybePlayer != null && IsAimTraining( maybePlayer.userID ) )
                        {
                            if( player != null )
                            {
        ...
        private object OnPlayerCommand( BasePlayer player, string command, string[] args )
        {
            ...
            {
                if( cmdLower == blacklistedCommand || cmdLower.StartsWith( blacklistedCommand ) )
                {
                    // Block if the sender is in AimTrain
                    if( _playersCache.ContainsKey( player.userID ) )
                    {
                        SendReply( player, Lang( "CantWhileAimTrain" ) );
                        return _falseObject;
                    ...
                    // Also block if any target specified in args is in AimTrain (e.g., /tpr <name>, /home tp <name>)
                    if( args != null )
                    {
                        foreach( var a in args )
                        {
                            ...
                            BasePlayer maybePlayer = BasePlayer.Find( a );
                            if( maybePlayer == null && ulong.TryParse( a, out var uid ) )
                            {
                                maybePlayer = BasePlayer.FindByID( uid );
                            }

                            if( maybePlayer != null && _playersCache.ContainsKey( maybePlayer.userID ) )
                            {
                                SendReply( player, Lang( "CantWhileAimTrain" ) );
                                return _falseObject;
                            }
                    ...
                    // Not involving AimTrain participants
                    return null;
                }
            }
        ...
        // Finds the nearest valid surface (floors, constructions, world) below the given position
        // and returns the same XZ with adjusted Y. Falls back to terrain height.
        private Vector3 SnapToSurface( Vector3 position )
        {
            RaycastHit hitInfo;
            // Cast from a bit above to catch ceilings/floors directly under the point
            var origin = position + Vector3.up * 5f;
            if( Physics.Raycast( origin, Vector3.down, out hitInfo, 200f, LayerMask.GetMask( "Construction", "Clutter", "World" ) ) )
            {
                var y = Math.Max( hitInfo.point.y, TerrainMeta.HeightMap.GetHeight( position ) );
                return new Vector3( position.x, y, position.z );
            }

            // Fallback: use terrain height
            var terrainY = TerrainMeta.HeightMap.GetHeight( position );
            return new Vector3( position.x, terrainY, position.z );
        }
        ...
        private void CreateBot( string arenaName )
        {
            ...
            // Snap bot spawn to surface to avoid spawning below floors/terrain
            spawn = SnapToSurface( spawn );
        ...
        private void EnterAimTrain( BasePlayer player, string arenaName )
        {
            ...
            // Snap player target spawn to surface to avoid embedding in floors
            var targetPlayerSpawn = _arenasCache[arenaName].SpawnsPlayer[randomSpawn].ToVector3();
            MovePlayer( player, SnapToSurface( targetPlayerSpawn ) );
            if( _config.EnableUI )
            {
        ...
        private void LeaveAimTrain( BasePlayer player, Vector3 position = default( Vector3 ) )
        {
            ...
            {
                // Snap leave position to surface
                MovePlayer( player, SnapToSurface( _config.TpPosLeftAimTrain.ToVector3() ) );
            }
            else
            {
                MovePlayer( player, SnapToSurface( _playersCache[player.userID].Pos.ToVector3() ) );
            }
        ...
        {
            if( !_config.EnableAimTrain )
            {
                return;
            }

            // Require permission only if enabled in config (backwards compatible)
            if( _config.RequireJoinPermission && !permission.UserHasPermission( player.UserIDString, "aimtrain.join" ) )
            {
                return;
            }
        ...
        {
            if( _config.RequireAdminPermission && !permission.UserHasPermission( player.UserIDString, "aimtrain.admin" ) )
            {
                SendReply( player, Lang( "NoPerm" ) );
                return;
            }
        ...
        private void CmdAdminAimTrain( BasePlayer player, string command, string[] args )
        {
            ...
            if( !_config.EnableAimTrain )
            {
                SendReply( player, Lang( "EnableAimTrain", null, false ) );
                return;
            }
            ...
            switch( args[0] )
            {
                ...
                case "sbot":
                {
                    ...
                    if( args.Length < 2 )
                    {
                        ...
                        pos = SnapToSurface( pos );
                        _arenasCache[arenaEdit].SpawnsBot[_arenasCache[arenaEdit].SpawnsBot.Count + 1] = new Position( pos.x, pos.y, pos.z );
                ...
                case "splayer":
                {
                    ...
                    if( args.Length < 2 )
                    {
                        var pos = player.transform.position;
                        pos = SnapToSurface( pos );
                        _arenasCache[arenaEdit].SpawnsPlayer[_arenasCache[arenaEdit].SpawnsPlayer.Count + 1] = new Position( pos.x, pos.y, pos.z );
        ...
        public class Configuration
        {
            ...
            [JsonProperty( PropertyName = "Require aimtrain.join permission" )]
            public bool RequireJoinPermission = false;

            [JsonProperty( PropertyName = "Require aimtrain.admin permission" )]
            public bool RequireAdminPermission = true;
            ...
            [JsonProperty( PropertyName = "Blacklisted TP/chat commands during AimTrain" )]
        ...
        protected override void LoadDefaultMessages()
        {
            ...
            {
                ["ClearInv"] = "Please clear your inventory before you join AimTrain.перед входом очистите весь рюкзак и слоты одежды",
                ["JoinAT"] = "You've joined AimTrain! Use /at to leave. Чтобы выйти из аима нажмите кнопку leave",
        ...
        {
            AimUI.Border( min, max, ref _cachedContainer, 0.001f, "1 1 1 1", _mainContainer );
        }
        ...
        private CuiElementContainer DrawTimer( BasePlayer player )
        {
            ...
            var container = AimUI.Container( _statsContainer, "0 0 0 0", new Anchor( 0f, 0.35f ), new Anchor( 0.1f, 0.6f ), "Hud.Menu" );
            AimUI.Text( "", _statsContainer, ref container, TextAnchor.MiddleRight, "0 0 0 1", 13, _playersCache[player.userID].Headshots.ToString() + " ", new Anchor( 0f, 0.8f ), new Anchor( 0.99f, 0.9f ) );
            AimUI.Text( "", _statsContainer, ref container, TextAnchor.MiddleRight, "0 0 0 1", 13, _playersCache[player.userID].Bullets.ToString() + " ", new Anchor( 0f, 0.7f ), new Anchor( 0.99f, 0.8f ) );
            AimUI.Text( "", _statsContainer, ref container, TextAnchor.MiddleRight, "0 0 0 1", 13, _playersCache[player.userID].Hits.ToString() + " ", new Anchor( 0f, 0.6f ), new Anchor( 0.99f, 0.7f ) );
            AimUI.Text( "", _statsContainer, ref container, TextAnchor.MiddleRight, "0 0 0 1", 13, percentComplete.ToString() + "%" + " ", new Anchor( 0f, 0.5f ), new Anchor( 0.99f, 0.6f ) );
            AimUI.Text( "", _statsContainer, ref container, TextAnchor.MiddleLeft, "0 0 0 1", 13, "  " + AmmoStatus( player.userID ), new Anchor( 0f, 0.4f ), new Anchor( 0.99f, 0.5f ) );
            return container;
        }

        private void ConstructUi()
        {
            _cachedContainer = AimUI.Container( _mainContainer, "0 0 0 0", new Anchor( 0f, 0.35f ), new Anchor( 0.1f, 0.6f ), "Overlay" );
            AimUI.Text( "Main.Name", _mainContainer, ref _cachedContainer, TextAnchor.MiddleCenter, "0 0 0 1", 15, "AimTrain", new Anchor( 0f, 0.9f ), new Anchor( 1f, 1f ) );
            AimUI.Text( "", _mainContainer, ref _cachedContainer, TextAnchor.MiddleLeft, "0 0 0 1", 13, "  Headshots: ", new Anchor( 0f, 0.8f ), new Anchor( 0.99f, 0.9f ) );
            AimUI.Text( "", _mainContainer, ref _cachedContainer, TextAnchor.MiddleLeft, "0 0 0 1", 13, "  Bullets fired: ", new Anchor( 0f, 0.7f ), new Anchor( 0.99f, 0.8f ) );
            AimUI.Text( "", _mainContainer, ref _cachedContainer, TextAnchor.MiddleLeft, "0 0 0 1", 13, "  Hits: ", new Anchor( 0f, 0.6f ), new Anchor( 0.99f, 0.7f ) );
            AimUI.Text( "", _mainContainer, ref _cachedContainer, TextAnchor.MiddleLeft, "0 0 0 1", 13, "  Accuracy: ", new Anchor( 0f, 0.5f ), new Anchor( 0.99f, 0.6f ) );
            AimUI.Button( "Button.Reset", _mainContainer, ref _cachedContainer, new Anchor( 0f, 0.27f ), new Anchor( 0.99f, 0.4f ), $"global.ResetAT", "Reset", "0 0 0", 12, "0 0 0 0" );
            AimUI.Button( "Button.Ammo", _mainContainer, ref _cachedContainer, new Anchor( 0f, 0.14f ), new Anchor( 0.99f, 0.27f ), $"global.AmmoAT", "Toggle Ammo", "0 0 0", 12, "0 0 0 0" );
            AimUI.Button( "Button.Leave", _mainContainer, ref _cachedContainer, new Anchor( 0f, 0f ), new Anchor( 0.99f, 0.14f ), $"global.LeaveAT", "Leave", "0 0 0", 12, "0 0 0 0" );
            ...
            AimUI.Element( "", _mainContainer, ref _cachedContainer, new Anchor( 0.993f, 0f ), new Anchor( 0.99f, 1f ), "1 1 1 1" );
