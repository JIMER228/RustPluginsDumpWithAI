...
{
	[Info("ServerPanel", "Mevent", "1.0.19")]
	public class ServerPanel : RustPlugin
	{
		...
		public class MenuCategory
		{
			...
			{
				if (Pages.Count != 1 || Pages[0].Type != CategoryPage.PageType.Plugin) return;

				if (!string.IsNullOrEmpty(pluginName) && Pages[0].Plugin.Name != pluginName)
					return;

				var plugin = Pages[0].Plugin;
				if (plugin == null) return;
			...
			string cmdMove)
		{
			...
			AddButton(cmdRemove, "ServerPanel_Editor_Btn_Remove", "-106 -8", "-90 8");
			AddButton(cmdClone, "ServerPanel_Editor_Btn_Clone", "-81 -8", "-65 8");
			AddButton(cmdEdit, "ServerPanel_Editor_Btn_Edit", "-56 -8", "-40 8");
