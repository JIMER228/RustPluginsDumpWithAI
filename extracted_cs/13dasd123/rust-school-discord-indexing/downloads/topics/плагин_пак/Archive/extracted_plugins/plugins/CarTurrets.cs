...
namespace Oxide.Plugins
{
    ...
    internal class CarTurrets : CovalencePlugin
    {
        ...
        private void Unload()
        {
            if (ImmortalProtection != null)
            {
                UnityEngine.Object.Destroy(ImmortalProtection);
                ImmortalProtection = null;
            }
        }
        ...
        private void OnServerInitialized()
        {
            ...
            foreach (var car in BaseNetworkable.serverEntities.OfType<ModularCar>().ToArray())
            {
                ...
                foreach (var module in car.AttachedModuleEntities)
                {
                    ...
                    try { RefreshCarTurret(turret); } catch {}
                }
        ...
        private object CanMoveItem(Item item, PlayerInventory playerInventory, ItemContainerId targetContainerId, int targetSlot, int amount)
        {
            ...
            var targetContainer = targetContainerId.Value != 0
                ...
                : (playerInventory.loot.containers != null && playerInventory.loot.containers.Count > 1 ? playerInventory.loot.containers[1] : null);
        ...
        private void OnSwitchToggled(ElectricSwitch electricSwitch, BasePlayer player)
        {
            ...
            if (electricSwitch.IsOn())
            {
                ...
                {
                    try { turret.InitiateStartup(); } catch {}
                }
            }
            else
            {
                try { turret.InitiateShutdown(); } catch {}
            }
        }
        ...
        private object OnEntityUse(BaseEntity entity, BasePlayer player)
        {
            if (player == null || entity == null)
                return null;

            // If a seat was directly used, try mounting it first to prevent engine UI from opening.
            var directSeat = entity as BaseMountable;
            if (directSeat != null)
            {
                if (!directSeat.IsMounted())
                {
                    try
                    {
                        directSeat.MountPlayer(player);
                        if (player.isMounted)
                            return False;
                    }
                    catch {}
            ...
            if (car == null)
            {
                ...
                car = module != null ? module.Vehicle as ModularCar : null;
            }

            if (car == null)
                return null;

            // Try to mount the closest available seat to the player's aim to prioritize entering over opening engine UI
            try
            {
                // First, raycast to detect a seat or car surface under the crosshair (e.g., when clicking the door area)
                var headRay = player.eyes.HeadRay();
                RaycastHit hitInfo;
                if (Physics.Raycast(headRay, out hitInfo, 5f, Physics.DefaultRaycastLayers, QueryTriggerInteraction.Collide))
                {
                    var hitEntity = hitInfo.GetEntity();
                    var hitSeat = hitEntity as BaseMountable ?? hitInfo.collider?.GetComponentInParent<BaseMountable>();
                    if (hitSeat != null && !hitSeat.IsMounted())
                    {
                        hitSeat.MountPlayer(player);
                        if (player.isMounted)
                            return False;
                    }

                    // If we clicked the car/module (e.g., door), choose the nearest seat to the hit point
                    if (hitEntity is ModularCar || hitEntity is BaseVehicleModule)
                    {
                        BaseMountable nearestToDoor = null;
                        float nearestDist = float.MaxValue;
                        var seats = car.GetComponentsInChildren<BaseMountable>();
                        foreach (var seat in seats)
                        {
                            if (seat == null || !seat.isActiveAndEnabled || seat.IsMounted())
                                continue;

                            var dist = Vector3.Distance(seat.CenterPoint(), hitInfo.point);
                            if (dist < nearestDist)
                            {
                                nearestDist = dist;
                                nearestToDoor = seat;
                        ...
                        if (nearestToDoor != null)
                        {
                            nearestToDoor.MountPlayer(player);
                            if (player.isMounted)
                                return False;
                        }
                    }
                ...
                headRay = player.eyes.HeadRay();
                BaseMountable bestSeat = null;
                float bestDistance = float.MaxValue;

                var allSeats = car.GetComponentsInChildren<BaseMountable>();
                foreach (var seat in allSeats)
                {
                    if (seat == null || !seat.isActiveAndEnabled || seat.IsMounted())
                        continue;

                    var distance = Vector3.Cross(headRay.direction, seat.CenterPoint() - headRay.origin).magnitude;
        ...
        private AutoTurret DeployAutoTurret(ModularCar car, BaseVehicleModule vehicleModule, Vector3 position, float conditionFraction = 1, ulong ownerId = 0)
        {
            ...
            {
                try { autoTurret.Kill(); } catch {}
                return null;
            }

            SetupCarTurret(autoTurret);
            try { AttachTurretSwitch(autoTurret); } catch {}

            try { Effect.server.Run(PrefabEffectDeployAutoTurret, autoTurret.transform.position); } catch {}
        ...
        private void SetupTurretSwitch(ElectricSwitch electricSwitch)
        {
            ...
            if (electricSwitch != null && electricSwitch.HasParent())
            {
                ...
                if (transform.localPosition != TurretSwitchPosition)
                {
                    ...
                    try { electricSwitch.InvalidateNetworkCache(); } catch {}
                    try { electricSwitch.SendNetworkUpdate_Position(); } catch {}
                }
            }
        }
