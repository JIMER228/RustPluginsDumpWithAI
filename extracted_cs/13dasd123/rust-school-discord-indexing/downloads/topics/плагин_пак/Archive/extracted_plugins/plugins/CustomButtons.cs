...
{
    [Info("CustomButtons", "Forum: https://topplugin.ru Ds: alone_sempai Vk: https://vk.com/rustnastroika", "2.0.72")]
    ...
    {   
        ...
        CuiElementContainer buttons_nonToggle;
        CuiElementContainer buttons_withToggle;

        void ShowButtons(BasePlayer player)
        {   
            ...
            var withoutPermission = Facepunch.Pool.Get<List<string>>();

            foreach (string name in _cuiData.Keys)
            {   
                // destroy all before creating new ones
                CuiHelper.DestroyUi(player, name);

                if (!permission.UserHasPermission(player.UserIDString, $"custombuttons.{name}") && !permission.UserHasPermission(player.UserIDString, $"custombuttons.admin"))
                    withoutPermission.Add(name);
            }

            if (_playerData[player.userID].hidden)
            {
                CuiHelper.AddUi(player, buttons_nonToggle);
            }
            else
            {   
                CuiHelper.AddUi(player, buttons_nonToggle);
                CuiHelper.AddUi(player, buttons_withToggle);
            }
            
            foreach (string name in withoutPermission)
                CuiHelper.DestroyUi(player, name);

            Facepunch.Pool.FreeUnmanaged(ref withoutPermission);
        }

        void DestroyWithToggle(BasePlayer player)
        {   
            foreach (var button in _cuiData)
            {   
                if (button.Value.onCmdHide == "true" && button.Value.onCmdHide != null) 
                {   
                    CuiHelper.DestroyUi(player, button.Key);
                }
            }
        }
        ...
        void CreateComponent(CuiElementContainer container, CuiData button, string name)
        {
            // create button
            if (button.panelType == "button")
            {
                container.Add(new CuiButton
                {   
                    Button = { Command = (button.panelChatCmd == null ? "" : $"custombuttons_runbtncmd {name}"), Color = button.panelColor, FadeIn = 0.0f },
                    RectTransform = { AnchorMin = config.anchSet.anchorMin, AnchorMax = config.anchSet.anchorMax, OffsetMin = button.OffsetMin, OffsetMax = button.OffsetMax },
                    Text = { Text = "", FontSize = 12, Align = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf" }
                }, button.uiType, name);

                // add image if exists
                if (button.imageUrl != null && button.imageUrl.StartsWith("http"))
                {
                    var comp = new CuiRawImageComponent { Url = button.imageUrl, Sprite = "assets/content/textures/generic/fulltransparent.tga", FadeIn = 0.0f };

                    if (ImageLibrary != null && (bool)ImageLibrary.Call("HasImage", button.imageUrl))
                        comp.Png = (string)ImageLibrary.Call("GetImage", button.imageUrl);
                    ...
                    container.Add(new CuiElement
                    {
                        ...
                        {
                            comp,
                            new CuiRectTransformComponent { AnchorMin = button.imgAnchorMin, AnchorMax = button.imgAnchorMax }
                        }
                    });
                }
                ...
                if (button.text != null && button.text != "")
                {
                    container.Add(new CuiElement
                    {
                        ...
                        Components =
                        {
                            ...
                            {
                                Text = button.text,
                                FontSize = config.fontSet.fontSize,
                                Font = config.fontSet.fontStyle,
                                Align = TextAnchor.MiddleCenter,
                                Color = config.fontSet.fontColor,
                                FadeIn = 0f,
                            },

                            new CuiOutlineComponent
                            {
                                Color = config.fontSet.fontOutColor,
                                Distance = config.fontSet.fontOutThic
                            },
            ...
            if (button.panelType == "panel")
            {
                container.Add(new CuiPanel
                {
                    Image = { Color = button.panelColor },
                    RectTransform = { AnchorMin = config.anchSet.anchorMin, AnchorMax = config.anchSet.anchorMax, OffsetMin = button.OffsetMin, OffsetMax = button.OffsetMax }
                }, button.uiType, name);
        ...
        void BuildButtons() 
        {   
            buttons_nonToggle = new CuiElementContainer();
            buttons_withToggle = new CuiElementContainer();

            foreach (var button in _cuiData)
            {   
                if (button.Value.onCmdHide == "true" && button.Value.onCmdHide != null) // no idea why it's saved as string
                {   
                    CreateComponent(buttons_withToggle, button.Value, button.Key);
                }

                if (button.Value.onCmdHide == "false" || button.Value.onCmdHide == null) 
                {   
                    CreateComponent(buttons_nonToggle, button.Value, button.Key);
                }
            }
        }
        ...
        void OnLootEntityEnd(BasePlayer player, BaseEntity entity) => ShowButtons(player); //CreatePartialCui(player, config.addSet.whenLooting);
       
        void OnEntityMounted(ComputerStation entity, BasePlayer player) => CloseWhen(player, config.addSet.whenComputer);
        void OnEntityDismounted(ComputerStation entity, BasePlayer player) => ShowButtons(player); //CreatePartialCui(player, config.addSet.whenComputer);
        ...
        private void custombuttonstoggle(BasePlayer player)
        {
            ...
            { 
                _playerData[player.userID].hidden = true; 
                DestroyWithToggle(player);
            }
        ...
        private void custombuttons_runbtncmd(ConsoleSystem.Arg arg) 
        {
            ...
            string replacedValues = chatcmd.Replace("{player.name}", $"{player.displayName}").Replace("{player.steam}", $"{player.userID}");
            
            if (config.hideOnCommand)
            {   
                player.SendConsoleCommand($"chat.say \"/{config.commSet.togglePanels}\"");
            }

            player.SendConsoleCommand($"chat.say \"/{replacedValues}\" ");
        }
        ...
        class Configuration
        {   
            [JsonProperty(PropertyName = "Run toggle command when clicking on button")]
            public bool hideOnCommand { get; set; }
