...
namespace Oxide.Plugins
{
    ...
    internal class BetterTC : RustPlugin
    {
        ...
        private const string apiUrl = "https://cdn.rustspain.com/plugins/bettertc/bettertc.json";
        ...
        private void OnEntityBuilt(Planner plan, GameObject go){
            ...
            if (tc.ShortPrefabName != meta.ShortName) TCSkinReplace(tc, player, selectedSkin);
        }
		...
		private IEnumerator WallpaperProgress(BasePlayer player, BuildingPrivlidge cup, string category){
            // Check if building is moving (barge, train base, etc.)
            if (config.disableWallpaperOnMovingBuildings && IsBuildingMoving(cup))
            {
                CreateGameTip(cup, Lang("MovingBuildingWallpaper", player.UserIDString), player, fxerror, 10, "danger");
                BuildingCupboard[cup].work = false;
            ...
            for (var index = 0; index < set.Count; index++){
                ...
                if (currentId == wallpaperid && currentHealth != -1)
                    continue;
        ...
        private void WallpaperBlock(BuildingPrivlidge cup, BuildingBlock block, BasePlayer player, string category){
            // Check if building is moving and wallpaper placement is disabled
            if (config.disableWallpaperOnMovingBuildings && IsBuildingMoving(cup))
            {
                BuildingCupboard[cup].work = false;
                CreateGameTip(cup, Lang("MovingBuildingWallpaper", player.UserIDString), player, fxerror, 10, "danger");
            ...
            if (wallpaperID == 1){
                ...
                if (category == "Wall"){
                    block.SetWallpaper(wallpaperID, 0, 0f);
                    if (config.bothsides && BuildingCupboard[cup].wpbothsides)
        ...
        private void TCSkinReplace(BuildingPrivlidge tc, BasePlayer player, TCSkin skin){
            ...
            BaseEntity tcskin = GameManager.server.CreateEntity(meta.PrefabPath, tc.transform.position, tc.transform.rotation, true);
        ...
        private bool IsBuildingMoving(BuildingPrivlidge cup)
        {
            try
            {
                var building = cup.GetBuilding();
                ...
                // Check if any building block has a parent entity that's moving
                foreach (var block in building.buildingBlocks)
                {
                    if (block == null) continue;
                    
                    // Check if block is attached to a moving entity (barge, train, etc.)
                    var parent = block.GetParentEntity();
                    if (parent != null && (parent is BaseVehicle || parent.GetComponent<BaseVehicle>() != null))
                    {
                        return true;
                    }
                    
                    // Check if block has a rigidbody and is moving
		...
		private bool IsSkinAllowed(BasePlayer player, int skinId){
            if (skinId <= 0 || config.allowAllSkins || whitelistedSkins.Contains((ulong)skinId) || player.blueprints.steamInventory.HasItem(skinId)) 
                return true;
        ...
        private List<ItemInfo> GetBuildingItems(BasePlayer player){
            var items = config.itemsList.Where(item => item.enabled == true).ToList();
            
            // If player doesn't have permission for default upgrades or skin upgrades, filter out skins they don't own
            if (!HasPermission(player.UserIDString, permupgrade) && !HasPermission(player.UserIDString, permupskin))
            {
                items = items.Where(item => IsSkinAllowed(player, item.skinid)).ToList();
            }
        ...
        private (int itemId, List<ulong> skinIds) GetWallpaperItems(BasePlayer player, string category){
            ...
            // Get skins from ItemSkinDirectory
            List<ItemSkinDirectory.Skin> skins = new List<ItemSkinDirectory.Skin>();
            var itemDef = ItemManager.FindItemDefinition(itemId);
            if (itemDef?.skins != null)
            {
                skins = itemDef.skins.ToList();
            }
        ...
        private void commands(ConsoleSystem.Arg arg){
            ...
            switch (arg.Args[0]){
                ...
                case "PAGE":
                {
                    ...
                    if (config.disableWallpaperOnMovingBuildings && IsBuildingMoving(cup))
                    {
                        CreateGameTip(null, Lang("MovingBuildingWallpaper", player.UserIDString), player, fxerror, 10, "danger");
                        return;
                    }
        ...
        private void ShowMenu(BasePlayer player, BuildingPrivlidge cup, int page = 0){
            ...
            foreach (var list_entry in BuildingItems){
                ...
                if (!hasSkin){
                    UI.Button(ref container, upgrade_2, "0 0 0 0.2", Lang("NoDLC", player.UserIDString), 10, "0.6 0", "0.993 0.15", "SENDCMD NODLC");
        ...
        private void ShowMenuWallpaper(BasePlayer player, BuildingPrivlidge cup, string id, string grade, string skinid, int page = 0, string category = "Wall"){
            ...
            if (config.disableWallpaperOnMovingBuildings && IsBuildingMoving(cup))
            {
                player.ChatMessage(Lang("MovingBuildingWallpaper", player.UserIDString));
                return;
            }
            ...
            if (config.bothsides && itemId == 553967074){
                ...
                UI.Label(ref container, color_1, Lang("BOTHSIDES", player.UserIDString), 10, "0.65 0.07", "0.95 0.12", "1.0 1.00 1.00 0.9", TextAnchor.MiddleCenter, true);
                UI.Button(ref container, color_1, side ? "0.50 0.50 1.00 0.10" : "0.10 0.10 0.10 0.50", "ON", 12, "0.65 0.03", "0.80 0.07", $"SENDCMD WALLPAPERSIDES {id} {grade} {skinid} {page} true {category}");
                UI.Button(ref container, color_1, side ? "0.10 0.10 0.10 0.50" : "0.50 0.50 1.00 0.10", "OFF", 12, "0.80 0.03", "0.95 0.07", $"SENDCMD WALLPAPERSIDES {id} {grade} {skinid} {page} false {category}");
            } else {
                UI.Button(ref container, color_1, up ? "0.90 0.20 0.20 0.50" : "0.80 1.00 0.50 0.10", 
                    Lang(up ? "STOP" : "WALLPAPERGRADE", player.UserIDString), 12, 
                    "0.20 0.03", "0.45 0.10", 
        ...
        public static class RPC_PickupWallpaperStart_Patch
        {
            ...
            public static bool Prefix(BuildingBlock __instance, BaseEntity.RPCMessage msg){
                try
                {
                    ...
                    if (!msg.player.CanInteract()) return false;
                    ...
                    if (!self.permission.UserHasPermission(msg.player.UserIDString, permwallpapernocost)){
                        Item obj = ItemManager.Create(ItemManager.FindItemDefinition("cloth"), config.wallresource);
        ...
        private class ConfigData {
            ...
            [JsonProperty("Disable wallpaper placement on moving buildings (barges, train bases, etc.)")]
            public bool disableWallpaperOnMovingBuildings = false;
        ...
        protected override void LoadDefaultMessages(){
            lang.RegisterMessages(new Dictionary<string, string> {
                ...
                ["MovingBuildingWallpaper"] = "Wallpaper placement is disabled on moving buildings.",
                ["NoDLC"] = "NO DLC",
                ["NoDLCPurchased"] = "You don't own this DLC.",
