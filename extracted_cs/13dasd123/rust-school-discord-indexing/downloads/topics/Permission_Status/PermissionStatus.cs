...
namespace Oxide.Plugins
{
    ...
    public class PermissionStatus : RustPlugin
    {
        ...
        bool IsComplyingConditions(BasePlayer player, Status status)
        {
            ...
            bool hasTimedPerm = Dependencies?.Condition(player, status.Key, status.StatusSettings.IsGroup) ?? false;
            bool hasUsualPerm = status.StatusSettings.IsGroup ? 
                permission.UserHasGroup(player.UserIDString, status.Key) : 
                permission.UserHasPermission(player.UserIDString, status.Key);
        ...
        void UpdateStatus(Status status, BasePlayer player)
        {
            ...
            bool isComplying = IsComplyingConditions(player, status);
            int duration = isComplying ? int.MaxValue : 0;
            ...
            if (duration > 0)
            {
                ...
                if (string.IsNullOrEmpty(statusText) || 
                    (status.StatusSettings.HideUnlimited && 
                     statusText == GetMessage(LangKeys.Unlimited, player.UserIDString)))
                {
                    duration = 0;
                }
            }
            ...
            if (duration > 0)
            {
                string title = GetMessage(status.Key, player.UserIDString);
                if (!string.IsNullOrEmpty(title))
                {
                    SetTitle(player.userID, status.Key, title);
        ...
        void OnServerInitialized()
        {
            ...
            if (TimedPermissions != null)
            {
                ...
                {
                    PrintError("TimedPermissions references failed to initialize!");
                    return;
                }
            }
            else if (IQPermissions != null)
            {
                Dependencies = new __IQPermissions(IQPermissions);
            }
            else
            {
                PrintError("No permission plugin found!");
                return;
            }
