...
namespace Oxide.Plugins
{
    ...
    public class SimplePVE : RustPlugin
    {
        ...
        /// 1.1.2  - Fixed MLRS damages conflicts, Fixed Now TC authed player can access loot boxes, Fixed NPC raiders cant damage building, Zombie Horde Compatible
        /// 1.1.3  - Fixed RaidableBases TC bug, Code Cleanup, Optimization on Hooks
        /// 1.1.4  - Patch for latest oxide update, Bug fixes, Design changes
        /// 1.1.5  - Fix a bug, it was my mistake obs need more testing
        ...
        /// 1.1.9  - Fixed Modular cars Collision damage, Fixed bradley fire damage, Fixed patrol heli fire & explosion damage, Added campfire damages, Fixed Tesla Damages
        /// 1.1.10 - Added SupplyDrop Loot Rule, Fixed TeslaCoil damages players, Added TeslaCoil without owner can damage players(For RaidableBases), Added Racked Weapon Support, Added Damage block from ReactiveTarget(ComputerStation Turrets), Added Anti-Ladder/Twig placement on other player base(Teams/Clan Exception), Added new rules in ZoneRules config, Added PVP delay time when exiting a pvp zone, Added Legay Wooden Shelter Support 
        /// --------
        ...
        private class Configuration
        {
            ...
            [JsonProperty(PropertyName = "PVP delay time in seconds(If enabled from ZoneRules)")]
        ...
        {           
            ...
            if (entity is BasePlayer && hit.Initiator is BasePlayer)
            {
                ...
                else if (attacker.userID.IsSteamId() && victim.userID.IsSteamId() && !IsAlliedWithPlayer(attacker.userID, victim.userID))
                {
                    ...
                    else if (config.zoneRules.enablePVPDelay && CheckPVPDelay(victim, attacker))
                    {
                        return true;
                    }
            ...
            {
                if (!config.entityRules.trapD && victim3.userID.IsSteamId() || ((!config.entityRules.trapD || config.entityRules.trapD) && victim3.IsNpc) || hit.Initiator.OwnerID == 0)
                {
        ...
        private object CanLootEntity(BasePlayer player, StorageContainer container)
        {
            ...
            if (entity.OwnerID == player.userID || (config.lootRules.teamsAccessLoot && IsAlliedWithPlayer(entity.OwnerID, player.userID)) || IsAuthtoTC(entity, player) || (entity is SupplyDrop && config.lootRules.anyoneOpenSupplyDrop))
            {
                return null;
            }
        ...
        private bool CheckRackBox(BasePlayer player, BaseEntity entity)
        {
            ...
            if (entity.OwnerID == 0) return true;
            if (config.lootRules.adminCanLootAll && permission.UserHasPermission(player.UserIDString, PvEAdminLootPerms)) return true;
            if (entity.OwnerID == player.userID || (config.lootRules.teamsAccessLoot && IsAlliedWithPlayer(entity.OwnerID, player.userID)) || IsAuthtoTC(entity, player)) return true;
        ...
        private void SchTimeCMD(ConsoleSystem.Arg args)
        {
            ...
            switch (command)
            {
                case "openday":
                    ...
                    SchedulesTime.Add($"{args.GetString(1)}/{args.GetString(2)}/{args.GetString(3)}", new PVPTime { StartHour = 12, StartMinute = 30, EndHour = 23, EndMinute = 30, StartDate = $"{args.GetString(1)}/{args.GetString(2)}/{args.GetString(3)}", EndDate = $"{args.GetString(1)}/{args.GetString(2)}/{args.GetString(3)}", IsWeekDay = false });
                    SaveSchTime();
                    CreateSch(player, args.GetString(1), args.GetString(2), args.GetString(3));
                    break;
                case "openweekday":
                    ...
                    SchedulesTime.Add($"{args.GetString(1)}", new PVPTime { StartHour = 12, StartMinute = 30, EndHour = 23, EndMinute = 30, StartDate = $"{args.GetString(1)}", EndDate = $"{args.GetString(1)}", IsWeekDay = true });
                    SaveSchTime();
                    CreateSchForWeekDay(player, args.GetString(1));
                    break;
                case "selectdate":
                ...
                case "changetodays": //new schedules day
                    ...
                    break;
                case "changesmday":
                    if (!SchedulesTime.ContainsKey($"{args.GetString(1)}")) return;
                    time = args.GetInt(2);
                    if (time > 59) time = 59;
                    if (time < 0) time = 0;
                    SchedulesTime[$"{args.GetString(1)}"].StartMinute = time;
                    SaveSchTime();
                    CreateSchForWeekDay(player, args.GetString(1));
                    break;
                case "changeshday":
                    ...
                    if (time > 24) time = 24;
                    if (time < 0) time = 0;
                    SchedulesTime[$"{args.GetString(1)}"].StartHour = time;
                    SaveSchTime();
                    CreateSchForWeekDay(player, args.GetString(1));
                    break;
                case "changeemday":
                    ...
                    if (time > 59) time = 59;
                    if (time < 0) time = 0;
                    SchedulesTime[$"{args.GetString(1)}"].EndMinute = time;
                    SaveSchTime();
                    CreateSchForWeekDay(player, args.GetString(1));
                    break;
                case "changeehday":
                    ...
                    if (time > 24) time = 24;
                    if (time < 0) time = 0;
                    SchedulesTime[$"{args.GetString(1)}"].EndHour = time;
                    SaveSchTime();
                    CreateSchForWeekDay(player, args.GetString(1));
                    break;
        ...
        private void CreateSchForWeekDay(BasePlayer player, string day)
        {
            ...
            container.Add(new CuiElement
            {
                ...
                Parent = "StartTime",
                Components = {
                    new CuiTextComponent { Text = $"{SchedulesTime[$"{day}"].StartMinute}M", Color = "0.2039216 0.2078432 0.2117647 1", Font = "robotocondensed-regular.ttf", FontSize = 14, Align = TextAnchor.MiddleCenter },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "47.633 -13", OffsetMax = "131.567 13" }
                }
            });
            container.Add(new CuiElement
            {
                ...
                Parent = "StartTime",
                Components = {
                    new CuiInputFieldComponent { Command = $"sch changesmday {day}", Color = "0.2039216 0.2078432 0.2117647 1", Font = "robotocondensed-regular.ttf", FontSize = 14, Align = TextAnchor.MiddleCenter, CharsLimit = 0, IsPassword = false },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "47.633 -13", OffsetMax = "131.567 13" }
                }
            });
            container.Add(new CuiElement
            {
                ...
                Parent = "StartTime",
                Components = {
                    new CuiTextComponent { Text = $"{SchedulesTime[$"{day}"].StartHour}H",Color = "0.2039216 0.2078432 0.2117647 1", Font = "robotocondensed-regular.ttf", FontSize = 14, Align = TextAnchor.MiddleCenter },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-57.735 -13", OffsetMax = "22.809 13" }
                }
            });
            container.Add(new CuiElement
            {
                ...
                Parent = "StartTime",
                Components = {
                    new CuiInputFieldComponent { Command = $"sch changeshday {day}", Color = "0.2039216 0.2078432 0.2117647 1", Font = "robotocondensed-regular.ttf", FontSize = 14, Align = TextAnchor.MiddleCenter, CharsLimit = 0, IsPassword = false },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-57.735 -13", OffsetMax = "22.809 13" }
                }
            });
            ...
            container.Add(new CuiElement
            {
                ...
                Parent = "StartDate",
                Components = {
                    new CuiTextComponent { Text = $"{SchedulesTime[$"{day}"].StartDate}", Font = "robotocondensed-regular.ttf", FontSize = 14, Align = TextAnchor.MiddleCenter, Color = "1 1 1 1" },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "0.001 -18.051", OffsetMax = "113.601 18.052" }
                }
            });
            ...
            container.Add(new CuiElement
            {
                ...
                Parent = "EndDate",
                Components = {
                    new CuiTextComponent { Text = $"{SchedulesTime[$"{day}"].EndDate}", Font = "robotocondensed-regular.ttf", FontSize = 14, Align = TextAnchor.MiddleCenter, Color = "1 1 1 1" },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "48.241 -18.051", OffsetMax = "157.453 18.051" }
                }
            });
            ...
            container.Add(new CuiElement
            {
                ...
                Parent = "EndTime",
                Components = {
                    new CuiTextComponent { Text = $"{SchedulesTime[$"{day}"].EndMinute}M",Color = "0.2039216 0.2078432 0.2117647 1", Font = "robotocondensed-regular.ttf", FontSize = 14, Align = TextAnchor.MiddleCenter},
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "47.633 -13", OffsetMax = "131.567 13" }
                }
            });
            container.Add(new CuiElement
            {
                ...
                Parent = "EndTime",
                Components = {
                    new CuiInputFieldComponent { Command = $"sch changeemday {day}", Color = "0.2039216 0.2078432 0.2117647 1", Font = "robotocondensed-regular.ttf", FontSize = 14, Align = TextAnchor.MiddleCenter, CharsLimit = 0, IsPassword = false },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "47.633 -13", OffsetMax = "131.567 13" }
                }
            });
            container.Add(new CuiElement
            {
                ...
                Parent = "EndTime",
                Components = {
                    new CuiTextComponent { Text = $"{SchedulesTime[$"{day}"].EndHour}H", Color = "0.2039216 0.2078432 0.2117647 1", Font = "robotocondensed-regular.ttf", FontSize = 14, Align = TextAnchor.MiddleCenter },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-57.735 -13", OffsetMax = "22.809 13" }
                }
            });
            container.Add(new CuiElement
            {
                ...
                Parent = "EndTime",
                Components = {
                    new CuiInputFieldComponent { Command = $"sch changeehday {day}", Color = "0.2039216 0.2078432 0.2117647 1", Font = "robotocondensed-regular.ttf", FontSize = 14, Align = TextAnchor.MiddleCenter, CharsLimit = 0, IsPassword = false },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-57.735 -13", OffsetMax = "22.809 13" }
                }
            });
        ...
        private void OnExitZone(string ZoneID, BasePlayer player) // Called when a player leaves a zone
        {
            ...
            delay.DelayTimer = timer.Once(config.pvpdelaytime, () =>
            {
                RemovePVPDelay(player.userID);
        ...
        {
            PVPDelay Delay;
            if (pvp_delays.TryGetValue(userID, out Delay))
            {
                Delay.DelayTimer?.Destroy();
        ...
        public void API_SetProperty(string PropertyName)
        {
            SetProperty(PropertyName);
        } 
