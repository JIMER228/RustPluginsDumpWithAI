...
{
    [Info("PersonalNPC", "walkinrey", "1.6.41")]
    ...
    public class PersonalNPC : RustPlugin
    {
        ...
        private void OnPlayerDisconnected(BasePlayer player, string reason) => _pendingSpawnBots.Remove(player.userID);
        ...
        private object CanMoveItem(Item item, PlayerInventory playerLoot, uint targetContainer, ItemContainerId param3, int targetSlot, int amount, ItemMoveModifier param6)
        {
        ...
        private object OnDispenserGather(ResourceDispenser dispenser, BasePlayer entity, Item item)
        {
        ...
        public class PlayerBotController : FacepunchBehaviour
        {
            ...
            public class StagedResourceEntity : MonoBehaviour
            {
            ...
            {
                var stagedEntity = GetComponent<StagedResourceEntity>();
                if (stagedEntity != null)
                {
                    var stage = stagedEntity.stage; 
            ...
            public virtual bool ShotTest()
            {
                ...
                if (baseProjectile.primaryMagazine.contents <= 0)
                {
                    ...
                    {
                        int amount = bot.inventory.Take(null, baseProjectile.primaryMagazine.ammoType.itemid, baseProjectile.primaryMagazine.capacity);
                        if (amount > 0)
                        {
                            baseProjectile.primaryMagazine.contents += amount;
                ...
                else
                {
                    if (baseProjectile.primaryMagazine.capacity == 1)
                    {
                        ...
                        else
                        {
                            ...
                            int ammoNeeded = baseProjectile.primaryMagazine.capacity - baseProjectile.primaryMagazine.contents;
                            int ammoTaken = bot.inventory.Take(null, baseProjectile.primaryMagazine.ammoType.itemid, ammoNeeded);

                            if (ammoTaken > 0)
                            {
                                baseProjectile.primaryMagazine.contents += ammoTaken;
                                baseProjectile.ServerReload(); 
                            }
                            else
                            {
            ...
            private Item EquipItem(bool needWeapon = false, bool needAxe = false, bool needPickaxe = false, bool needMelee = false, bool needThrowable = false)
            {
                if (needWeapon)
                {
                    ...
                    foreach (var item in bot.inventory.containerMain.itemList)
                    {
                        if (item.info.category == ItemCategory.Weapon && item.info.shortname != "bow.hunting" && item.info.shortname != "speargun" && item.info.shortname != "crossbow")
                            weapons.Add(item);
                    }
                            ...
                            if (held != null)
                            {
                                if (held is BaseProjectile projectile)
                                {
                                    if (projectile.primaryMagazine.contents <= 0)
                                    {
                                        if (!botSetup.infiniteAmmo)
                                        {
                                            var ammo = bot.inventory.FindItemByItemID(projectile.primaryMagazine.ammoType.itemid);
                                            if (ammo != null && ammo.amount > 0)
                                            {
                                                int amountToLoad = Math.Min(projectile.primaryMagazine.capacity, ammo.amount);
                                                projectile.primaryMagazine.contents += amountToLoad;
                                                ammo.UseItem(amountToLoad);
            ...
            private bool MoveWeaponToBelt(Item weapon)
            {
                var itemSlot = bot.inventory.containerBelt.GetSlot(0);
                ...
                {
                    return weapon.MoveToContainer(bot.inventory.containerBelt, 0, true, true, null, true);
                }
            }
            ...
            public void RenderHierarchy()
            {
                ...
                for (int i = 0; i < _config.gui.accessButtons.Count; i++)
                {
                    ...
                    container.Add(new CuiElement
                    {
                        Name = $"CP_Hierarchy_Element{i}_Text",
                        Parent = $"CP_Hierarchy_Element{i}",
                        Components =
                        {
                            new CuiTextComponent
                            {
            ...
            public static string WorldToGrid(Vector3 position)
            {
                char gridLetter = (char)('A' + Mathf.FloorToInt(position.z / 150)); 
                int gridNumber = Mathf.FloorToInt(position.x / 150) + 1;
            ...
            public void RenderIcon()
            {
                ...
                var gridPos = WorldToGrid(bot.transform.position);
                if (IsGUIHidden) return;
