    ...
    class Space : RustPlugin
    {
        ...
        static class EventManager
        {
            ...
            static IEnumerator PeriodicEventCorountine()
            {
                while (eventTime > 0 || (ins._config.mainConfig.dontStopIfPlayerInSpace && EventZone.playersInSpace.Any(x => x != null && x.IsConnected && !x.IsSleeping() && x.transform != null && SpaceClass.IsInSpace(x.transform.position, false))))
                {
                    ...
                    EventMarker.UpdateMapMarkerTime(eventTime);
                    eventTime--;
        ...
        sealed class SpaceClass : FacepunchBehaviour
        {
            ...
            internal static void CreateSpaceThirdStage()
            {
                ...
                EventMarker.CreateMapMarker(spaceClass.transform.position);
        ...
        sealed class EventMarker
        {
            internal static EventMarker eventMarker;
            ...
            internal static void CreateMapMarker(Vector3 position)
            {
                ...
                eventMarker.Init(position);
            }

            void Init(Vector3 position)
            {
                CreateInvisibleVendingMachine(position);
                SpawnMapMarker(position);
                nameMarker = SpawnVendingMarker(position);
            }

            void CreateInvisibleVendingMachine(Vector3 position)
            {
                invisibleVendingMachine = BuildManager.CreateRegularEntity("assets/prefabs/deployable/vendingmachine/npcvendingmachines/shopkeeper_vm_invis.prefab", position, Quaternion.identity) as InvisibleVendingMachine;
                invisibleVendingMachine.shopName = EventManager.eventConfig.displayName;
                invisibleVendingMachine.SetFlag(BaseEntity.Flags.Busy, true);

                invisibleVendingMachine.Invoke(() =>
                {
            ...
            void UpdateVendingMachine()
            {
                ...
                invisibleVendingMachine.inventory.ClearItemsContainer();
                invisibleVendingMachine.sellOrders.sellOrders.Clear();
            ...
            {
                radiusMarker = GameManager.server.CreateEntity("assets/prefabs/tools/map/genericradiusmarker.prefab", new Vector3(position.x, 10, position.z)) as MapMarkerGenericRadius;
            ...
            internal static void UpdateMapMarkerTime(int eventTime)
            {
                ...
                if (eventMarker.nameMarker != null)
                {
                    eventMarker.nameMarker.markerShopName = $"{EventManager.eventConfig.displayName}";
                    eventMarker.nameMarker.SendNetworkUpdate();
                }
                if (eventMarker.invisibleVendingMachine != null)
                {
                    eventMarker.invisibleVendingMachine.shopName = $"{NotifyManager.GetTimeMessage(null, eventTime)}";
                    eventMarker.invisibleVendingMachine.SendNetworkUpdate();
                }
                if (eventMarker.radiusMarker != null)
                {
                    eventMarker.radiusMarker.SendUpdate();
                    eventMarker.radiusMarker.SendNetworkUpdate();
                }
            }
            ...
            {
                if (eventMarker == null) return;
                if (eventMarker.radiusMarker.IsExists()) eventMarker.radiusMarker.Kill();
                if (eventMarker.nameMarker.IsExists()) eventMarker.nameMarker.Kill();
                if (eventMarker.invisibleVendingMachine.IsExists()) eventMarker.invisibleVendingMachine.Kill();
                eventMarker = null;
            }
        }
        ...
        private class PluginConfig
        {
            ...
            public static PluginConfig DefaultConfig()
            {
                ...
                {
                    version = new VersionNumber(1, 3, 2),
                    prefix = "[Space]",
