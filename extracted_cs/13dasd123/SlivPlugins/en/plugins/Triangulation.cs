...
{
    [Info("Triangulation", "KpucTaJl", "1.0.5")]
    internal class Triangulation : RustPlugin
    {
            ...
            if (Controller.Receiver != null)
            {
                ...
                if (entity is BaseAnimalNPC)
                {
                    ...
                    if (Controller.Receiver.Animals.Contains(animal))
                    {
                        ...
                        if (weaponPrefab != null && _weaponsBlockedDamage.Contains(weaponPrefab.ShortPrefabName)) return true;
        ...
        internal readonly List<Location> Positions = new List<Location>();
        ...
        {
            if (!plugins.Exists("AnimalSpawn"))
            {
                PrintError("AnimalSpawn plugin doesn`t exist! (https://drive.google.com/drive/folders/1-18L-mG7yiGxR-PQYvd11VvXC2RQ4ZCu?usp=sharing)");
        ...
        private void Finish()
        {
            ...
            Active = false;
            SendBalance();
            Interface.Oxide.CallHook("OnTriangulationEnd");
            Puts("Triangulation has ended");
            StartTimer();
        }
        ...
        internal class ControllerTriangulation : FacepunchBehaviour
        {
            ...
            private void SpawnReceiver(ReceiverConfig config)
            {
                ...
                _ins.AlertToAllPlayers("ReceiverSpawn", _config.Chat.Prefix, CurrentStageEvent, PhoneController.PositionToGridCoord(Receiver.transform.position), GetTimeFormat((int)_config.ScanWaitTime));
            }
            ...
            private void SpawnDrill()
            {
                ...
                _ins.AlertToAllPlayers("DrillSpawn", _config.Chat.Prefix, PhoneController.PositionToGridCoord(Drill.transform.position), GetTimeFormat(_config.Drill.LifeTime));
            }
        ...
        internal class ControllerReceiver : FacepunchBehaviour
        {
            ...
            private InvisibleVendingMachine VendingMachineText { get; set; } = null;
            private InvisibleVendingMachine VendingMachineTime { get; set; } = null;
            private InvisibleVendingMachine VendingMachineOwner { get; set; } = null;
            ...
            private void OnDestroy()
            {
                ...
                if (VendingMachineText.IsExists()) VendingMachineText.Kill();
                if (VendingMachineTime.IsExists()) VendingMachineTime.Kill();
                if (VendingMachineOwner.IsExists()) VendingMachineOwner.Kill();
            ...
            private void SpawnMapMarker(MarkerConfig config)
            {
                ...
                VendingMachineOwner = SpawnVendingMachine("No Owner");
                VendingMachineTime = SpawnVendingMachine("0%");
                if (!string.IsNullOrEmpty(_config.Marker.Text)) VendingMachineText = SpawnVendingMachine(_config.Marker.Text);

                UpdateMapMarkers();
            }

            private InvisibleVendingMachine SpawnVendingMachine(string shopName)
            {
                InvisibleVendingMachine vending = GameManager.server.CreateEntity("assets/prefabs/deployable/vendingmachine/npcvendingmachines/shopkeeper_vm_invis.prefab", transform.position) as InvisibleVendingMachine;
                vending.shopName = shopName;
                ...
                for (int i = 0; i < 5; i++) vending.AddSellOrder(696029452, 1, 696029452, 1, 0);
                return vending;
            }
            ...
            private void UpdateVendingMarker(int progress)
            {
                ...
                VendingMachineTime.shopName = $"{progress} %";
                VendingMachineTime.UpdateMapMarker();

                VendingMachineOwner.shopName = Owner != null ? Owner.displayName : "No Owner";
                VendingMachineOwner.UpdateMapMarker();
            }
        ...
        internal class ControllerDrill : FacepunchBehaviour
        {
            ...
            private void SpawnMapMarker(MarkerConfig config)
            {
                ...
                if (!_config.SwitchDrill || !_config.LootCrate) VendingMachineOwner = SpawnVendingMachine(Owner != null ? Owner.displayName : "No Owner");
                VendingMachineTime = SpawnVendingMachine($"{GetTimeFormat(Seconds)} | {GetHealth()} %");
                if (!string.IsNullOrEmpty(_config.Marker.Text)) VendingMachineText = SpawnVendingMachine(_config.Marker.Text);
            ...
            {
                if (!_config.Marker.Enabled) return;
                VendingMachineTime.shopName = $"{GetTimeFormat(Seconds)} | {GetHealth()} %";
                VendingMachineTime.UpdateMapMarker();
                if (VendingMachineOwner != null)
                {
                    VendingMachineOwner.shopName = Owner != null ? Owner.displayName : "No Owner";
                    VendingMachineOwner.UpdateMapMarker();
                }
        ...
        private void SendBalance()
        {
            ...
            Interface.Oxide.CallHook("OnTriangulationWinner", winnerId);
        ...
        private readonly HashSet<string> _weaponsBlockedDamage = new HashSet<string>
        {
