...
{
    [Info("ArcticBaseEvent", "KpucTaJl", "1.1.3")]
    internal class ArcticBaseEvent : RustPlugin
    {
        ...
        private void Start(BasePlayer player)
        {
            ...
            {
                Puts("ArcticBaseEvent has begun");
                ...
                Interface.Oxide.CallHook("OnArcticBaseEventStart", Controller.transform.position, _config.Radius);
                AlertToAllPlayers("Start", _config.Prefix, PhoneController.PositionToGridCoord(Controller.transform.position));
            });
        }
        ...
        internal class ControllerArcticBaseEvent : FacepunchBehaviour
        {
            ...
            private void OnDestroy()
            {
                ...
                if (Pilot.IsExists()) Pilot.Kill();
                if (Scientist.IsExists()) Scientist.Kill();
                ...
                foreach (SnowmobileData data in Snowmobiles)
                {
                    ...
                    if (data.Passenger.IsExists()) data.Passenger.Kill();
                    if (data.Driver.IsExists()) data.Driver.Kill();
            ...
            internal void TryToOpenDoor(BasePlayer target, CodeLock codeLock, string code)
            {
                if (codeLock == CodeLockPilot)
                {
                    ...
                    _ins.AlertToAllPlayers("OpenDoorPilot", _config.Prefix, target.displayName);
                ...
                else if (codeLock == CodeLockScientist)
                {
                    ...
                    if (_ins.ActivePveMode && _ins.PveMode.Call("CanActionEvent", _ins.Name, target) != null) return;
                    _ins.AlertToAllPlayers("OpenDoorScientist", _config.Prefix, target.displayName);
            ...
            private void SpawnHackCrate()
            {
                ...
                if (_config.HackCrate.TypeLootTable == 1 || _config.HackCrate.TypeLootTable == 4 || _config.HackCrate.TypeLootTable == 5)
                {
                    _ins.NextTick(() =>
                    {
                        ...
                        if (_config.HackCrate.TypeLootTable == 4 || _config.HackCrate.TypeLootTable == 5) _ins.AddToContainerPrefab(HackCrate.inventory, _config.HackCrate.PrefabLootTable);
                        if (_config.HackCrate.TypeLootTable == 1 || _config.HackCrate.TypeLootTable == 5) _ins.AddToContainerItem(HackCrate.inventory, _config.HackCrate.OwnLootTable);
                    });
                }
            }
            ...
            internal void TakeDamageScientist(float damage)
            {
                ...
                ScientistHealth = 0f;
                if (Scientist.IsExists()) Scientist.Kill();
                if (Pilot.isMounted) MinicopterGoToCardReader2();
                _ins.AlertToAllPlayers("ScientistKill", _config.Prefix);
            }
            ...
            private IEnumerator StartResearch()
            {
                ...
                foreach (BasePlayer player in Players) _ins.AlertToPlayer(player, _ins.GetMessage("ScientistSuccess", player.UserIDString, _config.Prefix));
            ...
            private void DestroyMinicopter()
            {
                ...
                MiniCopter.GetFuelSystem().GetFuelContainer().inventory.ClearItemsContainer();
                MiniCopter.Kill();
            }

            private IEnumerator SpawnSnowmobiles()
            {
                foreach (BasePlayer player in Players) _ins.AlertToPlayer(player, _ins.GetMessage("Snowmobiles", player.UserIDString, _config.Prefix));
            ...
            private IEnumerator DestroySnowmobile(SnowmobileData data)
            {
                ...
                {
                    data.Snowmobile.GetFuelSystem().GetFuelContainer().inventory.ClearItemsContainer();
            ...
            internal void FinishPathBasePlayer(BasePlayer basePlayer)
            {
                if (basePlayer == Pilot)
                {
                    ...
                    else if (basePlayer.transform.position == PilotLastPointToCardReader2)
                    {
                        ...
                        foreach (BasePlayer player in Players) _ins.AlertToPlayer(player, _ins.GetMessage("PilotOpenLoot", player.UserIDString, _config.Prefix));
                        PilotGoToMinicopter2();
                    }
                    else if (basePlayer.transform.position == PilotLastPointToMinicopter2)
                    {
                        foreach (BasePlayer player in Players) _ins.AlertToPlayer(player, _ins.GetMessage("PilotRepair", player.UserIDString, _config.Prefix));
        ...
        private void SendBalance()
        {
            ...
            Interface.Oxide.CallHook("OnArcticBaseEventWinner", winnerId);
        ...
        private readonly HashSet<ImageUrl> _urls = new HashSet<ImageUrl>
        {
            ...
            new ImageUrl { Name = "Pilot_KpucTaJl", Url = "Images/Pilot_KpucTaJl.png" },
            new ImageUrl { Name = "Scientist_KpucTaJl", Url = "Images/Scientist_KpucTaJl.png" }
        };
