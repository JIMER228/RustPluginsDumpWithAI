...
{
    [Info("DefendableHomes", "KpucTaJl", "1.1.3")]
    internal class DefendableHomes : RustPlugin
    {
        ...
        private object OnEntityTakeDamage(ScientistNPC entity, HitInfo info)
        {
            ...
            if (attacker.IsPlayer() && (weaponPrefab == null || weaponPrefab.ShortPrefabName == "grenade.molotov.deployed" || weaponPrefab.ShortPrefabName == "rocket_fire") && info.damageTypes.GetMajorityDamageType() == Rust.DamageType.Heat) return true;
        ...
        {
            if (entity == null || !entity.ShortPrefabName.Contains("foundation")) return;
        ...
        private void OnExplosiveThrown(BasePlayer player, BaseEntity entity, ThrownWeapon weapon)
        {
            ...
            if (cupboard == null || !cupboard.authorizedPlayers.Any(x => IsTeam(player.userID, x.userid)))
            {
                ReturnFlare(player, entity, flareConfig);
            ...
            HashSet<BuildingPrivlidge> cupboards; HashSet<BuildingBlock> foundations;
            FindAllBuildings(cupboard, out cupboards, out foundations);
            ...
            Vector3 center3 = new Vector3(center2.x, TerrainMeta.HeightMap.GetHeight(new Vector3(center2.x, 0f, center2.y)), center2.y);
            ...
            float radius = Vector2.Distance(center2, new Vector2(farthest.transform.position.x, farthest.transform.position.z));
            float maxRadius = radius * 2f > 50f ? radius * 2f : 50f;
            ...
            if (Controllers.Count == 1) Subscribes();
        ...
        private void OnEntitySpawned(HackableLockedCrate crate)
        {
            ...
            crate.hackSeconds = HackableLockedCrate.requiredHackSeconds - controller.DifficultyLevel.HackCrate.UnlockTime;
            if (controller.DifficultyLevel.HackCrate.TypeLootTable == 1 || controller.DifficultyLevel.HackCrate.TypeLootTable == 4 || controller.DifficultyLevel.HackCrate.TypeLootTable == 5)
            {
                NextTick(() =>
                {
                    crate.inventory.ClearItemsContainer();
                    if (controller.DifficultyLevel.HackCrate.TypeLootTable == 1 || controller.DifficultyLevel.HackCrate.TypeLootTable == 5)
                    {
                        if (controller.DifficultyLevel.HackCrate.OwnLootTable.UseCount) crate.inventory.capacity = 36 - controller.DifficultyLevel.HackCrate.OwnLootTable.Max;
                        else crate.inventory.capacity = 36 - controller.DifficultyLevel.HackCrate.OwnLootTable.Items.Count;
                    }
                    else crate.inventory.capacity = 36;
                    if (controller.DifficultyLevel.HackCrate.TypeLootTable == 4 || controller.DifficultyLevel.HackCrate.TypeLootTable == 5) AddToContainerPrefab(crate.inventory, controller.DifficultyLevel.HackCrate.PrefabLootTable);
                    if (controller.DifficultyLevel.HackCrate.TypeLootTable == 1 || controller.DifficultyLevel.HackCrate.TypeLootTable == 5) AddToContainerItem(crate.inventory, controller.DifficultyLevel.HackCrate.OwnLootTable);
                });
            }
        }
        ...
        internal void Finish(ControllerHomeRaid controller)
        {
            ...
            if (Controllers.Count == 0) Unsubscribes();
        ...
        internal class ControllerHomeRaid : FacepunchBehaviour
        {
                ...
                while (Seconds > 0)
                {
                    ...
                    if (timerNpc == Seconds)
                    {
                        ...
                        foreach (PresetConfig preset in wave.Presets)
                        {
                            ...
                            JObject objectConfig = preset.ShortName == "Sledge" ? GetSledgeObjectConfig() : preset.ShortName == "Juggernaut" ? GetJuggernautObjectConfig() : preset.ShortName == "Rocketman" ? GetRocketmanObjectConfig() : preset.ShortName == "Bomber" ? GetBomberObjectConfig() : null;
                            for (int i = 0; i < count; i++)
                            {
                                ...
                                else if (preset.ShortName == "Rocketman") SetRocketmanWeapons(objectConfig);
                                ScientistNPC npc = (ScientistNPC)_ins.NpcSpawn.Call("SpawnNpc", pos, objectConfig);
                                if (npc != null)
                                {
                                    Scientists.Add(npc);
                                    if (preset.ShortName == "Sledge" || preset.ShortName == "Rocketman" || preset.ShortName == "Bomber") _ins.NpcSpawn.Call("AddTargetRaid", npc, Foundations);
            ...
            {
                float chance = UnityEngine.Random.Range(0f, 100f);
                List<SledgeBelt> list = Pool.GetList<SledgeBelt>();
            ...
            {
                float chance = UnityEngine.Random.Range(0f, 100f);
                List<JuggernautBelt> list = Pool.GetList<JuggernautBelt>();
            ...
            {
                float chance = UnityEngine.Random.Range(0f, 100f);
                List<RocketmanBelt> list = Pool.GetList<RocketmanBelt>();
            ...
            private IEnumerator ProcessCh47()
            {
                ...
                List<float> list = Pool.GetList<float>();
        ...
        private void OnBomberExplosion(ScientistNPC npc, BaseEntity target)
        {
            ...
            if (target.IsExists() && !(target is BasePlayer) && !(target is BuildingBlock))
            {
        ...
        private void FindAllBuildings(BuildingPrivlidge cupboard, out HashSet<BuildingPrivlidge> cupboards, out HashSet<BuildingBlock> foundations)
        {
            cupboards = new HashSet<BuildingPrivlidge> { cupboard };
            foundations = cupboard.GetBuilding().buildingBlocks.Where(x => x.ShortPrefabName.Contains("foundation"));
            HashSet<ulong> ids = cupboard.authorizedPlayers.Select(x => x.userid);
            while (true)
            {
                ...
                foreach (BuildingBlock foundation in foundations)
                {
                    ...
                    for (int i = 0; i < list.Count; i++)
                    {
                        ...
                        if (!block.ShortPrefabName.Contains("foundation")) continue;
                        BuildingPrivlidge buildingPrivlidge = block.GetBuildingPrivilege();
                        if (buildingPrivlidge == null) continue;
                        if (cupboards.Contains(buildingPrivlidge) || anotherCupboards.Contains(buildingPrivlidge)) continue;
                        if (buildingPrivlidge.authorizedPlayers.Any(x => ids.Contains(x.userid) || ids.Any(y => IsTeam(y, x.userid)))) anotherCupboards.Add(buildingPrivlidge);
                    }
                    Pool.FreeList(ref list);
                }
                ...
                {
                    cupboards.Add(anotherCupboard);
                    foundations.UnionWith(anotherCupboard.GetBuilding().buildingBlocks.Where(x => x.ShortPrefabName.Contains("foundation")));
                    foreach (ulong id in anotherCupboard.authorizedPlayers.Select(x => x.userid)) if (!ids.Contains(id)) ids.Add(id);
                }
            }
        }
        ...
        private readonly HashSet<ImageUrl> _urls = new HashSet<ImageUrl>
        {
            ...
            new ImageUrl { Name = "Foundation_KpucTaJl", Url = "Images/Foundation_KpucTaJl.png" },
        ...
        private static string GetTimeFormat(int time)
        {
            ...
            else
            {
                ...
                if (min == 0 && sec == 0) return $"{hour} h.";
                else if (sec == 0) return $"{hour} h. {min} min.";
                else return $"{hour} h. {min} min. {sec} sec.";
            }
        }
        ...
        private void ChatCheckFoundations(BasePlayer player)
        {
            ...
            if (!player.IsAdmin && !cupboard.authorizedPlayers.Any(x => IsTeam(player.userID, x.userid))) return;
