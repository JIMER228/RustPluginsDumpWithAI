...
namespace Oxide.Plugins
{
    ...
    class Sputnik : RustPlugin
    {
        ...
        [PluginReference] Plugin NpcSpawn, PveMode, GUIAnnouncements, DiscordMessages, ZoneManager, RaidableBases, Economics, ServerRewards, IQEconomic, Friends, Clans;
        ...
        void OnServerInitialized()
        {
            ...
            if (_config.mainConfig.isAutoEvent)
                autoEventCorountine = ServerMgr.Instance.StartCoroutine(AutoEventCorountine());
        }
        ...
        void Unload()
        {
            ...
            if (findSpawnPointsCorountine != null)
                ServerMgr.Instance.StopCoroutine(findSpawnPointsCorountine);

            if (autoEventCorountine != null)
                ServerMgr.Instance.StopCoroutine(autoEventCorountine);
        }

        object OnCardSwipe(CardReader cardReader, Keycard keycard, BasePlayer player)
        {
            if (eventClass == null || player == null || cardReader == null || keycard == null)
                return null;

            SputnikClass sputnikClass = eventClass.GetSputnikClassFromCardReaderUid(cardReader.net.ID.Value);
        ...
        {
            if (!_config.customCardConfig.enableSpawnInDefaultCrates || container == null || container.inventory == null)
                return;

            float chance = 0;
            if (!_config.customCardConfig.spawnSetting.TryGetValue(container.PrefabName, out chance) || chance == 0)
                return;
        ...
        {
            if (eventClass == null || player == null || crate == null)
                return null;

            SputnikClass sputnikClass = eventClass.GetSputnikClassFromCrateUid(crate.net.ID.Value);
            if (sputnikClass != null)
            {
                if (!sputnikClass.IsSputnikAgressive())
                {
                    sputnikClass.MakeSputnikAgressive();
                    return true;
                }

                if (!sputnikClass.CanOpenCrate(crate.net.ID.Value))
                {
        ...
        {
            if (eventClass == null || player == null || container == null || container.net == null)
                return null;

            SputnikClass sputnikClass = eventClass.GetSputnikClassFromCrateUid(container.net.ID.Value);
            if (sputnikClass != null)
            {
                if (!sputnikClass.IsSputnikAgressive())
                {
                ...
                if (!sputnikClass.CanOpenCrate(container.net.ID.Value))
                {
                    NotifyManager.SendMessageToPlayer(player, "NeedUseCard", _config.prefix);
                    return true;
                }
        ...
        {
            if (eventClass == null || player == null || container == null)
                return;
            ...
            {
                sputnikClass.MakeSputnikAgressive();
                sputnikClass.OnCratelooted(container.net.ID.Value);
        ...
        {
            if (eventClass == null || scientistNPC == null || info == null || info.Initiator == null)
                return null;
            ...
            SputnikClass sputnikClass = eventClass.GetSputnikClassByNpcUid(scientistNPC.net.ID.Value);

            if (sputnikClass != null && info.InitiatorPlayer.IsRealPlayer())
                sputnikClass.MakeSputnikAgressive();

            AutoTurret autoTurret = info.Initiator as AutoTurret;

            if (autoTurret != null && eventClass.GetSputnikClassByTurretUid(autoTurret.net.ID.Value) != null)
                return true;
        ...
        {
            if (eventClass == null || autoTurret == null)
                return null;

            SputnikClass sputnikClass = eventClass.GetSputnikClassByTurretUid(autoTurret.net.ID.Value);
        ...
        {
            if (eventClass == null || patrolHelicopter == null || patrolHelicopter.net == null)
                return;

            SputnikClass sputnikClass = eventClass.GetSputnikClassByHeliUid(patrolHelicopter.net.ID.Value);

            if (sputnikClass != null && info.InitiatorPlayer.IsRealPlayer())
            {
                sputnikClass.MakeSputnikAgressive();
                sputnikClass.sputnikHeli.OnPlayerAttacked(info.InitiatorPlayer.userID);
            }
        }

        void OnEntityDeath(PatrolHelicopter patrolHelicopter, HitInfo info)
        {
            if (patrolHelicopter == null || eventClass == null || patrolHelicopter.net == null)
                return;

            SputnikClass sputnikClass = eventClass.GetSputnikClassByHeliUid(patrolHelicopter.net.ID.Value);
            if (sputnikClass != null)
            {
                ulong lastHeliAttackerUserId = sputnikClass.sputnikHeli.lastAttackedPlayer;
                if (lastHeliAttackerUserId != 0)
                    EconomyManager.ActionEconomy(lastHeliAttackerUserId, "Heli");
            }
        }
        ...
        void OnEntityDeath(BasePlayer player, HitInfo info)
        {
            ...
            {
                if (eventClass == null || info == null)
                    return;
                else if (info.InitiatorPlayer.IsRealPlayer() && _config.npcConfigs.Any(x => x != null && x.name == scientistNPC.displayName))
        ...
        {
            if (autoTurret == null || eventClass == null || autoTurret.net == null || !info.InitiatorPlayer.IsRealPlayer())
                return;
        ...
        {
            if (eventClass == null || !player.IsRealPlayer())
                return;
        ...
        {
            if (eventClass == null || entity == null || corpse == null)
                return;
            ...
            if (entity is ScientistNPC)
            {
                ...
                NextTick(() =>
                {
                    ...
                    if (npcConfig.typeLootTable == 0)
                    {
                        ...
                        if (npcConfig.deleteCorpse && corpse.IsExists())
                            corpse.Kill();
                        return;
                    }
                    ...
                    if (npcConfig.typeLootTable == 1 && npcConfig.lootTable.minAmount > 0)
                        AddToContainerItem(container, npcConfig.lootTable, npcConfig.typeLootTable);
        ...
        object OnTrapTrigger(BaseTrap baseTrap, GameObject gameObject)
        {
            ...
            if (scientistNPC != null && _config.npcConfigs.Any(x => x.name == scientistNPC.displayName))
                return true;
        ...
        {
            if (eventClass == null || !turret.IsExists() || player == null)
                return null;

            SputnikClass sputnikClass = eventClass.GetSputnikClassByTurretUid(turret.net.ID.Value);
            if (sputnikClass != null)
            {
                if (!player.IsRealPlayer())
                    return true;

                if (!sputnikClass.IsSputnikAgressive() && !_config.agressiveConfig.turretAgressiveMode)
                    return true;
            }
        ...
        {
            if (eventClass == null || heli == null)
                return null;

            SputnikClass sputnikClass = eventClass.GetSputnikClassByHeliUid(heli.helicopterBase.net.ID.Value);
            if (sputnikClass != null)
            {
                if (!player.IsRealPlayer())
                    return false;
                if (!sputnikClass.IsSputnikAgressive() && !_config.agressiveConfig.heliAgressiveMode)
                    return false;
            }
        ...
        {
            if (eventClass == null || npc == null || npc.net == null)
                return null;

            if (!_config.npcConfigs.Any(x => x.name == npc.displayName))
                return null;

            SputnikClass sputnikClass = eventClass.GetSputnikClassByNpcUid(npc.net.ID.Value);
            if (sputnikClass != null)
            {
                if (!sputnikClass.IsSputnikAgressive() && !_config.agressiveConfig.npcAgressiveMode)
                    return false;
            }
            return null;
        }
        ...
        object CanPopulateLoot(ScientistNPC entity, NPCPlayerCorpse corpse)
        {
            ...
            if (npcConfig != null && npcConfig.typeLootTable != 2)
                return true;
        ...
        {
            if (eventClass == null || container == null)
                return null;
            ...
            if (sputnikClass.lockedCrates.Any(y => y.Key != null && y.Key.net.ID == container.net.ID))
                cratePresetName = sputnikClass.lockedCrates[container];
            else if (sputnikClass.crates.Any(y => y.Key != null && y.Key.net.ID == container.net.ID))
                cratePresetName = sputnikClass.crates[container];
            ...
            if (crateConfig != null && crateConfig.typeLootTable != 2)
                return true;
        ...
        {
            if (eventClass == null || netID == null)
                return null;

            SputnikClass sputnikClass = eventClass.GetSputnikClassFromCrateUid(netID.Value);
            if (sputnikClass == null)
                return null;

            string cratePresetName = null;
            if (sputnikClass.lockedCrates.Any(y => y.Key != null && y.Key.net.ID.Value == netID.Value))
                cratePresetName = sputnikClass.lockedCrates.FirstOrDefault(y => y.Key != null && y.Key.net.ID.Value == netID.Value).Value;
            else if (sputnikClass.crates.Any(y => y.Key != null && y.Key.net.ID.Value == netID.Value))
                cratePresetName = sputnikClass.crates.FirstOrDefault(y => y.Key != null && y.Key.net.ID.Value == netID.Value).Value;
            ...
            if (crateConfig != null && crateConfig.typeLootTable != 3)
                return true;
        ...
        {
            if (eventClass == null || autoTurret == null || hitinfo == null)
                return null;
            ...
            if (sputnikClass != null)
            {
                ...
                if (hitinfo.InitiatorPlayer != null && !sputnikClass.pveModeController.IsPveModeBlockAction(hitinfo.InitiatorPlayer))
                    return true;
                else
                    return null;
            }
        ...
        {
            if (eventClass == null || landmine == null || hitinfo == null)
                return null;

            SputnikClass sputnikClass = eventClass.GetSputnikClassByMineUid(landmine.net.ID.Value);
        ...
        {
            if (eventClass == null || scientistNPC == null || hitinfo == null || hitinfo.Initiator == null)
                return null;
            ...
            if (hitinfo.Initiator is AutoTurret && eventClass.GetSputnikClassByTurretUid(hitinfo.Initiator.net.ID.Value) != null)
                return false;
        ...
        {
            if (eventClass == null || victim == null || hitinfo == null || !victim.userID.IsSteamId())
                return null;
            ...
            if (attacker != null)
            {
                ...
                if (sputnikClass != null && sputnikClass.sputnikDebrisConfig.zoneConfig.isCreateZonePVP && !_config.supportedPluginsConfig.pveMode.enable && (attacker == null || (attacker != null && eventClass.GetSputnikClassFromPLayerInZoneUid(attacker.userID) != null)))
                    return true;
            }
            else if (hitinfo.Initiator != null)
            {
                ...
                if (autoTurret != null)
                {
                    ...
                    if (sputnikClass != null)
                    {
                        if (victim != null && !sputnikClass.pveModeController.IsPveModeBlockAction(victim))
                            return true;
                    }
                }
                ...
                if (baseTrap != null)
                {
                    SputnikClass sputnikClass = eventClass.GetSputnikClassByMineUid(baseTrap.net.ID.Value);
                    if (sputnikClass != null)
                    {
                        if (victim != null && !sputnikClass.pveModeController.IsPveModeBlockAction(victim))
                            return true;
        ...
        {
            if (eventClass == null || trap == null || !player.IsRealPlayer())
                return null;

            if (eventClass.GetSputnikClassByMineUid(trap.net.ID.Value) != null)
                return true;
        ...
        {
            if (eventClass == null || turret == null || turret.net == null)
                return null;
            ...
            if (sputnikClass != null)
            {
                if (!player.IsRealPlayer())
                    return false;
                else if (!sputnikClass.IsSputnikAgressive() && !_config.agressiveConfig.turretAgressiveMode)
                    return false;
                else if (player != null && !sputnikClass.pveModeController.IsPveModeBlockAction(player))
                    return true;
            }
        ...
        {
            if (player == null || eventClass == null)
                return null;
        ...
        void SetOwnerPveMode(string eventName, BasePlayer owner)
        {
            ...
            SputnikClass sputnikClass = eventClass.GetSputnikClassByShortName(eventName);

            if (sputnikClass != null)
                sputnikClass.pveModeController.SetPveModeOwner(owner);
        }
        ...
        void ClearOwnerPveMode(string eventName)
        {
            ...
            if (sputnikClass != null)
                sputnikClass.pveModeController.ClearPveModeOwner();
        }
        ...
        void ChatStartCommand(BasePlayer player, string command, string[] arg)
        {
            ...
            string eventPresetName = "";

            if (arg != null && arg.Length >= 1)
                eventPresetName = arg[0];

            StartEvent(eventPresetName, player);
        }
        ...
        void ConsoleStartCommand(ConsoleSystem.Arg arg)
        {
            ...
            if (arg.Args != null && arg.Args.Length > 0)
                eventPresetName = arg.Args[0];

            StartEvent(eventPresetName);
        ...
        void UpdateConfig()
        {
            ...
            if (_config.version.Minor == 2)
            {
                ...
                if (_config.version.Patch <= 1)
                {
                    foreach (SputnikDebrisConfig sputnikDebrisConfig in _config.sputnikDebrisConfigs)
                    {
                        sputnikDebrisConfig.markerConfig.useRingMarker = true;
                        sputnikDebrisConfig.markerConfig.useShopMarker = true;
                    }
                ...
                {
                    if (_config.supportedPluginsConfig.pveMode.scaleDamage.Any(x => x.Type == "Turret"))
                    {
                        ScaleDamageConfig scaleDamageConfig = new ScaleDamageConfig
                        {
                            Type = "Turret",
                            Scale = 1f
                        };
        ...
        void Unsubscribes()
        {
            foreach (string hook in subscribeMethods)
                Unsubscribe(hook);
            if (!_config.customCardConfig.enableSpawnInDefaultCrates)
        ...
        void StartEvent(string presetName = "", BasePlayer activator = null)
        {
            ...
            if (eventConfig == null)
            {
                NotifyManager.PrintError(activator, "PresetNotFound_Exeption", presetName);
                return;
            }
        ...
        class EventClass
        {
            ...
            internal SputnikClass GetSputnikClassFromCrateUid(ulong crateUID) => sputnikClasses.FirstOrDefault(x => x != null && (x.IsEventCrate(crateUID) || x.IsEventLockedCrate(crateUID)));

            internal SputnikClass GetSputnikClassFromCardReaderUid(ulong cardReaderUID) => sputnikClasses.FirstOrDefault(x => x != null && x.IsClassCardReader(cardReaderUID));

            internal SputnikClass GetSputnikClassByTurretUid(ulong netID) => sputnikClasses.FirstOrDefault(x => x != null && x.IsDebrisTurret(netID));

            internal SputnikClass GetSputnikClassByMineUid(ulong netID) => sputnikClasses.FirstOrDefault(x => x != null && x.IsDebrisMine(netID));

            internal SputnikClass GetSputnikClassByNpcUid(ulong netID) => sputnikClasses.FirstOrDefault(x => x != null && x.IsNpcMine(netID));

            internal SputnikClass GetSputnikClassByHeliUid(ulong netID) => sputnikClasses.FirstOrDefault(x => x != null && x.IsHeliMine(netID));

            internal SputnikClass GetSputnikClassByShortName(string shortname) => sputnikClasses.FirstOrDefault(x => x != null && shortname == x.sputnikDebrisConfig.presetName);
            ...
            {
                if (ins._config.notificatonConfig.preStartTime > 0)
                {
                    NotifyManager.SendMessageToAll("PreStartEvent", ins._config.prefix, eventConfig.displayName, ins._config.notificatonConfig.preStartTime);
                    yield return CoroutineEx.waitForSeconds(ins._config.notificatonConfig.preStartTime);
                }

                if (ins.autoEventCorountine != null)
                    ServerMgr.Instance.StopCoroutine(ins.autoEventCorountine);
            ...
            IEnumerator EventCorountine()
            {
                ...
                {
                    eventTime -= 1;
                    if (ins._config.notificatonConfig.timeNotifications.Contains(eventTime))
                    {
        ...
        class SputnikClass : FacepunchBehaviour
        {
            ...
            internal SputnikHeli sputnikHeli;
            ...
            internal HashSet<ulong> spaceCardCratesIDs = new HashSet<ulong>();
            internal int destroyTime = 0;
            internal int agressiveTime = 0;

            internal PveModeController pveModeController;
            float lastEventOvnerNameCheckTime = 0;
            string eventOwnerName = "";

            internal bool IsSputnikAgressive()
            {
                return ins._config.agressiveConfig.agressiveSecurityMode || agressiveTime > 0;
            }

            internal void MakeSputnikAgressive()
            {
                if (ins._config.agressiveConfig.agressiveSecurityMode) return;
                agressiveTime = ins._config.agressiveConfig.agressiveTime;
            }

            internal bool IsClassCardReader(ulong netId) => cardReader != null && cardReader.net.ID.Value == netId;

            internal bool CanOpenCrate(ulong netId) => !spaceCardCratesIDs.Contains(netId) || !cardReader.IsExists() || cardReaderOpen;

            internal bool IsEventCrate(ulong netId) => crates.Keys.Any(x => x.IsExists() && x.net != null && x.net.ID.Value == netId);

            internal bool IsEventLockedCrate(ulong netId) => lockedCrates.Keys.Any(x => x.IsExists() && x.net != null && x.net.ID.Value == netId);

            internal bool IsPlayerInZone(ulong userId) => zoneClass != null && zoneClass.playersInZone.Any(x => x.userID == userId);

            internal bool IsDebrisTurret(ulong netId) => turrets.Any(x => x.IsExists() && x.net.ID.Value == netId);

            internal bool IsDebrisMine(ulong netId) => mines.Any(x => x.IsExists() && x.net.ID.Value == netId);

            internal bool IsNpcMine(ulong netId) => npcs.Any(x => x.IsExists() && x.net.ID.Value == netId);

            internal bool IsHeliMine(ulong netId) => sputnikHeli != null && sputnikHeli.patrolHelicopter.IsExists() && sputnikHeli.patrolHelicopter.net.ID.Value == netId;
            ...
            internal void RemovePlayerFromZone(BasePlayer player)
            {
                ...
                if (sputnikDebrisConfig.zoneConfig.isCreateZonePVP)
                    NotifyManager.SendMessageToPlayer(player, "ExitPVP", ins._config.prefix);
            }

            internal void OnCratelooted(ulong netId)
            {
                ...
                if (crates.Keys.Any(x => x != null && !openedCrates.Contains(x.net.ID.Value)) || lockedCrates.Keys.Any(x => x != null && !openedCrates.Contains(x.net.ID.Value))) return;
            ...
            internal void Init(SputnikDebrisConfig sputnikDebrisConfig)
            {
                ...
                pveModeController = new PveModeController();
            }
            ...
            void DefineEventPosition()
            {
                ...
                if (sputnikDebrisConfig.useCustomSpawnPoints)
                {
                    string newPosition = sputnikDebrisConfig.customSpawnPoints.GetRandom();
                    if (newPosition == null)
                    {
                        ins.PrintError("Couldn't find a custom position");
                    }
                    else position = newPosition.ToVector3();
                }
                if (position == Vector3.zero || position == null)
                {
                    while (position == Vector3.zero && LocationDefiner.spawnPositions.Count > 0)
                    {
                        Vector3 newPosition = LocationDefiner.spawnPositions.FirstOrDefault(x => true);
                        LocationDefiner.spawnPositions.Remove(newPosition);
            ...
            IEnumerator OnSputnikFellCorountine()
            {
                ...
                NotifyManager.SendMessageToAll("Crash", ins._config.prefix, PhoneController.PositionToGridCoord(gameObject.transform.position));
            }
            ...
            void SpawnMapMarker()
            {
                if (!sputnikDebrisConfig.markerConfig.useRingMarker) return;
            ...
            {
                if (!sputnikDebrisConfig.markerConfig.useShopMarker) return;
                ...
                vendingMarker.markerShopName = $"{sputnikDebrisConfig.markerConfig.displayName} {NotifyManager.GetTimeMessage(null, ins.eventClass.eventTime)}";
                vendingMarker.Spawn();
            }
            ...
            void UpdateMapMarker()
            {
                ...
                {
                    string displayEventOwnerName = eventOwnerName != "" ? ins.GetMessage("Marker_EventOwner", null, eventOwnerName) : "";
                    string text = destroyTime > 0 ? $"{sputnikDebrisConfig.markerConfig.displayName} {NotifyManager.GetTimeMessage(null, destroyTime)} " + displayEventOwnerName : $"{sputnikDebrisConfig.markerConfig.displayName} {NotifyManager.GetTimeMessage(null, ins.eventClass.eventTime)} " + displayEventOwnerName;
                    if (ins._config.supportedPluginsConfig.pveMode.enable && ins._config.supportedPluginsConfig.pveMode.showEventOwnerNameOnMap && ins.plugins.Exists("PveMode"))
                    {
                        if (Time.realtimeSinceStartup - lastEventOvnerNameCheckTime >= 5)
                        {
                            GetEventOwnerDisplayName();
            ...
            void GetEventOwnerDisplayName()
            {
                BasePlayer eventOwner = pveModeController.GetEventOwner();

                ulong ownerId = (ulong)ins.PveMode.Call("GetEventOwner", sputnikDebrisConfig.presetName);
                if (ownerId != 0)
                {
                    BasePlayer player = BasePlayer.FindByID(ownerId);
                    if (player != null)
                        eventOwnerName = player.displayName;
                }
                else
                    eventOwnerName = "";
            ...
            void CreateGrounSputnik()
            {
                ...
                {
                    decorEntities.Add(BuildManager.CreateDecorEntity(entData, gameObject.transform));
                }
            }
            ...
            void CreateNPCs()
            {
                foreach (var pair in sputnikDebrisConfig.NPCs)
                {
                    ...
                    JObject npcConfigObject = BuildManager.GetNpcConfig(npcConfig);
                    foreach (string positionString in pair.Value)
                    {
                        Vector3 globalPosition = LocationDefiner.GetGlobalPosition(gameObject.transform, positionString.ToVector3());
                        Vector3 spawnPosition = LocationDefiner.GetGroundPositionInPoint(globalPosition);

                        if (spawnPosition == Vector3.zero)
                            continue;

                        ScientistNPC scientistNPC = (ScientistNPC)ins.NpcSpawn.Call("SpawnNpc", spawnPosition, npcConfigObject);
            ...
            void CreateCrate(string cratePresetName, Vector3 position, Quaternion rotation)
            {
                ...
                LootContainer lootContainer = BuildManager.CreateRegularEntity(crateConfig.prefab, position, rotation) as LootContainer;
                ...
                if (supplyDrop != null)
                {
                    lootContainer.SetFlag(BaseEntity.Flags.Reserved2, false);
                ...
                if (cardReader.IsExists() && crateConfig.needSpaceCard) spaceCardCratesIDs.Add(lootContainer.net.ID.Value);
            }

            void CreateCardReader()
            {
                if (!sputnikDebrisConfig.enableCardReader) return;
                cardReader = BuildManager.CreateRegularEntity("assets/prefabs/io/electric/switches/cardreader.prefab", LocationDefiner.GetGlobalPosition(gameObject.transform, sputnikDebrisConfig.cardRaderLocation.position.ToVector3()), LocationDefiner.GetGlobalRotation(gameObject.transform, sputnikDebrisConfig.cardRaderLocation.rotation.ToVector3())) as CardReader;
                if (cardReader.IsExists())
                {
                    UpdateCardreaderAccessLevel();
                    cardReaderOpen = false;
                    cardReader.UpdateFromInput(100, 0);
                }
            }

            void UpdateCardreaderAccessLevel()
            {
            ...
            void CreateMines()
            {
                foreach (string positionString in sputnikDebrisConfig.mines)
                {
                    ...
                    mines.Add(BuildManager.CreateRegularEntity("assets/prefabs/deployable/landmine/landmine.prefab", spawnPosition, Quaternion.identity));
                }
            }
            ...
            void CreateHeli()
            {
                ...
                PatrolHelicopter heli = GameManager.server.CreateEntity("assets/prefabs/npc/patrol helicopter/patrolhelicopter.prefab", gameObject.transform.position + new Vector3(0, heliConfig.height, 0)) as PatrolHelicopter;
                ...
                sputnikHeli.InitCustomHelicopter(heli, heliConfig, gameObject.transform.position);
            ...
            {
                Vector3 globalPosition = LocationDefiner.GetGlobalPosition(gameObject.transform, locationConfig.position.ToVector3());
                Vector3 spawnPosition = turretConfig.autoHeight ? LocationDefiner.GetGroundPositionInPoint(globalPosition) : globalPosition;

                if (spawnPosition == Vector3.zero)
                    return;

                AutoTurret autoTurret = BuildManager.CreateRegularEntity("assets/prefabs/npc/autoturret/autoturret_deployed.prefab", spawnPosition, LocationDefiner.GetGlobalRotation(gameObject.transform, locationConfig.rotation.ToVector3())) as AutoTurret;
            ...
            void OnDestroy()
            {
                ...
                if (sputnikHeli != null && sputnikHeli.patrolHelicopter.IsExists()) sputnikHeli.patrolHelicopter.Kill();
        ...
        class ZoneClass : FacepunchBehaviour
        {
            ...
            internal void UpdateGui()
            {
                if (ins._config.guiConfig.IsGUI)
                {
                    foreach (BasePlayer player in playersInZone)
                    {
                        ...
                        MessageGUI(player, ins.GetMessage("GUI", player.UserIDString, sputnikClass != null && sputnikClass.destroyTime > 0 ? NotifyManager.GetTimeMessage(player.UserIDString, sputnikClass.destroyTime) : NotifyManager.GetTimeMessage(player.UserIDString, ins.eventClass.eventTime)));
                    }
                }
            }
            ...
            void CreateZone()
            {
                ...
                if (sputnikClass.sputnikDebrisConfig.zoneConfig.radiation > 0)
                    CreateRadiationTrigger();

                if (ins._config.supportedPluginsConfig.pveMode.enable)
                    CreatePveModeZone();
                else if (sputnikClass.sputnikDebrisConfig.zoneConfig.isDome)
                    CreateSphere();
            }
            ...
            void CreatePveModeZone()
            {
                ...
                foreach (ScientistNPC scientistNPC in sputnikClass.npcs)
                    if (scientistNPC.IsExists())
                        npcs.Add(scientistNPC.net.ID.Value);
                foreach (LootContainer lootContainer in sputnikClass.crates.Keys)
                    if (lootContainer.IsExists())
                        crates.Add(lootContainer.net.ID.Value);
                foreach (LootContainer lootContainer in sputnikClass.lockedCrates.Keys)
                    if (lootContainer.IsExists())
                        crates.Add(lootContainer.net.ID.Value);
                foreach (AutoTurret autoTurret in sputnikClass.turrets)
                    if (autoTurret.IsExists())
                        turrets.Add(autoTurret.net.ID.Value);

                if (sputnikClass.sputnikHeli != null)
                    helies.Add(sputnikClass.sputnikHeli.GetHeliNetId());

                pveModeZoneName = sputnikClass.sputnikDebrisConfig.presetName + "_" + transform.position.ToString();

                ins.PveMode.Call("EventAddPveMode", pveModeZoneName, config, sputnikClass.gameObject.transform.position, sputnikClass.sputnikDebrisConfig.zoneConfig.radius, crates, npcs, bradleys, helies, new HashSet<ulong>(), null);
                ins.PveMode.Call("EventAddTurrets", pveModeZoneName, turrets);
            }
            ...
            void OnTriggerEnter(Collider other)
            {
                ...
                if (player.IsRealPlayer() && !playersInZone.Contains(player))
                {
                    ...
                    if (sputnikClass.sputnikDebrisConfig.zoneConfig.isCreateZonePVP)
            ...
            void OnDestroy()
            {
                ...
                {
                    ins.PveMode.Call("EventRemovePveMode", pveModeZoneName, false);
                }
            }
        }
        ...
        class FallSputnikClass : FacepunchBehaviour
        {
            ...
            void CreateEntities()
            {
                BaseEntity mainEntity = BuildManager.CreateRegularEntity("assets/bundled/prefabs/oilfireballsmall.prefab", startPosition, Quaternion.identity);
                ...
                foreach (EntityData entityData in ins.fallSputnicData)
                {
                    ...
                    {
                        BaseEntity entity = BuildManager.CreateRegularEntity(entityData.prefabName, LocationDefiner.GetGlobalPosition(mainEntity.transform, location.position), Quaternion.identity);
        ...
        class SputnikHeli : FacepunchBehaviour
        {
            ...
            Vector3 position;
            int ounsideTime = 0;
            internal ulong lastAttackedPlayer = 0;
            ...
            internal void OnPlayerAttacked(ulong userId)
            {
            ...
            void UpdatePosition()
            {
                ...
                {
                    if (Vector3.Distance(position, patrolHelicopter.transform.position) > heliConfig.distance)
                    {
                        ounsideTime++;
                ...
                else if (Vector3.Distance(position, patrolHelicopter.transform.position) > heliConfig.distance)
                {
                    patrolHelicopter.myAI.State_Move_Enter(position);
                    ounsideTime = 0;
                }
        ...
        {
            BasePlayer eventOwner;

            internal void SetPveModeOwner(BasePlayer newEventOwner)
            {
                eventOwner = newEventOwner;
            }

            internal void ClearPveModeOwner()
            {
                eventOwner = null;
            }
        ...
        static class LocationDefiner
        {
            ...
            internal static HashSet<Vector3> spawnPositions = new HashSet<Vector3>();
            ...
            {
                bool isPrintMessage = spawnPositions.Count == 0;

                if (isPrintMessage)
                    ins.PrintWarning("The search for event locations has begun!");
                ...
                spawnPositions.RemoveWhere(x => !ChechOldPoint(x));
                ...
                if (isPrintMessage)
                    ins.PrintWarning($"Found {spawnPositions.Count} points for the event");
            }

            internal static bool PostChechPoint(Vector3 postition) => postition != Vector3.zero && CheckColliders(postition) && CheckRaidableBases(postition) && CheckZoneManager(postition);
            ...
            internal static bool ChechOldPoint(Vector3 postition)
            {
                return postition != Vector3.zero && CheckUpSpace(postition) && CheckDeltaHeightInRadius(postition) && CheckColliders(postition) && CheckNewSpawnEntity(postition) && CheckZoneManager(postition) && CheckBuildingPrivileges(postition) && CheckRaidableBases(postition);
            }
            ...
            {
                foreach (Collider collider in UnityEngine.Physics.OverlapSphere(postition, 15f))
                {
                    if (collider.name.Contains("Rail Mesh") || collider.name.Contains("Road Mesh") || collider.name.Contains("train_track") || collider.name.Contains("Safe") || collider.name.Contains("Trigger (8)") || collider.name.Contains("Water")) return false;
                    SphereCollider sphereCollider = collider as SphereCollider;
                    if (sphereCollider != null && sphereCollider.isTrigger) return false;

                    BaseEntity entity = collider.ToBaseEntity();
                    if (entity == null) continue;
                    if (entity is BuildingBlock) return false;
                    if (entity is SimpleBuildingBlock) return false;
                }
                return true;
            }
            ...
            {
                if (!ins._config.supportedPluginsConfig.zoneManager.enable || !ins.plugins.Exists("ZoneManager") || zoneManagerDatas.Count == 0)
                    return true;

                if (zoneManagerDatas.Any(x => x != null && Vector3.Distance(x.position, position) < x.radius + 10))
                    return false;
            ...
            {
                if (!ins._config.supportedPluginsConfig.raidableBases.enable || !ins.plugins.Exists("RaidableBases"))
                    return true;
                return
                    !(bool)ins.RaidableBases.Call("EventTerritory", position);
            }
        ...
        static class EconomyManager
        {
            ...
            internal static void ActionEconomy(ulong playerId, string type, string arg = "")
            {
                switch (type)
                {
                    ...
                    case "LockedCrate":
                        ...
                        break;
                    case "Turret":
                        AddBalance(playerId, ins._config.supportedPluginsConfig.economic.turretPoint);
                        break;
                    case "Heli":
                        AddBalance(playerId, ins._config.supportedPluginsConfig.economic.heliPoint);
                        break;
                }
            }
            ...
            static void DefineEventWinner()
            {
                ...
                if (winnerPair.Value > 0)
                {
                    ...
                    if (ins._config.supportedPluginsConfig.economic.enable)
        ...
        static class NotifyManager
        {
            ...
            {
                if (player == null)
                    ins.PrintError(ClearColorAndSize(ins.GetMessage(langKey, null, args)));
                else
                    ins.PrintToChat(player, ins.GetMessage(langKey, player.UserIDString, args));
            }
            ...
            internal static void PrintLogMessage(string langKey, params object[] args)
            {
                ...
                ins.Puts(ClearColorAndSize(ins.GetMessage(langKey, null, args)));
            }
            ...
            {
                foreach (BasePlayer player in BasePlayer.activePlayerList) if (player != null)
            ...
            internal static void SendMessageToPlayer(BasePlayer player, string langKey, params object[] args)
            {
                ...
                if (ins._config.notificatonConfig.chat)
                    ins.PrintToChat(player, ins.GetMessage(langKey, player.UserIDString, args));

                if (ins._config.supportedPluginsConfig.GUIAnnouncements.enable) ins.GUIAnnouncements?.Call("CreateAnnouncement",
                    ClearColorAndSize(ins.GetMessage(langKey, player.UserIDString, args)), ins._config.supportedPluginsConfig.GUIAnnouncements.bannerColor, ins._config.supportedPluginsConfig.GUIAnnouncements.textColor, player, ins._config.supportedPluginsConfig.GUIAnnouncements.apiAdjustVPosition);

                if (ins._config.supportedPluginsConfig.notify.enable)
                    player.SendConsoleCommand($"notify.show {ins._config.supportedPluginsConfig.notify.type} {ClearColorAndSize(ins.GetMessage(langKey, player.UserIDString, args))}");
            }
            ...
            static void TrySendDiscordMessage(string langKey, params object[] args)
            {
                if (CanSendDiscordMessage() && ins._config.supportedPluginsConfig.discord.keys.Contains(langKey))
                {
                    ...
                    object fields = new[] { new { name = ins.Title, value = ClearColorAndSize(ins.GetMessage(langKey, null, args)), inline = false } };
            ...
            static bool CanSendDiscordMessage()
            {
                return ins._config.supportedPluginsConfig.discord.enable && !string.IsNullOrEmpty(ins._config.supportedPluginsConfig.discord.webhookUrl) && ins._config.supportedPluginsConfig.discord.webhookUrl != "https://support.discordapp.com/hc/en-us/articles/228383668-Intro-to-Webhooks";
            }
            ...
            internal static string GetTimeMessage(string userIDString, int seconds)
            {
                ...
                if (timeSpan.Hours > 0) message += $" {timeSpan.Hours} {ins.GetMessage("Hour", userIDString)}";
                if (timeSpan.Minutes > 0) message += $" {timeSpan.Minutes} {ins.GetMessage("Min", userIDString)}";
                if (message == "") message += $" {timeSpan.Seconds} {ins.GetMessage("Sec", userIDString)}";
        ...
        public class SupportedPluginsConfig
        {
            ...
            [JsonProperty(en ? "Economy Setting" : "Настройка экономики")] public EconomyConfig economic { get; set; }
        ...
        private class PluginConfig
        {
            ...
            [JsonProperty(en ? "Notification Settings" : "Настройки уведомлений")] public NotifyConfig notificatonConfig { get; set; }
            ...
            public static PluginConfig DefaultConfig()
            {
                return new PluginConfig()
                {
                    ...
                    notificatonConfig = new NotifyConfig
                    {
                        preStartTime = 10,
                        chat = true,
