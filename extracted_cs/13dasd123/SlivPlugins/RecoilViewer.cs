...
namespace Oxide.Plugins{
    ...
    [Info("RecoilViewer", "ModHub.to", "1.4.1")]
    ...
    public class RecoilViewer : RustPlugin {
        ...
        void ConsoleCmdRecoil(ConsoleSystem.Arg arg){
            ...
            if(arg.Args == null){
                ...
                var ID = 0ul;
                BasePlayer search = (ulong.TryParse(arg.Args[0], out ID)) ? FindPlayerByID(ID) : FindPlayerByPartialName(arg.Args[0]);
                if (search == null){
                    PrintToChat(player, Lang("ErrorSearch", arg.Args[0]));
                    return;
                }
