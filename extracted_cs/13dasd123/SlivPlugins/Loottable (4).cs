 ...
 * 
 * V 1.0.1
 * - replaced Find Objects Of Type with BaseNetworkable.serverEntities.Find
 * 
 * V 1.0.2
 ...
 * 
 * V 1.0.32
 * - modify OnCustomAirdrop hook to resolve issue with VehicleAirdrops
 * - performance optimization for stacking hooks
 * - performance optimization for npc loot hooks
 * - add hook time tracker for debugging
 * - npc loot system optimization - should now consume less memory
 * 
 * V 1.0.33
 * - add support for legacy furnace
 * - fix error when opening ui
 * 
 * V 1.0.34
 * - replace discord images
 * - fix return behavior of CanCombineDroppedItem
 * - remove Conditional for carbon compatibility
 * - fix issues with scrap multiplier
 * 
 * V 1.0.35
 * - fix issues with military flame thrower
 * 
 * V 1.0.36
 * - upgrade to harmony v2
 * 
 * V 1.0.37
 * - fix error with npc spawn points that could prevent plugin from loading
 * 
**/
...
namespace Oxide.Plugins
{
    ...
    [Info("Loottable", "The_Kiiiing", "1.0.37")]
    [Description("A configurable loot table with GUI")]
    public class Loottable : RustPlugin
    {
        #region Hook time tracker

        #if HOOKTIME_DEBUG
        private sealed class HookTimeTracker : IDisposable
        {
            public static readonly Dictionary<string, HookInfo> hookInfo = new();
            ...
            public HookTimeTracker(string hookName)
            {
                this.hookName = hookName;
                breakpoints = Pool.GetList<double>();
            ...
            public void Dispose()
            {
                ...
                if (!hookInfo.TryGetValue(hookName, out var info))
                {
                    info = new HookInfo(hookName, true);
                    hookInfo.Add(hookName, info);
                }

                info.AddCall(sw.Elapsed.TotalMilliseconds, breakpoints);

                Pool.FreeList(ref breakpoints);
            }
            ...
            public class HookInfo
            {
                ...
                public List<HookInfo> breakpoints;

                public HookInfo(string hookName)
                    : this (hookName, false) { }

                public HookInfo(string hookName, bool hasBreakpoints)
                {
                    this.hookName = hookName;
                    if (hasBreakpoints)
                    {
                        breakpoints = new List<HookInfo>();
                    }
                }

                public void AddCall(double time, IList<double> breakpointTime)
                {
                    calls++;
                    averageTime = (averageTime + time) / calls;
                    totalTime += time;
                    if (time > maxTime)
                    {
                        maxTime = time;
                    ...
                    if (breakpoints != null && breakpointTime != null)
                    {
                        for(int i = 0; i < breakpointTime.Count; i++)
                        {
                            if (i < breakpoints.Count)
                            {
                                breakpoints[i].AddCall(breakpointTime[i], null);
                            }
                            else
                            {
                                var info = new HookInfo($"Sec {i}");
                                info.AddCall(breakpointTime[i], null);
        ...
        [ConsoleCommand("l.hooktime")]
        void CmdHookTime(ConsoleSystem.Arg arg)
        {
            var player = arg.Player();
            if (player != null && !permission.UserHasPermission(player.UserIDString, PERM_DEBUG))
            {
                arg.ReplyWith("No permission");
                return;
            }

            var sb = new StringBuilder($"Hook time overview (Plugin uptime {Mathf.RoundToInt(Time.time - HookTimeTracker.loadTime)}s)");
            sb.AppendLine();
            sb.AppendLine(ToFixed("Hook", 35) + ToFixed("Calls") + ToFixed("Avg") + ToFixed("Min") + ToFixed("Max") + ToFixed("Total"));

            foreach (var info in HookTimeTracker.hookInfo.Values)
            {
                sb.AppendLine(ToFixed(info.hookName, 35) + ToFixed(info.calls.ToString()) + ToFixed(info.averageTime.ToString("N6")) + ToFixed(info.minTime.ToString("N6")) + ToFixed(info.maxTime.ToString("N6")) + ToFixed(info.totalTime.ToString("N2")));
                if (info.breakpoints != null) foreach(var bp in info.breakpoints)
                {
                    sb.AppendLine("    " + ToFixed(bp.hookName, 31) + ToFixed(bp.calls.ToString()) + ToFixed(bp.averageTime.ToString("N6")) + ToFixed(bp.minTime.ToString("N6")) + ToFixed(bp.maxTime.ToString("N6")) + ToFixed(bp.totalTime.ToString("N2")));
                }
            }

            arg.ReplyWith(sb.ToString());

            static string ToFixed(string str, int length = 14)
            {
                ...
                var fill = length - str.Length;
                if (fill > 0)
                {
                    return str + new string(' ', fill);
                }
        ...
        private static bool CanUseCustomAirdrop(SupplySignal signal, ThrownWeapon item)
        {
        ...
        {
            #if HOOKTIME_DEBUG
            using var htt = new HookTimeTracker("OnLootSpawn");
            #endif

            var lootConfig = LootManager.GetContainerConfig(container);
            if (lootConfig == null || !CanPopulateContainerHook(container))
            {
                return null;
            }
        ...
        {
            #if HOOKTIME_DEBUG
            using var htt = new HookTimeTracker("OnEntitySpawned [LootableCorpse]");
            #endif
            ...
            if (config == null || !CanPopulateCorpseHook(corpse))
            {
                return;
            }
        ...
        {
            #if HOOKTIME_DEBUG
            using var htt = new HookTimeTracker("OnEntitySpawned [BasePlayer]");
            #endif

            if (npc.IsNpc)
            {
                LootManager.CacheNpc(npc);
            }
        }
        ...
        {
            #if HOOKTIME_DEBUG
            using var htt = new HookTimeTracker(combine ? "CanCombineDroppedItem" : "CanStackItem");
            #endif
            ...
            if (heldEntity is BaseProjectile weapon && !combine)
            {
                var mag = weapon.primaryMagazine;
                if (mag.contents > 0)
                {
            ...
            if (heldEntity is Chainsaw chainsaw && !combine)
            {
                if (chainsaw.ammo > 0)
                {
            ...
            if (heldEntity is FlameThrower flameThrower && !combine)
            {
            ...
            if (heldEntity is BaseLiquidVessel && (!item.contents.IsEmpty() || !target.contents.IsEmpty()))
            {
                return false;
            }
            ...
            if ((item.info.shortname == "hat.miner" || item.info.shortname == "hat.candle") && (item.contents?.itemList.Count ?? 0) > 0 && !combine && target.info.stackable > target.amount)
            {
                ...
                {
                    var content = item.contents.itemList.First();

                    var drop = ItemManager.Create(content.info, content.amount, content.skin);
        ...
        {
#if HOOKTIME_DEBUG
            using var htt = new HookTimeTracker("OnItemSplit");
#endif
            ...
            var heldEntity = split.GetHeldEntity();
            ...
            if (heldEntity is Chainsaw chainsaw)
            {
                if (chainsaw.ammo > 0)
                {
                    chainsaw.ammo = 0;
        ...
        {
#if HOOKTIME_DEBUG
            using var htt = new HookTimeTracker("OnItemAddedToContainer");
#endif
        ...
        {
            var res = CanStackItem(item1.item, item2.item, true);
        ...
        void OnExplosiveThrown(BasePlayer player, BaseEntity entity, ThrownWeapon item)
        {
            ...
            ConfigManager.AirdropConfig.OnSupplySignalThrown(signal, item);
        }
...
#if HOOKTIME_DEBUG
            HookTimeTracker.loadTime = Time.time;
#endif
        ...
        private static class ConfigManager
        {
            ...
            public static Dictionary<string, string> GetCustomImages()
            {
                return new Dictionary<string, string>
                {
                    ...
                    ["scrapheli"] = "https://i.postimg.cc/SRZQkZqP/scrappy0.png",
                    ["attack"] = "https://i.postimg.cc/SRZQkZqP/scrappy0.png",
        ...
        private class AirDropConfiguration
        {
            ...
            public void OnSupplySignalThrown(SupplySignal signal, ThrownWeapon item)
            {
                ...
                if (!CanUseCustomAirdrop(signal, item))
                {
                    DPrint("custom airdrop canceled");
                    return;
                }

                DPrint("serving custom airdrop");
        ...
        private class FurnaceConfiguration
        {
            ...
            private float? GetFurnaceSpeed(BaseOven oven)
            {
                ...
                if (oven.ShortPrefabName == "furnace" ||
                    oven.ShortPrefabName == "legacy_furnace")
                ...
                case "itemselect":
                    ...
                    switch (ci)
                    {
                        ...
                        case "select":
                            ...
                            {
                                UiHelper.EditorCache.Shortname = String.Join(' ', args.Skip(1));
                            }
                        ...
                        case "selectcustom":
                            ...
                            if (args.Length > 3)
                                UiHelper.EditorCache.item_name = String.Join(" ", args.Skip(3));
                            else
        ...
        public static class LootManager
        {
            ...
            private static Dictionary<ulong, int> npcCache = new();
            ...
            private static readonly List<NpcCorpse> npcList = new List<NpcCorpse>
            {
                ...
                new NpcCorpse(101, StringPool.Get("assets/rust.ai/agents/npcplayer/humannpc/scientist/scientistnpc_roam_nvg_variant.prefab"), "assets/bundled/prefabs/autospawn/monument/medium/nuclear_missile_silo.prefab", "npc_missilesilo_nvg", "NVG Missile Silo NPC"),

                new NpcCorpse(103, StringPool.Get("assets/rust.ai/agents/npcplayer/humannpc/scientist/scientistnpc_roam.prefab"), "assets/bundled/prefabs/autospawn/monument/medium/nuclear_missile_silo.prefab", "npc_missilesilo", "Missile Silo NPC"),
                new NpcCorpse(56, StringPool.Get("assets/rust.ai/agents/npcplayer/humannpc/scientist/scientistnpc_patrol.prefab"), "assets/bundled/prefabs/autospawn/monument/large/trainyard_1.prefab", "npc_trainyard", "Train Yard NPC", defaultConfig: "uArXCFye"),
                new NpcCorpse(57, StringPool.Get("assets/rust.ai/agents/npcplayer/humannpc/scientist/scientistnpc_patrol.prefab"), "assets/bundled/prefabs/autospawn/monument/large/airfield_1.prefab", "npc_airfield", "Airfield NPC", defaultConfig: "wuWVamUF"),
                new NpcCorpse(102, StringPool.Get("assets/rust.ai/agents/npcplayer/humannpc/scientist/scientistnpc_patrol.prefab"), "assets/bundled/prefabs/autospawn/monument/xlarge/launch_site_1.prefab", "npc_launchsite", "Launch Site NPC"),
                ...
                new NpcCorpse(59, new HashSet<uint>{
                    ...
                    StringPool.Get("assets/rust.ai/agents/npcplayer/humannpc/scientist/scientistnpc_roamtethered.prefab")
                }, "assets/bundled/prefabs/autospawn/monument/arctic_bases/arctic_research_base_a.prefab", "npc_arctic", "Arctic Base NPC", defaultConfig: "1XJiXGR7"),
            ...
            {
                //Normal
                new Crate(60, new string[] {"assets/bundled/prefabs/radtown/crate_basic.prefab"}, "crate_basic", "Basic Crate", "https://i.postimg.cc/wvRNspyz/crate-basic0.png",
                ...
                new Crate(2, new string[] {"assets/bundled/prefabs/radtown/crate_normal.prefab" }, "crate_military", "Military Crate", "https://i.postimg.cc/59wYcp8s/dm-t20.png",
                    defaultConfig: "wNwBm3rG"),
                new Crate(3, new string[] {"assets/bundled/prefabs/radtown/crate_elite.prefab" }, "crate_elite", "Elite Crate", "https://i.postimg.cc/wMhs0QQb/crate-t30.png",
                ...
                new Crate(84, new string[] {"assets/bundled/prefabs/modding/lootables/invisible/invisible_lootable_prefabs/invisible_crate_basic.prefab"}, "crate_basic_invisible",
                    "Invisible Basic Crate", "https://i.postimg.cc/wvRNspyz/crate-basic0.png",
                    defaultConfig: "pK6mg7rr"),
                ...
                new Crate(86, new string[] {"assets/bundled/prefabs/modding/lootables/invisible/invisible_lootable_prefabs/invisible_crate_normal.prefab" }, "crate_military_invisible", 
                    "Invisible Military Crate", "https://i.postimg.cc/59wYcp8s/dm-t20.png",
                    defaultConfig: "wNwBm3rG"),
                new Crate(85, new string[] {"assets/bundled/prefabs/modding/lootables/invisible/invisible_lootable_prefabs/invisible_crate_elite.prefab" }, "crate_elite_invisible", 
                    "Invisible Elite Crate", "https://i.postimg.cc/wMhs0QQb/crate-t30.png",
                    defaultConfig: "uhZNzFRQ"),
                new Crate(88, new string[] {"assets/bundled/prefabs/modding/lootables/invisible/invisible_lootable_prefabs/invisible_crate_normal_2_food.prefab" }, "crate_food_invisible",
                    "Invisible Food Crate", "https://i.postimg.cc/NGTTsQv7/crate-food0.png",
                ...
                new Crate(89, new string[] {"assets/bundled/prefabs/modding/lootables/invisible/invisible_lootable_prefabs/invisible_crate_normal_2_medical.prefab" }, "crate_medical_invisible",
                    "Invisible Medical Crate", "https://i.postimg.cc/htCQyjVg/crate-medical0.png",
                ...
                new Crate(90, new string[] {"assets/bundled/prefabs/modding/lootables/invisible/invisible_lootable_prefabs/invisible_crate_tools.prefab" }, "crate_tools_invisible",
                    "Invisible Tool Crate", "https://i.postimg.cc/Pq9pHR6R/crate-tools0.png",
                ...
                new Crate(91, new string[] { "assets/bundled/prefabs/modding/lootables/invisible/invisible_lootable_prefabs/invisible_foodbox.prefab" }, "crate_food_2_invisible",
                    "Invisible Food Box", "https://i.postimg.cc/VL8t52Yy/crate-food-20.png",
                ...
                new Crate(92, new string[] { "assets/bundled/prefabs/modding/lootables/invisible/invisible_lootable_prefabs/invisible_vehicle_parts.prefab" }, "crate_vehicle_parts_invisible",
                    "Invisible Vehicle Parts Crate", "https://i.postimg.cc/nzfF0bSp/vp0.png",
                    defaultConfig: "0BWnWbqK"),

                //Underwater
                new Crate(4, new string[] {"assets/bundled/prefabs/radtown/underwater_labs/crate_normal_2.prefab" }, "crate_basic_underwater", "Underwater Crate", "https://i.postimg.cc/FFgRCzQq/normal-uw0.png",
                    defaultConfig: "sFtPTBGM"),
                new Crate(5, new string[] {"assets/bundled/prefabs/radtown/underwater_labs/crate_normal.prefab" }, "crate_military_underwater", "Underwater Military Crate", "https://i.postimg.cc/wMVj1fVH/military-uw0.png",
                    defaultConfig: "v1ZuJ4HS"),
                new Crate(6, new string[] {"assets/bundled/prefabs/radtown/underwater_labs/crate_elite.prefab" }, "crate_elite_underwater", "Underwater Elite Crate", "https://i.postimg.cc/wMhs0QQb/crate-t30.png",
                    defaultConfig: "qBWGLy30"),
                new Crate(7, new string[] {"assets/bundled/prefabs/radtown/underwater_labs/crate_ammunition.prefab" }, "crate_ammo_underwater", "Underwater Ammo Crate", "https://i.postimg.cc/D4sHVYf0/ammo-uw0.png",
                ...
                new Crate(8, new string[] {"assets/bundled/prefabs/radtown/underwater_labs/crate_food_1.prefab", "assets/bundled/prefabs/radtown/underwater_labs/crate_food_2.prefab"},
                    "crate_food_underwater", "Underwater Food Crate", "https://i.postimg.cc/sXWBg4qD/food-uw0.png",
                    defaultConfig: "dhtjm8Es"),
                new Crate(9, new string[] {"assets/bundled/prefabs/radtown/underwater_labs/crate_fuel.prefab" }, "crate_fuel_underwater", "Underwater Fuel Crate", "https://i.postimg.cc/0jqzMJ1G/fuel-uw0.png",
                    defaultConfig: "bpgUnZbF"),
                new Crate(10, new string[] {"assets/bundled/prefabs/radtown/underwater_labs/crate_medical.prefab" }, "crate_medical_underwater", "Underwater Medical Crate", "https://i.postimg.cc/Kvf1jzqL/meds-uw0.png",
                    defaultConfig: "0yDE0uSG"),
                new Crate(11, new string[] {"assets/bundled/prefabs/radtown/underwater_labs/crate_tools.prefab" }, "crate_tools_underwater", "Underwater Tool Crate", "https://i.postimg.cc/Pq9pHR6R/crate-tools0.png",
                    defaultConfig: "JgDdqU3w"),
                new Crate(26, new string[] {"assets/bundled/prefabs/radtown/underwater_labs/tech_parts_1.prefab",
                                            "assets/bundled/prefabs/radtown/underwater_labs/tech_parts_2.prefab"}, "crate_techparts_underwater", "Underwater Tech Parts Crate", "https://i.postimg.cc/52GNQFwH/techparts-uw0.png",
                    defaultConfig: "DDjwh69R"),
                new Crate(27, new string[] {"assets/bundled/prefabs/radtown/underwater_labs/vehicle_parts.prefab" }, "crate_vehicle_parts_underwater", "Underwater Vehicle Parts Crate", "https://i.postimg.cc/nzfF0bSp/vp0.png",
                ...
                new Crate(13, new string[] {"assets/bundled/prefabs/radtown/crate_normal_2_food.prefab" }, "crate_food", "Food Crate", "https://i.postimg.cc/NGTTsQv7/crate-food0.png",
                    defaultConfig: "wpKKFfk8"),
                new Crate(14, new string[] {"assets/bundled/prefabs/radtown/crate_normal_2_medical.prefab" }, "crate_medical", "Medical Crate", "https://i.postimg.cc/htCQyjVg/crate-medical0.png",
                    defaultConfig: "Zzmvc8Qv"),
                new Crate(15, new string[] {"assets/bundled/prefabs/radtown/crate_tools.prefab" }, "crate_tools", "Tool Crate", "https://i.postimg.cc/Pq9pHR6R/crate-tools0.png",
                    defaultConfig: "iTpemWGA"),
                new Crate(24, new string[] {"assets/bundled/prefabs/radtown/foodbox.prefab", "assets/bundled/prefabs/autospawn/resource/loot/trash-pile-1.prefab" }, "crate_food_2", "Food Box", "https://i.postimg.cc/VL8t52Yy/crate-food-20.png",
                    defaultConfig: "kPMyegdY"),
                new Crate(25, new string[] {"assets/bundled/prefabs/radtown/minecart.prefab" }, "crate_minecart", "Minecart", "https://i.postimg.cc/yYLNBGxy/minecart0.png",
                    defaultConfig: "r6Cr3kFW"),
                new Crate(28, new string[] {"assets/bundled/prefabs/radtown/vehicle_parts.prefab" }, "crate_vehicle_parts", "Vehicle Parts Crate", "https://i.postimg.cc/nzfF0bSp/vp0.png",
                    defaultConfig: "0BWnWbqK"),

                // Diving
                new Crate(16, new string[] {"assets/bundled/prefabs/radtown/crate_underwater_basic.prefab" }, "crate_underwater_basic", "Basic Underwater Crate", "https://i.postimg.cc/wvRNspyz/crate-basic0.png",
                    defaultConfig: "mvsKr0WJ"),
                new Crate(17, new string[] {"assets/bundled/prefabs/radtown/crate_underwater_advanced.prefab" }, "crate_underwater_advanced", "Advanced Underwater Crate", "https://i.postimg.cc/r073PLk7/adv-underwater0.png",
                    defaultConfig: "ntn5ntkr"),

                // Special
                new Crate(18, new string[] {"assets/prefabs/deployable/chinooklockedcrate/codelockedhackablecrate.prefab" }, "crate_hackable", "Hackable Crate", "https://i.postimg.cc/QxJT7StF/crate-hackable0.png", 36, 18,
                    defaultConfig: "h8gAqQNt"),
                new Crate(19, new string[] {"assets/prefabs/deployable/chinooklockedcrate/codelockedhackablecrate_oilrig.prefab" }, "crate_hackable_oilrig", "Oilrig Hackable Crate", "https://i.postimg.cc/QxJT7StF/crate-hackable0.png", 36, 18,
                    defaultConfig: "zMNfjt34"),
                new Crate(20, new string[] {"assets/prefabs/npc/m2bradley/bradley_crate.prefab" }, "crate_apc", "APC Crate", "https://i.postimg.cc/wMhs0QQb/crate-t30.png", defaultSlots: 12,
                    defaultConfig: "yr6HcY3M"),
                new Crate(21, new string[] {"assets/prefabs/npc/patrol helicopter/heli_crate.prefab" }, "crate_heli", "Helicopter Crate", "https://i.postimg.cc/59wYcp8s/dm-t20.png",
                    defaultConfig: "yjauqcKJ"),
                new Crate(39, new string[] {"assets/prefabs/misc/supply drop/supply_drop.prefab" }, "crate_supplydrop", "Supply Drop", "https://i.postimg.cc/sg1g2JZd/spl0.png", 18, 18,
                ...
                new Crate(22, new string[] {"assets/bundled/prefabs/radtown/loot_barrel_1.prefab","assets/bundled/prefabs/autospawn/resource/loot/loot-barrel-2.prefab",
                                            ...
                                            "barrel", "Loot Barrel", "https://i.postimg.cc/t49Fvwfd/barrel0.png", defaultConfig: "u3Sqt4DZ"),
                new Crate(23, new string[] {"assets/bundled/prefabs/radtown/oil_barrel.prefab" }, "barrel_oil", "Oil Barrel", "https://i.postimg.cc/8cWLxSN4/barrel-oil0.png",
                ...
                // Deathmatch
                new Crate(29, new string[] {"assets/bundled/prefabs/radtown/dmloot/dm ammo.prefab" }, "dm_ammo", "DM Ammo Crate", "https://i.postimg.cc/wB7ySpkN/dm-ammo0.png"),
                new Crate(30, new string[] {"assets/bundled/prefabs/radtown/dmloot/dm c4.prefab" }, "dm_c4", "DM C4 Crate", "https://i.postimg.cc/fRbJ9NpP/dm-c40.png"),
                ...
                new Crate(33, new string[] {"assets/bundled/prefabs/radtown/dmloot/dm food.prefab" }, "dm_food", "DM Food Crate", "https://i.postimg.cc/NGTTsQv7/crate-food0.png"),
                new Crate(34, new string[] {"assets/bundled/prefabs/radtown/dmloot/dm medical.prefab" }, "dm_medical", "DM Medical Crate", "https://i.postimg.cc/htCQyjVg/crate-medical0.png"),
                ...
                new Crate(37, new string[] {"assets/bundled/prefabs/radtown/dmloot/dm tier2 lootbox.prefab" }, "dm_tier2", "DM Tier 2 Crate", "https://i.postimg.cc/59wYcp8s/dm-t20.png"),
                new Crate(38, new string[] {"assets/bundled/prefabs/radtown/dmloot/dm tier3 lootbox.prefab" }, "dm_tier3", "DM Tier 3 Crate", "https://i.postimg.cc/wMhs0QQb/crate-t30.png"),
                ...
                new Crate(97, "assets/content/vehicles/trains/wagons/subents/wagon_crate_normal_2.prefab", "wagon_crate_normal", "Wagon Crate", "https://files.facepunch.com/rust/item/cratecostume_512.png",
                    defaultConfig: "iJV3q75X"),
                new Crate(98, "assets/content/vehicles/trains/wagons/subents/wagon_crate_normal.prefab", "wagon_crate_military", "Military Wagon Crate","https://i.postimg.cc/59wYcp8s/dm-t20.png",
                    defaultConfig: "wNwBm3rG"),
                new Crate(99, "assets/content/vehicles/trains/wagons/subents/wagon_crate_normal_2_food.prefab", "wagon_crate_food", "Food Wagon Crate", "https://i.postimg.cc/NGTTsQv7/crate-food0.png",
                    defaultConfig: "wpKKFfk8"),
                new Crate(100, "assets/content/vehicles/trains/wagons/subents/wagon_crate_normal_2_medical.prefab", "wagon_crate_medical", "Medical Wagon Crate", "https://i.postimg.cc/htCQyjVg/crate-medical0.png",
                    defaultConfig: "Zzmvc8Qv"),
            };

            private static readonly List<Quarry> quarryList = new List<Quarry>
            {
                new Quarry(41, Quarry.Type.Excavator, new int[]{ -2099697608, -1157596551, 69511070, -1982036270}, "quarry_excavator", "Giant Excavator", "https://i.postimg.cc/QxhBj5bh/excv0.png", LootableType.Excavator),
            ...
            private static readonly List<Collectible> collectibleList = new List<Collectible>
            {
                new Collectible(62, new string[]{"assets/content/structures/excavator/prefabs/diesel_collectable.prefab"}, new ItemAmount[]{
                    new ItemAmount(ItemManager.FindItemDefinition("diesel_barrel"), 1)}, "collectible_diesel", "Diesel Barrel", "https://i.postimg.cc/YC2mzgGb/diesel0.png"),
                new Collectible(63, new string[]{"assets/bundled/prefabs/autospawn/collectable/wood/wood-collectable.prefab", "assets/bundled/prefabs/autospawn/collectable/stone/halloween/halloween-wood-collectable.prefab"},
                    new ItemAmount[]{ new ItemAmount(ItemManager.FindItemDefinition("wood"), 50)}, "collectible_wood", "Collectable Wood", "https://i.postimg.cc/RF1MYs2f/wood-coll0.png"),
                new Collectible(64, new string[]{ "assets/bundled/prefabs/autospawn/collectable/stone/stone-collectable.prefab", "assets/bundled/prefabs/autospawn/collectable/stone/halloween/halloween-stone-collectable.prefab"},
                    new ItemAmount[]{ new ItemAmount(ItemManager.FindItemDefinition("stones"), 50)}, "collectible_stone", "Collectable Stone", "https://i.postimg.cc/7L2P3YVK/stone-coll0.png"),
                new Collectible(65, new string[]{ "assets/bundled/prefabs/autospawn/collectable/stone/sulfur-collectable.prefab", "assets/bundled/prefabs/autospawn/collectable/stone/halloween/halloween-sulfur-collectible.prefab"},
                    new ItemAmount[]{ new ItemAmount(ItemManager.FindItemDefinition("sulfur.ore"), 50)}, "collectible_sulfur", "Collectable Sulfur", "https://i.postimg.cc/gkz0Cpz0/sulfur-ore0.png"),
                new Collectible(66, new string[]{ "assets/bundled/prefabs/autospawn/collectable/stone/halloween/halloween-metal-collectable.prefab", "assets/bundled/prefabs/autospawn/collectable/stone/metal-collectable.prefab"},
                    new ItemAmount[]{ new ItemAmount(ItemManager.FindItemDefinition("metal.ore"), 50)}, "collectible_metal", "Collectable Metal", "https://i.postimg.cc/nrYCvcny/metal-ore0.png"),
                new Collectible(67, new string[]{ "assets/bundled/prefabs/autospawn/collectable/pumpkin/pumpkin-collectable.prefab"}, new ItemAmount[]{
                    new ItemAmount(ItemManager.FindItemDefinition("pumpkin"), 1), new ItemAmount(ItemManager.FindItemDefinition("seed.pumpkin"), 1)}, "collectible_pumpkin", "Collectable Pumpkin", "https://i.postimg.cc/3wsRVDNM/pumpkin-coll.png"),
                new Collectible(68, new string[]{ "assets/bundled/prefabs/autospawn/collectable/potato/potato-collectable.prefab"}, new ItemAmount[]{
                    new ItemAmount(ItemManager.FindItemDefinition("potato"), 1), new ItemAmount(ItemManager.FindItemDefinition("seed.potato"), 1)}, "collectible_potato", "Collectable Potato", "https://i.postimg.cc/hGzv20Sb/potato-coll.png"),
                ...
                new Collectible(70, new string[]{ "assets/bundled/prefabs/autospawn/collectable/hemp/hemp-collectable.prefab"}, new ItemAmount[]{
                    new ItemAmount(ItemManager.FindItemDefinition("cloth"), 10), new ItemAmount(ItemManager.FindItemDefinition("seed.hemp"), 1)}, "collectible_hemp", "Collectable Hemp", "https://i.postimg.cc/Z5D0PQh5/hemp-coll0.png"),
                ...
                new Collectible(72, new string[]{ "assets/bundled/prefabs/autospawn/collectable/berry-yellow/berry-yellow-collectable.prefab"}, new ItemAmount[]{
                    new ItemAmount(ItemManager.FindItemDefinition("yellow.berry"), 1), new ItemAmount(ItemManager.FindItemDefinition("seed.yellow.berry"), 1)}, "collectible_berry_yellow", "Collectable Yellow Berry", "https://i.postimg.cc/nr8D4PBD/berry-yellow0.png"),
                new Collectible(73, new string[]{ "assets/bundled/prefabs/autospawn/collectable/berry-white/berry-white-collectable.prefab"}, new ItemAmount[]{
                    new ItemAmount(ItemManager.FindItemDefinition("white.berry"), 1), new ItemAmount(ItemManager.FindItemDefinition("seed.white.berry"), 1)}, "collectible_berry_white", "Collectable White Berry", "https://i.postimg.cc/wjFLCF8C/berry-white0.png"),
                new Collectible(74, new string[]{ "assets/bundled/prefabs/autospawn/collectable/berry-red/berry-red-collectable.prefab"}, new ItemAmount[]{
                    new ItemAmount(ItemManager.FindItemDefinition("red.berry"), 1), new ItemAmount(ItemManager.FindItemDefinition("seed.red.berry"), 1)}, "collectible_berry_red", "Collectable Red Berry", "https://i.postimg.cc/sDYpcsnD/berry-red0.png"),
                new Collectible(75, new string[]{ "assets/bundled/prefabs/autospawn/collectable/berry-green/berry-green-collectable.prefab"}, new ItemAmount[]{
                    new ItemAmount(ItemManager.FindItemDefinition("green.berry"), 1), new ItemAmount(ItemManager.FindItemDefinition("seed.green.berry"), 1)}, "collectible_berry_green", "Collectable Green Berry", "https://i.postimg.cc/wT8XL7QG/berry-green0.png"),
                new Collectible(76, new string[]{ "assets/bundled/prefabs/autospawn/collectable/berry-blue/berry-blue-collectable.prefab"}, new ItemAmount[]{
                    new ItemAmount(ItemManager.FindItemDefinition("blue.berry"), 1), new ItemAmount(ItemManager.FindItemDefinition("seed.blue.berry"), 1)}, "collectible_berry_blue", "Collectable Blue Berry", "https://i.postimg.cc/tgMPTTgD/berry-blue0.png"),
                new Collectible(77, new string[]{ "assets/bundled/prefabs/autospawn/collectable/berry-black/berry-black-collectable.prefab"}, new ItemAmount[]{
                    new ItemAmount(ItemManager.FindItemDefinition("black.berry"), 1), new ItemAmount(ItemManager.FindItemDefinition("seed.black.berry"), 1)}, "collectible_berry_black", "Collectable Black Berry", "https://i.postimg.cc/q7Ry4npz/berry-black0.png"),
            };
            ...
            public static void Initialize()
            {
                ...
                // Populate npcConfigLookup
                CPrint("Loading NPC spawn points ...");
                spawnPointPrefabIds.Clear();
                npcConfigLookup.Clear();
                foreach (var npc in npcList)
                {
                    foreach (var prefabId in npc.prefabIds)
                    {
                        var sp = new NpcSpawnPointId(npc.spawnPointPrefabId, prefabId);
                        npcConfigLookup.Add(sp, npc.id);
                        if (npc.spawnPointPrefabId > 0)
                        {
                            spawnPointPrefabIds.Add(npc.spawnPointPrefabId);
                            //DPrint($"Cached npc with custom spawn point {sp}");
                        }
                    }
                }
            }
            ...
            {
                // Remove npc from dict to prevent dict from growing
                if (npcCache.Remove(corpse.playerSteamID, out var lootableId))
                {
                    return crateConfigCache[lootableId];
                }
            ...
            public static void PopulateNpcCache()
            {
            ...
            private static readonly Dictionary<NpcSpawnPointId, int> npcConfigLookup = new();
            private static readonly HashSet<uint> spawnPointPrefabIds = new();

            private static NpcSpawnPointId GetSpawnPointId(BasePlayer npc)
            {
                string spawnPointPrefab = null;
                try
                {
                ...
                {
                    CErr($"Failed to get spawn point prefab for {npc?.ShortPrefabName} at {npc?.transform?.position}: {ex}");
                }

                var spawnPointPrefabId = spawnPointPrefab != null ? StringPool.Get(spawnPointPrefab) : 0;
                if (spawnPointPrefabIds.Contains(spawnPointPrefabId))
                {
            ...
            private readonly struct NpcSpawnPointId
            {
                public readonly uint spawnPointPrefabId;
                public readonly uint prefabId;

                public NpcSpawnPointId(uint spawnPointPrefabId, uint prefabId)
                {
                    this.spawnPointPrefabId = spawnPointPrefabId;
                    this.prefabId = prefabId;
                }

                public override bool Equals(object obj)
                {
                    return obj is NpcSpawnPointId id &&
                           spawnPointPrefabId == id.spawnPointPrefabId &&
                           prefabId == id.prefabId;
                }

                public override int GetHashCode()
                {
                    return HashCode.Combine(spawnPointPrefabId, prefabId);
                }

                public override string ToString()
                {
                    return $"{(spawnPointPrefabId > 0 ? StringPool.Get(spawnPointPrefabId) : "0")} {StringPool.Get(prefabId)}";
                }
            }
            

            public static void CacheNpc(BasePlayer npc)
            {
                var spawnPointId = GetSpawnPointId(npc);

                if (npcConfigLookup.TryGetValue(spawnPointId, out var configId))
                {
                    npcCache[npc.userID] = configId;
                }
                else
                {
                    //DPrint($"Failed to cache NPC with id {spawnPointId}");
                }
            }
        ...
        public class LootableConfig : IMultiplyable
        {
            ...
            public void ApplyToCrate(LootContainer container, bool clear = false, bool cover = false)
            {
                Apply(container.inventory, clear, cover, false, false);
                var scrapAmount = 0;
                foreach (var item in container.inventory.itemList)
                {
                    if (item.info.itemid == -932201673)
                    {
        ...
        private class Collectible : BaseLootable
        {
            ...
            public Collectible(int id, string[] prefabNames, ItemAmount[] vanillaDrop, string saveName, string displayName, string image)
            {
        ...
        private class NpcCorpse : BaseLootable
        {
            ...
            public uint spawnPointPrefabId;

            public const int defaultSlots = 24;

            public NpcCorpse(int id, HashSet<uint> prefabIds, string spawnPointPrefab, string saveName, string displayName, string image = "https://files.facepunch.com/rust/item/hazmatsuit_scientist_512.png", string defaultConfig = null)
            {
                ...
                {
                    this.spawnPointPrefabId = StringPool.Get(spawnPointPrefab);
                }
                this.saveName = saveName;
                this.displayName = displayName;
                this.image = image;
            ...
            public NpcCorpse(int id, uint prefabId, string spawnPointPrefab, string saveName, string displayName, string image = "https://files.facepunch.com/rust/item/hazmatsuit_scientist_512.png", string defaultConfig = null)
            {
                this.type = LootManager.LootableType.NpcCorpse;
                this.id = id;
