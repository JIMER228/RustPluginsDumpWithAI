...
1.1.11
 - Fixed: For 7th Sep 2023 Rust update
...
1.1.8
 - Fixed: Final fix for language defaulting to English (hopefully)
...
1.1.5
 - Fixed: "KeyNotFoundException" error when clear cooldown command used
 - Fixed: Bug which prevented players looting own boxes after crate lock expired
 - Fixed: Bug which prevented harvesting gibs when unlock gibs timer expired
 - Fixed: Retire command retiring other peoples helis in some cases in error
...
{
    [Info("Heli Signals", "ZEODE", "1.2.16")]
    [Description("Call Patrol Helicopters to your location with custom supply signals.")]
    public class HeliSignals: RustPlugin
    {
        #region Plugin References
        
        [PluginReference] Plugin Friends, Clans, FancyDrop, RewardPlugin, Vanish, NoEscape, BotReSpawn, DynamicPVP, XPerience, SkillTree, NpcSpawn;
        ...
        private const string heliExplosion = "assets/prefabs/npc/patrol helicopter/effects/heli_explosion.prefab";
        ...
        protected override void LoadDefaultMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                ["DmgReport"] = "<color=orange>Damage Report</color> / <color=orange>Rotor Accuracy</color>\n\n{0}\n{1}",
                ["DmgReportOwner"] = "<size=11>Type: <color=orange>{0}</color> Owner: <color=green>{1}</color> Status: <color=red>{2}</color>\n</size>",
                ["DmgReportIndex"] = "<size=11>{0}. <color=green>{1}</color> -> {2} HP / {3}%\n</size>",
                ...
                ["HeliRetired"] = "<color=green>{0}</color> is low on fuel and is breaking off engagementg to return to base.",
        ...
        private void Init()
        {
            ...
            foreach (var key in config.heli.heliConfig.Keys)
            {
                ...
                HeliSkins.Add(config.heli.heliConfig[key].SignalSkinID);
            }
            ...
            foreach (var key in config.heli.waveConfig.Keys)
            {
                ...
                HeliSkins.Add(config.heli.waveConfig[key].SkinId);

                if (config.heli.waveConfig[key].WaveProfiles.Count == 0)
                {
                    // If loading fresh, populate default wav profiles
        ...
        private void OnItemAddedToContainer(ItemContainer container, Item item)
        {
            if (item.info.itemid == supplySignalId)
                if (HeliSkins.Contains(item.skin)) NextTick(()=> FixSignalInfo(item));
        }
        ...
        private void OnExplosiveThrown(BasePlayer player, SupplySignal entity, ThrownWeapon item)
        {
            ...
            if (HeliSkins.Contains(signal.skin))
            {
        ...
        private object OnEntityTakeDamage(BaseCombatEntity entity, HitInfo info)
        {
            ...
            if (Initiator is PatrolHelicopter)
            {
                ...
                if (heliProfile == null)
                    return null;
                else if (heliProfile.Contains("patrolhelicopter"))
                    return null;
                ...
                if (config.heli.heliConfig.ContainsKey(heliProfile))
                {
                    if (entity is BasePlayer)
                    {
                        ...
                        if (config.heli.heliConfig[heliProfile].BlockPlayerDamage && !IsOwnerOrFriend(heliOwnerId, player.userID))
                        {
                        ...
                        else if (info.damageTypes.GetMajorityDamageType() == DamageType.Bullet)
                        {
                            ...
                            if (config.heli.heliConfig[heliProfile].BulletAccuracy < hitProb)
                            {
                                info?.damageTypes.ScaleAll(0);
                                return true;
                            }
                    ...
                    {
                        if (config.heli.heliConfig[heliProfile].BlockProtectedList && config.heli.protectedPrefabs.Contains(entity.name))
                        {
                            info?.damageTypes.ScaleAll(0);
                            return true;
                        }
                        else if (config.heli.heliConfig[heliProfile].BlockOtherDamage && !IsOwnerOrFriend(heliOwnerId, ownerId))
                        {
                            info?.damageTypes.ScaleAll(0);
                            return true;
                        }
                        else if (config.heli.heliConfig[heliProfile].BlockOwnerDamage && IsOwnerOrFriend(heliOwnerId, ownerId))
                        {
                            info?.damageTypes.ScaleAll(0);
                            return true;
                        }
            ...
            else if (Initiator.ShortPrefabName.Equals("napalm"))
            {
                ...
                else if (ownerId.IsSteamId())
                {
                    ...
                    else if (config.heli.heliConfig[heliProfile].BlockOtherDamage && !IsOwnerOrFriend(heliOwner, ownerId))
                    {
                        info?.damageTypes.ScaleAll(0);
                        return true;
                    }
                    else if (config.heli.heliConfig[heliProfile].BlockOwnerDamage && IsOwnerOrFriend(heliOwner, ownerId))
                    {
                        info?.damageTypes.ScaleAll(0);
                        return true;
                    }
        ...
        private object OnEntityTakeDamage(PatrolHelicopter heli, HitInfo info)
        {
            ...
            if (!isVanilla)
            {
                ...
                if (config.heli.heliConfig.ContainsKey(heliProfile))
                {
                    if ((Initiator is AutoTurret) && !config.heli.allowTurretDamage)
                    {
                        info?.damageTypes.ScaleAll(0);
            ...
            HeliSignalData[heliId].Attackers[attackerId].DamageDealt += info.damageTypes.Total();
        ...
        private object OnEntityKill(PatrolHelicopter heli)
        {
            ...
            if (config.announce.reportChat)
            {
                if (!isVanilla || (isVanilla && config.announce.reportVanilla))
                {
                    ...
                    string ownerReport = string.Format(lang.GetMessage("DmgReportOwner", this, lastAttacker.UserIDString), heliDisplayName, heliOwnerName, "Killed");
                    int count = 1;
                    foreach (var key in HeliSignalData[heliId].Attackers.Keys.OrderByDescending(x => HeliSignalData[heliId].Attackers[x].DamageDealt))
                    {
                        ...
                        topReport += string.Format(lang.GetMessage("DmgReportIndex", this, lastAttacker.UserIDString), count, playerName, Math.Round(damageDealt, 2), Math.Round(rotorAccuracy, 2));
                        count++;
                    }
        ...
        private object CanHelicopterStrafe(PatrolHelicopterAI heliAI)
        {
            ...
            if (heliComp != null)
            {
                if (heliComp.isRetiring || heliComp.isDying)
                    ...
                    return false;
                else if (UnityEngine.Time.realtimeSinceStartup - heliAI.lastStrafeTime < UnityEngine.Random.Range(15f, 25f))
                    return false;
                else
                    return true;
            }
        ...
        {
            var heliProfile = heliAI.helicopterBase._name;
            if (heliProfile == null) return null;
            if (Oxide.Core.Random.Range(0f, 1f) <= config.heli.heliConfig[heliProfile].OrbitStrafeChance)
            {
                heliAI.ExitCurrentState();
                heliAI.strafe_target = strafeTarget;
                heliAI.strafe_target_position = strafePosition;
                heliAI.lastStrafeTime = UnityEngine.Time.realtimeSinceStartup;
        ...
        private void HeliSignalThrown(BasePlayer player, SupplySignal entity, Item signal)
        {
            ...
            if (config.heli.heliConfig.Keys.Contains(heliProfile))
            {
            ...
            else if (config.heli.waveConfig.Keys.Contains(heliProfile))
            {
                ...
                {
                    foreach (var item in config.heli.waveConfig[heliProfile].WaveProfiles)
                    {
                        if (!config.heli.heliConfig.Keys.Contains(item))
                        {
                            PrintError($"ERROR: WaveHeli config contains a profile with an incorrect name ({item}) please correct!");
            ...
            if (config.heli.playerCooldown > 0f && !permission.UserHasPermission(player.UserIDString, permBypasscooldown))
            {
                ...
                else
                {
                    ...
                    if (!TierCooldowns[userId].ContainsKey(heliProfile))
                        TierCooldowns[userId].Add(heliProfile, UnityEngine.Time.time + cooldown);
                    else
                        TierCooldowns[userId][heliProfile] = UnityEngine.Time.time + cooldown;
        ...
        private void SpawnHeliCrateLoot(LootContainer lootContainer, string heliProfile)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (UnityEngine.Random.Range(0f, 101f) <= lootItem.BlueprintChance && itemDef.Blueprint != null && IsBP(itemDef) && bps < config.heli.heliConfig[heliProfile].Loot.MaxBP)
                {
        ...
        private void AddExtraHeliCrateLoot (LootContainer lootContainer, string heliProfile)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (lootItem.Chance < UnityEngine.Random.Range(0f, 101f))
                {
                    if (given < minItems)
                    {
                        i--;
                ...
                if (UnityEngine.Random.Range(0f, 101f) <= lootItem.BlueprintChance && IsBP(itemDef) && bps < config.heli.heliConfig[heliProfile].ExtraLoot.MaxBP)
                {
                    ItemDefinition bpDef = ItemManager.FindItemDefinition("blueprintbase");
                    item = ItemManager.Create(bpDef, 1, 0uL);
                    item.blueprintTarget = itemDef.itemid;
        ...
        private void SpawnLockedCrateLoot(LootContainer lootContainer, string heliProfile)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (UnityEngine.Random.Range(0f, 101f) <= lootItem.BlueprintChance && IsBP(itemDef) && bps <= config.heli.heliConfig[heliProfile].LockedCrateLoot.MaxBP)
                {
                    ItemDefinition bpDef = ItemManager.FindItemDefinition("blueprintbase");
                    item = ItemManager.Create(bpDef, 1, 0uL);
                    item.blueprintTarget = itemDef.itemid;
        ...
        private void SpawnLockedCrates(ulong ownerId, ulong skinId, string heliProfile, Vector3 position)
        {
            ...
            {
                Vector2 rand;
                rand = UnityEngine.Random.insideUnitCircle * 5f;
        ...
        object IsHeliSignalObject(ulong skinId) => HeliSkins.Contains(skinId) ? true : (object)null;
        ...
        object ShouldFancyDrop(NetworkableId netId)
        {
            ...
            if (signal != null)
            {
                if (HeliSkins.Contains(signal.skinID))
                    return true;
            }
            return null;
        }
        ...
        {
            return HeliSkins.Contains(lootContainer.skinID) ? true : (object)null;
        }
        ...
        private object OnCreateDynamicPVP(string eventName, BaseEntity entity)
        {
            ...
            {
                if (HeliSignalData.ContainsKey(entity.net.ID.Value) && HeliSkins.Contains(entity.skinID))
                    return true;
            }
        ...
        private object FixSignalInfo(Item signal)
        {
            ...
            foreach (var key in config.heli.waveConfig.Keys)
            {
                if (signal.skin == config.heli.waveConfig[key].SkinId)
                {
                    signal.name = key;
                    signal.skin = config.heli.waveConfig[key].SkinId;
                    return signal;
                }
            }
            return signal;
        ...
        private class HeliSignalComponent : MonoBehaviour
        {
            ...
            void CustomExplode()
            {
                ...
                else
                {
                    ...
                    var WaveTime = UnityEngine.Time.realtimeSinceStartup;
                    ...
                    for (int i = 0; i < heliAmount; i++)
                    {
                        ...
                        Instance.NextTick(() =>
                        {
                            ...
                            // DEBUG: keep an eye on this, see if this helps prevent the error when fast heli glitches under terrain
                            heliAI.terrainPushForce = 75f; // default = 50f
                            heliAI.obstaclePushForce = 30f; // default = 20f
                            heliAI.oceanDepthTargetCutoff = 8f; // default = 3
                            // ##########################
        ...
        private class HeliComponent : MonoBehaviour
        {
            ...
            void Start()
            {
                ...
                GetCallingTeam();
                SetupHeli();
                InvokeRepeating(nameof(PositionCheck), 1f, 0.25f);
                InvokeRepeating(nameof(UpdateTargetList), 2f, 1f);
                InvokeRepeating(nameof(CheckHeliDeath), 2f, 0.5f);
            }
            ...
            public void CallNextWaveHeli(string waveHeliProfile)
            {
                ...
                float WaveTime = UnityEngine.Time.realtimeSinceStartup;
                ...
                for (int i = 0; i < config.heli.heliConfig[waveHeliProfile].HeliAmount; i++)
                {
                    ...
                    Instance.NextTick(() =>
                    {
                        ...
                        // debug keep an eye on this, see if this helps prevent the error when fast heli glitches under terrain
                        newHeliAI.terrainPushForce = 75f;
                        newHeliAI.obstaclePushForce = 30f;
                        newHeliAI.oceanDepthTargetCutoff = 8f;
                        // #
...
#else
                            FieldInfo maxOrbitDuration = typeof(PatrolHelicopterAI).GetField("maxOrbitDuration", (BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public));
                            if (maxOrbitDuration == null) return;
            ...
            public void UpdateTargetList()
            {
                ...
                if (heliAI._currentState == PatrolHelicopterAI.aiState.DEATH && !isDying)
                {
                    // Give rewards as soon as heli enters death state, otherwise if
                    // players die before it crashes they might not get rewards
                    /*Instance.NextTick(()=> Instance.ProcessRewards(heliId, owner.userID, heliProfile));
                            ...
                            if (timeSinceSeen >= 5f && player.IsAlive())
                            {
                                if (heliAI.CanStrafe() && heliAI.IsAlive() && !isStrafing && !player.IsDead())
                                {
                                    if (player == heliAI.leftGun._target || player == heliAI.rightGun._target)
                                    {
                                        ...
                                        strafeTarget = player; // debug changed for rust update, check this when home
                                    }
                                }
                            }
                        }
                ...
                if (isStrafing)
                {
                    ...
                    heliAI.lastStrafeTime = lastStrafeTime;
                    heliAI.strafe_target = strafeTarget;
                    heliAI.strafe_target_position = strafePosition;
                    heliAI.State_Strafe_Enter(strafeTarget, UseNapalm()); // debug changed for rust update, check this
                    InvokeRepeating(nameof(StrafeThink), 1f, 0.25f);
                }
            }
            ...
            public void CheckHeliDeath()
            {
                ...
                switch (heliAI._currentState)
                {
                    case PatrolHelicopterAI.aiState.DEATH:
                    {
                        // Give rewards as soon as heli enters death state, otherwise if
                        // players die before it crashes they might not get rewards
                        // Also moved from OnEntityTakeDamage to fix bug where if heli
                        // entered death state and wasn't shot while crashing, rewards not given
                        isDying = true;
                        Instance.NextTick(()=> Instance.ProcessRewards(heliId, owner.userID, heliProfile));
                        Instance.timer.Once(11f, ()=>
                        {
                            if (heli != null)
                                heli.Hurt(heli.health * 2f, DamageType.Generic, null, false); // debug: Kill heli in case it isn't already
                        });
                        CancelInvoke(nameof(CheckHeliDeath));
                        return;
                    }
                    default:
                    {
            ...
            public void StrafeThink()
            {
                ...
                switch (heliAI._currentState)
                {
                    case PatrolHelicopterAI.aiState.STRAFE:
                    {
                        ...
                        UpdateSerializeableFields(config.heli.heliConfig[heliProfile].MaxHeliRockets);
                        return;
                    }
                    case PatrolHelicopterAI.aiState.ORBITSTRAFE:
                    {
                        ...
                        if (orbitRocketAmount == 0)
                        {
                            var rocketVariance = UnityEngine.Random.Range(config.heli.heliConfig[heliProfile].MinOrbitMultiplier,
                                                                          config.heli.heliConfig[heliProfile].MaxOrbitMultiplier);
                            orbitRocketAmount = config.heli.heliConfig[heliProfile].MaxOrbitRockets + rocketVariance;
                        }

                        UpdateSerializeableFields(orbitRocketAmount);
                        return;
                    }
                    default:
                    {
            ...
            public void ReturnHeliToPlayer()
            {
                ...
                if (target == null || !target.IsConnected || !target.IsAlive() || target.IsSleeping() || config.heli.returnToPosition)
                {
                ...
                if (Vector3Ex.Distance2D(heliAI.transform.position, returnPosition) > config.heli.maxDistanceFromPlayer)
                {
                    ...
                    InvokeRepeating(nameof(PositionCheck), 0.25f, 0.25f);
                }
            }
            ...
            private void OnDestroy()
            {
                ...
                if (!this.isDying || this.isRetiring)
                {
                    return;
                }
                else if (this.isWaveHeli && this.waveProfileCache.Count > 0)
                {
                    float waveTime = 0;
        ...
        private static HashSet<ulong> HeliSkins = new HashSet<ulong>();
