...
{
    [Info("WaterPatrol", "KpucTaJl", "1.0.7")]
    internal class WaterPatrol : RustPlugin
    {
        ...
        private void UpdateConfigValues()
        {
            ...
            if (_config.PluginVersion < new VersionNumber(1, 0, 3))
            {
                foreach (BoatConfig boat in _config.Boats)
                    boat.Positions = new List<string>();
            }
            _config.PluginVersion = Version;
        ...
        public class BoatConfig
        {
            ...
            [JsonProperty(En ? "List of positions for spawn on the map" : "Список позиций для появления на карте")] public List<string> Positions { get; set; }
        }
        ...
        private class PluginConfig
        {
            ...
            [JsonProperty(En ? "Do patrol boats take damage? [true/false]" : "Возможно ли наносить урон патрульной лодке? [true/false]")] public bool ApplyDamageBoat { get; set; }
        ...
        private IEnumerator SpawnAllBoat()
        {
            for (int i = 0; i < _config.Boats.Count; i++)
            {
                BoatConfig config = _config.Boats[i];

                for (int j = config.Positions.Count - 1; j >= 0; j--)
                {
                    string _pos = config.Positions[j];
                    Vector3 pos = GetNearPoint(_pos.ToVector3());
                    config.Positions.Remove(_pos);
                    if (pos != Vector3.zero) config.Positions.Add(pos.ToString());
        ...
        {
            Vector3 pos = GetConfigSpawnPos(index);

            RHIB rhib = GameManager.server.CreateEntity("assets/content/vehicles/boats/rhib/rhib.prefab", pos == Vector3.zero ? GetRandomNavMeshPos() : pos, Quaternion.identity) as RHIB;
        ...
        internal class ControllerRHIB : FacepunchBehaviour
        {
            ...
            internal void InitBoat(int index)
            {
                ...
                foreach (BaseEntity entity in Rhib.children)
                {
                    ...
                    else if (entity.ShortPrefabName == "rhibdriver")
                    {
            ...
            private void FixedUpdate()
            {
                ...
                if (Rhib.buoyancy.submergedFraction < 0.35f || NearShip)
                {
                    _timeKill += Time.deltaTime;
                    if (_timeKill > _ins._config.TimeKillGround) _ins.CrashBoat(this, false);
                    return;
                }
            ...
            private static JObject GetObjectConfigDriver(DriverConfig config)
            {
            ...
            private bool NearShip => _ins.Ships.Any(x => x.IsExists() && Vector3.Distance(x.transform.position, transform.position) < 60f);
        }
        ...
        internal void CrashBoat(ControllerRHIB controller, bool isSpawnLoot)
        {
            if (isSpawnLoot) controller.SpawnLoot();
        ...
        private void OnServerInitialized()
        {
            ...
            _coroutine = ServerMgr.Instance.StartCoroutine(SpawnAllBoat());
        }
        ...
        private void Unload()
        {
            ...
            foreach (KeyValuePair<ulong, RHIB> dic in _deathBoats) if (dic.Value.IsExists()) dic.Value.Kill();
            _ins = null;
        }
        ...
        private object OnEntityTakeDamage(RHIB rhib, HitInfo info)
        {
            ...
            if (_config.ApplyDamageBoat && info.InitiatorPlayer.IsPlayer()) return null;
            if (_controllers.Any(x => x.Rhib == rhib)) return true;
            else return null;
        }

        private void OnEntityDeath(RHIB rhib, HitInfo info)
        {
            if (rhib == null) return;
            ControllerRHIB controller = _controllers.FirstOrDefault(x => x.Rhib == rhib);
            if (controller != null) CrashBoat(controller, true);
        }
                ...
                else
                {
                    ...
                    if (controller.Passengers.Count == 0)
                    {
                        ...
                        else
                        {
                            ...
                            _deathBoats.Add(controller.Crate.net.ID.Value, controller.Rhib);
                            _controllers.Remove(controller);
                            BoatConfig config = _config.Boats[controller.IndexConfig];
                            int amountChance = UnityEngine.Random.Range(config.MinAmount, config.MaxAmount + 1);
                            int amountInWorld = _controllers.Where(x => x.IndexConfig == controller.IndexConfig).Count;
        ...
        private readonly Dictionary<ulong, RHIB> _deathBoats = new Dictionary<ulong, RHIB>();

        private void OnLootEntity(BasePlayer player, StorageContainer container)
        {
            if (!player.IsPlayer() || !container.IsExists()) return;
            RHIB rhib = null;
            if (_deathBoats.TryGetValue(container.net.ID.Value, out rhib))
            {
                _deathBoats.Remove(container.net.ID.Value);
        ...
        private void OnEntitySpawned(CargoShip cargo) { if (cargo != null && !Ships.Contains(cargo)) Ships.Add(cargo); }

        private void OnEntityKill(CargoShip cargo) { if (Ships.Contains(cargo)) Ships.Remove(cargo); }

        internal HashSet<CargoShip> Ships = new HashSet<CargoShip>();
        ...
        private static bool IsValidPosToHeight(Vector3 vector3)
        {
        ...
        private static bool IsValidPointNavMesh(PointNavMesh pointNavMesh)
        {
        ...
        private Vector3 GetConfigSpawnPos(int index)
        {
            BoatConfig config = _config.Boats[index];
            ...
            if (config.Positions.Count > 0)
            {
                ...
                {
                    attempts++;
                    Vector3 pos = config.Positions.GetRandom().ToVector3();
                    if (!_controllers.Any(x => Vector3.Distance(x.transform.position, pos) < 10f)) return pos;
                }
            }
        ...
        #region Alerts
        [PluginReference] private readonly Plugin GUIAnnouncements;
