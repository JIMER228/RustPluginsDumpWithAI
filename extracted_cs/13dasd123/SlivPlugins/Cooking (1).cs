...
/* 2.0.6 Change Log
 * Added config option to wipe NPCs on server wipe.
 * Fixed an issue with the language file.
*/

namespace Oxide.Plugins
{
	[Info("Cooking 2.0", "imthenewguy", "2.0.6")]
	...
	class Cooking : RustPlugin
	{
        ...
        public class IngredientsInfo
        {
            ...
            public IngredientsInfo(bool enabled, string shortname, ulong skin, List<GatherSource> gatherSources, int dropWeight = 100, int minDrops = 1, int maxDrops = 3, int sellToMarketPrice = 5, int BuyFromMarketPrice = 50)
            {
                this.enabled = enabled;
                this.shortname = shortname;
                this.skin = skin;
        ...
        void HandleIngredientChance(BasePlayer player, GatherSource source, bool player_grown, BuffManager buffManager = null, ulong sourceEntityID = 0, float modifier = 1)
        {
            ...
            if (IsValidItemForBag(item) && HasIngredientBag(player)) PlaceIngredientIntoBag(player, item);
            else
            {
                if (DropNotificationsEnabled(player.UserIDString)) PrintToChat(player, lang.GetMessage("IngredientFoundMessage", this, player.UserIDString), item.amount.ToString(), item.name ?? item.info.displayName.english);
        ...
        bool PlaceIngredientIntoBag(BasePlayer player, Item item)
        {
            ...
            foreach (var slot in bagData)
            {
                ...
                if (DropNotificationsEnabled(player.UserIDString)) PrintToChat(player, String.Format(lang.GetMessage("ItemStoredInBag", this, player.UserIDString), item.amount, item.name ?? item.info.displayName.english));
                item.Remove();                
                return true;
            }
            ...
            if (DropNotificationsEnabled(player.UserIDString)) PrintToChat(player, String.Format(lang.GetMessage("ItemStoredInBagNewSlot", this, player.UserIDString), item.amount, item.name ?? item.info.displayName.english));
            item.Remove();
            return true;
        }
        ...
        void SaveIngredientBag(BasePlayer player, StorageContainer container)
        {
            ...
            {
                PrintToChat(player, string.Format(lang.GetMessage("ItemReturnedBlackListed", this, player.UserIDString), item.amount, item.name ?? item.info.displayName.english, config.generalSettings.ingredientBagSettings.bagItemSettings.displayName ?? "ingredient bag"));
                player.GiveItem(item);
            }
        ...
        public class BuffManager : MonoBehaviour
        {
            ...
            public string AddBuff(string meal, Dictionary<Buff, float> buffs, float time, string shortname, ulong skin, Dictionary<string, string> commandsOnConsume, List<string> commandsOnEnd)
            {
                ...
                foreach (var buff in buffs)
                {
                    ...
                    if (!buffMods.ContainsKey(buff.Key)) buffMods.Add(buff.Key, buff.Value);
                    else buffMods[buff.Key] += buff.Value;
        ...
        {
            if (!config.mealSettings.cookbookSettings.enabled) return config.mealSettings.meals;
        ...
        private void SendRecipeDetailedMenu(BasePlayer player, string meal, string firstMealIndex, CoreElmentSelected fromMenu, string quantity = "1")
        {
            ...
            foreach (var ingredient in mealData.ingredients)
            {
                ...
                var hasEnough = ingredientCount >= ingredient.Value * quantity_int;
                container.Add(new CuiElement
                {
                    Name = "ingredients_menu_quantity_1",
                    Parent = "ingredients_menu_img_1",
                    Components = {
                    new CuiTextComponent { Text = String.Format(lang.GetMessage("UIIngredientCount", this, player.UserIDString), (hasEnough ? "<color=#42f105>":null), ingredientCount, ingredient.Value * quantity_int, (hasEnough ? "</color>":null)), Font = "robotocondensed-regular.ttf", FontSize = ingredientCount < 10000 ? 12 : 10, Align = TextAnchor.MiddleCenter, Color = "1 1 1 1" },
                    new CuiOutlineComponent { Color = "0 0 0 0.5", Distance = "1 -1" },
        ...
        private void MealInfoPanel(BasePlayer player, int page = 0)
        {
            ...
            foreach (var meal in foundMeals)
            {
                ...
                if (mealData.useCooldown > 0) durationString += $"\nCooldown: {mealData.useCooldown}";
