...
{
    [Info("UpLifted", "FuJiCuRa", "1.2.26", ResourceId = 110)]
    internal class UpLifted : RustPlugin
    {
        ...
        public class Elevator : FacepunchBehaviour
        {
            ...
            public bool HasLiftAccss(BasePlayer player)
            {
                ...
                if (clansEnabled && (_sharemode == ShareMode.Clan || _sharemode == ShareMode.Full) && SameClanS(ownerID.ToString(), player.UserIDString))
                    return true;

                if (friendsEnabled && (_sharemode == ShareMode.Friends || _sharemode == ShareMode.Full) && HasFriend(ownerID.ToString(), player.UserIDString))
                    return true;
            ...
            public bool HasFloorAccess(BasePlayer player, FloorShare share, int p = 0)
            {
                ...
                if (share == FloorShare.HiddenVIP)
                    if ((bool) GetAccess("AccessVipShare", player.UserIDString, false))
                        return true;
                if (share == FloorShare.Friends || share == FloorShare.Full)
                    if (friendsEnabled && HasFriend(ownerID.ToString(), player.UserIDString))
                        return true;
                if (share == FloorShare.Clan || share == FloorShare.Full)
        ...
        private static object GetAccess(string variable, string userid, object fallbackValue = default(object), bool isActive = true)
        {
            ...
            if (UpL.permissionBlock.TryGetValue(variable, out usagePerm))
            {
                ...
                object value;
                if (perms.TryGetValue(grouptype, out value))
                    return value;
            }
