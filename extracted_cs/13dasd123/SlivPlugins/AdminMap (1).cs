                ...
                switch (args[0])
                {
                    ...
                    case "ss":
                        ...
                        if (adminData.searchQuery != string.Empty)
                        {
                            ...
                            Func<BuildingPrivlidge, bool> func2 = (priv => priv.authorizedPlayers.Any(p => p.username.ToLower().Contains(adminData.searchQuery)));
                ...
                foreach (var admin in cupboardAdmins)
                {
                    using (MapNoteList markers = Facepunch.Pool.Get<MapNoteList>())
                    {
                        ...
                        if (admin.searchQuery != string.Empty)
                            searchResult = privlidges.Where(priv => priv.authorizedPlayers.Any(p => p.username.ToLower().Contains(admin.searchQuery.ToLower())));
