...
{
    [Info("IQAlcoholFarm", "BadMandarin / Support: Mercury", "1.1.28")]
    ...
    internal class IQAlcoholFarm : RustPlugin
    {
        ...
        /// - Запрещено переливать алкоголь в бассейны
        /// - Исправлено NRE после установки торговца
        /// </summary>
        ...
        #region Reference
        [PluginReference] private readonly Plugin TruePVE, NextGenPVE, Imperium, StackModifier;
                ...
                try
                {
                    webrequest.Enqueue($"http://iqsystem.skyplugins.ru/iqalcohol/getsound/W5fgSD3dfVBxc", null, (code, response) =>
                    {
                        switch (code)
                        {
                            case 404:
                                {
                                    PrintError($"ERROR #63445485  {response} | ERROR #:545445  (Discord - Mercury#5212)"); 
                                    _initiated = false;
                                    break;
                                }
                            case 503:
        ...
        {
            if (StackModifier is { IsLoaded: true })
            {
                Unsubscribe(nameof(OnItemSplit));
                Unsubscribe(nameof(CanCombineDroppedItem));
                Unsubscribe(nameof(CanStackItem));
        ...
        private void Unload()
        {
            ...
            if (_data is { DrunkPlayers: not null })
            {
        ...
        private ItemContainer.CanAcceptResult? CanAcceptItem(ItemContainer container, Item item, Int32 targetPos)
        {
            ...
            Item item1 = container.GetSlot(0);
            
            if (container.GetEntityOwner() != null && container.GetEntityOwner() is PaddlingPool)
            {
                if(item.skin != 0)
                    return ItemContainer.CanAcceptResult.CannotAccept; 
            }
            
            if (item1 != null && item.info.shortname.Equals("water") && item1.info.shortname.Equals("water") &&
                item1.skin != 0 && item.skin == 0)
            {
                return ItemContainer.CanAcceptResult.CannotAccept;
            }
            
            if (item1 != null && item.info.shortname.Equals("water.salt") && item1.info.shortname.Equals("water.salt") &&
                item1.skin != 0 && item.skin == 0)
            {
                return ItemContainer.CanAcceptResult.CannotAccept;
            }
        ...
        {
            foreach (KeyValuePair<NetworkableId, BaseNetworkable> keyValuePair in BaseNetworkable.serverEntities.entityList.Get())
            {
        ...
        private List<Item> GetAllItems(BasePlayer player, Boolean getItemInBackpack = false)
        {
            List<Item> itemList = Facepunch.Pool.Get<List<Item>>();
            player.inventory.GetAllItems(itemList);
            if(getItemInBackpack)
        ...
        private Boolean HaveItem(BasePlayer player, String Shortname, Int32 Amount, UInt64 SkinID = 0, ItemContainer contaner = null)
        {
            ...
            List<Item> itemList = GetAllItems(player);
            
            foreach (Item ItemRequires in contaner == null ? itemList : contaner.itemList)
            {
            ...
            Facepunch.Pool.FreeUnmanaged(ref itemList);
        ...
        {
            List<Item> ItemList = container == null ? GetAllItems(player) : container.itemList;
            Int32 TargetAmount = BuyItem.PriceItems.Amount * Amount;
            List<Item> acceptedItems = new List<Item>();
            Int32 itemAmount = 0;
            List<Item> items = GetAllItems(player);
        ...
        private Dictionary<BasePlayer, Double> playerBuyingCooldown = new();
        private void BuyItem(BasePlayer player, PluginConfig.ShopStore.CategorySettings.ItemsPair BuyItem, CategoryType category, Int32 Page, Int32 Amount)
        {
            ...
            if (playerBuyingCooldown.ContainsKey(player))
            {
                if (playerBuyingCooldown[player] > curTime)
                    return; 
                playerBuyingCooldown[player] = curTime + 3f;
            }
            else playerBuyingCooldown.Add(player, curTime + 3f);
        ...
        public void InitializeNPC(Vector3 pos, Quaternion rot)
        {
            ...
            vending.CancelInvoke(vending.CheckSellOrderRefresh);
            vending.BypassDynamicPricing = true;
            ...
            return new PluginConfig
            {
                ...
                storeNpc = new PluginConfig.ShopStore
                {
                    ...
                    ExchangerCategory = new PluginConfig.ShopStore.CategorySettings
                    {
                        ItemsList = new List<PluginConfig.ShopStore.CategorySettings.ItemsPair>
                        {
                            new PluginConfig.ShopStore.CategorySettings.ItemsPair("hazmatsuit.arcticsuit", 1, 0, String.Empty, "water.salt", 2500, 2469834291, String.Empty),
                            new PluginConfig.ShopStore.CategorySettings.ItemsPair("rifle.ak.ice", 1, 0, String.Empty, "water.salt", 3500, 2469834291, String.Empty),
                            new PluginConfig.ShopStore.CategorySettings.ItemsPair("stones", 1000, 0, String.Empty, "water.salt", 350, 2469834291, String.Empty),
                            new PluginConfig.ShopStore.CategorySettings.ItemsPair("wood", 1000, 0, String.Empty, "water.salt", 150, 2469834291, String.Empty),
                            new PluginConfig.ShopStore.CategorySettings.ItemsPair("metal.fragments", 1000, 0, String.Empty, "water.salt", 700, 2469834291, String.Empty),
                            new PluginConfig.ShopStore.CategorySettings.ItemsPair("lowgradefuel", 500, 0, String.Empty, "water.salt", 950, 2469834291, String.Empty),
                            new PluginConfig.ShopStore.CategorySettings.ItemsPair("sulfur", 1000, 0, String.Empty, "water.salt", 1250, 2469834291, String.Empty),
                            new PluginConfig.ShopStore.CategorySettings.ItemsPair("metal.refined", 25, 0, String.Empty, "water.salt", 950, 2469834291, String.Empty),
                            new PluginConfig.ShopStore.CategorySettings.ItemsPair("charcoal", 100, 0, String.Empty, "water.salt", 750, 2469834291, String.Empty),
                            new PluginConfig.ShopStore.CategorySettings.ItemsPair("cloth", 500, 0, String.Empty, "water.salt", 250, 2469834291, String.Empty),
                            new PluginConfig.ShopStore.CategorySettings.ItemsPair("leather", 100, 0, String.Empty, "water.salt", 950, 2469834291, String.Empty),
                            new PluginConfig.ShopStore.CategorySettings.ItemsPair("targeting.computer", 1, 0, String.Empty, "water.salt", 650, 2469834291, String.Empty),
                            new PluginConfig.ShopStore.CategorySettings.ItemsPair("cctv.camera", 1, 0, String.Empty, "water.salt", 450, 2469834291, String.Empty),
                        }
                    }
                }
            };
        ...
        #region Interface Image Downloader
        ...
        private class ImageUi
        {
            ...
            private static IEnumerator AddImage()
            {
                ...
                _plugin.GetSounds();
                _plugin.FullLoad();
                _plugin.PrintWarning(LanguageEn ? "Plugin succesfully loaded!" : "Плагин успешно загружен");

                if (!_plugin.IsInitializeNPCShop() && _plugin.CustomPosNPC.Count == 0)
                {
            ...
            public static void Initialize()
            {
                ...
                // {
                //     if(Image.BuyItems.SkinID == 994832328499) return;
        ...
        private class BarrelController : MonoBehaviour
        {
            ...
            private void ProcessTick()
            {
                ...
                if (item == null)
                {
                    Item water = ItemManager.CreateByName("water.salt", TickOut == 0 ? 1 : TickOut, _config.itemSettings.WortItemSkinId);
                    if (water == null)
                        return;
