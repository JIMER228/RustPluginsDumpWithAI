...
namespace Oxide.Plugins
{
    ...
    class AlphaLoot : RustPlugin
    {
        ...
        private void RefreshLootContents(ConsoleSystem.Arg arg = null, bool setDefaultScrap = false)
        {
            ...
            for (int i = 0; i < lootContainers?.Length; i++)
            {
                ...
                if (lootContainer != null && !lootContainer.IsDestroyed)
                {
                    ...
                    if (setDefaultScrap)
                    {
                        if (defaultScrapAmounts.TryGetValue(lootContainer.ShortPrefabName, out int scrapAmount))
        ...
        private bool PopulateLoot(LootContainer container) => PopulateLoot(container, container.ShortPrefabName);
        ...
        {
            if (storedData.TryGetNPCProfile(entity.ShortPrefabName, out BaseLootProfile lootProfile))
            {
        ...
        private void AddItemsToLootTable(List<int> items, out int additions)
        {
            ...
            foreach (KeyValuePair<string, AdvancedLootContainerProfile> kvp in defaultLootTable.loot_advanced)
            {
                foreach (int itemid in items)
                {
                    ...
                    if (score > 0)
                    {
                        if (storedData.loot_advanced.TryGetValue(kvp.Key, out AdvancedLootContainerProfile advancedLootProfile))
                        {
                        ...
                        else if (storedData.loot_simple.TryGetValue(kvp.Key, out SimpleLootContainerProfile simpleLootContainerProfile))
                        {
            ...
            foreach (KeyValuePair<string, AdvancedNPCLootProfile> kvp in defaultLootTable.npcs_advanced)
            {
                foreach (int itemid in items)
                {
                    ...
                    if (score > 0)
                    {
                        if (storedData.npcs_advanced.TryGetValue(kvp.Key, out AdvancedNPCLootProfile advancedNPCLootProfile))
                        {
                            LootSpawnSlot newLootSpawnSlot = new LootSpawnSlot
                            {
                                LootDefinition = new LootSpawn()
                        ...
                        else if (storedData.npcs_simple.TryGetValue(kvp.Key, out SimpleNPCLootProfile simpleNPCLootProfile))
                        {
                            ItemAmountWeighted itemAmountWeighted = new ItemAmountWeighted()
                            {
                                BlueprintChance = newLootItem.Item.BlueprintChance,
            ...
            foreach (KeyValuePair<string, AdvancedLootContainerProfile> kvp in defaultHeliLootTable.loot_advanced)
            {
                foreach (int itemid in items)
                {
                    ...
                    if (score > 0)
                    {
                        if (heliData.loot_advanced.TryGetValue(kvp.Key, out AdvancedLootContainerProfile advancedLootProfile))
                        {
                            LootSpawnSlot newLootSpawnSlot = new LootSpawnSlot
                            {
                                LootDefinition = new LootSpawn()
                        ...
                        {
                            if (heliData.loot_simple.TryGetValue(kvp.Key, out SimpleLootContainerProfile simpleLootContainerProfile))
                            {
                                ItemAmountWeighted itemAmountWeighted = new ItemAmountWeighted()
                                {
                                    BlueprintChance = newLootItem.Item.BlueprintChance,
            ...
            foreach (KeyValuePair<string, AdvancedLootContainerProfile> kvp in defaultBradleyLootTable.loot_advanced)
            {
                foreach (int itemid in items)
                {
                    ...
                    if (score > 0)
                    {
                        if (bradleyData.loot_advanced.TryGetValue(kvp.Key, out AdvancedLootContainerProfile advancedLootProfile))
                        {
                            LootSpawnSlot newLootSpawnSlot = new LootSpawnSlot
                            {
                                LootDefinition = new LootSpawn()
                        ...
                        {
                            if (bradleyData.loot_simple.TryGetValue(kvp.Key, out SimpleLootContainerProfile simpleLootContainerProfile))
                            {
                                ItemAmountWeighted itemAmountWeighted = new ItemAmountWeighted()
                                {
                                    BlueprintChance = newLootItem.Item.BlueprintChance,
        ...
        public class AdvancedLootContainerProfile : BaseLootContainerProfile
        {
            ...
            public AdvancedLootContainerProfile(LootContainer container)
            {
                ...
                if (!defaultScrapAmounts.TryGetValue(container.ShortPrefabName, out int scrapAmount))
                    scrapAmount = 1;
