...
namespace Oxide.Plugins
{
    ...
    class IQChat : RustPlugin
    {
                ...
                return new Configuration
                {
                    ...
                    ControllerMessages = new ControllerMessage
                    {
                        Formatting = new ControllerMessage.FormattingMessage
                        {
                            ...
                            {
                                BadNicks = new List<String> { "Администратор", "Модератор", "Админ", "Модер", "Овнер", "skulidropek", "IQchat" },
                                ReplaceBadNick = "",
        ...
        private void DiscordLoggChat(BasePlayer player, String MessageLogged)
        {
            List<Fields> fields = new List<Fields>
                        {
                            new Fields("Ник", player.displayName, true),
            ...
            foreach (BasePlayer p in BasePlayer.activePlayerList)
            {
                if (OutMessage.Contains("@"))
                {
                    ...
                    if (playerTags != null)
                    {
                        ...
                        else p.SendConsoleCommand("chat.add", new object[] { RenameID, OutMessage });
                    }
                    else p.SendConsoleCommand("chat.add", new object[] { RenameID, OutMessage });
                }
                else p.SendConsoleCommand("chat.add", new object[] { RenameID, OutMessage });
        ...
        void ReplySystem(BasePlayer player, String Message, String CustomPrefix = null, String CustomAvatar = null, String CustomHex = null)
        {
            ...
            player.SendConsoleCommand("chat.add", AvatarID, $"{Prefix}<color={Hex}>{Message}</color>");
        }

        void ReplyBroadcast(String Message, String CustomPrefix = null, String CustomAvatar = null, Boolean AdminAlert = false)
        {
            foreach (BasePlayer p in !AdminAlert ? BasePlayer.activePlayerList.Where(p => UserInformation.ContainsKey(p.userID) && UserInformation[p.userID].Settings.TurnBroadcast) : BasePlayer.activePlayerList)
        ...
        private void SeparatorChat(BasePlayer player, String Message)
        {
            ...
            SendFormat = $"{RankTime} {Rank} <size={OtherController.SizePrefix}>{Prefixes}</size> <size={OtherController.SizeNick}>{DisplayName}</size>: <size={OtherController.SizeMessage}>{ResultMessage}</size>";
        ...
        {
            private static Coroutine _coroutineImg = null;
            private static Dictionary<string, string> _images = new Dictionary<string, string>();

            public static void DownloadImages() { _coroutineImg = ServerMgr.Instance.StartCoroutine(AddImage()); }
            ...
            private static IEnumerator AddImage()
            {
                ...
                foreach (var imageCfg in config.KeyImages)
                {
                    ...
                    if (texture != null)
                    {
                        ...
                        if (!_images.ContainsKey(imageCfg.Key))
                            _images.Add(imageCfg.Key, image);
                        else
                            _images[imageCfg.Key] = image;
        ...
        private class InterfaceBuilder
        {
            ...
            private void BuildingModerationStatic()
            {
                ...
                container.Add(new CuiElement
                {
                    ...
                    Components = {
                    new CuiTextComponent { Text = "%TITLE%", Font = "robotocondensed-regular.ttf", FontSize = 8, Align = TextAnchor.MiddleLeft, Color = "1 1 1 1" },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "11.0752913 -126.612", OffsetMax = "126.125 -112.988" }
                }
                });
            ...
            private void BuildingMuteAllChat()
            {
                ...
                container.Add(new CuiElement
                {
                    ...
                    Components = {
                    new CuiRawImageComponent { Color = "1 1 1 1", Png = ImageUi.GetImage("IQCHAT_ELEMENT_PANEL_ICON") },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "11.07 -161.818", OffsetMax = "152.882913 -147.382" }
                }
                });
            ...
            private void BuildingAlertUI()
            {
                ...
                container.Add(new CuiElement
                {
                    ...
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-184.193 -27.133", OffsetMax = "189.223 9.1192913" }
                }
                });
        ...
        void API_ALERT(String Message, String CustomPrefix = null, String CustomAvatar = null, String CustomHex = null)
        {
            foreach (BasePlayer p in BasePlayer.activePlayerList)
