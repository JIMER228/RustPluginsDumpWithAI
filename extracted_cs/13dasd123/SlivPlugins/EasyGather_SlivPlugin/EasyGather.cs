...
namespace Oxide.Plugins
{
    ...
    public class EasyGather : RustPlugin
    {
        #region [Vars]
        [PluginReference]
        private Plugin ZoneManager, CopyPaste, ImageLibrary, Clans;
        ...
        public class PlayerData
        {
            ...
            public Dictionary<ulong, Vector3> playerPosition = new Dictionary<ulong, Vector3>();
        }
        ...
        private void OnServerInitialized()
        {
            ...
            {
                PrintWarning("ImageLibrary is not initialized. Check if it is properly loaded.");
            }
        }
        ...
        private void OnEntityDeath(BaseEntity ent, HitInfo info)
        {
            ...
            var clan = Clans?.Call<String>("GetClanTag", player.userID);
            if (string.IsNullOrEmpty(clan)) return;

            if (playerData.ContainsKey(clan))
            {
                if (playerData[clan].gatherTime >= 0)
                {
                    ...
                    if (!playerData[clan].topAmount.ContainsKey(ent.PrefabName))
                        playerData[clan].topAmount.Add(ent.PrefabName, 1);
                    else
                        playerData[clan].topAmount[ent.PrefabName]++;

                    playerData[clan].topPoints += config.top[ent.PrefabName];
                    return;
                }
                else
                {
                    player.metabolism.radiation_poison.value += 500;
                    player.health -= 150;
                }
            }
            else
        ...
        {
            playerData = Interface.Oxide.DataFileSystem.ReadObject<Dictionary<string, PlayerData>>("EasyGather/PlayerData/data");
        }
        ...
        private class GatherManager : MonoBehaviour
        {
            ...
            private void Timer()
            {
                ...
                List<String> playerClan = new List<String>();
                foreach (var player in players)
                {
                    if (player == null || !player.IsConnected) continue;

                    var clan = plugin.Clans?.Call<String>("GetClanTag", player.userID);
                    ...
                    if(!plugin.playerData.ContainsKey(clan))
                    {
                    ...
                    if(plugin.playerData[clan].gatherTime <= 0)
                    {
                        if (plugin.playerData[clan].playerPosition[player.userID] != Vector3.zero)
                        {
                            plugin.TeleportPlayerToPos(player, plugin.playerData[clan].playerPosition[player.userID]);
                            plugin.playerData[clan].playerPosition[player.userID] = Vector3.zero;
                        }
                        else
                        {
                            player.Die();
                    ...
                    if (!playerClan.Contains(clan))
                    {
                        plugin.playerData[clan].gatherTime--;
                        playerClan.Add(clan);
                    }
                    time = TimeSpan.FromSeconds(plugin.playerData[clan].gatherTime);
                    SetPlayerFlag(player, BasePlayer.PlayerFlags.IsAdmin, true);
        ...
        private void cmd_UI_ADMIN_MAIN_ACTION(ConsoleSystem.Arg arg)
        {
            ...
            switch (arg.Args[0])
            {
                ...
                case "ftime":
                    if (!playerData.ContainsKey(clan))
                        playerData.Add(clan, new PlayerData() { gatherTime = 0, lastPosition = Vector3.zero, lastTeleport = 0, topAmount = new Dictionary<string, int>(), topPoints = 0 });

                    playerData[clan].gatherTime += 3600;
        ...
        private void CreatePlayerUI(BasePlayer player)
        {
            ...
            if (config.main.isTopActive)
            {
                ...
                if (playerData.ContainsKey(clan))
                {
                    container.Add(new CuiElement
                    {
                        ...
                        Parent = "mypanel",
                        Components = {
                    new CuiTextComponent { Text = playerData[clan].topPoints.ToString(), Font = "robotocondensed-bold.ttf", FontSize = 14, Align = TextAnchor.MiddleLeft, Color = "1 1 1 0.4901961" },
                    new CuiOutlineComponent { Color = "0 0 0 0.5", Distance = "1 -1" },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-22.745 -10.475", OffsetMax = "55.345 10.405" }
                    }
                    });
                    ...
                    container.Add(new CuiButton
                    {
                        Button = { Color = "0 0 0 0", Command = $"UI_PLAYER_ACTION top {clan}" },
                        Text = { Text = "", Font = "robotocondensed-regular.ttf", FontSize = 18, Align = TextAnchor.MiddleCenter, Color = "0 0 0 0" },
                        RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1" }
                    }, "mypanel", "TopBtn_My");
                }
            ...
            if (playerData.ContainsKey(clan))
            {
                var time = TimeSpan.FromSeconds(playerData[clan].gatherTime);
                container.Add(new CuiElement
                {
                    Name = "FarmTime",
                    Parent = "ChangePanel",
        ...
        private void CreateTopUI(BasePlayer player, string clan)
        {
            if (!playerData.ContainsKey(clan))
                return;
            ...
            switch(action)
            {
                ...
                case "change":
                    ...
                    if(removeTicket(player, skin))
                    {
                        ...
                        playerData[clan].gatherTime += ticket.time;
                        CreatePlayerUI(player);
                        return;
                    }
                ...
                case "leave":
                    if (!gatherManager.players.Contains(player) || !playerData.ContainsKey(clan))
                    {
                        NoticePlayer(player, GetMsg("UI_Player_Notice_NoGather", player));
                        return;
                    }

                    if (playerData[clan].playerPosition[player.userID] == Vector3.zero)
                    {
                        NoticePlayer(player, GetMsg("UI_Player_Notice_NoReturnPos", player));
                        return;
                    }

                    TeleportPlayerToPos(player, playerData[clan].playerPosition[player.userID]);
                    playerData[clan].playerPosition[player.userID] = Vector3.zero;
                    CuiHelper.DestroyUi(player, "EasyGatherPlayer");
                    break;
                ...
                case "tp":
                    ...
                    if(!playerData.ContainsKey(clan) || playerData[clan].gatherTime <= 0)
                    {
                        NoticePlayer(player, GetMsg("UI_Player_Notice_NoTime", player));
                        return;
                    }

                    if(Facepunch.Math.Epoch.Current - playerData[clan].lastTeleport < config.main.teleportCoolDown)
                    {

                        var time = TimeSpan.FromSeconds(playerData[clan].lastTeleport + config.main.teleportCoolDown - Facepunch.Math.Epoch.Current);
                    ...
                    if (!playerData[clan].playerPosition.ContainsKey(player.userID))
                    {
                        playerData[clan].playerPosition.Add(player.userID, player.transform.position);
                    }
                    else
                    {
                        playerData[clan].playerPosition[player.userID] = player.transform.position;
                    }
                    playerData[clan].lastTeleport = Facepunch.Math.Epoch.Current;
                    CuiHelper.DestroyUi(player, "EasyGatherPlayer");
