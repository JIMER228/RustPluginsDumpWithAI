    ...
    public class HeliSignals: RustPlugin
    {
        ...
        private void Init()
        {
            ...
            foreach (var key in config.heli.waveConfig.Keys)
            {
                ...
                {
                    // If loading fresh, populate default wav profiles without multi profiles
                    foreach (var profile in config.heli.heliConfig.Keys)
                    {
                        if (!profile.Contains("Multi"))
        ...
        {
            var Initiator = info?.Initiator;
            if (heli == null || Initiator == null)
                return null;

            var heliProfile = heli._name;
            bool isVanilla = false;
            if (heliProfile == null && heli.skinID == 0)
                isVanilla = true;
            else if (heliProfile == null && heli.skinID != 0)
                return null;
            else if (heliProfile != null && !config.heli.heliConfig.ContainsKey(heliProfile))
                return null;
            ...
            if (!isVanilla)
            {
                ...
                if (config.heli.RetireWarning && attacker.IsBuildingBlocked() && IsOwnerOrFriend(attackerId, ownerId))
                {
                    ...
                    info.damageTypes.ScaleAll(0);
                    Message(attacker, "RetireWarn", heliDisplayName, HeliSignalData[heliId].WarningLevel, config.heli.WarningThreshold);
                    return true;
                }
            }
            ...
            if (!isVanilla)
            {
                if (heliAI._currentState == PatrolHelicopterAI.aiState.DEATH && !heliComp.isDying)
                {
                    // Give rewards as soon as heli enters death state, otherwise if
                    // players die before it crashes they might not get rewards
                    heliComp.isDying = true;
                    NextTick(()=> ProcessRewards(heliId, ownerId, heliProfile));
        ...
        private object OnHelicopterRetire(PatrolHelicopterAI heliAI)
        {
            ...
            if (config.heli.heliConfig.ContainsKey(heliProfile) && heliAI.IsAlive())
            {
                ...
                {
                    Message(heliComp.owner, "WaveRetired", heliProfile);
                }
                ...
                if (config.announce.reportChat && config.announce.reportRetire)
                {
                    ...
                    {
                        string topReport = string.Empty;
                        string ownerReport = string.Format(lang.GetMessage("DmgReportOwner", this, lastAttacker.UserIDString), config.heli.heliConfig[heliProfile].HeliName, heliComp.owner.displayName, "Retired");
                        int count = 1;
                        foreach (var key in HeliSignalData[heliId].Attackers.Keys.OrderByDescending(x => HeliSignalData[heliId].Attackers[x].DamageDealt))
                        {
                            if (count >= config.announce.maxReported)
            ...
            else if (config.heli.waveConfig.Keys.Contains(heliProfile))
            {
                ...
                if (config.heli.waveConfig[heliProfile].WaveProfiles.Count > 0)
                {
                    foreach (var item in config.heli.waveConfig[heliProfile].WaveProfiles)
                    {
                        ...
                        {
                            Puts($"ERROR: WaveHeli config contains a profile with an incorrect name ({item}) please correct!");
        ...
        private class HeliSignalComponent : MonoBehaviour
        {
            ...
            void CustomExplode()
            {
                ...
                else
                {
                    ...
                    var heliDisplayName = heliAmount + " X " + (isWaveHeli ? waveProfile : config.heli.heliConfig[heliProfile].HeliName);
                    ...
                    for (int i = 0; i < heliAmount; i++)
                    {
                        ...
                        Instance.NextTick(() =>
                        {
                            ...
                            if (config.heli.heliConfig[heliProfile].DespawnTime > 0)
                                Instance.RetireHeli(heliAI, heliId, config.heli.heliConfig[heliProfile].DespawnTime);
                        });
                    }
                }
            ...
            public bool SignalAborted()
            {
                ...
                if (config.heli.allowMonuments && anyMonument != null && anyMonument.ToString().Contains("/monument/"))
                {
                    Instance.Message(player, "InMonument", heliProfile);
                    return true;
                }
        ...
        private class HeliComponent : MonoBehaviour
        {
            ...
            void Start()
            {
                ...
                InvokeRepeating(nameof(UpdateTargetList), 2f, 2f);
            }
            
            public void CallNextWaveHeli(string waveHeliProfile)
            {
                if (!HeliProfiles.Contains(waveHeliProfile))
                    return;
                
                PatrolHelicopter newHeli = GameManager.server.CreateEntity(heliPrefab, new Vector3(), new Quaternion(), true) as PatrolHelicopter;
                newHeli.OwnerID = owner.userID;
                newHeli.skinID = heli.skinID;
                newHeli._name = waveHeliProfile;
                
                StrafeHeliAI strafeHeliAI = newHeli.gameObject.AddComponent<StrafeHeliAI>();
                if (strafeHeliAI == null) return;
                strafeHeliAI.heliProfile = waveHeliProfile;
                newHeli.Spawn();
                ...
                Instance.NextTick(() =>
                {
                    ...
                    newHeliAI.SetInitialDestination(calledPosition, config.heli.mapScaleDistance);
                    ...
                    Instance.Message(owner, "NextHeliCalled", heliProfile, waveProfileCache[0]);
                });
            }
            ...
            void SetupHeli()
            {
                Instance.NextTick(() =>
                {
                    ...
                    heliAI.interestZoneOrigin = calledPosition;
                    heliAI.hasInterestZone = true;
            ...
            public void UpdateTargetList()
            {
                ...
                {
                    heliAI.ExitCurrentState();
                    bool useNapalm = (Oxide.Core.Random.Range(0f, 1f) <= config.heli.heliConfig[heliProfile].NapalmChance);
                    heliAI.State_Strafe_Enter(strafePosition, useNapalm);
            ...
            private void OnDestroy()
            {
                ...
                else if (this.isWaveHeli && this.waveProfileCache.Count > 0)
                {
                    Instance.NextTick(()=>
                    {
                        ...
                        {
                            Instance.Message(owner, "WaveFinished", heliProfile);
                            return;
                        }
