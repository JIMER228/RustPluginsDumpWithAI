...
{
	[Info("Statistics", "Mevent", "1.1.3")]
	public class StatisticsRu : RustPlugin
	{
		#region Fields
		...
		private static StatisticsRu _instance;
		...
		private class Configuration
		{
			...
			[JsonProperty(PropertyName = "Включить работу с PlayTimeRewards?")]
			public bool UsePlayTimeRewards = true;

			[JsonProperty(PropertyName = "Включить работу с PlaytimeTracker?")]
			public bool UsePlaytimeTracker = true;

			[JsonProperty(PropertyName = "Включить Профиль?")]
			public bool EnableProfile = true;

			[JsonProperty(PropertyName = "Команды Статистики",
				ObjectCreationHandling = ObjectCreationHandling.Replace)]
			public string[] StatisticsCommands = {"stats", "statistics"};

			[JsonProperty(PropertyName = "Включить Таблицу лидеров?")]
			public bool EnableLeaderboard = true;

			[JsonProperty(PropertyName = "Команды Таблицы лидеров",
				ObjectCreationHandling = ObjectCreationHandling.Replace)]
			public string[] LeaderboardCommands = {"leaders", "leaderboard"};

			[JsonProperty(PropertyName = "Автовайп при вайпе")]
			public bool AutoWipe = false;

			[JsonProperty(PropertyName = "Доступ по разрешению (statistics.use)")]
			public bool AccessByPerm = false;

			[JsonProperty(PropertyName = "Открывать профиль игрока по нажатию в таблице лидеров?")]
			public bool ProfileByBoard = true;

			[JsonProperty(PropertyName =
				"Разрешение для открыия профиля игрока по нажатию в таблице лидеров (пример: statisticsru.profile)")]
			public string ProfileBoardPerm = string.Empty;

			[JsonProperty(PropertyName = "Оружия",
				ObjectCreationHandling = ObjectCreationHandling.Replace)]
			...
			[JsonProperty(PropertyName = "Таблица очков (shortname - score)",
				ObjectCreationHandling = ObjectCreationHandling.Replace)]
			...
			[JsonProperty(PropertyName = "Таблица очков за NPCs (UserID - очки)",
				ObjectCreationHandling = ObjectCreationHandling.Replace)]
			public Dictionary<string, float> NPCScoreTable = new Dictionary<string, float>
			{
				["7654321"] = 4f,
				["1234567"] = 3f
			};

			[JsonProperty(PropertyName = "Считать суицид как смерть")]
			public bool CountSuicide = true;

			[JsonProperty(PropertyName = "Считать суицид как убийств")]
			public bool CountSuicideKill = true;

			[JsonProperty(PropertyName = "Считать убийства ботов")]
			public bool CountBots = true;

			[JsonProperty(PropertyName = "Считать смерть от бота")]
			public bool CountFromBots = true;

			[JsonProperty(PropertyName = "Считать убийства/смерти от тиммейтов")]
			public bool CountFromTeammates = true;

			[JsonProperty(PropertyName = "Использовать округление значений?")]
			public bool ValueRounding = false;

			[JsonProperty(PropertyName = "Считать убийства ботов в убийствах?")]
			public bool CountBotKillsInKills = false;

			[JsonProperty(PropertyName = "PlayerDatabase")]
			public PlayerDatabaseConf PlayerDatabase = new PlayerDatabaseConf(false, "Statistics");

			[JsonProperty(PropertyName = "Использовать награды?")]
			public bool UseAwards = false;

			[JsonProperty(PropertyName = "Включить возможность просматривать профиль другого игрока?")]
			public bool SeeProfile = true;

			[JsonProperty(PropertyName =
				"Разрешение на просмотр профиля другого игрока (пример: statisticsru.profile)")]
			public string PermissionSeeProfile = string.Empty;

			[JsonProperty(PropertyName = "Включить сбор статистики только для игроков с разрешением?")]
			public bool UseCollectionPerm = false;

			[JsonProperty(PropertyName = "Разрешение на сбор статистики (ghbvth: statisticsru.collect)")]
			public string CollectionPerm = string.Empty;

			[JsonProperty(PropertyName = "Награды (позиция в топе - награда)",
				ObjectCreationHandling = ObjectCreationHandling.Replace)]
			...
			public InterfaceSettings UI = new InterfaceSettings
			{
				...
				{
					Color1 = new IColor("#000000", 90),
					Color2 = new IColor("#0E0E10"),
					Color3 = new IColor("#FFFFFF"),
					Color4 = new IColor("#818181"),
					Color5 = new IColor("#161617"),
					Color6 = new IColor("#4B68FF", 33),
					Color7 = new IColor("#4B68FF"),
					Color8 = new IColor("#4B68FF", 53),
					Color9 = new IColor("#4B68FF", 23),
					Color10 = new IColor("#BFBFBF")
				}
			};
		}
		...
		{
			[JsonProperty(PropertyName = "Цвета")] public ColorsInfo Colors;
		...
		{
			[JsonProperty(PropertyName = "Количество предметов")]
			public int Amount;
		...
		private class AwardItem
		{
			...
			[JsonProperty(PropertyName = "Команда (%steamid%)")]
			public string Command;

			[JsonProperty(PropertyName = "Плагин")]
			public PluginItem Plugin;

			[JsonProperty(PropertyName = "Отображаемое имя (пусто - по умолчанию)")]
			public string DisplayName;

			[JsonProperty(PropertyName = "ShortName")]
			public string ShortName;

			[JsonProperty(PropertyName = "Скин")] public ulong Skin;

			[JsonProperty(PropertyName = "Количество")]
			public int Amount;

			[JsonProperty(PropertyName = "Шанс")] public float Chance;
		...
		private class PlayerDatabaseConf
		{
			...
			[JsonProperty(PropertyName = "Таблица")]
			public string Field;
		...
		private class PlayerData
		{
			...
			[JsonProperty(PropertyName = "NPC Stats", ObjectCreationHandling = ObjectCreationHandling.Replace)]
			public Dictionary<string, float> NPCStats = new Dictionary<string, float>();
			...
			[JsonIgnore] public float TotalScore => Score + NPCScore;
			...
			public float Score
			{
				get
				{
					return Convert.ToSingle(Math.Round(Stats
			...
			public float NPCScore
			{
				...
				{
					return Convert.ToSingle(Math.Round(Stats
						.Where(x => _config.NPCScoreTable.ContainsKey(x.Key))
						.Sum(x => x.Value * _config.NPCScoreTable[x.Key])));
				}
			}
		...
		{
			_playersData.TryAdd(member, new PlayerData());
		...
		private void AddToStats(ulong member, string shortName, int amount = 1, bool npc = false)
		{
			if (!member.IsSteamId()) return;

			if (_config.UseCollectionPerm &&
			    !string.IsNullOrEmpty(_config.CollectionPerm) &&
			    !permission.UserHasPermission(member.ToString(), _config.CollectionPerm))
				return;
			...
			if (npc)
			{
				if (data.NPCStats.ContainsKey(shortName))
					data.NPCStats[shortName] += amount;
				else
					data.NPCStats.Add(shortName, amount);
			}
			else
			{
		...
		private string GetStatsValue(ulong member, string shortname)
		{
			...
			switch (shortname)
			{
				case "time":
				{
					if (_config.UsePlayTimeRewards)
						return FormatTime(PlayTimeRewards_GetPlayTime(member.ToString()));

					if (_config.UsePlaytimeTracker)
						return FormatTime(PlaytimeTracker_GetPlayTime(member.ToString()));
				...
				case "total":
				{
					return GetValue(data.TotalScore);
				}
				case "kd":
				{
		...
		private void OnPlayerDeath(BasePlayer player, HitInfo info)
		{
			...
			var isSuicide = player == attacker;

			if (!player.IsNpc && player.userID.IsSteamId())
			{
				if (!isSuicide || _config.CountSuicideKill)
					AddToStats(attacker.userID, "kills");

				if (!isSuicide || _config.CountSuicide)
					AddToStats(player.userID, "deaths");
			}
			else
			{
				AddToStats(attacker.userID, _config.CountBotKillsInKills ? "kills" : player.ShortPrefabName);
			}
		...
		private readonly Dictionary<ulong, List<ulong>> _loots = new Dictionary<ulong, List<ulong>>();

		private void OnLootEntity(BasePlayer player, BaseEntity entity)
		{
			if (player == null || entity == null || entity.net == null || (_loots.ContainsKey(player.userID) &&
			                                                               _loots[player.userID]
				                                                               .Contains(entity.net.ID.Value))) return;

			AddToStats(player.userID, entity.ShortPrefabName);

			List<ulong> loot;
			if (_loots.TryGetValue(player.userID, out loot))
				loot.Add(entity.net.ID.Value);
			else
				_loots.Add(player.userID, new List<ulong> {entity.net.ID.Value});
		}
		...
		private void OnEntityDeath(BaseCombatEntity entity, HitInfo info)
		{
			...
			if (entity is BaseHelicopter)
			{
				...
				if (_lastHeli.TryGetValue(entity.net.ID.Value, out targetPlayer))
					if (targetPlayer != null)
						AddToStats(targetPlayer.userID, "helicopter");
			...
			if (victimPlayer != null && _config.NPCScoreTable.ContainsKey(victimPlayer.displayName))
			{
				AddToStats(player.userID, victimPlayer.displayName, npc: true);
				return;
			}
		...
		#region Image Library && Last in hooks Queue
		...
		private void OnPluginLoaded(Plugin plugin)
		{
			if (plugin.Name == "ImageLibrary")
				...
				NextTick(() =>
				{
					foreach (var hook in gatherHooks)
						SetLastHooksQueue(hook);
				});
		}
		...
		private void CmdStatsProfile(IPlayer cov, string command, string[] args)
		{
			...
			if (_config.SeeProfile && HasPermission(player, _config.PermissionSeeProfile) && args.Length > 0)
				ulong.TryParse(args[0], out target);

			ProfileUi(player, target, true);
		}

		private void CmdStatsLeaderboard(IPlayer cov, string command, string[] args)
		{
			var player = cov?.Object as BasePlayer;
			if (player == null) return;
			...
			LeaderboardUi(player, first: true);
		}
		...
		private void ProfileUi(BasePlayer player, ulong targetId, bool first = false)
		{
			...
			container.Add(new CuiPanel
			{
				...
				Image = {Color = _config.UI.Colors.Color5.Get}
			}, Layer + ".Main");
			...
			if (_config.EnableLeaderboard)
				container.Add(new CuiButton
				{
					...
					Button =
					{
						Command = $"{_config.LeaderboardCommands[0]}",
						Color = _config.UI.Colors.Color6.Get
					}
				}, Layer + ".Main");
		...
		private void LeaderboardUi(BasePlayer player, int page = 0, int mode = 0, bool first = false)
		{
			...
			container.Add(new CuiLabel
			{
				...
				Text =
				{
					...
					Font = "robotocondensed-regular.ttf",
					FontSize = 10,
					Color = _config.UI.Colors.Color10.Get
				}
			}, Layer + ".Main");
			...
			CreateOutLine(ref container, Layer + ".My.Profile", _config.UI.Colors.Color5.Get, 1);
			...
			for (var i = 0; i < list.Count; i++)
			{
				...
				container.Add(new CuiPanel
				{
					...
					{
						Color =
							top == 1 ? _config.UI.Colors.Color7.Get :
							top == 2 ? _config.UI.Colors.Color8.Get :
							top == 3 ? _config.UI.Colors.Color9.Get :
							"0 0 0 0"
					}
				...
				if (_config.EnableProfile && _config.ProfileByBoard)
					container.Add(new CuiButton
					{
						RectTransform =
						{
		...
		#region Last in hooks Queue

		private string[] gatherHooks =
		{
			nameof(OnCollectiblePickup),
			nameof(OnCropGather),
			nameof(OnDispenserBonus),
			nameof(OnDispenserGather)
		};

		private void SetLastHooksQueue(string hook)
		{
			Unsubscribe(hook);
		...
		{
			if (_config.EnableLeaderboard)
				AddCovalenceCommand(_config.LeaderboardCommands, nameof(CmdStatsLeaderboard));

			if (_config.EnableProfile)
				AddCovalenceCommand(_config.StatisticsCommands, nameof(CmdStatsProfile));
		}
		...
		private void RegisterPermissions()
		{
			...
			if (!string.IsNullOrEmpty(_config.PermissionSeeProfile) &&
			    !permission.PermissionExists(_config.PermissionSeeProfile))
				permission.RegisterPermission(_config.PermissionSeeProfile, this);

			if (_config.UseCollectionPerm && !string.IsNullOrEmpty(_config.CollectionPerm) &&
			    !permission.PermissionExists(_config.CollectionPerm))
				permission.RegisterPermission(_config.CollectionPerm, this);
		}
		...
		private double PlaytimeTracker_GetPlayTime(string playerid)
		{
			return Convert.ToDouble(PlaytimeTracker?.Call("GetPlayTime", playerid));
		}
		...
		protected override void LoadDefaultMessages()
		{
			...
			lang.RegisterMessages(new Dictionary<string, string>
			{
				...
				[Close] = "Закрыть",
				[Mining] = "Добыча",
				[ResourcesAmount] = "{0} шт",
				[DeathsTitle] = "Смерти",
				[KillsTitle] = "Убийства",
				[Top] = "Топ",
				[FavoriteWeapon] = "Любимое оружие",
				[ScoreTitle] = "Очки",
				[GameTimeOnServer] = "Время на сервере",
				[Username] = "Имя",
				[KdAmount] = "Кол-во убийств / смертей",
				[Leaderboard] = "Таблица лидеров",
				[NoPermission] = "У вас нет прав на использование данной команды!",
