    ...
    class MeteorFall : RustPlugin
    {
        ...
        private void CreateNpc(Vector3 position, int amount = 0)
        {
            for (int i = 0; i < amount; i++)
            {
                ...
                ScientistNPC NPCPlayer = GameManager.server.CreateEntity("assets/prefabs/npc/NPCPlayer/NPCPlayer.prefab", pos) as ScientistNPC;
                NPCPlayer.Spawn();
                npcPos.Add(NPCPlayer, pos);
                NpcList.Add(NPCPlayer);
            }
            ToSpawnPoint();
        }
        ...
        void ToSpawnPoint()
        {
            npcTimer = timer.Every(5f, () =>
            {
                foreach (var npc in npcPos.Keys.ToList())
                    if (npc != null && npcPos.ContainsKey(npc) && npc.GetAttackEntity() == null)//AttackTarget
        ...
        [ChatCommand("meteor")]
        void ChatCmdControll(BasePlayer player, string command, string[] args, ulong playerid = 4570112)
        {
        ...
        void LoadMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                ["colding"] = "<size=14>NPCPlayers have discovered a <color=#FF4500><b>METEORITE</b></color>, its location is marked on the map! \nThe meteorite cools through {time}</size>",
