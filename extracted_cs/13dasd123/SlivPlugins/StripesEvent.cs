...
{
    [Info("StripesEvent", "Xavier", "0.0.8")]
    ...
    public class StripesEvent : RustPlugin
    {
        ...
        private Dictionary<ulong, string> LastHeliHit = new Dictionary<ulong, string>();

        object OnEntityTakeDamage(BaseCombatEntity entity, HitInfo info)
        {
            if (entity == null || info == null) return null;
            if (entity is BaseHelicopter && info.Initiator is BasePlayer)
                LastHeliHit[entity.net.ID.Value] = info.InitiatorPlayer.UserIDString;
            return null;
        }
        ...
        void OnEntityDeath(BaseCombatEntity entity, HitInfo info)
        {
            ...
            if (entity is BaseHelicopter)
            {
                ...
                {
                    if (LastHeliHit[entity.net.ID.Value] != null)
                    {
                        BasePlayer player = BasePlayer.Find(LastHeliHit[entity.net.ID.Value]);
                        if (player != null)
                        {
                            var entnames = entity.ShortPrefabName;
                            if (CheckStripes(player, entnames, TypeEvent.Kill))
                            {
                                LastHeliHit.Remove(entity.net.ID.Value);
