...
namespace Oxide.Plugins
{
...

    {
...

        {
            ...
            [JsonProperty("Enable AntiLagSwitch (beta)")]
            public bool AntiLagSwitch { get; private set; } = true;
        ...
        void OnEntityTakeDamage(BaseCombatEntity entity, HitInfo info)
        {
            ...
            if (_config.EnableShootingHeightCheck && isInTheAir && !isInVehicle && !IsNearExcavator(attacker.transform.position))
            {
                ...
                {
                    if (_config.debugMode) { Puts($"Shooting Height Violation: {attacker.displayName} - Height: {heightAboveGround}, Max Height: {_config.MaxShootingHeight}"); }
            ...
            if (_config.EnableMaxShootingDistanceCheck && distanceSquared > maxDistanceForWeapon * maxDistanceForWeapon)
            {
                ...
                { Puts($"Max Shooting Distance Violation: {attacker.displayName} - Distance: {Mathf.Sqrt(distanceSquared)}, Max Distance: {maxDistanceForWeapon}"); }
            ...
            foreach (BasePlayer player in BasePlayer.activePlayerList)
            {
                ...
                if (isInTheAir && heightAboveGround > _config.BetterFlyhackThreshold)
                {
                    ...
                    else if (DateTime.UtcNow - elevatedPlayers[player.userID] > TimeSpan.FromSeconds(_config.BetterFlyhackHoldTime))
                    {
                        ...
                        AddViolation(cachedViolationsFlyhackViolation, player);
        ...
        private bool IsPingSuspicious(List<int> history)
        {
            ...
            if (latestPing > averagePing * 3 && latestPing > 350)
            {
                return true;
            }
            return false;
        ...
        private void SendDiscordMessage(string content, string imageLink = null)
        {
            ...
            webrequest.Enqueue(_config.DiscordWebhookUrl, jsonPayload, (code, response) =>
            {
