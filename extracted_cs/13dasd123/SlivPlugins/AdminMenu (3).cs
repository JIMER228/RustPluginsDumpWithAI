...
{
    [Info("AdminMenu", "0xF [dsc.gg/0xf-plugins]", "1.0.6")]
    ...
    public class AdminMenu : RustPlugin
    {
        ...
        public class QuickMenuButtonArray : ButtonArray<Button>
        {
            public QuickMenuButtonArray() : base()
            {
                Capacity = 5;
            }
        }

        public class UserInfoButtonArray : ButtonArray<Button>
        {
            public UserInfoButtonArray() : base()
            {
                Capacity = 3;
            }
        }
        ...
        public class Button
        {
            ...
            public Button.State GetState(ConnectionData connectionData)
            {
            ...
            public void SetState(ConnectionData connectionData, Button.State state)
            {
        ...
        public class ToggleButton : Button
        {
            ...
            public void Toggle(ConnectionData connectionData)
            {
                State = !State;
                SetState(connectionData, State ? Button.State.Toggled : Button.State.Normal);
            }
        }
        ...
        public class BaseCustomButton
        {
            ...
            {
                return new Button(Label, BaseCommand, "cb.exec", Commands.GetHashCode().ToString())
                {
        ...
        public class CUI
        {
            ...
            public class Element : CuiElement
            {
                ...
                public Element AddInputfield(string command, string text = "Enter text here...", string color = "1 1 1 1", Font font = Font.RobotoCondensedRegular, int fontSize = 14, TextAnchor align = TextAnchor.UpperLeft, InputField.LineType lineType = InputField.LineType.SingleLine, string anchorMin = "0 0", string anchorMax = "1 1", string offsetMin = "0 0", string offsetMax = "0 0", bool @readonly = false, bool needsKeyboard = true, bool autoFocus = false, bool isPassword = false, int charsLimit = 0, string name = null)
                {
                    return Add(new Element(this) { Components = { new CuiInputFieldComponent { Text = text, Color = color, Font = FontNames[(int)font], FontSize = fontSize, Align = align, LineType = lineType, Autofocus = autoFocus, Command = command, ReadOnly = @readonly, IsPassword = isPassword, CharsLimit = charsLimit, NeedsKeyboard = needsKeyboard, }, new CuiRectTransformComponent() { AnchorMin = anchorMin, AnchorMax = anchorMax, OffsetMin = offsetMin, OffsetMax = offsetMax }, }, Name = name });
                }
            ...
            public class Root : Element
            {
                ...
                {
                    return JsonConvert.SerializeObject(elements, format ? Newtonsoft.Json.Formatting.Indented : Newtonsoft.Json.Formatting.None, new JsonSerializerSettings { DefaultValueHandling = DefaultValueHandling.Ignore }).Replace("\\n", "\n");
                }
            ...
            protected override void Render(CUI.Element root, ConnectionData connectionData, Dictionary<string, object> userData)
            {
                ...
                for (int i = 0; i < rows; i++)
                {
                    ...
                    for (int j = 0; j < buttonsToRender.Count; j++)
                    {
                        ...
                        panel.AddButton(command: $"adminmenu {button.Command} {pluginName}", sprite: "assets/content/ui/ui.background.rounded.png", imageType: UnityEngine.UI.Image.Type.Tiled, color: button.Style.BackgroundColor, anchorMin: "0.99 0.1", anchorMax: "0.99 0.9", offsetMin: $"-{(j + 1) * 100 + j * 5} 0", offsetMax: $"-{j * 100 + j * 5} 0").AddText(text: button.Label.Localize(connectionData.connection), align: TextAnchor.MiddleCenter, offsetMin: "10 0", offsetMax: "-10 0");
                    }
                }
        ...
        {
            public QuickMenuButtonArray[] buttons;
            protected override void Render(CUI.Element root, ConnectionData connectionData, Dictionary<string, object> userData)
            {
                ...
                foreach (QuickMenuButtonArray buttonArray in this.buttons)
                {
                ...
                for (int a = 0; a < buttons.Count; a++)
                {
                    ...
                    for (int b = 0; b < rowButtons.Count(); b++)
                    {
                        ...
                        if (button is ToggleButton)
                        {
                            ...
                            backgroundColor = toggleButton.State ? toggleButton.Style.ActiveBackgroundColor : button.Style.BackgroundColor;
                        }
                        else
                        {
                        ...
                        container.AddButton(color: backgroundColor, command: $"adminmenu {button.Command} {string.Join(" ", button.Args)}", anchorMin: "0 1", anchorMax: "0 1", offsetMin: $"{b * 150 + b * offset} -{(a + 1) * 35 + a * offset}", offsetMax: $"{(b + 1) * 150 + b * offset} -{a * 35 + a * offset}").AddText(text: button.Label.Localize(connectionUserId), color: button.Style.TextColor, font: CUI.Font.RobotoMonoRegular, fontSize: 12, align: TextAnchor.MiddleCenter);
                    }
                }
            }
        }
        ...
        public class UserInfoContent : Content
        {
            ...
            public UserInfoButtonArray[] buttons;
            public override void LoadDefaultUserData(Dictionary<string, object> userData)
            {
                userData["userinfo[popup:ban]"] = false;
            }

            protected override void Render(CUI.Element root, ConnectionData connectionData, Dictionary<string, object> userData)
            {
                if ((bool)userData["userinfo[popup:ban]"])
                {
                    banPopup.AddUI(root, connectionData, userData);
                    return;
                }
                else
                {
                    root.Add(new CUI.Element { DestroyUi = "AdminMenu_Popup_BanPopup" });
                }
                ...
                else
                {
                    ...
                    {
                        Instance.ImageLibrary.Call("AddImage", steamInfo.Avatars[2], avatarId, 0UL, new Action(() => Render(connectionData)));
                    }
                }
                ...
                text_info_container.AddInputfield(command: null, text: name, font: CUI.Font.RobotoCondensedBold, fontSize: 24, align: TextAnchor.MiddleLeft, @readonly: true, anchorMin: "0 1", anchorMax: "1 1", offsetMin: "40 -30", offsetMax: "0 0");
                ...
                text_info_container.AddInputfield(command: null, color: "1 1 1 1", text: player.Id, font: CUI.Font.RobotoCondensedBold, fontSize: 18, align: TextAnchor.MiddleLeft, @readonly: true, anchorMin: "0 1", anchorMax: "1 1", offsetMin: "90 -60", offsetMax: "0 -35");
                List<string> leftColumn = new List<string>
                {
                    ...
                    string.Format(GRID_LABEL.Localize(connectionUserId), (position != Vector3.zero ? PhoneController.PositionToGridCoord(position) : UNKNOWN_LABEL.Localize(connectionUserId))),
                    string.Format("{0} (P: {1})", player.Address ?? NOTCONNECTED_LABEL.Localize(connectionUserId), (player.IsConnected ? player.Ping : -1))
                };
                ...
                for (int a = 0; a < buttons.Length; a++)
                {
                    ...
                    for (int b = 0; b < rowButtons.Count(); b++)
                    {
                        ...
                        if (button == null)
                            continue;
                        actionButtonsContainer.AddButton(color: button.Style.BackgroundColor, command: $"adminmenu {button.Command} {string.Join(" ", button.Args)}", anchorMin: "0 1", anchorMax: "0 1", offsetMin: $"{b * 150 + b * offset} -{(a + 1) * 35 + a * offset}", offsetMax: $"{(b + 1) * 150 + b * offset} -{a * 35 + a * offset}").AddText(text: button.Label.Localize(connectionUserId), color: button.Style.TextColor, font: CUI.Font.RobotoMonoRegular, fontSize: 12, align: TextAnchor.MiddleCenter);
                    }
                }
            }
        ...
        public class BanPopup : BasePopup
        {
            ...
            public BanPopup()
            {
                Width = 400;
                Height = 180;
                CloseCommand = "adminmenu userinfo[popup:ban] close";
        ...
        public abstract class BasePopup : IUIModule
        {
            ...
            public virtual CUI.Element AddUI(CUI.Element parent, ConnectionData connectionData, Dictionary<string, object> userData)
            {
                ...
                var backgroundButton = parent.AddButton(color: "0 0 0 0.8", command: CloseCommand ?? $"adminmenu popup {typeName} onClose", close: CloseCommand == null ? elementName : null, anchorMin: "0 0", anchorMax: "1 1", name: elementName).AddDestroySelfAttribute();
                var panel = backgroundButton.AddButton(command: null, anchorMin: "0.5 0.5", anchorMax: "0.5 0.5", offsetMin: $"-{Width / 2} -{Height / 2}", offsetMax: $"{Width / 2} {Height / 2}").AddPanel(color: Extensions.ToCuiString(Color));
        ...
        protected override void LoadConfig()
        {
            ...
            foreach (BaseCustomButton cb in config.CustomQuickButtons.Concat<BaseCustomButton>(config.UserInfoCustomButtons))
            {
                int cmdHashCode = cb.Commands.GetHashCode();
                if (!config.HashedCommands.ContainsKey(cmdHashCode))
                    config.HashedCommands.Add(cmdHashCode, cb.Commands);
            }
        }
                    ...
                    if (connectionData != null)
                    {
                        ...
                        switch (action)
                        {
                            ...
                            case "ban":
                                if (args.Length == 2 && args[1] == "showpopup")
                                {
                                    HandleCommand(connection, "userinfo[popup:ban]", "show");
                                    return;
                                }
                            ...
                            case "cb.exec":
                                ...
                                foreach (string cmd in config.HashedCommands[hashCode])
                                    admin.SendConsoleCommand(cmd.Replace("{steamid}", user.UserIDString).Replace("{steamID}", user.UserIDString).Replace("{userID}", user.UserIDString).Replace("{STEAMID}", user.UserIDString).Replace("{USERID}", user.UserIDString).Replace("{adminUID}", admin.UserIDString));
                                break;
                            default:
                                break;
                        }
        ...
        object OnMapMarkerAdd(BasePlayer player, ProtoBuf.MapNote note)
        {
            ...
            if (connectionData == null)
                return null;
            Button teleportToMarkerButton = (panelList["quickmenu"].DefaultContent as QuickMenuContent).buttons[0].Find(b => b.Args[0] == "toggle_teleport_to_marker");
        ...
        void FormatPanelList()
        {
            ...
            {
                buttons = new QuickMenuButtonArray[16]
                {
                    new QuickMenuButtonArray
                    {
                        new Button("Teleport to 0 0 0", "quickmenu.action", "teleportto_000")
                        {
                            Permission = "quickmenu.teleportto000"
                        },
                        new Button("Teleport to\nDeathpoint", "quickmenu.action", "teleportto_deathpoint")
                        {
                            Permission = "quickmenu.teleporttodeath"
                        },
                        new Button("Teleport to\nSpawn point", "quickmenu.action", "teleportto_randomspawnpoint")
                        {
                            Permission = "quickmenu.teleporttospawnpoint"
                        },
                        new ToggleButton("Teleport to Marker", "quickmenu.action", "toggle_teleport_to_marker")
                        {
                            Permission = "quickmenu.teleport_to_marker"
                        },
                    },
                    new QuickMenuButtonArray
                    {
                        new Button("Kill Self", "quickmenu.action", "killself"),
                        new Button("Heal Self", "quickmenu.action", "healself")
                        {
                            Permission = "quickmenu.healself"
                        },
                        new Button("Time to 12", "quickmenu.action", "settime", "12")
                        {
                    ...
                    new QuickMenuButtonArray
                    {
                        new Button("Giveaway\nto online players", "quickmenu.action", "giveaway_online")
                        {
                            Permission = "quickmenu.giveaway"
                        },
                        new Button("Giveaway\nto everyone", "quickmenu.action", "giveaway_everyone")
                        {
                            Permission = "quickmenu.giveaway"
                        },
                        new Button("Time to 0", "quickmenu.action", "settime", "0")
                        {
                            Permission = "quickmenu.settime"
                        },
                    },
                    new QuickMenuButtonArray
                    {
                        new Button("Call Heli", "quickmenu.action", "helicall")
                        {
                            Permission = "quickmenu.helicall"
                        },
                        new Button("Spawn Bradley", "quickmenu.action", "spawnbradley")
                        {
                            Permission = "quickmenu.spawnbradley"
                        },
                        new Button("Spawn Cargo", "quickmenu.action", "spawncargo")
                        {
            ...
            foreach (QMCustomButton cb in config.CustomQuickButtons)
            {
                ...
                if (row >= quickMenuContent.buttons.Length)
                    continue;
                var rowButtons = quickMenuContent.buttons[row];
                if (rowButtons.Count >= rowButtons.Capacity || index >= rowButtons.Capacity)
                    continue;
                rowButtons.Insert(index, cb.Button);
            }

            UserInfoContent userInfoContent = new UserInfoContent()
            {
                buttons = new UserInfoButtonArray[10]
                {
                    new UserInfoButtonArray
                    {
                        new Button("Teleport Self To", "userinfo.action", "teleportselfto")
                        {
                            Permission = "userinfo.teleportselfto"
                        },
                        new Button("Teleport To Self", "userinfo.action", "teleporttoself")
                        {
                            Permission = "userinfo.teleporttoself"
                        },
                        new Button("Teleport To Auth", "userinfo.action", "teleporttoauth")
                        {
                    ...
                    new UserInfoButtonArray
                    {
                        new Button("Heal", "userinfo.action", "heal")
                        {
                            Permission = "userinfo.fullheal"
                        },
                        new Button("Heal 50%", "userinfo.action", "heal50")
                        {
                            Permission = "userinfo.halfheal"
                        },
                        new Button("Teleport to\nDeathpoint", "userinfo.action", "teleporttodeathpoint")
                        {
                    ...
                    new UserInfoButtonArray
                    {
                        new Button("View Inventory", "userinfo.action", "viewinv")
                        {
                            Permission = "userinfo.viewinv"
                        },
                        new Button("Unlock Blueprints", "userinfo.action", "unlockblueprints")
                        {
                            Permission = "userinfo.unlockblueprints"
                        },
                        new Button("Spectate", "userinfo.action", "spectate")
                        {
                    ...
                    new UserInfoButtonArray
                    {
                        new Button("Mute", "userinfo.action", "mute")
                        {
                            Permission = "userinfo.mute"
                        },
                        new Button("Unmute", "userinfo.action", "unmute")
                        {
                    ...
                    {
                        null,
                        new Button("<color=olive>Strip Inventory</color>", "userinfo.action", "stripinventory")
                        {
                            Permission = "userinfo.stripinventory"
                        },
                        new Button("<color=olive>Revoke Blueprints</color>", "userinfo.action", "revokeblueprints")
                        {
                    ...
                    new UserInfoButtonArray
                    {
                        new Button("Kill", "userinfo.action", "kill")
                        {
                            Permission = "userinfo.kill"
                        },
                        new Button("<color=red>Kick</color>", "userinfo.action", "kick", "showpopup")
                        {
                            Permission = "userinfo.kick"
                        },
                        new Button("<color=red>Ban</color>", "userinfo.action", "ban", "showpopup")
                        {
                            Permission = "userinfo.ban"
                        },
                    },
            ...
            foreach (UserInfoCustomButton cb in config.UserInfoCustomButtons)
            {
                int row = cb.Position[0];
                int index = cb.Position[1];
                if (row >= userInfoContent.buttons.Length)
                    continue;
                var rowButtons = userInfoContent.buttons[row];
                if (rowButtons.Count >= rowButtons.Capacity || index >= rowButtons.Capacity)
                    continue;
                rowButtons.Insert(index, cb.Button);
            }
