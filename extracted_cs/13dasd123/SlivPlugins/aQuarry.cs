 ...
 * 
 * Copyright © 2022-2023 AvG Лаймон (Email: alias.dev@yandex.ru | Discord: AvG Лаймон#0680) */
...
namespace Oxide.Plugins
{
    ...
    class aQuarry : RustPlugin
    {
        ...
        {
            foreach (MiningQuarry entity in BaseNetworkable.serverEntities.OfType<MiningQuarry>())
                if (cfg.Quarries.ContainsKey(entity.skinID) && cfg.Quarries[entity.skinID].Fuel.Custom && entity.HasFlag(BaseEntity.Flags.On))
                    activated.Add(entity.net.ID.Value);
            SaveData();
        }
        #endregion
        #region [Data]
        ...
        private HashSet<ulong> activated = new HashSet<ulong>();
        private void SaveData() => Interface.Oxide.DataFileSystem.GetFile($"{Name}_cash").WriteObject(activated);
        private void LoadData() => activated = Interface.Oxide.DataFileSystem.GetFile($"{Name}_cash").ReadObject<HashSet<ulong>>();
        #endregion
        ...
        private void UpdateConfigValues()
        {
            ...
            if (cfg.PluginVersion < new VersionNumber(1, 1, 4))
            {
        ...
        private class QuarrySetting
        {
            ...
            [JsonProperty(Rus ? "Инвентарь" : "Inventory")] internal QInventory Inventory = new QInventory();
        ...
        {
            if (plan == null || obj == null || !cfg.Quarries.ContainsKey(plan.skinID)) return;
            MiningQuarry quarry;
            BasePlayer player = plan.GetOwnerPlayer();
            if (cfg.Quarries[plan.skinID].SubPrefab.Custom)
            {
        ...
        private object OnQuarryConsumeFuel(MiningQuarry quarry, Item item)
        {
            ...
            if (set.Custom)
                item = quarry.fuelStoragePrefab.instance.GetComponent<StorageContainer>().inventory.FindItemByItemName(set.ShortName);
            if (item == null)
            {
        ...
        private void UpdateQuarry(MiningQuarry quarry, BasePlayer player = null)
        {
            ...
            fuel.inventory.capacity = set.Inventory.FuelSize;
            loot.skinID = quarry.skinID;
            loot.OwnerID = quarry.OwnerID;
            loot.inventory.capacity = set.Inventory.LootSize;
            ...
            if (set.Fuel.Custom)
            {
                ...
                if (player == null && activated.Contains(quarry.net.ID.Value)) 
                    quarry.EngineSwitch(true);
            }
            quarry.SendNetworkUpdate();
