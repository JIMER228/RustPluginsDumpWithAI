...
namespace Oxide.Plugins
{
    ...
    class Giveaways : RustPlugin
    {
        ...
        private void RunEvent(ConfigData.Automation.Prize prize)
        {
            ...
            {
                int amount = 0;
                if (!int.TryParse(prize.Amount.ToString(), out amount))
                    return;
            ...
            else
            {
                ...
                ulong skinId = 0UL;
                ulong.TryParse(prize.SkinID.ToString(), out skinId);
        ...
        {
            if (winnerData.rewardData is StoredData.EconomicsData || winnerData.rewardData is StoredData.ServerRewardsData)
            {
                        ...
                        case "inventory":
                            {
                                ...
                                ProcessEvent(new StoredData.InventoryData(player.inventory.AllItems()));
                                return;
                            }
                        default:
        ...
        public class StoredData
        {
            ...
            {
                List<JObject> list;
                if (!pendingRewards.TryGetValue(playerId, out list))
            ...
            {
                List<JObject> list;
                if (!pendingRewards.TryGetValue(player.userID, out list))
                    return;
            ...
            public class InventoryData : RewardData
            {
                ...
                public InventoryData(Item[] items)
                {
                    ...
                    {
                        this.items.Add(new ItemData(items[i]));
                    }                    
                }
