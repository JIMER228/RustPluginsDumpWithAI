...
{
    [Info("SkinBox", "RustPlugins.ru", "2.2.3")]
    ...
    class SkinBox : ChaosPlugin
    {
        ...
        private object CanMoveItem(Item item, PlayerInventory inventory, ItemContainerId targetContainerID, int targetSlot, int amount, bool flag)
        {
            if (item.parent?.entityOwner)
            {
                LootHandler lootHandler = item.parent.entityOwner.GetComponent<LootHandler>(); 
        ...
        private LootHandler CreateSkinBox<T>(BasePlayer player, DeployableHandler.ReskinTarget reskinTarget) where T : LootHandler
        {
            ...
            container.inventorySlots = typeof(T) == typeof(SkinSetHandler) ? 12 : 48;
        ...
        private class DeployableHandler : LootHandler
        {
            ...
            {
                _filteredSkins = Pool.GetList<ulong>();
        ...
        private class LootHandler : MonoBehaviour
        {
            ...
            {                
                _availableSkins = Pool.GetList<ulong>();
                _filteredSkins = Pool.GetList<ulong>();
            ...
            protected virtual void OnDestroy()
            {
                ...
                Pool.FreeList(ref _availableSkins);
                Pool.FreeList(ref _filteredSkins);
            ...
            private class InputItem
            {
                ...
                internal InputItem(string shortname, Item item)
                {
                    ...
                    {
                        contents = Pool.GetList<InputItem>();
        ...
        private class SkinSetHandler : LootHandler
        {
            ...
            internal override BasePlayer Looter
            {
                ...
                set
                {
                    ...
                    {
                        for (int i = 0; i < _maximumPages - _sets.Count; i++)
                            _sets.Add(new StoredData.SetItem[12]);
                    }
                    
                    _itemList = Pool.GetList<Item>();
            ...
            private int CalculateCost()
            {
                ...
                for (int i = 0; i < currentSet.Length; i++)
                {
                    ...
                    if (itemDefinition)
                    {
                        if (_itemList.Any(x => x.info.shortname == setItem.Shortname && x.skin != setItem.SkinId))
        ...
        private class SprayCanWatcher : MonoBehaviour
        {
            ...
            private const int LAYER_MASK = 1 << 3 | 1 << (int) Rust.Layer.Construction | 1 << (int) Rust.Layer.Deployed;
            private void Awake()
            {
            ...
            private void LateUpdate()
            {
                ...
                if (m_Player.serverInput.WasJustPressed(BUTTON.FIRE_PRIMARY))
                {
                    ...
                    if (activeItem == null || activeItem.skin != SPRAYCAN_SKIN)
                    {
                        Destroy(this);
                        return;
                    }
                    ...
                    if (Physics.SphereCast(m_Player.eyes.HeadRay(), 0.1f, out m_RaycastHit, 3f, LAYER_MASK, QueryTriggerInteraction.Ignore))
                    {
                        ...
                        if (baseEntity is WorldItem)
                        {
                            ...
                            LootHandler lootHandler = Instance.CreateSkinBox<LootHandler>(m_Player, null);

                            Item item = (baseEntity as WorldItem).item;
                            (baseEntity as WorldItem).RemoveItem();
                            lootHandler.Entity.inventory.Insert(item);
                        }
                        else
                        {
                            ItemDefinition itemDefinition;
                            if (!DeployableHandler.GetItemDefinitionForEntity(baseEntity, out itemDefinition, false))
                                return;
        ...
        private void ccmdSkinBoxSprayCan(ConsoleSystem.Arg arg)
        {
            ...
            List<BasePlayer> candidates = Pool.GetList<BasePlayer>();
