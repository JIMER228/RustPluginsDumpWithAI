...
{
    [Info("PowerPlantEvent", "KpucTaJl", "2.0.9")]
    internal class PowerPlantEvent : RustPlugin
    {
        ...
        private object OnEntityKill(BaseEntity entity)
        {
            ...
            if (!_controller.KillEntities && _controller.Entities.Contains(entity)) return true;
            return null;
        }
        ...
        private void Start()
        {
            ...
            {
                Puts("PowerPlantEvent has begun");
                Subscribes();
                _controller = new GameObject().AddComponent<ControllerPowerPlantEvent>();
                Interface.Oxide.CallHook("OnPowerPlantEventStart", _controller.transform.position, Radius);
                if (_config.PveMode.Pve && plugins.Exists("PveMode"))
                {
                    ...
                    PveMode.Call("EventAddPveMode", Name, config, _controller.transform.position, Radius, new HashSet<ulong>(), _controller.Scientists.Select(x => x.net.ID), new HashSet<ulong>(), new HashSet<ulong>(), new HashSet<ulong>(), null);
                }
        ...
        private void Finish()
        {
            ...
            Puts("PowerPlantEvent has ended");
            if (_config.EnabledTimer)
            {
                timer.In(UnityEngine.Random.Range(_config.MinStartTime, _config.MaxStartTime), () =>
                {
        ...
        internal class ControllerPowerPlantEvent : FacepunchBehaviour
        {
            ...
            private void SpawnWorkcart()
            {
                ...
                Train = GameManager.server.CreateEntity("assets/content/vehicles/trains/workcart/workcart.entity.prefab", pos, rot) as TrainEngine;
            ...
            private void SpawnTrainEntities()
            {
                ...
                GroundWatch groundWatch = _trainWaterBarrel.GetComponent<GroundWatch>();
                if (groundWatch != null) DestroyImmediate(groundWatch);
                DestroyOnGroundMissing destroyOnGroundMissing = _trainWaterBarrel.GetComponent<DestroyOnGroundMissing>();
                if (destroyOnGroundMissing != null) DestroyImmediate(destroyOnGroundMissing);
                _trainWaterBarrel.Spawn();
                _trainWaterBarrel.pickup.enabled = false;
        ...
        private object OnCustomLootNPC(ulong netID)
        {
            if (_controller == null) return null;
            ScientistNPC entity = _controller.Scientists.FirstOrDefault(x => x.IsExists() && x.net.ID.Value == netID);
            if (entity != null)
            {
                PresetConfig preset = _config.NpcStart.FirstOrDefault(x => x.Config.Name == entity.displayName);
                if (preset == null) preset = _config.NpcTrain.FirstOrDefault(x => x.Config.Name == entity.displayName);
        ...
        private object OnCustomLootContainer(ulong netID)
        {
            if (_controller == null) return null;
            if (_controller.TrainCrate.IsExists() && _controller.TrainCrate.net.ID.Value == netID) return true;
            else if (_controller.Crates.Any(x => x.IsExists() && x.net.ID.Value == netID))
            {
                if (_config.TypeLootTableCrates == 3) return null;
                else return true;
            }
            else if (_controller.HackCrates.Any(x => x.IsExists() && x.net.ID.Value == netID))
            {
        ...
        private void SendBalance()
        {
            ...
            Interface.Oxide.CallHook("OnPowerPlantEventWinner", winnerId);
