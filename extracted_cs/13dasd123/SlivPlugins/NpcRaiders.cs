...
namespace Oxide.Plugins
{
    ...
    public class NpcRaiders : RustPlugin
    {
        ...
        [PluginReference]
        private Plugin Kits, TruePVE, ServerRewards, Economics, LifeSupport;
        ...
        private static int groundLayerWater;
        ...
        private void OnServerInitialized()
        {
            ...
            groundLayerWater = LayerMask.GetMask("Terrain", "World");
        ...
        public class mlrsConfig
        {
            [JsonProperty(PropertyName = "Fire On Wave Start")]
            public int wave;
            [JsonProperty(PropertyName = "Total To Fire")]
        ...
        public class raidConfig
        {
            ...
            [JsonProperty(PropertyName = "Mlrs waves and amounts")]
            public Dictionary<int, int> mlrsOption;
            ...
            public raidConfig(int TotalNpcs, bool OnlyAuthedMembers, float Health, int RunTimeSeconds, List<string> kits, List<string> explosive, int waves, string perms, int reward, double damage)
            {
                ...
                this.mlrsOption = new Dictionary<int, int>() { { 1, 6 } };
            }
        }
        ...
        public class EventRaidManager
        {
            ...
            internal List<int> MlrsEventFired = new List<int>();
            internal int MlrsWaves = 0;
            ...
            internal static void setupMlrs(EventRaidManager man, int totalToFire, float damage = 1.0f)
            {
                if (isFireingMlrs || man == null || man.building == null) return;

                _.timer.Repeat(0.5f, totalToFire, () =>
                {
                    ...
                    if (CreateAndSpawnRocket(startPoint, aimToTarget, damage, out projectile) == false)
                        return;
            ...
            internal static bool CreateAndSpawnRocket(Vector3 firingPos, Vector3 firingDir, float damage, out ServerProjectile mlrsRocketProjectile)
            {
                RaycastHit hitInfo;
                ...
                TimedExplosive component = mlrsRocket.GetComponent<TimedExplosive>();
                component.SendMessage("SetDamageScale", damage);
                component.timerAmountMin = 20f;
                component.timerAmountMax = 30f;
               // component.explosionRadius += 10f;
            mlrsRocket.Spawn();
            ...
            internal void update()
            {
               ...
               /* if (config.FireMlrs && MlrsEventTime < Time.time && !EventRaidManager.isFireingMlrs && MlrsEventTime != 0)
                ...
                if (config.FireMlrs && config.mlrsOption.ContainsKey(MlrsWaves) && !MlrsEventFired.Contains(MlrsWaves))
                {
                    MlrsEventFired.Add(MlrsWaves);
                    setupMlrs(this, config.mlrsOption[MlrsWaves]);
                }
            ...
            internal static List<string> _prefabs = new List<string> { "rock", "formation", "cliff", "dunes" };
        ...
        public class NpcRaider : NPCPlayer, IAISenses, IAIAttack, IThinker
        {
            ...
            public override BaseCorpse CreateCorpse(BasePlayer.PlayerFlags flagsOnDeath, Vector3 posOnDeath, Quaternion rotOnDeath, List<TriggerBase> triggersOnDeath)
            {
