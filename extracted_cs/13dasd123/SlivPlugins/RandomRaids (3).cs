...
namespace Oxide.Plugins
{
    ...
    public class RandomRaids : RustPlugin
    {
        ...
        //  private const string easyPerm = "randomraids.easy";
        //   private const string hardPerm = "randomraids.hard";
        //   private const string expertPerm = "randomraids.expert";
        //   private const string nightmarePerm = "randomraids.nightmare";
        ...
        [PluginReference]
        private Plugin Kits, TruePVE, RaidableBases, CustomLoot, BotReSpawn;
        ...
        private void Init()
        {
            ...
            RAIDDATA = Interface.Oxide.DataFileSystem.GetFile(Name + "/RaidData");
            LOGDATA = Interface.Oxide.DataFileSystem.GetFile(Name + "/PlayerLog");
            LoadData();

            if (configData.raidSettings.raidTypes.Count <= 0)
            {
                configData.raidSettings.raidTypes.Add("easy", new raidConfig(1, 10, true, 100, 640, new List<string>(), new List<string>() { "explosive.timed", "explosive.satchel" }, 1, 1.0, false, new List<string>() { "easy" }, 200));
                configData.raidSettings.raidTypes.Add("medium", new raidConfig(2, 15, true, 200, 900, new List<string>(), new List<string>() { "explosive.timed", "explosive.satchel" }, 1, 1.0, true, new List<string>() { "easy" }, 400));
                configData.raidSettings.raidTypes.Add("hard", new raidConfig(3, 20, true, 400, 1200, new List<string>(), new List<string>() { "explosive.timed", "explosive.satchel" }, 1, 1.0, true, new List<string>() { "easy" }, 600));
                configData.raidSettings.raidTypes.Add("expert", new raidConfig(4, 30, true, 400, 1500, new List<string>(), new List<string>() { "explosive.timed", "explosive.satchel" }, 1, 1.0, true, new List<string>() { "medium" }, 800));
                configData.raidSettings.raidTypes.Add("nightmare", new raidConfig(5, 30, true, 400, 2100, new List<string>(), new List<string>() { "explosive.timed", "explosive.satchel" }, 1, 1.0, true, new List<string>() { "medium" }, 1000));
                SaveConfig();
            }
            ...
            {
                configData.settingsRB.raidSettings.Add(0, new RBConfig(3, false, 10, 320, new List<string>() { "easy" }));
                configData.settingsRB.raidSettings.Add(1, new RBConfig(3, false, 10, 320, new List<string>() { "easy", "medium" }));
                configData.settingsRB.raidSettings.Add(2, new RBConfig(3, false, 10, 320, new List<string>() { "easy", "medium" }));
                configData.settingsRB.raidSettings.Add(3, new RBConfig(3, false, 10, 320, new List<string>() { "easy", "medium", "hard" }));
                configData.settingsRB.raidSettings.Add(4, new RBConfig(3, false, 10, 320, new List<string>() { "hard", "expert", "nightmare" }));
                SaveConfig();
            }
        ...
        private void Unload()
        {
            ...
            StopAllRaids();
            foreach (BasePlayer current in BasePlayer.activePlayerList.ToList())
            {
        ...
        {
            foreach (var eventRunning in EventRandomManager._AllRaids.ToList())
            {
        ...
        private void LoadData()
        {
            ...
            try
            {
                raidData = Interface.GetMod().DataFileSystem.ReadObject<RaidData>(Name + "/RaidData");
            }
            catch
            {
                PrintWarning("Couldn't load Raid data, creating new RaidData file");
        ...
        class ConfigData
        {
            ...
            public class Settings
            {
                ...
                [JsonProperty(PropertyName = "Taunt players with .mp3 voices")]
        ...
        private ConfigData GetBaseConfig()
        {
            return new ConfigData
            {
                settings = new ConfigData.Settings
                {
                    ...
                    useChat = true,
                    useMarker = true,
                    tauntList = new List<string>(), /*{ "https://dl.dropboxusercontent.com/s/61743zv8vh6r8j8/088512791-fantasy-lizard-monster-voice-i.mp3",
      "https://dl.dropboxusercontent.com/s/a6t5uv26qcz5nhs/111605736-demon-voice-nice-try.mp3https://dl.dropboxusercontent.com/s/a6t5uv26qcz5nhs/111605736-demon-voice-nice-try.mp3",
      "https://dl.dropboxusercontent.com/s/zh2jp80zoy9oinp/145014836-male-vocal-video-game-taunt-5.mp3" }*/
                    pauseWave = false,
                    musthavePerm = false,
                    worldMarker = false,
                    worldMarkerTime = 60f
        ...
        public class raidConfig
        {
            ...
            public raidConfig(int TotalAuth, int TotalNpcs, bool OnlyAuthedMembers, float Health, int RunTimeSeconds, List<string> kits, List<string> explosive, int waves, double damage, bool attackCopter, List<string> jconfig, int surrenderCosts1)
            {
        ...
        {
            EventRandomManager RM { get; set; }

            public void Setup(EventRandomManager R)
            {
                RM = R;
            }
        ...
        public class EventRandomManager
        {
            internal static List<EventRandomManager> _AllRaids = new List<EventRandomManager>();
            internal static List<RandomRaider> _AllMembers = new List<RandomRaider>();
            internal List<juggernautConfig> jconfig = new List<juggernautConfig>();
            internal static bool isFireingMlrs;
            internal List<BuildingBlock> _AllFoundations = new List<BuildingBlock>();
            internal List<ulong> _authedPlayers = new List<ulong>();

            internal List<RandomRaider> members;
            ...
            internal PatrolHelicopter heli;
            internal MapMarkerExplosion marker;
            internal DateTime lastTaunt;
            internal DeployableBoomBox boombox;
            internal List<Vector3> savedPosition = new List<Vector3>();
            internal List<BasePlayer> _onlinePlayers = new List<BasePlayer>();
            internal DateTime activeTime = DateTime.Now;
            internal int Duration;
            internal DateTime time = DateTime.Now;
            internal TimeSpan timeOffset;
            internal DateTime lastBlock = DateTime.Now;
            internal DateTime lastNotification = DateTime.MinValue;
            internal DateTime lastUINotification = DateTime.MinValue;
            internal int currentWave = 0;
            internal bool sendGUI = false;
            internal bool showFinle = false;
            internal bool curentlySawning = false;
            internal bool showInfinity = false;

            internal static EventRandomManager Create(Vector3 l, raidConfig c, string t, RaidThinkManager timer, List<ulong> authedPlayers, GameObject gobj, List<BuildingBlock> blocks)
            {
                EventRandomManager manager = new EventRandomManager
                {
                    members = Pool.GetList<RandomRaider>(),
                    newWave = true,
                    config = c,
                    waveType = t,
                    location = l,
                    timerObject = timer,
                    _authedPlayers = authedPlayers,
                    thetimerObject = gobj,
                    _AllFoundations = blocks,
                    lastTaunt = DateTime.Now.AddSeconds(20)
                };
            ...
            public static void ChangeUrlAndPlay(BoomBox boxController, string url, Vector3 npcLoc, Quaternion rotation)
            {
                if (boxController != null)
                {
                    if (!boxController.IsPowered())
                        (boxController.baseEntity as IOEntity).UpdateFromInput(100, 0);

                    boxController.CurrentRadioIp = url;
                    boxController.ServerTogglePlay(false);
                    boxController.baseEntity.ClientRPC<string>(null, "OnRadioIPChanged", boxController.CurrentRadioIp);
                    boxController.baseEntity.transform.position = npcLoc + Vector3.down * 3;
                    boxController.baseEntity.transform.rotation = rotation;
                    boxController.baseEntity.transform.hasChanged = true;
                    boxController.baseEntity.SendNetworkUpdateImmediate();
            ...
            // Thanks to HellFire on UMOD
            internal static void setupMlrs(EventRandomManager man)
            {
                ...
                _.timer.Repeat(0.5f, 6, () =>
                {
                    ...
                    projectile.gravityModifier = 1.5f;//baseGravity / (0f - Physics.gravity.y);
                });
            }
            ...
            internal static Vector3 GetAimToTarget(Vector3 startPosition, Vector3 targetPos, out float baseGravity)
            {
                ...
                //vector = new Vector3(4.8654f, 2.6241f, 1.3869f);
                float num = 90f;
            ...
            internal void update()
            {
                ...
                else
                {
                    ...
                    else if (curentlySawning && _AllMembers.Count <= 0 && lastBlock.AddSeconds(Duration) - DateTime.Now <= TimeSpan.Zero)
                    {
                        Duration = 30;
                        TimeSpan timeOffset = new TimeSpan();
                        lastBlock = DateTime.Now;
                ...
                /* if ( lastTaunt < DateTime.Now && members.Count > 0 && _.configData.settings.tauntList != null && _.configData.settings.tauntList.Count > 0)
                 {
                     lastTaunt = DateTime.Now.AddSeconds(15);
                     RandomRaider speakerMember = members.GetRandom();
                     if (_ != null && speakerMember != null && !speakerMember.IsDestroyed)
                     {
                         Quaternion newVec = Quaternion.Euler((speakerMember.transform.position - location));
                         if (boombox == null)
                             boombox = SpawnBox(speakerMember.transform.position, newVec);
                         if (boombox != null)
                         {      
                             ChangeUrlAndPlay(boombox.BoxController, _.configData.settings.tauntList.GetRandom(), speakerMember.transform.position, newVec);
                         }
                     }
                 }*/
            }
            ...
            {
                foreach (var user in _authedPlayers.ToList())
                {
            ...
            internal void SendClockGUI(BasePlayer current)
            {
                if (current == null)
                    return;
                ...
                string message = string.Format(_.lang.GetMessage("guiMessage", _), currentWave, totalWavesNext, _AllMembers.Count, waveMessage, countDownM, countDownS);
                string message2 = string.Format(_.lang.GetMessage("guiMessageSurrender", _), config.surrenderCosts);

                if (_.configData.settings.pauseWave)

                {
                    if (_AllMembers.Count > 0 || showInfinity)
                        message = string.Format(_.lang.GetMessage("guiMessage", _), currentWave, totalWavesNext, _AllMembers.Count, waveMessage, "∞", "∞");
                }
            ...
            public static DeployableBoomBox SpawnBox(Vector3 spawnLoc, Quaternion rot)
            {
                DeployableBoomBox box = GameManager.server.CreateEntity("assets/prefabs/voiceaudio/boombox/boombox.deployed.prefab", spawnLoc, rot) as DeployableBoomBox;
                if (box == null) return null;
                box.enableSaving = false;

                box.transform.hasChanged = true;
                box.Spawn();
            ...
            internal void RemoveNpc(RandomRaider player)
            {
            ...
            internal void Destroy(bool win = false)
            {
                ...
                _.NextTick(() =>
                {
                    ...
                    Pool.FreeList(ref _authedPlayers);
                    _AllFoundations.Clear();
                    Pool.FreeList(ref _AllFoundations);
            ...
            internal void stopSpawning(bool destroying = false)
            {
                if (QueuedRoutine != null)
            ...
            #region Spawning
            internal RandomRaider SpawnNPC(Vector3 pos, juggernautConfig isJug = null)
            {
            ...
            private void GetRaidReady()                      //fix
            {
                ...
                if (nextRaidTime < Time.time)
                {
                    ...
                    if (this == null || this.IsDestroyed || this.IsDead())
                        return;
                    if (currentTargetOverride == null && manager._AllFoundations.ToList().Count > 0)
                    {
                        currentTargetOverride = manager._AllFoundations.ToList().GetRandom();  // fix maybe need to remove nulls
            ...
            public class BaseLauncherState : BasicAIState
            {
                    ...
                    if (nextStrafeTime < Time.time)
                    {
                        ...
                        if (!isFireing && nextFireTime < Time.time && entityAI.Brain.spawnAge + 10 < Time.time)
                        {
                            ...
                            else
                            {
                                ...
                                entityAI.StartCoroutine(WeaponFireTest(position, 6));
                ...
                public IEnumerator WeaponFireTest(Vector3 locations, int slot, bool rocket = true)
                {
                    ...
                    if (entityAI == null || entityAI.IsDestroyed)
                        ServerMgr.Instance.StopCoroutine(WeaponFireTest(locations, slot, rocket));
        ...
        private void surrender(BasePlayer player, string command, string[] args)
        {
            ...
            foreach (EventRandomManager eventRunning in EventRandomManager._AllRaids)
            {
                ...
                if (distance < helperDistance && eventRunning._authedPlayers.Contains(player.userID))
                    nearestEvent = eventRunning;
            }

            if (nearestEvent != null) {
                var itemDef = ItemManager.FindItemDefinition("scrap");
                int npcCount = nearestEvent.members.Count;
                int totalCosts = nearestEvent.config.surrenderCosts;

                if (itemDef != null) {
                    if (player.inventory.GetAmount(itemDef.itemid) >= totalCosts) {
                        player.inventory.Take(null, itemDef.itemid, totalCosts);

                        foreach (var user in nearestEvent._authedPlayers.ToList())
                        {
                            BasePlayer TCplayer = BasePlayer.FindByID(user);
                            if (TCplayer != null)
                                SendReply(TCplayer, lang.GetMessage("payedAndSurrendered", this, TCplayer.UserIDString), player.displayName, totalCosts);
                        }

                        nearestEvent.Destroy();
                    } else {
                        SendReply(player, lang.GetMessage("negotiateMessage", this, player.UserIDString), totalCosts);
                    }
                }
            }
        }
        ...
        public static void dropWin(Vector3 location, int amount, List<string> custloot, List<ulong> playerID)
        {
            for (int i = 0; i < amount; i++)
            {
                ...
                if (entity != null)
                {
                    ...
                    //entity.inventory.maxStackSize = 900000000;
                    entity.onlyAcceptCategory = ItemCategory.All;
                    entity.SendNetworkUpdateImmediate();
                    Interface.Oxide.CallHook("OnRandomRaidWin", entity, playerID);
                    //Interface.CallHook("OnSupplyDropDropped", (object)entity, null);
                    _.NextTick(() =>
                    {
                        if (entity != null && custloot != null && custloot.Count > 0)
                        {
        ...
        {
            List<ulong> tcDataTemp = Pool.GetList<ulong>();
            List<ProtoBuf.PlayerNameID> authedTemp = Pool.GetList<ProtoBuf.PlayerNameID>();
            ...
            timer.Once(10f, () =>
            {
                ...
                if (raiders.Count > 0 && configData.settingsRB.raidSettings.ContainsKey(mode))
                {
                    ...
                    if (rando <= config.raidChance && pcdData.tcData.Count > 0)
                    {
                        ...
                        foreach (var p in raiders)
                        {
                            ...
                            if (pcdData.tcData.ContainsKey(p.userID))
                            {
                                if (debug) PrintWarning($"Player: {p.displayName} Does Have Tc Data");

                                int[] totalRaids = RaidableBases.CallHook("GetPlayerAmount", p.UserIDString, mode) as int[];

                                if (totalRaids == null || totalRaids.Length <= 0 || totalRaids[0] + 1 < config.totalRaids)
                                {
                                    if (debug) PrintWarning($"Made it to this Section TotalRaids and failed");
                                    continue;
                                }
                                else
                                {
                                    if (debug) PrintWarning($"Player: {p.displayName} Has A Total Of: {totalRaids[0]} Completed Raids");
                                }

                                if (debug) PrintWarning($"Testing Total Bases Raids for mode Player: {p.displayName} Total: {totalRaids[0]}");

                                tcDataTemp.AddRange(pcdData.tcData[p.userID]);
                                ...
                                foreach (ulong key in tcDataTemp)
                                {
                                    ...
                                    if (privFind == null)
                                    {
                                        ...
                                        pcdData.tcData[p.userID].Remove(key);
                                        SaveData();
                                    }
                                    else if (priv == null)
                                    {
                ...
                Pool.FreeList(ref tcDataTemp);
                Pool.FreeList(ref authedTemp);
        ...
        private string GetDirectionAngle(float angle, string UserIDString) //Thanks to FancyDrop
        {
        ...
        internal IEnumerator FindRandomBaseToRaid(bool offline = false, bool useChance = true, BasePlayer player = null)
        {
            ...
            if (useChance)
            {
                ...
                {
                    stopSearch();
                    yield return CoroutineEx.waitForSeconds(10);
                }
            }
            ...
            foreach (var p in !offline ? BasePlayer.allPlayerList.ToList() : BasePlayer.activePlayerList.ToList())
            {
                if (configData.settings.musthavePerm && !permission.UserHasPermission(p.UserIDString, usePerm))
                    continue;
                ...
                if (pcdData.tcData.ContainsKey(p.userID))
                {
                    foreach (ulong key in pcdData.tcData[p.userID].ToList())
                    {
                        type = "";
                        ...
                        else if (priv == null)
                        {
                            ...
                            {
                                if (total >= keyNext.Value.TotalAuth)
                                    type = keyNext.Key;
                            }

                            if (!string.IsNullOrEmpty(type))
                            {
                                foreach (var user in privFind.authorizedPlayers.ToList())
                                {
        ...
        private bool CanBeRaidedFlair(BuildingPrivlidge priv)
        {
            if (priv != null)
            {
                foreach (var eventRunning in EventRandomManager._AllRaids)
        ...
        public static void startRandomRaid(Vector3 loc, List<ulong> authed, string type, List<BuildingBlock> blocks)
        {
            if (_.configData.raidSettings.raidTypes.ContainsKey(type))
            {
                ...
                EventRandomManager managers = EventRandomManager.Create(loc, _.configData.raidSettings.raidTypes[type], type, newThinker, authed, theObject, blocks);
                newThinker.Setup(managers);
                managers.Start();
            }
        }

        static List<BuildingBlock> GenerateTargets(BuildingPrivlidge priv)
        {
            ...
            else
            {
                ...
                foreach (BuildingBlock found in _AllFoundations.ToList())
                {
                    if (found != null && !found.blockDefinition.canRotateAfterPlacement)
            ...
            NextTick(() =>
            {
                if (privilege != null && player != null && privilege.IsAuthed(player))
                {
                    if (!pcdData.tcData.ContainsKey(player.userID))
                        pcdData.tcData.Add(player.userID, new List<ulong>() { privilege.net.ID.Value });
                    else if (!pcdData.tcData[player.userID].Contains(privilege.net.ID.Value))
        ...
        private void OnCupboardDeauthorize(BuildingPrivlidge privilege, BasePlayer player)
        {
            if (pcdData.tcData.ContainsKey(player.userID))
                if (pcdData.tcData[player.userID].Contains(privilege.net.ID.Value))
                    pcdData.tcData[player.userID].Remove(privilege.net.ID.Value);
                SaveData();
        }

        private object OnCupboardClearList(BuildingPrivlidge privilege, BasePlayer player)
        {
            if (pcdData.tcData.ContainsKey(player.userID) && pcdData.tcData[player.userID].Contains(privilege.net.ID.Value))
                pcdData.tcData[player.userID].Remove(privilege.net.ID.Value);
        ...
        {
            if (!configData.settingsNPCKILLS.enableKills || player == null || info == null || info.InitiatorPlayer == null || !info.InitiatorPlayer.userID.IsSteamId()) return;           
            ...
            {  
                ...
                if (Convert.ToBoolean(BotReSpawn?.CallHook("IsBotReSpawn", player.userID)))
                    return;

                if (!logData.pLogs.ContainsKey(info.InitiatorPlayer.userID))
                    logData.pLogs.Add(info.InitiatorPlayer.userID, new lData());

                if (configData.settingsNPCKILLS.enableKillsCombine && configData.settingsNPCKILLS.CombineNpcList.Contains(name))
                {
                    logData.pLogs[info.InitiatorPlayer.userID]._totalNpcKills++;
                    SaveLogData();
                    if (logData.pLogs[info.InitiatorPlayer.userID]._totalNpcKills >= configData.settingsNPCKILLS.CombineProfile.totalRaids)
                    {
                        CallRaidOnNpcKills(info.InitiatorPlayer, name, configData.settingsNPCKILLS.CombineProfile);
                        if (configData.settingsNPCKILLS.CombineProfile.resetKillCount)
                            logData.pLogs[info.InitiatorPlayer.userID]._totalNpcKills = 0;
                    }
                    return;
                }

                if (configData.settingsNPCKILLS.npcSettings.ContainsKey(name))
                { 
                    if (!logData.pLogs[info.InitiatorPlayer.userID].NPCKILLS.ContainsKey(name))
                        logData.pLogs[info.InitiatorPlayer.userID].NPCKILLS.Add(name, 1);
                    else
                        logData.pLogs[info.InitiatorPlayer.userID].NPCKILLS[name]++;
                    SaveLogData();
                    if (logData.pLogs[info.InitiatorPlayer.userID].NPCKILLS[name] >= configData.settingsNPCKILLS.npcSettings[name].totalRaids)
                    { 
                        ...
                        if(configData.settingsNPCKILLS.npcSettings[name].resetKillCount)
                            logData.pLogs[info.InitiatorPlayer.userID].NPCKILLS[name] = 0;
                    }
                }
            }           
        }
        ...
        void OnBotReSpawnNPCKilled(ScientistNPC player, string profilename, string group, HitInfo info)
        {
            ...
            if (!string.IsNullOrEmpty(profilename))
            {
                ...
                if (configData.settingsNPCKILLS.npcSettings.ContainsKey(profilename))
                {
                    if (!logData.pLogs.ContainsKey(info.InitiatorPlayer.userID))
                        logData.pLogs.Add(info.InitiatorPlayer.userID, new lData());
                    if (!logData.pLogs[info.InitiatorPlayer.userID].NPCKILLS.ContainsKey(profilename))
                        logData.pLogs[info.InitiatorPlayer.userID].NPCKILLS.Add(profilename, 1);
                    else
                        logData.pLogs[info.InitiatorPlayer.userID].NPCKILLS[profilename]++;
                    SaveLogData();
                    if (logData.pLogs[info.InitiatorPlayer.userID].NPCKILLS[profilename] >= configData.settingsNPCKILLS.npcSettings[profilename].totalRaids)
                    {
                        ...
                        if(configData.settingsNPCKILLS.npcSettings[profilename].resetKillCount)
                                logData.pLogs[info.InitiatorPlayer.userID].NPCKILLS[profilename] = 0;
                    }
                }
            }
        }
        ...
        private void CallRaidOnNpcKills(BasePlayer p, string Rtype, NPCConfig config)
        {
            if (p != null)
            {				
                if (configData.settings.musthavePerm && !permission.UserHasPermission(p.UserIDString, usePerm))
                ...
                if (rando <= config.raidChance && pcdData.tcData.Count > 0)
                {
                    ...
                    if (configData.settingsNPCKILLS.enforceAuth)
                    {
                        int totalAuth = priv.authorizedPlayers.Count();
                        List<string> raidsToUse = Pool.GetList<string>();

                        if (totalAuth > 0)
                        {
                            foreach (var keyNext in config.sendRaids)
                            {
                                if (configData.raidSettings.raidTypes.ContainsKey(keyNext) && totalAuth >= configData.raidSettings.raidTypes[keyNext].TotalAuth)
                                    raidsToUse.Add(keyNext);
                            }
                        }

                        type = raidsToUse.GetRandom();
                        Pool.FreeList(ref raidsToUse);
                    }
                    else
                        type = config.sendRaids.GetRandom();
                    ...
                    if (!string.IsNullOrEmpty(type))
                    {
                        ...
                        timer.Once(config.raidDelay, () =>
                        {
                                startRandomRaid(location, authPlayers, type, block);
                                LogToFile("NpcKillsRaids", $"[{DateTime.Now}] Started raid at {priv.transform.position}, For player {p.displayName}", this);
                        });
                    }
                }
            }
        ...
        private void OnExplosiveDropped(BasePlayer player, TimedExplosive entity)
        {
            OnExplosiveThrown(player, entity, null);
        }
        ...
        private bool CanstartFlareRaid(BasePlayer player, string configName)
        {
            ...
            foreach (var user in privCheck.authorizedPlayers.ToList())
            {
                authPlayers.Add(user.userid);
            }
        ...
        public class attackHelicopterAI : MonoBehaviour
        {
            ...
            private void UpdateMono()
            {
                ...
                {
                    strafe = DateTime.Now.AddSeconds(strafeTime);
                    heliAI.State_Strafe_Enter(location, false);
                    totalStrafe--;
                }
        ...
        private new void LoadDefaultMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
				...
				["negotiateMessage"] = "<color=orange>[</color><color=red>Cobalt Scientist</color><color=orange>]</color> {0} and we forget about this little mistake you made.",
				["payedAndSurrendered"] = "<color=orange>[</color><color=red>{0}</color><color=orange>]</color> surrendered by paying {1} scrap.",
                ["msgNorth"] = "North",
				...
				["guiMessageSurrender"] = "<color=#ce422b>Surrender with</color> /surrender <color=#ce422b>for</color> <color=#FFFF00>{0}</color> scrap.",
                ["nextWave"] = "Next Wave",
                ["endEvent"] = "Ends In",
