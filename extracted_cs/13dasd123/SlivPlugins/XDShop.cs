...
{
    /*ПЛАГИН БЫЛ ПОФИКШЕН С ПОМОЩЬЮ ПРОГРАММЫ СКАЧАНОЙ С https://discord.gg/dNGbxafuJn */ /*ПЛАГИН БЫЛ ПОФИКШЕН С ПОМОЩЬЮ ПРОГРАММЫ СКАЧАНОЙ С https://discord.gg/dNGbxafuJn */ [Info("XDShop", "https://discord.gg/dNGbxafuJn", "1.4.1")]
    [Description("Large easily customizable store")] 
    class XDShop : RustPlugin
    {
        ...
        private void CheckingProducts()
        {
            foreach (Configuration.CategoryShop category in _config.product)
            {
                foreach (Configuration.CategoryShop.Product product in category.product)
                {
                    ...
                    if (product.Type is ItemType.Item or ItemType.Blueprint or ItemType.CustomItem)
                    {
        ...
        {
            public DateTime Buy = new(1970, 1, 1, 0, 0, 0);
		   		 		  						  	   		  	 	 		  	  			  			 		  				
            public DateTime Sell = new(1970, 1, 1, 0, 0, 0);
        }
        ...
        private readonly Dictionary<string, ItemDisplayName> _itemName = new Dictionary<string, ItemDisplayName>();
                ...
                private Dictionary<ulong, ThemeType> _playerThemeSelect = new();
        private void Selling(BasePlayer player, int itemId, int category, int amount, ThemeType themetype)
        {
            ...
            Item[] playerItems = Enumerable.ToArray(Enumerable.Concat(player.inventory.containerMain?.itemList ?? Enumerable.Empty<Item>(), Enumerable.Concat(player.inventory.containerBelt?.itemList ?? Enumerable.Empty<Item>(), player.inventory.containerWear?.itemList ?? Enumerable.Empty<Item>())));
            ...
            {
                int cooldown = GetCooldownTime(player.userID.Get(), product, false);
                if (cooldown > 0)
                {
            ...
            switch (_config.economicsCustomization.TypeEconomic)
            {
                case EconomicsType.Economics:
                    if ((bool)Economics?.Call("Deposit", player.userID.Get(), (double)price))
                    {
                ...
                case EconomicsType.ServerRewards:
                    if (ServerRewards?.Call<object>("AddPoints", player.userID.Get(), (int)price) != null)
                    {
                        TakeItem(playerItems, product.ShortName, needAmount, product.SkinID, blueprint);
                        UpdateBalance(player, themetype);
                        NotificationUI(player, NotificationType.Success, GetLang("XDSHOP_UI_NOTIFICATION_SUCCESSFUL_SALE", player.UserIDString, product.GetProductName(player.UserIDString)));
                ...
                case EconomicsType.IQEconomic:
                    IQEconomic?.Call("API_SET_BALANCE", player.userID.Get(), (int)price);
                    TakeItem(playerItems, product.ShortName, needAmount, product.SkinID, blueprint);
                    UpdateBalance(player, themetype);
                    NotificationUI(player, NotificationType.Success, GetLang("XDSHOP_UI_NOTIFICATION_SUCCESSFUL_SALE", player.UserIDString, product.GetProductName(player.UserIDString)));
                    break;
                ...
                private void UICATEGORY(BasePlayer player, ThemeType themetype, int page = 0, int categoryIndex = 0)
        {
            ...
            foreach (Configuration.CategoryShop cat in category.Page(page, 6))
            {
                ...
                container.Add(new CuiElement
                {
                    ...
                    Components = {
                        new CuiRawImageComponent { Color = colorLine, Png = _imageUI.GetImage("18") },
                        ...
                        if (category.Count > 6)
            {
                ...
                if (page > 0)
                {
                    container.Add(new CuiElement
                    {
                        ...
                        Components = {
                        new CuiRawImageComponent { Color = theme.color1, Png = _imageUI.GetImage("3") },
                ...
                if (page + 1 < (int)Math.Ceiling((double)category.Count / 6))
                {
                    container.Add(new CuiElement
                    {
                        ...
                        Parent = "CATEGORY_PAGE",
                        Components = {
                            new CuiRawImageComponent { Color = theme.color1, Png = _imageUI.GetImage("4") },
                ...
                private float GetBalance(BasePlayer player)
        {
            switch (_config.economicsCustomization.TypeEconomic)
            {
                case EconomicsType.Economics:
                    if (!Economics)
                        return 0f;
                    return (float)Economics?.Call<double>("Balance", player.userID.Get());
                case EconomicsType.ServerRewards:
                    if (!ServerRewards)
                        return 0f;
                    return (int)ServerRewards?.Call("CheckPoints", player.userID.Get());
                case EconomicsType.IQEconomic:
                    if (!IQEconomic)
                        return 0f;
                    return (int)IQEconomic?.Call("API_GET_BALANCE", player.userID.Get());
                case EconomicsType.Item:
                    return GetItemAmount(Enumerable.ToArray(Enumerable.Concat(player.inventory.containerMain?.itemList ?? Enumerable.Empty<Item>(), Enumerable.Concat(player.inventory.containerBelt?.itemList ?? Enumerable.Empty<Item>(), player.inventory.containerWear?.itemList ?? Enumerable.Empty<Item>()))), _config.economicsCustomization.EconomicShortname, _config.economicsCustomization.EconomicSkinId);
                default:
                    return 0f;
            }
        }
        ...
        private void OnLeaveNPC(BasePlayer npc, BasePlayer player)
        {
            ...
            if (!_humanNpcPlayerOpen.ContainsKey(player.userID.Get())) return;
            _humanNpcPlayerOpen.Remove(player.userID.Get());
            if (_uiPlayerUseNow.Contains(player.userID.Get()))
                _uiPlayerUseNow.Remove(player.userID.Get());
        ...
        private void CmdConsoleHandler(ConsoleSystem.Arg args)
        {
            ...
            switch (args.Args[0])
            {
                ...
                case "AMOUNT_CHANGE_KEYBOARD":
                {
                    int index = int.Parse(args.Args[1]);
                    int indexCategory = int.Parse(args.Args[2]);
                    ThemeType theme = (ThemeType)Enum.Parse(typeof(ThemeType), args.Args[3]);
                    if (!int.TryParse(args.Args[4], out int amount))
                        amount = 1;
                    
                    CuiHelper.DestroyUi(player, ConfirmationsPanel);
                    UPDATEAMOUNTUI(player, index, indexCategory, amount, theme);
                ...
                case "THEME_SWITCH":
                {
                    ...
                    _playerThemeSelect[player.userID.Get()] = theme == ThemeType.Dark ? ThemeType.Light : ThemeType.Dark;
                    break;
                }
                case "CLOSE_ALL_UI":
                {
                    if (_config.humanNpcs.UseHumanNpcs && _humanNpcPlayerOpen.ContainsKey(player.userID.Get()))
                        _humanNpcPlayerOpen.Remove(player.userID.Get());
                    if (_uiPlayerUseNow.Contains(player.userID.Get()))
                        _uiPlayerUseNow.Remove(player.userID.Get());
                    CuiHelper.DestroyUi(player, "NOTIFICATION_POLOSA");
        ...
        private void OnPlayerDisconnected(BasePlayer player, string reason)
        {
            ...
            if (_config.humanNpcs.UseHumanNpcs)
                if (_humanNpcPlayerOpen.ContainsKey(player.userID.Get()))
                    _humanNpcPlayerOpen.Remove(player.userID.Get());
        }
        ...
        {
            string img = themetype == ThemeType.Dark ? _imageUI.GetImage("12") : _imageUI.GetImage("11");
            CuiElementContainer container = new CuiElementContainer
            {
                new CuiElement
                {
        ...
        private void OnUseNPC(BasePlayer npc, BasePlayer player)
        {
            if (!_config.humanNpcs.NPCs.ContainsKey(npc.UserIDString) ||
                _humanNpcPlayerOpen.ContainsKey(player.userID.Get())) return;
            _humanNpcPlayerOpen.Add(player.userID.Get(), npc.UserIDString);
            XDShopUI(player);
        }
        ...
        private void UIMAIN(BasePlayer player, ThemeType themetype)
        {
            ...
            container.Add(new CuiElement
            {
                ...
                Parent = MainLayer,
                Components = {
                    new CuiRawImageComponent { Color = theme.colorMainBG, Png = _imageUI.GetImage("0") },
            ...
            container.Add(new CuiElement
            {
                ...
                Parent = PanelProduct,
                Components = {
                    new CuiImageComponent { Color = theme.colorImgTitle, Png = _imageUI.GetImage("7") },
            ...
            if (_config.discountStores.DiscountPerm.Count != 0)
            {
                ...
                container.Add(new CuiElement
                {
                    ...
                    Components = {
                        new CuiRawImageComponent { Color = theme.colorImgDiscount, Png = _imageUI.GetImage("5") },
                ...
                private void UpdateBalance(BasePlayer player, ThemeType themetype)
        {
            ...
            CuiElementContainer container = new CuiElementContainer
            {
                ...
                new CuiElement
                {
                    ...
                    Parent = "BALANCE_INFO",
                    Components = {
                    new CuiImageComponent { Color = theme.colorImgBalance, Png = _imageUI.GetImage("6") },
                ...
                private void XDShopUI(BasePlayer player)
        {
            ...
            if (!_uiPlayerUseNow.Contains(player.userID.Get()))
                _uiPlayerUseNow.Add(player.userID.Get());
            else return;
            if (!_playerThemeSelect.ContainsKey(player.userID.Get()))
                _playerThemeSelect.Add(player.userID.Get(), _config.interfaceSettings.ThemeTypeDefault);
            ThemeType themeType = _playerThemeSelect[player.userID.Get()];
                ...
                private void CONFIRMATIONSUI(BasePlayer player, int itemID, int indexCategory, int amount, ThemeType themetype)
        {
            ...
            CuiElementContainer container = new CuiElementContainer
            {
                new CuiElement
                {
                    ...
                    Parent = PanelProduct,
                    Components = {
                    new CuiRawImageComponent { Color = theme.colorMainBG, Png = _imageUI.GetImage("13") },
                ...
                new CuiElement
                {
                    ...
                    Components = {
                    new CuiRawImageComponent { Color = theme.colortext6, Png = _imageUI.GetImage("15") },
                 ...
                 container.Add(new CuiElement
                 {
                     ...
                     Components = {
                        new CuiImageComponent {
                         ItemId = ItemManager.FindItemDefinition(product.ShortName).itemid,
            ...
            container.Add(new CuiElement
            {
                ...
                Parent = ConfirmationsPanel,
                Components = {
                    new CuiRawImageComponent { Color = theme.colortext8, Png = _imageUI.GetImage("14") },
            ...
            container.Add(new CuiElement
            {
                ...
                Parent = ConfirmationsPanel,
                Components = {
                    new CuiRawImageComponent { Color = theme.closeBtnColor2, Png = _imageUI.GetImage("14") },
        ...
        {
            string npcId = _humanNpcPlayerOpen.TryGetValue(player.userID.Get(), out string value) ? value : string.Empty;
        ...
        private void SavePlayerData()
        {
            Interface.Oxide.DataFileSystem.WriteObject($"{Name}/Players", _playerThemeSelect);
                ...
                private List<Items> _itemList = new();
        ...
        private PlayerLimits _playerDataLimits;
                ...
                [PluginReference] private readonly Plugin ImageLibrary, Economics, ServerRewards, IQEconomic, Kits, IQKits, Battles, Duel, ItemCostCalculator, NoEscape, RaidBlock, Duelist, EventHelper, ArenaTournament;
                ...
                private void CONFIRMATIONSSALEUI(BasePlayer player, int itemId, int indexCategory, int amount, ThemeType themetype)
        {
            ...
            container.Add(new CuiElement
            {
                ...
                Components = {
                    new CuiRawImageComponent { Color = themetype == ThemeType.Dark ? "1 1 1 0.2" : "1 1 1 1", Png = _imageUI.GetImage("9")},
        ...
        {
            return _config.economicsCustomization.TypeEconomic switch
            {
                EconomicsType.Economics => Economics,
                EconomicsType.ServerRewards => ServerRewards,
                EconomicsType.IQEconomic => IQEconomic,
                EconomicsType.Item => true,
                _ => false
            };
        }
                ...
                private void UPDATEAMOUNTUI(BasePlayer player, int index, int cat, int amount, ThemeType themetype)
        {
            ...
            int limit = item.GetLimitLot(player, true);
            if (amount <= 0)
                amount = 1;
            if ((limit != 0 && amount > limit) || amount >= 100)
                amount = limit;
            float discount = GetUserDiscount(player);
            ...
            {
                {
                    new CuiElement
                    {
                        Parent = ProductAmountChangeBg + $"_{index}",
                        Name  = $"PRODUCT_LOT_COUNT_{index}",
                        Components =
                        {
                            new CuiInputFieldComponent
                            {
                                ...
                                FontSize = 9,
                                Color = themetype == ThemeType.Dark ? "1 1 1 1" : "0 0 0 1",
                                CharsLimit = 2,
                                Command = $"UI_HandlerShop AMOUNT_CHANGE_KEYBOARD {index} {cat} {themetype} ",
                                IsPassword = false,
                                NeedsKeyboard = true,
                            ...
                            new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-10.064 -8", OffsetMax = "9.372 8" }
                        }
                    }
                },
                {
                ...
                container.Add(new CuiButton
                    {
                        ...
                        Button = { Command = $"UI_HandlerShop PRODUCT_SALE_CONFIRMATION {item.ID} {cat} {amount} {themetype}", Color = theme.colortext9 },
                        Text = { Text = GetLang("XDSHOP_UI_BTN_SALLE", player.UserIDString), FontSize = 12, Align = TextAnchor.MiddleCenter }
                    }, $"PRODUCT_BACKGROUND_{index}", $"PRODUCT_SELL_BTN_{index}");
            }
            if (item.Price != 0)
            {
                container.Add(new CuiButton
                    {
                        ...
                        Button = { Command = $"UI_HandlerShop PRODUCT_BUY_CONFIRMATION {item.ID} {cat} {amount} {themetype}", Color = theme.colortext8 },
                        Text = { Text = GetLang("XDSHOP_UI_BTN_BUY", player.UserIDString), FontSize = 12, Align = TextAnchor.MiddleCenter }
                    }, $"PRODUCT_BACKGROUND_{index}", $"PRODUCT_BUY_BTN_{index}");
            }
            ...
            CuiHelper.DestroyUi(player, $"PRODUCT_SELL_BTN_{index}");
                ...
                private void NotificationUI(BasePlayer player, NotificationType type, string msg)
        {
            ...
            CuiElementContainer container = new CuiElementContainer
            {
                new CuiElement
                {
                    ...
                    Parent = "Overlay",
                    Components = {
                        new CuiRawImageComponent { Color = theme.colorMainBG,  Png = _imageUI.GetImage("16"), FadeIn = 0.30f },
                ...
                new CuiElement
                {
                    ...
                    Components = {
                        new CuiRawImageComponent { Color = color, Png = _imageUI.GetImage("17") , FadeIn = 0.30f},
                ...
                new CuiElement
                {
                    ...
                    Parent = NotificationMain,
                    Components = {
                        new CuiRawImageComponent { Color = color, Png = _imageUI.GetImage("10") , FadeIn = 0.30f},
        ...
        {
            PlayerLimits.GetOrAddPlayer(player.userID.Get()).AddItem(item, lots, purchase);
        }
        ...
        private void Purchasing(BasePlayer player, int itemId, int category, int amount, ThemeType themetype)
        {
            ...
            if (product != null)
            {
                ...
                if (product.Type is ItemType.Blueprint or ItemType.CustomItem or ItemType.Item)
                {
                ...
                {
                    int cooldown = GetCooldownTime(player.userID.Get(), product, true);
                    if (cooldown > 0)
                    {
                ...
                {
                    case EconomicsType.Economics:
                        if ((bool)Economics?.Call("Withdraw", player.userID.Get(), (double)price))
                        {
                        ...
                        break;
                    case EconomicsType.ServerRewards:
                        if (ServerRewards?.Call<object>("TakePoints", player.userID.Get(), (int)price) != null)
                        {
                            GiveProduct(player, product, amount);
                            UpdateBalance(player, themetype);
                            NotificationUI(player, NotificationType.Success, GetLang("XDSHOP_UI_NOTIFICATION_SUCCESSFUL_PURCHASE", player.UserIDString, product.GetProductName(player.UserIDString)));
                        ...
                        break;
                    case EconomicsType.IQEconomic:
                        IQEconomic?.Call("API_REMOVE_BALANCE", player.userID.Get(), (int)price);
                        GiveProduct(player, product, amount);
                        UpdateBalance(player, themetype);
                        NotificationUI(player, NotificationType.Success, GetLang("XDSHOP_UI_NOTIFICATION_SUCCESSFUL_PURCHASE", player.UserIDString, product.GetProductName(player.UserIDString)));
                        break;
                    case EconomicsType.Item:
                        TakeItem(Enumerable.ToArray(Enumerable.Concat(player.inventory.containerMain?.itemList ?? Enumerable.Empty<Item>(), Enumerable.Concat(player.inventory.containerBelt?.itemList ?? Enumerable.Empty<Item>(), player.inventory.containerWear?.itemList ?? Enumerable.Empty<Item>()))), _config.economicsCustomization.EconomicShortname, (int)price, _config.economicsCustomization.EconomicSkinId);
                        GiveProduct(player, product, amount);
                        UpdateBalance(player, themetype);
                        NotificationUI(player, NotificationType.Success, GetLang("XDSHOP_UI_NOTIFICATION_SUCCESSFUL_PURCHASE", player.UserIDString, product.GetProductName(player.UserIDString)));
                        break;
        ...
        private class Configuration
        {
            ...
            internal class InterfaceSettings
            {
                ...
                public ThemeCustomization LightTheme = new ThemeCustomization
                {
                    ...
                    colortext2 = "0.627451 0.6313726 0.6393464 1",
                    colortext3 = "0 0 0 1",
        ...
        private void SetCooldown(BasePlayer player, Configuration.CategoryShop.Product item, bool purchase)
        {
            ...
            if (_playerDataCooldowns.ContainsKey(player.userID.Get()))
                _playerDataCooldowns[player.userID.Get()].SetCooldown(item, purchase);
            else
                _playerDataCooldowns.Add(player.userID.Get(), new PlayerCooldown().SetCooldown(item, purchase));
        }
                ...
                private void UIPRODUCT(BasePlayer player, ThemeType themetype, int page = 0, int categoryIndex = 0)
        {
            ...
            foreach (Configuration.CategoryShop.Product item in products.Where(x => !x.ItemsError).Page(page, 12))
            {
                ...
                container.Add(new CuiElement
                {
                    ...
                    Components = {
                        new CuiRawImageComponent { Color = theme.colortext6,  Png = _imageUI.GetImage("8") },
                ...
                else if (item.Type == ItemType.CustomItem)
                {
                    container.Add(new CuiElement
                    {
                        ...
                        Components = {
                            new CuiImageComponent {
                                ItemId = ItemManager.FindItemDefinition(item.ShortName).itemid,
                                SkinId = item.SkinID},
                            new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-30 10.5", OffsetMax = "30 70.5"  }
                        }
                    });
                ...
                container.Add(new CuiElement
                {
                    Parent = ProductAmountChangeBg + $"_{item.ID}",
                    Name = $"PRODUCT_LOT_COUNT_{item.ID}",
                    Components =
                    {
                        new CuiInputFieldComponent
                        {
                            ...
                            Color = themetype == ThemeType.Dark ? "1 1 1 1" : "0 0 0 1",
                            CharsLimit = 2,
                            Command = $"UI_HandlerShop AMOUNT_CHANGE_KEYBOARD {item.ID} {categoryIndex} {themetype} ",
                            NeedsKeyboard = true,
                            Align = TextAnchor.MiddleCenter,
                            Font = "robotocondensed-regular.ttf"
                        },
                        ...
                        if (products.Count > 12)
            {
                ...
                if (page > 0)
                {
                    container.Add(new CuiElement
                    {
                        ...
                        Parent = PageProducts,
                        Components = {
                    new CuiRawImageComponent { Color = theme.color1, Png = _imageUI.GetImage("1") },
                ...
                if (page + 1 < (int)Math.Ceiling((double)products.Count / 12))
                {
                    container.Add(new CuiElement
                    {
                        ...
                        Parent = PageProducts,
                        Components = {
                    new CuiRawImageComponent { Color = theme.color1, Png = _imageUI.GetImage("2") },
        ...
        private int GetLimit(BasePlayer player, Configuration.CategoryShop.Product item, bool purchase)
        {
            ...
            int used = PlayerLimits.GetOrAddPlayer(player.userID.Get()).GetLimit(item, purchase);
                ...
                private void GiveProduct(BasePlayer player, Configuration.CategoryShop.Product product, int amount)
        {
            switch (product.Type)
            {
                ...
                case ItemType.CustomItem:
                    GetStacks(product.Definition, product.Amount * amount)?.ForEach(stack =>
                    {
                        Item customItem = ItemManager.CreateByName(product.ShortName, stack, product.SkinID);
