...
{
    [Info("MeteorEvent", "ThePitereq", "2.0.2")]
#if CARBON
...
#endif
    {
        ...
        private class FallingMeteorController : FacepunchBehaviour
        {
            ...
            private void SpawnLandedMeteor(float radius)
            {
                ...
                if (config.allMeteors.destroyUnderwater && sphere.IsInWaterVolume(spherePos)) return;
        ...
        private void UpdateDisplay(BasePlayer player, string oldKey)
        {
            ...
            {
                //Admin Spawn Input
                elements.Add(cui.UpdateProtectedInputField("MeteorEventUI_adminSetInput", BetterColors.RedText, "0", 15, 3, false, align: TextAnchor.MiddleCenter, font: CUI.Handler.FontTypes.RobotoCondensedBold, command: $"UI_MeteorEvent forceStart {currEvent}"));
            }
            bool onCooldown = cooldowns.ContainsKey(player.userID) && cooldowns[player.userID].ContainsKey(currEvent);
            string bottomText;
            if (!hasRequiredItem)
