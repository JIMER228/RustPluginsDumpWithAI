...
{
    [Info("ExplosiveWeapons", "Cameron", "1.0.6")]
    ...
    public class ExplosiveWeapons : CovalencePlugin
    {
                ...
                foreach (Collider col in colliders)
                {
                    ...
                    if (ent.IsValid())
                    {
                        if (ent is BasePlayer && ent != player )
                        {
                            ...
                            TracerRocket(player.transform.position, player.GetNetworkRotation(), ent as BaseCombatEntity);
                            if(heatSeekerBlowUp)
                                invItem.Remove();
                            return;
                        }
        ...
        private void TracerRocket(Vector3 firePoint,Quaternion rot, BaseCombatEntity target)
        {
