...
{
    [Info("BetterSentryTurrets", "Razor", "1.7.0")]
    ...
    public class BetterSentryTurrets : RustPlugin
    {
        ...
        #else
                private class CustomScanQueue : AutoTurret.UpdateAutoTurretScanQueue
                {
        ...
        public class TurretMod : MonoBehaviour
        {
            ...
            public void FireAttachedGun(Vector3 targetPos, float aimCone, Transform muzzleToUse = null, BaseCombatEntity target = null)
            {
                ...
                attachedWeapon.ServerUse(1f, turret.IsBeingControlled ? turret.RCEyes : pitch);
        ...
        private class SamMod : MonoBehaviour
        {
            ...
            public void TargetScan()
            {
                ...
                {
                    List<SamSite.ISamSiteTarget> list = Facepunch.Pool.Get<List<SamSite.ISamSiteTarget>>();
                    Vis.Entities<SamSite.ISamSiteTarget>(turret.eyePos.transform.position, scanRadius, list, layerMask, QueryTriggerInteraction.Ignore);
            ...
            private void Update()
            {
                if (triggerPlayers.Count > 0)
                {
                    ...
                    foreach (var triggerPlayer in triggerPlayers)
                    {
                        ...
                        if (turret != null && turret.IsOnline())
                        {
        ...
        private void BuildCuiIdent(BasePlayer player, NPCAutoTurret turret, string identID)
        {
            ...
            container.Add(createButton($"<B>{lang.GetMessage("set", this, player.UserIDString)}</B>", "1 1 1 1", TextAnchor.MiddleCenter, "0.145 0.255 0.09 0.7", $"sentrycommands set", $"0.502 0.49", $"0.58 0.53", 16), "Sentry.MainPanelIdent", "Sentry.MainPanel.button55");
