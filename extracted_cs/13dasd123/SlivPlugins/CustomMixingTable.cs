...
{
    [Info("CustomMixingTable", "David", "1.4.9")]
    public class CustomMixingTable : RustPlugin
    {
                        ...
                        if (rcp.ContainsKey(recipe))
                        {
                            ...
                            if (TryMix(tableContent, rcp[recipe].Ingredients))
                            {
                                foreach (var player in run.playerLooting)
                                    CreateMixingButton(player, table.net.ID, 1, recipe);
                            }
                            else
                            {
                                foreach (var player in run.playerLooting)
                                    CreateMixingButton(player, table.net.ID, 0, recipe);
                            }
                        }
                    }
                }
        ...
        object CanLootEntity(BasePlayer player, MixingTable table)
        {
            ...
            if (run != null)
            {
                ...
                if (run.recipe != null && run.IsMixing())
                {
                    ...
                    CreateMixingButton(player, table.net.ID, 2, run.recipe);
        ...
        private void dm_set(ConsoleSystem.Arg arg)
        {
            ...
            var table = BaseNetworkable.serverEntities.Find(new NetworkableId(Convert.ToUInt64(args[0]))) as MixingTable;
        ...
        private void CreatePageBtns(BasePlayer player, int currentPage, string tableId)
        {
            ...
            CUIClass.CreateButton(ref DrugMixing_pageBtns, "dm_page_down", "DrugMixing_base_recipies_title", "0.80 0.25 0.16 0.0", "▼", 11, "0.89 0.16", $"0.94 0.84", $"DrugMixing_cmd pagedown {pagedown} {tableId}", "", "1 1 1 0.4", 0.1f, TextAnchor.MiddleCenter, $"robotocondensed-bold.ttf", "assets/icons/iconmaterial.mat");
        ...
        private void CreateMixingButton(BasePlayer player, NetworkableId tableID, int type, string recipe = "syringe.medical{0}") // 0 = not enough, 1 = ready, 2 = stop  /0.70 0.67 0.65 0.07 /0.38 0.51 0.16 0.2
        {
        ...
        private class TableBehavior : FacepunchBehaviour
        {
            ...
            public void ShowMixingProgress()
            {
                if (IsInvoking(nameof(RunSQ)) == true)
                {
                    foreach (var player in playerLooting)
                    {
                        ...
                        plugin.CreateMixingButton(player, table.net.ID, 2);
                    }
                }
            }
            ...
            public void StopMixing()
            {
                ...
                {
                    CuiHelper.DestroyUi(player, "prog_base");
                    plugin.CreateMixingButton(player, table.net.ID, 1, recipe);
                }
            }
