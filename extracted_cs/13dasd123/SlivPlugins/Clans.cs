...
namespace Oxide.Plugins
{
    ...
    class Clans : RustPlugin
    {
                ...
                switch (arg.Args[0].ToLower())
                {
                    ...
                    case "kick":
                        if (authLevel >= configData.Commands.Auth.Kick)
                        {
                            ...
                            Interface.Oxide.CallHook("OnClanMemberGone", player.userID, tag);
                            Interface.Oxide.CallHook("OnClanMemberGone", player.userID, clan.ClanMembers.Select(x => x.Key).ToList());
        ...
        private void cmdFakeClan(BasePlayer player, string command, string[] args)
        {
            ...
            myClan.MemberInvites[76561199087683923] = new Clan.MemberInvite()
            {
                DisplayName = "Dummy Player 3",
                ExpiryTime = UnixTimeStampUTC()
            };
            myClan.MemberInvites[76561199083683923] = new Clan.MemberInvite()
            {
                DisplayName = "Dummy Player 4",
                ExpiryTime = UnixTimeStampUTC()
            };
            myClan.MemberInvites[76561299083683923] = new Clan.MemberInvite()
            {
                DisplayName = "Dummy Player 5",
                ExpiryTime = UnixTimeStampUTC()
            };
