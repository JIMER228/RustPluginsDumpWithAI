	...
	public class Crafts : RustPlugin
	{
		...
		private class Configuration
		{
			...
			[JsonProperty(PropertyName = "Custom Workbench Stations (Entity ID - settings)",
				ObjectCreationHandling = ObjectCreationHandling.Replace)]
			public readonly Dictionary<ulong, CustomWorkbenchConf> CustomWorkbench =
		...
		private class EconomyConf
		{
            ...
            public double ShowBalance(BasePlayer player)
            {
                switch (Type)
                {
                    ...
                    case EconomyType.Item:
                        {
                            // Преобразуем List<Item> в Item[]
                            return ItemCount(player.inventory.containerMain.itemList.ToArray(), ShortName, Skin);
                        }
                    default:
                        return 0;
                }
            ...
            public bool RemoveBalance(BasePlayer player, double amount)
            {
                switch (Type)
                {
                    ...
                    case EconomyType.Item:
                        {
                            // Заменяем player.inventory.AllItems() на player.inventory.containerMain.itemList.ToArray()
                            var playerItems = player.inventory.containerMain.itemList.ToArray();
                            var am = (int)amount;

                            // Используем ItemCount с преобразованным массивом
		...
		private object CanResearchItem(BasePlayer player, Item item)
		{
			if (player == null || item == null ||
			    !_crafts.Exists(craft => craft.Enabled && craft.Type == CraftType.Vehicle && craft.ShortName == item.info.shortname &&
				...
				case "craft":
				{
					...
					Item[] allItems = null;
                        ...
                        if (!bypass)
                        {
                            ...
                            // Замените allItems на получение предметов из контейнера
                            var AllItems = player.inventory.containerMain.itemList.ToArray();
