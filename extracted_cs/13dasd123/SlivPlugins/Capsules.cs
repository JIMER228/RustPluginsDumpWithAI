...
{
    [Info("Capsules", "LAGZYA", "1.0.5")]
    [Description("Система капсул.")] 
    class Capsules : RustPlugin
    {
        ...
        private void StartUi(BasePlayer player)
        {
            ...
            cont.Add(new CuiElement()
            {
                ...
                Components =
                {
                    new CuiImageComponent{Color = "0.13988100 0.51312 00.15720 0", Material = "assets/content/ui/uibackgroundblur.mat"},
        ...
        object OnDispenserGather(ResourceDispenser dispenser, BaseEntity entity, Item item)
        {
            ...
            timer.Once(0.001f, () =>
            {
                ...
                if (p < ss) return;
                SendReply(player, string.Format(lang.GetMessage("AddExp", this, player.UserIDString), Settings.ExpDesp));
        ...
        private object OnItemAction(Item item, string action, BasePlayer player)
        {
            ...
            if (itemList.Command == "")
            {
                var giveItem = ItemManager.CreateByName(itemList.shortname, itemList.Ammount);
                if (giveItem == null)
                {
                    SendReply(player, "Сообщите администратору об ошибке![Capsules]");
                    Puts($"Что то пошло не так у игрока {player}");
                    return false;
                }
            ...
            else
            {
                ...
                SendReply(player, $"[CAPSULES] Из капсулы вы получили: {itemList.shortname}");
            }
            return false;
        }
