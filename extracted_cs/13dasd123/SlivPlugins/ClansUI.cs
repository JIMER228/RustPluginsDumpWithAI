...
namespace Oxide.Plugins
{
    ...
    class ClansUI : ChaosPlugin
    {
        ...
        private void OpenClanMenu(BasePlayer player, Clans.Clan clan, Menu menu = Menu.Members)
        {
            BaseContainer root = ImageContainer.Create(UI_MENU, Layer.Overlay, Anchor.Center, new Offset(-200f, -245f, 600f, 245f))
	            ...
	            .WithChildren(parent =>
	            {
                    ...
                    CreateHeaderBar(player, parent, clan, menu);

                    if (menu == Menu.Members)
                        CreateMemberView(player, parent, clan);
                    else CreateAllianceView(player, parent, clan);
        ...
        private void CreateHeaderBar(BasePlayer player, BaseContainer parent, Clans.Clan clan, Menu menu)
        {
                                        ...
                                        .WithChildren(allyToggle =>
                                        {
                                            ...
                                            ButtonContainer.Create(allyToggle, Anchor.FullStretch, Offset.zero)
                                                ...
                                                .WithCallback(m_CallbackHandler, arg =>
                                                {
                                                    ...
                                                    if (ownerOnlyAFF)
                                                    {
                                                        foreach (var kvp in clan.ClanMembers)
                                                        {
                        ...
                        ImageContainer.Create(header, Anchor.CenterRight, new Offset(-85f, -10f, -5f, 10f))
                            ...
                            .WithChildren(viewalliances =>
                            {
                                ...
                                ButtonContainer.Create(viewalliances, Anchor.FullStretch, Offset.zero)
                                    .WithColor(Color.Clear)
                                    .WithCallback(m_CallbackHandler, arg => OpenClanMenu(player, clan, Menu.Alliances), $"{player.UserIDString}.viewalliances");
                            });
                    }
                }
                else
                {
                    ImageContainer.Create(header, Anchor.CenterRight, new Offset(-85f, -10f, -5f, 10f))
                        ...
                        .WithChildren(viewalliances =>
                        {
                            ...
                            ButtonContainer.Create(viewalliances, Anchor.FullStretch, Offset.zero)
                                .WithColor(Color.Clear)
                                .WithCallback(m_CallbackHandler, arg => OpenClanMenu(player, clan, Menu.Members), $"{player.UserIDString}.viewmembers");
                        });
                }
            });
        }
        ...
        #region Member View
        private void CreateMemberView(BasePlayer player, BaseContainer parent, Clans.Clan clan)
        {
                    ...
                    ImageContainer.Create(members, Anchor.CenterLeft, new Offset(0f, -187.5f, 635f, 187.5f))
                        ...
                        .WithChildren(memberView =>
                        {
                            ...
                            foreach (var kvp in clan.ClanMembers)
                            {
                                Anchor anchor;
                                Offset offset;
                                m_MemberLayout.Evaluate(i, out anchor, out offset);
                                
                                CreateMemberField(player, clan, ulong.Parse(kvp.Key.ToString()), kvp.Value, memberLayout, anchor, offset);
                                i += 1;
                                
                                if (i > m_MemberLayout.PerPage)
                                    break;
                            }
                        });
                        ...
                        .WithChildren(inviteView =>
                        {
                            ...
                            BaseContainer invitePlayerButton = ImageContainer.Create(inviteView, Anchor.TopCenter, new Offset(-69f, -44f, 69f, -26f))
                                ...
                                .WithChildren(invitePlayer =>
                                {
                                    ...
                                    List<BasePlayer> allPlayerList = Facepunch.Pool.GetList<BasePlayer>();
                                            ...
                                            if (canManageInvites)
                                            {
                                                InviteData<BasePlayer> data = new InviteData<BasePlayer>
                                                {
                                                    ...
                                                    OnSelectFunction = (basePlayer) =>
                                                    {
                                                        ...
                                                        Facepunch.Pool.FreeList(ref allPlayerList);
                                                    }
                                                };
                            ...
                            int i = 0;
                            foreach (var kvp in clan.MemberInvites)
                            {
                                Anchor anchor;
                                Offset offset;
                                m_InviteLayout.Evaluate(i, out anchor, out offset);
                                CreateMemberInviteField(player, clan, canManageInvites, kvp.Key.ToString(), kvp.Value, inviteLayout, anchor, offset);
                                i += 1;

                                if (i > m_InviteLayout.PerPage)
                                    break;
                            }
                        });
                });
        ...
        private void CreateMemberField(BasePlayer player, Clans.Clan clan, ulong memberId, Clans.Clan.Member member, BaseContainer parent, Anchor anchor, Offset offset)
        {
            ImageContainer.Create(parent, anchor, offset)
                ...
                {
                    Color roleColor;
                    if (!m_RoleColors.TryGetValue(member.Role, out roleColor))
        ...
        private void CreateMemberInviteField(BasePlayer player, Clans.Clan clan, bool canManageInvites, string memberId, Clans.Clan.MemberInvite member, BaseContainer parent, Anchor anchor, Offset offset)
        {
            ImageContainer.Create(parent, anchor, offset)
                .WithStyle(m_PanelStyle)
                .WithChildren(template =>
        ...
        private void CreateAllianceView(BasePlayer player, BaseContainer parent, Clans.Clan clan)
        {
            BaseContainer.Create(parent, Anchor.FullStretch, new Offset(5f, 40f, -5f, -75f))
				...
				ImageContainer.Create(alliances, Anchor.CenterLeft, new Offset(0f, -187.5f, 260f, 187.5f))
					...
					.WithChildren(current =>
					{
						...
						BaseContainer.Create(current, Anchor.FullStretch, new Offset(0f, 0f, 0f, -20f))
							.WithLayoutGroup(m_CurrentAllianceLayout, clan.Alliances, 0, (int i, string t, BaseContainer layout, Anchor anchor, Offset offset) =>
								{
                                                ...
                                                .WithChildren(revoke =>
												{
                                                    ...
                                                    ButtonContainer.Create(revoke, Anchor.FullStretch, Offset.zero)
                                                        ...
                                                        .WithCallback(m_CallbackHandler, arg =>
                                                        {
                                                            ...
                                                            OpenClanMenu(player, clan, Menu.Alliances);
					...
					.WithChildren(invites =>
					{
						...
						BaseContainer requestAllianceButton = ImageContainer.Create(invites, Anchor.TopCenter, new Offset(-125f, -44f, 125f, -26f))
                            ...
                            .WithChildren(invite =>
							{
                                ...
                                List<string> allClanTags = Facepunch.Pool.GetList<string>();
                                        ...
                                        if (canManageAlliance)
                                        {
                                            InviteData<string> data = new InviteData<string>
                                            {
                                                ...
                                                OnSelectFunction = (s) =>
                                                {
                                                    ...
                                                    OpenClanMenu(player, clan, Menu.Alliances);
                                                    Facepunch.Pool.FreeList(ref allClanTags);
                                                }
                                            };
						...
						BaseContainer.Create(invites, Anchor.FullStretch, new Offset(0f, 0f, 0f, -45f))
							.WithLayoutGroup(m_AllianceInvitesLayout, clan.AllianceInvites.Keys, 0, (int i, string t, BaseContainer layout, Anchor anchor, Offset offset) =>
								{
									ImageContainer.Create(layout, anchor, offset)
										.WithStyle(m_PanelStyle)
										.WithChildren(template =>
				...
				ImageContainer.Create(alliances, Anchor.CenterRight, new Offset(-260f, -187.5f, 0f, 187.5f))
					...
					.WithChildren(requests =>
					{
						...
						BaseContainer.Create(requests, Anchor.FullStretch, new Offset(0f, 0f, 0f, -20f))
							.WithLayoutGroup(m_AllianceRequestsLayout, clan.IncomingAlliances, 0, (int i, string t, BaseContainer layout, Anchor anchor, Offset offset) =>
								{
									ImageContainer.Create(layout, anchor, offset)
										.WithStyle(m_PanelStyle)
										.WithChildren(template =>
        ...
        private void OpenInviteMenu<T>(BasePlayer player, InviteData<T> data)
        {
            BaseContainer root = ImageContainer.Create(UI_MENU, Layer.Overall, Anchor.Center, new Offset(-200f, -245f, 600f, 245f))
                .WithStyle(m_BackgroundStyle)
                .WithChildren(parent =>
                {
                    CreateTitleBar(player, parent, () => OpenClanMenu(player, data.Clan, data.Menu));

                    List<T> dst = Facepunch.Pool.GetList<T>();
                    ...
                    CreateInviteHeader(player, parent, data, dst.Count);
        ...
        private void CreateInviteHeader<T>(BasePlayer player, BaseContainer parent, InviteData<T> data, int listCount)
        {
            ImageContainer.Create(parent, Anchor.TopStretch, new Offset(5f, -70f, -5f, -40f))
			.WithStyle(m_PanelStyle)
			.WithChildren(header =>
