...
{
    [Info("Skyfall", "AuthorColon Blow", "1.0.22")]
    class Skyfall : RustPlugin
    {

        // fix for Rust update Oct 1 2020
        ...
        void cmdConsoleGiveChute(ConsoleSystem.Arg arg)
        {
            ...
            {
                GiveChutePack(BasePlayer.FindByID(Convert.ToUInt64(arg.Args[0])));
            }
        }
        ...
        void cmdConsoleDoRespawn(ConsoleSystem.Arg arg)
        {
            ...
            {
                if (UseCooldown && CooldownListConstainsPlayerID(BasePlayer.FindByID(player.userID))) return;
                if (UseCooldown) { CooldownAddPlayerID(BasePlayer.FindByID(player.userID)); }
                if (UseRandomRespawn) { RespawnAtRandom(BasePlayer.FindByID(player.userID), true); return; }
                else
                    RespawnAtPlane(BasePlayer.FindByID(player.userID), true);
            }
        }
        ...
        private void ActivateChaosDrop()
        {
        ...
        private bool CooldownListConstainsPlayerID(BasePlayer player)
        {
        ...
        private Vector3 FindPlayerSKyfallPoint()
        {
            Vector3 spawnpoint = new Vector3();
        ...
        private Vector3 GetLocalSkyfallPoint(BasePlayer player)
        {
        ...
        private SkyfallPlane FindJumpPlane()
        {
        ...
        private void ActivateJumpPlane(BasePlayer player)
        {
        ...
        private void ActivateSkyfall(BasePlayer player, SkyfallPlane plane)
        {
        ...
        private void GiveChutePack(BasePlayer player)
        {
            if (player == null) return;
        ...
        private void AddPlayerButton(BasePlayer player)
        {
            if (player == null) return;
