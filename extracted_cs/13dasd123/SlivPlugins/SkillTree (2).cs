...
{
    [Info("Skill Tree", "imthenewguy", "1.4.13")]
    ...
    class SkillTree : RustPlugin
    {
        ...
        public void AwardXP(BasePlayer player, double value, BaseEntity source = null, bool noMod = false, bool hookAlreadyCalled = false, string source_string = null)
        {
            ...
            if (IsSkillTreeXPEventLoaded && SkillTreeXPEventRunning && source != null)
            {
        ...
        void GiveSkillPointsToAllConsole(ConsoleSystem.Arg arg)
        {
            ...
            bool onlineOnly;
            int amount;
            if (arg.Args == null || arg.Args.Length < 2 || int.TryParse(arg.Args[0], out amount) || !Boolean.TryParse(arg.Args[1], out onlineOnly))
            {
                arg.ReplyWith("Usage: givesptoall <amount> <online only: true/false>");
                return;
            }

            List<ulong> onlinePlayers = Pool.GetList<ulong>();
            foreach (var p in BasePlayer.activePlayerList)
            {
                GiveSkillPoints(p, amount);
                onlinePlayers.Add(p.userID);
            }

            if (onlineOnly)
            {
                Pool.FreeList(ref onlinePlayers);
            ...
            foreach (var kvp in pcdData.pEntity)
            {
                ...
                kvp.Value.available_points += amount;
            }

            Pool.FreeList(ref onlinePlayers);

            arg.ReplyWith($"Added {amount} skill points to all players.");
        }
        ...
        void GiveXPToAllConsole(ConsoleSystem.Arg arg)
        {
            ...
            bool onlineOnly;
            double amount;
            if (arg.Args == null || arg.Args.Length < 2 || double.TryParse(arg.Args[0], out amount) || !Boolean.TryParse(arg.Args[1], out onlineOnly))
            {
            ...
            arg.ReplyWith($"Added {amount} xp to all players.");
        }
...
#else
            _harmony = new Harmony(Name + "Patch");
            System.Reflection.MethodInfo method = AccessTools.Method(typeof(FishingRod_Cast_Patch), "Postfix");
