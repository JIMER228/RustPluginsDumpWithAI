...
{
    [Info("ComponentBox", "k1lly0u", "2.0.3")]
    ...
    class ComponentBox : RustPlugin
    {
        ...
        private object CanMoveItem(Item movedItem, PlayerInventory inventory, ItemContainerId targetContainerID, int targetSlot, int amount)
        {
            ...
            if (Configuration.DepositAutomatically)
            {
                ...
                {
                    if ((inventory.loot.entitySource is LootContainer || inventory.loot.entitySource is LootableCorpse ||
                        (inventory.loot.itemSource?.GetOwnerPlayer() != inventory.baseEntity && inventory.loot.itemSource?.skin == PRESENT_SKIN_ID)))
                    {
                        if (targetContainerID.Equals(0) && targetSlot == -1)
                        {
