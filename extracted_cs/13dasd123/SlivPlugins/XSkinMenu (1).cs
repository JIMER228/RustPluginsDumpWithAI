    ...
    class XSkinMenu : RustPlugin
    {
        ...
        private class SkinConfig
        {
			...
			internal class GeneralSetting
			{
				...
				[JsonProperty(LanguageEnglish ? "Reset Admin skins from items if they are used by a player without permission" : "Сбрасывать Админ скины с предметов если их использует игрок без разрешения")] public bool DeleteAdminSkins = true;
				[JsonProperty(LanguageEnglish ? "Reset Vip skins from items if they are used by a player without permission" : "Сбрасывать Вип скины с предметов если их использует игрок без разрешения")] public bool DeleteVipSkins = true;
		...
		private void SaveDataSkins() => Interface.Oxide.DataFileSystem.WriteObject("XDataSystem/XSkinMenu/Skins", StoredDataSkins);
		private void SaveDataSkinsName() => Interface.Oxide.DataFileSystem.WriteObject("XDataSystem/XSkinMenu/SkinsName", StoredDataSkinsName);
		...
		private void ccmdCategoryS(ConsoleSystem.Arg args)
		{
			...
			switch(args.Args[0])
			{
				...
				case "searchskin":
				{
					if(StoredData[player.userID].Comfort)
					{
						if(args.Args.Length >= 5)
							SkinGUI(player, args.Args[1], 0, args.Args[2], int.Parse(args.Args[3]), args.Args[4].ToLower());
					}
					else
					{
						if(args.Args.Length >= 4)
							SkinGUI(player, args.Args[1], 0, args.Args[2], 0, args.Args[3].ToLower());
					}
				...
				case "setskinitem":
				{
					...
					{
						string shortname = item.info.shortname, sitem = args.Args[1];
						ulong skin = ulong.Parse(args.Args[2]);
	    ...
	    private void OnServerInitialized()
		{
			...
			if(!config.API.GameIMG && _coroutine == null && ImageLibrary && _key != "")
			...
			if(!config.API.GameIMG && _key == "")
			{
				timer.Once(2, () =>
				{
					PrintError(LanguageEnglish ? "The code is missing the key for the API. If you don't have it, contact the developer.\nIf you have a key, paste it into the code, code line - 332 - string _key = '';" : "В коде отсутствует ключ для API. Если у вас его нет, обратитесь к разработчику.\nЕсли у вас есть ключ, вставьте его в код, строка кода - 332 - string _key = '';");
		...
		public Dictionary<ulong, string> errorskins = new Dictionary<ulong, string>
		{
			...
			[10222] = "hazmatsuit.diver",
			[13070] = "rifle.ak.ice",
			[13076] = "rifle.ak.diver",
			...
			[13058] = "skylantern.skylantern.green",
			[13059] = "skylantern.skylantern.purple",
			[13067] = "skylantern.skylantern.orange",
			[13069] = "skylantern.skylantern.red",
			//[] = "sofa.pattern" 
		};
		...
		private void OnItemCraftFinished(ItemCraftTask task, Item item, ItemCrafter crafter)
		{
			if(task.skinID == 0)
			{
				...
				if(!StoredData[player.userID].Skins.ContainsKey(item.info.shortname) || !permission.UserHasPermission(player.UserIDString, permCraft)) return;
		...
		private void SSI(BasePlayer player, Item item, ulong skin) 
		{
		...
		private void SetSkinCraftGive(BasePlayer player, Item item, bool isgivepickup = false, ItemContainer container = null)
		{
			...
			if(isgivepickup && skin == 0 && StoredData[player.userID].ChangeSGN) return;
		...
		private void OnItemAddedToContainer(ItemContainer container, Item item)
		{
			...
			if(container?.playerOwner != null)
			{
				...
				if(_adminSkins.Contains(item.skin) && config.Setting.DeleteAdminSkins && !permission.UserHasPermission(player.UserIDString, permAdminS))
				{
					ResetItemSkin(item);
					return;
				}
				
				if(_vipSkins.Contains(item.skin) && config.Setting.DeleteVipSkins && !permission.UserHasPermission(player.UserIDString, permVipS))
				{
					ResetItemSkin(item);
					return;
				}
				...
				if(StoredData[player.userID].ChangeSG)
					SetSkinCraftGive(player, item, true, container);
			}
		}
		...
		private void SkinGUI(BasePlayer player, string item, int Page = 0, string category = "null", int PageC = 0, string search = "")
		{
			...
			foreach(var skin in list_skins.Skip(Page * count))
			{
				...
				if(permission.UserHasPermission(player.UserIDString, permAdmin) && !_adminUiFD.Contains(player.userID))
				{
					if(_adminAndVipSkins.Contains(skin))
						container.Add(new CuiButton
						{
							...
							Button = { Color = "1 1 1 0.75", Sprite = "assets/icons/refresh.png", Command = $"xskin refresh_ui {item} {skin} {Page} {(isAdminSkin ? "admin_to_default" : isVipSkin ? "vip_to_default" : "")} {category}" },
							Text = { Text = "" }
						}, ".Skin");
					else
					{
                    ...
                    {
                        RectTransform = { AnchorMin = "1 0", AnchorMax = "1 0", OffsetMin = "-20 5", OffsetMax = "-5 20" },
                        Button = { Color = "1 1 1 0.75", Sprite = "assets/icons/clear.png", Command = $"xskin remove_ui {item} {skin} {Page} {(isAdminSkin ? "admin" : isVipSkin ? "vip" : "default")} {category}" },
                        Text = { Text = "" }
                    }, ".Skin");
				}
				...
				container.Add(new CuiButton
                {
                    ...
                    Button = { Color = "1 1 1 0.75", Sprite = config.GUI.IconZoom, Command = $"skin_c zoomskin {itemid} {skin} false" },
                    Text = { Text = "" }
                }, ".Skin");
		...
		private void SetItemGUI(BasePlayer player, string item, int Page = 0, bool isentity = false)
		{
			CuiElementContainer container = new CuiElementContainer();
			...
			container.Add(new CuiElement
			{
				...
				Components =
				{
					...
					new CuiRectTransformComponent { AnchorMin = "0.5 0", AnchorMax = "0.5 0", OffsetMin = isentity ? "-389.75 0" : "-399.5 90", OffsetMax = isentity ? "390.25 226.75" : "380.5 316.75" },
			...
			foreach(var skin in list_skins.Skip(Page * 18))
			{
			    container.Add(new CuiPanel
                {
				...
				container.Add(new CuiButton
                {
                    ...
                    Button = { Color = "1 1 1 0.75", Sprite = config.GUI.IconZoom, Command = $"skin_c zoomskin {itemid} {skin} true" },
                    Text = { Text = "" }
                }, ".SkinB");
			...
			container.Add(new CuiPanel
            {
                RectTransform = { AnchorMin = "0 0", AnchorMax = "1 0", OffsetMin = "0 -90", OffsetMax = "0 0" },
                Image = { Color = "0 0 0 0" }
            }, ".SetItem");
		...
		private void SettingGUI(BasePlayer player)
		{
			...
			{		
			    ...
			    container.Add(new CuiLabel
                {
                    ...
                    Text = { Text = lang.GetMessage(s.Key, this, player.UserIDString), Align = TextAnchor.MiddleCenter, Font = "robotocondensed-regular.ttf", FontSize = 11, Color = "0.75 0.75 0.75 1" }
                }, ".SM");
		...
		private void DefaultKitsGUI(BasePlayer player, int Page = 0)
		{
			...
			if(permission.UserHasPermission(player.UserIDString, permKitsD))
			{
				...
				container.Add(new CuiLabel
				{
					...
					Text = { Text = lang.GetMessage("DEFAULT_KITS_NOPERM", this, player.UserIDString), Align = TextAnchor.MiddleCenter, FontSize = 20, Color = "0.8 0.8 0.8 0.75" }
				}, ".DefaultKitsGUI");
			...
			if(permission.UserHasPermission(player.UserIDString, permKitsC))
			{
				if(Page == 0)
				{
					...
					container.Add(new CuiLabel
					{
						...
						Text = { Text = string.Format(lang.GetMessage("CUSTOM_KITS_AMOUNT", this, player.UserIDString), StoredData[player.userID].Kits.Count, GetMaxCountKits(player)), Align = TextAnchor.MiddleCenter, FontSize = 12, Font = "robotocondensed-regular.ttf", Color = "0.8 0.8 0.8 0.75" }
					}, ".CKitAdd");
				}
				...
				{
					RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-220 -57.5", OffsetMax = "220 12.5" },
					Text = { Text = lang.GetMessage("CUSTOM_KITS_NOPERM", this, player.UserIDString), Align = TextAnchor.MiddleCenter, FontSize = 20, Color = "0.8 0.8 0.8 0.75" }
				}, ".CustomKitsGUI");
		...
		private void ZoomGUI(BasePlayer player, int itemid, ulong skin, bool issetitem)
		{
			...
			if(StoredDataSkinsName.ContainsKey(skin))
				container.Add(new CuiLabel
				{
					...
					Text = { Text = StoredDataSkinsName[skin], Align = TextAnchor.MiddleCenter, FontSize = 14, Font = "robotocondensed-regular.ttf", Color = "0.8 0.8 0.8 1" }
				}, ".ZoomIMG");
		...
		private void SendInfo(BasePlayer player, string message)
        {
            ...
            {
				FadeOut = 0.5f,
                RectTransform = { AnchorMin = "0 0", AnchorMax = "0 0", OffsetMin = "0 0", OffsetMax = "427.75 36.75" },
        ...
        private void InitializeLang()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
				...
				["spray"] = "USE SKINS WITH A SPRAY CAN",
            ...
            lang.RegisterMessages(new Dictionary<string, string>
            {
				...
				["spray"] = "ИСПОЛЬЗОВАТЬ СКИНЫ С ПОМОЩЬЮ БАЛЛОНЧИКА",
			...
			lang.RegisterMessages(new Dictionary<string, string>
            {
				...
				["spray"] = "ВИКОРИСТОВУВАТИ СКІНИ ЗА ДОПОМОГОЮ БАЛОНЧИКА",
            ...
            lang.RegisterMessages(new Dictionary<string, string>
            {
				...
				["spray"] = "UTILIZAR PIELES CON LATA DE AEROSOL",
