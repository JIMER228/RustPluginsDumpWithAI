...
{
    [Info("HomeRecycler", "k1lly0u", "2.0.6")]
    ...
    class HomeRecycler : RustPlugin
    {
        ...
        private readonly Dictionary<ulong, ulong> startedRecyclers = new Dictionary<ulong, ulong>();
        ...
        private void OnEntityBuilt(Planner plan, GameObject obj)
        {
            ...
            if (entity && entity.ShortPrefabName == configData.recyclerShortPrefabName && entity.skinID == RECYCLER_SKIN_ID)
            {
                ...
                recycler.OwnerID = configData.adminSpawnsPublicRecycler && player.net.connection.authLevel == 2 ? 0UL : player.userID;
                recycler.Spawn();
        ...
        private void OnRecyclerToggle(Recycler recycler, BasePlayer player)
        {
            ...
            startedRecyclers[recycler.net.ID.Value] = player.userID;
            NextTick(() =>
            {
                if (!recycler.IsOn()) 
                    return;
        ...
        private object OnItemRecycle(Item item, Recycler recycler)
        {
            ...
            if (startedRecyclers.ContainsKey(recycler.net.ID.Value))
            {
                foreach (KeyValuePair<string, ConfigData.Rates> perm in configData.PermissionsRates)
                {
                    if (permission.UserHasPermission(startedRecyclers[recycler.net.ID.Value].ToString(), string.Format(PERMISSION_GENERIC, perm.Key)))
                    {
        ...
        private void cmdGiveRecycler(ConsoleSystem.Arg arg)
        {
            ...
            {
                Server.Command($"ownerid {arg.Player().userID}");
            }
            else
            {
