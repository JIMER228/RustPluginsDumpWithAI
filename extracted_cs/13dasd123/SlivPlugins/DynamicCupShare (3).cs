...
{
    [Info("DynamicCupShare", "k1lly0u", "3.1.7")]
    ...
    class DynamicCupShare : ChaosPlugin
    {
        ...
        {
            if (!samSite || !Configuration.Turrets.IncludeSameSites || samSite.OwnerID == 0UL)
                return null;

            if (samSiteTarget is HotAirBalloon)
            {
                HotAirBalloon hotAirBalloon = (samSiteTarget as HotAirBalloon);
            ...
            if (samSiteTarget is BaseVehicle)
            {
                BaseVehicle baseVehicle = samSiteTarget as BaseVehicle;
