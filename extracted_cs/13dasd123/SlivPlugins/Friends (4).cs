    ...
    public class Friends : RustPlugin
    {
        ...
        private object OnSamSiteTarget(SamSite entity, BaseCombatEntity target)
        {
            ...
            if (targetpcopter != null || targetpcopterBig != null)
            {
                ...
                if (targetpcopter != null && targetpcopter.mountPoints.Count > 0 && targetpcopter.mountPoints[0].mountable != null)
                {
                    targetPlayer = targetpcopter.mountPoints[0].mountable.GetMounted();
                }
                else if (targetpcopterBig != null && targetpcopterBig.mountPoints.Count > 0 && targetpcopterBig.mountPoints[0].mountable != null)
                {
                    targetPlayer = targetpcopterBig.mountPoints[0].mountable.GetMounted();
                }
                if (targetPlayer == null) return false;
                if (entity.OwnerID == targetPlayer.userID) return false;
                if (!friendData.TryGetValue(entity.OwnerID, out targetPlayerData)) return null;
                FriendData.FriendAcces ss;
                if (!targetPlayerData.friendList.TryGetValue(targetPlayer.userID, out ss)) return null;
        ...
        private string API_KEY = "Friends-233255123123sadfvqfgr";
        ...
        private Dictionary<TakePlayers, bool> _iqFake = new Dictionary<TakePlayers, bool>();
        private void OnServerInitialized()
        {
            PrintWarning("\n-----------------------------------------------------------------------------------------\n" +
            "     Загрузка плагина...\n" +
            "     URL: https://slivplugin.ru\n" +
            "     DISCORD: https://discord.gg/pFgKw6Dyyq\n" +
            "     Этот плагин исправлен inkub1372 под заказ [Rust Plugin Sliv]\n" +
            "     Приятного использования!\n" +
            "-----------------------------------------------------------------------------------------");
            if (cfg.fake && IQFakeActive != null)
            {
                int online = (int)IQFakeActive.CallHook("GetOnline");
                for (int i = 0; i < online; i++)
                {
                    var steamId = (ulong) IQFakeActive.CallHook("GetFakeIDRandom");
                    var t = new TakePlayers()
                    {
                        SteamId = steamId,
                        DisplayName = (string) IQFakeActive.CallHook("GetFakeNameRandom"),
                    };
                    ImageLibrary.CallHook("GetPlayerAvatar", steamId.ToString());
                    _iqFake.Add(t, false);
                }
            ...
            AddImage( $"https://i.imgur.com/lTURAvf.png",$"button_friend");
            AddImage( $"https://media.discordapp.net/attachments/1061898344269627392/1085155060427792404/ss.png?width=670&height=671",$"setting_friend");
            AddImage( $"https://media.discordapp.net/attachments/1061898344269627392/1085154324897861702/add.png",$"addfriend_friend");
            AddImage( $"https://media.discordapp.net/attachments/1061898344269627392/1085166897500278804/lft.png?width=671&height=671",$"pageright_friend");
            AddImage( $"https://media.discordapp.net/attachments/1061898344269627392/1085166865988464781/r5.png?width=671&height=671",$"pageleft_friend");
            AddImage( $"https://i.imgur.com/N2kuCg0.png",$"settingback_friend");
            AddImage( $"https://i.imgur.com/NlPacX1.png",$"remove_friend");
            AddImage( $"https://media.discordapp.net/attachments/1061898344269627392/1085157396780945449/1315532.png",$"off_friend");
            AddImage( $"https://i.imgur.com/K0Kc8aZ.png",$"on_friend");
        ...
        private void SettingInit(BasePlayer player, ulong steamdIdTarget, string b)
        {
            ...
            cont.Add(new CuiElement()
            {
                ...
                Components =
                {
                    new CuiTextComponent()
                    {
                        Text = $"{lang.GetMessage("UISETTINGS", this, player.UserIDString)}", Align = TextAnchor.MiddleCenter, Font = regular, FontSize = 12, Color = HexToRustFormat("#52eb80")
                    },
                    new CuiRectTransformComponent()
            ...
            cont.Add(new CuiButton()
            {
                ...
                Text =
                {
                    Text = lang.GetMessage("UIREMOVEFRIENDv2", this, player.UserIDString), Align = TextAnchor.MiddleCenter, FontSize = 10,
                }
            }, Layer + "Set");
            cont.Add(new CuiElement()
            {
        ...
        private void AuthBuild(BasePlayer player, ulong friendId)
        {
            ...
            foreach (BaseNetworkable entity in BaseNetworkable.serverEntities.Where(p => p is BuildingPrivlidge && (p as BuildingPrivlidge).OwnerID == player.userID))
            {
                var bp = entity.GetComponent<BuildingPrivlidge>();
                if(bp == null) continue;
                if(bp.OwnerID != player.userID) return;
                if (bp.authorizedPlayers.Any(p => p.userid == player.userID))
        ...
        private void RemoveBuild(BasePlayer player, ulong friendId)
        {
            foreach (BaseNetworkable entity in BaseNetworkable.serverEntities.Where(p => p is BuildingPrivlidge && (p as BuildingPrivlidge).OwnerID == player.userID))
            {
                ...
                if (bp == null) continue;
                if (bp.OwnerID != player.userID) return;
                if (bp.authorizedPlayers.Contains(new PlayerNameID { userid = player.userID }))
                {
                    if (!bp.authorizedPlayers.Contains(new PlayerNameID { userid = friendId })) continue;
        ...
        { 
            ...
            foreach (var keyValuePair in _iqFake)
            {
                playerList.Add(keyValuePair.Key, false);
            }
            foreach (var sellItem in playerList.Where(p => find == "" || find != "" && ( p.Key.DisplayName.ToLower().Contains(find.ToLower()) || p.Key.SteamId.ToString().Contains(find))).Select((i, t) => new {A = i, B = t -(page - 1) * 30}).Skip((page - 1) * 30).Take(30))
            {
                cont.Add(new CuiElement()
                {
                    Parent = Layer + "-Search",
        ...
        private bool AddFriend(ulong playerId, ulong friendId)
        {
            ...
            playerData.friendList[friendId] = new FriendData.FriendAcces()
            {
            ...
            playerData2.friendList[playerId] = new FriendData.FriendAcces()
            {
