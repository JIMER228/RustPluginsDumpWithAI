...
{
    [Info("DReportSystem", "Drop Dead", "2.0.2")]
    public class DReportSystem : RustPlugin
    {
        ...
        public Dictionary<ulong, Cooldown> cooldown = new Dictionary<ulong, Cooldown>();

        public class Cooldown
        {
            public DateTime vk;
            public DateTime discord;
            public DateTime rustplus;
        }
        ...
        public class PluginConfig
        {
            ...
            public class AlertSettings
            {
                ...
                [JsonProperty("[Rust+] Кулдаун на отправку повторного репорта (в сек.)")]
                public int rustpluscooldown = 900;
                ...
                [JsonProperty("[VK] Кулдаун на отправку повторного репорта (в сек.)")]
                public int vkcooldown = 900;
                ...
                [JsonProperty("[Discord] Кулдаун на отправку повторного репорта (в сек.)")]
                public int discordcooldown = 900;
            }
        }
        ...
        void OnServerInitialized()
        {
            ...
            if (cfg.plugininterface.ip == "127.0.0.1" || String.IsNullOrEmpty(cfg.plugininterface.ip))
            {
                PrintWarning("[RU] Вы не указали IP сервера в конфигурации. ");
                PrintWarning("[ENG] You didn't specify the server IP in the configuration. ");
            }
        ...
        bool HasCooldown(ulong player, string type)
        {
            if (!cooldown.ContainsKey(player)) return false;
            var cooldowns = cooldown[player];
            if (type == "vk")
            {
                if (cooldowns.vk == null) return false;
                var cooldownz = cooldowns.vk + TimeSpan.FromSeconds(cfg.alerts.vkcooldown);
                if ((cooldownz - DateTime.UtcNow).TotalSeconds > 0) return true;
                else return false;
            }
            if (type == "discord")
            {
                if (cooldowns.discord == null) return false;
                var cooldownz = cooldowns.discord + TimeSpan.FromSeconds(cfg.alerts.discordcooldown);
                if ((cooldownz - DateTime.UtcNow).TotalSeconds > 0) return true;
                else return false;
            }
            if (type == "rustplus")
            {
                if (cooldowns.rustplus == null) return false;
                var cooldownz = cooldowns.rustplus + TimeSpan.FromSeconds(cfg.alerts.rustpluscooldown);
                if ((cooldownz - DateTime.UtcNow).TotalSeconds > 0) return true;
                else return false;
            }
        ...
        void GiveCooldown(ulong playerid, string type)
        {
            if (type == "vk")
            {
                if (cooldown.ContainsKey(playerid))
                {
                    cooldown[playerid].vk = DateTime.UtcNow;
                }
                else cooldown.Add(playerid, new Cooldown { vk = DateTime.UtcNow });
            }
            if (type == "discord")
            {
                if (cooldown.ContainsKey(playerid))
                {
                    cooldown[playerid].discord = DateTime.UtcNow;
                }
                else cooldown.Add(playerid, new Cooldown { discord = DateTime.UtcNow });
            }
            if (type == "rustplus")
            {
                if (cooldown.ContainsKey(playerid))
                {
                    cooldown[playerid].rustplus = DateTime.UtcNow;
                }
                else cooldown.Add(playerid, new Cooldown { rustplus = DateTime.UtcNow });
            }
        }
        ...
        {
			if (debug) Puts("OnPlayerReported work!");
			
            if (string.IsNullOrEmpty(targetId) || player == null) return;

            SendReport(player, ulong.Parse(targetId), subject, message);
        }
        ...
        public void API_SendReport(BasePlayer player, BasePlayer suspect, string subject, string message)
        {
            ...
            SendReport(player, suspect.userID, subject, message);
        }
        ...
        private void SendVKChatMessage(ulong userid, string message) 
        {
            var apiver = "v=5.92";
        ...
        void DMessage(string sendername, string reportedname, ulong senderid, ulong reportedid, string title, string message, string reportscount)
        {
        ...
        void SendReport(BasePlayer player, ulong suspect, string subject, string message)
        {
            if (debug) PrintError($"API works! {player.displayName} reported {Players[suspect].name} for {subject}, {message}");
            if (player == null || string.IsNullOrEmpty(suspect.ToString()) || player.userID == suspect) return;

            if (!Players.ContainsKey(suspect))
                Players.Add(suspect, new player { name = suspect.ToString(), reportscount = 0 });
            if (!Players.ContainsKey(player.userID))
                Players.Add(player.userID, new player { name = player.displayName, reportscount = 0});

            var suspectz = Players[suspect];
            suspectz.reportscount++;
            suspectz.Reports.Add(new report { name = player.displayName, steamid = player.userID, subject = subject, message = message });

            if (!cfg.plugininterface.english) Log($"{ConVar.Server.hostname}:\nconnect {cfg.plugininterface.ip}:{ConVar.Server.port}\n\nИгрок {player.displayName}/{player.userID} отправил жалобу на игрока: {suspectz.name}/{suspect}. Заголовок жалобы: {subject.Replace("[cheat] ", string.Empty).Replace("[spam] ", string.Empty).Replace("[name] ", string.Empty).Replace("[abusive] ", string.Empty)}, текст: {message}");
            else Log($"{ConVar.Server.hostname}:\nconnect {cfg.plugininterface.ip}:{ConVar.Server.port}\n\nPlayer {player.displayName}/{player.userID} filed a complaint against player: {suspectz.name}/{suspect}. Complaint title: {subject.Replace("[cheat] ", string.Empty).Replace("[spam] ", string.Empty).Replace("[name] ", string.Empty).Replace("[abusive] ", string.Empty)}, text: {message}");
            foreach (var players in BasePlayer.activePlayerList.Where(x => permission.UserHasPermission(x.UserIDString, cfg.access.perm)))
            {
                if (cfg.alerts.rustplusalert) 
                {
                    if (!HasCooldown(player.userID, "rustplus"))
                        RustPlusAlert(players.userID, player.displayName, suspectz.name, player.userID, suspect, subject.Replace("[cheat] ", string.Empty).Replace("[spam] ", string.Empty).Replace("[name] ", string.Empty).Replace("[abusive] ", string.Empty), message);
                }
                if (cfg.alerts.chatalert) players.ChatMessage(GetMsg("New Report", players.UserIDString).Replace("{hostname and connect}", "").Replace("{0}", $"{player.displayName}").Replace("{1}", $"{player.userID}").Replace("{2}", $"{suspectz.name}").Replace("{3}", $"{suspect}").Replace("{4}", $"{subject.Replace("[cheat] ", string.Empty).Replace("[spam] ", string.Empty).Replace("[name] ", string.Empty).Replace("[abusive] ", string.Empty)}").Replace("{5}", $"{message}"));
            }
            ...
            && cfg.alerts.chatid != "Если не собираетесь использовать оповещения в беседу VK, не изменяйте это значение") 
            {
                if (!HasCooldown(player.userID, "vk"))
                    SendVKChatMessage(player.userID, GetMsg("New Report").Replace("{hostname and connect}", text).Replace("{0}", $"{player.displayName}").Replace("{1}", $"{player.userID}").Replace("{2}", $"{suspectz.name}").Replace("{3}", $"{suspect}").Replace("{4}", $"{subject.Replace("[cheat] ", string.Empty).Replace("[spam] ", string.Empty).Replace("[name] ", string.Empty).Replace("[abusive] ", string.Empty)}").Replace("{5}", $"{message}"));
            }
            if (cfg.alerts.discordalert)
            {
                if (!HasCooldown(player.userID, "discord"))
                    DMessage(player.displayName, suspectz.name, player.userID, suspect, subject.Replace("[cheat] ", string.Empty).Replace("[spam] ", string.Empty).Replace("[name] ", string.Empty).Replace("[abusive] ", string.Empty), message, suspectz.reportscount.ToString());
            }
        }
        ...
        void DeletePlayerFromDict(ulong id)
        {
        ...
        void CancelCheck(BasePlayer suspect, BasePlayer moderator)
        {
            ...
            DeletePlayerFromDict(suspect.userID);
        ...
        private void DRSTEST(ConsoleSystem.Arg args)
        {
            ...
            DMessage("Test bot", "Test bot 2", 76561198155015818, 76561198155015819, "Test title", "Test message", "10");
        ...
        private void DeletePlayer(ConsoleSystem.Arg args)
        {
            ...
            DeletePlayerFromDict(id);
            string name = "";
            if (BasePlayer.FindByID(id) != null) name = BasePlayer.FindByID(id).displayName;
            else name = Players[id].name;
        ...
        private void DrawUI(BasePlayer player, int page)
        {
            ...
            for (int i = 0; i < dict.Count(); i++)
            {
                ...
                else
                {
                    ...
                    {
                        RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1" },
                        Button = { Close = Layer, Command = $"drsclear {suspectid}", Color = "0 0 0 0" },
                        Text = { Text = "" }
        ...
        private void DrawPlayerInfo(BasePlayer player, ulong steamid, int pages)
        {
            ...
            List<report> list = Players[steamid].Reports.Skip(8 * pages).Take(8).ToList();
