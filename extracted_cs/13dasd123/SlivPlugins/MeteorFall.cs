...
{
    [Info("MeteorFall", "EcoSmile", "2.0.9")]
    ...
    class MeteorFall : RustPlugin
    {
        ...
        protected override void LoadDefaultConfig()
        {
            config = new PluginConfig()
            {
                ...
                {
                    useUi = true,
                    IconImage = "https://gspics.org/images/2024/01/13/0luXdy.png",
        ...
        List<ulong> MeteorList = new List<ulong>();
        List<ulong> MeteorFound = new List<ulong>();
        List<NPCPlayer> NpcList = new List<NPCPlayer>();
        List<ulong> FireBall = new List<ulong>();
        ...
        private void StartRainOfFire(Vector3 origin)
        {
            ...
            for (int i = 0; i < (meteorcount > 2 ? meteorcount : 2); i++)
            {
                ...
                MeteorList.Add(met.net.ID.Value);
                MeteorFound.Add(met.net.ID.Value);
            }
        ...
        private void CreatePrivateMap(Vector3 pos)
        {
            ...
            MarkerT.markerShopName = "Метеорит";
        ...
        private void ColdMeteorite()
        {
            ...
            {
                var entityId = new NetworkableId(check);
            ...
            mystimer5 = timer.Every(1f, () =>
            {
                if (MeteorFound.Count > 0)
                {
                    ...
                    else if (1 < desptime && desptime <= config.Settings.DespawnTime * 60)
                    {
                        foreach (var player in BasePlayer.activePlayerList)
                        {
        ...
        Dictionary<ScientistNPC, Vector3> npcPos = new Dictionary<ScientistNPC, Vector3>();
        private void CreateNpc(Vector3 position, int amount = 0)
        {
            for (int i = 0; i < amount; i++)
            {
                var pos = RandomCircle(position, 10);
                ScientistNPC scientist = GameManager.server.CreateEntity("assets/rust.ai/agents/npcplayer/humannpc/scientist/scientistnpc_full_any.prefab", pos) as ScientistNPC;
                scientist.Spawn();
        ...
        void ToSpawnPoint()
        {
            npcTimer = timer.Every(5f, () =>
            {
                foreach (var npc in npcPos.Keys.ToList())
                    if (npc != null && npcPos.ContainsKey(npc) && npc?.GetBestTarget() == null)
        ...
        private void CreateFireBall(Vector3 position, Vector3 additional)
        {
            ...
            fireball_.Spawn();
            FireBall.Add(fireball_.net.ID.Value);
        }

        private void Despawn()
        {
            List<NetworkableId> MeteorList = new List<NetworkableId>();
            ...
            List<NetworkableId> FireBall = new List<NetworkableId>();
        ...
        [ChatCommand("meteor")]
        void ChatCmdControll(BasePlayer player, string command, string[] args, ulong playerid = 0)
        {
        ...
        {
            if (player == null) return;
            ulong ID = Convert.ToUInt64(dispenser.GetComponent<BaseEntity>().net.ID);
        ...
        void SetFirstStage(Vector3 pos)
        {
            ...
            ore.Spawn();
            ore.health = 1f;
            ore.GetComponent<StagedResourceEntity>().UpdateNetworkStage();
        ...
        public class MeteorSeed : FacepunchBehaviour
        {
            ...
            void Awake()
            {
                ...
                healChange = ore.MaxHealth() / ins.config.seedSettings.TimeToRelise;
                InvokeRepeating(OreProgress, 1,1);
            }

            void OreProgress()
            {
                if(ore.MaxHealth() >= ore.Health())
                { 
                    ore.health += healChange;
                    ore.GetComponent<StagedResourceEntity>().UpdateNetworkStage();
                }
                if (ore.stage == 0)
                {
        ...
        void OnEntityKill(BaseNetworkable entity)
        {
            ...
            if (MeteorFound.Contains(entity.net.ID.Value))
            {
                ...
                MeteorFound.Remove(entity.net.ID.Value);
            }
        }
        ...
        void LoadMessages()
        {
            ...
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                ["SeedCrash"] = "Вы помешали росту семени теперь оно не будет так плодоносно.",
        ...
        public static string FormatTime(TimeSpan time)
        {
            ...
            if (time.Days != 0) result += $"{Format(time.Days, "d.", "d.", "d.")} ";
            if (time.Hours != 0) result += $"{Format(time.Hours, "h.", "h.", "h.")} ";
            if (time.Minutes != 0) result += $"{Format(time.Minutes, "m.", "m.", "m.")} ";
            if (time.Seconds != 0) result += $"{Format(time.Seconds, "s.", "s.", "s.")} ";
            return result;
        }
