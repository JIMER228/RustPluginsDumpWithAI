...
{
    [Info("Limit Entities", "MON@H", "2.1.4")]
    ...
    public class LimitEntities : RustPlugin
    {
        ...
        public bool IsMergeBlocked(Construction component, Construction.Target placement, BasePlayer player, PlayerData playerData, BuildingEntities buildingEntities)
        {
            ...
            List<BuildingBlock> adjoiningBlocks = Pool.GetList<BuildingBlock>();
            ...
            if (adjoiningBlocks.Count > 0)
            {
                ...
                foreach (BuildingBlock adjoiningBlock in adjoiningBlocks)
                {
                    ...
                    BuildingEntities adjoiningBuilding = _cache.Buildings[adjoiningBlock.buildingID];
                    ...
                    foreach (KeyValuePair<uint, int> adjoiningEntity in adjoiningBuilding.EntitiesCount)
                    {
                        ...
                        {
                            Log($"{oBB.position} {player.UserIDString} prevented from merge building {buildingEntities.BuildingID} Limit Building Total", LogLevel.Debug);
                            HandleNotification(player, Lang(LangKeys.Error.LimitBuilding.MergeBlocked, player.UserIDString, allowedBuildingTotal * -1), true);
                            mergeBlocked = true;
                            break;
                        }
                        ...
                        if (count < 0)
                        {
                            Log($"{oBB.position} {player.UserIDString} prevented from building merge block entity {GetShortName(adjoiningEntity.Key)} in building {buildingEntities.BuildingID}", LogLevel.Debug);
                            HandleNotification(player, Lang(LangKeys.Error.LimitBuilding.EntityMergeBlocked, player.UserIDString, count * -1, GetItemDisplayName(adjoiningEntity.Key, player.UserIDString)), true);
                            mergeBlocked = true;
                            break;
                        }
                    }
            ...
            Pool.FreeList(ref adjoiningBlocks);
        ...
        public object HandleCanBuild(BasePlayer player, Construction component, Construction.Target placement)
        {
            ...
            if (playerData.IsGlobalLimit())
            {
                ...
                HandleNotification(player, Lang(LangKeys.Error.LimitGlobal.Reached, player.UserIDString, playerData.Entities.TotalCount, playerData.Perms.LimitsGlobal.LimitTotal), true);
                return _static.False;
            }
            ...
            if (playerData.IsGlobalLimit(prefabID))
            {
                ...
                HandleNotification(player, Lang(LangKeys.Error.LimitGlobal.EntityReached, player.UserIDString, entities.PlayersEntities[prefabID], playerData.Perms.LimitsGlobal.LimitEntitiesCache[prefabID], GetItemDisplayName(prefabID, player.UserIDString)), true);
                return _static.False;
            }
            ...
            if (building != null)
            {
                if (playerData.IsBuildingLimit(building))
                {
                    ...
                    HandleNotification(player, Lang(LangKeys.Error.LimitBuilding.Reached, player.UserIDString, building.EntitiesIds.Count, playerData.Perms.LimitsBuilding.LimitTotal), true);
                    return _static.False;
                }
                ...
                {
                    Log($"{position} {playerId} prevented from building entity {GetShortName(prefabID)} in building {buildingID}", LogLevel.Debug);
                    HandleNotification(player, Lang(LangKeys.Error.LimitBuilding.EntityReached, player.UserIDString, building.EntitiesCount[prefabID], playerData.Perms.LimitsBuilding.LimitEntitiesCache[prefabID], GetItemDisplayName(prefabID, player.UserIDString)), true);
                    return _static.False;
                }
        ...
        public void HandleEntityNotification(BasePlayer player, PlayerData playerData, BuildingEntities building, uint prefabID)
        {
            ...
            {
                HandleNotification(player, Lang(LangKeys.Info.LimitBuildingEntity, player.UserIDString, building.EntitiesCount[prefabID], playerData.Perms.LimitsBuilding.LimitEntitiesCache[prefabID], GetItemDisplayName(prefabID, player.UserIDString)));
            }
            else if (building != null && playerData.GetBuildingPercentage(building) >= _pluginConfig.WarnPercent)
            {
                HandleNotification(player, Lang(LangKeys.Info.LimitBuilding, player.UserIDString, building.EntitiesIds.Count, playerData.Perms.LimitsBuilding.LimitTotal));
            }
            else if (playerData.GetGlobalPercentage(prefabID) >= _pluginConfig.WarnPercent)
            {
                HandleNotification(player, Lang(LangKeys.Info.LimitGlobalEntity, player.UserIDString, playerData.Entities.PlayersEntities[prefabID], playerData.Perms.LimitsGlobal.LimitEntitiesCache[prefabID], GetItemDisplayName(prefabID, player.UserIDString)));
            }
            else if (playerData.GetGlobalPercentage() >= _pluginConfig.WarnPercent)
            {
                HandleNotification(player, Lang(LangKeys.Info.LimitGlobal, player.UserIDString, playerData.Entities.TotalCount, playerData.Perms.LimitsGlobal.LimitTotal));
            }
        }
        ...
        public string GetPlayerLimitString(BasePlayer player, BasePlayer target)
        {
            ...
            PlayerData playerData = _cache.PlayerData[target.userID];
            if (playerData?.Perms == null)
            {
                return Lang(LangKeys.Info.Unlimited, player.UserIDString);
            }
        ...
        private void API_RemoveBuildingOwner(List<BaseEntity> entitiesList)
        {
            ...
            foreach (BaseEntity entity in entitiesList)
            {
                ...
                GetPlayerData(entity.OwnerID).RemoveEntity(GetPrefabID(entity.prefabID));
        ...
        private HashSet<ulong> API_ChangeBuildingOwner(List<BaseEntity> entitiesList, ulong newOwner)
        {
            ...
            foreach (BaseEntity entity in entitiesList)
            {
                if (!entity.IsValid())
                {
                    PrintError($"API_ChangeBuildingOwner: {newOwner} prevented from ChangeBuildingOwner invalid entity {entity}");
                    return null;
                }
                ...
                int count = ++newEntities[buildingID][prefabID];
                ...
                if (limitEntities.ContainsKey(prefabID) && count > limitEntities[prefabID])
                {
                    ...
                    newEntities[buildingID][prefabID]--;
                    limitedEntitiesIds.Add(networkableId);
                }
