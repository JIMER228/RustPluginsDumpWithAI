...
{
[Info("PersonalAnimal", "walkinrey", "1.2.95")]
    public class PersonalAnimal : RustPlugin 
    {
        ...
        public class Configuration 
        {
            ...
            public class ControlsSetup 
            {
                ...
                [JsonIgnore] public BUTTON controlButton = BUTTON.FIRE_THIRD;
                [JsonIgnore] public BUTTON inventoryButton = BUTTON.RELOAD;
                [JsonIgnore] public BUTTON mountButton = BUTTON.USE;
            }
            ...
            public class AnimalInfo 
            {
                ...
                public class FunctionsSetup 
                {
                    ...
                    [JsonProperty("Can an animal collect resources within a radius of 50 meters? (/panimal auto-collect)")]
        ...
        protected override void LoadDefaultConfig()
        {
            ...
            {
                AnchorMin = "1 1", AnchorMax = "1 1",
                OffsetMin = "-335 -104", OffsetMax = "-175 -10"
            };
        ...
        private void OnPlayerInput(BasePlayer player, InputState input)
        {
            ...
            if(input.WasJustPressed(_config.controlsSetup.controlButton)) controller.OnInput();
            ...
            if(controller.IsDriving)
            {
                ...
                if(_config.controlsSetup.byDirection)
                {
                    ...
                    {
                        if(controller.bot == null) return;
                        if(controller.bot.transform == null) return;

                        Vector3 direction = (player.eyes.HeadRay().direction).normalized * 10;
                        Vector3 destination = controller.bot.transform.position + new Vector3(direction.x, 0, direction.z);

                        controller.DriveDestination = destination;
                ...
                else 
                {
                    ...
                    if(isClickedFwd)
                    {
                        ...
                        Vector3 direction = (controller.bot.transform.forward).normalized * 10;
                        Vector3 destination = controller.bot.transform.position + new Vector3(direction.x, 0, direction.z);
                        ...
                        {
                            controller.DriveDestination += (-controller.transform.right) * 2f;
                        }
            ...
            if(input.WasJustPressed(_config.controlsSetup.inventoryButton)) 
            {
                controller.OpenBag();
                return;
            }

            if(input.WasJustPressed(_config.controlsSetup.mountButton)) 
            {
                controller.Drive();
                return;
            }
        ...
        private void OnEntityBuilt(Planner plan, GameObject go)
        {
            ...
            foreach(var loopInfo in _config.installItem)
            {
                if(loopInfo.skin == item.skin)
                {
                    ...
                    {
                        comp.bot.Hurt(comp.bot.MaxHealth() + 1);
                    ...
                    NextTick(() =>
                    {
                        ...
                        {
                            case "E":
                                _config.controlsSetup.controlButton = BUTTON.USE;
                                break;

                            case "MIDDLE_MOUSE":
                                _config.controlsSetup.controlButton = BUTTON.FIRE_THIRD;
                                break;

                            case "RELOAD":
                                _config.controlsSetup.controlButton = BUTTON.RELOAD;
                                break;
                            
                            case "SPRINT":
                                _config.controlsSetup.controlButton = BUTTON.SPRINT;
                                break;

                            case "SECOND_MOUSE":
                                _config.controlsSetup.controlButton = BUTTON.FIRE_SECONDARY;
                                break;
                        }
                        ...
                        {
                            case "E":
                                _config.controlsSetup.inventoryButton = BUTTON.USE;
                                break;

                            case "MIDDLE_MOUSE":
                                _config.controlsSetup.inventoryButton = BUTTON.FIRE_THIRD;
                                break;

                            case "RELOAD":
                                _config.controlsSetup.inventoryButton = BUTTON.RELOAD;
                                break;
                            
                            case "SPRINT":
                                _config.controlsSetup.inventoryButton = BUTTON.SPRINT;
                                break;

                            case "SECOND_MOUSE":
                                _config.controlsSetup.inventoryButton = BUTTON.FIRE_SECONDARY;
                                break;
                        }
                        ...
                        {
                            case "E":
                                _config.controlsSetup.mountButton = BUTTON.USE;
                                break;

                            case "MIDDLE_MOUSE":
                                _config.controlsSetup.mountButton = BUTTON.FIRE_THIRD;
                                break;

                            case "RELOAD":
                                _config.controlsSetup.mountButton = BUTTON.RELOAD;
                                break;
                            
                            case "SPRINT":
                                _config.controlsSetup.mountButton = BUTTON.SPRINT;
                                break;

                            case "SECOND_MOUSE":
                                _config.controlsSetup.mountButton = BUTTON.FIRE_SECONDARY;
                                break;
                        }
        ...
        private void chatCommand(BasePlayer player, string command, string[] args) 
        {
            if(args == null || args?.Length == 0) 
            {
                ...
                if(player.TryGetComponent<PlayerAnimalController>(out playerAnimalController)) 
                {
                    ...
                    {
                        player.EndLooting();
                        playerAnimalController.bot.Hurt(playerAnimalController.bot.MaxHealth() + 1);
                    }
            ...
            if(bots.Count != 0)
            {
                ...
                if(bot != null)
                {
                    ...
                    if(permission.UserHasPermission(player.UserIDString, perm))
                    {
                        ...
                        if(player.TryGetComponent<PlayerAnimalController>(out comp)) 
                        {
                            if(comp.bot) comp.bot.Hurt(comp.bot.MaxHealth() + 1);
                            SendMsg(player, "ChatCommand_Success_Despawn");
        ...
        private class AnimalAutoMode : MonoBehaviour
        {
            ...
            {
                var colliders = Physics.OverlapSphere(StartPos, 50f);
        ...
        private class AnimalFeedingTrigger : FacepunchBehaviour
        {
            ...
            {
                Invoke(() => CheckItem(other.ToBaseEntity()), 0.1f);
            }
        ...
        private class PlayerAnimalController : FacepunchBehaviour
        {
            ...
            public struct ItemData
            {
                ...
                public Item ToItem()
                {
                    if(Amount == 0)
                    {
                        // Debug.LogWarning("[PersonalAnimal] Couldn't create item with shortname {Shortname} and skin {Skin}, because it have a zero amount!");
                        return null;
                    }
            ...
            private void RefreshMenu()
            {
                if(npcPlugin)
                {
                    ...
                    if(hasBot != _firstTimeRenderOnSecond)
                    {
                        RenderMenu(true);
            ...
            private bool StartAttack()
            {
                if (_lastTarget != null)
                {
                    if (Vector3.Distance(bot.transform.position, _lastTarget.transform.position) < animalSetup.interactionSetup.enemyDistance && _lastTimeAttack < Time.realtimeSinceStartup)
                    {  
            ...
            public void OpenBag()
            {
                ...
                {
                    if(!bagItem.ToItem().MoveToContainer(storage.inventory)) storage.inventory.Insert(bagItem.ToItem());
                }
