...
{
    [Info("Stack Modifier", "Khan", "1.7.2")]
    ...
    public class StackModifier : RustPlugin
    {
        ...
        private CuiElementContainer CreateEditorItemEntry(_Items dataItem, float ymax, float ymin, string catName, string text)
        {
            ...
            container.Add(new CuiButton
                {
                    Text =
                    {
                        ...
                        Color = dataItem.Disable ? _config.Colors.Disable.Rgb : _config.Colors.Enable.Rgb,
            ...
            int multiplier = _defaults[dataItem.ShortName] * _config.StackCategoryMultipliers[_itemMap[dataItem.ShortName]];

            var newmsg = dataItem.Disable ? 1 :
                _config.StackCategoryMultipliers[_itemMap[dataItem.ShortName]] > 0 && dataItem.Modified == _defaults[dataItem.ShortName] ? multiplier
                    : dataItem.Modified != _defaults[dataItem.ShortName] ? dataItem.Modified : _defaults[dataItem.ShortName];
                ...
                foreach (ItemDefinition id in ItemManager.itemList)
                {
                    ...
                    if (stackItem.DisplayName == item)
                    {
                        if (_config.StackCategories[categoryName][id.shortname].Disable)
                            id.stackable = 1;
                        else if (_config.StackCategoryMultipliers[categoryName] > 0 && _config.StackCategories[categoryName][id.shortname].Modified == _defaults[id.shortname])
                            id.stackable *= _config.StackCategoryMultipliers[categoryName];
                        else if (_config.StackCategories[categoryName][id.shortname].Modified != _defaults[id.shortname])
                            id.stackable = _config.StackCategories[categoryName][id.shortname].Modified;
                        break;
                    }
                }
        ...
        private void ConsoleEditSearch(ConsoleSystem.Arg arg)
        {
            ...
            if (commandis == 0)
            {
                ...
                {
                    foreach (ItemDefinition id in ItemManager.itemList)
                        if (_defaults.ContainsKey(id.shortname))
            ...
            if (commandis == 1)
            {
                ...
                foreach (ItemDefinition id in ItemManager.itemList)
                {
                    if (!(all && _config.StackCategories[catid].ContainsKey(id.shortname) || _config.StackCategories[catid].ContainsKey(id.shortname))) continue;
                    if (_config.StackCategories[catid][id.shortname].Disable)
                        id.stackable = 1;
                    else if (_config.StackCategoryMultipliers[catid] > 0 && _config.StackCategories[catid][id.shortname].Modified == _defaults[id.shortname])
                        id.stackable *= _config.StackCategoryMultipliers[catid];
                    else if (_config.StackCategories[catid][id.shortname].Modified != _defaults[id.shortname])
