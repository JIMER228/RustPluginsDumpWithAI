    ...
    public class RandomRaids : RustPlugin
    {
     ...
     //   private const string nightmarePerm = "randomraids.nightmare";
        ...
        private void Init()
        {
            ...
            {
                configData.raidSettings.raidTypes.Add("easy", new raidConfig(1, 10, true, 100, 640, new List<string>(), new List<string>() { "explosive.timed", "explosive.satchel" }, 1, 1.0, false, new List<string>() { "easy" }));
                configData.raidSettings.raidTypes.Add("medium", new raidConfig(2, 15, true, 200, 900, new List<string>(), new List<string>() { "explosive.timed", "explosive.satchel" }, 1, 1.0, true, new List<string>() { "easy" }));
                configData.raidSettings.raidTypes.Add("hard", new raidConfig(3, 20, true, 400, 1200, new List<string>(), new List<string>() { "explosive.timed", "explosive.satchel" }, 1, 1.0, true, new List<string>() { "easy" }));
                configData.raidSettings.raidTypes.Add("expert", new raidConfig(4, 30, true, 400, 1500, new List<string>(), new List<string>() { "explosive.timed", "explosive.satchel" }, 1, 1.0, true, new List<string>() { "medium" }));
                configData.raidSettings.raidTypes.Add("nightmare", new raidConfig(5, 30, true, 400, 2100, new List<string>(), new List<string>() { "explosive.timed", "explosive.satchel" }, 1, 1.0, true, new List<string>() { "medium" }));
                SaveConfig();
            }
            ...
            {
                configData.settingsRB.raidSettings.Add(0, new RBConfig(3, 10, 320, new List<string>() { "easy" }));
                configData.settingsRB.raidSettings.Add(1, new RBConfig(3, 10, 320, new List<string>() { "easy", "medium" }));
                configData.settingsRB.raidSettings.Add(2, new RBConfig(3, 10, 320, new List<string>() { "easy", "medium" }));
                configData.settingsRB.raidSettings.Add(3, new RBConfig(3, 10, 320, new List<string>() { "easy", "medium", "hard" }));
                configData.settingsRB.raidSettings.Add(4, new RBConfig(3, 10, 320, new List<string>() { "hard", "expert", "nightmare" }));
                SaveConfig();
            }
            ...
            {
                configData.settingsNPCKILLS.npcSettings.Add("scientistnpc_heavy", new NPCConfig(30, 10, 320, new List<string>() { "medium" }));
                configData.settingsNPCKILLS.npcSettings.Add("scientistnpc_oilrig", new NPCConfig(60, 10, 320, new List<string>() { "easy" }));
                configData.settingsNPCKILLS.npcSettings.Add("scientistnpc_patrol", new NPCConfig(60, 10, 320, new List<string>() { "easy" }));
                configData.settingsNPCKILLS.npcSettings.Add("scientistnpc_junkpile", new NPCConfig(60, 10, 320, new List<string>() { "easy" }));
                SaveConfig();
            }
      ...
      "https://dl.dropboxusercontent.com/s/zh2jp80zoy9oinp/145014836-male-vocal-video-game-taunt-5.mp3" }*/
        ...
        public class raidConfig
        {
            ...
            public raidConfig(int TotalAuth, int TotalNpcs, bool OnlyAuthedMembers, float Health, int RunTimeSeconds, List<string> kits, List<string> explosive, int waves, double damage, bool attackCopter, List<string> jconfig)
            {
        ...
        public class RBConfig
        {
            ...
            public RBConfig(int totalRaids1, int raidChance1, float raidableBaseCompleteDelay1, List<string> sendRaids1)
            {
        ...
        public class NPCConfig
        {
            ...
            public NPCConfig(int totalRaids1, int raidChance1, float raidDelay1, List<string> sendRaids1)
            {
                this.totalRaids = totalRaids1;
        ...
        public class EventRandomManager
        {
            ...
            internal static Vector3 GetAimToTarget(Vector3 startPosition, Vector3 targetPos, out float baseGravity)
            {
                ...
                //vector = new Vector3(3.2009f, 2.639f, 0.2845f);
                float num = 90f;
        ...
        public class RandomRaider : NPCPlayer, IAISenses, IAIAttack, IThinker
        {
            ...
            {
                if (baseCombatEntity is BasePlayer)
                    return IsPlayerVisibleToUs(baseCombatEntity as BasePlayer, 1218519041);
            ...
            public bool CanSeeTarget(BaseEntity entity)
            {
                ...
                if ((UnityEngine.Object)otherPlayer == (UnityEngine.Object)null)
                    return true;
                return this.AdditionalLosBlockingLayer == 0 ? this.IsPlayerVisibleToUs(otherPlayer, 1218519041) : this.IsPlayerVisibleToUs(otherPlayer, 1218519041 | 1 << this.AdditionalLosBlockingLayer);
            }
        ...
        private void randomRaidChat(BasePlayer player, string command, string[] args)
        {
            ...
            if (args.Length >= 1)
            {
                switch (args[0].ToLower())
                {
                    ...
                    case "reload":
                        SendReply(player, "Stoping all raid's!");
                        StopAllRaids();
                        return;
        ...
        {
            foreach (var playerNameID in privilege.authorizedPlayers.ToList())
            {
            ...
            {  
                ...
                if (configData.settingsNPCKILLS.npcSettings.ContainsKey(name))
                {
                    ...
                    if (logData.pLogs[info.InitiatorPlayer.userID].NPCKILLS[name] >= configData.settingsNPCKILLS.npcSettings[name].totalRaids)
                        CallRaidOnNpcKills(info.InitiatorPlayer, name);
                }
            }           
        }
            ...
            if (!string.IsNullOrEmpty(profilename))
            {
                if (configData.settingsNPCKILLS.npcSettings.ContainsKey(profilename))
                {
                    ...
                    if (logData.pLogs[info.InitiatorPlayer.userID].NPCKILLS[profilename] >= configData.settingsNPCKILLS.npcSettings[profilename].totalRaids)
                        CallRaidOnNpcKills(info.InitiatorPlayer, profilename);
                }
            }
        }
        ...
        private void CallRaidOnNpcKills(BasePlayer p, string Rtype)
        {
            if (p != null)
            {
                ...
                NPCConfig config = configData.settingsNPCKILLS.npcSettings[Rtype];
                if (config == null) return;
                        ...
                        if (configData.settingsNPCKILLS.warnPlayer)
                        {
                            foreach (var user in priv.authorizedPlayers.ToList())
                            {
                                ...
                                if (p != null)
                                {
                                    ...
                                    SendReply(p, lang.GetMessage("warningRevengeGridNpc", this, p.UserIDString), GetGrid(priv.transform.position), FormatTime(newTime), FormatTimeS(newTime));
                                    if (configData.settings.gameTip)
                                        GameTipMessage(p, string.Format(lang.GetMessage("warningRevengeGridNpc", this, p.UserIDString), GetGrid(priv.transform.position), FormatTime(newTime), FormatTimeS(newTime)));
                                    if (configData.settings.worldMarker)
                                        LocateBaseWithMarker(p, priv.transform.position, configData.settings.worldMarkerTime);
                                }
                            }
                        }
        ...
        private void GetFlareItem(BasePlayer player, string name, int total, bool message)
        {
            ...
            Vector3 velocity = new Vector3(3.2009f, 2.639f, 0.2845f);
        ...
        private new void LoadDefaultMessages()
        {
            ...
            {
                ["warningRevengeGrid"] = "<color=orange>[</color><color=red>Cobalt Scientist</color><color=orange>]</color> So you raided are {0} base now we are coming for your base at {1}",
                ["warningRevengeGridNpc"] = "<color=orange>[</color><color=red>Cobalt Scientist</color><color=orange>]</color> So you think you can keep killing us? Now we are coming for your base at {0}",
