...
{
    [Info("AdvancedAlerts", "ProCelle", "1.0.10")]
    public class AdvancedAlerts : RustPlugin
    {
        [PluginReference]
        private Plugin ImageLibrary, RaidableBases, Convoy, Sputnik;
        ...
        void OnEntitySpawned(BaseNetworkable entity)
        {
            ...
            if(entity is BaseHelicopter && enableCopter){
                timer.Once(1f, () =>
                {
            ...
            else if(entity is MLRSRocket && CanAlert) {
        ...
        {
            if (entity.ShortPrefabName.Contains("survey_charge") && CanAlert == false)
            CanAlert = true;

            Vector3 pos = entity.transform.position;
            if(entity is BaseHelicopter && enableCopterDestroyed)
            {
                    var isconvoy = Convoy?.Call("IsConvoyHeli", entity);
                    bool myBool = Convert.ToBoolean(isconvoy);
                    if(myBool) return;
        ...
        class GeneralSettings {
            ...
            public string AlertImg = "https://gspics.org/images/2024/02/20/0ba7gn.png";
        ...
        class CrateSettings {
            ...
            [JsonProperty(PropertyName = "Alert text when spawning")]
            public string CrateText = "A locked crate has apeared!";
