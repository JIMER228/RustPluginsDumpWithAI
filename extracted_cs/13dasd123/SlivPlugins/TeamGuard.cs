...
{
    [Info("TeamGuard", "RustPlugin.ru -Vlad-00003", "2.1.10")]
    ...
    internal class TeamGuard : RustPlugin
    {
        ...
        private class TGChecker : TriggerBase
        {
            ...
            private bool ShouldCount(BasePlayer player)
            {
                ...
                return player.IsConnected && !InWhitelistedZone(player) && !InSafeZone(player) &&
                       !IsDuelPlayer(player) && !IsOnTournament(player) &&
        ...
        private readonly Dictionary<BasePlayer, ulong> _lastUsed = new Dictionary<BasePlayer, ulong>();
        ...
        private object OnCodeEntered(CodeLock codeLock, BasePlayer player, string code)
        {
            ...
            if (!_lastUsed.TryGetValue(player, out lastUsedId))
            {
                ...
                _lastUsed[player] = codeLock.net.ID.Value;
                return false;
            }

            if (lastUsedId != codeLock.net.ID.Value)
            {
        ...
        private object OnCupboardAuthorize(BuildingPrivlidge privilege, BasePlayer player)
        {
            ...
            if (!_lastUsed.TryGetValue(player, out lastUsedId))
            {
                ...
                _lastUsed[player] = privilege.net.ID.Value;
                return false;
            }
            ...
            var element = privilege.authorizedPlayers.ElementAt(0);
            privilege.authorizedPlayers.Remove(element);
        ...
        private object OnTurretAssign(AutoTurret turret, ulong targetSteamId, BasePlayer assigner)
        {
            turret.authorizedPlayers.RemoveWhere(x => x.userid == targetSteamId);
            if (HasAuth(targetSteamId))
                return null;
            ...
            ulong lastUsedId;
            if (!_lastUsed.TryGetValue(assigner, out lastUsedId))
            {
                _config.Turret.GetLimitMessage(ChatMessage.TurretName).SendToChat(assigner);
                Log("LogLimit", assigner.displayName, ChatMessage.TurretName, owner, assigner.ServerPosition,
                    turret.ServerPosition);
                _lastUsed[assigner] = turret.net.ID.Value;
                return false;
            }

            if (_config.Turret.ClearAll)
            {
                Log("LogCleared", assigner.displayName, ChatMessage.TurretName, owner, assigner.ServerPosition,
                    turret.ServerPosition);
            ...
            Log("LogReplaced", assigner.displayName, GetName(replaced.userid), ChatMessage.TurretName, owner,
                assigner.ServerPosition, turret.ServerPosition);
            _lastUsed.Remove(assigner);
            return null;
        }
        ...
        private object OnTurretAuthorize(AutoTurret turret, BasePlayer player)
        {
            ...
            if (!_lastUsed.TryGetValue(player, out lastUsedId))
            {
                ...
                Log("LogLimit", player.displayName, ChatMessage.TurretName, owner, player.ServerPosition,
                    turret.ServerPosition);
                _lastUsed[player] = turret.net.ID.Value;
                return false;
            }
        ...
        private static bool InSafeZone(BasePlayer player, ulong playerid = 0)
        {
        ...
        private bool HasAuth(ulong playerid = 0)
        {
        ...
        private static bool IsOnTournament(BasePlayer player)
        {
            if (_inst.ArenaTournament == null)
                return false;
            var isOnTournament = _inst.ArenaTournament.Call("IsOnTournament", player) as bool?;
            if (isOnTournament.HasValue && isOnTournament.Value)
                return true;
            isOnTournament = _inst.ArenaTournament.Call("IsOnTournament", player.userID) as bool?;
            if (isOnTournament.HasValue && isOnTournament.Value)
                return true;
            return false;
        }
