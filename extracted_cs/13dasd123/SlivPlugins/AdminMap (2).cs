...
#region Methods
        ...
        static IEnumerable<PlayerNameID> GetDistrictAuthorizedPlayers(HashSet<PlayerNameID> players) => players.GroupBy(x => x.username).Select(y => y.First());
        ...
        MapNote GetTCNote(BuildingPrivlidge privlidge, string label = null) => new MapNote
        {
            ...
            isPing = true,
            icon = 2,
            colourIndex = Mathf.Clamp(GetDistrictAuthorizedPlayers(privlidge.authorizedPlayers).Count() - 1, 0, 5),
                ...
                foreach (var admin in cupboardAdmins)
                {
                    using (MapNoteList markers = Facepunch.Pool.Get<MapNoteList>())
                    {
                        ...
                        foreach (var priv in privlidges)
                        {
                            ...
                            {
                                var districtAP = GetDistrictAuthorizedPlayers(priv.authorizedPlayers);
                                var __1 = districtAP.Where(authPlayer => authPlayer.username.ToLower().Contains(admin.searchQuery.ToLower()));
                                var __2 = districtAP.Where(authPlayer => !__1.Contains(authPlayer));
                ...
                private void RenderCupboardInfoGui(BuildingPrivlidge cupboard)
                {
                    ...
                    cui.AddText(
                        text:
                            ...
                            string.Concat(GetDistrictAuthorizedPlayers(cupboard.authorizedPlayers).Select(player => (searchQuery != string.Empty && player.username.ToLower().Contains(searchQuery.ToLower()) ? $"<color=yellow>{player.username}</color>\n" : $"{player.username}\n"))) +
                            "<b>Upkeep Cost:</b>\n" +
            ...
            public static AdminData Get(BasePlayer player)
            {
                ...
                if (player.IsConnected && player.UserIDString != null && Instance.permission.UserHasPermission(player.UserIDString, PERMISSION_TO_USE))
                {
                    if (data == null)
                    {
                        ...
                        Instance.Subscribe(nameof(OnPlayerTick));
                    }
                }
                else if (data != null)
                {
                    if (Database.Remove(player.userID))
                    {
                        data.Dispose();
                        if (Database.Count == 0)
                            Instance.Unsubscribe(nameof(OnPlayerTick));
                        return null;
                    }
                }
                return data;
