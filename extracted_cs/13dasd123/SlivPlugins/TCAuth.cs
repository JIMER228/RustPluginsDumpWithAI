    ...
    internal class TCAuth : RustPlugin
    {
        ...
        private void OnEntitySpawned(AutoTurret entity) =>
            NextTick(() =>
            {
                ...
                {
                    entity.authorizedPlayers.RemoveAll(x => x.userid == player.userid);
        ...
        private object OnCupboardAuthorize(BuildingPrivlidge privilege, BasePlayer player)
        {
            ...
            NextTick(() =>
            {
                ...
                if (_config.AROpen)
                    AuthPlayerInLock(player.userID, privilege.transform.position, privilege.authorizedPlayers);   
            });
            return null;
        }
        ...
        private object OnSamSiteTarget(SamSite samSite, MiniCopter target)
        {
        ...
        private void AuthPlayerInTurrets(ulong id, Vector3 position)
        {
            ...
            foreach (var check in turrets)
            {
                ...
                check.authorizedPlayers.RemoveAll(x => x.userid == id);
