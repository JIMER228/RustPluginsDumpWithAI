...
{
    [Info("Skill Tree", "imthenewguy", "1.4.12")]
    ...
    class SkillTree : RustPlugin
    {
        ...
        void OnPlayerDeath(BasePlayer player, HitInfo info)
        {
            ...
            {
                if (config.roguelikeSettings.enabled && config.roguelikeSettings.wipeOnFallDeath && PlayerIsReal(player) && (!config.roguelikeSettings.requireConnection || player.IsConnected))
                {
            ...
            {
                if (config.roguelikeSettings.enabled && config.roguelikeSettings.wipeOnSuicideDeath && PlayerIsReal(player) && (!config.roguelikeSettings.requireConnection || player.IsConnected))
                {
                    HandleDataReset(player);
                    Player.Message(player, lang.GetMessage("RoguelikeDeathMessage", this, player.UserIDString), config.misc_settings.ChatID);
                    CreateGameTip(lang.GetMessage("RoguelikeDeathMessage", this, player.UserIDString), player);
            ...
            if (info.InitiatorPlayer != null && !info.InitiatorPlayer.IsNpc && info.InitiatorPlayer.userID.IsSteamId())
            {
                switch (player.GetType().Name)
                {
                    ...
                    default:
                        ...
                        {
                            if (config.roguelikeSettings.enabled && config.roguelikeSettings.wipeOnPVPDeath && (!config.roguelikeSettings.requireConnection || player.IsConnected))
                            {
                                HandleDataReset(player);
                                Player.Message(player, lang.GetMessage("RoguelikeDeathMessage", this, player.UserIDString), config.misc_settings.ChatID);
                                CreateGameTip(lang.GetMessage("RoguelikeDeathMessage", this, player.UserIDString), player);
            ...
            {
                if (config.roguelikeSettings.enabled && config.roguelikeSettings.wipeOnPVEDeath && PlayerIsReal(player) && (!config.roguelikeSettings.requireConnection || player.IsConnected))
                {
                    HandleDataReset(player);
                    Player.Message(player, lang.GetMessage("RoguelikeDeathMessage", this, player.UserIDString), config.misc_settings.ChatID);
                    CreateGameTip(lang.GetMessage("RoguelikeDeathMessage", this, player.UserIDString), player);
        ...
        void HandleSonarBuff(BasePlayer player)
        {
            ...
            foreach (var entity in validEntities)
            {
                ...
                player.SendConsoleCommand("ddraw.text", config.buff_settings.sonar_settings.time_on_screen, col, entity.transform.position, lang.GetMessage(type.ToString(), this, player.UserIDString));
            }
