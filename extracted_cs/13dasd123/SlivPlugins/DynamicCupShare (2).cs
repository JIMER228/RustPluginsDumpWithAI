...
{
    [Info("DynamicCupShare", "k1lly0u", "3.1.8")]
    ...
    class DynamicCupShare : ChaosPlugin
    {
        ...
        private object OnEntityEnter(TargetTrigger targetTrigger, BasePlayer player)
        {
            ...
            if (!TargetTriggerEntity.TryGet(targetTrigger, out TargetTriggerEntity targetTriggerEntity))
            {
                ...
                if (baseEntity is NPCAutoTurret)
                {
        ...
        private object OnSamSiteTarget(SamSite samSite, SamSite.ISamSiteTarget samSiteTarget)
        {
            ...
            if (!(samSiteTarget is BaseCombatEntity baseCombatEntity))
                return null;
        ...
        private object CanBuild(Planner planner, Construction construction, Construction.Target target)
        {
            ...
            Construction.Placement placement;
            ...
            {
                placement = new Construction.Placement(target); // Передача аргумента target в конструктор
                ...
                if (placement.rotation == Quaternion.identity)
                    placement.rotation = Quaternion.Euler(0, player.transform.rotation.y, 0); // Исправление получения угла поворота
            }
        ...
        private class PlayerPrivilege : MonoBehaviour
        {
            ...
            {
                if (m_PlayerLookup.TryGetValue(player, out PlayerPrivilege playerPrivilege) && playerPrivilege)
        ...
        private class TargetTriggerEntity
        {
            ...
            public object ShouldIgnore(BasePlayer player) 
                => _shouldIgnorePlayer(_entity, player) ? (object)true : null;
        ...
        private static class TemporaryShares
        {
            ...
            public static IEnumerator RebuildSharesFor(BuildingPrivlidge buildingPrivlidge)
            {
                ...
                if (buildingPrivlidge && buildingPrivlidge.OwnerID != 0UL && (!Configuration.Security.PreventShareNoOwner || buildingPrivlidge.IsAuthed(buildingPrivlidge.OwnerID)))
                {
                    ...
                    if (m_BuildingPrivilegeShares.TryGetValue(buildingPrivlidge, out List<ulong> currentShares))
                    {
                        currentShares.Clear();

                        foreach (PlayerNameID authorizedPlayer in buildingPrivlidge.authorizedPlayers)
                        {
                            if (currentShares.Contains(authorizedPlayer.userid))
                            {
                                buildingPrivlidge.authorizedPlayers.RemoveWhere(playerNameID => playerNameID.userid == authorizedPlayer.userid);
                                hasChanges = true;
                            }
                        }
                    }
            ...
            {
                if (m_BuildingPrivilegeShares.TryGetValue(buildingPrivlidge, out List<ulong> currentShares) && currentShares.Contains(player.userID))
                    currentShares.Remove(player.userID);

                buildingPrivlidge.authorizedPlayers.RemoveWhere(playerNameID => playerNameID.userid == player.userID);
                buildingPrivlidge.SendNetworkUpdate(BasePlayer.NetworkQueue.Update);
            }
            ...
            public static IEnumerator RebuildSharesFor(AutoTurret autoTurret)
            {
                ...
                {
                    bool hasChanges = false;
                    if (m_AutoTurretShares.TryGetValue(autoTurret, out List<ulong> currentShares))
                    {
                        List<PlayerNameID> playersToRemove = new List<PlayerNameID>(); 

                        foreach (PlayerNameID authorizedPlayer in autoTurret.authorizedPlayers)
                        {
                            if (currentShares.Contains(authorizedPlayer.userid))
                            {
                                playersToRemove.Add(authorizedPlayer); 
                                hasChanges = true;
                            }
                        }

                        
                        foreach (PlayerNameID playerToRemove in playersToRemove)
                        {
                            autoTurret.authorizedPlayers.Remove(playerToRemove);
                        }
            ...
            {
                if (m_AutoTurretShares.TryGetValue(autoTurret, out List<ulong> currentShares) && currentShares.Contains(player.userID))
                    currentShares.Remove(player.userID);

                autoTurret.authorizedPlayers.RemoveWhere(playerNameID => playerNameID.userid == player.userID);
                autoTurret.SendNetworkUpdate(BasePlayer.NetworkQueue.Update);
            }
            ...
            public static IEnumerator RebuildSharesFor(ShareType shareType, CodeLock codeLock)
            {
                ...
                {
                    bool hasChanges = false;
                    if (m_CodeLockShares.TryGetValue(codeLock, out List<ulong> currentShares))
                    {
        ...
        private void SetupLockableContainers()
        {
            ...
            foreach (BaseNetworkable baseNetworkable in BaseNetworkable.serverEntities)
            {
                ...
                if (m_CustomLockableReferences.TryGetValue(baseNetworkable.PrefabName, out StorageContainer storageContainer))
        ...
        private void FindRegisterEntities()
        {
            ...
            foreach (BaseNetworkable baseNetworkable in BaseNetworkable.serverEntities)
            {
                if (baseNetworkable is BuildingPrivlidge)
                {
                    ...
                    {
                        foreach (var playerId in tempShareData.ToList())
                        {
                            (baseNetworkable as BuildingPrivlidge).authorizedPlayers.RemoveWhere(playerNameID => playerNameID.userid == playerId);
                        }
                    }
                ...
                else if (baseNetworkable is AutoTurret)
                {
                    if (temporaryShareData.temporaryTurretShares.TryGetValue(baseNetworkable.net.ID.Value, out tempShareData))
                    {
                        foreach (var playerId in tempShareData.ToList())
                        {
                            (baseNetworkable as AutoTurret).authorizedPlayers.RemoveWhere(playerNameID => playerNameID.userid == playerId);
                        }
                    }
                ...
                else if (baseNetworkable is CodeLock)
                {
                    if (temporaryShareData.temporaryCodeLockShare.TryGetValue(baseNetworkable.net.ID.Value, out tempShareData))
                    {
                        foreach (var playerId in tempShareData.ToList())
                        {
                            (baseNetworkable as CodeLock).guestPlayers.Remove(playerId);
                        }
                    }
        ...
        private bool CanUseSamSite(SamSite samSite, BaseCombatEntity baseCombatEntity)
        {
            ...
            if (baseCombatEntity is BaseVehicle baseVehicle)
            {
                ...
                {
                    if (!mountPoint.mountable || !(mountPoint.mountable.GetMounted() is BasePlayer player)) 
                        return false;
