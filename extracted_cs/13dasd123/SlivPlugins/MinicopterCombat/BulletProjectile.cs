...
{
    [Info("BulletProjectile", "Bxrflip", "1.0.1")]
    public class BulletProjectile : RustPlugin
    {
        ...
        public class ProjectileSystem : MonoBehaviour
        {
            ...
            private void DoMovement(float deltaTime)
            {
                ...
                {
                    if (object.ReferenceEquals(list[index], null) || !list[index].valid)
                        continue;

                    list[index].UpdateHitTest(this.hitTest);

                    if (!object.ReferenceEquals(this.hitTest, null))
                    {
            ...
            public static Attack BuildAttackMessage(HitTest test)
            {
                ...
                if (test.DidHit)
                {
                    ...
                    {
                        attack.hitID = new NetworkableId((uint)0);
                        attack.hitBone = 0U;
            ...
            public static void LoadFromAttack(HitInfo info, Attack attack, bool serverSide)
            {
                ...
                if (attack.hitID != new NetworkableId((uint)0))
                {
                    info.DidHit = true;
