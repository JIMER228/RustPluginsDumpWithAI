...
{
    [Info("Game Mode Manager", "Amino", "1.1.2")]
    ...
    public class GameModeManager : RustPlugin
    {
        ...
        public void ClearEntities(string zoneId)
        {
            ...
            {
                if (entity.HasFlag(BaseEntity.Flags.Locked) && entity.OwnerID == 11111111111111111)
                {
                    continue;
                }
                entity.AdminKill();
        ...
        private object OnPlayerChat(BasePlayer player, string message, Chat.ChatChannel channel)
        {
            ...
            if (channel == Chat.ChatChannel.Team)
            {
                ...
                team.BroadcastTeamChat(player.userID, userName, formattedMessage, "white");

                var onlineMemberConnections = team.GetOnlineMemberConnections();
                if (onlineMemberConnections != null)
                {
                    ConsoleNetwork.SendClientCommand(onlineMemberConnections, "chat.add", (int)channel, player.userID, formattedMessage);
                }
            }
            else
            {
                var playersToSendChat = new List<BasePlayer>();
                ...
                if (lobby != null)
                {
                    playersToSendChat.AddRange(lobby.ActivePlayers.Select(x => x.Player));
                }
                else
                {
                    var players = Interface.Call(nameof(IAdvancedLobby.GetPlayerMatchPlayers), player.userID) as List<BasePlayer>;
                    if (players != null)
                    {
                        playersToSendChat.AddRange(players);
                    }
                }
                foreach (var basePlayer in playersToSendChat)
                {
                    basePlayer.SendConsoleCommand("chat.add", new object[] { (int)channel, player.UserIDString, formattedMessage });
                }
            }
            return false;
        }
