...
namespace Oxide.Plugins
{
    ...
    class NPCTaxi : RustPlugin, IPathFinder
    {
        ...
        private List<GridNode> _roadNodes = ListPool.Get<GridNode>();
        private static Hash<ulong, float> _cooldowns = new Hash<ulong, float>();

        private static int _scrapItemID;

        private static NPCTaxi Instance;

        private static Oxide.Ext.AStar.PathFinder PathFinder;

        private static List<BasePlayer> PathRequests;
        private static List<VehicleAI> ActiveVehicles;
        ...
        private void Loaded()
        {
            ...
            PathRequests = ListPool.Get<BasePlayer>();
            ActiveVehicles = ListPool.Get<VehicleAI>();            
        ...
        private void OnServerInitialized()
        {
            ...
            PathFinder = PathManager.RegisterPathFinder(this, new PathFinderConfig { NodeEvaluator = EvaluateNode, WorldSize = World.Size, CellSize = WORLD_GRID_SIZE, PathInterval = 10f, BlurRate = 2 });
        }
        ...
        private void OnEntityDismounted(BaseMountable mountable, BasePlayer player)
        {
            ...
            if (vehicleAI != null)
            {
                if (player == vehicleAI.Caller)
                {
                    ...
                    _cooldowns[player.userID] = Time.time + Configuration.CooldownTime;
                }
            }
        }
        ...
        private void Unload()
        {
            ...
            ListPool.Free(ref ActiveVehicles);
            ListPool.Free(ref PathRequests);
            ListPool.Free(ref _roadNodes);
        ...
        {        
            ...
            PathRequests.Remove(pathResult.Requester);

            pathResult.Requester.ChatMessage(Message("Notification.Inbound", pathResult.Requester.userID, true));
            SendMapMarker(pathResult.Requester, results[results.Count - 1], "Taxi Pickup");

            VehicleAI vehicleAI = CreateTaxiVehicle(results[0] + Vector3.up, Quaternion.LookRotation((results[1] - results[0]).normalized));
            vehicleAI.InitializeVehicle(pathResult.Requester, results);

            ActiveVehicles.Add(vehicleAI);

            _cooldowns[pathResult.Requester.userID] = Time.time + Configuration.CooldownTime;
        ...
        private void OnPathFailed(PathFailedResult pathResult)
        {
            ...
            pathResult.Requester.ChatMessage(string.Format(Message("Error.PathFail", pathResult.Requester.userID), pathResult.Reason));
            PathRequests.Remove(pathResult.Requester);
        }
        #endregion
        ...
        public void EvaluateNode(GridNode gridNode, Vector3 position)
        {
            ...
            if (topologyInRadius.ContainsTopology(TerrainTopology.Enum.Road))
            {
            ...
            if (topologyInRadius.ContainsTopology(TerrainTopology.Enum.Ocean | TerrainTopology.Enum.River | TerrainTopology.Enum.Lake | TerrainTopology.Enum.Cliff | 
            ...
            if (topologyInRadius.ContainsTopology(TerrainTopology.Enum.Roadside))            
                return;
            
            if (topologyInRadius.ContainsTopology(TerrainTopology.Enum.Monument | TerrainTopology.Enum.Forest | TerrainTopology.Enum.Forestside | TerrainTopology.Enum.Cliffside))
                gridNode.C_Cost = 750;
        ...
        private class VehicleAI : MonoBehaviour
        {
            ...
            internal void InitializeVehicle(BasePlayer caller, List<Vector3> path)
            {
                ...
                _targetNode = _path[_targetIndex];
                UpdateNodePlane();
            ...
            {
                StorageContainer fuelSystem = Entity.GetFuelSystem().GetFuelContainer();
            ...
            private void Update()
            {
                if (_driver == null || _driver.IsDead())
                {
                ...
                if (Entity.IsFlipped())
                {
                    _tr.position = _tr.position + Vector3.up;
            ...
            private Vector3 GetNode(int index)
            {
                if (index > _path.Count - 1)
                    index = index - _path.Count - 1;

                if (index < 0)
                    index = _path.Count + index;
            ...
            private void DoSteering(Vector3 relativeTarget)
            {
                ...
                _inputState.current.buttons |= (int)BUTTON.DUCK;
            ...
            internal void OnMarkerPlaced(Vector3 position, MapNote mapNote)
            {
                ...
                FareCost = 0;
                PathFinder.EvaluateGrid(new PathRequest(_tr.position, position, Caller, OnPathGenerated, OnPathFailed));                
            }
            ...
            private void OnPathGenerated(PathCompletedResult pathResult)
            {
                ...
                if (Caller == null || !Caller.IsConnected)
                {
                    ListPool.Free(ref results);
                    return;
                }

                if (this._path != null)
                    ListPool.Free(ref this._path);                
                ...
                else
                {
                    ...
                    for (int i = 1; i < results.Count; i++)
                        distance += Vector3.Distance(_path[i - 1], _path[i]);
            ...
            {
                PathRequests.Remove(Caller);
                Caller.ChatMessage(pathResult.Reason);
            ...
            internal void CancelService()
            {
                ...
                if (HasPaidFare && _targetIndex < _path.Count - 1)
                {
                    int refundFare = FareCost - Mathf.RoundToInt((((float)_targetIndex / (float)_path.Count) * (float)FareCost));
                ...
                if (_targetIndex > _path.Count * 0.5f)
                {
                    _path.Reverse();
                    _targetIndex = 1;
                }
        ...
        private void RequestTaxi(BasePlayer player, bool isTelephone)
        {
            ...
            float cooldown;
            if (_cooldowns.TryGetValue(player.userID, out cooldown))
            {
            ...
            PathFinder.EvaluateGrid(new PathRequest(spawnPoint, closestNode, player, OnPathGenerated, OnPathFailed));
        }
        ...
        private void DrawPath(BasePlayer player, string command, string[] args)
        {
            ...
            {
                for (int i = 0; i < vehicleAI._path.Count; i++)
                {
                    player.SendConsoleCommand("ddraw.sphere", 10f, Color.blue, vehicleAI._path[i], 0.5f);
                }
            }
        }
        ...
        private void CellDebug(BasePlayer player, string command, string[] args)
        {
            ...
            List<GridNode> list = ListPool.Get<GridNode>();
        ...
        private Dictionary<string, string> Messages = new Dictionary<string, string>
        {
            ...
            ["Error.PathFail"] = "Path calculation failed : {0}",
