...
{
    [Info("IQGradeRemove", " Автор: Mercury", "2.29.35")]
    ...
    class IQGradeRemove : RustPlugin
    {
        ...
        public Dictionary<ulong, int> BuildingRemoveTimers = new Dictionary<ulong, int>();
        public Dictionary<ulong, int> BuildingRemoveBlock = new Dictionary<ulong, int>();
        void ReadData()
        {
            BuildingRemoveTimers = Oxide.Core.Interface.Oxide.DataFileSystem.ReadObject<Dictionary<ulong, int>>("IQGradeRemove/BlockBuilding");
            BuildingRemoveBlock = Oxide.Core.Interface.Oxide.DataFileSystem.ReadObject<Dictionary<ulong, int>>("IQGradeRemove/BuildingRemoveBlock");
        }
        void WriteData()
        {
        ...
        public String GradeErrorParse(BasePlayer player, BuildingBlock buildingBlock, BuildingGrade.Enum grade = BuildingGrade.Enum.None)
        {
            ...
            if (!permission.UserHasPermission(player.UserIDString, PermissionGRNoResource))
            {
                ...
                if (!buildingBlock.CanAffordUpgrade((BuildingGrade.Enum)Grade, 0, player))
        ...
        public void GradeBuilding(BasePlayer player, BuildingBlock buildingBlock)
        {
            ...
            if (!permission.UserHasPermission(player.UserIDString, PermissionGRNoResource))
                buildingBlock.PayForUpgrade(buildingBlock.blockDefinition.GetGrade((BuildingGrade.Enum)Data.GradeLevel, 0 ), player);
        ...
        public void GradeAll(BasePlayer player, BuildingBlock buildingBlock)
        {
            ...
            foreach (var Block in buildingBlock.GetBuildingPrivilege().GetBuilding().buildingBlocks.Where(x => x.grade != (BuildingGrade.Enum)DataPlayer[player.userID].GradeLevel))
            {
                if (!permission.UserHasPermission(player.UserIDString, PermissionGRNoResource))
                {
                    if (!Block.CanAffordUpgrade(((BuildingGrade.Enum)DataPlayer[player.userID].GradeLevel), 0,  player))
                    {
                    ...
                    {
                        Block.PayForUpgrade(buildingBlock.blockDefinition.GetGrade((BuildingGrade.Enum)DataPlayer[player.userID].GradeLevel, 0 ), player);
        ...
        private void AddRemoveBlockBuild(UInt64 netID, UInt64 userID)
        {
        ...
        private Boolean IsBlockAvailablePermanent(UInt64 netID)
        {
            if (BuildingRemoveBlock.ContainsKey(netID))
                return true;
            else return false;
        }
        private Boolean IsBlockBuildPemanent(UInt64 netID)
        {
        ...
        private void AddBlockBuild(UInt64 netID, UInt64 userID)
        {
            var RemoveTime = config.RemoveSetting.TimedSetting;
        ...
        private Boolean IsBlockAvailable(UInt64 netID)
        {
            if (BuildingRemoveTimers.ContainsKey(netID))
                return true;
            else return false;
        }
        private Boolean IsBlockBuild(UInt64 netID)
        {
        ...
        private Int32 GetTimerBlock(UInt64 netID)
        {
            Int32 Time = 0;
            if (IsBlockAvailable(netID))
        ...
        public string RemoveErrorParseBuilding(BasePlayer player, BaseEntity buildingBlock)
        {
            ...
            if (RemoveTimed.UseTimesBlock && IsBlockBuild(buildingBlock.net.ID.Value))
                return GetLang("REMOVE_TIME_EXECUTE", player.UserIDString, FormatTime(TimeSpan.FromSeconds(GetTimerBlock(buildingBlock.net.ID.Value))));

            if (RemoveTimed.UseAllBlock && IsBlockBuildPemanent(buildingBlock.net.ID.Value))
        ...
        private void OnEntityBuilt(Planner plan, GameObject go)
        {
            ...
            {
                AddRemoveBlockBuild(go.ToBaseEntity().net.ID.Value, player.userID);
                AddBlockBuild(go.ToBaseEntity().net.ID.Value, player.userID);
            }
