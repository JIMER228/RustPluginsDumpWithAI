...
{
    [Info("IQCraftSystem", "https://topplugin.ru/ / https://discord.com/invite/5DPTsRmd3G", "0.0.8")]
    ...
    class IQCraftSystem : RustPlugin
    {
        ...
        /// Обновление 0.0.6
        /// - Исправил проблему с крафтом предметов в условии которых есть прдемет со SkinID
        /// - Исправил проблему когда при смене категорий учмитывалась страница старых категорий
        /// - Изменен метод отрисовки категорий
        /// - Добавлена возможность отключать каждую категорию  
        /// - Добавлено удаление всего UI при выгрузке плагина
        /// - Добавлена возможность крафта предметов по пермишенсу
        /// - Добавлена перезарядка на крафт предмета
        /// - Добавил обводку текста в требываемых предметах для крафта
        /// Обновление 0.0.7
        /// - Поправил метод с крафтом предмета
        /// 
        ...
        #region Data
        [JsonProperty("Информация о крафте")] public Hash<ulong, DataCraft> DataCrafts = new Hash<ulong, DataCraft>();
        public class DataCraft
        {
            [JsonProperty("Информация о предметах")] public Dictionary<String, Int32> CooldownCraft = new Dictionary<String, Int32>();
        }
        void ReadData()
        {
            DataCrafts = Oxide.Core.Interface.Oxide.DataFileSystem.ReadObject<Hash<ulong, DataCraft>>("IQCraftSystem/CraftUsers");
        }
        void WriteData()
        {
        ...
        void RegisteredDataUser(ulong player)
        {
            ...
            if (!DataCrafts.ContainsKey(player))
                DataCrafts.Add(player, new DataCraft { CooldownCraft = new Dictionary<string, int> { } });
        }
        ...
        private class Configuration
        {
            ...
            {
                [JsonProperty("Перезарядка на крафт предмета (Если она не нужна, поставьте значение 0")]
                public Int32 Cooldown;
                [JsonProperty("Права для доступа к этому крафту (оставьте поле пустым - будет доступно всем)")]
            ...
            internal class InterfaceSettings
            {
                ...
                internal class CategorySettings
                {
                    ...
                    [JsonProperty("Включения и отключения категорий")]
                    ...
                    {
                        [JsonProperty("Включить категорию оружие?")]
                        public Boolean TurnWeapon;
                        [JsonProperty("Включить категории инструментов")]
                        public Boolean TurnTools;
                        [JsonProperty("Включить категории конструкций")]
                        public Boolean TurnConstruction;
                        [JsonProperty("Включить категории предметов")]
                        public Boolean TurnItems;
                        [JsonProperty("Включить категории одежды")]
                        public Boolean TurnAttirie;
                        [JsonProperty("Включить категории электрики")]
                        public Boolean TurnElectrical;
                        [JsonProperty("Включить категории транспорта")]
                        public Boolean TurnTransport;
                        [JsonProperty("Включить категории фана")]
                        public Boolean TurnFun;
                        [JsonProperty("Включить кастомных предметов")]
        ...
        private void OnServerInitialized()
        {
            ...
            foreach (var Item in config.ItemSetting)
                ...
                Components =
                    {
                        ...
                        new CuiRectTransformComponent{ AnchorMin = "0.96250011397 0.9351854", AnchorMax = "0.9958335 0.9944444" },  
                    }
            });
        ...
        void CategoryListLoad()
        {
            var Interface = config.InterfaceSetting;
            var Category = Interface.CategorySetting.PNGCategoryIcons;
            var TurnedCategory = Interface.CategorySetting.TurnedCategorys;
        ...
        void LoadedCategory(BasePlayer player)
        {
            ...
            {
                FadeOut = FadeOut, 
                RectTransform = { AnchorMin = "0.309375 0.833331397", AnchorMax = "0.5677084 0.8787037" }, 
                Image = { FadeIn = FadeIn, Color = "0 0 0 0" }
            }, CRAFTSYSTEM_HUD, "CATEGORY_PANEL");


            Int32 Category = 0;
            foreach(var Categorys in CategoryList)
            {
                container.Add(new CuiElement
                {
                    FadeOut = FadeOut,
