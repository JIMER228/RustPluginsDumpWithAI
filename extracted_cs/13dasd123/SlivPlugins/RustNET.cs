...
namespace Oxide.Plugins
{
    ...
    class RustNET : RustPlugin
    {
        ...
        private List<DroppedItem> placementItems = new List<DroppedItem>();

        private bool wipeDetected;
        private bool isInitialized;

        private const string USE_PERMISSION = "rustnet.use";
        private const string PUBLIC_PERMISSION = "rustnet.public";

        private const string BURLAP_SACK_PREFAB = "assets/prefabs/misc/burlap sack/generic_world.prefab";
        private const string PLAYER_PREFAB = "assets/prefabs/player/player.prefab";

        private const int TERMINAL_ID = 1523195708;

        private readonly string dataDirectory = $"file://{Interface.Oxide.DataDirectory}{Path.DirectorySeparatorChar}RustNET{Path.DirectorySeparatorChar}Images{Path.DirectorySeparatorChar}";
        #endregion
        ...
        private void Loaded()
        {
            ...
            permission.RegisterPermission(PUBLIC_PERMISSION, this);
        ...
        private void OnServerInitialized()
        {
            ...
            foreach (KeyValuePair<Colors, ConfigData.InterfaceOptions.UIColor> color in configData.Interface.UIColors)
        ...
        private void OnPlayerInput(BasePlayer player, InputState input)
        {
            ...
            if (input.WasJustPressed(BUTTON.USE))
            {
                ...
                if (Physics.SphereCast(player.eyes.position, 0.2f, Quaternion.Euler(player.serverInput.current.aimAngles) * Vector3.forward, out hit, 3f))
                {
                    ...
                    TerminalManager terminal = baseEntity.GetComponent<TerminalManager>();
                    if (terminal != null)
                    {
        ...
        {
            if (entity && entity.GetComponent<Controller>())                            
                return true;            
            return null;
        }
        #endregion
        ...
        private void InitializeAllTerminals()
        {
            for (int i = 0; i < storedData.terminals.Count; i++)
            {
                ...
                DroppedItem droppedItem = SpawnDroppedItem(TERMINAL_ID, new Vector3(savedEntity.x, savedEntity.y, savedEntity.z), new Vector3(savedEntity.eX, savedEntity.eY, savedEntity.eZ));
        ...
        private void InitializeNewTerminal(BasePlayer player, DroppedItem droppedItem)
        {
            ...
            if (building != null)
            {
                if (linkManager.GetLinkOf(building) != null)
                {
                    ...
                    player.GiveItem(ItemManager.CreateByItemID(TERMINAL_ID), BaseEntity.GiveItemReason.PickedUp);
                    return;
                }
            }
            TerminalManager terminal = SpawnDroppedItem(TERMINAL_ID, droppedItem.transform.position, droppedItem.transform.eulerAngles).gameObject.AddComponent<TerminalManager>();
        ...
        private object CanSpawnInZone(DroppedItem droppedItem)
        {
            if (placementItems.Contains(droppedItem))
                return true;

            if (droppedItem.GetComponent<TerminalManager>())
                return true;
            return null;
        }
        ...
        private string GetHelpString(ulong playerId, bool title)
        {
            ...
            else
            {
                ...
                if (permission.UserHasPermission(playerId.ToString(), PUBLIC_PERMISSION))
        ...
        public static DroppedItem SpawnDroppedItem(int itemId, Vector3 position, Vector3 rotation)
        {
            ...
            BaseEntity worldEntity = GameManager.server.CreateEntity(BURLAP_SACK_PREFAB, position, Quaternion.Euler(rotation));
        ...
        {
            const int LAYERS = 1 << 0 | 1 << 8 | 1 << 15 | 1 << 16 | 1 << 17 | 1 << 20 | 1 << 23 | 1 << 26;
            
            RaycastHit raycastHit;

            if (Physics.Raycast(player.eyes.HeadRay(), out raycastHit, ins.configData.Placement.MaxDistance, LAYERS, QueryTriggerInteraction.Ignore))
        ...
        public class Controller : MonoBehaviour
        {
            ...
            {
                dummyPlayer = (BasePlayer)GameManager.server.CreateEntity(PLAYER_PREFAB, player.transform.position, player.transform.rotation);                
        ...
        public class ItemPlacement : MonoBehaviour
        {
            ...
            private void FixedUpdate()
            {
                ...
                if (Physics.Raycast(new Ray(player.transform.position + (player.modelState.ducked ? Vector3.up * 0.7f : Vector3.up * 1.5f), Quaternion.Euler(input.current.aimAngles) * Vector3.forward), out hit, placementDistance, layerPlcmnt))
                {
                    ...
                    isValidPlacement = hitEntity = hit.GetEntity() as BuildingBlock;
                }
                else
                {
            ...
            private void SpawnWorldItem()
            {
                ...
                ins.placementItems.Add(droppedItem);
                enabled = true;
            }
            ...
            public void CancelPlacement()
            {
                ...
                ins.placementItems.Remove(droppedItem);
                droppedItem.DestroyItem();
                droppedItem.Kill();
                Destroy(this);
            }
        ...
        private void LoadPluginImages(int attempts = 0)
        {
            ...
            foreach (KeyValuePair<string, string> image in queuedImages)
        ...
        private void cmdTerminal(BasePlayer player, string command, string[] args)
        {
            if (args.Length == 0)
            {
                ...
                if (permission.UserHasPermission(player.UserIDString, PUBLIC_PERMISSION))
            ...
            switch (args[0].ToLower())
            {
                case "add":
                    Item activeItem = player.GetActiveItem();
                    if (activeItem == null || activeItem.info.itemid != TERMINAL_ID)
                    {
                        SendReply(player, msg("Error.HoldComputer", player.userID));
                        return;
                    }

                    bool ignoreRestriction = permission.UserHasPermission(player.UserIDString, PUBLIC_PERMISSION) && (args.Length > 1 && args[1].ToLower() == "public");

                    ItemPlacement placement = player.gameObject.AddComponent<ItemPlacement>();
                    if (ignoreRestriction)
                        placement.SetRequiredItem(TERMINAL_ID, new Vector3(0, 180, 0), new Vector3(-0.3f, 0, 0), false, true, InitializeNewTerminalPublic, null, null);
                    else placement.SetRequiredItem(TERMINAL_ID, new Vector3(0, 180, 0), new Vector3(-0.3f, 0, 0), false, false, InitializeNewTerminal, null, null);
                    return;
                case "remove":
                    BaseEntity baseEntity = FindEntityFromRay(player);

                    TerminalManager terminal = baseEntity != null ? baseEntity.GetComponent<TerminalManager>() : null;
                    if (terminal == null)
                    {
