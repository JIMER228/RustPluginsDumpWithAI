...
namespace Oxide.Plugins
{
    ...
    class RadHouse : RustPlugin
    {
        ...
        void CreateRadHouse(bool IsAdminCreate)
        {
            ...
            if (BoxID == 0) BoxID = Box.net.ID.Value; 
            ...
            Server.Broadcast($"Радиактивный дом появился, координаты: {pos.ToString()}\nЕсли никто не успеет авторизоваться за {cfg.General.TimeToRemove} секунд, он пропадет");
        ...
        void CreateLoot(StorageContainer Container, BaseEntity Box)
        {
            ...
            if (Container != null)
            {
                ...
                InitializeZone(Box.transform.position, 1, 10, 2145);
            }
        }
        ...
        private bool ValidPosition(ref Vector3 randomPos)
        {
            ...
            if (WaterLevel.GetWaterDepth(randomPos, true, false, null) <= 0.01f) return false;
