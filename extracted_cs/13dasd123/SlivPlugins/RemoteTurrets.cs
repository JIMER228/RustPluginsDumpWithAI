...
{
    [Info("RemoteTurrets", "k1lly0u", "0.2.48")]
    ...
    class RemoteTurrets : RustPlugin
    {
        ...
        private void Loaded()
        {
            ...
            foreach (string key in configData.Options.Permissions.Keys)
            {
        ...
        private object OnTurretShutdown(AutoTurret turret) => OnTurretInteractionGeneric(turret);

        private object OnTurretToggle(AutoTurret turret) => OnTurretInteractionGeneric(turret);

        private object OnTurretModeToggle(AutoTurret turret) => OnTurretInteractionGeneric(turret);

        private object OnTurretInteractionGeneric(AutoTurret turret)
        {
            TurretManager manager = turret.GetComponent<TurretManager>();
            if (manager != null && manager.isBeingUsed)
                return false;
            return null;
        }
        ...
        private void InitializeAllLinks()
        {
            for (int i = storedData.registeredTurrets.Length - 1; i >= 0; i--)
            {
                ...
                NetworkableId networkableId = new NetworkableId(turretData.turretId);
                AutoTurret turret = BaseEntity.serverEntities.Find(networkableId) as AutoTurret;
        ...
        {
            int max = 0;
            foreach (KeyValuePair<string, int> entry in configData.Options.Permissions)
            {
        ...
        private bool IsEntityEnabled(int terminalId, ulong managerId)
        {
            LinkManager.TurretLink link = linkManager.GetLinkOf(terminalId);
            if (link != null)
            {
                TurretManager manager = link.managers.Find(x => x.turret.net.ID.Value == managerId);
                if (manager != null)                
        ...
        private void InitializeController(BasePlayer player, ulong managerId, int terminalId)
        {
            LinkManager.TurretLink link = linkManager.GetLinkOf(terminalId);
            if (link != null)
        ...
        private void ToggleAutomation(ulong managerId, int terminalId, bool active)
        {
            LinkManager.TurretLink link = linkManager.GetLinkOf(terminalId);
            if (link != null)
            {
        ...
        private void OpenInventory(BasePlayer player, ulong managerId, int terminalId)
        {
            LinkManager.TurretLink link = linkManager.GetLinkOf(terminalId);
            if (link != null)
            {
        ...
        private class LinkManager
        {
            ...
            public class TurretLink
            {
                ...
                {
                    TurretManager manager = turret.GetComponent<TurretManager>() ?? turret.gameObject.AddComponent<TurretManager>();
                ...
                public void InitiateLink(BasePlayer player, ulong managerId)
                {
                    TurretManager manager = managers.FirstOrDefault(x => x.turret.net.ID.Value == managerId);
                    if (manager != null)
                    {
                        player.inventory.crafting.CancelAll(true);
                        Controller controller = player.GetComponent<Controller>() ?? player.gameObject.AddComponent<Controller>();
        ...
        private class Controller : RustNET.Controller
        {
            ...
            public override void Awake()
            {
                base.Awake();
                enabled = false;
                canCycle = ins.configData.Remote.CanCycle;
            }
            ...
            private void TryFireGun()
            {
                ...
                else
                {
                    if (Time.realtimeSinceStartup >= nextShotTime)
                    {
                        ...
                        if (manager.turret.HasClipAmmo())
                        {
                            BaseProjectile attachedWeapon = manager.turret.GetAttachedWeapon();
                            if (attachedWeapon)
                            {
                                manager.turret.FireAttachedGun(Vector3.zero, 0f);
                                ins.CreateAmmoUI(player, this);

                                float delay = 0;
                                if (attachedWeapon.primaryMagazine.contents <= 0)
                                {
                                    delay = Mathf.Min(attachedWeapon.GetReloadDuration() * 0.5f, 2f);
                                    manager.turret.Reload();
                                    InvokeHandler.Invoke(this, () => ins.CreateAmmoUI(player, this), delay + 0.5f);
                                }
                                else delay = attachedWeapon.ScaleRepeatDelay((attachedWeapon.isSemiAuto ? attachedWeapon.repeatDelay * 1.5f : attachedWeapon.repeatDelay));

                                nextShotTime = Time.realtimeSinceStartup + delay;
        ...
        private class TurretManager : MonoBehaviour
        {
            ...
            public bool IsPowered
            {
                get
                {
                    if (!ins.configData.Remote.RequirePower || (ins.configData.Remote.RequirePower && turret.IsPowered()))
                        return true;
                    return false;
                }
            }
            ...
            public void SetTurretStatus(bool status)
            {
                if (!status)
                {
                    ...
                    else InvokeHandler.Invoke(this, TurretShutdown, 1f);
                }
                else
                {
                    ...
                    if (IsPowered)
                    {
                        ...
                        {
                            Effect.server.Run(turret.onlineSound.resourcePath, turret, 0, Vector3.zero, Vector3.zero, null, false);
                            turret.Invoke(SetIsOnline, 2f);
                            turret.booting = true;
                        }
                        else
                        {
            ...
            public void UpdateAimDirection(InputState input)
            {
                ...
                if (Physics.Raycast(new Ray(turret.muzzlePos.transform.position, Quaternion.Euler(input.current.aimAngles) * Vector3.forward), out hit, 500f, layerMask))
                {
                    ...
                    float dt = Time.deltaTime; // Get the time delta
                    turret.UpdateAiming(dt);
                }
            }
            ...
            public class TurretData
            {
                ...
                {
                    turretId = manager.turret.net.ID.Value;
        ...
        private void CreateAmmoUI(BasePlayer player, Controller controller)
        {
            ...
            string ammoAmount = string.Empty;

            BaseProjectile attachedWeapon = controller.manager.turret.GetAttachedWeapon();
            if (attachedWeapon != null)
            {
                int clipAmmo = attachedWeapon.primaryMagazine.contents;
                int storedAmmo = 0;

                List<Item> list = Facepunch.Pool.GetList<Item>();
                controller.manager.turret.inventory.FindAmmo(list, attachedWeapon.primaryMagazine.definition.ammoTypes);

                for (int i = 0; i < list.Count; i++)                
                    storedAmmo += list[i].amount;
                
                Facepunch.Pool.FreeList<Item>(ref list);

                ammoAmount = $"{clipAmmo} / {storedAmmo}";
            }
            else ammoAmount = "0 / 0";            
        ...
        void cmdRemote(BasePlayer player, string command, string[] args)
        {
            ...
            if (entity == null || !(entity is AutoTurret) || entity is NPCAutoTurret)
            {
        ...
        {            
            ...
            {
                [JsonProperty(PropertyName = "Allow players to cycle through all linked turrets")]
                ...
                [JsonProperty(PropertyName = "Require power to access the turret")]
            ...
            public class OtherOptions
            {
                ...
                [JsonProperty(PropertyName = "Max placeable turrets using permission")]
            ...
            public class TurretOptions
            {
                ...
                [JsonProperty(PropertyName = "Maximum turrets allowed per base")]
        ...
        private ConfigData GetBaseConfig()
        {
            return new ConfigData
            {
                ...
                Remote = new ConfigData.RemoteOptions
                {
                    ...
                    RemoteControl = true,
                    ToggleEnabled = true,
                    OverlayImage = "http://www.rustedit.io/images/RustNET/turretoverlay.png",
                    Overlay = true,
                    RustNETIcon = "http://www.rustedit.io/images/RustNET/turreticon.png",
                    RequirePower = true
                },
                Version = Version
            };
        ...
        private void UpdateConfigValues()
        {
            ...
            if (configData.Version < new VersionNumber(0, 2, 13))
                configData.Remote.CanCycle = baseConfig.Remote.CanCycle;

            if (configData.Version < new VersionNumber(0, 2, 20))
                configData.Options.Permissions = baseConfig.Options.Permissions;

            if (configData.Version < new VersionNumber(0, 2, 40))
                configData.Remote.RequirePower = true;

            if (configData.Version < new VersionNumber(0, 2, 43))
                configData.Options.Permissions = baseConfig.Options.Permissions;
        ...
        {
            if (storedData == null || storedData.registeredTurrets == null)
                storedData = new StoredData();
