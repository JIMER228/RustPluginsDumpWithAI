    ...
    internal class NpcSpawn : RustPlugin
    {
        ...
        public class CustomScientistNpc : ScientistNPC
        {
            ...
            internal bool CanTargetNpcPlayer(NPCPlayer target)
            {
                ...
                if (_ins.IsGasStationNpc(target)) return true;
            ...
            internal bool IsAttackingNow { get; set; } = false;
            ...
            public override bool IsNpc
            {
                get
                {
                    ...
                    if (IsAttackingNow && (_ins._config.CanTargetNpc || _ins._config.CanTargetAnimal) && (Brain.CurrentState.StateType == AIState.Combat || Brain.CurrentState.StateType == AIState.CombatStationary)) return false;
            ...
            {
                List<BoxCollider> list = Pool.GetList<BoxCollider>();
            ...
            private BaseEntity GuardTarget { get; set; } = null;
        ...
        public class CustomScientistBrain : ScientistBrain
        {
            ...
            public new class CombatState : BasicAIState
            {
                ...
                public override StateStatus StateThink(float delta, BaseAIBrain brain, BaseEntity entity)
                {
                    ...
                    {
                        if (_npc.DistanceToTarget < _npc.CurrentWeapon.effectiveRange * 2f)
                        {
                            _npc.IsAttackingNow = true;
                            _npc.UseMeleeWeapon(); 
                            _npc.IsAttackingNow = false;
            ...
            public new class CombatStationaryState : BasicAIState
            {
                ...
                public override StateStatus StateThink(float delta, BaseAIBrain brain, BaseEntity entity)
                {
                    ...
                    else if (_npc.CurrentWeapon is FlameThrower && _npc.DistanceToTarget < _npc.CurrentWeapon.effectiveRange)
                    {
                        _npc.IsAttackingNow = true;
                        _npc.FireFlameThrower();
                        _npc.IsAttackingNow = false;
                    }
                    else if (_npc.CurrentWeapon is BaseMelee && _npc.DistanceToTarget < _npc.CurrentWeapon.effectiveRange * 2f)
                    {
                        _npc.IsAttackingNow = true;
                        _npc.UseMeleeWeapon();
                        _npc.IsAttackingNow = false;
        ...
        private object OnEntityTakeDamage(BaseCombatEntity victim, HitInfo info)
        {
            ...
            if (IsCustomScientist(victim))
            {
                ...
                if (attacker is BasePlayer)
                {
                    ...
                    if (attackerBp is NPCPlayer)
                    {
                        ...
                        {
                            if (victimNpc.CurrentTarget == null) victimNpc.CurrentTarget = attackerNpc;
                            return null;
                        }
                    }
                }
                ...
                if (attacker is BaseAnimalNPC)
                {
                    ...
                    {
                        if (victimNpc.CurrentTarget == null) victimNpc.CurrentTarget = attackerAnimal;
                        return null;
                    }
                }
            ...
            if (IsCustomScientist(attacker))
            {
                ...
                if (victim is BaseAnimalNPC && _config.CanTargetAnimal) return null;
        ...
        private void GenerateBiomePositions(int attempts)
        {
            ...
            for (int i = 0; i < attempts; i++)
            {
                ...
                if (IsRaycast(position, out raycastHit)) position.y = raycastHit.point.y;
                else continue;

                if (IsNavMesh(position, out navMeshHit)) position = navMeshHit.position;
                else continue;

                if (IsEntities(position, 10f)) continue;
            ...
            Puts($"List of biome positions: Arid = {BiomePoints["Arid"].Count}, Temperate = {BiomePoints["Temperate"].Count}, Tundra = {BiomePoints["Tundra"].Count}, Arctic = {BiomePoints["Arctic"].Count}");
        }

        private object GetSpawnPoint(string biome)
        {
            if (!BiomePoints.ContainsKey(biome)) return null;

            List<Vector3> positions = BiomePoints[biome];
            ...
            while (attempts > 0)
            {
                ...
                if (!IsEntities(position, 10f)) return position;

                positions.Remove(position);
                if (positions.Count == 0) GenerateBiomePositions(1000);
            }
