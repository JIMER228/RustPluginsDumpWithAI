...
/* 1.1.15
 * Fixed issue with DeleteBackpacks not deleting backpacks after game finishes.
 * Updated NightVision integration and removed permission handling.
 */

namespace Oxide.Plugins
{
    [Info("HungerGames", "imthenewguy", "1.1.15")]
    ...
    class HungerGames : RustPlugin
    {
        ...
        class PlayerEntity
        {
            ...
            public Dictionary<string, uint> ButtonIDs = new Dictionary<string, uint>();
            ...
            foreach (var pookie in BaseNetworkable.serverEntities)
            {
                ...
                foreach (var entity in BaseNetworkable.serverEntities)
                {
                    ...
                    if (dist <= 0.80)
                    {
                        ...
                        pcdData.ButtonIDs.Add("start", button.net.ID);
                        SaveData();
                        found++;
                    ...
                    if (dist >= 1.20 && dist <= 2.6)
                    {
                        ...
                        pcdData.ButtonIDs.Add("ele", button.net.ID);
                        SaveData();
                        found++;
        ...
        private void StoreButtonEntities()
        {
            foreach (var entity in BaseNetworkable.serverEntities)
            {
                ...
                if (button.net.ID == pcdData.ButtonIDs["start"])
                {
                    start_button = button;
                    continue;
                }
                if (button.net.ID == pcdData.ButtonIDs["ele"])
                {
        ...
        void CheckAndSetup()
        {
            ...
            call_ele_button.Press(default(BaseEntity.RPCMessage));
            if (pcdData.centre_points.Count == 0)
            {
                pcdData.centre_points.Add(pcdData.Top_Centre_Point);
        ...
        private void InitiateGame(int TimeOverride = 0, string profile = null)
        {
            ...
            Puts("InitiateGame 10");
            call_ele_button.Press(default(BaseEntity.RPCMessage));
            Puts("InitiateGame 11");
            pcdData.chosen_centre = pcdData.centre_points.GetRandom();
            Puts("InitiateGame 12");
            ...
            {
                Puts("InitiateGame recruit timer repeating");
                time++;
            ...
            Puts("InitiateGame 14");
            TimerData.LobbyTimer = timer.Once(waitTime + 1, () =>
            {
                Puts("lobby timer repeating");
            ...
            Puts("InitiateGame 15");
        }

        void KillAFKs()
        {
            List<BasePlayer> killList = Pool.GetList<BasePlayer>();
            foreach (var player in Participants)
            {
        ...
        private void StartElevators()
        {
            ...
            start_button.Press(default(BaseEntity.RPCMessage));
            timer.Once(20f, () =>
            {
                foreach (var player in Participants)
                {
        ...
        void DeleteBackpacks()
        {
            ...
            {
                List<BaseNetworkable> backpacks = Pool.GetList<BaseNetworkable>();
