            ...
            private void UpdateInventory()
            {
                ...
                if (Config.BeltItems.Count > 0)
                {
                    foreach (NpcBelt npcItem in Config.BeltItems)
                    {
                        ...
                        {
                            foreach (ItemDefinition itemDefinition in npcItem.Mods.Select(ItemManager.FindItemDefinition))
                                if (itemDefinition != null)
                                    item.contents.AddItem(itemDefinition, 1);
                        }
                        else item.Remove();
                    }
                }
