...
/* 1.0.10
 * Removed log debugs.
 * Fixed an issue where enhancing an item that's stacked, enhanced the whole stack.
 * Fixed an issue where the woodcutting luck notification setting would control multiple others.
 * Fixed an issue with all worn items being rolled with the regen perk.
 */

namespace Oxide.Plugins
{
    [Info("Item Perks", "imthenewguy", "1.0.10")]
    ...
    class ItemPerks : RustPlugin
    {
        ...
        List<KeyValuePair<Perk, float>> ParsePerks(IEnumerable<string> args)
        {
            ...
            foreach (var arg in args)
            {
                ...
                {
                    result.Add(new KeyValuePair<Perk, float>(PerksList.GetRandom(), 0));
                }
                Perk _perk;
                if (parsedEnums.TryGetValue(arg, out _perk)) result.Add(new KeyValuePair<Perk, float>(_perk, 0));
            }

            return result;
        }

        void GiveItemWithPerks(BasePlayer target, string shortname, List<KeyValuePair<Perk, float>> perks)
        {
