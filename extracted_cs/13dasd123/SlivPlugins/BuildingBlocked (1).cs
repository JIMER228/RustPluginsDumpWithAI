...
{
    [Info("BuildingBlocked", "RusPliginsSliv", "1.1.1")]
        ...
        public static class RefundHelper
        {
            ...
            public static void Refund(BasePlayer player, BaseEntity entity, float percent = 1)
            {
                ...
                if (block != null)
                {
                    try
                    {
                        if (block.currentGrade == null)
                            return;

                        ConstructionGrade grade = block.currentGrade;

                        var costToBuildProperty = grade.GetType().GetProperty("costToBuild");
                        if (costToBuildProperty != null)
                        {
                            var costToBuildValue = costToBuildProperty.GetValue(grade, null);
                            if (costToBuildValue is IEnumerable<object> costToBuild)
                            {
                                foreach (var item in costToBuild)
                                {
                                    var itemProperty = item.GetType().GetProperty("itemDef");
                                    var itemDef = (ItemDefinition)itemProperty.GetValue(item);
                                    var amountProperty = item.GetType().GetProperty("amount");
                                    var amount = (int)amountProperty.GetValue(item) * (Mathf.Approximately(percent, -1) ? 0.5f : percent);
                                    if (amount < 1) amount = 1;
                                    player.GiveItem(ItemManager.Create(itemDef, (int)amount, 0));
                                }
                            }
                        }
                        else
                        {
                            Console.WriteLine("Свойство costToBuild не найдено в классе ConstructionGrade");
                        }
                    }
                    catch (Exception ex)
                    {
