...
{
    [Info("HarborEvent", "RustPlugins.ru", "2.1.8")]
    internal class HarborEvent : RustPlugin
    {
        ...
        private void OnEntitySpawned(ScientistNPC entity)
        {
            if (entity != null && (entity.ShortPrefabName == "scientistnpc_cargo_turret_lr300" || entity.ShortPrefabName == "scientistnpc_cargo_turret_any" || entity.ShortPrefabName == "scientistnpc_cargo") && Vector3.Distance(entity.transform.position, _controller.transform.position) < _config.Radius)
                timer.In(1.5f, () => _controller.SpawnCargoScientist(entity));
        }
        ...
        private void OnEntitySpawned(LootContainer entity)
        {
            ...
            timer.In(1.5f, () => _controller.SpawnCargoCrate(entity));
        }
        ...
        private void Start()
        {
            ...
            timer.In(_config.PreStartTime, () =>
            {
                ...
                if (plugins.Exists("MonumentOwner")) MonumentOwner.Call("RemoveZone", FindMonumentInfo(_controller.transform.position, $"harbor_{_controller.Type}"));
        ...
        private void Finish()
        {
            ...
            if (plugins.Exists("MonumentOwner") && _controller != null) MonumentOwner.Call("CreateZone", FindMonumentInfo(_controller.transform.position, $"harbor_{_controller.Type}"));
        ...
        internal class ControllerHarborEvent : FacepunchBehaviour
        {
            ...
            internal PatrolHelicopter Helicopter;
            ...
            private IEnumerator SpawnEntities()
            {
                ...
                if (_ins._config.PveMode.Pve && _ins.plugins.Exists("PveMode"))
                {
                    ...
                    _ins.PveMode.Call("EventAddPveMode", _ins.Name, config, transform.position, _ins._config.Radius, crates, Scientists.Select(x => x.net.ID.Value), _ins._config.Bradley.IsBradley ? new HashSet<ulong> { Bradley.net.ID.Value } : new HashSet<ulong>(), new HashSet<ulong>(), new HashSet<ulong>(), null);
                }
            ...
            private void UpdateMarkerForPlayers()
            {
                ...
                if (_ins._config.AdditionalPoint.Enabled && !Door.IsExists() && !Bradley.IsExists() && Crates.Count + HackCrates.Count > 0)
                {
                    HashSet<Vector3> points = new HashSet<Vector3>();
        ...
        {
            if (_controller == null) return null;
            if (Vector3.Distance(cargo.transform.position, _controller.transform.position) < _config.Radius) return true;
            else return null;
        }
        #endregion BetterNpc
        ...
        {
            foreach (MonumentInfo monument in TerrainMeta.Path.Monuments) if (monument.name.Contains(name) && Vector3.Distance(pos, monument.transform.position) < 1f) return monument;
            return null;
        }
        #endregion Helpers
