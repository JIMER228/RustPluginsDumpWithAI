...
{
    [Info("SubmersiblePump", "ingorek", "1.0.5")]
    public class SubmersiblePump : RustPlugin
    {
        ...
        {
            LoadData();
            foreach (var pumpId in data.pumps)
            {
                NetworkableId pumpNetworkId = new NetworkableId(pumpId);
                WaterPump pumpEntity = BaseNetworkable.serverEntities.Find(pumpNetworkId) as WaterPump;
                if (pumpEntity != null)
        ...
        {
            if (data.pumps.Contains((uint)entity.net.ID.Value))
            {
        ...
        private void OnEntityBuilt(Planner plan, GameObject go)
        {
            ...
            NextTick(() => generator.Kill());
            pump.Spawn();
            data.pumps.Add((uint)pump.net.ID.Value);
        ...
        private class PluginData
        {
            ...
            public List<uint> pumps = new List<uint>();
        }
