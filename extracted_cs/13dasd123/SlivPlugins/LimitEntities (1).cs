...
{
    [Info("Limit Entities", "MON@H", "2.1.2")]
    ...
    public class LimitEntities : RustPlugin
    {
        ...
        public class Cache
        {
            ...
            public readonly Hash<ulong, ulong> Reskin = new();
        ...
        public class PlayerData
        {
            ...
            {
                return Perms == null || Perms.LimitsGlobal.LimitTotal != 0;
            }
            ...
            public bool IsGlobalLimit(uint prefabId)
            {
                ...
                Hash<uint, int> cache = Perms.LimitsGlobal.LimitEntitiesCache;
                if (!cache.ContainsKey(prefabId))
                {
                    return false;
                }

                return Entities.PlayersEntities[prefabId] >= cache[prefabId];
            }
            ...
            public bool IsBuildingLimit(BuildingEntities entities, uint prefabId)
            {
                ...
                Hash<uint, int> cache = Perms.LimitsBuilding.LimitEntitiesCache;
                if (!cache.ContainsKey(prefabId))
                {
                    return false;
                }

                return entities.EntitiesCount[prefabId] >= cache[prefabId];
            }
            ...
            public float GetGlobalPercentage(uint prefabId)
            {
                ...
                return (float)Entities.PlayersEntities[prefabId] / cache[prefabId] * 100;
            }
            ...
            public float GetBuildingPercentage(BuildingEntities entities, uint prefabId)
            {
                ...
                return (float)entities.EntitiesCount[prefabId] / cache[prefabId] * 100;
            }
        }
        ...
        private void OnServerInitialized()
        {
            ...
            {
                GetPlayerCache(activePlayer.userID);
            }
        ...
        public class PluginConfig
        {
            ...
            [JsonProperty(PropertyName = "Log Level (Debug, Info, Warning, Off)", Order = 4)]
        ...
        public void DataLoad()
        {
            ...
            if (_storedData.BuildingsOwners.Count > 0)
            {
                ...
                List<uint> buildingsToRemove = Pool.GetList<uint>();
                ...
                Pool.FreeList(ref buildingsToRemove);
        ...
        private readonly Dictionary<string, string> _langUK = new()
        {
            ...
            [LangKeys.Error.LimitBuilding.EntityReached] = "Ви досягли ліміту <color=#FFA500>{0}</color> із <color=#FFA500>{1}</color> для <color=#FFA500>{2}</color> в цьому будинку",
        ...
        private void CmdLimitEntities(BasePlayer player, string cmd, string[] args)
        {
            ...
            PlayerSendMessage(player, GetPlayerLimit(player, target));
        }
        ...
        private void OnPlayerConnected(BasePlayer player) => GetPlayerCache(player.userID);
        ...
        private void OnEntitySpawned(BaseEntity entity)
        {
            ...
            PlayerData cache = GetPlayerCache(ownerID);
            cache.AddEntity(entity);

            if (_pluginConfig.LoggingLevel >= LogLevel.Debug)
            {
                PlayerEntities entities = cache.Entities;
            ...
            if ((_pluginConfig.ChatNotificationsEnabled || _pluginConfig.GameTipNotificationsEnabled)
                ...
                && !permission.UserHasPermission(entity.OwnerID.ToString(), PermissionImmunity))
            {
                HandleEntityNotification(BasePlayer.FindByID(entity.OwnerID), cache, _cache.Buildings[buildingID], prefabID);
            }
        }
        ...
        private void OnEntityReskin(BaseEntity baseEntity, ItemSkinDirectory.Skin skin, BasePlayer player)
        {
            ...
            _cache.Reskin[player.userID] = baseEntity.OwnerID;
        }

        private void OnEntityReskinned(BaseEntity baseEntity, ItemSkinDirectory.Skin skin, BasePlayer player)
        {
            ulong ownerID = _cache.Reskin[player.userID];
            if (!baseEntity.OwnerID.IsSteamId() && ownerID.IsSteamId())
            {
                baseEntity.OwnerID = ownerID;
                OnEntitySpawned(baseEntity);
            }

            _cache.Reskin.Remove(player.userID);
        }
        ...
        public void CacheGroupIds()
        {
            ...
            foreach (EntityGroup entityGroup in _pluginConfig.EntityGroups)
            {
                ...
                {
                    PrintError($"You have 0 valid entities in {entityGroup.Name} group in your config file! To get a list of all supported prefabs use console command limitentities.list");
                    continue;
                }
        ...
        public void CachePrefabIds()
        {
            ...
            List<string> groupNames = Pool.GetList<string>();
            List<string> groupPrefabs = Pool.GetList<string>();
            List<string> prefabWrong = Pool.GetList<string>();
            ...
            foreach (EntityGroup entityGroup in _pluginConfig.EntityGroups)
            {
                ...
                {
                    //PrintError($"Group names must be unique! '{entityGroup.Name}'");
                    continue;
                }
            ...
            foreach (PermissionEntry entry in _pluginConfig.Permissions)
            {
                foreach (string entityPrefab in entry.LimitsBuilding.LimitsEntities.Keys)
                {
                    ...
                    {
                        PrintError($"You can't use the same prefabs in groups and individually. Choose one and remove it from the other '{entityPrefab}'");
                        continue;
                    }
                    ...
                    uint prefabID = StringPool.Get(entityPrefab);
            ...
            {
                PrintError($"You have {prefabWrong.Count} untracked prefabs in your config file! To get a list of all supported prefabs use console command limitentities.list\n{string.Join("\n", prefabWrong)}");
            }

            Pool.FreeList(ref groupNames);
            Pool.FreeList(ref groupPrefabs);
            Pool.FreeList(ref prefabWrong);
        ...
        public void CacheEntities()
        {
            ...
            foreach (BaseEntity entity in BaseEntity.saveList)
            {
                ...
                GetPlayerCache(ownerID).AddEntity(entity);
            }
        ...
        public PlayerData GetPlayerCache(ulong playerId)
        {
            PlayerData cache = _cache.PlayerData[playerId];
            if (cache == null)
            {
                cache = new(playerId);
                _cache.PlayerData[playerId] = cache;
            }
        ...
        public BuildingEntities GetBuildingCache(uint buildingId)
        {
            BuildingEntities cache = _cache.Buildings[buildingId];
            if (cache == null)
            {
                cache = new();
                _cache.Buildings[buildingId] = cache;
            }
        ...
        public object HandleCanBuild(BasePlayer player, Construction entity, Construction.Target target)
        {
            if (!player.IsValid()
            || !player.userID.IsSteamId()
            || permission.UserHasPermission(player.UserIDString, PermissionImmunity)
            || !_cache.Prefabs.Tracked.Contains(entity.prefabID))
            {
                return null;
            }

            ulong playerId = player.userID;
            PlayerData cache = GetPlayerCache(playerId);
            if (cache.Perms == null)
            {
                return null;
            }
            ...
            Vector3 position = target.entity.IsValid() && target.socket ? target.GetWorldPosition() : target.position;
            uint prefabID = GetPrefabID(entity.prefabID);
            if (!cache.CanBuild())
            {
                Log($"{position} {playerId} prevented from building entity {GetShortName(prefabID)} cannot build", LogLevel.Debug);
                HandleNotification(player, Lang(LangKeys.Error.EntityIsNotAllowed, player.UserIDString, GetItemDisplayName(prefabID, player.UserIDString)), true);
                return _static.False;
            }

            if (cache.IsGlobalLimit())
            {
                Log($"{position} {playerId} prevented from building entity {GetShortName(prefabID)} global limit", LogLevel.Debug);
                HandleNotification(player, Lang(LangKeys.Error.LimitGlobal.Reached, player.UserIDString, cache.Entities.TotalCount, cache.Perms.LimitsGlobal.LimitTotal), true);
                return _static.False;
            }

            if (cache.IsGlobalLimit(prefabID))
            {
                Log($"{position} {playerId} prevented from building entity {GetShortName(prefabID)} global entity limit", LogLevel.Debug);
                HandleNotification(player, Lang(LangKeys.Error.LimitGlobal.EntityReached, player.UserIDString, entities.PlayersEntities[prefabID], cache.Perms.LimitsGlobal.LimitEntitiesCache[prefabID], GetItemDisplayName(prefabID, player.UserIDString)), true);
                return _static.False;
            }
            ...
            if (building != null)
            {
                if (cache.IsBuildingLimit(building))
                {
                    Log($"{position} {playerId} prevented from building entity {GetShortName(prefabID)} in building {buildingID}", LogLevel.Debug);
                    HandleNotification(player, Lang(LangKeys.Error.LimitBuilding.Reached, player.UserIDString, building.EntitiesIds.Count, cache.Perms.LimitsBuilding.LimitTotal), true);
                    return _static.False;
                }

                if (cache.IsBuildingLimit(building, prefabID))
                {
                    Log($"{position} {playerId} prevented from building entity {GetShortName(prefabID)} in building {buildingID}", LogLevel.Debug);
                    HandleNotification(player, Lang(LangKeys.Error.LimitBuilding.EntityReached, player.UserIDString, building.EntitiesCount[prefabID], cache.Perms.LimitsBuilding.LimitEntitiesCache[prefabID], GetItemDisplayName(prefabID, player.UserIDString)), true);
                    return _static.False;
                }
            }
        ...
        public void HandleBuildingСhange(uint oldId, uint newId, bool split)
        {
            ...
            List<BaseEntity> movedEntites = Pool.GetList<BaseEntity>();
            ...
            Pool.FreeList(ref movedEntites);
        }
        ...
        public uint AddBuildingEntity(BaseEntity entity)
        {
            ...
            BuildingEntities cache = GetBuildingCache(buildingId);
            cache.AddEntity(entity);
            ...
            if (_pluginConfig.LoggingLevel >= LogLevel.Debug)
            {
                Vector3 position = entity.transform.position;
                uint prefabID = GetPrefabID(entity.prefabID);
                Log($"{position} {entity.OwnerID} Increased building {buildingId} entity {GetShortName(prefabID)} count {cache.EntitiesCount[prefabID]}", LogLevel.Debug);
                Log($"{position} {entity.OwnerID} Increased building {buildingId} total entities count {cache.EntitiesIds.Count}", LogLevel.Debug);
            }
        ...
        public uint RemoveBuildingEntity(BaseEntity entity)
        {
            ...
            if (_pluginConfig.LoggingLevel >= LogLevel.Debug)
            {
                Vector3 position = entity.transform.position;
                uint prefabID = GetPrefabID(entity.prefabID);
                Log($"{position} {entity.OwnerID} Reduced building {buildingId} entities {GetShortName(prefabID)} count {cache.EntitiesCount[prefabID]}", LogLevel.Debug);
                Log($"{position} {entity.OwnerID} Reduced building {buildingId} total entities count {cache.EntitiesIds.Count}", LogLevel.Debug);
            }
        ...
        public void HandleEntityNotification(BasePlayer player, PlayerData cache, BuildingEntities building, uint prefabID)
        {
            ...
            if (building != null && cache.GetBuildingPercentage(building, prefabID) >= _pluginConfig.WarnPercent)
            {
                HandleNotification(player, Lang(LangKeys.Info.LimitBuildingEntity, player.UserIDString, building.EntitiesCount[prefabID], cache.Perms.LimitsBuilding.LimitEntitiesCache[prefabID], GetItemDisplayName(prefabID, player.UserIDString)));
            }
            else if (building != null && cache.GetBuildingPercentage(building) >= _pluginConfig.WarnPercent)
            {
                HandleNotification(player, Lang(LangKeys.Info.LimitBuilding, player.UserIDString, building.EntitiesIds.Count, cache.Perms.LimitsBuilding.LimitTotal));
            }
            else if (cache.GetGlobalPercentage(prefabID) >= _pluginConfig.WarnPercent)
            {
                HandleNotification(player, Lang(LangKeys.Info.LimitGlobalEntity, player.UserIDString, cache.Entities.PlayersEntities[prefabID], cache.Perms.LimitsGlobal.LimitEntitiesCache[prefabID], GetItemDisplayName(prefabID, player.UserIDString)));
            }
            else if (cache.GetGlobalPercentage() >= _pluginConfig.WarnPercent)
            {
                HandleNotification(player, Lang(LangKeys.Info.LimitGlobal, player.UserIDString, cache.Entities.TotalCount, cache.Perms.LimitsGlobal.LimitTotal));
            }
        }

        public string GetPlayerLimit(BasePlayer player, BasePlayer target)
        {
            if (permission.UserHasPermission(target.UserIDString, PermissionImmunity))
            {
                return Lang(LangKeys.Info.Unlimited, player.UserIDString);
            }

            PlayerData cache = _cache.PlayerData[target.userID];
            if (cache?.Perms == null)
            {
                return Lang(LangKeys.Info.Unlimited, player.UserIDString);
            }
            ...
            PlayerEntities entities = cache.Entities;
            PermissionEntry perms = cache.Perms;
        ...
        public void HooksUnsubscribe()
        {
            ...
            Unsubscribe(nameof(OnEntityReskin));
            Unsubscribe(nameof(OnEntityReskinned));
        ...
        public void HooksSubscribe()
        {
            ...
            Subscribe(nameof(OnEntityReskin));
            Subscribe(nameof(OnEntityReskinned));
        ...
        public void RegisterPermissions()
        {
            ...
            List<PermissionEntry> entries = Pool.GetList<PermissionEntry>();
            ...
            foreach (PermissionEntry entry in _pluginConfig.Permissions)
            {
                ...
                {
                    PrintError("You have empty 'Permission' in config file! Skipped.");
                    continue;
                }
        ...
        public void Log(string text, LogLevel level)
        {
            ...
            {
                LogToFile("log", $"{DateTime.Now:HH:mm:ss} {StripRustTags(text)}", this);
            }
        }
        ...
        public string GetShortName(uint prefabId)
        {
            ...
            if (string.IsNullOrWhiteSpace(name))
            {
                ...
                if (string.IsNullOrWhiteSpace(name))
                {
                    PrintWarning($"The string for {prefabId} was not found in StringPool");
                    return string.Empty;
                }
                ...
                if (string.IsNullOrWhiteSpace(itemDisplayName))
                {
                    ...
                    if (string.IsNullOrWhiteSpace(itemDisplayName))
                    {
                        ...
                        if (string.IsNullOrWhiteSpace(itemDisplayName))
                        {
                            ...
                            if (string.IsNullOrWhiteSpace(itemDisplayName))
                            {
                                PrintWarning($"There is no translation for shortname {itemShortName} found!");
