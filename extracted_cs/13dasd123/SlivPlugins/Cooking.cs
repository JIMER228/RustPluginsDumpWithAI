...
/* 2.0.9 Change Log
 * Added a few missing language translations for ingredient drops.
 * Added a config option to set market quantities to 0 on a new wipe.
 * Added new command: setcookingmarketstock <amount> - sets the global ingredient stock to the specified value.
 * Added new command: setcookingmarketstockfor <ingredient> <amount> - sets the stock for ingredient to the specified value.
 * Added option to class: allowDefaultEffects. This will ignore the config setting.
 * Added buffs: Hydration and Hunger. These will be banes that can be added to meals for balance.
 * Fixed a potential exception error under HandleDispenserDrops method.
 * Added a redundancy for the quantity input field under recipes. Should prevent exceptions when letters are entered.
 * Added option for meals and/or ingredients to be added to loot crates.
*/

namespace Oxide.Plugins
{
	[Info("Cooking 2.0", "imthenewguy", "2.0.9")]
	...
	class Cooking : RustPlugin
	{
        ...
        public class GeneralSettings
        {
            ...
            [JsonProperty("Drop sources")]
            public Dictionary<GatherSource, float> dropSources = new Dictionary<GatherSource, float>();
        ...
        public class MarketSettings
        {
            ...
            [JsonProperty("Reset market stock to 0 on wipe?")]
            public bool reset_market_on_wipe = true;

            public bool allow_sell = true;
            public bool allow_buy = true;
        ...
        public class FavouritesMenuSettings
        {
            ...
            public string recipe_title_panel_colour = "0.2712 0.3956 0.4528 1";
        ...
        Dictionary<string, MealInfo> DefaultBasicMeals
        {
            get
            {
                return new Dictionary<string, MealInfo>()
                {
                    ...
                    ["bread"] = new MealInfo(true, "apple", 2783707218, "A useful cooking ingredient.", 0, 10, new Dictionary<Buff, float>() { [Buff.Ingredient] = 0 }, new Dictionary<string, int>()
                    {
                        ["flour"] = 3
                    }),
        ...
        Dictionary<string, IngredientsInfo> DefaultBasicIngredients
        {
            get
            {
                ...
                {
                    ["flour"] = new IngredientsInfo(true, "apple", 2572180263, new List<GatherSource>() { GatherSource.Potato, GatherSource.Corn }),
                    ...
                    ["bread"] = new IngredientsInfo(true, "apple", 2783707218, new List<GatherSource>() { GatherSource.Crafted }),
                    ["milk"] = new IngredientsInfo(true, "apple", 2572181796, new List<GatherSource>() { GatherSource.Bear, GatherSource.Boar, GatherSource.Deer, GatherSource.Wolf }),
                    ...
                    ["egg"] = new IngredientsInfo(true, "apple", 2572180411, new List<GatherSource>() { GatherSource.Chicken }),
        ...
        // 1.0 = always drops.
        Dictionary<GatherSource, float> DefaultGatherSources()
        {
            List<GatherSource> sources = Pool.GetList<GatherSource>();
            sources.AddRange(Enum.GetValues(typeof(Buff)).Cast<GatherSource>());

            Dictionary<GatherSource, float> result = new Dictionary<GatherSource, float>();
            ...
            foreach (var source in sources)
            {
                ...
                switch (source)
                {
                    ...
                    case GatherSource.Fishing:
                        result.Add(source, 0.1f);
                        break;

                    case GatherSource.Mushroom:
                        result.Add(source, 0.075f);
                        break;

                    default: 
                        result.Add(source, 0.05f);
                        break;
                }
            }
            Pool.FreeList(ref sources);
        ...
        protected override void LoadConfig()
        {
            ...
            try
            {
                ...
                else Puts($"Config is null. Loading default config.");
                SaveConfig();
            }
            catch (Exception ex)
            {
        ...
        {
            if (config.mealSettings.meals.Count == 0 || config.ingredientSettings.ingredients.Count == 0) UpdateOldConfig();
        ...
        [PluginReference]
        private Plugin ServerRewards, Economics, RandomTrader, SkillTree;
        ...
        void Unload()
        {
            ...
            // Handle clearing dictionaries

            if (MealCooldowns != null)
            {
        ...
        class BagInfo
        {
            ...
            public int amount;
            public int slot;
            public BagInfo(string shortname, string displayName, int amount, int slot, ulong skin)
            {
                this.shortname = shortname;
                this.displayName = displayName;
                this.amount = amount;
        ...
        {
            List<GatherSource> gatherSource = Pool.GetList<GatherSource>();
            ...
            Pool.FreeList(ref gatherSource);
        }
        ...
        protected override void LoadDefaultMessages()
        {
            ...
            Dictionary<string, string> langEntries = new Dictionary<string, string>()
            {
                ...
                //["_MenuDescription"] = "<color=#42f105>{0}%</color>.",
                ...
                ["EatMealMessage"] = "You eat the delicious <color=#DFF008>{0}</color> and receive the following buff: {1}",
        ...
        List<StorageContainer> IBagContainers = new List<StorageContainer>();
        ...
        void OnLootEntity(BasePlayer player, StorageContainer container)
        {
            ...
            if (lootContainer != null)
            {
                ...
                if (IsFoodBox(container)) HandleIngredientChance(player, GatherSource.Foodbox, false, null, container.net.ID.Value);
                ...
                if (config.mealSettings.lootSettings.enable)
                {
                    ...
                    {
                        List<KeyValuePair<string, MealInfo>> meals = Pool.GetList<KeyValuePair<string, MealInfo>>();
                        ...
                        foreach (var meal in meals)
                        {
                            ...
                            var reward = CreateItem(meal.Value.shortname, UnityEngine.Random.Range(config.mealSettings.lootSettings.min, config.mealSettings.lootSettings.max + 1), meal.Value.skin, meal.Value.skin > 0 ? meal.Key : null);
                ...
                if (config.ingredientSettings.lootSettings.enable)
                {
                    ...
                    {
                        List<KeyValuePair<string, IngredientsInfo>> ingredients = Pool.GetList<KeyValuePair<string, IngredientsInfo>>();
                        ...
                        foreach (var ingredient in ingredients)
                        {
                            ...
                            if (roll > count) continue;
                            var reward = CreateItem(ingredient.Value.shortname, UnityEngine.Random.Range(config.ingredientSettings.lootSettings.min, config.ingredientSettings.lootSettings.max + 1), ingredient.Value.skin, ingredient.Value.skin > 0 ? ingredient.Key : null);
                            container.inventorySlots++;
                            container.inventory.capacity++;
                ...
                if (config.mealSettings.cookbookSettings.enabled && config.mealSettings.cookbookSettings.recipeDropChance.TryGetValue(container.ShortPrefabName, out value))
                {
                    ...
                    if (roll >= 100 - value) RollForRecipe(player, lootContainer);
                }                
            }
        }

        public Item CreateItem(string shortname, int quantity, ulong skin, string displayName)
        {
            var item = ItemManager.CreateByName(shortname, quantity, skin);
            if (displayName != null) item.name = displayName;
            return item;
        }

        bool IsFoodBox(StorageContainer container)
        {
        ...
        void OnServerInitialized(bool initial)
        {
            ...
            foreach (var recipe in config.mealSettings.meals)
            {
                ...
                Puts($"{recipe.Key} cooldown: {recipe.Value.useCooldown}");
                switch (recipe.Key)
                {
                    case "chicken burger": 
                        if (recipe.Value.buffs.Count == 1 && recipe.Value.buffs.ElementAt(0).Key == Buff.Animal_Resist)
                        {
                            recipe.Value.buffs.Clear();
                            foreach (var buff in DefaultBasicMeals["chicken burger"].buffs)
                                recipe.Value.buffs.Add(buff.Key, buff.Value);

                            Puts($"Updated the buff for chicken burger.");
                            updateConfig = true;
                        }
                        break;
                    case "bear stew":
                        ...
                        {
                            recipe.Value.buffs.Clear();
                            foreach (var buff in DefaultBasicMeals["bear stew"].buffs)
                                recipe.Value.buffs.Add(buff.Key, buff.Value);

                            Puts($"Updated the buff for bear stew.");
                            updateConfig = true;
                        }
                        break;
                }
                ...
                {
                    List<string> mealsToDelete = Pool.GetList<string>();
                    ...
                    Pool.FreeList(ref mealsToDelete);
                }
                ...
                List<string> ingredients_to_remove = Pool.GetList<string>();
                ...
                Pool.FreeList(ref ingredients_to_remove);
            }
            ...
            foreach (var source in DefaultGatherSources())
            {
                ...
                {
                    Puts($"Added drop source: {source.Key} [{source.Value * 100}%]");
        ...
        void CacheRecipes(BasePlayer player)
        {
            ...
            playerData.favouriteDishes.RemoveAll(x => !playerData.learntRecipes.Contains(x));            
        ...
        void AddCachedRecipe(BasePlayer player, string recipe)
        {
            ...
            if (!CachedRecipes.TryGetValue(player.userID, out mealData)) CachedRecipes.Add(player.userID, mealData = new Dictionary<string, MealInfo>());
        ...
        object OnPlayerAddModifiers(BasePlayer player, Item item, ItemModConsumable consumable)
        {
            ...
            string chatBuffDescription = string.Format(lang.GetMessage("EatMealMessage", this, player.UserIDString), item.name.TitleCase(), mealData.buffs.Count > 1 ? lang.GetMessage("S", this, player.UserIDString) : null);
        ...
        void OnFishCatch(Item item, BaseFishingRod rod, BasePlayer player)
        {
            ...
            if (buffManager.GetBuffModifiers.TryGetValue(Buff.Fishing_Luck, out value))
            {
                ...
                var newItem = ItemManager.CreateByName(randItem, UnityEngine.Random.Range(config.generalSettings.buffSettings.fishingLuckSettings.min, config.generalSettings.buffSettings.fishingLuckSettings.max + 1));
        ...
        {
            List<string> defs = Pool.GetList<string>();
            defs.AddRange(ItemDefs.Keys);
            foreach (var entry in blacklist)
                defs.Remove(entry);

            string randomItem = defs.GetRandom();
            Pool.FreeList(ref defs);
            return randomItem;
        }
        ...
        void HandleBagDrop(BasePlayer player)
        {
            ...
            if (container.inventory.itemList?.Count > 0)
            {
                timer.Once(0.1f, () =>
                {
                    ...
                    IBagContainers.Remove(container);
                    container.Invoke(container.KillMessage, 0.01f);
                    bagData.Clear();
                });
            }
        ...
        {
            List<string> remove = Pool.GetList<string>();
        ...
        void OnEntityDeath(BaseCombatEntity entity, HitInfo info)
        {
            ...
            switch (entity.ShortPrefabName)
            {
                ...
                case "loot_barrel_2":
                    float recipeChance;
                    if (config.mealSettings.cookbookSettings.enabled && permission.UserHasPermission(player.UserIDString, perm_recipecards) && config.mealSettings.cookbookSettings.recipeDropChance.TryGetValue(entity.ShortPrefabName, out recipeChance) && (recipeChance >= 100 - UnityEngine.Random.Range(0, 100f))) RollForRecipe(player, entity as LootContainer);
            ...
            if (victim != null)
            {
                ...
                if (buffManager_victim != null)
                {
                    ...
                    if (!AttackerIsRealPlayer)
                    {
                        ...
                        var animal = info.Initiator as BaseAnimalNPC;
                        if (animal != null)
                        {
        ...
        void OnNewSave(string filename)
        {
            ...
            if (config.generalSettings.marketSettings.reset_market_on_wipe)
            {
                foreach (var kvp in config.ingredientSettings.ingredients)
                {
                    kvp.Value.marketInfo.availableStock = 0;
                }
            }
        }
        ...
        void OnPlayerHealthChange(BasePlayer player, float oldValue, float newValue)
        {
            ...
            Pool.FreeList(ref surroundingPlayers);
        }
        ...
        {
            if (permission.GroupHasPermission(groupName, perm_recipemenu_chat) || permission.GroupHasPermission(groupName, perm_bag_cmd))
        ...
        void CheckButtonUpdate(string id, string permName)
        {
            ...
            if (permName.Equals(perm_recipemenu_chat, StringComparison.OrdinalIgnoreCase) || permName.Equals(perm_bag_cmd, StringComparison.OrdinalIgnoreCase))
                CreatButtons(player);
        }
        ...
        void RollForRecipe(BasePlayer player, LootContainer container)
        {
            ...
            List<string> learnable = Pool.GetList<string>();
            
            foreach (var meal in config.mealSettings.meals)
            {
                if (!meal.Value.enabled) continue;
                if (!config.mealSettings.cookbookSettings.allowFindDuplicates && playerData.learntRecipes.Contains(meal.Key)) continue;
                learnable.Add(meal.Key);
            }
            
            if (learnable.Count == 0)
            {
                Pool.FreeList(ref learnable);
                return;
            }
            
            recipe = learnable.GetRandom();
            Pool.FreeList(ref learnable);
            ...
            if (!string.IsNullOrEmpty(config.mealSettings.cookbookSettings.blueprint.displayName)) item.name = string.Format(config.mealSettings.cookbookSettings.blueprint.displayName, recipe);
            item.text = recipe;

            if (container.inventory.itemList != null && container.inventory.itemList.Count >= 6)
            {
                player.GiveItem(item);
                if (TriggerNotificationsEnabled(player.UserIDString)) PrintToChat(player, String.Format(lang.GetMessage("FoundNewRecipe", this, player.UserIDString), lang.GetMessage(recipe, this, player.UserIDString)) + lang.GetMessage("FoundNewRecipeAppendContainerFull", this, player.UserIDString));
                return;
            }
            ...
            if (TriggerNotificationsEnabled(player.UserIDString)) PrintToChat(player, String.Format(lang.GetMessage("FoundNewRecipe", this, player.UserIDString), lang.GetMessage(recipe, this, player.UserIDString)));
        }
        ...
        public class HorseInfo
        {
            ...
            public HorseInfo(float current_maxSpeed, float current_runSpeed, float current_walkSpeed, float current_trotSpeed, float current_turnSpeed, BasePlayer player, float modifier)
            {
        ...
        void ModifyHorse(RidableHorse horse, BasePlayer player, float modifier)
        {
            ...
            if (ModifiedHorses.ContainsKey(horse)) RestoreModifiedHorse(horse);
            ModifiedHorses.Add(horse, new HorseInfo(horse.maxSpeed, horse.runSpeed, horse.walkSpeed, horse.trotSpeed, horse.turnSpeed, player, modifier));

            horse.maxSpeed += horse.maxSpeed * modifier;
            horse.runSpeed += horse.runSpeed * modifier;
            horse.walkSpeed += horse.walkSpeed * modifier;
            horse.trotSpeed += horse.trotSpeed * modifier;
            horse.turnSpeed += horse.turnSpeed * modifier;
        }
        ...
        void RestoreModifiedHorse(RidableHorse horse, bool removeFromList = true)
        {
            ...
            horse.maxSpeed = restoreData.current_runSpeed;
            horse.runSpeed = restoreData.current_runSpeed;
            horse.walkSpeed = restoreData.current_walkSpeed;
            horse.trotSpeed = restoreData.current_trotSpeed;
            horse.turnSpeed = restoreData.current_turnSpeed;
        ...
        void HandleIngredientChance(BasePlayer player, GatherSource source, bool player_grown, BuffManager buffManager = null, ulong sourceEntityID = 0, float modifier = 1)
        {
            ...
            float baseLuck;
            if (!config.generalSettings.dropSources.TryGetValue(source, out baseLuck)) return;
            ...
            if (ingredientData.skin > 0) item.name = drop;
            if (config.generalSettings.ingredientBagSettings.gather_directly_into_bag && IsValidItemForBag(item) && HasIngredientBag(player)) PlaceIngredientIntoBag(player, item);
            else
            {
        ...
        string GetDrop(List<string> drops)
        {
            ...
            List<string> errors = Pool.GetList<string>();
        ...
        void GiveMeal(BasePlayer player, string shortname, ulong skin, string displayName, int amount = 1)
        {
            var item = ItemManager.CreateByName(shortname, amount, skin);
            if (skin > 0) item.name = displayName;
            player.GiveItem(item);
        }

        void PayForMeal(BasePlayer player, Dictionary<string, int> ingredients)
        {
            ...
            foreach (var ingredient in ingredients)
            {
                ...
                if (bagData != null)
                {
                    List<BagInfo> entries_to_delete = Pool.GetList<BagInfo>();
                    ...
                    Pool.FreeList(ref entries_to_delete);
                }

                if (found >= ingredient.Value) continue;
        ...
        bool PlaceIngredientIntoBag(BasePlayer player, Item item)
        {
            ...
            bagData.Add(new BagInfo(item.info.shortname, item.name ?? item.info.displayName.english.ToLower(), item.amount, -1, item.skin));
        ...
        void SaveIngredientBag(BasePlayer player, StorageContainer container)
        {
            ...
            List<Item> items_to_return = Pool.GetList<Item>();
            foreach (var item in container.inventory.itemList)
            {
                ...
                if (!foundExisting) bagData.Add(new BagInfo(item.info.shortname, item.name ?? item.info.displayName.english.ToLower(), item.amount, item.position, item.skin));
            }

            foreach (var item in items_to_return)
            {
                PrintToChat(player, string.Format(lang.GetMessage("ItemReturnedBlackListed", this, player.UserIDString), item.amount, item.amount, item.name != null ? lang.GetMessage(item.name, this, player.UserIDString) : item.info.displayName.english, config.generalSettings.ingredientBagSettings.bagItemSettings.displayName ?? "ingredient bag"));
                player.GiveItem(item);
            }
            Pool.FreeList(ref items_to_return);
            bagData.RemoveAll(x => x.amount == 0);
        }

        void CloseIngredientBag(BasePlayer player, StorageContainer container)
        {
            if (!IBagContainers.Contains(container)) return;
        ...
        {
            foreach (var ibag in IBagContainers)
                if (ibag.OwnerID == player.userID)
                    return true;
        ...
        StorageContainer GetBagContainer(BasePlayer player)
        {
            ...
            try
            {
                ...
                count = 7;
                List<BagInfo> invalid = Pool.GetList<BagInfo>();
                List<BagInfo> slotless = Pool.GetList<BagInfo>();
                List<Item> failMoveSlot = Pool.GetList<Item>();
                foreach (var slotData in bagData)
                {
                ...
                Pool.FreeList(ref invalid);
                Pool.FreeList(ref slotless);
                Pool.FreeList(ref failMoveSlot);
                count = 10;
                return container;
            }
            catch (Exception e)
            {
                LogToFile("GetBagContainer_Error", $"Failed at {count}. Error: {e?.Message}.", this, true, true);
            }
            return null;
        }
        ...
        void RemoveAllMarketNPCs(BasePlayer player)
        {
            ...
            List<NPCTalking> marketNPCs = Pool.GetList<NPCTalking>();
            marketNPCs.AddRange(MarketNPCs);
            foreach (var npc in marketNPCs)
            {
                if (!RemoveNPC(npc)) PrintToChat(player, $"Failed to remove {npc.displayName}[{npc.transform.position}]");
            }
            Pool.FreeList(ref marketNPCs);
        }
        ...
        void RemoveNPCCMD(BasePlayer player)
        {
            ...
            var npc = GetTargetEntity(player) as NPCTalking;
            if (npc == null)
            {
        ...
        NPCTalking IsNPCSpawned(Vector3 pos)
        {
            var npcs = FindEntitiesOfType<NPCTalking>(pos, 5);
            NPCTalking result = null;
        ...
        {
            var npc = GameManager.server.CreateEntity("assets/prefabs/npc/bandit/missionproviders/missionprovider_bandit_a.prefab", pos, rot) as NPCTalking;
        ...
        public class BuffManager : MonoBehaviour
        {
            ...
            private List<BasePlayer> comfortedPlayers = Pool.GetList<BasePlayer>();
            ...
            public string AddBuff(string meal, Dictionary<Buff, float> buffs, float time, string shortname, ulong skin, Dictionary<string, string> commandsOnConsume, List<string> commandsOnEnd)
            {
                ...
                foreach (var buff in buffs)
                {
                    ...
                    if (buff.Key == Buff.Comfort && !hasComfort)
                    {
                        AddComfort();
                        hasComfort = true;
                        //Interface.Oxide.LogInfo($"Found comfort: {hasComfort}");
                    }
            ...
            private void OnDestroy()
            {
                ...
                {
                    List<string> meals = Pool.GetList<string>();
                ...
                RemoveComfort();
                RemoveMetabolismOverload();
                Pool.FreeList(ref comfortedPlayers);
        ...
        public class CookingManager : MonoBehaviour
        {
            ...
            public class CookingInfo
            {
                ...
                public ulong skinID;
                public string shortname;
                public Dictionary<string, int> ingredientsPerMeal = new Dictionary<string, int>();

                public CookingInfo(string shortname, string displayName, ulong skinID, float cookTime, Dictionary<string, int> ingredientsPerMeal)
                {
                    this.shortname = shortname;
                    this.displayName = displayName;
                    this.skinID = skinID;
            ...
            void CheckCookingProgress()
            {
                ...
                if (Time.time > meal.finishCookingAt)
                {
                    ...
                    Interface.CallHook("OnMealCrafed", player, meal.displayName, meal.ingredientsPerMeal, Instance.config.ingredientSettings.ingredients.ContainsKey(meal.displayName));
                }
                else
                {
            ...
            public void AddMeal(string shortname, string displayName, ulong skin, float cookTime, Dictionary<string, int> ingredientsPerMeal)
            {
                var queMeal = new CookingInfo(shortname, displayName, skin, cookTime, ingredientsPerMeal);
            ...
            private class IngredientsInfo
            {
                ...
                public IngredientsInfo(string shortname, ulong skin, int amount)
                {
                    this.shortname = shortname;
                    this.skin = skin;
                    this.amount = amount;
            ...
            public void CancelCurrentCraft()
            {
                ...
                foreach (var ingredient in meal.ingredientsPerMeal)
                {
                    ...
                    var item = ItemManager.CreateByName(ingredientProfile.shortname, ingredient.Value, ingredientProfile.skin);
                    item.name = ingredient.Key;
                ...
                foreach (var craft in queuedMeals)
                {
                    ...
                    foreach (var ingredient in craft.ingredientsPerMeal)
                    {
                        ...
                        if (!ingredientsToRefund.ContainsKey(ingredient.Key))
                        {
                            ...
                            ingredientsToRefund.Add(ingredient.Key, new IngredientsInfo(ingredientProfile.shortname, ingredientProfile.skin, ingredient.Value));
                        }
                        else
                        {
                            //Interface.Oxide.LogInfo("Adding to existing key for refunds.");
                            ingredientsToRefund[ingredient.Key].amount += ingredient.Value;
                        }
                    }
                }
        ...
        {
            if (!config.mealSettings.cookbookSettings.enabled) return LoadedMeals;
        ...
        private void SendRecipesMenu(BasePlayer player, int firstRecipeToShowIndex)
        {
            ...
            {                
                ...
                container.Add(new CuiPanel
                {
                    ...
                    Image = { Color = config.menuSettings.recipeMenuSettings.recipe_title_panel_colour },
        ...
        private void SendRecipeDetailedMenu(BasePlayer player, string meal, string firstMealIndex, CoreElmentSelected fromMenu, string quantity = "1")
        {
            ...
            container.Add(new CuiElement
            {
                ...
                Components = {
                    new CuiTextComponent { Text = lang.GetMessage("UICraft", this, player.UserIDString), Font = "robotocondensed-bold.ttf", FontSize = 25, Align = TextAnchor.MiddleCenter, Color = "1 1 1 1" },
                    new CuiOutlineComponent { Color = "0 0 0 0.5", Distance = "1 -1" },
            ...
            container.Add(new CuiButton
            {
                Button = { Color = "1 1 1 0", Command = $"trycraftdish {quantity} {firstMealIndex} {(int)fromMenu} {meal}" },
                Text = { Text = " ", Font = "robotocondensed-regular.ttf", FontSize = 14, Align = TextAnchor.MiddleCenter, Color = "0 0 0 1" },
                RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-50 -21", OffsetMax = "50 21" }
        ...
        string GetBuffDescriptions(BasePlayer player, Buff buff, float modifier)
        {
            switch (buff)
            {
                case Buff.Wealth: return $"<color=#07a99d>{lang.GetMessage($"{buff}_ToText", this, player.UserIDString)}:</color> {string.Format(lang.GetMessage($"{buff}_MenuDescription", this, player.UserIDString), modifier * 100, lang.GetMessage("CurrencyString", this, player.UserIDString))}";
                case Buff.Passive_Regen: return $"<color=#07a99d>{lang.GetMessage($"{buff}_ToText", this, player.UserIDString)}:</color> {string.Format(lang.GetMessage($"{buff}_MenuDescription", this, player.UserIDString), modifier)}";
                case Buff.Damage_Over_Time: return $"<color=#07a99d>{lang.GetMessage($"{buff}_ToText", this, player.UserIDString)}:</color> {string.Format(lang.GetMessage($"{buff}_MenuDescription", this, player.UserIDString), modifier)}";
                default: return $"<color=#07a99d>{lang.GetMessage($"{buff}_ToText", this, player.UserIDString)}:</color> {string.Format(lang.GetMessage($"{buff}_MenuDescription", this, player.UserIDString), modifier * 100)}";
            }
        }
        ...
        void TryCraftDish(ConsoleSystem.Arg arg)
        {
            ...
            if (!permission.UserHasPermission(player.UserIDString, perm_free))
            {
                ...
                PayForMeal(player, totalIngredients);
            ...
            if (!permission.UserHasPermission(player.UserIDString, perm_instant) && cookTime > 0)
            {
                ...
                {
                    cookingComponent.AddMeal(mealData.shortname, meal, mealData.skin, cookTime, wasFree ? new Dictionary<string, int>() : mealData.ingredients);
                }
            }
            else
            {
                GiveMeal(player, mealData.shortname, mealData.skin, meal, quantity);
        ...
        {
            var locationString = GetGatherSourcesString(player, ingredientData.gatherSources);
            var container = new CuiElementContainer();
            container.Add(new CuiElement
            {
            ...
            if (locationString != null) Pool.FreeList(ref locationString);

            CuiHelper.DestroyUi(player, "SendIngredientText");
            CuiHelper.AddUi(player, container);
        }

        List<string> GetGatherSourcesString(BasePlayer player, List<GatherSource> sources)
        {
        ...
        private void FavouritesMenu(BasePlayer player, int firstRecipeToShowIndex)
        {
            ...
            for (int i = 0; i < pi.favouriteDishes.Count; i++)
            {
                ...
                container.Add(new CuiPanel
                {
                    ...
                    //Image = { Color = "0.490566 0.490566 0.490566 1" },
                    Image = { Color = config.menuSettings.favouriteMenuSettings.recipe_title_panel_colour },
                    RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-97 -47.5", OffsetMax = "97 -27.5" }
            ...
            foreach (var ingredient in ingredients)
            {
                ...
                foreach (var source in ingredient.Value.gatherSources)
                {
                    container.Add(new CuiElement
                    {
                        ...
                        Parent = $"Ingredient_{count}_{row}",
                        Components = {
                        new CuiTextComponent { Text = $"{lang.GetMessage("UIDash", this, player.UserIDString)} {lang.GetMessage($"{source}_LocationText", this, player.UserIDString)}", Font = "robotocondensed-regular.ttf", FontSize = 12, Align = TextAnchor.UpperLeft, Color = "1 1 1 1" },
                        new CuiOutlineComponent { Color = "0 0 0 0.5", Distance = "1 -1" },
        ...
        void SendMealDescription(ConsoleSystem.Arg arg)
        {
            ...
            List<string> buffStrings = Pool.GetList<string>();
            foreach (var buff in mealData.buffs)
                buffStrings.Add(lang.GetMessage($"{buff.Key}_ToText", this, player.UserIDString));

            CreateGameTip(string.Format(lang.GetMessage("BuffToolTip", this, player.UserIDString), string.Join(", ", buffStrings)), player);
            Pool.FreeList(ref buffStrings);
        }
        ...
        private void CookingButton(BasePlayer player)
        {
            if (!permission.UserHasPermission(player.UserIDString, perm_use)) return;
            var container = new CuiElementContainer();
            container.Add(new CuiButton
            {
                ...
                RectTransform = { AnchorMin = config.menuSettings.anchorSettings.ovenButton.anchorMin, AnchorMax = config.menuSettings.anchorSettings.ovenButton.anchorMax, OffsetMin = config.menuSettings.anchorSettings.ovenButton.offsetMin, OffsetMax = config.menuSettings.anchorSettings.ovenButton.offsetMax }
        ...
        private void FarmersMarket(BasePlayer player, int index)
        {
            ...
            foreach (var ingredient in MarketIngredients)
            {
                ...
                string sellText = String.Format(lang.GetMessage("UIMarketSellText", this, player.UserIDString), ingredient.Value.marketInfo.marketBuyForPrice > 0 ? ingredient.Value.marketInfo.marketBuyForPrice.ToString() : lang.GetMessage("UIMarketNA", this, player.UserIDString));
                container.Add(new CuiElement
                {
                    Name = "selltext",
                    Parent = $"marketIngredient_{row}_{count}",
                ...
                string buytext = String.Format(lang.GetMessage("UIMarketBuyText", this, player.UserIDString), ingredient.Value.marketInfo.marketSellForPrice > 0 ? ingredient.Value.marketInfo.marketSellForPrice.ToString() : lang.GetMessage("UIMarketNA", this, player.UserIDString));
                container.Add(new CuiElement
                {
                    Name = "buytext",
                    Parent = $"marketIngredient_{row}_{count}",
        ...
        void BuyMarketOrder(ConsoleSystem.Arg arg)
        {
            ...
            GiveMeal(player, ingredientData.shortname, ingredientData.skin, ingredient, amount);
        ...
        void SellMarketOrder(ConsoleSystem.Arg arg)
        {
            ...
            var found = 0;
            List<Item> foundToSell = Pool.GetList<Item>();
            foreach (var item in player.inventory.AllItems())
            {
            ...
            switch (config.generalSettings.marketSettings.currency)
            {
                case "economics":
                    ...
                    if (Economics != null && Economics.IsLoaded)
                        if (!Convert.ToBoolean(Economics.Call("Deposit", player.userID, amountDouble))) PrintToChat(player, lang.GetMessage("MarketCashBug", this, player.UserIDString));
                ...
                case "serverrewards":
                    ...
                    if (ServerRewards != null && ServerRewards.IsLoaded)
                        if (!Convert.ToBoolean(ServerRewards.Call("AddPoints", player.userID, amountSRP))) PrintToChat(player, lang.GetMessage("MarketPointsBug", this, player.UserIDString));
        ...
        bool HasEnoughCurrency(BasePlayer player, int amount)
        {
            ...
            {
                case "economics":
                    var balance = Economics != null && Economics.IsLoaded ? (double)Economics.Call("Balance", player.userID) : 0;
                    return balance >= amount;

                case "serverrewards":
                    var srpbalance = ServerRewards != null && ServerRewards.IsLoaded ? (int)ServerRewards.Call("CheckPoints", player.userID) : 0;
        ...
        bool TakeCurrency(BasePlayer player, int amount)
        {
            switch (config.generalSettings.marketSettings.currency)
            {
                case "economics":
                    ...
                    var econResult = Economics != null && Economics.IsLoaded ? Convert.ToBoolean(Economics.Call("Withdraw", player.userID, amountDouble)) : false;
                    return econResult;

                case "serverrewards":
                    var srpResult = ServerRewards != null && ServerRewards.IsLoaded ? Convert.ToBoolean(ServerRewards.Call("TakePoints", player.userID, amount)) : false;
                    return srpResult;
        ...
        void CreatButtons(BasePlayer player)
        {
            ...
            List<HudButtonInfo> buttonData = Pool.GetList<HudButtonInfo>();
        ...
        {
            List<string> foundMeals = Pool.GetList<string>();
            foreach (var item in player.inventory.AllItems())
            {
        ...
        {
            List<Buff> perks_to_remove = Pool.GetList<Buff>();
        ...
        {
            if (entity != null && entity.net != null && IBagContainers.Contains(entity)) return true;
            return null;
        }
        ...
        object CanJoinEvent(BasePlayer player, string eventName)
        {
            var buffManager = GetBuffManager(player, false);
            if (buffManager == null) return null;
            switch (eventName)
        ...
        void SetCookingMarketStockCMD(ConsoleSystem.Arg arg)
        {
            ...
            if (arg.Args.Length == null || arg.Args.Length <  1 || !arg.Args[0].IsNumeric())
            {
        ...
        void GiveRecipeCommand(BasePlayer player, string[] args, CommandType commandType)
        {
            ...
            var item = ItemManager.CreateByName(config.mealSettings.cookbookSettings.blueprint.shortname, 1, config.mealSettings.cookbookSettings.blueprint.skin);
            if (!string.IsNullOrEmpty(config.mealSettings.cookbookSettings.blueprint.displayName)) item.name = item.name = string.Format(config.mealSettings.cookbookSettings.blueprint.displayName, meal);
            item.text = meal;
        ...
        void GiveMealFromCommand(BasePlayer player, string[] args, CommandType commandType)
        {
            ...
            GiveMeal(target, mealData.shortname, mealData.skin, meal, amount);
            SendFeedbackToCommandUser(player, $"You gave {amount}x {meal} to {target.displayName}.", commandType);
        }
        ...
        void GiveIngredientFromCMD(BasePlayer player, string[] args, CommandType commandType)
        {
            ...
            GiveMeal(target, ingredientData.shortname, ingredientData.skin, ingredient, amount);
        ...
        #region Updating old recipes and ingredients

        public class RootObject
        {
            [JsonProperty("Ingredient information")]
            public Dictionary<string, IngredientInfo> IngredientInformation { get; set; }
        }

        public class MealObject
        {
            [JsonProperty("Recipes information")]
            public Dictionary<string, RecipeInfo> RecipeInformation { get; set; }
        }
        ...
        public class RecipeInfo
        {
            ...
            public string base_shortname;
            public string imageURL;
            public string description;
            public ulong skin;
            public List<IngredientInfo> ingredients;
            public int buff_duration = 30;
            public int cookTime = 30;
            public Dictionary<string, float> buffTypes = new Dictionary<string, float>();
            public float coolDown = 0f;
            //public float BuffModifier = 0f;
            public string required_permission = null;
            public string[] perms;
            public float loot_container_drop_weight = 100f;
            public RecipeInfo(string base_shortname, string imageURL, string description, ulong skin, Dictionary<string, float> buffTypes, int cookTime, int buff_duration, float coolDown = 0f, bool enabled = true, string[] perms = null, float loot_container_drop_weight = 100f, string required_permission = null)
            {
                this.enabled = enabled;
                this.base_shortname = base_shortname;
                ...
                this.perms = perms;
                this.loot_container_drop_weight = loot_container_drop_weight;
            ...
            public class IngredientInfo
            {
                public string displayName;
                public int quantity;
                public IngredientInfo(string displayName, int quantity)
                {
                    this.displayName = displayName;
        ...
        public class IngredientInfo
        {
            ...
            public string base_shortname;
            public ulong skin;
            public int[] gathered_from;
            public int dropWeight = 100;
            public int max_count = 1;
            public string imageURL;
            public bool market_enable = true;
            public double market_buy_price = 10;
            public double market_sell_price = 5;
            public int market_quantity = 0;
            public float loot_container_drop_weight = 100f;
            public IngredientInfo(string base_shortname, ulong skin, int[] gathered_from, int max_count, string imageURL, float market_buy_price, float market_sell_price, int dropWeight, bool market_enable = true, bool enabled = true, float loot_container_drop_weight = 100f)
            {
                this.enabled = enabled;
                this.base_shortname = base_shortname;
                this.skin = skin;
                this.gathered_from = gathered_from;
                this.max_count = max_count;
                this.imageURL = imageURL;
                this.market_buy_price = market_buy_price;
                this.market_sell_price = market_sell_price;
                this.dropWeight = dropWeight;
        ...
        {
            // Read the JSON file
            string jsonString = File.ReadAllText("oxide/config/Cooking.json");

            if (config.ingredientSettings.ingredients.Count == 0)
            {
                RootObject root = JsonConvert.DeserializeObject<RootObject>(jsonString);

                // Access the Ingredient information
                Dictionary<string, IngredientInfo> ingredientInfo = root.IngredientInformation;

                Dictionary<string, IngredientsInfo> convertedIngredients = new Dictionary<string, IngredientsInfo>();
                
                foreach (var ingredient in ingredientInfo)
                {
                    convertedIngredients.Add(ingredient.Key, ConvertOldIngredient(ingredient.Value));
                }

                config.ingredientSettings.ingredients = convertedIngredients;
                Puts($"Converted {config.ingredientSettings.ingredients.Count}x ingredients from the old config.");
            }

            if (config.mealSettings.meals.Count == 0)
            {
                MealObject mealObj = JsonConvert.DeserializeObject<MealObject>(jsonString);
                // Access the Meal information
                Dictionary<string, RecipeInfo> recipeInfo = mealObj.RecipeInformation;
                Dictionary<string, MealInfo> convertedMeals = new Dictionary<string, MealInfo>();
                foreach (var meal in recipeInfo)
                {
                    convertedMeals.Add(meal.Key, ConvertMeal(meal.Value));
                }

                config.mealSettings.meals = convertedMeals;
                Puts($"Converted {config.mealSettings.meals.Count}x meals from the old config.");
            }            
        }

        #region Ingredient conversion

        IngredientsInfo ConvertOldIngredient(IngredientInfo oldIngredient)
        {
            var result = new IngredientsInfo()
            {
                enabled = oldIngredient.enabled,
                shortname = oldIngredient.base_shortname,
                skin = oldIngredient.skin,
                minDrops = 1,
                maxDrops = oldIngredient.max_count,
                dropWeight = oldIngredient.dropWeight,
                marketInfo = new MarketItemInfo(0, 100, 50, 10)
            };
            foreach (var source in oldIngredient.gathered_from)
            {
                result.gatherSources.Add(ConvertSource(source));
            }

            return result;
        }

        GatherSource ConvertSource(int source)
        {
            switch (source)
            {
                case 0: return GatherSource.AnyNode;
                case 1: return GatherSource.AnyTree;
                case 2: return GatherSource.DesertTree;
                case 3: return GatherSource.Hemp;
                case 4: return GatherSource.CollectableStone;
                case 5: return GatherSource.BerryBush;
                case 6: return GatherSource.Pumpkin;
                case 7: return GatherSource.Potato;
                case 8: return GatherSource.Corn;
                case 9: return GatherSource.Gut;
                case 10: return GatherSource.Boar;
                case 11: return GatherSource.Chicken;
                case 12: return GatherSource.Bear;
                case 13: return GatherSource.Deer;
                case 14: return GatherSource.Wolf;
                case 15: return GatherSource.horse;
                case 16: return GatherSource.Mushroom;
                case 17: return GatherSource.AnyAnimal;
                case 18: return GatherSource.Default;
                case 19: return GatherSource.Crafted;
                case 20: return GatherSource.AnyDesertNode;
                case 21: return GatherSource.AnyTemperateNode;
                case 22: return GatherSource.AnyTundraNode;
                case 23: return GatherSource.AnyArcticNode;
                case 24: return GatherSource.TemperateTree;
                case 25: return GatherSource.TundraTree;
                case 26: return GatherSource.ArcticTree;
                case 27: return GatherSource.StoneNode;
                case 28: return GatherSource.MetalNode;
                case 29: return GatherSource.SulfurNode;
                case 30: return GatherSource.PolarBear;
                case 31: return GatherSource.Fishing;
                case 32: return GatherSource.Cactus;
                case 33: return GatherSource.Foodbox;
                default: return GatherSource.Default;
            }
        }

        #endregion

        #region Recipe conversion

        MealInfo ConvertMeal(RecipeInfo oldRecipe)
        {
            var result = new MealInfo()
            {
                enabled = oldRecipe.enabled,
                shortname = oldRecipe.base_shortname,
                skin = oldRecipe.skin,
                description = oldRecipe.description,
                duration = oldRecipe.buff_duration,
                cookTime = oldRecipe.cookTime,
                useCooldown = oldRecipe.coolDown,
                dropWeight = Convert.ToInt32(oldRecipe.loot_container_drop_weight),
                ingredients = ConvertIngredientItem(oldRecipe.ingredients),
                buffs = ConvertOldBuffs(oldRecipe.buffTypes),
            };

            foreach (var buff in result.buffs)
            {
                if (buff.Key == Buff.Permission)
                {
                    foreach (var perm in oldRecipe.perms)
                    {
                        result.commandsOnConsume.Add(string.Format("o.grant user {0} {1}", "id", perm), "Add your command description here.");
                        result.commandsOnRemove.Add(string.Format("o.revoke user {0} {1}", "id", perm));
                    }
                }
            }
        ...
        Dictionary<Buff, float> ConvertOldBuffs(Dictionary<string, float> oldBuffs)
        {
            Dictionary<Buff, float> result = new Dictionary<Buff, float>();
            foreach (var buff in oldBuffs)
                result.Add(ConvertToBuff(buff.Key), buff.Value);

            return result;
        }

        Buff ConvertToBuff(string value)
        {
            switch (value)
            {
                case "Healshare": return Buff.Heal_Share;
                case "HealthOverload": return Buff.Max_Health;
                case "FoodShare": return Buff.Food_Share;
                case "FoodOverload": return Buff.Metabolism_Overload;
                case "Comfort": return Buff.Comfort;
                case "WaterBreathing": return Buff.Water_Breathing;
                case "FireResist": return Buff.Fire_Resist;
                case "ColdResist": return Buff.Cold_Resist;
                case "ExplosionResist": return Buff.Explosion_Resist;
                case "AnimalResist": return Buff.Animal_Resist;
                case "MeleeResist": return Buff.Melee_Resist;
                case "WoundedResist": return Buff.Wounded_Resist;
                case "Spectre": return Buff.Spectre;
                case "Madness": return Buff.Madness;
                case "Wealth": return Buff.Wealth;
                case "AntiBradleyRadar": return Buff.Anti_Bradley_Radar;
                case "Ingredient": return Buff.Ingredient;
                case "BarrelSmasher": return Buff.Barrel_Smasher;
                case "CraftingRefund": return Buff.Crafting_Refund;
                case "PassiveRegen": return Buff.Passive_Regen;
                case "DamageOverTime": return Buff.Damage_Over_Time;
                case "HorseStats": return Buff.Horse_Stats;
                case "NoFallDamage": return Buff.Fall_Damage_resist;
                case "NoConditionLoss": return Buff.Condition_Loss_Reduction;
                case "IngredientChance": return Buff.Ingredient_Chance;
                case "UngradeRefund": return Buff.Upgrade_Refund;
                case "ResearchRefund": return Buff.Research_Refund;
                case "RolePlay": return Buff.Role_Play;
                case "FishingLuck": return Buff.Fishing_Luck;
                case "MiningYield": return Buff.Mining_Yield;
                case "WoodcuttingYield": return Buff.Woodcutting_Yield;
                case "FarmingYield": return Buff.Farming_Yield;
                case "ComponentLuck": return Buff.Component_Luck;
                case "ElectronicsLuck": return Buff.Electronics_Luck;
                case "Permission": return Buff.Permission;
                case "NoBleed": return Buff.Bleed_Resist;
                case "RadiationResist": return Buff.Radiation_Resist;
                case "MaxRepair": return Buff.Max_Repair;
                case "Heal": return Buff.Heal;
                case "SmeltOnMine": return Buff.Smelt_On_Mine;
                case "LootPickup": return Buff.Loot_Pickup;
                case "Reviver": return Buff.Reviver;
                case "Duplicator": return Buff.Duplicator;
                default: return Buff.Role_Play;
            }
        }

        Dictionary<string, int> ConvertIngredientItem(List<RecipeInfo.IngredientInfo> oldIngredients)
        {
            Dictionary<string, int> result = new Dictionary<string, int>();
            foreach (var oldIngredient in oldIngredients)
            {
                result.Add(oldIngredient.displayName, oldIngredient.quantity);
            }
            return result;
        }
