...
{
    [Info("Zone Manager", "k1lly0u", "3.1.4")]
    ...
    public class ZoneManager : RustPlugin
    {
        ...
        private object OnEntityTakeDamage(BaseCombatEntity entity, HitInfo hitinfo)
        {
            ...
            {                
                ...
                if (attacker != null)
                {
                    ...
                    else if (HasPlayerFlag(attacker, ZoneFlags.PvpGod) && !IsNpc(attacker))                    
                        return true;                    
                }
            ...
            if (!(entity is LootContainer) && !(entity is PatrolHelicopter))
            {
        ...
        {
            if (baseVehicle is BaseVehicleModule)
                baseVehicle = (baseVehicle as BaseVehicleModule).Vehicle;
        ...
        public class Zone : MonoBehaviour
        {
            ...
            public List<BasePlayer> players = Pool.GetList<BasePlayer>();

            public List<BaseEntity> entities = Pool.GetList<BaseEntity>();

            private List<IOEntity> ioEntities = Pool.GetList<IOEntity>();


            public List<ulong> keepInList = Pool.GetList<ulong>();

            public List<ulong> whitelist = Pool.GetList<ulong>();
            ...
            private void OnTriggerEnter(Collider col)
            {
                ...
                if (baseEntity is BasePlayer && !(baseEntity as BasePlayer).IsNpc)
                {
            ...
            public void OnPlayerEnterZone(BasePlayer player)
            {
                ...
                EntityZones entityZone;
                if (zonedPlayers.TryGetValue(player.userID, out entityZone))
            ...
            public void OnEntityEnterZone(BaseEntity baseEntity)
            {
                ...
                EntityZones entityZone;
                if (zonedEntities.TryGetValue(baseEntity.net.ID, out entityZone))
                ...
                {
                    if (baseEntity is BaseAnimalNPC)
                    {
                        (baseEntity as BaseAnimalNPC).brain.SetEnabled(false);
                        return;
                    }

                    if (baseEntity is global::HumanNPC)
                    {
                        (baseEntity as global::HumanNPC).Brain.SetEnabled(false);
                        return;
                    }

                    if (baseEntity is ScarecrowNPC)
                    {
                        (baseEntity as ScarecrowNPC).Brain.SetEnabled(false);
                        return;
                    }
                ...
                if (baseEntity is SmartSwitch || baseEntity is ElectricSwitch || baseEntity is RFReceiver)
                {
                    ioEntities.Add(baseEntity as IOEntity);

                    if (HasFlag(ZoneFlags.PoweredSwitches))
                    {
                        (baseEntity as IOEntity).SetFlag(BaseEntity.Flags.Reserved8, true);
                        (baseEntity as IOEntity).currentEnergy = int.MaxValue;
                    }
                }
            ...
            public void OnEntityExitZone(BaseEntity baseEntity, bool resetDecay, bool isDead = false)
            {
                ...
                {                    
                    if (baseEntity is BaseAnimalNPC)
                    {
                        (baseEntity as BaseAnimalNPC).brain.SetEnabled(true);
                        return;
                    }

                    if (baseEntity is global::HumanNPC)
                    {
                        (baseEntity as global::HumanNPC).Brain.SetEnabled(true);
                        return;
                    }

                    if (baseEntity is ScarecrowNPC)
                    {
                        (baseEntity as ScarecrowNPC).Brain.SetEnabled(true);
                        return;
                    }
                ...
                if (baseEntity is SmartSwitch || baseEntity is ElectricSwitch || baseEntity is RFReceiver)
                {
                    ioEntities.Remove(baseEntity as IOEntity);

                    if (HasFlag(ZoneFlags.PoweredSwitches))
                    {
                        (baseEntity as IOEntity).SetFlag(BaseEntity.Flags.Reserved8, false);
                        (baseEntity as IOEntity).currentEnergy = 0;

                        for (int i = 0; i < (baseEntity as IOEntity).inputs.Length; i++)
                        {
                            IOEntity fromEntity = (baseEntity as IOEntity).inputs[i].connectedTo.Get();
                            if (fromEntity)
            ...
            {
                float lastReversedTime;
                if (lastReversedTimes.TryGetValue(baseVehicle, out lastReversedTime))
            ...
            public bool HasPermission(BasePlayer player)
            {
                ...
                return string.IsNullOrEmpty(definition.Permission) ? true : Instance.permission.UserHasPermission(player.UserIDString, definition.Permission);
            }
        ...
        private bool CanBypass(string playerId, int flag)
        {
            ...
            if (ZoneFlags.IndexToName.TryGetValue(flag, out flagName))
        ...
        private bool HasFlag(string zoneId, string flagName)
        {
            ...
            int v;
            if (!ZoneFlags.NameToIndex.TryGetValue(flagName, out v))             
            {
        ...
        public class ZoneFlags
        {
            ...
            public bool this[int key]
            {
                get
                {
                    return m_BitArray[key];
                }
                set
                {
                    m_BitArray[key] = value;
                }
            }
            ...
            {
                int v;
                if (!NameToIndex.TryGetValue(flagName, out v))   
                {
            ...
            public void SetFlags(params string[] array)
            {
                ...
                {
                    int v;
                    if (NameToIndex.TryGetValue(array[i], out v))                        
            ...
            for (int i = 0; i < args.Length; i = i + 2)
            {
                ...
                switch (args[i].ToLower())
                {
                    ...
                    default:
                        ...
                        int v;
                        if (ZoneFlags.Find(args[i], out v))
                        {
                            if (active)
                                zone.AddFlag(v);
            ...
            public override object ReadJson(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
            {
                ...
                if (reader.TokenType == JsonToken.String)
                {
                    ...
                    for (int i = 0; i < values.Length; i++)
                    {
                        ...
                        int v;
                        if (ZoneFlags.NameToIndex.TryGetValue(value, out v))                        
                            zoneFlags.AddFlag(v);                       
                    }
                }
                return zoneFlags;
