...
namespace Oxide.Plugins
{
    ...
    [Description("This plugin was downloaded from the discord channel [Rust Plugins Sliv]: https://discord.gg/pFgKw6Dyyq")]
    class DeepWater : RustPlugin
    {
        ...
        private void RestoreSavedRigs()
        {
            if (storedData.saveRigData.Count > 0)
            {
                ...
                if (objects != null)
                {
                    ...
                    {
                        if (storedData.saveRigData.ContainsKey(obj.net.ID.Value))
                        {
                            var getent = obj.GetComponent<BaseEntity>();
                            if (getent) getent.Invoke("KillMessage", 0.1f);

                            var signownerid = storedData.saveRigData[obj.net.ID.Value].ownerid;
                            var signpos = StringToVector3(storedData.saveRigData[obj.net.ID.Value].pos);
                            var signrot = StringToQuaternion(storedData.saveRigData[obj.net.ID.Value].rot);
                            var signangle = StringToVector3(storedData.saveRigData[obj.net.ID.Value].eangles);
                            var rigcompound = storedData.saveRigData[obj.net.ID.Value].iscompound;
                            timer.Once(2f, () => SpawnRig(signownerid, signpos, signrot, signangle, rigcompound));

                            storedData.saveRigData.Remove(obj.net.ID.Value);
                            SaveData();
                        }
                    }
                }
        ...
        static ulong HeliPadRugLogo1 = 1371746398;
        static ulong HeliPadRugLogo2 = 1371746398;
        ...
        {
            public Dictionary<ulong, StoredRigData> saveRigData = new Dictionary<ulong, StoredRigData>();
            public StoredData() { }
        }
        ...
        void AddData(ulong entnetid, ulong sownerid, string spos, string seangles, string srot, int compound = 0)
        {
        ...
        void RemoveData(ulong entnetid)
        {
            if (storedData.saveRigData.ContainsKey(entnetid)) storedData.saveRigData.Remove(entnetid);
        }
        ...
        void OnEntityDeath(BaseCombatEntity target, HitInfo info)
        {
            ...
            if (storedData.saveRigData.ContainsKey(target.net.ID.Value))
            {
                storedData.saveRigData.Remove(target.net.ID.Value);
                SaveData();
            }
        }
        ...
        public void BuildRandomDeepWater(bool eventstarted, bool autospawned = false)
        {
            ...
            if (!eventstarted) AddData(newrig.net.ID.Value, newrig.OwnerID, newrig.transform.position.ToString(), newrig.transform.eulerAngles.ToString(), newrig.transform.rotation.ToString());
        ...
        public void BuildLocalDeepWater(BasePlayer player, bool eventstarted, int compound = 0)
        {
            ...
            newrig?.Spawn();
            var addrigstructure = newrig.gameObject.AddComponent<DeepWaterEntity>();
            if (!eventstarted) AddData(newrig.net.ID.Value, newrig.OwnerID, newrig.transform.position.ToString(), newrig.transform.eulerAngles.ToString(), newrig.transform.rotation.ToString(), compound);
        ...
        void SpawnRig(ulong ownerid, Vector3 pos, Quaternion rot, Vector3 angle, int compound = 0)
        {
            ...
            var addrigstructure = newrig.gameObject.AddComponent<DeepWaterEntity>();
            if (compound == 1) addrigstructure.iscompound = true;
            AddData(newrig.net.ID.Value, ownerid, pos.ToString(), angle.ToString(), rot.ToString(), compound);
        }
        ...
        public class DeepWaterEntity : BaseEntity
        {
            ...
            void Awake()
            {
                ...
                entitynetid = entity.net.ID.Value;
                setactive = true;
                dorigdestroy = false;
            ...
            private BaseEntity SpawnPart(string prefab, BaseEntity entitypart, bool setactive, int eulangx, int eulangy, int eulangz, float locposx, float locposy, float locposz, ulong skinid = 0)
            {
                if (entitypart == null)
                {
