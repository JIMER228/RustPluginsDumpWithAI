...
{
    [Info("VirtualQuarries", "ThePitereq", "2.3.12")]
    public class VirtualQuarries : RustPlugin
    {
        ...
        private readonly Dictionary<ulong, ulong> cachedInventories = new Dictionary<ulong, ulong>();
        ...
        private void OnServerInitialized()
        {
            ...
            Dictionary<string, ulong> reservedImagesLoad = new Dictionary<string, ulong>();
            foreach (var profile in config.quarryProfiles.Values)
            {
                reservedImagesLoad.TryAdd(profile.icon.shortname, profile.icon.skin);
                if (profile.icon.iconUrl != "")
                    AddImage(profile.icon.iconUrl, profile.icon.shortname, profile.icon.skin);
                foreach (var resource in profile.resources)
                {
                    reservedImagesLoad.TryAdd(resource.Value.shortname, resource.Value.skin);
                    if (resource.Value.iconUrl != "")
                        AddImage(resource.Value.iconUrl, resource.Value.shortname, resource.Value.skin);
                    foreach (var additional in resource.Value.additionalItems)
                    {
                        reservedImagesLoad.TryAdd(additional.shortname, additional.skin);
                        if (additional.iconUrl != "")
                            AddImage(additional.iconUrl, additional.shortname, additional.skin);
                    }
                }
                foreach (var item in profile.requiredItems)
                {
                    reservedImagesLoad.TryAdd(item.shortname, item.skin);
                    if (item.iconUrl != "")
            ...
            {
                reservedImagesLoad.TryAdd(survey.surveyItem.shortname, survey.surveyItem.skin);
                if (survey.surveyItem.iconUrl != "")
                    AddImage(survey.surveyItem.iconUrl, survey.surveyItem.shortname, survey.surveyItem.skin);
            }
            if (config.forceLoadSteamImages)
                ImageLibrary?.Call("LoadImageList", Name, reservedImagesLoad);
                ...
                else
                {
                    ...
                    if (storage.children.Count == 0)
                    {
                        ...
                        if (quarry.Value.level < upgrades.Count)
                            fuelStorage.inventory.capacity = config.quarryProfiles[quarry.Value.profile].upgrades[quarry.Value.level].fuelCapacity;
        ...
        private void OnLootEntityEnd(BasePlayer player)
        {
            if (cachedInventories.ContainsKey(player.userID))
            {
                NextTick(() => {
                    ...
                    {
                        BaseEntity entity = BaseNetworkable.serverEntities.Find(new NetworkableId(cachedInventories[player.userID])) as BaseEntity;
        ...
        private object OnExcavatorResourceSet(ExcavatorArm arm, string type, BasePlayer player)
        {
            ...
            storage.GetComponent<VirtualQuarry>().mineType = type;
            if (!arm.IsOn())
            {
                arm.SetFlag(BaseEntity.Flags.On, true);
        ...
        private object CanLootEntity(BasePlayer player, StorageContainer storage)
        {
            ...
            if (config.excavatorQuarry && storage is ExcavatorOutputPile)
            {
                ExcavatorOutputPile excavatorStorage = storage as ExcavatorOutputPile;
                ...
                if (!data.staticQuarries[excavatorId].ContainsKey(player.userID))
                    SpawnStaticQuarry(excavatorId, player, false, storage);
                else
                    OpenExcavatorQuarry(player, excavatorStorage);
                return false;
            }
            else if (config.excavatorQuarry && storage is DieselEngine)
            {
                DieselEngine dieselStorage = storage as DieselEngine;
                if (config.excavatorPerm && !permission.UserHasPermission(player.UserIDString, "virtualquarries.static.excavator"))
                {
                    PopUpAPI?.Call("ShowPopUp", player, config.popUpPreset, Lang("NoPermissionQuarry", player.UserIDString));
                    return false;
                ...
                if (!data.staticQuarries[excavatorId].ContainsKey(player.userID))
                    ...
                    OpenExcavatorQuarry(player, dieselStorage);
                return false;
            }
            else if (config.staticQuarries && storage is ResourceExtractorFuelStorage)
            {
                ResourceExtractorFuelStorage quarryStorage = storage as ResourceExtractorFuelStorage;
                ...
                if (!data.staticQuarries[quarry.net.ID.Value].ContainsKey(player.userID))
                    SpawnStaticQuarry(quarry.net.ID.Value, player, true, storage, quarry);
                else
        ...
        private void SpawnStaticQuarry(ulong netId, BasePlayer player, bool staticQuarry, StorageContainer storage, MiningQuarry quarry = null)
        {
            ...
            data.staticQuarries[netId].TryAdd(player.userID, resourceStorage.net.ID.Value);
            data.staticQuarries[netId][player.userID] = resourceStorage.net.ID.Value;
            ...
            NextTick(() => {
                vQuarry.SetupQuarry();
                if (storage is ResourceExtractorFuelStorage)
                    OpenStaticQuarry(player, storage as ResourceExtractorFuelStorage, quarry);
                else if (storage is DieselEngine)
                    OpenExcavatorQuarry(player, storage as DieselEngine);
                else if (storage is ExcavatorOutputPile)
                    OpenExcavatorQuarry(player, storage as ExcavatorOutputPile);
            });
        }
        ...
        private void ShowQuarryAccessList(BasePlayer player, int quarryId)
        {
            ...
            PopUpAPI?.Call("ShowPopUp", player, config.popUpPreset, accessList, 10f, "Hud.Menu", 12);
        }
        ...
        private void TryUpgradeQuarry(BasePlayer player, int quarryId, string type)
        {
            ...
            if (type == "currency")
            {
                ...
                {
                    if (ShoppyStock.Call<int>("GetCurrencyAmount", config.economyCurrency, player.userID) >= upgrades[quarry.level + 1].requiredRp)
                    {
                        ShoppyStock.Call("TakeCurrency", config.economyCurrency, player.userID, upgrades[quarry.level + 1].requiredRp);
                        UpgradeQuarry(player, quarryId, type);
                    }
                    else
                        PopUpAPI?.Call("ShowPopUp", player, config.popUpPreset, Lang("NotEnoughCurrency", player.UserIDString));
        ...
        {
            if (!data.quarries.ContainsKey(quarryId))
            {
            ...
            if (config.refundRemove)
            {
                ...
                {
                    if (!refundItems2.ContainsKey(resource.configKey)) continue;
            ...
            {
                foreach (var upgrade in data.quarries[quarryId].upgradedForRp)
                {
                    if (config.quarryProfiles[data.quarries[quarryId].profile].upgrades.Count - 1 >= upgrade.Key)
                    {
                        UpgradeConfig upgradeConfig = config.quarryProfiles[data.quarries[quarryId].profile].upgrades[upgrade.Key];
                        if (upgrade.Value)
                        {
                            if (config.economyPlugin == 1)
                                Economics.Call("Deposit", player.userID, Convert.ToDouble(upgradeConfig.requiredRp));
                            else if (config.economyPlugin == 2)
                                ServerRewards.Call("AddPoints", player.userID, upgradeConfig.requiredRp);
                            else if (config.economyPlugin == 4)
                                BankSystem.Call("Deposit", player.userID, upgradeConfig.requiredRp);
                            else if (config.economyPlugin == 5)
                                ShoppyStock.Call("GiveCurrency", config.economyCurrency, player.userID, upgradeConfig.requiredRp);
                        }
                        else
                        {
        ...
        private void TryStartQuarry(BasePlayer player, int quarryId)
        {
            ...
            {
                FuelItem fuelItem = config.quarryProfiles[data.quarries[quarryId].profile].fuelItem;
                ...
                PopUpAPI?.Call("ShowPopUp", player, config.popUpPreset, Lang("NoFuelHint", player.UserIDString, (int)Math.Ceiling(fuelItem.amount), displayedFuelName));
            }
        ...
        private void TryOpenFuelContainer(BasePlayer player, int quarryId)
        {
            ...
            if (config.lockAccesNoPerm)
            {
                ...
                foreach (var configPerm in config.permissions)
                    if (permission.UserHasPermission(player.UserIDString, configPerm.Key))
                        playerPerm = configPerm.Value;
                if (!playerPerm.ContainsKey(data.quarries[quarryId].profile))
                {
            ...
            entity.Spawn();
            cachedInventories.Remove(player.userID);
            cachedInventories.Add(player.userID, entity.net.ID.Value);
            BoxStorage fuelStorage = quarry.children[0] as BoxStorage;
        ...
        private void TryOpenResourceContainer(BasePlayer player, int quarryId)
        {
            ...
            quarry.inventory.canAcceptItem = (item, i) => IsResourceOutput(item, data.quarries[quarryId].profile);
        ...
        {
            ulong storageId = data.staticQuarries[quarry.net.ID.Value][player.userID];
            BoxStorage storage = BaseNetworkable.serverEntities.Find(new NetworkableId(storageId)) as BoxStorage;
            if (storage == null)
            {
                SpawnStaticQuarry(quarry.net.ID.Value, player, true, quarryStorage);
                return;
            }
        ...
        private void OpenExcavatorQuarry(BasePlayer player, StorageContainer excavatorStorage)
        {
            ulong storageId = data.staticQuarries[uint.MaxValue][player.userID];
            BoxStorage storage = BaseNetworkable.serverEntities.Find(new NetworkableId(storageId)) as BoxStorage;
            if (storage == null)
            {
                SpawnStaticQuarry(uint.MaxValue, player, false, excavatorStorage);
                return;
            }
            if (excavatorStorage is DieselEngine)
                storage = storage.children[0] as BoxStorage;
            player.EndLooting();
            if (excavatorStorage is DieselEngine)
            {
        ...
        private BoxStorage GetQuarry(int quarryId)
        {
            ...
            BoxStorage storage = BaseNetworkable.serverEntities.Find(new NetworkableId(data.quarries[quarryId].netId)) as BoxStorage;
            if (storage != null) return storage;
            SpawnQuarry(0, quarryId);
            storage = BaseNetworkable.serverEntities.Find(new NetworkableId(data.quarries[quarryId].netId)) as BoxStorage;
            if (storage != null) return storage;
            else return null;
        }
        ...
        private void ShowQuarryMenuUI(BasePlayer player, int page = 1, int quarryId = -1)
        {
            ...
            foreach (var quarry in data.quarries)
            {
                ...
                UI_AddItemImage(container, "VirtualQuarriesUI_Button", config.quarryProfiles[quarry.Value.profile].icon.shortname, config.quarryProfiles[quarry.Value.profile].icon.skin, "0.5 0.5", "0.5 0.5", "-16 -16", "16 16"); //Quarry Icon
            ...
            if (quarryId != -1 && (data.quarries[quarryId].owner == player.userID || data.quarries[quarryId].authPlayers.Contains(player.userID)))
            {
                ...
                foreach (var resource in quarry.resources)
                {
                    ...
                    string perMinute = outputAmount < 10 ? $"{outputAmount.ToString("0.###")}/m" : $"{outputAmount.ToString("0.##")}/m";
        ...
        private void ShowNewQuarryUI(BasePlayer player, bool firstJoin = false)
        {
            ...
            for (int i = 0; i < 9; i++)
            {
                ...
                {
                    UI_AddItemImage(container, "VirtualQuarriesUI_New", resources[cachedSurveys[player.userID].resources[i].configKey].shortname, resources[cachedSurveys[player.userID].resources[i].configKey].skin, "0.5 0.5", "0.5 0.5", $"{offset + 5} 3", $"{offset + 40} 38"); //Output Item Icon
                    float outputAmount = 60f / config.quarryTick * cachedSurveys[player.userID].resources[i].work;
                    string perMinute = outputAmount < 10 ? $"{outputAmount.ToString("0.###")}/m" : $"{outputAmount.ToString("0.##")}/m";
        ...
        private void AddQuarryLevelsUI(BasePlayer player, int quarryId)
        {
            ...
            else
            {
                ...
                {
                    UI_AddItemImage(container, "VirtualQuarriesUI_Levels", upgrades[level + 1].requiredItems[i].shortname, upgrades[level + 1].requiredItems[i].skin, "0.5 0", "0.5 0", $"{start + 2} {cornerY + 40}", $"{start + 43} {cornerY + 81}");
        ...
        private void UI_AddItemImage(CuiElementContainer container, string parentName, string shortname, ulong skin, string anchorMin, string anchorMax, string offsetMin, string offsetMax, string color = "1 1 1 1")
        {
            ...
            {
                UI_AddImage(container, parentName, shortname, skin, anchorMin, anchorMax, offsetMin, offsetMax, color);
                return;
            }
        ...
        private class VirtualQuarry : FacepunchBehaviour
        {
            ...
            public void SetupQuarry()
            {
                if (staticQuarry == null)
                {
                    if (excavator)
                    {
                        CancelInvoke(MineExcavatorResources);
                        InvokeRepeating(MineExcavatorResources, config.excavatorTick, config.excavatorTick);
                    }
                    else
                    {
                        level = data.quarries[dataId].level;
                        profile = data.quarries[dataId].profile;
                        resources = data.quarries[dataId].resources;
                        isRunning = data.quarries[dataId].isRunning;
                        if (!config.quarryProfiles.ContainsKey(profile))
                ...
                else
                {
                    ...
                    CancelInvoke(MineStaticResources);
                    InvokeRepeating(MineStaticResources, config.staticQuarryTick, config.staticQuarryTick);
                }
            }
            ...
            private void ConfigureOutput()
            {
                ...
                Dictionary<string, ResourceConfig> configResources = config.quarryProfiles[profile].resources;
                List<UpgradeConfig> levels = config.quarryProfiles[profile].upgrades;
            ...
            private void ConfigureStaticOutput()
            {
                output = new List<OutputInfo>();
                output.Clear();
                List<StaticQuarryOutput> quarryOutput = config.staticMetalOutput;
            ...
            private void MineExcavatorResources()
            {
                if (mineType == "") return;
                if (!HasExcavatorFuel()) return;
                foreach (var resource in config.excavatorResources[mineType])
                {
                    ...
                    if (!string.IsNullOrEmpty(resource.displayName))
                        item.name = resource.displayName;
                    if (!item.MoveToContainer(storage.inventory)) return;
                }
            }
            ...
            private bool CalculateFuel(bool takeFuel = true)
            {
                ...
                FuelItem requiredFuel = new FuelItem()
                {
                    shortname = config.quarryProfiles[profile].fuelItem.shortname,
                    amount = config.quarryProfiles[profile].fuelItem.amount,
                    skin = config.quarryProfiles[profile].fuelItem.skin
                };
                List<UpgradeConfig> levels = config.quarryProfiles[profile].upgrades;
                nonIntOutput.TryAdd("fuel", 0);
                float notRoundedAmount = (float)requiredFuel.amount * levels[level].fuelMultiplier;
                int amount = Convert.ToInt32(Math.Ceiling(notRoundedAmount));
                if (takeFuel)
                    nonIntOutput["fuel"] += (float)(amount - notRoundedAmount) % 1;
                requiredFuel.amount = amount;
                if (TakeFuel(fuelStorage, requiredFuel, takeFuel))
                    return true;
                else return false;
            }

            private bool HasStaticFuel()
            {
                if (fuelStorage == null) return false;
                if (TakeFuel(fuelStorage, config.staticFuelItem)) return true;
                else return false;
            }

            private bool HasExcavatorFuel()
            {
                if (fuelStorage == null) return false;
                if (TakeFuel(fuelStorage, config.excavatorFuelItem)) return true;
                else return false;
            }

            private bool TakeFuel(BoxStorage storage, FuelItem fuel, bool takeFuel = true)
            {
                int fuelToTakeCeiling = (int)Math.Ceiling(fuel.amount);
                nonIntOutput.TryAdd("fuel", 0);
                if (nonIntOutput["fuel"] >= fuel.amount)
                {
                    if (takeFuel)
                        nonIntOutput["fuel"] -= fuel.amount;
                    return true;
                }
                bool haveRequired = false;
                int inventoryAmount = 0;
                foreach (var item in storage.inventory.itemList)
                {
                    ...
                    {
                        inventoryAmount += item.amount;
                        if (inventoryAmount >= fuelToTakeCeiling)
                        {
                            haveRequired = true;
                            break;
                        }
                ...
                if (takeFuel)
                    foreach (var item in storage.inventory.itemList.ToList())
                    {
                        if (item.skin == fuel.skin && item.info.shortname == fuel.shortname)
                        {
                            int needToTake = fuelToTakeCeiling - takenItems;
                            if (needToTake <= 0) break;
                            if (item.amount > needToTake)
                            {
                                nonIntOutput["fuel"] += needToTake - fuel.amount;
        ...
        private class PluginConfig
        {
            ...
            [JsonProperty("Additional Steam Image Download")]
            public bool forceLoadSteamImages = false;
            ...
            [JsonProperty("Excavator Quarry - Lock Air Drops")]
            public bool excavatorLockDrops = false;
