...
{
    [Info("PersonalNPC", "MoriF#1113", "1.5.5")]
    public class PersonalNPC : RustPlugin 
    {
        [PluginReference] private Plugin ImageLibrary, VehicleDeployedLocks, MarkerManager;

        private Dictionary<uint, PlayerBotController> _existsControllers = new Dictionary<uint, PlayerBotController>();
        private Dictionary<uint, BotOwnerComponent> _existsBots = new Dictionary<uint, BotOwnerComponent>();
        ...
        private object OnEntityTakeDamage(BaseCombatEntity entity, HitInfo info)
        {
            ...
            var initiatorController = GetController(target.net.ID);
            ...
            var initiatorBotOwner = GetOwnerComponent(target.net.ID);
            ...
            if(initiatorBotOwner != null && initiatorBotOwner?.controller != null)
            {
                ...
                {
                    if(GetOwnerComponent(entity.net.ID) != null)
                    {
                        RemoveDamage(info);
                        return true;
                    }
            ...
            var controller = GetController(entity.net.ID);
            ...
            var ownerComponent = GetOwnerComponent(entity.net.ID);
        ...
        private object CanBeTargeted(BaseCombatEntity player, MonoBehaviour behaviour)
        {
            ...
            BotOwnerComponent comp = GetOwnerComponent(player.net.ID);
        ...
        {
            var markers = DeathMarker.DeathMarkers;

            if(markers.ContainsValue(entity))
            {
                if(!markers.ContainsKey(target.userID)) return false;
                else 
                {
                    var marker = markers[target.userID];
        ...
        private object OnPlayerDeath(BasePlayer player, HitInfo info)
        {
            ...
            if(IsBot(player))
            {
                var botComponent = GetOwnerComponent(player.net.ID);
                ...
                if(botComponent != null)
                {
                    ...
                    if(controller != null)
                    {
                        ...
                        if(controller.botSetup.dropActiveItem)
                        {
        ...
        private object CanMoveItem(Item item, PlayerInventory playerLoot, uint targetContainer, int targetSlot, int amount)
        {
            ...
            var controller = GetController(player.net.ID);
            if(controller == null) return null;

            if(controller != null)
            {
                if(controller.bot.inventory.containerMain.uid == targetContainer
                    || controller.bot.inventory.containerBelt.uid == targetContainer 
                        || controller.bot.inventory.containerWear.uid == targetContainer)
                        {
                ...
                if(controller.botSetup.infiniteAmmo)
                {
                    ...
                    if(looting != null)
                    {
                        if(looting is LootableCorpse)
                        {
                            ...
                            {
                                if(playerLoot.containerMain.uid == targetContainer 
                                    || playerLoot.containerBelt.uid == targetContainer 
                                        || playerLoot.containerWear.uid == targetContainer)
                                        {
        ...
        private void OnEntityBuilt(Planner plan, GameObject go)
        {
            ...
            foreach(var loopInfo in _config.installItem)
            {
                ...
                {
                    if(_existsControllers.ContainsKey(player.net.ID)) UnityEngine.Object.Destroy(_existsControllers[player.net.ID]);
        ...
        private void OnPlayerInput(BasePlayer player, InputState input)
        {
            ...
            if(input.WasJustPressed(BUTTON.USE))
            {
                ...
                if(Physics.Raycast(player.eyes.HeadRay(), out hit, 2f))
                {
                    ...
                    if(ent == controller.bot && ent != null && controller.owner == player) OpenInventory(player, controller);
                }
            }
        }
        ...
        private object OnDispenserGather(ResourceDispenser dispenser, BaseEntity entity, Item item)
        {
            ...
            if(bot != null)
            {
                BotOwnerComponent botComponent = GetOwnerComponent(bot.net.ID);
        ...
        private object CanLootEntity(BasePlayer player, LootableCorpse corpse)
        {
            ...
            {
                BotOwnerComponent ownerComponent = GetOwnerComponent(corpse.net.ID);
        ...
        private void OnLootEntity(BasePlayer player, BaseEntity entity)
        {
            ...
            {
                BotOwnerComponent ownerComponent = GetOwnerComponent(entity.net.ID);

                if(ownerComponent != null)
                {
                    if(player.userID != ownerComponent.botOwnerSteamID && !ownerComponent.canLootCorpse)
                    {
        ...
        private PlayerBotController GetController(uint netID) => _existsControllers.ContainsKey(netID) ? _existsControllers[netID] : null;
        private BotOwnerComponent GetOwnerComponent(uint netID) => _existsBots.ContainsKey(netID) ? _existsBots[netID] : null;
        ...
        private void SetupController(BasePlayer player, Configuration.BotSetup bot)
        {
            ...
            _existsControllers.Remove(player.net.ID);
            _existsBots.Remove(controller.bot.net.ID);

            _existsControllers.Add(player.net.ID, controller);
            _existsBots.Add(controller.bot.net.ID, controller.bot.GetComponent<BotOwnerComponent>());
        ...
        private void DropLoot(BasePlayer player, PlayerBotController controller, HitInfo info)
        {
            if(_config.enableBackpackOnDeath)
            {
                ...
                droppedContainer.TakeFrom(player.inventory.containerMain, player.inventory.containerWear, player.inventory.containerBelt);
                ...
                _existsBots.Remove(droppedContainer.net.ID);
                _existsBots.Add(droppedContainer.net.ID, botOwner);
            }
            else 
            {
                PlayerCorpse playerCorpse = player.DropCorpse("assets/prefabs/player/player_corpse.prefab") as PlayerCorpse;

                playerCorpse.TakeFrom(player.inventory.containerMain, player.inventory.containerWear, player.inventory.containerBelt);
                ...
                _existsBots.Remove(playerCorpse.net.ID);
                _existsBots.Add(playerCorpse.net.ID, botOwner);
        ...
        private void OpenInventory(BasePlayer player, PlayerBotController controller)
        {
            ...
            timer.Once(.25f, () => player.ClientRPCPlayer(null, player, "RPC_OpenLootPanel", "player_corpse"));
        }
        ...
        private void chatCommand(BasePlayer player, string command, string[] args) 
        {
            ...
            if(args[0] == "where")
            {
                ...
                SendMsg(player, "ChatCommand_Notice_Location", new string[] {PhoneController.PositionToGridCoord(controller.bot.transform.position), Vector3.Distance(controller.bot.transform.position, player.transform.position).ToString()} );
                return;
            }
            ...
            if(bots.Count != 0)
            {
                ...
                if(bot != null)
                {
                    ...
                    {
                        PlayerBotController comp = GetController(player.net.ID);
        ...
        public class BotAutoMode : MonoBehaviour
        {
            ...
            public enum Resources {All, None, Wood, Stone, Metal, Sulfur, Hemp, Berries, Corn, Pumpkin, Mushroom};
            ...
            public void AddResource(Resources resource) 
            {
                ...
                {
                    _resources = new List<string>() {"Wood", "Stone", "Metal", "Sulfur"};
                    if(_mode == AutoMode.Pickup) _resources.AddRange(new string[] {"Hemp", "Corn", "Berries", "Pumpkin", "Mushroom"});
                }
                else
                {
        ...
        public class PlayerBotController : FacepunchBehaviour
        {
            ...
            public Core.Plugins.Plugin plugin = null;
            ...
            private Configuration _config;
            private List<uint> _unreachableEntities = new List<uint>(); // List of entities id, which didn't gather because they were unreachable
            ...
            private float _lastTimeGathered, _lastTimeGUI, _nextShootTime, _lastConditionWhileGather;

            private bool _isIgnore, _isCombat, _isIdle, _isPVP;

            public UnityAction<BaseHelicopterVehicle> ControlHeli;
            public UnityAction<BaseHelicopterVehicle> MountedHeli;
            ...
            private static string[] _blacklistedEntities = {"cactus-3", "cactus-4", "cactus-5", "cactus-6", "cactus-7", "dead_log_a", "dead_log_b", "dead_log_c", "driftwood_1", "driftwood_2", "driftwood_3", "driftwood_4", "driftwood_5", "driftwood_set_1", "driftwood_set_2", "driftwood_set_3"};
                ...
                StartCoroutine(Timer(() =>
                {
                    ...
                    try 
                    {
                        if(_dispenserTarget != null)
                        {
                                        ...
                                        if(active != null && botSetup.gather.toolForStones.Contains(active.info.shortname))
                                        {
                                            ...
                                            else 
                                            {
                                                if(_lastTimeGathered + 4f < UnityEngine.Time.realtimeSinceStartup)
                                                {
                                                    ...
                                                    if(!mode.IsDisabled())
                                                    {
                                                        if(!_unreachableEntities.Contains(_dispenserTarget.baseEntity.net.ID)) _unreachableEntities.Add(_dispenserTarget.baseEntity.net.ID);
                                                    }
                                                }
                                            }
                                        }
                            ...
                            else 
                            {
                                if(_dispenserTarget.baseEntity.IsDestroyed)
                                {
                                    _dispenserTarget.AssignFinishBonus(bot, 0.5f);
                                    Nullify();
                            ...
                            if(vehicle != null)
                            {
                                if((enableCopterLocksAPI ? (((bool)Interface.Oxide.CallHook("API_CanAccessVehicle", bot, vehicle.GetEntity(), false)) == true) : true))
                                {
                                    if(vehicle.NumMounted() < vehicle.MaxMounted())
                                    {
                                        ...
                                        if(MountedHeli != null && vehicle is BaseHelicopterVehicle) MountedHeli.Invoke(vehicle as BaseHelicopterVehicle);
                    ...
                    if(bot.isMounted && owner.isMounted)
                    {
                        ...
                        if(vehicle != null)
                        {
                            ...
                            {
                                if(vehicle is BaseHelicopterVehicle)
                                {
                                    if(ControlHeli != null) ControlHeli.Invoke(vehicle as BaseHelicopterVehicle);
                                }
                    ...
                    if(_dispenserTarget == null && _lastTarget == null && !bot.isMounted && _collectibleTarget == null && !_isFollowPlayer && _corpseTarget == null && _containerTarget == null && _dispenserTarget == null && !_isIdle && _itemTarget == null)
                    {
            ...
            public bool StartAutoMode()
            {
                ...
                if(modeType == BotAutoMode.AutoMode.Farm)
                {
                    ...
                    var trees = GetFarmResourcesInRadius(true, false);
                    var stones = GetFarmResourcesInRadius(false, true);

                    Item axe = null, pickaxe = null;

                    if(_dispenserTarget != null)
                    {
                        if(trees.Contains(_dispenserTarget.baseEntity) || stones.Contains(_dispenserTarget.baseEntity)) return true;
                    }

                    if(resources.Contains("Wood") && trees.Length != 0)
                    {
                        ...
                        if(axe != null)
                        {
                            ...
                            var resource = trees[0];

                            _currentDestination = resource.transform.position;
                            SetDestination(_currentDestination);
                            _dispenserTarget = resource.GetComponent<ResourceDispenser>();
                    ...
                    var sulfur = FilterFarmEntities("sulfur.ore", stones);

                    if(resources.Contains("Sulfur") && sulfur.Length != 0)
                    {
                        ...
                        if(pickaxe != null)
                        {
                            ...
                            var resource = sulfur[0];

                            _currentDestination = resource.transform.position;
                            SetDestination(_currentDestination);
                            _dispenserTarget = resource;
                    ...
                    var stone = FilterFarmEntities("stones", stones);

                    if(resources.Contains("Stone") && stone.Length != 0)
                    {
                        if(pickaxe == null) pickaxe = EquipItem(false, false, true);
                    ...
                    var metal = FilterFarmEntities("metal.ore", stones);

                    if(resources.Contains("Metal") && metal.Length != 0)
                    {
                        if(pickaxe == null) pickaxe = EquipItem(false, false, true);
            ...
            private ResourceDispenser[] FilterFarmEntities(string shortname, BaseEntity[] ents)
            {
                List<ResourceDispenser> dispensers = new List<ResourceDispenser>();

                foreach(var ent in ents)
                {
                    var dispenser = ent.GetComponent<ResourceDispenser>();

                    if(dispenser != null)
                    {
                        if(_unreachableEntities.Contains(ent.net.ID) || _blacklistedEntities.Contains(ent.ShortPrefabName)) continue;

                        foreach(var item in dispenser.containedItems)
                        {
                            if(item.itemDef.shortname == shortname)
                            {
                ...
                return dispensers.ToArray();
            }

            private BaseEntity[] GetFarmResourcesInRadius(bool wood, bool stagedResourceEntity)
            {
                var colliders = Physics.OverlapSphere(mode.StartPos, botSetup.gather.autoModeRadius);
                List<BaseEntity> finded = new List<BaseEntity>();
                ...
                if(colliders.Length != 0)
                {
                    foreach(var collider in colliders)
                    {
                        ...
                        if(ent != null)
                        {
                            ...
                            if(ent is StagedResourceEntity && stagedResourceEntity) finded.Add(ent);
                            else if(ent.TryGetComponent<ResourceDispenser>(out dispenser) && wood)
                            {
                                if(dispenser.gatherType == ResourceDispenser.GatherType.Tree && !_blacklistedEntities.Contains(ent.ShortPrefabName)) finded.Add(ent);
                            }
                        }
                    }
                }
            ...
            private bool StartAttack()
            {
                if (_lastTarget != null)
                {
                    ...
                    {  
                        ...
                        {
                            if(targetPlayer.IsWounded()) dir -= new Vector3(0, 1f);
                        }
            ...
            public virtual bool ShotTest()
            {
                ...
                {                    
                    if (baseProjectile.primaryMagazine.contents <= 0)
                    {
                        ...
                        {
                            baseProjectile.primaryMagazine.Reload(bot);
                            ...
                            if(held != null)
                            {
                                if(held is BaseProjectile)
                                {
                                    ...
                                    if(projectile.primaryMagazine.contents <= 0)
                                    {
                                        if(!botSetup.infiniteAmmo)
                                        {
                                            projectile.primaryMagazine.Reload(bot);
            ...
            private void OnDestroy() 
            {
                ...
                if(owner != null) 
                {
                    ...
                    if(botSetup.deathMarker.enableMarker)
                    {
                        ...
                        if(DeathMarker.DeathMarkers.ContainsKey(owner.userID))
                        {
                            var found = DeathMarker.DeathMarkers[owner.userID];
                            if(found != null) UnityEngine.Object.Destroy(found.gameObject);

                            DeathMarker.DeathMarkers.Remove(owner.userID);
                        }
                    }
                }
            }
            ...
            private void RefreshMenu()
            {
                ...
                container.Add(new CuiElement 
                {
                    ...
                    Components = 
                    {
                        ...
                        {
                            Text = $"{PhoneController.PositionToGridCoord(bot.transform.position)}: {Mathf.RoundToInt(Vector3.Distance(bot.transform.position, owner.transform.position))}m",
                            FontSize = 14,
        ...
        {
            public static Dictionary<ulong, BaseNetworkable> DeathMarkers = new Dictionary<ulong, BaseNetworkable>();
            ...
            private void Start()
            {
                ...
                UpdateMarkers();
                DeathMarkers.Add(playerID, generic);
            }
        ...
        private bool HasBot(BasePlayer player) => GetController(player.net.ID) != null;
