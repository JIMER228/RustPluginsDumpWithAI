...
{
    [Info("AirfieldEvent", "Fruster", "1.3.2")]
    ...
    class AirfieldEvent : CovalencePlugin
    {
        ...
        [PluginReference] Plugin Kits, SimpleLootTable, ZoneManager;
        ...
        private GameObject mon = new GameObject();
        ...
        class ConfigData
        {
            ...
            [JsonProperty("The time after which the owner of the event will be deleted if he left the server(for PVE mode)")]
            public int ownerLeftTime = 300;
            [JsonProperty("Radius for event(for PVE mode)")]
            public int eventRadius = 380;
            [JsonProperty("Create a sphere for PVE mode")]
            public bool createSphere = false;
            [JsonProperty("Sphere offset")]
            public Vector3 sphereOffset = new Vector3(0, 0, 30);
            [JsonProperty("Do not allow other players into the event(only for PVE mode if there is a sphere)")]
            public bool ejectPlayers = false;
            [JsonProperty("Message when a player is ejected from the event sphere(only for PVE mode if there is a sphere)")]
            ...
            [JsonProperty("Drops amount(number of cargo spawns after plane landing, maximum 10)")]
            public int dropsAmount = 3;
            [JsonProperty("Crates amount(spawn every cargo drop)")]
            ...
            [JsonProperty("Airdrops amount(spawn every cargo drop)")]
            ...
            [JsonProperty("NPCs amount(spawn every cargo drop)")]
            ...
            [JsonProperty("Calls a helicopter to the airfield after landing a cargo plane")]
            public bool callHeli = false;
        ...
        private void OnServerInitialized()
        {
            ...
            {
                mon.transform.position = Configuration.customPosition;
                mon.transform.rotation = Quaternion.Euler(Configuration.customRotation.x, Configuration.customRotation.y, Configuration.customRotation.z);
                eventFlag = true;
            }
            else
                foreach (var monument in TerrainMeta.Path.Monuments)
                    ...
                    {
                        mon.transform.position = monument.transform.position;
                        mon.transform.rotation = monument.transform.rotation;
                        eventFlag = true;
                        break;
                    }
        ...
        void ChangeSpeed(int stage)
        {
            switch (stage)
            {
                ...
                case 30:
                    ...
                    speed = maxSpeed / 1.6f;
                    timer.Repeat(6.5f / (dropsAmount + 1), dropsAmount, () => { SpawnDrop(); });
                    break;
                case 31:
                    fix = false;
                    break;
        ...
        void SpawnDrop()
        {
            ...
            for (int i = 0; i < npcsAmount; i++)
            {
                ...
                player.name = npcName;
                player.displayName = Configuration.displayNameNPC;
                ...
                if (y < Configuration.displayNamesListNPC.Count)
                    if (Configuration.displayNamesListNPC[y] != null)
                        player.displayName = Configuration.displayNamesListNPC[y];
        ...
        public void NpcAI(BaseEntity npc)
        {
...

            {
                ...
                if (brain.Senses.Players[0])
                    if (Vector3.Distance(brain.Senses.Players[0].transform.position, npc.transform.position) < chaseRadius)
                    {
        ...
        void OnServerShutdown() { RemoveEvent(); }
        void Unload() { RemoveEvent(); }
        ...
        private void AddEventSphere()
        {
            ...
            for (float i = 0; i < 6.283185307f; i += 12f / radius)
            {
                ...
                Vector3 point = GroundPos(new Vector3(Convert.ToSingle(x), pos.y, Convert.ToSingle(z)));
                points.Add(point);
            }

            SphereEntity sphereEntity = GameManager.server.CreateEntity(spherePrefab, pos, Quaternion.identity) as SphereEntity;
            ...
            if (Configuration.ejectPlayers) AddTrigger.AddToEntity(sphereEntity);
        }
                            ...
                            if (owner.Team != null)
                                if (owner.Team.members.Count > 1)
                                    foreach (BasePlayer findPlayer in BasePlayer.activePlayerList)
                                        if (owner.Team.members.Contains(findPlayer.userID) && findPlayer.IsConnected)
                                        {
