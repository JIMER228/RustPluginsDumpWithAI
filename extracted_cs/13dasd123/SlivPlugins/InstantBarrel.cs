...
{
    [Info("Instant Barrel", "Tryhard", "1.2.5")]
    ...
    public class InstantBarrel : RustPlugin
    {
        ...
        private readonly HashSet<string> lootBarrelsNames = new HashSet<string> { "loot_barrel_1", "loot_barrel_2", "loot-barrel-1", "loot-barrel-2", "oil_barrel", "roadsign1", "roadsign2", "roadsign3", "roadsign4", "roadsign5", "roadsign6", "roadsign7", "roadsign8", "roadsign9" };
        ...
        private object OnEntityTakeDamage(LootContainer lootContainer, HitInfo hitInfo)
        {
            ...
            if (!lootBarrelsNames.Contains(lootContainerName))
                return null;
            ...
            {
                switch ((int)player.modifiers.GetValue(Modifier.ModifierType.Scrap_Yield))
                {
                    case 1:
                        scrapAmount = 4;
                        break;
            ...
            for (int i = itemContainer.itemList.Count - 1; i >= 0; i--)
            {
                ...
                if (scrapAmount != 0 && item.info.itemid == -932201673)
                {
                    item.amount = scrapAmount;
                }   
