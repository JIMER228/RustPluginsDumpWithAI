...
{
    [Info("Skill Tree", "imthenewguy", "1.5.0")]
    ...
    class SkillTree : RustPlugin
    {
        ...
        void StopSavingPlayerData(bool unloading)
        {
            ...
            if (unloading && Saving != null) Pool.FreeList(ref Saving);
        }
        ...
        void LoadData()
        {
            if (CheckOldDataStructure()) return;
        ...
        PlayerInfo LoadPlayerData(string userID, bool createNew = true)
        {
            ...
            if (!File.Exists(NewDirectory + userID + ".json"))
            {
                ...
                PlayerDataLogs += $"\nFound no data for {userID}, creating it.";
...
#if CARBON
		string FileDirectory = Carbon.Core.Defines.GetDataFolder() + "/";
        string NewDirectory = Carbon.Core.Defines.GetDataFolder() + "/SkillTree/";
#else
        string FileDirectory = "oxide/data/";
        string NewDirectory = "oxide/data/SkillTree/";
#endif
        const string FileName = "SkillTree.json";

        #region Handle Data Conversion

        bool HasOldData;

        bool CheckOldDataStructure()
        {
            if (!File.Exists(FileDirectory + FileName))
            {
                Puts($"File does not exist: {FileDirectory + FileName}");
                return false;
            }
            var json = File.ReadAllText(FileDirectory + FileName);

            var OldData = JsonConvert.DeserializeObject<PluginInfoOld>(json);
            if (OldData == null || OldData.pEntity == null || OldData.pEntity.Count == 0)
            {
                // Using new data structure or their current data is empty.
                return false;
            }

            File.WriteAllText(FileDirectory + $"SkillTree_Backup_{DateTime.Now.ToString("yyyyMMddd_HHmmss")}.json", json);

            HasOldData = true;           
            pcdData = new PluginInfo(OldData.wipeTime, OldData.highest_player == null ? 0 : (OldData.highest_player is ulong) ? (ulong)OldData.highest_player : ulong.TryParse(OldData.highest_player as string, out var useridStr) ? Convert.ToUInt64(useridStr) : 0ul, OldData.pEntity);
            Puts($"Loading data [{pcdData.pEntity.Count}] using old structure.");

            // There is data that we need to convert.
            return true;
        }

        public IEnumerator ConvertOldData(bool initial)
        {
            if (!HasOldData || pcdData.pEntity.Count == 0)
            {
                HasOldData = false;
                yield break;
            }
            Puts($"Starting data conversion for {pcdData.pEntity.Count} players.");
            var count = 0;

            if (!Directory.Exists(NewDirectory)) Directory.CreateDirectory(NewDirectory);
            string log = "Successfully converted data for:";
            List<KeyValuePair<ulong, PlayerInfo>> playerDATA = Pool.GetList<KeyValuePair<ulong, PlayerInfo>>();
            playerDATA.AddRange(pcdData.pEntity);
            foreach (var playerData in playerDATA)
            {
                try
                {
                    var data_json = JsonConvert.SerializeObject(playerData.Value, Formatting.Indented);
                    File.WriteAllText(NewDirectory + $"{playerData.Key}.json", data_json);
                    log += $"\n- {playerData.Key} - Level: {playerData.Value.achieved_level}. XP: {playerData.Value.xp}";
                }
                catch
                {
                    Puts($"Failed to serialize data for {playerData.Key}");
                }
            ...
            LogToFile("Conversion_Results", log, this);
            Pool.FreeList(ref playerDATA);

            Puts($"Finished converting data for {Directory.GetFiles(NewDirectory)?.Length ?? 0} players.");
            HasOldData = false;
            SaveData();
        ...
        public class ScoreboardInfo
        {
            ...
            {
                orderedList.Clear();
                orderedList.AddRange(data.OrderByDescending(x => x.Value.xp));
            }
        ...
        void OnNewSave(string filename)
        {
            ...
            List<string> files = Pool.GetList<string>();
        ...
        void OnPlayerHealthChange(BasePlayer player, float oldValue, float newValue)
        {
            ...
            if (buffDetails.TryGetValue(player.userID, out var bd) && bd.buff_values.TryGetValue(Buff.Heal_Share, out var value)) HandleHPShare(player, oldValue, newValue, value);
        }
        ...
        void OnEntityBuilt(Planner plan, GameObject go)
        {
            ...
            if (plant != null)
            {
                ...
                if ((item == null || item.info.shortname != "clone.hemp") && buffDetails.TryGetValue(player.userID, out bd) && bd.buff_values.ContainsKey(Buff.Harvester_Ultimate) && IsUltimateEnabled(player, Buff.Harvester_Ultimate) && pcdData.pEntity.TryGetValue(player.userID, out pi))
                {
        ...
        {
            BuffDetails bd;
            if (buffDetails.TryGetValue(player.userID, out bd) && bd.buff_values.TryGetValue(Buff.Loot_Pickup, out var value) && entity.inventory?.itemList != null && entity.inventory.itemList.Count > 0)
            {
        ...
        void AddItemsToBarrel(ItemDefinition itemDef, int quantity, LootContainer entity)
        {
            ...
            if (item == null)
            {
                Puts($"Tried to place a null item into container. Check config settings for: {itemDef.shortname}");
                return;
            }
        ...
        void HandleMetabolismSharing(BasePlayer player, Item item, ItemModConsumable consumable, float Modifier)
        {
            ...
            foreach (var _player in nearby)
            {
                ...
                PrintToChat(player, $"You shared some of your calories with {_player.displayName}");
                PrintToChat(_player, $"You received some calories from {player.displayName}");
            }
        ...
        void HandleTeaSharing(BasePlayer player, Item item, ItemModConsumable consumable, float value)
        {
            ...
            foreach (var _player in nearby)
            {
                ...
                if (!HasBetterBuff)
                {
                    ...
                    PrintToChat(player, $"You shared {value * 100}% of your tea with {_player.displayName}");
                    PrintToChat(_player, $"{player.displayName} shared {value * 100}% of their tea with you");
                }
            }
        ...
        {
            List<ModifierDefintion> modsToAdd = Pool.GetList<ModifierDefintion>();
        ...
        void OnPlayerDeath(BasePlayer player, HitInfo info)
        {
            ...
            if (info.InitiatorPlayer != null && !info.InitiatorPlayer.IsNpc && info.InitiatorPlayer.userID.IsSteamId())
            {
                switch (player.GetType().Name)
                {
                    ...
                    default:
                        if (config.xp_settings.xp_loss_settings.pvp_death_penalty > 0 && PlayerIsReal(player))
                        {
        ...
        void HandleResurrection(BasePlayer player, BaseEntity attacker)
        {
            ...
            {
                if (!config.ultimate_settings.ultimate_medical.allow_in_pvp_death)
                {
        ...
        [PluginReference]
        private Plugin ImageLibrary, Economics, ServerRewards, EventManager, BotReSpawn, Cooking, UINotify, ZombieHorde, EventHelper, RaidableBases, LootDefender, SkillTreeXPEvent;
        ...
        void OnServerInitialized(bool initial)
        {
            if (HasOldData)
            {
                ServerMgr.Instance.StartCoroutine(ConvertOldData(initial));
                return;
            }
        ...
        int CheckLevel(BasePlayer player)
        {
            ...
            if (playerData.current_level < level)
            {
                ...
                if (config.general_settings.max_player_level == 0 || level <= config.general_settings.max_player_level)
                {
                    ...
                    if (playerData.prestige_level == 0 || !config.prestige_settings.ignore_level_rewards) GiveLevelRewards(player, level, level - playerData.current_level);
        ...
        void RunResetCommands(string playerID, int level_achieved)
        {
            ...
            {
                if (level.Key > level_achieved) continue;
        ...
        private static List<BasePlayer> FindNearbyTeamMates(BasePlayer player, float distance)
        {
            ...
            List<BasePlayer> result = Pool.GetList<BasePlayer>();
        ...
        void HandleSharedXP(BasePlayer player, double value, BaseEntity source = null, bool noMod = false, string source_string = null)
        {
            ...
            if (player.Team == null || !buffDetails.TryGetValue(player.userID, out var bd) || !bd.buff_values.TryGetValue(Buff.XP_Share, out var shareMod)) return;
        ...
        void HandlePerms(string id, string tree, string node, int level)
        {
            ...
            try
            {
                if (config.trees.TryGetValue(tree, out var ti) && ti.nodes.TryGetValue(node, out var ni) && ni.permissions != null)
                {
                    ...
                    if (!Tracked_perms.TryGetValue(id, out var playerPerms)) Tracked_perms.Add(id, new Dictionary<string, Dictionary<string, string>>());
        ...
        void UpdateInstancedData(BasePlayer player)
        {
            ...
            foreach (var cfg in config.trees)
            {
                ...
                var points_should_have = config.general_settings.points_per_level * playerData.achieved_level;
        ...
        void SendSkillTreeMenu(BasePlayer player, string tree = null, string selected_node = null)
        {
            ...
            if (config.prestige_settings.enabled && config.general_settings.max_player_level > 0 && (config.prestige_settings.show_button_before_level || playerData.current_level >= config.general_settings.max_player_level) && playerData.prestige_level < GetHighestPrestigeLevel)
            {
                ...
                container.Add(new CuiElement
                {
                    ...
                    Parent = "SkillTree",
                    Components = {
                    new CuiTextComponent { Text = lang.GetMessage("UIPrestigeUpButton", this, player.UserIDString), Font = "robotocondensed-bold.ttf", FontSize = 10, Align = TextAnchor.MiddleCenter, Color = "1 1 1 1" },
                    new CuiOutlineComponent { Color = "0 0 0 0.5", Distance = "1 -1" },
        ...
        void GiveXPToAllConsole(ConsoleSystem.Arg arg)
        {
            ...
            {
                arg.ReplyWith("Usage: givexptoall <amount> <online only: true/false>");
                return;
            }

            foreach (var p in BasePlayer.activePlayerList)
            {
                AwardXP(p, amount, null, false, false, "command");
            }
            ...
            OfflineXP_Routine = ServerMgr.Instance.StartCoroutine(GiveOfflinePlayersXP(amount, arg));                        
        }

        Coroutine OfflineXP_Routine;
        bool IsGivingOfflineXP = false;
        IEnumerator GiveOfflinePlayersXP(double amount, ConsoleSystem.Arg arg)
        {
            var count = 0;
        ...
        float GetBuffValue(BasePlayer player, string skill)
        {
            ...
            if (buffDetails.TryGetValue(player.userID, out buffData) && buffData.buff_values.TryGetValue(buff, out result)) return result;
        ...
        void UpdateScoreBoard(bool initialize)
        {
            if (initialize)
            {
                ...
                foreach (var file in Directory.GetFiles(NewDirectory))
                {
                    try
                    {
                        ...
                        data.Add(Convert.ToUInt64(userid), new ScoreboardInfo.ScoreInfo(obj.name ?? userid, obj.xp));
                    }
                    catch
                    {
                ...
                foreach (var playerData in pcdData.pEntity)
                {
                    ...
                    data[playerData.Key] = new ScoreboardInfo.ScoreInfo(playerData.Value.name ?? playerData.Key.ToString(), playerData.Value.xp);
                }
            ...
            foreach (var player in BasePlayer.activePlayerList)
            {
                if (!pcdData.pEntity.TryGetValue(player.userID, out var pi)) continue;
                if (ScoreBoard.data.TryGetValue(player.userID, out var scoreData)) scoreData.xp = pi.xp;
                else ScoreBoard.data.Add(player.userID, new ScoreboardInfo.ScoreInfo(player.displayName, pi.xp));
            }    
            ScoreBoard.UpdateScores();
        }
        ...
        private void ScoreBoardPanel(BasePlayer player, int lastElement = 0)
        {
            ...
            for (int i = lastElement; i < lastElement + 15; i++)
            {
                ...
                container.Add(new CuiElement
                {
                    ...
                    Parent = "ScoreBoardPanel",
                    Components = {
                    new CuiTextComponent { Text = ScoreBoard.orderedList.ElementAt(i).Value.name, Font = "robotocondensed-regular.ttf", FontSize = 14, Align = TextAnchor.UpperCenter, Color = "0.9137255 0.8039216 0 1" },
                    new CuiOutlineComponent { Color = "0 0 0 0.5", Distance = "1 -1" },
                ...
                container.Add(new CuiElement
                {
                    ...
                    Parent = "ScoreBoardPanel",
                    Components = {
                    new CuiTextComponent { Text = $"{ScoreBoard.orderedList.ElementAt(i).Value.xp:###,###,##0}", Font = "robotocondensed-regular.ttf", FontSize = 14, Align = TextAnchor.UpperLeft, Color = "0 0.8666667 0.8509804 1" },
                    new CuiOutlineComponent { Color = "0 0 0 0.5", Distance = "1 -1" },
        ...
        void RemoveAnimalBuff(BasePlayer player, bool timed_out = false)
        {
            ...
            if (BuffedPlayers.TryGetValue(player, out _buffs))
            {
                ...
                if (timed_out && player != null && player.IsAlive() && player.IsConnected) Player.Message(player, lang.GetMessage("AnimalBuffFinishedMsgNew", this, player.UserIDString), config.misc_settings.ChatID);
            }
        }
        ...
        void HandleSonarBuff(BasePlayer player)
        {
            ...
            BuffDetails bd;
            if (!buffDetails.TryGetValue(player.userID, out bd) || !bd.buff_values.ContainsKey(Buff.Sonar)) return;
            ...
            List<BaseEntity> validEntities = Pool.GetList<BaseEntity>();
            ...
            Pool.FreeList(ref validEntities);
        }
        ...
        private void Loaded()
        {
            Saving = Pool.GetList<KeyValuePair<ulong, PlayerInfo>>();
            LoadData();
        ...
        {
            List<string> groupUsers = Pool.GetList<string>();
            ...
            Pool.FreeList(ref groupUsers);
        }

        string GetPrestigeBetterChatTitle(ulong id)
        {
            PlayerInfo playerData;
            if (!pcdData.pEntity.TryGetValue(id, out playerData)) return null;
            ...
            return string.Format(highest.Value.better_title_format, highest.Value.better_title_default_col, highest.Key);
        }
        ...
        private void PrestigeConfirmation(BasePlayer player, int nextLevel, bool hasLevel)
        {
            ...
            container.Add(new CuiElement
            {
                ...
                Parent = "PrestigeConfirmation",
                Components = {
                    new CuiTextComponent { Text = "The prestige system will allow you to reset your Skill Tree progress and start from the beginning, but with additional benefits. <color=#ec3209>This process cannot be undone!</color>\n\n<b>You will be provided with the following benefits:</b>\n" + benefits, Font = "robotocondensed-regular.ttf", FontSize = 12, Align = TextAnchor.UpperLeft, Color = "1 1 1 1" },
                    new CuiOutlineComponent { Color = "0 0 0 0.5", Distance = "1 -1" },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-158.492 -133.651", OffsetMax = "158.492 119.6" }
                }
            });
            ...
            if (hasLevel)
            {
                container.Add(new CuiButton
                {
                    ...
                    Text = { Text = "CONFIRM", Font = "robotocondensed-bold.ttf", FontSize = 14, Align = TextAnchor.MiddleCenter, Color = "0.07280672 1 0 1" },
                    RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-80 -163.65", OffsetMax = "-20 -133.65" }
                }, "PrestigeConfirmation", "Confirm");
            }

            container.Add(new CuiButton
            {
                Button = { Color = "1 1 1 0", Command = "closeprestige" },
                Text = { Text = "CANCEL", Font = "robotocondensed-bold.ttf", FontSize = 14, Align = TextAnchor.MiddleCenter, Color = "1 0.3820755 0.3820755 1" },
                RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "20 -163.65", OffsetMax = "80 -133.65" }
            }, "PrestigeConfirmation", "Cancel");
        ...
        void SpawnWorkbench(int level)
        {
            ...
            var workbench = GameManager.server.CreateEntity(prefab, Vector3.zero) as Workbench;
            if (workbench == null)
            {
                return;
            }
        ...
        public class MobileWorkbenchSkill : MonoBehaviour
        {
            ...
            private void CheckForNewPlayers()
            {
                ...
                if (!hasTeam)
                {
                    if (!TrackedPlayers.ContainsKey(player)) TrackedPlayers.Add(player, AddTriggerToPlayer(player));
                    return;
                }
