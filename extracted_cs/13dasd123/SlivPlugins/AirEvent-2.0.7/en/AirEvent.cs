...
namespace Oxide.Plugins
{
    ...
    internal class AirEvent : RustPlugin
    {
        ...
        internal class ControllerAirEvent : FacepunchBehaviour
        {
            ...
            private IEnumerator SpawnEntities()
            {
                ...
                if (_ins._config.PveMode.Pve && _ins.plugins.Exists("PveMode"))
                {
                    ...
                    _ins.PveMode.Call("EventAddPveMode", _ins.Name, config, transform.position, _ins.Radius, crates, Scientists.Select(x => x.net.ID.Value), new HashSet<ulong>(), new HashSet<ulong>(), new HashSet<ulong>(), null);
                }
                InvokeRepeating(ChangeToFinishTime, 1f, 1f);
                Interface.Oxide.CallHook("OnAirEventStart", Entities, transform.position, _ins.Radius);
            }
        ...
        {
            if (_controller == null) return null;
            if (_controller.Crates.Any(x => x.IsExists() && x.net.ID.Value == netID))
            {
