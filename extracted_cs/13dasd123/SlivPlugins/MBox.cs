    ...
    public class MBox : RustPlugin
    {
        ...
        private void OnLootEntityEnd(BasePlayer player, LootContainer container)
        {
            ...
            if (container.GetComponent<BoxComponent>())
            {
                ...
                {
                    Server.Broadcast($"<color=#ff2400>МЕГАЯЩИК</color> был залутан игроком <color=#ff2400>{player.displayName}</color>");
        ...
        bool GenerateBox()
        {
            ...
            NotifyOpen("<color=#d3d3d3>МЕГАЯЩИК!</color>", $"<color=#ff2400>Мегаящик</color> заспавнился на <color=#ff2400>{spawnMonument.NameMonument}</color>. До открытия <color=#ff2400>{TimeExtensions.FormatShortTime(TimeSpan.FromSeconds(config.BoxToOpen))}</color>");
