...
{
    [Info("Clans", "TopPlugin.ru", "1.1.0", ResourceId = 14)]
    public class Clans : RustPlugin
    {
        ...
        protected override void LoadDefaultMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string> {
                ...
                ,
                {
                 ...
                 {
                    "clanUItitle", "Clan System by TopPlugin"
                },
                  {
        ...
        private void DoCleanUp(BasePlayer player)
        {
            ...
            if (clan != null)
            {
                ...
                if (enableClanTagging)
                {
                    ...
                    if (player.net != null)
                        player._name = string.Format("{1}[{0}/{2}]", player.net.ID.Value, name, player.userID);
                }
        ...
        void cmdClansGetSkinList(ConsoleSystem.Arg args)
        {
            ...
            if (!int.TryParse(args.Args[1], out page))
                page = 0;
            if (!SkinList.ContainsKey(args.Args[0]))
            {
                return;
            }
        ...
        {
            if (info == null || entity?.net?.ID.Value == null) return;
