...
namespace Oxide.Plugins
{
     ...
     * - Fix discord payload to large error.
     * - Fix console record command not adhereing to minutes passed.
     */
    [Info("Auto Demo Record", "Pho3niX90", "1.2.72")]
    ...
    internal class AutoDemoRecord : RustPlugin
    {
        ...
        private Dictionary<ulong, BlacklistEntry> _blacklistPlayers = new Dictionary<ulong, BlacklistEntry>();
        private ARConfig config;
        int lastSavedCount = 0;
        int lastSavedCount_blacklist = 0;
        int _discordSize = 7581863;
        //int _discordSize = 500000;
        ...
        private void cmdRecord(BasePlayer player, string command, string[] args)
        {
            ...
            API_StartRecordingWithWebhook(player, $"Player {player.displayName} initiated self recording.",
        ...
        {
            ulong playerId;
            int minutes = config.AR_Report_Length;

            if (ulong.TryParse(arg.Args[0], out playerId) && int.TryParse(arg.Args[1], out minutes))
            {
                BasePlayer player = BasePlayer.FindByID(playerId);
                API_StartRecording4(player, $"Recording started of {player.displayName}.", minutes, config.AR_Discord_Webhook);
            }
            else
            {
        ...
        void _recordOnInterval(ulong playerId, BlacklistEntry entry)
        {
            ...
            if (player != null && player.IsConnected)
                API_StartRecordingWithLength(player, $"Recording started of {player.displayName}.", entry.recordingMinutes);
        }
                ...
                case "add":
                        ...
                        int.TryParse(arg.Args[2], out minutes) && int.TryParse(arg.Args[3], out interval))
                    {
                        ...
                        else
                        {
                            ...
                            var _blacklistPlayers = new ConcurrentDictionary<ulong, AutoDemoRecord.BlacklistEntry>();
        ...
        private void API_StartRecording(BasePlayer player, string reason, int length) => StartRecording(player, null, reason, null, length);
        private void API_StartRecording(BasePlayer player, string reason, string webhook) => StartRecording(player, null, reason, webhook);
        private void API_StartRecording(BasePlayer player, string reason, int length, string webhook) => StartRecording(player, null, reason, webhook, length);

        // broken method overloading fixes
        private void API_StartRecordingWithLength(BasePlayer player, string reason, int length) => StartRecording(player, null, reason, null, length);
        private void API_StartRecordingWithWebhook(BasePlayer player, string reason, string webhook) => StartRecording(player, null, reason, webhook);
        private void API_StartRecording4(BasePlayer player, string reason, int length, string webhook) => StartRecording(player, null, reason, webhook, length);

        void StartRecording(BasePlayer player, ARReport report = null, string reason = null, string webhook = null,
            int length = 0)
        {
            Puts("API_StartRecording called");
            if (player.Connection.IsRecording)
            {
            ...
            if (length > 0)
            {
                _timers[player.userID] = timer.Repeat(10, recordingTime / 10,
                    () => MonitorSplit(player, _timers[player.userID], webhook));
            }
        }
        ...
        void StopRecording(BasePlayer player, string wh = "")
        {
            if (player.Connection.IsRecording)
            {
                ...
                {
                    if (!wh.IsNullOrEmpty() || !GetWebhook(false).IsNullOrEmpty())
                    {
        ...
        void MonitorSplit(BasePlayer player, Timer timer, string wh)
        {
            Connection conn = player.Connection;
            int recordLength = config.AR_Report_Length * 60;

            if (conn.IsRecording)
            {
                if ((recordLength <= GetTotalSecondsRecorded(conn.userid)) || timer.Destroyed)
                {
        ...
        byte[] GetDemoFileBytes(Connection conn)
        {
            ...
            // Temporary reflection until fields are exposed.
            var recordHeader = typeof(Connection)
                .GetField("recordHeader", BindingFlags.Instance | BindingFlags.NonPublic).GetValue(conn);
            if (recordHeader != null)
            {
                // Manually converting data to a byte array since System.IO is unavailable to us.
                var demoRecordHeader = (recordHeader as Demo.Header);
        ...
        private void GenCombatLog(BasePlayer target, string filename, int logSize = 30)
        {
            ...
            timer.Once(delay, () =>
            {
                foreach (CombatLog.Event evt in cLog)
                {
                    ...
                    string attackerName = isPlayer ? target.displayName : UintFind((uint)evt.attacker_id);

                    string attacker_id = isPlayer ? target.UserIDString : evt.attacker_id.ToString();

                    isPlayer = evt.target == "you";

                    string targetName = isPlayer ? target.displayName : UintFind((uint)evt.target_id);
                ...
                if (config.AR_Upload && !filename.IsNullOrEmpty())
                {
                    if (config.AR_SaveRecording)
                    {
                    ...
                    {
                        Puts(
                            $"Cannot upload combatlog due to an error: size={textTable.ToString().Length}, msg={e.Message}");
                    }
                }
            });
        }
        ...
        private string UintFind(uint id)
        {
            BasePlayer player = null;
            try
            {
                player = BasePlayer.activePlayerList.First(x => x.net.ID.Value == id);
            }
            catch (Exception )
            {
            }
        ...
        public double GetTotalSecondsRecorded(ulong userid)
        {
            return _pendingUploads.Find(x => x.userID == userid).GetTotalSeconds();
        }
        ...
        {
            _pendingUploads.Find(x => x.userID == userid).AddReason(reason);
        }
