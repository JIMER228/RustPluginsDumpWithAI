...
{
    [Info("PlayerSkins", "k1lly0u", "3.0.4")]
    class PlayerSkins : ChaosPlugin
    {
                ...
                else
                {
                    foreach (KeyValuePair<string, Hash<ulong, SkinData>> kvp in m_SkinData.Data)
                    {
                        foreach (KeyValuePair<ulong, SkinData> idData in kvp.Value)
                        {
                            ...
                            {
                                if (uiUser.ShowAvailable && !string.IsNullOrEmpty(idData.Value.permission) && !uiUser.Player.HasPermission(idData.Value.permission))
                                    continue;
            ...
            else
            {
                ...
                else
                {
                    foreach (KeyValuePair<ulong, SkinData> skin in skinList)
                    {
                        ...
                        {
                            if (uiUser.ShowAvailable && !string.IsNullOrEmpty(skin.Value.permission) && !uiUser.Player.HasPermission(skin.Value.permission))
                                continue;
        ...
        private void CreateFooterToggles(UIUser uiUser, BaseContainer parent, bool minimal)
        {
            ...
            ImageContainer.Create(parent, Anchor.CenterLeft, minimal ? new Offset(90f, -10f, 110f, 10f) : new Offset(295f, -10f, 315f, 10f))
                .WithStyle(m_ButtonStyle)
                .WithChildren(ownedToggle =>
                {
                    ...
                    TextContainer.Create(ownedToggle, Anchor.CenterRight, new Offset(5f, -10f, 65f, 10f))
        ...
        protected override void OnConfigurationUpdated(VersionNumber oldVersion)
        {
            ...
            if (oldVersion < new VersionNumber(3, 0, 4))
                (ConfigurationData as ConfigData).Shop.SkinRefund = 100;
        }
