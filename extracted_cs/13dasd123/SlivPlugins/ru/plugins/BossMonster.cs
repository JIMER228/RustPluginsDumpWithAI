...
{
    [Info("BossMonster", "KpucTaJl", "2.1.2")]
    internal class BossMonster : RustPlugin
    {
        ...
        internal class ControllerBoss : FacepunchBehaviour
        {
            ...
            private static bool CanTargetPlayer(BasePlayer target) => !(target.IsSleeping() || target.IsWounded() || target._limitedNetworking);
            ...
            private void RadiusActions()
            {
                ...
                if (Players.Count == 0 || !Players.Any(CanTargetPlayer)) return;
                ...
                if (_timeToAnimal == 0)
                {
                    ...
                    if (target == null || !CanTargetPlayer(target)) return;
                    _timeToAnimal = radiusActions.AnimalAbility.Time;
                ...
                if (_timeToNpc == 0)
                {
                    ...
                    for (int i = 0; i < radiusActions.NpcAbility.Count; i++)
                    {
                        ...
                        if (npc != null)
                        {
                            Interface.Oxide.CallHook("OnBossSpawnedAdditionalNpc", Npc, npc);
                            Scientists.Add(npc);
                        }
                    }
