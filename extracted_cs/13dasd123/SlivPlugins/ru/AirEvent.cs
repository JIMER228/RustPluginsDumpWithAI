...
namespace Oxide.Plugins
{
    ...
    internal class AirEvent : RustPlugin
    {
        ...
        private void UpdateConfigValues()
        {
            ...
            {
                _config.Radius = 50f;
                _config.Marker.Name = "AirEvent ({time})";
            }
            _config.PluginVersion = Version;
            SaveConfig();
        ...
        private void OnEntitySpawned(DroppedItemContainer container) { if (container != null && Vector3.Distance(_controller.transform.position, container.transform.position) < _config.Radius) _controller.Backpacks.Add(container); }

        private void OnEntitySpawned(SimpleShark shark) { if (shark.IsExists() && Vector2.Distance(new Vector2(_controller.transform.position.x, _controller.transform.position.z), new Vector2(shark.transform.position.x, shark.transform.position.z)) < _config.Radius) shark.Kill(); }
        ...
        private object OnSamSiteTarget(SamSite entity, HotAirBalloon target)
        {
            ...
            if (_controller.AirBalloons.Contains(target)) return true;
            else return null;
        }
        ...
        internal class ControllerAirEvent : FacepunchBehaviour
        {
            ...
            private void FixedUpdate() { foreach (HotAirBalloon airBalloon in AirBalloons) airBalloon.inflationLevel = 0.9f; }
            ...
            private IEnumerator SpawnEntities()
            {
                ...
                Interface.Oxide.CallHook("OnAirEventStart", Entities, transform.position, _ins._config.Radius);
            }
