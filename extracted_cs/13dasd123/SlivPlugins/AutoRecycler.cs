...
{
    [Info("Auto Recycler", "M&B-Studios", "1.0.5")]
    public class AutoRecycler : RustPlugin
    {
        ...
        #region Fields
        private const string Layer = "ui.AutoRecycler.bg";

        private const string PermAdmin = "autorecycler.admin";
        private const string PermAutorec = "autorecycler.autorec";
        private const string PermRecvirtual = "autorecycler.recvirtual";
        private const string PermRecyclercratetest = "autorecycler.recyclercratetest";
        private const string PermCharacteristics = "autorecycler.characteristics";
        private const string PermRecboxBuy = "autorecycler.recboxbuy";
   ...
   ["pistol.m92"] = new()
            ...
            ["ammo.rocket.seeker"] = new()
    {
        ...
        {
            SaveData();
            BasePlayer.activePlayerList.ToList().ForEach(UI_Destroy);
        ...
        {
            var posy = p.transform.position.y > 500 ? Vector3.down * 40 : Vector3.down * 500;
            
            Recycler r =
                GameManager.server.CreateEntity("assets/bundled/prefabs/static/recycler_static.prefab", p.transform.position + posy) as
                    Recycler;
            if (r == null) return;
            ...
            _recyclers.Add(r.net.ID.Value, new EntityAndPlayer { Entity = r, Player = p });
        }
        ...
        private IEnumerable<KeyValuePair<string, List<ItemAmount>>> GetItemsByCategory(BasePlayer player, ItemCategory category)
        {
        ...
        private void RecycleItem(BasePlayer player, Item item, List<ItemAmount> recycleItems, StorageContainer sc = null)
        {
        ...
        private void UI_UpdateItem(BasePlayer player, string shortname)
        {
            ...
            }, Layer + ".main" + ".items.div" + $".item.{shortname}.bg", Layer + ".main" + ".items.div" + $".item.{shortname}" + ".enabled");
            ...
            if (CanUseAction(player, ActionType.Characteristics))
            {
                ...
                container.Add(new CuiButton
                {
                    Button = { Color = isEnabled ? "0.09545212 0.3679245 0.1133182 0.8235294" : "1 0.03030553 0 0.8235294", Command = $"ar.togglestate {shortname} {!isEnabled}" },
                    Text =
                    {
                        Text = isEnabled ? "ENABLED" : "DISABLED", Font = "robotocondensed-bold.ttf", FontSize = 13,
                        Align = TextAnchor.MiddleCenter, Color = isEnabled ? "0.263648 1 0 1" : "0.6226415 0.1380384 0.1380384 1"
                    },
                    RectTransform =
                    {
                ...
                }, Layer + ".main" + ".items.div" + $".item.{shortname}.bg", Layer + ".autorecycler" + ".main" + ".items.div" + $".item.{shortname}" + ".adminbtn");
            }
        ...
        private void UI_DrawSearch(BasePlayer player)
        {
            ...
            container.Add(new CuiPanel
            {
                ...
                RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-104.786 -46.797", OffsetMax = "104.786 -20.326" }
            ...
            container.Add(new CuiElement
            {
                ...
                Components = {
                    new CuiInputFieldComponent { Color = "0.8078431 0.7803922 0.7411765 1", Font = "robotocondensed-bold.ttf", FontSize = 14, Align = TextAnchor.MiddleCenter, CharsLimit = 20, IsPassword = false, NeedsKeyboard = true },
                    new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-96.1 -13.235", OffsetMax = "96.172 13.236" }
                }
            });
        ...
        private void UI_DrawMain(BasePlayer player)
        {
            ...
            container.Add(new CuiPanel
            {
                ...
                RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-434.913 -260.419", OffsetMax = "434.913 260.419" }
            }, "Overlay", Layer);
            ...
            container.Add(new CuiPanel
            {
                CursorEnabled = false,
                Image = { Color = "1 1 1 0" },
                RectTransform = { AnchorMin = "0 0.5", AnchorMax = "0 0.5", OffsetMin = "0.09 -260.419", OffsetMax = "660.17 260.421" }
            }, Layer, Layer + ".main");
            ...
            container.Add(new CuiPanel
            {
                CursorEnabled = false,
                Image = { Color = "1 1 1 0" },
                RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-330.04 219.767", OffsetMax = "330.04 260.42" }
            }, Layer + ".main", Layer + ".main" + ".header.div");

            container.Add(new CuiButton
            {
                Button = { Color = "1 1 1 0" },
                Text = { Text = "SEARCH", Font = "robotocondensed-bold.ttf", FontSize = 29, Align = TextAnchor.MiddleCenter, Color = "0.8078431 0.7803922 0.7411765 1" },
                RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-76.091 -20.326", OffsetMax = "76.091 20.326" }
        ...
        private void UI_DrawItems(BasePlayer player, ItemCategory activeCategory, int page = 0)
        {
            ...
            container.Add(new CuiPanel
            {
                ...
                RectTransform = { AnchorMin = "0 0.5", AnchorMax = "0 0.5", OffsetMin = "-0.04 -226.279", OffsetMax = "660.04 219.771" }
            ...
            foreach (var item in GetItemsByCategory(player, activeCategory).Skip(page * ItemsPerPage).Take(ItemsPerPage))
            {
        ...
        private void UI_DrawPages(BasePlayer player, ItemCategory activeCategory, int page = 0)
        {
            ...
            container.Add(new CuiPanel
            {
                ...
                RectTransform = { AnchorMin = "0 0", AnchorMax = "0 0", OffsetMin = "0 -34.166", OffsetMax = "118.486 -0.024" }
            ...
            container.Add(new CuiButton
            {
                Button = { Color = "1 1 1 0", Command = canPrevious ? $"ar.page {activeCategory.ToString()} {page - 1}" : "" },
                Text =
                {
            ...
            container.Add(new CuiButton
            {
                Button = { Color = "1 1 1 0", Command = canNext ? $"ar.page {activeCategory.ToString()} {page + 1}" : "" },
                Text =
                {
                    Text = ">", Font = "robotocondensed-bold.ttf", FontSize = 24, Align = TextAnchor.MiddleCenter,
...
//
        private void UI_DrawCategories(BasePlayer player, ItemCategory activeCategory)
        {
            ...
            container.Add(new CuiPanel
            {
                ...
                RectTransform = { AnchorMin = "1 0.5", AnchorMax = "1 0.5", OffsetMin = "-209.746 -260.42", OffsetMax = "-0.09 260.42" }
            ...
            for (int i = 0; i < Categories.Count; i++)
            {
                ...
                container.Add(new CuiElement
                {
                    ...
                    Components = {
                        new CuiTextComponent { Text = category.ToString().ToUpper(), Font = "robotocondensed-bold.ttf", FontSize = 20, Align = TextAnchor.MiddleRight, Color = isActive ? "0.8078431 0.7803922 0.7411765 1" : "0.6470588 0.6235294 0.6 1" },
                        new CuiRectTransformComponent { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = "-104.83 -20.88", OffsetMax = "95.141 20.88" }
                    }
                });
        ...
        private void CmdAutoRecToggle(BasePlayer player)
        {
            ...
            player.ChatMessage($"Autorecycling is now {(PlayerAutoRecState[player.userID] ? "<color=green>enabled</color>" : "<color=red>disabled</color>")}");
        }
