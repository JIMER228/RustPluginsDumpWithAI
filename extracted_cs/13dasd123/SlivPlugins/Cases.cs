...
namespace Oxide.Plugins
{
	...
	public class Cases : RustPlugin
	{
		...
		private class Configuration
		{
			...
			public LogSettings Logs = new LogSettings
			{
				Console = false,
				File = false,
		...
		private class LogSettings
		{
			...
			[JsonProperty(PropertyName = "Discord Logging")]
		...
		private void UpdateConfigValues()
		{
			...
			if (_config.Version == default(VersionNumber) || _config.Version < new VersionNumber(1, 1, 7))
			{
		...
		private void CmdConsoleCases(ConsoleSystem.Arg arg)
		{
			...
			switch (arg.Args[0])
			{
				...
				{
					int caseId;
					if (!CanEditCase(player) || !arg.HasArgs(2) || !int.TryParse(arg.Args[1], out caseId)) return;
				...
				case "remove_edit":
				{
					...
					if (oldCase != null)
					{
						_config.Cases.RemoveAll(x => x.ID == editData.CaseInfo.ID);
				...
				{
					int itemId;
					if (!CanEditCase(player) || !arg.HasArgs(2) || !int.TryParse(arg.Args[1], out itemId)) return;
		...
		private const int CasesOnPage = 6;
		...
		private void MainUI(BasePlayer player, int page = 0, bool first = false)
		{
			...
			container.Add(new CuiPanel
			{
				RectTransform =
				{
					...
					OffsetMin = $"-300 {(_config.BonusCaseInfo.Enabled ? -310 : -260)}",
					OffsetMax = $"300 {(_config.BonusCaseInfo.Enabled ? 335 : 265)}"
				},
				Image =
				{
		...
		private void RefCaseUi(BasePlayer player, ref CuiElementContainer container, CaseInfo caseInfoData)
		{
			...
			else
			{
...
#endif
				if (caseInfoData.HasCase(player))
				{
#if TESTING
					...
					container.Add(new CuiButton
						{
							...
							Text =
							{
								Text = $"{caseInfoData.CaseAmount(player)}",
								Align = TextAnchor.MiddleCenter,
								Font = "robotocondensed-bold.ttf",
								FontSize = 10,
								Color = _config.UI.Colors.Color3.Get()
				...
				else
				{
					...
					{
						var custom = caseInfoData.CustomCurrency?.Enabled == true;
		...
		private void GiveItem(BasePlayer player, int itemId, CaseInfo caseInfo)
		{
		...
		private void LoadImages()
		{
			...
			else
			{
				...
				foreach (var caseInfo in AllCases)
				{
					...
					if (!string.IsNullOrEmpty(caseInfo.Image))
						imagesList.TryAdd(caseInfo.Image, caseInfo.Image);
		...
		private void SetCooldown(BasePlayer player, CaseInfo caseInfoData)
		{
			...
			if (Cooldowns.TryGetValue(player.userID, out cooldown))
				cooldown.SetCooldown(caseInfoData);
			else
		...
		private class Cooldown
		{
			...
			{
				CooldownData data;
				if (Data.TryGetValue(caseInfoData.ID, out data))
