...
namespace Oxide.Plugins
{
	...
	public class Crafts : RustPlugin
	{
		...
		private readonly ListHashSet<RecyclerComponent> _recyclers = new ListHashSet<RecyclerComponent>();

		private readonly ListHashSet<CarController> _cars = new ListHashSet<CarController>();
		...
		private readonly ListHashSet<SafeZoneComponent> _safeZones = new ListHashSet<SafeZoneComponent>();
		...
		private readonly Dictionary<int, CraftInfo> _craftsById = new Dictionary<int, CraftInfo>();
			...
			switch (arg.Args[0])
			{
				...
				case "buy_craft":
				{
					...
					switch (craft.Type)
					{
						case CraftType.Item:
						{
							totalAmount = craft.GetItemStacks(amount).Count;
							break;
						}
					}
		...
		private string GetImage(string name) => ImageLibrary?.Call("GetImage", name)?.ToString() ?? string.Empty;
		...
		private IEnumerator TakeAndGiveCraftItems(BasePlayer player, int amount, Item[] allItems, CraftInfo craft,
			bool bypass)
		{
		...
		private void GiveCraft(BasePlayer player, CraftInfo cfg)
		{
			switch (cfg.Type)
			{
