...
{
    [Info("CargoTrainEvent", "Nikedemos", VERSION)]
    ...
    public class CargoTrainEvent : RustPlugin
    {
        ...
        public const ulong INTERNAL_SKIN = 2847372289;
        ...
        public const string PREFAB_WORKCART = "assets/content/vehicles/workcart/workcart.entity.prefab";
        public const string PREFAB_LOCOMOTIVE = "assets/content/vehicles/trains/locomotive/locomotive.entity.prefab";
        public const string PREFAB_SEDAN_TRAIN = "assets/content/vehicles/sedan_a/sedanrail.entity.prefab";
        public const string PREFAB_ABOVEGROUND_TRAIN = "assets/content/vehicles/workcart/workcart_aboveground.entity.prefab";
        public const string PREFAB_ABOVEGROUND_TRAIN_2 = "assets/content/vehicles/workcart/workcart_aboveground2.entity.prefab";
        ...
        {
            PrintWarning("\n-----------------------------\n" +
            "     По всем вопросам: - discord: Rust Sliv Plugin\n" +
            "     https://discord.gg/6zcvpxD6Ce\n" +
            "-----------------------------");
            Instance = this;
            ...
            foreach (var entity in BaseNetworkable.serverEntities.OfType<BaseEntity>().Where(e => e.skinID == INTERNAL_SKIN || e.skinID == SKIN_SPLAT1))
            {
                entity.Kill(BaseNetworkable.DestroyMode.None);
            }

            foreach (var train in BaseNetworkable.serverEntities.OfType<TrainEngine>())
            {
        ...
        private void OnBookmarkControlStarted(ComputerStation station, BasePlayer player, string text, CCTV_RC cctv)
        {
            ...
            if (cctv.skinID != INTERNAL_SKIN) return;
        ...
        private void OnEntitySpawned(TrainEngine train)
        {
            ...
            if (train.PrefabName == PREFAB_LOCOMOTIVE)
            {
                return;
            }
        ...
        private void OnCrateHack(HackableLockedCrate crate)
        {
            ...
            if (crate.skinID != INTERNAL_SKIN) return;
        ...
        object OnCounterModeToggle(PowerCounter counter, BasePlayer player, bool wants)
        {
            ...
            if (counter.skinID != INTERNAL_SKIN) return null;
        ...
        private void OnPlayerDeath(BasePlayer player, HitInfo info)
        {
            ...
            SpecialTrain.TrainDriverNetIDToSpecialTrain[player.net.ID.Value].OnTrainDriverKilled(info);
        }
        ...
        private object OnEntityTakeDamage(TrainEngine train, HitInfo info)
        {
            if (Instance == null) return null;
        ...
        */
        ...
        private void TellMessage(BasePlayer player, string message, bool alsoPrintWarning = false)
        {
            ...
            if (player == null)
            {
                ...
                if (Instance.configData.UseNotifyPlugin)
                {
                    foreach (var playah in BasePlayer.activePlayerList)
                    {
                        ...
                        Interface.Call("SendNotify", playah, (int)0, ReusableString2);
                    }
                }
            ...
            else
            {
                ...
                if (Instance.configData.UseNotifyPlugin)
                {
                    Interface.Call("SendNotify", player, (int)0, ReusableString2);
                }
        ...
        public class SpecialTrain : MonoBehaviour
        {
            ...
            public void Prepare(TrainEngine train)
            {
                ...
                PreviousSkinID = Train.skinID;
                Train.skinID = INTERNAL_SKIN;
                ...
                TrainDriverNetID = TrainDriver.net.ID.Value;

                TrainDriver.skinID = INTERNAL_SKIN;
            ...
            */
            ...
            public void OnTrainDriverKilled(HitInfo info)
            {
                if (TrainDriver != null)
                {
                    //1.0.5 hotfix
                ...
                if (SelfDestructionShouldHappenNow) return;
        ...
        private void ConsoleCommandTrainEventAt(ConsoleSystem.Arg arg)
        {
            ...
            TrainEngine newTrain = GameManager.server.CreateEntity(PREFAB_WORKCART, pos) as TrainEngine;
