...
{
    [Info("Furnace Upgrades", "yUN", "2.0.3")]
    ...
    internal class FurnaceUpgrades : RustPlugin
    {
        ...
        [PluginReference]
        private Plugin Economics, ServerRewards, Toastify, Notify, ImageLibrary;
        ...
        private void LoadData()
        {
            try
            {
                _furnacesData = Interface.Oxide.DataFileSystem.ReadObject<Dictionary<ulong, FurnaceData>>(Name);
            }
            catch
            {
        ...
        {
            Interface.Oxide.DataFileSystem.WriteObject(Name, _furnacesData);
        }
        ...
        private class GeneralSettings
        {
            ...
            [JsonProperty("Maintain the same speed for split items.")]
            public bool SameSpeedForSplitItems { get; set; }
        }
        ...
        private void SendNotification(BasePlayer player, string message, params object[] args)
        {
            ...
            if (_config.Visual.NotificationPlugin == NotificationPlugin.Toastify)
            {
                ...
                Toastify.CallHook("SendToast", player, _config.Visual.ToastifyNotificationId, GetMessage(player, message, args));
            }
            else if (Notify)
            {
        ...
        protected override void LoadDefaultMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>()
            {
                ...
                { "UI.SmeltingSpeedStatus", "Smeelting speed: <color=orange>{0}x</color>" },
        ...
        private void OnServerInitialized()
        {
            ...
            {
                if (plugins.Exists("StacksExtended"))
                    PrintWarning("The CanMoveItem hook was unsubscribed due to conflicts with the StackExtended plugin");
                else
            ...
            if (ImageLibrary)
            {
                ...
                var imageList = _ovensPrefab.Keys.Where((x) => x != "legacyfurnace").Select((x) => new KeyValuePair<string, ulong>(x, 0ul)).ToList();
        ...
        private object CanMoveItem(Item item, PlayerInventory playerLoot, ItemContainerId targetContainerId, int targetSlot, int amount)
        {
            ...
            if (oven.allowByproductCreation && _config.Features.AutoFuel && furnaceData.useAutoFuel)
                NextTick(() => AutoAddFuel(playerLoot, oven));
        ...
        private void OnEntityBuilt(Planner planner, GameObject gameObject)
        {
            ...
            if (item == null || !_ovensPrefab.ContainsKey(item.info.shortname))
                return;
        ...
        private bool MoveSplitItem(Item item, BaseOven oven, int splitAmount)
        {
            ItemContainer container = oven.inventory;

            if (oven.inputSlots < 1)
                return false;

            int moved = 0;
            int itemAmount = item.amount > splitAmount ? splitAmount : item.amount;
            int totalAmount = Math.Min(
                itemAmount + container.itemList.Where(x => x.info == item.info).Sum(x => x.amount),
                Math.Abs(item.info.stackable * oven.inputSlots)
            );

            int totalStackSize = Math.Min(totalAmount / oven.inputSlots, item.info.stackable);

            List<int> addedSlots = Pool.GetList<int>();
            List<OvenSlot> ovenSlots = Pool.GetList<OvenSlot>();

            for (int i = 0; i < oven.inputSlots; ++i)
            {
                int slot = FindMatchingSlotIndex(oven, container, out var existingItem, item.info, addedSlots);
                if (slot == -1)
                    return false;
                ...
                int missingAmount = totalStackSize - currentAmount;
                ovenSlot.DeltaAmount = missingAmount;

                if ((currentAmount + missingAmount) < 1)
                    continue;

                ovenSlots.Add(ovenSlot);
            }

            Pool.FreeList(ref addedSlots);
            ...
            foreach (var slot in ovenSlots)
            {
                ...
                moved += slot.DeltaAmount;
            }
            container.MarkDirty();

            Pool.FreeList(ref ovenSlots);
        ...
        private void AutoAddFuel(PlayerInventory playerInventory, BaseOven oven)
        {
            ...
            int neededFuel = (int)ovenInfo.FuelNeeded - oven.inventory.GetAmount(oven.fuelType.itemid, false);
            ...
            int fuelSlotIndex = 0;
            foreach (var item in playerFuel)
            {
                ...
                {
                    Item splitItem = item.SplitItem(toTake);
        ...
        private bool TryTakeMoney(BasePlayer player, int amount)
        {
            switch (_config.Currency.Type)
            {
                ...
                default:
                    {
                        int acc = 0;
                        List<Item> toTake = Pool.GetList<Item>();
        ...
        private OvenInfo GetOvenInfo(BaseOven oven)
        {
            ...
            float speedMultiplier = component ? component.Properties.SmeltingSpeed : 1f;

            float ETA = GetSmeltDuration(oven) / oven.smeltSpeed / speedMultiplier;
        ...
        private float GetFuelNeeded(BaseOven oven, float ETA)
        {
            ...
            float neededFuel = Mathf.Ceil(ETA * (oven.cookingTemperature / 200f) / fuel * fuelSpeedMultiplier);
            return neededFuel;
        }
        ...
        private float GetSmeltDuration(BaseOven oven)
        {
            ...
            for (int i = oven._inputSlotIndex; i < inputEndSlot; i++)
            {
                ...
                if (_config.General.SameSpeedForSplitItems)
                {
                    ETA += cookable.cookTime * item.amount;
                }
                else
                {
                    float value = cookable.cookTime * item.amount;
                    if (value > ETA)
                        ETA = value;
                }
            }
        ...
        private class OvenComponent : FacepunchBehaviour
        {
            ...
            private readonly global::Item.Flag _cookingFlag = global::Item.Flag.OnFire;
            ...
            private void Cook()
            {
                ...
                {
                    if (IsCookable(item) && !item.HasFlag(_cookingFlag))
                    {
                        item.SetFlag(_cookingFlag, true);
                        item.MarkDirty();
                    }
                }
                ...
                if (fuel != null)
                {
                    ...
                    fuel.fuel -= 0.5f * (_oven.cookingTemperature / 200f) * Properties.FuelSpeed;

                    if (!fuel.HasFlag(_cookingFlag))
                    {
                        fuel.SetFlag(_cookingFlag, true);
                        fuel.MarkDirty();
                    }
            ...
            private void SmeltItems()
            {
                ...
                foreach (var item in itemList)
                {
                    ...
                    {
                        if (cookable.setCookingFlag && item.HasFlag(_cookingFlag))
                        {
                            item.SetFlag(_cookingFlag, false);
                            item.MarkDirty();
                        }
                    ...
                    int itemSlots = _config.General.SameSpeedForSplitItems ? itemList.Where((x) => x.info == item.info).Count() : 1;
        ...
        private void ShowUpgradeButton(BasePlayer player, BaseOven oven)
        {
            ...
            ui.AddButton(
                ...
                _config.UI.UpgradeButton.BackgroundColor,
                _config.UI.UpgradeButton.FontSize,
                _config.UI.UpgradeButton.Anchor,
                _config.UI.UpgradeButton.Offset,
                UI_UPGRADE_BUTTON
            );
        ...
        private void ShowModal(BasePlayer player, BaseOven oven)
        {
            ...
            if (_config.Features.AutoFuel && HasPermission(player, AUTO_FUEL_PERMISSION) && oven.fuelSlots > 0)
                RenderOptionBox(ref ui, optionIndex++, GetMessage(player, "UI.AutoFuelOptionLabel"), furnaceData.useAutoFuel, "furnaceupgrades.ui toggle_option 0");
            if (_config.Features.AutoSplit && HasPermission(player, AUTO_SPLIT_PERMISSION))
                RenderOptionBox(ref ui, optionIndex++, GetMessage(player, "UI.AutoSplitOptionLabel"), furnaceData.useAutoSplit, "furnaceupgrades.ui toggle_option 1");
