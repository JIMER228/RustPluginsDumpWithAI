...
{
    [Info("HitMarker", "https://discord.gg/4KEQ7MBdKZ", "1.0.1")]
    class HitMarker : RustPlugin
    {
        ...
        void cmdHitMarker(BasePlayer player, string cmd, string[] args)
        {
            if (!hitmarkeron.Contains(player))
            {
                ...
                SendReply(player,
                    "<color=#d60000>HitMarker</color>:" + " " + "<color=orange>Вы включили показ урона.</color>");
            }
            else
            {
                hitmarkeron.Remove(player);
                SendReply(player,
                    "<color=#d60000>HitMarker</color>:" + " " + "<color=orange>Вы отключили показ урона.</color>");
            }
        }
        ...
        Dictionary<string, string> Images = new Dictionary<string, string>()
        {
            ...
            { "hitmarker.hit.normal", "https://i.imgur.com/prm1j7r.png" },
            { "hitmarker.hit.head", "https://i.imgur.com/57YVZVh.png" },
            { "hitmarker.hit.friend", "https://imgur.com/hzmd2Ud.png" },
            { "hitmarker.hit.wound", "https://i.imgur.com/vn9mCo6.png" },
        };
