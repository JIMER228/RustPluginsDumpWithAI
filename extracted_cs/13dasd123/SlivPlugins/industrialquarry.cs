...
{
    [Info("industrialquarry", "bmgjet", "1.2.1")]
    ...
    class industrialquarry : RustPlugin
    {
        ...
        private void ChatIndustrialQuarry(BasePlayer player, string command, string[] args)
        {
            if (player.IsAdmin) //limit to admin
            {
                foreach (BaseNetworkable bn in BaseNetworkable.serverEntities.entityList.Values) //Scan all server ents
                {
                ...
                Startup(10); //restart plugin so it respawns and reconnects
            }
        }
        ...
        private void OnEntitySpawned(MiningQuarry quarry) { NextFrame(() => { AttachIndustrialParts(quarry); }); }
        ...
        //Admin Kill
        private object OnEntityKill(BaseNetworkable bn)
        {
            if (bn == null) return null;
            if ((bn as BaseEntity) != null)
            {
                if (bn is IOEntity && (bn as BaseEntity).skinID == 1234567890)
                {
                    (bn as IOEntity).Heal(200); //Set back to max health
                    return true; //Block parts being destroyed
                }
                if (bn is MiningQuarry)
                {
                    RemoveQuarry((MiningQuarry)bn);
                }
            }
            return null;
        }
            ...
            timer.Once(delay, () =>
             {
                 ...
                 foreach (BaseNetworkable bn in BaseNetworkable.serverEntities.entityList.Values)
                 {
                     ...
                     if (bn is MiningQuarry)
                     {
                         AttachIndustrialParts(bn as MiningQuarry);
                     }
        ...
        private void Unload() { Interface.Oxide.DataFileSystem.WriteObject("industrialquarry", storedData); }
        ...
        private void RUNIO(IOEntity source, IOEntity dest, int dsocket = 0, int ssocket = 0)
        {
            Vector3[] QuarryPipe = new Vector3[2]
                { source.transform.InverseTransformPoint(dest.transform.position), new Vector3(0, 0, 0) };
            try
            {
                //Clear Connections
                try //Try catch since likely to fail if IO hasnt had a markdirty
                {
                    if (source.outputs[ssocket].connectedTo.Get() != null)
                    {
                        WireTool.AttemptClearSlot(source.outputs[ssocket].connectedTo.Get(), null, ssocket, true);
                    }

                    if (dest.inputs[dsocket].connectedTo.Get() != null)
                    {
                        WireTool.AttemptClearSlot(
                            dest.inputs[dsocket].connectedTo.Get().outputs[dsocket].connectedTo.Get(), null, dsocket,
                            true);
                    }
                }
                catch
                ...
                //Logic to connect the pipe/wire
                source.outputs[ssocket].worldSpaceLineEndRotation =
                source.transform.TransformDirection(source.outputs[ssocket].handleDirection);
                source.outputs[ssocket].connectedTo.Set(dest);
                source.outputs[ssocket].linePoints = QuarryPipe;
                source.outputs[ssocket].connectedToSlot = dsocket;
                source.outputs[ssocket].connectedTo.Init();
                dest.inputs[dsocket].connectedTo.Set(source);
                dest.inputs[dsocket].connectedToSlot = ssocket;
                dest.inputs[dsocket].connectedTo.Init();
                //Send updates
                dest.MarkDirtyForceUpdateOutputs();
                source.MarkDirtyForceUpdateOutputs();
                source.SendNetworkUpdate(BasePlayer.NetworkQueue.Update);
                dest.SendNetworkUpdate(BasePlayer.NetworkQueue.Update);
                dest.SendChangedToRoot(true);
                source.SendChangedToRoot(true);
                source.NotifyIndustrialNetworkChanged();
                dest.NotifyIndustrialNetworkChanged();
            }
            catch
            {
            }
        ...
        public void AttachIndustrialParts(MiningQuarry quarry)
        {
            ...
            if (quarry.isStatic && !Underground) { BBDistance = 35.0f; }
            ...
            foreach (BaseEntity be in quarry.children)
            {
                ...
                if (be is StorageContainer && (be as StorageContainer).inventorySlots == 18)
                {
                    ...
                    //Check if pumpjack
                    if (be.ShortPrefabName == "crudeoutput")
                    {
            ...
            if (pumpjack) { qs += (rock.transform.forward * 1.2f); }
            else { qs += (rock.transform.forward * -0.85f); }
            ...
            {
                quarryswitch = SpawnPart(SwitchPrefab, quarry, qs, ConvayerRot) as ElectricSwitch;
            }
            ...
            if (storedData.LoadedIQuarrys.Count > 0)
            {
                foreach (var q in storedData.LoadedIQuarrys)
                {
                    ...
                    {
                        //Update Datafile
                        q.ParentPosition = quarry.transform.position;
                        q.ParentQuarry = quarry.net.ID.Value;
                        q.OwnerID = quarry.OwnerID;
                        q.StoneNetID = rock.net.ID.Value;
                        q.FuelAdapterNetID0 = adapterfuel.net.ID.Value;
                        q.FuelAdapterNetID1 = conveyorfuel.net.ID.Value;
                        q.ResourceAdapterNetID0 = adapter.net.ID.Value;
                        q.ResourceAdapterNetID1 = conveyor.net.ID.Value;
                        q.SwitchNetID = quarryswitch.net.ID.Value;
                        q.Pipes0 = adapterfuel.inputs[0].connectedTo.Get().outputs[0].linePoints;
                        q.Pipes1 = adapter.outputs[0].linePoints;
                        return;
                    }
                }
            }
            ...
            quarry1.ParentPosition = quarry.transform.position;
            quarry1.ParentQuarry = quarry.net.ID.Value;
            quarry1.OwnerID = quarry.OwnerID;
            quarry1.StoneNetID = rock.net.ID.Value;
            quarry1.FuelAdapterNetID0 = adapterfuel.net.ID.Value;
            quarry1.FuelAdapterNetID1 = conveyorfuel.net.ID.Value;
            quarry1.ResourceAdapterNetID0 = adapter.net.ID.Value;
            quarry1.ResourceAdapterNetID1 = conveyor.net.ID.Value;
            quarry1.SwitchNetID = quarryswitch.net.ID.Value;
            quarry1.Pipes0 = adapterfuel.inputs[0].connectedTo.Get().outputs[0].linePoints;
            quarry1.Pipes1 = adapter.outputs[0].linePoints;
            storedData.LoadedIQuarrys.Add(quarry1);
            Interface.Oxide.DataFileSystem.WriteObject("industrialquarry", storedData);
        }
