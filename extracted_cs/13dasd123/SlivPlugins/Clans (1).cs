...
namespace Oxide.Plugins
{
	...
	public class Clans : RustPlugin
	{
        ...
        private Regex _tagFilter, _hexFilter = new Regex("^([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$");
		...
		private class Configuration
		{
			...
			[JsonProperty(PropertyName = "Pages Settings")]
			public PagesSettings PagesConfig = new PagesSettings
			{
			...
			{
				[JsonProperty(PropertyName = "Default Pages")]
				public DefaultPages Default;

				[JsonProperty(PropertyName = "Stay on page of item for which skin is selected")]
				...
				public class DefaultPages
				{
					...
					[JsonProperty(PropertyName = "Default page when a clan not found")]
					public int ClanPageNotFound;

					[JsonProperty(PropertyName = "Default page when a clan exists")]
		...
		private void OnPlayerStatsGathered(ulong member, string shortName, int amount = 1)
		{
#if TESTING
...
#endif
			{
				...
				float score;
				if (!_config.ScoreTable.TryGetValue(shortName, out score) && !_config.AvailableStandartItems.Contains(shortName)) return;
				
				var data = PlayerData.GetOrCreate(member.ToString());
				if (data == null) return;
				
				if (!data.Stats.TryAdd(shortName, amount)) data.Stats[shortName] += amount;
		...
		private void OnPlayerDeath(BasePlayer player, HitInfo info)
		{
...
#endif
			{
				...
				{
					OnPlayerStatsGathered(attacker.userID, "kills");
					
					OnPlayerStatsGathered(player.userID, "deaths");
				}
				else
				{
					OnPlayerStatsGathered(attacker.userID, player.ShortPrefabName);
				}
			}
		}
		...
		private void OnItemGather(ulong player, string shortname, int amount)
		{
...
#endif
			{
				...
				OnPlayerStatsGathered(player, shortname, amount);
			}
		}
...
#if TESTING
					PrintError(
						$"Failed to add item {item.info.shortname} UID {item.uid.Value} to looters dictionary");
#endif
				}
			}
		}
			...
			int amount)
		{
...
#endif
			{
				...
				if (!targetContainer.IsValid && targetSlot == -1)
				{
#if TESTING
					Puts($"[CanMoveItem] Adding {item.amount} {item.info.shortname} to player {player.userID} stats");
#endif
					OnPlayerStatsGathered(player.userID, item.info.shortname, item.amount);

					_looters.TryAdd(item.uid.Value, player.userID);
				}
			}
		}
		...
		private void OnItemPickup(Item item, BasePlayer player)
		{
...
#endif
			{
				...
				if (_looters.ContainsKey(item.uid.Value))
				{
					...
					{
#if TESTING
						Puts($"[OnItemPickup] player {player.userID} tried to pick up {item.info.shortname}");
#endif
...
#endif
			{
				...
				if (entity is BaseHelicopter)
				{
					if (_lastHeli.TryGetValue(entity.net.ID.Value, out player))
					{
						...
						{
#if TESTING
							Puts($"[OnEntityDeath] adding helicopter kill to player {player.userID} stats");
#endif

							OnPlayerStatsGathered(player.userID, "helicopter");
						}
#if TESTING
						else
						{
...
#endif
					OnPlayerStatsGathered(player.userID, "bradley");
				}
...
#endif
					OnPlayerStatsGathered(player.userID, "barrel");
				}
...
#endif
					OnPlayerStatsGathered(player.userID, entity.ShortPrefabName);
				}
			}
		}
		...
		private void TryUpdateSkins(ItemContainer container, Item item, BasePlayer player)
		{
			...
			ulong skin;
			if (!clan.Skins.TryGetValue(item.info.shortname, out skin) || skin == 0) return;
...
#endif
			{
				...
				switch (args[0].ToLower())
				{
					...
					case "create":
					{
						...
						if (createClan == "success")
							cov.Reply($"You created the clan {args[1]} and set {args[2]} as the owner");
						else
							cov.Reply(createClan);
						break;
					}
					...
					case "rename":
					{
						...
						if (renameClan == "success")
							cov.Reply($"The clan {args[1]} was renamed to {args[2]}");
						else
							cov.Reply(renameClan);
						break;
					}
					...
					case "disband":
					{
						...
						if (disbandClan == "success")
							cov.Reply($"Successfully disbanded clan {args[1]}");
						else
							cov.Reply(disbandClan);
						break;
					}
				}
			}
						...
						case "editskin":
						{
							...
							if (page == -1)
							{
								if (_config.PagesConfig?.StayOnPageWithSelectedSkin == true)
								{
									...
									ulong nowSkin;
									if (clan.Skins.TryGetValue(shortName, out nowSkin) && nowSkin != 0UL)
		...
		private void ShowClanMainUI(BasePlayer player, int page = 0, int localPage = 0, string search = "", bool first = false)
		{
#if TESTING
		...
		private void ShowClanSkinSelectionUI(BasePlayer player, string shortName, int page = 0, bool First = false)
		{
...
#endif
			{
				...
				var constSwitchX = -(UI_SKIN_SELECTION_PAGE_SIZE * UI_SKIN_SELECTION_ITEM_WIDTH + (UI_SKIN_SELECTION_PAGE_SIZE - 1) * UI_SKIN_SELECTION_ITEM_MARGIN) / 2f;
				...
				var skins = _config.Skins.ItemSkins[shortName].SkipAndTake(page * UI_SKIN_SELECTION_TOTAL_AMOUNT, UI_SKIN_SELECTION_TOTAL_AMOUNT);
...
#if TESTING
			string name;
			if (_testingPlayers.TryGetValue(userID, out name))
				return name;
#endif
		...
		{
			ClanData clan;
			return !string.IsNullOrEmpty(userId) && _playerToClan.TryGetValue(userId, out clan) ? clan : null;
		}
		...
		{
			return (ulong) (76561197960265728UL + Random.Range(0, 100f));
		}
		...
		private void LoadTestingPlayers()
		{
			for (var i = 0; i < 20; i++)
			{
				...
				if (i < _testingAvatars.Count)
					ImageLibrary?.Call("AddImage", _testingAvatars[i], $"avatar_{steamID}");
			}
		}
