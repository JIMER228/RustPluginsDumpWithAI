...
{
    [Info("JunkyardEvent", "KpucTaJl", "2.1.5")]
    internal class JunkyardEvent : RustPlugin
    {
        ...
        private void OnEntitySpawned(MagnetCrane crane)
        {
            ...
            if (Vector3.Distance(crane.transform.position, _controller.transform.position) < _config.Radius)
            {
        ...
        private void OnEntityKill(LockedByEntCrate crate)
        {
            ...
            if (_controller.HeliCrates.Contains(crate)) _controller.HeliCrates.Remove(crate);
            else if (_controller.BradleyCrates.Contains(crate)) _controller.BradleyCrates.Remove(crate);
        }

        private void OnEntityKill(HackableLockedCrate crate) { if (crate != null && _controller.HackCrates.Contains(crate)) _controller.HackCrates.Remove(crate); }

        private void OnEntityEnter(LargeShredderTrigger trigger, BaseCombatEntity entity)
        {
            if (trigger == null || entity == null) return;
            if (trigger == _controller.Shredder.trigger) _controller.CheckCar(entity.net.ID.Value);
        }
        ...
        #region Controller
        internal HashSet<Vector3> Marker = new HashSet<Vector3>
        {
        ...
        private void Start()
        {
            ...
            timer.In(_config.PreStartTime, () =>
            {
                ...
                Interface.Oxide.CallHook("OnJunkyardEventStart", _controller.transform.position, _config.Radius);
                if (_config.RemoveBetterNpc && plugins.Exists("BetterNpc")) BetterNpc.Call("DestroyController", "Junkyard");
                if (plugins.Exists("MonumentOwner")) MonumentOwner.Call("RemoveZone", FindMonumentInfo(_controller.transform.position, "Junkyard"));
                if (_config.PveMode.Pve && plugins.Exists("PveMode"))
                {
                    ...
                    PveMode.Call("EventAddPveMode", Name, config, _controller.transform.position, _config.Radius, new HashSet<ulong>(), _controller.Scientists.Select(x => x.net.ID.Value), new HashSet<ulong>(), new HashSet<ulong>(), new HashSet<ulong>(), null);
                }
        ...
        private void Finish()
        {
            ...
            if (plugins.Exists("MonumentOwner") && _controller != null) MonumentOwner.Call("CreateZone", FindMonumentInfo(_controller.transform.position, "Junkyard"));
        ...
        {
            private HashSet<MapMarkerGenericRadius> _markers { get; } = new HashSet<MapMarkerGenericRadius>();
            ...
            private BaseVehicle _parachute;
            private BasePlayer _playerParachute;
            ...
            internal ulong BrokenCarId;
            private ulong _truckId;
            private readonly HashSet<ulong> _checkedCars = new HashSet<ulong>();
            ...
            internal HashSet<LockedByEntCrate> HeliCrates = new HashSet<LockedByEntCrate>();
            internal HashSet<LockedByEntCrate> BradleyCrates = new HashSet<LockedByEntCrate>();
            ...
            internal BasePlayer Owner = null;

            private readonly Dictionary<Vector3, Vector3> SamSitePositions = new Dictionary<Vector3, Vector3>
            {
            ...
            private readonly HashSet<SamSite> SamSites = new HashSet<SamSite>();
            ...
            private void Awake()
            {
                ...
                if (_ins._config.Marker.Enabled) SpawnMapMarker();
                ...
                Cranes = GetEntities<MagnetCrane>(transform.position, _ins._config.Radius, -1);
            ...
            private void OnDestroy()
            {
                ...
                foreach (MapMarkerGenericRadius marker in _markers) if (marker.IsExists()) marker.Kill();
            ...
            private void OnTriggerEnter(Collider other)
            {
                ...
                {
                    Players.Add(player);
                    if (_ins._config.Gui.IsGui) _ins.CreateTabs(player, new Dictionary<string, string> { ["Clock_KpucTaJl"] = GetTimeFormat(TimeToFinish) });
            ...
            {
                if (_spawnCratesCoroutine != null && HeliCrates.Count + BradleyCrates.Count + HackCrates.Count == 0 && TimeToFinish > _ins._config.PreFinishTime) TimeToFinish = _ins._config.PreFinishTime;
                else TimeToFinish--;
                ...
                if (HeliCrates.Count + BradleyCrates.Count + HackCrates.Count > 0) UpdateMarkerForPlayers();
                if (TimeToFinish == _ins._config.PreFinishTime) _ins.AlertToAllPlayers("PreFinish", _ins._config.Prefix, GetTimeFormat(_ins._config.PreFinishTime));
            ...
            private void SpawnMapMarker()
            {
                ...
                background.Spawn();
                background.radius = _ins._config.Marker.Type == 0 ? _ins._config.Marker.Radius : 0.37967f;
                background.alpha = _ins._config.Marker.Alpha;
                background.color1 = new Color(_ins._config.Marker.Color.R, _ins._config.Marker.Color.G, _ins._config.Marker.Color.B);
                background.color2 = new Color(_ins._config.Marker.Color.R, _ins._config.Marker.Color.G, _ins._config.Marker.Color.B);
                _markers.Add(background);

                if (_ins._config.Marker.Type == 1)
                {
                    foreach (Vector3 pos in _ins.Marker)
                    {
                        ...
                        marker.alpha = 1f;
                        marker.color1 = new Color(_ins._config.Marker.Color.R, _ins._config.Marker.Color.G, _ins._config.Marker.Color.B);
                        marker.color2 = new Color(_ins._config.Marker.Color.R, _ins._config.Marker.Color.G, _ins._config.Marker.Color.B);
                        _markers.Add(marker);
                    }
                }
            ...
            {
                foreach (MapMarkerGenericRadius marker in _markers) marker.SendUpdate();
                _vendingMarker.markerShopName = _ins._config.Marker.Text + "\n" + GetTimeFormat(TimeToFinish);
                if (Owner != null) _vendingMarker.markerShopName += $"\n{Owner.displayName}";
            ...
            {
                foreach (BaseCombatEntity entity in GetEntities<BaseCombatEntity>(transform.position, _ins._config.Radius, 1 << 15)) if (entity.ShortPrefabName == "shreddable_pickuptruck") BrokenCars.Add(entity);
            ...
            private void SpawnEntityInConveyor(string name)
            {
                ...
                {
                    if (entity.ShortPrefabName == "heli_crate") HeliCrates.Add(entity as LockedByEntCrate);
                    else BradleyCrates.Add(entity as LockedByEntCrate);
            ...
            private void UpdateMarkerForPlayerInCrane()
            {
                if (_ins._config.MainPoint.Enabled)
                {
                    ...
                    if (points.Count > 0) foreach (BasePlayer player in PlayersInCrane) foreach (Vector3 point in points) UpdateMarkerForPlayer(player, point, _ins._config.MainPoint);
                    points = null;
                }
            }
            ...
            private void SpawnParachute()
            {
                ...
                _parachute = parachute.gameObject.AddComponent<BaseVehicle>();
                CopySerializableFields(parachute, _parachute);
                DestroyImmediate(parachute, true);

                _parachute.enableSaving = false;

                _parachute.transform.localPosition = new Vector3(0f, 0.5f, 0f);
                _parachute.SetParent(Truck);

                _parachute.Spawn();

                _parachute.SetToKinematic();

                _playerParachute = GameManager.server.CreateEntity("assets/prefabs/player/player.prefab", Truck.transform.position) as BasePlayer;
                _playerParachute.Spawn();

                _playerParachute.DisablePlayerCollider();
                _playerParachute.playerRigidbody.isKinematic = true;

                _parachute.AttemptMount(_playerParachute, false);
            }

            private void DestroyParachute()
            {
                if (_parachute.IsExists()) _parachute.Kill();
                if (_playerParachute.IsExists()) _playerParachute.Kill();
            }
        ...
        private object CanPopulateLoot(LootContainer container)
        {
            ...
            if (container is LockedByEntCrate)
            {
                ...
                if (_controller.HeliCrates.Contains(crate))
                {
                    if (_config.HeliCrates.TypeLootTable == 2) return null;
                    else return true;
                }
                else if (_controller.BradleyCrates.Contains(crate))
                {
        ...
        {
            if (_controller == null) return null;
            if (_controller.HeliCrates.Any(x => x.IsExists() && x.net.ID.Value == netID.Value))
            {
                if (_config.HeliCrates.TypeLootTable == 3) return null;
                else return true;
            }
            else if (_controller.BradleyCrates.Any(x => x.IsExists() && x.net.ID.Value == netID.Value))
            {
