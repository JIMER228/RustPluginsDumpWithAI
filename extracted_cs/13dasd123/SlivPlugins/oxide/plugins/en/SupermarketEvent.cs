...
{
    [Info("SupermarketEvent", "KpucTaJl", "1.0.4")]
    internal class SupermarketEvent : RustPlugin
    {
        ...
        private void Start(BasePlayer player)
        {
            ...
            timer.In(_config.PreStartTime, () =>
            {
                ...
                Interface.Oxide.CallHook("OnSupermarketEventStart", Controller.transform.position, _config.Radius);
                AlertToAllPlayers("Start", _config.Chat.Prefix, PhoneController.PositionToGridCoord(Controller.transform.position));
            });
        }
        ...
        private void Finish()
        {
            ...
            Interface.Oxide.CallHook("OnSupermarketEventEnd");
        ...
        internal class ControllerSupermarketEvent : FacepunchBehaviour
        {
            ...
            private void SpawnMapMarker(MarkerConfig config)
            {
                ...
                if (_ins.ActivePveMode) VendingMachineOwner = SpawnVendingMachine("No Owner");
                VendingMachineTime = SpawnVendingMachine(GetTimeFormat(TimeToFinish));
            ...
            private void UpdateVendingMarker()
            {
                ...
                if (TimeToFinish > 0) VendingMachineTime.shopName = GetTimeFormat(TimeToFinish);
                else if (TimeTransaction > 0 && TimeTransaction < _config.TimeTransaction) VendingMachineTime.shopName = GetTimeFormat(TimeTransaction);
                else VendingMachineTime.shopName = string.Empty;
                if (VendingMachineTime.IsBroadcasting())
                {
                    if (string.IsNullOrEmpty(VendingMachineTime.shopName))
                        VendingMachineTime.SetFlag(BaseEntity.Flags.Reserved4, false);
                }
                else
                {
                    if (!string.IsNullOrEmpty(VendingMachineTime.shopName))
                        VendingMachineTime.SetFlag(BaseEntity.Flags.Reserved4, true);
                }
            ...
            private void SpawnPreset(PresetConfig preset, Vector3 spawnPos)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    if (preset == _config.SecurityOutside)
                    {
        ...
        private void SendBalance()
        {
            ...
            Interface.Oxide.CallHook("OnSupermarketEventWinner", winnerId);
