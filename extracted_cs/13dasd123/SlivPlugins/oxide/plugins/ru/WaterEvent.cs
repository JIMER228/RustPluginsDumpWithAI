...
{
    [Info("WaterEvent", "KpucTaJl", "2.1.3")]
    internal class WaterEvent : RustPlugin
    {
        ...
        public class ChatConfig
        {
            ...
            [JsonProperty(En ? "The distance from the event to the player for global chat messages (0 - no limit)" : "Расстояние от ивента до игрока для глобальных сообщений в чате (0 - нет ограничений)")] public float Distance { get; set; }
        }
        ...
        {
            if (trigger == null || target.IsPlayer() || ActivePveMode) return null;
        ...
        private void Start(BasePlayer player)
        {
            ...
            timer.In(_config.PreStartTime, () =>
            {
                ...
                AlertToAllPlayers("Start", _config.Chat.Prefix, PhoneController.PositionToGridCoord(SpawnPos), GetTimeFormat(_config.FinishTime));
            });
        }
        ...
        internal class ControllerWaterEvent : FacepunchBehaviour
        {
            ...
            private IEnumerator SpawnEntities(BasePlayer player)
            {
                                ...
                                _ins.NextTick(() =>
                                {
                                    ...
                                    foreach (ItemConfig item in crateConfig.LootTable.Items)
                                    {
                                        ...
                                        if (newItem.info.category == ItemCategory.Attire && countLootInContainerWear < 21)
                                        {
                                            ...
                                            if (countLootInContainerWear < 7) targetPos = countLootInContainerWear;
                                            else if (countLootInContainerWear < 14) targetPos = 14 + countLootInContainerWear - 7;
                                            else targetPos = 28 + countLootInContainerWear - 14;
                                        ...
                                        else if (countLootInContainerBelt < 18)
                                        {
                                            ...
                                            if (countLootInContainerBelt < 6) targetPos = countLootInContainerBelt + 8;
                                            else if (countLootInContainerBelt < 12) targetPos = 22 + countLootInContainerBelt - 6;
                                            else targetPos = 36 + countLootInContainerBelt - 12;
                ...
                Interface.Oxide.CallHook("OnWaterEventStart", Entities, transform.position, _config.Radius);
            ...
            private void SpawnHackCrates()
            {
                foreach (LocationConfig location in _config.HackCrate.Coordinates)
                {
                    ...
                    if (hackCrate.mapMarkerInstance.IsExists()) hackCrate.mapMarkerInstance.Kill();
        ...
        {
            if (victim == null || hitinfo == null || Controller == null || ActivePveMode) return null;
        ...
        {
            if (!player.IsPlayer() || turret == null || Controller == null || ActivePveMode) return null;
        ...
        private void AlertToAllPlayers(string langKey, params object[] args)
        {
            ...
            foreach (BasePlayer player in BasePlayer.activePlayerList)
                if (_config.Chat.Distance == 0f || Vector3.Distance(player.transform.position, Controller.transform.position) <= _config.Chat.Distance)
