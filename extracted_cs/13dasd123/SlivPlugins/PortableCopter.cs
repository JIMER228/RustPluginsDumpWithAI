...
{
    [Info("Portable Copter", "OxideBro", "2.3.12")]
	...
	class PortableCopter : RustPlugin
	{
		...
		int citemid = -939424778; //ID предмета который меняем
		...
		public object GetItem(BasePlayer player, int itemId, ulong skinID, int amount, bool check = false)
        {
            if (check)
            {
                List<Item> list = player.inventory.FindItemsByItemID(itemId);
            ...
            {
                List<Item> customTake = player.inventory.FindItemsByItemID(itemId);
