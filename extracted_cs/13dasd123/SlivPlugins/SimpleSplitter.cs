...
{
    [Info("Simple Splitter", "Shady14u", "2.4.7")]
    ...
    public partial class SimpleSplitter : RustPlugin
    {
        ...
        {
            PrintWarning($"Composter MoveSplitItem: {item.info.shortname} : {item.skin} : {composter.ShortPrefabName}");
            ...
            var totalMoved = 0;
            var lastSlotIdx = 0;
            var existingAmount = container.GetAmount(item.info.itemid, false);
            ...
            for (var i = composter.inventorySlots - 1; i > -1; --i)
            {
                ...
                if (inputItem != null && inputItem.info == item.info)
                {
            ...
            if (numUsedSlots <= 0 || existingAmount >= numUsedSlots * GetStackSize(item))
            {
                return MoveResult.SlotsFilled;
            }
            ...
            for (var i = composter.inventorySlots - 1; i > -1; --i)
            {
                ...
                if (existingItem != null && existingItem.info != item.info)
                {
                    continue;
                }
        ...
        public partial class SmeltingController : FacepunchBehaviour
        {
            ...
            private bool HasFuel(BaseOven baseOven)
            {
                ...
                {
                    hasFuel = parent.HasFlag(BaseEntity.Flags.Reserved1);
                }
                return hasFuel;
            }
