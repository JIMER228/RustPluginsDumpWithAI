...
{
    [Info("MagicTree", "OxideBro", "1.3.41")]
        ...
        public void SpawnBox(Wood wood, int i, BaseEntity tree, ulong ownerID, int countBox = 0)
        {
            ...
            if (wood != null)
            {
                ...
                for (int count = 0; count < countBox; count++)
                {
                    var reply = 0;
                    if (reply == 0) { }
                    Vector3 pos = new Vector3(UnityEngine.Random.Range(-config.VerticalRadius, config.VerticalRadius), UnityEngine.Random.Range(config.VerticalRadius / 2, config.VerticalRadius), UnityEngine.Random.Range(-config.VerticalRadius, config.VerticalRadius));
