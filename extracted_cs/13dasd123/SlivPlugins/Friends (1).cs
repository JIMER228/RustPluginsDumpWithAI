...
namespace Oxide.Plugins
{
    ...
    public class Friends : RustPlugin
    {
        ...
        private object OnCupboardAuthorize(BuildingPrivlidge privilege, BasePlayer player)
        {
            ...
            {
                var newAuthorizedPlayers = new HashSet<PlayerNameID>();

                int index = 0;
                foreach (var playerID in privilege.authorizedPlayers)
                {
                    newAuthorizedPlayers.Add(playerID);
                    index++;
                    if (index >= RelationshipManager.maxTeamSize)
                        break;
                }
                privilege.authorizedPlayers = newAuthorizedPlayers;
            }
        ...
        {
            var enumerable = GetPlayerData(playerId)?.Friends.Select(x => x.UserId);
            var array = enumerable as ulong[] ?? enumerable?.ToArray();
            return array;
        }
