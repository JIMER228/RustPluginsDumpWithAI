...
{
    [Info("SecurityCameras", "k1lly0u", "0.2.33")]
    ...
    class SecurityCameras : RustPlugin
    {
        ...
        {
            if (droppedItem.GetComponent<CameraManager>())
                return true;
            return null;
        }
        ...
        {
            int max = 0;
            foreach(KeyValuePair<string, int> entry in configData.Camera.Max)
            {
        ...
        private class LinkManager
        {
            ...
            public class CameraLink
            {
                ...
                {                   
                    CameraManager manager = Managers.FirstOrDefault(x => x.Camera.net.ID.Value == managerId);
                    if (manager != null)
                    {
        ...
        private void ccmdControl(ConsoleSystem.Arg arg)
        {
            ...
            CuiHelper.DestroyUi(player, RustNET.RustNET_Panel);
            InitializeController(player, arg.GetUInt64(0), arg.GetInt(1));            
        }
        #endregion
