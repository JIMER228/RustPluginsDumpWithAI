...
{
    [Info("DefendableBases", "KpucTaJl", "1.1.3")]
    internal class DefendableBases : RustPlugin
    {
        ...
        private void LoadConfigs()
        {
            ...
            foreach (string name in Interface.Oxide.DataFileSystem.GetFiles("DefendableBases/Config/"))
            {
                ...
                if (config != null)
                {
                    ...
                    {
                        wave.Missions = wave.Missions.OrderBy(x => x.Chance);
                        foreach (MissionConfig mission in wave.Missions)
                        {
                            mission.Reward.Items = mission.Reward.Items.OrderBy(x => x.Chance);
                        ...
                        foreach (PresetConfig preset in wave.Presets) preset.AmountConfig = preset.AmountConfig.OrderBy(x => x.Chance);
                    }
        ...
        private void OnCorpsePopulate(ScientistNPC entity, NPCPlayerCorpse corpse)
        {
            ...
            if (_controller.Scientists.Contains(entity))
            {
                ...
                {
                    foreach (CustomBarricade barricade in _controller.CustomBarricades) if (barricade.Zombies.Contains(entity.net.ID)) barricade.ExitSledge(entity);
                    if (_controller.GeneralSphere.Zombies.Contains(entity.net.ID)) _controller.GeneralSphere.ExitSledge(entity);
                }
        ...
        private void Start()
        {
            ...
            timer.In(_config.PreStartTime, () =>
            {
                ...
                if (_config.PveMode.Pve && plugins.Exists("PveMode"))
                {
                    ...
                    PveMode.Call("EventAddPveMode", Name, config, _controller.transform.position, _controller.Config.Radius, new HashSet<uint>(), new HashSet<uint>(), new HashSet<uint>(), new HashSet<ulong>(), null);
                }
        ...
        internal class ControllerDefendableBase : FacepunchBehaviour
        {
            ...
            internal Dictionary<uint, int> CrateToTypeLootTable = new Dictionary<uint, int>();
            ...
            internal readonly Dictionary<uint, SimpleLight> Lights = new Dictionary<uint, SimpleLight>();
            internal readonly Dictionary<uint, PowerCounter> Counters = new Dictionary<uint, PowerCounter>();
            ...
            private IEnumerator SpawnEntities()
            {
                ...
                foreach (Prefab prefab in _ins.Prefabs[Config.Name])
                {
                    ...
                    if (skinConfig != null) entity.skinID = skinConfig.SkinIDs.GetRandom();

                    if (entity is BuildingBlock)
                    {
                        BuildingBlock buildingBlock = entity as BuildingBlock;
                        buildingBlock.SetGrade(Config.BuildingGrade == 0 ? BuildingGrade.Enum.Twigs : Config.BuildingGrade == 1 ? BuildingGrade.Enum.Wood : Config.BuildingGrade == 2 ? BuildingGrade.Enum.Stone : Config.BuildingGrade == 3 ? BuildingGrade.Enum.Metal : BuildingGrade.Enum.TopTier);
                ...
                foreach (KeyValuePair<SimpleLight, Vector3> dic in lights)
                {
                    ...
                    Lights.Add(wallFrame.net.ID, dic.Key);
                }

                foreach (KeyValuePair<PowerCounter, Vector3> dic in counters)
                {
                    BuildingBlock wallFrame = WallFrames.FirstOrDefault(x => IsEqualVector3(x.transform.position, dic.Value));
                    Counters.Add(wallFrame.net.ID, dic.Key);
                }
            ...
            private void SpawnCrates()
            {
                foreach (RoomConfig room in Config.Rooms)
                {
                    foreach (CrateConfig crateConfig in room.Crates)
                    {
                        ...
                        CrateToTypeLootTable.Add(crate.net.ID, crateConfig.TypeLootTable);
            ...
            internal IEnumerator ProcessWave(int level)
            {
                ...
                while (Seconds > 0)
                {
                    ...
                    if (timerNpc == Seconds)
                    {
                        ...
                        foreach (PresetConfig preset in wave.Presets)
                        {
                            ...
                            JObject objectConfig = preset.ShortName == "Sledge" ? GetSledgeObjectConfig() : preset.ShortName == "Blazer" ? GetBlazerObjectConfig() : GetJuggernautObjectConfig();
                            for (int i = 0; i < count; i++)
                            {
                                ...
                                else SetJuggernautWeapons(objectConfig);
                ...
                if (RoomCrates.ContainsKey(level))
                {
                    foreach (LootContainer crate in RoomCrates[level])
                    {
                        ...
                        if (_ins._config.PveMode.Pve && _ins.plugins.Exists("PveMode")) _ins.PveMode.Call("EventAddCrates", _ins.Name, new HashSet<uint> { crate.net.ID });
                    }
                }
            ...
            private void SetBlazerWeapons(JObject objectConfig)
            {
                float chance = UnityEngine.Random.Range(0.0f, 100.0f);
        ...
        {
            private SphereCollider _sphereCollider;
            internal HashSet<uint> Zombies = new HashSet<uint>();
        ...
        private bool IsCustomBarricade(uint id) => _controller.CustomBarricades.Any(x => x.NetID == id);
        ...
        internal class CustomBarricade : BuildingBlock
        {
            ...
            internal void Init(int level, SimpleLight light, PowerCounter counter, uint netID)
            {
                NetID = netID;
        ...
        internal class ControllerPlayerCustomBarricade : FacepunchBehaviour
        {
            ...
            private void FixedUpdate()
            {
                if (_serverInput.WasJustPressed(BUTTON.FIRE_PRIMARY))
                {
                    ...
                    HashSet<BuildingBlock> wallFramesRadiusVision = _ins._controller.WallFrames.Where(x => DistanceToPlayer(x.transform.position) < 3f && DotVisionPlayer(x.transform.position + new Vector3(0f, 1.5f, 0f)) > 0f && !_ins.IsCustomBarricade(x.net.ID));
                    ...
                    uint wallFrameNetID = wallFrame.net.ID;
        ...
        private object CanPopulateLoot(LootContainer container)
        {
            ...
            if (_controller.CrateToTypeLootTable.ContainsKey(container.net.ID))
            {
                if (_controller.CrateToTypeLootTable[container.net.ID] == 2) return null;
                else return true;
            }
            else return null;
        }
        ...
        private object OnCustomLootContainer(uint netID)
        {
            ...
            if (_controller.CrateToTypeLootTable.ContainsKey(netID))
            {
                if (_controller.CrateToTypeLootTable[netID] == 3) return null;
                else return true;
            }
            else return null;
        }
        ...
        private object OnContainerPopulate(LootContainer container)
        {
            ...
            if (_controller.CrateToTypeLootTable.ContainsKey(container.net.ID))
            {
                if (_controller.CrateToTypeLootTable[container.net.ID] == 6) return null;
                else return true;
            }
            else return null;
        }
        ...
        private static DroppedItem SpawnDroppedItem(string shortname, Vector3 pos, Quaternion rot, bool allowPickup = false)
        {
            ...
            UnityEngine.Object.Destroy(droppedItem.GetComponent<Rigidbody>());
