...
{
    [Info("Advanced Airstrike", "k1lly0u", "0.1.78")]
    class AdvAirstrike : RustPlugin
    {
        #region Fields
        [PluginReference]
        private Plugin Economics, ServerRewards, Spawns, NoEscape;
        ...
        private readonly List<StrikeType> availableTypes = new List<StrikeType>();
        private readonly List<RadiationEntry> radiationZones = new List<RadiationEntry>();

        private readonly List<BaseEntity> strikeSignals = new List<BaseEntity>();

        private static AdvAirstrike ins;

        private const string CARGOPLANEPREFAB = "assets/prefabs/npc/cargo plane/cargo_plane.prefab";
        private const string BASICROCKET = "assets/prefabs/npc/patrol helicopter/rocket_heli.prefab";
        private const string FIREROCKET = "assets/prefabs/npc/patrol helicopter/rocket_heli_napalm.prefab";
        private const string TANKSHELL = "assets/prefabs/npc/m2bradley/maincannonshell.prefab";
        private const string HELIEXPLOSION = "assets/prefabs/npc/patrol helicopter/effects/heli_explosion.prefab";
        private const string FIREBALL = "assets/bundled/prefabs/oilfireballsmall.prefab";
        private const string C4EXPLOSION = "assets/prefabs/tools/c4/effects/c4_explosion.prefab";
        private const string BULLETEXP = "assets/bundled/prefabs/fx/impacts/additive/explosion.prefab";
        private const string FIRECANNONEFFECT = "assets/prefabs/npc/m2bradley/effects/maincannonattack.prefab";

        private static readonly int LAYERMASKEXPL = LayerMask.GetMask("Deployed", "AI", "Vehicle_Movement", "Player_Server", "Construction");

        private enum StrikeType { None, Strike, Squad, Napalm, Nuke, Spectre }

        private enum FireType { Rocket, Bullet, Cannon, Combined }
        #endregion
        ...
        private void Loaded()
        {
            ...
            foreach(object type in Enum.GetValues(typeof(StrikeType)))
            {
        ...
        private void OnExplosiveThrown(BasePlayer player, SupplySignal supplySignal)
        {
            ...
            StrikeType strikeType = StrikeType.None;

            if (supplySignal.skinID != 0UL)
            {
                foreach(KeyValuePair<StrikeType, OtherOptions.StrikeSignal> kvp in configData.Other.SignalSkinIDs)
                {
                    if (kvp.Value.SkinID == supplySignal.skinID)
                    {
                        strikeType = kvp.Key;
                        break;
                    }
            ...
            if (strikeType != StrikeType.None)
            {
                strikeSignals.Add(supplySignal);

                supplySignal.CancelInvoke(supplySignal.Explode);
                supplySignal.Invoke(() =>
                {
                    strikeSignals.Remove(supplySignal);
                    supplySignal.KillMessage();
                }, 30f);

                timer.Once(3, () =>
                {
                    Effect.server.Run("assets/bundled/prefabs/fx/smoke_signal_full.prefab", supplySignal, 0, Vector3.zero, Vector3.zero);
                    SendStrike(player, strikeType, supplySignal.transform.position);
                });
            }
        }
        #endregion
        ...
        private void PrepareRandomStrikes()
        {
            ...
            foreach (string type in configData.Other.RandomStrikes)
        ...
        private bool CanBuyStrike(BasePlayer player, StrikeType type)
        {
            ...
            foreach (KeyValuePair<string, int> item in costToBuy)
            {
                if (item.Key == "RP")
                {
                    if (ServerRewards)
                ...
                if (ItemManager.itemDictionaryByName.TryGetValue(item.Key, out itemDefinition))
                {
                    if (player.inventory.GetAmount(itemDefinition.itemid) < item.Value)
                    {
                        SendReply(player, string.Format(msg("buyItem", player.UserIDString), item.Value, itemDefinition.displayName.translated));
                        Effect.server.Run("assets/prefabs/locks/keypad/effects/lock.code.denied.prefab", player.transform.position);
                        return false;
                    }
                }
        ...
        private void BuyStrike(BasePlayer player, StrikeType type, Vector3 targetPosition, string targetName = "")
        {
            ...
            foreach (KeyValuePair<string, int> item in costToBuy)
            {
                ...
                ItemDefinition itemDefinition;
                if (ItemManager.itemDictionaryByName.TryGetValue(item.Key, out itemDefinition))
                    player.inventory.Take(null, itemDefinition.itemid, item.Value);
            }
        ...
        private CargoPlane CreatePlane() => (CargoPlane)GameManager.server.CreateEntity(CARGOPLANEPREFAB, Vector3.zero, Quaternion.identity, true);
        ...
        private string FormatTime(double time)
        {
            ...
            return string.Format("{0:00}:{1:00}:{2:00}", dateDifference.Hours + (dateDifference.Days * 24), dateDifference.Minutes, dateDifference.Seconds);
        }
                ...
                switch (args[0].ToLower())
                {
                    case "signal":
                        if ((type == StrikeType.Strike && configData.Other.SignalStrike) || (type == StrikeType.Squad && configData.Other.SignalSquad) || (type == StrikeType.Napalm && configData.Other.SignalNapalm) || (type == StrikeType.Nuke && configData.Other.SignalNuke) || (type == StrikeType.Spectre && configData.Other.SignalSpectre))
                        {
                            ...
                            OtherOptions.StrikeSignal signal;
                            if (!configData.Other.SignalSkinIDs.TryGetValue(type, out signal))
                            {
                                SendReply(player, msg("signalTypeNotSetup", player.UserIDString));
                                return;
                            }

                            Item item = ItemManager.CreateByName("supply.signal", 1, signal.SkinID);
                            item.name = signal.DisplayName;
        ...
        private class OtherOptions
        {
            ...
            [JsonProperty(PropertyName = "Airstrike supply signal skin IDs")]
            public Hash<StrikeType, StrikeSignal> SignalSkinIDs { get; set; }
        ...
        private ConfigData GetBaseConfig()
        {
            return new ConfigData
            {
                ...
                Other = new OtherOptions
                {
                    ...
                    SignalNuke = true,
                    SignalSpectre = true,
                    SignalSkinIDs = new Hash<StrikeType, OtherOptions.StrikeSignal>
                    {
                        [StrikeType.Napalm] = new OtherOptions.StrikeSignal { DisplayName = "Napalm Strike Signal", SkinID = 2791026167 },
                        [StrikeType.Nuke] = new OtherOptions.StrikeSignal { DisplayName = "Nuke Strike Signal", SkinID = 2791029890 },
                        [StrikeType.Spectre] = new OtherOptions.StrikeSignal { DisplayName = "Spectre Strike Signal", SkinID = 2791030040 },
                        [StrikeType.Squad] = new OtherOptions.StrikeSignal { DisplayName = "Squad Strike Signal", SkinID = 2791030261 },
                        [StrikeType.Strike] = new OtherOptions.StrikeSignal { DisplayName = "Airstrike Signal", SkinID = 2791030397 }
                    },
        ...
        private void UpdateConfigValues()
        {
            ...
            if (configData.Version < new VersionNumber(0, 1, 77))
                configData.Other.SignalSkinIDs = baseConfig.Other.SignalSkinIDs;
        ...
        Dictionary<string, string> Messages = new Dictionary<string, string>
        {
            ...
            ["signalTypeNotSetup"] = "<color=#939393>That strike type has not been setup to work with supply signals</color>",
