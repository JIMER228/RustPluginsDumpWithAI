...
namespace Oxide.Plugins
{
    ...
    public class RoamTasks : RustPlugin
    {
        ...
        private float uiBarFadeInDuration = 1f;
        private float uiBarFadeOutDuration = 1f;
        ...
        private class PluginConfig
        {
            ...
            public Dictionary<string, List<TaskConfig>> HookTasks { get; set; } = new Dictionary<string, List<TaskConfig>>
            {
                ...
                {
                    "OnFishCaught", new List<TaskConfig>
                    {
        ...
        private void AddLatestTasks()
        {
            ...
            if (!TaskExists("attackhelidriver"))
            {
                var taskToAdd = new TaskConfig
                {
                    Name = "Fly Attackheli", Enabled = true, MaxComplete = 6, RoamDuration = 800, RequiredAmount = 5000, RewardPool = "Tier3", ShortName = new List<string> { "attackhelidriver" }
                };
                config.HookTasks["OnEntityMounted"].Add(taskToAdd);
                Puts($"Added new Task - {taskToAdd.Name}");
            }
            
            if (!TaskExists("parachuteseat"))
            {
                var taskToAdd = new TaskConfig
                {
                    Name = "Use Parachutes", Enabled = true, MaxComplete = 6, RoamDuration = 1200, RequiredAmount = 1500, RewardPool = "Tier3", ShortName = new List<string> { "parachuteseat" }
                };
                config.HookTasks["OnEntityMounted"].Add(taskToAdd);
                Puts($"Added new Task - {taskToAdd.Name}");
            }
       ...
       private void SendRoamMessage(int nextRoamInterval, int timeOffset)
        ...
        private void RoamTaskConsoleCmd(ConsoleSystem.Arg arg)
        {
            ...
            switch (subCommand)
            {
                ...
                if (!isUiBarVisible)
                {
                    ...
                    if (!string.IsNullOrEmpty(specifiedTask) && string.IsNullOrEmpty(selectedTask))
                    {
        ...
        private void OnCollectiblePickup(CollectibleEntity entity, BasePlayer player)
        {
            if (player == null || playersWithCompletedTask.Contains(player))
                return;
            if (selectedHook == "OnCollectiblePickup") 
            {
                List<string> selectedTaskHookShortNames = GetSelectedTaskHookShortName().Split(',').Select(name => name.ToLower()).ToList();
                string itemShortname = entity.itemList.FirstOrDefault()?.itemDef.shortname.ToLower();
                if (selectedTaskHookShortNames.Contains(itemShortname))
                {
                    IncrementPlayerPoints(player, (int)entity.itemList.Sum(item => item.amount));
                    int requiredAmount = GetSelectedTaskRequiredAmount();
                    if (GetPlayerPoints(player) >= requiredAmount) 
                    {
                        CompleteTask(player);
        ...
        private void OnFishCaught(ItemDefinition definition, BaseFishingRod rod, BasePlayer player)
        {
            if (player == null || playersWithCompletedTask.Contains(player))
                return;
            if (selectedHook == "OnFishCaught")
            {
                List<string> selectedTaskHookShortNames = GetSelectedTaskHookShortName().Split(',').ToList();
                string itemShortname = definition.shortname.ToLower();
                if (selectedTaskHookShortNames.Contains(itemShortname))
                {
                    IncrementPlayerPoints(player, 1);
                    int requiredAmount = GetSelectedTaskRequiredAmount();
            ...
            foreach (int rewardType in rewardConfig.RewardType)
            {
                switch (rewardType)
                {
                    case 1: 
                        ...
                        break;
                    case 2: 
                        rewardDisplayNames.Add(rewardConfig.CommandRewardCustomName);
                        break;
                    case 3: 
                        rewardDisplayNames.Add($"{rewardConfig.EconomicsRewardAmount} {rewardConfig.EconomicsRewardCustomName}");
                        break;
                    case 4: 
                        rewardDisplayNames.Add($"{rewardConfig.ServerRewardsAmount} {rewardConfig.ServerRewardsRewardCustomName}");
                        break;
                }
            }
        ...
        private void GiveItemReward(BasePlayer player, RewardConfig reward)
        {
            ...
            {
                GiveItem(player, rewardItem.ItemType, rewardItem.ItemAmount, rewardItem.ItemSkinID);
                ItemDefinition itemDefinition = ItemManager.FindItemDefinition(rewardItem.ItemType);
                string itemDisplayName = string.IsNullOrEmpty(rewardItem.ItemCustomName) ? itemDefinition.displayName.english : rewardItem.ItemCustomName;
                string rewardDisplayName = $"{rewardItem.ItemAmount} {itemDisplayName}";
                string rewardDescription = itemDisplayName;
            }
        }
        ...
        private void CompleteTask(BasePlayer player)
        {
            ...
            CuiHelper.AddUi(player, temporaryUiBarContainer);
            timer.Once(4.3f, () => 
            {
                DestroyUiElementsTemp(player);
            });
        ...
        private void OnSendCommand(Connection cn, string command, object[] args)
        {
            ...
            string userName = args[2]?.ToString()?.Trim();
            if (userName != null && config != null && config.General != null)
            {
                if (config.General.scientistName != null)
                {
                    ...
                    {
                        ApplySteamAvatarUserID(command, args);
                    }
                }
                else
                {
                    PrintWarning("config.General.scientistName is null.");
                }
            }
            else
            {
                PrintWarning("One or more objects in the hierarchy are null.");
            }
        }
            ...
            if (uiBarContainer != null)
            {
                ...
                var completionLabel = new CuiElement
                {
                    ...
                    Components =
                    {
                        ...
                        new CuiRectTransformComponent { AnchorMin = "0 0", AnchorMax = "0.1 0.625" },
                        new CuiOutlineComponent { Color = "0 0 0 0.4", Distance = "0.4 0.4", UseGraphicAlpha = true }
                    }
                };
