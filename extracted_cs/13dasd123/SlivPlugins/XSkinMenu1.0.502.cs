...
{ 
    ...
    { 
		...
		{ 
            var Data = Interface.Oxide.DataFileSystem.ReadObject<Data>($"Admin/XSkinMenu/UserSettings/{player.userID}");
		...
		private void SaveData(BasePlayer player) => Interface.Oxide.DataFileSystem.WriteObject($"Admin/XSkinMenu/UserSettings/{player.userID}", StoredData[player.userID]);
	    ...
	    private void Unload()
		{
			...
			Interface.Oxide.DataFileSystem.WriteObject("Admin/XSkinMenu/Friends", StoredDataFriends);
		}
		...
		private void ccmdAdmin(ConsoleSystem.Arg args)
		{
			if (args.Player() == null || args.Player().IsAdmin)
			{
				...
				Interface.Oxide.DataFileSystem.WriteObject("Admin/XSkinMenu/Skins", StoredDataSkins);
			}
		}
	    ...
	    private void OnServerInitialized()
		{
			...
			if (Interface.Oxide.DataFileSystem.ExistsDatafile("Admin/XSkinMenu/Friends"))
                StoredDataFriends = Interface.Oxide.DataFileSystem.ReadObject<Dictionary<ulong, bool>>("Admin/XSkinMenu/Friends");			
			if (Interface.Oxide.DataFileSystem.ExistsDatafile("Admin/XSkinMenu/Skins"))
                StoredDataSkins = Interface.Oxide.DataFileSystem.ReadObject<Dictionary<string, List<ulong>>>("Admin/XSkinMenu/Skins");			
			...
			timer.Every(200, () => Interface.Oxide.DataFileSystem.WriteObject("Admin/XSkinMenu/Friends", StoredDataFriends));
		...
		private void GUI(BasePlayer player)
		{
			...
			container.Add(new CuiPanel
            {
                ...
                Image = { Color = HexToCuiColor("#161617"), Material = "assets/icons/greyout.mat" }
            }, "Overlay", ".GUIS");
			...
			container.Add(new CuiPanel
            {
                ...
                Image = { Color = HexToCuiColor("#161617"), Material = "assets/content/ui/uibackgroundblur.mat" }
            }, ".SGUI");				
		...
		private void CategoryGUI(BasePlayer player, int page = 0)
		{
			...
			foreach(var category in config.Category)
			{
				...
				container.Add(new CuiButton
                {
                    ...
                    Button = { Color = HexToCuiColor("#eb4034", 50f), Material = "assets/icons/greyout.mat", Command = $"skin_c category {category.Key} {x}" },
		...
		private void ItemGUI(BasePlayer player, string category, int Page = 0)
		{
				...
				if(StoredDataSkins.ContainsKey(item) && StoredDataSkins[item].Count != 0 && StoredData[player.userID].Skins.ContainsKey(item))
				{
				    ...
				    if(StoredData[player.userID].Skins[item] != 0)
				        container.Add(new CuiButton
                        {
                            ...
                            Button = { Color = HexToCuiColor("#161617"), Sprite = "assets/icons/clear.png", Command = $"skin_c clear {item} {z}" },
                            Text = { Text = "" }
                        }, ".Item", $".I + {z}");
				}
			...
			container.Add(new CuiPanel
            {
                ...
                Image = { Color = HexToCuiColor("#161617"), Material = "assets/content/ui/uibackgroundblur-ingamemenu.mat" }
            }, ".ItemGUI");			
			...
			if(config.Setting.ButtonClear)
			{
				container.Add(new CuiButton
				{
					...
					Button = { Color = HexToCuiColor("#ff4b4b"), Command = "skin_c clearall" },
					Text = { Text = lang.GetMessage("CLEARALL", this, player.UserIDString), Align = TextAnchor.MiddleCenter, FontSize = 12, Font = "robotocondensed-regular.ttf", Color = HexToCuiColor("#ffffff") }
				}, ".ItemGUI");
		...
		private void SkinGUI(BasePlayer player, string item, int Page = 0)
		{
			...
			{		
                ...
                {
                    RectTransform = { AnchorMin = "0.5 0.5", AnchorMax = "0.5 0.5", OffsetMin = $"{-497.5 + (x * 100)} {123.25 - (y * 100)}", OffsetMax = $"{-402.5 + (x * 100)} {218.25 - (y * 100)}" },
                    Image = { Color = s == skin ? HexToCuiColor("#161617") : HexToCuiColor("#161617"), Material = "assets/content/ui/uibackgroundblur-ingamemenu.mat" }
                }, ".SkinGUI", ".Skin");
		...
		private void SettingGUI(BasePlayer player)
		{
			...
			{		
			    container.Add(new CuiPanel
                {
                    ...
                    Image = { Color = HexToCuiColor("#ff4b4b", 50f), Material = "assets/content/ui/uibackgroundblur-ingamemenu.mat" }
                }, ".SGUIMM", ".SM");
