...
namespace Oxide.Plugins
{
    ...
    class Biplane : RustPlugin
    {
        ...
        void OnServerInitialized()
        {
            ...
            if (_config.spawnSetting.monumentSpawnSettings.Any(x => x.Value.enableSpawn)) spawnCorountine = ServerMgr.Instance.StartCoroutine(SpawnCorountine());
            ...
            foreach (ModularCar modularCar in ModularCar.allCarsList)
            {
                ...
                if (modularCar.NumAttachedModules == 0 || (modularCar.NumAttachedModules == 1 && baseVehicleModule != null && baseVehicleModule.name == "assets/content/vehicles/modularcar/module_entities/1module_engine.prefab")) LoadBiplane(modularCar, saveBiplaneData);
            }

            saveCoroutine = ServerMgr.Instance.StartCoroutine(SaveCorountine());
        }
        ...
        object CanMountEntity(BasePlayer player, BaseMountable entity)
        {
            ...
            if (biplaneComponent != null)
            {
                ...
                else if (!biplaneComponent.modularCar.PlayerHasUnlockPermission(player))
                {
        ...
        {
            foreach (BiplaneComponent biplaneComponent in biplanes.Where(x => !x.grounded))
            {
                if (biplaneComponent == null) continue;
                BuildingPrivlidge buildingPrivlidge = samSite.GetBuildingPrivilege();
                if (buildingPrivlidge != null && biplaneComponent.modularCar.mountPoints.Any(x => x.mountable._mounted != null && buildingPrivlidge.IsAuthed(x.mountable._mounted))) continue;
                if (samSite.ShortPrefabName == "sam_static")
                {
                    if (_config.samSiteMonuments) targetList.Add(biplaneComponent.samSiteComponent);
                    continue;
                }
                else if (_config.samSite) targetList.Add(biplaneComponent.samSiteComponent);
            }
        }
        ...
        object OnEngineStart(BaseVehicle vehicle, BasePlayer driver)
        {
            ...
            if (biplaneComponent == null) return null;
            if (!biplaneComponent.modularCar.PlayerHasUnlockPermission(driver))
            {
        ...
        object CanLootEntity(BasePlayer player, StorageContainer container)
        {
            ...
            if (!biplaneComponent.modularCar.PlayerHasUnlockPermission(player))
            {
                PrintToChat(player, GetMessage("Lock", player.UserIDString, ins._config.prefics));
                return true;
            }
        ...
        #endregion Methods
        public class BiplaneComponent : FacepunchBehaviour
        {
            internal Coroutine controlCorountine;
            internal SAMTargetComponent samSiteComponent;
            ...
            internal void Init(string presetName, bool firstSpawn = true, bool addModule = true)
            {
                ...
                if (ins._config.samSite) samSiteComponent = gameObject.AddComponent<SAMTargetComponent>();
            }
            ...
            IEnumerator ControlCorountine()
            {
                ...
                while (!grounded || pilotSeat._mounted != null)
                {
                    ...
                    if (player != null)
                    {
                        ...
                        if (upDown != 0) rigidbody.AddTorque(modularCar.transform.right * upDown * -4000 * speedScale);

                        if (!grounded)
                        {

                            rigidbody.AddTorque(modularCar.transform.up * leftRight * 4000 * speedScale);
                            if (player.serverInput.IsDown(BUTTON.LEFT)) rigidbody.AddTorque(modularCar.transform.forward * 4000 * speedScale);
                            if (player.serverInput.IsDown(BUTTON.RIGHT)) rigidbody.AddTorque(modularCar.transform.forward * -4000 * speedScale);
                        }
                    }
            ...
            void SpawnSubEntity()
            {
                foreach (SubEntityData subEntityData in ins.subEntityDatas)
                {
                    ...
                    {
                        BaseCombatEntity newEntity = entity.gameObject.AddComponent<BaseCombatEntity>();
            ...
            void FireBomb()
            {
                ...
                delay = biplaneSetting.rocketTime;
            ...
            void FireRocker()
            {
                ...
                left = !left;
                Vector3 vector3;
                if (left) vector3 = rockets[0].transform.position + modularCar.transform.forward.normalized * 3;
                else vector3 = rockets[1].transform.position + modularCar.transform.forward.normalized * 3;
        ...
        internal class SAMTargetComponent : FacepunchBehaviour, SamSite.ISamSiteTarget
        {
            public BaseEntity baseEntity;

            private void Awake() => baseEntity = GetComponent<BaseEntity>();
        ...
        {
            public uint biplaneId;
            public string biplanePrefab;
            public int keyIndex;
            public List<SaveItemData> items = new List<SaveItemData>();

            public SaveBiplaneData(uint biplaneId, string biplanePrefab, int keyIndex)
            {
