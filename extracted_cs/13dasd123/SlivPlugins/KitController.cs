...
{
    [Info("KitController", "Amino", "2.1.0")]
    ...
    public class OldItemData
        ...
        {
            if (plugin.Name == "WelcomeController") usingWC = false;
        ...
        private void AddItemData(ref List<kitContents> kitContents, OldItemData[] oldItemData, string container)
        {
            ...
            {
                var theItem = ItemManager.FindItemDefinition(item.Shortname);
        ...
        CanPurchase CanBuy(BasePlayer player, aKit kit)
        {
            ...
            switch (kitCurrency)
            {
                ...
                case "economics":
                    var eco = Economics?.Call("Balance", player.userID);
                ...
                default:
                    ...
                    var inventoryItemCount = player.inventory.AllItems().Where(x => x.info.shortname.Equals(theItem.info.shortname)).Sum(x => x.amount);
        ...
        void BuyKit(BasePlayer player, aKit kit)
        {
            ...
            {
                case "rp":
                    ServerRewards?.Call("TakePoints", player.userID, kit.Cost);
                    break;
                case "economics":
                    Economics?.Call("Withdraw", player.userID, (double)kit.Cost);
                    break;
                default:
                    List<ItemChange> needsEditedList = Facepunch.Pool.GetList<ItemChange>();
                    List<ItemChange> needsRemovedList = Facepunch.Pool.GetList<ItemChange>();
                    int amountLeft = kit.Cost;

                    foreach (var item in player.inventory.AllItems().Where(x => x.info.shortname.Equals(kit.Currency, StringComparison.OrdinalIgnoreCase)))
                    {
        ...
        void GiveAutoKit(BasePlayer player)
        {
            ...
            if (kitInfo != null && kitInfo.Items.Count > 0)
            {
                ...
                {
                    Item createdItem = ItemManager.CreateByItemID(item.ItemId, item.Amount, item.Skin);
        ...
        void HandleItem(ref List<kitContents> savedItems, Item item, string section)
        {
            if (item.info.category.ToString() == "Weapon")
            {
                ...
                if (weapon != null && weapon.primaryMagazine != null)
                    savedItems.Add(new kitContents { ItemId = item.info.itemid, Shortname = item.info.shortname, Skin = item.skin, Position = item.position, Amount = item.amount, AmmoType = weapon.primaryMagazine.ammoType.shortname, Ammo = weapon.primaryMagazine.capacity, Container = section, Contents = mods });
                else savedItems.Add(new kitContents { ItemId = item.info.itemid, Shortname = item.info.shortname, Skin = item.skin, Position = item.position, Amount = item.amount, Contents = mods, Container = section });
            }
            else savedItems.Add(new kitContents { ItemId = item.info.itemid, Shortname = item.info.shortname, Skin = item.skin, Position = item.position, Amount = item.amount, Container = section });
        }
        ...
        ClaimKitResponse ClaimKit(BasePlayer player, string kitName, bool fromAdmin = false, bool fromSpawn = false)
        {
            ...
            if (kitInfo.Items != null && kitInfo.Items.Count > 0)
            {
                ...
                if (kitInfo.Items.Count(x => x.Container == "main") > mainSpaceLeft) return new ClaimKitResponse { CanRedeem = false, Reason = Lang("ChatTooManyItems") };
                ...
                foreach (var item in kitInfo.Items)
                {
                    ...
                    var createdItem = ItemManager.CreateByItemID(item.ItemId);
        ...
        void UICreateKits(BasePlayer player, PlayerOptions playerOptions, int page = 0, string kitName = null)
        {
            ...
            var kits = _kitData.Where(x => isAdmin || !x.Hidden || string.IsNullOrEmpty(x.Permission) ? true : permission.UserHasPermission(player.UserIDString, x.Permission));
            if (!string.IsNullOrEmpty(playerOptions.ActiveCategory) && !playerOptions.ActiveCategory.Equals("all", StringComparison.OrdinalIgnoreCase)) kits = _kitData.Where(x => x.Category.Equals(playerOptions.ActiveCategory, StringComparison.OrdinalIgnoreCase));
