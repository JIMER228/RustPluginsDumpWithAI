...
{
    [Info("XDCobaltLaboratory", "DezLife", "2.6.0")]
    internal class XDCobaltLaboratory : RustPlugin
    {
        ...
        private class Configuration
        {
            ...
            internal class SpawnPositionGenerateSetting
            {
                ...
                [JsonProperty(Ru ? "Разрешить спавн на берегах" : "Allow spawn on the beach")]
                public bool spawnOnBeach = false;
        ...
        private const float MaxElevation = 2.0f;
        ...
        private bool IsFlat(ref Vector3 position)
        {
        ...
        private void PosValidation(Vector3 pos)
        {
            ...
            if (WaterLevel.GetWaterDepth(pos, false, false) > 0.5f)
            {
                return;
            }
            ...
            if (!config.spawnPositionGenerateSetting.spawnOnRiver && ContainsTopology(TerrainTopology.Enum.Riverside | TerrainTopology.Enum.River, pos, 12.5f))
            {
                return;
            }
            
            if (!config.spawnPositionGenerateSetting.spawnOnBeach && ContainsTopology(TerrainTopology.Enum.Beach | TerrainTopology.Enum.Beachside | TerrainTopology.Enum.Ocean | TerrainTopology.Enum.Oceanside, pos, 20f))
            {
                return;
            }
        ...
        // public static List<string> GetTopologyStrings(int topologyMask)
        // {
        //     List<string> topologyList = new List<string>();
        //
        //     // Проходим по всем значениям перечисления
        //     foreach (TerrainTopology.Enum topology in Enum.GetValues(typeof(TerrainTopology.Enum)))
        //     {
        //         // Если значение из перечисления входит в маску, добавляем в список
        //         if ((topologyMask & (int)topology) != 0)
        //         {
        //             topologyList.Add(topology.ToString());
        //         }
        //     }
        //
        //     return topologyList;
        ...
        private void OnCrateHack(HackableLockedCrate crate)
        {
            ...
            if (crate == CrateEnt)
            {
                ...
                SendChatAll(helpSpawn ? "XD_IVENT_CRATEHACKHELP" : "XD_IVENT_CRATEHACK", crate.originalHackerPlayer.displayName, PosIvent);
                Interface.CallHook("OnHackCobaltCrate", crate.originalHackerPlayer);
            }
        }
        ...
        private IEnumerator SetupBuilding()
        {
            ...
            if (comp != null && cam.Count > 0)
            {
                ...
                {
                    comp.ForceAddBookmark(sd.GetIdentifier());
                }
            }
        ...
        public class NpcZones : MonoBehaviour
        {
            ...
            internal List<BasePlayer> playersInZone = Pool.Get<List<BasePlayer>>();
