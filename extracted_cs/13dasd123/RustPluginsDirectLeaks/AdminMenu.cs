...
{
    [Info("AdminMenu", "0xF // dsc.gg/0xf-plugins", "1.3.1")]
    ...
    public class AdminMenu : RustPlugin
    {
        ...
        public class Configuration
        {
            ...
            public class LogsProperties
            {
                ...
                public string Info { get; } = "You can override the url for each log type, just specify the url as string instead of true/false.";

                [JsonProperty(PropertyName = "Give")]
                public object Give { get; set; } = true;

                [JsonProperty(PropertyName = "Admin teleports")]
                public object AdminTeleport { get; set; } = true;

                [JsonProperty(PropertyName = "Spectate")]
                public object Spectate { get; set; } = true;

                [JsonProperty(PropertyName = "Heal")]
                public object Heal { get; set; } = true;

                [JsonProperty(PropertyName = "Kill")]
                public object Kill { get; set; } = true;

                [JsonProperty(PropertyName = "Look inventory")]
                public object LookInventory { get; set; } = true;

                [JsonProperty(PropertyName = "Strip inventory")]
                public object StripInventory { get; set; } = true;

                [JsonProperty(PropertyName = "Blueprints")]
                public object Blueprints { get; set; } = true;

                [JsonProperty(PropertyName = "Mute/Unmute")]
                public object MuteUnmute { get; set; } = true;

                [JsonProperty(PropertyName = "Toggle Creative")]
                public object ToggleCreative { get; set; } = true;

                [JsonProperty(PropertyName = "Cuff")]
                public object Cuff { get; set; } = true;

                [JsonProperty(PropertyName = "Kick the player")]
                public object Kick { get; set; } = true;

                [JsonProperty(PropertyName = "Ban the player")]
                public object Ban { get; set; } = true;

                [JsonProperty(PropertyName = "Using custom buttons")]
                public object CustomButtons { get; set; } = true;

                [JsonProperty(PropertyName = "Spawn entities")]
                public object SpawnEntities { get; set; } = true;

                [JsonProperty(PropertyName = "Set time")]
                public object SetTime { get; set; } = true;

                [JsonProperty(PropertyName = "ConVars")]
                public object ConVars { get; set; } = true;

                [JsonProperty(PropertyName = "Plugin Manager")]
                public object PluginManager { get; set; } = true;

                [JsonProperty(PropertyName = "Permission Manager")]
                public object PermissionManager { get; set; } = true;
            }
        }
        ...
        [HarmonyPatch(typeof(ConVar.Server), "readcfg")]
        private static class ConVar_Server_readcfg_Patch
        {
            private static void Postfix()
            {
                string serverFolder = ConVar.Server.GetServerFolder("cfg");
                string adminMenuConfigPath = Path.Combine(serverFolder, "adminmenu.cfg");
                if (File.Exists(adminMenuConfigPath))
                    ConsoleSystem.RunFile(ConsoleSystem.Option.Server.Quiet(), File.ReadAllText(adminMenuConfigPath));
            }
        }
        ...
        public class Translator15
        {
            ...
            {
                if (ignoreLang.Contains(language))
                    return;
            ...
            {
                return Instance.lang.GetLanguage(userIDString);
            }
            ...
            public string Get(string key, string def = null)
            {
                ...
                if (Translator15.translations.TryGetValue(language, out var lang) && lang.TryGetValue(key, out string result))
                    return result;
                return def;
            }
        ...
        public class CUI
        {
            ...
            private static readonly Dictionary<TimerFormat, string> TimerFormatToString = new Dictionary<TimerFormat, string>
            {
                {
                    TimerFormat.None,
                    TimerFormat.None.ToString()
                },
                {
                    TimerFormat.SecondsHundreth,
                    TimerFormat.SecondsHundreth.ToString()
                },
                {
                    TimerFormat.MinutesSeconds,
                    TimerFormat.MinutesSeconds.ToString()
                },
                {
                    TimerFormat.MinutesSecondsHundreth,
                    TimerFormat.MinutesSecondsHundreth.ToString()
                },
                {
                    TimerFormat.HoursMinutes,
                    TimerFormat.HoursMinutes.ToString()
                },
                {
                    TimerFormat.HoursMinutesSeconds,
                    TimerFormat.HoursMinutesSeconds.ToString()
                },
                {
                ...
                {
                    TimerFormat.HoursMinutesSecondsTenths,
                    TimerFormat.HoursMinutesSecondsTenths.ToString()
                },
                {
                    TimerFormat.DaysHoursMinutes,
                    TimerFormat.DaysHoursMinutes.ToString()
                },
                {
                    TimerFormat.DaysHoursMinutesSeconds,
                    TimerFormat.DaysHoursMinutesSeconds.ToString()
                },
                {
                    TimerFormat.Custom,
                    TimerFormat.Custom.ToString()
                },
            };
            ...
            public class Element : CuiElement
            {
                ...
                public class ComponentList : List<ICuiComponent>
                {
                    ...
                    public ComponentList AddCountdown(string command = null, float endTime = 0, float startTime = 0, float step = 1, float interval = 1f, TimerFormat timerFormat = TimerFormat.None, string numberFormat = "0.####", bool destroyIfDone = true)
                    {
                        Add(new CuiCountdownComponent { Command = command, EndTime = endTime, StartTime = startTime, Step = step, Interval = interval, TimerFormat = timerFormat, NumberFormat = numberFormat, DestroyIfDone = destroyIfDone });
                        return this;
                    }
                }
            }
        ...
        public class ConVarsContent : Content
        {
            ...
            public static List<ConsoleSystem.Command> modifiedConvars = new List<ConsoleSystem.Command>();
        ...
        public class GiveMenuContent : Content
        {
            ...
            private static readonly Label OWNERSHIP_LABEL = new Label("OWNERSHIP");
            private static readonly Label SEARCH_LABEL = new Label("Search..");
            private static readonly Label ITEM_COUNTER_LABEL = new Label("{0} items");
            private static readonly Label NONE_LABEL = new Label("NONE");
            private static List<Timer> sequentialLoad = new List<Timer>();
            internal static Translate.Phrase[] OwnerShipPhrases = typeof(ItemOwnershipPhrases).GetFields(BindingFlags.Public | BindingFlags.Static).Select(x => x.GetValue(null) as Translate.Phrase).ToArray();
            public override void LoadDefaultUserData(Dictionary<string, object> userData)
            {
            ...
            private void Popup(CUI.Element root, ConnectionData connectionData, Dictionary<string, object> userData)
            {
                ...
                Translate.Phrase ownershipPhrase = (Translate.Phrase)userData["givemenu.popup.ownershipPhrase"];
                ...
                var panel = backgroundButton.AddButton(command: null, anchorMin: "0.5 0.5", anchorMax: "0.5 0.5", offsetMin: "-200 -262.5", offsetMax: "200 262.5").AddPanel(color: "0.05 0.05 0.05 1");
                ...
                var outer = panel.AddPanel(color: "0.1 0.1 0.1 1", anchorMin: "0 0", anchorMax: "1 0", offsetMin: "0 0", offsetMax: "0 225", name: "GiveMenu_Outer");
                ...
                outer.AddText(text: OWNERSHIP_LABEL.Localize(connectionData.connection), font: CUI.Font.RobotoCondensedBold, fontSize: 18, align: TextAnchor.MiddleCenter, overflow: VerticalWrapMode.Truncate, anchorMin: "0.5 1", anchorMax: "0.5 1", offsetMin: "-170 -180", offsetMax: "-60 -155");
                var hideCheckbox = outer.AddButton(command: "adminmenu givemenu.popup show_ownership_phrases", color: "0.05 0.05 0.05 1", anchorMin: "0.5 1", anchorMax: "0.5 1", offsetMin: "-40 -180", offsetMax: "170 -155").AddText(text: ownershipPhrase?.english ?? NONE_LABEL.Localize(connectionData.connection), font: CUI.Font.RobotoCondensedRegular, fontSize: 14, align: TextAnchor.MiddleLeft, offsetMin: "10 0", offsetMax: "-10 0");
            ...
            public void ShowOwnershipDropdown(Connection connection)
            {
                CUI.Root root = new CUI.Root("GiveMenu_Outer");
                int count = OwnerShipPhrases.Length;
                int buttonHeight = 20;
                var container = root.AddPanel(color: "0 0 0 0", anchorMin: "0.5 1", anchorMax: "0.5 1", offsetMin: $"-40 -{155 + (7 * buttonHeight)}", offsetMax: $"170 -{155}");
                container.Components.AddScrollView(vertical: true, scrollSensitivity: 10, anchorMin: "0 1", anchorMax: "1 1", offsetMin: $"0 -{count * buttonHeight}");
                for (int i = 0; i < count; i++)
                {
                    Translate.Phrase phrase = OwnerShipPhrases[i];
                    container.AddButton(command: $"adminmenu givemenu.popup select_ownership {phrase.token}", color: i % 2 == 0 ? "0.13 0.13 0.13 1" : "0.1 0.1 0.1 1", anchorMin: "0 1", anchorMax: "1 1", offsetMin: $"0 -{(i + 1) * buttonHeight}", offsetMax: $"0 -{i * buttonHeight}").AddText(text: phrase.english, fontSize: 12, align: TextAnchor.MiddleCenter);
                }
        ...
        public class UserInfoContent : Content
        {
            ...
            protected override void Render(CUI.Element root, ConnectionData connectionData, Dictionary<string, object> userData)
            {
                ...
                bool disablePlayerAvatars = config.DisablePlayerAvatars;
                if (!disablePlayerAvatars && hasAvatar)
                {
                ...
                else
                {
                    ...
                    if (!disablePlayerAvatars && steamInfo != null && Instance.ImageLibrary != null)
                    {
                ...
                List<string> leftColumn = new List<string>
                {
                    ...
                    string.Format(GRID_LABEL.Localize(connectionUserId), (position != Vector3.zero ? MapHelper.PositionToString(position) : UNKNOWN_LABEL.Localize(connectionUserId))),
            ...
            switch (command)
            {
                ...
                case "permissionmanager.usergroups":
                    ...
                    if (connectionData != null)
                    {
                        ...
                        TryLogToDiscord(config.Logs.PermissionManager, connection, isGrant ? "Grant user group to the player" : "Revoke user group from the player", $"The administrator change **{groupName}** user group to **[{ServerMgr.Instance.persistance.GetPlayerName(ulong.Parse(target))}](<https://steamcommunity.com/profiles/{target}>)** player!");
                ...
                case "groupinfo.open":
                    ...
                    if (connectionData != null)
                    {
                        connectionData.userData["groupinfo.groupName"] = string.Join(" ", args);
                ...
                case "userinfo.action":
                    ...
                    if (connectionData != null)
                    {
                        ...
                        switch (action)
                        {
                            ...
                            case "teleportselfto":
                                admin.Teleport(user);
                                TryLogToDiscord(config.Logs.AdminTeleport, connection, "Teleport admin to player", $"The {admin.displayName} admin teleported to **[{user.displayName}](<https://steamcommunity.com/profiles/{user.UserIDString}>)** player!");
                                break;
                            case "teleporttoself":
                                user.EnsureDismounted();
                                user.Teleport(admin);
                                TryLogToDiscord(config.Logs.AdminTeleport, connection, "Teleport player to admin", $"The {admin.displayName} admin teleported **[{user.displayName}](<https://steamcommunity.com/profiles/{user.UserIDString}>)** player to himself!");
                                break;
                            case "teleporttoauth":
                                ...
                                if (entities.Length > 0)
                                {
                                    ...
                                    TryLogToDiscord(config.Logs.AdminTeleport, connection, "Teleport admin to auth", $"The {admin.displayName} admin teleported to **[{user.displayName}](<https://steamcommunity.com/profiles/{user.UserIDString}>)**'s auth entity!");
                                }
                            ...
                            case "teleporttodeathpoint":
                                ...
                                if (UserDeathNote != null)
                                {
                                    ...
                                    TryLogToDiscord(config.Logs.AdminTeleport, connection, "Teleport admin to death point", $"The {admin.displayName} admin teleported to **[{user.displayName}](<https://steamcommunity.com/profiles/{user.UserIDString}>)**'s death point!");
                                }
                            ...
                            case "heal":
                                ...
                                connectionData.currentContent.Render(connectionData);
                                TryLogToDiscord(config.Logs.Heal, connection, "Full healing of the player", $"The {admin.displayName} admin heal **[{user.displayName}](<https://steamcommunity.com/profiles/{user.UserIDString}>)** player!");
                                break;
                            case "heal50":
                                if (user.IsWounded())
                                    user.StopWounded();
                                ...
                                connectionData.currentContent.Render(connectionData);
                                TryLogToDiscord(config.Logs.Heal, connection, "Half healing of the player", $"The {admin.displayName} admin heal **[{user.displayName}](<https://steamcommunity.com/profiles/{user.UserIDString}>)** player!");
                                break;
                            case "kill":
                                user.DieInstantly();
                                connectionData.currentContent.Render(connectionData);
                                TryLogToDiscord(config.Logs.Kill, connection, "Kill the player", $"The {admin.displayName} admin kill **[{user.displayName}](<https://steamcommunity.com/profiles/{user.UserIDString}>)** player!");
                                break;
                            case "viewinv":
                                ...
                                HandleCommand(connection, "close");
                                TryLogToDiscord(config.Logs.LookInventory, connection, "Look the player's inventory", $"The {admin.displayName} admin look the **[{user.displayName}](<https://steamcommunity.com/profiles/{user.UserIDString}>)**'s inventory!");
                                break;
                            case "stripinventory":
                                user.inventory.Strip();
                                TryLogToDiscord(config.Logs.StripInventory, connection, "Strip the player's inventory", $"The {admin.displayName} admin strip the **[{user.displayName}](<https://steamcommunity.com/profiles/{user.UserIDString}>)**'s inventory!");
                                break;
                            case "unlockblueprints":
                                user.blueprints.UnlockAll();
                                TryLogToDiscord(config.Logs.Blueprints, connection, "Unlock the player's blueprints", $"The {admin.displayName} admin unlock the **[{user.displayName}](<https://steamcommunity.com/profiles/{user.UserIDString}>)**'s blueprints!");
                                break;
                            case "revokeblueprints":
                                user.blueprints.Reset();
                                TryLogToDiscord(config.Logs.Blueprints, connection, "Revoke the player's blueprints", $"The {admin.displayName} admin revoke the **[{user.displayName}](<https://steamcommunity.com/profiles/{user.UserIDString}>)**'s blueprints!");
                                break;
                            case "spectate":
                                if (!admin.IsDead())
                                    admin.DieInstantly();
                                ...
                                TryLogToDiscord(config.Logs.Spectate, connection, "Spectate to the player", $"The {admin.displayName} admin spectate to **[{user.displayName}](<https://steamcommunity.com/profiles/{user.UserIDString}>)** player!");
                                break;
                            case "mute":
                                user.SetPlayerFlag(BasePlayer.PlayerFlags.ChatMute, true);
                                connectionData.currentContent.Render(connectionData);
                                TryLogToDiscord(config.Logs.MuteUnmute, connection, "Mute the player", $"The {admin.displayName} admin mute **[{user.displayName}](<https://steamcommunity.com/profiles/{user.UserIDString}>)** player!");
                                break;
                            case "unmute":
                                user.SetPlayerFlag(BasePlayer.PlayerFlags.ChatMute, false);
                                connectionData.currentContent.Render(connectionData);
                                TryLogToDiscord(config.Logs.MuteUnmute, connection, "Unmute the player", $"The {admin.displayName} admin unmute **[{user.displayName}](<https://steamcommunity.com/profiles/{user.UserIDString}>)** player!");
                                break;
                            case "creative":
                                user.SetPlayerFlag(BasePlayer.PlayerFlags.CreativeMode, !user.HasPlayerFlag(BasePlayer.PlayerFlags.CreativeMode));
                                connectionData.currentContent.Render(connectionData);
                                TryLogToDiscord(config.Logs.ToggleCreative, connection, "Toggle creative mode for player", $"The {admin.displayName} admin toggle creative mode for **[{user.displayName}](<https://steamcommunity.com/profiles/{user.UserIDString}>)** player to **{user.IsInCreativeMode}**!");
                                break;
                            case "cuff":
                                ...
                                TryLogToDiscord(config.Logs.Cuff, connection, "Toggle cuff for player", $"The {admin.displayName} admin toggle cuff for **[{user.displayName}](<https://steamcommunity.com/profiles/{user.UserIDString}>)** player to **{user.IsRestrained}**!");
                                break;
                            case "kick":
                                ...
                                connectionData.currentContent.Render(connectionData);
                                TryLogToDiscord(config.Logs.Kick, connection, "Kick the player", $"The {admin.displayName} admin kick **[{user.displayName}](<https://steamcommunity.com/profiles/{user.UserIDString}>)** player!");
                                break;
                            case "ban":
                                if (args.Length == 2 && args[1] == "showpopup")
                                {
                                ...
                                connectionData.currentContent.Render(connectionData);
                                TryLogToDiscord(config.Logs.Ban, connection, "Ban the player", $"The {admin.displayName} admin ban **[{user.displayName}](<https://steamcommunity.com/profiles/{user.UserIDString}>)** player for {expiry} with the reason {banReason}!");
                                break;
                            case "cb.exec":
                                ...
                                TryLogToDiscord(config.Logs.CustomButtons, connection, "Using a custom button", $"The {admin.displayName} admin used a custom button with name **{uicb.Label}** at **[{user.displayName}](<https://steamcommunity.com/profiles/{user.UserIDString}>)** player!");
                                break;
                            default:
                                break;
                        }
                    ...
                    if (connectionData != null)
                    {
                        switch (args[0])
                        {
                            case "show":
                                ...
                                connectionData.userData["givemenu.popup.ownershipPhrase"] = null;
                                break;
                            case "show_custom":
                                connectionData.userData["givemenu.popup.shown"] = true;
                                connectionData.userData["givemenu.popup.itemid"] = int.Parse(args[1]);
                            ...
                            case "isblueprint_toggle":
                                ...
                                break;
                            case "show_ownership_phrases":
                                (connectionData.currentContent as GiveMenuContent).ShowOwnershipDropdown(connection);
                                return;
                            case "select_ownership":
                                connectionData.userData["givemenu.popup.ownershipPhrase"] = GiveMenuContent.OwnerShipPhrases.FirstOrDefault(x => x.token == args[1]);
                                break;
                            case "give":
                                connectionData = ConnectionData.Get(connection);
                                if (connectionData != null)
                                {
                                    ...
                                    Translate.Phrase ownershipPhrase = (Translate.Phrase)connectionData.userData["givemenu.popup.ownershipPhrase"];
                                    ...
                                    foreach (ulong targetUserId in targets)
                                    {
                                        ...
                                        if (newItem != null)
                                        {
                                            ...
                                            if (ownershipPhrase != null && ownershipPhrase.IsValid())
                                                newItem.SetItemOwnership(playerToGive, ownershipPhrase);
                    ...
                    if (connectionData != null)
                    {
                        ...
                        switch (action)
                        {
                            case "teleportto_000":
                                ...
                                TryLogToDiscord(config.Logs.AdminTeleport, connection, "Teleport admin to 0 0 0", $"The {admin.displayName} admin teleported to 0 0 0 coordinates!");
                                break;
                            case "teleportto_deathpoint":
                                ...
                                TryLogToDiscord(config.Logs.AdminTeleport, connection, "Teleported to death point", $"The {admin.displayName} admin teleported to his point of death.");
                                break;
                            ...
                            case "healself":
                                ...
                                TryLogToDiscord(config.Logs.Heal, connection, "Heal self", $"The {admin.displayName} admin healed himself.");
                                break;
                            case "killself":
                                admin.DieInstantly();
                                TryLogToDiscord(config.Logs.Kill, connection, "Kill self", $"The {admin.displayName} admin killed himself.");
                                break;
                            case "helicall":
                                ...
                                TryLogToDiscord(config.Logs.SpawnEntities, connection, "Heli call", $"The administrator called a **patrol helicopter**!");
                                break;
                            case "spawnbradley":
                                ...
                                TryLogToDiscord(config.Logs.SpawnEntities, connection, "Spawn Bradley", $"The {admin.displayName} admin spawned a **bradley**!");
                                break;
                            case "spawncargo":
                                ...
                                TryLogToDiscord(config.Logs.SpawnEntities, connection, "Spawn Cargo", $"The {admin.displayName} admin spawned a **cargo**!");
                                break;
                            ...
                            case "settime":
                                ...
                                TryLogToDiscord(config.Logs.SetTime, connection, "Set Time", $"The {admin.displayName} admin set the time at **{time}**!");
                                break;
                            case "cb.exec":
                                int hashCode = int.Parse(args[1]);
                                ...
                                TryLogToDiscord(config.Logs.CustomButtons, connection, "Using a custom button", $"The {admin.displayName} admin used a custom button with name **{qmcb.Label}** from quick menu!");
                                break;
                        }
                    }
                ...
                case "convars.save":
                    ...
                    {
                        string serverFolder = ConVar.Server.GetServerFolder("cfg");
                        File.WriteAllText(Path.Combine(serverFolder, "serverauto.cfg"), ConsoleSystem.SaveToConfigString(true));
                        Dictionary<string, string> convars = new Dictionary<string, string>();
                        string adminMenuConfigPath = Path.Combine(serverFolder, $"adminmenu.cfg");
                        if (File.Exists(adminMenuConfigPath))
                        {
                            string currectConfigText = File.ReadAllText(adminMenuConfigPath);
                            foreach (string str in currectConfigText.Split(new char[] { '\n' }, StringSplitOptions.RemoveEmptyEntries))
                            {
                                int spaceIndex = str.IndexOf(' ');
                                convars[str.Substring(0, spaceIndex)] = str.Substring(spaceIndex + 1);
                            }
                        }

                        foreach (ConsoleSystem.Command consoleCommand in ConVarsContent.modifiedConvars)
                            convars[consoleCommand.FullName] = consoleCommand.String.QuoteSafe();
                        string text = string.Empty;
                        foreach (var pair in convars)
                            text += pair.Key + " " + pair.Value + Environment.NewLine;
                        File.WriteAllText(adminMenuConfigPath, text);
                        TryLogToDiscord(config.Logs.ConVars, connection, "Config save", $"The {connection.username} admin saved convars!");
                    }
                ...
                case "convar.setvalue":
                    ...
                    if (convar != null)
                    {
                        ...
                        if (!ConVarsContent.modifiedConvars.Contains(convar))
                            ConVarsContent.modifiedConvars.Add(convar);
                        connectionData = ConnectionData.Get(connection);
                        if (connectionData != null)
                            connectionData.currentContent?.Render(connectionData);
                        TryLogToDiscord(config.Logs.ConVars, connection, "ConVar set value", $"The {connection.username} admin set convar **{convar.FullName}** to **{convar.String}**!");
                    }
                ...
                case "pluginmanager.favorite":
                    ...
                    if (connectionData != null)
                    {
                        ...
                        TryLogToDiscord(config.Logs.PluginManager, connection, "Favorite plugin", $"The administrator favorite **{args[0]}** plugin!");
                    }
                ...
                case "pluginmanager.unfavorite":
                    ...
                    if (connectionData != null)
                    {
                        ...
                        TryLogToDiscord(config.Logs.PluginManager, connection, "Unfavorite plugin", $"The administrator unfavorite **{args[0]}** plugin!");
                    }
                ...
                case "pluginmanager.load":
                    ...
                    TryLogToDiscord(config.Logs.PluginManager, connection, "Load plugin", $"The administrator load **{args[0]}** plugin!");
                    break;
                case "pluginmanager.unload":
                    ...
                    if (connectionData != null)
                        connectionData.userData["pluginmanager.lastusedplugin"] = args[0];
#if CARBON
                    RustPlugin plugin = Carbon.Core.ModLoader.Packages.FindPlugin(args[0]);
                    if (plugin != null)
                    {
                        Carbon.Core.ModLoader.UninitializePlugin(plugin, false, true);
                        plugin.Processor.Ignore(plugin.Name);
                    }
#else
                    Oxide.Core.Interface.Oxide.UnloadPlugin(args[0]);
#endif
                    TryLogToDiscord(config.Logs.PluginManager, connection, "Unload plugin", $"The administrator unload **{args[0]}** plugin!");
                    break;
                case "pluginmanager.reload":
                    ...
                    TryLogToDiscord(config.Logs.PluginManager, connection, "Reload plugin", $"The administrator reload **{args[0]}** plugin!");
                    break;
...
#endif
                    TryLogToDiscord(config.Logs.PluginManager, connection, "Reload all plugins", $"The administrator reload **all** plugins!");
                    break;
                default:
                    break;
            }
        ...
        private void TryLogToDiscord(object configVariable, Connection author, string title, string content, string thumbnail = null, string image = null)
        {
            if (configVariable is bool state)
            {
                if (state)
                    LogToDiscord(author, title, content, thumbnail, image);
            }
            else if (configVariable is string overrideWebhookUrl)
            {
                LogToDiscord(author, title, content, thumbnail, image, overrideWebhookUrl);
            }
        }

        private void LogToDiscord(Connection author, string title, string content, string thumbnail = null, string image = null, string overrideWebhookUrl = null)
        {
            string webhookUrl = overrideWebhookUrl ?? config.Logs.WebhookURL;
            ...
            webrequest.Enqueue(webhookUrl, jsonString, DiscordSendMessageCallback, this, RequestMethod.POST, HEADERS);
        }
