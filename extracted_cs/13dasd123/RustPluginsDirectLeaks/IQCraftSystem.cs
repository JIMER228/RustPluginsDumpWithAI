...
{
    [Info("IQCraftSystem", "Mercury", "1.2.12")]
    [Description("Convenient crafting system")]
    internal class IQCraftSystem : RustPlugin
    {

        private System.Collections.Generic.IEnumerable<Item> EnumerateAllItems(BasePlayer player)
        {
            if (player == null || player.inventory == null)
                yield break;

            var inv = player.inventory;
            var containers = new ItemContainer[] { inv.containerMain, inv.containerBelt, inv.containerWear };

            foreach (var c in containers)
            {
                var list = c == null ? null : c.itemList;
        ...
        #region Reference
        [PluginReference] private readonly Plugin ImageLibrary, IQEconomic, IQPlagueSkill, IQRankSystem, IQChat, Economics, IQTurret;
                ...
                return new Configuration
                {
                    ...
                    ItemSetting = new Dictionary<String, ItemSettings>
                    {
                        ...
                        ["tree"] = new ItemSettings
                        {
                            ...
                            PrefabEntity = "assets/bundled/prefabs/autospawn/resource/v3_temp_forest_pine/pine_a.prefab",
                            PrefabNameTransport = "",
                            Shortname = "",
                            SkinID = 1337,
                            WorkBenchLevel = 1,
        ...
        {
            Int32 ItemAmount = 0;
            foreach (Item ItemRequires in EnumerateAllItems(player))
            {
        ...
        private void CraftingItem(BasePlayer player, String ItemKey)
        {
            ...
            foreach (Configuration.ItemSettings.ItemForCraft ItemTake in Item.ItemListForCraft)
            {
                ...
                foreach (Item ItemPlayer in EnumerateAllItems(player).Where(x => x.skin == ItemTake.SkinID && x.info.shortname == ItemTake.Shortname))
                {
        ...
        private void OnHammerHit(BasePlayer player, HitInfo info)
        {
            ...
            if(IQTurret)
            {
                if(Entity is ElectricSwitch || Entity is AutoTurret || Entity is SamSite)
        ...
        private void OnPlayerAttack(BasePlayer player, HitInfo info)
        {
            ...
            if(IQTurret)
            {
                if(entity is ElectricSwitch || entity is AutoTurret)
                    if ((Boolean)IQTurret.CallHook("API_IS_TURRETLIST", entity))
                        return;
            }
        ...
        private void LoadedItems(BasePlayer player, CategoryItem SortCategory = CategoryItem.All, Int32 SlotActive = 0)
        {
            ...
            {
                {
                    ...
                    {
                        FadeOut = FadeOut,
                        RectTransform = { AnchorMin = "0.309375 0.1444", AnchorMax = "0.5677084 0.8296" }, //xxxxxxxxxx
                        Image = { FadeIn = FadeIn, Color = "0 0 0 0" }
                    },
                    CRAFTSYSTEM_HUD,
                    "ITEM_PANEL"
            ...
            foreach (KeyValuePair<String, Configuration.ItemSettings> Item in Items)
            {
                ...
                if (CategoryItem == CategoryItem.Transport || CategoryItem == CategoryItem.Custom ||
                    CategoryItem == CategoryItem.Entity)
                {
                    container.Add(new CuiElement
                    {
                        ...
                        Components =
                        {
                            new CuiRawImageComponent { FadeIn = FadeIn, Png = GetImage($"ITEM_{Item.Value.PNG}") },
                ...
                else
                {
                    container.Add(new CuiElement
                    {
                        ...
                        Name = $"ITEM_ICON_{ItemCount}",
                        Components = {
                            new CuiImageComponent() {FadeIn = FadeIn, Color = "1 1 1 1", ItemId = ItemManager.FindItemDefinition(Item.Value.Shortname).itemid, SkinId = Item.Value.SkinID},
                            new CuiRectTransformComponent { AnchorMin = AnchorMinResize, AnchorMax = AnchorMaxResize }
                        }
                    });
                }
        ...
        private void UI_Information_Craft(BasePlayer player, String ItemKey)
        {
                ...
                CategoryItem == CategoryItem.Entity)
            {
                container.Add(new CuiElement
                {
                    ...
                    Components =
                    {
                        new CuiRawImageComponent { FadeIn = FadeIn, Png = GetImage($"ITEM_{Item.PNG}") },
                        new CuiRectTransformComponent
                            { AnchorMin = "1 1", AnchorMax = "1 1", OffsetMin = "-140 -130", OffsetMax = "100 80" },
                    }
                });
            }
            else
            ...
            container.Add(new CuiLabel
                {
                    ...
                    Text =
                    {
                        FadeIn = FadeIn, Text = $"<b>{Item.DisplayName}</b>", FontSize = 30,
            ...
            container.Add(new CuiLabel
                {
                    ...
                    Text =
                    {
                        FadeIn = FadeIn, Text = $"{Item.Description}", FontSize = 15, Align = TextAnchor.UpperLeft
                    }
                },
                "CRAFT_INFORMATION_PANEL",
            ...
            if (CountRequires(ItemKey) != 0)
            {
                ...
                if (Item.WorkBenchLevel != 0)
                {
                    ...
                    {
                        FadeOut = FadeOut,
                        RectTransform = { AnchorMin = "0.1137938 0.72163232", AnchorMax = "1 0.7648644" },
            ...
            if (RequiresItemList != null)
            {
                if (Is_Full_Item_Pack(player, RequiresItemList) && RequiresYes >= Requires)
                {
                    container.Add(new CuiButton
                    {
                        RectTransform = { AnchorMin = "0.6379313 0.001351424", AnchorMax = "0.9965519 0.064863232" },
                ...
                foreach (Configuration.ItemSettings.ItemForCraft ItemCraft in RequiresItemList)
                {
                    ...
                    if (!String.IsNullOrWhiteSpace(ItemCraft.PNG))
                    {
                        container.Add(new CuiElement
                        {
                            FadeOut = FadeOut,
                            ...
                            Components =
                            {
                                new CuiRawImageComponent { FadeIn = FadeIn, Png = GetImage($"ITEM_CRAFT_{ItemCraft.PNG}") },
                                new CuiRectTransformComponent { AnchorMin = "0 0", AnchorMax = "1 1" },
                            }
                        });
                    }
                    else
                    {
                        container.Add(new CuiElement
                        {
                            ...
                            Components =
                            {
                                ...
                                {
                                    FadeIn = FadeIn, Color = "1 1 1 1",
                                    ItemId = ItemManager.FindItemDefinition(ItemCraft.Shortname).itemid,
                                    SkinId = ItemCraft.SkinID
                                },
                                new CuiRectTransformComponent { AnchorMin = "0 0", AnchorMax = "1 1" },
                            }
                        });
