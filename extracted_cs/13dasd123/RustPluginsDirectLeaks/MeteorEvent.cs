...
{
    [Info("MeteorEvent", "ThePitereq", "2.1.0")]
#if CARBON
...
#endif 
    {
        ...
        private static void ClearMeteors()
        {
            ...
            foreach (var meteor in fallingMeteors)
            {
                if (meteor == null || meteor.IsDestroyed) continue;
		...
		private object OnEntityTakeDamage(BaseEntity ent)
		{
            SphereEntity sphere = ent.GetParentEntity() as SphereEntity;
            if (!sphere) return null;
            MeteorController controller = sphere.GetComponent<MeteorController>();
            if (!controller) return null;
            if (ent is OreResourceEntity) return null;
			return false;
		}
        ...
        private void OnTimedExplosiveExplode(TimedExplosive explosive)
        {
            if (!checkExplosives || explosive.OwnerID == 0) return;
            BaseEntity node = explosive.GetParentEntity();
            if (node == null)
            {
                if (explosive.GetComponent<ServerProjectile>() == null) return;
                List<BaseEntity> nearbyOres = Pool.Get<List<BaseEntity>>(); 
                nearbyOres.Clear();
                ...
                Pool.FreeUnmanaged(ref nearbyOres);
				if (validEntities > 0)
					explosive.SetDamageScale(0);
                return;
            }
        ...
        private static void DamageNode(BaseEntity node, MeteorController controller, BasePlayer player, string prefabName)
        {
            ...
            float health = 0;
            if (node is BaseCombatEntity combatEnt)
            {
                combatEnt.health -= configValue.bigMeteors.explosiveConfig[prefabName].damageDealt;
                health = combatEnt.health;
            }
			else if (node is ResourceEntity resEnt)
            {
                resEnt.health -= configValue.bigMeteors.explosiveConfig[prefabName].damageDealt;
                health = resEnt.health;
            }
			if (node is StagedResourceEntity stagedEnt)
				stagedEnt.UpdateNetworkStage();
            foreach (var item in configValue.bigMeteors.output)
            {
                if (item.lastHitOnly && health > 0) continue;
                ...
                if (configValue.bigMeteors.callHook && node is ResourceEntity resEnt)
                    Interface.CallHook("OnDispenserGather", resEnt.resourceDispenser, player, createdItem);
        ...
        private void SpawnMeteor(string meteorKey, Vector3 position, Vector3 angle, bool allowLarge = true)
        {
            ...
            meteorEntity.Spawn();
			//meteorEntity.SetFlag(BaseEntity.Flags.Protected, true);
            ...
            {
                FireBall flame = GameManager.server.CreateEntity("assets/prefabs/npc/m2bradley/oilfireball2.prefab", sphere.transform.position) as FireBall;
                UnityEngine.Object.Destroy(flame.GetComponent<Rigidbody>());
                flame.Spawn();
                flame.SetParent(sphere, true);
                flame.syncPosition = false;
				flame.damagePerSecond = 0;
				flame.canMerge = false;
				flame.SetFlag(BaseEntity.Flags.Reserved1, true);
            }
            if (config.enableHooks)
        ...
        private class FallingMeteorController : FacepunchBehaviour
        {
            ...
            {
                Vector3 spherePos = sphere.transform.position + new Vector3(0, 0.5f, 0);
                if (config.allMeteors.destroyUnderwater && sphere.IsInWaterVolume(spherePos, out bool natural)) return;
            ...
            private void DestroyTrees()
            {
                ...
                List<TreeEntity> trees = Pool.Get<List<TreeEntity>>();
                Vis.Entities(spherePos, config.allMeteors.destroyTrees, trees);
                foreach (var tree in trees)
                    if (tree.OwnerID == 0)
                        tree.OnDied(new HitInfo() { PointStart = spherePos, PointEnd = tree.transform.position });
        ...
        private int HaveCurrency(BasePlayer player, int currencyPlugin, string currency)
        {
            ...
            {
                1 => (int)Math.Floor(Economics.Call<double>("Balance", userId)),
                2 => ServerRewards.Call<int>("CheckPoints", userId),
                3 => BankSystem.Call<int>("Balance", userId),
                4 => ShoppyStock.Call<int>("GetCurrencyAmount", currency, userId),
                _ => 0,
            };
        }
        ...
        private void TakeCurrency(BasePlayer player, int amount, int currencyPlugin, string currency)
        {
            ...
            {
                case 1: Economics.Call("Withdraw", userId, Convert.ToDouble(amount)); break;
                case 2: ServerRewards.Call("TakePoints", userId, amount); break;
                case 3: BankSystem.Call("Withdraw", userId, amount); break;
                case 4: ShoppyStock.Call("TakeCurrency", currency, userId, amount); break;
            };
        }

        private static void SendJson(BasePlayer player, string json) => CommunityEntity.ServerInstance.ClientRPC<string>(RpcTarget.Player("AddUI", player.Connection), json);
