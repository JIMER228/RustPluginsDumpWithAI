...
{
    /*ПЛАГИН БЫЛ ПОФИКШЕН С ПОМОЩЬЮ ПРОГРАММЫ СКАЧАНОЙ С https://discord.gg/dNGbxafuJn */
    [Info("MeteorEvent", "https://discord.gg/dNGbxafuJn", "2.0.2")]
#if CARBON
...
#endif
    {
        ...
        private void MeteorEventCommand(BasePlayer player, string command, string[] args)
        {
            ...
            else if (args.Length < 3)
            {
                ...
                SendReply(player, Lang("CommandHelp", player.UserIDString));
            }
        ...
        private void OnEntityKill(TimedExplosive explosive)
        {
            ...
            if (node == null)
            {
                ...
                List<OreResourceEntity> nearbyOres = Facepunch.Pool.GetList<OreResourceEntity>();
                ...
                Facepunch.Pool.FreeList(ref nearbyOres);
                return;
            }
        ...
        private void OnDispenserBonus(ResourceDispenser dispenser, BasePlayer player, Item item) => TryGiveBonusItems(dispenser, player, item, true);

        private static void TryGiveBonusItems(ResourceDispenser dispenser, BasePlayer player, Item item, bool finalHit = false)
        {
...
#endif
            if (tool == null) return;
            if (!configValue.customGather.customGatherTools.ContainsKey(tool.info.shortname))
            {
                SendReply(player, Lang($"WrongToolToGather_{controller.profile}", player.UserIDString));
                return;
            }
            CustomToolConfig customToolConfig = configValue.customGather.customGatherTools[tool.info.shortname];
            if (customToolConfig.requireSkin && tool.skin != customToolConfig.skin)
            {
                SendReply(player, Lang($"WrongToolToGather_{controller.profile}", player.UserIDString));
                return;
            }
            Puts($"{info.HitEntity.MaxHealth()} {info.HitEntity.MaxHealth() / 100f * customToolConfig.damageDealtPercentage}");
            entity.health -= info.HitEntity.MaxHealth() / 100f * customToolConfig.damageDealtPercentage;
        ...
        private class FallingMeteorController : FacepunchBehaviour
        {
            ...
            private void DestroyFallingMeteor(bool spawnLanded)
            {
            ...
            private void SpawnLandedMeteor(float radius)
            {
                ...
                if (config.allMeteors.destroyUnderwater && sphere.IsInWaterVolume(spherePos, out bool isNatural)) return;
            ...
            {
                Vector3 spherePos = sphere.transform.position;
                List<TreeEntity> trees = Facepunch.Pool.GetList<TreeEntity>();
        ...
        private void LoadDynamicBottomRight(CUI cui, CuiElementContainer corePanel, BasePlayer player)
        {
            ...
            if (permission.UserHasPermission(player.UserIDString, "meteorevent.admin"))
            {
                ...
                //Admin Panel Input
                cui.CreateProtectedInputField(corePanel, "MeteorEventUI_adminSetPanel", BetterColors.RedText, "0", 15, 3, false, 0f, 0f, 0f, 0f, 0, 77, 0, 20, $"UI_MeteorEvent forceStart {currEvent}", TextAnchor.MiddleCenter, CUI.Handler.FontTypes.RobotoCondensedBold, false, false, 0f, 0f, false, false, "MeteorEventUI_adminSetInput", null, false);
            }
        ...
        private void UpdateDisplay(BasePlayer player, string oldKey)
        {
            ...
            {
                //Admin Spawn Input
                elements.Add(cui.UpdateProtectedInputField(
    "MeteorEventUI_adminSetInput",
    BetterColors.RedText,
    "0",
    15,
    3,
    false,
    0f, 1f, 0f, 1f, 0f, 0f, 0f, 0f,
    $"UI_MeteorEvent forceStart {currEvent}",
    TextAnchor.MiddleCenter,
    CUI.Handler.FontTypes.RobotoCondensedBold,
    false,
    false,
    InputField.LineType.SingleLine, // LineType is now explicitly specified
    0f,
    0f,
    false,
    false,
    null));
        ...
        private void LoadMessages()
        {
            ...
            foreach (var prefab in config.meteorPrefabs)
                if (prefab.Value.bigMeteors.bigMeteorChance > 0)
