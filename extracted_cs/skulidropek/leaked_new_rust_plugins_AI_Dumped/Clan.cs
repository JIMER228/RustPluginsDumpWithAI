...
{
    [Info("Clan", "fixe by @puntofila", "1.1.2")]
    public class Clan : RustPlugin
    {
        ...
        void OnServerInitialized(bool initial)
        {
            try 
            {
                if (config == null)
                {
                    PrintError("Config is null - plugin initialization failed");
                    return;
                }
                ...
                {
                    PrintError("ImageLibrary plugin not found - please ensure it is installed and enabled");
                    return;
                }
        ...
        void LoadData()
        {
            try 
            {
                var mod = Interface.GetMod();
                if (mod?.DataFileSystem == null)
                {
                    PrintError("Failed to access data file system");
                    _clanList = new List<ClanData>();
                    return;
                }
                
                if (mod.DataFileSystem.ExistsDatafile(Name))
                {
                    _clanList = mod.DataFileSystem.ReadObject<List<ClanData>>(Name);
                    if (_clanList == null)
                    {
                        _clanList = new List<ClanData>();
                    }
            ...
            {
                PrintError($"Error loading clan data: {ex.Message}");
                _clanList = new List<ClanData>();
            }
        }
        ...
        public void MainUI(BasePlayer player)
        {
            ...
            foreach (var check in dictionary.Select((i, t) => new { A = i, B = t }))
            {
                container.Add(new CuiButton
                {
                    ...
                    Button = { Color = HexToCuiColor($"#000000FF", 50), Command = $"", },
                    Text = { Text = $"", Align = TextAnchor.LowerRight, Font = "robotocondensed-bold.ttf", FontSize = 15 }
        ...
        public void LoadSkin(BasePlayer player, ClanData clan, bool refresh = false)
        {
            ...
            foreach (var check in clan.ItemList.Select((i, t) => new { A = i, B = t }).Take(7))
            {
                ...
                container.Add(new CuiElement
                {
                    ...
                    Components = { new CuiImageComponent { ItemId = itemID, SkinId = check.A.Value }, new CuiRectTransformComponent { AnchorMin = "0 0", AnchorMax = "1 1", OffsetMin = "5 5", OffsetMax = "-5 -5"} } 
                });
        ...
        public void LoadGatherList(BasePlayer player, ClanData clan, bool refresh = false)
        {
            ...
            foreach (var check in clan.GatherList.Select((i, t) => new { A = i, B = t }).Take(9))
            {
                ...
                container.Add(new CuiElement()
                {
                    ...
                    Components =
                    {
                        new CuiTextComponent{Color = HexToCuiColor("#fcf7f6"),Text = $"x{check.A.Value.TotalFarm}", Align = TextAnchor.LowerRight, FontSize = 12, Font = "robotocondensed-regular.ttf"},
        ...
        public void LoadClanList(BasePlayer player, int page, bool refresh = false)
        {
            ...
            foreach (var check in _clanList.OrderByDescending(p => p.TotalScores).Select((i, t) => new { A = i, B = t - (page - 1) * 10 }).Skip((page - 1) * 10).Take(10))
            {
                ...
                container.Add(new CuiButton
                {
                    ...
                    Button = { Color = HexToCuiColor($"#000000FF", 50), Command = $"UI_ClanHandler loadInfoClan {check.A.ClanTag}", }, Text = { Text = $"", }
            ...
            LoadInfoClan(player, _clanList.OrderByDescending(p => p.TotalScores).Select((i, t) => new { A = i, B = t - (page - 1) * 10 }).Skip((page - 1) * 10).Take(10).First().A);
