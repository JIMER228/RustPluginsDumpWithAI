...
{
    /*ПЛАГИН БЫЛ ПОФИКШЕН С ПОМОЩЬЮ ПРОГРАММЫ СКАЧАНОЙ С https://discord.gg/dNGbxafuJn */ /*ПЛАГИН БЫЛ ПОФИКШЕН С ПОМОЩЬЮ ПРОГРАММЫ СКАЧАНОЙ С https://discord.gg/dNGbxafuJn */ [Info("AntiHack", "https://discord.gg/dNGbxafuJn", "1.0.0")]
    public class AntiHack : RustPlugin
    {
		private static Dictionary<ulong, HashSet<ulong>> playersHidenEntities = new Dictionary<ulong, HashSet<ulong>>();
        ...
        {
            AntiHack.Log log1 = Plugins.AntiHack.db.logs.Find((Predicate<Log>)(x => x.steamID == player.userID.Get()));
        ...
        {
            if (!Plugins.AntiHack.wallHackPlayersEnabled || !this.isLoaded || (!Plugins.AntiHack.playersHidenEntities.ContainsKey(target.userID) || !Plugins.AntiHack.playersHidenEntities[target.userID].Contains(entity.net.ID.Value)))
                return (object)null;
            return (object)false;
        }
        ...
        private void OnEntitySpawned(BaseNetworkable entity)
        {
            if (!Plugins.AntiHack.wallHackObjectsEnabled 
                ...
                || entity.GetComponent<LootContainer>() != null 
                || entity.GetComponent<StorageContainer>() == null 
                && !this.neededEntities.Contains(entity.ShortPrefabName))
            {
                return;
            }

            // Приведение entity к типу BaseEntity
            BaseEntity baseEntity = entity as BaseEntity;
            if (baseEntity == null)
            {
                return; // Если объект не может быть приведен, выход
            }

            AntiHack.Chunk chunkFromPoint = Plugins.AntiHack.GetChunkFromPoint(baseEntity.transform.position);
            if (chunkFromPoint == null)
            {
                return;
            }

            chunkFromPoint.entities.Add(baseEntity); // Используем baseEntity
        }

        [HookMethod("OnEntityKill")]
        private void OnEntityKill(BaseNetworkable entity)
        {
            if (!Plugins.AntiHack.wallHackObjectsEnabled || !this.isLoaded || (UnityEngine.Object)(entity as BasePlayer) != (UnityEngine.Object)null || (UnityEngine.Object)entity.GetComponent<LootContainer>() != (UnityEngine.Object)null || (UnityEngine.Object)entity.GetComponent<StorageContainer>() == (UnityEngine.Object)null && !this.neededEntities.Contains(entity.ShortPrefabName))
                return;
            AntiHack.Chunk chunkFromPoint = Plugins.AntiHack.GetChunkFromPoint(entity.transform.position);
            if (chunkFromPoint == null || !chunkFromPoint.entities.Contains(entity as BaseEntity))
                return;
            chunkFromPoint.entities.Remove(entity as BaseEntity);
        }
        ...
        private void OnServerInitialized(bool firstInitialization)
        {
            if (Plugins.AntiHack.wallHackObjectsEnabled)
            {
                int num = 0;
        ...
        private class HackHandler : MonoBehaviour
        {
            ...
            private void Awake()
            {
                ...
                Plugins.AntiHack.playersHidenEntities[this.player.userID] = new HashSet<ulong>();
            ...
            public void Hide(BaseEntity entity)
            {
                ...
                {
                    nw.PacketID(Network.Message.Type.EntityDestroy);
                    nw.EntityID(entity.net.ID);
                    nw.UInt8((byte)0);
                    nw.Send(new SendInfo(this.connection));
                }
            ...
            private bool IsVisible(BaseEntity target, bool isSleeper = false)
            {
                ...
                if ((double)num > 75.0)
                    return false;
                bool flag1 = !UnityEngine.Physics.Linecast(position1, this.player.transform.rotation * Vector3.left + position1, Plugins.AntiHack.cM);
                bool flag2 = !UnityEngine.Physics.Linecast(position1, this.player.transform.rotation * Vector3.right + position1, Plugins.AntiHack.cM);
                return flag1 && this.DoLine(this.player.transform.rotation * Vector3.left + position1, position2, 1.1f, 1.5f) || flag2 && this.DoLine(this.player.transform.rotation * Vector3.right + position1, position2, 1.1f, 1.5f) || (flag1 && this.DoLine(this.player.transform.rotation * Vector3.left + position1, position2, 1.1f, 1.1f) || flag2 && this.DoLine(this.player.transform.rotation * Vector3.right + position1, position2, 1.1f, 1.1f));
            }
            ...
            private void ShowPlayer(BasePlayer target, bool needRemove = true, bool isSleeper = false)
            {
                ...
                Plugins.AntiHack.playersHidenEntities[this.player.userID].Remove(target.net.ID.Value);
                foreach (BaseEntity baseEntity in this.hidedPlayersEntities[target.userID])
                {
                    if (!((UnityEngine.Object)baseEntity == (UnityEngine.Object)null) && !baseEntity.IsDestroyed)
                    {
                        ...
                        Plugins.AntiHack.playersHidenEntities[this.player.userID].Remove(baseEntity.net.ID.Value);
                    }
                }
                if (needRemove)
           ...
           private void HidePlayer(BasePlayer target, bool isSleeper = false)
            {
                ...
                // Добавляем обязательный аргумент peer
                NetWrite nw = Net.sv.StartWrite();

                nw.PacketID(Network.Message.Type.EntityDestroy);
                nw.EntityID(target.net.ID);
                nw.UInt8((byte)0);
                nw.Send(new SendInfo(this.connection));

                Item activeItem = target.GetActiveItem();
                nw = Net.sv.StartWrite();
                if ((activeItem != null ? (UnityEngine.Object)activeItem.GetHeldEntity() : (UnityEngine.Object)null) != (UnityEngine.Object)null)
                {
                    nw.PacketID(Network.Message.Type.EntityDestroy);
                    nw.EntityID(activeItem.GetHeldEntity().net.ID);
                    nw.UInt8((byte)0);
                    nw.Send(new SendInfo(this.connection));
                    this.hidedPlayersEntities[target.userID].Add(activeItem.GetHeldEntity());
                    Plugins.AntiHack.playersHidenEntities[this.player.userID].Add(activeItem.GetHeldEntity().net.ID.Value);
                }
                this.HidePlayersHostile(target);
                this.hidedPlayersEntities[target.userID].Add((BaseEntity)target);
                Plugins.AntiHack.playersHidenEntities[this.player.userID].Add(target.net.ID.Value);
            }
                ...
                foreach (Item obj in target.inventory.containerBelt.itemList)
                {
                    if (target.IsHostileItem(obj))
                    {
                        if (!((obj != null ? (UnityEngine.Object)obj.GetHeldEntity() : (UnityEngine.Object)null) == (UnityEngine.Object)null))
                        {
                            ...
                            {
                                nw.PacketID(Network.Message.Type.EntityDestroy);
                                nw.EntityID(obj.GetHeldEntity().net.ID);
                                nw.UInt8((byte)0);
                                nw.Send(new SendInfo(this.connection));
                                this.hidedPlayersEntities[target.userID].Add(obj.GetHeldEntity());
                                Plugins.AntiHack.playersHidenEntities[this.player.userID].Add(obj.GetHeldEntity().net.ID.Value);
                            }
                        }
                        else
                            break;
