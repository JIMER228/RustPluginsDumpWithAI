...
{
    /*ПЛАГИН БЫЛ ПОФИКШЕН С ПОМОЩЬЮ ПРОГРАММЫ СКАЧАНОЙ С https://discord.gg/dNGbxafuJn */ [Info("RaidProtection", "https://discord.gg/dNGbxafuJn", "3.4.5")]
    ...
    partial class RaidProtection : CovalencePlugin
    {
        ...
        private void SaveProtectedCupboards()
        {
            try
            {
                var allCupboards = ProtectedCupboardManager._protectedCupboards.Where(x => x.Value?.BaseEntity != null && !x.Value.IsNpcBase).ToDictionary(t => t.Key, t => t.Value);
...
namespace Oxide.Plugins
{
    partial class RaidProtection
    {
        ...
        private void CmdTcLedgerRollback(IPlayer player, string command, string[] args)
        {
            ...
            var successes = 0;
            foreach(var tc in ProtectedCupboardManager._protectedCupboards.Values)
            {
...
namespace Oxide.Plugins
{
    partial class RaidProtection
    {
		...
		private class Configuration
        {
			...
			[JsonProperty(PropertyName = "Protection Tabs Offset")]
			public int ProtectionTabsOffset = 0;
...
namespace Oxide.Plugins
{
    partial class RaidProtection
    {
        ...
        {
            public static Dictionary<ulong, ProtectedEntity> _protectedCupboards = new Dictionary<ulong, ProtectedEntity>();

            public static ProtectedEntity InitProtectedCupboard(BaseEntity entity)
            {
                if (entity == null || entity.net == null || entity.net.ID == null || _protectedCupboards == null || PLUGIN == null || PLUGIN.IsNpcBase(entity)) { return null; }
                if (_protectedCupboards.ContainsKey(entity.net.ID.Value))
                {
                    return _protectedCupboards[entity.net.ID.Value];
                }
                ...
                _protectedCupboards.Add(entity.net.ID.Value, tc);
                return tc;
            }
            ...
            public static bool ProtectedCupboardExists(ulong entityID)
            {
                ...
                return _protectedCupboards.Keys.Any(x => x == entityID);
            }
            public static ProtectedEntity Transfer(ProtectedEntity oldTc, BaseEntity newEntity)
            {
                oldTc.SetBaseEntity(newEntity);
                _protectedCupboards[newEntity.net.ID.Value] = oldTc;
                var newTc = _protectedCupboards[newEntity.net.ID.Value];
            ...
            {
                return _protectedCupboards.Values.Where(x => x.OwnerUserIds.Contains(userId)).ToList();
            }
            public static List<ProtectedEntity> GetCupboardsForFounder(ulong userId)
            {
                return _protectedCupboards.Values.Where(x => x.HasFounder && x.FounderUserId == userId).ToList();
            }
            public static int GetCupboardFounderCount(ulong userId)
            {
                return _protectedCupboards.Values.Where(x => x.HasFounder && x.FounderUserId == userId && !x.DisableProtectionFromExceedingFounderLimit).Count();
            }
        }
    }
}
        ...
        public class ProtectedEntity
        {
            ...
            public bool CanClickStatusUI
            {
                get
                {
                    ...
                    return DateTime.Now >= LastStatusUIClickTime.Value.AddSeconds(0.1);
                }
            }
            ...
            private StatusReason? _protectionTimerReason = null;
...
namespace Oxide.Plugins
{
    partial class RaidProtection
    {
        ...
        private object OnEntityTakeDamage(BaseTrap entity, HitInfo info)
        {
            return config.Protection.ProtectedEntities.Traps ? OnEntityTakeDamageHelper(entity, info) : null;
        }

        private object OnEntityTakeDamage(Barricade entity, HitInfo info)
        {
            return config.Protection.ProtectedEntities.Traps ? OnEntityTakeDamageHelper(entity, info) : null;
        }

        private object OnEntityTakeDamage(GunTrap entity, HitInfo info)
        {
            return config.Protection.ProtectedEntities.Traps ? OnEntityTakeDamageHelper(entity, info) : null;
        }

        private object OnEntityTakeDamage(FlameTurret entity, HitInfo info)
        {
            return config.Protection.ProtectedEntities.Traps ? OnEntityTakeDamageHelper(entity, info) : null;
        }

        private object OnEntityTakeDamage(TeslaCoil entity, HitInfo info)
        {
            return config.Protection.ProtectedEntities.Traps ? OnEntityTakeDamageHelper(entity, info) : null;
        }

        private object OnEntityTakeDamage(SamSite entity, HitInfo info)
        {
            return config.Protection.ProtectedEntities.Traps ? OnEntityTakeDamageHelper(entity, info) : null;
        }

        private object OnEntityTakeDamage(AutoTurret entity, HitInfo info)
        {
            return config.Protection.ProtectedEntities.Traps ? OnEntityTakeDamageHelper(entity, info) : null;
        }
...
namespace Oxide.Plugins
{
    partial class RaidProtection
    {
        ...
        public static class BalanceLedger
        {
            ...
            {
                foreach (var kvp in ProtectedCupboardManager._protectedCupboards)
                {
                    try
                    {
...
namespace Oxide.Plugins
{
    partial class RaidProtection
    {
        ...
        private void ShowProtectionStatusOverlayTabs(BasePlayer basePlayer, ProtectedEntity tc, int page = 0)
        {
            ...
            var x = 285;
            var w = 200;
            var y = 620 + config.ProtectionTabsOffset;
            container.Add(new CuiElement
            {
                Parent = "Overlay",
        ...
        private void ShowProtectionStatusOverlay(BasePlayer basePlayer, ProtectedEntity tc)
        {
            ...
            var rightButtons = new[]
            {
                ...
                new
                {
                    ...
                    Command = $"tc.ui.balance {Security.Token} {tc.TotalProtectionCostPerHour} {tc.ID}",
                ...
                new
                {
                    ...
                    Command = $"tc.ui.balance {Security.Token} {tc.TotalProtectionCostPerHour*24} {tc.ID}",
                    Visible = !tc.HasFreeProtection && !tc.DisableProtectionFromExceedingFounderLimit,
                ...
                new
                {
                    ...
                    Command = $"tc.ui.balance {Security.Token} {tc.TotalProtectionCostPerHour*maxHours} {tc.ID}",
