...
{
    [Info("UIKits", "XAVIER", "1.0.0")]
    public class UIKits : RustPlugin
    {
        ...
        void OnServerInitialized()
        {
            ...
            else
            {
                ...
                {
                    var list = listCategory.KitList.Where(p => !string.IsNullOrEmpty(p.Permission)).ToList();
                ...
                {       
                    imagesList.TryAdd(item.shortname + 512, $"http://api.skyplugins.ru/api/getimage/{item.shortname}/512");
                }
        ...
        public List<CategoryList> GetKitsForPlayer(BasePlayer player)
        {
            ...
            {
                var find = key.KitList.FirstOrDefault(p => permission.UserHasPermission(player.UserIDString, p.Permission));
        ...
        {
            List<ItemList> kititems = new List<ItemList>();
                ...
                switch (args.Args[0])
                {
                    ...
                    case "giveKit":
                    {
                        ...
                        if (findCooldown != null)
                        {
                            ...
                            {
                                player.ChatMessage(GetLang("Cooldown", player.UserIDString));
                                return;
                            }
                        ...
                        {
                            player.ChatMessage(GetLang("NoPlace", player.UserIDString));
                            return;
                        }

                        var itemList = findKit.GiveToItem();
                        ...
                        player.ChatMessage(GetLang("Accept", player.UserIDString));
        ...
        public void ViewKitList(BasePlayer player)
        {
            ...
            int stringAmount = Mathf.CeilToInt(list.Count / 3f); 
            
            float position   = 0.506f - list.Skip((String - 1) * 3).Take(3).Count() / 2f * width - (list.Skip((String - 1) * 3).Take(3).Count()) / 2f * margin;
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                for (int j = 0; j < 3; j++)
                {
                    ...
                    if (findCooldown == null)
                    {
                        ...
                        firstContainer.Add(new CuiElement
                        {
                            ...
                            Components =
                            {
                                new CuiImageComponent         {Sprite    = "assets/icons/picked up.png", Color = HexToCuiColor("#ccd68a", 100)},
                                new CuiRectTransformComponent {AnchorMin = "0 0", AnchorMax = "1 1", OffsetMax = "-6 -6", OffsetMin = "6 6" },
                            }
                        });
                    }
                ...
                if (index % 3 == 0)
                {
                    ...
                    index      = 0;
                    minHeight -= 0.2225f;
                    position   = 0.506f - list.Skip((String - 1) * 3).Take(3).Count() / 2f * width - (list.Skip((String - 1) * 3).Take(3).Count()) / 2f * margin;
                }
