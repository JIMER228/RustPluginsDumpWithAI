...
{
    [Info("Punch Patch", "Shady", "0.0.1")]
    class PunchPatch : RustPlugin
    {
        /*
        ...
        #endregion
        #region Patch
        [HarmonyPatch(typeof(HeldEntity), "SendPunch")]
        private class SendPunchPatch
        {
            private static bool Prefix(HeldEntity __instance, Vector3 amount, float duration)
            {
                try
                {
                    if (__instance == null)
                    ...
                    var hookVal = Interface.Oxide.CallHook("CanSendPunch", __instance, amount, duration);

                    if (hookVal != null && !(bool)hookVal)
                        return false;

                    object[] arguments = { __instance, amount, duration };

                    Interface.Oxide.CallHook("OnSendPunch", arguments);

                    amount = (Vector3)arguments[1];
                    duration = (float)arguments[2];

                    __instance.ClientRPCPlayer(null, __instance.GetOwnerPlayer(), "CL_Punch", amount, duration);
                    return false;
                }
