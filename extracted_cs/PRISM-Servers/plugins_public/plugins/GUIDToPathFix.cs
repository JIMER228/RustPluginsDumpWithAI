...
{
    [Info("GUIDToPathFix", "Shady", "0.0.2", ResourceId = 0)]
    internal class GUIDToPathFix : RustPlugin
    {
        /*
        ...
        #endregion
        #region Patch
        [HarmonyPatch(typeof(GameManifest), "GUIDToPath")]
        private class GUIDToPathPatch
        {
            private static bool Prefix(string guid, ref string __result)
            {
                if (string.IsNullOrEmpty(guid))
                {
                    UnityEngine.Debug.LogWarning("GUIDToPath: guid is empty");
                    return false;
                }
                ...
                UnityEngine.Debug.LogWarning("GUIDToPath: no path found for guid " + guid);
