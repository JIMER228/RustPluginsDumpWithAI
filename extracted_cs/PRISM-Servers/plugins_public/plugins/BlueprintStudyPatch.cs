...
{
    [Info("Blueprint Study Patch", "Shady", "0.0.1")]
    internal class BlueprintStudyPatch : RustPlugin
    {
        /*
         ...
         * 
         */
        public static BlueprintStudyPatch Instance = null;
        ...
        #region Blueprint Study Patch
        [HarmonyPatch(typeof(ItemModStudyBlueprint), "ServerCommand")]
        private class ServerCommandPatch
        {
            private static bool Prefix(ItemModStudyBlueprint __instance, Item item, string command, BasePlayer player)
            {
                if (item.GetOwnerPlayer() != player)
                {

                    bool flag = false;
                    for (int i = 0; i < player.inventory.loot.containers.Count; i++)
                    {
                        var container = player.inventory.loot.containers[i];
                ...
                bool flag1 = player.blueprints.IsUnlocked(blueprintTargetDef);

                if (flag1 && blueprint != null && blueprint.additionalUnlocks != null && blueprint.additionalUnlocks.Count > 0)
                {
                    for (int i = 0; i < blueprint.additionalUnlocks.Count; i++)
                    {
                        var additionalUnlock = blueprint.additionalUnlocks[i];
                ...
                if (Interface.CallHook("OnPlayerTryStudyBlueprint", player, item) != null || flag1 || Interface.CallHook("OnPlayerStudyBlueprint", player, item) != null)
                    return false;
                ...
                if (!__instance.studyEffect.isValid)
                    return false;

                Effect.server.Run(__instance.studyEffect.resourcePath, player, StringPool.Get("head"), Vector3.zero, Vector3.zero, null, false);
