...
namespace Oxide.Plugins
{
    ...
    internal class WeaponDamageScaler : RustPlugin
    {
            ...
            switch (args[0].ToLower())
            {
                ...
                case "list":
                    ...
                    for (int i = 0; i < Bodyparts.Length; i++)
                    {
                        ...
                        BPsb.Append(bp).Append(" (").Append(bpName).Append("), ").Append(Environment.NewLine);
                    }

                    if (BPsb.Length > 1) BPsb.Length -= 1; //trim newline
        ...
        private bool PrefabExists(string prefabName) //yes, this uses linq. yes, linq is bad. however, this is called only once on a user command requiring input.
        {
