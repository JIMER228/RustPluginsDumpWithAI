...
{
    [Info("Throw Water Vessel Patch", "Shady", "1.0.0")]
    internal class ThrowWaterVesselPatch : RustPlugin
    {
        /*
         ...
         * 
         */
        public static ThrowWaterVesselPatch Instance = null;
        ...
        #region Patch
        [HarmonyPatch(typeof(BaseLiquidVessel), "DoThrow")]
        private class ThrowWaterVesselPatchClass
        {
            private static bool Prefix(BaseLiquidVessel __instance, Vector3 pos, Vector3 velocity)
            {
                ...
                if (Interface.CallHook("CanThrowWater", ownerPlayer, __instance, pos, velocity) != null) 
                    return false;

                var waterThrownAmount = slot.amount;

                Vector3 pos1 = ownerPlayer.eyes.position + ownerPlayer.eyes.BodyForward() * 1f;
                WaterBall entity = GameManager.server.CreateEntity(__instance.thrownWaterObject.resourcePath, pos1, Quaternion.identity, true) as WaterBall;
                ...
                Interface.CallHook("OnWaterThrown", ownerPlayer, __instance, pos, velocity, waterThrownAmount);
