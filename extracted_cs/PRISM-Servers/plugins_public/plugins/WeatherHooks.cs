...
{
    [Info("Weather Hooks", "Shady", "0.0.2")]
    class WeatherHooks : RustPlugin
    {
        /*
        ...
        private string _thunderStormCalledDay = string.Empty;
        ...
        {
            StartCoroutine(CheckIfThunderstormStarted());
        }
        ...
        private readonly TimeCachedValue<string> _inGameDay = new()
        {
            refreshCooldown = 10,
            refreshRandomRange = 5,
            updateValue = new Func<string>(() =>
            {

                return TOD_Sky.Instance?.Cycle?.DateTime.ToString("d") ?? string.Empty;
            })
        };


        private IEnumerator CheckIfThunderstormStarted()
        {
            while (true)
            {
                yield return CoroutineEx.waitForSecondsRealtime(8f);

                var ingameDay = _inGameDay.Get(false);

                if (Climate.Instance.WeatherState.Thunder < 0.1f)
                {
                    if (_thunderStormCalledDay.Equals(ingameDay, StringComparison.OrdinalIgnoreCase))
                    {
                        Interface.Oxide.CallHook("OnEndThunderstorm");
                        _thunderStormCalledDay = string.Empty;
                    }

                    continue;
                }

                if (_thunderStormCalledDay.Equals(ingameDay))
                    continue;

                _thunderStormCalledDay = ingameDay;

                Interface.Oxide.CallHook("OnStartThunderstorm");
            }
        }
