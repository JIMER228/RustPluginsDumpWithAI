...
{
    [Info("Radiation Protection Patch", "Shady", "0.0.1")]
    class RadiationProtectionPatch : RustPlugin
    {
        /*
            ...
            for (int i = 0; i < types.Length; i++)
            {
                ...
                if (t != type && t.IsClass && t.FullName.Contains(type.FullName))
                {
                    ...
                    for (int j = 0; j < attributes.Length; j++)
                    {
                        try
                        {
                            if (attributes[j] is not HarmonyPatch patch) continue;
        ...
        private class RadiationPatch
        {
            private static bool Prefix(BasePlayer __instance, ref float __result)
            {
                var hookVal = Interface.Oxide.CallHook("OnGetRadiationProtection", __instance, __result);
                if (hookVal == null) return true;

                __result = (float)hookVal;
