...
{
    [Info("BattlefieldCompilation", "Shady", "0.0.2")]
    class BattlefieldCompilation : RustPlugin
    {
        /*
         ...
         * 
         */
        private List<int> allBlueprints = new List<int>();
        void OnServerInitialized()
        {
            ...
            for (int i = 0; i < BasePlayer.activePlayerList.Count; i++)
            {
                ...
                if (p != null) GiveAllBlueprints(p.userID);
            }
        ...
        void GiveAllBlueprints(ulong userId)
        {
            if (userId == 0) return;
            var persist = ServerMgr.Instance?.persistance ?? null;
            if (persist == null)
            {
                PrintWarning("persist is null!!");
                return;
            }
            var sourceInfo = persist?.GetPlayerInfo(userId) ?? null;
            if (sourceInfo == null)
            {
                PrintWarning("sourceInfo is null!!");
                return;
            }
            
            sourceInfo.unlockedItems = allBlueprints.ToList();
            persist.SetPlayerInfo(userId, sourceInfo);

            var pObj = BasePlayer.FindByID(userId);
        ...
        void OnPlayerConnected(BasePlayer player)
        {
            ...
            GiveAllBlueprints(player.userID);
        }
    }
}