...
{
    [Info("AntiHeavyJacketAim", "Shady", "0.0.4")]

    public class AntiHeavyJacketAim : RustPlugin
    {
        /*
        ...
        #endregion
        private const int HEAVY_JACKET_ID = -1102429027;
        #endregion
        #region Hooks
        private void OnPlayerInput(BasePlayer player, InputState input)
        {
            ...
            if (input.WasJustPressed(BUTTON.FIRE_SECONDARY) || input.IsDown(BUTTON.FIRE_SECONDARY))
            {
                ...
                if (isAiming)
                {
                    var baseProj = player?.GetActiveItem()?.GetHeldEntity() as BaseProjectile;
                    if (baseProj == null || baseProj is BowWeapon) return;
                    ...
                    if (wearItems?.Count > 0)
                    {
                        for (int i = 0; i < wearItems.Count; i++)
                        {
                            ...
                            if (item?.info != null && !item.isBroken && item.info.itemid == HEAVY_JACKET_ID)
                            {
                                hasHeavy = true;
                                break;
                            }
                    ...
                    if (hasHeavy)
                    {
                        ...
                        try
                        {
                            var warnStr = sb.Clear().Append(player?.displayName).Append(" (").Append(player?.UserIDString).Append(") is aiming with a Heavy Plate Jacket equipped (").Append(baseProj?.ShortPrefabName).Append("!").ToString();
