...
{
    [Info("Dropped Item Container Patch", "Shady", "0.0.1")]
    class DroppedItemContainerPatch : RustPlugin
    {
        /*
        ...
        #endregion
        #region PlayerStoppedLooting Patch
        [HarmonyPatch(typeof(DroppedItemContainer), "PlayerStoppedLooting")]
        private class PlayerStoppedLootingPatch
        {
            private static bool Prefix(DroppedItemContainer __instance, BasePlayer player)
            {
                Interface.Oxide.CallHook("OnLootEntityEnd", player, __instance);

                var didKill = false;

                if (__instance?.inventory?.itemList == null || __instance.inventory.itemList.Count == 0)
                {
                    var hookVal = Interface.Oxide.CallHook("CanKillDroppedItemContainer", __instance, player);
                ...
                if (!didKill)
                {
                    __instance.ResetRemovalTime();
                    __instance.SetFlag(BaseEntity.Flags.Open, false, false, true);
        ...
        #region RemoveMe Patch
        [HarmonyPatch(typeof(DroppedItemContainer), "RemoveMe")]
        private class RemoveMePatch
        {
            private static bool Prefix(DroppedItemContainer __instance)
            {
                var hookVal = Interface.Oxide.CallHook("CanRemoveDroppedItemContainer", __instance);

                return !(hookVal != null && !(bool)hookVal);
            }
        }
        #endregion
    }
