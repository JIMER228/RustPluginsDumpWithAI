...
{
    [Info("Fish Patch", "Shady", "0.0.1")]
    class FishPatch : RustPlugin
    {
        /*
                ...
                if (t != type && t.IsClass && t.FullName.Contains(type.FullName))
                {
                    ...
                    for (int j = 0; j < attributes.Length; j++)
                    {
                        try
                        {
                            ...
                            var transpilerMethod = t.GetMethod("Transpiler", BindingFlags.Static | BindingFlags.NonPublic | BindingFlags.Public);

                            if (prefixMethod != null) prefix = new HarmonyMethod(prefixMethod);
                            if (postFixMethod != null) postfix = new HarmonyMethod(postFixMethod);
                            if (transpilerMethod != null) transpiler = new HarmonyMethod(transpilerMethod);
                            
                            harmony.Patch(originalMethod, prefix, postfix, transpiler);
        ...
        private static class TheFishPatch
        {

            private static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
            {

                Interface.Oxide.LogWarning("Transpiler");
                ...
                for (int i = 0; i < instructions.Count(); i++)
                {
                    ...
                    try
                    {
                        ...
                        if (c != null && c.opcode != null && c.operand != null)
                        {
                            sb.Append("opcode: ").Append(c.opcode).Append(", operand: ").Append(c.operand).Append(Environment.NewLine);


                            if (c.opcode == OpCodes.Call && c.operand.ToString().Equals("Single MoveTowards(Single, Single, Single)") && instructions.ElementAt(i + 1).operand.ToString().Equals("System.Single strainTimer"))
                            {
                                Interface.Oxide.LogWarning("found desired opcode? lol");
                    ...
                    /*/
                    if (c.opcode == OpCodes.Ldstr && c.operand == "IOnUserApprove")
                    {
                        c.operand = "BeginCustomAuth";
                        yield return c;
                        yield return new CodeInstruction(OpCodes.Ldarg_0);
                        CodeInstruction call = instructions.ElementAt(i + 2);
                        call.operand = typeof(Interface).GetMethods().First(m => m.Name == "CallHook" && m.GetParameters().Length == 3);
                        continue;
                    }
                ...
                Interface.Oxide.LogWarning(sb.ToString());
