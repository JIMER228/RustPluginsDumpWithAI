...
{
    [Info("Copy Paste", "Reneb & MiRror & Misstake & misticos & Shady & MBR", "4.1.22")]
    ...
    public class CopyPaste : RustPlugin
    {
        ...
        private Dictionary<string, HashSet<BaseEntity>> undoEntities = new Dictionary<string, HashSet<BaseEntity>>();
        ...
        private void Init()
        {
            ...
            AddCovalenceCommand("paste", "cmdChatPaste");
            AddCovalenceCommand("paste2", "cmdChatPasteManual");
            AddCovalenceCommand("pasteall", "cmdChatPasteAll");
            AddCovalenceCommand("undoall", "cmdChatUndoAll");
            //PrintWarning("Added covalence commands");
        }
        ...
        private object TryCopyFromSteamID(ulong userID, string filename, string[] args, Action callback = null)
        {
            ...
            if (!Physics.Raycast(player.eyes.HeadRay(), out var hit, 1000f, rayCopy))
        ...
        private object TryPasteFromSteamID(ulong userID, string filename, string[] args, Action callback = null)
        {
            ...
            if (!Physics.Raycast(player.eyes.HeadRay(), out var hit, 1000f, rayPaste))
                return Lang("NO_ENTITY_RAY", player.UserIDString);

            return TryPaste(hit.point, filename, player.UserIDString, DegreeToRadian(player.GetNetworkRotation().eulerAngles.y), args, callback: callback);
        }
        ...
        private object cmdPasteBack(BasePlayer player, string[] args)
        {
            ...
            var success = TryPasteBack(args[0], userIDString, args.Skip(1).ToArray());
        ...
        private void UndoLoop(HashSet<BaseEntity> entities, BasePlayer player, int count = 0)
        {
            ...
            {
                if (!storageContainer.IsDestroyed) storageContainer.Kill();
            }
        ...
        private Dictionary<string, object> EntityData(BaseEntity entity, Vector3 entPos, Vector3 entRot, CopyData copyData)
        {
            ...
            if (sign != null)
            {
                var imageByte = FileStorage.server.Get(sign.textureIDs[0], FileStorage.Type.png, sign.net.ID);
                ...
                if (sign.textureIDs[0] > 0 && imageByte != null)
        ...
        private bool FindRayEntity(Vector3 sourcePos, Vector3 sourceDir, out Vector3 point, out BaseEntity entity, int rayLayer)
        {
            ...
            if (!Physics.Raycast(sourcePos, sourceDir, out var hitinfo, 1000f, rayLayer))
                return false;
        ...
        private void FixSignage(Signage sign, byte[] imageBytes)
        {
            ...
            sign.textureIDs[0] = FileStorage.server.Store(resizedImage, FileStorage.Type.png, sign.net.ID);
        }
        ...
        private object GetGround(Vector3 pos)
        {
            ...
            if (Physics.Raycast(pos, Vector3.down, out var hitInfo, 200, groundLayer))
        ...
        {
            return player.IsServer || player.IsAdmin || permission.UserHasPermission(player.Id, permName);
        }
        ...
        private Vector3 GetVector3FromString(string vectorStr)
        {
        ...
        private void Paste(ICollection<Dictionary<string, object>> entities, Dictionary<string, object> protocol, Vector3 startPos, string userId, bool stability, float RotationCorrection, float heightAdj, bool auth, Action callback)
        {
        ...
        private void PasteLoop(PasteData pasteData)
        {
            ...
            var userId = pasteData.UserID;

            if (!string.IsNullOrWhiteSpace(pasteData?.UserID) || !ulong.TryParse(userId, out var userIdUL))
                userIdUL = 0;


            //has cache (relationshipmanager)
            var player = RelationshipManager.FindByID(userIdUL); //BasePlayer.FindByID(userIdUL) ?? BasePlayer.FindSleeping(userIdUL);
            ...
            foreach (var data in todo)
            {
                ...
                ulong ownerId = userIdUL;
                if (data.TryGetValue("ownerid", out var dataOwnerId))
                    ownerId = Convert.ToUInt64(dataOwnerId);
                ...
                if (decayEntity != null)
                {
                    ...
                    {
                        var nearBy = decayEntity.GetNearbyBuildingBlock();
                        if (nearBy != null && nearBy.buildingID != 0)
                            decayEntity.AttachToBuilding(nearBy.buildingID);
                        else decayEntity.AttachToBuilding(BuildingManager.server.NewBuildingID());

                    }
                    else decayEntity.AttachToBuilding(pasteData.BuildingID);
                }
                ...
                if (baseCombat != null)
                    baseCombat.health = baseCombat.MaxHealth();
                ...
                if (box != null)
                {
                    ...
                    if (data.TryGetValue("items", out var dataItems))
                        items = dataItems as List<object>;
                    ...
                    foreach (var itemDef in items)
                    {
                        ...
                        if (i != null)
                        {
                            ...
                            if (item.TryGetValue("text", out var dataText))
                                i.text = dataText.ToString();
                ...
                if (autoturret != null)
                {
                    ...
                    if (userIdUL > 0 && !authorizedPlayers.Contains(userIdUL) && pasteData.Auth)
                        authorizedPlayers.Add(userIdUL);
                    ...
                    foreach (var userID in authorizedPlayers)
                    {
                        ...
                        {
                            userid = userID,
                            username = (BasePlayer.FindByID(userID) ?? BasePlayer.FindSleeping(userID))?.displayName ?? "Unknown Name"
                        });
                    }
                ...
                if (ioEntity != null)
                {
                    ...
                    if (ioData.TryGetValue("oldID", out var oldIdObject))
                    {
                        if (!uint.TryParse(oldIdObject.ToString(), out var oldId))
                        {
                            PrintWarning("no oldID!!!");
                        }
                        else pasteData.IoEntities.Add(oldId, ioData);
                    }
                }
                ...
                {
                    if (Enum.TryParse(flagData.Key, out BaseEntity.Flags baseFlag))
            ...
            else
            {
                foreach (var ioData in pasteData.IoEntities.Values.ToArray())
                {
                    ...
                    if (inputs != null && inputs.Count > 0)
                    {
                        for (int index = 0; index < inputs.Count; index++)
                        {
                            ...
                            if (!input.TryGetValue("connectedID", out var oldIdObject))
                                continue;

                            if (uint.TryParse(oldIdObject?.ToString() ?? string.Empty, out var oldId) && oldId != 0 && pasteData.IoEntities.ContainsKey(oldId))
                            {
                               ...
                               // object temp;
                                ...
                                if (ioConnection.TryGetValue("newId", out var newIdObj) && uint.TryParse(newIdObj.ToString(), out var newId)) //.ContainsKey("newId"))
                                {
                                    ioEntity.inputs[index].connectedTo.entityRef.uid =
                                        new NetworkableId(newId);
                                }
                            }
                        }
                    }
                    ...
                    if (outputs != null && outputs.Count > 0)
                    {
                        for (int index = 0; index < outputs.Count; index++)
                        {
                            ...
                            if (uint.TryParse(output["connectedID"].ToString(), out var oldId) && oldId != 0 && pasteData.IoEntities.ContainsKey(oldId))
                            {
                ...
                if (!lastPastes.TryGetValue(userId ?? serverID, out var stk)) lastPastes[userId ?? serverID] = new Stack<List<BaseEntity>>();

                lastPastes[userId ?? serverID].Push(pasteData.PastedEntities);
                if (!string.IsNullOrEmpty(userId))
                {
                    var ents = new HashSet<BaseEntity>();
                    for (int i = 0; i < pasteData.PastedEntities.Count; i++) ents.Add(pasteData.PastedEntities[i]);
                    undoEntities[userId] = ents;
                }

                pasteData.callback?.Invoke();

                Interface.CallHook("OnPasteFinished", pasteData.PastedEntities);
                //Interface.CallHook("OnPasteFinishedVerbose", pasteData.)
            }
        }
        ...
        private HashSet<Dictionary<string, object>> PreLoadData(List<object> entities, Vector3 startPos, float RotationCorrection, bool deployables, bool inventories, bool auth, bool vending)
        {
            ...
            foreach (var entity in entities)
            {
                ...
                data["position"] = quaternionRotation * (new Vector3(Convert.ToSingle(pos["x"]), Convert.ToSingle(pos["y"]), Convert.ToSingle(pos["z"]))) + startPos;
                data["rotation"] = Quaternion.EulerRotation(eulerRotation + new Vector3(Convert.ToSingle(rot["x"]), Convert.ToSingle(rot["y"]), Convert.ToSingle(rot["z"])));
        ...
        private object TryPasteFromData(Vector3 startPos, string jsonData, string userID, float RotationCorrection, string[] args, bool autoHeight = true, Action callback = null)
        {
            ...
            var data = JsonConvert.DeserializeObject<Dictionary<string, object>>(jsonData, jsonSettings);
            if (data == null)
            {
                PrintWarning("data is null!");
                return Lang("FILE_BROKEN", userID);
            }
            if (data == null || !data.TryGetValue("default", out var nullOut) || !data.TryGetValue("entities", out nullOut))
            {
                PrintWarning("fucke ddata");
                return Lang("FILE_BROKEN", userID);
            }
            ...
            for (int i = 0; ; i += 2)
            {
                ...
                switch (param)
                {
                    ...
                    case "oid":
                    case "ownerid":
                        if (!ulong.TryParse(args[valueIndex], out ownerId))
                            return Lang("SYNTAX_OWNERID", userID);
            ...
            if (ownerId > 0) foreach (var entity in preloadData) entity["ownerid"] = ownerId;
            ...
            if (data.TryGetValue("protocol", out nullOut)) protocol = data["protocol"] as Dictionary<string, object>;

            BasePlayer player = null;
            if (ulong.TryParse(userID, out var userIdUL)) player = BasePlayer.FindByID(userIdUL) ?? BasePlayer.FindSleeping(userIdUL) ?? null;
            
            Paste(preloadData, protocol, startPos, userID, stability, RotationCorrection, autoHeight ? heightAdj : 0, auth, callback);
            return true;
        }
            ...
            foreach (BaseEntity.Slot slot in checkSlots)
            {
                ...
                if (slotEntity.GetComponent<CodeLock>())
                {
                    ...
                    if (!string.IsNullOrEmpty(code))
                    {
                        ...
                        if (pasteData.Auth && !string.IsNullOrEmpty(pasteData.UserID))
                            codeLock.whitelistPlayers.Add(Convert.ToUInt64(pasteData.UserID));
        ...
        private object TryPasteBack(string filename, string userID, string[] args)
        {
            string path = subDirectory + filename;
            ...
            return TryPaste(startPos, filename, userID, rotationCorrection, args, autoHeight: false);
        }
        ...
        private void cmdChatPaste(IPlayer player, string command, string[] args)
        {
            if (!HasAccess(player, pastePermission))
            {
                SendReply(player, Lang("NO_ACCESS", player.Id));
                return;
            }

            if (args.Length < 1)
            {
                SendReply(player, Lang("SYNTAX_PASTE_OR_PASTEBACK", player.Id));
                return;
            }
            ...
            if (pObj == null)
            {
                SendReply(player, Lang("NOT_FOUND_PLAYER", player.Id));
                return;
            }

            var success = TryPasteFromSteamID(pObj.userID, args[0], args.Skip(1).ToArray());
        ...
        void cmdChatPasteAll(IPlayer player, string command, string[] args)
        {
            if (!HasAccess(player, pastePermission))
            {
                SendReply(player, Lang("NO_ACCESS", player.Id));
            ...
            if (!Physics.Raycast(pObj.eyes.HeadRay(), out var hit, 1000f, rayPaste))
            {
                SendReply(player, Lang("NO_ENTITY_RAY", player.Id));
                return;
            }
            var degreeRadian = DegreeToRadian(pObj.GetNetworkRotation().eulerAngles.y);
            var skipArgs = args.Skip(1).ToArray();
            var success = TryPaste(hit.point, args[0], player.Id, degreeRadian, skipArgs);
            if (success == null)
            {
                PrintWarning("success is null?!");
                SendReply(player, "Success returned null object!");
                return;
            }
            //TryPaste(hit.point, filename, player, DegreeToRadian(pObj.GetNetworkRotation().eulerAngles.y), args, callback: callback);
            //  var success = TryPasteFromSteamID(pObj.userID, args[0], args.Skip(1).ToArray());
            ...
            SendReply(player, "Attempting to paste on all servers");
            var path = subDirectory + args[0];
            var data = Interface.Oxide.DataFileSystem.GetDatafile(path);
            if (data == null || data["default"] == null || data["entities"] == null)
            {
                PrintWarning("Data is null!!!");
                SendReply(player, "Null data!");
                return;
            }
            var dataDic = new Dictionary<string, object>();
            foreach (var kvp in data) dataDic[kvp.Key] = kvp.Value;
            var jsonSettings = new JsonSerializerSettings();
            jsonSettings.Converters.Add(new KeyValuesConverter());
            DiscordAPI2.Call("BroadcastPaste", player.Id, hit.point, degreeRadian, JsonConvert.SerializeObject(dataDic, Formatting.Indented, jsonSettings), skipArgs);
            SendReply(player, "Called BroadcastPaste");
        }

        void cmdChatUndoAll(IPlayer player, string command, string[] args)
        {
            if (!HasAccess(player, pastePermission))
            {
                SendReply(player, Lang("NO_ACCESS", player.Id));
            ...
            UndoAll(player.Id);
            DiscordAPI2.Call("BroadcastUndo", player.Id);
            SendReply(player, "Called UndoAll");
        }

        private void UndoAll(string userId)
        {
            if (undoEntities.TryGetValue(userId, out var ents))
            {
                ...
                undoEntities.Remove(userId);
            }
        }

        private void cmdChatPasteManual(IPlayer player, string command, string[] args)
        {
            if (!HasAccess(player, pastePermission))
            {
                SendReply(player, Lang("NO_ACCESS", player.Id));
            ...
            var pos = GetVector3FromString(args[1]);
            if (pos == Vector3.zero)
            {
                SendReply(player, Lang("SYNTAX_PASTE_OR_PASTEBACK", player.Id));
                return;
            }
            var rotCorrection = 0f;
            if (args.Length > 2 && !float.TryParse(args[2], out rotCorrection))
            {
                SendReply(player, Lang("SYNTAX_PASTE_OR_PASTEBACK", player.Id));
                return;
            }
            var success = TryPasteFromVector3(pos, rotCorrection, args[0], args.Skip(3).ToArray());
            //var success = TryPasteFromSteamID(pObj.userID, args[0], args.Skip(1).ToArray());
        ...
        private readonly Dictionary<string, Dictionary<string, string>> messages = new Dictionary<string, Dictionary<string, string>>
        {
            ...
            {"SYNTAX_OWNERID", new Dictionary<string, string>() {
                {"en", "Option ownerID must be a user's ID"}
            }},
