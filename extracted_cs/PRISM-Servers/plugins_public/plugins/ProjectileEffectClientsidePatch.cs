...
{
    [Info("Projectile Effect Clientside Patch", "Shady", "0.0.2")]
    class ProjectileEffectClientsidePatch : RustPlugin
    {
        /*
        ...
        private readonly FieldInfo _reusableInstance = typeof(BaseProjectile).GetField("reusableInstance", BindingFlags.Static | BindingFlags.NonPublic | BindingFlags.FlattenHierarchy);

        private Effect _effect = null;

        private readonly DateTime _lastReusableGetAttempt = DateTime.MinValue;

        public Effect ReusableInstance
        {
            get
            {
                if (_effect == null && (_lastReusableGetAttempt <= DateTime.MinValue || (DateTime.UtcNow - _lastReusableGetAttempt).TotalSeconds > 5))
                    _effect = (Effect)_reusableInstance.GetValue(null);
        ...
        public static ProjectileEffectClientsidePatch Instance = null;
        #endregion
        #region Hooks
        private void Init()
        {
        ...
        #endregion
        #region CreateProjectileEffectClientside Patch
        [HarmonyPatch(typeof(BaseProjectile), "CreateProjectileEffectClientside")]
        private class CreateProjectileEffectClientsidePatch
        {


            private static bool Prefix(BaseProjectile __instance, string prefabName, Vector3 pos, Vector3 velocity, int seed, Network.Connection sourceConnection, bool silenced = false, bool forceClientsideEffects = false)
            {
                if (Interface.CallHook("OnClientProjectileEffectCreate", sourceConnection, __instance, prefabName, pos, velocity, seed, silenced, forceClientsideEffects) != null)
                {
                    return false;
                }

                Effect reusableInstance = Instance?.ReusableInstance;
                ...
                reusableInstance.scale = silenced ? 0.0f : 1f;
                if (forceClientsideEffects)
                    reusableInstance.scale = 2f;
                reusableInstance.pooledString = prefabName;
                reusableInstance.number = seed;
