...
{
    [Info("Login Reward", "Shady", "0.0.2")]
    public class LoginReward : RustPlugin
    {
        /*
        ...
        {
            var lastCon = Playtimes?.Call<DateTime>("GetLastConnection", player.UserIDString) ?? DateTime.MinValue;

            if (lastCon <= DateTime.MinValue)
            {
                PrintWarning("lastCon <= datetime.minval!!!!");
                return;
            }
            ...
            //temp isadmin
            if (!player.IsAdmin && now.Day <= lastCon.Day && now.Month <= lastCon.Month)
                return;

            var creds = 1;

            KNGSCredits?.Call("GiveCredits", player.UserIDString, creds);

            //SB needed
            SendReply(player, PRISM_RAINBOW_TXT + ": Thanks for logging in today! You've been gifted <color=#33ccff>" + creds.ToString("N0") + "</color> credit" + ((creds > 1) ? "s" : "") + "! Use these in our <i>shop</i> via <color=#33ccff>/credits</color>.");
        }
        #endregion
