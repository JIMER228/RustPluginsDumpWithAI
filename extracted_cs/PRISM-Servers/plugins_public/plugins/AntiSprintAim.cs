...
{
    [Info("AntiSprintAim", "Shady", "0.0.2")]

    public class AntiSprintAim : RustPlugin
    {
        /*
        ...
        private void OnPlayerInput(BasePlayer player, InputState input)
        {
            ...
            if (baseProj != null)
            {
                ...
                if (input.IsDown(BUTTON.FIRE_SECONDARY) || input.WasJustPressed(BUTTON.FIRE_SECONDARY))
                {
                    var eyesPosition = player?.eyes?.position ?? Vector3.zero;

                    player.Invoke(() =>
                    {
                        var isAiming = player?.modelState?.aiming ?? player?.IsAiming ?? false;
                        var isRunning = player?.IsRunning() ?? false;
                        ...
                        if (isAiming && isRunning)
                        {
                            ...
                            try
                            {
                                var warnMsg = sb.Clear().Append("<color=yellow>").Append(player.displayName).Append(" may be aiming while sprinting (").Append(activeItem?.info?.displayName?.english ?? string.Empty).Append(")!</color>").ToString();
                                core.Call("SendWarning", warnMsg);
                                core.Call("AddViolationFull", player.UserIDString, 13, eyesPosition, eyesPosition, warnMsg, 6f, GetPing(player)); //ViolationType.AimSprint = 13
                                core.Call("GiveAntiHack", player, 12f, AntiHackType.AttackHack);
                            }
