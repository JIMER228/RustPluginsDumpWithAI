...
{
    [Info("Workbench Level Patch", "Shady", "1.0.0")]
    internal class WorkbenchLevelPatch : RustPlugin
    {
        /*
         ...
         * 
         */
        public static WorkbenchLevelPatch Instance = null;
        ...
        #region Craft Level Patch Patch
        [HarmonyPatch(typeof(BasePlayer), "currentCraftLevel", MethodType.Getter)]
        private class CurrentCraftLevelPatch
        {
            private static bool Prefix(BasePlayer __instance, ref float __result)
            {
                ...
                var hook = Interface.Oxide.CallHook("OnGetCraftLevel", __instance);
                if (hook == null) return true;
                
                __result = Convert.ToSingle(hook);
