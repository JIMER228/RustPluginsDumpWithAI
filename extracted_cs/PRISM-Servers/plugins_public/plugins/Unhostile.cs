...
{
    [Info("Unhostile", "Shady", "0.0.4")]
    internal class Unhostile : RustPlugin
    {
        /*
        ...
        #region Init
        private void Init() => AddCovalenceCommand("unhostile", nameof(cmdUnhostile));
        #endregion
        #region Command
        private void cmdUnhostile(IPlayer player, string command, string[] args)
        {
            if (!player.IsAdmin) return;
            var target = args.Length > 0 ? FindPlayerByPartialName(args[0]) : (player?.Object as BasePlayer);
            if (target == null)
            {
                SendReply(player, "No target");
                return;
            }
            StopHostile(target);
            SendReply(player, "Removed hostility for: " + target.displayName);
        }
        #endregion
        #region Main
        ...
        public char[] GetValidCharacters() { return ValidCharacters.ToCharArray(); }
