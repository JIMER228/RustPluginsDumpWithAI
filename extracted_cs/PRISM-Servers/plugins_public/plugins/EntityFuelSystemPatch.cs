...
{
    [Info("EntityFuelSystemPatch", "Shady", "0.0.3", ResourceId = 0)]
    internal class EntityFuelSystemPatch : RustPlugin
    {
        /*
        ...
        #endregion
        #region Patch
        [HarmonyPatch(typeof(EntityFuelSystem), "AddStartingFuel")]
        private class HookPatch
        {
            private static bool Prefix(EntityFuelSystem __instance)
            {

                var canAdd = Interface.Oxide.CallHook("CanAddStartingFuel", __instance);
                if (canAdd != null) return false;

                var addItem = Interface.Oxide.CallHook("OnAddStartingFuel", __instance);
                if (addItem != null)
                {
                    var item = addItem as Item;
                    if (item != null && !item.MoveToContainer(__instance.GetFuelContainer().inventory)) RemoveFromWorld(item);
