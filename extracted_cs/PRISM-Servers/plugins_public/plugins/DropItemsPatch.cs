...
{
    [Info("Drop Items Patch", "Shady", "1.0.0")]
    class DropItemsPatch : RustPlugin
    {
        /*
        ...
        [HarmonyPatch(typeof(StorageContainer), "DropItems", new Type[] { typeof(IItemContainerEntity), typeof(BaseEntity) })]
        private class PatchDropItems
        {
            private static bool Prefix(IItemContainerEntity containerEntity, BaseEntity initiator = null)
            {
                return Interface.Oxide.CallHook("OnDropItems", containerEntity, initiator) == null; //"true" (which is the case if == null, aka no hook override, means we use original code. return false means we use our code.
            }
