...
{
    [Info("Loot Spawn Patch", "Shady", "1.0.0")]
    class LootSpawnPatch : RustPlugin
    {
        /*
                ...
                if (t != type && t.IsClass && t.FullName.Contains(type.FullName))
                {
                    ...
                    for (int j = 0; j < attributes.Length; j++)
                    {
                        try
                        {
                            ...
                            var originalMethod = patch.info.argumentTypes != null ? patch.info.declaringType.GetMethod(patch.info.methodName, (flags != null && flags.HasValue) ? (BindingFlags)flags : BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.FlattenHierarchy, null, patch.info.argumentTypes, null) : patch.info.declaringType.GetMethod(patch.info.methodName, (flags != null && flags.HasValue) ? (BindingFlags)flags : BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.FlattenHierarchy);
        ...
        #region DropItems Patch
        [HarmonyPatch(typeof(LootSpawn), "SpawnIntoContainer")]
        private class SpawnIntoContainerPatch
        {
            private static bool Prefix(LootSpawn __instance, LootContainer container)
            {
                return Interface.Oxide.CallHook("OnSpawnIntoContainer", __instance, container) == null; //"true" (which is the case if == null, aka no hook override, means we use original code. return false means we use our code.
            }
