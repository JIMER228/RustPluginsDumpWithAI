...
{
    [Info("Timed VM Block", "Shady", "0.0.1", ResourceId = 0)]
    internal class TimedVMBlock : RustPlugin
    {
        /*
        ...
        private void Init()
        {
            ItemUnavailableForSeconds = Interface.Oxide?.DataFileSystem?.ReadObject<Dictionary<int, int>>(nameof(TimedVMBlock)) ?? new Dictionary<int, int>();
            ...
            string[] cmdAliases = { "vmblock", };
            AddCovalenceCommand(cmdAliases, nameof(cmdVmBlock));
        }
        ...
        object OnBuyVendingItem(VendingMachine vending, BasePlayer player, int sellOrderId, int numberOfTransactions)
        {
            if (!(vending is NPCVendingMachine)) return null; //only NPC vending machines!!
            ...
            if (vending?.sellOrders?.sellOrders == null || vending.sellOrders.sellOrders.Count < 1) return null;
            ...
            var blockedStr = IsItemBlocked(getOrder.itemToSellID)?.ToString();
            if (!string.IsNullOrEmpty(blockedStr))
            {
                SendBlueprintBlockedFX(player, blockedStr);
                return true;
            }
        ...
        {
            Interface.Oxide.DataFileSystem.WriteObject(nameof(TimedVMBlock), ItemUnavailableForSeconds);
        }
        ...
        private void cmdVmBlock(IPlayer player, string command, string[] args)
        {
        ...
        private object IsItemBlocked(int itemId)
        {
            ...
            if (seconds == 0)
            {
                return "<color=#ff5280>This blueprint cannot be purchased</color>."; //disabled permanently
            }
            ...
            try
            {
                ...
                return sb.Clear().Append("<color=#ffc72b>").Append(itemInfo?.displayName?.english ?? string.Empty).Append("</color> <color=#fff563>will be available to be purchased in</color><color=#ff5eb4>:</color> <color=#82ff29>").Append(ReadableTimeSpan(newSpan)).Append("</color>").ToString();
            }
            finally { Facepunch.Pool.Free(ref sb); }
        }

        private bool IsItemPermanentlyBlocked(int itemTarget)
        {
            int seconds;
            return ItemUnavailableForSeconds.TryGetValue(itemTarget, out seconds) && seconds == 0;
        }

        private bool IsItemUnlockedYet(int itemTarget)
        {
            int seconds;
            if (!ItemUnavailableForSeconds.TryGetValue(itemTarget, out seconds)) return true;
        ...
        private string ReadableTimeSpan(TimeSpan span, string stringFormat = "N0") //I'm sure some of you uMod code snobs absolutely LOVE this one
        {
            ...
            if (span.TotalHours >= 24)
            {
                ...
                var showHours = totalHoursToShowAsNumber < 24;

                str = (int)span.TotalDays + (!showHours ? 1 : 0) + " day" + (span.TotalDays >= 2 ? "s" : "") + (showHours ? (" " + totalHoursWereGoingToShowToTheUserAsAString + " hour(s)") : "");
            }
