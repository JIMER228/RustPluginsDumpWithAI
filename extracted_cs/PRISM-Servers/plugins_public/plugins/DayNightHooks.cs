...
{
    [Info("Day Night Hooks", "Shady", "0.0.2")]
    class DayNightHooks : RustPlugin
    {
        /*
        ...
        private string _morningCalledDay = string.Empty;
        ...
        {
            StartCoroutine(CheckIfNightStarted());
            StartCoroutine(CheckIfMorningStarted());
        }
        ...
        private IEnumerator CheckIfNightStarted()
        {
            while (true)
            {
                var cycleInstance = TOD_Sky.Instance.Cycle;

                //this may be slightly redundant (the if checks), but it works. leave it be or commit 30 different times testing it.
        ...
        private IEnumerator CheckIfMorningStarted()
        {
            while (true)
            {
                var cycleInstance = TOD_Sky.Instance.Cycle;

                if (IsAMAndPastSunrise() && !HasMorningStartedForToday())
                {
                    _morningCalledDay = cycleInstance.DateTime.ToString("d");
                    Interface.Oxide.CallHook("OnStartMorning");
                }
        ...
        private bool HasMorningStartedForToday()
        {
            return _morningCalledDay.Equals(TOD_Sky.Instance.Cycle.DateTime.ToString("d"), StringComparison.OrdinalIgnoreCase);
        }
        #endregion
