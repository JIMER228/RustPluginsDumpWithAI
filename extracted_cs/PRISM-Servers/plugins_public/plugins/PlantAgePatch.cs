...
{
    [Info("Plant Age Patch", "Shady", "1.0.0")]
    internal class PlantAgePatch : RustPlugin
    {
        /*
         ...
         * 
         */
        public static PlantAgePatch Instance = null;
        ...
        #region Patch
        [HarmonyPatch(typeof(GrowableEntity), "GetGrowthBonus")]
        private class GrowableEntityPatch
        {
            private static bool Prefix(GrowableEntity __instance, float overallQuality, ref float __result)
            {
                if (__instance == null || __instance.IsDestroyed)
                    return false;


                float num = (float)(1.0 + (double)__instance.Genes.GetGeneTypeCount(GrowableGenetics.GeneType.GrowthSpeed) * 0.25);
                if ((double)overallQuality <= 0.0)
                    num = 1f;

                var hookRes = Interface.Oxide.CallHook("OnGrowableGetGrowthBonus", __instance, overallQuality, num);
                if (hookRes != null)
                    num = Convert.ToSingle(hookRes);
                

                /*/

                Interface.Oxide.LogWarning("old growth bonus: " + num + ", now: " + (num *= 2000));

                __instance.Invoke(() =>
                {
                    Interface.Oxide.LogWarning("Force running update again");
                    __instance.RunUpdate();
                }, 2f);/*/
