...
{
    [Info("ConnectionLimiter", "Exel80", "1.0.21", ResourceId = 2649)]
    ...
    class ConnectionLimiter : CovalencePlugin
    {
        ...
        #region Oxide Hooks
        object CanClientLogin(Connection conn)
        {
            string id = conn.userid.ToString();

            if (ServerUsers.Is(conn.userid, ServerUsers.UserGroup.Owner))
                return null;
