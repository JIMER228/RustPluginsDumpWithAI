...
{
    [Info("MemDbg", "Jake_Rich", 0.1)]
    [Description("Figure out where memory leaks probally are")]

    public class JakeMemoryDebug : RustPlugin
    {
        public static Oxide.Core.Plugins.Plugin debugPlugin { get; set; }

        public MemoryTableUIManager UI { get; set; }

        public static JakeMemoryDebug _plugin { get; set; }
        ...
        public void SelectPlugin(Oxide.Core.Plugins.Plugin plugin)
        {
            debugPlugin = plugin;
            Puts($"Now debugging: {plugin.Name}");
        }

        [ConsoleCommand("dbg")]
        void SelectPluginCommandConsole(ConsoleSystem.Arg arg)
        {
            if (!(arg.IsAdmin || arg.IsServerside))
            {
                Puts("This command is for admins only!");
                return;
            }

            if (!arg.HasArgs(1))
            {
                Puts($"Wrong number of args! Format is \"{arg.cmd.Name} pluginName\"");
                return;
            }

            foreach (var _plugin in Interface.Oxide.RootPluginManager.GetPlugins())
            {
                if (_plugin.Name.ToLower().Contains(arg.Args[0].ToLower()))
                {
                    SelectPlugin(_plugin);
                    return;
                }
            }
            Puts($"No plugin found containing {arg.Args[0]}!");
        }

        [ConsoleCommand("dump")]
        void TestReflectionSizeOfConsole(ConsoleSystem.Arg arg)
        {
            if (!(arg.IsAdmin || arg.IsServerside))
            {
                Puts("This command is for admins only!");
            ...
            if (!arg.HasArgs(1))
            {
                Puts($"Wrong number of args! Format is \"{arg.cmd.Name} (int)depth\"");
                return;
            }

            int layers = 1;
            if (!int.TryParse(arg.Args[0], out layers))
            {
                Puts($"{arg.Args[0]} is not a number!");
                return;
            }

            var obj = new ObjectMemoryInfo(debugPlugin, layers);

            Oxide.Core.Interface.Oxide.DataFileSystem.WriteObject($"MemDump-{debugPlugin.Name}", obj);
            Puts("Dumped Memory!");
        }
        ...
        public static bool IsValueType(object obj)
        {
            ...
            return (obj is ValueType || obj is string) && !obj.GetType().IsGenericType;
        }
        ...
        public class UIElement
        {
            ...
            private JakeMemoryDebug _plugin;
        ...
        public class MemoryObjectInfoUI : UIContainer
        {
            ...
            public MemoryObjectInfoUI(MethodInfo method, Vector2 pos, MemoryTableUIManager manager, object _target)
            {
                background = new UIPanel(this, pos, 1f - pos.x - 0.05f, (0.90f / objectsPerScreen));
                AddElement(background);
                ...
                expand.AddCallback((arg) =>
                {
                    if (method.ReturnType != null)
                    {
                        ...
                        {
                            _plugin.PrintToChat(manager._player, method.Invoke(_target, null)?.ToString());
                        }
                        else
                        {
                            var objectReturn = method.Invoke(_target, null);
                            var rootObject = new ObjectMemoryInfo(method.Invoke(_target,null), int.MaxValue, objectReturn.GetType().Name);
        ...
        public class MemoryTableUIManager : UIContainer
        {
            ...
            public void ShowObject(ObjectMemoryInfo obj, bool changePage = false)
            {
                ...
                foreach(var method in obj.methods)
                {
                    ...
                    shownObjects.Add(new MemoryObjectInfoUI(method, pos, this, obj._target));
                    pos.y -= (0.90f / objectsPerScreen);
                    i++;
                }
