...
namespace Oxide.Plugins
{
    ...
    class Kits : RustPlugin
    {
        ...
        {
            List<KitItem> kititems = ( from item in player.inventory.containerWear.itemList where item != null select ProcessItem( item, "wear" ) ).ToList();
            kititems.AddRange( from item in player.inventory.containerMain.itemList where item != null select ProcessItem( item, "main" ) );
            kititems.AddRange( from item in player.inventory.containerBelt.itemList where item != null select ProcessItem( item, "belt" ) );
            return kititems;
        }
        ...
        void TryGiveKit( BasePlayer player, string kitname )
        {
            ...
            SendReply( player, "НАБОР ПОЛУЧЕН!" );
        ...
        object GiveKit( BasePlayer player, string kitname )
        {
            ...
            {
                if (kitem.weapon)
                    player.inventory.GiveItem( BuildWeapon( kitem.itemid, (ulong) kitem.skinid, kitem.mods ), kitem.container == "belt" ? player.inventory.containerBelt : kitem.container == "wear" ? player.inventory.containerWear : player.inventory.containerMain );
                else player.inventory.GiveItem( BuildItem( kitem.itemid, kitem.amount, (ulong) kitem.skinid ), kitem.container == "belt" ? player.inventory.containerBelt : kitem.container == "wear" ? player.inventory.containerWear : player.inventory.containerMain );
        ...
        private Item BuildItem( int itemid, int amount, ulong skin )
        {
            ...
            Item item = ItemManager.CreateByItemID( itemid, amount, (ulong) skin );
            item.skin = skin;
            return item;
        }
        ...
        object CanRedeemKit( BasePlayer player, string kitname )
        {
            ...
            if (thereturn != null)
            {
                ...
                return "Вы не можете активировать набор в данный момент.";
            }
            ...
            if (!string.IsNullOrEmpty( kit.permission ))
                if (player.net.connection.authLevel < 2 && !permission.UserHasPermission( player.UserIDString, kit.permission ))
                    return "У вас нет прав  на использование набора.";

            var kitData = GetKitData( player.userID, kitname );
            if (kit.max > 0)
                if (kitData.max >= kit.max)
                    return "Вы уже использовали этот набор.";

            if (kit.cooldown > 0)
            {
                var ct = CurrentTime();
                if (kitData.cooldown > ct && kitData.cooldown != 0.0)
                    return $"Для использования набора подождите {Math.Abs( Math.Ceiling( kitData.cooldown - ct ) )} сек...";
            }
        ...
        [HookMethod( "GetAvailableKitList" )]
        private List<object[]> GetAvailableKitList( BasePlayer player )
        {
        ...
        [HookMethod( "TryGiveKitToPlayer" )]
        private void TryGiveKitToPlayer( BasePlayer player, string kitname )
        {
            TryGiveKit( player, kitname );
        }
        //////////////////////////////////////////////////////////////////////////////////////
