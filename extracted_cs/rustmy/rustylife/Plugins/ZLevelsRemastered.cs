    ...
    class ZLevelsRemastered : RustPlugin
    {
        ...
        */
        ...
        public Dictionary<string, long> getConnectedPlayerDetailsData(ulong userID)
        {
            ...
            statsInit.Add("CLevel", 0);
            statsInit.Add("CPoints", 0);
        ...
        public static class Skills
        {
            ...
            public static string[] ALL = { WOODCUTTING, MINING, SKINNING };
        }
        ...
        void ZlvlCommand(ConsoleSystem.Arg arg)
        {
            ...
            if (p != null || (playerName == "*" || playerName == "**"))
            {
                ...
                if (skill == Skills.WOODCUTTING || skill == Skills.MINING || skill == Skills.SKINNING || skill == "*")
                {
                    var allSkills = skill == "*";
            ...
            if (args.Length == 1)
            {
                ...
                switch (statname)
                {
                    ...
                    default:
                        ...
                        messagesText += "\nYou must choose from these stats: <color=" + colors[Skills.MINING] + ">Mining</color>, <color=" + colors[Skills.SKINNING] + ">Skinning</color>, <color=" + colors[Skills.WOODCUTTING] + ">Woodcutting</color>";
                        break;
                }
            }
            else
            {
                messagesText = "You must choose from these stats: <color=" + colors[Skills.MINING] + ">Mining</color>, <color=" + colors[Skills.SKINNING] + ">Skinning</color>, <color=" + colors[Skills.WOODCUTTING] + ">Woodcutting</color>";
            }
        ...
        private void RenderUI(BasePlayer player)
        {
            ...
            int enabledSkillCount = Skills.ALL.Count(skill => !IsSkillDisabled(skill));
            ...
            RenderPlayerUI(ref elements, mainName, player);


            CuiHelper.AddUi(player, elements);
        }

        void RenderPlayerUI(ref CuiElementContainer container, string mainPanel, BasePlayer player)
        {
            var rt = CuiHorizontalGroup(3, 0, 0, 1, 1, 0.025f, false, 1);
            for (int i = 0; i < Skills.ALL.Length; i++)
            {
                ...
                ProgressBar(ref container,
                    container.Add(
                        new CuiPanel()
                        {
                            RectTransform = { AnchorMin = rt[i].AnchorMin, AnchorMax = rt[i].AnchorMax },
                            Image = { Color = "0 0 0 0.2" }
                        }, mainPanel), getExperiencePercentInt(player, skill), player, skill);
            }
        }

        void ProgressBar(ref CuiElementContainer container, string panel, float progress, BasePlayer player, string skill)
        {
            ...
            {
                {"WC", "<color=#286a02>ЛЕСОРУБ</color>" },
                {"M","<color=#024067>РУДОКОП</color>" },
                {"S","<color=#670b0a>МЯСНИК</color>" },
            };
            ...
            string xpBarAnchorMin = "0.16 0.1";
            string xpBarAnchorMax = "0.88 0.9";
            var lvl = getLevel(player.userID, skill);
            var percent = getExperiencePercentInt(player,
                skill);
            var points = getPoints(player.userID, skill);
            var maxPoints = skillMaxed ? "∞" : getLevelPoints(getLevel(player.userID, skill) + 1).ToString();

            var pbname = container.Add(new CuiPanel()
            {
                CursorEnabled = false,
                RectTransform =
                {
                    AnchorMin = xpBarAnchorMin,
                    AnchorMax = xpBarAnchorMax
                },
                Image = { Color = $"{skillColors[skill]}0.45" }
            }, panel);
            var pb = container.Add(new CuiPanel()
            {
                CursorEnabled = false,
                RectTransform = { AnchorMin = "0 0", AnchorMax = $"{progress / 100} 1" },
                Image = { Color = $"{skillColors[skill]}0.99" }
            }, pbname);

            container.Add(new CuiLabel()
            {
                RectTransform = { AnchorMin = "0 0", AnchorMax = "1 1" },
                Text = { Text = skillNames[skill], Color = "0 0 0 1", FontSize = fontSize, Align = TextAnchor.MiddleCenter }
            }, pbname);

            container.Add(new CuiLabel()
            {
                RectTransform = { AnchorMin = "0.88 0", AnchorMax = "1 1" },
                Text = { Align = TextAnchor.MiddleCenter, Text = $"{progress}%", Color = "1 0.76 0.52 1", FontSize = fontSize }
            }, panel);

            container.Add(new CuiLabel()
            {
                RectTransform = { AnchorMin = "0 0", AnchorMax = "0.16 1" },
                Text = { Align = TextAnchor.MiddleCenter, Text = $"lvl.{lvl}", Color = "1 0.76 0.52 1", FontSize = fontSize }
            }, panel);
        }
        public List<CuiRectTransformComponent> CuiHorizontalGroup(ushort lenght, float x1, float y1, float x2, float y2,
            float spacing, bool useAspect, [DefaultValue(0.0f)] float aspect)
        {
            return CuiMatrixGroup(lenght, 1, x1, y1, x2, y2, spacing, useAspect, aspect);
        }

        public List<CuiRectTransformComponent> CuiMatrixGroup(ushort matrixWidth, ushort matrixHeight, float x1, float y1, float x2, float y2, float spacing, bool useAspect, [DefaultValue(0.0f)]float aspect)
        {
            var cuiRectTransformComponents = new List<CuiRectTransformComponent>();
            if (!useAspect)
                aspect = (x2 - x1) / (y2 - y1);
            var aspect05 = aspect * 0.5f;
            var width = (x2 - x1 - (matrixWidth + 1) * (spacing / aspect)) / matrixWidth;
            var height = (y2 - y1 - (matrixHeight + 1) * spacing * aspect) / matrixHeight;

            for (var y = matrixHeight - 1; y >= 0; y--)
                for (var x = 0; x < matrixWidth; x++)
                {
                    cuiRectTransformComponents.Add(new CuiRectTransformComponent()
                    {
                        AnchorMin =
                            $"{spacing / aspect * (x + 1) + width * x} {spacing * aspect * (y + 1) + height * y}",
                        AnchorMax =
                            $"{spacing / aspect * (x + 1) + width * (x + 1)} {spacing * aspect * (y + 1) + height * (y + 1)}"
                    });
                }
            return cuiRectTransformComponents;
        }
        string getStatPrint(BasePlayer player, string skill)
        {
            ...
            var bonusText = ((getGathMult(getLevel(player.userID, skill), skill) - 1) * 100).ToString("0.##");
        ...
        void OnEntityDeath(BaseCombatEntity entity, HitInfo hitInfo)
        {
            if (entity is BasePlayer)
            {
                ...
                if (!inPlayerList(player.userID) || (isPlaying is bool && (bool)isPlaying) || InDuel(player)) return;
