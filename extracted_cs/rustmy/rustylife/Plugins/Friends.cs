    ...
    public class Friends : CovalencePlugin
    {
        ...
        protected override void LoadDefaultMessages()
        {
            ...
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                {"PendingSuccessSend", "Запрос успешно отправлен, ожидайте"},
                {"PendingNotFound", "К вам нет запросов в друзья"},
                {"Pending", "{0} отправил вам запрос в друзья, чтобы принять введите в чат /friend accept"},
            }, this);
        }
        ...
        private void Reply(IPlayer player, string langKey, params object[] args)
        {
            ...
            {
                Puts($"'{langKey}' not found in messages");
                return;
            }
            player.Reply(Messages[langKey], args);
            Puts("REPLY: "+ string.Format( Messages[ langKey ], args) );
        }
