...
namespace Oxide.Plugins
{
    ...
    class Give : RustPlugin
    {
        ...
        public object GiveItem(BasePlayer player, string itemname, int amount, ItemContainer pref, out string description)
        {
            ...
            else
            {
                for (var i = amount; i > 0; i = i - stack)
                {
                    ...
                    player.inventory.GiveItem(ItemManager.CreateByItemID((int)definition.itemid, giveamount), pref);
                    SendReply(player, string.Format("<size=18><color=#ff0048>Вы получили {0} x {1}</color></size>", description, giveamount.ToString()));
                }
            }
            return true;
        }
        ...
        void cmdConsoleGivePlayer(ConsoleSystem.Arg arg)
        {
            ...
            if (hasKit(arg.FullString.ToString()))
            {
            ...
            if (arg.Connection != null)
            {
                ...
                if (logAdmins)
                    Puts(string.Format("GIVE: {0} used inv.giveplayer {1}", ((BasePlayer)arg.Connection.player).displayName.ToString(), arg.FullString.ToString()));
            }
            else
                if (logAdmins)
                    Puts(string.Format("GIVE: {0} used inv.giveplayer {1}", "CONSOLE", arg.FullString.ToString()));
        ...
        void cmdConsoleGive(ConsoleSystem.Arg arg)
        {
            ...
            if (arg.Connection != null)
            {
                ...
                if (logAdmins)
                    Puts(string.Format("GIVE: {0} used inv.give {1}", ((BasePlayer)arg.Connection.player).displayName.ToString(), arg.FullString.ToString()));
            }
        ...
        void cmdConsoleGiveAll(ConsoleSystem.Arg arg)
        {
            ...
            if (arg.Connection != null)
            {
                ...
                if (logAdmins)
                    Puts(string.Format("GIVE: {0} used inv.giveall {1}", ((BasePlayer)arg.Connection.player).displayName.ToString(), arg.FullString.ToString()));
            }
            else
                if (logAdmins)
                    Puts(string.Format("GIVE: {0} used inv.giveall {1}", "CONSOLE", arg.FullString.ToString()));
            GiveToAll(arg);
        }
    }
}