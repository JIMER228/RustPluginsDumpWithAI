...
{
    [Info("Copy Paste", "Reneb", "3.0.7", ResourceId = 5981)]
    class CopyPaste : RustPlugin
    {
        ...
        void TryCopyLock(BaseEntity lockableEntity, IDictionary<string, object> housedata)
        {
            ...
            if (slotentity != null)
            {
                ...
                {
                    {"prefabname", slotentity.LookupPrefabName()}
                };
        ...
        Dictionary<string,object> EntityData(BaseEntity entity, Vector3 sourcePos, Vector3 sourceRot, Vector3 entPos, Vector3 entRot, float diffRot, bool saveInventories)
        {
            ...
            {
                { "prefabname", entity.LookupPrefabName() },
            ...
            foreach(var data in entities)
            {
                ...
                if (checkPlaced)
                {
                    foreach (var col in Physics.OverlapSphere(pos, 1f))
                    {
                        ...
                        if (ent != null)
                        {
                            if (ent.LookupPrefabName() == prefabname && ent.transform.position == pos && ent.transform.rotation == rot)
                            {
                                isplaced = true;
                                break;
                            }
                ...
                if (entity != null)
                {
                    ...
                    if (box != null)
                    {
                        ...
                        foreach (var itemDef in items)
                        {
                            ...
                            var itembp = Convert.ToBoolean(item["blueprint"]);
                            var itemskin = Convert.ToInt32(item["skinid"]);
                            var itemcondition = Convert.ToSingle(item["condition"]);

                            var i = ItemManager.CreateByItemID(itemid, itemamount, itembp, itemskin);
