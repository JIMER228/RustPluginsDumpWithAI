            ...
            switch(playerGUI[player.userID].subsection)
            {
                ...
                case "banned":
                    ...
                    foreach(KeyValuePair<string,object> pair in ebslist)
                    {
                        ...
                        if (playerList.ContainsKey(pair.Key)) continue;
                        var playerbanlistdata = pair.Value as Dictionary<string, object>;
                        if (playerbanlistdata == null) continue;
                        var name = "Unknown Player";
                        if(playerbanlistdata != null)
                            name = playerbanlistdata["name"].ToString();
                        var playerdata = (PlayerDatabase.Call("GetPlayerData", pair.Key.ToString(), "default") as Dictionary<string, object>);
                        if (playerdata != null)
                        {
                            if (playerdata["name"] != null)
                            {
