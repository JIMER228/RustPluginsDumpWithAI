...
namespace Oxide.Plugins
{
    ...
    class Give : RustPlugin
    {
        ...
        void GiveKit(object source, string[] Args, string ttype)
        {
            ...
            if(!(kitsplugin is Oxide.Core.Plugins.Plugin))
            {
            ...
            {
                SendTheReply(source, "===== Avaible kits to give =====");
                ((Oxide.Core.Plugins.Plugin)kitsplugin).CallHook("SendList", new object[] { source });
                return;
            }
            object target = false;
            if (ttype == "player")
            ...
            if (ttype == "all")
            {
                ...
                {
                    object trytogivekit = ((Oxide.Core.Plugins.Plugin)kitsplugin).CallHook("GiveKit", new object[] { onlineplayer, targetkit });
            ...
            {
                object trytogivekit = ((Oxide.Core.Plugins.Plugin)kitsplugin).CallHook("GiveKit", new object[] { target, targetkit });
                if (trytogivekit == null || (trytogivekit is bool && (!(bool)trytogivekit)))
                {
                    SendTheReply(source, "Couldn't give the kit, does it really exist?");
                    return;
