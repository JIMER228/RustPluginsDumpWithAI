...
- Players can get around kick by sleeping?
*/
...
{
    [Info("AFK", "Wulf/lukespragg", "1.1.4", ResourceId = 1922)]
    ...
    class AFK : CovalencePlugin
    {
        ...
        void OnUserInit(IPlayer player) => AfkCheck(player);
        void OnUserSpawned(IPlayer player) => AfkCheck(player);
        ...
        void AfkCheck(IPlayer player)
        {
            ...
            lastPosition[player.Id] = player.Character.Position();
        ...
        bool IsPlayerAfk(IPlayer player)
        {
            ...
            var current = player.Character.Position();
            ...
            if (last.X.Equals(current.X) || last.X.Equals(current.Y) || last.X.Equals(current.Z)) return true;
