...
{
    [Info("Tickets", "LaserHydra", "3.1.1", ResourceId = 1065)]
    [Description("Gives players the opportunity to send Tickets to admins.")]
    internal class Tickets : CovalencePlugin
    {
        #region Variable Declaration

        [PluginReference] private Plugin Slack, PushAPI, EmailAPI;
        ...
        private string GetCommandInfo(IPlayer player, string command)
        {
            ...
            for (int i = 0; i < commands.Length; i++)
                commands[i] = (player.IsServerConsole() || player.LastCommand == CommandType.Console ? $"{command} " : $"/{command} ") + commands[i];
...
namespace Oxide.Plugins.TicketsExtensions
{
    internal static class Extend
    {
        public static void ReplySafe(this IPlayer player, string message) =>
            player.Reply(player.IsServerConsole() ? Tickets.StripTags(message) : Tickets.FormatText(message));

        public static bool IsServerConsole(this IPlayer player) => player.Id == "server_console";
    }
}
