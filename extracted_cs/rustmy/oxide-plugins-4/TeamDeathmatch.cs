...
{
    [Info("TeamDeathmatch", "k1lly0u", "0.3.3", ResourceId = 1484)]
    class TeamDeathmatch : RustPlugin
    {
        ...
        #region Commands
		[ChatCommand("team")]
		void cmdSwitchTem(BasePlayer player, string command, string[] args)
		{
			if (!UseTDM || !Started || GameEnding) return;
            var p = player.GetComponent<TDMPlayer>();
            if (!p) return;

			var newTeam = Team.NONE;
			if (p.team == Team.A) newTeam = Team.B;
			else newTeam = Team.A;
			p.team = newTeam;
            ...
            EventManager.BroadcastToChat(string.Format("{0} has been moved to Team {1}", p.player.displayName, newTeam.ToString().ToUpper()));
