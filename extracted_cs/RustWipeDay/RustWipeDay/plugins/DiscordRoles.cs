...
{
    [Info("Discord Roles", "MJSU", "2.0.10")]
    ...
    class DiscordRoles : CovalencePlugin
    {
        ...
        private void OnDiscordGuildMemberUpdated(GuildMember update, GuildMember oldMember, DiscordGuild guild)
        {
            ...
            string playerId = _link.GetSteamId(update.User.Id);
            if (string.IsNullOrEmpty(playerId))
            {
                Debug(DebugEnum.Info, $"{nameof(OnDiscordGuildMemberUpdated)} Player ID is null or empty for user {update.User.GetFullUserName}");
                return;
            }
        ...
        public void HandleDiscordChange(DiscordUser user, bool isLeaving, SyncEvent syncEvent)
        {
            ...
            if (string.IsNullOrEmpty(playerId))
            {
                Debug(DebugEnum.Info, $"{nameof(HandleDiscordChange)} Player ID is null or empty for user {user.GetFullUserName}");
                return;
            }
            ...
            if (player == null)
            {
                Debug(DebugEnum.Info, $"{nameof(HandleDiscordChange)} IPlayer not found for user {user.GetFullUserName}");
                return;
            }
