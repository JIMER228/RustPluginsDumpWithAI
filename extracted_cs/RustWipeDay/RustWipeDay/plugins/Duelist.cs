    ...
    public class Duelist : RustPlugin
    {
        ...
        public class Rematch
        {
            ...
            public bool AddRange(List<BasePlayer> players, Team team)
            {
                ...
                {
                    if (!player || !player.IsConnected || _.InEvent(player) || Good.Contains(player) || Evil.Contains(player))
                        break;
        ...
        public class GoodVersusEvilMatch
        {
            ...
            {
                if (!CanRematch || _goodRematch.Exists(player => !player || !player.IsConnected) || _evilRematch.Exists(player => !player || !player.IsConnected))
                    return false;
        ...
        {
            if (entity == null || entity.transform == null || !DuelTerritory(entity.transform.position))
                return;
            ...
            if (prefabs.ContainsKey(entity.PrefabName) && entity.name.Contains("barricade."))
            {
                if (morphBarricadesStoneWalls || morphBarricadesWoodenWalls)
                {
                    ...
                    if (wall != null)
                    {
                        ...
                        if (!duelEntities.ContainsKey(wall.OwnerID))
                            duelEntities.Add(wall.OwnerID, new List<BaseEntity>() { wall });
                        else duelEntities[wall.OwnerID].Add(wall);
                    }
        ...
        {
            if (!player || !player.IsConnected || AimTrain.CanCall() && Convert.ToBoolean(AimTrain?.Call("IsAimTraining", player.userID)))
                return;
...
namespace Oxide.Plugins.DuelistExtensionMethods
{
    public static class ExtensionMethods
    {
        ...
        public static bool IsHuman(this BasePlayer a) { if (a.IsNpc || !a.userID.IsSteamId()) { return false; } return true; }
