...
{
    [Info("GUIAnnouncements", "JoeSheep", "2.0.3", ResourceId = 1222)]
    ...
    class GUIAnnouncements : RustPlugin
    {
        ...
        private HashSet<uint> CH47NetIDs = new HashSet<uint>();
        private HashSet<uint> KilledCH47NetIDs = new HashSet<uint>();
        ...
        void OnHelicopterDropCrate(CH47HelicopterAIController heli)
        {
            if (!RestartCountdown)
            {
                if (CH47CrateDroppedAnnouncementsEnabled && !CH47NetIDs.Contains(heli.net.ID))
                {
