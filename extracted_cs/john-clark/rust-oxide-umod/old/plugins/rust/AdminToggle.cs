...
{
    [Info("Admin Toggle", "LaserHydra", "1.0.5", ResourceId = 1371)]
    ...
    class AdminToggle : RustPlugin
    {
        ...
        void Loaded()
        {
            permission.RegisterPermission("admintoggle.use", this);
            data = Interface.Oxide.DataFileSystem.ReadObject<Data>("AdminToggle_Data");
            LoadConfig();
        }
        ...
        void SaveData() => Interface.Oxide.DataFileSystem.WriteObject("AdminToggle_Data", data);
        ...
        void OnPlayerInit(BasePlayer player)
        {
            ...
            {
                player.displayName = data.AdminData[player.userID.ToString()].PlayerName;
            }
        }

        bool CheckPermission(BasePlayer player)
        {
            if(permission.UserHasPermission(player.userID.ToString(), "admintoggle.use")) return true;
        ...
        void ToggleAdmin(BasePlayer player)
        {
            ...
            player.displayName = data.AdminData[player.userID.ToString()].EnabledPlayerMode ? data.AdminData[player.userID.ToString()].AdminName : data.AdminData[player.userID.ToString()].PlayerName;
