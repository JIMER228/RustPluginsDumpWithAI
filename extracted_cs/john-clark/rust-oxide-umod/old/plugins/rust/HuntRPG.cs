...
{
    [Info("Hunt RPG", "Default", "1.7.65")]
    public class HuntRPG : RustPlugin
    {
        /*
         * Credits to PedraozauM for creating this plugin, and to Nogrod for maintaining it after him.
         * 
         * 
         * TODO
         * Implement Economics better
         * Reintroduce increasing max health <-- Working on currently
         * Patch up the skills.
         * Remove researcher.
        ...
        void OnEntityTakeDamage(BasePlayer player, HitInfo hitInfo, BaseCombatEntity entity)
        {
        ...
        void OnPlayerDie(BasePlayer player, HitInfo hitInfo, BaseCombatEntity entity)
        {
            var rpgInfo = FindRpgInfo(player);
            if (rpgInfo == null) return;
            if (player.displayName == null) return;
        ...
        void OnPlayerHealthChange(BasePlayer player)
        {
            if (player == null)
                return;
        ...
        private void UpdateLang()
        {
            var messagesConfig = new Dictionary<HMK, string>
            {
                ...
                {HMK.Dodged, "You <color=red>dodged</color> an attack!"},
                {HMK.TargetDodged, "Your target <color=red>Dodged</color>"},
                ...
                {HMK.ProfileHeader, "========{0}========"},
                {
                    HMK.Loaded, string.Join("\n", new[]
                    {
                        ...
                        "<color=lightblue>Hunt</color>: To open the GUI type \"/hg\""
                    })
                },
                ...
                {HMK.AgiDesc, "Agillity gives you the chance to <colour=red>dodge</color> an attack!"},
