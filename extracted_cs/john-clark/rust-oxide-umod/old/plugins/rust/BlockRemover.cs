...
{
    [Info("Block Remover", "bawNg / Nogrod", "0.4.5", ResourceId = 864)]
    ...
    class BlockRemover : RustPlugin
    {
            ...
            foreach (var cupboard in toolCupboards)
            {
                ...
                for (var i = 0; i < count; i ++)
                {
                    ...
                    else
                    {
                        ...
                        foreach (var item in batch.meshLookup.src.data)
                        {
        ...
        {
            var started_at = Time.realtimeSinceStartup;
            var blocks = new HashSet<BuildingBlock>(BaseNetworkable.serverEntities.OfType<BuildingBlock>().Where(block => block.grade == grade));
        ...
        {
            var started_at = Time.realtimeSinceStartup;
            var stabilityEntities = new HashSet<StabilityEntity>(BaseNetworkable.serverEntities.OfType<StabilityEntity>());
