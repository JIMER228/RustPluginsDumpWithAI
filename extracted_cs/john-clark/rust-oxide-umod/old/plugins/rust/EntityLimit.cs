...
{
    [Info("Entity Limit", "PaiN", "0.6.3", ResourceId = 1947)]
    class EntityLimit : RustPlugin
    {
                        ...
                        if (Cfg.MaxLimits.Any(x => x.Key == playerEnt.Name))
                        {
                            if (playerEnt.Count == Cfg.MaxLimits[playerEnt.Name])
                            {
                                ...
                                if (entity.ShortPrefabName == "autoturret_deployed")
                                    item = ItemManager.CreateByName("autoturret");
                                if (entity.ShortPrefabName == "landmine")
                                    item = ItemManager.CreateByName("trap.landmine");
        ...
        void OnRemovedEntity(BaseEntity entity) => EntityDestroyed(entity);
