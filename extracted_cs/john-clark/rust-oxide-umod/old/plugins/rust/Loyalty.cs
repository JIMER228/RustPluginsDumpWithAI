...
{
    [Info("Loyalty", "Bamabo", "1.3.6", ResourceId = 1978)]
    ...
    public class Loyalty : RustPlugin
    {
                        ...
                        else
                        {
                            ...
                            foreach (var reward in data.rewards)
                            {
                                if (data.players[player.userID].loyalty == reward.requirement)
                                {
                                    ...
                                    {
                                        permission.GrantUserPermission(player.UserIDString, reward.permission, null);
                                    ...
                                    {
                                        permission.RevokeUserPermission(player.UserIDString, reward.permission.Replace('-', ' ').Trim());
                                ...
                                if (data.players[player.userID].loyalty == usergroup.requirement)
                                {
                                    if (!usergroup.usergroup.StartsWith("\"-"))
                                    {
                                        ...
                                        if (!String.IsNullOrEmpty(data.players[player.userID].group))
                                            permission.RemoveUserGroup(player.UserIDString, data.players[player.userID].group);
                                    ...
                                    {
                                        permission.RemoveUserGroup(player.UserIDString, usergroup.usergroup.Replace('-', ' ').Trim());
                                    }
        ...
        void CmdReset(BasePlayer sender, string playerName)
        {
            ...
            foreach (var reward in data.rewards)
                permission.RevokeUserPermission(player.name, reward.permission.Replace('-', ' ').Trim());
            foreach (var group in data.usergroups)
                permission.RemoveUserGroup(player.name, group.usergroup.Replace('-', ' ').Trim());
            player.group = "";
        ...
        void CmdSet(BasePlayer sender, string playerName, string newLoyalty)
        {
            ...
            foreach (var reward in data.rewards)
            {
                if (newLoy >= reward.requirement)
                {
                    if (reward.permission.StartsWith("\"-"))
                        ...
                        permission.GrantUserPermission(player.name, reward.permission.Replace('-', ' ').Trim(), null);
                }
                else
                {
                    permission.RevokeUserPermission(player.name, reward.permission.Replace('-', ' ').Trim());
            ...
            {
                permission.RemoveUserGroup(player.name, player.group);
            }
            ...
            if (newGroup != null)
            {
                ...
                {
                    permission.RemoveUserGroup(player.name, newGroup.usergroup.Replace('-', ' ').Trim());
                    if (newGroup.usergroup.TrimStart('-') == data.players[player.id].group)
                        data.players[player.id].group = "";
                }
                else
                {
                    permission.AddUserGroup(player.name, newGroup.usergroup.Replace('-', ' ').Trim());
