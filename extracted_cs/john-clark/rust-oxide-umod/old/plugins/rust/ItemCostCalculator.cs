...
namespace Oxide.Plugins
{
    ...
    class ItemCostCalculator : RustPlugin
    {
        ...
        void Loaded()
        {
            CData = Interface.Oxide.DataFileSystem.GetFile("GUIShop_File");
        }
        ...
        class CostData
        {
            public Dictionary<string, Dictionary<string, object>> CopyBelowThis = new Dictionary<string, Dictionary<string, object>>();
        }
        ...
        class ConfigData
        {
            ...
            public int GatherRateOffset { get; set; }
            public Dictionary<string, int> rarityList { get; set; }
        }
        ...
        rarityList = new Dictionary<string, int>
        {
            {"timed.explosive", 5 },
            {"rifle.bolt", 2 },
            {"hammer", 0 }
        ...
        [ConsoleCommand("shopfile")]
        private void cmdshopfile(ConsoleSystem.Arg arg)
        {
            GetItemCost();
        }
        ...
        private void GetItemCost()
        {
            ...
            {
                if (configData.materials.ContainsKey(item.shortname))
                {
                    costs.Add(item.shortname, configData.materials[item.shortname]);
                    continue;
                }
                double cost = 0;
                var bp = ItemManager.FindBlueprint(ItemManager.FindItemDefinition(item.itemid));
                if (bp != null)
                    foreach (var i in bp.ingredients)
                        if (i.amount > 0)
                            if (!string.IsNullOrEmpty(i.itemDef.shortname))
                                if (configData.materials.ContainsKey(i.itemDef.shortname))
                                    cost += ((double)i.amount * configData.materials[i.itemDef.shortname]);
                                else if (costs.ContainsKey(i.itemDef.shortname))
                                    cost += (double)i.amount * costs[i.itemDef.shortname];

                if (cost != 0)
                {
                    if (configData.GatherRateOffset > 0)
                        cost = cost * configData.GatherRateOffset;
                    if (configData.rarityList.ContainsKey(item.shortname))
                    {
                        if (configData.rarityList[item.shortname] == 0)
                            cost = 0;
                        else cost = cost + cost*(configData.rarityList[item.shortname]/100);
                    }
                    //Puts($"{item.shortname}: {cost}");
                    if (!costs.ContainsKey(item.shortname))
                        costs.Add(item.shortname, cost);
                }
            }
            SaveShopFile();
        }
        ...
        {
            if (cdata.CopyBelowThis == null)
                cdata.CopyBelowThis = new Dictionary<string, Dictionary<string, object>>();
            else cdata.CopyBelowThis.Clear();
            foreach (var entry in costs)
            {
                var img = "";
                if (ItemImages.ContainsKey(entry.Key))
                    img = ItemImages[entry.Key][0];
                if (!cdata.CopyBelowThis.ContainsKey(entry.Key))
                {
                    cdata.CopyBelowThis.Add(entry.Key, new Dictionary<string, object>());
                    cdata.CopyBelowThis[entry.Key].Add("item", $"{entry.Key}");
                    cdata.CopyBelowThis[entry.Key].Add("buy", $"{entry.Value}");
                    cdata.CopyBelowThis[entry.Key].Add("sell", $"{entry.Value - (entry.Value * .95)}");
                    cdata.CopyBelowThis[entry.Key].Add("img", $"{img}");
                }
            }
        ...
        void LoadData()
        {
            try
            {
                cdata = CData.ReadObject<CostData>();
            }
            catch
            {
                cdata = new CostData();
