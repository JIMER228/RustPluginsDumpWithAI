    ...
    class ZLevelsRemastered : RustPlugin
    {
		...
		void OnServerInitialized()
		{
			...
			foreach (var player in BasePlayer.activePlayerList)
			{
				...
				if (player != null && !(player is HTNPlayer) && !(player is NPCPlayer))
				{
					UpdatePlayer(player);
					if (cuiEnabled)
						CreateGUI(player);
	    ...
	    private void OnPlayerDie(BasePlayer player, HitInfo hitInfo)
        {
            if (!initialized || !penaltyOnDeath)
				return;
			if (player is NPCPlayer || player is HTNPlayer) return;
			...
			ConsoleSystem.Run(ConsoleSystem.Option.Server.Quiet(), "zl.lvl " + player.UserIDString + " * /2" );
			PrintToChat(player, "<color=\"#ff5a5a\">You lost 50% of your ZLevels because you died.</color>");
        ...
        void ZlvlCommand(ConsoleSystem.Arg arg)
        {
            ...
            if (p != null || playerName == "*" || playerName == "**")
            {
                    ...
                    skill == Skills.CRAFTING || skill == "*")
                {
                    ...
                    else if (arg.Args[2][0] == 'x')
                    {
                        mode = 5;
                        correct = int.TryParse(arg.Args[2].Replace("x", ""), out value);
                    }
                    else if (arg.Args[2][0] == '%')
                    {
                        mode = 6;
                    ...
                    if (correct)
                    {
                        if (mode == 5) // Change XP Multiplier.
                        {
                            if (!allSkills)
                            {
            ...
            if (skill == "*")
            {
				...
				foreach (var currSkill in Skills.ALL)
                {
					...
					if (mode == 0) // SET
						...
						modifiedLevel -= level;
					else if (mode == 3) // Multiplication
						modifiedLevel *= level;
					else if (mode == 4) // DIVIDE
						modifiedLevel /= level;
					else if (mode == 6) // Percentage
						modifiedLevel = (int)((double)modifiedLevel / 100 * level);
					if (modifiedLevel < 1)
						modifiedLevel = 1;
		...
		*/
		...
		{
			if (player is NPCPlayer || player is HTNPlayer || player.userID < 76561197960265728L)
				return false;
			return true;
		}
