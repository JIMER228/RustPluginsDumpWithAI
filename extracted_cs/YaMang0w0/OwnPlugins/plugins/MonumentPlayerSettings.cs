...
using UnityEngine;
using static Oxide.Plugins.MonumentPlayerSettings;
...
namespace Oxide.Plugins
{
    ...
    class MonumentPlayerSettings : RustPlugin
    {
        ...
        void OnServerInitialized()
        {
            ...
            {
                if (!_config.monumentSetting.ContainsKey(item.name.ToLower()))
                {
                    var m = new MonumentSetting
                    {
                        ...
                        blockInMPickup = false,
                        blockInMSpray = false
                    };
                    _config.monumentSetting.Add(item.name.ToLower(), m);
                }
            }

            bool spray = !_config.monumentSetting.Values.Any(setting => !setting.blockInMSpray);
            bool pickup = !_config.monumentSetting.Values.Any(setting => !setting.blockInMPickup);
            bool commands = !_config.monumentSetting.Values.Any(setting => setting.blockInMCommands.Count == 0);
            bool active = !_config.monumentSetting.Values.Any(setting => setting.blockInMActiveItems.Count == 0);
            if (spray) Unsubscribe(nameof(OnSprayCreate));
            if(pickup) Unsubscribe(nameof(OnPlayerCommand));
            if(commands) Unsubscribe(nameof(OnPlayerCommand));
            if(active) Unsubscribe(nameof(OnActiveItemChanged));
            Puts($"s:{spray} p:{pickup} c:{commands} a:{active}");
        ...
        private class ConfigData
        {
            ...
            [JsonProperty(PropertyName = "Monument Settings")] public Dictionary<string, MonumentSetting> monumentSetting { get; set; }
        ...
        private ConfigData GetBaseConfig()
        {
            return new ConfigData
            {
                ...
                monumentSetting = new Dictionary<string, MonumentSetting>
                {
                    {
                        "outpost", new MonumentSetting
                        {
                            blockInMSpray = true,
                            blockInMPickup = true,
                            blockInMActiveItems = new List<string>
