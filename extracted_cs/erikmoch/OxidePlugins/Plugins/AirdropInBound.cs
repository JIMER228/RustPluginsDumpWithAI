...
namespace Oxide.Plugins
{
    ...
    class AirdropInBound : RustLegacyPlugin
    {
        ...
        {
            string message = "[color green][Airdrop negada] [color yellow]";
            ...
            foreach(PlayerClient player in PlayerClient.All)
            {
                ...
                if (showDistance)
                {
                    distance = "aproximadamente. " + Math.Ceiling(Math.Abs(Vector3.Distance(targetposition, player.lastKnownPosition))).ToString() + "metros";
                }
                if (showDirection)
                {
                ...
                {
                    currentmessage = string.Format("{0} ({1} {2} da sua posicao)", currentmessage, distance, direction);
                }
                ConsoleNetworker.SendClientCommand(player.netPlayer, "chat.add Corree " + Facepunch.Utility.String.QuoteSafe(currentmessage));
            }
