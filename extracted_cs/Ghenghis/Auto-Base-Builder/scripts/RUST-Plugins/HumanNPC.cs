...
namespace Oxide.Plugins
{
    ...
    public class HumanNPC : RustPlugin
    {
        ...
        private bool IsHumanNpc(ulong userID) => humannpcs.ContainsKey(userID);

        private object CanHelicopterTarget(PatrolHelicopterAI entity, HumanPlayer player)
        {
            return false;
        }
...
//                    npc.Invoke("AllowMove",0);
            ...
            public void CreateProjectileEffect(BaseCombatEntity target, BaseProjectile baseProjectile, float dmg, bool miss = false)
            {
                ...
                {
                    hit = target.transform.position + new Vector3(0.0f, 0.55f, 0.0f);
                }
...
//                if (IsInvoking("AllowMove")) { CancelInvoke("AllowMove"); AllowMove(); }
        ...
        {
            if (humannpcs != null && humannpcs.Count > 0)
            {
...
//        }
        ...
        [AutoPatch]
        [HarmonyPatch(typeof(BasePlayer), "get_IsBot")]
        internal class PatchIsBot
        {
            [HarmonyPrefix]
            internal static bool Prefix(BasePlayer __instance, ref bool __result)
            {
                if ((object)__instance == null || __instance.userID == null || ins == null || ins.humannpcs == null || !ins.humannpcs.TryGetValue(__instance.userID, out var info))
                    return true;
