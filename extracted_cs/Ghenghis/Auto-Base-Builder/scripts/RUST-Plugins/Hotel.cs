...
//Hotel created with PluginMerge v(1.0.8.0) by MJSU @ https://github.com/dassjosh/Plugin.Merge
namespace Oxide.Plugins
{
    [Info("Hotel", "Shady14u", "2.0.28")]
    ...
    public partial class Hotel : RustPlugin
    {
        ...
        private void NewRoomOwner(CodeLock codeLock, BasePlayer player, HotelData hotel, Room room)
        {
            ...
            if (string.IsNullOrEmpty(room.lastRenter) || room.lastRenter != player.UserIDString)
            {
                ...
                if (!string.IsNullOrEmpty(room.lastRenter))
                {
                    ...
                    if (oldTenant != null)
                    {
                        SetPlayerAsRenter(oldTenant.UserIDString, false);
            ...
            SetPlayerAsRenter(player.UserIDString, true);
            LockLock(codeLock);
        ...
        {
            var position = (player.transform.position.XZ3D() * 60f);
...
#region 2.Hotel.Localization.cs
