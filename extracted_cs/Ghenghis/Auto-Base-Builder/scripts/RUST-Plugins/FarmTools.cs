...
{
    [Info("Farm Tools", "Clearshot", "1.2.3")]
    ...
    class FarmTools : CovalencePlugin
    {
        ...
        private void PlantAllCommand(IPlayer player, string command, string[] args)
        {
            ...
            ****************************************************************************/
            ...
            List<Construction.Target> targets = Facepunch.Pool.Get<List<Construction.Target>>();
            foreach (Socket_Base socket in PrefabAttribute.server.FindAll<Socket_Base>(planter.prefabID).Where(x => x.female))
            {
                ...
                target.buildingBlocked = true;

                Socket_Base maleSocket = construction.allSockets.Where(x => x.male).FirstOrDefault();
                var placement = maleSocket.DoPlacement(target);
                if (maleSocket != null && !maleSocket.CheckSocketMods(ref placement))
                    continue;
            ...
            Facepunch.Pool.FreeUnmanaged(ref targets);
            // end credits
        }
