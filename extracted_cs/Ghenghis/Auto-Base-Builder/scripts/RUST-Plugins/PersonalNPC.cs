...
{
    [Info("PersonalNPC", "walkinrey", "1.9.1")] 
    public class PersonalNPC : RustPlugin 
    {
        public static PersonalNPC Instance;
        private const bool ru = false;

        [PluginReference] private Plugin ImageLibrary, VehicleDeployedLocks, MarkerManager, PNPCAddonHeli, ZoneManager, Friends, Clans, DeployableNature;
        ...
        private object CanLootEntity(BasePlayer player, DroppedItemContainer droppedItemContainer)
        {
            ...
            if(player.InSafeZone())
            {
                ...
                if(player.userID.Get() == ownerComponent?.botOwnerSteamID)
                {
                    ...
                    player.inventory.loot.StartLootingEntity(droppedItemContainer, false);
                    player.inventory.loot.entitySource = droppedItemContainer;
                    
                    player.inventory.loot.AddContainer(droppedItemContainer.inventory);
            ...
            if(args[0] == "tugboat")
            {
                ...
                if(controller.botSetup.functions.canMount && controller.botSetup.functions.enableBoatAddon)
                {
                    if(!controller.bot.isMounted && controller.owner.GetParentEntity() is Tugboat tugboat)
                    {
                        if(tugboat != null)
                        {
                            if(tugboat.IsAuthed(controller.owner))
                            {
                                ...
                                controller.bot.SendNetworkUpdateImmediate(true);
                            }
        ...
        public class PlayerBotController : FacepunchBehaviour
        {
            ...
            private bool IsOwnerTugboat => owner.GetParentEntity() is Tugboat tugboat && tugboat.IsAuthed(owner);
            ...
            public void OnOwnerAttack(BaseEntity victim)
            {
                ...
                if(_isCombat && victim != _lastBanTarget)
                {
                    ...
                    if((victim is BaseCorpse) == false && victim is BaseCombatEntity baseCombatEntity && baseCombatEntity.IsAlive())
                    {
                        if(victim is BasePlayer basePlayer)
                        {
                            ...
                            {
                                if(botSetup.functions.combatSetup.CanAttackedNpc) goto SetTarget;
                            }
                            else if(botSetup.functions.combatSetup.CanAttackedUsers) goto SetTarget;
                            ...
                            if(ent is CollectibleEntity)
                            {
                                if(!botSetup.functions.canCollectDeployableNatureAutoPickup) 
                                {
                                    if(plugin.DeployableNature != null)
                                    {
                                        if (plugin.DeployableNature.IsLoaded)
                                        {
                                            if(plugin.DeployableNature.Call<bool>("IsDeployableNature", (ent as BaseEntity))) continue;
                                        }
                                    }
                                }
