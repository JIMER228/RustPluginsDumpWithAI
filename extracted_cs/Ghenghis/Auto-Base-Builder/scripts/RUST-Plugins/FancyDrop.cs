...
{
    [Info("FancyDrop", "FastBurst", "3.2.7")]
    ...
    class FancyDrop : RustPlugin
    {
        ...
        {
            referencePlane = GameManager.server.FindPrefab(PLANE_PREFAB).GetComponent<CargoPlane>();
            spawnFilter = GameManager.server.FindPrefab(PLANE_PREFAB).GetComponent<CargoPlane>().filter;
        ...
        class DropTiming : FacepunchBehaviour
        {
            ...
            private void Update()
            {
                ...
                if ((float)GetComponent<CargoPlane>().secondsTaken > updatedTime && dropCount < cratesToDrop)
                {
        ...
        */
		...
		// Produce a ctor (constructor) error after aug 2025
		// init()hook is also too early
		// initialise in OnserverInitialised
        CargoPlane referencePlane = null; // GameManager.server.FindPrefab(PLANE_PREFAB).GetComponent<CargoPlane>();
        SpawnFilter spawnFilter = null; // GameManager.server.FindPrefab(PLANE_PREFAB).GetComponent<CargoPlane>().filter;
