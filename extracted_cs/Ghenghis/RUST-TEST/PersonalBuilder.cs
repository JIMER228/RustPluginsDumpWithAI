...
{
    [Info("Personal Builder", "walkinrey & Max39ru", "1.1.1")] 
    public class PersonalBuilder : RustPlugin
    {
        ...
        public class Configuration 
        {
            ...
            [JsonProperty("Builders purchasable via Economics plugins")]
            public List<CostEconomicSetup> economicBots = new();
            ...
            public class CostEconomicSetup
            {
                [JsonProperty("Price")]
                public float price;
        ...
        protected override void LoadDefaultConfig() 
        {
            _config = new Configuration
            {
                ...
                installItem = new List<Configuration.ItemInfo>(),
                economicBots = new()
                {
                    new()
                    {
        ...
        protected override void LoadDefaultMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                ["ChatCommand_NoBalanceEconomics"] = "Balance is undefined",
                ["ChatCommand_Notice_InsufficientBalance"] = "Not enough money, you need {0}",
                ["ChatCommand_Withdraw"] = "From your account was withdrawn {0} for builder.",
                ["ChatCommand_WithdrawFail"] = "Failed to withdraw payment from your balance for builder.",
                ["ChatCommand_NotBuildToPermissions"] = "Not found your permissions to build here",
                ["ChatCommand_AccessBuildsToPermissions"] = "Available buildings:\n{0}",
            ...
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                ["ChatCommand_NoBalanceEconomics"] = "Не найден ваш баланс в экономике сервера",
                ["ChatCommand_Notice_InsufficientBalance"] = "Не достаточный баланс, не хватает {0}",
                ["ChatCommand_Withdraw"] = "С вашего счета списана сумма {0} за строительство.",
                ["ChatCommand_WithdrawFail"] = "Не удалось списать с вашего счета оплату за строительство.",
                ["ChatCommand_NotBuildToPermissions"] = "Не найдены разрешенные строительства.",
                ["ChatCommand_AccessBuildsToPermissions"] = "Разрешенные постройки:\n{0}",
            }, this, "ru");
        }
        ...
        public static bool TryGetBalance(string userId, out float balance)
        {
            balance = 0;
            if(string.IsNullOrEmpty(userId)) return false;
            var _balance = Interface.CallHook("Balance", userId);
            if(_balance is double num)
            {
                balance = (float)num;
                return true;
            }
        ...
        public static bool Withdraw(string userId, float amount)
        {
            return Interface.CallHook("Withdraw", userId, (double)amount) is bool result ? result : false;
        }
        public void SendMsg(BasePlayer player, string key, params object[] args) 
        {
        ...
        private void chatMsg(BasePlayer player, string command, string[] args)
        {
            ...
            if(args[0] == "buildings") 
            {
                BobCommand(player, command, args);
                return;
            }
            ...
            if(length > 0)
            {
                var build = _config.economicBots.Find(x => !string.IsNullOrEmpty(x.bot.spawnName) && x.bot.spawnName.ToLower() == args[0].ToLower());
                if(build != null)
                {
                    if(controller != null) 
                    {
                    ...
                    if(_cooldownInfo.ContainsKey(player.userID))
                    {
                        ...
                        if(DateTime.Now > lastTimeSpawn.AddSeconds(build.bot.cooldown))
                        {
                        ...
                        {
                            SendMsg(player, "ChatCommand_Notice_Cooldown", Mathf.RoundToInt((float)(lastTimeSpawn.AddSeconds(build.bot.cooldown) - DateTime.Now).TotalSeconds).ToString());
                            return;
                        }
                    }
                    else
                    {
                        if(TryGetBalance(player.UserIDString, out var balance))
                        {
                            if(balance >= build.price)
                            {
                                _cooldownInfo.Add(player.userID, DateTime.Now);
                                controller = PlayerBuilderController.CreateBot<EconomicBuild>(player, build.bot, null, buildCostInfo, this);
                                ((EconomicBuild)controller).Price = build.price;
                            ...
                            {
                                SendMsg(player, "ChatCommand_Notice_InsufficientBalance", (build.price - balance).ToString("0"));
                                return;
                            }
                        }
                        else
                        {
                            SendMsg(player, "ChatCommand_NoBalanceEconomics");
                            return;
                        }
                    }
                }
                ...
                if(bot != null)
                {
                    ...
                    if(permission.UserHasPermission(player.UserIDString, perm))
                    {
                        ...
                        if(_cooldownInfo.ContainsKey(player.userID))
                        {
                            ...
                            {
                                SendMsg(player, "ChatCommand_Notice_Cooldown", Mathf.RoundToInt((float)(lastTimeSpawn.AddSeconds(bot.cooldown) - DateTime.Now).TotalSeconds).ToString());
                                return;
                            }
                        }
                        else _cooldownInfo.Add(player.userID, DateTime.Now);
                        
                        controller = PlayerBuilderController.CreateBot<PlayerBuilderController>(player, bot, null, buildCostInfo, this);
        ...
        {
            List<string> availableBases = Pool.Get<List<string>>();
            foreach (var baseEntry in _config.permissionBot)
            {
                if (permission.UserHasPermission(player.UserIDString, baseEntry.Key))
                {
                    foreach(var build in baseEntry.Value)
                    {
                        availableBases.Add($"- <color=#3399FF>{build.filename}</color>(/pbuilder {build.spawnName})");
                    }
            ...
            {
                SendMsg(player, "ChatCommand_NotBuildToPermissions");
            }
            else
            {
                SendMsg(player, "ChatCommand_AccessBuildsToPermissions", string.Join("\n", availableBases));
            }

            Pool.FreeUnmanaged(ref availableBases);
        }
        #endregion
        ...
        private void OnEntityBuilt(Planner plan, GameObject go)
        {
            ...
            foreach(var loopInfo in _config.installItem)
            {
                if(loopInfo.skin == item.skin)
                {
                    ...
                    controller = PlayerBuilderController.CreateBot<PlayerBuilderController>(player, bot, loopInfo, buildCostInfo, this, go.transform.position);
        ...
        public class EconomicBuild : PlayerBuilderController
        {
            public float Price;

            protected override bool OnBuildStart()
            {
                if(TryGetBalance(Owner?.UserIDString, out var balance))
                {
                    if(balance >= Price)
                    {
                        if(Withdraw(Owner?.UserIDString, Price))
                        {
                            BuilderInstance.SendMsg(Owner, "ChatCommand_Withdraw", Price);
                            return true;
                        }
                        else
                        {
                            BuilderInstance.SendMsg(Owner, "ChatCommand_WithdrawFail");
                            return false;
                        }
                    }
                    else
                    {
                        BuilderInstance.SendMsg(Owner, "ChatCommand_Notice_InsufficientBalance", (balance - Price).ToString("0"));
                        return false;
                    }
                }
                else
                {
                    BuilderInstance.SendMsg(Owner, "ChatCommand_NoBalanceEconomics");
                    return false;
                }
            }
        }
        public class PlayerBuilderController : FacepunchBehaviour
        {
            ...
            public BasePlayer Owner => _owner;
            ...
            private void FixedUpdate()
            {
                if(isInitBuilders && Physics.Raycast(_owner.eyes.HeadRay(), out var hitInfo, 2f, LayerMask.GetMask("AI")) && hitInfo.GetEntity() == _botPlayer)
                {
                    if(!guiOpen)
                    {
                        List<CuiElement> list = Facepunch.Pool.Get<List<CuiElement>>();
            ...
            private void OnDestroy() 
            {
                ...
                CuiHelper.DestroyUi(_owner, "open_loot_builder");
            }
            ...
            protected virtual bool OnBuildStart() => true;
            public void EndLoot()
            {
                bot_paused = false;
            ...
            public static T CreateBot<T>(BasePlayer caller, Configuration.BotSetup botSetup, Configuration.ItemInfo? itemInfo, BuildCostInfo info, PersonalBuilder pluginInstance, Vector3? pos = null) where T : PlayerBuilderController
            {
