...
namespace Oxide.Plugins
{
    ...
    {
        [PluginReference]
        private Plugin Clans, Economics, ServerRewards, Friends, CompoundTeleport, ZoneManager, NoEscape, PopupNotifications, Ignore;
        ...
        {
            if (config.Home.UseBlockedUsers && Ignore != null && Ignore.IsLoaded)
            {
                Puts("Checking Blocked Users...");
#if DEBUG
        Puts("Checking Blocked Users...");
#endif
                var fr = Ignore?.CallHook("AreBlockedUser", playerid, ownerid);
                if (fr != null && fr is bool && (bool)fr)
                {
#if DEBUG
            Puts("  IsBlockedUser: true based on Ignore plugin");
#endif
                    return true;
                }
            }
