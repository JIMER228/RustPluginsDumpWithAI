...
{
    [Info("Traffic Drivers", "ModHub.to", "1.0.8")]
    public class TrafficDrivers : RustPlugin
    {
        ...
        private TrafficDriver SpawnTrafficCarParams(Vector3[] path, Vector3 position, TrafficDriver.DriveSide side)
        {
            ...
            foreach(var module in car.AttachedModuleEntities)
            {
                ...
                {
                    if(module is VehicleModuleEngine) ((VehicleModuleEngine)module).GetContainer().inventory.SetLocked(true);
                }
        ...
        private void AddFlashingLights(VehicleModuleSeating module)
        {
            ...
            if(light1.TryGetComponent<DecayEntity>(out decayEntity)) UnityEngine.Object.Destroy(decayEntity);
            if(light2.TryGetComponent<DecayEntity>(out decayEntity)) UnityEngine.Object.Destroy(decayEntity);
