...
namespace Oxide.Plugins
{
    ...
    class Oysters : RustPlugin
    {
        ...
        [AutoPatch]
        [HarmonyPatch(typeof(SurvivalFishTrap), nameof(SurvivalFishTrap.TrapThink))]
        public static class TrapWildlife
        {
            [HarmonyPostfix]
            static void Postfix(SurvivalFishTrap __instance)
            {
                if (ins == null || __instance == null) return;
                ins.TryGetOyster(__instance);
            }
        }

        private void TryGetOyster(WildlifeTrap trap)
        {
            Item item = null;
            ...
            if (item != null)
            {
                if (!MoveToContainer(item, trap.inventory, -1, true))
                    ...
                    trap.SetFlag(BaseEntity.Flags.Reserved1, true, false, true);
            }
        }
