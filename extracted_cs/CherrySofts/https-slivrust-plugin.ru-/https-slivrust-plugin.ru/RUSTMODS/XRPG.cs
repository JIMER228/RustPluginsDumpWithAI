...
{
     /* Плагин был разыгран дискордом https://discord.gg/8CWJadmA5w */ [Info("XRPG", "https://discord.gg/8CWJadmA5w", "1.1.3")]
    class XRPG : RustPlugin
    {
        ...
        private class RPGConfig
        {
            ...
            internal class SSetting
            {
				...
				[JsonProperty("Удалять автоматически дату после вайпа")] public bool ClearData;
                [JsonProperty("Стартовый умножитель прокачки рейтов")] public float Boost;
                [JsonProperty("Размер текста")] public int TextSize;
                [JsonProperty("Размер текста уведомлений")] public int TextSizeNotify;
		...
		private void AnimalUI(BasePlayer player)
		{
			...
			string pregressanimal = rateanimal >= maxrateanimal ? "0.98 0.9" : config.Panel.Progress ?  $"{0.13 + ((rateanimal - config.Animal.RateStart) * (0.84 / (maxrateanimal - config.Animal.RateStart)))} 0.9" : $"{0.13 + (rateanimal * (0.84 / maxrateanimal))} 0.9";
            ...
            {
                RectTransform = { AnchorMin = "0 0", AnchorMax = "1 0.31", OffsetMax = "0 0" },
                Image = { Color = "0.96 0.91 0.87 0.02", Material = "assets/icons/greyout.mat" }
            }, ".RPG", ".Animal");

            container.Add(new CuiButton
            {
                RectTransform = { AnchorMin = "0.01 0.15", AnchorMax = "0.11 0.84", OffsetMax = "0 0" },
            ...
            container.Add(new CuiPanel
            {
                RectTransform = { AnchorMin = "0.13 0.1", AnchorMax = pregressanimal, OffsetMax = "0 0" },
                Image = { Color = "0.78 0.44 0.21 0.92", Material = "assets/icons/greyout.mat" }
            }, ".Animal");

            container.Add(new CuiLabel
            {
                RectTransform = { AnchorMin = "0.17 0", AnchorMax = "1 1", OffsetMax = "0 0" },
                Text = { Text = textanimal, Align = TextAnchor.MiddleLeft, FontSize = config.Setting.TextSize, Color = "1 1 1 0.60" }
            }, ".Animal");
        ...
        private void OnCollectiblePickup(CollectibleEntity collectible, BasePlayer player)
        {
			...
			foreach(ItemAmount item in collectible.itemList)
			{
				...
				if(config.Wood.Item.ContainsKey(shortname))
				{
					item.amount = (int)Math.Round(item.amount * StoredData[player.userID].Wood);
					
					if(config.Wood.Pickup)
						RateWood(player, shortname);
				}
				
				if(config.Ore.Item.ContainsKey(shortname))
				{
					item.amount = (int)Math.Round(item.amount * StoredData[player.userID].Ore);
					
					if(config.Ore.Pickup)
						RateOre(player, shortname);
				}
				
				if(config.Animal.Item.ContainsKey(shortname))
				{
					item.amount = (int)Math.Round(item.amount * StoredData[player.userID].Animal);
					
					if(config.Animal.Pickup)
						RateAnimal(player, shortname);
				}
			}
        }

        private void UPMessageAnimal(BasePlayer player, string Messages)
        {
            ...
            {
                FadeOut = 0.5f,
                RectTransform = { AnchorMin = "0.64 0", AnchorMax = "0.97 1", OffsetMax = "0 0" },
                Text = { FadeIn = 0.5f, Text = $"{Messages}", Align = TextAnchor.MiddleRight, Font = "robotocondensed-regular.ttf", FontSize = config.Setting.TextSizeNotify, Color = "1 1 1 0.60" }
		...
		private void CheckPermission(string id, string permName)
		{
			...
			if(player != null)
				if(config.Permisssion.ContainsKey(permName))
					OnPlayerConnected(player);
		}
        ...
        private void OnEntityDeath(BaseCombatEntity entity, HitInfo info)
        {
            ...
            if(config.Animal.Kill)
            {
                ...
                if(!(data.Animal >= data.AnimalRate))
                    if(config.Animal.Animal.ContainsKey(entity.ShortPrefabName))
                    {
						...
						if(config.Setting.Slow)
                            boost = (itema - (itema / data.AnimalRate * (int)data.Animal)) * data.Boost;
						else
							boost = itema * data.Boost;
		...
		{ 
			...
			container.Add(new CuiButton
            {
                RectTransform = { AnchorMin = "-0.14 0", AnchorMax = "-0.01 0.31", OffsetMax = "0 0" },
                Button = { Color = "0.96 0.91 0.87 0.02", Material = "assets/icons/greyout.mat", Command = "ui hide" },
		...
		private void RateAnimal(BasePlayer player, string shortname)
		{
			var data = StoredData[player.userID];
			
			if(!(data.Animal >= data.AnimalRate))
            {
				var itema = config.Animal.Item[shortname];
				float arate = 0;
							
				if(config.Setting.Slow)
                    arate = (itema - (itema / data.AnimalRate * (int)data.Animal)) * data.Boost;
				else
					arate = itema * data.Boost;
		...
		private void ShowUI(BasePlayer player)
		{
			...
			container.Add(new CuiButton
            {
                ...
                Button = { Color = "0.96 0.91 0.87 0.02", Material = "assets/icons/greyout.mat", Command = "ui show" },
        ...
        private void MaxRate(BasePlayer player)
        {
            ...
            if(x == 0)
            {
                if(data.Wood >= config.Wood.RateMax)
                {
                    data.Wood = config.Wood.RateMax;
                    data.WoodRate = config.Wood.RateMax;
                }
                else
                    data.WoodRate = config.Wood.RateMax;

                if(data.Ore >= config.Ore.RateMax)
                {
                    data.Ore = config.Ore.RateMax;
                    data.OreRate = config.Ore.RateMax;
                }
                else
                    data.OreRate = config.Ore.RateMax;

                if(data.Animal >= config.Animal.RateMax)
                {
                    data.Animal = config.Animal.RateMax;
                    data.AnimalRate = config.Animal.RateMax;
                }
                else
                    data.AnimalRate = config.Animal.RateMax;

                data.Boost = config.Setting.Boost;
            }
            else
            {
                data.WoodRate = wrate;
                data.OreRate = orate;
                data.AnimalRate = arate;
		...
		private void OnUserGroupAdded(string id) => CheckGroup(id);
		...
		private void RateOre(BasePlayer player, string shortname)
		{
			var data = StoredData[player.userID];
			
			if(!(data.Ore >= data.OreRate))
            {
				var itemo = config.Ore.Item[shortname];
				float orate = 0;
							
				if(config.Setting.Slow)
                    orate = (itemo - (itemo / data.OreRate * (int)data.Ore)) * data.Boost;
				else
					orate = itemo * data.Boost;
        ...
        private void OnPlayerConnected(BasePlayer player)
        {
            ...
            if(!StoredData.ContainsKey(player.userID))
            {
                ...
                data.Wood = config.Wood.RateStart;
                data.Ore = config.Ore.RateStart;
                data.Animal = config.Animal.RateStart;
        ...
        private void UPMessageWood(BasePlayer player, string Messages)
        {
        ...
        private void UPMessageOre(BasePlayer player, string Messages)
        {
		...
		private void SaveData() => Interface.Oxide.DataFileSystem.WriteObject("XRPG", StoredData);
		protected override void LoadDefaultConfig() => config = RPGConfig.GetNewConfiguration();
		private void OnUserGroupRemoved(string id) => CheckGroup(id);
        ...
        {
            [JsonProperty("Лесоруб")] public float Wood;
            [JsonProperty("Рудокоп")] public float Ore;
            [JsonProperty("Охотник")] public float Animal;
            [JsonProperty("Максимальные рейты лесоруба")] public float WoodRate;
            [JsonProperty("Максимальные рейты рудокопа")] public float OreRate;
            [JsonProperty("Максимальные рейты охотника")] public float AnimalRate;
            [JsonProperty("Множитель прокачки рейтов")] public float Boost;            
		...
		private void WoodUI(BasePlayer player)
		{
            ...
            string pregresswood = ratewood >= maxratewood ? "0.98 0.9" : config.Panel.Progress ?  $"{0.13 + ((ratewood - config.Wood.RateStart) * (0.84 / (maxratewood - config.Wood.RateStart)))} 0.9" : $"{0.13 + (ratewood * (0.84 / maxratewood))} 0.9";
            ...
            container.Add(new CuiPanel
            {
                RectTransform = { AnchorMin = "0 0.68", AnchorMax = "1 1", OffsetMax = "0 0" },
                Image = { Color = "0.96 0.91 0.87 0.02", Material = "assets/icons/greyout.mat" }
            }, ".RPG", ".Wood");
            ...
            container.Add(new CuiPanel
            {
                RectTransform = { AnchorMin = "0.13 0.1", AnchorMax = pregresswood, OffsetMax = "0 0" },
                Image = { Color = "0.55 0.73 0.24 0.92", Material = "assets/icons/greyout.mat" }
            }, ".Wood");    	
		   		 		  						  	   		  	 	 		  						  	   		  	   
            container.Add(new CuiLabel
            {
                RectTransform = { AnchorMin = "0.17 0", AnchorMax = "1 1", OffsetMax = "0 0" },
                Text = { Text = textwood, Align = TextAnchor.MiddleLeft, FontSize = config.Setting.TextSize, Color = "1 1 1 0.60" }
            }, ".Wood");
			
			CuiHelper.AddUi(player, container);
		}		
		
		private void RateWood(BasePlayer player, string shortname)
		{
			var data = StoredData[player.userID];
			
			if(!(data.Wood >= data.WoodRate))
            {
				var itemw = config.Wood.Item[shortname];
				float wrate = 0;
						
				if(config.Setting.Slow)
                    wrate = (itemw - (itemw / data.WoodRate * (int)data.Wood)) * data.Boost;
				else
					wrate = itemw * data.Boost;
		...
		private void OreUI(BasePlayer player)
		{
			...
			string pregressore = rateore >= maxrateore ? "0.98 0.9" : config.Panel.Progress ?  $"{0.13 + ((rateore - config.Ore.RateStart) * (0.84 / (maxrateore - config.Ore.RateStart)))} 0.9" : $"{0.13 + (rateore * (0.84 / maxrateore))} 0.9";
            ...
            container.Add(new CuiPanel
            {
                RectTransform = { AnchorMin = "0 0.34", AnchorMax = "1 0.66", OffsetMax = "0 0" },
                Image = { Color = "0.96 0.91 0.87 0.02", Material = "assets/icons/greyout.mat" }
            }, ".RPG", ".Ore");
            ...
            container.Add(new CuiPanel
            {
                RectTransform = { AnchorMin = "0.13 0.1", AnchorMax = pregressore, OffsetMax = "0 0" },
                Image = { Color = "0.29 0.60 0.83 0.92", Material = "assets/icons/greyout.mat" }
            }, ".Ore");

            container.Add(new CuiLabel
            {
                RectTransform = { AnchorMin = "0.17 0", AnchorMax = "1 1", OffsetMax = "0 0" },
                Text = { Text = textore, Align = TextAnchor.MiddleLeft, FontSize = config.Setting.TextSize, Color = "1 1 1 0.60" }
            }, ".Ore");
        ...
        private void OnDispenserBonus(ResourceDispenser dispenser, BasePlayer player, Item item)
        {
			...
			if(config.Setting.BlockItem.Contains(player.GetActiveItem().info.shortname))
			{
				if(config.Wood.Item.ContainsKey(shortname))
					item.amount = (int)Math.Round(item.amount * StoredData[player.userID].Wood);
				if(config.Ore.Item.ContainsKey(shortname))
