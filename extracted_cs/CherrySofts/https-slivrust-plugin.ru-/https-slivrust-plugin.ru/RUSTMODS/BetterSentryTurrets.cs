...
{
    [Info("BetterSentryTurrets", "Razor", "1.8.0")]
    [Description("Player Placed Sentry Turrets")]
    public class BetterSentryTurrets : RustPlugin
    {
        ...
        private const string noPower = "BetterSentryTurrets.nopower";
        ...
        private void OnServerInitialized()
        {
            ...
            permission.RegisterPermission(noPower, this);
        ...
        void SaveData(string filename = "AQt3ZQH=")
        {
        ...
        class ConfigData
        {
            ...
            public class Settings
            {
                ...
                [JsonProperty(PropertyName = "Call OnEntityBuil hook")]
                public bool callOnEntityBuilt { get; set; }
        ...
        private void OnEntityBuilt(Planner plan, GameObject go)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (configData.settings.callOnEntityBuilt)
        ...
        object OnItemAction(Item item, string action, BasePlayer player)
        {
            ...
            {
                if (action == "unload_ammo" && configData.settings.amoRequired)
                    return null;
        ...
        {
            if (OwnerPlayer != null && configData.settings.callOnEntityBuilt)
            {
        ...
        public class TurretMod : MonoBehaviour
        {
            ...
            private void Awake()
            {
                ...
                needsPower = !_.configData.settings.powerRequirementNeeded && !_.permission.UserHasPermission(turret.OwnerID.ToString(), noPower);
            ...
            {
                if (!turret.authDirty && !hasPotentialUnauthedTarget)
                    return;
                    ...
                    if (turret.targetTrigger.entityContents != null)
                    {
                        foreach (BaseEntity entityContent in turret.targetTrigger.entityContents)
                        {
                            ...
                            if (!((UnityEngine.Object)baseCombatEntity == (UnityEngine.Object)null))
                            {
                                ...
                                if ((UnityEngine.Object)player != (UnityEngine.Object)null && (turret.IsAuthed(player)))
                                    continue;
            ...
            public bool TurretCanSeePlayer(BasePlayer player)
            {
                if (player == null)
                    return true;

                return turret.ObjectVisible(player);
            }


            public virtual bool Ignore(BasePlayer player) => !TurretCanSeePlayer(player);
            ...
            public void TargetTick()
            {
                ...
                if ((double)UnityEngine.Time.time >= (double)turret.nextShotTime && turret.targetVisible)
                {
            ...
            public Matrix4x4 GetMuzzleMatrixLookingAt(Vector3 fromPosition, Vector3 targetPosition, Vector3 up)
            {
                Vector3 forward = (targetPosition - fromPosition).normalized;
                ...
                matrix.SetColumn(1, new Vector4(correctedUp.x, correctedUp.y, correctedUp.z, 0f));
                matrix.SetColumn(2, new Vector4(forward.x, forward.y, forward.z, 0f));
                matrix.SetColumn(3, new Vector4(fromPosition.x, fromPosition.y, fromPosition.z, 1f));
            ...
            {
                return turret.isServer ? turret.cachedTransf.localToWorldMatrix * turret.gunAimInitialLS * Matrix4x4.Rotate(turret.gunAimRotLS) : turret.gun_pitch.localToWorldMatrix;
            }
            ...
            public void FireAttachedGun(Vector3 targetPos, float aimCone, Transform muzzleToUse = null, BaseCombatEntity target = null)
            {
                ...
                {
                    Vector3 muzzlePosition = turret.gun_pitch.position;
                    Vector3 targetPosition = targetPos;
                    Vector3 worldUp = Vector3.up;
                    Matrix4x4 aimMatrix = GetMuzzleMatrixLookingAt(muzzlePosition, targetPosition, worldUp);

                    Matrix4x4 pitch = aimMatrix;
                    attachedWeapon.ServerUse(1f, 1f, new Matrix4x4?(pitch), false);
                }
                else
                {
                    Matrix4x4 pitch = GetCenterMuzzle();
                    attachedWeapon.ServerUse(1f, 1f, new Matrix4x4?(pitch), false);
                }
            ...
            public void onSwitchToggle()
            {
                if (needsPower)
                {
                    if (_.permission.UserHasPermission(turret.OwnerID.ToString(), noPower))
                    {
                        needsPower = false;
                        CancelInvoke(nameof(HavePowerStill));
                        button?.UpdateFromInput((int)_.configData.settings.powerRequirement + 1, 0);
                    }
                }
            ...
            public void dropLoot(string weaponname = "AQt3ZQH=")
            {
                if (gunItem != null)
                {
                ...
                int totalItems = containerDrop.inventory.itemList.Count;
                ...
                if (containerDrop != null && (containerDrop.inventory.itemList.Count <= 0 || totalItems <= 0))
                    containerDrop.Kill();
            }
            ...
            private void unloadAmmo(DroppedItemContainer containerDrop = null)
            {
                ...
                if (!obj.MoveToContainer(inventory, -1, ignoreStackLimit: true))
                {
                    Vector3 velocity = Vector3.zero;
            ...
            public void OnDestroy()
            {
                ...
                if (_ != null && button != null && !needsPower)
                ...
                if (turret.inventory != null && !_.configData.settings.gunRequired && !_.configData.settings.amoRequired)
                    ...
                    if (inputState.IsDown(BUTTON.FIRE_PRIMARY))
                    {
                        ...
                        if ((bool)(UnityEngine.Object)attachedWeapon)
                        {
                            if (attachedWeapon.primaryMagazine.contents > 0)
                            {
                                ...
                                {
                                    Vector3 origin = turret.gun_pitch.transform.position - turret.gun_pitch.forward * 0.25f;
                                    Vector3 vector3 = turret.gun_pitch.transform.forward;
        ...
        #region patch Looting
        [AutoPatch]
        [HarmonyPatch(typeof(AutoTurret), nameof(AutoTurret.UpdateTotalAmmo))]
        internal class PatchUpdateTotalAmmo
        {
            [HarmonyPrefix]
            internal static bool Prefix(AutoTurret __instance)
            {
                if ((object)__instance == null || __instance is not NPCAutoTurret)
                    return true;

                __instance.totalAmmo = __instance.GetTotalAmmo();                
                return false;
            }
        }
        #endregion
