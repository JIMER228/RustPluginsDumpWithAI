...
{
    [Info("ATMSystem", "RustMods.ru", "2.0.4")]
    public class ATMSystem : RustPlugin
    {
            ...
            if (initial)
            {
                timer.Once(
                    ...
                    () =>
                    {
                        ...
                        foreach (
                            VendingMachine machine in GameObject.FindObjectsOfType(
        ...
        object OnCardSwipe(CardReader cardReader, Keycard card, BasePlayer player)
        {
            ...
            {
                if (
                    cardReader.GetParentEntity() != null
                    && cardReader.GetParentEntity().skinID == config.atm.skinID
                )
                {
                    OpenInterface(player);
                    return false;
        ...
        bool TakeFromInventory(BasePlayer player, string shortname, int amount, ulong skinid = 0)
        {
            ...
            if (item == null)
            {
                SendReply(
                    player,
                    "Plugin configuration error, please let server admin know about it."
                );
        ...
        private void testg(ConsoleSystem.Arg arg)
        {
            ...
            stopwatch.Start();
            Puts(
                $"[{stopwatch.ElapsedMilliseconds}ms] Player has {GetAmount(player, "paper", 2570661100)} of paper"
            );
            RemoveItem(player, 10, "paper", 2570661100);
            Puts($"[{stopwatch.ElapsedMilliseconds}ms] Removing 10 paper from player's inventory");

            Puts(
                $"[{stopwatch.ElapsedMilliseconds}ms] Amount left now:{GetAmount(player, "paper", 2570661100)} paper"
            );
        ...
        int GetAmount(BasePlayer player, string shortname, ulong? skin)
        {
            ...
            items.AddRange(
                player.inventory.containerBelt.itemList.Where(item =>
                    item.skin == skin && item.info.shortname == shortname
                )
            );
            items.AddRange(
                player.inventory.containerMain.itemList.Where(item =>
                    item.skin == skin && item.info.shortname == shortname
                )
            );
        ...
        bool RemoveItem(BasePlayer player, int amount, string shortname, ulong? skin)
        {
            ...
            catch
            {
                SendReply(
                    player,
                    "Something failed while taking items, please let server admin know about it."
                );
                Puts(
                    $"Something failed while taking items from {player} -> (shortname:{shortname}/amount:{amount}/skin:{skin})"
                );
                return false;
            }
        ...
        void UpdateBalancesUI(BasePlayer player, bool forceZero = false)
        {
            ...
            if (forceZero)
                ...
                Create.Text(
                    ...
                    "atm_acc_cash",
                    "1 1 1 0.75",
                    $"$ {GetAmount(player, config.currency.shortname, config.currency.skinID)}",
                    13,
                    "0.05 0",
                    "1 1",
                    TextAnchor.MiddleLeft,
            ...
            Create.Text(
                ref c,
                "atm_acc_balance_content",
                "atm_acc_balance",
                "1 1 1 0.75",
                $"$ {Economics.Call<double>("Balance", player.UserIDString)}",
                13,
                "0.05 0",
                "1 1",
                TextAnchor.MiddleLeft,
        ...
        private void consolecmd_atmbtn(ConsoleSystem.Arg arg)
        {
            ...
            {
                bool forceZero =
                    GetAmount(player, config.currency.shortname, config.currency.skinID) == amount;
                if (
                    TakeFromInventory(
                        player,
                        config.currency.shortname,
                        amount,
                        config.currency.skinID
                    )
                )
                {
                    ...
                    Notification(
                        player,
                        true,
                        gl("ui_deposit_success").Replace("{value}", amount.ToString())
                    );
            ...
            if (args[0] == "withdraw")
            {
                ...
                if (item != null)
                {
                    ...
                    Notification(
                        player,
                        true,
                        gl("ui_withdraw_success").Replace("{value}", amount.ToString())
                    );
                    PlayFx(
                        player,
                ...
                else
                {
                    ...
                    PlayFx(player, "assets/prefabs/locks/keypad/effects/lock.code.denied.prefab");
                    Puts(
                        $"Something went wrong while giving {player} money -> (shortname:{config.currency.shortname}/amount:{amount}/skin:{config.currency.skinID})"
                    );
                }
            }
        }
        ...
        public class Create
        {
            ...
            public static void Button(
                ...
                string command = "",
                string _close = "",
                string textColor = "0.843 0.816 0.78 1",
                float fade = 1f,
                TextAnchor align = TextAnchor.MiddleCenter,
            ...
            public static void Input(
                ...
                string anchorMax,
                string defaultText,
                string font = "permanentmarker.ttf",
                string command = "command.processinput",
                TextAnchor align = TextAnchor.MiddleCenter,
                int charsLimit = 200
            )
            {
        ...
        protected override void LoadDefaultMessages()
        {
            lang.RegisterMessages(
                new Dictionary<string, string>
                {
                    ...
                    ["ui_info"] =
                        "This field is used to explain how economy works with ATMs on your server, where money can be found etc...",
            ...
            lang.RegisterMessages(
                new Dictionary<string, string>
                {
                    ["need_card"] = "Для доступа к банкомату нужна карта.",
                    ["no_perms"] = "У вас нет прав для использования банкомата.",
                    ["ui_title"] = "<size=70><color=#ce422b>RUST</color> БАНКОМАТ</size>",
                    ["ui_info_title"] = "ИНФОРМАЦИЯ",
                    ["ui_info"] =
                        "Это поле используется для объяснения как работает экономика с банкоматами на вашем сервере, где можно найти деньги и т.д...",
                    ["ui_account_title"] = "АККАУНТ",
                    ["ui_account_holder"] = "ВЛАДЕЛЕЦ СЧЕТА",
                    ["ui_balance"] = "БАЛАНС",
                    ["ui_cash"] = "НАЛИЧНЫЕ",
                    ["ui_controls_title"] = "УПРАВЛЕНИЕ",
                    ["ui_enter_amount"] = "ВВЕДИТЕ СУММУ",
                    ["ui_btn_withdraw"] = "<color=#bad56aff>СНЯТЬ</color>",
                    ["ui_btn_deposit"] = "<color=#66b4d3ff>ВНЕСТИ</color>",
                    ["ui_withdraw_success"] = "Вы сняли {value}$ со своего счета.",
                    ["ui_withdraw_fail"] = "На вашем счету недостаточно средств.",
                    ["ui_deposit_success"] = "Вы внесли {value}$ на свой счет.",
                    ["ui_deposit_fail_inv"] = "У вас недостаточно денег в инвентаре.",
                    ["ui_deposit_fail_inv_fail"] = "Что-то пошло не так при выдаче денег.",
                    ["ui_input_only_digits"] = "В поле ввода допустимы только цифры.",
                },
                this,
                "ru"
            );
