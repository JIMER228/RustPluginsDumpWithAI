 ...
 *
 * V 2.0.12
 * - fix a bug that prevents players from putting items in the repair bench if there is a crafting recipe with skin id 0
 * - rework gene config to support every type of gene
 * 
 **/

namespace Oxide.Plugins
{
    [Info(nameof(Ganja), "rustmods.ru", "2.0.12")]
    public class Ganja : BasePlugin<Ganja, Ganja.Configuration>
    {
        ...
        // Migrate old gene config to new gene config
        ...
        protected override void LoadConfig()
        {
            ...
            foreach (var config in Config.weedConfig)
            {
                MigrateConfig(config);
            }

            #if COCA
            foreach (var config in Config.berryConfig)
            {
                MigrateConfig(config);
            }
            #endif
            ...
            {
                SaveConfig();
                PrintWarning("Gene config migrated successfully");
            }

            void MigrateConfig(WeedConfig config)
            {
                if (config.geneConfig == null)
                {
                    config.geneConfig = new Dictionary<string, GeneConfig>
                    {
                        ["H"] = new(config.minHGenesChance, config.minHGenesGuaranteed),
                        ["G"] = new(config.minGGenesChance, config.minGGenesGuaranteed)
                    };
                    config.minGGenesChance = null;
                    config.minGGenesGuaranteed = null;
                    config.minHGenesChance = null;
                    config.minHGenesGuaranteed = null;
        ...
        public class Configuration
        {
            ...
            public List<WeedConfig> weedConfig = new()
            {
                new WeedConfig
                {
                    ...
                    geneConfig = new Dictionary<string, GeneConfig>()
                    {
                        ["h"] = new(1, 3)
                    }
                },
            ...
            {
                jointSkins ??= new HashSet<ulong>(recipes.Where(x => x.isJoint && x.producedItem.skinId != 0).Select(x => x.producedItem.skinId));
        ...
        public class WeedConfig : CustomItem
        {
            ...
            [Obsolete, JsonProperty("Minimum amount of H genes for a chance to yield weed", DefaultValueHandling = DefaultValueHandling.Ignore, NullValueHandling = NullValueHandling.Ignore)]
            public int? minHGenesChance;
            [Obsolete, JsonProperty("Minimum amount of H genes for guaranteed yield", DefaultValueHandling = DefaultValueHandling.Ignore, NullValueHandling = NullValueHandling.Ignore)]
            public int? minHGenesGuaranteed;
            [Obsolete, JsonProperty("Minimum amount of G genes for a chance to yield weed", DefaultValueHandling = DefaultValueHandling.Ignore, NullValueHandling = NullValueHandling.Ignore)]
            public int? minGGenesChance;
            [Obsolete, JsonProperty("Minimum amount of G genes for guaranteed yield", DefaultValueHandling = DefaultValueHandling.Ignore, NullValueHandling = NullValueHandling.Ignore)]
            public int? minGGenesGuaranteed;
            // end remove

            [JsonProperty("Gene configuration", ObjectCreationHandling = ObjectCreationHandling.Replace)]
            public Dictionary<string, GeneConfig> geneConfig;
            ...
            private bool IsValidGatherLocation(ushort locationMask)
            {
                return (biomeMask & locationMask) > 0;
            }

            public bool MaybeGather(ushort locationMask, bool isCollectable, [CanBeNull] Func<GrowableGenetics.GeneType, int> getGeneCount, out Item weed)
            {
                weed = null;
                ...
                if (!isCollectable && getGeneCount != null)
                {
                    foreach (var geneType in geneConfig)
                    {
                        var count = getGeneCount(MapGeneType(geneType.Key));
                        if (count < geneType.Value.ChanceAmount)
                        {
                            // Not enough of 1 gene for a yield
                            return false;
                        }
                        if (count >= geneType.Value.GuaranteedAmount)
                        {
                            // Enough of 1 gene for 100% drop chance
                            chance = 1f;
                            break;
                        }
                    }
                ...
                if (amount > 0)
                {
                    ...
                    if (weed == null)
                    {
                        LogError("Failed to create weed item, check if your config is correct");
                    }
                }
            ...
            private static GrowableGenetics.GeneType MapGeneType(string type)
            {
                return type switch
                {
                    "G" or "g" => GrowableGenetics.GeneType.GrowthSpeed,
                    "H" or "h" => GrowableGenetics.GeneType.Hardiness,
                    "Y" or "y" => GrowableGenetics.GeneType.Yield,
                    "W" or "w" => GrowableGenetics.GeneType.WaterRequirement,
                    "X" or "x" => GrowableGenetics.GeneType.Empty,
                    _ => throw new Exception($"Invalid gene type '{type}' in gene configuration")
                };
            }
        }
        ...
        {
            [JsonProperty("Minimum amount for a chance to yield")]
            public int ChanceAmount { get; set; }
            [JsonProperty("Minimum amount for guaranteed yield")]
            public int GuaranteedAmount { get; set; }

            public GeneConfig(int? chanceAmount, int? guaranteedAmount)
            {
                ChanceAmount = chanceAmount ?? 0;
                GuaranteedAmount = guaranteedAmount ?? 0;
            }
        }
        ...
        private void OnHempGather(BasePlayer player, Vector3 plantPosition, bool isCollectable, Func<GrowableGenetics.GeneType, int> getGeneCount = null)
        {
            ushort locationMask = (ushort)TerrainMeta.BiomeMap.GetBiomeMaxType(plantPosition);

            foreach(var cfg in Config.weedConfig)
            {
                if (cfg.MaybeGather(locationMask, isCollectable, getGeneCount, out var weed))
                {
        ...
        #if COCA
        private void OnBerryGather(BasePlayer player, Vector3 plantPosition, bool isCollectable, Func<GrowableGenetics.GeneType, int> getGeneCount = null)
        {
            ushort locationMask = (ushort)TerrainMeta.BiomeMap.GetBiomeMaxType(plantPosition);

            foreach (var cfg in Config.berryConfig)
            {
                if (cfg.MaybeGather(locationMask, isCollectable, getGeneCount, out var coca))
                {
                    if (player.inventory.containerMain.IsFull() &&
                        player.inventory.containerBelt.IsFull())
                    {
        ...
        [Hook] void OnGrowableGather(GrowableEntity plant, BasePlayer player)
        {
            ...
            {
                OnHempGather(player, plant.transform.position, false, plant.Genes.GetGeneTypeCount);
            }

#if COCA
            if (plant.prefabID == 4038822397 && plant.State == PlantProperties.State.Ripe && AllowedToGather(player))
            {
                OnBerryGather(player, plant.transform.position, false, plant.Genes.GetGeneTypeCount);
            }
#endif
        }
        ...
        // Support AutoFarm (legacy)
        // TODO remove in future version
        [Hook] void OnAutoFarmGather(string userID, StorageContainer container, Vector3 plantPosition, bool isCollectable, int hGenes = 0, int gGenes = 0)
        {
            LogError("You are using an outdated version of AutoFarm, which is not compatible with Ganja. Please update AutoFarm to the latest version");
        }

        // Support AutoFarm
        [Hook] void OnAutoFarmGather(string userID, StorageContainer container, Vector3 plantPosition, bool isCollectable, GrowableEntity plant)
        {
            if (!AllowedToGather(userID) || plant == null)
            {
                return;
            }
            ...
            foreach (var cfg in Config.weedConfig)
            {
                if (cfg.MaybeGather(locationMask, isCollectable, plant.Genes.GetGeneTypeCount, out var weed))
                {
