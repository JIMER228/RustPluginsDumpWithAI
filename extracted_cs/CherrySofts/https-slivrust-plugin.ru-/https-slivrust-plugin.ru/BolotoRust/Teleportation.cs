...
{
    [Info("Teleportation", "https://rust-plug.ru/", "1.5.2")]
    class Teleportation : RustPlugin
    {
        ...
        void cmdChatRemoveHome(BasePlayer player, string command, string[] args)
        {
            ...
            if (!homes.ContainsKey(player.userID))
            {
                ...
                player.SendConsoleCommand("gametip.showtoast", "Ошибка", "Нет сохранённых точек!");
            ...
            if (!playerHomes.ContainsKey(name))
            {
                ...
                player.SendConsoleCommand("gametip.showtoast", "Ошибка", "Местоположение не найдено!");
        ...
        private void cmdHomeList(BasePlayer player, string command, string[] args)
        {
            if (!homes.ContainsKey(player.userID) || homes[player.userID].Count == 0)
            {
                ...
                player.SendConsoleCommand("gametip.showtoast", "Ошибка", "У вас нет сохранённых точек дома!");
        ...
        void cmdChatHome(BasePlayer player, string command, string[] args)
        {
            ...
            if (!playerHomes.ContainsKey(name))
            {
                ...
                player.SendConsoleCommand("gametip.showtoast", "Ошибка", "Такое местоположение не найдено!");
        ...
        void cmdChatTpr(BasePlayer player, string command, string[] args)
        {
            ...
            if (target == null || !target.IsConnected)
            {
                ...
                player.SendConsoleCommand("gametip.showtoast", "Ошибка", "Игрок не найден!");
                return;
            }
            ...
            player.SendConsoleCommand("gametip.showtoast", "Запрос на телепорт", $"Вы отправили запрос {target.displayName}");

            SendReply(target, string.Format(Messages["tprpending"], player.displayName));
            target.SendConsoleCommand("gametip.showtoast", "Запрос на телепорт", $"Игрок {player.displayName} хочет телепортироваться к вам");
            ...
            switch (args[0])
            {
                ...
                case "list":
                    if (tpsave.Count <= 0)
                    {
                        ...
                        player.SendConsoleCommand("gametip.showtoast", "Ошибка", "У вас нет сохранённых точек!");
...
void TeleportationTimerHandle()
        ...
        void SetHome(BasePlayer player, string name, Vector3 pos)
        {
            ...
            if (!homes.TryGetValue(player.userID, out playerHomes)) playerHomes = homes[player.userID] = new Dictionary<string, Vector3>();

            if (GetHomeLimit(player.userID) == playerHomes.Count)
            {
                Effect.server.Run(EffectPrefab, player, 0, Vector3.zero, Vector3.forward);
                player.SendConsoleCommand("gametip.showtoast", "Ошибка", "Превышено количество точек дома</color>");
            ...
            if (foundationOwner && entity.OwnerID != player.userID)
            {
                ...
                else if (!IsFriends(entity.OwnerID, player.userID) && !IsClanMember(entity.OwnerID, player.userID) && !IsTeamate(player, entity.OwnerID))
                {
                    Effect.server.Run(EffectPrefab, player, 0, Vector3.zero, Vector3.forward);
