...
{
    [Info("BCraftSystem", "FIXED SLITOVSKIY", "1.0.0")]
    public class BCraftSystem : RustPlugin
    {
        ...
        private void CraftUi(BasePlayer player, int ID)
        {
            ...
            var allItems = player.inventory.containerMain.itemList.Concat(player.inventory.containerBelt.itemList).Concat(player.inventory.containerWear.itemList).ToArray();
            var container = new CuiElementContainer();
