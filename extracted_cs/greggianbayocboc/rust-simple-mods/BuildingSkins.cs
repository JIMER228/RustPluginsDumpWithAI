...
{
    [Info("Building Skins", "Marat", "2.0.8")]
    [Description("Automatic application of DLC skins for building blocks, Unofficial fix for Oct 2024 Force wipe by sneakygregor")]
    class BuildingSkins : RustPlugin
    {
        ...
        private void OnHammerHit(BasePlayer player, HitInfo info)
        {
            ...
            if (block.skinID != 0 && block.grade.ToString() == "Metal" && !playerData.RandomColor) block.SetCustomColour(playerData.Color);
        ...
        private object OnStructureUpgrade(BuildingBlock block, BasePlayer player, BuildingGrade.Enum grade, ulong skin)
        {
            ...
            if (block.skinID != 0 && grade.ToString() == "Metal" && !playerData.RandomColor) block.SetCustomColour(playerData.Color);
        ...
        private void ImageLayers(BasePlayer player, int index, int skinIndex)
        {
            ...
            if (grades.Key.ToString() == "Metal" && !isBlocked)
            {
                if (skinIndex != 0)
                {
                    var colorIcon = colors.TryGetValue(playerData.Color, out var value) ? value : "0.20 0.30 0.40 1.0";
        ...
        {
            var playerData = storedData.PlayerData[player.userID.Get()];
            var playerColor = playerData.Color;
            CuiElementContainer container = new CuiElementContainer();
