...
namespace Oxide.Plugins
{
    ...
    class RemoverTool : RustLegacyPlugin
    {
		...
		public bool entityisvalid(float test)
		{
			if(test == 0f)
				return true;
			return false;
		}
		void entitycheck(float test)
		{
			Debug.Log("entity is valid");
		}
        void TryToRemove(TakeDamage takedamage, RemoveHandler rplayer)
        {
            ...
            {
			    var cacheddeployable2 = cachedDeployable.GetComponent<TakeDamage>();
			    var times = (cacheddeployable2.health * 0);
			    if(!entityisvalid(times))
				{
					return;
				}
        ...
        void DeployableRemove(DeployableObject deployable, RemoveHandler rplayer)
        {
            ...
            string Name = deployable.gameObject.name;
			TakeDamage.KillSelf(deployable.GetComponent<IDMain>());
			Debug.Log(deployable);
			if (canRefund) TryRefund(deployableCloneToGood[Name], rplayer);
