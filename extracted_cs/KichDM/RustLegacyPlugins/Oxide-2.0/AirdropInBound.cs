...
namespace Oxide.Plugins
{
    ...
    class AirdropInBound : RustLegacyPlugin
    {
        ...
        {
            string message = "Airdrop En Camino A";
            ...
            foreach(PlayerClient player in PlayerClient.All)
            {
                ...
                if (showDistance)
                {
                    distance = "Aproximadamente A. " + Math.Ceiling(Math.Abs(Vector3.Distance(targetposition, player.lastKnownPosition))).ToString() + "m";
                }
                if (showDirection)
                {
                ...
                {
                    currentmessage = string.Format("{0} ({1} {2} De Tu Posicion)", currentmessage, distance, direction);
                }
