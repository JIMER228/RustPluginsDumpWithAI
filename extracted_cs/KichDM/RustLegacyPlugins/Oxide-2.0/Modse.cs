    ...
    class Modse : RustLegacyPlugin
    {
		...
		[ChatCommand("mod")]		
			void Modeject(NetUser netuser, string command, string[] args)
			{
				...
				if (inv.activeItem==null){rust.SendChatMessage(netuser, chatName, string.Format("[COLOR#EE00EE]Você deve pegar uma arma da qual você precisa remover mods")); return;}
				if (inv.GetItem(inv.activeItem.slot, out item))
                    {
						...
						if (qty == 0)								
						{
							rust.SendChatMessage(netuser, chatName, string.Format("[COLOR#EE00EE]Você precisa carregar a munição na arma!"));						
							return;
						}
						...
						rust.SendChatMessage(netuser, chatName, string.Format("[COLOR#EE00EE]Você removeu todos os mods de [COLOR#00FF00] \"{0}\"", name));
                    }
			}
