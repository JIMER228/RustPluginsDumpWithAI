...
{
    [Info("Obb", "RND and KoT", "1.3.6")]
    class Obb : RustLegacyPlugin 
	...
	[ChatCommand("obb")]
			void Sellder(NetUser netuser, string command, string[] args)
		{
			...
			string nameInput = "Bolt Action Rifle"; //что выдать
			string nameInput2 = "556 Ammo"; //что выдать
			string nameInput3 = "Wood"; //что выдать
			int QuantityOut = 80;////////сколько забрать
			...
			if (QuantityOut<=SoldAmount)
			{
				...
				rust.Notice(netuser, "Вы получили Bolt Action Rifle и 556 Ammo" );
