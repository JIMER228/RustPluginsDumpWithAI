    ...
    class RemoverTool : RustLegacyPlugin
    {
        ...
        public class RemoveHandler : MonoBehaviour
        {
            ...
            public void Activate()
            {
                ...
                ConsoleNetworker.SendClientCommand(playerclient.netUser.networkPlayer, "chat.add Oz-Remove " + Facepunch.Utility.String.QuoteSafe(string.Format(removeActivated,removeType,(deactivateTime - Time.realtimeSinceStartup).ToString())));
            }
            void OnDestroy()
            {
                if(playerclient != null && playerclient.netUser != null)
                    ConsoleNetworker.SendClientCommand(playerclient.netUser.networkPlayer, "chat.add Oz-Remove " + Facepunch.Utility.String.QuoteSafe(removeDeactivated));
            }
            void FixedUpdate()
            {
        ...
        void OnHurt(TakeDamage takedamage, DamageEvent damage, RemoveHandler rplayer)
        {
        ...
        void DeployableRemove(DeployableObject deployable, RemoveHandler rplayer)
        {
            ...
            bool canRemoveLootable = true;
            if (deployable.GetComponent<LootableObject>()) 
            { 
                var box = deployable.GetComponent<LootableObject>();
                for (int i = 0; i < box._inventory.slotCount; i++)
                {
                    if (box._inventory.GetItem(i, out item))
                    {
                        //rust.Notice(rplayer.playerclient.netUser, "Teste");
                        SendReply(rplayer.playerclient.netUser, "[color #0077FF]En Este Servidor No Podes [color red]Quemar Loot [color #0077FF]Saca Las Cosas De Las [color red]Caja/Horno [color #0077FF]Para [color red]Remover");
                        canRemoveLootable = false;
                    }
                }
            }
            if(!canRemoveLootable) return;
