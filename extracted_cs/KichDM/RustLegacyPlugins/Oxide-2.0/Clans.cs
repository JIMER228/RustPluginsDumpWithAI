...
namespace Oxide.Plugins
{
    ...
    class Clans : CovalencePlugin
    {
        ...
        {
            var formatMsg = $"{configData.MessageOptions.MSG} [color blue]{message} ";
            if (!string.IsNullOrEmpty(message2))
                formatMsg = $"{configData.MessageOptions.Main} [color blue]{message2} {formatMsg}";
            player.Reply(formatMsg);
        }
        void ReplyKey(IPlayer player, string msg, string arg)
        {
            var message = $"{configData.MessageOptions.MSG} [color blue]{msg}".Replace("{0}", $"{configData.MessageOptions.Main} {arg} [color #0080FF]{configData.MessageOptions.MSG}");
            if (message.StartsWith("[color blue]")) message = message.Substring(9).Trim();            
        ...
        public void ShowMembers(IPlayer player)
        {
            ...
            {
                var clan = clanCache[playerClans[player.Id]];
                var returnString = $"[color blue]{clans.configData.MessageOptions.ClanChat} {msg("clanmembers", player.Id)}:\n";
                int i = 1;
                foreach (var member in clan.members)
                {
                    returnString += $"{(clan.IsOwner(member.Key) ? $"{msg("owner", player.Id)} - " : clan.IsModerator(member.Key) ? $"{msg("moderator", player.Id)} - " : $"{msg("member", player.Id)} - ")}{member.Value} - {((covalence.Players.FindPlayerById(member.Key)?.IsConnected ?? false) ? $"[color blue]{msg("online", player.Id)}" : $"[color blue]{msg("offline", player.Id)}")}{(i < clan.members.Count ? "\n" : "")}";                    
                    i++;
                }
        ...
        public class Clan
        {
            ...
            public void Broadcast(string message, string sender = "Clan")
            {
                foreach (var member in members)
                {
                    ...
                    if (target != null && target.IsConnected)
                        target.Reply($"[color blue]{clans.configData.MessageOptions.ClanChat}[{sender}] :{clans.configData.MessageOptions.MSG} {message}");
                }
            }
