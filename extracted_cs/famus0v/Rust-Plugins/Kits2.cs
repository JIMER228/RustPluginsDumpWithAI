...
{
    [Info("Kits", "RustPlugin.ru", "1.2.5")]
    class Kitsf : RustPlugin
    {
        ...
        class PluginConfig
        {
            ...
            public static PluginConfig CreateDefault()
            {
                return new PluginConfig
                {
                    ...
                    DefaultKitImage = "file://" + Interface.Oxide.DataDirectory + Path.DirectorySeparatorChar + "Kits" +
                                      Path.DirectorySeparatorChar + "rust.png",
                    KitWidth = 0.12f,
                    MarginTop = 0.04f,
                    MarginBottom = 0.03f,
        ...
        public class ImagesCache : MonoBehaviour
        {
            ...
            public void Add(string name, string url)
            {
                ...
                using (var www = new WWW(url))
                {
                    ...
                    {
                        if (www.bytes.Count() == 0)
                        {
                            Interface.Oxide.LogError(
                                $"Failed to add image for {name}. File address possibly invalide\n {url}");
                            return;
                        }
                        ...
                        _images[name] = FileStorage.server
                            .Store(www.bytes, FileStorage.Type.png, CommunityEntity.ServerInstance.net.ID).ToString();
                    }
                }
            }
        ...
        protected override void LoadDefaultConfig()
        {
            ...
            PrintWarning(
                "Благодарим за приобритение плагина на сайте RustPlugin.ru.\n Если вы приобрели плагин в дргом месте - вы теряете все гарантии.");
        }
        ...
        private void LoadMessages()
        {
            ...
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                ["Permission Denied"] =
                    "<color=#008B8B>[Сервер]:</color> У вас нет полномочий использовать этот комплект",
                ["Limite Denied"] =
                    "<color=#008B8B>[Сервер]:</color> Вы уже использовали этот комплект максимальное количество раз",
                ["Cooldown Denied"] =
                    "<color=#008B8B>[Сервер]:</color> Вы сможете использовать этот комплект через {time}",
                ["Reset"] =
                    "<color=#008B8B>[Сервер]:</color> Вы обнулили все данные о использовании комплектов игроков",
        ...
        private void Loaded()
        {
            ...
            _kitsData =
                Interface.Oxide.DataFileSystem.ReadObject<Dictionary<ulong, Dictionary<string, KitData>>>("Kits_Data");
        ...
        private void CommandConsoleKit(ConsoleSystem.Arg arg)
        {
            ...
            if (kit.Amount > 0)
            {
                ...
                InitilizeAmountLabelUI(ref container, value,
                    GetMsg("UI Amount", player).Replace("{amount}", (kit.Amount - playerData.Amount).ToString()));
            }
            ...
            if (kit.Cooldown > 0)
            {
                ...
                if (playerData.Cooldown > currentTime)
                {
                    ...
                    InitilizeCooldownLabelUI(ref container, value,
        ...
        private void CommandChatKit(BasePlayer player, string command, string[] args)
        {
            ...
            if (args.Length == 0)
            {
                ...
                // SendReply(player, $"");
                // SendReply(player, $"Бинд кнопки на КИТ bind keypad5 kit ui");
                return;
            }
        ...
        private bool GiveKit(BasePlayer player, string kitname)
        {
            ...
            if (kit.Cooldown > 0)
            {
                ...
                {
                    SendReply(player,
                        GetMsg("Cooldown Denied", player).Replace("{time}",
                            TimeExtensions.FormatTime(TimeSpan.FromSeconds(playerData.Cooldown - currentTime))));
                    return false;
                }
            }
        ...
        private void KitCommandAdd(BasePlayer player, string kitname)
        {
            ...
            _kits.Add(new Kit
            {
                ...
                Cooldown = 600,
                Hide = true,
                Permission = "kits." + kitname,
                Amount = 0,
                Png = _config.DefaultKitImage,
                Items = GetPlayerItems(player)
            });
            permission.RegisterPermission($"kits.{kitname}", this);
            ...
            List<ItemContent> Content)
        {
            ...
            if (weapon != null)
            {
                ...
                (item.GetHeldEntity() as BaseProjectile).primaryMagazine.ammoType =
        ...
        private void InitilizeUI(BasePlayer player)
        {
            ...
            container.Add(new CuiButton
            {
                ...
                RectTransform =
                {
                    AnchorMin = _config.CloseButtonPosition.AnchorMin, AnchorMax = _config.CloseButtonPosition.AnchorMax
                },
                Text =
                {
                    Text = "X", FontSize = 20, Font = "robotocondensed-regular.ttf", Align = TextAnchor.MiddleCenter
                }
            }, "ui.kits");
            ...
            foreach (var playerGUIData in _kitsGUI)
            {
                ...
                foreach (var kitname in playerGUIData.Value)
                {
                    ...
                    if (playerKitData.Cooldown > 0)
                    {
                        ...
                        {
                            InitilizeCooldownLabelUI(ref container, kitname,
                                TimeSpan.FromSeconds(playerKitData.Cooldown - currentTime));
                        }
                    }
                }
        ...
        private void InitilizeKitsUI(ref CuiElementContainer container, BasePlayer player)
        {
            ...
            var currentTime = GetCurrentTime();
            var kits = GetKitsForPlayer(player).Take((int) (1.0f / (_config.KitWidth + _config.MarginBetween)))
                .ToList();
            ...
            foreach (var kit in kits)
            {
                ...
                if (kit.Amount > 0)
                {
                    InitilizeAmountLabelUI(ref container, kit.Name,
                        GetMsg("UI Amount", player).Replace("{amount}", (kit.Amount - playerData.Amount).ToString()));
                }
                ...
                {
                    InitilizeMaskUI(ref container, kit.Name);
                    InitilizeCooldownLabelUI(ref container, kit.Name,
                        TimeSpan.FromSeconds(playerData.Cooldown - currentTime));
                }
                else
                {
            ...
            container.Add(new CuiElement
            {
                ...
                Components =
                {
                    ...
                    new CuiRectTransformComponent
                        {AnchorMin = _config.Image.Position.AnchorMin, AnchorMax = _config.Image.Position.AnchorMax}
                }
            });
        }
        ...
        private void InitilizeNameLabelUI(ref CuiElementContainer container, string kitname, string text)
        {
            var name = container.Add(new CuiPanel
            {
                ...
                RectTransform =
                    {AnchorMin = _config.Label.Position.AnchorMin, AnchorMax = _config.Label.Position.AnchorMax}
            }, $"ui.kits.{kitname}");
            ...
            container.Add(new CuiLabel
            {
                Text =
                {
                    Color = HexToRustFormat(_config.Label.ForegroundColor), FontSize = _config.Label.FontSize,
                    Align = _config.Label.TextAnchor, Text = text
                }
            }, name);
        }
        ...
        private void InitilizeAmountLabelUI(ref CuiElementContainer container, string kitname, string text)
        {
            container.Add(new CuiPanel
            {
                ...
                RectTransform =
                    {AnchorMin = _config.Amount.Position.AnchorMin, AnchorMax = _config.Amount.Position.AnchorMax}
            ...
            container.Add(new CuiLabel
            {
                Text =
                {
                    Color = HexToRustFormat(_config.Amount.ForegroundColor), FontSize = _config.Amount.FontSize,
                    Align = _config.Amount.TextAnchor, Text = text
                }
        ...
        private void InitilizeCooldownLabelUI(ref CuiElementContainer container, string kitname, TimeSpan time)
        {
            container.Add(new CuiPanel
            {
                ...
                RectTransform =
                    {AnchorMin = _config.Time.Position.AnchorMin, AnchorMax = _config.Time.Position.AnchorMax}
            ...
            container.Add(new CuiLabel
            {
                Text =
                {
                    Color = HexToRustFormat(_config.Time.ForegroundColor), FontSize = _config.Time.FontSize,
                    Align = _config.Time.TextAnchor, Text = TimeExtensions.FormatShortTime(time)
                }
        ...
        string GetMsg(string key, BasePlayer player = null) =>
            lang.GetMessage(key, this, player == null ? null : player.UserIDString);
        ...
        private List<Kit> GetKitsForPlayer(BasePlayer player)
        {
            return _kits.Where(kit =>
                ...
                (string.IsNullOrEmpty(kit.Permission) ||
                 permission.UserHasPermission(player.UserIDString, kit.Permission)) && (kit.Amount == 0 ||
                    (kit.Amount > 0 && GetPlayerData(player.userID, kit.Name).Amount < kit.Amount))).ToList();
        }
        ...
        private BasePlayer FindPlayer(BasePlayer player, string nameOrID)
        {
            ...
            var foundPlayers =
                BasePlayer.activePlayerList.Where(x => x.displayName.ToLower().Contains(nameOrID.ToLower()));
