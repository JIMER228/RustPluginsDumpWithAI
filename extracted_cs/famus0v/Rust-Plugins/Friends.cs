    ...
    public class Friends : RustPlugin
    {
        ...
        private void StartUi(BasePlayer player)
        {
            ...
            cont.Add(new CuiElement()
            {
                ...
                Components =
                {
                    ...
                    new CuiRectTransformComponent()
                        { AnchorMin = "0.3442708 0.6361111", AnchorMax = "0.41875 0.6549382" }
                }
            });
        ...
        private void SettingInit(BasePlayer player, ulong steamdIdTarget, string b)
        {
            ...
            cont.Add(new CuiElement()
            {
                ...
                Components =
                {
                    new CuiTextComponent()
                    {
                        Text = $"{lang.GetMessage("UISETTINGS", this, player.UserIDString)} {target.Name}",
                        Align = TextAnchor.MiddleCenter, Font = regular, FontSize = 12,
            ...
            cont.Add(new CuiButton()
            {
                ...
                Text =
                {
                    Text = lang.GetMessage("UIREMOVEFRIEND", this, player.UserIDString),
                    Align = TextAnchor.MiddleCenter, FontSize = 10, Color = HexToRustFormat("#8c472e"),
                    Font = "robotocondensed-regular.ttf"
                }
            }, Layer + "Set");
            if (access.Damage)
        ...
        private void FriendsInit(BasePlayer player, int page, string find = "")
        {
            ...
            foreach (var sellItem in playerList
                         .Where(p => find == "" || find != "" &&
                             (p.Key.DisplayName.ToLower().Contains(find.ToLower()) ||
                              p.Key.SteamId.ToString().Contains(find)))
                         .Select((i, t) => new { A = i, B = t - (page - 1) * 98 }).Skip((page - 1) * 98).Take(98))
            {
                cont.Add(new CuiElement()
                {
                    Parent = Layer + "-Search",
