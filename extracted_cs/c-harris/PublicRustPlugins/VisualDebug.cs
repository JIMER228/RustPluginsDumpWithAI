...
{
    [Info("VisualDebug", "Jake_Rich", "1.0.0")]
    [Description("Figure out where memory leaks probally are")]

    public class VisualDebug : RustPlugin
    {
        public static Oxide.Core.Plugins.Plugin targetPlugin { get; set; }
        ...
        void TestReflectionSizeOfConsole(ConsoleSystem.Arg arg)
        {
            ...
            var obj = new ObjectMemoryInfo(targetPlugin, layers);

            Oxide.Core.Interface.Oxide.DataFileSystem.WriteObject($"MemDump-{targetPlugin.Name}", obj);
            Puts("Dumped Memory!");
        }
        */
        ...
        #region Jake's UI Framework - Old
