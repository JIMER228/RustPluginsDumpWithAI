...
namespace Solution
{
    class JakeFramework : RustPlugin
    {
        public static JakeFramework _plugin { get; set; }
        ...
        public class UIReflectionElement : UIPanel
        {
            ...
            public UIReflectionElement(Vector2 min, Vector2 max, FieldInfo field, object configuration, UIBaseElement parent = null) : base(min, max, parent)
            {
                ...
                Text = new UILabel(new Vector2(0.05f, 0f), new Vector2(0.4f, 1f), "Test", parent: this, alignment: TextAnchor.MiddleLeft);
        ...
        public class InstanceConfigFile<Type> where Type : class
        {
            ...
            public InstanceConfigFile(string name, ConfigLocation location = ConfigLocation.Data, string path = null, string extension = ".json")
            {
            ...
            {
                if (!Interface.Oxide.DataFileSystem.ExistsDatafile(_name))
                {
                    Save();
                }
                Instance = Interface.Oxide.DataFileSystem.ReadObject<Type>(_name);
            }

            public virtual void Save()
            {
                Interface.Oxide.DataFileSystem.WriteObject(_name, Instance);
            }
        ...
        #region Object Orientated Lang File
