...
{
    [Info("RangeFinder", "Jake_Rich", "1.0.0")]
    [Description("Tool to measure distances")]

    public partial class RangeFinder : RustPlugin
    {
        public static string ItemShortname { get; set; } = "battery.small";
        public static ulong ItemSkinID { get; set; }

        public UIBaseElement MainPanel { get; set; }

        void OnPlayerInput(BasePlayer player, InputState input)
        {
            var activeItem = player.GetActiveItem();
            var data = GetPlayerData(player);
            if (activeItem != data.LastActiveItem)
            {
                if (IsRangeFinder(activeItem))
                {
                    MainPanel.Show(player);
                }
                else if (IsRangeFinder(data.LastActiveItem))
                {
            ...
            data.LastActiveItem = activeItem;
        }

        public void GetRange(BasePlayer player)
        {
            RaycastHit hit;
            if (!Physics.Raycast(player.eyes.BodyRay(), out hit, 1000f, (int)Layer.Reserved1))
            {
                PrintToChat(player, $"Distance could not be determined");
            }
            else
            {
                PrintToChat(player, $"Distance: {Mathf.RoundToInt(hit.distance)}m");
            }
        }

        public bool IsRangeFinder(Item item)
        {
            return item.info.shortname == ItemShortname && item.skin == ItemSkinID;
        }

        #region Hooks

        public static RangeFinder _plugin { get; set; }
        ...
        public class PlayerData
        {
            ...
            public Item LastActiveItem { get; set; } 
